
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 60 c6 10 80       	mov    $0x8010c660,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 e7 33 10 80       	mov    $0x801033e7,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 1c 86 10 	movl   $0x8010861c,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100049:	e8 48 4e 00 00       	call   80104e96 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 90 db 10 80 84 	movl   $0x8010db84,0x8010db90
80100055:	db 10 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 94 db 10 80 84 	movl   $0x8010db84,0x8010db94
8010005f:	db 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 94 c6 10 80 	movl   $0x8010c694,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 94 db 10 80       	mov    0x8010db94,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 94 db 10 80       	mov    %eax,0x8010db94

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate fresh block.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801000bd:	e8 f5 4d 00 00       	call   80104eb7 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 94 db 10 80       	mov    0x8010db94,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	89 c2                	mov    %eax,%edx
801000f5:	83 ca 01             	or     $0x1,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100104:	e8 10 4e 00 00       	call   80104f19 <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 60 c6 10 	movl   $0x8010c660,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 7c 49 00 00       	call   80104aa0 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 90 db 10 80       	mov    0x8010db90,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010017c:	e8 98 4d 00 00       	call   80104f19 <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 23 86 10 80 	movl   $0x80108623,(%esp)
8010019f:	e8 99 03 00 00       	call   8010053d <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 dc 25 00 00       	call   801027b4 <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 34 86 10 80 	movl   $0x80108634,(%esp)
801001f6:	e8 42 03 00 00       	call   8010053d <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	89 c2                	mov    %eax,%edx
80100202:	83 ca 04             	or     $0x4,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 9f 25 00 00       	call   801027b4 <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 3b 86 10 80 	movl   $0x8010863b,(%esp)
80100230:	e8 08 03 00 00       	call   8010053d <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010023c:	e8 76 4c 00 00       	call   80104eb7 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 94 db 10 80       	mov    0x8010db94,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 94 db 10 80       	mov    %eax,0x8010db94

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	89 c2                	mov    %eax,%edx
8010028f:	83 e2 fe             	and    $0xfffffffe,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 0c 4a 00 00       	call   80104cae <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801002a9:	e8 6b 4c 00 00       	call   80104f19 <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	53                   	push   %ebx
801002b4:	83 ec 14             	sub    $0x14,%esp
801002b7:	8b 45 08             	mov    0x8(%ebp),%eax
801002ba:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002be:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
801002c2:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
801002c6:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
801002ca:	ec                   	in     (%dx),%al
801002cb:	89 c3                	mov    %eax,%ebx
801002cd:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
801002d0:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
801002d4:	83 c4 14             	add    $0x14,%esp
801002d7:	5b                   	pop    %ebx
801002d8:	5d                   	pop    %ebp
801002d9:	c3                   	ret    

801002da <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002da:	55                   	push   %ebp
801002db:	89 e5                	mov    %esp,%ebp
801002dd:	83 ec 08             	sub    $0x8,%esp
801002e0:	8b 55 08             	mov    0x8(%ebp),%edx
801002e3:	8b 45 0c             	mov    0xc(%ebp),%eax
801002e6:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002ea:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002ed:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002f1:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002f5:	ee                   	out    %al,(%dx)
}
801002f6:	c9                   	leave  
801002f7:	c3                   	ret    

801002f8 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002f8:	55                   	push   %ebp
801002f9:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002fb:	fa                   	cli    
}
801002fc:	5d                   	pop    %ebp
801002fd:	c3                   	ret    

801002fe <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002fe:	55                   	push   %ebp
801002ff:	89 e5                	mov    %esp,%ebp
80100301:	83 ec 48             	sub    $0x48,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
80100304:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100308:	74 19                	je     80100323 <printint+0x25>
8010030a:	8b 45 08             	mov    0x8(%ebp),%eax
8010030d:	c1 e8 1f             	shr    $0x1f,%eax
80100310:	89 45 10             	mov    %eax,0x10(%ebp)
80100313:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100317:	74 0a                	je     80100323 <printint+0x25>
    x = -xx;
80100319:	8b 45 08             	mov    0x8(%ebp),%eax
8010031c:	f7 d8                	neg    %eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100321:	eb 06                	jmp    80100329 <printint+0x2b>
  else
    x = xx;
80100323:	8b 45 08             	mov    0x8(%ebp),%eax
80100326:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100329:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100330:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80100333:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100336:	ba 00 00 00 00       	mov    $0x0,%edx
8010033b:	f7 f1                	div    %ecx
8010033d:	89 d0                	mov    %edx,%eax
8010033f:	0f b6 90 04 90 10 80 	movzbl -0x7fef6ffc(%eax),%edx
80100346:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100349:	03 45 f4             	add    -0xc(%ebp),%eax
8010034c:	88 10                	mov    %dl,(%eax)
8010034e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  }while((x /= base) != 0);
80100352:	8b 55 0c             	mov    0xc(%ebp),%edx
80100355:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80100358:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010035b:	ba 00 00 00 00       	mov    $0x0,%edx
80100360:	f7 75 d4             	divl   -0x2c(%ebp)
80100363:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100366:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010036a:	75 c4                	jne    80100330 <printint+0x32>

  if(sign)
8010036c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100370:	74 23                	je     80100395 <printint+0x97>
    buf[i++] = '-';
80100372:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100375:	03 45 f4             	add    -0xc(%ebp),%eax
80100378:	c6 00 2d             	movb   $0x2d,(%eax)
8010037b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)

  while(--i >= 0)
8010037f:	eb 14                	jmp    80100395 <printint+0x97>
    consputc(buf[i]);
80100381:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100384:	03 45 f4             	add    -0xc(%ebp),%eax
80100387:	0f b6 00             	movzbl (%eax),%eax
8010038a:	0f be c0             	movsbl %al,%eax
8010038d:	89 04 24             	mov    %eax,(%esp)
80100390:	e8 bb 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
80100395:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100399:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010039d:	79 e2                	jns    80100381 <printint+0x83>
    consputc(buf[i]);
}
8010039f:	c9                   	leave  
801003a0:	c3                   	ret    

801003a1 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a1:	55                   	push   %ebp
801003a2:	89 e5                	mov    %esp,%ebp
801003a4:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a7:	a1 f4 b5 10 80       	mov    0x8010b5f4,%eax
801003ac:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003af:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b3:	74 0c                	je     801003c1 <cprintf+0x20>
    acquire(&cons.lock);
801003b5:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
801003bc:	e8 f6 4a 00 00       	call   80104eb7 <acquire>

  if (fmt == 0)
801003c1:	8b 45 08             	mov    0x8(%ebp),%eax
801003c4:	85 c0                	test   %eax,%eax
801003c6:	75 0c                	jne    801003d4 <cprintf+0x33>
    panic("null fmt");
801003c8:	c7 04 24 42 86 10 80 	movl   $0x80108642,(%esp)
801003cf:	e8 69 01 00 00       	call   8010053d <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d4:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003da:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e1:	e9 20 01 00 00       	jmp    80100506 <cprintf+0x165>
    if(c != '%'){
801003e6:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003ea:	74 10                	je     801003fc <cprintf+0x5b>
      consputc(c);
801003ec:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ef:	89 04 24             	mov    %eax,(%esp)
801003f2:	e8 59 03 00 00       	call   80100750 <consputc>
      continue;
801003f7:	e9 06 01 00 00       	jmp    80100502 <cprintf+0x161>
    }
    c = fmt[++i] & 0xff;
801003fc:	8b 55 08             	mov    0x8(%ebp),%edx
801003ff:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100403:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100406:	01 d0                	add    %edx,%eax
80100408:	0f b6 00             	movzbl (%eax),%eax
8010040b:	0f be c0             	movsbl %al,%eax
8010040e:	25 ff 00 00 00       	and    $0xff,%eax
80100413:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100416:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010041a:	0f 84 08 01 00 00    	je     80100528 <cprintf+0x187>
      break;
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4d                	je     80100475 <cprintf+0xd4>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0x9f>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13b>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xae>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x149>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 53                	je     80100498 <cprintf+0xf7>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2b                	je     80100475 <cprintf+0xd4>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x149>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8b 00                	mov    (%eax),%eax
80100454:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
80100458:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010045f:	00 
80100460:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100467:	00 
80100468:	89 04 24             	mov    %eax,(%esp)
8010046b:	e8 8e fe ff ff       	call   801002fe <printint>
      break;
80100470:	e9 8d 00 00 00       	jmp    80100502 <cprintf+0x161>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100475:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100478:	8b 00                	mov    (%eax),%eax
8010047a:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
8010047e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100485:	00 
80100486:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
8010048d:	00 
8010048e:	89 04 24             	mov    %eax,(%esp)
80100491:	e8 68 fe ff ff       	call   801002fe <printint>
      break;
80100496:	eb 6a                	jmp    80100502 <cprintf+0x161>
    case 's':
      if((s = (char*)*argp++) == 0)
80100498:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049b:	8b 00                	mov    (%eax),%eax
8010049d:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004a0:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004a4:	0f 94 c0             	sete   %al
801004a7:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
801004ab:	84 c0                	test   %al,%al
801004ad:	74 20                	je     801004cf <cprintf+0x12e>
        s = "(null)";
801004af:	c7 45 ec 4b 86 10 80 	movl   $0x8010864b,-0x14(%ebp)
      for(; *s; s++)
801004b6:	eb 17                	jmp    801004cf <cprintf+0x12e>
        consputc(*s);
801004b8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004bb:	0f b6 00             	movzbl (%eax),%eax
801004be:	0f be c0             	movsbl %al,%eax
801004c1:	89 04 24             	mov    %eax,(%esp)
801004c4:	e8 87 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004c9:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004cd:	eb 01                	jmp    801004d0 <cprintf+0x12f>
801004cf:	90                   	nop
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 de                	jne    801004b8 <cprintf+0x117>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x161>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x161>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 c0 fe ff ff    	jne    801003e6 <cprintf+0x45>
80100526:	eb 01                	jmp    80100529 <cprintf+0x188>
      consputc(c);
      continue;
    }
    c = fmt[++i] & 0xff;
    if(c == 0)
      break;
80100528:	90                   	nop
      consputc(c);
      break;
    }
  }

  if(locking)
80100529:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052d:	74 0c                	je     8010053b <cprintf+0x19a>
    release(&cons.lock);
8010052f:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100536:	e8 de 49 00 00       	call   80104f19 <release>
}
8010053b:	c9                   	leave  
8010053c:	c3                   	ret    

8010053d <panic>:

void
panic(char *s)
{
8010053d:	55                   	push   %ebp
8010053e:	89 e5                	mov    %esp,%ebp
80100540:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100543:	e8 b0 fd ff ff       	call   801002f8 <cli>
  cons.locking = 0;
80100548:	c7 05 f4 b5 10 80 00 	movl   $0x0,0x8010b5f4
8010054f:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
80100552:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100558:	0f b6 00             	movzbl (%eax),%eax
8010055b:	0f b6 c0             	movzbl %al,%eax
8010055e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100562:	c7 04 24 52 86 10 80 	movl   $0x80108652,(%esp)
80100569:	e8 33 fe ff ff       	call   801003a1 <cprintf>
  cprintf(s);
8010056e:	8b 45 08             	mov    0x8(%ebp),%eax
80100571:	89 04 24             	mov    %eax,(%esp)
80100574:	e8 28 fe ff ff       	call   801003a1 <cprintf>
  cprintf("\n");
80100579:	c7 04 24 61 86 10 80 	movl   $0x80108661,(%esp)
80100580:	e8 1c fe ff ff       	call   801003a1 <cprintf>
  getcallerpcs(&s, pcs);
80100585:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100588:	89 44 24 04          	mov    %eax,0x4(%esp)
8010058c:	8d 45 08             	lea    0x8(%ebp),%eax
8010058f:	89 04 24             	mov    %eax,(%esp)
80100592:	e8 d1 49 00 00       	call   80104f68 <getcallerpcs>
  for(i=0; i<10; i++)
80100597:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059e:	eb 1b                	jmp    801005bb <panic+0x7e>
    cprintf(" %p", pcs[i]);
801005a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a3:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a7:	89 44 24 04          	mov    %eax,0x4(%esp)
801005ab:	c7 04 24 63 86 10 80 	movl   $0x80108663,(%esp)
801005b2:	e8 ea fd ff ff       	call   801003a1 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b7:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005bb:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bf:	7e df                	jle    801005a0 <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005c1:	c7 05 a0 b5 10 80 01 	movl   $0x1,0x8010b5a0
801005c8:	00 00 00 
  for(;;)
    ;
801005cb:	eb fe                	jmp    801005cb <panic+0x8e>

801005cd <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005cd:	55                   	push   %ebp
801005ce:	89 e5                	mov    %esp,%ebp
801005d0:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d3:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005da:	00 
801005db:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005e2:	e8 f3 fc ff ff       	call   801002da <outb>
  pos = inb(CRTPORT+1) << 8;
801005e7:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005ee:	e8 bd fc ff ff       	call   801002b0 <inb>
801005f3:	0f b6 c0             	movzbl %al,%eax
801005f6:	c1 e0 08             	shl    $0x8,%eax
801005f9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005fc:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100603:	00 
80100604:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010060b:	e8 ca fc ff ff       	call   801002da <outb>
  pos |= inb(CRTPORT+1);
80100610:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100617:	e8 94 fc ff ff       	call   801002b0 <inb>
8010061c:	0f b6 c0             	movzbl %al,%eax
8010061f:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
80100622:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100626:	75 30                	jne    80100658 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100628:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010062b:	ba 67 66 66 66       	mov    $0x66666667,%edx
80100630:	89 c8                	mov    %ecx,%eax
80100632:	f7 ea                	imul   %edx
80100634:	c1 fa 05             	sar    $0x5,%edx
80100637:	89 c8                	mov    %ecx,%eax
80100639:	c1 f8 1f             	sar    $0x1f,%eax
8010063c:	29 c2                	sub    %eax,%edx
8010063e:	89 d0                	mov    %edx,%eax
80100640:	c1 e0 02             	shl    $0x2,%eax
80100643:	01 d0                	add    %edx,%eax
80100645:	c1 e0 04             	shl    $0x4,%eax
80100648:	89 ca                	mov    %ecx,%edx
8010064a:	29 c2                	sub    %eax,%edx
8010064c:	b8 50 00 00 00       	mov    $0x50,%eax
80100651:	29 d0                	sub    %edx,%eax
80100653:	01 45 f4             	add    %eax,-0xc(%ebp)
80100656:	eb 32                	jmp    8010068a <cgaputc+0xbd>
  else if(c == BACKSPACE){
80100658:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065f:	75 0c                	jne    8010066d <cgaputc+0xa0>
    if(pos > 0) --pos;
80100661:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100665:	7e 23                	jle    8010068a <cgaputc+0xbd>
80100667:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
8010066b:	eb 1d                	jmp    8010068a <cgaputc+0xbd>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100675:	01 d2                	add    %edx,%edx
80100677:	01 c2                	add    %eax,%edx
80100679:	8b 45 08             	mov    0x8(%ebp),%eax
8010067c:	66 25 ff 00          	and    $0xff,%ax
80100680:	80 cc 07             	or     $0x7,%ah
80100683:	66 89 02             	mov    %ax,(%edx)
80100686:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x119>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 90 10 80       	mov    0x80109000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 90 10 80       	mov    0x80109000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 22 4b 00 00       	call   801051d9 <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	01 c0                	add    %eax,%eax
801006c5:	8b 15 00 90 10 80    	mov    0x80109000,%edx
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 ca                	add    %ecx,%edx
801006d2:	89 44 24 08          	mov    %eax,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 14 24             	mov    %edx,(%esp)
801006e1:	e8 20 4a 00 00       	call   80105106 <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 e0 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 c7 fb ff ff       	call   801002da <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 b3 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 9d fb ff ff       	call   801002da <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 a0 b5 10 80       	mov    0x8010b5a0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 94 fb ff ff       	call   801002f8 <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 06 65 00 00       	call   80106c81 <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 fa 64 00 00       	call   80106c81 <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 ee 64 00 00       	call   80106c81 <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 e1 64 00 00       	call   80106c81 <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 22 fe ff ff       	call   801005cd <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
801007ba:	e8 f8 46 00 00       	call   80104eb7 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 41 01 00 00       	jmp    80100905 <consoleintr+0x158>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 68                	je     8010083e <consoleintr+0x91>
801007d6:	e9 94 00 00 00       	jmp    8010086f <consoleintr+0xc2>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 59                	je     8010083e <consoleintr+0x91>
801007e5:	e9 85 00 00 00       	jmp    8010086f <consoleintr+0xc2>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 65 45 00 00       	call   80104d54 <procdump>
      break;
801007ef:	e9 11 01 00 00       	jmp    80100905 <consoleintr+0x158>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100816:	a1 58 de 10 80       	mov    0x8010de58,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	0f 84 db 00 00 00    	je     801008fe <consoleintr+0x151>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100823:	a1 5c de 10 80       	mov    0x8010de5c,%eax
80100828:	83 e8 01             	sub    $0x1,%eax
8010082b:	83 e0 7f             	and    $0x7f,%eax
8010082e:	0f b6 80 d4 dd 10 80 	movzbl -0x7fef222c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100835:	3c 0a                	cmp    $0xa,%al
80100837:	75 bb                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100839:	e9 c0 00 00 00       	jmp    801008fe <consoleintr+0x151>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083e:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100844:	a1 58 de 10 80       	mov    0x8010de58,%eax
80100849:	39 c2                	cmp    %eax,%edx
8010084b:	0f 84 b0 00 00 00    	je     80100901 <consoleintr+0x154>
        input.e--;
80100851:	a1 5c de 10 80       	mov    0x8010de5c,%eax
80100856:	83 e8 01             	sub    $0x1,%eax
80100859:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
8010085e:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100865:	e8 e6 fe ff ff       	call   80100750 <consputc>
      }
      break;
8010086a:	e9 92 00 00 00       	jmp    80100901 <consoleintr+0x154>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100873:	0f 84 8b 00 00 00    	je     80100904 <consoleintr+0x157>
80100879:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
8010087f:	a1 54 de 10 80       	mov    0x8010de54,%eax
80100884:	89 d1                	mov    %edx,%ecx
80100886:	29 c1                	sub    %eax,%ecx
80100888:	89 c8                	mov    %ecx,%eax
8010088a:	83 f8 7f             	cmp    $0x7f,%eax
8010088d:	77 75                	ja     80100904 <consoleintr+0x157>
        c = (c == '\r') ? '\n' : c;
8010088f:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
80100893:	74 05                	je     8010089a <consoleintr+0xed>
80100895:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100898:	eb 05                	jmp    8010089f <consoleintr+0xf2>
8010089a:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089f:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
801008a2:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008a7:	89 c1                	mov    %eax,%ecx
801008a9:	83 e1 7f             	and    $0x7f,%ecx
801008ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801008af:	88 91 d4 dd 10 80    	mov    %dl,-0x7fef222c(%ecx)
801008b5:	83 c0 01             	add    $0x1,%eax
801008b8:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(c);
801008bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008c0:	89 04 24             	mov    %eax,(%esp)
801008c3:	e8 88 fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c8:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008cc:	74 18                	je     801008e6 <consoleintr+0x139>
801008ce:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008d2:	74 12                	je     801008e6 <consoleintr+0x139>
801008d4:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008d9:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
801008df:	83 ea 80             	sub    $0xffffff80,%edx
801008e2:	39 d0                	cmp    %edx,%eax
801008e4:	75 1e                	jne    80100904 <consoleintr+0x157>
          input.w = input.e;
801008e6:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008eb:	a3 58 de 10 80       	mov    %eax,0x8010de58
          wakeup(&input.r);
801008f0:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
801008f7:	e8 b2 43 00 00       	call   80104cae <wakeup>
        }
      }
      break;
801008fc:	eb 06                	jmp    80100904 <consoleintr+0x157>
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
801008fe:	90                   	nop
801008ff:	eb 04                	jmp    80100905 <consoleintr+0x158>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100901:	90                   	nop
80100902:	eb 01                	jmp    80100905 <consoleintr+0x158>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
          input.w = input.e;
          wakeup(&input.r);
        }
      }
      break;
80100904:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
80100905:	8b 45 08             	mov    0x8(%ebp),%eax
80100908:	ff d0                	call   *%eax
8010090a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010090d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100911:	0f 89 ad fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
80100917:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
8010091e:	e8 f6 45 00 00       	call   80104f19 <release>
}
80100923:	c9                   	leave  
80100924:	c3                   	ret    

80100925 <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
80100925:	55                   	push   %ebp
80100926:	89 e5                	mov    %esp,%ebp
80100928:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
8010092b:	8b 45 08             	mov    0x8(%ebp),%eax
8010092e:	89 04 24             	mov    %eax,(%esp)
80100931:	e8 80 10 00 00       	call   801019b6 <iunlock>
  target = n;
80100936:	8b 45 10             	mov    0x10(%ebp),%eax
80100939:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
8010093c:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100943:	e8 6f 45 00 00       	call   80104eb7 <acquire>
  while(n > 0){
80100948:	e9 a8 00 00 00       	jmp    801009f5 <consoleread+0xd0>
    while(input.r == input.w){
      if(proc->killed){
8010094d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100953:	8b 40 24             	mov    0x24(%eax),%eax
80100956:	85 c0                	test   %eax,%eax
80100958:	74 21                	je     8010097b <consoleread+0x56>
        release(&input.lock);
8010095a:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100961:	e8 b3 45 00 00       	call   80104f19 <release>
        ilock(ip);
80100966:	8b 45 08             	mov    0x8(%ebp),%eax
80100969:	89 04 24             	mov    %eax,(%esp)
8010096c:	e8 f7 0e 00 00       	call   80101868 <ilock>
        return -1;
80100971:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100976:	e9 a9 00 00 00       	jmp    80100a24 <consoleread+0xff>
      }
      sleep(&input.r, &input.lock);
8010097b:	c7 44 24 04 a0 dd 10 	movl   $0x8010dda0,0x4(%esp)
80100982:	80 
80100983:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
8010098a:	e8 11 41 00 00       	call   80104aa0 <sleep>
8010098f:	eb 01                	jmp    80100992 <consoleread+0x6d>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100991:	90                   	nop
80100992:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
80100998:	a1 58 de 10 80       	mov    0x8010de58,%eax
8010099d:	39 c2                	cmp    %eax,%edx
8010099f:	74 ac                	je     8010094d <consoleread+0x28>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
801009a1:	a1 54 de 10 80       	mov    0x8010de54,%eax
801009a6:	89 c2                	mov    %eax,%edx
801009a8:	83 e2 7f             	and    $0x7f,%edx
801009ab:	0f b6 92 d4 dd 10 80 	movzbl -0x7fef222c(%edx),%edx
801009b2:	0f be d2             	movsbl %dl,%edx
801009b5:	89 55 f0             	mov    %edx,-0x10(%ebp)
801009b8:	83 c0 01             	add    $0x1,%eax
801009bb:	a3 54 de 10 80       	mov    %eax,0x8010de54
    if(c == C('D')){  // EOF
801009c0:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009c4:	75 17                	jne    801009dd <consoleread+0xb8>
      if(n < target){
801009c6:	8b 45 10             	mov    0x10(%ebp),%eax
801009c9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009cc:	73 2f                	jae    801009fd <consoleread+0xd8>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009ce:	a1 54 de 10 80       	mov    0x8010de54,%eax
801009d3:	83 e8 01             	sub    $0x1,%eax
801009d6:	a3 54 de 10 80       	mov    %eax,0x8010de54
      }
      break;
801009db:	eb 20                	jmp    801009fd <consoleread+0xd8>
    }
    *dst++ = c;
801009dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801009e0:	89 c2                	mov    %eax,%edx
801009e2:	8b 45 0c             	mov    0xc(%ebp),%eax
801009e5:	88 10                	mov    %dl,(%eax)
801009e7:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
    --n;
801009eb:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009ef:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009f3:	74 0b                	je     80100a00 <consoleread+0xdb>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009f5:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f9:	7f 96                	jg     80100991 <consoleread+0x6c>
801009fb:	eb 04                	jmp    80100a01 <consoleread+0xdc>
      if(n < target){
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
      }
      break;
801009fd:	90                   	nop
801009fe:	eb 01                	jmp    80100a01 <consoleread+0xdc>
    }
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
80100a00:	90                   	nop
  }
  release(&input.lock);
80100a01:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100a08:	e8 0c 45 00 00       	call   80104f19 <release>
  ilock(ip);
80100a0d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a10:	89 04 24             	mov    %eax,(%esp)
80100a13:	e8 50 0e 00 00       	call   80101868 <ilock>

  return target - n;
80100a18:	8b 45 10             	mov    0x10(%ebp),%eax
80100a1b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a1e:	89 d1                	mov    %edx,%ecx
80100a20:	29 c1                	sub    %eax,%ecx
80100a22:	89 c8                	mov    %ecx,%eax
}
80100a24:	c9                   	leave  
80100a25:	c3                   	ret    

80100a26 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a26:	55                   	push   %ebp
80100a27:	89 e5                	mov    %esp,%ebp
80100a29:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a2c:	8b 45 08             	mov    0x8(%ebp),%eax
80100a2f:	89 04 24             	mov    %eax,(%esp)
80100a32:	e8 7f 0f 00 00       	call   801019b6 <iunlock>
  acquire(&cons.lock);
80100a37:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a3e:	e8 74 44 00 00       	call   80104eb7 <acquire>
  for(i = 0; i < n; i++)
80100a43:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a4a:	eb 1d                	jmp    80100a69 <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a4f:	03 45 0c             	add    0xc(%ebp),%eax
80100a52:	0f b6 00             	movzbl (%eax),%eax
80100a55:	0f be c0             	movsbl %al,%eax
80100a58:	25 ff 00 00 00       	and    $0xff,%eax
80100a5d:	89 04 24             	mov    %eax,(%esp)
80100a60:	e8 eb fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a65:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a6c:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a6f:	7c db                	jl     80100a4c <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a71:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a78:	e8 9c 44 00 00       	call   80104f19 <release>
  ilock(ip);
80100a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a80:	89 04 24             	mov    %eax,(%esp)
80100a83:	e8 e0 0d 00 00       	call   80101868 <ilock>

  return n;
80100a88:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a8b:	c9                   	leave  
80100a8c:	c3                   	ret    

80100a8d <consoleinit>:

void
consoleinit(void)
{
80100a8d:	55                   	push   %ebp
80100a8e:	89 e5                	mov    %esp,%ebp
80100a90:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a93:	c7 44 24 04 67 86 10 	movl   $0x80108667,0x4(%esp)
80100a9a:	80 
80100a9b:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100aa2:	e8 ef 43 00 00       	call   80104e96 <initlock>
  initlock(&input.lock, "input");
80100aa7:	c7 44 24 04 6f 86 10 	movl   $0x8010866f,0x4(%esp)
80100aae:	80 
80100aaf:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100ab6:	e8 db 43 00 00       	call   80104e96 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100abb:	c7 05 0c e8 10 80 26 	movl   $0x80100a26,0x8010e80c
80100ac2:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ac5:	c7 05 08 e8 10 80 25 	movl   $0x80100925,0x8010e808
80100acc:	09 10 80 
  cons.locking = 1;
80100acf:	c7 05 f4 b5 10 80 01 	movl   $0x1,0x8010b5f4
80100ad6:	00 00 00 

  picenable(IRQ_KBD);
80100ad9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae0:	e8 bc 2f 00 00       	call   80103aa1 <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ae5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100aec:	00 
80100aed:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100af4:	e8 7d 1e 00 00       	call   80102976 <ioapicenable>
}
80100af9:	c9                   	leave  
80100afa:	c3                   	ret    
	...

80100afc <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100afc:	55                   	push   %ebp
80100afd:	89 e5                	mov    %esp,%ebp
80100aff:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  if((ip = namei(path)) == 0)
80100b05:	8b 45 08             	mov    0x8(%ebp),%eax
80100b08:	89 04 24             	mov    %eax,(%esp)
80100b0b:	e8 fa 18 00 00       	call   8010240a <namei>
80100b10:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b13:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b17:	75 0a                	jne    80100b23 <exec+0x27>
    return -1;
80100b19:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1e:	e9 da 03 00 00       	jmp    80100efd <exec+0x401>
  ilock(ip);
80100b23:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b26:	89 04 24             	mov    %eax,(%esp)
80100b29:	e8 3a 0d 00 00       	call   80101868 <ilock>
  pgdir = 0;
80100b2e:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b35:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b3c:	00 
80100b3d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b44:	00 
80100b45:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b4b:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4f:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b52:	89 04 24             	mov    %eax,(%esp)
80100b55:	e8 04 12 00 00       	call   80101d5e <readi>
80100b5a:	83 f8 33             	cmp    $0x33,%eax
80100b5d:	0f 86 54 03 00 00    	jbe    80100eb7 <exec+0x3bb>
    goto bad;
  if(elf.magic != ELF_MAGIC)
80100b63:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b69:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6e:	0f 85 46 03 00 00    	jne    80100eba <exec+0x3be>
    goto bad;

  if((pgdir = setupkvm(kalloc)) == 0)
80100b74:	c7 04 24 ff 2a 10 80 	movl   $0x80102aff,(%esp)
80100b7b:	e8 45 72 00 00       	call   80107dc5 <setupkvm>
80100b80:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b83:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b87:	0f 84 30 03 00 00    	je     80100ebd <exec+0x3c1>
    goto bad;

  // Load program into memory.
  sz = 0;
80100b8d:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b94:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b9b:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100ba1:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100ba4:	e9 c5 00 00 00       	jmp    80100c6e <exec+0x172>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba9:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100bac:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bb3:	00 
80100bb4:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb8:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bbe:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bc2:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bc5:	89 04 24             	mov    %eax,(%esp)
80100bc8:	e8 91 11 00 00       	call   80101d5e <readi>
80100bcd:	83 f8 20             	cmp    $0x20,%eax
80100bd0:	0f 85 ea 02 00 00    	jne    80100ec0 <exec+0x3c4>
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
80100bd6:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bdc:	83 f8 01             	cmp    $0x1,%eax
80100bdf:	75 7f                	jne    80100c60 <exec+0x164>
      continue;
    if(ph.memsz < ph.filesz)
80100be1:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be7:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bed:	39 c2                	cmp    %eax,%edx
80100bef:	0f 82 ce 02 00 00    	jb     80100ec3 <exec+0x3c7>
      goto bad;
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 7e 75 00 00       	call   80108197 <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	0f 84 a0 02 00 00    	je     80100ec6 <exec+0x3ca>
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c26:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2c:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c32:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c38:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3c:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c40:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c43:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4b:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4e:	89 04 24             	mov    %eax,(%esp)
80100c51:	e8 52 74 00 00       	call   801080a8 <loaduvm>
80100c56:	85 c0                	test   %eax,%eax
80100c58:	0f 88 6b 02 00 00    	js     80100ec9 <exec+0x3cd>
80100c5e:	eb 01                	jmp    80100c61 <exec+0x165>
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
      continue;
80100c60:	90                   	nop
  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c61:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c65:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c68:	83 c0 20             	add    $0x20,%eax
80100c6b:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6e:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c75:	0f b7 c0             	movzwl %ax,%eax
80100c78:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7b:	0f 8f 28 ff ff ff    	jg     80100ba9 <exec+0xad>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c81:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c84:	89 04 24             	mov    %eax,(%esp)
80100c87:	e8 60 0e 00 00       	call   80101aec <iunlockput>
  ip = 0;
80100c8c:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c93:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c96:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca0:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ca6:	05 00 20 00 00       	add    $0x2000,%eax
80100cab:	89 44 24 08          	mov    %eax,0x8(%esp)
80100caf:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb2:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cb6:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cb9:	89 04 24             	mov    %eax,(%esp)
80100cbc:	e8 d6 74 00 00       	call   80108197 <allocuvm>
80100cc1:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc4:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100cc8:	0f 84 fe 01 00 00    	je     80100ecc <exec+0x3d0>
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cce:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd1:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cd6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cda:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cdd:	89 04 24             	mov    %eax,(%esp)
80100ce0:	e8 d6 76 00 00       	call   801083bb <clearpteu>
  sp = sz;
80100ce5:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ce8:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100ceb:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf2:	e9 81 00 00 00       	jmp    80100d78 <exec+0x27c>
    if(argc >= MAXARG)
80100cf7:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100cfb:	0f 87 ce 01 00 00    	ja     80100ecf <exec+0x3d3>
      goto bad;
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d01:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d04:	c1 e0 02             	shl    $0x2,%eax
80100d07:	03 45 0c             	add    0xc(%ebp),%eax
80100d0a:	8b 00                	mov    (%eax),%eax
80100d0c:	89 04 24             	mov    %eax,(%esp)
80100d0f:	e8 70 46 00 00       	call   80105384 <strlen>
80100d14:	f7 d0                	not    %eax
80100d16:	03 45 dc             	add    -0x24(%ebp),%eax
80100d19:	83 e0 fc             	and    $0xfffffffc,%eax
80100d1c:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d1f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d22:	c1 e0 02             	shl    $0x2,%eax
80100d25:	03 45 0c             	add    0xc(%ebp),%eax
80100d28:	8b 00                	mov    (%eax),%eax
80100d2a:	89 04 24             	mov    %eax,(%esp)
80100d2d:	e8 52 46 00 00       	call   80105384 <strlen>
80100d32:	83 c0 01             	add    $0x1,%eax
80100d35:	89 c2                	mov    %eax,%edx
80100d37:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d3a:	c1 e0 02             	shl    $0x2,%eax
80100d3d:	03 45 0c             	add    0xc(%ebp),%eax
80100d40:	8b 00                	mov    (%eax),%eax
80100d42:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d46:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d4a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d51:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d54:	89 04 24             	mov    %eax,(%esp)
80100d57:	e8 13 78 00 00       	call   8010856f <copyout>
80100d5c:	85 c0                	test   %eax,%eax
80100d5e:	0f 88 6e 01 00 00    	js     80100ed2 <exec+0x3d6>
      goto bad;
    ustack[3+argc] = sp;
80100d64:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d67:	8d 50 03             	lea    0x3(%eax),%edx
80100d6a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6d:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d74:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d78:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d7b:	c1 e0 02             	shl    $0x2,%eax
80100d7e:	03 45 0c             	add    0xc(%ebp),%eax
80100d81:	8b 00                	mov    (%eax),%eax
80100d83:	85 c0                	test   %eax,%eax
80100d85:	0f 85 6c ff ff ff    	jne    80100cf7 <exec+0x1fb>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100d8b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d8e:	83 c0 03             	add    $0x3,%eax
80100d91:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100d98:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100d9c:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100da3:	ff ff ff 
  ustack[1] = argc;
80100da6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100da9:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 01             	add    $0x1,%eax
80100db5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100dbc:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100dbf:	29 d0                	sub    %edx,%eax
80100dc1:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100dc7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dca:	83 c0 04             	add    $0x4,%eax
80100dcd:	c1 e0 02             	shl    $0x2,%eax
80100dd0:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 04             	add    $0x4,%eax
80100dd9:	c1 e0 02             	shl    $0x2,%eax
80100ddc:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100de0:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100de6:	89 44 24 08          	mov    %eax,0x8(%esp)
80100dea:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100ded:	89 44 24 04          	mov    %eax,0x4(%esp)
80100df1:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100df4:	89 04 24             	mov    %eax,(%esp)
80100df7:	e8 73 77 00 00       	call   8010856f <copyout>
80100dfc:	85 c0                	test   %eax,%eax
80100dfe:	0f 88 d1 00 00 00    	js     80100ed5 <exec+0x3d9>
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e04:	8b 45 08             	mov    0x8(%ebp),%eax
80100e07:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e0d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e10:	eb 17                	jmp    80100e29 <exec+0x32d>
    if(*s == '/')
80100e12:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e15:	0f b6 00             	movzbl (%eax),%eax
80100e18:	3c 2f                	cmp    $0x2f,%al
80100e1a:	75 09                	jne    80100e25 <exec+0x329>
      last = s+1;
80100e1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e1f:	83 c0 01             	add    $0x1,%eax
80100e22:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e25:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e2c:	0f b6 00             	movzbl (%eax),%eax
80100e2f:	84 c0                	test   %al,%al
80100e31:	75 df                	jne    80100e12 <exec+0x316>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e33:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e39:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e3c:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e43:	00 
80100e44:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e4b:	89 14 24             	mov    %edx,(%esp)
80100e4e:	e8 e3 44 00 00       	call   80105336 <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e53:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e59:	8b 40 04             	mov    0x4(%eax),%eax
80100e5c:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e5f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e65:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e68:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e6b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e71:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e74:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e76:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7c:	8b 40 18             	mov    0x18(%eax),%eax
80100e7f:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100e85:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100e88:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8e:	8b 40 18             	mov    0x18(%eax),%eax
80100e91:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100e94:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100e97:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e9d:	89 04 24             	mov    %eax,(%esp)
80100ea0:	e8 11 70 00 00       	call   80107eb6 <switchuvm>
  freevm(oldpgdir);
80100ea5:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ea8:	89 04 24             	mov    %eax,(%esp)
80100eab:	e8 7d 74 00 00       	call   8010832d <freevm>
  return 0;
80100eb0:	b8 00 00 00 00       	mov    $0x0,%eax
80100eb5:	eb 46                	jmp    80100efd <exec+0x401>
  ilock(ip);
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
    goto bad;
80100eb7:	90                   	nop
80100eb8:	eb 1c                	jmp    80100ed6 <exec+0x3da>
  if(elf.magic != ELF_MAGIC)
    goto bad;
80100eba:	90                   	nop
80100ebb:	eb 19                	jmp    80100ed6 <exec+0x3da>

  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;
80100ebd:	90                   	nop
80100ebe:	eb 16                	jmp    80100ed6 <exec+0x3da>

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
80100ec0:	90                   	nop
80100ec1:	eb 13                	jmp    80100ed6 <exec+0x3da>
    if(ph.type != ELF_PROG_LOAD)
      continue;
    if(ph.memsz < ph.filesz)
      goto bad;
80100ec3:	90                   	nop
80100ec4:	eb 10                	jmp    80100ed6 <exec+0x3da>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
80100ec6:	90                   	nop
80100ec7:	eb 0d                	jmp    80100ed6 <exec+0x3da>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
80100ec9:	90                   	nop
80100eca:	eb 0a                	jmp    80100ed6 <exec+0x3da>

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
    goto bad;
80100ecc:	90                   	nop
80100ecd:	eb 07                	jmp    80100ed6 <exec+0x3da>
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
    if(argc >= MAXARG)
      goto bad;
80100ecf:	90                   	nop
80100ed0:	eb 04                	jmp    80100ed6 <exec+0x3da>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
80100ed2:	90                   	nop
80100ed3:	eb 01                	jmp    80100ed6 <exec+0x3da>
  ustack[1] = argc;
  ustack[2] = sp - (argc+1)*4;  // argv pointer

  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;
80100ed5:	90                   	nop
  switchuvm(proc);
  freevm(oldpgdir);
  return 0;

 bad:
  if(pgdir)
80100ed6:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100eda:	74 0b                	je     80100ee7 <exec+0x3eb>
    freevm(pgdir);
80100edc:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100edf:	89 04 24             	mov    %eax,(%esp)
80100ee2:	e8 46 74 00 00       	call   8010832d <freevm>
  if(ip)
80100ee7:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100eeb:	74 0b                	je     80100ef8 <exec+0x3fc>
    iunlockput(ip);
80100eed:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef0:	89 04 24             	mov    %eax,(%esp)
80100ef3:	e8 f4 0b 00 00       	call   80101aec <iunlockput>
  return -1;
80100ef8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100efd:	c9                   	leave  
80100efe:	c3                   	ret    
	...

80100f00 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f00:	55                   	push   %ebp
80100f01:	89 e5                	mov    %esp,%ebp
80100f03:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f06:	c7 44 24 04 75 86 10 	movl   $0x80108675,0x4(%esp)
80100f0d:	80 
80100f0e:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f15:	e8 7c 3f 00 00       	call   80104e96 <initlock>
}
80100f1a:	c9                   	leave  
80100f1b:	c3                   	ret    

80100f1c <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f1c:	55                   	push   %ebp
80100f1d:	89 e5                	mov    %esp,%ebp
80100f1f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f22:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f29:	e8 89 3f 00 00       	call   80104eb7 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f2e:	c7 45 f4 94 de 10 80 	movl   $0x8010de94,-0xc(%ebp)
80100f35:	eb 29                	jmp    80100f60 <filealloc+0x44>
    if(f->ref == 0){
80100f37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f3a:	8b 40 04             	mov    0x4(%eax),%eax
80100f3d:	85 c0                	test   %eax,%eax
80100f3f:	75 1b                	jne    80100f5c <filealloc+0x40>
      f->ref = 1;
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f4b:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f52:	e8 c2 3f 00 00       	call   80104f19 <release>
      return f;
80100f57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f5a:	eb 1e                	jmp    80100f7a <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f5c:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f60:	81 7d f4 f4 e7 10 80 	cmpl   $0x8010e7f4,-0xc(%ebp)
80100f67:	72 ce                	jb     80100f37 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f69:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f70:	e8 a4 3f 00 00       	call   80104f19 <release>
  return 0;
80100f75:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f7a:	c9                   	leave  
80100f7b:	c3                   	ret    

80100f7c <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f7c:	55                   	push   %ebp
80100f7d:	89 e5                	mov    %esp,%ebp
80100f7f:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f82:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f89:	e8 29 3f 00 00       	call   80104eb7 <acquire>
  if(f->ref < 1)
80100f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80100f91:	8b 40 04             	mov    0x4(%eax),%eax
80100f94:	85 c0                	test   %eax,%eax
80100f96:	7f 0c                	jg     80100fa4 <filedup+0x28>
    panic("filedup");
80100f98:	c7 04 24 7c 86 10 80 	movl   $0x8010867c,(%esp)
80100f9f:	e8 99 f5 ff ff       	call   8010053d <panic>
  f->ref++;
80100fa4:	8b 45 08             	mov    0x8(%ebp),%eax
80100fa7:	8b 40 04             	mov    0x4(%eax),%eax
80100faa:	8d 50 01             	lea    0x1(%eax),%edx
80100fad:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb0:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fb3:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fba:	e8 5a 3f 00 00       	call   80104f19 <release>
  return f;
80100fbf:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fc2:	c9                   	leave  
80100fc3:	c3                   	ret    

80100fc4 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fc4:	55                   	push   %ebp
80100fc5:	89 e5                	mov    %esp,%ebp
80100fc7:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fca:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fd1:	e8 e1 3e 00 00       	call   80104eb7 <acquire>
  if(f->ref < 1)
80100fd6:	8b 45 08             	mov    0x8(%ebp),%eax
80100fd9:	8b 40 04             	mov    0x4(%eax),%eax
80100fdc:	85 c0                	test   %eax,%eax
80100fde:	7f 0c                	jg     80100fec <fileclose+0x28>
    panic("fileclose");
80100fe0:	c7 04 24 84 86 10 80 	movl   $0x80108684,(%esp)
80100fe7:	e8 51 f5 ff ff       	call   8010053d <panic>
  if(--f->ref > 0){
80100fec:	8b 45 08             	mov    0x8(%ebp),%eax
80100fef:	8b 40 04             	mov    0x4(%eax),%eax
80100ff2:	8d 50 ff             	lea    -0x1(%eax),%edx
80100ff5:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff8:	89 50 04             	mov    %edx,0x4(%eax)
80100ffb:	8b 45 08             	mov    0x8(%ebp),%eax
80100ffe:	8b 40 04             	mov    0x4(%eax),%eax
80101001:	85 c0                	test   %eax,%eax
80101003:	7e 11                	jle    80101016 <fileclose+0x52>
    release(&ftable.lock);
80101005:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
8010100c:	e8 08 3f 00 00       	call   80104f19 <release>
    return;
80101011:	e9 82 00 00 00       	jmp    80101098 <fileclose+0xd4>
  }
  ff = *f;
80101016:	8b 45 08             	mov    0x8(%ebp),%eax
80101019:	8b 10                	mov    (%eax),%edx
8010101b:	89 55 e0             	mov    %edx,-0x20(%ebp)
8010101e:	8b 50 04             	mov    0x4(%eax),%edx
80101021:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80101024:	8b 50 08             	mov    0x8(%eax),%edx
80101027:	89 55 e8             	mov    %edx,-0x18(%ebp)
8010102a:	8b 50 0c             	mov    0xc(%eax),%edx
8010102d:	89 55 ec             	mov    %edx,-0x14(%ebp)
80101030:	8b 50 10             	mov    0x10(%eax),%edx
80101033:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101036:	8b 40 14             	mov    0x14(%eax),%eax
80101039:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
8010103c:	8b 45 08             	mov    0x8(%ebp),%eax
8010103f:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
8010104f:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80101056:	e8 be 3e 00 00       	call   80104f19 <release>
  
  if(ff.type == FD_PIPE)
8010105b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010105e:	83 f8 01             	cmp    $0x1,%eax
80101061:	75 18                	jne    8010107b <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
80101063:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101067:	0f be d0             	movsbl %al,%edx
8010106a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010106d:	89 54 24 04          	mov    %edx,0x4(%esp)
80101071:	89 04 24             	mov    %eax,(%esp)
80101074:	e8 e2 2c 00 00       	call   80103d5b <pipeclose>
80101079:	eb 1d                	jmp    80101098 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
8010107b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010107e:	83 f8 02             	cmp    $0x2,%eax
80101081:	75 15                	jne    80101098 <fileclose+0xd4>
    begin_trans();
80101083:	e8 95 21 00 00       	call   8010321d <begin_trans>
    iput(ff.ip);
80101088:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010108b:	89 04 24             	mov    %eax,(%esp)
8010108e:	e8 88 09 00 00       	call   80101a1b <iput>
    commit_trans();
80101093:	e8 b9 21 00 00       	call   80103251 <commit_trans>
  }
}
80101098:	c9                   	leave  
80101099:	c3                   	ret    

8010109a <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
8010109a:	55                   	push   %ebp
8010109b:	89 e5                	mov    %esp,%ebp
8010109d:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010a0:	8b 45 08             	mov    0x8(%ebp),%eax
801010a3:	8b 00                	mov    (%eax),%eax
801010a5:	83 f8 02             	cmp    $0x2,%eax
801010a8:	75 38                	jne    801010e2 <filestat+0x48>
    ilock(f->ip);
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 40 10             	mov    0x10(%eax),%eax
801010b0:	89 04 24             	mov    %eax,(%esp)
801010b3:	e8 b0 07 00 00       	call   80101868 <ilock>
    stati(f->ip, st);
801010b8:	8b 45 08             	mov    0x8(%ebp),%eax
801010bb:	8b 40 10             	mov    0x10(%eax),%eax
801010be:	8b 55 0c             	mov    0xc(%ebp),%edx
801010c1:	89 54 24 04          	mov    %edx,0x4(%esp)
801010c5:	89 04 24             	mov    %eax,(%esp)
801010c8:	e8 4c 0c 00 00       	call   80101d19 <stati>
    iunlock(f->ip);
801010cd:	8b 45 08             	mov    0x8(%ebp),%eax
801010d0:	8b 40 10             	mov    0x10(%eax),%eax
801010d3:	89 04 24             	mov    %eax,(%esp)
801010d6:	e8 db 08 00 00       	call   801019b6 <iunlock>
    return 0;
801010db:	b8 00 00 00 00       	mov    $0x0,%eax
801010e0:	eb 05                	jmp    801010e7 <filestat+0x4d>
  }
  return -1;
801010e2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010e7:	c9                   	leave  
801010e8:	c3                   	ret    

801010e9 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010e9:	55                   	push   %ebp
801010ea:	89 e5                	mov    %esp,%ebp
801010ec:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010ef:	8b 45 08             	mov    0x8(%ebp),%eax
801010f2:	0f b6 40 08          	movzbl 0x8(%eax),%eax
801010f6:	84 c0                	test   %al,%al
801010f8:	75 0a                	jne    80101104 <fileread+0x1b>
    return -1;
801010fa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801010ff:	e9 9f 00 00 00       	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_PIPE)
80101104:	8b 45 08             	mov    0x8(%ebp),%eax
80101107:	8b 00                	mov    (%eax),%eax
80101109:	83 f8 01             	cmp    $0x1,%eax
8010110c:	75 1e                	jne    8010112c <fileread+0x43>
    return piperead(f->pipe, addr, n);
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 40 0c             	mov    0xc(%eax),%eax
80101114:	8b 55 10             	mov    0x10(%ebp),%edx
80101117:	89 54 24 08          	mov    %edx,0x8(%esp)
8010111b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010111e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101122:	89 04 24             	mov    %eax,(%esp)
80101125:	e8 b3 2d 00 00       	call   80103edd <piperead>
8010112a:	eb 77                	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_INODE){
8010112c:	8b 45 08             	mov    0x8(%ebp),%eax
8010112f:	8b 00                	mov    (%eax),%eax
80101131:	83 f8 02             	cmp    $0x2,%eax
80101134:	75 61                	jne    80101197 <fileread+0xae>
    ilock(f->ip);
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 40 10             	mov    0x10(%eax),%eax
8010113c:	89 04 24             	mov    %eax,(%esp)
8010113f:	e8 24 07 00 00       	call   80101868 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
80101144:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101147:	8b 45 08             	mov    0x8(%ebp),%eax
8010114a:	8b 50 14             	mov    0x14(%eax),%edx
8010114d:	8b 45 08             	mov    0x8(%ebp),%eax
80101150:	8b 40 10             	mov    0x10(%eax),%eax
80101153:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101157:	89 54 24 08          	mov    %edx,0x8(%esp)
8010115b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010115e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101162:	89 04 24             	mov    %eax,(%esp)
80101165:	e8 f4 0b 00 00       	call   80101d5e <readi>
8010116a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010116d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101171:	7e 11                	jle    80101184 <fileread+0x9b>
      f->off += r;
80101173:	8b 45 08             	mov    0x8(%ebp),%eax
80101176:	8b 50 14             	mov    0x14(%eax),%edx
80101179:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010117c:	01 c2                	add    %eax,%edx
8010117e:	8b 45 08             	mov    0x8(%ebp),%eax
80101181:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
80101184:	8b 45 08             	mov    0x8(%ebp),%eax
80101187:	8b 40 10             	mov    0x10(%eax),%eax
8010118a:	89 04 24             	mov    %eax,(%esp)
8010118d:	e8 24 08 00 00       	call   801019b6 <iunlock>
    return r;
80101192:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101195:	eb 0c                	jmp    801011a3 <fileread+0xba>
  }
  panic("fileread");
80101197:	c7 04 24 8e 86 10 80 	movl   $0x8010868e,(%esp)
8010119e:	e8 9a f3 ff ff       	call   8010053d <panic>
}
801011a3:	c9                   	leave  
801011a4:	c3                   	ret    

801011a5 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011a5:	55                   	push   %ebp
801011a6:	89 e5                	mov    %esp,%ebp
801011a8:	53                   	push   %ebx
801011a9:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011ac:	8b 45 08             	mov    0x8(%ebp),%eax
801011af:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011b3:	84 c0                	test   %al,%al
801011b5:	75 0a                	jne    801011c1 <filewrite+0x1c>
    return -1;
801011b7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011bc:	e9 23 01 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_PIPE)
801011c1:	8b 45 08             	mov    0x8(%ebp),%eax
801011c4:	8b 00                	mov    (%eax),%eax
801011c6:	83 f8 01             	cmp    $0x1,%eax
801011c9:	75 21                	jne    801011ec <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 40 0c             	mov    0xc(%eax),%eax
801011d1:	8b 55 10             	mov    0x10(%ebp),%edx
801011d4:	89 54 24 08          	mov    %edx,0x8(%esp)
801011d8:	8b 55 0c             	mov    0xc(%ebp),%edx
801011db:	89 54 24 04          	mov    %edx,0x4(%esp)
801011df:	89 04 24             	mov    %eax,(%esp)
801011e2:	e8 06 2c 00 00       	call   80103ded <pipewrite>
801011e7:	e9 f8 00 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_INODE){
801011ec:	8b 45 08             	mov    0x8(%ebp),%eax
801011ef:	8b 00                	mov    (%eax),%eax
801011f1:	83 f8 02             	cmp    $0x2,%eax
801011f4:	0f 85 de 00 00 00    	jne    801012d8 <filewrite+0x133>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
801011fa:	c7 45 ec 00 06 00 00 	movl   $0x600,-0x14(%ebp)
    int i = 0;
80101201:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101208:	e9 a8 00 00 00       	jmp    801012b5 <filewrite+0x110>
      int n1 = n - i;
8010120d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101210:	8b 55 10             	mov    0x10(%ebp),%edx
80101213:	89 d1                	mov    %edx,%ecx
80101215:	29 c1                	sub    %eax,%ecx
80101217:	89 c8                	mov    %ecx,%eax
80101219:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
8010121c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010121f:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80101222:	7e 06                	jle    8010122a <filewrite+0x85>
        n1 = max;
80101224:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101227:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_trans();
8010122a:	e8 ee 1f 00 00       	call   8010321d <begin_trans>
      ilock(f->ip);
8010122f:	8b 45 08             	mov    0x8(%ebp),%eax
80101232:	8b 40 10             	mov    0x10(%eax),%eax
80101235:	89 04 24             	mov    %eax,(%esp)
80101238:	e8 2b 06 00 00       	call   80101868 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
8010123d:	8b 5d f0             	mov    -0x10(%ebp),%ebx
80101240:	8b 45 08             	mov    0x8(%ebp),%eax
80101243:	8b 48 14             	mov    0x14(%eax),%ecx
80101246:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101249:	89 c2                	mov    %eax,%edx
8010124b:	03 55 0c             	add    0xc(%ebp),%edx
8010124e:	8b 45 08             	mov    0x8(%ebp),%eax
80101251:	8b 40 10             	mov    0x10(%eax),%eax
80101254:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80101258:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010125c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101260:	89 04 24             	mov    %eax,(%esp)
80101263:	e8 61 0c 00 00       	call   80101ec9 <writei>
80101268:	89 45 e8             	mov    %eax,-0x18(%ebp)
8010126b:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010126f:	7e 11                	jle    80101282 <filewrite+0xdd>
        f->off += r;
80101271:	8b 45 08             	mov    0x8(%ebp),%eax
80101274:	8b 50 14             	mov    0x14(%eax),%edx
80101277:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010127a:	01 c2                	add    %eax,%edx
8010127c:	8b 45 08             	mov    0x8(%ebp),%eax
8010127f:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
80101282:	8b 45 08             	mov    0x8(%ebp),%eax
80101285:	8b 40 10             	mov    0x10(%eax),%eax
80101288:	89 04 24             	mov    %eax,(%esp)
8010128b:	e8 26 07 00 00       	call   801019b6 <iunlock>
      commit_trans();
80101290:	e8 bc 1f 00 00       	call   80103251 <commit_trans>

      if(r < 0)
80101295:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101299:	78 28                	js     801012c3 <filewrite+0x11e>
        break;
      if(r != n1)
8010129b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010129e:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012a1:	74 0c                	je     801012af <filewrite+0x10a>
        panic("short filewrite");
801012a3:	c7 04 24 97 86 10 80 	movl   $0x80108697,(%esp)
801012aa:	e8 8e f2 ff ff       	call   8010053d <panic>
      i += r;
801012af:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012b2:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012b8:	3b 45 10             	cmp    0x10(%ebp),%eax
801012bb:	0f 8c 4c ff ff ff    	jl     8010120d <filewrite+0x68>
801012c1:	eb 01                	jmp    801012c4 <filewrite+0x11f>
        f->off += r;
      iunlock(f->ip);
      commit_trans();

      if(r < 0)
        break;
801012c3:	90                   	nop
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c7:	3b 45 10             	cmp    0x10(%ebp),%eax
801012ca:	75 05                	jne    801012d1 <filewrite+0x12c>
801012cc:	8b 45 10             	mov    0x10(%ebp),%eax
801012cf:	eb 05                	jmp    801012d6 <filewrite+0x131>
801012d1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012d6:	eb 0c                	jmp    801012e4 <filewrite+0x13f>
  }
  panic("filewrite");
801012d8:	c7 04 24 a7 86 10 80 	movl   $0x801086a7,(%esp)
801012df:	e8 59 f2 ff ff       	call   8010053d <panic>
}
801012e4:	83 c4 24             	add    $0x24,%esp
801012e7:	5b                   	pop    %ebx
801012e8:	5d                   	pop    %ebp
801012e9:	c3                   	ret    
	...

801012ec <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012ec:	55                   	push   %ebp
801012ed:	89 e5                	mov    %esp,%ebp
801012ef:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f2:	8b 45 08             	mov    0x8(%ebp),%eax
801012f5:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801012fc:	00 
801012fd:	89 04 24             	mov    %eax,(%esp)
80101300:	e8 a1 ee ff ff       	call   801001a6 <bread>
80101305:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
80101308:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010130b:	83 c0 18             	add    $0x18,%eax
8010130e:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80101315:	00 
80101316:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010131d:	89 04 24             	mov    %eax,(%esp)
80101320:	e8 b4 3e 00 00       	call   801051d9 <memmove>
  brelse(bp);
80101325:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101328:	89 04 24             	mov    %eax,(%esp)
8010132b:	e8 e7 ee ff ff       	call   80100217 <brelse>
}
80101330:	c9                   	leave  
80101331:	c3                   	ret    

80101332 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101332:	55                   	push   %ebp
80101333:	89 e5                	mov    %esp,%ebp
80101335:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
80101338:	8b 55 0c             	mov    0xc(%ebp),%edx
8010133b:	8b 45 08             	mov    0x8(%ebp),%eax
8010133e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101342:	89 04 24             	mov    %eax,(%esp)
80101345:	e8 5c ee ff ff       	call   801001a6 <bread>
8010134a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
8010134d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101350:	83 c0 18             	add    $0x18,%eax
80101353:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135a:	00 
8010135b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101362:	00 
80101363:	89 04 24             	mov    %eax,(%esp)
80101366:	e8 9b 3d 00 00       	call   80105106 <memset>
  log_write(bp);
8010136b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010136e:	89 04 24             	mov    %eax,(%esp)
80101371:	e8 27 1f 00 00       	call   8010329d <log_write>
  brelse(bp);
80101376:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101379:	89 04 24             	mov    %eax,(%esp)
8010137c:	e8 96 ee ff ff       	call   80100217 <brelse>
}
80101381:	c9                   	leave  
80101382:	c3                   	ret    

80101383 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101383:	55                   	push   %ebp
80101384:	89 e5                	mov    %esp,%ebp
80101386:	53                   	push   %ebx
80101387:	83 ec 34             	sub    $0x34,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138a:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101391:	8b 45 08             	mov    0x8(%ebp),%eax
80101394:	8d 55 d8             	lea    -0x28(%ebp),%edx
80101397:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139b:	89 04 24             	mov    %eax,(%esp)
8010139e:	e8 49 ff ff ff       	call   801012ec <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013aa:	e9 11 01 00 00       	jmp    801014c0 <balloc+0x13d>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013af:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b2:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013b8:	85 c0                	test   %eax,%eax
801013ba:	0f 48 c2             	cmovs  %edx,%eax
801013bd:	c1 f8 0c             	sar    $0xc,%eax
801013c0:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c3:	c1 ea 03             	shr    $0x3,%edx
801013c6:	01 d0                	add    %edx,%eax
801013c8:	83 c0 03             	add    $0x3,%eax
801013cb:	89 44 24 04          	mov    %eax,0x4(%esp)
801013cf:	8b 45 08             	mov    0x8(%ebp),%eax
801013d2:	89 04 24             	mov    %eax,(%esp)
801013d5:	e8 cc ed ff ff       	call   801001a6 <bread>
801013da:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013dd:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e4:	e9 a7 00 00 00       	jmp    80101490 <balloc+0x10d>
      m = 1 << (bi % 8);
801013e9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013ec:	89 c2                	mov    %eax,%edx
801013ee:	c1 fa 1f             	sar    $0x1f,%edx
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 d3                	mov    %edx,%ebx
80101402:	89 c1                	mov    %eax,%ecx
80101404:	d3 e3                	shl    %cl,%ebx
80101406:	89 d8                	mov    %ebx,%eax
80101408:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
8010140b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140e:	8d 50 07             	lea    0x7(%eax),%edx
80101411:	85 c0                	test   %eax,%eax
80101413:	0f 48 c2             	cmovs  %edx,%eax
80101416:	c1 f8 03             	sar    $0x3,%eax
80101419:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141c:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101421:	0f b6 c0             	movzbl %al,%eax
80101424:	23 45 e8             	and    -0x18(%ebp),%eax
80101427:	85 c0                	test   %eax,%eax
80101429:	75 61                	jne    8010148c <balloc+0x109>
        bp->data[bi/8] |= m;  // Mark block in use.
8010142b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142e:	8d 50 07             	lea    0x7(%eax),%edx
80101431:	85 c0                	test   %eax,%eax
80101433:	0f 48 c2             	cmovs  %edx,%eax
80101436:	c1 f8 03             	sar    $0x3,%eax
80101439:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143c:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101441:	89 d1                	mov    %edx,%ecx
80101443:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101446:	09 ca                	or     %ecx,%edx
80101448:	89 d1                	mov    %edx,%ecx
8010144a:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144d:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
80101451:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101454:	89 04 24             	mov    %eax,(%esp)
80101457:	e8 41 1e 00 00       	call   8010329d <log_write>
        brelse(bp);
8010145c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145f:	89 04 24             	mov    %eax,(%esp)
80101462:	e8 b0 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101467:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010146a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146d:	01 c2                	add    %eax,%edx
8010146f:	8b 45 08             	mov    0x8(%ebp),%eax
80101472:	89 54 24 04          	mov    %edx,0x4(%esp)
80101476:	89 04 24             	mov    %eax,(%esp)
80101479:	e8 b4 fe ff ff       	call   80101332 <bzero>
        return b + bi;
8010147e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101481:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101484:	01 d0                	add    %edx,%eax
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
}
80101486:	83 c4 34             	add    $0x34,%esp
80101489:	5b                   	pop    %ebx
8010148a:	5d                   	pop    %ebp
8010148b:	c3                   	ret    

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010148c:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101490:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101497:	7f 15                	jg     801014ae <balloc+0x12b>
80101499:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010149c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010149f:	01 d0                	add    %edx,%eax
801014a1:	89 c2                	mov    %eax,%edx
801014a3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a6:	39 c2                	cmp    %eax,%edx
801014a8:	0f 82 3b ff ff ff    	jb     801013e9 <balloc+0x66>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014ae:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014b1:	89 04 24             	mov    %eax,(%esp)
801014b4:	e8 5e ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b9:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014c0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014c3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c6:	39 c2                	cmp    %eax,%edx
801014c8:	0f 82 e1 fe ff ff    	jb     801013af <balloc+0x2c>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014ce:	c7 04 24 b1 86 10 80 	movl   $0x801086b1,(%esp)
801014d5:	e8 63 f0 ff ff       	call   8010053d <panic>

801014da <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014da:	55                   	push   %ebp
801014db:	89 e5                	mov    %esp,%ebp
801014dd:	53                   	push   %ebx
801014de:	83 ec 34             	sub    $0x34,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014e1:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014e4:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e8:	8b 45 08             	mov    0x8(%ebp),%eax
801014eb:	89 04 24             	mov    %eax,(%esp)
801014ee:	e8 f9 fd ff ff       	call   801012ec <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014f3:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f6:	89 c2                	mov    %eax,%edx
801014f8:	c1 ea 0c             	shr    $0xc,%edx
801014fb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014fe:	c1 e8 03             	shr    $0x3,%eax
80101501:	01 d0                	add    %edx,%eax
80101503:	8d 50 03             	lea    0x3(%eax),%edx
80101506:	8b 45 08             	mov    0x8(%ebp),%eax
80101509:	89 54 24 04          	mov    %edx,0x4(%esp)
8010150d:	89 04 24             	mov    %eax,(%esp)
80101510:	e8 91 ec ff ff       	call   801001a6 <bread>
80101515:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101518:	8b 45 0c             	mov    0xc(%ebp),%eax
8010151b:	25 ff 0f 00 00       	and    $0xfff,%eax
80101520:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
80101523:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101526:	89 c2                	mov    %eax,%edx
80101528:	c1 fa 1f             	sar    $0x1f,%edx
8010152b:	c1 ea 1d             	shr    $0x1d,%edx
8010152e:	01 d0                	add    %edx,%eax
80101530:	83 e0 07             	and    $0x7,%eax
80101533:	29 d0                	sub    %edx,%eax
80101535:	ba 01 00 00 00       	mov    $0x1,%edx
8010153a:	89 d3                	mov    %edx,%ebx
8010153c:	89 c1                	mov    %eax,%ecx
8010153e:	d3 e3                	shl    %cl,%ebx
80101540:	89 d8                	mov    %ebx,%eax
80101542:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
80101545:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101548:	8d 50 07             	lea    0x7(%eax),%edx
8010154b:	85 c0                	test   %eax,%eax
8010154d:	0f 48 c2             	cmovs  %edx,%eax
80101550:	c1 f8 03             	sar    $0x3,%eax
80101553:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101556:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010155b:	0f b6 c0             	movzbl %al,%eax
8010155e:	23 45 ec             	and    -0x14(%ebp),%eax
80101561:	85 c0                	test   %eax,%eax
80101563:	75 0c                	jne    80101571 <bfree+0x97>
    panic("freeing free block");
80101565:	c7 04 24 c7 86 10 80 	movl   $0x801086c7,(%esp)
8010156c:	e8 cc ef ff ff       	call   8010053d <panic>
  bp->data[bi/8] &= ~m;
80101571:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101574:	8d 50 07             	lea    0x7(%eax),%edx
80101577:	85 c0                	test   %eax,%eax
80101579:	0f 48 c2             	cmovs  %edx,%eax
8010157c:	c1 f8 03             	sar    $0x3,%eax
8010157f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101582:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101587:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010158a:	f7 d1                	not    %ecx
8010158c:	21 ca                	and    %ecx,%edx
8010158e:	89 d1                	mov    %edx,%ecx
80101590:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101593:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 fb 1c 00 00       	call   8010329d <log_write>
  brelse(bp);
801015a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015a5:	89 04 24             	mov    %eax,(%esp)
801015a8:	e8 6a ec ff ff       	call   80100217 <brelse>
}
801015ad:	83 c4 34             	add    $0x34,%esp
801015b0:	5b                   	pop    %ebx
801015b1:	5d                   	pop    %ebp
801015b2:	c3                   	ret    

801015b3 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015b3:	55                   	push   %ebp
801015b4:	89 e5                	mov    %esp,%ebp
801015b6:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015b9:	c7 44 24 04 da 86 10 	movl   $0x801086da,0x4(%esp)
801015c0:	80 
801015c1:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801015c8:	e8 c9 38 00 00       	call   80104e96 <initlock>
}
801015cd:	c9                   	leave  
801015ce:	c3                   	ret    

801015cf <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015cf:	55                   	push   %ebp
801015d0:	89 e5                	mov    %esp,%ebp
801015d2:	83 ec 48             	sub    $0x48,%esp
801015d5:	8b 45 0c             	mov    0xc(%ebp),%eax
801015d8:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015dc:	8b 45 08             	mov    0x8(%ebp),%eax
801015df:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015e2:	89 54 24 04          	mov    %edx,0x4(%esp)
801015e6:	89 04 24             	mov    %eax,(%esp)
801015e9:	e8 fe fc ff ff       	call   801012ec <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015ee:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015f5:	e9 98 00 00 00       	jmp    80101692 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015fd:	c1 e8 03             	shr    $0x3,%eax
80101600:	83 c0 02             	add    $0x2,%eax
80101603:	89 44 24 04          	mov    %eax,0x4(%esp)
80101607:	8b 45 08             	mov    0x8(%ebp),%eax
8010160a:	89 04 24             	mov    %eax,(%esp)
8010160d:	e8 94 eb ff ff       	call   801001a6 <bread>
80101612:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101615:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101618:	8d 50 18             	lea    0x18(%eax),%edx
8010161b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010161e:	83 e0 07             	and    $0x7,%eax
80101621:	c1 e0 06             	shl    $0x6,%eax
80101624:	01 d0                	add    %edx,%eax
80101626:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
80101629:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010162c:	0f b7 00             	movzwl (%eax),%eax
8010162f:	66 85 c0             	test   %ax,%ax
80101632:	75 4f                	jne    80101683 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101634:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010163b:	00 
8010163c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101643:	00 
80101644:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101647:	89 04 24             	mov    %eax,(%esp)
8010164a:	e8 b7 3a 00 00       	call   80105106 <memset>
      dip->type = type;
8010164f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101652:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101656:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
80101659:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010165c:	89 04 24             	mov    %eax,(%esp)
8010165f:	e8 39 1c 00 00       	call   8010329d <log_write>
      brelse(bp);
80101664:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101667:	89 04 24             	mov    %eax,(%esp)
8010166a:	e8 a8 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
8010166f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101672:	89 44 24 04          	mov    %eax,0x4(%esp)
80101676:	8b 45 08             	mov    0x8(%ebp),%eax
80101679:	89 04 24             	mov    %eax,(%esp)
8010167c:	e8 e3 00 00 00       	call   80101764 <iget>
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
}
80101681:	c9                   	leave  
80101682:	c3                   	ret    
      dip->type = type;
      log_write(bp);   // mark it allocated on the disk
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
80101683:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101686:	89 04 24             	mov    %eax,(%esp)
80101689:	e8 89 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010168e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101692:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101695:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101698:	39 c2                	cmp    %eax,%edx
8010169a:	0f 82 5a ff ff ff    	jb     801015fa <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
801016a0:	c7 04 24 e1 86 10 80 	movl   $0x801086e1,(%esp)
801016a7:	e8 91 ee ff ff       	call   8010053d <panic>

801016ac <iupdate>:
}

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
801016ac:	55                   	push   %ebp
801016ad:	89 e5                	mov    %esp,%ebp
801016af:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016b2:	8b 45 08             	mov    0x8(%ebp),%eax
801016b5:	8b 40 04             	mov    0x4(%eax),%eax
801016b8:	c1 e8 03             	shr    $0x3,%eax
801016bb:	8d 50 02             	lea    0x2(%eax),%edx
801016be:	8b 45 08             	mov    0x8(%ebp),%eax
801016c1:	8b 00                	mov    (%eax),%eax
801016c3:	89 54 24 04          	mov    %edx,0x4(%esp)
801016c7:	89 04 24             	mov    %eax,(%esp)
801016ca:	e8 d7 ea ff ff       	call   801001a6 <bread>
801016cf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016d5:	8d 50 18             	lea    0x18(%eax),%edx
801016d8:	8b 45 08             	mov    0x8(%ebp),%eax
801016db:	8b 40 04             	mov    0x4(%eax),%eax
801016de:	83 e0 07             	and    $0x7,%eax
801016e1:	c1 e0 06             	shl    $0x6,%eax
801016e4:	01 d0                	add    %edx,%eax
801016e6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016f6:	8b 45 08             	mov    0x8(%ebp),%eax
801016f9:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101700:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
80101704:	8b 45 08             	mov    0x8(%ebp),%eax
80101707:	0f b7 50 14          	movzwl 0x14(%eax),%edx
8010170b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170e:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101712:	8b 45 08             	mov    0x8(%ebp),%eax
80101715:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101720:	8b 45 08             	mov    0x8(%ebp),%eax
80101723:	8b 50 18             	mov    0x18(%eax),%edx
80101726:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101729:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010172c:	8b 45 08             	mov    0x8(%ebp),%eax
8010172f:	8d 50 1c             	lea    0x1c(%eax),%edx
80101732:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101735:	83 c0 0c             	add    $0xc,%eax
80101738:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
8010173f:	00 
80101740:	89 54 24 04          	mov    %edx,0x4(%esp)
80101744:	89 04 24             	mov    %eax,(%esp)
80101747:	e8 8d 3a 00 00       	call   801051d9 <memmove>
  log_write(bp);
8010174c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174f:	89 04 24             	mov    %eax,(%esp)
80101752:	e8 46 1b 00 00       	call   8010329d <log_write>
  brelse(bp);
80101757:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010175a:	89 04 24             	mov    %eax,(%esp)
8010175d:	e8 b5 ea ff ff       	call   80100217 <brelse>
}
80101762:	c9                   	leave  
80101763:	c3                   	ret    

80101764 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101764:	55                   	push   %ebp
80101765:	89 e5                	mov    %esp,%ebp
80101767:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010176a:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101771:	e8 41 37 00 00       	call   80104eb7 <acquire>

  // Is the inode already cached?
  empty = 0;
80101776:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010177d:	c7 45 f4 94 e8 10 80 	movl   $0x8010e894,-0xc(%ebp)
80101784:	eb 59                	jmp    801017df <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101786:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101789:	8b 40 08             	mov    0x8(%eax),%eax
8010178c:	85 c0                	test   %eax,%eax
8010178e:	7e 35                	jle    801017c5 <iget+0x61>
80101790:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101793:	8b 00                	mov    (%eax),%eax
80101795:	3b 45 08             	cmp    0x8(%ebp),%eax
80101798:	75 2b                	jne    801017c5 <iget+0x61>
8010179a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179d:	8b 40 04             	mov    0x4(%eax),%eax
801017a0:	3b 45 0c             	cmp    0xc(%ebp),%eax
801017a3:	75 20                	jne    801017c5 <iget+0x61>
      ip->ref++;
801017a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a8:	8b 40 08             	mov    0x8(%eax),%eax
801017ab:	8d 50 01             	lea    0x1(%eax),%edx
801017ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b1:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017b4:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801017bb:	e8 59 37 00 00       	call   80104f19 <release>
      return ip;
801017c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c3:	eb 6f                	jmp    80101834 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017c5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017c9:	75 10                	jne    801017db <iget+0x77>
801017cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017ce:	8b 40 08             	mov    0x8(%eax),%eax
801017d1:	85 c0                	test   %eax,%eax
801017d3:	75 06                	jne    801017db <iget+0x77>
      empty = ip;
801017d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017d8:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017db:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017df:	81 7d f4 34 f8 10 80 	cmpl   $0x8010f834,-0xc(%ebp)
801017e6:	72 9e                	jb     80101786 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017e8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017ec:	75 0c                	jne    801017fa <iget+0x96>
    panic("iget: no inodes");
801017ee:	c7 04 24 f3 86 10 80 	movl   $0x801086f3,(%esp)
801017f5:	e8 43 ed ff ff       	call   8010053d <panic>

  ip = empty;
801017fa:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017fd:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
80101800:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101803:	8b 55 08             	mov    0x8(%ebp),%edx
80101806:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
80101808:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010180b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010180e:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101811:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101814:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010181b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010181e:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101825:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
8010182c:	e8 e8 36 00 00       	call   80104f19 <release>

  return ip;
80101831:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101834:	c9                   	leave  
80101835:	c3                   	ret    

80101836 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101836:	55                   	push   %ebp
80101837:	89 e5                	mov    %esp,%ebp
80101839:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010183c:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101843:	e8 6f 36 00 00       	call   80104eb7 <acquire>
  ip->ref++;
80101848:	8b 45 08             	mov    0x8(%ebp),%eax
8010184b:	8b 40 08             	mov    0x8(%eax),%eax
8010184e:	8d 50 01             	lea    0x1(%eax),%edx
80101851:	8b 45 08             	mov    0x8(%ebp),%eax
80101854:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101857:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
8010185e:	e8 b6 36 00 00       	call   80104f19 <release>
  return ip;
80101863:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101866:	c9                   	leave  
80101867:	c3                   	ret    

80101868 <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
80101868:	55                   	push   %ebp
80101869:	89 e5                	mov    %esp,%ebp
8010186b:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
8010186e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101872:	74 0a                	je     8010187e <ilock+0x16>
80101874:	8b 45 08             	mov    0x8(%ebp),%eax
80101877:	8b 40 08             	mov    0x8(%eax),%eax
8010187a:	85 c0                	test   %eax,%eax
8010187c:	7f 0c                	jg     8010188a <ilock+0x22>
    panic("ilock");
8010187e:	c7 04 24 03 87 10 80 	movl   $0x80108703,(%esp)
80101885:	e8 b3 ec ff ff       	call   8010053d <panic>

  acquire(&icache.lock);
8010188a:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101891:	e8 21 36 00 00       	call   80104eb7 <acquire>
  while(ip->flags & I_BUSY)
80101896:	eb 13                	jmp    801018ab <ilock+0x43>
    sleep(ip, &icache.lock);
80101898:	c7 44 24 04 60 e8 10 	movl   $0x8010e860,0x4(%esp)
8010189f:	80 
801018a0:	8b 45 08             	mov    0x8(%ebp),%eax
801018a3:	89 04 24             	mov    %eax,(%esp)
801018a6:	e8 f5 31 00 00       	call   80104aa0 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 e0 01             	and    $0x1,%eax
801018b4:	84 c0                	test   %al,%al
801018b6:	75 e0                	jne    80101898 <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018b8:	8b 45 08             	mov    0x8(%ebp),%eax
801018bb:	8b 40 0c             	mov    0xc(%eax),%eax
801018be:	89 c2                	mov    %eax,%edx
801018c0:	83 ca 01             	or     $0x1,%edx
801018c3:	8b 45 08             	mov    0x8(%ebp),%eax
801018c6:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018c9:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801018d0:	e8 44 36 00 00       	call   80104f19 <release>

  if(!(ip->flags & I_VALID)){
801018d5:	8b 45 08             	mov    0x8(%ebp),%eax
801018d8:	8b 40 0c             	mov    0xc(%eax),%eax
801018db:	83 e0 02             	and    $0x2,%eax
801018de:	85 c0                	test   %eax,%eax
801018e0:	0f 85 ce 00 00 00    	jne    801019b4 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018e6:	8b 45 08             	mov    0x8(%ebp),%eax
801018e9:	8b 40 04             	mov    0x4(%eax),%eax
801018ec:	c1 e8 03             	shr    $0x3,%eax
801018ef:	8d 50 02             	lea    0x2(%eax),%edx
801018f2:	8b 45 08             	mov    0x8(%ebp),%eax
801018f5:	8b 00                	mov    (%eax),%eax
801018f7:	89 54 24 04          	mov    %edx,0x4(%esp)
801018fb:	89 04 24             	mov    %eax,(%esp)
801018fe:	e8 a3 e8 ff ff       	call   801001a6 <bread>
80101903:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101906:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101909:	8d 50 18             	lea    0x18(%eax),%edx
8010190c:	8b 45 08             	mov    0x8(%ebp),%eax
8010190f:	8b 40 04             	mov    0x4(%eax),%eax
80101912:	83 e0 07             	and    $0x7,%eax
80101915:	c1 e0 06             	shl    $0x6,%eax
80101918:	01 d0                	add    %edx,%eax
8010191a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 10             	movzwl (%eax),%edx
80101923:	8b 45 08             	mov    0x8(%ebp),%eax
80101926:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010192a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192d:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101931:	8b 45 08             	mov    0x8(%ebp),%eax
80101934:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
80101938:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193b:	0f b7 50 04          	movzwl 0x4(%eax),%edx
8010193f:	8b 45 08             	mov    0x8(%ebp),%eax
80101942:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101946:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101949:	0f b7 50 06          	movzwl 0x6(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101954:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101957:	8b 50 08             	mov    0x8(%eax),%edx
8010195a:	8b 45 08             	mov    0x8(%ebp),%eax
8010195d:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101960:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101963:	8d 50 0c             	lea    0xc(%eax),%edx
80101966:	8b 45 08             	mov    0x8(%ebp),%eax
80101969:	83 c0 1c             	add    $0x1c,%eax
8010196c:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101973:	00 
80101974:	89 54 24 04          	mov    %edx,0x4(%esp)
80101978:	89 04 24             	mov    %eax,(%esp)
8010197b:	e8 59 38 00 00       	call   801051d9 <memmove>
    brelse(bp);
80101980:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101983:	89 04 24             	mov    %eax,(%esp)
80101986:	e8 8c e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010198b:	8b 45 08             	mov    0x8(%ebp),%eax
8010198e:	8b 40 0c             	mov    0xc(%eax),%eax
80101991:	89 c2                	mov    %eax,%edx
80101993:	83 ca 02             	or     $0x2,%edx
80101996:	8b 45 08             	mov    0x8(%ebp),%eax
80101999:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010199c:	8b 45 08             	mov    0x8(%ebp),%eax
8010199f:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801019a3:	66 85 c0             	test   %ax,%ax
801019a6:	75 0c                	jne    801019b4 <ilock+0x14c>
      panic("ilock: no type");
801019a8:	c7 04 24 09 87 10 80 	movl   $0x80108709,(%esp)
801019af:	e8 89 eb ff ff       	call   8010053d <panic>
  }
}
801019b4:	c9                   	leave  
801019b5:	c3                   	ret    

801019b6 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019b6:	55                   	push   %ebp
801019b7:	89 e5                	mov    %esp,%ebp
801019b9:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019bc:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019c0:	74 17                	je     801019d9 <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 0c             	mov    0xc(%eax),%eax
801019c8:	83 e0 01             	and    $0x1,%eax
801019cb:	85 c0                	test   %eax,%eax
801019cd:	74 0a                	je     801019d9 <iunlock+0x23>
801019cf:	8b 45 08             	mov    0x8(%ebp),%eax
801019d2:	8b 40 08             	mov    0x8(%eax),%eax
801019d5:	85 c0                	test   %eax,%eax
801019d7:	7f 0c                	jg     801019e5 <iunlock+0x2f>
    panic("iunlock");
801019d9:	c7 04 24 18 87 10 80 	movl   $0x80108718,(%esp)
801019e0:	e8 58 eb ff ff       	call   8010053d <panic>
  acquire(&icache.lock);
801019e5:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801019ec:	e8 c6 34 00 00       	call   80104eb7 <acquire>
  ip->flags &= ~I_BUSY;
801019f1:	8b 45 08             	mov    0x8(%ebp),%eax
801019f4:	8b 40 0c             	mov    0xc(%eax),%eax
801019f7:	89 c2                	mov    %eax,%edx
801019f9:	83 e2 fe             	and    $0xfffffffe,%edx
801019fc:	8b 45 08             	mov    0x8(%ebp),%eax
801019ff:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
80101a02:	8b 45 08             	mov    0x8(%ebp),%eax
80101a05:	89 04 24             	mov    %eax,(%esp)
80101a08:	e8 a1 32 00 00       	call   80104cae <wakeup>
  release(&icache.lock);
80101a0d:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a14:	e8 00 35 00 00       	call   80104f19 <release>
}
80101a19:	c9                   	leave  
80101a1a:	c3                   	ret    

80101a1b <iput>:
// be recycled.
// If that was the last reference and the inode has no links
// to it, free the inode (and its content) on disk.
void
iput(struct inode *ip)
{
80101a1b:	55                   	push   %ebp
80101a1c:	89 e5                	mov    %esp,%ebp
80101a1e:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a21:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a28:	e8 8a 34 00 00       	call   80104eb7 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a30:	8b 40 08             	mov    0x8(%eax),%eax
80101a33:	83 f8 01             	cmp    $0x1,%eax
80101a36:	0f 85 93 00 00 00    	jne    80101acf <iput+0xb4>
80101a3c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a3f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a42:	83 e0 02             	and    $0x2,%eax
80101a45:	85 c0                	test   %eax,%eax
80101a47:	0f 84 82 00 00 00    	je     80101acf <iput+0xb4>
80101a4d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a50:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a54:	66 85 c0             	test   %ax,%ax
80101a57:	75 76                	jne    80101acf <iput+0xb4>
    // inode has no links: truncate and free inode.
    if(ip->flags & I_BUSY)
80101a59:	8b 45 08             	mov    0x8(%ebp),%eax
80101a5c:	8b 40 0c             	mov    0xc(%eax),%eax
80101a5f:	83 e0 01             	and    $0x1,%eax
80101a62:	84 c0                	test   %al,%al
80101a64:	74 0c                	je     80101a72 <iput+0x57>
      panic("iput busy");
80101a66:	c7 04 24 20 87 10 80 	movl   $0x80108720,(%esp)
80101a6d:	e8 cb ea ff ff       	call   8010053d <panic>
    ip->flags |= I_BUSY;
80101a72:	8b 45 08             	mov    0x8(%ebp),%eax
80101a75:	8b 40 0c             	mov    0xc(%eax),%eax
80101a78:	89 c2                	mov    %eax,%edx
80101a7a:	83 ca 01             	or     $0x1,%edx
80101a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a80:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a83:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a8a:	e8 8a 34 00 00       	call   80104f19 <release>
    itrunc(ip);
80101a8f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a92:	89 04 24             	mov    %eax,(%esp)
80101a95:	e8 72 01 00 00       	call   80101c0c <itrunc>
    ip->type = 0;
80101a9a:	8b 45 08             	mov    0x8(%ebp),%eax
80101a9d:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101aa3:	8b 45 08             	mov    0x8(%ebp),%eax
80101aa6:	89 04 24             	mov    %eax,(%esp)
80101aa9:	e8 fe fb ff ff       	call   801016ac <iupdate>
    acquire(&icache.lock);
80101aae:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101ab5:	e8 fd 33 00 00       	call   80104eb7 <acquire>
    ip->flags = 0;
80101aba:	8b 45 08             	mov    0x8(%ebp),%eax
80101abd:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ac4:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac7:	89 04 24             	mov    %eax,(%esp)
80101aca:	e8 df 31 00 00       	call   80104cae <wakeup>
  }
  ip->ref--;
80101acf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ad2:	8b 40 08             	mov    0x8(%eax),%eax
80101ad5:	8d 50 ff             	lea    -0x1(%eax),%edx
80101ad8:	8b 45 08             	mov    0x8(%ebp),%eax
80101adb:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ade:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101ae5:	e8 2f 34 00 00       	call   80104f19 <release>
}
80101aea:	c9                   	leave  
80101aeb:	c3                   	ret    

80101aec <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101aec:	55                   	push   %ebp
80101aed:	89 e5                	mov    %esp,%ebp
80101aef:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101af2:	8b 45 08             	mov    0x8(%ebp),%eax
80101af5:	89 04 24             	mov    %eax,(%esp)
80101af8:	e8 b9 fe ff ff       	call   801019b6 <iunlock>
  iput(ip);
80101afd:	8b 45 08             	mov    0x8(%ebp),%eax
80101b00:	89 04 24             	mov    %eax,(%esp)
80101b03:	e8 13 ff ff ff       	call   80101a1b <iput>
}
80101b08:	c9                   	leave  
80101b09:	c3                   	ret    

80101b0a <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101b0a:	55                   	push   %ebp
80101b0b:	89 e5                	mov    %esp,%ebp
80101b0d:	53                   	push   %ebx
80101b0e:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b11:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b15:	77 3e                	ja     80101b55 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b17:	8b 45 08             	mov    0x8(%ebp),%eax
80101b1a:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b1d:	83 c2 04             	add    $0x4,%edx
80101b20:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b24:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b27:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b2b:	75 20                	jne    80101b4d <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b30:	8b 00                	mov    (%eax),%eax
80101b32:	89 04 24             	mov    %eax,(%esp)
80101b35:	e8 49 f8 ff ff       	call   80101383 <balloc>
80101b3a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b40:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b43:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b46:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b49:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b50:	e9 b1 00 00 00       	jmp    80101c06 <bmap+0xfc>
  }
  bn -= NDIRECT;
80101b55:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b59:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b5d:	0f 87 97 00 00 00    	ja     80101bfa <bmap+0xf0>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b63:	8b 45 08             	mov    0x8(%ebp),%eax
80101b66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b69:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b6c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b70:	75 19                	jne    80101b8b <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b72:	8b 45 08             	mov    0x8(%ebp),%eax
80101b75:	8b 00                	mov    (%eax),%eax
80101b77:	89 04 24             	mov    %eax,(%esp)
80101b7a:	e8 04 f8 ff ff       	call   80101383 <balloc>
80101b7f:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b82:	8b 45 08             	mov    0x8(%ebp),%eax
80101b85:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b88:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101b8e:	8b 00                	mov    (%eax),%eax
80101b90:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b93:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b97:	89 04 24             	mov    %eax,(%esp)
80101b9a:	e8 07 e6 ff ff       	call   801001a6 <bread>
80101b9f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101ba2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ba5:	83 c0 18             	add    $0x18,%eax
80101ba8:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101bab:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bae:	c1 e0 02             	shl    $0x2,%eax
80101bb1:	03 45 ec             	add    -0x14(%ebp),%eax
80101bb4:	8b 00                	mov    (%eax),%eax
80101bb6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bbd:	75 2b                	jne    80101bea <bmap+0xe0>
      a[bn] = addr = balloc(ip->dev);
80101bbf:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bc2:	c1 e0 02             	shl    $0x2,%eax
80101bc5:	89 c3                	mov    %eax,%ebx
80101bc7:	03 5d ec             	add    -0x14(%ebp),%ebx
80101bca:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcd:	8b 00                	mov    (%eax),%eax
80101bcf:	89 04 24             	mov    %eax,(%esp)
80101bd2:	e8 ac f7 ff ff       	call   80101383 <balloc>
80101bd7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bda:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdd:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be2:	89 04 24             	mov    %eax,(%esp)
80101be5:	e8 b3 16 00 00       	call   8010329d <log_write>
    }
    brelse(bp);
80101bea:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101bed:	89 04 24             	mov    %eax,(%esp)
80101bf0:	e8 22 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf8:	eb 0c                	jmp    80101c06 <bmap+0xfc>
  }

  panic("bmap: out of range");
80101bfa:	c7 04 24 2a 87 10 80 	movl   $0x8010872a,(%esp)
80101c01:	e8 37 e9 ff ff       	call   8010053d <panic>
}
80101c06:	83 c4 24             	add    $0x24,%esp
80101c09:	5b                   	pop    %ebx
80101c0a:	5d                   	pop    %ebp
80101c0b:	c3                   	ret    

80101c0c <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0c:	55                   	push   %ebp
80101c0d:	89 e5                	mov    %esp,%ebp
80101c0f:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c19:	eb 44                	jmp    80101c5f <itrunc+0x53>
    if(ip->addrs[i]){
80101c1b:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c21:	83 c2 04             	add    $0x4,%edx
80101c24:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c28:	85 c0                	test   %eax,%eax
80101c2a:	74 2f                	je     80101c5b <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2c:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c32:	83 c2 04             	add    $0x4,%edx
80101c35:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c39:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3c:	8b 00                	mov    (%eax),%eax
80101c3e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c42:	89 04 24             	mov    %eax,(%esp)
80101c45:	e8 90 f8 ff ff       	call   801014da <bfree>
      ip->addrs[i] = 0;
80101c4a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c50:	83 c2 04             	add    $0x4,%edx
80101c53:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c5a:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c5b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5f:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c63:	7e b6                	jle    80101c1b <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c65:	8b 45 08             	mov    0x8(%ebp),%eax
80101c68:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c6b:	85 c0                	test   %eax,%eax
80101c6d:	0f 84 8f 00 00 00    	je     80101d02 <itrunc+0xf6>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c73:	8b 45 08             	mov    0x8(%ebp),%eax
80101c76:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c79:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7c:	8b 00                	mov    (%eax),%eax
80101c7e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c82:	89 04 24             	mov    %eax,(%esp)
80101c85:	e8 1c e5 ff ff       	call   801001a6 <bread>
80101c8a:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c90:	83 c0 18             	add    $0x18,%eax
80101c93:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c96:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9d:	eb 2f                	jmp    80101cce <itrunc+0xc2>
      if(a[j])
80101c9f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca2:	c1 e0 02             	shl    $0x2,%eax
80101ca5:	03 45 e8             	add    -0x18(%ebp),%eax
80101ca8:	8b 00                	mov    (%eax),%eax
80101caa:	85 c0                	test   %eax,%eax
80101cac:	74 1c                	je     80101cca <itrunc+0xbe>
        bfree(ip->dev, a[j]);
80101cae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb1:	c1 e0 02             	shl    $0x2,%eax
80101cb4:	03 45 e8             	add    -0x18(%ebp),%eax
80101cb7:	8b 10                	mov    (%eax),%edx
80101cb9:	8b 45 08             	mov    0x8(%ebp),%eax
80101cbc:	8b 00                	mov    (%eax),%eax
80101cbe:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cc2:	89 04 24             	mov    %eax,(%esp)
80101cc5:	e8 10 f8 ff ff       	call   801014da <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cca:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cce:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cd1:	83 f8 7f             	cmp    $0x7f,%eax
80101cd4:	76 c9                	jbe    80101c9f <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101cd6:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101cd9:	89 04 24             	mov    %eax,(%esp)
80101cdc:	e8 36 e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ce1:	8b 45 08             	mov    0x8(%ebp),%eax
80101ce4:	8b 50 4c             	mov    0x4c(%eax),%edx
80101ce7:	8b 45 08             	mov    0x8(%ebp),%eax
80101cea:	8b 00                	mov    (%eax),%eax
80101cec:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cf0:	89 04 24             	mov    %eax,(%esp)
80101cf3:	e8 e2 f7 ff ff       	call   801014da <bfree>
    ip->addrs[NDIRECT] = 0;
80101cf8:	8b 45 08             	mov    0x8(%ebp),%eax
80101cfb:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	89 04 24             	mov    %eax,(%esp)
80101d12:	e8 95 f9 ff ff       	call   801016ac <iupdate>
}
80101d17:	c9                   	leave  
80101d18:	c3                   	ret    

80101d19 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d19:	55                   	push   %ebp
80101d1a:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d1c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d1f:	8b 00                	mov    (%eax),%eax
80101d21:	89 c2                	mov    %eax,%edx
80101d23:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d26:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d29:	8b 45 08             	mov    0x8(%ebp),%eax
80101d2c:	8b 50 04             	mov    0x4(%eax),%edx
80101d2f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d32:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d35:	8b 45 08             	mov    0x8(%ebp),%eax
80101d38:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d3c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3f:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d42:	8b 45 08             	mov    0x8(%ebp),%eax
80101d45:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d49:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d4c:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d50:	8b 45 08             	mov    0x8(%ebp),%eax
80101d53:	8b 50 18             	mov    0x18(%eax),%edx
80101d56:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d59:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d5c:	5d                   	pop    %ebp
80101d5d:	c3                   	ret    

80101d5e <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d5e:	55                   	push   %ebp
80101d5f:	89 e5                	mov    %esp,%ebp
80101d61:	53                   	push   %ebx
80101d62:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d65:	8b 45 08             	mov    0x8(%ebp),%eax
80101d68:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d6c:	66 83 f8 03          	cmp    $0x3,%ax
80101d70:	75 60                	jne    80101dd2 <readi+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d72:	8b 45 08             	mov    0x8(%ebp),%eax
80101d75:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d79:	66 85 c0             	test   %ax,%ax
80101d7c:	78 20                	js     80101d9e <readi+0x40>
80101d7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d81:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d85:	66 83 f8 09          	cmp    $0x9,%ax
80101d89:	7f 13                	jg     80101d9e <readi+0x40>
80101d8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d92:	98                   	cwtl   
80101d93:	8b 04 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%eax
80101d9a:	85 c0                	test   %eax,%eax
80101d9c:	75 0a                	jne    80101da8 <readi+0x4a>
      return -1;
80101d9e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101da3:	e9 1b 01 00 00       	jmp    80101ec3 <readi+0x165>
    return devsw[ip->major].read(ip, dst, n);
80101da8:	8b 45 08             	mov    0x8(%ebp),%eax
80101dab:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101daf:	98                   	cwtl   
80101db0:	8b 14 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%edx
80101db7:	8b 45 14             	mov    0x14(%ebp),%eax
80101dba:	89 44 24 08          	mov    %eax,0x8(%esp)
80101dbe:	8b 45 0c             	mov    0xc(%ebp),%eax
80101dc1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101dc5:	8b 45 08             	mov    0x8(%ebp),%eax
80101dc8:	89 04 24             	mov    %eax,(%esp)
80101dcb:	ff d2                	call   *%edx
80101dcd:	e9 f1 00 00 00       	jmp    80101ec3 <readi+0x165>
  }

  if(off > ip->size || off + n < off)
80101dd2:	8b 45 08             	mov    0x8(%ebp),%eax
80101dd5:	8b 40 18             	mov    0x18(%eax),%eax
80101dd8:	3b 45 10             	cmp    0x10(%ebp),%eax
80101ddb:	72 0d                	jb     80101dea <readi+0x8c>
80101ddd:	8b 45 14             	mov    0x14(%ebp),%eax
80101de0:	8b 55 10             	mov    0x10(%ebp),%edx
80101de3:	01 d0                	add    %edx,%eax
80101de5:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de8:	73 0a                	jae    80101df4 <readi+0x96>
    return -1;
80101dea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101def:	e9 cf 00 00 00       	jmp    80101ec3 <readi+0x165>
  if(off + n > ip->size)
80101df4:	8b 45 14             	mov    0x14(%ebp),%eax
80101df7:	8b 55 10             	mov    0x10(%ebp),%edx
80101dfa:	01 c2                	add    %eax,%edx
80101dfc:	8b 45 08             	mov    0x8(%ebp),%eax
80101dff:	8b 40 18             	mov    0x18(%eax),%eax
80101e02:	39 c2                	cmp    %eax,%edx
80101e04:	76 0c                	jbe    80101e12 <readi+0xb4>
    n = ip->size - off;
80101e06:	8b 45 08             	mov    0x8(%ebp),%eax
80101e09:	8b 40 18             	mov    0x18(%eax),%eax
80101e0c:	2b 45 10             	sub    0x10(%ebp),%eax
80101e0f:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e19:	e9 96 00 00 00       	jmp    80101eb4 <readi+0x156>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e1e:	8b 45 10             	mov    0x10(%ebp),%eax
80101e21:	c1 e8 09             	shr    $0x9,%eax
80101e24:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e28:	8b 45 08             	mov    0x8(%ebp),%eax
80101e2b:	89 04 24             	mov    %eax,(%esp)
80101e2e:	e8 d7 fc ff ff       	call   80101b0a <bmap>
80101e33:	8b 55 08             	mov    0x8(%ebp),%edx
80101e36:	8b 12                	mov    (%edx),%edx
80101e38:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e3c:	89 14 24             	mov    %edx,(%esp)
80101e3f:	e8 62 e3 ff ff       	call   801001a6 <bread>
80101e44:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e47:	8b 45 10             	mov    0x10(%ebp),%eax
80101e4a:	89 c2                	mov    %eax,%edx
80101e4c:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101e52:	b8 00 02 00 00       	mov    $0x200,%eax
80101e57:	89 c1                	mov    %eax,%ecx
80101e59:	29 d1                	sub    %edx,%ecx
80101e5b:	89 ca                	mov    %ecx,%edx
80101e5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e60:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e63:	89 cb                	mov    %ecx,%ebx
80101e65:	29 c3                	sub    %eax,%ebx
80101e67:	89 d8                	mov    %ebx,%eax
80101e69:	39 c2                	cmp    %eax,%edx
80101e6b:	0f 46 c2             	cmovbe %edx,%eax
80101e6e:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e71:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e74:	8d 50 18             	lea    0x18(%eax),%edx
80101e77:	8b 45 10             	mov    0x10(%ebp),%eax
80101e7a:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7f:	01 c2                	add    %eax,%edx
80101e81:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e84:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e88:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e8c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e8f:	89 04 24             	mov    %eax,(%esp)
80101e92:	e8 42 33 00 00       	call   801051d9 <memmove>
    brelse(bp);
80101e97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e9a:	89 04 24             	mov    %eax,(%esp)
80101e9d:	e8 75 e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ea5:	01 45 f4             	add    %eax,-0xc(%ebp)
80101ea8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eab:	01 45 10             	add    %eax,0x10(%ebp)
80101eae:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb1:	01 45 0c             	add    %eax,0xc(%ebp)
80101eb4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101eb7:	3b 45 14             	cmp    0x14(%ebp),%eax
80101eba:	0f 82 5e ff ff ff    	jb     80101e1e <readi+0xc0>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec0:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101ec3:	83 c4 24             	add    $0x24,%esp
80101ec6:	5b                   	pop    %ebx
80101ec7:	5d                   	pop    %ebp
80101ec8:	c3                   	ret    

80101ec9 <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ec9:	55                   	push   %ebp
80101eca:	89 e5                	mov    %esp,%ebp
80101ecc:	53                   	push   %ebx
80101ecd:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed0:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed3:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed7:	66 83 f8 03          	cmp    $0x3,%ax
80101edb:	75 60                	jne    80101f3d <writei+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edd:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee0:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee4:	66 85 c0             	test   %ax,%ax
80101ee7:	78 20                	js     80101f09 <writei+0x40>
80101ee9:	8b 45 08             	mov    0x8(%ebp),%eax
80101eec:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef0:	66 83 f8 09          	cmp    $0x9,%ax
80101ef4:	7f 13                	jg     80101f09 <writei+0x40>
80101ef6:	8b 45 08             	mov    0x8(%ebp),%eax
80101ef9:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101efd:	98                   	cwtl   
80101efe:	8b 04 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%eax
80101f05:	85 c0                	test   %eax,%eax
80101f07:	75 0a                	jne    80101f13 <writei+0x4a>
      return -1;
80101f09:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f0e:	e9 46 01 00 00       	jmp    80102059 <writei+0x190>
    return devsw[ip->major].write(ip, src, n);
80101f13:	8b 45 08             	mov    0x8(%ebp),%eax
80101f16:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1a:	98                   	cwtl   
80101f1b:	8b 14 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%edx
80101f22:	8b 45 14             	mov    0x14(%ebp),%eax
80101f25:	89 44 24 08          	mov    %eax,0x8(%esp)
80101f29:	8b 45 0c             	mov    0xc(%ebp),%eax
80101f2c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f30:	8b 45 08             	mov    0x8(%ebp),%eax
80101f33:	89 04 24             	mov    %eax,(%esp)
80101f36:	ff d2                	call   *%edx
80101f38:	e9 1c 01 00 00       	jmp    80102059 <writei+0x190>
  }

  if(off > ip->size || off + n < off)
80101f3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101f40:	8b 40 18             	mov    0x18(%eax),%eax
80101f43:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f46:	72 0d                	jb     80101f55 <writei+0x8c>
80101f48:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4b:	8b 55 10             	mov    0x10(%ebp),%edx
80101f4e:	01 d0                	add    %edx,%eax
80101f50:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f53:	73 0a                	jae    80101f5f <writei+0x96>
    return -1;
80101f55:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5a:	e9 fa 00 00 00       	jmp    80102059 <writei+0x190>
  if(off + n > MAXFILE*BSIZE)
80101f5f:	8b 45 14             	mov    0x14(%ebp),%eax
80101f62:	8b 55 10             	mov    0x10(%ebp),%edx
80101f65:	01 d0                	add    %edx,%eax
80101f67:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6c:	76 0a                	jbe    80101f78 <writei+0xaf>
    return -1;
80101f6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f73:	e9 e1 00 00 00       	jmp    80102059 <writei+0x190>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f78:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f7f:	e9 a1 00 00 00       	jmp    80102025 <writei+0x15c>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f84:	8b 45 10             	mov    0x10(%ebp),%eax
80101f87:	c1 e8 09             	shr    $0x9,%eax
80101f8a:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80101f91:	89 04 24             	mov    %eax,(%esp)
80101f94:	e8 71 fb ff ff       	call   80101b0a <bmap>
80101f99:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9c:	8b 12                	mov    (%edx),%edx
80101f9e:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa2:	89 14 24             	mov    %edx,(%esp)
80101fa5:	e8 fc e1 ff ff       	call   801001a6 <bread>
80101faa:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fad:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb0:	89 c2                	mov    %eax,%edx
80101fb2:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101fb8:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbd:	89 c1                	mov    %eax,%ecx
80101fbf:	29 d1                	sub    %edx,%ecx
80101fc1:	89 ca                	mov    %ecx,%edx
80101fc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc6:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc9:	89 cb                	mov    %ecx,%ebx
80101fcb:	29 c3                	sub    %eax,%ebx
80101fcd:	89 d8                	mov    %ebx,%eax
80101fcf:	39 c2                	cmp    %eax,%edx
80101fd1:	0f 46 c2             	cmovbe %edx,%eax
80101fd4:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fda:	8d 50 18             	lea    0x18(%eax),%edx
80101fdd:	8b 45 10             	mov    0x10(%ebp),%eax
80101fe0:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fe5:	01 c2                	add    %eax,%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 dc 31 00 00       	call   801051d9 <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 95 12 00 00       	call   8010329d <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 53 ff ff ff    	jb     80101f84 <writei+0xbb>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18d>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18d>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 56 f6 ff ff       	call   801016ac <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	83 c4 24             	add    $0x24,%esp
8010205c:	5b                   	pop    %ebx
8010205d:	5d                   	pop    %ebp
8010205e:	c3                   	ret    

8010205f <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205f:	55                   	push   %ebp
80102060:	89 e5                	mov    %esp,%ebp
80102062:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102065:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
8010206c:	00 
8010206d:	8b 45 0c             	mov    0xc(%ebp),%eax
80102070:	89 44 24 04          	mov    %eax,0x4(%esp)
80102074:	8b 45 08             	mov    0x8(%ebp),%eax
80102077:	89 04 24             	mov    %eax,(%esp)
8010207a:	e8 fe 31 00 00       	call   8010527d <strncmp>
}
8010207f:	c9                   	leave  
80102080:	c3                   	ret    

80102081 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80102081:	55                   	push   %ebp
80102082:	89 e5                	mov    %esp,%ebp
80102084:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102087:	8b 45 08             	mov    0x8(%ebp),%eax
8010208a:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208e:	66 83 f8 01          	cmp    $0x1,%ax
80102092:	74 0c                	je     801020a0 <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102094:	c7 04 24 3d 87 10 80 	movl   $0x8010873d,(%esp)
8010209b:	e8 9d e4 ff ff       	call   8010053d <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
801020a0:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a7:	e9 87 00 00 00       	jmp    80102133 <dirlookup+0xb2>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020ac:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020b3:	00 
801020b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b7:	89 44 24 08          	mov    %eax,0x8(%esp)
801020bb:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020be:	89 44 24 04          	mov    %eax,0x4(%esp)
801020c2:	8b 45 08             	mov    0x8(%ebp),%eax
801020c5:	89 04 24             	mov    %eax,(%esp)
801020c8:	e8 91 fc ff ff       	call   80101d5e <readi>
801020cd:	83 f8 10             	cmp    $0x10,%eax
801020d0:	74 0c                	je     801020de <dirlookup+0x5d>
      panic("dirlink read");
801020d2:	c7 04 24 4f 87 10 80 	movl   $0x8010874f,(%esp)
801020d9:	e8 5f e4 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801020de:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020e2:	66 85 c0             	test   %ax,%ax
801020e5:	74 47                	je     8010212e <dirlookup+0xad>
      continue;
    if(namecmp(name, de.name) == 0){
801020e7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ea:	83 c0 02             	add    $0x2,%eax
801020ed:	89 44 24 04          	mov    %eax,0x4(%esp)
801020f1:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f4:	89 04 24             	mov    %eax,(%esp)
801020f7:	e8 63 ff ff ff       	call   8010205f <namecmp>
801020fc:	85 c0                	test   %eax,%eax
801020fe:	75 2f                	jne    8010212f <dirlookup+0xae>
      // entry matches path element
      if(poff)
80102100:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102104:	74 08                	je     8010210e <dirlookup+0x8d>
        *poff = off;
80102106:	8b 45 10             	mov    0x10(%ebp),%eax
80102109:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210c:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210e:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102112:	0f b7 c0             	movzwl %ax,%eax
80102115:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102118:	8b 45 08             	mov    0x8(%ebp),%eax
8010211b:	8b 00                	mov    (%eax),%eax
8010211d:	8b 55 f0             	mov    -0x10(%ebp),%edx
80102120:	89 54 24 04          	mov    %edx,0x4(%esp)
80102124:	89 04 24             	mov    %eax,(%esp)
80102127:	e8 38 f6 ff ff       	call   80101764 <iget>
8010212c:	eb 19                	jmp    80102147 <dirlookup+0xc6>

  for(off = 0; off < dp->size; off += sizeof(de)){
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      continue;
8010212e:	90                   	nop
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212f:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102133:	8b 45 08             	mov    0x8(%ebp),%eax
80102136:	8b 40 18             	mov    0x18(%eax),%eax
80102139:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010213c:	0f 87 6a ff ff ff    	ja     801020ac <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
80102142:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102147:	c9                   	leave  
80102148:	c3                   	ret    

80102149 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102149:	55                   	push   %ebp
8010214a:	89 e5                	mov    %esp,%ebp
8010214c:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214f:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102156:	00 
80102157:	8b 45 0c             	mov    0xc(%ebp),%eax
8010215a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215e:	8b 45 08             	mov    0x8(%ebp),%eax
80102161:	89 04 24             	mov    %eax,(%esp)
80102164:	e8 18 ff ff ff       	call   80102081 <dirlookup>
80102169:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010216c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80102170:	74 15                	je     80102187 <dirlink+0x3e>
    iput(ip);
80102172:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102175:	89 04 24             	mov    %eax,(%esp)
80102178:	e8 9e f8 ff ff       	call   80101a1b <iput>
    return -1;
8010217d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102182:	e9 b8 00 00 00       	jmp    8010223f <dirlink+0xf6>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102187:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218e:	eb 44                	jmp    801021d4 <dirlink+0x8b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102193:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010219a:	00 
8010219b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219f:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021a2:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a6:	8b 45 08             	mov    0x8(%ebp),%eax
801021a9:	89 04 24             	mov    %eax,(%esp)
801021ac:	e8 ad fb ff ff       	call   80101d5e <readi>
801021b1:	83 f8 10             	cmp    $0x10,%eax
801021b4:	74 0c                	je     801021c2 <dirlink+0x79>
      panic("dirlink read");
801021b6:	c7 04 24 4f 87 10 80 	movl   $0x8010874f,(%esp)
801021bd:	e8 7b e3 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801021c2:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c6:	66 85 c0             	test   %ax,%ax
801021c9:	74 18                	je     801021e3 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021ce:	83 c0 10             	add    $0x10,%eax
801021d1:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d4:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d7:	8b 45 08             	mov    0x8(%ebp),%eax
801021da:	8b 40 18             	mov    0x18(%eax),%eax
801021dd:	39 c2                	cmp    %eax,%edx
801021df:	72 af                	jb     80102190 <dirlink+0x47>
801021e1:	eb 01                	jmp    801021e4 <dirlink+0x9b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      break;
801021e3:	90                   	nop
  }

  strncpy(de.name, name, DIRSIZ);
801021e4:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021eb:	00 
801021ec:	8b 45 0c             	mov    0xc(%ebp),%eax
801021ef:	89 44 24 04          	mov    %eax,0x4(%esp)
801021f3:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f6:	83 c0 02             	add    $0x2,%eax
801021f9:	89 04 24             	mov    %eax,(%esp)
801021fc:	e8 d4 30 00 00       	call   801052d5 <strncpy>
  de.inum = inum;
80102201:	8b 45 10             	mov    0x10(%ebp),%eax
80102204:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102208:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010220b:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102212:	00 
80102213:	89 44 24 08          	mov    %eax,0x8(%esp)
80102217:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010221a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221e:	8b 45 08             	mov    0x8(%ebp),%eax
80102221:	89 04 24             	mov    %eax,(%esp)
80102224:	e8 a0 fc ff ff       	call   80101ec9 <writei>
80102229:	83 f8 10             	cmp    $0x10,%eax
8010222c:	74 0c                	je     8010223a <dirlink+0xf1>
    panic("dirlink");
8010222e:	c7 04 24 5c 87 10 80 	movl   $0x8010875c,(%esp)
80102235:	e8 03 e3 ff ff       	call   8010053d <panic>
  
  return 0;
8010223a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223f:	c9                   	leave  
80102240:	c3                   	ret    

80102241 <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
80102241:	55                   	push   %ebp
80102242:	89 e5                	mov    %esp,%ebp
80102244:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102247:	eb 04                	jmp    8010224d <skipelem+0xc>
    path++;
80102249:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
8010224d:	8b 45 08             	mov    0x8(%ebp),%eax
80102250:	0f b6 00             	movzbl (%eax),%eax
80102253:	3c 2f                	cmp    $0x2f,%al
80102255:	74 f2                	je     80102249 <skipelem+0x8>
    path++;
  if(*path == 0)
80102257:	8b 45 08             	mov    0x8(%ebp),%eax
8010225a:	0f b6 00             	movzbl (%eax),%eax
8010225d:	84 c0                	test   %al,%al
8010225f:	75 0a                	jne    8010226b <skipelem+0x2a>
    return 0;
80102261:	b8 00 00 00 00       	mov    $0x0,%eax
80102266:	e9 86 00 00 00       	jmp    801022f1 <skipelem+0xb0>
  s = path;
8010226b:	8b 45 08             	mov    0x8(%ebp),%eax
8010226e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
80102271:	eb 04                	jmp    80102277 <skipelem+0x36>
    path++;
80102273:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102277:	8b 45 08             	mov    0x8(%ebp),%eax
8010227a:	0f b6 00             	movzbl (%eax),%eax
8010227d:	3c 2f                	cmp    $0x2f,%al
8010227f:	74 0a                	je     8010228b <skipelem+0x4a>
80102281:	8b 45 08             	mov    0x8(%ebp),%eax
80102284:	0f b6 00             	movzbl (%eax),%eax
80102287:	84 c0                	test   %al,%al
80102289:	75 e8                	jne    80102273 <skipelem+0x32>
    path++;
  len = path - s;
8010228b:	8b 55 08             	mov    0x8(%ebp),%edx
8010228e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102291:	89 d1                	mov    %edx,%ecx
80102293:	29 c1                	sub    %eax,%ecx
80102295:	89 c8                	mov    %ecx,%eax
80102297:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
8010229a:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
8010229e:	7e 1c                	jle    801022bc <skipelem+0x7b>
    memmove(name, s, DIRSIZ);
801022a0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a7:	00 
801022a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022ab:	89 44 24 04          	mov    %eax,0x4(%esp)
801022af:	8b 45 0c             	mov    0xc(%ebp),%eax
801022b2:	89 04 24             	mov    %eax,(%esp)
801022b5:	e8 1f 2f 00 00       	call   801051d9 <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022ba:	eb 28                	jmp    801022e4 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022bc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022bf:	89 44 24 08          	mov    %eax,0x8(%esp)
801022c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c6:	89 44 24 04          	mov    %eax,0x4(%esp)
801022ca:	8b 45 0c             	mov    0xc(%ebp),%eax
801022cd:	89 04 24             	mov    %eax,(%esp)
801022d0:	e8 04 2f 00 00       	call   801051d9 <memmove>
    name[len] = 0;
801022d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022d8:	03 45 0c             	add    0xc(%ebp),%eax
801022db:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022de:	eb 04                	jmp    801022e4 <skipelem+0xa3>
    path++;
801022e0:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e4:	8b 45 08             	mov    0x8(%ebp),%eax
801022e7:	0f b6 00             	movzbl (%eax),%eax
801022ea:	3c 2f                	cmp    $0x2f,%al
801022ec:	74 f2                	je     801022e0 <skipelem+0x9f>
    path++;
  return path;
801022ee:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022f1:	c9                   	leave  
801022f2:	c3                   	ret    

801022f3 <namex>:
// Look up and return the inode for a path name.
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022f3:	55                   	push   %ebp
801022f4:	89 e5                	mov    %esp,%ebp
801022f6:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;
  if(*path == '/')
801022f9:	8b 45 08             	mov    0x8(%ebp),%eax
801022fc:	0f b6 00             	movzbl (%eax),%eax
801022ff:	3c 2f                	cmp    $0x2f,%al
80102301:	75 1c                	jne    8010231f <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
80102303:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010230a:	00 
8010230b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102312:	e8 4d f4 ff ff       	call   80101764 <iget>
80102317:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
8010231a:	e9 af 00 00 00       	jmp    801023ce <namex+0xdb>
{
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102325:	8b 40 68             	mov    0x68(%eax),%eax
80102328:	89 04 24             	mov    %eax,(%esp)
8010232b:	e8 06 f5 ff ff       	call   80101836 <idup>
80102330:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while((path = skipelem(path, name)) != 0){
80102333:	e9 96 00 00 00       	jmp    801023ce <namex+0xdb>
    ilock(ip);
80102338:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010233b:	89 04 24             	mov    %eax,(%esp)
8010233e:	e8 25 f5 ff ff       	call   80101868 <ilock>
    if(ip->type != T_DIR){
80102343:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102346:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010234a:	66 83 f8 01          	cmp    $0x1,%ax
8010234e:	74 15                	je     80102365 <namex+0x72>
      iunlockput(ip);
80102350:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102353:	89 04 24             	mov    %eax,(%esp)
80102356:	e8 91 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
8010235b:	b8 00 00 00 00       	mov    $0x0,%eax
80102360:	e9 a3 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102365:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102369:	74 1d                	je     80102388 <namex+0x95>
8010236b:	8b 45 08             	mov    0x8(%ebp),%eax
8010236e:	0f b6 00             	movzbl (%eax),%eax
80102371:	84 c0                	test   %al,%al
80102373:	75 13                	jne    80102388 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102375:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102378:	89 04 24             	mov    %eax,(%esp)
8010237b:	e8 36 f6 ff ff       	call   801019b6 <iunlock>
      return ip;
80102380:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102383:	e9 80 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102388:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238f:	00 
80102390:	8b 45 10             	mov    0x10(%ebp),%eax
80102393:	89 44 24 04          	mov    %eax,0x4(%esp)
80102397:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010239a:	89 04 24             	mov    %eax,(%esp)
8010239d:	e8 df fc ff ff       	call   80102081 <dirlookup>
801023a2:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a9:	75 12                	jne    801023bd <namex+0xca>
      iunlockput(ip);
801023ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023ae:	89 04 24             	mov    %eax,(%esp)
801023b1:	e8 36 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
801023b6:	b8 00 00 00 00       	mov    $0x0,%eax
801023bb:	eb 4b                	jmp    80102408 <namex+0x115>
    }
    iunlockput(ip);
801023bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023c0:	89 04 24             	mov    %eax,(%esp)
801023c3:	e8 24 f7 ff ff       	call   80101aec <iunlockput>
    ip = next;
801023c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023cb:	89 45 f4             	mov    %eax,-0xc(%ebp)
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
801023ce:	8b 45 10             	mov    0x10(%ebp),%eax
801023d1:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d5:	8b 45 08             	mov    0x8(%ebp),%eax
801023d8:	89 04 24             	mov    %eax,(%esp)
801023db:	e8 61 fe ff ff       	call   80102241 <skipelem>
801023e0:	89 45 08             	mov    %eax,0x8(%ebp)
801023e3:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e7:	0f 85 4b ff ff ff    	jne    80102338 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023ed:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023f1:	74 12                	je     80102405 <namex+0x112>
    iput(ip);
801023f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f6:	89 04 24             	mov    %eax,(%esp)
801023f9:	e8 1d f6 ff ff       	call   80101a1b <iput>
    return 0;
801023fe:	b8 00 00 00 00       	mov    $0x0,%eax
80102403:	eb 03                	jmp    80102408 <namex+0x115>
  }
  return ip;
80102405:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102408:	c9                   	leave  
80102409:	c3                   	ret    

8010240a <namei>:

struct inode*
namei(char *path)
{
8010240a:	55                   	push   %ebp
8010240b:	89 e5                	mov    %esp,%ebp
8010240d:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102410:	8d 45 ea             	lea    -0x16(%ebp),%eax
80102413:	89 44 24 08          	mov    %eax,0x8(%esp)
80102417:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241e:	00 
8010241f:	8b 45 08             	mov    0x8(%ebp),%eax
80102422:	89 04 24             	mov    %eax,(%esp)
80102425:	e8 c9 fe ff ff       	call   801022f3 <namex>
}
8010242a:	c9                   	leave  
8010242b:	c3                   	ret    

8010242c <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
8010242c:	55                   	push   %ebp
8010242d:	89 e5                	mov    %esp,%ebp
8010242f:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
80102432:	8b 45 0c             	mov    0xc(%ebp),%eax
80102435:	89 44 24 08          	mov    %eax,0x8(%esp)
80102439:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102440:	00 
80102441:	8b 45 08             	mov    0x8(%ebp),%eax
80102444:	89 04 24             	mov    %eax,(%esp)
80102447:	e8 a7 fe ff ff       	call   801022f3 <namex>
}
8010244c:	c9                   	leave  
8010244d:	c3                   	ret    
	...

80102450 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102450:	55                   	push   %ebp
80102451:	89 e5                	mov    %esp,%ebp
80102453:	53                   	push   %ebx
80102454:	83 ec 14             	sub    $0x14,%esp
80102457:	8b 45 08             	mov    0x8(%ebp),%eax
8010245a:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010245e:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102462:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102466:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
8010246a:	ec                   	in     (%dx),%al
8010246b:	89 c3                	mov    %eax,%ebx
8010246d:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102470:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102474:	83 c4 14             	add    $0x14,%esp
80102477:	5b                   	pop    %ebx
80102478:	5d                   	pop    %ebp
80102479:	c3                   	ret    

8010247a <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
8010247a:	55                   	push   %ebp
8010247b:	89 e5                	mov    %esp,%ebp
8010247d:	57                   	push   %edi
8010247e:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010247f:	8b 55 08             	mov    0x8(%ebp),%edx
80102482:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102485:	8b 45 10             	mov    0x10(%ebp),%eax
80102488:	89 cb                	mov    %ecx,%ebx
8010248a:	89 df                	mov    %ebx,%edi
8010248c:	89 c1                	mov    %eax,%ecx
8010248e:	fc                   	cld    
8010248f:	f3 6d                	rep insl (%dx),%es:(%edi)
80102491:	89 c8                	mov    %ecx,%eax
80102493:	89 fb                	mov    %edi,%ebx
80102495:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102498:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
8010249b:	5b                   	pop    %ebx
8010249c:	5f                   	pop    %edi
8010249d:	5d                   	pop    %ebp
8010249e:	c3                   	ret    

8010249f <outb>:

static inline void
outb(ushort port, uchar data)
{
8010249f:	55                   	push   %ebp
801024a0:	89 e5                	mov    %esp,%ebp
801024a2:	83 ec 08             	sub    $0x8,%esp
801024a5:	8b 55 08             	mov    0x8(%ebp),%edx
801024a8:	8b 45 0c             	mov    0xc(%ebp),%eax
801024ab:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801024af:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801024b2:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024b6:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024ba:	ee                   	out    %al,(%dx)
}
801024bb:	c9                   	leave  
801024bc:	c3                   	ret    

801024bd <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024bd:	55                   	push   %ebp
801024be:	89 e5                	mov    %esp,%ebp
801024c0:	56                   	push   %esi
801024c1:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024c2:	8b 55 08             	mov    0x8(%ebp),%edx
801024c5:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024c8:	8b 45 10             	mov    0x10(%ebp),%eax
801024cb:	89 cb                	mov    %ecx,%ebx
801024cd:	89 de                	mov    %ebx,%esi
801024cf:	89 c1                	mov    %eax,%ecx
801024d1:	fc                   	cld    
801024d2:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024d4:	89 c8                	mov    %ecx,%eax
801024d6:	89 f3                	mov    %esi,%ebx
801024d8:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024db:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024de:	5b                   	pop    %ebx
801024df:	5e                   	pop    %esi
801024e0:	5d                   	pop    %ebp
801024e1:	c3                   	ret    

801024e2 <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024e2:	55                   	push   %ebp
801024e3:	89 e5                	mov    %esp,%ebp
801024e5:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024e8:	90                   	nop
801024e9:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024f0:	e8 5b ff ff ff       	call   80102450 <inb>
801024f5:	0f b6 c0             	movzbl %al,%eax
801024f8:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024fb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024fe:	25 c0 00 00 00       	and    $0xc0,%eax
80102503:	83 f8 40             	cmp    $0x40,%eax
80102506:	75 e1                	jne    801024e9 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
80102508:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010250c:	74 11                	je     8010251f <idewait+0x3d>
8010250e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102511:	83 e0 21             	and    $0x21,%eax
80102514:	85 c0                	test   %eax,%eax
80102516:	74 07                	je     8010251f <idewait+0x3d>
    return -1;
80102518:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010251d:	eb 05                	jmp    80102524 <idewait+0x42>
  return 0;
8010251f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102524:	c9                   	leave  
80102525:	c3                   	ret    

80102526 <ideinit>:

void
ideinit(void)
{
80102526:	55                   	push   %ebp
80102527:	89 e5                	mov    %esp,%ebp
80102529:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
8010252c:	c7 44 24 04 64 87 10 	movl   $0x80108764,0x4(%esp)
80102533:	80 
80102534:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
8010253b:	e8 56 29 00 00       	call   80104e96 <initlock>
  picenable(IRQ_IDE);
80102540:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102547:	e8 55 15 00 00       	call   80103aa1 <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
8010254c:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80102551:	83 e8 01             	sub    $0x1,%eax
80102554:	89 44 24 04          	mov    %eax,0x4(%esp)
80102558:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010255f:	e8 12 04 00 00       	call   80102976 <ioapicenable>
  idewait(0);
80102564:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010256b:	e8 72 ff ff ff       	call   801024e2 <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
80102570:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
80102577:	00 
80102578:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010257f:	e8 1b ff ff ff       	call   8010249f <outb>
  for(i=0; i<1000; i++){
80102584:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010258b:	eb 20                	jmp    801025ad <ideinit+0x87>
    if(inb(0x1f7) != 0){
8010258d:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102594:	e8 b7 fe ff ff       	call   80102450 <inb>
80102599:	84 c0                	test   %al,%al
8010259b:	74 0c                	je     801025a9 <ideinit+0x83>
      havedisk1 = 1;
8010259d:	c7 05 38 b6 10 80 01 	movl   $0x1,0x8010b638
801025a4:	00 00 00 
      break;
801025a7:	eb 0d                	jmp    801025b6 <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
801025a9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801025ad:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025b4:	7e d7                	jle    8010258d <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025b6:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025bd:	00 
801025be:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025c5:	e8 d5 fe ff ff       	call   8010249f <outb>
}
801025ca:	c9                   	leave  
801025cb:	c3                   	ret    

801025cc <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025cc:	55                   	push   %ebp
801025cd:	89 e5                	mov    %esp,%ebp
801025cf:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025d2:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025d6:	75 0c                	jne    801025e4 <idestart+0x18>
    panic("idestart");
801025d8:	c7 04 24 68 87 10 80 	movl   $0x80108768,(%esp)
801025df:	e8 59 df ff ff       	call   8010053d <panic>

  idewait(0);
801025e4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025eb:	e8 f2 fe ff ff       	call   801024e2 <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025f0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025f7:	00 
801025f8:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025ff:	e8 9b fe ff ff       	call   8010249f <outb>
  outb(0x1f2, 1);  // number of sectors
80102604:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010260b:	00 
8010260c:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102613:	e8 87 fe ff ff       	call   8010249f <outb>
  outb(0x1f3, b->sector & 0xff);
80102618:	8b 45 08             	mov    0x8(%ebp),%eax
8010261b:	8b 40 08             	mov    0x8(%eax),%eax
8010261e:	0f b6 c0             	movzbl %al,%eax
80102621:	89 44 24 04          	mov    %eax,0x4(%esp)
80102625:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
8010262c:	e8 6e fe ff ff       	call   8010249f <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
80102631:	8b 45 08             	mov    0x8(%ebp),%eax
80102634:	8b 40 08             	mov    0x8(%eax),%eax
80102637:	c1 e8 08             	shr    $0x8,%eax
8010263a:	0f b6 c0             	movzbl %al,%eax
8010263d:	89 44 24 04          	mov    %eax,0x4(%esp)
80102641:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102648:	e8 52 fe ff ff       	call   8010249f <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
8010264d:	8b 45 08             	mov    0x8(%ebp),%eax
80102650:	8b 40 08             	mov    0x8(%eax),%eax
80102653:	c1 e8 10             	shr    $0x10,%eax
80102656:	0f b6 c0             	movzbl %al,%eax
80102659:	89 44 24 04          	mov    %eax,0x4(%esp)
8010265d:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
80102664:	e8 36 fe ff ff       	call   8010249f <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102669:	8b 45 08             	mov    0x8(%ebp),%eax
8010266c:	8b 40 04             	mov    0x4(%eax),%eax
8010266f:	83 e0 01             	and    $0x1,%eax
80102672:	89 c2                	mov    %eax,%edx
80102674:	c1 e2 04             	shl    $0x4,%edx
80102677:	8b 45 08             	mov    0x8(%ebp),%eax
8010267a:	8b 40 08             	mov    0x8(%eax),%eax
8010267d:	c1 e8 18             	shr    $0x18,%eax
80102680:	83 e0 0f             	and    $0xf,%eax
80102683:	09 d0                	or     %edx,%eax
80102685:	83 c8 e0             	or     $0xffffffe0,%eax
80102688:	0f b6 c0             	movzbl %al,%eax
8010268b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010268f:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102696:	e8 04 fe ff ff       	call   8010249f <outb>
  if(b->flags & B_DIRTY){
8010269b:	8b 45 08             	mov    0x8(%ebp),%eax
8010269e:	8b 00                	mov    (%eax),%eax
801026a0:	83 e0 04             	and    $0x4,%eax
801026a3:	85 c0                	test   %eax,%eax
801026a5:	74 34                	je     801026db <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
801026a7:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
801026ae:	00 
801026af:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026b6:	e8 e4 fd ff ff       	call   8010249f <outb>
    outsl(0x1f0, b->data, 512/4);
801026bb:	8b 45 08             	mov    0x8(%ebp),%eax
801026be:	83 c0 18             	add    $0x18,%eax
801026c1:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026c8:	00 
801026c9:	89 44 24 04          	mov    %eax,0x4(%esp)
801026cd:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026d4:	e8 e4 fd ff ff       	call   801024bd <outsl>
801026d9:	eb 14                	jmp    801026ef <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026db:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026e2:	00 
801026e3:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026ea:	e8 b0 fd ff ff       	call   8010249f <outb>
  }
}
801026ef:	c9                   	leave  
801026f0:	c3                   	ret    

801026f1 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026f1:	55                   	push   %ebp
801026f2:	89 e5                	mov    %esp,%ebp
801026f4:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026f7:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801026fe:	e8 b4 27 00 00       	call   80104eb7 <acquire>
  if((b = idequeue) == 0){
80102703:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102708:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010270b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010270f:	75 11                	jne    80102722 <ideintr+0x31>
    release(&idelock);
80102711:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102718:	e8 fc 27 00 00       	call   80104f19 <release>
    // cprintf("spurious IDE interrupt\n");
    return;
8010271d:	e9 90 00 00 00       	jmp    801027b2 <ideintr+0xc1>
  }
  idequeue = b->qnext;
80102722:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102725:	8b 40 14             	mov    0x14(%eax),%eax
80102728:	a3 34 b6 10 80       	mov    %eax,0x8010b634

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
8010272d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102730:	8b 00                	mov    (%eax),%eax
80102732:	83 e0 04             	and    $0x4,%eax
80102735:	85 c0                	test   %eax,%eax
80102737:	75 2e                	jne    80102767 <ideintr+0x76>
80102739:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102740:	e8 9d fd ff ff       	call   801024e2 <idewait>
80102745:	85 c0                	test   %eax,%eax
80102747:	78 1e                	js     80102767 <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102749:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010274c:	83 c0 18             	add    $0x18,%eax
8010274f:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102756:	00 
80102757:	89 44 24 04          	mov    %eax,0x4(%esp)
8010275b:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
80102762:	e8 13 fd ff ff       	call   8010247a <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
80102767:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010276a:	8b 00                	mov    (%eax),%eax
8010276c:	89 c2                	mov    %eax,%edx
8010276e:	83 ca 02             	or     $0x2,%edx
80102771:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102774:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102776:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102779:	8b 00                	mov    (%eax),%eax
8010277b:	89 c2                	mov    %eax,%edx
8010277d:	83 e2 fb             	and    $0xfffffffb,%edx
80102780:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102783:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80102785:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102788:	89 04 24             	mov    %eax,(%esp)
8010278b:	e8 1e 25 00 00       	call   80104cae <wakeup>
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
80102790:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102795:	85 c0                	test   %eax,%eax
80102797:	74 0d                	je     801027a6 <ideintr+0xb5>
    idestart(idequeue);
80102799:	a1 34 b6 10 80       	mov    0x8010b634,%eax
8010279e:	89 04 24             	mov    %eax,(%esp)
801027a1:	e8 26 fe ff ff       	call   801025cc <idestart>

  release(&idelock);
801027a6:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801027ad:	e8 67 27 00 00       	call   80104f19 <release>
}
801027b2:	c9                   	leave  
801027b3:	c3                   	ret    

801027b4 <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027b4:	55                   	push   %ebp
801027b5:	89 e5                	mov    %esp,%ebp
801027b7:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027ba:	8b 45 08             	mov    0x8(%ebp),%eax
801027bd:	8b 00                	mov    (%eax),%eax
801027bf:	83 e0 01             	and    $0x1,%eax
801027c2:	85 c0                	test   %eax,%eax
801027c4:	75 0c                	jne    801027d2 <iderw+0x1e>
    panic("iderw: buf not busy");
801027c6:	c7 04 24 71 87 10 80 	movl   $0x80108771,(%esp)
801027cd:	e8 6b dd ff ff       	call   8010053d <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027d2:	8b 45 08             	mov    0x8(%ebp),%eax
801027d5:	8b 00                	mov    (%eax),%eax
801027d7:	83 e0 06             	and    $0x6,%eax
801027da:	83 f8 02             	cmp    $0x2,%eax
801027dd:	75 0c                	jne    801027eb <iderw+0x37>
    panic("iderw: nothing to do");
801027df:	c7 04 24 85 87 10 80 	movl   $0x80108785,(%esp)
801027e6:	e8 52 dd ff ff       	call   8010053d <panic>
  if(b->dev != 0 && !havedisk1)
801027eb:	8b 45 08             	mov    0x8(%ebp),%eax
801027ee:	8b 40 04             	mov    0x4(%eax),%eax
801027f1:	85 c0                	test   %eax,%eax
801027f3:	74 15                	je     8010280a <iderw+0x56>
801027f5:	a1 38 b6 10 80       	mov    0x8010b638,%eax
801027fa:	85 c0                	test   %eax,%eax
801027fc:	75 0c                	jne    8010280a <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027fe:	c7 04 24 9a 87 10 80 	movl   $0x8010879a,(%esp)
80102805:	e8 33 dd ff ff       	call   8010053d <panic>

  acquire(&idelock);  //DOC: acquire-lock
8010280a:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102811:	e8 a1 26 00 00       	call   80104eb7 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
80102816:	8b 45 08             	mov    0x8(%ebp),%eax
80102819:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC: insert-queue
80102820:	c7 45 f4 34 b6 10 80 	movl   $0x8010b634,-0xc(%ebp)
80102827:	eb 0b                	jmp    80102834 <iderw+0x80>
80102829:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282c:	8b 00                	mov    (%eax),%eax
8010282e:	83 c0 14             	add    $0x14,%eax
80102831:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102834:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102837:	8b 00                	mov    (%eax),%eax
80102839:	85 c0                	test   %eax,%eax
8010283b:	75 ec                	jne    80102829 <iderw+0x75>
    ;
  *pp = b;
8010283d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102840:	8b 55 08             	mov    0x8(%ebp),%edx
80102843:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
80102845:	a1 34 b6 10 80       	mov    0x8010b634,%eax
8010284a:	3b 45 08             	cmp    0x8(%ebp),%eax
8010284d:	75 22                	jne    80102871 <iderw+0xbd>
    idestart(b);
8010284f:	8b 45 08             	mov    0x8(%ebp),%eax
80102852:	89 04 24             	mov    %eax,(%esp)
80102855:	e8 72 fd ff ff       	call   801025cc <idestart>
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010285a:	eb 15                	jmp    80102871 <iderw+0xbd>
    sleep(b, &idelock);
8010285c:	c7 44 24 04 00 b6 10 	movl   $0x8010b600,0x4(%esp)
80102863:	80 
80102864:	8b 45 08             	mov    0x8(%ebp),%eax
80102867:	89 04 24             	mov    %eax,(%esp)
8010286a:	e8 31 22 00 00       	call   80104aa0 <sleep>
8010286f:	eb 01                	jmp    80102872 <iderw+0xbe>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102871:	90                   	nop
80102872:	8b 45 08             	mov    0x8(%ebp),%eax
80102875:	8b 00                	mov    (%eax),%eax
80102877:	83 e0 06             	and    $0x6,%eax
8010287a:	83 f8 02             	cmp    $0x2,%eax
8010287d:	75 dd                	jne    8010285c <iderw+0xa8>
    sleep(b, &idelock);
  }

  release(&idelock);
8010287f:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102886:	e8 8e 26 00 00       	call   80104f19 <release>
}
8010288b:	c9                   	leave  
8010288c:	c3                   	ret    
8010288d:	00 00                	add    %al,(%eax)
	...

80102890 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102890:	55                   	push   %ebp
80102891:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102893:	a1 34 f8 10 80       	mov    0x8010f834,%eax
80102898:	8b 55 08             	mov    0x8(%ebp),%edx
8010289b:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
8010289d:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028a2:	8b 40 10             	mov    0x10(%eax),%eax
}
801028a5:	5d                   	pop    %ebp
801028a6:	c3                   	ret    

801028a7 <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
801028a7:	55                   	push   %ebp
801028a8:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
801028aa:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028af:	8b 55 08             	mov    0x8(%ebp),%edx
801028b2:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
801028b4:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028b9:	8b 55 0c             	mov    0xc(%ebp),%edx
801028bc:	89 50 10             	mov    %edx,0x10(%eax)
}
801028bf:	5d                   	pop    %ebp
801028c0:	c3                   	ret    

801028c1 <ioapicinit>:

void
ioapicinit(void)
{
801028c1:	55                   	push   %ebp
801028c2:	89 e5                	mov    %esp,%ebp
801028c4:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028c7:	a1 04 f9 10 80       	mov    0x8010f904,%eax
801028cc:	85 c0                	test   %eax,%eax
801028ce:	0f 84 9f 00 00 00    	je     80102973 <ioapicinit+0xb2>
    return;

  ioapic = (volatile struct ioapic*)IOAPIC;
801028d4:	c7 05 34 f8 10 80 00 	movl   $0xfec00000,0x8010f834
801028db:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028de:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028e5:	e8 a6 ff ff ff       	call   80102890 <ioapicread>
801028ea:	c1 e8 10             	shr    $0x10,%eax
801028ed:	25 ff 00 00 00       	and    $0xff,%eax
801028f2:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028f5:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028fc:	e8 8f ff ff ff       	call   80102890 <ioapicread>
80102901:	c1 e8 18             	shr    $0x18,%eax
80102904:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
80102907:	0f b6 05 00 f9 10 80 	movzbl 0x8010f900,%eax
8010290e:	0f b6 c0             	movzbl %al,%eax
80102911:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80102914:	74 0c                	je     80102922 <ioapicinit+0x61>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102916:	c7 04 24 b8 87 10 80 	movl   $0x801087b8,(%esp)
8010291d:	e8 7f da ff ff       	call   801003a1 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102922:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102929:	eb 3e                	jmp    80102969 <ioapicinit+0xa8>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
8010292b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010292e:	83 c0 20             	add    $0x20,%eax
80102931:	0d 00 00 01 00       	or     $0x10000,%eax
80102936:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102939:	83 c2 08             	add    $0x8,%edx
8010293c:	01 d2                	add    %edx,%edx
8010293e:	89 44 24 04          	mov    %eax,0x4(%esp)
80102942:	89 14 24             	mov    %edx,(%esp)
80102945:	e8 5d ff ff ff       	call   801028a7 <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
8010294a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010294d:	83 c0 08             	add    $0x8,%eax
80102950:	01 c0                	add    %eax,%eax
80102952:	83 c0 01             	add    $0x1,%eax
80102955:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010295c:	00 
8010295d:	89 04 24             	mov    %eax,(%esp)
80102960:	e8 42 ff ff ff       	call   801028a7 <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102965:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102969:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010296c:	3b 45 f0             	cmp    -0x10(%ebp),%eax
8010296f:	7e ba                	jle    8010292b <ioapicinit+0x6a>
80102971:	eb 01                	jmp    80102974 <ioapicinit+0xb3>
ioapicinit(void)
{
  int i, id, maxintr;

  if(!ismp)
    return;
80102973:	90                   	nop
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
80102974:	c9                   	leave  
80102975:	c3                   	ret    

80102976 <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
80102976:	55                   	push   %ebp
80102977:	89 e5                	mov    %esp,%ebp
80102979:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
8010297c:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80102981:	85 c0                	test   %eax,%eax
80102983:	74 39                	je     801029be <ioapicenable+0x48>
    return;

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
80102985:	8b 45 08             	mov    0x8(%ebp),%eax
80102988:	83 c0 20             	add    $0x20,%eax
8010298b:	8b 55 08             	mov    0x8(%ebp),%edx
8010298e:	83 c2 08             	add    $0x8,%edx
80102991:	01 d2                	add    %edx,%edx
80102993:	89 44 24 04          	mov    %eax,0x4(%esp)
80102997:	89 14 24             	mov    %edx,(%esp)
8010299a:	e8 08 ff ff ff       	call   801028a7 <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
8010299f:	8b 45 0c             	mov    0xc(%ebp),%eax
801029a2:	c1 e0 18             	shl    $0x18,%eax
801029a5:	8b 55 08             	mov    0x8(%ebp),%edx
801029a8:	83 c2 08             	add    $0x8,%edx
801029ab:	01 d2                	add    %edx,%edx
801029ad:	83 c2 01             	add    $0x1,%edx
801029b0:	89 44 24 04          	mov    %eax,0x4(%esp)
801029b4:	89 14 24             	mov    %edx,(%esp)
801029b7:	e8 eb fe ff ff       	call   801028a7 <ioapicwrite>
801029bc:	eb 01                	jmp    801029bf <ioapicenable+0x49>

void
ioapicenable(int irq, int cpunum)
{
  if(!ismp)
    return;
801029be:	90                   	nop
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
}
801029bf:	c9                   	leave  
801029c0:	c3                   	ret    
801029c1:	00 00                	add    %al,(%eax)
	...

801029c4 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029c4:	55                   	push   %ebp
801029c5:	89 e5                	mov    %esp,%ebp
801029c7:	8b 45 08             	mov    0x8(%ebp),%eax
801029ca:	05 00 00 00 80       	add    $0x80000000,%eax
801029cf:	5d                   	pop    %ebp
801029d0:	c3                   	ret    

801029d1 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029d1:	55                   	push   %ebp
801029d2:	89 e5                	mov    %esp,%ebp
801029d4:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029d7:	c7 44 24 04 ea 87 10 	movl   $0x801087ea,0x4(%esp)
801029de:	80 
801029df:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
801029e6:	e8 ab 24 00 00       	call   80104e96 <initlock>
  kmem.use_lock = 0;
801029eb:	c7 05 74 f8 10 80 00 	movl   $0x0,0x8010f874
801029f2:	00 00 00 
  freerange(vstart, vend);
801029f5:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f8:	89 44 24 04          	mov    %eax,0x4(%esp)
801029fc:	8b 45 08             	mov    0x8(%ebp),%eax
801029ff:	89 04 24             	mov    %eax,(%esp)
80102a02:	e8 26 00 00 00       	call   80102a2d <freerange>
}
80102a07:	c9                   	leave  
80102a08:	c3                   	ret    

80102a09 <kinit2>:

void
kinit2(void *vstart, void *vend)
{
80102a09:	55                   	push   %ebp
80102a0a:	89 e5                	mov    %esp,%ebp
80102a0c:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
80102a0f:	8b 45 0c             	mov    0xc(%ebp),%eax
80102a12:	89 44 24 04          	mov    %eax,0x4(%esp)
80102a16:	8b 45 08             	mov    0x8(%ebp),%eax
80102a19:	89 04 24             	mov    %eax,(%esp)
80102a1c:	e8 0c 00 00 00       	call   80102a2d <freerange>
  kmem.use_lock = 1;
80102a21:	c7 05 74 f8 10 80 01 	movl   $0x1,0x8010f874
80102a28:	00 00 00 
}
80102a2b:	c9                   	leave  
80102a2c:	c3                   	ret    

80102a2d <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a2d:	55                   	push   %ebp
80102a2e:	89 e5                	mov    %esp,%ebp
80102a30:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a33:	8b 45 08             	mov    0x8(%ebp),%eax
80102a36:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a3b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a40:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a43:	eb 12                	jmp    80102a57 <freerange+0x2a>
    kfree(p);
80102a45:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a48:	89 04 24             	mov    %eax,(%esp)
80102a4b:	e8 16 00 00 00       	call   80102a66 <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a50:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a5a:	05 00 10 00 00       	add    $0x1000,%eax
80102a5f:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a62:	76 e1                	jbe    80102a45 <freerange+0x18>
    kfree(p);
}
80102a64:	c9                   	leave  
80102a65:	c3                   	ret    

80102a66 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a66:	55                   	push   %ebp
80102a67:	89 e5                	mov    %esp,%ebp
80102a69:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a6c:	8b 45 08             	mov    0x8(%ebp),%eax
80102a6f:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a74:	85 c0                	test   %eax,%eax
80102a76:	75 1b                	jne    80102a93 <kfree+0x2d>
80102a78:	81 7d 08 fc 28 11 80 	cmpl   $0x801128fc,0x8(%ebp)
80102a7f:	72 12                	jb     80102a93 <kfree+0x2d>
80102a81:	8b 45 08             	mov    0x8(%ebp),%eax
80102a84:	89 04 24             	mov    %eax,(%esp)
80102a87:	e8 38 ff ff ff       	call   801029c4 <v2p>
80102a8c:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a91:	76 0c                	jbe    80102a9f <kfree+0x39>
    panic("kfree");
80102a93:	c7 04 24 ef 87 10 80 	movl   $0x801087ef,(%esp)
80102a9a:	e8 9e da ff ff       	call   8010053d <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a9f:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102aa6:	00 
80102aa7:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102aae:	00 
80102aaf:	8b 45 08             	mov    0x8(%ebp),%eax
80102ab2:	89 04 24             	mov    %eax,(%esp)
80102ab5:	e8 4c 26 00 00       	call   80105106 <memset>

  if(kmem.use_lock)
80102aba:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102abf:	85 c0                	test   %eax,%eax
80102ac1:	74 0c                	je     80102acf <kfree+0x69>
    acquire(&kmem.lock);
80102ac3:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102aca:	e8 e8 23 00 00       	call   80104eb7 <acquire>
  r = (struct run*)v;
80102acf:	8b 45 08             	mov    0x8(%ebp),%eax
80102ad2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102ad5:	8b 15 78 f8 10 80    	mov    0x8010f878,%edx
80102adb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ade:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ae0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ae3:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102ae8:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102aed:	85 c0                	test   %eax,%eax
80102aef:	74 0c                	je     80102afd <kfree+0x97>
    release(&kmem.lock);
80102af1:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102af8:	e8 1c 24 00 00       	call   80104f19 <release>
}
80102afd:	c9                   	leave  
80102afe:	c3                   	ret    

80102aff <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102aff:	55                   	push   %ebp
80102b00:	89 e5                	mov    %esp,%ebp
80102b02:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102b05:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102b0a:	85 c0                	test   %eax,%eax
80102b0c:	74 0c                	je     80102b1a <kalloc+0x1b>
    acquire(&kmem.lock);
80102b0e:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102b15:	e8 9d 23 00 00       	call   80104eb7 <acquire>
  r = kmem.freelist;
80102b1a:	a1 78 f8 10 80       	mov    0x8010f878,%eax
80102b1f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b22:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b26:	74 0a                	je     80102b32 <kalloc+0x33>
    kmem.freelist = r->next;
80102b28:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b2b:	8b 00                	mov    (%eax),%eax
80102b2d:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102b32:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102b37:	85 c0                	test   %eax,%eax
80102b39:	74 0c                	je     80102b47 <kalloc+0x48>
    release(&kmem.lock);
80102b3b:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102b42:	e8 d2 23 00 00       	call   80104f19 <release>
  return (char*)r;
80102b47:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b4a:	c9                   	leave  
80102b4b:	c3                   	ret    

80102b4c <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b4c:	55                   	push   %ebp
80102b4d:	89 e5                	mov    %esp,%ebp
80102b4f:	53                   	push   %ebx
80102b50:	83 ec 14             	sub    $0x14,%esp
80102b53:	8b 45 08             	mov    0x8(%ebp),%eax
80102b56:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b5a:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102b5e:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102b62:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80102b66:	ec                   	in     (%dx),%al
80102b67:	89 c3                	mov    %eax,%ebx
80102b69:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102b6c:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102b70:	83 c4 14             	add    $0x14,%esp
80102b73:	5b                   	pop    %ebx
80102b74:	5d                   	pop    %ebp
80102b75:	c3                   	ret    

80102b76 <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b76:	55                   	push   %ebp
80102b77:	89 e5                	mov    %esp,%ebp
80102b79:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b7c:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b83:	e8 c4 ff ff ff       	call   80102b4c <inb>
80102b88:	0f b6 c0             	movzbl %al,%eax
80102b8b:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b8e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b91:	83 e0 01             	and    $0x1,%eax
80102b94:	85 c0                	test   %eax,%eax
80102b96:	75 0a                	jne    80102ba2 <kbdgetc+0x2c>
    return -1;
80102b98:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b9d:	e9 23 01 00 00       	jmp    80102cc5 <kbdgetc+0x14f>
  data = inb(KBDATAP);
80102ba2:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102ba9:	e8 9e ff ff ff       	call   80102b4c <inb>
80102bae:	0f b6 c0             	movzbl %al,%eax
80102bb1:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102bb4:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102bbb:	75 17                	jne    80102bd4 <kbdgetc+0x5e>
    shift |= E0ESC;
80102bbd:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102bc2:	83 c8 40             	or     $0x40,%eax
80102bc5:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102bca:	b8 00 00 00 00       	mov    $0x0,%eax
80102bcf:	e9 f1 00 00 00       	jmp    80102cc5 <kbdgetc+0x14f>
  } else if(data & 0x80){
80102bd4:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bd7:	25 80 00 00 00       	and    $0x80,%eax
80102bdc:	85 c0                	test   %eax,%eax
80102bde:	74 45                	je     80102c25 <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102be0:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102be5:	83 e0 40             	and    $0x40,%eax
80102be8:	85 c0                	test   %eax,%eax
80102bea:	75 08                	jne    80102bf4 <kbdgetc+0x7e>
80102bec:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bef:	83 e0 7f             	and    $0x7f,%eax
80102bf2:	eb 03                	jmp    80102bf7 <kbdgetc+0x81>
80102bf4:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bf7:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bfa:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bfd:	05 20 90 10 80       	add    $0x80109020,%eax
80102c02:	0f b6 00             	movzbl (%eax),%eax
80102c05:	83 c8 40             	or     $0x40,%eax
80102c08:	0f b6 c0             	movzbl %al,%eax
80102c0b:	f7 d0                	not    %eax
80102c0d:	89 c2                	mov    %eax,%edx
80102c0f:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c14:	21 d0                	and    %edx,%eax
80102c16:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102c1b:	b8 00 00 00 00       	mov    $0x0,%eax
80102c20:	e9 a0 00 00 00       	jmp    80102cc5 <kbdgetc+0x14f>
  } else if(shift & E0ESC){
80102c25:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c2a:	83 e0 40             	and    $0x40,%eax
80102c2d:	85 c0                	test   %eax,%eax
80102c2f:	74 14                	je     80102c45 <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c31:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c38:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c3d:	83 e0 bf             	and    $0xffffffbf,%eax
80102c40:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  }

  shift |= shiftcode[data];
80102c45:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c48:	05 20 90 10 80       	add    $0x80109020,%eax
80102c4d:	0f b6 00             	movzbl (%eax),%eax
80102c50:	0f b6 d0             	movzbl %al,%edx
80102c53:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c58:	09 d0                	or     %edx,%eax
80102c5a:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  shift ^= togglecode[data];
80102c5f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c62:	05 20 91 10 80       	add    $0x80109120,%eax
80102c67:	0f b6 00             	movzbl (%eax),%eax
80102c6a:	0f b6 d0             	movzbl %al,%edx
80102c6d:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c72:	31 d0                	xor    %edx,%eax
80102c74:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c79:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c7e:	83 e0 03             	and    $0x3,%eax
80102c81:	8b 04 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%eax
80102c88:	03 45 fc             	add    -0x4(%ebp),%eax
80102c8b:	0f b6 00             	movzbl (%eax),%eax
80102c8e:	0f b6 c0             	movzbl %al,%eax
80102c91:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c94:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c99:	83 e0 08             	and    $0x8,%eax
80102c9c:	85 c0                	test   %eax,%eax
80102c9e:	74 22                	je     80102cc2 <kbdgetc+0x14c>
    if('a' <= c && c <= 'z')
80102ca0:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102ca4:	76 0c                	jbe    80102cb2 <kbdgetc+0x13c>
80102ca6:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102caa:	77 06                	ja     80102cb2 <kbdgetc+0x13c>
      c += 'A' - 'a';
80102cac:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102cb0:	eb 10                	jmp    80102cc2 <kbdgetc+0x14c>
    else if('A' <= c && c <= 'Z')
80102cb2:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102cb6:	76 0a                	jbe    80102cc2 <kbdgetc+0x14c>
80102cb8:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102cbc:	77 04                	ja     80102cc2 <kbdgetc+0x14c>
      c += 'a' - 'A';
80102cbe:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102cc2:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102cc5:	c9                   	leave  
80102cc6:	c3                   	ret    

80102cc7 <kbdintr>:

void
kbdintr(void)
{
80102cc7:	55                   	push   %ebp
80102cc8:	89 e5                	mov    %esp,%ebp
80102cca:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102ccd:	c7 04 24 76 2b 10 80 	movl   $0x80102b76,(%esp)
80102cd4:	e8 d4 da ff ff       	call   801007ad <consoleintr>
}
80102cd9:	c9                   	leave  
80102cda:	c3                   	ret    
	...

80102cdc <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cdc:	55                   	push   %ebp
80102cdd:	89 e5                	mov    %esp,%ebp
80102cdf:	83 ec 08             	sub    $0x8,%esp
80102ce2:	8b 55 08             	mov    0x8(%ebp),%edx
80102ce5:	8b 45 0c             	mov    0xc(%ebp),%eax
80102ce8:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102cec:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cef:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102cf3:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102cf7:	ee                   	out    %al,(%dx)
}
80102cf8:	c9                   	leave  
80102cf9:	c3                   	ret    

80102cfa <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cfa:	55                   	push   %ebp
80102cfb:	89 e5                	mov    %esp,%ebp
80102cfd:	53                   	push   %ebx
80102cfe:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102d01:	9c                   	pushf  
80102d02:	5b                   	pop    %ebx
80102d03:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80102d06:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102d09:	83 c4 10             	add    $0x10,%esp
80102d0c:	5b                   	pop    %ebx
80102d0d:	5d                   	pop    %ebp
80102d0e:	c3                   	ret    

80102d0f <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102d0f:	55                   	push   %ebp
80102d10:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102d12:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d17:	8b 55 08             	mov    0x8(%ebp),%edx
80102d1a:	c1 e2 02             	shl    $0x2,%edx
80102d1d:	01 c2                	add    %eax,%edx
80102d1f:	8b 45 0c             	mov    0xc(%ebp),%eax
80102d22:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102d24:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d29:	83 c0 20             	add    $0x20,%eax
80102d2c:	8b 00                	mov    (%eax),%eax
}
80102d2e:	5d                   	pop    %ebp
80102d2f:	c3                   	ret    

80102d30 <lapicinit>:
//PAGEBREAK!

void
lapicinit(int c)
{
80102d30:	55                   	push   %ebp
80102d31:	89 e5                	mov    %esp,%ebp
80102d33:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d36:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d3b:	85 c0                	test   %eax,%eax
80102d3d:	0f 84 47 01 00 00    	je     80102e8a <lapicinit+0x15a>
    return;

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d43:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d4a:	00 
80102d4b:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d52:	e8 b8 ff ff ff       	call   80102d0f <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d57:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d5e:	00 
80102d5f:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d66:	e8 a4 ff ff ff       	call   80102d0f <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d6b:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d72:	00 
80102d73:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d7a:	e8 90 ff ff ff       	call   80102d0f <lapicw>
  lapicw(TICR, 10000000); 
80102d7f:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d86:	00 
80102d87:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d8e:	e8 7c ff ff ff       	call   80102d0f <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d93:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d9a:	00 
80102d9b:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102da2:	e8 68 ff ff ff       	call   80102d0f <lapicw>
  lapicw(LINT1, MASKED);
80102da7:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102dae:	00 
80102daf:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102db6:	e8 54 ff ff ff       	call   80102d0f <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102dbb:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102dc0:	83 c0 30             	add    $0x30,%eax
80102dc3:	8b 00                	mov    (%eax),%eax
80102dc5:	c1 e8 10             	shr    $0x10,%eax
80102dc8:	25 ff 00 00 00       	and    $0xff,%eax
80102dcd:	83 f8 03             	cmp    $0x3,%eax
80102dd0:	76 14                	jbe    80102de6 <lapicinit+0xb6>
    lapicw(PCINT, MASKED);
80102dd2:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102dd9:	00 
80102dda:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102de1:	e8 29 ff ff ff       	call   80102d0f <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102de6:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102ded:	00 
80102dee:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102df5:	e8 15 ff ff ff       	call   80102d0f <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102dfa:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e01:	00 
80102e02:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e09:	e8 01 ff ff ff       	call   80102d0f <lapicw>
  lapicw(ESR, 0);
80102e0e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e15:	00 
80102e16:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e1d:	e8 ed fe ff ff       	call   80102d0f <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102e22:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e29:	00 
80102e2a:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e31:	e8 d9 fe ff ff       	call   80102d0f <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e36:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e3d:	00 
80102e3e:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e45:	e8 c5 fe ff ff       	call   80102d0f <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e4a:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e51:	00 
80102e52:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e59:	e8 b1 fe ff ff       	call   80102d0f <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e5e:	90                   	nop
80102e5f:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102e64:	05 00 03 00 00       	add    $0x300,%eax
80102e69:	8b 00                	mov    (%eax),%eax
80102e6b:	25 00 10 00 00       	and    $0x1000,%eax
80102e70:	85 c0                	test   %eax,%eax
80102e72:	75 eb                	jne    80102e5f <lapicinit+0x12f>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e74:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e7b:	00 
80102e7c:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e83:	e8 87 fe ff ff       	call   80102d0f <lapicw>
80102e88:	eb 01                	jmp    80102e8b <lapicinit+0x15b>

void
lapicinit(int c)
{
  if(!lapic) 
    return;
80102e8a:	90                   	nop
  while(lapic[ICRLO] & DELIVS)
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102e8b:	c9                   	leave  
80102e8c:	c3                   	ret    

80102e8d <cpunum>:

int
cpunum(void)
{
80102e8d:	55                   	push   %ebp
80102e8e:	89 e5                	mov    %esp,%ebp
80102e90:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e93:	e8 62 fe ff ff       	call   80102cfa <readeflags>
80102e98:	25 00 02 00 00       	and    $0x200,%eax
80102e9d:	85 c0                	test   %eax,%eax
80102e9f:	74 29                	je     80102eca <cpunum+0x3d>
    static int n;
    if(n++ == 0)
80102ea1:	a1 40 b6 10 80       	mov    0x8010b640,%eax
80102ea6:	85 c0                	test   %eax,%eax
80102ea8:	0f 94 c2             	sete   %dl
80102eab:	83 c0 01             	add    $0x1,%eax
80102eae:	a3 40 b6 10 80       	mov    %eax,0x8010b640
80102eb3:	84 d2                	test   %dl,%dl
80102eb5:	74 13                	je     80102eca <cpunum+0x3d>
      cprintf("cpu called from %x with interrupts enabled\n",
80102eb7:	8b 45 04             	mov    0x4(%ebp),%eax
80102eba:	89 44 24 04          	mov    %eax,0x4(%esp)
80102ebe:	c7 04 24 f8 87 10 80 	movl   $0x801087f8,(%esp)
80102ec5:	e8 d7 d4 ff ff       	call   801003a1 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102eca:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ecf:	85 c0                	test   %eax,%eax
80102ed1:	74 0f                	je     80102ee2 <cpunum+0x55>
    return lapic[ID]>>24;
80102ed3:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ed8:	83 c0 20             	add    $0x20,%eax
80102edb:	8b 00                	mov    (%eax),%eax
80102edd:	c1 e8 18             	shr    $0x18,%eax
80102ee0:	eb 05                	jmp    80102ee7 <cpunum+0x5a>
  return 0;
80102ee2:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102ee7:	c9                   	leave  
80102ee8:	c3                   	ret    

80102ee9 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102ee9:	55                   	push   %ebp
80102eea:	89 e5                	mov    %esp,%ebp
80102eec:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102eef:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ef4:	85 c0                	test   %eax,%eax
80102ef6:	74 14                	je     80102f0c <lapiceoi+0x23>
    lapicw(EOI, 0);
80102ef8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102eff:	00 
80102f00:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102f07:	e8 03 fe ff ff       	call   80102d0f <lapicw>
}
80102f0c:	c9                   	leave  
80102f0d:	c3                   	ret    

80102f0e <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102f0e:	55                   	push   %ebp
80102f0f:	89 e5                	mov    %esp,%ebp
}
80102f11:	5d                   	pop    %ebp
80102f12:	c3                   	ret    

80102f13 <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102f13:	55                   	push   %ebp
80102f14:	89 e5                	mov    %esp,%ebp
80102f16:	83 ec 1c             	sub    $0x1c,%esp
80102f19:	8b 45 08             	mov    0x8(%ebp),%eax
80102f1c:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
80102f1f:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102f26:	00 
80102f27:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102f2e:	e8 a9 fd ff ff       	call   80102cdc <outb>
  outb(IO_RTC+1, 0x0A);
80102f33:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f3a:	00 
80102f3b:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f42:	e8 95 fd ff ff       	call   80102cdc <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f47:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f4e:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f51:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f56:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f59:	8d 50 02             	lea    0x2(%eax),%edx
80102f5c:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f5f:	c1 e8 04             	shr    $0x4,%eax
80102f62:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f65:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f69:	c1 e0 18             	shl    $0x18,%eax
80102f6c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f70:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f77:	e8 93 fd ff ff       	call   80102d0f <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f7c:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f83:	00 
80102f84:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f8b:	e8 7f fd ff ff       	call   80102d0f <lapicw>
  microdelay(200);
80102f90:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f97:	e8 72 ff ff ff       	call   80102f0e <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f9c:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102fa3:	00 
80102fa4:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fab:	e8 5f fd ff ff       	call   80102d0f <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102fb0:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102fb7:	e8 52 ff ff ff       	call   80102f0e <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fbc:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102fc3:	eb 40                	jmp    80103005 <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102fc5:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102fc9:	c1 e0 18             	shl    $0x18,%eax
80102fcc:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fd0:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fd7:	e8 33 fd ff ff       	call   80102d0f <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fdc:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fdf:	c1 e8 0c             	shr    $0xc,%eax
80102fe2:	80 cc 06             	or     $0x6,%ah
80102fe5:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fe9:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102ff0:	e8 1a fd ff ff       	call   80102d0f <lapicw>
    microdelay(200);
80102ff5:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102ffc:	e8 0d ff ff ff       	call   80102f0e <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80103001:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103005:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80103009:	7e ba                	jle    80102fc5 <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
8010300b:	c9                   	leave  
8010300c:	c3                   	ret    
8010300d:	00 00                	add    %al,(%eax)
	...

80103010 <initlog>:

static void recover_from_log(void);

void
initlog(void)
{
80103010:	55                   	push   %ebp
80103011:	89 e5                	mov    %esp,%ebp
80103013:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80103016:	c7 44 24 04 24 88 10 	movl   $0x80108824,0x4(%esp)
8010301d:	80 
8010301e:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103025:	e8 6c 1e 00 00       	call   80104e96 <initlock>
  readsb(ROOTDEV, &sb);
8010302a:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010302d:	89 44 24 04          	mov    %eax,0x4(%esp)
80103031:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103038:	e8 af e2 ff ff       	call   801012ec <readsb>
  log.start = sb.size - sb.nlog;
8010303d:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103040:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103043:	89 d1                	mov    %edx,%ecx
80103045:	29 c1                	sub    %eax,%ecx
80103047:	89 c8                	mov    %ecx,%eax
80103049:	a3 b4 f8 10 80       	mov    %eax,0x8010f8b4
  log.size = sb.nlog;
8010304e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103051:	a3 b8 f8 10 80       	mov    %eax,0x8010f8b8
  log.dev = ROOTDEV;
80103056:	c7 05 c0 f8 10 80 01 	movl   $0x1,0x8010f8c0
8010305d:	00 00 00 
  recover_from_log();
80103060:	e8 97 01 00 00       	call   801031fc <recover_from_log>
}
80103065:	c9                   	leave  
80103066:	c3                   	ret    

80103067 <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
80103067:	55                   	push   %ebp
80103068:	89 e5                	mov    %esp,%ebp
8010306a:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
8010306d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103074:	e9 89 00 00 00       	jmp    80103102 <install_trans+0x9b>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103079:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
8010307e:	03 45 f4             	add    -0xc(%ebp),%eax
80103081:	83 c0 01             	add    $0x1,%eax
80103084:	89 c2                	mov    %eax,%edx
80103086:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
8010308b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010308f:	89 04 24             	mov    %eax,(%esp)
80103092:	e8 0f d1 ff ff       	call   801001a6 <bread>
80103097:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
8010309a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010309d:	83 c0 10             	add    $0x10,%eax
801030a0:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
801030a7:	89 c2                	mov    %eax,%edx
801030a9:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
801030ae:	89 54 24 04          	mov    %edx,0x4(%esp)
801030b2:	89 04 24             	mov    %eax,(%esp)
801030b5:	e8 ec d0 ff ff       	call   801001a6 <bread>
801030ba:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801030bd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030c0:	8d 50 18             	lea    0x18(%eax),%edx
801030c3:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030c6:	83 c0 18             	add    $0x18,%eax
801030c9:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801030d0:	00 
801030d1:	89 54 24 04          	mov    %edx,0x4(%esp)
801030d5:	89 04 24             	mov    %eax,(%esp)
801030d8:	e8 fc 20 00 00       	call   801051d9 <memmove>
    bwrite(dbuf);  // write dst to disk
801030dd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030e0:	89 04 24             	mov    %eax,(%esp)
801030e3:	e8 f5 d0 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801030e8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030eb:	89 04 24             	mov    %eax,(%esp)
801030ee:	e8 24 d1 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801030f3:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030f6:	89 04 24             	mov    %eax,(%esp)
801030f9:	e8 19 d1 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801030fe:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103102:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103107:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010310a:	0f 8f 69 ff ff ff    	jg     80103079 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
80103110:	c9                   	leave  
80103111:	c3                   	ret    

80103112 <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
80103112:	55                   	push   %ebp
80103113:	89 e5                	mov    %esp,%ebp
80103115:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103118:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
8010311d:	89 c2                	mov    %eax,%edx
8010311f:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
80103124:	89 54 24 04          	mov    %edx,0x4(%esp)
80103128:	89 04 24             	mov    %eax,(%esp)
8010312b:	e8 76 d0 ff ff       	call   801001a6 <bread>
80103130:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
80103133:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103136:	83 c0 18             	add    $0x18,%eax
80103139:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
8010313c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010313f:	8b 00                	mov    (%eax),%eax
80103141:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  for (i = 0; i < log.lh.n; i++) {
80103146:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010314d:	eb 1b                	jmp    8010316a <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
8010314f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103152:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103155:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103159:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010315c:	83 c2 10             	add    $0x10,%edx
8010315f:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
80103166:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010316a:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010316f:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103172:	7f db                	jg     8010314f <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
80103174:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103177:	89 04 24             	mov    %eax,(%esp)
8010317a:	e8 98 d0 ff ff       	call   80100217 <brelse>
}
8010317f:	c9                   	leave  
80103180:	c3                   	ret    

80103181 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103181:	55                   	push   %ebp
80103182:	89 e5                	mov    %esp,%ebp
80103184:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103187:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
8010318c:	89 c2                	mov    %eax,%edx
8010318e:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
80103193:	89 54 24 04          	mov    %edx,0x4(%esp)
80103197:	89 04 24             	mov    %eax,(%esp)
8010319a:	e8 07 d0 ff ff       	call   801001a6 <bread>
8010319f:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
801031a2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031a5:	83 c0 18             	add    $0x18,%eax
801031a8:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
801031ab:	8b 15 c4 f8 10 80    	mov    0x8010f8c4,%edx
801031b1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031b4:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801031b6:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801031bd:	eb 1b                	jmp    801031da <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
801031bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801031c2:	83 c0 10             	add    $0x10,%eax
801031c5:	8b 0c 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%ecx
801031cc:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031cf:	8b 55 f4             	mov    -0xc(%ebp),%edx
801031d2:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801031d6:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801031da:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801031df:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801031e2:	7f db                	jg     801031bf <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801031e4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031e7:	89 04 24             	mov    %eax,(%esp)
801031ea:	e8 ee cf ff ff       	call   801001dd <bwrite>
  brelse(buf);
801031ef:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031f2:	89 04 24             	mov    %eax,(%esp)
801031f5:	e8 1d d0 ff ff       	call   80100217 <brelse>
}
801031fa:	c9                   	leave  
801031fb:	c3                   	ret    

801031fc <recover_from_log>:

static void
recover_from_log(void)
{
801031fc:	55                   	push   %ebp
801031fd:	89 e5                	mov    %esp,%ebp
801031ff:	83 ec 08             	sub    $0x8,%esp
  read_head();      
80103202:	e8 0b ff ff ff       	call   80103112 <read_head>
  install_trans(); // if committed, copy from log to disk
80103207:	e8 5b fe ff ff       	call   80103067 <install_trans>
  log.lh.n = 0;
8010320c:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
80103213:	00 00 00 
  write_head(); // clear the log
80103216:	e8 66 ff ff ff       	call   80103181 <write_head>
}
8010321b:	c9                   	leave  
8010321c:	c3                   	ret    

8010321d <begin_trans>:

void
begin_trans(void)
{
8010321d:	55                   	push   %ebp
8010321e:	89 e5                	mov    %esp,%ebp
80103220:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
80103223:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010322a:	e8 88 1c 00 00       	call   80104eb7 <acquire>
  while (log.busy) {
8010322f:	90                   	nop
80103230:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
80103235:	85 c0                	test   %eax,%eax
80103237:	75 f7                	jne    80103230 <begin_trans+0x13>
    //sleep(&log, &log.lock);
  }
  log.busy = 1;
80103239:	c7 05 bc f8 10 80 01 	movl   $0x1,0x8010f8bc
80103240:	00 00 00 
  release(&log.lock);
80103243:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010324a:	e8 ca 1c 00 00       	call   80104f19 <release>
}
8010324f:	c9                   	leave  
80103250:	c3                   	ret    

80103251 <commit_trans>:

void
commit_trans(void)
{
80103251:	55                   	push   %ebp
80103252:	89 e5                	mov    %esp,%ebp
80103254:	83 ec 18             	sub    $0x18,%esp
  if (log.lh.n > 0) {
80103257:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010325c:	85 c0                	test   %eax,%eax
8010325e:	7e 19                	jle    80103279 <commit_trans+0x28>
    write_head();    // Write header to disk -- the real commit
80103260:	e8 1c ff ff ff       	call   80103181 <write_head>
    install_trans(); // Now install writes to home locations
80103265:	e8 fd fd ff ff       	call   80103067 <install_trans>
    log.lh.n = 0; 
8010326a:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
80103271:	00 00 00 
    write_head();    // Erase the transaction from the log
80103274:	e8 08 ff ff ff       	call   80103181 <write_head>
  }
  
  acquire(&log.lock);
80103279:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103280:	e8 32 1c 00 00       	call   80104eb7 <acquire>
  log.busy = 0;
80103285:	c7 05 bc f8 10 80 00 	movl   $0x0,0x8010f8bc
8010328c:	00 00 00 
  //wakeup(&log);
  release(&log.lock);
8010328f:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103296:	e8 7e 1c 00 00       	call   80104f19 <release>
}
8010329b:	c9                   	leave  
8010329c:	c3                   	ret    

8010329d <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
8010329d:	55                   	push   %ebp
8010329e:	89 e5                	mov    %esp,%ebp
801032a0:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
801032a3:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801032a8:	83 f8 09             	cmp    $0x9,%eax
801032ab:	7f 12                	jg     801032bf <log_write+0x22>
801032ad:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801032b2:	8b 15 b8 f8 10 80    	mov    0x8010f8b8,%edx
801032b8:	83 ea 01             	sub    $0x1,%edx
801032bb:	39 d0                	cmp    %edx,%eax
801032bd:	7c 0c                	jl     801032cb <log_write+0x2e>
    panic("too big a transaction");
801032bf:	c7 04 24 28 88 10 80 	movl   $0x80108828,(%esp)
801032c6:	e8 72 d2 ff ff       	call   8010053d <panic>
  if (!log.busy)
801032cb:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
801032d0:	85 c0                	test   %eax,%eax
801032d2:	75 0c                	jne    801032e0 <log_write+0x43>
    panic("write outside of trans");
801032d4:	c7 04 24 3e 88 10 80 	movl   $0x8010883e,(%esp)
801032db:	e8 5d d2 ff ff       	call   8010053d <panic>

  for (i = 0; i < log.lh.n; i++) {
801032e0:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801032e7:	eb 1d                	jmp    80103306 <log_write+0x69>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
801032e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801032ec:	83 c0 10             	add    $0x10,%eax
801032ef:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
801032f6:	89 c2                	mov    %eax,%edx
801032f8:	8b 45 08             	mov    0x8(%ebp),%eax
801032fb:	8b 40 08             	mov    0x8(%eax),%eax
801032fe:	39 c2                	cmp    %eax,%edx
80103300:	74 10                	je     80103312 <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (!log.busy)
    panic("write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
80103302:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103306:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010330b:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010330e:	7f d9                	jg     801032e9 <log_write+0x4c>
80103310:	eb 01                	jmp    80103313 <log_write+0x76>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
      break;
80103312:	90                   	nop
  }
  log.lh.sector[i] = b->sector;
80103313:	8b 45 08             	mov    0x8(%ebp),%eax
80103316:	8b 40 08             	mov    0x8(%eax),%eax
80103319:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010331c:	83 c2 10             	add    $0x10,%edx
8010331f:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
  struct buf *lbuf = bread(b->dev, log.start+i+1);
80103326:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
8010332b:	03 45 f4             	add    -0xc(%ebp),%eax
8010332e:	83 c0 01             	add    $0x1,%eax
80103331:	89 c2                	mov    %eax,%edx
80103333:	8b 45 08             	mov    0x8(%ebp),%eax
80103336:	8b 40 04             	mov    0x4(%eax),%eax
80103339:	89 54 24 04          	mov    %edx,0x4(%esp)
8010333d:	89 04 24             	mov    %eax,(%esp)
80103340:	e8 61 ce ff ff       	call   801001a6 <bread>
80103345:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(lbuf->data, b->data, BSIZE);
80103348:	8b 45 08             	mov    0x8(%ebp),%eax
8010334b:	8d 50 18             	lea    0x18(%eax),%edx
8010334e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103351:	83 c0 18             	add    $0x18,%eax
80103354:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010335b:	00 
8010335c:	89 54 24 04          	mov    %edx,0x4(%esp)
80103360:	89 04 24             	mov    %eax,(%esp)
80103363:	e8 71 1e 00 00       	call   801051d9 <memmove>
  bwrite(lbuf);
80103368:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010336b:	89 04 24             	mov    %eax,(%esp)
8010336e:	e8 6a ce ff ff       	call   801001dd <bwrite>
  brelse(lbuf);
80103373:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103376:	89 04 24             	mov    %eax,(%esp)
80103379:	e8 99 ce ff ff       	call   80100217 <brelse>
  if (i == log.lh.n)
8010337e:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103383:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103386:	75 0d                	jne    80103395 <log_write+0xf8>
    log.lh.n++;
80103388:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010338d:	83 c0 01             	add    $0x1,%eax
80103390:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  b->flags |= B_DIRTY; // XXX prevent eviction
80103395:	8b 45 08             	mov    0x8(%ebp),%eax
80103398:	8b 00                	mov    (%eax),%eax
8010339a:	89 c2                	mov    %eax,%edx
8010339c:	83 ca 04             	or     $0x4,%edx
8010339f:	8b 45 08             	mov    0x8(%ebp),%eax
801033a2:	89 10                	mov    %edx,(%eax)
}
801033a4:	c9                   	leave  
801033a5:	c3                   	ret    
	...

801033a8 <v2p>:
801033a8:	55                   	push   %ebp
801033a9:	89 e5                	mov    %esp,%ebp
801033ab:	8b 45 08             	mov    0x8(%ebp),%eax
801033ae:	05 00 00 00 80       	add    $0x80000000,%eax
801033b3:	5d                   	pop    %ebp
801033b4:	c3                   	ret    

801033b5 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801033b5:	55                   	push   %ebp
801033b6:	89 e5                	mov    %esp,%ebp
801033b8:	8b 45 08             	mov    0x8(%ebp),%eax
801033bb:	05 00 00 00 80       	add    $0x80000000,%eax
801033c0:	5d                   	pop    %ebp
801033c1:	c3                   	ret    

801033c2 <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801033c2:	55                   	push   %ebp
801033c3:	89 e5                	mov    %esp,%ebp
801033c5:	53                   	push   %ebx
801033c6:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
801033c9:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033cc:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
801033cf:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033d2:	89 c3                	mov    %eax,%ebx
801033d4:	89 d8                	mov    %ebx,%eax
801033d6:	f0 87 02             	lock xchg %eax,(%edx)
801033d9:	89 c3                	mov    %eax,%ebx
801033db:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801033de:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801033e1:	83 c4 10             	add    $0x10,%esp
801033e4:	5b                   	pop    %ebx
801033e5:	5d                   	pop    %ebp
801033e6:	c3                   	ret    

801033e7 <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801033e7:	55                   	push   %ebp
801033e8:	89 e5                	mov    %esp,%ebp
801033ea:	83 e4 f0             	and    $0xfffffff0,%esp
801033ed:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
801033f0:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
801033f7:	80 
801033f8:	c7 04 24 fc 28 11 80 	movl   $0x801128fc,(%esp)
801033ff:	e8 cd f5 ff ff       	call   801029d1 <kinit1>
  kvmalloc();      // kernel page table
80103404:	e8 79 4a 00 00       	call   80107e82 <kvmalloc>
  mpinit();        // collect info about this machine
80103409:	e8 63 04 00 00       	call   80103871 <mpinit>
  lapicinit(mpbcpu());
8010340e:	e8 2e 02 00 00       	call   80103641 <mpbcpu>
80103413:	89 04 24             	mov    %eax,(%esp)
80103416:	e8 15 f9 ff ff       	call   80102d30 <lapicinit>
  seginit();       // set up segments
8010341b:	e8 05 44 00 00       	call   80107825 <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103420:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103426:	0f b6 00             	movzbl (%eax),%eax
80103429:	0f b6 c0             	movzbl %al,%eax
8010342c:	89 44 24 04          	mov    %eax,0x4(%esp)
80103430:	c7 04 24 55 88 10 80 	movl   $0x80108855,(%esp)
80103437:	e8 65 cf ff ff       	call   801003a1 <cprintf>
  picinit();       // interrupt controller
8010343c:	e8 95 06 00 00       	call   80103ad6 <picinit>
  ioapicinit();    // another interrupt controller
80103441:	e8 7b f4 ff ff       	call   801028c1 <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
80103446:	e8 42 d6 ff ff       	call   80100a8d <consoleinit>
  uartinit();      // serial port
8010344b:	e8 20 37 00 00       	call   80106b70 <uartinit>
  pinit();         // process table
80103450:	e8 96 0b 00 00       	call   80103feb <pinit>
  tvinit();        // trap vectors
80103455:	e8 b9 32 00 00       	call   80106713 <tvinit>
  binit();         // buffer cache
8010345a:	e8 d5 cb ff ff       	call   80100034 <binit>
  fileinit();      // file table
8010345f:	e8 9c da ff ff       	call   80100f00 <fileinit>
  iinit();         // inode cache
80103464:	e8 4a e1 ff ff       	call   801015b3 <iinit>
  ideinit();       // disk
80103469:	e8 b8 f0 ff ff       	call   80102526 <ideinit>
  if(!ismp)
8010346e:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80103473:	85 c0                	test   %eax,%eax
80103475:	75 05                	jne    8010347c <main+0x95>
    timerinit();   // uniprocessor timer
80103477:	e8 da 31 00 00       	call   80106656 <timerinit>
  startothers();   // start other processors
8010347c:	e8 87 00 00 00       	call   80103508 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103481:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
80103488:	8e 
80103489:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
80103490:	e8 74 f5 ff ff       	call   80102a09 <kinit2>
  userinit();      // first user process
80103495:	e8 52 0e 00 00       	call   801042ec <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
8010349a:	e8 22 00 00 00       	call   801034c1 <mpmain>

8010349f <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
8010349f:	55                   	push   %ebp
801034a0:	89 e5                	mov    %esp,%ebp
801034a2:	83 ec 18             	sub    $0x18,%esp
  switchkvm(); 
801034a5:	e8 ef 49 00 00       	call   80107e99 <switchkvm>
  seginit();
801034aa:	e8 76 43 00 00       	call   80107825 <seginit>
  lapicinit(cpunum());
801034af:	e8 d9 f9 ff ff       	call   80102e8d <cpunum>
801034b4:	89 04 24             	mov    %eax,(%esp)
801034b7:	e8 74 f8 ff ff       	call   80102d30 <lapicinit>
  mpmain();
801034bc:	e8 00 00 00 00       	call   801034c1 <mpmain>

801034c1 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801034c1:	55                   	push   %ebp
801034c2:	89 e5                	mov    %esp,%ebp
801034c4:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
801034c7:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034cd:	0f b6 00             	movzbl (%eax),%eax
801034d0:	0f b6 c0             	movzbl %al,%eax
801034d3:	89 44 24 04          	mov    %eax,0x4(%esp)
801034d7:	c7 04 24 6c 88 10 80 	movl   $0x8010886c,(%esp)
801034de:	e8 be ce ff ff       	call   801003a1 <cprintf>
  idtinit();       // load idt register
801034e3:	e8 9f 33 00 00       	call   80106887 <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
801034e8:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034ee:	05 a8 00 00 00       	add    $0xa8,%eax
801034f3:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801034fa:	00 
801034fb:	89 04 24             	mov    %eax,(%esp)
801034fe:	e8 bf fe ff ff       	call   801033c2 <xchg>
  scheduler();     // start running processes
80103503:	e8 ec 13 00 00       	call   801048f4 <scheduler>

80103508 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
80103508:	55                   	push   %ebp
80103509:	89 e5                	mov    %esp,%ebp
8010350b:	53                   	push   %ebx
8010350c:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
8010350f:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
80103516:	e8 9a fe ff ff       	call   801033b5 <p2v>
8010351b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
8010351e:	b8 8a 00 00 00       	mov    $0x8a,%eax
80103523:	89 44 24 08          	mov    %eax,0x8(%esp)
80103527:	c7 44 24 04 0c b5 10 	movl   $0x8010b50c,0x4(%esp)
8010352e:	80 
8010352f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103532:	89 04 24             	mov    %eax,(%esp)
80103535:	e8 9f 1c 00 00       	call   801051d9 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
8010353a:	c7 45 f4 20 f9 10 80 	movl   $0x8010f920,-0xc(%ebp)
80103541:	e9 86 00 00 00       	jmp    801035cc <startothers+0xc4>
    if(c == cpus+cpunum())  // We've started already.
80103546:	e8 42 f9 ff ff       	call   80102e8d <cpunum>
8010354b:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103551:	05 20 f9 10 80       	add    $0x8010f920,%eax
80103556:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103559:	74 69                	je     801035c4 <startothers+0xbc>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
8010355b:	e8 9f f5 ff ff       	call   80102aff <kalloc>
80103560:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
80103563:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103566:	83 e8 04             	sub    $0x4,%eax
80103569:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010356c:	81 c2 00 10 00 00    	add    $0x1000,%edx
80103572:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
80103574:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103577:	83 e8 08             	sub    $0x8,%eax
8010357a:	c7 00 9f 34 10 80    	movl   $0x8010349f,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
80103580:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103583:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103586:	c7 04 24 00 a0 10 80 	movl   $0x8010a000,(%esp)
8010358d:	e8 16 fe ff ff       	call   801033a8 <v2p>
80103592:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
80103594:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103597:	89 04 24             	mov    %eax,(%esp)
8010359a:	e8 09 fe ff ff       	call   801033a8 <v2p>
8010359f:	8b 55 f4             	mov    -0xc(%ebp),%edx
801035a2:	0f b6 12             	movzbl (%edx),%edx
801035a5:	0f b6 d2             	movzbl %dl,%edx
801035a8:	89 44 24 04          	mov    %eax,0x4(%esp)
801035ac:	89 14 24             	mov    %edx,(%esp)
801035af:	e8 5f f9 ff ff       	call   80102f13 <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
801035b4:	90                   	nop
801035b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801035b8:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
801035be:	85 c0                	test   %eax,%eax
801035c0:	74 f3                	je     801035b5 <startothers+0xad>
801035c2:	eb 01                	jmp    801035c5 <startothers+0xbd>
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
    if(c == cpus+cpunum())  // We've started already.
      continue;
801035c4:	90                   	nop
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
801035c5:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
801035cc:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801035d1:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801035d7:	05 20 f9 10 80       	add    $0x8010f920,%eax
801035dc:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801035df:	0f 87 61 ff ff ff    	ja     80103546 <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801035e5:	83 c4 24             	add    $0x24,%esp
801035e8:	5b                   	pop    %ebx
801035e9:	5d                   	pop    %ebp
801035ea:	c3                   	ret    
	...

801035ec <p2v>:
801035ec:	55                   	push   %ebp
801035ed:	89 e5                	mov    %esp,%ebp
801035ef:	8b 45 08             	mov    0x8(%ebp),%eax
801035f2:	05 00 00 00 80       	add    $0x80000000,%eax
801035f7:	5d                   	pop    %ebp
801035f8:	c3                   	ret    

801035f9 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801035f9:	55                   	push   %ebp
801035fa:	89 e5                	mov    %esp,%ebp
801035fc:	53                   	push   %ebx
801035fd:	83 ec 14             	sub    $0x14,%esp
80103600:	8b 45 08             	mov    0x8(%ebp),%eax
80103603:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103607:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
8010360b:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
8010360f:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80103613:	ec                   	in     (%dx),%al
80103614:	89 c3                	mov    %eax,%ebx
80103616:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80103619:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
8010361d:	83 c4 14             	add    $0x14,%esp
80103620:	5b                   	pop    %ebx
80103621:	5d                   	pop    %ebp
80103622:	c3                   	ret    

80103623 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103623:	55                   	push   %ebp
80103624:	89 e5                	mov    %esp,%ebp
80103626:	83 ec 08             	sub    $0x8,%esp
80103629:	8b 55 08             	mov    0x8(%ebp),%edx
8010362c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010362f:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103633:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103636:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
8010363a:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
8010363e:	ee                   	out    %al,(%dx)
}
8010363f:	c9                   	leave  
80103640:	c3                   	ret    

80103641 <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
80103641:	55                   	push   %ebp
80103642:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
80103644:	a1 44 b6 10 80       	mov    0x8010b644,%eax
80103649:	89 c2                	mov    %eax,%edx
8010364b:	b8 20 f9 10 80       	mov    $0x8010f920,%eax
80103650:	89 d1                	mov    %edx,%ecx
80103652:	29 c1                	sub    %eax,%ecx
80103654:	89 c8                	mov    %ecx,%eax
80103656:	c1 f8 02             	sar    $0x2,%eax
80103659:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
8010365f:	5d                   	pop    %ebp
80103660:	c3                   	ret    

80103661 <sum>:

static uchar
sum(uchar *addr, int len)
{
80103661:	55                   	push   %ebp
80103662:	89 e5                	mov    %esp,%ebp
80103664:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
80103667:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
8010366e:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103675:	eb 13                	jmp    8010368a <sum+0x29>
    sum += addr[i];
80103677:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010367a:	03 45 08             	add    0x8(%ebp),%eax
8010367d:	0f b6 00             	movzbl (%eax),%eax
80103680:	0f b6 c0             	movzbl %al,%eax
80103683:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
80103686:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010368a:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010368d:	3b 45 0c             	cmp    0xc(%ebp),%eax
80103690:	7c e5                	jl     80103677 <sum+0x16>
    sum += addr[i];
  return sum;
80103692:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103695:	c9                   	leave  
80103696:	c3                   	ret    

80103697 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103697:	55                   	push   %ebp
80103698:	89 e5                	mov    %esp,%ebp
8010369a:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
8010369d:	8b 45 08             	mov    0x8(%ebp),%eax
801036a0:	89 04 24             	mov    %eax,(%esp)
801036a3:	e8 44 ff ff ff       	call   801035ec <p2v>
801036a8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
801036ab:	8b 45 0c             	mov    0xc(%ebp),%eax
801036ae:	03 45 f0             	add    -0x10(%ebp),%eax
801036b1:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
801036b4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036b7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801036ba:	eb 3f                	jmp    801036fb <mpsearch1+0x64>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801036bc:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801036c3:	00 
801036c4:	c7 44 24 04 80 88 10 	movl   $0x80108880,0x4(%esp)
801036cb:	80 
801036cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036cf:	89 04 24             	mov    %eax,(%esp)
801036d2:	e8 a6 1a 00 00       	call   8010517d <memcmp>
801036d7:	85 c0                	test   %eax,%eax
801036d9:	75 1c                	jne    801036f7 <mpsearch1+0x60>
801036db:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801036e2:	00 
801036e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036e6:	89 04 24             	mov    %eax,(%esp)
801036e9:	e8 73 ff ff ff       	call   80103661 <sum>
801036ee:	84 c0                	test   %al,%al
801036f0:	75 05                	jne    801036f7 <mpsearch1+0x60>
      return (struct mp*)p;
801036f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036f5:	eb 11                	jmp    80103708 <mpsearch1+0x71>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
801036f7:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
801036fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036fe:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103701:	72 b9                	jb     801036bc <mpsearch1+0x25>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
80103703:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103708:	c9                   	leave  
80103709:	c3                   	ret    

8010370a <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
8010370a:	55                   	push   %ebp
8010370b:	89 e5                	mov    %esp,%ebp
8010370d:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
80103710:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103717:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010371a:	83 c0 0f             	add    $0xf,%eax
8010371d:	0f b6 00             	movzbl (%eax),%eax
80103720:	0f b6 c0             	movzbl %al,%eax
80103723:	89 c2                	mov    %eax,%edx
80103725:	c1 e2 08             	shl    $0x8,%edx
80103728:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010372b:	83 c0 0e             	add    $0xe,%eax
8010372e:	0f b6 00             	movzbl (%eax),%eax
80103731:	0f b6 c0             	movzbl %al,%eax
80103734:	09 d0                	or     %edx,%eax
80103736:	c1 e0 04             	shl    $0x4,%eax
80103739:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010373c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103740:	74 21                	je     80103763 <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
80103742:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103749:	00 
8010374a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010374d:	89 04 24             	mov    %eax,(%esp)
80103750:	e8 42 ff ff ff       	call   80103697 <mpsearch1>
80103755:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103758:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010375c:	74 50                	je     801037ae <mpsearch+0xa4>
      return mp;
8010375e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103761:	eb 5f                	jmp    801037c2 <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103763:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103766:	83 c0 14             	add    $0x14,%eax
80103769:	0f b6 00             	movzbl (%eax),%eax
8010376c:	0f b6 c0             	movzbl %al,%eax
8010376f:	89 c2                	mov    %eax,%edx
80103771:	c1 e2 08             	shl    $0x8,%edx
80103774:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103777:	83 c0 13             	add    $0x13,%eax
8010377a:	0f b6 00             	movzbl (%eax),%eax
8010377d:	0f b6 c0             	movzbl %al,%eax
80103780:	09 d0                	or     %edx,%eax
80103782:	c1 e0 0a             	shl    $0xa,%eax
80103785:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103788:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010378b:	2d 00 04 00 00       	sub    $0x400,%eax
80103790:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103797:	00 
80103798:	89 04 24             	mov    %eax,(%esp)
8010379b:	e8 f7 fe ff ff       	call   80103697 <mpsearch1>
801037a0:	89 45 ec             	mov    %eax,-0x14(%ebp)
801037a3:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801037a7:	74 05                	je     801037ae <mpsearch+0xa4>
      return mp;
801037a9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801037ac:	eb 14                	jmp    801037c2 <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
801037ae:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
801037b5:	00 
801037b6:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
801037bd:	e8 d5 fe ff ff       	call   80103697 <mpsearch1>
}
801037c2:	c9                   	leave  
801037c3:	c3                   	ret    

801037c4 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
801037c4:	55                   	push   %ebp
801037c5:	89 e5                	mov    %esp,%ebp
801037c7:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801037ca:	e8 3b ff ff ff       	call   8010370a <mpsearch>
801037cf:	89 45 f4             	mov    %eax,-0xc(%ebp)
801037d2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801037d6:	74 0a                	je     801037e2 <mpconfig+0x1e>
801037d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037db:	8b 40 04             	mov    0x4(%eax),%eax
801037de:	85 c0                	test   %eax,%eax
801037e0:	75 0a                	jne    801037ec <mpconfig+0x28>
    return 0;
801037e2:	b8 00 00 00 00       	mov    $0x0,%eax
801037e7:	e9 83 00 00 00       	jmp    8010386f <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
801037ec:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037ef:	8b 40 04             	mov    0x4(%eax),%eax
801037f2:	89 04 24             	mov    %eax,(%esp)
801037f5:	e8 f2 fd ff ff       	call   801035ec <p2v>
801037fa:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
801037fd:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103804:	00 
80103805:	c7 44 24 04 85 88 10 	movl   $0x80108885,0x4(%esp)
8010380c:	80 
8010380d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103810:	89 04 24             	mov    %eax,(%esp)
80103813:	e8 65 19 00 00       	call   8010517d <memcmp>
80103818:	85 c0                	test   %eax,%eax
8010381a:	74 07                	je     80103823 <mpconfig+0x5f>
    return 0;
8010381c:	b8 00 00 00 00       	mov    $0x0,%eax
80103821:	eb 4c                	jmp    8010386f <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
80103823:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103826:	0f b6 40 06          	movzbl 0x6(%eax),%eax
8010382a:	3c 01                	cmp    $0x1,%al
8010382c:	74 12                	je     80103840 <mpconfig+0x7c>
8010382e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103831:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103835:	3c 04                	cmp    $0x4,%al
80103837:	74 07                	je     80103840 <mpconfig+0x7c>
    return 0;
80103839:	b8 00 00 00 00       	mov    $0x0,%eax
8010383e:	eb 2f                	jmp    8010386f <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
80103840:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103843:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103847:	0f b7 c0             	movzwl %ax,%eax
8010384a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010384e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103851:	89 04 24             	mov    %eax,(%esp)
80103854:	e8 08 fe ff ff       	call   80103661 <sum>
80103859:	84 c0                	test   %al,%al
8010385b:	74 07                	je     80103864 <mpconfig+0xa0>
    return 0;
8010385d:	b8 00 00 00 00       	mov    $0x0,%eax
80103862:	eb 0b                	jmp    8010386f <mpconfig+0xab>
  *pmp = mp;
80103864:	8b 45 08             	mov    0x8(%ebp),%eax
80103867:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010386a:	89 10                	mov    %edx,(%eax)
  return conf;
8010386c:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010386f:	c9                   	leave  
80103870:	c3                   	ret    

80103871 <mpinit>:

void
mpinit(void)
{
80103871:	55                   	push   %ebp
80103872:	89 e5                	mov    %esp,%ebp
80103874:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80103877:	c7 05 44 b6 10 80 20 	movl   $0x8010f920,0x8010b644
8010387e:	f9 10 80 
  if((conf = mpconfig(&mp)) == 0)
80103881:	8d 45 e0             	lea    -0x20(%ebp),%eax
80103884:	89 04 24             	mov    %eax,(%esp)
80103887:	e8 38 ff ff ff       	call   801037c4 <mpconfig>
8010388c:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010388f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103893:	0f 84 9c 01 00 00    	je     80103a35 <mpinit+0x1c4>
    return;
  ismp = 1;
80103899:	c7 05 04 f9 10 80 01 	movl   $0x1,0x8010f904
801038a0:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
801038a3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038a6:	8b 40 24             	mov    0x24(%eax),%eax
801038a9:	a3 7c f8 10 80       	mov    %eax,0x8010f87c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801038ae:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038b1:	83 c0 2c             	add    $0x2c,%eax
801038b4:	89 45 f4             	mov    %eax,-0xc(%ebp)
801038b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038ba:	0f b7 40 04          	movzwl 0x4(%eax),%eax
801038be:	0f b7 c0             	movzwl %ax,%eax
801038c1:	03 45 f0             	add    -0x10(%ebp),%eax
801038c4:	89 45 ec             	mov    %eax,-0x14(%ebp)
801038c7:	e9 f4 00 00 00       	jmp    801039c0 <mpinit+0x14f>
    switch(*p){
801038cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038cf:	0f b6 00             	movzbl (%eax),%eax
801038d2:	0f b6 c0             	movzbl %al,%eax
801038d5:	83 f8 04             	cmp    $0x4,%eax
801038d8:	0f 87 bf 00 00 00    	ja     8010399d <mpinit+0x12c>
801038de:	8b 04 85 c8 88 10 80 	mov    -0x7fef7738(,%eax,4),%eax
801038e5:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
801038e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038ea:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
801038ed:	8b 45 e8             	mov    -0x18(%ebp),%eax
801038f0:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801038f4:	0f b6 d0             	movzbl %al,%edx
801038f7:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801038fc:	39 c2                	cmp    %eax,%edx
801038fe:	74 2d                	je     8010392d <mpinit+0xbc>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
80103900:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103903:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103907:	0f b6 d0             	movzbl %al,%edx
8010390a:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010390f:	89 54 24 08          	mov    %edx,0x8(%esp)
80103913:	89 44 24 04          	mov    %eax,0x4(%esp)
80103917:	c7 04 24 8a 88 10 80 	movl   $0x8010888a,(%esp)
8010391e:	e8 7e ca ff ff       	call   801003a1 <cprintf>
        ismp = 0;
80103923:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
8010392a:	00 00 00 
      }
      if(proc->flags & MPBOOT)
8010392d:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103930:	0f b6 40 03          	movzbl 0x3(%eax),%eax
80103934:	0f b6 c0             	movzbl %al,%eax
80103937:	83 e0 02             	and    $0x2,%eax
8010393a:	85 c0                	test   %eax,%eax
8010393c:	74 15                	je     80103953 <mpinit+0xe2>
        bcpu = &cpus[ncpu];
8010393e:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103943:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103949:	05 20 f9 10 80       	add    $0x8010f920,%eax
8010394e:	a3 44 b6 10 80       	mov    %eax,0x8010b644
      cpus[ncpu].id = ncpu;
80103953:	8b 15 00 ff 10 80    	mov    0x8010ff00,%edx
80103959:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010395e:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80103964:	81 c2 20 f9 10 80    	add    $0x8010f920,%edx
8010396a:	88 02                	mov    %al,(%edx)
      ncpu++;
8010396c:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103971:	83 c0 01             	add    $0x1,%eax
80103974:	a3 00 ff 10 80       	mov    %eax,0x8010ff00
      p += sizeof(struct mpproc);
80103979:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
8010397d:	eb 41                	jmp    801039c0 <mpinit+0x14f>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
8010397f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103982:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
80103985:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103988:	0f b6 40 01          	movzbl 0x1(%eax),%eax
8010398c:	a2 00 f9 10 80       	mov    %al,0x8010f900
      p += sizeof(struct mpioapic);
80103991:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103995:	eb 29                	jmp    801039c0 <mpinit+0x14f>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
80103997:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
8010399b:	eb 23                	jmp    801039c0 <mpinit+0x14f>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
8010399d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039a0:	0f b6 00             	movzbl (%eax),%eax
801039a3:	0f b6 c0             	movzbl %al,%eax
801039a6:	89 44 24 04          	mov    %eax,0x4(%esp)
801039aa:	c7 04 24 a8 88 10 80 	movl   $0x801088a8,(%esp)
801039b1:	e8 eb c9 ff ff       	call   801003a1 <cprintf>
      ismp = 0;
801039b6:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
801039bd:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801039c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039c3:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801039c6:	0f 82 00 ff ff ff    	jb     801038cc <mpinit+0x5b>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
801039cc:	a1 04 f9 10 80       	mov    0x8010f904,%eax
801039d1:	85 c0                	test   %eax,%eax
801039d3:	75 1d                	jne    801039f2 <mpinit+0x181>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
801039d5:	c7 05 00 ff 10 80 01 	movl   $0x1,0x8010ff00
801039dc:	00 00 00 
    lapic = 0;
801039df:	c7 05 7c f8 10 80 00 	movl   $0x0,0x8010f87c
801039e6:	00 00 00 
    ioapicid = 0;
801039e9:	c6 05 00 f9 10 80 00 	movb   $0x0,0x8010f900
    return;
801039f0:	eb 44                	jmp    80103a36 <mpinit+0x1c5>
  }

  if(mp->imcrp){
801039f2:	8b 45 e0             	mov    -0x20(%ebp),%eax
801039f5:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
801039f9:	84 c0                	test   %al,%al
801039fb:	74 39                	je     80103a36 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
801039fd:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103a04:	00 
80103a05:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103a0c:	e8 12 fc ff ff       	call   80103623 <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103a11:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a18:	e8 dc fb ff ff       	call   801035f9 <inb>
80103a1d:	83 c8 01             	or     $0x1,%eax
80103a20:	0f b6 c0             	movzbl %al,%eax
80103a23:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a27:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a2e:	e8 f0 fb ff ff       	call   80103623 <outb>
80103a33:	eb 01                	jmp    80103a36 <mpinit+0x1c5>
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
80103a35:	90                   	nop
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
  }
}
80103a36:	c9                   	leave  
80103a37:	c3                   	ret    

80103a38 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103a38:	55                   	push   %ebp
80103a39:	89 e5                	mov    %esp,%ebp
80103a3b:	83 ec 08             	sub    $0x8,%esp
80103a3e:	8b 55 08             	mov    0x8(%ebp),%edx
80103a41:	8b 45 0c             	mov    0xc(%ebp),%eax
80103a44:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103a48:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103a4b:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103a4f:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103a53:	ee                   	out    %al,(%dx)
}
80103a54:	c9                   	leave  
80103a55:	c3                   	ret    

80103a56 <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80103a56:	55                   	push   %ebp
80103a57:	89 e5                	mov    %esp,%ebp
80103a59:	83 ec 0c             	sub    $0xc,%esp
80103a5c:	8b 45 08             	mov    0x8(%ebp),%eax
80103a5f:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
80103a63:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a67:	66 a3 00 b0 10 80    	mov    %ax,0x8010b000
  outb(IO_PIC1+1, mask);
80103a6d:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a71:	0f b6 c0             	movzbl %al,%eax
80103a74:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a78:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103a7f:	e8 b4 ff ff ff       	call   80103a38 <outb>
  outb(IO_PIC2+1, mask >> 8);
80103a84:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a88:	66 c1 e8 08          	shr    $0x8,%ax
80103a8c:	0f b6 c0             	movzbl %al,%eax
80103a8f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a93:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103a9a:	e8 99 ff ff ff       	call   80103a38 <outb>
}
80103a9f:	c9                   	leave  
80103aa0:	c3                   	ret    

80103aa1 <picenable>:

void
picenable(int irq)
{
80103aa1:	55                   	push   %ebp
80103aa2:	89 e5                	mov    %esp,%ebp
80103aa4:	53                   	push   %ebx
80103aa5:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
80103aa8:	8b 45 08             	mov    0x8(%ebp),%eax
80103aab:	ba 01 00 00 00       	mov    $0x1,%edx
80103ab0:	89 d3                	mov    %edx,%ebx
80103ab2:	89 c1                	mov    %eax,%ecx
80103ab4:	d3 e3                	shl    %cl,%ebx
80103ab6:	89 d8                	mov    %ebx,%eax
80103ab8:	89 c2                	mov    %eax,%edx
80103aba:	f7 d2                	not    %edx
80103abc:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103ac3:	21 d0                	and    %edx,%eax
80103ac5:	0f b7 c0             	movzwl %ax,%eax
80103ac8:	89 04 24             	mov    %eax,(%esp)
80103acb:	e8 86 ff ff ff       	call   80103a56 <picsetmask>
}
80103ad0:	83 c4 04             	add    $0x4,%esp
80103ad3:	5b                   	pop    %ebx
80103ad4:	5d                   	pop    %ebp
80103ad5:	c3                   	ret    

80103ad6 <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
80103ad6:	55                   	push   %ebp
80103ad7:	89 e5                	mov    %esp,%ebp
80103ad9:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103adc:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103ae3:	00 
80103ae4:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103aeb:	e8 48 ff ff ff       	call   80103a38 <outb>
  outb(IO_PIC2+1, 0xFF);
80103af0:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103af7:	00 
80103af8:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103aff:	e8 34 ff ff ff       	call   80103a38 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80103b04:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b0b:	00 
80103b0c:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103b13:	e8 20 ff ff ff       	call   80103a38 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80103b18:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103b1f:	00 
80103b20:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b27:	e8 0c ff ff ff       	call   80103a38 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80103b2c:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103b33:	00 
80103b34:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b3b:	e8 f8 fe ff ff       	call   80103a38 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
80103b40:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b47:	00 
80103b48:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b4f:	e8 e4 fe ff ff       	call   80103a38 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80103b54:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b5b:	00 
80103b5c:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103b63:	e8 d0 fe ff ff       	call   80103a38 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80103b68:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103b6f:	00 
80103b70:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b77:	e8 bc fe ff ff       	call   80103a38 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80103b7c:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103b83:	00 
80103b84:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b8b:	e8 a8 fe ff ff       	call   80103a38 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
80103b90:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b97:	00 
80103b98:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b9f:	e8 94 fe ff ff       	call   80103a38 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
80103ba4:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103bab:	00 
80103bac:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bb3:	e8 80 fe ff ff       	call   80103a38 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
80103bb8:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103bbf:	00 
80103bc0:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bc7:	e8 6c fe ff ff       	call   80103a38 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
80103bcc:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103bd3:	00 
80103bd4:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103bdb:	e8 58 fe ff ff       	call   80103a38 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
80103be0:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103be7:	00 
80103be8:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103bef:	e8 44 fe ff ff       	call   80103a38 <outb>

  if(irqmask != 0xFFFF)
80103bf4:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103bfb:	66 83 f8 ff          	cmp    $0xffff,%ax
80103bff:	74 12                	je     80103c13 <picinit+0x13d>
    picsetmask(irqmask);
80103c01:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103c08:	0f b7 c0             	movzwl %ax,%eax
80103c0b:	89 04 24             	mov    %eax,(%esp)
80103c0e:	e8 43 fe ff ff       	call   80103a56 <picsetmask>
}
80103c13:	c9                   	leave  
80103c14:	c3                   	ret    
80103c15:	00 00                	add    %al,(%eax)
	...

80103c18 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103c18:	55                   	push   %ebp
80103c19:	89 e5                	mov    %esp,%ebp
80103c1b:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103c1e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103c25:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c28:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103c2e:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c31:	8b 10                	mov    (%eax),%edx
80103c33:	8b 45 08             	mov    0x8(%ebp),%eax
80103c36:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103c38:	e8 df d2 ff ff       	call   80100f1c <filealloc>
80103c3d:	8b 55 08             	mov    0x8(%ebp),%edx
80103c40:	89 02                	mov    %eax,(%edx)
80103c42:	8b 45 08             	mov    0x8(%ebp),%eax
80103c45:	8b 00                	mov    (%eax),%eax
80103c47:	85 c0                	test   %eax,%eax
80103c49:	0f 84 c8 00 00 00    	je     80103d17 <pipealloc+0xff>
80103c4f:	e8 c8 d2 ff ff       	call   80100f1c <filealloc>
80103c54:	8b 55 0c             	mov    0xc(%ebp),%edx
80103c57:	89 02                	mov    %eax,(%edx)
80103c59:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c5c:	8b 00                	mov    (%eax),%eax
80103c5e:	85 c0                	test   %eax,%eax
80103c60:	0f 84 b1 00 00 00    	je     80103d17 <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103c66:	e8 94 ee ff ff       	call   80102aff <kalloc>
80103c6b:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103c6e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103c72:	0f 84 9e 00 00 00    	je     80103d16 <pipealloc+0xfe>
    goto bad;
  p->readopen = 1;
80103c78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c7b:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103c82:	00 00 00 
  p->writeopen = 1;
80103c85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c88:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103c8f:	00 00 00 
  p->nwrite = 0;
80103c92:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c95:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103c9c:	00 00 00 
  p->nread = 0;
80103c9f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ca2:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103ca9:	00 00 00 
  initlock(&p->lock, "pipe");
80103cac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103caf:	c7 44 24 04 dc 88 10 	movl   $0x801088dc,0x4(%esp)
80103cb6:	80 
80103cb7:	89 04 24             	mov    %eax,(%esp)
80103cba:	e8 d7 11 00 00       	call   80104e96 <initlock>
  (*f0)->type = FD_PIPE;
80103cbf:	8b 45 08             	mov    0x8(%ebp),%eax
80103cc2:	8b 00                	mov    (%eax),%eax
80103cc4:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103cca:	8b 45 08             	mov    0x8(%ebp),%eax
80103ccd:	8b 00                	mov    (%eax),%eax
80103ccf:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103cd3:	8b 45 08             	mov    0x8(%ebp),%eax
80103cd6:	8b 00                	mov    (%eax),%eax
80103cd8:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103cdc:	8b 45 08             	mov    0x8(%ebp),%eax
80103cdf:	8b 00                	mov    (%eax),%eax
80103ce1:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103ce4:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103ce7:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cea:	8b 00                	mov    (%eax),%eax
80103cec:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103cf2:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cf5:	8b 00                	mov    (%eax),%eax
80103cf7:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103cfb:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cfe:	8b 00                	mov    (%eax),%eax
80103d00:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103d04:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d07:	8b 00                	mov    (%eax),%eax
80103d09:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103d0c:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103d0f:	b8 00 00 00 00       	mov    $0x0,%eax
80103d14:	eb 43                	jmp    80103d59 <pipealloc+0x141>
  p = 0;
  *f0 = *f1 = 0;
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
    goto bad;
80103d16:	90                   	nop
  (*f1)->pipe = p;
  return 0;

//PAGEBREAK: 20
 bad:
  if(p)
80103d17:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103d1b:	74 0b                	je     80103d28 <pipealloc+0x110>
    kfree((char*)p);
80103d1d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d20:	89 04 24             	mov    %eax,(%esp)
80103d23:	e8 3e ed ff ff       	call   80102a66 <kfree>
  if(*f0)
80103d28:	8b 45 08             	mov    0x8(%ebp),%eax
80103d2b:	8b 00                	mov    (%eax),%eax
80103d2d:	85 c0                	test   %eax,%eax
80103d2f:	74 0d                	je     80103d3e <pipealloc+0x126>
    fileclose(*f0);
80103d31:	8b 45 08             	mov    0x8(%ebp),%eax
80103d34:	8b 00                	mov    (%eax),%eax
80103d36:	89 04 24             	mov    %eax,(%esp)
80103d39:	e8 86 d2 ff ff       	call   80100fc4 <fileclose>
  if(*f1)
80103d3e:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d41:	8b 00                	mov    (%eax),%eax
80103d43:	85 c0                	test   %eax,%eax
80103d45:	74 0d                	je     80103d54 <pipealloc+0x13c>
    fileclose(*f1);
80103d47:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d4a:	8b 00                	mov    (%eax),%eax
80103d4c:	89 04 24             	mov    %eax,(%esp)
80103d4f:	e8 70 d2 ff ff       	call   80100fc4 <fileclose>
  return -1;
80103d54:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103d59:	c9                   	leave  
80103d5a:	c3                   	ret    

80103d5b <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103d5b:	55                   	push   %ebp
80103d5c:	89 e5                	mov    %esp,%ebp
80103d5e:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103d61:	8b 45 08             	mov    0x8(%ebp),%eax
80103d64:	89 04 24             	mov    %eax,(%esp)
80103d67:	e8 4b 11 00 00       	call   80104eb7 <acquire>
  if(writable){
80103d6c:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103d70:	74 1f                	je     80103d91 <pipeclose+0x36>
    p->writeopen = 0;
80103d72:	8b 45 08             	mov    0x8(%ebp),%eax
80103d75:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103d7c:	00 00 00 
    wakeup(&p->nread);
80103d7f:	8b 45 08             	mov    0x8(%ebp),%eax
80103d82:	05 34 02 00 00       	add    $0x234,%eax
80103d87:	89 04 24             	mov    %eax,(%esp)
80103d8a:	e8 1f 0f 00 00       	call   80104cae <wakeup>
80103d8f:	eb 1d                	jmp    80103dae <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103d91:	8b 45 08             	mov    0x8(%ebp),%eax
80103d94:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103d9b:	00 00 00 
    wakeup(&p->nwrite);
80103d9e:	8b 45 08             	mov    0x8(%ebp),%eax
80103da1:	05 38 02 00 00       	add    $0x238,%eax
80103da6:	89 04 24             	mov    %eax,(%esp)
80103da9:	e8 00 0f 00 00       	call   80104cae <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103dae:	8b 45 08             	mov    0x8(%ebp),%eax
80103db1:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103db7:	85 c0                	test   %eax,%eax
80103db9:	75 25                	jne    80103de0 <pipeclose+0x85>
80103dbb:	8b 45 08             	mov    0x8(%ebp),%eax
80103dbe:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103dc4:	85 c0                	test   %eax,%eax
80103dc6:	75 18                	jne    80103de0 <pipeclose+0x85>
    release(&p->lock);
80103dc8:	8b 45 08             	mov    0x8(%ebp),%eax
80103dcb:	89 04 24             	mov    %eax,(%esp)
80103dce:	e8 46 11 00 00       	call   80104f19 <release>
    kfree((char*)p);
80103dd3:	8b 45 08             	mov    0x8(%ebp),%eax
80103dd6:	89 04 24             	mov    %eax,(%esp)
80103dd9:	e8 88 ec ff ff       	call   80102a66 <kfree>
80103dde:	eb 0b                	jmp    80103deb <pipeclose+0x90>
  } else
    release(&p->lock);
80103de0:	8b 45 08             	mov    0x8(%ebp),%eax
80103de3:	89 04 24             	mov    %eax,(%esp)
80103de6:	e8 2e 11 00 00       	call   80104f19 <release>
}
80103deb:	c9                   	leave  
80103dec:	c3                   	ret    

80103ded <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103ded:	55                   	push   %ebp
80103dee:	89 e5                	mov    %esp,%ebp
80103df0:	53                   	push   %ebx
80103df1:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103df4:	8b 45 08             	mov    0x8(%ebp),%eax
80103df7:	89 04 24             	mov    %eax,(%esp)
80103dfa:	e8 b8 10 00 00       	call   80104eb7 <acquire>
  for(i = 0; i < n; i++){
80103dff:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103e06:	e9 a6 00 00 00       	jmp    80103eb1 <pipewrite+0xc4>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || proc->killed){
80103e0b:	8b 45 08             	mov    0x8(%ebp),%eax
80103e0e:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103e14:	85 c0                	test   %eax,%eax
80103e16:	74 0d                	je     80103e25 <pipewrite+0x38>
80103e18:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103e1e:	8b 40 24             	mov    0x24(%eax),%eax
80103e21:	85 c0                	test   %eax,%eax
80103e23:	74 15                	je     80103e3a <pipewrite+0x4d>
        release(&p->lock);
80103e25:	8b 45 08             	mov    0x8(%ebp),%eax
80103e28:	89 04 24             	mov    %eax,(%esp)
80103e2b:	e8 e9 10 00 00       	call   80104f19 <release>
        return -1;
80103e30:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103e35:	e9 9d 00 00 00       	jmp    80103ed7 <pipewrite+0xea>
      }
      wakeup(&p->nread);
80103e3a:	8b 45 08             	mov    0x8(%ebp),%eax
80103e3d:	05 34 02 00 00       	add    $0x234,%eax
80103e42:	89 04 24             	mov    %eax,(%esp)
80103e45:	e8 64 0e 00 00       	call   80104cae <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103e4a:	8b 45 08             	mov    0x8(%ebp),%eax
80103e4d:	8b 55 08             	mov    0x8(%ebp),%edx
80103e50:	81 c2 38 02 00 00    	add    $0x238,%edx
80103e56:	89 44 24 04          	mov    %eax,0x4(%esp)
80103e5a:	89 14 24             	mov    %edx,(%esp)
80103e5d:	e8 3e 0c 00 00       	call   80104aa0 <sleep>
80103e62:	eb 01                	jmp    80103e65 <pipewrite+0x78>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103e64:	90                   	nop
80103e65:	8b 45 08             	mov    0x8(%ebp),%eax
80103e68:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103e6e:	8b 45 08             	mov    0x8(%ebp),%eax
80103e71:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103e77:	05 00 02 00 00       	add    $0x200,%eax
80103e7c:	39 c2                	cmp    %eax,%edx
80103e7e:	74 8b                	je     80103e0b <pipewrite+0x1e>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103e80:	8b 45 08             	mov    0x8(%ebp),%eax
80103e83:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103e89:	89 c3                	mov    %eax,%ebx
80103e8b:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103e91:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103e94:	03 55 0c             	add    0xc(%ebp),%edx
80103e97:	0f b6 0a             	movzbl (%edx),%ecx
80103e9a:	8b 55 08             	mov    0x8(%ebp),%edx
80103e9d:	88 4c 1a 34          	mov    %cl,0x34(%edx,%ebx,1)
80103ea1:	8d 50 01             	lea    0x1(%eax),%edx
80103ea4:	8b 45 08             	mov    0x8(%ebp),%eax
80103ea7:	89 90 38 02 00 00    	mov    %edx,0x238(%eax)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
80103ead:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103eb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103eb4:	3b 45 10             	cmp    0x10(%ebp),%eax
80103eb7:	7c ab                	jl     80103e64 <pipewrite+0x77>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103eb9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ebc:	05 34 02 00 00       	add    $0x234,%eax
80103ec1:	89 04 24             	mov    %eax,(%esp)
80103ec4:	e8 e5 0d 00 00       	call   80104cae <wakeup>
  release(&p->lock);
80103ec9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ecc:	89 04 24             	mov    %eax,(%esp)
80103ecf:	e8 45 10 00 00       	call   80104f19 <release>
  return n;
80103ed4:	8b 45 10             	mov    0x10(%ebp),%eax
}
80103ed7:	83 c4 24             	add    $0x24,%esp
80103eda:	5b                   	pop    %ebx
80103edb:	5d                   	pop    %ebp
80103edc:	c3                   	ret    

80103edd <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103edd:	55                   	push   %ebp
80103ede:	89 e5                	mov    %esp,%ebp
80103ee0:	53                   	push   %ebx
80103ee1:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103ee4:	8b 45 08             	mov    0x8(%ebp),%eax
80103ee7:	89 04 24             	mov    %eax,(%esp)
80103eea:	e8 c8 0f 00 00       	call   80104eb7 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103eef:	eb 3a                	jmp    80103f2b <piperead+0x4e>
    if(proc->killed){
80103ef1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103ef7:	8b 40 24             	mov    0x24(%eax),%eax
80103efa:	85 c0                	test   %eax,%eax
80103efc:	74 15                	je     80103f13 <piperead+0x36>
      release(&p->lock);
80103efe:	8b 45 08             	mov    0x8(%ebp),%eax
80103f01:	89 04 24             	mov    %eax,(%esp)
80103f04:	e8 10 10 00 00       	call   80104f19 <release>
      return -1;
80103f09:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103f0e:	e9 b6 00 00 00       	jmp    80103fc9 <piperead+0xec>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103f13:	8b 45 08             	mov    0x8(%ebp),%eax
80103f16:	8b 55 08             	mov    0x8(%ebp),%edx
80103f19:	81 c2 34 02 00 00    	add    $0x234,%edx
80103f1f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103f23:	89 14 24             	mov    %edx,(%esp)
80103f26:	e8 75 0b 00 00       	call   80104aa0 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103f2b:	8b 45 08             	mov    0x8(%ebp),%eax
80103f2e:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f34:	8b 45 08             	mov    0x8(%ebp),%eax
80103f37:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f3d:	39 c2                	cmp    %eax,%edx
80103f3f:	75 0d                	jne    80103f4e <piperead+0x71>
80103f41:	8b 45 08             	mov    0x8(%ebp),%eax
80103f44:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103f4a:	85 c0                	test   %eax,%eax
80103f4c:	75 a3                	jne    80103ef1 <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f4e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103f55:	eb 49                	jmp    80103fa0 <piperead+0xc3>
    if(p->nread == p->nwrite)
80103f57:	8b 45 08             	mov    0x8(%ebp),%eax
80103f5a:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f60:	8b 45 08             	mov    0x8(%ebp),%eax
80103f63:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f69:	39 c2                	cmp    %eax,%edx
80103f6b:	74 3d                	je     80103faa <piperead+0xcd>
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
80103f6d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f70:	89 c2                	mov    %eax,%edx
80103f72:	03 55 0c             	add    0xc(%ebp),%edx
80103f75:	8b 45 08             	mov    0x8(%ebp),%eax
80103f78:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103f7e:	89 c3                	mov    %eax,%ebx
80103f80:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103f86:	8b 4d 08             	mov    0x8(%ebp),%ecx
80103f89:	0f b6 4c 19 34       	movzbl 0x34(%ecx,%ebx,1),%ecx
80103f8e:	88 0a                	mov    %cl,(%edx)
80103f90:	8d 50 01             	lea    0x1(%eax),%edx
80103f93:	8b 45 08             	mov    0x8(%ebp),%eax
80103f96:	89 90 34 02 00 00    	mov    %edx,0x234(%eax)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f9c:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103fa0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103fa3:	3b 45 10             	cmp    0x10(%ebp),%eax
80103fa6:	7c af                	jl     80103f57 <piperead+0x7a>
80103fa8:	eb 01                	jmp    80103fab <piperead+0xce>
    if(p->nread == p->nwrite)
      break;
80103faa:	90                   	nop
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80103fab:	8b 45 08             	mov    0x8(%ebp),%eax
80103fae:	05 38 02 00 00       	add    $0x238,%eax
80103fb3:	89 04 24             	mov    %eax,(%esp)
80103fb6:	e8 f3 0c 00 00       	call   80104cae <wakeup>
  release(&p->lock);
80103fbb:	8b 45 08             	mov    0x8(%ebp),%eax
80103fbe:	89 04 24             	mov    %eax,(%esp)
80103fc1:	e8 53 0f 00 00       	call   80104f19 <release>
  return i;
80103fc6:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80103fc9:	83 c4 24             	add    $0x24,%esp
80103fcc:	5b                   	pop    %ebx
80103fcd:	5d                   	pop    %ebp
80103fce:	c3                   	ret    
	...

80103fd0 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80103fd0:	55                   	push   %ebp
80103fd1:	89 e5                	mov    %esp,%ebp
80103fd3:	53                   	push   %ebx
80103fd4:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103fd7:	9c                   	pushf  
80103fd8:	5b                   	pop    %ebx
80103fd9:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80103fdc:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103fdf:	83 c4 10             	add    $0x10,%esp
80103fe2:	5b                   	pop    %ebx
80103fe3:	5d                   	pop    %ebp
80103fe4:	c3                   	ret    

80103fe5 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
80103fe5:	55                   	push   %ebp
80103fe6:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80103fe8:	fb                   	sti    
}
80103fe9:	5d                   	pop    %ebp
80103fea:	c3                   	ret    

80103feb <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
80103feb:	55                   	push   %ebp
80103fec:	89 e5                	mov    %esp,%ebp
80103fee:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
80103ff1:	c7 44 24 04 e1 88 10 	movl   $0x801088e1,0x4(%esp)
80103ff8:	80 
80103ff9:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104000:	e8 91 0e 00 00       	call   80104e96 <initlock>
}
80104005:	c9                   	leave  
80104006:	c3                   	ret    

80104007 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
80104007:	55                   	push   %ebp
80104008:	89 e5                	mov    %esp,%ebp
8010400a:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
8010400d:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104014:	e8 9e 0e 00 00       	call   80104eb7 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104019:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104020:	eb 11                	jmp    80104033 <allocproc+0x2c>
    if(p->state == UNUSED)
80104022:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104025:	8b 40 0c             	mov    0xc(%eax),%eax
80104028:	85 c0                	test   %eax,%eax
8010402a:	74 26                	je     80104052 <allocproc+0x4b>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010402c:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
80104033:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
8010403a:	72 e6                	jb     80104022 <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
8010403c:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104043:	e8 d1 0e 00 00       	call   80104f19 <release>
  return 0;
80104048:	b8 00 00 00 00       	mov    $0x0,%eax
8010404d:	e9 b5 00 00 00       	jmp    80104107 <allocproc+0x100>
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == UNUSED)
      goto found;
80104052:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80104053:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104056:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
8010405d:	a1 04 b0 10 80       	mov    0x8010b004,%eax
80104062:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104065:	89 42 10             	mov    %eax,0x10(%edx)
80104068:	83 c0 01             	add    $0x1,%eax
8010406b:	a3 04 b0 10 80       	mov    %eax,0x8010b004
  release(&ptable.lock);
80104070:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104077:	e8 9d 0e 00 00       	call   80104f19 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
8010407c:	e8 7e ea ff ff       	call   80102aff <kalloc>
80104081:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104084:	89 42 08             	mov    %eax,0x8(%edx)
80104087:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010408a:	8b 40 08             	mov    0x8(%eax),%eax
8010408d:	85 c0                	test   %eax,%eax
8010408f:	75 11                	jne    801040a2 <allocproc+0x9b>
    p->state = UNUSED;
80104091:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104094:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
8010409b:	b8 00 00 00 00       	mov    $0x0,%eax
801040a0:	eb 65                	jmp    80104107 <allocproc+0x100>
  }
  sp = p->kstack + KSTACKSIZE;
801040a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040a5:	8b 40 08             	mov    0x8(%eax),%eax
801040a8:	05 00 10 00 00       	add    $0x1000,%eax
801040ad:	89 45 f0             	mov    %eax,-0x10(%ebp)
  
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801040b0:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
801040b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040b7:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040ba:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
801040bd:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
801040c1:	ba c8 66 10 80       	mov    $0x801066c8,%edx
801040c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040c9:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
801040cb:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
801040cf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040d2:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040d5:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
801040d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040db:	8b 40 1c             	mov    0x1c(%eax),%eax
801040de:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801040e5:	00 
801040e6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801040ed:	00 
801040ee:	89 04 24             	mov    %eax,(%esp)
801040f1:	e8 10 10 00 00       	call   80105106 <memset>
  p->context->eip = (uint)forkret;
801040f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040f9:	8b 40 1c             	mov    0x1c(%eax),%eax
801040fc:	ba 74 4a 10 80       	mov    $0x80104a74,%edx
80104101:	89 50 10             	mov    %edx,0x10(%eax)

  return p;
80104104:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104107:	c9                   	leave  
80104108:	c3                   	ret    

80104109 <createInternalProcess>:


void createInternalProcess(const char *name, void (*entrypoint)())
{
80104109:	55                   	push   %ebp
8010410a:	89 e5                	mov    %esp,%ebp
8010410c:	57                   	push   %edi
8010410d:	56                   	push   %esi
8010410e:	53                   	push   %ebx
8010410f:	83 ec 2c             	sub    $0x2c,%esp
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
80104112:	e8 f0 fe ff ff       	call   80104007 <allocproc>
80104117:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010411a:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010411e:	74 7a                	je     8010419a <createInternalProcess+0x91>
    return;

  // Copy process state from p.
  if((np->pgdir = setupkvm()) == 0){
80104120:	e8 a0 3c 00 00       	call   80107dc5 <setupkvm>
80104125:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104128:	89 42 04             	mov    %eax,0x4(%edx)
      np->state = UNUSED;
      return;
    }*/
  }
  //switchuvm(np);
  np->sz = PGSIZE;
8010412b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010412e:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  np->parent = initproc;
80104134:	8b 15 48 b6 10 80    	mov    0x8010b648,%edx
8010413a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010413d:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *initproc->tf;
80104140:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80104143:	8b 50 18             	mov    0x18(%eax),%edx
80104146:	a1 48 b6 10 80       	mov    0x8010b648,%eax
8010414b:	8b 40 18             	mov    0x18(%eax),%eax
8010414e:	89 c3                	mov    %eax,%ebx
80104150:	b8 13 00 00 00       	mov    $0x13,%eax
80104155:	89 d7                	mov    %edx,%edi
80104157:	89 de                	mov    %ebx,%esi
80104159:	89 c1                	mov    %eax,%ecx
8010415b:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eip = (uint)entrypoint;
8010415d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80104160:	8b 40 18             	mov    0x18(%eax),%eax
80104163:	8b 55 0c             	mov    0xc(%ebp),%edx
80104166:	89 50 38             	mov    %edx,0x38(%eax)
  inswapper = np;
80104169:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010416c:	a3 4c b6 10 80       	mov    %eax,0x8010b64c

  np->state = RUNNABLE;
80104171:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80104174:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  safestrcpy(np->name, name, sizeof(name));
8010417b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010417e:	8d 50 6c             	lea    0x6c(%eax),%edx
80104181:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80104188:	00 
80104189:	8b 45 08             	mov    0x8(%ebp),%eax
8010418c:	89 44 24 04          	mov    %eax,0x4(%esp)
80104190:	89 14 24             	mov    %edx,(%esp)
80104193:	e8 9e 11 00 00       	call   80105336 <safestrcpy>
80104198:	eb 01                	jmp    8010419b <createInternalProcess+0x92>
{
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
    return;
8010419a:	90                   	nop
  inswapper = np;

  np->state = RUNNABLE;
  safestrcpy(np->name, name, sizeof(name));

}
8010419b:	83 c4 2c             	add    $0x2c,%esp
8010419e:	5b                   	pop    %ebx
8010419f:	5e                   	pop    %esi
801041a0:	5f                   	pop    %edi
801041a1:	5d                   	pop    %ebp
801041a2:	c3                   	ret    

801041a3 <swapIn>:

void swapIn()
{
801041a3:	55                   	push   %ebp
801041a4:	89 e5                	mov    %esp,%ebp
801041a6:	81 ec 28 10 00 00    	sub    $0x1028,%esp
  cprintf("swapin\n");
801041ac:	c7 04 24 e8 88 10 80 	movl   $0x801088e8,(%esp)
801041b3:	e8 e9 c1 ff ff       	call   801003a1 <cprintf>
  struct proc* t;
  acquire(&ptable.lock);
801041b8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801041bf:	e8 f3 0c 00 00       	call   80104eb7 <acquire>
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
801041c4:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801041cb:	e9 f4 00 00 00       	jmp    801042c4 <swapIn+0x121>
    {
      if(t->state != RUNNABLE_SUSPENDED)
801041d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041d3:	8b 40 0c             	mov    0xc(%eax),%eax
801041d6:	83 f8 07             	cmp    $0x7,%eax
801041d9:	0f 85 dd 00 00 00    	jne    801042bc <swapIn+0x119>
	continue;
      
      //open file pid.swap
      
      char buf[PGSIZE];
      int read=0;
801041df:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
      
      // allocate virtual memory
      if(!allocuvm(t->pgdir, 0, t->sz))
801041e6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041e9:	8b 10                	mov    (%eax),%edx
801041eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041ee:	8b 40 04             	mov    0x4(%eax),%eax
801041f1:	89 54 24 08          	mov    %edx,0x8(%esp)
801041f5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801041fc:	00 
801041fd:	89 04 24             	mov    %eax,(%esp)
80104200:	e8 92 3f 00 00       	call   80108197 <allocuvm>
80104205:	85 c0                	test   %eax,%eax
80104207:	75 11                	jne    8010421a <swapIn+0x77>
      {
	cprintf("allocuvm failed\n");
80104209:	c7 04 24 f0 88 10 80 	movl   $0x801088f0,(%esp)
80104210:	e8 8c c1 ff ff       	call   801003a1 <cprintf>
	break;
80104215:	e9 b7 00 00 00       	jmp    801042d1 <swapIn+0x12e>
      }
      
      uint a = 0;
8010421a:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
      for(; a < t->sz; a += PGSIZE)
80104221:	eb 68                	jmp    8010428b <swapIn+0xe8>
      {
	if((read = fileread(t->swap, buf, PGSIZE)) > 0)
80104223:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104226:	8b 40 7c             	mov    0x7c(%eax),%eax
80104229:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80104230:	00 
80104231:	8d 95 ec ef ff ff    	lea    -0x1014(%ebp),%edx
80104237:	89 54 24 04          	mov    %edx,0x4(%esp)
8010423b:	89 04 24             	mov    %eax,(%esp)
8010423e:	e8 a6 ce ff ff       	call   801010e9 <fileread>
80104243:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104246:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010424a:	7e 38                	jle    80104284 <swapIn+0xe1>
	{
	  if(copyout(t->pgdir,a, buf, read) < 0)
8010424c:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010424f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104252:	8b 40 04             	mov    0x4(%eax),%eax
80104255:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104259:	8d 95 ec ef ff ff    	lea    -0x1014(%ebp),%edx
8010425f:	89 54 24 08          	mov    %edx,0x8(%esp)
80104263:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104266:	89 54 24 04          	mov    %edx,0x4(%esp)
8010426a:	89 04 24             	mov    %eax,(%esp)
8010426d:	e8 fd 42 00 00       	call   8010856f <copyout>
80104272:	85 c0                	test   %eax,%eax
80104274:	79 0e                	jns    80104284 <swapIn+0xe1>
	  {
	    cprintf("copyout failed\n");
80104276:	c7 04 24 01 89 10 80 	movl   $0x80108901,(%esp)
8010427d:	e8 1f c1 ff ff       	call   801003a1 <cprintf>
	    break;
80104282:	eb 11                	jmp    80104295 <swapIn+0xf2>
	cprintf("allocuvm failed\n");
	break;
      }
      
      uint a = 0;
      for(; a < t->sz; a += PGSIZE)
80104284:	81 45 f0 00 10 00 00 	addl   $0x1000,-0x10(%ebp)
8010428b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010428e:	8b 00                	mov    (%eax),%eax
80104290:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80104293:	77 8e                	ja     80104223 <swapIn+0x80>
	    break;
	  }
	}
      }
      
      t->state = RUNNABLE;
80104295:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104298:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      t->isSwapped = 0;
8010429f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042a2:	c7 80 80 00 00 00 00 	movl   $0x0,0x80(%eax)
801042a9:	00 00 00 
      fileclose(t->swap);
801042ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042af:	8b 40 7c             	mov    0x7c(%eax),%eax
801042b2:	89 04 24             	mov    %eax,(%esp)
801042b5:	e8 0a cd ff ff       	call   80100fc4 <fileclose>
801042ba:	eb 01                	jmp    801042bd <swapIn+0x11a>
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
    {
      if(t->state != RUNNABLE_SUSPENDED)
	continue;
801042bc:	90                   	nop
  cprintf("swapin\n");
  struct proc* t;
  acquire(&ptable.lock);
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
801042bd:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
801042c4:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
801042cb:	0f 82 ff fe ff ff    	jb     801041d0 <swapIn+0x2d>
      fileclose(t->swap);
      
      // delete fild pid.swap
    }
    
    sleep(proc,&ptable.lock);
801042d1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042d7:	c7 44 24 04 20 ff 10 	movl   $0x8010ff20,0x4(%esp)
801042de:	80 
801042df:	89 04 24             	mov    %eax,(%esp)
801042e2:	e8 b9 07 00 00       	call   80104aa0 <sleep>
  }
801042e7:	e9 d8 fe ff ff       	jmp    801041c4 <swapIn+0x21>

801042ec <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
801042ec:	55                   	push   %ebp
801042ed:	89 e5                	mov    %esp,%ebp
801042ef:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
801042f2:	e8 10 fd ff ff       	call   80104007 <allocproc>
801042f7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
801042fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042fd:	a3 48 b6 10 80       	mov    %eax,0x8010b648
  if((p->pgdir = setupkvm(kalloc)) == 0)
80104302:	c7 04 24 ff 2a 10 80 	movl   $0x80102aff,(%esp)
80104309:	e8 b7 3a 00 00       	call   80107dc5 <setupkvm>
8010430e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104311:	89 42 04             	mov    %eax,0x4(%edx)
80104314:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104317:	8b 40 04             	mov    0x4(%eax),%eax
8010431a:	85 c0                	test   %eax,%eax
8010431c:	75 0c                	jne    8010432a <userinit+0x3e>
    panic("userinit: out of memory?");
8010431e:	c7 04 24 11 89 10 80 	movl   $0x80108911,(%esp)
80104325:	e8 13 c2 ff ff       	call   8010053d <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
8010432a:	ba 2c 00 00 00       	mov    $0x2c,%edx
8010432f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104332:	8b 40 04             	mov    0x4(%eax),%eax
80104335:	89 54 24 08          	mov    %edx,0x8(%esp)
80104339:	c7 44 24 04 e0 b4 10 	movl   $0x8010b4e0,0x4(%esp)
80104340:	80 
80104341:	89 04 24             	mov    %eax,(%esp)
80104344:	e8 d4 3c 00 00       	call   8010801d <inituvm>
  p->sz = PGSIZE;
80104349:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010434c:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
80104352:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104355:	8b 40 18             	mov    0x18(%eax),%eax
80104358:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
8010435f:	00 
80104360:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104367:	00 
80104368:	89 04 24             	mov    %eax,(%esp)
8010436b:	e8 96 0d 00 00       	call   80105106 <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104370:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104373:	8b 40 18             	mov    0x18(%eax),%eax
80104376:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
8010437c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010437f:	8b 40 18             	mov    0x18(%eax),%eax
80104382:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
80104388:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010438b:	8b 40 18             	mov    0x18(%eax),%eax
8010438e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104391:	8b 52 18             	mov    0x18(%edx),%edx
80104394:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104398:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
8010439c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010439f:	8b 40 18             	mov    0x18(%eax),%eax
801043a2:	8b 55 f4             	mov    -0xc(%ebp),%edx
801043a5:	8b 52 18             	mov    0x18(%edx),%edx
801043a8:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801043ac:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
801043b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043b3:	8b 40 18             	mov    0x18(%eax),%eax
801043b6:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
801043bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043c0:	8b 40 18             	mov    0x18(%eax),%eax
801043c3:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
801043ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043cd:	8b 40 18             	mov    0x18(%eax),%eax
801043d0:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
801043d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043da:	83 c0 6c             	add    $0x6c,%eax
801043dd:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
801043e4:	00 
801043e5:	c7 44 24 04 2a 89 10 	movl   $0x8010892a,0x4(%esp)
801043ec:	80 
801043ed:	89 04 24             	mov    %eax,(%esp)
801043f0:	e8 41 0f 00 00       	call   80105336 <safestrcpy>
  p->cwd = namei("/");
801043f5:	c7 04 24 33 89 10 80 	movl   $0x80108933,(%esp)
801043fc:	e8 09 e0 ff ff       	call   8010240a <namei>
80104401:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104404:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
80104407:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010440a:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)

  //createInternalProcess("inswapper", swapIn);
  cprintf("after createinternalproc\n");
80104411:	c7 04 24 35 89 10 80 	movl   $0x80108935,(%esp)
80104418:	e8 84 bf ff ff       	call   801003a1 <cprintf>
}
8010441d:	c9                   	leave  
8010441e:	c3                   	ret    

8010441f <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
8010441f:	55                   	push   %ebp
80104420:	89 e5                	mov    %esp,%ebp
80104422:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
80104425:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010442b:	8b 00                	mov    (%eax),%eax
8010442d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
80104430:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104434:	7e 34                	jle    8010446a <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
80104436:	8b 45 08             	mov    0x8(%ebp),%eax
80104439:	89 c2                	mov    %eax,%edx
8010443b:	03 55 f4             	add    -0xc(%ebp),%edx
8010443e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104444:	8b 40 04             	mov    0x4(%eax),%eax
80104447:	89 54 24 08          	mov    %edx,0x8(%esp)
8010444b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010444e:	89 54 24 04          	mov    %edx,0x4(%esp)
80104452:	89 04 24             	mov    %eax,(%esp)
80104455:	e8 3d 3d 00 00       	call   80108197 <allocuvm>
8010445a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010445d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104461:	75 41                	jne    801044a4 <growproc+0x85>
      return -1;
80104463:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104468:	eb 58                	jmp    801044c2 <growproc+0xa3>
  } else if(n < 0){
8010446a:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010446e:	79 34                	jns    801044a4 <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
80104470:	8b 45 08             	mov    0x8(%ebp),%eax
80104473:	89 c2                	mov    %eax,%edx
80104475:	03 55 f4             	add    -0xc(%ebp),%edx
80104478:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010447e:	8b 40 04             	mov    0x4(%eax),%eax
80104481:	89 54 24 08          	mov    %edx,0x8(%esp)
80104485:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104488:	89 54 24 04          	mov    %edx,0x4(%esp)
8010448c:	89 04 24             	mov    %eax,(%esp)
8010448f:	e8 dd 3d 00 00       	call   80108271 <deallocuvm>
80104494:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104497:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010449b:	75 07                	jne    801044a4 <growproc+0x85>
      return -1;
8010449d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801044a2:	eb 1e                	jmp    801044c2 <growproc+0xa3>
  }
  proc->sz = sz;
801044a4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044aa:	8b 55 f4             	mov    -0xc(%ebp),%edx
801044ad:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
801044af:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044b5:	89 04 24             	mov    %eax,(%esp)
801044b8:	e8 f9 39 00 00       	call   80107eb6 <switchuvm>
  return 0;
801044bd:	b8 00 00 00 00       	mov    $0x0,%eax
}
801044c2:	c9                   	leave  
801044c3:	c3                   	ret    

801044c4 <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
801044c4:	55                   	push   %ebp
801044c5:	89 e5                	mov    %esp,%ebp
801044c7:	57                   	push   %edi
801044c8:	56                   	push   %esi
801044c9:	53                   	push   %ebx
801044ca:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
801044cd:	e8 35 fb ff ff       	call   80104007 <allocproc>
801044d2:	89 45 e0             	mov    %eax,-0x20(%ebp)
801044d5:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
801044d9:	75 0a                	jne    801044e5 <fork+0x21>
    return -1;
801044db:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801044e0:	e9 3a 01 00 00       	jmp    8010461f <fork+0x15b>
  
  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
801044e5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044eb:	8b 10                	mov    (%eax),%edx
801044ed:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044f3:	8b 40 04             	mov    0x4(%eax),%eax
801044f6:	89 54 24 04          	mov    %edx,0x4(%esp)
801044fa:	89 04 24             	mov    %eax,(%esp)
801044fd:	e8 ff 3e 00 00       	call   80108401 <copyuvm>
80104502:	8b 55 e0             	mov    -0x20(%ebp),%edx
80104505:	89 42 04             	mov    %eax,0x4(%edx)
80104508:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010450b:	8b 40 04             	mov    0x4(%eax),%eax
8010450e:	85 c0                	test   %eax,%eax
80104510:	75 2c                	jne    8010453e <fork+0x7a>
    kfree(np->kstack);
80104512:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104515:	8b 40 08             	mov    0x8(%eax),%eax
80104518:	89 04 24             	mov    %eax,(%esp)
8010451b:	e8 46 e5 ff ff       	call   80102a66 <kfree>
    np->kstack = 0;
80104520:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104523:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
8010452a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010452d:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
80104534:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104539:	e9 e1 00 00 00       	jmp    8010461f <fork+0x15b>
  }
  np->sz = proc->sz;
8010453e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104544:	8b 10                	mov    (%eax),%edx
80104546:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104549:	89 10                	mov    %edx,(%eax)
  np->parent = proc;
8010454b:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104552:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104555:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *proc->tf;
80104558:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010455b:	8b 50 18             	mov    0x18(%eax),%edx
8010455e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104564:	8b 40 18             	mov    0x18(%eax),%eax
80104567:	89 c3                	mov    %eax,%ebx
80104569:	b8 13 00 00 00       	mov    $0x13,%eax
8010456e:	89 d7                	mov    %edx,%edi
80104570:	89 de                	mov    %ebx,%esi
80104572:	89 c1                	mov    %eax,%ecx
80104574:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
80104576:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104579:	8b 40 18             	mov    0x18(%eax),%eax
8010457c:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
80104583:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
8010458a:	eb 3d                	jmp    801045c9 <fork+0x105>
    if(proc->ofile[i])
8010458c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104592:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104595:	83 c2 08             	add    $0x8,%edx
80104598:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
8010459c:	85 c0                	test   %eax,%eax
8010459e:	74 25                	je     801045c5 <fork+0x101>
      np->ofile[i] = filedup(proc->ofile[i]);
801045a0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045a6:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801045a9:	83 c2 08             	add    $0x8,%edx
801045ac:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801045b0:	89 04 24             	mov    %eax,(%esp)
801045b3:	e8 c4 c9 ff ff       	call   80100f7c <filedup>
801045b8:	8b 55 e0             	mov    -0x20(%ebp),%edx
801045bb:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
801045be:	83 c1 08             	add    $0x8,%ecx
801045c1:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *proc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
801045c5:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
801045c9:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
801045cd:	7e bd                	jle    8010458c <fork+0xc8>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
801045cf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045d5:	8b 40 68             	mov    0x68(%eax),%eax
801045d8:	89 04 24             	mov    %eax,(%esp)
801045db:	e8 56 d2 ff ff       	call   80101836 <idup>
801045e0:	8b 55 e0             	mov    -0x20(%ebp),%edx
801045e3:	89 42 68             	mov    %eax,0x68(%edx)
 
  pid = np->pid;
801045e6:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045e9:	8b 40 10             	mov    0x10(%eax),%eax
801045ec:	89 45 dc             	mov    %eax,-0x24(%ebp)
  np->state = RUNNABLE;
801045ef:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045f2:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  safestrcpy(np->name, proc->name, sizeof(proc->name));
801045f9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045ff:	8d 50 6c             	lea    0x6c(%eax),%edx
80104602:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104605:	83 c0 6c             	add    $0x6c,%eax
80104608:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010460f:	00 
80104610:	89 54 24 04          	mov    %edx,0x4(%esp)
80104614:	89 04 24             	mov    %eax,(%esp)
80104617:	e8 1a 0d 00 00       	call   80105336 <safestrcpy>
  return pid;
8010461c:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
8010461f:	83 c4 2c             	add    $0x2c,%esp
80104622:	5b                   	pop    %ebx
80104623:	5e                   	pop    %esi
80104624:	5f                   	pop    %edi
80104625:	5d                   	pop    %ebp
80104626:	c3                   	ret    

80104627 <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
80104627:	55                   	push   %ebp
80104628:	89 e5                	mov    %esp,%ebp
8010462a:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
8010462d:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104634:	a1 48 b6 10 80       	mov    0x8010b648,%eax
80104639:	39 c2                	cmp    %eax,%edx
8010463b:	75 0c                	jne    80104649 <exit+0x22>
    panic("init exiting");
8010463d:	c7 04 24 4f 89 10 80 	movl   $0x8010894f,(%esp)
80104644:	e8 f4 be ff ff       	call   8010053d <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104649:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104650:	eb 44                	jmp    80104696 <exit+0x6f>
    if(proc->ofile[fd]){
80104652:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104658:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010465b:	83 c2 08             	add    $0x8,%edx
8010465e:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104662:	85 c0                	test   %eax,%eax
80104664:	74 2c                	je     80104692 <exit+0x6b>
      fileclose(proc->ofile[fd]);
80104666:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010466c:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010466f:	83 c2 08             	add    $0x8,%edx
80104672:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104676:	89 04 24             	mov    %eax,(%esp)
80104679:	e8 46 c9 ff ff       	call   80100fc4 <fileclose>
      proc->ofile[fd] = 0;
8010467e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104684:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104687:	83 c2 08             	add    $0x8,%edx
8010468a:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80104691:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104692:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104696:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
8010469a:	7e b6                	jle    80104652 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  iput(proc->cwd);
8010469c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046a2:	8b 40 68             	mov    0x68(%eax),%eax
801046a5:	89 04 24             	mov    %eax,(%esp)
801046a8:	e8 6e d3 ff ff       	call   80101a1b <iput>
  proc->cwd = 0;
801046ad:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046b3:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
801046ba:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801046c1:	e8 f1 07 00 00       	call   80104eb7 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
801046c6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046cc:	8b 40 14             	mov    0x14(%eax),%eax
801046cf:	89 04 24             	mov    %eax,(%esp)
801046d2:	e8 96 05 00 00       	call   80104c6d <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801046d7:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801046de:	eb 3b                	jmp    8010471b <exit+0xf4>
    if(p->parent == proc){
801046e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046e3:	8b 50 14             	mov    0x14(%eax),%edx
801046e6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046ec:	39 c2                	cmp    %eax,%edx
801046ee:	75 24                	jne    80104714 <exit+0xed>
      p->parent = initproc;
801046f0:	8b 15 48 b6 10 80    	mov    0x8010b648,%edx
801046f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046f9:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
801046fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046ff:	8b 40 0c             	mov    0xc(%eax),%eax
80104702:	83 f8 05             	cmp    $0x5,%eax
80104705:	75 0d                	jne    80104714 <exit+0xed>
        wakeup1(initproc);
80104707:	a1 48 b6 10 80       	mov    0x8010b648,%eax
8010470c:	89 04 24             	mov    %eax,(%esp)
8010470f:	e8 59 05 00 00       	call   80104c6d <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104714:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
8010471b:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
80104722:	72 bc                	jb     801046e0 <exit+0xb9>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
80104724:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010472a:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
80104731:	e8 5a 02 00 00       	call   80104990 <sched>
  panic("zombie exit");
80104736:	c7 04 24 5c 89 10 80 	movl   $0x8010895c,(%esp)
8010473d:	e8 fb bd ff ff       	call   8010053d <panic>

80104742 <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
80104742:	55                   	push   %ebp
80104743:	89 e5                	mov    %esp,%ebp
80104745:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
80104748:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010474f:	e8 63 07 00 00       	call   80104eb7 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
80104754:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010475b:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104762:	e9 9d 00 00 00       	jmp    80104804 <wait+0xc2>
      if(p->parent != proc)
80104767:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010476a:	8b 50 14             	mov    0x14(%eax),%edx
8010476d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104773:	39 c2                	cmp    %eax,%edx
80104775:	0f 85 81 00 00 00    	jne    801047fc <wait+0xba>
        continue;
      havekids = 1;
8010477b:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
80104782:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104785:	8b 40 0c             	mov    0xc(%eax),%eax
80104788:	83 f8 05             	cmp    $0x5,%eax
8010478b:	75 70                	jne    801047fd <wait+0xbb>
        // Found one.
        pid = p->pid;
8010478d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104790:	8b 40 10             	mov    0x10(%eax),%eax
80104793:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
80104796:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104799:	8b 40 08             	mov    0x8(%eax),%eax
8010479c:	89 04 24             	mov    %eax,(%esp)
8010479f:	e8 c2 e2 ff ff       	call   80102a66 <kfree>
        p->kstack = 0;
801047a4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047a7:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
801047ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047b1:	8b 40 04             	mov    0x4(%eax),%eax
801047b4:	89 04 24             	mov    %eax,(%esp)
801047b7:	e8 71 3b 00 00       	call   8010832d <freevm>
        p->state = UNUSED;
801047bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047bf:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
801047c6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047c9:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
801047d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047d3:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
801047da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047dd:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
801047e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047e4:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
801047eb:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801047f2:	e8 22 07 00 00       	call   80104f19 <release>
        return pid;
801047f7:	8b 45 ec             	mov    -0x14(%ebp),%eax
801047fa:	eb 56                	jmp    80104852 <wait+0x110>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc)
        continue;
801047fc:	90                   	nop

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801047fd:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
80104804:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
8010480b:	0f 82 56 ff ff ff    	jb     80104767 <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
80104811:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104815:	74 0d                	je     80104824 <wait+0xe2>
80104817:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010481d:	8b 40 24             	mov    0x24(%eax),%eax
80104820:	85 c0                	test   %eax,%eax
80104822:	74 13                	je     80104837 <wait+0xf5>
      release(&ptable.lock);
80104824:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010482b:	e8 e9 06 00 00       	call   80104f19 <release>
      return -1;
80104830:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104835:	eb 1b                	jmp    80104852 <wait+0x110>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
80104837:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010483d:	c7 44 24 04 20 ff 10 	movl   $0x8010ff20,0x4(%esp)
80104844:	80 
80104845:	89 04 24             	mov    %eax,(%esp)
80104848:	e8 53 02 00 00       	call   80104aa0 <sleep>
  }
8010484d:	e9 02 ff ff ff       	jmp    80104754 <wait+0x12>
}
80104852:	c9                   	leave  
80104853:	c3                   	ret    

80104854 <register_handler>:

void
register_handler(sighandler_t sighandler)
{
80104854:	55                   	push   %ebp
80104855:	89 e5                	mov    %esp,%ebp
80104857:	83 ec 28             	sub    $0x28,%esp
  char* addr = uva2ka(proc->pgdir, (char*)proc->tf->esp);
8010485a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104860:	8b 40 18             	mov    0x18(%eax),%eax
80104863:	8b 40 44             	mov    0x44(%eax),%eax
80104866:	89 c2                	mov    %eax,%edx
80104868:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010486e:	8b 40 04             	mov    0x4(%eax),%eax
80104871:	89 54 24 04          	mov    %edx,0x4(%esp)
80104875:	89 04 24             	mov    %eax,(%esp)
80104878:	e8 95 3c 00 00       	call   80108512 <uva2ka>
8010487d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if ((proc->tf->esp & 0xFFF) == 0)
80104880:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104886:	8b 40 18             	mov    0x18(%eax),%eax
80104889:	8b 40 44             	mov    0x44(%eax),%eax
8010488c:	25 ff 0f 00 00       	and    $0xfff,%eax
80104891:	85 c0                	test   %eax,%eax
80104893:	75 0c                	jne    801048a1 <register_handler+0x4d>
    panic("esp_offset == 0");
80104895:	c7 04 24 68 89 10 80 	movl   $0x80108968,(%esp)
8010489c:	e8 9c bc ff ff       	call   8010053d <panic>

    /* open a new frame */
  *(int*)(addr + ((proc->tf->esp - 4) & 0xFFF))
801048a1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048a7:	8b 40 18             	mov    0x18(%eax),%eax
801048aa:	8b 40 44             	mov    0x44(%eax),%eax
801048ad:	83 e8 04             	sub    $0x4,%eax
801048b0:	25 ff 0f 00 00       	and    $0xfff,%eax
801048b5:	03 45 f4             	add    -0xc(%ebp),%eax
          = proc->tf->eip;
801048b8:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801048bf:	8b 52 18             	mov    0x18(%edx),%edx
801048c2:	8b 52 38             	mov    0x38(%edx),%edx
801048c5:	89 10                	mov    %edx,(%eax)
  proc->tf->esp -= 4;
801048c7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048cd:	8b 40 18             	mov    0x18(%eax),%eax
801048d0:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801048d7:	8b 52 18             	mov    0x18(%edx),%edx
801048da:	8b 52 44             	mov    0x44(%edx),%edx
801048dd:	83 ea 04             	sub    $0x4,%edx
801048e0:	89 50 44             	mov    %edx,0x44(%eax)

    /* update eip */
  proc->tf->eip = (uint)sighandler;
801048e3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048e9:	8b 40 18             	mov    0x18(%eax),%eax
801048ec:	8b 55 08             	mov    0x8(%ebp),%edx
801048ef:	89 50 38             	mov    %edx,0x38(%eax)
}
801048f2:	c9                   	leave  
801048f3:	c3                   	ret    

801048f4 <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
801048f4:	55                   	push   %ebp
801048f5:	89 e5                	mov    %esp,%ebp
801048f7:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  for(;;){
    // Enable interrupts on this processor.
    sti();
801048fa:	e8 e6 f6 ff ff       	call   80103fe5 <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
801048ff:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104906:	e8 ac 05 00 00       	call   80104eb7 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010490b:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104912:	eb 62                	jmp    80104976 <scheduler+0x82>
      if(p->state != RUNNABLE)
80104914:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104917:	8b 40 0c             	mov    0xc(%eax),%eax
8010491a:	83 f8 03             	cmp    $0x3,%eax
8010491d:	75 4f                	jne    8010496e <scheduler+0x7a>
        continue;

      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
8010491f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104922:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
80104928:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010492b:	89 04 24             	mov    %eax,(%esp)
8010492e:	e8 83 35 00 00       	call   80107eb6 <switchuvm>
      p->state = RUNNING;
80104933:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104936:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
8010493d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104943:	8b 40 1c             	mov    0x1c(%eax),%eax
80104946:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
8010494d:	83 c2 04             	add    $0x4,%edx
80104950:	89 44 24 04          	mov    %eax,0x4(%esp)
80104954:	89 14 24             	mov    %edx,(%esp)
80104957:	e8 50 0a 00 00       	call   801053ac <swtch>
      switchkvm();
8010495c:	e8 38 35 00 00       	call   80107e99 <switchkvm>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
80104961:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80104968:	00 00 00 00 
8010496c:	eb 01                	jmp    8010496f <scheduler+0x7b>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
8010496e:	90                   	nop
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010496f:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
80104976:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
8010497d:	72 95                	jb     80104914 <scheduler+0x20>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
8010497f:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104986:	e8 8e 05 00 00       	call   80104f19 <release>

  }
8010498b:	e9 6a ff ff ff       	jmp    801048fa <scheduler+0x6>

80104990 <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
80104990:	55                   	push   %ebp
80104991:	89 e5                	mov    %esp,%ebp
80104993:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
80104996:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010499d:	e8 33 06 00 00       	call   80104fd5 <holding>
801049a2:	85 c0                	test   %eax,%eax
801049a4:	75 0c                	jne    801049b2 <sched+0x22>
    panic("sched ptable.lock");
801049a6:	c7 04 24 78 89 10 80 	movl   $0x80108978,(%esp)
801049ad:	e8 8b bb ff ff       	call   8010053d <panic>
  if(cpu->ncli != 1)
801049b2:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801049b8:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
801049be:	83 f8 01             	cmp    $0x1,%eax
801049c1:	74 0c                	je     801049cf <sched+0x3f>
    panic("sched locks");
801049c3:	c7 04 24 8a 89 10 80 	movl   $0x8010898a,(%esp)
801049ca:	e8 6e bb ff ff       	call   8010053d <panic>
  if(proc->state == RUNNING)
801049cf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049d5:	8b 40 0c             	mov    0xc(%eax),%eax
801049d8:	83 f8 04             	cmp    $0x4,%eax
801049db:	75 0c                	jne    801049e9 <sched+0x59>
    panic("sched running");
801049dd:	c7 04 24 96 89 10 80 	movl   $0x80108996,(%esp)
801049e4:	e8 54 bb ff ff       	call   8010053d <panic>
  if(readeflags()&FL_IF)
801049e9:	e8 e2 f5 ff ff       	call   80103fd0 <readeflags>
801049ee:	25 00 02 00 00       	and    $0x200,%eax
801049f3:	85 c0                	test   %eax,%eax
801049f5:	74 0c                	je     80104a03 <sched+0x73>
    panic("sched interruptible");
801049f7:	c7 04 24 a4 89 10 80 	movl   $0x801089a4,(%esp)
801049fe:	e8 3a bb ff ff       	call   8010053d <panic>
  intena = cpu->intena;
80104a03:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a09:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104a0f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80104a12:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a18:	8b 40 04             	mov    0x4(%eax),%eax
80104a1b:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104a22:	83 c2 1c             	add    $0x1c,%edx
80104a25:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a29:	89 14 24             	mov    %edx,(%esp)
80104a2c:	e8 7b 09 00 00       	call   801053ac <swtch>
  cpu->intena = intena;
80104a31:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a37:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104a3a:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104a40:	c9                   	leave  
80104a41:	c3                   	ret    

80104a42 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
80104a42:	55                   	push   %ebp
80104a43:	89 e5                	mov    %esp,%ebp
80104a45:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104a48:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104a4f:	e8 63 04 00 00       	call   80104eb7 <acquire>
  proc->state = RUNNABLE;
80104a54:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104a5a:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
80104a61:	e8 2a ff ff ff       	call   80104990 <sched>
  release(&ptable.lock);
80104a66:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104a6d:	e8 a7 04 00 00       	call   80104f19 <release>
}
80104a72:	c9                   	leave  
80104a73:	c3                   	ret    

80104a74 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80104a74:	55                   	push   %ebp
80104a75:	89 e5                	mov    %esp,%ebp
80104a77:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80104a7a:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104a81:	e8 93 04 00 00       	call   80104f19 <release>

  if (first) {
80104a86:	a1 24 b0 10 80       	mov    0x8010b024,%eax
80104a8b:	85 c0                	test   %eax,%eax
80104a8d:	74 0f                	je     80104a9e <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
80104a8f:	c7 05 24 b0 10 80 00 	movl   $0x0,0x8010b024
80104a96:	00 00 00 
    initlog();
80104a99:	e8 72 e5 ff ff       	call   80103010 <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
80104a9e:	c9                   	leave  
80104a9f:	c3                   	ret    

80104aa0 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80104aa0:	55                   	push   %ebp
80104aa1:	89 e5                	mov    %esp,%ebp
80104aa3:	53                   	push   %ebx
80104aa4:	83 ec 24             	sub    $0x24,%esp
  if(proc == 0)
80104aa7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104aad:	85 c0                	test   %eax,%eax
80104aaf:	75 0c                	jne    80104abd <sleep+0x1d>
    panic("sleep");
80104ab1:	c7 04 24 b8 89 10 80 	movl   $0x801089b8,(%esp)
80104ab8:	e8 80 ba ff ff       	call   8010053d <panic>

  if(lk == 0)
80104abd:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104ac1:	75 0c                	jne    80104acf <sleep+0x2f>
    panic("sleep without lk");
80104ac3:	c7 04 24 be 89 10 80 	movl   $0x801089be,(%esp)
80104aca:	e8 6e ba ff ff       	call   8010053d <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104acf:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
80104ad6:	74 17                	je     80104aef <sleep+0x4f>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104ad8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104adf:	e8 d3 03 00 00       	call   80104eb7 <acquire>
    release(lk);
80104ae4:	8b 45 0c             	mov    0xc(%ebp),%eax
80104ae7:	89 04 24             	mov    %eax,(%esp)
80104aea:	e8 2a 04 00 00       	call   80104f19 <release>
  }

  
  if(swapFlag)
80104aef:	a1 08 b0 10 80       	mov    0x8010b008,%eax
80104af4:	85 c0                	test   %eax,%eax
80104af6:	0f 84 ec 00 00 00    	je     80104be8 <sleep+0x148>
  {
    if(proc->pid > 1)
80104afc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b02:	8b 40 10             	mov    0x10(%eax),%eax
80104b05:	83 f8 01             	cmp    $0x1,%eax
80104b08:	0f 8e da 00 00 00    	jle    80104be8 <sleep+0x148>
    {
      int i = 0;
80104b0e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
      char name[8];
      name[2] = '.'; name[3] = 's'; name[4] = 'w'; name[5] = 'a'; name[6] = 'p'; name[7] = 0;
80104b15:	c6 45 ee 2e          	movb   $0x2e,-0x12(%ebp)
80104b19:	c6 45 ef 73          	movb   $0x73,-0x11(%ebp)
80104b1d:	c6 45 f0 77          	movb   $0x77,-0x10(%ebp)
80104b21:	c6 45 f1 61          	movb   $0x61,-0xf(%ebp)
80104b25:	c6 45 f2 70          	movb   $0x70,-0xe(%ebp)
80104b29:	c6 45 f3 00          	movb   $0x0,-0xd(%ebp)
      name[1] = (char)(((int)'0')+proc->pid % 10);
80104b2d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b33:	8b 48 10             	mov    0x10(%eax),%ecx
80104b36:	ba 67 66 66 66       	mov    $0x66666667,%edx
80104b3b:	89 c8                	mov    %ecx,%eax
80104b3d:	f7 ea                	imul   %edx
80104b3f:	c1 fa 02             	sar    $0x2,%edx
80104b42:	89 c8                	mov    %ecx,%eax
80104b44:	c1 f8 1f             	sar    $0x1f,%eax
80104b47:	29 c2                	sub    %eax,%edx
80104b49:	89 d0                	mov    %edx,%eax
80104b4b:	c1 e0 02             	shl    $0x2,%eax
80104b4e:	01 d0                	add    %edx,%eax
80104b50:	01 c0                	add    %eax,%eax
80104b52:	89 ca                	mov    %ecx,%edx
80104b54:	29 c2                	sub    %eax,%edx
80104b56:	89 d0                	mov    %edx,%eax
80104b58:	83 c0 30             	add    $0x30,%eax
80104b5b:	88 45 ed             	mov    %al,-0x13(%ebp)
      if((i=proc->pid/10) == 0)
80104b5e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b64:	8b 48 10             	mov    0x10(%eax),%ecx
80104b67:	ba 67 66 66 66       	mov    $0x66666667,%edx
80104b6c:	89 c8                	mov    %ecx,%eax
80104b6e:	f7 ea                	imul   %edx
80104b70:	c1 fa 02             	sar    $0x2,%edx
80104b73:	89 c8                	mov    %ecx,%eax
80104b75:	c1 f8 1f             	sar    $0x1f,%eax
80104b78:	89 d1                	mov    %edx,%ecx
80104b7a:	29 c1                	sub    %eax,%ecx
80104b7c:	89 c8                	mov    %ecx,%eax
80104b7e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104b81:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104b85:	75 06                	jne    80104b8d <sleep+0xed>
	name[0] = '0';
80104b87:	c6 45 ec 30          	movb   $0x30,-0x14(%ebp)
80104b8b:	eb 09                	jmp    80104b96 <sleep+0xf6>
      else
	name[0] = (char)(((int)'0')+i);
80104b8d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b90:	83 c0 30             	add    $0x30,%eax
80104b93:	88 45 ec             	mov    %al,-0x14(%ebp)
      release(&ptable.lock);
80104b96:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104b9d:	e8 77 03 00 00       	call   80104f19 <release>
      proc->swap = fileopen(name,(O_CREATE | O_RDWR));
80104ba2:	65 8b 1d 04 00 00 00 	mov    %gs:0x4,%ebx
80104ba9:	c7 44 24 04 02 02 00 	movl   $0x202,0x4(%esp)
80104bb0:	00 
80104bb1:	8d 45 ec             	lea    -0x14(%ebp),%eax
80104bb4:	89 04 24             	mov    %eax,(%esp)
80104bb7:	e8 79 12 00 00       	call   80105e35 <fileopen>
80104bbc:	89 43 7c             	mov    %eax,0x7c(%ebx)
      acquire(&ptable.lock);
80104bbf:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104bc6:	e8 ec 02 00 00       	call   80104eb7 <acquire>
      proc->state = SLEEPING_SUSPENDED;
80104bcb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bd1:	c7 40 0c 06 00 00 00 	movl   $0x6,0xc(%eax)
      //freevm(proc->pgdir);
       //proc->state = SLEEPING_SUSPENDED;
      proc->isSwapped = 1;
80104bd8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bde:	c7 80 80 00 00 00 01 	movl   $0x1,0x80(%eax)
80104be5:	00 00 00 
    }
  
  }
  // Go to sleep.
  proc->chan = chan;
80104be8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bee:	8b 55 08             	mov    0x8(%ebp),%edx
80104bf1:	89 50 20             	mov    %edx,0x20(%eax)
  if(!swapFlag && proc->pid>1)
80104bf4:	a1 08 b0 10 80       	mov    0x8010b008,%eax
80104bf9:	85 c0                	test   %eax,%eax
80104bfb:	75 1b                	jne    80104c18 <sleep+0x178>
80104bfd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c03:	8b 40 10             	mov    0x10(%eax),%eax
80104c06:	83 f8 01             	cmp    $0x1,%eax
80104c09:	7e 0d                	jle    80104c18 <sleep+0x178>
  proc->state = SLEEPING;
80104c0b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c11:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
  
  // Swap out
  
  sched();
80104c18:	e8 73 fd ff ff       	call   80104990 <sched>
  if(proc->isSwapped)
80104c1d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c23:	8b 80 80 00 00 00    	mov    0x80(%eax),%eax
80104c29:	85 c0                	test   %eax,%eax
80104c2b:	74 0d                	je     80104c3a <sleep+0x19a>
  {
      //proc->state = RUNNABLE_SUSPENDED;
      proc->state = RUNNABLE;
80104c2d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c33:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
    // wakeup(inswapper);
  }

  // Tidy up.
  proc->chan = 0;
80104c3a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c40:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104c47:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
80104c4e:	74 17                	je     80104c67 <sleep+0x1c7>
    release(&ptable.lock);
80104c50:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104c57:	e8 bd 02 00 00       	call   80104f19 <release>
    acquire(lk);
80104c5c:	8b 45 0c             	mov    0xc(%ebp),%eax
80104c5f:	89 04 24             	mov    %eax,(%esp)
80104c62:	e8 50 02 00 00       	call   80104eb7 <acquire>
  }
}
80104c67:	83 c4 24             	add    $0x24,%esp
80104c6a:	5b                   	pop    %ebx
80104c6b:	5d                   	pop    %ebp
80104c6c:	c3                   	ret    

80104c6d <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104c6d:	55                   	push   %ebp
80104c6e:	89 e5                	mov    %esp,%ebp
80104c70:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104c73:	c7 45 fc 54 ff 10 80 	movl   $0x8010ff54,-0x4(%ebp)
80104c7a:	eb 27                	jmp    80104ca3 <wakeup1+0x36>
    if(p->state == SLEEPING && p->chan == chan)
80104c7c:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c7f:	8b 40 0c             	mov    0xc(%eax),%eax
80104c82:	83 f8 02             	cmp    $0x2,%eax
80104c85:	75 15                	jne    80104c9c <wakeup1+0x2f>
80104c87:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c8a:	8b 40 20             	mov    0x20(%eax),%eax
80104c8d:	3b 45 08             	cmp    0x8(%ebp),%eax
80104c90:	75 0a                	jne    80104c9c <wakeup1+0x2f>
      p->state = RUNNABLE;
80104c92:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c95:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104c9c:	81 45 fc 84 00 00 00 	addl   $0x84,-0x4(%ebp)
80104ca3:	81 7d fc 54 20 11 80 	cmpl   $0x80112054,-0x4(%ebp)
80104caa:	72 d0                	jb     80104c7c <wakeup1+0xf>
    if(p->state == SLEEPING && p->chan == chan)
      p->state = RUNNABLE;
}
80104cac:	c9                   	leave  
80104cad:	c3                   	ret    

80104cae <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104cae:	55                   	push   %ebp
80104caf:	89 e5                	mov    %esp,%ebp
80104cb1:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
80104cb4:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104cbb:	e8 f7 01 00 00       	call   80104eb7 <acquire>
  wakeup1(chan);
80104cc0:	8b 45 08             	mov    0x8(%ebp),%eax
80104cc3:	89 04 24             	mov    %eax,(%esp)
80104cc6:	e8 a2 ff ff ff       	call   80104c6d <wakeup1>
  release(&ptable.lock);
80104ccb:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104cd2:	e8 42 02 00 00       	call   80104f19 <release>
}
80104cd7:	c9                   	leave  
80104cd8:	c3                   	ret    

80104cd9 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104cd9:	55                   	push   %ebp
80104cda:	89 e5                	mov    %esp,%ebp
80104cdc:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104cdf:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104ce6:	e8 cc 01 00 00       	call   80104eb7 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104ceb:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104cf2:	eb 44                	jmp    80104d38 <kill+0x5f>
    if(p->pid == pid){
80104cf4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cf7:	8b 40 10             	mov    0x10(%eax),%eax
80104cfa:	3b 45 08             	cmp    0x8(%ebp),%eax
80104cfd:	75 32                	jne    80104d31 <kill+0x58>
      p->killed = 1;
80104cff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d02:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104d09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d0c:	8b 40 0c             	mov    0xc(%eax),%eax
80104d0f:	83 f8 02             	cmp    $0x2,%eax
80104d12:	75 0a                	jne    80104d1e <kill+0x45>
        p->state = RUNNABLE;
80104d14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d17:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104d1e:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104d25:	e8 ef 01 00 00       	call   80104f19 <release>
      return 0;
80104d2a:	b8 00 00 00 00       	mov    $0x0,%eax
80104d2f:	eb 21                	jmp    80104d52 <kill+0x79>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104d31:	81 45 f4 84 00 00 00 	addl   $0x84,-0xc(%ebp)
80104d38:	81 7d f4 54 20 11 80 	cmpl   $0x80112054,-0xc(%ebp)
80104d3f:	72 b3                	jb     80104cf4 <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104d41:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104d48:	e8 cc 01 00 00       	call   80104f19 <release>
  return -1;
80104d4d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104d52:	c9                   	leave  
80104d53:	c3                   	ret    

80104d54 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104d54:	55                   	push   %ebp
80104d55:	89 e5                	mov    %esp,%ebp
80104d57:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104d5a:	c7 45 f0 54 ff 10 80 	movl   $0x8010ff54,-0x10(%ebp)
80104d61:	e9 db 00 00 00       	jmp    80104e41 <procdump+0xed>
    if(p->state == UNUSED)
80104d66:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d69:	8b 40 0c             	mov    0xc(%eax),%eax
80104d6c:	85 c0                	test   %eax,%eax
80104d6e:	0f 84 c5 00 00 00    	je     80104e39 <procdump+0xe5>
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104d74:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d77:	8b 40 0c             	mov    0xc(%eax),%eax
80104d7a:	83 f8 05             	cmp    $0x5,%eax
80104d7d:	77 23                	ja     80104da2 <procdump+0x4e>
80104d7f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d82:	8b 40 0c             	mov    0xc(%eax),%eax
80104d85:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104d8c:	85 c0                	test   %eax,%eax
80104d8e:	74 12                	je     80104da2 <procdump+0x4e>
      state = states[p->state];
80104d90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d93:	8b 40 0c             	mov    0xc(%eax),%eax
80104d96:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104d9d:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104da0:	eb 07                	jmp    80104da9 <procdump+0x55>
    else
      state = "???";
80104da2:	c7 45 ec cf 89 10 80 	movl   $0x801089cf,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80104da9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104dac:	8d 50 6c             	lea    0x6c(%eax),%edx
80104daf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104db2:	8b 40 10             	mov    0x10(%eax),%eax
80104db5:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104db9:	8b 55 ec             	mov    -0x14(%ebp),%edx
80104dbc:	89 54 24 08          	mov    %edx,0x8(%esp)
80104dc0:	89 44 24 04          	mov    %eax,0x4(%esp)
80104dc4:	c7 04 24 d3 89 10 80 	movl   $0x801089d3,(%esp)
80104dcb:	e8 d1 b5 ff ff       	call   801003a1 <cprintf>
    if(p->state == SLEEPING){
80104dd0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104dd3:	8b 40 0c             	mov    0xc(%eax),%eax
80104dd6:	83 f8 02             	cmp    $0x2,%eax
80104dd9:	75 50                	jne    80104e2b <procdump+0xd7>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80104ddb:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104dde:	8b 40 1c             	mov    0x1c(%eax),%eax
80104de1:	8b 40 0c             	mov    0xc(%eax),%eax
80104de4:	83 c0 08             	add    $0x8,%eax
80104de7:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80104dea:	89 54 24 04          	mov    %edx,0x4(%esp)
80104dee:	89 04 24             	mov    %eax,(%esp)
80104df1:	e8 72 01 00 00       	call   80104f68 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
80104df6:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104dfd:	eb 1b                	jmp    80104e1a <procdump+0xc6>
        cprintf(" %p", pc[i]);
80104dff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e02:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104e06:	89 44 24 04          	mov    %eax,0x4(%esp)
80104e0a:	c7 04 24 dc 89 10 80 	movl   $0x801089dc,(%esp)
80104e11:	e8 8b b5 ff ff       	call   801003a1 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80104e16:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104e1a:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104e1e:	7f 0b                	jg     80104e2b <procdump+0xd7>
80104e20:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e23:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104e27:	85 c0                	test   %eax,%eax
80104e29:	75 d4                	jne    80104dff <procdump+0xab>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104e2b:	c7 04 24 e0 89 10 80 	movl   $0x801089e0,(%esp)
80104e32:	e8 6a b5 ff ff       	call   801003a1 <cprintf>
80104e37:	eb 01                	jmp    80104e3a <procdump+0xe6>
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED)
      continue;
80104e39:	90                   	nop
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104e3a:	81 45 f0 84 00 00 00 	addl   $0x84,-0x10(%ebp)
80104e41:	81 7d f0 54 20 11 80 	cmpl   $0x80112054,-0x10(%ebp)
80104e48:	0f 82 18 ff ff ff    	jb     80104d66 <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
80104e4e:	c9                   	leave  
80104e4f:	c3                   	ret    

80104e50 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104e50:	55                   	push   %ebp
80104e51:	89 e5                	mov    %esp,%ebp
80104e53:	53                   	push   %ebx
80104e54:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104e57:	9c                   	pushf  
80104e58:	5b                   	pop    %ebx
80104e59:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80104e5c:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80104e5f:	83 c4 10             	add    $0x10,%esp
80104e62:	5b                   	pop    %ebx
80104e63:	5d                   	pop    %ebp
80104e64:	c3                   	ret    

80104e65 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80104e65:	55                   	push   %ebp
80104e66:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80104e68:	fa                   	cli    
}
80104e69:	5d                   	pop    %ebp
80104e6a:	c3                   	ret    

80104e6b <sti>:

static inline void
sti(void)
{
80104e6b:	55                   	push   %ebp
80104e6c:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104e6e:	fb                   	sti    
}
80104e6f:	5d                   	pop    %ebp
80104e70:	c3                   	ret    

80104e71 <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80104e71:	55                   	push   %ebp
80104e72:	89 e5                	mov    %esp,%ebp
80104e74:	53                   	push   %ebx
80104e75:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
80104e78:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104e7b:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
80104e7e:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104e81:	89 c3                	mov    %eax,%ebx
80104e83:	89 d8                	mov    %ebx,%eax
80104e85:	f0 87 02             	lock xchg %eax,(%edx)
80104e88:	89 c3                	mov    %eax,%ebx
80104e8a:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80104e8d:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80104e90:	83 c4 10             	add    $0x10,%esp
80104e93:	5b                   	pop    %ebx
80104e94:	5d                   	pop    %ebp
80104e95:	c3                   	ret    

80104e96 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104e96:	55                   	push   %ebp
80104e97:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80104e99:	8b 45 08             	mov    0x8(%ebp),%eax
80104e9c:	8b 55 0c             	mov    0xc(%ebp),%edx
80104e9f:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80104ea2:	8b 45 08             	mov    0x8(%ebp),%eax
80104ea5:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80104eab:	8b 45 08             	mov    0x8(%ebp),%eax
80104eae:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104eb5:	5d                   	pop    %ebp
80104eb6:	c3                   	ret    

80104eb7 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80104eb7:	55                   	push   %ebp
80104eb8:	89 e5                	mov    %esp,%ebp
80104eba:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80104ebd:	e8 3d 01 00 00       	call   80104fff <pushcli>
  if(holding(lk))
80104ec2:	8b 45 08             	mov    0x8(%ebp),%eax
80104ec5:	89 04 24             	mov    %eax,(%esp)
80104ec8:	e8 08 01 00 00       	call   80104fd5 <holding>
80104ecd:	85 c0                	test   %eax,%eax
80104ecf:	74 0c                	je     80104edd <acquire+0x26>
    panic("acquire");
80104ed1:	c7 04 24 0c 8a 10 80 	movl   $0x80108a0c,(%esp)
80104ed8:	e8 60 b6 ff ff       	call   8010053d <panic>

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80104edd:	90                   	nop
80104ede:	8b 45 08             	mov    0x8(%ebp),%eax
80104ee1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80104ee8:	00 
80104ee9:	89 04 24             	mov    %eax,(%esp)
80104eec:	e8 80 ff ff ff       	call   80104e71 <xchg>
80104ef1:	85 c0                	test   %eax,%eax
80104ef3:	75 e9                	jne    80104ede <acquire+0x27>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80104ef5:	8b 45 08             	mov    0x8(%ebp),%eax
80104ef8:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104eff:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
80104f02:	8b 45 08             	mov    0x8(%ebp),%eax
80104f05:	83 c0 0c             	add    $0xc,%eax
80104f08:	89 44 24 04          	mov    %eax,0x4(%esp)
80104f0c:	8d 45 08             	lea    0x8(%ebp),%eax
80104f0f:	89 04 24             	mov    %eax,(%esp)
80104f12:	e8 51 00 00 00       	call   80104f68 <getcallerpcs>
}
80104f17:	c9                   	leave  
80104f18:	c3                   	ret    

80104f19 <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80104f19:	55                   	push   %ebp
80104f1a:	89 e5                	mov    %esp,%ebp
80104f1c:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80104f1f:	8b 45 08             	mov    0x8(%ebp),%eax
80104f22:	89 04 24             	mov    %eax,(%esp)
80104f25:	e8 ab 00 00 00       	call   80104fd5 <holding>
80104f2a:	85 c0                	test   %eax,%eax
80104f2c:	75 0c                	jne    80104f3a <release+0x21>
    panic("release");
80104f2e:	c7 04 24 14 8a 10 80 	movl   $0x80108a14,(%esp)
80104f35:	e8 03 b6 ff ff       	call   8010053d <panic>

  lk->pcs[0] = 0;
80104f3a:	8b 45 08             	mov    0x8(%ebp),%eax
80104f3d:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80104f44:	8b 45 08             	mov    0x8(%ebp),%eax
80104f47:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
80104f4e:	8b 45 08             	mov    0x8(%ebp),%eax
80104f51:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104f58:	00 
80104f59:	89 04 24             	mov    %eax,(%esp)
80104f5c:	e8 10 ff ff ff       	call   80104e71 <xchg>

  popcli();
80104f61:	e8 e1 00 00 00       	call   80105047 <popcli>
}
80104f66:	c9                   	leave  
80104f67:	c3                   	ret    

80104f68 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80104f68:	55                   	push   %ebp
80104f69:	89 e5                	mov    %esp,%ebp
80104f6b:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
80104f6e:	8b 45 08             	mov    0x8(%ebp),%eax
80104f71:	83 e8 08             	sub    $0x8,%eax
80104f74:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80104f77:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80104f7e:	eb 32                	jmp    80104fb2 <getcallerpcs+0x4a>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104f80:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80104f84:	74 47                	je     80104fcd <getcallerpcs+0x65>
80104f86:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80104f8d:	76 3e                	jbe    80104fcd <getcallerpcs+0x65>
80104f8f:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80104f93:	74 38                	je     80104fcd <getcallerpcs+0x65>
      break;
    pcs[i] = ebp[1];     // saved %eip
80104f95:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104f98:	c1 e0 02             	shl    $0x2,%eax
80104f9b:	03 45 0c             	add    0xc(%ebp),%eax
80104f9e:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104fa1:	8b 52 04             	mov    0x4(%edx),%edx
80104fa4:	89 10                	mov    %edx,(%eax)
    ebp = (uint*)ebp[0]; // saved %ebp
80104fa6:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104fa9:	8b 00                	mov    (%eax),%eax
80104fab:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80104fae:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104fb2:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104fb6:	7e c8                	jle    80104f80 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104fb8:	eb 13                	jmp    80104fcd <getcallerpcs+0x65>
    pcs[i] = 0;
80104fba:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104fbd:	c1 e0 02             	shl    $0x2,%eax
80104fc0:	03 45 0c             	add    0xc(%ebp),%eax
80104fc3:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104fc9:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104fcd:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104fd1:	7e e7                	jle    80104fba <getcallerpcs+0x52>
    pcs[i] = 0;
}
80104fd3:	c9                   	leave  
80104fd4:	c3                   	ret    

80104fd5 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80104fd5:	55                   	push   %ebp
80104fd6:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
80104fd8:	8b 45 08             	mov    0x8(%ebp),%eax
80104fdb:	8b 00                	mov    (%eax),%eax
80104fdd:	85 c0                	test   %eax,%eax
80104fdf:	74 17                	je     80104ff8 <holding+0x23>
80104fe1:	8b 45 08             	mov    0x8(%ebp),%eax
80104fe4:	8b 50 08             	mov    0x8(%eax),%edx
80104fe7:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104fed:	39 c2                	cmp    %eax,%edx
80104fef:	75 07                	jne    80104ff8 <holding+0x23>
80104ff1:	b8 01 00 00 00       	mov    $0x1,%eax
80104ff6:	eb 05                	jmp    80104ffd <holding+0x28>
80104ff8:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104ffd:	5d                   	pop    %ebp
80104ffe:	c3                   	ret    

80104fff <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80104fff:	55                   	push   %ebp
80105000:	89 e5                	mov    %esp,%ebp
80105002:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
80105005:	e8 46 fe ff ff       	call   80104e50 <readeflags>
8010500a:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
8010500d:	e8 53 fe ff ff       	call   80104e65 <cli>
  if(cpu->ncli++ == 0)
80105012:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105018:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
8010501e:	85 d2                	test   %edx,%edx
80105020:	0f 94 c1             	sete   %cl
80105023:	83 c2 01             	add    $0x1,%edx
80105026:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
8010502c:	84 c9                	test   %cl,%cl
8010502e:	74 15                	je     80105045 <pushcli+0x46>
    cpu->intena = eflags & FL_IF;
80105030:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105036:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105039:	81 e2 00 02 00 00    	and    $0x200,%edx
8010503f:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80105045:	c9                   	leave  
80105046:	c3                   	ret    

80105047 <popcli>:

void
popcli(void)
{
80105047:	55                   	push   %ebp
80105048:	89 e5                	mov    %esp,%ebp
8010504a:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
8010504d:	e8 fe fd ff ff       	call   80104e50 <readeflags>
80105052:	25 00 02 00 00       	and    $0x200,%eax
80105057:	85 c0                	test   %eax,%eax
80105059:	74 0c                	je     80105067 <popcli+0x20>
    panic("popcli - interruptible");
8010505b:	c7 04 24 1c 8a 10 80 	movl   $0x80108a1c,(%esp)
80105062:	e8 d6 b4 ff ff       	call   8010053d <panic>
  if(--cpu->ncli < 0)
80105067:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010506d:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80105073:	83 ea 01             	sub    $0x1,%edx
80105076:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
8010507c:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105082:	85 c0                	test   %eax,%eax
80105084:	79 0c                	jns    80105092 <popcli+0x4b>
    panic("popcli");
80105086:	c7 04 24 33 8a 10 80 	movl   $0x80108a33,(%esp)
8010508d:	e8 ab b4 ff ff       	call   8010053d <panic>
  if(cpu->ncli == 0 && cpu->intena)
80105092:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105098:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
8010509e:	85 c0                	test   %eax,%eax
801050a0:	75 15                	jne    801050b7 <popcli+0x70>
801050a2:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801050a8:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
801050ae:	85 c0                	test   %eax,%eax
801050b0:	74 05                	je     801050b7 <popcli+0x70>
    sti();
801050b2:	e8 b4 fd ff ff       	call   80104e6b <sti>
}
801050b7:	c9                   	leave  
801050b8:	c3                   	ret    
801050b9:	00 00                	add    %al,(%eax)
	...

801050bc <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
801050bc:	55                   	push   %ebp
801050bd:	89 e5                	mov    %esp,%ebp
801050bf:	57                   	push   %edi
801050c0:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
801050c1:	8b 4d 08             	mov    0x8(%ebp),%ecx
801050c4:	8b 55 10             	mov    0x10(%ebp),%edx
801050c7:	8b 45 0c             	mov    0xc(%ebp),%eax
801050ca:	89 cb                	mov    %ecx,%ebx
801050cc:	89 df                	mov    %ebx,%edi
801050ce:	89 d1                	mov    %edx,%ecx
801050d0:	fc                   	cld    
801050d1:	f3 aa                	rep stos %al,%es:(%edi)
801050d3:	89 ca                	mov    %ecx,%edx
801050d5:	89 fb                	mov    %edi,%ebx
801050d7:	89 5d 08             	mov    %ebx,0x8(%ebp)
801050da:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
801050dd:	5b                   	pop    %ebx
801050de:	5f                   	pop    %edi
801050df:	5d                   	pop    %ebp
801050e0:	c3                   	ret    

801050e1 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
801050e1:	55                   	push   %ebp
801050e2:	89 e5                	mov    %esp,%ebp
801050e4:	57                   	push   %edi
801050e5:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
801050e6:	8b 4d 08             	mov    0x8(%ebp),%ecx
801050e9:	8b 55 10             	mov    0x10(%ebp),%edx
801050ec:	8b 45 0c             	mov    0xc(%ebp),%eax
801050ef:	89 cb                	mov    %ecx,%ebx
801050f1:	89 df                	mov    %ebx,%edi
801050f3:	89 d1                	mov    %edx,%ecx
801050f5:	fc                   	cld    
801050f6:	f3 ab                	rep stos %eax,%es:(%edi)
801050f8:	89 ca                	mov    %ecx,%edx
801050fa:	89 fb                	mov    %edi,%ebx
801050fc:	89 5d 08             	mov    %ebx,0x8(%ebp)
801050ff:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80105102:	5b                   	pop    %ebx
80105103:	5f                   	pop    %edi
80105104:	5d                   	pop    %ebp
80105105:	c3                   	ret    

80105106 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80105106:	55                   	push   %ebp
80105107:	89 e5                	mov    %esp,%ebp
80105109:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
8010510c:	8b 45 08             	mov    0x8(%ebp),%eax
8010510f:	83 e0 03             	and    $0x3,%eax
80105112:	85 c0                	test   %eax,%eax
80105114:	75 49                	jne    8010515f <memset+0x59>
80105116:	8b 45 10             	mov    0x10(%ebp),%eax
80105119:	83 e0 03             	and    $0x3,%eax
8010511c:	85 c0                	test   %eax,%eax
8010511e:	75 3f                	jne    8010515f <memset+0x59>
    c &= 0xFF;
80105120:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
80105127:	8b 45 10             	mov    0x10(%ebp),%eax
8010512a:	c1 e8 02             	shr    $0x2,%eax
8010512d:	89 c2                	mov    %eax,%edx
8010512f:	8b 45 0c             	mov    0xc(%ebp),%eax
80105132:	89 c1                	mov    %eax,%ecx
80105134:	c1 e1 18             	shl    $0x18,%ecx
80105137:	8b 45 0c             	mov    0xc(%ebp),%eax
8010513a:	c1 e0 10             	shl    $0x10,%eax
8010513d:	09 c1                	or     %eax,%ecx
8010513f:	8b 45 0c             	mov    0xc(%ebp),%eax
80105142:	c1 e0 08             	shl    $0x8,%eax
80105145:	09 c8                	or     %ecx,%eax
80105147:	0b 45 0c             	or     0xc(%ebp),%eax
8010514a:	89 54 24 08          	mov    %edx,0x8(%esp)
8010514e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105152:	8b 45 08             	mov    0x8(%ebp),%eax
80105155:	89 04 24             	mov    %eax,(%esp)
80105158:	e8 84 ff ff ff       	call   801050e1 <stosl>
8010515d:	eb 19                	jmp    80105178 <memset+0x72>
  } else
    stosb(dst, c, n);
8010515f:	8b 45 10             	mov    0x10(%ebp),%eax
80105162:	89 44 24 08          	mov    %eax,0x8(%esp)
80105166:	8b 45 0c             	mov    0xc(%ebp),%eax
80105169:	89 44 24 04          	mov    %eax,0x4(%esp)
8010516d:	8b 45 08             	mov    0x8(%ebp),%eax
80105170:	89 04 24             	mov    %eax,(%esp)
80105173:	e8 44 ff ff ff       	call   801050bc <stosb>
  return dst;
80105178:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010517b:	c9                   	leave  
8010517c:	c3                   	ret    

8010517d <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
8010517d:	55                   	push   %ebp
8010517e:	89 e5                	mov    %esp,%ebp
80105180:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80105183:	8b 45 08             	mov    0x8(%ebp),%eax
80105186:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
80105189:	8b 45 0c             	mov    0xc(%ebp),%eax
8010518c:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
8010518f:	eb 32                	jmp    801051c3 <memcmp+0x46>
    if(*s1 != *s2)
80105191:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105194:	0f b6 10             	movzbl (%eax),%edx
80105197:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010519a:	0f b6 00             	movzbl (%eax),%eax
8010519d:	38 c2                	cmp    %al,%dl
8010519f:	74 1a                	je     801051bb <memcmp+0x3e>
      return *s1 - *s2;
801051a1:	8b 45 fc             	mov    -0x4(%ebp),%eax
801051a4:	0f b6 00             	movzbl (%eax),%eax
801051a7:	0f b6 d0             	movzbl %al,%edx
801051aa:	8b 45 f8             	mov    -0x8(%ebp),%eax
801051ad:	0f b6 00             	movzbl (%eax),%eax
801051b0:	0f b6 c0             	movzbl %al,%eax
801051b3:	89 d1                	mov    %edx,%ecx
801051b5:	29 c1                	sub    %eax,%ecx
801051b7:	89 c8                	mov    %ecx,%eax
801051b9:	eb 1c                	jmp    801051d7 <memcmp+0x5a>
    s1++, s2++;
801051bb:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801051bf:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
801051c3:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801051c7:	0f 95 c0             	setne  %al
801051ca:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801051ce:	84 c0                	test   %al,%al
801051d0:	75 bf                	jne    80105191 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
801051d2:	b8 00 00 00 00       	mov    $0x0,%eax
}
801051d7:	c9                   	leave  
801051d8:	c3                   	ret    

801051d9 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
801051d9:	55                   	push   %ebp
801051da:	89 e5                	mov    %esp,%ebp
801051dc:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
801051df:	8b 45 0c             	mov    0xc(%ebp),%eax
801051e2:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
801051e5:	8b 45 08             	mov    0x8(%ebp),%eax
801051e8:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
801051eb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801051ee:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801051f1:	73 54                	jae    80105247 <memmove+0x6e>
801051f3:	8b 45 10             	mov    0x10(%ebp),%eax
801051f6:	8b 55 fc             	mov    -0x4(%ebp),%edx
801051f9:	01 d0                	add    %edx,%eax
801051fb:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801051fe:	76 47                	jbe    80105247 <memmove+0x6e>
    s += n;
80105200:	8b 45 10             	mov    0x10(%ebp),%eax
80105203:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
80105206:	8b 45 10             	mov    0x10(%ebp),%eax
80105209:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
8010520c:	eb 13                	jmp    80105221 <memmove+0x48>
      *--d = *--s;
8010520e:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
80105212:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
80105216:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105219:	0f b6 10             	movzbl (%eax),%edx
8010521c:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010521f:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
80105221:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105225:	0f 95 c0             	setne  %al
80105228:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
8010522c:	84 c0                	test   %al,%al
8010522e:	75 de                	jne    8010520e <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80105230:	eb 25                	jmp    80105257 <memmove+0x7e>
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
      *d++ = *s++;
80105232:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105235:	0f b6 10             	movzbl (%eax),%edx
80105238:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010523b:	88 10                	mov    %dl,(%eax)
8010523d:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105241:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105245:	eb 01                	jmp    80105248 <memmove+0x6f>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
80105247:	90                   	nop
80105248:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010524c:	0f 95 c0             	setne  %al
8010524f:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105253:	84 c0                	test   %al,%al
80105255:	75 db                	jne    80105232 <memmove+0x59>
      *d++ = *s++;

  return dst;
80105257:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010525a:	c9                   	leave  
8010525b:	c3                   	ret    

8010525c <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
8010525c:	55                   	push   %ebp
8010525d:	89 e5                	mov    %esp,%ebp
8010525f:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
80105262:	8b 45 10             	mov    0x10(%ebp),%eax
80105265:	89 44 24 08          	mov    %eax,0x8(%esp)
80105269:	8b 45 0c             	mov    0xc(%ebp),%eax
8010526c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105270:	8b 45 08             	mov    0x8(%ebp),%eax
80105273:	89 04 24             	mov    %eax,(%esp)
80105276:	e8 5e ff ff ff       	call   801051d9 <memmove>
}
8010527b:	c9                   	leave  
8010527c:	c3                   	ret    

8010527d <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
8010527d:	55                   	push   %ebp
8010527e:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80105280:	eb 0c                	jmp    8010528e <strncmp+0x11>
    n--, p++, q++;
80105282:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105286:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010528a:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
8010528e:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105292:	74 1a                	je     801052ae <strncmp+0x31>
80105294:	8b 45 08             	mov    0x8(%ebp),%eax
80105297:	0f b6 00             	movzbl (%eax),%eax
8010529a:	84 c0                	test   %al,%al
8010529c:	74 10                	je     801052ae <strncmp+0x31>
8010529e:	8b 45 08             	mov    0x8(%ebp),%eax
801052a1:	0f b6 10             	movzbl (%eax),%edx
801052a4:	8b 45 0c             	mov    0xc(%ebp),%eax
801052a7:	0f b6 00             	movzbl (%eax),%eax
801052aa:	38 c2                	cmp    %al,%dl
801052ac:	74 d4                	je     80105282 <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
801052ae:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801052b2:	75 07                	jne    801052bb <strncmp+0x3e>
    return 0;
801052b4:	b8 00 00 00 00       	mov    $0x0,%eax
801052b9:	eb 18                	jmp    801052d3 <strncmp+0x56>
  return (uchar)*p - (uchar)*q;
801052bb:	8b 45 08             	mov    0x8(%ebp),%eax
801052be:	0f b6 00             	movzbl (%eax),%eax
801052c1:	0f b6 d0             	movzbl %al,%edx
801052c4:	8b 45 0c             	mov    0xc(%ebp),%eax
801052c7:	0f b6 00             	movzbl (%eax),%eax
801052ca:	0f b6 c0             	movzbl %al,%eax
801052cd:	89 d1                	mov    %edx,%ecx
801052cf:	29 c1                	sub    %eax,%ecx
801052d1:	89 c8                	mov    %ecx,%eax
}
801052d3:	5d                   	pop    %ebp
801052d4:	c3                   	ret    

801052d5 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
801052d5:	55                   	push   %ebp
801052d6:	89 e5                	mov    %esp,%ebp
801052d8:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
801052db:	8b 45 08             	mov    0x8(%ebp),%eax
801052de:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
801052e1:	90                   	nop
801052e2:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801052e6:	0f 9f c0             	setg   %al
801052e9:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801052ed:	84 c0                	test   %al,%al
801052ef:	74 30                	je     80105321 <strncpy+0x4c>
801052f1:	8b 45 0c             	mov    0xc(%ebp),%eax
801052f4:	0f b6 10             	movzbl (%eax),%edx
801052f7:	8b 45 08             	mov    0x8(%ebp),%eax
801052fa:	88 10                	mov    %dl,(%eax)
801052fc:	8b 45 08             	mov    0x8(%ebp),%eax
801052ff:	0f b6 00             	movzbl (%eax),%eax
80105302:	84 c0                	test   %al,%al
80105304:	0f 95 c0             	setne  %al
80105307:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010530b:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
8010530f:	84 c0                	test   %al,%al
80105311:	75 cf                	jne    801052e2 <strncpy+0xd>
    ;
  while(n-- > 0)
80105313:	eb 0c                	jmp    80105321 <strncpy+0x4c>
    *s++ = 0;
80105315:	8b 45 08             	mov    0x8(%ebp),%eax
80105318:	c6 00 00             	movb   $0x0,(%eax)
8010531b:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010531f:	eb 01                	jmp    80105322 <strncpy+0x4d>
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
80105321:	90                   	nop
80105322:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105326:	0f 9f c0             	setg   %al
80105329:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
8010532d:	84 c0                	test   %al,%al
8010532f:	75 e4                	jne    80105315 <strncpy+0x40>
    *s++ = 0;
  return os;
80105331:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105334:	c9                   	leave  
80105335:	c3                   	ret    

80105336 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
80105336:	55                   	push   %ebp
80105337:	89 e5                	mov    %esp,%ebp
80105339:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
8010533c:	8b 45 08             	mov    0x8(%ebp),%eax
8010533f:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
80105342:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105346:	7f 05                	jg     8010534d <safestrcpy+0x17>
    return os;
80105348:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010534b:	eb 35                	jmp    80105382 <safestrcpy+0x4c>
  while(--n > 0 && (*s++ = *t++) != 0)
8010534d:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105351:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105355:	7e 22                	jle    80105379 <safestrcpy+0x43>
80105357:	8b 45 0c             	mov    0xc(%ebp),%eax
8010535a:	0f b6 10             	movzbl (%eax),%edx
8010535d:	8b 45 08             	mov    0x8(%ebp),%eax
80105360:	88 10                	mov    %dl,(%eax)
80105362:	8b 45 08             	mov    0x8(%ebp),%eax
80105365:	0f b6 00             	movzbl (%eax),%eax
80105368:	84 c0                	test   %al,%al
8010536a:	0f 95 c0             	setne  %al
8010536d:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80105371:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80105375:	84 c0                	test   %al,%al
80105377:	75 d4                	jne    8010534d <safestrcpy+0x17>
    ;
  *s = 0;
80105379:	8b 45 08             	mov    0x8(%ebp),%eax
8010537c:	c6 00 00             	movb   $0x0,(%eax)
  return os;
8010537f:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105382:	c9                   	leave  
80105383:	c3                   	ret    

80105384 <strlen>:

int
strlen(const char *s)
{
80105384:	55                   	push   %ebp
80105385:	89 e5                	mov    %esp,%ebp
80105387:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
8010538a:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80105391:	eb 04                	jmp    80105397 <strlen+0x13>
80105393:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105397:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010539a:	03 45 08             	add    0x8(%ebp),%eax
8010539d:	0f b6 00             	movzbl (%eax),%eax
801053a0:	84 c0                	test   %al,%al
801053a2:	75 ef                	jne    80105393 <strlen+0xf>
    ;
  return n;
801053a4:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801053a7:	c9                   	leave  
801053a8:	c3                   	ret    
801053a9:	00 00                	add    %al,(%eax)
	...

801053ac <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
801053ac:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
801053b0:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
801053b4:	55                   	push   %ebp
  pushl %ebx
801053b5:	53                   	push   %ebx
  pushl %esi
801053b6:	56                   	push   %esi
  pushl %edi
801053b7:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
801053b8:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
801053ba:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
801053bc:	5f                   	pop    %edi
  popl %esi
801053bd:	5e                   	pop    %esi
  popl %ebx
801053be:	5b                   	pop    %ebx
  popl %ebp
801053bf:	5d                   	pop    %ebp
  ret
801053c0:	c3                   	ret    
801053c1:	00 00                	add    %al,(%eax)
	...

801053c4 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from process p.
int
fetchint(struct proc *p, uint addr, int *ip)
{
801053c4:	55                   	push   %ebp
801053c5:	89 e5                	mov    %esp,%ebp
  if(addr >= p->sz || addr+4 > p->sz)
801053c7:	8b 45 08             	mov    0x8(%ebp),%eax
801053ca:	8b 00                	mov    (%eax),%eax
801053cc:	3b 45 0c             	cmp    0xc(%ebp),%eax
801053cf:	76 0f                	jbe    801053e0 <fetchint+0x1c>
801053d1:	8b 45 0c             	mov    0xc(%ebp),%eax
801053d4:	8d 50 04             	lea    0x4(%eax),%edx
801053d7:	8b 45 08             	mov    0x8(%ebp),%eax
801053da:	8b 00                	mov    (%eax),%eax
801053dc:	39 c2                	cmp    %eax,%edx
801053de:	76 07                	jbe    801053e7 <fetchint+0x23>
    return -1;
801053e0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801053e5:	eb 0f                	jmp    801053f6 <fetchint+0x32>
  *ip = *(int*)(addr);
801053e7:	8b 45 0c             	mov    0xc(%ebp),%eax
801053ea:	8b 10                	mov    (%eax),%edx
801053ec:	8b 45 10             	mov    0x10(%ebp),%eax
801053ef:	89 10                	mov    %edx,(%eax)
  return 0;
801053f1:	b8 00 00 00 00       	mov    $0x0,%eax
}
801053f6:	5d                   	pop    %ebp
801053f7:	c3                   	ret    

801053f8 <fetchstr>:
// Fetch the nul-terminated string at addr from process p.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(struct proc *p, uint addr, char **pp)
{
801053f8:	55                   	push   %ebp
801053f9:	89 e5                	mov    %esp,%ebp
801053fb:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= p->sz)
801053fe:	8b 45 08             	mov    0x8(%ebp),%eax
80105401:	8b 00                	mov    (%eax),%eax
80105403:	3b 45 0c             	cmp    0xc(%ebp),%eax
80105406:	77 07                	ja     8010540f <fetchstr+0x17>
    return -1;
80105408:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010540d:	eb 45                	jmp    80105454 <fetchstr+0x5c>
  *pp = (char*)addr;
8010540f:	8b 55 0c             	mov    0xc(%ebp),%edx
80105412:	8b 45 10             	mov    0x10(%ebp),%eax
80105415:	89 10                	mov    %edx,(%eax)
  ep = (char*)p->sz;
80105417:	8b 45 08             	mov    0x8(%ebp),%eax
8010541a:	8b 00                	mov    (%eax),%eax
8010541c:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
8010541f:	8b 45 10             	mov    0x10(%ebp),%eax
80105422:	8b 00                	mov    (%eax),%eax
80105424:	89 45 fc             	mov    %eax,-0x4(%ebp)
80105427:	eb 1e                	jmp    80105447 <fetchstr+0x4f>
    if(*s == 0)
80105429:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010542c:	0f b6 00             	movzbl (%eax),%eax
8010542f:	84 c0                	test   %al,%al
80105431:	75 10                	jne    80105443 <fetchstr+0x4b>
      return s - *pp;
80105433:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105436:	8b 45 10             	mov    0x10(%ebp),%eax
80105439:	8b 00                	mov    (%eax),%eax
8010543b:	89 d1                	mov    %edx,%ecx
8010543d:	29 c1                	sub    %eax,%ecx
8010543f:	89 c8                	mov    %ecx,%eax
80105441:	eb 11                	jmp    80105454 <fetchstr+0x5c>

  if(addr >= p->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)p->sz;
  for(s = *pp; s < ep; s++)
80105443:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105447:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010544a:	3b 45 f8             	cmp    -0x8(%ebp),%eax
8010544d:	72 da                	jb     80105429 <fetchstr+0x31>
    if(*s == 0)
      return s - *pp;
  return -1;
8010544f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105454:	c9                   	leave  
80105455:	c3                   	ret    

80105456 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
80105456:	55                   	push   %ebp
80105457:	89 e5                	mov    %esp,%ebp
80105459:	83 ec 0c             	sub    $0xc,%esp
  return fetchint(proc, proc->tf->esp + 4 + 4*n, ip);
8010545c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105462:	8b 40 18             	mov    0x18(%eax),%eax
80105465:	8b 50 44             	mov    0x44(%eax),%edx
80105468:	8b 45 08             	mov    0x8(%ebp),%eax
8010546b:	c1 e0 02             	shl    $0x2,%eax
8010546e:	01 d0                	add    %edx,%eax
80105470:	8d 48 04             	lea    0x4(%eax),%ecx
80105473:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105479:	8b 55 0c             	mov    0xc(%ebp),%edx
8010547c:	89 54 24 08          	mov    %edx,0x8(%esp)
80105480:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80105484:	89 04 24             	mov    %eax,(%esp)
80105487:	e8 38 ff ff ff       	call   801053c4 <fetchint>
}
8010548c:	c9                   	leave  
8010548d:	c3                   	ret    

8010548e <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
8010548e:	55                   	push   %ebp
8010548f:	89 e5                	mov    %esp,%ebp
80105491:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
80105494:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105497:	89 44 24 04          	mov    %eax,0x4(%esp)
8010549b:	8b 45 08             	mov    0x8(%ebp),%eax
8010549e:	89 04 24             	mov    %eax,(%esp)
801054a1:	e8 b0 ff ff ff       	call   80105456 <argint>
801054a6:	85 c0                	test   %eax,%eax
801054a8:	79 07                	jns    801054b1 <argptr+0x23>
    return -1;
801054aa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054af:	eb 3d                	jmp    801054ee <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
801054b1:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054b4:	89 c2                	mov    %eax,%edx
801054b6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054bc:	8b 00                	mov    (%eax),%eax
801054be:	39 c2                	cmp    %eax,%edx
801054c0:	73 16                	jae    801054d8 <argptr+0x4a>
801054c2:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054c5:	89 c2                	mov    %eax,%edx
801054c7:	8b 45 10             	mov    0x10(%ebp),%eax
801054ca:	01 c2                	add    %eax,%edx
801054cc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054d2:	8b 00                	mov    (%eax),%eax
801054d4:	39 c2                	cmp    %eax,%edx
801054d6:	76 07                	jbe    801054df <argptr+0x51>
    return -1;
801054d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054dd:	eb 0f                	jmp    801054ee <argptr+0x60>
  *pp = (char*)i;
801054df:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054e2:	89 c2                	mov    %eax,%edx
801054e4:	8b 45 0c             	mov    0xc(%ebp),%eax
801054e7:	89 10                	mov    %edx,(%eax)
  return 0;
801054e9:	b8 00 00 00 00       	mov    $0x0,%eax
}
801054ee:	c9                   	leave  
801054ef:	c3                   	ret    

801054f0 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
801054f0:	55                   	push   %ebp
801054f1:	89 e5                	mov    %esp,%ebp
801054f3:	83 ec 1c             	sub    $0x1c,%esp
  int addr;
  if(argint(n, &addr) < 0)
801054f6:	8d 45 fc             	lea    -0x4(%ebp),%eax
801054f9:	89 44 24 04          	mov    %eax,0x4(%esp)
801054fd:	8b 45 08             	mov    0x8(%ebp),%eax
80105500:	89 04 24             	mov    %eax,(%esp)
80105503:	e8 4e ff ff ff       	call   80105456 <argint>
80105508:	85 c0                	test   %eax,%eax
8010550a:	79 07                	jns    80105513 <argstr+0x23>
    return -1;
8010550c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105511:	eb 1e                	jmp    80105531 <argstr+0x41>
  return fetchstr(proc, addr, pp);
80105513:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105516:	89 c2                	mov    %eax,%edx
80105518:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010551e:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105521:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105525:	89 54 24 04          	mov    %edx,0x4(%esp)
80105529:	89 04 24             	mov    %eax,(%esp)
8010552c:	e8 c7 fe ff ff       	call   801053f8 <fetchstr>
}
80105531:	c9                   	leave  
80105532:	c3                   	ret    

80105533 <syscall>:
[SYS_disableSwapping]	sys_disableSwapping,
};

void
syscall(void)
{
80105533:	55                   	push   %ebp
80105534:	89 e5                	mov    %esp,%ebp
80105536:	53                   	push   %ebx
80105537:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
8010553a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105540:	8b 40 18             	mov    0x18(%eax),%eax
80105543:	8b 40 1c             	mov    0x1c(%eax),%eax
80105546:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num >= 0 && num < SYS_open && syscalls[num]) {
80105549:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010554d:	78 2e                	js     8010557d <syscall+0x4a>
8010554f:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
80105553:	7f 28                	jg     8010557d <syscall+0x4a>
80105555:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105558:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
8010555f:	85 c0                	test   %eax,%eax
80105561:	74 1a                	je     8010557d <syscall+0x4a>
    proc->tf->eax = syscalls[num]();
80105563:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105569:	8b 58 18             	mov    0x18(%eax),%ebx
8010556c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010556f:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105576:	ff d0                	call   *%eax
80105578:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010557b:	eb 73                	jmp    801055f0 <syscall+0xbd>
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
8010557d:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
80105581:	7e 30                	jle    801055b3 <syscall+0x80>
80105583:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105586:	83 f8 17             	cmp    $0x17,%eax
80105589:	77 28                	ja     801055b3 <syscall+0x80>
8010558b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010558e:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105595:	85 c0                	test   %eax,%eax
80105597:	74 1a                	je     801055b3 <syscall+0x80>
    proc->tf->eax = syscalls[num]();
80105599:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010559f:	8b 58 18             	mov    0x18(%eax),%ebx
801055a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055a5:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
801055ac:	ff d0                	call   *%eax
801055ae:	89 43 1c             	mov    %eax,0x1c(%ebx)
801055b1:	eb 3d                	jmp    801055f0 <syscall+0xbd>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
801055b3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055b9:	8d 48 6c             	lea    0x6c(%eax),%ecx
801055bc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
  if(num >= 0 && num < SYS_open && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
801055c2:	8b 40 10             	mov    0x10(%eax),%eax
801055c5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801055c8:	89 54 24 0c          	mov    %edx,0xc(%esp)
801055cc:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801055d0:	89 44 24 04          	mov    %eax,0x4(%esp)
801055d4:	c7 04 24 3a 8a 10 80 	movl   $0x80108a3a,(%esp)
801055db:	e8 c1 ad ff ff       	call   801003a1 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
801055e0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055e6:	8b 40 18             	mov    0x18(%eax),%eax
801055e9:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
801055f0:	83 c4 24             	add    $0x24,%esp
801055f3:	5b                   	pop    %ebx
801055f4:	5d                   	pop    %ebp
801055f5:	c3                   	ret    
	...

801055f8 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
801055f8:	55                   	push   %ebp
801055f9:	89 e5                	mov    %esp,%ebp
801055fb:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
801055fe:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105601:	89 44 24 04          	mov    %eax,0x4(%esp)
80105605:	8b 45 08             	mov    0x8(%ebp),%eax
80105608:	89 04 24             	mov    %eax,(%esp)
8010560b:	e8 46 fe ff ff       	call   80105456 <argint>
80105610:	85 c0                	test   %eax,%eax
80105612:	79 07                	jns    8010561b <argfd+0x23>
    return -1;
80105614:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105619:	eb 50                	jmp    8010566b <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
8010561b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010561e:	85 c0                	test   %eax,%eax
80105620:	78 21                	js     80105643 <argfd+0x4b>
80105622:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105625:	83 f8 0f             	cmp    $0xf,%eax
80105628:	7f 19                	jg     80105643 <argfd+0x4b>
8010562a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105630:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105633:	83 c2 08             	add    $0x8,%edx
80105636:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
8010563a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010563d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105641:	75 07                	jne    8010564a <argfd+0x52>
    return -1;
80105643:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105648:	eb 21                	jmp    8010566b <argfd+0x73>
  if(pfd)
8010564a:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
8010564e:	74 08                	je     80105658 <argfd+0x60>
    *pfd = fd;
80105650:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105653:	8b 45 0c             	mov    0xc(%ebp),%eax
80105656:	89 10                	mov    %edx,(%eax)
  if(pf)
80105658:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010565c:	74 08                	je     80105666 <argfd+0x6e>
    *pf = f;
8010565e:	8b 45 10             	mov    0x10(%ebp),%eax
80105661:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105664:	89 10                	mov    %edx,(%eax)
  return 0;
80105666:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010566b:	c9                   	leave  
8010566c:	c3                   	ret    

8010566d <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
8010566d:	55                   	push   %ebp
8010566e:	89 e5                	mov    %esp,%ebp
80105670:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105673:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010567a:	eb 30                	jmp    801056ac <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
8010567c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105682:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105685:	83 c2 08             	add    $0x8,%edx
80105688:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
8010568c:	85 c0                	test   %eax,%eax
8010568e:	75 18                	jne    801056a8 <fdalloc+0x3b>
      proc->ofile[fd] = f;
80105690:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105696:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105699:	8d 4a 08             	lea    0x8(%edx),%ecx
8010569c:	8b 55 08             	mov    0x8(%ebp),%edx
8010569f:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
801056a3:	8b 45 fc             	mov    -0x4(%ebp),%eax
801056a6:	eb 0f                	jmp    801056b7 <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
801056a8:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801056ac:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
801056b0:	7e ca                	jle    8010567c <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
801056b2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801056b7:	c9                   	leave  
801056b8:	c3                   	ret    

801056b9 <sys_dup>:

int
sys_dup(void)
{
801056b9:	55                   	push   %ebp
801056ba:	89 e5                	mov    %esp,%ebp
801056bc:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
801056bf:	8d 45 f0             	lea    -0x10(%ebp),%eax
801056c2:	89 44 24 08          	mov    %eax,0x8(%esp)
801056c6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801056cd:	00 
801056ce:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801056d5:	e8 1e ff ff ff       	call   801055f8 <argfd>
801056da:	85 c0                	test   %eax,%eax
801056dc:	79 07                	jns    801056e5 <sys_dup+0x2c>
    return -1;
801056de:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056e3:	eb 29                	jmp    8010570e <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
801056e5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801056e8:	89 04 24             	mov    %eax,(%esp)
801056eb:	e8 7d ff ff ff       	call   8010566d <fdalloc>
801056f0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801056f3:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801056f7:	79 07                	jns    80105700 <sys_dup+0x47>
    return -1;
801056f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056fe:	eb 0e                	jmp    8010570e <sys_dup+0x55>
  filedup(f);
80105700:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105703:	89 04 24             	mov    %eax,(%esp)
80105706:	e8 71 b8 ff ff       	call   80100f7c <filedup>
  return fd;
8010570b:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010570e:	c9                   	leave  
8010570f:	c3                   	ret    

80105710 <sys_read>:

int
sys_read(void)
{
80105710:	55                   	push   %ebp
80105711:	89 e5                	mov    %esp,%ebp
80105713:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105716:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105719:	89 44 24 08          	mov    %eax,0x8(%esp)
8010571d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105724:	00 
80105725:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010572c:	e8 c7 fe ff ff       	call   801055f8 <argfd>
80105731:	85 c0                	test   %eax,%eax
80105733:	78 35                	js     8010576a <sys_read+0x5a>
80105735:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105738:	89 44 24 04          	mov    %eax,0x4(%esp)
8010573c:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105743:	e8 0e fd ff ff       	call   80105456 <argint>
80105748:	85 c0                	test   %eax,%eax
8010574a:	78 1e                	js     8010576a <sys_read+0x5a>
8010574c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010574f:	89 44 24 08          	mov    %eax,0x8(%esp)
80105753:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105756:	89 44 24 04          	mov    %eax,0x4(%esp)
8010575a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105761:	e8 28 fd ff ff       	call   8010548e <argptr>
80105766:	85 c0                	test   %eax,%eax
80105768:	79 07                	jns    80105771 <sys_read+0x61>
    return -1;
8010576a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010576f:	eb 19                	jmp    8010578a <sys_read+0x7a>
  return fileread(f, p, n);
80105771:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80105774:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105777:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010577a:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010577e:	89 54 24 04          	mov    %edx,0x4(%esp)
80105782:	89 04 24             	mov    %eax,(%esp)
80105785:	e8 5f b9 ff ff       	call   801010e9 <fileread>
}
8010578a:	c9                   	leave  
8010578b:	c3                   	ret    

8010578c <sys_write>:

int
sys_write(void)
{
8010578c:	55                   	push   %ebp
8010578d:	89 e5                	mov    %esp,%ebp
8010578f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105792:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105795:	89 44 24 08          	mov    %eax,0x8(%esp)
80105799:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801057a0:	00 
801057a1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801057a8:	e8 4b fe ff ff       	call   801055f8 <argfd>
801057ad:	85 c0                	test   %eax,%eax
801057af:	78 35                	js     801057e6 <sys_write+0x5a>
801057b1:	8d 45 f0             	lea    -0x10(%ebp),%eax
801057b4:	89 44 24 04          	mov    %eax,0x4(%esp)
801057b8:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801057bf:	e8 92 fc ff ff       	call   80105456 <argint>
801057c4:	85 c0                	test   %eax,%eax
801057c6:	78 1e                	js     801057e6 <sys_write+0x5a>
801057c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057cb:	89 44 24 08          	mov    %eax,0x8(%esp)
801057cf:	8d 45 ec             	lea    -0x14(%ebp),%eax
801057d2:	89 44 24 04          	mov    %eax,0x4(%esp)
801057d6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801057dd:	e8 ac fc ff ff       	call   8010548e <argptr>
801057e2:	85 c0                	test   %eax,%eax
801057e4:	79 07                	jns    801057ed <sys_write+0x61>
    return -1;
801057e6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057eb:	eb 19                	jmp    80105806 <sys_write+0x7a>
  return filewrite(f, p, n);
801057ed:	8b 4d f0             	mov    -0x10(%ebp),%ecx
801057f0:	8b 55 ec             	mov    -0x14(%ebp),%edx
801057f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057f6:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801057fa:	89 54 24 04          	mov    %edx,0x4(%esp)
801057fe:	89 04 24             	mov    %eax,(%esp)
80105801:	e8 9f b9 ff ff       	call   801011a5 <filewrite>
}
80105806:	c9                   	leave  
80105807:	c3                   	ret    

80105808 <sys_close>:

int
sys_close(void)
{
80105808:	55                   	push   %ebp
80105809:	89 e5                	mov    %esp,%ebp
8010580b:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
8010580e:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105811:	89 44 24 08          	mov    %eax,0x8(%esp)
80105815:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105818:	89 44 24 04          	mov    %eax,0x4(%esp)
8010581c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105823:	e8 d0 fd ff ff       	call   801055f8 <argfd>
80105828:	85 c0                	test   %eax,%eax
8010582a:	79 07                	jns    80105833 <sys_close+0x2b>
    return -1;
8010582c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105831:	eb 24                	jmp    80105857 <sys_close+0x4f>
  proc->ofile[fd] = 0;
80105833:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105839:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010583c:	83 c2 08             	add    $0x8,%edx
8010583f:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80105846:	00 
  fileclose(f);
80105847:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010584a:	89 04 24             	mov    %eax,(%esp)
8010584d:	e8 72 b7 ff ff       	call   80100fc4 <fileclose>
  return 0;
80105852:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105857:	c9                   	leave  
80105858:	c3                   	ret    

80105859 <sys_fstat>:

int
sys_fstat(void)
{
80105859:	55                   	push   %ebp
8010585a:	89 e5                	mov    %esp,%ebp
8010585c:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
8010585f:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105862:	89 44 24 08          	mov    %eax,0x8(%esp)
80105866:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010586d:	00 
8010586e:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105875:	e8 7e fd ff ff       	call   801055f8 <argfd>
8010587a:	85 c0                	test   %eax,%eax
8010587c:	78 1f                	js     8010589d <sys_fstat+0x44>
8010587e:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80105885:	00 
80105886:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105889:	89 44 24 04          	mov    %eax,0x4(%esp)
8010588d:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105894:	e8 f5 fb ff ff       	call   8010548e <argptr>
80105899:	85 c0                	test   %eax,%eax
8010589b:	79 07                	jns    801058a4 <sys_fstat+0x4b>
    return -1;
8010589d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801058a2:	eb 12                	jmp    801058b6 <sys_fstat+0x5d>
  return filestat(f, st);
801058a4:	8b 55 f0             	mov    -0x10(%ebp),%edx
801058a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058aa:	89 54 24 04          	mov    %edx,0x4(%esp)
801058ae:	89 04 24             	mov    %eax,(%esp)
801058b1:	e8 e4 b7 ff ff       	call   8010109a <filestat>
}
801058b6:	c9                   	leave  
801058b7:	c3                   	ret    

801058b8 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
801058b8:	55                   	push   %ebp
801058b9:	89 e5                	mov    %esp,%ebp
801058bb:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801058be:	8d 45 d8             	lea    -0x28(%ebp),%eax
801058c1:	89 44 24 04          	mov    %eax,0x4(%esp)
801058c5:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801058cc:	e8 1f fc ff ff       	call   801054f0 <argstr>
801058d1:	85 c0                	test   %eax,%eax
801058d3:	78 17                	js     801058ec <sys_link+0x34>
801058d5:	8d 45 dc             	lea    -0x24(%ebp),%eax
801058d8:	89 44 24 04          	mov    %eax,0x4(%esp)
801058dc:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801058e3:	e8 08 fc ff ff       	call   801054f0 <argstr>
801058e8:	85 c0                	test   %eax,%eax
801058ea:	79 0a                	jns    801058f6 <sys_link+0x3e>
    return -1;
801058ec:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801058f1:	e9 3c 01 00 00       	jmp    80105a32 <sys_link+0x17a>
  if((ip = namei(old)) == 0)
801058f6:	8b 45 d8             	mov    -0x28(%ebp),%eax
801058f9:	89 04 24             	mov    %eax,(%esp)
801058fc:	e8 09 cb ff ff       	call   8010240a <namei>
80105901:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105904:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105908:	75 0a                	jne    80105914 <sys_link+0x5c>
    return -1;
8010590a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010590f:	e9 1e 01 00 00       	jmp    80105a32 <sys_link+0x17a>

  begin_trans();
80105914:	e8 04 d9 ff ff       	call   8010321d <begin_trans>

  ilock(ip);
80105919:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010591c:	89 04 24             	mov    %eax,(%esp)
8010591f:	e8 44 bf ff ff       	call   80101868 <ilock>
  if(ip->type == T_DIR){
80105924:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105927:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010592b:	66 83 f8 01          	cmp    $0x1,%ax
8010592f:	75 1a                	jne    8010594b <sys_link+0x93>
    iunlockput(ip);
80105931:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105934:	89 04 24             	mov    %eax,(%esp)
80105937:	e8 b0 c1 ff ff       	call   80101aec <iunlockput>
    commit_trans();
8010593c:	e8 10 d9 ff ff       	call   80103251 <commit_trans>
    return -1;
80105941:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105946:	e9 e7 00 00 00       	jmp    80105a32 <sys_link+0x17a>
  }

  ip->nlink++;
8010594b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010594e:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105952:	8d 50 01             	lea    0x1(%eax),%edx
80105955:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105958:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
8010595c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010595f:	89 04 24             	mov    %eax,(%esp)
80105962:	e8 45 bd ff ff       	call   801016ac <iupdate>
  iunlock(ip);
80105967:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010596a:	89 04 24             	mov    %eax,(%esp)
8010596d:	e8 44 c0 ff ff       	call   801019b6 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
80105972:	8b 45 dc             	mov    -0x24(%ebp),%eax
80105975:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80105978:	89 54 24 04          	mov    %edx,0x4(%esp)
8010597c:	89 04 24             	mov    %eax,(%esp)
8010597f:	e8 a8 ca ff ff       	call   8010242c <nameiparent>
80105984:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105987:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010598b:	74 68                	je     801059f5 <sys_link+0x13d>
    goto bad;
  ilock(dp);
8010598d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105990:	89 04 24             	mov    %eax,(%esp)
80105993:	e8 d0 be ff ff       	call   80101868 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80105998:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010599b:	8b 10                	mov    (%eax),%edx
8010599d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059a0:	8b 00                	mov    (%eax),%eax
801059a2:	39 c2                	cmp    %eax,%edx
801059a4:	75 20                	jne    801059c6 <sys_link+0x10e>
801059a6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059a9:	8b 40 04             	mov    0x4(%eax),%eax
801059ac:	89 44 24 08          	mov    %eax,0x8(%esp)
801059b0:	8d 45 e2             	lea    -0x1e(%ebp),%eax
801059b3:	89 44 24 04          	mov    %eax,0x4(%esp)
801059b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059ba:	89 04 24             	mov    %eax,(%esp)
801059bd:	e8 87 c7 ff ff       	call   80102149 <dirlink>
801059c2:	85 c0                	test   %eax,%eax
801059c4:	79 0d                	jns    801059d3 <sys_link+0x11b>
    iunlockput(dp);
801059c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059c9:	89 04 24             	mov    %eax,(%esp)
801059cc:	e8 1b c1 ff ff       	call   80101aec <iunlockput>
    goto bad;
801059d1:	eb 23                	jmp    801059f6 <sys_link+0x13e>
  }
  iunlockput(dp);
801059d3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059d6:	89 04 24             	mov    %eax,(%esp)
801059d9:	e8 0e c1 ff ff       	call   80101aec <iunlockput>
  iput(ip);
801059de:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059e1:	89 04 24             	mov    %eax,(%esp)
801059e4:	e8 32 c0 ff ff       	call   80101a1b <iput>

  commit_trans();
801059e9:	e8 63 d8 ff ff       	call   80103251 <commit_trans>

  return 0;
801059ee:	b8 00 00 00 00       	mov    $0x0,%eax
801059f3:	eb 3d                	jmp    80105a32 <sys_link+0x17a>
  ip->nlink++;
  iupdate(ip);
  iunlock(ip);

  if((dp = nameiparent(new, name)) == 0)
    goto bad;
801059f5:	90                   	nop
  commit_trans();

  return 0;

bad:
  ilock(ip);
801059f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059f9:	89 04 24             	mov    %eax,(%esp)
801059fc:	e8 67 be ff ff       	call   80101868 <ilock>
  ip->nlink--;
80105a01:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a04:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105a08:	8d 50 ff             	lea    -0x1(%eax),%edx
80105a0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a0e:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105a12:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a15:	89 04 24             	mov    %eax,(%esp)
80105a18:	e8 8f bc ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80105a1d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a20:	89 04 24             	mov    %eax,(%esp)
80105a23:	e8 c4 c0 ff ff       	call   80101aec <iunlockput>
  commit_trans();
80105a28:	e8 24 d8 ff ff       	call   80103251 <commit_trans>
  return -1;
80105a2d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105a32:	c9                   	leave  
80105a33:	c3                   	ret    

80105a34 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
80105a34:	55                   	push   %ebp
80105a35:	89 e5                	mov    %esp,%ebp
80105a37:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105a3a:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80105a41:	eb 4b                	jmp    80105a8e <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105a43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a46:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105a4d:	00 
80105a4e:	89 44 24 08          	mov    %eax,0x8(%esp)
80105a52:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105a55:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a59:	8b 45 08             	mov    0x8(%ebp),%eax
80105a5c:	89 04 24             	mov    %eax,(%esp)
80105a5f:	e8 fa c2 ff ff       	call   80101d5e <readi>
80105a64:	83 f8 10             	cmp    $0x10,%eax
80105a67:	74 0c                	je     80105a75 <isdirempty+0x41>
      panic("isdirempty: readi");
80105a69:	c7 04 24 56 8a 10 80 	movl   $0x80108a56,(%esp)
80105a70:	e8 c8 aa ff ff       	call   8010053d <panic>
    if(de.inum != 0)
80105a75:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80105a79:	66 85 c0             	test   %ax,%ax
80105a7c:	74 07                	je     80105a85 <isdirempty+0x51>
      return 0;
80105a7e:	b8 00 00 00 00       	mov    $0x0,%eax
80105a83:	eb 1b                	jmp    80105aa0 <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105a85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a88:	83 c0 10             	add    $0x10,%eax
80105a8b:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105a8e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105a91:	8b 45 08             	mov    0x8(%ebp),%eax
80105a94:	8b 40 18             	mov    0x18(%eax),%eax
80105a97:	39 c2                	cmp    %eax,%edx
80105a99:	72 a8                	jb     80105a43 <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
80105a9b:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105aa0:	c9                   	leave  
80105aa1:	c3                   	ret    

80105aa2 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105aa2:	55                   	push   %ebp
80105aa3:	89 e5                	mov    %esp,%ebp
80105aa5:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80105aa8:	8d 45 cc             	lea    -0x34(%ebp),%eax
80105aab:	89 44 24 04          	mov    %eax,0x4(%esp)
80105aaf:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105ab6:	e8 35 fa ff ff       	call   801054f0 <argstr>
80105abb:	85 c0                	test   %eax,%eax
80105abd:	79 0a                	jns    80105ac9 <sys_unlink+0x27>
    return -1;
80105abf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105ac4:	e9 aa 01 00 00       	jmp    80105c73 <sys_unlink+0x1d1>
  if((dp = nameiparent(path, name)) == 0)
80105ac9:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105acc:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105acf:	89 54 24 04          	mov    %edx,0x4(%esp)
80105ad3:	89 04 24             	mov    %eax,(%esp)
80105ad6:	e8 51 c9 ff ff       	call   8010242c <nameiparent>
80105adb:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105ade:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105ae2:	75 0a                	jne    80105aee <sys_unlink+0x4c>
    return -1;
80105ae4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105ae9:	e9 85 01 00 00       	jmp    80105c73 <sys_unlink+0x1d1>

  begin_trans();
80105aee:	e8 2a d7 ff ff       	call   8010321d <begin_trans>

  ilock(dp);
80105af3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105af6:	89 04 24             	mov    %eax,(%esp)
80105af9:	e8 6a bd ff ff       	call   80101868 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105afe:	c7 44 24 04 68 8a 10 	movl   $0x80108a68,0x4(%esp)
80105b05:	80 
80105b06:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105b09:	89 04 24             	mov    %eax,(%esp)
80105b0c:	e8 4e c5 ff ff       	call   8010205f <namecmp>
80105b11:	85 c0                	test   %eax,%eax
80105b13:	0f 84 45 01 00 00    	je     80105c5e <sys_unlink+0x1bc>
80105b19:	c7 44 24 04 6a 8a 10 	movl   $0x80108a6a,0x4(%esp)
80105b20:	80 
80105b21:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105b24:	89 04 24             	mov    %eax,(%esp)
80105b27:	e8 33 c5 ff ff       	call   8010205f <namecmp>
80105b2c:	85 c0                	test   %eax,%eax
80105b2e:	0f 84 2a 01 00 00    	je     80105c5e <sys_unlink+0x1bc>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
80105b34:	8d 45 c8             	lea    -0x38(%ebp),%eax
80105b37:	89 44 24 08          	mov    %eax,0x8(%esp)
80105b3b:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105b3e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b42:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b45:	89 04 24             	mov    %eax,(%esp)
80105b48:	e8 34 c5 ff ff       	call   80102081 <dirlookup>
80105b4d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105b50:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105b54:	0f 84 03 01 00 00    	je     80105c5d <sys_unlink+0x1bb>
    goto bad;
  ilock(ip);
80105b5a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b5d:	89 04 24             	mov    %eax,(%esp)
80105b60:	e8 03 bd ff ff       	call   80101868 <ilock>

  if(ip->nlink < 1)
80105b65:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b68:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105b6c:	66 85 c0             	test   %ax,%ax
80105b6f:	7f 0c                	jg     80105b7d <sys_unlink+0xdb>
    panic("unlink: nlink < 1");
80105b71:	c7 04 24 6d 8a 10 80 	movl   $0x80108a6d,(%esp)
80105b78:	e8 c0 a9 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105b7d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b80:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105b84:	66 83 f8 01          	cmp    $0x1,%ax
80105b88:	75 1f                	jne    80105ba9 <sys_unlink+0x107>
80105b8a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b8d:	89 04 24             	mov    %eax,(%esp)
80105b90:	e8 9f fe ff ff       	call   80105a34 <isdirempty>
80105b95:	85 c0                	test   %eax,%eax
80105b97:	75 10                	jne    80105ba9 <sys_unlink+0x107>
    iunlockput(ip);
80105b99:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b9c:	89 04 24             	mov    %eax,(%esp)
80105b9f:	e8 48 bf ff ff       	call   80101aec <iunlockput>
    goto bad;
80105ba4:	e9 b5 00 00 00       	jmp    80105c5e <sys_unlink+0x1bc>
  }

  memset(&de, 0, sizeof(de));
80105ba9:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105bb0:	00 
80105bb1:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105bb8:	00 
80105bb9:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105bbc:	89 04 24             	mov    %eax,(%esp)
80105bbf:	e8 42 f5 ff ff       	call   80105106 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105bc4:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105bc7:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105bce:	00 
80105bcf:	89 44 24 08          	mov    %eax,0x8(%esp)
80105bd3:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105bd6:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bda:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bdd:	89 04 24             	mov    %eax,(%esp)
80105be0:	e8 e4 c2 ff ff       	call   80101ec9 <writei>
80105be5:	83 f8 10             	cmp    $0x10,%eax
80105be8:	74 0c                	je     80105bf6 <sys_unlink+0x154>
    panic("unlink: writei");
80105bea:	c7 04 24 7f 8a 10 80 	movl   $0x80108a7f,(%esp)
80105bf1:	e8 47 a9 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR){
80105bf6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105bf9:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105bfd:	66 83 f8 01          	cmp    $0x1,%ax
80105c01:	75 1c                	jne    80105c1f <sys_unlink+0x17d>
    dp->nlink--;
80105c03:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c06:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105c0a:	8d 50 ff             	lea    -0x1(%eax),%edx
80105c0d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c10:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105c14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c17:	89 04 24             	mov    %eax,(%esp)
80105c1a:	e8 8d ba ff ff       	call   801016ac <iupdate>
  }
  iunlockput(dp);
80105c1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c22:	89 04 24             	mov    %eax,(%esp)
80105c25:	e8 c2 be ff ff       	call   80101aec <iunlockput>

  ip->nlink--;
80105c2a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c2d:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105c31:	8d 50 ff             	lea    -0x1(%eax),%edx
80105c34:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c37:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105c3b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c3e:	89 04 24             	mov    %eax,(%esp)
80105c41:	e8 66 ba ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80105c46:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c49:	89 04 24             	mov    %eax,(%esp)
80105c4c:	e8 9b be ff ff       	call   80101aec <iunlockput>

  commit_trans();
80105c51:	e8 fb d5 ff ff       	call   80103251 <commit_trans>

  return 0;
80105c56:	b8 00 00 00 00       	mov    $0x0,%eax
80105c5b:	eb 16                	jmp    80105c73 <sys_unlink+0x1d1>
  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
    goto bad;
80105c5d:	90                   	nop
  commit_trans();

  return 0;

bad:
  iunlockput(dp);
80105c5e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c61:	89 04 24             	mov    %eax,(%esp)
80105c64:	e8 83 be ff ff       	call   80101aec <iunlockput>
  commit_trans();
80105c69:	e8 e3 d5 ff ff       	call   80103251 <commit_trans>
  return -1;
80105c6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105c73:	c9                   	leave  
80105c74:	c3                   	ret    

80105c75 <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105c75:	55                   	push   %ebp
80105c76:	89 e5                	mov    %esp,%ebp
80105c78:	83 ec 48             	sub    $0x48,%esp
80105c7b:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105c7e:	8b 55 10             	mov    0x10(%ebp),%edx
80105c81:	8b 45 14             	mov    0x14(%ebp),%eax
80105c84:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105c88:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105c8c:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];
  if((dp = nameiparent(path, name)) == 0)
80105c90:	8d 45 de             	lea    -0x22(%ebp),%eax
80105c93:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c97:	8b 45 08             	mov    0x8(%ebp),%eax
80105c9a:	89 04 24             	mov    %eax,(%esp)
80105c9d:	e8 8a c7 ff ff       	call   8010242c <nameiparent>
80105ca2:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105ca5:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105ca9:	75 0a                	jne    80105cb5 <create+0x40>
    return 0;
80105cab:	b8 00 00 00 00       	mov    $0x0,%eax
80105cb0:	e9 7e 01 00 00       	jmp    80105e33 <create+0x1be>
  ilock(dp);
80105cb5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cb8:	89 04 24             	mov    %eax,(%esp)
80105cbb:	e8 a8 bb ff ff       	call   80101868 <ilock>
  if((ip = dirlookup(dp, name, &off)) != 0){
80105cc0:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105cc3:	89 44 24 08          	mov    %eax,0x8(%esp)
80105cc7:	8d 45 de             	lea    -0x22(%ebp),%eax
80105cca:	89 44 24 04          	mov    %eax,0x4(%esp)
80105cce:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cd1:	89 04 24             	mov    %eax,(%esp)
80105cd4:	e8 a8 c3 ff ff       	call   80102081 <dirlookup>
80105cd9:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105cdc:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ce0:	74 47                	je     80105d29 <create+0xb4>
    iunlockput(dp);
80105ce2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ce5:	89 04 24             	mov    %eax,(%esp)
80105ce8:	e8 ff bd ff ff       	call   80101aec <iunlockput>
    ilock(ip);
80105ced:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cf0:	89 04 24             	mov    %eax,(%esp)
80105cf3:	e8 70 bb ff ff       	call   80101868 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105cf8:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105cfd:	75 15                	jne    80105d14 <create+0x9f>
80105cff:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d02:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105d06:	66 83 f8 02          	cmp    $0x2,%ax
80105d0a:	75 08                	jne    80105d14 <create+0x9f>
      return ip;
80105d0c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d0f:	e9 1f 01 00 00       	jmp    80105e33 <create+0x1be>
    iunlockput(ip);
80105d14:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d17:	89 04 24             	mov    %eax,(%esp)
80105d1a:	e8 cd bd ff ff       	call   80101aec <iunlockput>
    return 0;
80105d1f:	b8 00 00 00 00       	mov    $0x0,%eax
80105d24:	e9 0a 01 00 00       	jmp    80105e33 <create+0x1be>
  }
  if((ip = ialloc(dp->dev, type)) == 0)
80105d29:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80105d2d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d30:	8b 00                	mov    (%eax),%eax
80105d32:	89 54 24 04          	mov    %edx,0x4(%esp)
80105d36:	89 04 24             	mov    %eax,(%esp)
80105d39:	e8 91 b8 ff ff       	call   801015cf <ialloc>
80105d3e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105d41:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105d45:	75 0c                	jne    80105d53 <create+0xde>
    panic("create: ialloc");
80105d47:	c7 04 24 8e 8a 10 80 	movl   $0x80108a8e,(%esp)
80105d4e:	e8 ea a7 ff ff       	call   8010053d <panic>
  ilock(ip);
80105d53:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d56:	89 04 24             	mov    %eax,(%esp)
80105d59:	e8 0a bb ff ff       	call   80101868 <ilock>
  ip->major = major;
80105d5e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d61:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80105d65:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80105d69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d6c:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80105d70:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80105d74:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d77:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80105d7d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d80:	89 04 24             	mov    %eax,(%esp)
80105d83:	e8 24 b9 ff ff       	call   801016ac <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
80105d88:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105d8d:	75 6a                	jne    80105df9 <create+0x184>
    dp->nlink++;  // for ".."
80105d8f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d92:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105d96:	8d 50 01             	lea    0x1(%eax),%edx
80105d99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d9c:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105da0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105da3:	89 04 24             	mov    %eax,(%esp)
80105da6:	e8 01 b9 ff ff       	call   801016ac <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105dab:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dae:	8b 40 04             	mov    0x4(%eax),%eax
80105db1:	89 44 24 08          	mov    %eax,0x8(%esp)
80105db5:	c7 44 24 04 68 8a 10 	movl   $0x80108a68,0x4(%esp)
80105dbc:	80 
80105dbd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dc0:	89 04 24             	mov    %eax,(%esp)
80105dc3:	e8 81 c3 ff ff       	call   80102149 <dirlink>
80105dc8:	85 c0                	test   %eax,%eax
80105dca:	78 21                	js     80105ded <create+0x178>
80105dcc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dcf:	8b 40 04             	mov    0x4(%eax),%eax
80105dd2:	89 44 24 08          	mov    %eax,0x8(%esp)
80105dd6:	c7 44 24 04 6a 8a 10 	movl   $0x80108a6a,0x4(%esp)
80105ddd:	80 
80105dde:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105de1:	89 04 24             	mov    %eax,(%esp)
80105de4:	e8 60 c3 ff ff       	call   80102149 <dirlink>
80105de9:	85 c0                	test   %eax,%eax
80105deb:	79 0c                	jns    80105df9 <create+0x184>
      panic("create dots");
80105ded:	c7 04 24 9d 8a 10 80 	movl   $0x80108a9d,(%esp)
80105df4:	e8 44 a7 ff ff       	call   8010053d <panic>
  }
  if(dirlink(dp, name, ip->inum) < 0)
80105df9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dfc:	8b 40 04             	mov    0x4(%eax),%eax
80105dff:	89 44 24 08          	mov    %eax,0x8(%esp)
80105e03:	8d 45 de             	lea    -0x22(%ebp),%eax
80105e06:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e0d:	89 04 24             	mov    %eax,(%esp)
80105e10:	e8 34 c3 ff ff       	call   80102149 <dirlink>
80105e15:	85 c0                	test   %eax,%eax
80105e17:	79 0c                	jns    80105e25 <create+0x1b0>
    panic("create: dirlink");
80105e19:	c7 04 24 a9 8a 10 80 	movl   $0x80108aa9,(%esp)
80105e20:	e8 18 a7 ff ff       	call   8010053d <panic>
  iunlockput(dp);
80105e25:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e28:	89 04 24             	mov    %eax,(%esp)
80105e2b:	e8 bc bc ff ff       	call   80101aec <iunlockput>

  return ip;
80105e30:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105e33:	c9                   	leave  
80105e34:	c3                   	ret    

80105e35 <fileopen>:

struct file*
fileopen(char *path, int omode)
{
80105e35:	55                   	push   %ebp
80105e36:	89 e5                	mov    %esp,%ebp
80105e38:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  struct inode *ip;

  if(omode & O_CREATE){
80105e3b:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e3e:	25 00 02 00 00       	and    $0x200,%eax
80105e43:	85 c0                	test   %eax,%eax
80105e45:	74 40                	je     80105e87 <fileopen+0x52>
    begin_trans();
80105e47:	e8 d1 d3 ff ff       	call   8010321d <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80105e4c:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105e53:	00 
80105e54:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105e5b:	00 
80105e5c:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105e63:	00 
80105e64:	8b 45 08             	mov    0x8(%ebp),%eax
80105e67:	89 04 24             	mov    %eax,(%esp)
80105e6a:	e8 06 fe ff ff       	call   80105c75 <create>
80105e6f:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80105e72:	e8 da d3 ff ff       	call   80103251 <commit_trans>
    if(ip == 0)
80105e77:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105e7b:	75 5b                	jne    80105ed8 <fileopen+0xa3>
      return 0;
80105e7d:	b8 00 00 00 00       	mov    $0x0,%eax
80105e82:	e9 f9 00 00 00       	jmp    80105f80 <fileopen+0x14b>
  } else {
    if((ip = namei(path)) == 0)
80105e87:	8b 45 08             	mov    0x8(%ebp),%eax
80105e8a:	89 04 24             	mov    %eax,(%esp)
80105e8d:	e8 78 c5 ff ff       	call   8010240a <namei>
80105e92:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105e95:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105e99:	75 0a                	jne    80105ea5 <fileopen+0x70>
      return 0;
80105e9b:	b8 00 00 00 00       	mov    $0x0,%eax
80105ea0:	e9 db 00 00 00       	jmp    80105f80 <fileopen+0x14b>
    ilock(ip);
80105ea5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ea8:	89 04 24             	mov    %eax,(%esp)
80105eab:	e8 b8 b9 ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105eb0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eb3:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105eb7:	66 83 f8 01          	cmp    $0x1,%ax
80105ebb:	75 1b                	jne    80105ed8 <fileopen+0xa3>
80105ebd:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80105ec1:	74 15                	je     80105ed8 <fileopen+0xa3>
      iunlockput(ip);
80105ec3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ec6:	89 04 24             	mov    %eax,(%esp)
80105ec9:	e8 1e bc ff ff       	call   80101aec <iunlockput>
      return 0;
80105ece:	b8 00 00 00 00       	mov    $0x0,%eax
80105ed3:	e9 a8 00 00 00       	jmp    80105f80 <fileopen+0x14b>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80105ed8:	e8 3f b0 ff ff       	call   80100f1c <filealloc>
80105edd:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105ee0:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ee4:	74 14                	je     80105efa <fileopen+0xc5>
80105ee6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ee9:	89 04 24             	mov    %eax,(%esp)
80105eec:	e8 7c f7 ff ff       	call   8010566d <fdalloc>
80105ef1:	89 45 ec             	mov    %eax,-0x14(%ebp)
80105ef4:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80105ef8:	79 23                	jns    80105f1d <fileopen+0xe8>
    if(f)
80105efa:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105efe:	74 0b                	je     80105f0b <fileopen+0xd6>
      fileclose(f);
80105f00:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f03:	89 04 24             	mov    %eax,(%esp)
80105f06:	e8 b9 b0 ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
80105f0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f0e:	89 04 24             	mov    %eax,(%esp)
80105f11:	e8 d6 bb ff ff       	call   80101aec <iunlockput>
    return 0;
80105f16:	b8 00 00 00 00       	mov    $0x0,%eax
80105f1b:	eb 63                	jmp    80105f80 <fileopen+0x14b>
  }
  iunlock(ip);
80105f1d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f20:	89 04 24             	mov    %eax,(%esp)
80105f23:	e8 8e ba ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
80105f28:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f2b:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80105f31:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f34:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105f37:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80105f3a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f3d:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80105f44:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f47:	83 e0 01             	and    $0x1,%eax
80105f4a:	85 c0                	test   %eax,%eax
80105f4c:	0f 94 c2             	sete   %dl
80105f4f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f52:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105f55:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f58:	83 e0 01             	and    $0x1,%eax
80105f5b:	84 c0                	test   %al,%al
80105f5d:	75 0a                	jne    80105f69 <fileopen+0x134>
80105f5f:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f62:	83 e0 02             	and    $0x2,%eax
80105f65:	85 c0                	test   %eax,%eax
80105f67:	74 07                	je     80105f70 <fileopen+0x13b>
80105f69:	b8 01 00 00 00       	mov    $0x1,%eax
80105f6e:	eb 05                	jmp    80105f75 <fileopen+0x140>
80105f70:	b8 00 00 00 00       	mov    $0x0,%eax
80105f75:	89 c2                	mov    %eax,%edx
80105f77:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f7a:	88 50 09             	mov    %dl,0x9(%eax)
  return f;
80105f7d:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105f80:	c9                   	leave  
80105f81:	c3                   	ret    

80105f82 <sys_open>:

int
sys_open(void)
{
80105f82:	55                   	push   %ebp
80105f83:	89 e5                	mov    %esp,%ebp
80105f85:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105f88:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105f8b:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f8f:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f96:	e8 55 f5 ff ff       	call   801054f0 <argstr>
80105f9b:	85 c0                	test   %eax,%eax
80105f9d:	78 17                	js     80105fb6 <sys_open+0x34>
80105f9f:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105fa2:	89 44 24 04          	mov    %eax,0x4(%esp)
80105fa6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105fad:	e8 a4 f4 ff ff       	call   80105456 <argint>
80105fb2:	85 c0                	test   %eax,%eax
80105fb4:	79 0a                	jns    80105fc0 <sys_open+0x3e>
    return -1;
80105fb6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fbb:	e9 46 01 00 00       	jmp    80106106 <sys_open+0x184>
  if(omode & O_CREATE){
80105fc0:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105fc3:	25 00 02 00 00       	and    $0x200,%eax
80105fc8:	85 c0                	test   %eax,%eax
80105fca:	74 40                	je     8010600c <sys_open+0x8a>
    begin_trans();
80105fcc:	e8 4c d2 ff ff       	call   8010321d <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80105fd1:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105fd4:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105fdb:	00 
80105fdc:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105fe3:	00 
80105fe4:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105feb:	00 
80105fec:	89 04 24             	mov    %eax,(%esp)
80105fef:	e8 81 fc ff ff       	call   80105c75 <create>
80105ff4:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80105ff7:	e8 55 d2 ff ff       	call   80103251 <commit_trans>
    if(ip == 0)
80105ffc:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106000:	75 5c                	jne    8010605e <sys_open+0xdc>
      return -1;
80106002:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106007:	e9 fa 00 00 00       	jmp    80106106 <sys_open+0x184>
  } else {
    if((ip = namei(path)) == 0)
8010600c:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010600f:	89 04 24             	mov    %eax,(%esp)
80106012:	e8 f3 c3 ff ff       	call   8010240a <namei>
80106017:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010601a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010601e:	75 0a                	jne    8010602a <sys_open+0xa8>
      return -1;
80106020:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106025:	e9 dc 00 00 00       	jmp    80106106 <sys_open+0x184>
    ilock(ip);
8010602a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010602d:	89 04 24             	mov    %eax,(%esp)
80106030:	e8 33 b8 ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106035:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106038:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010603c:	66 83 f8 01          	cmp    $0x1,%ax
80106040:	75 1c                	jne    8010605e <sys_open+0xdc>
80106042:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106045:	85 c0                	test   %eax,%eax
80106047:	74 15                	je     8010605e <sys_open+0xdc>
      iunlockput(ip);
80106049:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010604c:	89 04 24             	mov    %eax,(%esp)
8010604f:	e8 98 ba ff ff       	call   80101aec <iunlockput>
      return -1;
80106054:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106059:	e9 a8 00 00 00       	jmp    80106106 <sys_open+0x184>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
8010605e:	e8 b9 ae ff ff       	call   80100f1c <filealloc>
80106063:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106066:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010606a:	74 14                	je     80106080 <sys_open+0xfe>
8010606c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010606f:	89 04 24             	mov    %eax,(%esp)
80106072:	e8 f6 f5 ff ff       	call   8010566d <fdalloc>
80106077:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010607a:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010607e:	79 23                	jns    801060a3 <sys_open+0x121>
    if(f)
80106080:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106084:	74 0b                	je     80106091 <sys_open+0x10f>
      fileclose(f);
80106086:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106089:	89 04 24             	mov    %eax,(%esp)
8010608c:	e8 33 af ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
80106091:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106094:	89 04 24             	mov    %eax,(%esp)
80106097:	e8 50 ba ff ff       	call   80101aec <iunlockput>
    return -1;
8010609c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060a1:	eb 63                	jmp    80106106 <sys_open+0x184>
  }
  iunlock(ip);
801060a3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060a6:	89 04 24             	mov    %eax,(%esp)
801060a9:	e8 08 b9 ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
801060ae:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060b1:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
801060b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060ba:	8b 55 f4             	mov    -0xc(%ebp),%edx
801060bd:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
801060c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060c3:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
801060ca:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060cd:	83 e0 01             	and    $0x1,%eax
801060d0:	85 c0                	test   %eax,%eax
801060d2:	0f 94 c2             	sete   %dl
801060d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060d8:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
801060db:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060de:	83 e0 01             	and    $0x1,%eax
801060e1:	84 c0                	test   %al,%al
801060e3:	75 0a                	jne    801060ef <sys_open+0x16d>
801060e5:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060e8:	83 e0 02             	and    $0x2,%eax
801060eb:	85 c0                	test   %eax,%eax
801060ed:	74 07                	je     801060f6 <sys_open+0x174>
801060ef:	b8 01 00 00 00       	mov    $0x1,%eax
801060f4:	eb 05                	jmp    801060fb <sys_open+0x179>
801060f6:	b8 00 00 00 00       	mov    $0x0,%eax
801060fb:	89 c2                	mov    %eax,%edx
801060fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106100:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80106103:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80106106:	c9                   	leave  
80106107:	c3                   	ret    

80106108 <sys_mkdir>:

int
sys_mkdir(void)
{
80106108:	55                   	push   %ebp
80106109:	89 e5                	mov    %esp,%ebp
8010610b:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
8010610e:	e8 0a d1 ff ff       	call   8010321d <begin_trans>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80106113:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106116:	89 44 24 04          	mov    %eax,0x4(%esp)
8010611a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106121:	e8 ca f3 ff ff       	call   801054f0 <argstr>
80106126:	85 c0                	test   %eax,%eax
80106128:	78 2c                	js     80106156 <sys_mkdir+0x4e>
8010612a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010612d:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106134:	00 
80106135:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010613c:	00 
8010613d:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106144:	00 
80106145:	89 04 24             	mov    %eax,(%esp)
80106148:	e8 28 fb ff ff       	call   80105c75 <create>
8010614d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106150:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106154:	75 0c                	jne    80106162 <sys_mkdir+0x5a>
    commit_trans();
80106156:	e8 f6 d0 ff ff       	call   80103251 <commit_trans>
    return -1;
8010615b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106160:	eb 15                	jmp    80106177 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80106162:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106165:	89 04 24             	mov    %eax,(%esp)
80106168:	e8 7f b9 ff ff       	call   80101aec <iunlockput>
  commit_trans();
8010616d:	e8 df d0 ff ff       	call   80103251 <commit_trans>
  return 0;
80106172:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106177:	c9                   	leave  
80106178:	c3                   	ret    

80106179 <sys_mknod>:

int
sys_mknod(void)
{
80106179:	55                   	push   %ebp
8010617a:	89 e5                	mov    %esp,%ebp
8010617c:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
8010617f:	e8 99 d0 ff ff       	call   8010321d <begin_trans>
  if((len=argstr(0, &path)) < 0 ||
80106184:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106187:	89 44 24 04          	mov    %eax,0x4(%esp)
8010618b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106192:	e8 59 f3 ff ff       	call   801054f0 <argstr>
80106197:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010619a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010619e:	78 5e                	js     801061fe <sys_mknod+0x85>
     argint(1, &major) < 0 ||
801061a0:	8d 45 e8             	lea    -0x18(%ebp),%eax
801061a3:	89 44 24 04          	mov    %eax,0x4(%esp)
801061a7:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801061ae:	e8 a3 f2 ff ff       	call   80105456 <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
801061b3:	85 c0                	test   %eax,%eax
801061b5:	78 47                	js     801061fe <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801061b7:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801061ba:	89 44 24 04          	mov    %eax,0x4(%esp)
801061be:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801061c5:	e8 8c f2 ff ff       	call   80105456 <argint>
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
801061ca:	85 c0                	test   %eax,%eax
801061cc:	78 30                	js     801061fe <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
801061ce:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801061d1:	0f bf c8             	movswl %ax,%ecx
801061d4:	8b 45 e8             	mov    -0x18(%ebp),%eax
801061d7:	0f bf d0             	movswl %ax,%edx
801061da:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801061dd:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801061e1:	89 54 24 08          	mov    %edx,0x8(%esp)
801061e5:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801061ec:	00 
801061ed:	89 04 24             	mov    %eax,(%esp)
801061f0:	e8 80 fa ff ff       	call   80105c75 <create>
801061f5:	89 45 f0             	mov    %eax,-0x10(%ebp)
801061f8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801061fc:	75 0c                	jne    8010620a <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    commit_trans();
801061fe:	e8 4e d0 ff ff       	call   80103251 <commit_trans>
    return -1;
80106203:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106208:	eb 15                	jmp    8010621f <sys_mknod+0xa6>
  }
  iunlockput(ip);
8010620a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010620d:	89 04 24             	mov    %eax,(%esp)
80106210:	e8 d7 b8 ff ff       	call   80101aec <iunlockput>
  commit_trans();
80106215:	e8 37 d0 ff ff       	call   80103251 <commit_trans>
  return 0;
8010621a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010621f:	c9                   	leave  
80106220:	c3                   	ret    

80106221 <sys_chdir>:

int
sys_chdir(void)
{
80106221:	55                   	push   %ebp
80106222:	89 e5                	mov    %esp,%ebp
80106224:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0)
80106227:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010622a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010622e:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106235:	e8 b6 f2 ff ff       	call   801054f0 <argstr>
8010623a:	85 c0                	test   %eax,%eax
8010623c:	78 14                	js     80106252 <sys_chdir+0x31>
8010623e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106241:	89 04 24             	mov    %eax,(%esp)
80106244:	e8 c1 c1 ff ff       	call   8010240a <namei>
80106249:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010624c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106250:	75 07                	jne    80106259 <sys_chdir+0x38>
    return -1;
80106252:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106257:	eb 57                	jmp    801062b0 <sys_chdir+0x8f>
  ilock(ip);
80106259:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010625c:	89 04 24             	mov    %eax,(%esp)
8010625f:	e8 04 b6 ff ff       	call   80101868 <ilock>
  if(ip->type != T_DIR){
80106264:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106267:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010626b:	66 83 f8 01          	cmp    $0x1,%ax
8010626f:	74 12                	je     80106283 <sys_chdir+0x62>
    iunlockput(ip);
80106271:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106274:	89 04 24             	mov    %eax,(%esp)
80106277:	e8 70 b8 ff ff       	call   80101aec <iunlockput>
    return -1;
8010627c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106281:	eb 2d                	jmp    801062b0 <sys_chdir+0x8f>
  }
  iunlock(ip);
80106283:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106286:	89 04 24             	mov    %eax,(%esp)
80106289:	e8 28 b7 ff ff       	call   801019b6 <iunlock>
  iput(proc->cwd);
8010628e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106294:	8b 40 68             	mov    0x68(%eax),%eax
80106297:	89 04 24             	mov    %eax,(%esp)
8010629a:	e8 7c b7 ff ff       	call   80101a1b <iput>
  proc->cwd = ip;
8010629f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062a5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801062a8:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
801062ab:	b8 00 00 00 00       	mov    $0x0,%eax
}
801062b0:	c9                   	leave  
801062b1:	c3                   	ret    

801062b2 <sys_exec>:

int
sys_exec(void)
{
801062b2:	55                   	push   %ebp
801062b3:	89 e5                	mov    %esp,%ebp
801062b5:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
801062bb:	8d 45 f0             	lea    -0x10(%ebp),%eax
801062be:	89 44 24 04          	mov    %eax,0x4(%esp)
801062c2:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801062c9:	e8 22 f2 ff ff       	call   801054f0 <argstr>
801062ce:	85 c0                	test   %eax,%eax
801062d0:	78 1a                	js     801062ec <sys_exec+0x3a>
801062d2:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
801062d8:	89 44 24 04          	mov    %eax,0x4(%esp)
801062dc:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801062e3:	e8 6e f1 ff ff       	call   80105456 <argint>
801062e8:	85 c0                	test   %eax,%eax
801062ea:	79 0a                	jns    801062f6 <sys_exec+0x44>
    return -1;
801062ec:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062f1:	e9 e2 00 00 00       	jmp    801063d8 <sys_exec+0x126>
  }
  memset(argv, 0, sizeof(argv));
801062f6:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801062fd:	00 
801062fe:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106305:	00 
80106306:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
8010630c:	89 04 24             	mov    %eax,(%esp)
8010630f:	e8 f2 ed ff ff       	call   80105106 <memset>
  for(i=0;; i++){
80106314:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
8010631b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010631e:	83 f8 1f             	cmp    $0x1f,%eax
80106321:	76 0a                	jbe    8010632d <sys_exec+0x7b>
      return -1;
80106323:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106328:	e9 ab 00 00 00       	jmp    801063d8 <sys_exec+0x126>
    if(fetchint(proc, uargv+4*i, (int*)&uarg) < 0)
8010632d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106330:	c1 e0 02             	shl    $0x2,%eax
80106333:	89 c2                	mov    %eax,%edx
80106335:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
8010633b:	8d 0c 02             	lea    (%edx,%eax,1),%ecx
8010633e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106344:	8d 95 68 ff ff ff    	lea    -0x98(%ebp),%edx
8010634a:	89 54 24 08          	mov    %edx,0x8(%esp)
8010634e:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80106352:	89 04 24             	mov    %eax,(%esp)
80106355:	e8 6a f0 ff ff       	call   801053c4 <fetchint>
8010635a:	85 c0                	test   %eax,%eax
8010635c:	79 07                	jns    80106365 <sys_exec+0xb3>
      return -1;
8010635e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106363:	eb 73                	jmp    801063d8 <sys_exec+0x126>
    if(uarg == 0){
80106365:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
8010636b:	85 c0                	test   %eax,%eax
8010636d:	75 26                	jne    80106395 <sys_exec+0xe3>
      argv[i] = 0;
8010636f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106372:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
80106379:	00 00 00 00 
      break;
8010637d:	90                   	nop
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
8010637e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106381:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
80106387:	89 54 24 04          	mov    %edx,0x4(%esp)
8010638b:	89 04 24             	mov    %eax,(%esp)
8010638e:	e8 69 a7 ff ff       	call   80100afc <exec>
80106393:	eb 43                	jmp    801063d8 <sys_exec+0x126>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
80106395:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106398:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
8010639f:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801063a5:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
801063a8:	8b 95 68 ff ff ff    	mov    -0x98(%ebp),%edx
801063ae:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801063b4:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801063b8:	89 54 24 04          	mov    %edx,0x4(%esp)
801063bc:	89 04 24             	mov    %eax,(%esp)
801063bf:	e8 34 f0 ff ff       	call   801053f8 <fetchstr>
801063c4:	85 c0                	test   %eax,%eax
801063c6:	79 07                	jns    801063cf <sys_exec+0x11d>
      return -1;
801063c8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063cd:	eb 09                	jmp    801063d8 <sys_exec+0x126>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
801063cf:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
801063d3:	e9 43 ff ff ff       	jmp    8010631b <sys_exec+0x69>
  return exec(path, argv);
}
801063d8:	c9                   	leave  
801063d9:	c3                   	ret    

801063da <sys_pipe>:

int
sys_pipe(void)
{
801063da:	55                   	push   %ebp
801063db:	89 e5                	mov    %esp,%ebp
801063dd:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
801063e0:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801063e7:	00 
801063e8:	8d 45 ec             	lea    -0x14(%ebp),%eax
801063eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801063ef:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063f6:	e8 93 f0 ff ff       	call   8010548e <argptr>
801063fb:	85 c0                	test   %eax,%eax
801063fd:	79 0a                	jns    80106409 <sys_pipe+0x2f>
    return -1;
801063ff:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106404:	e9 9b 00 00 00       	jmp    801064a4 <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
80106409:	8d 45 e4             	lea    -0x1c(%ebp),%eax
8010640c:	89 44 24 04          	mov    %eax,0x4(%esp)
80106410:	8d 45 e8             	lea    -0x18(%ebp),%eax
80106413:	89 04 24             	mov    %eax,(%esp)
80106416:	e8 fd d7 ff ff       	call   80103c18 <pipealloc>
8010641b:	85 c0                	test   %eax,%eax
8010641d:	79 07                	jns    80106426 <sys_pipe+0x4c>
    return -1;
8010641f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106424:	eb 7e                	jmp    801064a4 <sys_pipe+0xca>
  fd0 = -1;
80106426:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
8010642d:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106430:	89 04 24             	mov    %eax,(%esp)
80106433:	e8 35 f2 ff ff       	call   8010566d <fdalloc>
80106438:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010643b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010643f:	78 14                	js     80106455 <sys_pipe+0x7b>
80106441:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106444:	89 04 24             	mov    %eax,(%esp)
80106447:	e8 21 f2 ff ff       	call   8010566d <fdalloc>
8010644c:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010644f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106453:	79 37                	jns    8010648c <sys_pipe+0xb2>
    if(fd0 >= 0)
80106455:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106459:	78 14                	js     8010646f <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
8010645b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106461:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106464:	83 c2 08             	add    $0x8,%edx
80106467:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010646e:	00 
    fileclose(rf);
8010646f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106472:	89 04 24             	mov    %eax,(%esp)
80106475:	e8 4a ab ff ff       	call   80100fc4 <fileclose>
    fileclose(wf);
8010647a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010647d:	89 04 24             	mov    %eax,(%esp)
80106480:	e8 3f ab ff ff       	call   80100fc4 <fileclose>
    return -1;
80106485:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010648a:	eb 18                	jmp    801064a4 <sys_pipe+0xca>
  }
  fd[0] = fd0;
8010648c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010648f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106492:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
80106494:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106497:	8d 50 04             	lea    0x4(%eax),%edx
8010649a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010649d:	89 02                	mov    %eax,(%edx)
  return 0;
8010649f:	b8 00 00 00 00       	mov    $0x0,%eax
}
801064a4:	c9                   	leave  
801064a5:	c3                   	ret    
	...

801064a8 <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
801064a8:	55                   	push   %ebp
801064a9:	89 e5                	mov    %esp,%ebp
801064ab:	83 ec 08             	sub    $0x8,%esp
  return fork();
801064ae:	e8 11 e0 ff ff       	call   801044c4 <fork>
}
801064b3:	c9                   	leave  
801064b4:	c3                   	ret    

801064b5 <sys_exit>:

int
sys_exit(void)
{
801064b5:	55                   	push   %ebp
801064b6:	89 e5                	mov    %esp,%ebp
801064b8:	83 ec 08             	sub    $0x8,%esp
  exit();
801064bb:	e8 67 e1 ff ff       	call   80104627 <exit>
  return 0;  // not reached
801064c0:	b8 00 00 00 00       	mov    $0x0,%eax
}
801064c5:	c9                   	leave  
801064c6:	c3                   	ret    

801064c7 <sys_wait>:

int
sys_wait(void)
{
801064c7:	55                   	push   %ebp
801064c8:	89 e5                	mov    %esp,%ebp
801064ca:	83 ec 08             	sub    $0x8,%esp
  return wait();
801064cd:	e8 70 e2 ff ff       	call   80104742 <wait>
}
801064d2:	c9                   	leave  
801064d3:	c3                   	ret    

801064d4 <sys_kill>:

int
sys_kill(void)
{
801064d4:	55                   	push   %ebp
801064d5:	89 e5                	mov    %esp,%ebp
801064d7:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
801064da:	8d 45 f4             	lea    -0xc(%ebp),%eax
801064dd:	89 44 24 04          	mov    %eax,0x4(%esp)
801064e1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801064e8:	e8 69 ef ff ff       	call   80105456 <argint>
801064ed:	85 c0                	test   %eax,%eax
801064ef:	79 07                	jns    801064f8 <sys_kill+0x24>
    return -1;
801064f1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064f6:	eb 0b                	jmp    80106503 <sys_kill+0x2f>
  return kill(pid);
801064f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064fb:	89 04 24             	mov    %eax,(%esp)
801064fe:	e8 d6 e7 ff ff       	call   80104cd9 <kill>
}
80106503:	c9                   	leave  
80106504:	c3                   	ret    

80106505 <sys_getpid>:

int
sys_getpid(void)
{
80106505:	55                   	push   %ebp
80106506:	89 e5                	mov    %esp,%ebp
  return proc->pid;
80106508:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010650e:	8b 40 10             	mov    0x10(%eax),%eax
}
80106511:	5d                   	pop    %ebp
80106512:	c3                   	ret    

80106513 <sys_sbrk>:

int
sys_sbrk(void)
{
80106513:	55                   	push   %ebp
80106514:	89 e5                	mov    %esp,%ebp
80106516:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
80106519:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010651c:	89 44 24 04          	mov    %eax,0x4(%esp)
80106520:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106527:	e8 2a ef ff ff       	call   80105456 <argint>
8010652c:	85 c0                	test   %eax,%eax
8010652e:	79 07                	jns    80106537 <sys_sbrk+0x24>
    return -1;
80106530:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106535:	eb 24                	jmp    8010655b <sys_sbrk+0x48>
  addr = proc->sz;
80106537:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010653d:	8b 00                	mov    (%eax),%eax
8010653f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
80106542:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106545:	89 04 24             	mov    %eax,(%esp)
80106548:	e8 d2 de ff ff       	call   8010441f <growproc>
8010654d:	85 c0                	test   %eax,%eax
8010654f:	79 07                	jns    80106558 <sys_sbrk+0x45>
    return -1;
80106551:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106556:	eb 03                	jmp    8010655b <sys_sbrk+0x48>
  return addr;
80106558:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010655b:	c9                   	leave  
8010655c:	c3                   	ret    

8010655d <sys_sleep>:

int
sys_sleep(void)
{
8010655d:	55                   	push   %ebp
8010655e:	89 e5                	mov    %esp,%ebp
80106560:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
80106563:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106566:	89 44 24 04          	mov    %eax,0x4(%esp)
8010656a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106571:	e8 e0 ee ff ff       	call   80105456 <argint>
80106576:	85 c0                	test   %eax,%eax
80106578:	79 07                	jns    80106581 <sys_sleep+0x24>
    return -1;
8010657a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010657f:	eb 6c                	jmp    801065ed <sys_sleep+0x90>
  acquire(&tickslock);
80106581:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
80106588:	e8 2a e9 ff ff       	call   80104eb7 <acquire>
  ticks0 = ticks;
8010658d:	a1 a0 28 11 80       	mov    0x801128a0,%eax
80106592:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
80106595:	eb 34                	jmp    801065cb <sys_sleep+0x6e>
    if(proc->killed){
80106597:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010659d:	8b 40 24             	mov    0x24(%eax),%eax
801065a0:	85 c0                	test   %eax,%eax
801065a2:	74 13                	je     801065b7 <sys_sleep+0x5a>
      release(&tickslock);
801065a4:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
801065ab:	e8 69 e9 ff ff       	call   80104f19 <release>
      return -1;
801065b0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065b5:	eb 36                	jmp    801065ed <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
801065b7:	c7 44 24 04 60 20 11 	movl   $0x80112060,0x4(%esp)
801065be:	80 
801065bf:	c7 04 24 a0 28 11 80 	movl   $0x801128a0,(%esp)
801065c6:	e8 d5 e4 ff ff       	call   80104aa0 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
801065cb:	a1 a0 28 11 80       	mov    0x801128a0,%eax
801065d0:	89 c2                	mov    %eax,%edx
801065d2:	2b 55 f4             	sub    -0xc(%ebp),%edx
801065d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801065d8:	39 c2                	cmp    %eax,%edx
801065da:	72 bb                	jb     80106597 <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
801065dc:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
801065e3:	e8 31 e9 ff ff       	call   80104f19 <release>
  return 0;
801065e8:	b8 00 00 00 00       	mov    $0x0,%eax
}
801065ed:	c9                   	leave  
801065ee:	c3                   	ret    

801065ef <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
801065ef:	55                   	push   %ebp
801065f0:	89 e5                	mov    %esp,%ebp
801065f2:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
801065f5:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
801065fc:	e8 b6 e8 ff ff       	call   80104eb7 <acquire>
  xticks = ticks;
80106601:	a1 a0 28 11 80       	mov    0x801128a0,%eax
80106606:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
80106609:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
80106610:	e8 04 e9 ff ff       	call   80104f19 <release>
  return xticks;
80106615:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106618:	c9                   	leave  
80106619:	c3                   	ret    

8010661a <sys_enableSwapping>:

void
sys_enableSwapping(void)
{
8010661a:	55                   	push   %ebp
8010661b:	89 e5                	mov    %esp,%ebp
  swapFlag = 1;
8010661d:	c7 05 08 b0 10 80 01 	movl   $0x1,0x8010b008
80106624:	00 00 00 
}
80106627:	5d                   	pop    %ebp
80106628:	c3                   	ret    

80106629 <sys_disableSwapping>:

void
sys_disableSwapping(void)
{
80106629:	55                   	push   %ebp
8010662a:	89 e5                	mov    %esp,%ebp
  swapFlag = 0;
8010662c:	c7 05 08 b0 10 80 00 	movl   $0x0,0x8010b008
80106633:	00 00 00 
}
80106636:	5d                   	pop    %ebp
80106637:	c3                   	ret    

80106638 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106638:	55                   	push   %ebp
80106639:	89 e5                	mov    %esp,%ebp
8010663b:	83 ec 08             	sub    $0x8,%esp
8010663e:	8b 55 08             	mov    0x8(%ebp),%edx
80106641:	8b 45 0c             	mov    0xc(%ebp),%eax
80106644:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106648:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010664b:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
8010664f:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106653:	ee                   	out    %al,(%dx)
}
80106654:	c9                   	leave  
80106655:	c3                   	ret    

80106656 <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
80106656:	55                   	push   %ebp
80106657:	89 e5                	mov    %esp,%ebp
80106659:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
8010665c:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
80106663:	00 
80106664:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
8010666b:	e8 c8 ff ff ff       	call   80106638 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
80106670:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
80106677:	00 
80106678:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
8010667f:	e8 b4 ff ff ff       	call   80106638 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
80106684:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
8010668b:	00 
8010668c:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
80106693:	e8 a0 ff ff ff       	call   80106638 <outb>
  picenable(IRQ_TIMER);
80106698:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010669f:	e8 fd d3 ff ff       	call   80103aa1 <picenable>
}
801066a4:	c9                   	leave  
801066a5:	c3                   	ret    
	...

801066a8 <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
801066a8:	1e                   	push   %ds
  pushl %es
801066a9:	06                   	push   %es
  pushl %fs
801066aa:	0f a0                	push   %fs
  pushl %gs
801066ac:	0f a8                	push   %gs
  pushal
801066ae:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
801066af:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
801066b3:	8e d8                	mov    %eax,%ds
  movw %ax, %es
801066b5:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
801066b7:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
801066bb:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
801066bd:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
801066bf:	54                   	push   %esp
  call trap
801066c0:	e8 de 01 00 00       	call   801068a3 <trap>
  addl $4, %esp
801066c5:	83 c4 04             	add    $0x4,%esp

801066c8 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
801066c8:	61                   	popa   
  popl %gs
801066c9:	0f a9                	pop    %gs
  popl %fs
801066cb:	0f a1                	pop    %fs
  popl %es
801066cd:	07                   	pop    %es
  popl %ds
801066ce:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
801066cf:	83 c4 08             	add    $0x8,%esp
  iret
801066d2:	cf                   	iret   
	...

801066d4 <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
801066d4:	55                   	push   %ebp
801066d5:	89 e5                	mov    %esp,%ebp
801066d7:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
801066da:	8b 45 0c             	mov    0xc(%ebp),%eax
801066dd:	83 e8 01             	sub    $0x1,%eax
801066e0:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801066e4:	8b 45 08             	mov    0x8(%ebp),%eax
801066e7:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801066eb:	8b 45 08             	mov    0x8(%ebp),%eax
801066ee:	c1 e8 10             	shr    $0x10,%eax
801066f1:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
801066f5:	8d 45 fa             	lea    -0x6(%ebp),%eax
801066f8:	0f 01 18             	lidtl  (%eax)
}
801066fb:	c9                   	leave  
801066fc:	c3                   	ret    

801066fd <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
801066fd:	55                   	push   %ebp
801066fe:	89 e5                	mov    %esp,%ebp
80106700:	53                   	push   %ebx
80106701:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80106704:	0f 20 d3             	mov    %cr2,%ebx
80106707:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return val;
8010670a:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
8010670d:	83 c4 10             	add    $0x10,%esp
80106710:	5b                   	pop    %ebx
80106711:	5d                   	pop    %ebp
80106712:	c3                   	ret    

80106713 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
80106713:	55                   	push   %ebp
80106714:	89 e5                	mov    %esp,%ebp
80106716:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
80106719:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106720:	e9 c3 00 00 00       	jmp    801067e8 <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80106725:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106728:	8b 04 85 a0 b0 10 80 	mov    -0x7fef4f60(,%eax,4),%eax
8010672f:	89 c2                	mov    %eax,%edx
80106731:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106734:	66 89 14 c5 a0 20 11 	mov    %dx,-0x7feedf60(,%eax,8)
8010673b:	80 
8010673c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010673f:	66 c7 04 c5 a2 20 11 	movw   $0x8,-0x7feedf5e(,%eax,8)
80106746:	80 08 00 
80106749:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010674c:	0f b6 14 c5 a4 20 11 	movzbl -0x7feedf5c(,%eax,8),%edx
80106753:	80 
80106754:	83 e2 e0             	and    $0xffffffe0,%edx
80106757:	88 14 c5 a4 20 11 80 	mov    %dl,-0x7feedf5c(,%eax,8)
8010675e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106761:	0f b6 14 c5 a4 20 11 	movzbl -0x7feedf5c(,%eax,8),%edx
80106768:	80 
80106769:	83 e2 1f             	and    $0x1f,%edx
8010676c:	88 14 c5 a4 20 11 80 	mov    %dl,-0x7feedf5c(,%eax,8)
80106773:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106776:	0f b6 14 c5 a5 20 11 	movzbl -0x7feedf5b(,%eax,8),%edx
8010677d:	80 
8010677e:	83 e2 f0             	and    $0xfffffff0,%edx
80106781:	83 ca 0e             	or     $0xe,%edx
80106784:	88 14 c5 a5 20 11 80 	mov    %dl,-0x7feedf5b(,%eax,8)
8010678b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010678e:	0f b6 14 c5 a5 20 11 	movzbl -0x7feedf5b(,%eax,8),%edx
80106795:	80 
80106796:	83 e2 ef             	and    $0xffffffef,%edx
80106799:	88 14 c5 a5 20 11 80 	mov    %dl,-0x7feedf5b(,%eax,8)
801067a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067a3:	0f b6 14 c5 a5 20 11 	movzbl -0x7feedf5b(,%eax,8),%edx
801067aa:	80 
801067ab:	83 e2 9f             	and    $0xffffff9f,%edx
801067ae:	88 14 c5 a5 20 11 80 	mov    %dl,-0x7feedf5b(,%eax,8)
801067b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067b8:	0f b6 14 c5 a5 20 11 	movzbl -0x7feedf5b(,%eax,8),%edx
801067bf:	80 
801067c0:	83 ca 80             	or     $0xffffff80,%edx
801067c3:	88 14 c5 a5 20 11 80 	mov    %dl,-0x7feedf5b(,%eax,8)
801067ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067cd:	8b 04 85 a0 b0 10 80 	mov    -0x7fef4f60(,%eax,4),%eax
801067d4:	c1 e8 10             	shr    $0x10,%eax
801067d7:	89 c2                	mov    %eax,%edx
801067d9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067dc:	66 89 14 c5 a6 20 11 	mov    %dx,-0x7feedf5a(,%eax,8)
801067e3:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
801067e4:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801067e8:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
801067ef:	0f 8e 30 ff ff ff    	jle    80106725 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
801067f5:	a1 a0 b1 10 80       	mov    0x8010b1a0,%eax
801067fa:	66 a3 a0 22 11 80    	mov    %ax,0x801122a0
80106800:	66 c7 05 a2 22 11 80 	movw   $0x8,0x801122a2
80106807:	08 00 
80106809:	0f b6 05 a4 22 11 80 	movzbl 0x801122a4,%eax
80106810:	83 e0 e0             	and    $0xffffffe0,%eax
80106813:	a2 a4 22 11 80       	mov    %al,0x801122a4
80106818:	0f b6 05 a4 22 11 80 	movzbl 0x801122a4,%eax
8010681f:	83 e0 1f             	and    $0x1f,%eax
80106822:	a2 a4 22 11 80       	mov    %al,0x801122a4
80106827:	0f b6 05 a5 22 11 80 	movzbl 0x801122a5,%eax
8010682e:	83 c8 0f             	or     $0xf,%eax
80106831:	a2 a5 22 11 80       	mov    %al,0x801122a5
80106836:	0f b6 05 a5 22 11 80 	movzbl 0x801122a5,%eax
8010683d:	83 e0 ef             	and    $0xffffffef,%eax
80106840:	a2 a5 22 11 80       	mov    %al,0x801122a5
80106845:	0f b6 05 a5 22 11 80 	movzbl 0x801122a5,%eax
8010684c:	83 c8 60             	or     $0x60,%eax
8010684f:	a2 a5 22 11 80       	mov    %al,0x801122a5
80106854:	0f b6 05 a5 22 11 80 	movzbl 0x801122a5,%eax
8010685b:	83 c8 80             	or     $0xffffff80,%eax
8010685e:	a2 a5 22 11 80       	mov    %al,0x801122a5
80106863:	a1 a0 b1 10 80       	mov    0x8010b1a0,%eax
80106868:	c1 e8 10             	shr    $0x10,%eax
8010686b:	66 a3 a6 22 11 80    	mov    %ax,0x801122a6
  
  initlock(&tickslock, "time");
80106871:	c7 44 24 04 bc 8a 10 	movl   $0x80108abc,0x4(%esp)
80106878:	80 
80106879:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
80106880:	e8 11 e6 ff ff       	call   80104e96 <initlock>
}
80106885:	c9                   	leave  
80106886:	c3                   	ret    

80106887 <idtinit>:

void
idtinit(void)
{
80106887:	55                   	push   %ebp
80106888:	89 e5                	mov    %esp,%ebp
8010688a:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
8010688d:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
80106894:	00 
80106895:	c7 04 24 a0 20 11 80 	movl   $0x801120a0,(%esp)
8010689c:	e8 33 fe ff ff       	call   801066d4 <lidt>
}
801068a1:	c9                   	leave  
801068a2:	c3                   	ret    

801068a3 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
801068a3:	55                   	push   %ebp
801068a4:	89 e5                	mov    %esp,%ebp
801068a6:	57                   	push   %edi
801068a7:	56                   	push   %esi
801068a8:	53                   	push   %ebx
801068a9:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
801068ac:	8b 45 08             	mov    0x8(%ebp),%eax
801068af:	8b 40 30             	mov    0x30(%eax),%eax
801068b2:	83 f8 40             	cmp    $0x40,%eax
801068b5:	75 3e                	jne    801068f5 <trap+0x52>
    if(proc->killed)
801068b7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801068bd:	8b 40 24             	mov    0x24(%eax),%eax
801068c0:	85 c0                	test   %eax,%eax
801068c2:	74 05                	je     801068c9 <trap+0x26>
      exit();
801068c4:	e8 5e dd ff ff       	call   80104627 <exit>
    proc->tf = tf;
801068c9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801068cf:	8b 55 08             	mov    0x8(%ebp),%edx
801068d2:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
801068d5:	e8 59 ec ff ff       	call   80105533 <syscall>
    if(proc->killed)
801068da:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801068e0:	8b 40 24             	mov    0x24(%eax),%eax
801068e3:	85 c0                	test   %eax,%eax
801068e5:	0f 84 34 02 00 00    	je     80106b1f <trap+0x27c>
      exit();
801068eb:	e8 37 dd ff ff       	call   80104627 <exit>
    return;
801068f0:	e9 2a 02 00 00       	jmp    80106b1f <trap+0x27c>
  }

  switch(tf->trapno){
801068f5:	8b 45 08             	mov    0x8(%ebp),%eax
801068f8:	8b 40 30             	mov    0x30(%eax),%eax
801068fb:	83 e8 20             	sub    $0x20,%eax
801068fe:	83 f8 1f             	cmp    $0x1f,%eax
80106901:	0f 87 bc 00 00 00    	ja     801069c3 <trap+0x120>
80106907:	8b 04 85 64 8b 10 80 	mov    -0x7fef749c(,%eax,4),%eax
8010690e:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
80106910:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106916:	0f b6 00             	movzbl (%eax),%eax
80106919:	84 c0                	test   %al,%al
8010691b:	75 31                	jne    8010694e <trap+0xab>
      acquire(&tickslock);
8010691d:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
80106924:	e8 8e e5 ff ff       	call   80104eb7 <acquire>
      ticks++;
80106929:	a1 a0 28 11 80       	mov    0x801128a0,%eax
8010692e:	83 c0 01             	add    $0x1,%eax
80106931:	a3 a0 28 11 80       	mov    %eax,0x801128a0
      wakeup(&ticks);
80106936:	c7 04 24 a0 28 11 80 	movl   $0x801128a0,(%esp)
8010693d:	e8 6c e3 ff ff       	call   80104cae <wakeup>
      release(&tickslock);
80106942:	c7 04 24 60 20 11 80 	movl   $0x80112060,(%esp)
80106949:	e8 cb e5 ff ff       	call   80104f19 <release>
    }
    lapiceoi();
8010694e:	e8 96 c5 ff ff       	call   80102ee9 <lapiceoi>
    break;
80106953:	e9 41 01 00 00       	jmp    80106a99 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
80106958:	e8 94 bd ff ff       	call   801026f1 <ideintr>
    lapiceoi();
8010695d:	e8 87 c5 ff ff       	call   80102ee9 <lapiceoi>
    break;
80106962:	e9 32 01 00 00       	jmp    80106a99 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
80106967:	e8 5b c3 ff ff       	call   80102cc7 <kbdintr>
    lapiceoi();
8010696c:	e8 78 c5 ff ff       	call   80102ee9 <lapiceoi>
    break;
80106971:	e9 23 01 00 00       	jmp    80106a99 <trap+0x1f6>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80106976:	e8 a9 03 00 00       	call   80106d24 <uartintr>
    lapiceoi();
8010697b:	e8 69 c5 ff ff       	call   80102ee9 <lapiceoi>
    break;
80106980:	e9 14 01 00 00       	jmp    80106a99 <trap+0x1f6>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
            cpu->id, tf->cs, tf->eip);
80106985:	8b 45 08             	mov    0x8(%ebp),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106988:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
8010698b:	8b 45 08             	mov    0x8(%ebp),%eax
8010698e:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106992:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80106995:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010699b:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
8010699e:	0f b6 c0             	movzbl %al,%eax
801069a1:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801069a5:	89 54 24 08          	mov    %edx,0x8(%esp)
801069a9:	89 44 24 04          	mov    %eax,0x4(%esp)
801069ad:	c7 04 24 c4 8a 10 80 	movl   $0x80108ac4,(%esp)
801069b4:	e8 e8 99 ff ff       	call   801003a1 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
801069b9:	e8 2b c5 ff ff       	call   80102ee9 <lapiceoi>
    break;
801069be:	e9 d6 00 00 00       	jmp    80106a99 <trap+0x1f6>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
801069c3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801069c9:	85 c0                	test   %eax,%eax
801069cb:	74 11                	je     801069de <trap+0x13b>
801069cd:	8b 45 08             	mov    0x8(%ebp),%eax
801069d0:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
801069d4:	0f b7 c0             	movzwl %ax,%eax
801069d7:	83 e0 03             	and    $0x3,%eax
801069da:	85 c0                	test   %eax,%eax
801069dc:	75 46                	jne    80106a24 <trap+0x181>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801069de:	e8 1a fd ff ff       	call   801066fd <rcr2>
              tf->trapno, cpu->id, tf->eip, rcr2());
801069e3:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801069e6:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
801069e9:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801069f0:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801069f3:	0f b6 ca             	movzbl %dl,%ecx
              tf->trapno, cpu->id, tf->eip, rcr2());
801069f6:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801069f9:	8b 52 30             	mov    0x30(%edx),%edx
801069fc:	89 44 24 10          	mov    %eax,0x10(%esp)
80106a00:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106a04:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106a08:	89 54 24 04          	mov    %edx,0x4(%esp)
80106a0c:	c7 04 24 e8 8a 10 80 	movl   $0x80108ae8,(%esp)
80106a13:	e8 89 99 ff ff       	call   801003a1 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80106a18:	c7 04 24 1a 8b 10 80 	movl   $0x80108b1a,(%esp)
80106a1f:	e8 19 9b ff ff       	call   8010053d <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a24:	e8 d4 fc ff ff       	call   801066fd <rcr2>
80106a29:	89 c2                	mov    %eax,%edx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106a2b:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a2e:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106a31:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106a37:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a3a:	0f b6 f0             	movzbl %al,%esi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106a3d:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a40:	8b 58 34             	mov    0x34(%eax),%ebx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106a43:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a46:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106a49:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a4f:	83 c0 6c             	add    $0x6c,%eax
80106a52:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106a55:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106a5b:	8b 40 10             	mov    0x10(%eax),%eax
80106a5e:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106a62:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106a66:	89 74 24 14          	mov    %esi,0x14(%esp)
80106a6a:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106a6e:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106a72:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80106a75:	89 54 24 08          	mov    %edx,0x8(%esp)
80106a79:	89 44 24 04          	mov    %eax,0x4(%esp)
80106a7d:	c7 04 24 20 8b 10 80 	movl   $0x80108b20,(%esp)
80106a84:	e8 18 99 ff ff       	call   801003a1 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80106a89:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a8f:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80106a96:	eb 01                	jmp    80106a99 <trap+0x1f6>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80106a98:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106a99:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a9f:	85 c0                	test   %eax,%eax
80106aa1:	74 24                	je     80106ac7 <trap+0x224>
80106aa3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106aa9:	8b 40 24             	mov    0x24(%eax),%eax
80106aac:	85 c0                	test   %eax,%eax
80106aae:	74 17                	je     80106ac7 <trap+0x224>
80106ab0:	8b 45 08             	mov    0x8(%ebp),%eax
80106ab3:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106ab7:	0f b7 c0             	movzwl %ax,%eax
80106aba:	83 e0 03             	and    $0x3,%eax
80106abd:	83 f8 03             	cmp    $0x3,%eax
80106ac0:	75 05                	jne    80106ac7 <trap+0x224>
    exit();
80106ac2:	e8 60 db ff ff       	call   80104627 <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80106ac7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106acd:	85 c0                	test   %eax,%eax
80106acf:	74 1e                	je     80106aef <trap+0x24c>
80106ad1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106ad7:	8b 40 0c             	mov    0xc(%eax),%eax
80106ada:	83 f8 04             	cmp    $0x4,%eax
80106add:	75 10                	jne    80106aef <trap+0x24c>
80106adf:	8b 45 08             	mov    0x8(%ebp),%eax
80106ae2:	8b 40 30             	mov    0x30(%eax),%eax
80106ae5:	83 f8 20             	cmp    $0x20,%eax
80106ae8:	75 05                	jne    80106aef <trap+0x24c>
    yield();
80106aea:	e8 53 df ff ff       	call   80104a42 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106aef:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106af5:	85 c0                	test   %eax,%eax
80106af7:	74 27                	je     80106b20 <trap+0x27d>
80106af9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106aff:	8b 40 24             	mov    0x24(%eax),%eax
80106b02:	85 c0                	test   %eax,%eax
80106b04:	74 1a                	je     80106b20 <trap+0x27d>
80106b06:	8b 45 08             	mov    0x8(%ebp),%eax
80106b09:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106b0d:	0f b7 c0             	movzwl %ax,%eax
80106b10:	83 e0 03             	and    $0x3,%eax
80106b13:	83 f8 03             	cmp    $0x3,%eax
80106b16:	75 08                	jne    80106b20 <trap+0x27d>
    exit();
80106b18:	e8 0a db ff ff       	call   80104627 <exit>
80106b1d:	eb 01                	jmp    80106b20 <trap+0x27d>
      exit();
    proc->tf = tf;
    syscall();
    if(proc->killed)
      exit();
    return;
80106b1f:	90                   	nop
    yield();

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
    exit();
}
80106b20:	83 c4 3c             	add    $0x3c,%esp
80106b23:	5b                   	pop    %ebx
80106b24:	5e                   	pop    %esi
80106b25:	5f                   	pop    %edi
80106b26:	5d                   	pop    %ebp
80106b27:	c3                   	ret    

80106b28 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106b28:	55                   	push   %ebp
80106b29:	89 e5                	mov    %esp,%ebp
80106b2b:	53                   	push   %ebx
80106b2c:	83 ec 14             	sub    $0x14,%esp
80106b2f:	8b 45 08             	mov    0x8(%ebp),%eax
80106b32:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106b36:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80106b3a:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80106b3e:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80106b42:	ec                   	in     (%dx),%al
80106b43:	89 c3                	mov    %eax,%ebx
80106b45:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80106b48:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80106b4c:	83 c4 14             	add    $0x14,%esp
80106b4f:	5b                   	pop    %ebx
80106b50:	5d                   	pop    %ebp
80106b51:	c3                   	ret    

80106b52 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106b52:	55                   	push   %ebp
80106b53:	89 e5                	mov    %esp,%ebp
80106b55:	83 ec 08             	sub    $0x8,%esp
80106b58:	8b 55 08             	mov    0x8(%ebp),%edx
80106b5b:	8b 45 0c             	mov    0xc(%ebp),%eax
80106b5e:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106b62:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106b65:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106b69:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106b6d:	ee                   	out    %al,(%dx)
}
80106b6e:	c9                   	leave  
80106b6f:	c3                   	ret    

80106b70 <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106b70:	55                   	push   %ebp
80106b71:	89 e5                	mov    %esp,%ebp
80106b73:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106b76:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106b7d:	00 
80106b7e:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106b85:	e8 c8 ff ff ff       	call   80106b52 <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106b8a:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80106b91:	00 
80106b92:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106b99:	e8 b4 ff ff ff       	call   80106b52 <outb>
  outb(COM1+0, 115200/9600);
80106b9e:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80106ba5:	00 
80106ba6:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106bad:	e8 a0 ff ff ff       	call   80106b52 <outb>
  outb(COM1+1, 0);
80106bb2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106bb9:	00 
80106bba:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106bc1:	e8 8c ff ff ff       	call   80106b52 <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80106bc6:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80106bcd:	00 
80106bce:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106bd5:	e8 78 ff ff ff       	call   80106b52 <outb>
  outb(COM1+4, 0);
80106bda:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106be1:	00 
80106be2:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80106be9:	e8 64 ff ff ff       	call   80106b52 <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106bee:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106bf5:	00 
80106bf6:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106bfd:	e8 50 ff ff ff       	call   80106b52 <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106c02:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106c09:	e8 1a ff ff ff       	call   80106b28 <inb>
80106c0e:	3c ff                	cmp    $0xff,%al
80106c10:	74 6c                	je     80106c7e <uartinit+0x10e>
    return;
  uart = 1;
80106c12:	c7 05 50 b6 10 80 01 	movl   $0x1,0x8010b650
80106c19:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106c1c:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106c23:	e8 00 ff ff ff       	call   80106b28 <inb>
  inb(COM1+0);
80106c28:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106c2f:	e8 f4 fe ff ff       	call   80106b28 <inb>
  picenable(IRQ_COM1);
80106c34:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106c3b:	e8 61 ce ff ff       	call   80103aa1 <picenable>
  ioapicenable(IRQ_COM1, 0);
80106c40:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106c47:	00 
80106c48:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106c4f:	e8 22 bd ff ff       	call   80102976 <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106c54:	c7 45 f4 e4 8b 10 80 	movl   $0x80108be4,-0xc(%ebp)
80106c5b:	eb 15                	jmp    80106c72 <uartinit+0x102>
    uartputc(*p);
80106c5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c60:	0f b6 00             	movzbl (%eax),%eax
80106c63:	0f be c0             	movsbl %al,%eax
80106c66:	89 04 24             	mov    %eax,(%esp)
80106c69:	e8 13 00 00 00       	call   80106c81 <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106c6e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106c72:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c75:	0f b6 00             	movzbl (%eax),%eax
80106c78:	84 c0                	test   %al,%al
80106c7a:	75 e1                	jne    80106c5d <uartinit+0xed>
80106c7c:	eb 01                	jmp    80106c7f <uartinit+0x10f>
  outb(COM1+4, 0);
  outb(COM1+1, 0x01);    // Enable receive interrupts.

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
    return;
80106c7e:	90                   	nop
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
    uartputc(*p);
}
80106c7f:	c9                   	leave  
80106c80:	c3                   	ret    

80106c81 <uartputc>:

void
uartputc(int c)
{
80106c81:	55                   	push   %ebp
80106c82:	89 e5                	mov    %esp,%ebp
80106c84:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106c87:	a1 50 b6 10 80       	mov    0x8010b650,%eax
80106c8c:	85 c0                	test   %eax,%eax
80106c8e:	74 4d                	je     80106cdd <uartputc+0x5c>
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106c90:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106c97:	eb 10                	jmp    80106ca9 <uartputc+0x28>
    microdelay(10);
80106c99:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80106ca0:	e8 69 c2 ff ff       	call   80102f0e <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106ca5:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106ca9:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106cad:	7f 16                	jg     80106cc5 <uartputc+0x44>
80106caf:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106cb6:	e8 6d fe ff ff       	call   80106b28 <inb>
80106cbb:	0f b6 c0             	movzbl %al,%eax
80106cbe:	83 e0 20             	and    $0x20,%eax
80106cc1:	85 c0                	test   %eax,%eax
80106cc3:	74 d4                	je     80106c99 <uartputc+0x18>
    microdelay(10);
  outb(COM1+0, c);
80106cc5:	8b 45 08             	mov    0x8(%ebp),%eax
80106cc8:	0f b6 c0             	movzbl %al,%eax
80106ccb:	89 44 24 04          	mov    %eax,0x4(%esp)
80106ccf:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106cd6:	e8 77 fe ff ff       	call   80106b52 <outb>
80106cdb:	eb 01                	jmp    80106cde <uartputc+0x5d>
uartputc(int c)
{
  int i;

  if(!uart)
    return;
80106cdd:	90                   	nop
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
    microdelay(10);
  outb(COM1+0, c);
}
80106cde:	c9                   	leave  
80106cdf:	c3                   	ret    

80106ce0 <uartgetc>:

static int
uartgetc(void)
{
80106ce0:	55                   	push   %ebp
80106ce1:	89 e5                	mov    %esp,%ebp
80106ce3:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106ce6:	a1 50 b6 10 80       	mov    0x8010b650,%eax
80106ceb:	85 c0                	test   %eax,%eax
80106ced:	75 07                	jne    80106cf6 <uartgetc+0x16>
    return -1;
80106cef:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106cf4:	eb 2c                	jmp    80106d22 <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106cf6:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106cfd:	e8 26 fe ff ff       	call   80106b28 <inb>
80106d02:	0f b6 c0             	movzbl %al,%eax
80106d05:	83 e0 01             	and    $0x1,%eax
80106d08:	85 c0                	test   %eax,%eax
80106d0a:	75 07                	jne    80106d13 <uartgetc+0x33>
    return -1;
80106d0c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106d11:	eb 0f                	jmp    80106d22 <uartgetc+0x42>
  return inb(COM1+0);
80106d13:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106d1a:	e8 09 fe ff ff       	call   80106b28 <inb>
80106d1f:	0f b6 c0             	movzbl %al,%eax
}
80106d22:	c9                   	leave  
80106d23:	c3                   	ret    

80106d24 <uartintr>:

void
uartintr(void)
{
80106d24:	55                   	push   %ebp
80106d25:	89 e5                	mov    %esp,%ebp
80106d27:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80106d2a:	c7 04 24 e0 6c 10 80 	movl   $0x80106ce0,(%esp)
80106d31:	e8 77 9a ff ff       	call   801007ad <consoleintr>
}
80106d36:	c9                   	leave  
80106d37:	c3                   	ret    

80106d38 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80106d38:	6a 00                	push   $0x0
  pushl $0
80106d3a:	6a 00                	push   $0x0
  jmp alltraps
80106d3c:	e9 67 f9 ff ff       	jmp    801066a8 <alltraps>

80106d41 <vector1>:
.globl vector1
vector1:
  pushl $0
80106d41:	6a 00                	push   $0x0
  pushl $1
80106d43:	6a 01                	push   $0x1
  jmp alltraps
80106d45:	e9 5e f9 ff ff       	jmp    801066a8 <alltraps>

80106d4a <vector2>:
.globl vector2
vector2:
  pushl $0
80106d4a:	6a 00                	push   $0x0
  pushl $2
80106d4c:	6a 02                	push   $0x2
  jmp alltraps
80106d4e:	e9 55 f9 ff ff       	jmp    801066a8 <alltraps>

80106d53 <vector3>:
.globl vector3
vector3:
  pushl $0
80106d53:	6a 00                	push   $0x0
  pushl $3
80106d55:	6a 03                	push   $0x3
  jmp alltraps
80106d57:	e9 4c f9 ff ff       	jmp    801066a8 <alltraps>

80106d5c <vector4>:
.globl vector4
vector4:
  pushl $0
80106d5c:	6a 00                	push   $0x0
  pushl $4
80106d5e:	6a 04                	push   $0x4
  jmp alltraps
80106d60:	e9 43 f9 ff ff       	jmp    801066a8 <alltraps>

80106d65 <vector5>:
.globl vector5
vector5:
  pushl $0
80106d65:	6a 00                	push   $0x0
  pushl $5
80106d67:	6a 05                	push   $0x5
  jmp alltraps
80106d69:	e9 3a f9 ff ff       	jmp    801066a8 <alltraps>

80106d6e <vector6>:
.globl vector6
vector6:
  pushl $0
80106d6e:	6a 00                	push   $0x0
  pushl $6
80106d70:	6a 06                	push   $0x6
  jmp alltraps
80106d72:	e9 31 f9 ff ff       	jmp    801066a8 <alltraps>

80106d77 <vector7>:
.globl vector7
vector7:
  pushl $0
80106d77:	6a 00                	push   $0x0
  pushl $7
80106d79:	6a 07                	push   $0x7
  jmp alltraps
80106d7b:	e9 28 f9 ff ff       	jmp    801066a8 <alltraps>

80106d80 <vector8>:
.globl vector8
vector8:
  pushl $8
80106d80:	6a 08                	push   $0x8
  jmp alltraps
80106d82:	e9 21 f9 ff ff       	jmp    801066a8 <alltraps>

80106d87 <vector9>:
.globl vector9
vector9:
  pushl $0
80106d87:	6a 00                	push   $0x0
  pushl $9
80106d89:	6a 09                	push   $0x9
  jmp alltraps
80106d8b:	e9 18 f9 ff ff       	jmp    801066a8 <alltraps>

80106d90 <vector10>:
.globl vector10
vector10:
  pushl $10
80106d90:	6a 0a                	push   $0xa
  jmp alltraps
80106d92:	e9 11 f9 ff ff       	jmp    801066a8 <alltraps>

80106d97 <vector11>:
.globl vector11
vector11:
  pushl $11
80106d97:	6a 0b                	push   $0xb
  jmp alltraps
80106d99:	e9 0a f9 ff ff       	jmp    801066a8 <alltraps>

80106d9e <vector12>:
.globl vector12
vector12:
  pushl $12
80106d9e:	6a 0c                	push   $0xc
  jmp alltraps
80106da0:	e9 03 f9 ff ff       	jmp    801066a8 <alltraps>

80106da5 <vector13>:
.globl vector13
vector13:
  pushl $13
80106da5:	6a 0d                	push   $0xd
  jmp alltraps
80106da7:	e9 fc f8 ff ff       	jmp    801066a8 <alltraps>

80106dac <vector14>:
.globl vector14
vector14:
  pushl $14
80106dac:	6a 0e                	push   $0xe
  jmp alltraps
80106dae:	e9 f5 f8 ff ff       	jmp    801066a8 <alltraps>

80106db3 <vector15>:
.globl vector15
vector15:
  pushl $0
80106db3:	6a 00                	push   $0x0
  pushl $15
80106db5:	6a 0f                	push   $0xf
  jmp alltraps
80106db7:	e9 ec f8 ff ff       	jmp    801066a8 <alltraps>

80106dbc <vector16>:
.globl vector16
vector16:
  pushl $0
80106dbc:	6a 00                	push   $0x0
  pushl $16
80106dbe:	6a 10                	push   $0x10
  jmp alltraps
80106dc0:	e9 e3 f8 ff ff       	jmp    801066a8 <alltraps>

80106dc5 <vector17>:
.globl vector17
vector17:
  pushl $17
80106dc5:	6a 11                	push   $0x11
  jmp alltraps
80106dc7:	e9 dc f8 ff ff       	jmp    801066a8 <alltraps>

80106dcc <vector18>:
.globl vector18
vector18:
  pushl $0
80106dcc:	6a 00                	push   $0x0
  pushl $18
80106dce:	6a 12                	push   $0x12
  jmp alltraps
80106dd0:	e9 d3 f8 ff ff       	jmp    801066a8 <alltraps>

80106dd5 <vector19>:
.globl vector19
vector19:
  pushl $0
80106dd5:	6a 00                	push   $0x0
  pushl $19
80106dd7:	6a 13                	push   $0x13
  jmp alltraps
80106dd9:	e9 ca f8 ff ff       	jmp    801066a8 <alltraps>

80106dde <vector20>:
.globl vector20
vector20:
  pushl $0
80106dde:	6a 00                	push   $0x0
  pushl $20
80106de0:	6a 14                	push   $0x14
  jmp alltraps
80106de2:	e9 c1 f8 ff ff       	jmp    801066a8 <alltraps>

80106de7 <vector21>:
.globl vector21
vector21:
  pushl $0
80106de7:	6a 00                	push   $0x0
  pushl $21
80106de9:	6a 15                	push   $0x15
  jmp alltraps
80106deb:	e9 b8 f8 ff ff       	jmp    801066a8 <alltraps>

80106df0 <vector22>:
.globl vector22
vector22:
  pushl $0
80106df0:	6a 00                	push   $0x0
  pushl $22
80106df2:	6a 16                	push   $0x16
  jmp alltraps
80106df4:	e9 af f8 ff ff       	jmp    801066a8 <alltraps>

80106df9 <vector23>:
.globl vector23
vector23:
  pushl $0
80106df9:	6a 00                	push   $0x0
  pushl $23
80106dfb:	6a 17                	push   $0x17
  jmp alltraps
80106dfd:	e9 a6 f8 ff ff       	jmp    801066a8 <alltraps>

80106e02 <vector24>:
.globl vector24
vector24:
  pushl $0
80106e02:	6a 00                	push   $0x0
  pushl $24
80106e04:	6a 18                	push   $0x18
  jmp alltraps
80106e06:	e9 9d f8 ff ff       	jmp    801066a8 <alltraps>

80106e0b <vector25>:
.globl vector25
vector25:
  pushl $0
80106e0b:	6a 00                	push   $0x0
  pushl $25
80106e0d:	6a 19                	push   $0x19
  jmp alltraps
80106e0f:	e9 94 f8 ff ff       	jmp    801066a8 <alltraps>

80106e14 <vector26>:
.globl vector26
vector26:
  pushl $0
80106e14:	6a 00                	push   $0x0
  pushl $26
80106e16:	6a 1a                	push   $0x1a
  jmp alltraps
80106e18:	e9 8b f8 ff ff       	jmp    801066a8 <alltraps>

80106e1d <vector27>:
.globl vector27
vector27:
  pushl $0
80106e1d:	6a 00                	push   $0x0
  pushl $27
80106e1f:	6a 1b                	push   $0x1b
  jmp alltraps
80106e21:	e9 82 f8 ff ff       	jmp    801066a8 <alltraps>

80106e26 <vector28>:
.globl vector28
vector28:
  pushl $0
80106e26:	6a 00                	push   $0x0
  pushl $28
80106e28:	6a 1c                	push   $0x1c
  jmp alltraps
80106e2a:	e9 79 f8 ff ff       	jmp    801066a8 <alltraps>

80106e2f <vector29>:
.globl vector29
vector29:
  pushl $0
80106e2f:	6a 00                	push   $0x0
  pushl $29
80106e31:	6a 1d                	push   $0x1d
  jmp alltraps
80106e33:	e9 70 f8 ff ff       	jmp    801066a8 <alltraps>

80106e38 <vector30>:
.globl vector30
vector30:
  pushl $0
80106e38:	6a 00                	push   $0x0
  pushl $30
80106e3a:	6a 1e                	push   $0x1e
  jmp alltraps
80106e3c:	e9 67 f8 ff ff       	jmp    801066a8 <alltraps>

80106e41 <vector31>:
.globl vector31
vector31:
  pushl $0
80106e41:	6a 00                	push   $0x0
  pushl $31
80106e43:	6a 1f                	push   $0x1f
  jmp alltraps
80106e45:	e9 5e f8 ff ff       	jmp    801066a8 <alltraps>

80106e4a <vector32>:
.globl vector32
vector32:
  pushl $0
80106e4a:	6a 00                	push   $0x0
  pushl $32
80106e4c:	6a 20                	push   $0x20
  jmp alltraps
80106e4e:	e9 55 f8 ff ff       	jmp    801066a8 <alltraps>

80106e53 <vector33>:
.globl vector33
vector33:
  pushl $0
80106e53:	6a 00                	push   $0x0
  pushl $33
80106e55:	6a 21                	push   $0x21
  jmp alltraps
80106e57:	e9 4c f8 ff ff       	jmp    801066a8 <alltraps>

80106e5c <vector34>:
.globl vector34
vector34:
  pushl $0
80106e5c:	6a 00                	push   $0x0
  pushl $34
80106e5e:	6a 22                	push   $0x22
  jmp alltraps
80106e60:	e9 43 f8 ff ff       	jmp    801066a8 <alltraps>

80106e65 <vector35>:
.globl vector35
vector35:
  pushl $0
80106e65:	6a 00                	push   $0x0
  pushl $35
80106e67:	6a 23                	push   $0x23
  jmp alltraps
80106e69:	e9 3a f8 ff ff       	jmp    801066a8 <alltraps>

80106e6e <vector36>:
.globl vector36
vector36:
  pushl $0
80106e6e:	6a 00                	push   $0x0
  pushl $36
80106e70:	6a 24                	push   $0x24
  jmp alltraps
80106e72:	e9 31 f8 ff ff       	jmp    801066a8 <alltraps>

80106e77 <vector37>:
.globl vector37
vector37:
  pushl $0
80106e77:	6a 00                	push   $0x0
  pushl $37
80106e79:	6a 25                	push   $0x25
  jmp alltraps
80106e7b:	e9 28 f8 ff ff       	jmp    801066a8 <alltraps>

80106e80 <vector38>:
.globl vector38
vector38:
  pushl $0
80106e80:	6a 00                	push   $0x0
  pushl $38
80106e82:	6a 26                	push   $0x26
  jmp alltraps
80106e84:	e9 1f f8 ff ff       	jmp    801066a8 <alltraps>

80106e89 <vector39>:
.globl vector39
vector39:
  pushl $0
80106e89:	6a 00                	push   $0x0
  pushl $39
80106e8b:	6a 27                	push   $0x27
  jmp alltraps
80106e8d:	e9 16 f8 ff ff       	jmp    801066a8 <alltraps>

80106e92 <vector40>:
.globl vector40
vector40:
  pushl $0
80106e92:	6a 00                	push   $0x0
  pushl $40
80106e94:	6a 28                	push   $0x28
  jmp alltraps
80106e96:	e9 0d f8 ff ff       	jmp    801066a8 <alltraps>

80106e9b <vector41>:
.globl vector41
vector41:
  pushl $0
80106e9b:	6a 00                	push   $0x0
  pushl $41
80106e9d:	6a 29                	push   $0x29
  jmp alltraps
80106e9f:	e9 04 f8 ff ff       	jmp    801066a8 <alltraps>

80106ea4 <vector42>:
.globl vector42
vector42:
  pushl $0
80106ea4:	6a 00                	push   $0x0
  pushl $42
80106ea6:	6a 2a                	push   $0x2a
  jmp alltraps
80106ea8:	e9 fb f7 ff ff       	jmp    801066a8 <alltraps>

80106ead <vector43>:
.globl vector43
vector43:
  pushl $0
80106ead:	6a 00                	push   $0x0
  pushl $43
80106eaf:	6a 2b                	push   $0x2b
  jmp alltraps
80106eb1:	e9 f2 f7 ff ff       	jmp    801066a8 <alltraps>

80106eb6 <vector44>:
.globl vector44
vector44:
  pushl $0
80106eb6:	6a 00                	push   $0x0
  pushl $44
80106eb8:	6a 2c                	push   $0x2c
  jmp alltraps
80106eba:	e9 e9 f7 ff ff       	jmp    801066a8 <alltraps>

80106ebf <vector45>:
.globl vector45
vector45:
  pushl $0
80106ebf:	6a 00                	push   $0x0
  pushl $45
80106ec1:	6a 2d                	push   $0x2d
  jmp alltraps
80106ec3:	e9 e0 f7 ff ff       	jmp    801066a8 <alltraps>

80106ec8 <vector46>:
.globl vector46
vector46:
  pushl $0
80106ec8:	6a 00                	push   $0x0
  pushl $46
80106eca:	6a 2e                	push   $0x2e
  jmp alltraps
80106ecc:	e9 d7 f7 ff ff       	jmp    801066a8 <alltraps>

80106ed1 <vector47>:
.globl vector47
vector47:
  pushl $0
80106ed1:	6a 00                	push   $0x0
  pushl $47
80106ed3:	6a 2f                	push   $0x2f
  jmp alltraps
80106ed5:	e9 ce f7 ff ff       	jmp    801066a8 <alltraps>

80106eda <vector48>:
.globl vector48
vector48:
  pushl $0
80106eda:	6a 00                	push   $0x0
  pushl $48
80106edc:	6a 30                	push   $0x30
  jmp alltraps
80106ede:	e9 c5 f7 ff ff       	jmp    801066a8 <alltraps>

80106ee3 <vector49>:
.globl vector49
vector49:
  pushl $0
80106ee3:	6a 00                	push   $0x0
  pushl $49
80106ee5:	6a 31                	push   $0x31
  jmp alltraps
80106ee7:	e9 bc f7 ff ff       	jmp    801066a8 <alltraps>

80106eec <vector50>:
.globl vector50
vector50:
  pushl $0
80106eec:	6a 00                	push   $0x0
  pushl $50
80106eee:	6a 32                	push   $0x32
  jmp alltraps
80106ef0:	e9 b3 f7 ff ff       	jmp    801066a8 <alltraps>

80106ef5 <vector51>:
.globl vector51
vector51:
  pushl $0
80106ef5:	6a 00                	push   $0x0
  pushl $51
80106ef7:	6a 33                	push   $0x33
  jmp alltraps
80106ef9:	e9 aa f7 ff ff       	jmp    801066a8 <alltraps>

80106efe <vector52>:
.globl vector52
vector52:
  pushl $0
80106efe:	6a 00                	push   $0x0
  pushl $52
80106f00:	6a 34                	push   $0x34
  jmp alltraps
80106f02:	e9 a1 f7 ff ff       	jmp    801066a8 <alltraps>

80106f07 <vector53>:
.globl vector53
vector53:
  pushl $0
80106f07:	6a 00                	push   $0x0
  pushl $53
80106f09:	6a 35                	push   $0x35
  jmp alltraps
80106f0b:	e9 98 f7 ff ff       	jmp    801066a8 <alltraps>

80106f10 <vector54>:
.globl vector54
vector54:
  pushl $0
80106f10:	6a 00                	push   $0x0
  pushl $54
80106f12:	6a 36                	push   $0x36
  jmp alltraps
80106f14:	e9 8f f7 ff ff       	jmp    801066a8 <alltraps>

80106f19 <vector55>:
.globl vector55
vector55:
  pushl $0
80106f19:	6a 00                	push   $0x0
  pushl $55
80106f1b:	6a 37                	push   $0x37
  jmp alltraps
80106f1d:	e9 86 f7 ff ff       	jmp    801066a8 <alltraps>

80106f22 <vector56>:
.globl vector56
vector56:
  pushl $0
80106f22:	6a 00                	push   $0x0
  pushl $56
80106f24:	6a 38                	push   $0x38
  jmp alltraps
80106f26:	e9 7d f7 ff ff       	jmp    801066a8 <alltraps>

80106f2b <vector57>:
.globl vector57
vector57:
  pushl $0
80106f2b:	6a 00                	push   $0x0
  pushl $57
80106f2d:	6a 39                	push   $0x39
  jmp alltraps
80106f2f:	e9 74 f7 ff ff       	jmp    801066a8 <alltraps>

80106f34 <vector58>:
.globl vector58
vector58:
  pushl $0
80106f34:	6a 00                	push   $0x0
  pushl $58
80106f36:	6a 3a                	push   $0x3a
  jmp alltraps
80106f38:	e9 6b f7 ff ff       	jmp    801066a8 <alltraps>

80106f3d <vector59>:
.globl vector59
vector59:
  pushl $0
80106f3d:	6a 00                	push   $0x0
  pushl $59
80106f3f:	6a 3b                	push   $0x3b
  jmp alltraps
80106f41:	e9 62 f7 ff ff       	jmp    801066a8 <alltraps>

80106f46 <vector60>:
.globl vector60
vector60:
  pushl $0
80106f46:	6a 00                	push   $0x0
  pushl $60
80106f48:	6a 3c                	push   $0x3c
  jmp alltraps
80106f4a:	e9 59 f7 ff ff       	jmp    801066a8 <alltraps>

80106f4f <vector61>:
.globl vector61
vector61:
  pushl $0
80106f4f:	6a 00                	push   $0x0
  pushl $61
80106f51:	6a 3d                	push   $0x3d
  jmp alltraps
80106f53:	e9 50 f7 ff ff       	jmp    801066a8 <alltraps>

80106f58 <vector62>:
.globl vector62
vector62:
  pushl $0
80106f58:	6a 00                	push   $0x0
  pushl $62
80106f5a:	6a 3e                	push   $0x3e
  jmp alltraps
80106f5c:	e9 47 f7 ff ff       	jmp    801066a8 <alltraps>

80106f61 <vector63>:
.globl vector63
vector63:
  pushl $0
80106f61:	6a 00                	push   $0x0
  pushl $63
80106f63:	6a 3f                	push   $0x3f
  jmp alltraps
80106f65:	e9 3e f7 ff ff       	jmp    801066a8 <alltraps>

80106f6a <vector64>:
.globl vector64
vector64:
  pushl $0
80106f6a:	6a 00                	push   $0x0
  pushl $64
80106f6c:	6a 40                	push   $0x40
  jmp alltraps
80106f6e:	e9 35 f7 ff ff       	jmp    801066a8 <alltraps>

80106f73 <vector65>:
.globl vector65
vector65:
  pushl $0
80106f73:	6a 00                	push   $0x0
  pushl $65
80106f75:	6a 41                	push   $0x41
  jmp alltraps
80106f77:	e9 2c f7 ff ff       	jmp    801066a8 <alltraps>

80106f7c <vector66>:
.globl vector66
vector66:
  pushl $0
80106f7c:	6a 00                	push   $0x0
  pushl $66
80106f7e:	6a 42                	push   $0x42
  jmp alltraps
80106f80:	e9 23 f7 ff ff       	jmp    801066a8 <alltraps>

80106f85 <vector67>:
.globl vector67
vector67:
  pushl $0
80106f85:	6a 00                	push   $0x0
  pushl $67
80106f87:	6a 43                	push   $0x43
  jmp alltraps
80106f89:	e9 1a f7 ff ff       	jmp    801066a8 <alltraps>

80106f8e <vector68>:
.globl vector68
vector68:
  pushl $0
80106f8e:	6a 00                	push   $0x0
  pushl $68
80106f90:	6a 44                	push   $0x44
  jmp alltraps
80106f92:	e9 11 f7 ff ff       	jmp    801066a8 <alltraps>

80106f97 <vector69>:
.globl vector69
vector69:
  pushl $0
80106f97:	6a 00                	push   $0x0
  pushl $69
80106f99:	6a 45                	push   $0x45
  jmp alltraps
80106f9b:	e9 08 f7 ff ff       	jmp    801066a8 <alltraps>

80106fa0 <vector70>:
.globl vector70
vector70:
  pushl $0
80106fa0:	6a 00                	push   $0x0
  pushl $70
80106fa2:	6a 46                	push   $0x46
  jmp alltraps
80106fa4:	e9 ff f6 ff ff       	jmp    801066a8 <alltraps>

80106fa9 <vector71>:
.globl vector71
vector71:
  pushl $0
80106fa9:	6a 00                	push   $0x0
  pushl $71
80106fab:	6a 47                	push   $0x47
  jmp alltraps
80106fad:	e9 f6 f6 ff ff       	jmp    801066a8 <alltraps>

80106fb2 <vector72>:
.globl vector72
vector72:
  pushl $0
80106fb2:	6a 00                	push   $0x0
  pushl $72
80106fb4:	6a 48                	push   $0x48
  jmp alltraps
80106fb6:	e9 ed f6 ff ff       	jmp    801066a8 <alltraps>

80106fbb <vector73>:
.globl vector73
vector73:
  pushl $0
80106fbb:	6a 00                	push   $0x0
  pushl $73
80106fbd:	6a 49                	push   $0x49
  jmp alltraps
80106fbf:	e9 e4 f6 ff ff       	jmp    801066a8 <alltraps>

80106fc4 <vector74>:
.globl vector74
vector74:
  pushl $0
80106fc4:	6a 00                	push   $0x0
  pushl $74
80106fc6:	6a 4a                	push   $0x4a
  jmp alltraps
80106fc8:	e9 db f6 ff ff       	jmp    801066a8 <alltraps>

80106fcd <vector75>:
.globl vector75
vector75:
  pushl $0
80106fcd:	6a 00                	push   $0x0
  pushl $75
80106fcf:	6a 4b                	push   $0x4b
  jmp alltraps
80106fd1:	e9 d2 f6 ff ff       	jmp    801066a8 <alltraps>

80106fd6 <vector76>:
.globl vector76
vector76:
  pushl $0
80106fd6:	6a 00                	push   $0x0
  pushl $76
80106fd8:	6a 4c                	push   $0x4c
  jmp alltraps
80106fda:	e9 c9 f6 ff ff       	jmp    801066a8 <alltraps>

80106fdf <vector77>:
.globl vector77
vector77:
  pushl $0
80106fdf:	6a 00                	push   $0x0
  pushl $77
80106fe1:	6a 4d                	push   $0x4d
  jmp alltraps
80106fe3:	e9 c0 f6 ff ff       	jmp    801066a8 <alltraps>

80106fe8 <vector78>:
.globl vector78
vector78:
  pushl $0
80106fe8:	6a 00                	push   $0x0
  pushl $78
80106fea:	6a 4e                	push   $0x4e
  jmp alltraps
80106fec:	e9 b7 f6 ff ff       	jmp    801066a8 <alltraps>

80106ff1 <vector79>:
.globl vector79
vector79:
  pushl $0
80106ff1:	6a 00                	push   $0x0
  pushl $79
80106ff3:	6a 4f                	push   $0x4f
  jmp alltraps
80106ff5:	e9 ae f6 ff ff       	jmp    801066a8 <alltraps>

80106ffa <vector80>:
.globl vector80
vector80:
  pushl $0
80106ffa:	6a 00                	push   $0x0
  pushl $80
80106ffc:	6a 50                	push   $0x50
  jmp alltraps
80106ffe:	e9 a5 f6 ff ff       	jmp    801066a8 <alltraps>

80107003 <vector81>:
.globl vector81
vector81:
  pushl $0
80107003:	6a 00                	push   $0x0
  pushl $81
80107005:	6a 51                	push   $0x51
  jmp alltraps
80107007:	e9 9c f6 ff ff       	jmp    801066a8 <alltraps>

8010700c <vector82>:
.globl vector82
vector82:
  pushl $0
8010700c:	6a 00                	push   $0x0
  pushl $82
8010700e:	6a 52                	push   $0x52
  jmp alltraps
80107010:	e9 93 f6 ff ff       	jmp    801066a8 <alltraps>

80107015 <vector83>:
.globl vector83
vector83:
  pushl $0
80107015:	6a 00                	push   $0x0
  pushl $83
80107017:	6a 53                	push   $0x53
  jmp alltraps
80107019:	e9 8a f6 ff ff       	jmp    801066a8 <alltraps>

8010701e <vector84>:
.globl vector84
vector84:
  pushl $0
8010701e:	6a 00                	push   $0x0
  pushl $84
80107020:	6a 54                	push   $0x54
  jmp alltraps
80107022:	e9 81 f6 ff ff       	jmp    801066a8 <alltraps>

80107027 <vector85>:
.globl vector85
vector85:
  pushl $0
80107027:	6a 00                	push   $0x0
  pushl $85
80107029:	6a 55                	push   $0x55
  jmp alltraps
8010702b:	e9 78 f6 ff ff       	jmp    801066a8 <alltraps>

80107030 <vector86>:
.globl vector86
vector86:
  pushl $0
80107030:	6a 00                	push   $0x0
  pushl $86
80107032:	6a 56                	push   $0x56
  jmp alltraps
80107034:	e9 6f f6 ff ff       	jmp    801066a8 <alltraps>

80107039 <vector87>:
.globl vector87
vector87:
  pushl $0
80107039:	6a 00                	push   $0x0
  pushl $87
8010703b:	6a 57                	push   $0x57
  jmp alltraps
8010703d:	e9 66 f6 ff ff       	jmp    801066a8 <alltraps>

80107042 <vector88>:
.globl vector88
vector88:
  pushl $0
80107042:	6a 00                	push   $0x0
  pushl $88
80107044:	6a 58                	push   $0x58
  jmp alltraps
80107046:	e9 5d f6 ff ff       	jmp    801066a8 <alltraps>

8010704b <vector89>:
.globl vector89
vector89:
  pushl $0
8010704b:	6a 00                	push   $0x0
  pushl $89
8010704d:	6a 59                	push   $0x59
  jmp alltraps
8010704f:	e9 54 f6 ff ff       	jmp    801066a8 <alltraps>

80107054 <vector90>:
.globl vector90
vector90:
  pushl $0
80107054:	6a 00                	push   $0x0
  pushl $90
80107056:	6a 5a                	push   $0x5a
  jmp alltraps
80107058:	e9 4b f6 ff ff       	jmp    801066a8 <alltraps>

8010705d <vector91>:
.globl vector91
vector91:
  pushl $0
8010705d:	6a 00                	push   $0x0
  pushl $91
8010705f:	6a 5b                	push   $0x5b
  jmp alltraps
80107061:	e9 42 f6 ff ff       	jmp    801066a8 <alltraps>

80107066 <vector92>:
.globl vector92
vector92:
  pushl $0
80107066:	6a 00                	push   $0x0
  pushl $92
80107068:	6a 5c                	push   $0x5c
  jmp alltraps
8010706a:	e9 39 f6 ff ff       	jmp    801066a8 <alltraps>

8010706f <vector93>:
.globl vector93
vector93:
  pushl $0
8010706f:	6a 00                	push   $0x0
  pushl $93
80107071:	6a 5d                	push   $0x5d
  jmp alltraps
80107073:	e9 30 f6 ff ff       	jmp    801066a8 <alltraps>

80107078 <vector94>:
.globl vector94
vector94:
  pushl $0
80107078:	6a 00                	push   $0x0
  pushl $94
8010707a:	6a 5e                	push   $0x5e
  jmp alltraps
8010707c:	e9 27 f6 ff ff       	jmp    801066a8 <alltraps>

80107081 <vector95>:
.globl vector95
vector95:
  pushl $0
80107081:	6a 00                	push   $0x0
  pushl $95
80107083:	6a 5f                	push   $0x5f
  jmp alltraps
80107085:	e9 1e f6 ff ff       	jmp    801066a8 <alltraps>

8010708a <vector96>:
.globl vector96
vector96:
  pushl $0
8010708a:	6a 00                	push   $0x0
  pushl $96
8010708c:	6a 60                	push   $0x60
  jmp alltraps
8010708e:	e9 15 f6 ff ff       	jmp    801066a8 <alltraps>

80107093 <vector97>:
.globl vector97
vector97:
  pushl $0
80107093:	6a 00                	push   $0x0
  pushl $97
80107095:	6a 61                	push   $0x61
  jmp alltraps
80107097:	e9 0c f6 ff ff       	jmp    801066a8 <alltraps>

8010709c <vector98>:
.globl vector98
vector98:
  pushl $0
8010709c:	6a 00                	push   $0x0
  pushl $98
8010709e:	6a 62                	push   $0x62
  jmp alltraps
801070a0:	e9 03 f6 ff ff       	jmp    801066a8 <alltraps>

801070a5 <vector99>:
.globl vector99
vector99:
  pushl $0
801070a5:	6a 00                	push   $0x0
  pushl $99
801070a7:	6a 63                	push   $0x63
  jmp alltraps
801070a9:	e9 fa f5 ff ff       	jmp    801066a8 <alltraps>

801070ae <vector100>:
.globl vector100
vector100:
  pushl $0
801070ae:	6a 00                	push   $0x0
  pushl $100
801070b0:	6a 64                	push   $0x64
  jmp alltraps
801070b2:	e9 f1 f5 ff ff       	jmp    801066a8 <alltraps>

801070b7 <vector101>:
.globl vector101
vector101:
  pushl $0
801070b7:	6a 00                	push   $0x0
  pushl $101
801070b9:	6a 65                	push   $0x65
  jmp alltraps
801070bb:	e9 e8 f5 ff ff       	jmp    801066a8 <alltraps>

801070c0 <vector102>:
.globl vector102
vector102:
  pushl $0
801070c0:	6a 00                	push   $0x0
  pushl $102
801070c2:	6a 66                	push   $0x66
  jmp alltraps
801070c4:	e9 df f5 ff ff       	jmp    801066a8 <alltraps>

801070c9 <vector103>:
.globl vector103
vector103:
  pushl $0
801070c9:	6a 00                	push   $0x0
  pushl $103
801070cb:	6a 67                	push   $0x67
  jmp alltraps
801070cd:	e9 d6 f5 ff ff       	jmp    801066a8 <alltraps>

801070d2 <vector104>:
.globl vector104
vector104:
  pushl $0
801070d2:	6a 00                	push   $0x0
  pushl $104
801070d4:	6a 68                	push   $0x68
  jmp alltraps
801070d6:	e9 cd f5 ff ff       	jmp    801066a8 <alltraps>

801070db <vector105>:
.globl vector105
vector105:
  pushl $0
801070db:	6a 00                	push   $0x0
  pushl $105
801070dd:	6a 69                	push   $0x69
  jmp alltraps
801070df:	e9 c4 f5 ff ff       	jmp    801066a8 <alltraps>

801070e4 <vector106>:
.globl vector106
vector106:
  pushl $0
801070e4:	6a 00                	push   $0x0
  pushl $106
801070e6:	6a 6a                	push   $0x6a
  jmp alltraps
801070e8:	e9 bb f5 ff ff       	jmp    801066a8 <alltraps>

801070ed <vector107>:
.globl vector107
vector107:
  pushl $0
801070ed:	6a 00                	push   $0x0
  pushl $107
801070ef:	6a 6b                	push   $0x6b
  jmp alltraps
801070f1:	e9 b2 f5 ff ff       	jmp    801066a8 <alltraps>

801070f6 <vector108>:
.globl vector108
vector108:
  pushl $0
801070f6:	6a 00                	push   $0x0
  pushl $108
801070f8:	6a 6c                	push   $0x6c
  jmp alltraps
801070fa:	e9 a9 f5 ff ff       	jmp    801066a8 <alltraps>

801070ff <vector109>:
.globl vector109
vector109:
  pushl $0
801070ff:	6a 00                	push   $0x0
  pushl $109
80107101:	6a 6d                	push   $0x6d
  jmp alltraps
80107103:	e9 a0 f5 ff ff       	jmp    801066a8 <alltraps>

80107108 <vector110>:
.globl vector110
vector110:
  pushl $0
80107108:	6a 00                	push   $0x0
  pushl $110
8010710a:	6a 6e                	push   $0x6e
  jmp alltraps
8010710c:	e9 97 f5 ff ff       	jmp    801066a8 <alltraps>

80107111 <vector111>:
.globl vector111
vector111:
  pushl $0
80107111:	6a 00                	push   $0x0
  pushl $111
80107113:	6a 6f                	push   $0x6f
  jmp alltraps
80107115:	e9 8e f5 ff ff       	jmp    801066a8 <alltraps>

8010711a <vector112>:
.globl vector112
vector112:
  pushl $0
8010711a:	6a 00                	push   $0x0
  pushl $112
8010711c:	6a 70                	push   $0x70
  jmp alltraps
8010711e:	e9 85 f5 ff ff       	jmp    801066a8 <alltraps>

80107123 <vector113>:
.globl vector113
vector113:
  pushl $0
80107123:	6a 00                	push   $0x0
  pushl $113
80107125:	6a 71                	push   $0x71
  jmp alltraps
80107127:	e9 7c f5 ff ff       	jmp    801066a8 <alltraps>

8010712c <vector114>:
.globl vector114
vector114:
  pushl $0
8010712c:	6a 00                	push   $0x0
  pushl $114
8010712e:	6a 72                	push   $0x72
  jmp alltraps
80107130:	e9 73 f5 ff ff       	jmp    801066a8 <alltraps>

80107135 <vector115>:
.globl vector115
vector115:
  pushl $0
80107135:	6a 00                	push   $0x0
  pushl $115
80107137:	6a 73                	push   $0x73
  jmp alltraps
80107139:	e9 6a f5 ff ff       	jmp    801066a8 <alltraps>

8010713e <vector116>:
.globl vector116
vector116:
  pushl $0
8010713e:	6a 00                	push   $0x0
  pushl $116
80107140:	6a 74                	push   $0x74
  jmp alltraps
80107142:	e9 61 f5 ff ff       	jmp    801066a8 <alltraps>

80107147 <vector117>:
.globl vector117
vector117:
  pushl $0
80107147:	6a 00                	push   $0x0
  pushl $117
80107149:	6a 75                	push   $0x75
  jmp alltraps
8010714b:	e9 58 f5 ff ff       	jmp    801066a8 <alltraps>

80107150 <vector118>:
.globl vector118
vector118:
  pushl $0
80107150:	6a 00                	push   $0x0
  pushl $118
80107152:	6a 76                	push   $0x76
  jmp alltraps
80107154:	e9 4f f5 ff ff       	jmp    801066a8 <alltraps>

80107159 <vector119>:
.globl vector119
vector119:
  pushl $0
80107159:	6a 00                	push   $0x0
  pushl $119
8010715b:	6a 77                	push   $0x77
  jmp alltraps
8010715d:	e9 46 f5 ff ff       	jmp    801066a8 <alltraps>

80107162 <vector120>:
.globl vector120
vector120:
  pushl $0
80107162:	6a 00                	push   $0x0
  pushl $120
80107164:	6a 78                	push   $0x78
  jmp alltraps
80107166:	e9 3d f5 ff ff       	jmp    801066a8 <alltraps>

8010716b <vector121>:
.globl vector121
vector121:
  pushl $0
8010716b:	6a 00                	push   $0x0
  pushl $121
8010716d:	6a 79                	push   $0x79
  jmp alltraps
8010716f:	e9 34 f5 ff ff       	jmp    801066a8 <alltraps>

80107174 <vector122>:
.globl vector122
vector122:
  pushl $0
80107174:	6a 00                	push   $0x0
  pushl $122
80107176:	6a 7a                	push   $0x7a
  jmp alltraps
80107178:	e9 2b f5 ff ff       	jmp    801066a8 <alltraps>

8010717d <vector123>:
.globl vector123
vector123:
  pushl $0
8010717d:	6a 00                	push   $0x0
  pushl $123
8010717f:	6a 7b                	push   $0x7b
  jmp alltraps
80107181:	e9 22 f5 ff ff       	jmp    801066a8 <alltraps>

80107186 <vector124>:
.globl vector124
vector124:
  pushl $0
80107186:	6a 00                	push   $0x0
  pushl $124
80107188:	6a 7c                	push   $0x7c
  jmp alltraps
8010718a:	e9 19 f5 ff ff       	jmp    801066a8 <alltraps>

8010718f <vector125>:
.globl vector125
vector125:
  pushl $0
8010718f:	6a 00                	push   $0x0
  pushl $125
80107191:	6a 7d                	push   $0x7d
  jmp alltraps
80107193:	e9 10 f5 ff ff       	jmp    801066a8 <alltraps>

80107198 <vector126>:
.globl vector126
vector126:
  pushl $0
80107198:	6a 00                	push   $0x0
  pushl $126
8010719a:	6a 7e                	push   $0x7e
  jmp alltraps
8010719c:	e9 07 f5 ff ff       	jmp    801066a8 <alltraps>

801071a1 <vector127>:
.globl vector127
vector127:
  pushl $0
801071a1:	6a 00                	push   $0x0
  pushl $127
801071a3:	6a 7f                	push   $0x7f
  jmp alltraps
801071a5:	e9 fe f4 ff ff       	jmp    801066a8 <alltraps>

801071aa <vector128>:
.globl vector128
vector128:
  pushl $0
801071aa:	6a 00                	push   $0x0
  pushl $128
801071ac:	68 80 00 00 00       	push   $0x80
  jmp alltraps
801071b1:	e9 f2 f4 ff ff       	jmp    801066a8 <alltraps>

801071b6 <vector129>:
.globl vector129
vector129:
  pushl $0
801071b6:	6a 00                	push   $0x0
  pushl $129
801071b8:	68 81 00 00 00       	push   $0x81
  jmp alltraps
801071bd:	e9 e6 f4 ff ff       	jmp    801066a8 <alltraps>

801071c2 <vector130>:
.globl vector130
vector130:
  pushl $0
801071c2:	6a 00                	push   $0x0
  pushl $130
801071c4:	68 82 00 00 00       	push   $0x82
  jmp alltraps
801071c9:	e9 da f4 ff ff       	jmp    801066a8 <alltraps>

801071ce <vector131>:
.globl vector131
vector131:
  pushl $0
801071ce:	6a 00                	push   $0x0
  pushl $131
801071d0:	68 83 00 00 00       	push   $0x83
  jmp alltraps
801071d5:	e9 ce f4 ff ff       	jmp    801066a8 <alltraps>

801071da <vector132>:
.globl vector132
vector132:
  pushl $0
801071da:	6a 00                	push   $0x0
  pushl $132
801071dc:	68 84 00 00 00       	push   $0x84
  jmp alltraps
801071e1:	e9 c2 f4 ff ff       	jmp    801066a8 <alltraps>

801071e6 <vector133>:
.globl vector133
vector133:
  pushl $0
801071e6:	6a 00                	push   $0x0
  pushl $133
801071e8:	68 85 00 00 00       	push   $0x85
  jmp alltraps
801071ed:	e9 b6 f4 ff ff       	jmp    801066a8 <alltraps>

801071f2 <vector134>:
.globl vector134
vector134:
  pushl $0
801071f2:	6a 00                	push   $0x0
  pushl $134
801071f4:	68 86 00 00 00       	push   $0x86
  jmp alltraps
801071f9:	e9 aa f4 ff ff       	jmp    801066a8 <alltraps>

801071fe <vector135>:
.globl vector135
vector135:
  pushl $0
801071fe:	6a 00                	push   $0x0
  pushl $135
80107200:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80107205:	e9 9e f4 ff ff       	jmp    801066a8 <alltraps>

8010720a <vector136>:
.globl vector136
vector136:
  pushl $0
8010720a:	6a 00                	push   $0x0
  pushl $136
8010720c:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80107211:	e9 92 f4 ff ff       	jmp    801066a8 <alltraps>

80107216 <vector137>:
.globl vector137
vector137:
  pushl $0
80107216:	6a 00                	push   $0x0
  pushl $137
80107218:	68 89 00 00 00       	push   $0x89
  jmp alltraps
8010721d:	e9 86 f4 ff ff       	jmp    801066a8 <alltraps>

80107222 <vector138>:
.globl vector138
vector138:
  pushl $0
80107222:	6a 00                	push   $0x0
  pushl $138
80107224:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80107229:	e9 7a f4 ff ff       	jmp    801066a8 <alltraps>

8010722e <vector139>:
.globl vector139
vector139:
  pushl $0
8010722e:	6a 00                	push   $0x0
  pushl $139
80107230:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80107235:	e9 6e f4 ff ff       	jmp    801066a8 <alltraps>

8010723a <vector140>:
.globl vector140
vector140:
  pushl $0
8010723a:	6a 00                	push   $0x0
  pushl $140
8010723c:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80107241:	e9 62 f4 ff ff       	jmp    801066a8 <alltraps>

80107246 <vector141>:
.globl vector141
vector141:
  pushl $0
80107246:	6a 00                	push   $0x0
  pushl $141
80107248:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
8010724d:	e9 56 f4 ff ff       	jmp    801066a8 <alltraps>

80107252 <vector142>:
.globl vector142
vector142:
  pushl $0
80107252:	6a 00                	push   $0x0
  pushl $142
80107254:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
80107259:	e9 4a f4 ff ff       	jmp    801066a8 <alltraps>

8010725e <vector143>:
.globl vector143
vector143:
  pushl $0
8010725e:	6a 00                	push   $0x0
  pushl $143
80107260:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80107265:	e9 3e f4 ff ff       	jmp    801066a8 <alltraps>

8010726a <vector144>:
.globl vector144
vector144:
  pushl $0
8010726a:	6a 00                	push   $0x0
  pushl $144
8010726c:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80107271:	e9 32 f4 ff ff       	jmp    801066a8 <alltraps>

80107276 <vector145>:
.globl vector145
vector145:
  pushl $0
80107276:	6a 00                	push   $0x0
  pushl $145
80107278:	68 91 00 00 00       	push   $0x91
  jmp alltraps
8010727d:	e9 26 f4 ff ff       	jmp    801066a8 <alltraps>

80107282 <vector146>:
.globl vector146
vector146:
  pushl $0
80107282:	6a 00                	push   $0x0
  pushl $146
80107284:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80107289:	e9 1a f4 ff ff       	jmp    801066a8 <alltraps>

8010728e <vector147>:
.globl vector147
vector147:
  pushl $0
8010728e:	6a 00                	push   $0x0
  pushl $147
80107290:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80107295:	e9 0e f4 ff ff       	jmp    801066a8 <alltraps>

8010729a <vector148>:
.globl vector148
vector148:
  pushl $0
8010729a:	6a 00                	push   $0x0
  pushl $148
8010729c:	68 94 00 00 00       	push   $0x94
  jmp alltraps
801072a1:	e9 02 f4 ff ff       	jmp    801066a8 <alltraps>

801072a6 <vector149>:
.globl vector149
vector149:
  pushl $0
801072a6:	6a 00                	push   $0x0
  pushl $149
801072a8:	68 95 00 00 00       	push   $0x95
  jmp alltraps
801072ad:	e9 f6 f3 ff ff       	jmp    801066a8 <alltraps>

801072b2 <vector150>:
.globl vector150
vector150:
  pushl $0
801072b2:	6a 00                	push   $0x0
  pushl $150
801072b4:	68 96 00 00 00       	push   $0x96
  jmp alltraps
801072b9:	e9 ea f3 ff ff       	jmp    801066a8 <alltraps>

801072be <vector151>:
.globl vector151
vector151:
  pushl $0
801072be:	6a 00                	push   $0x0
  pushl $151
801072c0:	68 97 00 00 00       	push   $0x97
  jmp alltraps
801072c5:	e9 de f3 ff ff       	jmp    801066a8 <alltraps>

801072ca <vector152>:
.globl vector152
vector152:
  pushl $0
801072ca:	6a 00                	push   $0x0
  pushl $152
801072cc:	68 98 00 00 00       	push   $0x98
  jmp alltraps
801072d1:	e9 d2 f3 ff ff       	jmp    801066a8 <alltraps>

801072d6 <vector153>:
.globl vector153
vector153:
  pushl $0
801072d6:	6a 00                	push   $0x0
  pushl $153
801072d8:	68 99 00 00 00       	push   $0x99
  jmp alltraps
801072dd:	e9 c6 f3 ff ff       	jmp    801066a8 <alltraps>

801072e2 <vector154>:
.globl vector154
vector154:
  pushl $0
801072e2:	6a 00                	push   $0x0
  pushl $154
801072e4:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
801072e9:	e9 ba f3 ff ff       	jmp    801066a8 <alltraps>

801072ee <vector155>:
.globl vector155
vector155:
  pushl $0
801072ee:	6a 00                	push   $0x0
  pushl $155
801072f0:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
801072f5:	e9 ae f3 ff ff       	jmp    801066a8 <alltraps>

801072fa <vector156>:
.globl vector156
vector156:
  pushl $0
801072fa:	6a 00                	push   $0x0
  pushl $156
801072fc:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80107301:	e9 a2 f3 ff ff       	jmp    801066a8 <alltraps>

80107306 <vector157>:
.globl vector157
vector157:
  pushl $0
80107306:	6a 00                	push   $0x0
  pushl $157
80107308:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
8010730d:	e9 96 f3 ff ff       	jmp    801066a8 <alltraps>

80107312 <vector158>:
.globl vector158
vector158:
  pushl $0
80107312:	6a 00                	push   $0x0
  pushl $158
80107314:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80107319:	e9 8a f3 ff ff       	jmp    801066a8 <alltraps>

8010731e <vector159>:
.globl vector159
vector159:
  pushl $0
8010731e:	6a 00                	push   $0x0
  pushl $159
80107320:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80107325:	e9 7e f3 ff ff       	jmp    801066a8 <alltraps>

8010732a <vector160>:
.globl vector160
vector160:
  pushl $0
8010732a:	6a 00                	push   $0x0
  pushl $160
8010732c:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80107331:	e9 72 f3 ff ff       	jmp    801066a8 <alltraps>

80107336 <vector161>:
.globl vector161
vector161:
  pushl $0
80107336:	6a 00                	push   $0x0
  pushl $161
80107338:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
8010733d:	e9 66 f3 ff ff       	jmp    801066a8 <alltraps>

80107342 <vector162>:
.globl vector162
vector162:
  pushl $0
80107342:	6a 00                	push   $0x0
  pushl $162
80107344:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
80107349:	e9 5a f3 ff ff       	jmp    801066a8 <alltraps>

8010734e <vector163>:
.globl vector163
vector163:
  pushl $0
8010734e:	6a 00                	push   $0x0
  pushl $163
80107350:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80107355:	e9 4e f3 ff ff       	jmp    801066a8 <alltraps>

8010735a <vector164>:
.globl vector164
vector164:
  pushl $0
8010735a:	6a 00                	push   $0x0
  pushl $164
8010735c:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80107361:	e9 42 f3 ff ff       	jmp    801066a8 <alltraps>

80107366 <vector165>:
.globl vector165
vector165:
  pushl $0
80107366:	6a 00                	push   $0x0
  pushl $165
80107368:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
8010736d:	e9 36 f3 ff ff       	jmp    801066a8 <alltraps>

80107372 <vector166>:
.globl vector166
vector166:
  pushl $0
80107372:	6a 00                	push   $0x0
  pushl $166
80107374:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80107379:	e9 2a f3 ff ff       	jmp    801066a8 <alltraps>

8010737e <vector167>:
.globl vector167
vector167:
  pushl $0
8010737e:	6a 00                	push   $0x0
  pushl $167
80107380:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80107385:	e9 1e f3 ff ff       	jmp    801066a8 <alltraps>

8010738a <vector168>:
.globl vector168
vector168:
  pushl $0
8010738a:	6a 00                	push   $0x0
  pushl $168
8010738c:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80107391:	e9 12 f3 ff ff       	jmp    801066a8 <alltraps>

80107396 <vector169>:
.globl vector169
vector169:
  pushl $0
80107396:	6a 00                	push   $0x0
  pushl $169
80107398:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
8010739d:	e9 06 f3 ff ff       	jmp    801066a8 <alltraps>

801073a2 <vector170>:
.globl vector170
vector170:
  pushl $0
801073a2:	6a 00                	push   $0x0
  pushl $170
801073a4:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
801073a9:	e9 fa f2 ff ff       	jmp    801066a8 <alltraps>

801073ae <vector171>:
.globl vector171
vector171:
  pushl $0
801073ae:	6a 00                	push   $0x0
  pushl $171
801073b0:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
801073b5:	e9 ee f2 ff ff       	jmp    801066a8 <alltraps>

801073ba <vector172>:
.globl vector172
vector172:
  pushl $0
801073ba:	6a 00                	push   $0x0
  pushl $172
801073bc:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
801073c1:	e9 e2 f2 ff ff       	jmp    801066a8 <alltraps>

801073c6 <vector173>:
.globl vector173
vector173:
  pushl $0
801073c6:	6a 00                	push   $0x0
  pushl $173
801073c8:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
801073cd:	e9 d6 f2 ff ff       	jmp    801066a8 <alltraps>

801073d2 <vector174>:
.globl vector174
vector174:
  pushl $0
801073d2:	6a 00                	push   $0x0
  pushl $174
801073d4:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
801073d9:	e9 ca f2 ff ff       	jmp    801066a8 <alltraps>

801073de <vector175>:
.globl vector175
vector175:
  pushl $0
801073de:	6a 00                	push   $0x0
  pushl $175
801073e0:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
801073e5:	e9 be f2 ff ff       	jmp    801066a8 <alltraps>

801073ea <vector176>:
.globl vector176
vector176:
  pushl $0
801073ea:	6a 00                	push   $0x0
  pushl $176
801073ec:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
801073f1:	e9 b2 f2 ff ff       	jmp    801066a8 <alltraps>

801073f6 <vector177>:
.globl vector177
vector177:
  pushl $0
801073f6:	6a 00                	push   $0x0
  pushl $177
801073f8:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
801073fd:	e9 a6 f2 ff ff       	jmp    801066a8 <alltraps>

80107402 <vector178>:
.globl vector178
vector178:
  pushl $0
80107402:	6a 00                	push   $0x0
  pushl $178
80107404:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80107409:	e9 9a f2 ff ff       	jmp    801066a8 <alltraps>

8010740e <vector179>:
.globl vector179
vector179:
  pushl $0
8010740e:	6a 00                	push   $0x0
  pushl $179
80107410:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80107415:	e9 8e f2 ff ff       	jmp    801066a8 <alltraps>

8010741a <vector180>:
.globl vector180
vector180:
  pushl $0
8010741a:	6a 00                	push   $0x0
  pushl $180
8010741c:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
80107421:	e9 82 f2 ff ff       	jmp    801066a8 <alltraps>

80107426 <vector181>:
.globl vector181
vector181:
  pushl $0
80107426:	6a 00                	push   $0x0
  pushl $181
80107428:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
8010742d:	e9 76 f2 ff ff       	jmp    801066a8 <alltraps>

80107432 <vector182>:
.globl vector182
vector182:
  pushl $0
80107432:	6a 00                	push   $0x0
  pushl $182
80107434:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80107439:	e9 6a f2 ff ff       	jmp    801066a8 <alltraps>

8010743e <vector183>:
.globl vector183
vector183:
  pushl $0
8010743e:	6a 00                	push   $0x0
  pushl $183
80107440:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107445:	e9 5e f2 ff ff       	jmp    801066a8 <alltraps>

8010744a <vector184>:
.globl vector184
vector184:
  pushl $0
8010744a:	6a 00                	push   $0x0
  pushl $184
8010744c:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
80107451:	e9 52 f2 ff ff       	jmp    801066a8 <alltraps>

80107456 <vector185>:
.globl vector185
vector185:
  pushl $0
80107456:	6a 00                	push   $0x0
  pushl $185
80107458:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
8010745d:	e9 46 f2 ff ff       	jmp    801066a8 <alltraps>

80107462 <vector186>:
.globl vector186
vector186:
  pushl $0
80107462:	6a 00                	push   $0x0
  pushl $186
80107464:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
80107469:	e9 3a f2 ff ff       	jmp    801066a8 <alltraps>

8010746e <vector187>:
.globl vector187
vector187:
  pushl $0
8010746e:	6a 00                	push   $0x0
  pushl $187
80107470:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80107475:	e9 2e f2 ff ff       	jmp    801066a8 <alltraps>

8010747a <vector188>:
.globl vector188
vector188:
  pushl $0
8010747a:	6a 00                	push   $0x0
  pushl $188
8010747c:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80107481:	e9 22 f2 ff ff       	jmp    801066a8 <alltraps>

80107486 <vector189>:
.globl vector189
vector189:
  pushl $0
80107486:	6a 00                	push   $0x0
  pushl $189
80107488:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
8010748d:	e9 16 f2 ff ff       	jmp    801066a8 <alltraps>

80107492 <vector190>:
.globl vector190
vector190:
  pushl $0
80107492:	6a 00                	push   $0x0
  pushl $190
80107494:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80107499:	e9 0a f2 ff ff       	jmp    801066a8 <alltraps>

8010749e <vector191>:
.globl vector191
vector191:
  pushl $0
8010749e:	6a 00                	push   $0x0
  pushl $191
801074a0:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
801074a5:	e9 fe f1 ff ff       	jmp    801066a8 <alltraps>

801074aa <vector192>:
.globl vector192
vector192:
  pushl $0
801074aa:	6a 00                	push   $0x0
  pushl $192
801074ac:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
801074b1:	e9 f2 f1 ff ff       	jmp    801066a8 <alltraps>

801074b6 <vector193>:
.globl vector193
vector193:
  pushl $0
801074b6:	6a 00                	push   $0x0
  pushl $193
801074b8:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
801074bd:	e9 e6 f1 ff ff       	jmp    801066a8 <alltraps>

801074c2 <vector194>:
.globl vector194
vector194:
  pushl $0
801074c2:	6a 00                	push   $0x0
  pushl $194
801074c4:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
801074c9:	e9 da f1 ff ff       	jmp    801066a8 <alltraps>

801074ce <vector195>:
.globl vector195
vector195:
  pushl $0
801074ce:	6a 00                	push   $0x0
  pushl $195
801074d0:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
801074d5:	e9 ce f1 ff ff       	jmp    801066a8 <alltraps>

801074da <vector196>:
.globl vector196
vector196:
  pushl $0
801074da:	6a 00                	push   $0x0
  pushl $196
801074dc:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
801074e1:	e9 c2 f1 ff ff       	jmp    801066a8 <alltraps>

801074e6 <vector197>:
.globl vector197
vector197:
  pushl $0
801074e6:	6a 00                	push   $0x0
  pushl $197
801074e8:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
801074ed:	e9 b6 f1 ff ff       	jmp    801066a8 <alltraps>

801074f2 <vector198>:
.globl vector198
vector198:
  pushl $0
801074f2:	6a 00                	push   $0x0
  pushl $198
801074f4:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
801074f9:	e9 aa f1 ff ff       	jmp    801066a8 <alltraps>

801074fe <vector199>:
.globl vector199
vector199:
  pushl $0
801074fe:	6a 00                	push   $0x0
  pushl $199
80107500:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
80107505:	e9 9e f1 ff ff       	jmp    801066a8 <alltraps>

8010750a <vector200>:
.globl vector200
vector200:
  pushl $0
8010750a:	6a 00                	push   $0x0
  pushl $200
8010750c:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
80107511:	e9 92 f1 ff ff       	jmp    801066a8 <alltraps>

80107516 <vector201>:
.globl vector201
vector201:
  pushl $0
80107516:	6a 00                	push   $0x0
  pushl $201
80107518:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
8010751d:	e9 86 f1 ff ff       	jmp    801066a8 <alltraps>

80107522 <vector202>:
.globl vector202
vector202:
  pushl $0
80107522:	6a 00                	push   $0x0
  pushl $202
80107524:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80107529:	e9 7a f1 ff ff       	jmp    801066a8 <alltraps>

8010752e <vector203>:
.globl vector203
vector203:
  pushl $0
8010752e:	6a 00                	push   $0x0
  pushl $203
80107530:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107535:	e9 6e f1 ff ff       	jmp    801066a8 <alltraps>

8010753a <vector204>:
.globl vector204
vector204:
  pushl $0
8010753a:	6a 00                	push   $0x0
  pushl $204
8010753c:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80107541:	e9 62 f1 ff ff       	jmp    801066a8 <alltraps>

80107546 <vector205>:
.globl vector205
vector205:
  pushl $0
80107546:	6a 00                	push   $0x0
  pushl $205
80107548:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
8010754d:	e9 56 f1 ff ff       	jmp    801066a8 <alltraps>

80107552 <vector206>:
.globl vector206
vector206:
  pushl $0
80107552:	6a 00                	push   $0x0
  pushl $206
80107554:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
80107559:	e9 4a f1 ff ff       	jmp    801066a8 <alltraps>

8010755e <vector207>:
.globl vector207
vector207:
  pushl $0
8010755e:	6a 00                	push   $0x0
  pushl $207
80107560:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80107565:	e9 3e f1 ff ff       	jmp    801066a8 <alltraps>

8010756a <vector208>:
.globl vector208
vector208:
  pushl $0
8010756a:	6a 00                	push   $0x0
  pushl $208
8010756c:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80107571:	e9 32 f1 ff ff       	jmp    801066a8 <alltraps>

80107576 <vector209>:
.globl vector209
vector209:
  pushl $0
80107576:	6a 00                	push   $0x0
  pushl $209
80107578:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
8010757d:	e9 26 f1 ff ff       	jmp    801066a8 <alltraps>

80107582 <vector210>:
.globl vector210
vector210:
  pushl $0
80107582:	6a 00                	push   $0x0
  pushl $210
80107584:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
80107589:	e9 1a f1 ff ff       	jmp    801066a8 <alltraps>

8010758e <vector211>:
.globl vector211
vector211:
  pushl $0
8010758e:	6a 00                	push   $0x0
  pushl $211
80107590:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80107595:	e9 0e f1 ff ff       	jmp    801066a8 <alltraps>

8010759a <vector212>:
.globl vector212
vector212:
  pushl $0
8010759a:	6a 00                	push   $0x0
  pushl $212
8010759c:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
801075a1:	e9 02 f1 ff ff       	jmp    801066a8 <alltraps>

801075a6 <vector213>:
.globl vector213
vector213:
  pushl $0
801075a6:	6a 00                	push   $0x0
  pushl $213
801075a8:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
801075ad:	e9 f6 f0 ff ff       	jmp    801066a8 <alltraps>

801075b2 <vector214>:
.globl vector214
vector214:
  pushl $0
801075b2:	6a 00                	push   $0x0
  pushl $214
801075b4:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
801075b9:	e9 ea f0 ff ff       	jmp    801066a8 <alltraps>

801075be <vector215>:
.globl vector215
vector215:
  pushl $0
801075be:	6a 00                	push   $0x0
  pushl $215
801075c0:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
801075c5:	e9 de f0 ff ff       	jmp    801066a8 <alltraps>

801075ca <vector216>:
.globl vector216
vector216:
  pushl $0
801075ca:	6a 00                	push   $0x0
  pushl $216
801075cc:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
801075d1:	e9 d2 f0 ff ff       	jmp    801066a8 <alltraps>

801075d6 <vector217>:
.globl vector217
vector217:
  pushl $0
801075d6:	6a 00                	push   $0x0
  pushl $217
801075d8:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
801075dd:	e9 c6 f0 ff ff       	jmp    801066a8 <alltraps>

801075e2 <vector218>:
.globl vector218
vector218:
  pushl $0
801075e2:	6a 00                	push   $0x0
  pushl $218
801075e4:	68 da 00 00 00       	push   $0xda
  jmp alltraps
801075e9:	e9 ba f0 ff ff       	jmp    801066a8 <alltraps>

801075ee <vector219>:
.globl vector219
vector219:
  pushl $0
801075ee:	6a 00                	push   $0x0
  pushl $219
801075f0:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
801075f5:	e9 ae f0 ff ff       	jmp    801066a8 <alltraps>

801075fa <vector220>:
.globl vector220
vector220:
  pushl $0
801075fa:	6a 00                	push   $0x0
  pushl $220
801075fc:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
80107601:	e9 a2 f0 ff ff       	jmp    801066a8 <alltraps>

80107606 <vector221>:
.globl vector221
vector221:
  pushl $0
80107606:	6a 00                	push   $0x0
  pushl $221
80107608:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
8010760d:	e9 96 f0 ff ff       	jmp    801066a8 <alltraps>

80107612 <vector222>:
.globl vector222
vector222:
  pushl $0
80107612:	6a 00                	push   $0x0
  pushl $222
80107614:	68 de 00 00 00       	push   $0xde
  jmp alltraps
80107619:	e9 8a f0 ff ff       	jmp    801066a8 <alltraps>

8010761e <vector223>:
.globl vector223
vector223:
  pushl $0
8010761e:	6a 00                	push   $0x0
  pushl $223
80107620:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80107625:	e9 7e f0 ff ff       	jmp    801066a8 <alltraps>

8010762a <vector224>:
.globl vector224
vector224:
  pushl $0
8010762a:	6a 00                	push   $0x0
  pushl $224
8010762c:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
80107631:	e9 72 f0 ff ff       	jmp    801066a8 <alltraps>

80107636 <vector225>:
.globl vector225
vector225:
  pushl $0
80107636:	6a 00                	push   $0x0
  pushl $225
80107638:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
8010763d:	e9 66 f0 ff ff       	jmp    801066a8 <alltraps>

80107642 <vector226>:
.globl vector226
vector226:
  pushl $0
80107642:	6a 00                	push   $0x0
  pushl $226
80107644:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
80107649:	e9 5a f0 ff ff       	jmp    801066a8 <alltraps>

8010764e <vector227>:
.globl vector227
vector227:
  pushl $0
8010764e:	6a 00                	push   $0x0
  pushl $227
80107650:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
80107655:	e9 4e f0 ff ff       	jmp    801066a8 <alltraps>

8010765a <vector228>:
.globl vector228
vector228:
  pushl $0
8010765a:	6a 00                	push   $0x0
  pushl $228
8010765c:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
80107661:	e9 42 f0 ff ff       	jmp    801066a8 <alltraps>

80107666 <vector229>:
.globl vector229
vector229:
  pushl $0
80107666:	6a 00                	push   $0x0
  pushl $229
80107668:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
8010766d:	e9 36 f0 ff ff       	jmp    801066a8 <alltraps>

80107672 <vector230>:
.globl vector230
vector230:
  pushl $0
80107672:	6a 00                	push   $0x0
  pushl $230
80107674:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
80107679:	e9 2a f0 ff ff       	jmp    801066a8 <alltraps>

8010767e <vector231>:
.globl vector231
vector231:
  pushl $0
8010767e:	6a 00                	push   $0x0
  pushl $231
80107680:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
80107685:	e9 1e f0 ff ff       	jmp    801066a8 <alltraps>

8010768a <vector232>:
.globl vector232
vector232:
  pushl $0
8010768a:	6a 00                	push   $0x0
  pushl $232
8010768c:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
80107691:	e9 12 f0 ff ff       	jmp    801066a8 <alltraps>

80107696 <vector233>:
.globl vector233
vector233:
  pushl $0
80107696:	6a 00                	push   $0x0
  pushl $233
80107698:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
8010769d:	e9 06 f0 ff ff       	jmp    801066a8 <alltraps>

801076a2 <vector234>:
.globl vector234
vector234:
  pushl $0
801076a2:	6a 00                	push   $0x0
  pushl $234
801076a4:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
801076a9:	e9 fa ef ff ff       	jmp    801066a8 <alltraps>

801076ae <vector235>:
.globl vector235
vector235:
  pushl $0
801076ae:	6a 00                	push   $0x0
  pushl $235
801076b0:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
801076b5:	e9 ee ef ff ff       	jmp    801066a8 <alltraps>

801076ba <vector236>:
.globl vector236
vector236:
  pushl $0
801076ba:	6a 00                	push   $0x0
  pushl $236
801076bc:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
801076c1:	e9 e2 ef ff ff       	jmp    801066a8 <alltraps>

801076c6 <vector237>:
.globl vector237
vector237:
  pushl $0
801076c6:	6a 00                	push   $0x0
  pushl $237
801076c8:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
801076cd:	e9 d6 ef ff ff       	jmp    801066a8 <alltraps>

801076d2 <vector238>:
.globl vector238
vector238:
  pushl $0
801076d2:	6a 00                	push   $0x0
  pushl $238
801076d4:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
801076d9:	e9 ca ef ff ff       	jmp    801066a8 <alltraps>

801076de <vector239>:
.globl vector239
vector239:
  pushl $0
801076de:	6a 00                	push   $0x0
  pushl $239
801076e0:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
801076e5:	e9 be ef ff ff       	jmp    801066a8 <alltraps>

801076ea <vector240>:
.globl vector240
vector240:
  pushl $0
801076ea:	6a 00                	push   $0x0
  pushl $240
801076ec:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
801076f1:	e9 b2 ef ff ff       	jmp    801066a8 <alltraps>

801076f6 <vector241>:
.globl vector241
vector241:
  pushl $0
801076f6:	6a 00                	push   $0x0
  pushl $241
801076f8:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
801076fd:	e9 a6 ef ff ff       	jmp    801066a8 <alltraps>

80107702 <vector242>:
.globl vector242
vector242:
  pushl $0
80107702:	6a 00                	push   $0x0
  pushl $242
80107704:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
80107709:	e9 9a ef ff ff       	jmp    801066a8 <alltraps>

8010770e <vector243>:
.globl vector243
vector243:
  pushl $0
8010770e:	6a 00                	push   $0x0
  pushl $243
80107710:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80107715:	e9 8e ef ff ff       	jmp    801066a8 <alltraps>

8010771a <vector244>:
.globl vector244
vector244:
  pushl $0
8010771a:	6a 00                	push   $0x0
  pushl $244
8010771c:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
80107721:	e9 82 ef ff ff       	jmp    801066a8 <alltraps>

80107726 <vector245>:
.globl vector245
vector245:
  pushl $0
80107726:	6a 00                	push   $0x0
  pushl $245
80107728:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
8010772d:	e9 76 ef ff ff       	jmp    801066a8 <alltraps>

80107732 <vector246>:
.globl vector246
vector246:
  pushl $0
80107732:	6a 00                	push   $0x0
  pushl $246
80107734:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107739:	e9 6a ef ff ff       	jmp    801066a8 <alltraps>

8010773e <vector247>:
.globl vector247
vector247:
  pushl $0
8010773e:	6a 00                	push   $0x0
  pushl $247
80107740:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107745:	e9 5e ef ff ff       	jmp    801066a8 <alltraps>

8010774a <vector248>:
.globl vector248
vector248:
  pushl $0
8010774a:	6a 00                	push   $0x0
  pushl $248
8010774c:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
80107751:	e9 52 ef ff ff       	jmp    801066a8 <alltraps>

80107756 <vector249>:
.globl vector249
vector249:
  pushl $0
80107756:	6a 00                	push   $0x0
  pushl $249
80107758:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
8010775d:	e9 46 ef ff ff       	jmp    801066a8 <alltraps>

80107762 <vector250>:
.globl vector250
vector250:
  pushl $0
80107762:	6a 00                	push   $0x0
  pushl $250
80107764:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80107769:	e9 3a ef ff ff       	jmp    801066a8 <alltraps>

8010776e <vector251>:
.globl vector251
vector251:
  pushl $0
8010776e:	6a 00                	push   $0x0
  pushl $251
80107770:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80107775:	e9 2e ef ff ff       	jmp    801066a8 <alltraps>

8010777a <vector252>:
.globl vector252
vector252:
  pushl $0
8010777a:	6a 00                	push   $0x0
  pushl $252
8010777c:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80107781:	e9 22 ef ff ff       	jmp    801066a8 <alltraps>

80107786 <vector253>:
.globl vector253
vector253:
  pushl $0
80107786:	6a 00                	push   $0x0
  pushl $253
80107788:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
8010778d:	e9 16 ef ff ff       	jmp    801066a8 <alltraps>

80107792 <vector254>:
.globl vector254
vector254:
  pushl $0
80107792:	6a 00                	push   $0x0
  pushl $254
80107794:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80107799:	e9 0a ef ff ff       	jmp    801066a8 <alltraps>

8010779e <vector255>:
.globl vector255
vector255:
  pushl $0
8010779e:	6a 00                	push   $0x0
  pushl $255
801077a0:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
801077a5:	e9 fe ee ff ff       	jmp    801066a8 <alltraps>
	...

801077ac <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
801077ac:	55                   	push   %ebp
801077ad:	89 e5                	mov    %esp,%ebp
801077af:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
801077b2:	8b 45 0c             	mov    0xc(%ebp),%eax
801077b5:	83 e8 01             	sub    $0x1,%eax
801077b8:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801077bc:	8b 45 08             	mov    0x8(%ebp),%eax
801077bf:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801077c3:	8b 45 08             	mov    0x8(%ebp),%eax
801077c6:	c1 e8 10             	shr    $0x10,%eax
801077c9:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
801077cd:	8d 45 fa             	lea    -0x6(%ebp),%eax
801077d0:	0f 01 10             	lgdtl  (%eax)
}
801077d3:	c9                   	leave  
801077d4:	c3                   	ret    

801077d5 <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
801077d5:	55                   	push   %ebp
801077d6:	89 e5                	mov    %esp,%ebp
801077d8:	83 ec 04             	sub    $0x4,%esp
801077db:	8b 45 08             	mov    0x8(%ebp),%eax
801077de:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
801077e2:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
801077e6:	0f 00 d8             	ltr    %ax
}
801077e9:	c9                   	leave  
801077ea:	c3                   	ret    

801077eb <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
801077eb:	55                   	push   %ebp
801077ec:	89 e5                	mov    %esp,%ebp
801077ee:	83 ec 04             	sub    $0x4,%esp
801077f1:	8b 45 08             	mov    0x8(%ebp),%eax
801077f4:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
801077f8:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
801077fc:	8e e8                	mov    %eax,%gs
}
801077fe:	c9                   	leave  
801077ff:	c3                   	ret    

80107800 <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
80107800:	55                   	push   %ebp
80107801:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107803:	8b 45 08             	mov    0x8(%ebp),%eax
80107806:	0f 22 d8             	mov    %eax,%cr3
}
80107809:	5d                   	pop    %ebp
8010780a:	c3                   	ret    

8010780b <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
8010780b:	55                   	push   %ebp
8010780c:	89 e5                	mov    %esp,%ebp
8010780e:	8b 45 08             	mov    0x8(%ebp),%eax
80107811:	05 00 00 00 80       	add    $0x80000000,%eax
80107816:	5d                   	pop    %ebp
80107817:	c3                   	ret    

80107818 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80107818:	55                   	push   %ebp
80107819:	89 e5                	mov    %esp,%ebp
8010781b:	8b 45 08             	mov    0x8(%ebp),%eax
8010781e:	05 00 00 00 80       	add    $0x80000000,%eax
80107823:	5d                   	pop    %ebp
80107824:	c3                   	ret    

80107825 <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
80107825:	55                   	push   %ebp
80107826:	89 e5                	mov    %esp,%ebp
80107828:	53                   	push   %ebx
80107829:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
8010782c:	e8 5c b6 ff ff       	call   80102e8d <cpunum>
80107831:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80107837:	05 20 f9 10 80       	add    $0x8010f920,%eax
8010783c:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
8010783f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107842:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
80107848:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010784b:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
80107851:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107854:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
80107858:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010785b:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010785f:	83 e2 f0             	and    $0xfffffff0,%edx
80107862:	83 ca 0a             	or     $0xa,%edx
80107865:	88 50 7d             	mov    %dl,0x7d(%eax)
80107868:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010786b:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010786f:	83 ca 10             	or     $0x10,%edx
80107872:	88 50 7d             	mov    %dl,0x7d(%eax)
80107875:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107878:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010787c:	83 e2 9f             	and    $0xffffff9f,%edx
8010787f:	88 50 7d             	mov    %dl,0x7d(%eax)
80107882:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107885:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107889:	83 ca 80             	or     $0xffffff80,%edx
8010788c:	88 50 7d             	mov    %dl,0x7d(%eax)
8010788f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107892:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107896:	83 ca 0f             	or     $0xf,%edx
80107899:	88 50 7e             	mov    %dl,0x7e(%eax)
8010789c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010789f:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801078a3:	83 e2 ef             	and    $0xffffffef,%edx
801078a6:	88 50 7e             	mov    %dl,0x7e(%eax)
801078a9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078ac:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801078b0:	83 e2 df             	and    $0xffffffdf,%edx
801078b3:	88 50 7e             	mov    %dl,0x7e(%eax)
801078b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078b9:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801078bd:	83 ca 40             	or     $0x40,%edx
801078c0:	88 50 7e             	mov    %dl,0x7e(%eax)
801078c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078c6:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801078ca:	83 ca 80             	or     $0xffffff80,%edx
801078cd:	88 50 7e             	mov    %dl,0x7e(%eax)
801078d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078d3:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
801078d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078da:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
801078e1:	ff ff 
801078e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078e6:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
801078ed:	00 00 
801078ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078f2:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
801078f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078fc:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107903:	83 e2 f0             	and    $0xfffffff0,%edx
80107906:	83 ca 02             	or     $0x2,%edx
80107909:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
8010790f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107912:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107919:	83 ca 10             	or     $0x10,%edx
8010791c:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107922:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107925:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010792c:	83 e2 9f             	and    $0xffffff9f,%edx
8010792f:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107935:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107938:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010793f:	83 ca 80             	or     $0xffffff80,%edx
80107942:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107948:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010794b:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107952:	83 ca 0f             	or     $0xf,%edx
80107955:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
8010795b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010795e:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107965:	83 e2 ef             	and    $0xffffffef,%edx
80107968:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
8010796e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107971:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107978:	83 e2 df             	and    $0xffffffdf,%edx
8010797b:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107981:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107984:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
8010798b:	83 ca 40             	or     $0x40,%edx
8010798e:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107994:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107997:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
8010799e:	83 ca 80             	or     $0xffffff80,%edx
801079a1:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801079a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079aa:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
801079b1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079b4:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
801079bb:	ff ff 
801079bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079c0:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
801079c7:	00 00 
801079c9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079cc:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
801079d3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079d6:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
801079dd:	83 e2 f0             	and    $0xfffffff0,%edx
801079e0:	83 ca 0a             	or     $0xa,%edx
801079e3:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
801079e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079ec:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
801079f3:	83 ca 10             	or     $0x10,%edx
801079f6:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
801079fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079ff:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a06:	83 ca 60             	or     $0x60,%edx
80107a09:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a12:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a19:	83 ca 80             	or     $0xffffff80,%edx
80107a1c:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a22:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a25:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107a2c:	83 ca 0f             	or     $0xf,%edx
80107a2f:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107a35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a38:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107a3f:	83 e2 ef             	and    $0xffffffef,%edx
80107a42:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107a48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a4b:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107a52:	83 e2 df             	and    $0xffffffdf,%edx
80107a55:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107a5b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a5e:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107a65:	83 ca 40             	or     $0x40,%edx
80107a68:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107a6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a71:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107a78:	83 ca 80             	or     $0xffffff80,%edx
80107a7b:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107a81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a84:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107a8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a8e:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80107a95:	ff ff 
80107a97:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a9a:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80107aa1:	00 00 
80107aa3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aa6:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80107aad:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ab0:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107ab7:	83 e2 f0             	and    $0xfffffff0,%edx
80107aba:	83 ca 02             	or     $0x2,%edx
80107abd:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107ac3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ac6:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107acd:	83 ca 10             	or     $0x10,%edx
80107ad0:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107ad6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ad9:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107ae0:	83 ca 60             	or     $0x60,%edx
80107ae3:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107ae9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aec:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107af3:	83 ca 80             	or     $0xffffff80,%edx
80107af6:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107afc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aff:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b06:	83 ca 0f             	or     $0xf,%edx
80107b09:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b12:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b19:	83 e2 ef             	and    $0xffffffef,%edx
80107b1c:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b22:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b25:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b2c:	83 e2 df             	and    $0xffffffdf,%edx
80107b2f:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b38:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b3f:	83 ca 40             	or     $0x40,%edx
80107b42:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b4b:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b52:	83 ca 80             	or     $0xffffff80,%edx
80107b55:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b5b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b5e:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107b65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b68:	05 b4 00 00 00       	add    $0xb4,%eax
80107b6d:	89 c3                	mov    %eax,%ebx
80107b6f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b72:	05 b4 00 00 00       	add    $0xb4,%eax
80107b77:	c1 e8 10             	shr    $0x10,%eax
80107b7a:	89 c1                	mov    %eax,%ecx
80107b7c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b7f:	05 b4 00 00 00       	add    $0xb4,%eax
80107b84:	c1 e8 18             	shr    $0x18,%eax
80107b87:	89 c2                	mov    %eax,%edx
80107b89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b8c:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80107b93:	00 00 
80107b95:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b98:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80107b9f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ba2:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80107ba8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bab:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107bb2:	83 e1 f0             	and    $0xfffffff0,%ecx
80107bb5:	83 c9 02             	or     $0x2,%ecx
80107bb8:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107bbe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bc1:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107bc8:	83 c9 10             	or     $0x10,%ecx
80107bcb:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107bd1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bd4:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107bdb:	83 e1 9f             	and    $0xffffff9f,%ecx
80107bde:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107be4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be7:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107bee:	83 c9 80             	or     $0xffffff80,%ecx
80107bf1:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107bf7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bfa:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c01:	83 e1 f0             	and    $0xfffffff0,%ecx
80107c04:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c0d:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c14:	83 e1 ef             	and    $0xffffffef,%ecx
80107c17:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c1d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c20:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c27:	83 e1 df             	and    $0xffffffdf,%ecx
80107c2a:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c33:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c3a:	83 c9 40             	or     $0x40,%ecx
80107c3d:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c46:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c4d:	83 c9 80             	or     $0xffffff80,%ecx
80107c50:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c56:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c59:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80107c5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c62:	83 c0 70             	add    $0x70,%eax
80107c65:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80107c6c:	00 
80107c6d:	89 04 24             	mov    %eax,(%esp)
80107c70:	e8 37 fb ff ff       	call   801077ac <lgdt>
  loadgs(SEG_KCPU << 3);
80107c75:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80107c7c:	e8 6a fb ff ff       	call   801077eb <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80107c81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c84:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80107c8a:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80107c91:	00 00 00 00 
}
80107c95:	83 c4 24             	add    $0x24,%esp
80107c98:	5b                   	pop    %ebx
80107c99:	5d                   	pop    %ebp
80107c9a:	c3                   	ret    

80107c9b <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80107c9b:	55                   	push   %ebp
80107c9c:	89 e5                	mov    %esp,%ebp
80107c9e:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107ca1:	8b 45 0c             	mov    0xc(%ebp),%eax
80107ca4:	c1 e8 16             	shr    $0x16,%eax
80107ca7:	c1 e0 02             	shl    $0x2,%eax
80107caa:	03 45 08             	add    0x8(%ebp),%eax
80107cad:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107cb0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107cb3:	8b 00                	mov    (%eax),%eax
80107cb5:	83 e0 01             	and    $0x1,%eax
80107cb8:	84 c0                	test   %al,%al
80107cba:	74 17                	je     80107cd3 <walkpgdir+0x38>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80107cbc:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107cbf:	8b 00                	mov    (%eax),%eax
80107cc1:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107cc6:	89 04 24             	mov    %eax,(%esp)
80107cc9:	e8 4a fb ff ff       	call   80107818 <p2v>
80107cce:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107cd1:	eb 4b                	jmp    80107d1e <walkpgdir+0x83>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107cd3:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80107cd7:	74 0e                	je     80107ce7 <walkpgdir+0x4c>
80107cd9:	e8 21 ae ff ff       	call   80102aff <kalloc>
80107cde:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107ce1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107ce5:	75 07                	jne    80107cee <walkpgdir+0x53>
      return 0;
80107ce7:	b8 00 00 00 00       	mov    $0x0,%eax
80107cec:	eb 41                	jmp    80107d2f <walkpgdir+0x94>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107cee:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107cf5:	00 
80107cf6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107cfd:	00 
80107cfe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d01:	89 04 24             	mov    %eax,(%esp)
80107d04:	e8 fd d3 ff ff       	call   80105106 <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80107d09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d0c:	89 04 24             	mov    %eax,(%esp)
80107d0f:	e8 f7 fa ff ff       	call   8010780b <v2p>
80107d14:	89 c2                	mov    %eax,%edx
80107d16:	83 ca 07             	or     $0x7,%edx
80107d19:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d1c:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80107d1e:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d21:	c1 e8 0c             	shr    $0xc,%eax
80107d24:	25 ff 03 00 00       	and    $0x3ff,%eax
80107d29:	c1 e0 02             	shl    $0x2,%eax
80107d2c:	03 45 f4             	add    -0xc(%ebp),%eax
}
80107d2f:	c9                   	leave  
80107d30:	c3                   	ret    

80107d31 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80107d31:	55                   	push   %ebp
80107d32:	89 e5                	mov    %esp,%ebp
80107d34:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80107d37:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d3a:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107d3f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80107d42:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d45:	03 45 10             	add    0x10(%ebp),%eax
80107d48:	83 e8 01             	sub    $0x1,%eax
80107d4b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107d50:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80107d53:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80107d5a:	00 
80107d5b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d5e:	89 44 24 04          	mov    %eax,0x4(%esp)
80107d62:	8b 45 08             	mov    0x8(%ebp),%eax
80107d65:	89 04 24             	mov    %eax,(%esp)
80107d68:	e8 2e ff ff ff       	call   80107c9b <walkpgdir>
80107d6d:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107d70:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107d74:	75 07                	jne    80107d7d <mappages+0x4c>
      return -1;
80107d76:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107d7b:	eb 46                	jmp    80107dc3 <mappages+0x92>
    if(*pte & PTE_P)
80107d7d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107d80:	8b 00                	mov    (%eax),%eax
80107d82:	83 e0 01             	and    $0x1,%eax
80107d85:	84 c0                	test   %al,%al
80107d87:	74 0c                	je     80107d95 <mappages+0x64>
      panic("remap");
80107d89:	c7 04 24 ec 8b 10 80 	movl   $0x80108bec,(%esp)
80107d90:	e8 a8 87 ff ff       	call   8010053d <panic>
    *pte = pa | perm | PTE_P;
80107d95:	8b 45 18             	mov    0x18(%ebp),%eax
80107d98:	0b 45 14             	or     0x14(%ebp),%eax
80107d9b:	89 c2                	mov    %eax,%edx
80107d9d:	83 ca 01             	or     $0x1,%edx
80107da0:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107da3:	89 10                	mov    %edx,(%eax)
    if(a == last)
80107da5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107da8:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107dab:	74 10                	je     80107dbd <mappages+0x8c>
      break;
    a += PGSIZE;
80107dad:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80107db4:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
80107dbb:	eb 96                	jmp    80107d53 <mappages+0x22>
      return -1;
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
80107dbd:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80107dbe:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107dc3:	c9                   	leave  
80107dc4:	c3                   	ret    

80107dc5 <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm()
{
80107dc5:	55                   	push   %ebp
80107dc6:	89 e5                	mov    %esp,%ebp
80107dc8:	53                   	push   %ebx
80107dc9:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
80107dcc:	e8 2e ad ff ff       	call   80102aff <kalloc>
80107dd1:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107dd4:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107dd8:	75 0a                	jne    80107de4 <setupkvm+0x1f>
    return 0;
80107dda:	b8 00 00 00 00       	mov    $0x0,%eax
80107ddf:	e9 98 00 00 00       	jmp    80107e7c <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
80107de4:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107deb:	00 
80107dec:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107df3:	00 
80107df4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107df7:	89 04 24             	mov    %eax,(%esp)
80107dfa:	e8 07 d3 ff ff       	call   80105106 <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
80107dff:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80107e06:	e8 0d fa ff ff       	call   80107818 <p2v>
80107e0b:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
80107e10:	76 0c                	jbe    80107e1e <setupkvm+0x59>
    panic("PHYSTOP too high");
80107e12:	c7 04 24 f2 8b 10 80 	movl   $0x80108bf2,(%esp)
80107e19:	e8 1f 87 ff ff       	call   8010053d <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107e1e:	c7 45 f4 a0 b4 10 80 	movl   $0x8010b4a0,-0xc(%ebp)
80107e25:	eb 49                	jmp    80107e70 <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
80107e27:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107e2a:	8b 48 0c             	mov    0xc(%eax),%ecx
                (uint)k->phys_start, k->perm) < 0)
80107e2d:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107e30:	8b 50 04             	mov    0x4(%eax),%edx
80107e33:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e36:	8b 58 08             	mov    0x8(%eax),%ebx
80107e39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e3c:	8b 40 04             	mov    0x4(%eax),%eax
80107e3f:	29 c3                	sub    %eax,%ebx
80107e41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e44:	8b 00                	mov    (%eax),%eax
80107e46:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80107e4a:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107e4e:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80107e52:	89 44 24 04          	mov    %eax,0x4(%esp)
80107e56:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e59:	89 04 24             	mov    %eax,(%esp)
80107e5c:	e8 d0 fe ff ff       	call   80107d31 <mappages>
80107e61:	85 c0                	test   %eax,%eax
80107e63:	79 07                	jns    80107e6c <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80107e65:	b8 00 00 00 00       	mov    $0x0,%eax
80107e6a:	eb 10                	jmp    80107e7c <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107e6c:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80107e70:	81 7d f4 e0 b4 10 80 	cmpl   $0x8010b4e0,-0xc(%ebp)
80107e77:	72 ae                	jb     80107e27 <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
80107e79:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80107e7c:	83 c4 34             	add    $0x34,%esp
80107e7f:	5b                   	pop    %ebx
80107e80:	5d                   	pop    %ebp
80107e81:	c3                   	ret    

80107e82 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80107e82:	55                   	push   %ebp
80107e83:	89 e5                	mov    %esp,%ebp
80107e85:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80107e88:	e8 38 ff ff ff       	call   80107dc5 <setupkvm>
80107e8d:	a3 f8 28 11 80       	mov    %eax,0x801128f8
  switchkvm();
80107e92:	e8 02 00 00 00       	call   80107e99 <switchkvm>
}
80107e97:	c9                   	leave  
80107e98:	c3                   	ret    

80107e99 <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
80107e99:	55                   	push   %ebp
80107e9a:	89 e5                	mov    %esp,%ebp
80107e9c:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
80107e9f:	a1 f8 28 11 80       	mov    0x801128f8,%eax
80107ea4:	89 04 24             	mov    %eax,(%esp)
80107ea7:	e8 5f f9 ff ff       	call   8010780b <v2p>
80107eac:	89 04 24             	mov    %eax,(%esp)
80107eaf:	e8 4c f9 ff ff       	call   80107800 <lcr3>
}
80107eb4:	c9                   	leave  
80107eb5:	c3                   	ret    

80107eb6 <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
80107eb6:	55                   	push   %ebp
80107eb7:	89 e5                	mov    %esp,%ebp
80107eb9:	53                   	push   %ebx
80107eba:	83 ec 14             	sub    $0x14,%esp
  pushcli();
80107ebd:	e8 3d d1 ff ff       	call   80104fff <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
80107ec2:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107ec8:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107ecf:	83 c2 08             	add    $0x8,%edx
80107ed2:	89 d3                	mov    %edx,%ebx
80107ed4:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107edb:	83 c2 08             	add    $0x8,%edx
80107ede:	c1 ea 10             	shr    $0x10,%edx
80107ee1:	89 d1                	mov    %edx,%ecx
80107ee3:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107eea:	83 c2 08             	add    $0x8,%edx
80107eed:	c1 ea 18             	shr    $0x18,%edx
80107ef0:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
80107ef7:	67 00 
80107ef9:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
80107f00:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
80107f06:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f0d:	83 e1 f0             	and    $0xfffffff0,%ecx
80107f10:	83 c9 09             	or     $0x9,%ecx
80107f13:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107f19:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f20:	83 c9 10             	or     $0x10,%ecx
80107f23:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107f29:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f30:	83 e1 9f             	and    $0xffffff9f,%ecx
80107f33:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107f39:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f40:	83 c9 80             	or     $0xffffff80,%ecx
80107f43:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107f49:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107f50:	83 e1 f0             	and    $0xfffffff0,%ecx
80107f53:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107f59:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107f60:	83 e1 ef             	and    $0xffffffef,%ecx
80107f63:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107f69:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107f70:	83 e1 df             	and    $0xffffffdf,%ecx
80107f73:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107f79:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107f80:	83 c9 40             	or     $0x40,%ecx
80107f83:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107f89:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107f90:	83 e1 7f             	and    $0x7f,%ecx
80107f93:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107f99:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
80107f9f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107fa5:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
80107fac:	83 e2 ef             	and    $0xffffffef,%edx
80107faf:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
80107fb5:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107fbb:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
80107fc1:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107fc7:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80107fce:	8b 52 08             	mov    0x8(%edx),%edx
80107fd1:	81 c2 00 10 00 00    	add    $0x1000,%edx
80107fd7:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
80107fda:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
80107fe1:	e8 ef f7 ff ff       	call   801077d5 <ltr>
  if(p->pgdir == 0)
80107fe6:	8b 45 08             	mov    0x8(%ebp),%eax
80107fe9:	8b 40 04             	mov    0x4(%eax),%eax
80107fec:	85 c0                	test   %eax,%eax
80107fee:	75 0c                	jne    80107ffc <switchuvm+0x146>
    panic("switchuvm: no pgdir");
80107ff0:	c7 04 24 03 8c 10 80 	movl   $0x80108c03,(%esp)
80107ff7:	e8 41 85 ff ff       	call   8010053d <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
80107ffc:	8b 45 08             	mov    0x8(%ebp),%eax
80107fff:	8b 40 04             	mov    0x4(%eax),%eax
80108002:	89 04 24             	mov    %eax,(%esp)
80108005:	e8 01 f8 ff ff       	call   8010780b <v2p>
8010800a:	89 04 24             	mov    %eax,(%esp)
8010800d:	e8 ee f7 ff ff       	call   80107800 <lcr3>
  popcli();
80108012:	e8 30 d0 ff ff       	call   80105047 <popcli>
}
80108017:	83 c4 14             	add    $0x14,%esp
8010801a:	5b                   	pop    %ebx
8010801b:	5d                   	pop    %ebp
8010801c:	c3                   	ret    

8010801d <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
8010801d:	55                   	push   %ebp
8010801e:	89 e5                	mov    %esp,%ebp
80108020:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
80108023:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
8010802a:	76 0c                	jbe    80108038 <inituvm+0x1b>
    panic("inituvm: more than a page");
8010802c:	c7 04 24 17 8c 10 80 	movl   $0x80108c17,(%esp)
80108033:	e8 05 85 ff ff       	call   8010053d <panic>
  mem = kalloc();
80108038:	e8 c2 aa ff ff       	call   80102aff <kalloc>
8010803d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
80108040:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108047:	00 
80108048:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010804f:	00 
80108050:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108053:	89 04 24             	mov    %eax,(%esp)
80108056:	e8 ab d0 ff ff       	call   80105106 <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
8010805b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010805e:	89 04 24             	mov    %eax,(%esp)
80108061:	e8 a5 f7 ff ff       	call   8010780b <v2p>
80108066:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010806d:	00 
8010806e:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108072:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108079:	00 
8010807a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108081:	00 
80108082:	8b 45 08             	mov    0x8(%ebp),%eax
80108085:	89 04 24             	mov    %eax,(%esp)
80108088:	e8 a4 fc ff ff       	call   80107d31 <mappages>
  memmove(mem, init, sz);
8010808d:	8b 45 10             	mov    0x10(%ebp),%eax
80108090:	89 44 24 08          	mov    %eax,0x8(%esp)
80108094:	8b 45 0c             	mov    0xc(%ebp),%eax
80108097:	89 44 24 04          	mov    %eax,0x4(%esp)
8010809b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010809e:	89 04 24             	mov    %eax,(%esp)
801080a1:	e8 33 d1 ff ff       	call   801051d9 <memmove>
}
801080a6:	c9                   	leave  
801080a7:	c3                   	ret    

801080a8 <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
801080a8:	55                   	push   %ebp
801080a9:	89 e5                	mov    %esp,%ebp
801080ab:	53                   	push   %ebx
801080ac:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
801080af:	8b 45 0c             	mov    0xc(%ebp),%eax
801080b2:	25 ff 0f 00 00       	and    $0xfff,%eax
801080b7:	85 c0                	test   %eax,%eax
801080b9:	74 0c                	je     801080c7 <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
801080bb:	c7 04 24 34 8c 10 80 	movl   $0x80108c34,(%esp)
801080c2:	e8 76 84 ff ff       	call   8010053d <panic>
  for(i = 0; i < sz; i += PGSIZE){
801080c7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801080ce:	e9 ad 00 00 00       	jmp    80108180 <loaduvm+0xd8>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
801080d3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801080d6:	8b 55 0c             	mov    0xc(%ebp),%edx
801080d9:	01 d0                	add    %edx,%eax
801080db:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801080e2:	00 
801080e3:	89 44 24 04          	mov    %eax,0x4(%esp)
801080e7:	8b 45 08             	mov    0x8(%ebp),%eax
801080ea:	89 04 24             	mov    %eax,(%esp)
801080ed:	e8 a9 fb ff ff       	call   80107c9b <walkpgdir>
801080f2:	89 45 ec             	mov    %eax,-0x14(%ebp)
801080f5:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801080f9:	75 0c                	jne    80108107 <loaduvm+0x5f>
      panic("loaduvm: address should exist");
801080fb:	c7 04 24 57 8c 10 80 	movl   $0x80108c57,(%esp)
80108102:	e8 36 84 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
80108107:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010810a:	8b 00                	mov    (%eax),%eax
8010810c:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108111:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
80108114:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108117:	8b 55 18             	mov    0x18(%ebp),%edx
8010811a:	89 d1                	mov    %edx,%ecx
8010811c:	29 c1                	sub    %eax,%ecx
8010811e:	89 c8                	mov    %ecx,%eax
80108120:	3d ff 0f 00 00       	cmp    $0xfff,%eax
80108125:	77 11                	ja     80108138 <loaduvm+0x90>
      n = sz - i;
80108127:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010812a:	8b 55 18             	mov    0x18(%ebp),%edx
8010812d:	89 d1                	mov    %edx,%ecx
8010812f:	29 c1                	sub    %eax,%ecx
80108131:	89 c8                	mov    %ecx,%eax
80108133:	89 45 f0             	mov    %eax,-0x10(%ebp)
80108136:	eb 07                	jmp    8010813f <loaduvm+0x97>
    else
      n = PGSIZE;
80108138:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
8010813f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108142:	8b 55 14             	mov    0x14(%ebp),%edx
80108145:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80108148:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010814b:	89 04 24             	mov    %eax,(%esp)
8010814e:	e8 c5 f6 ff ff       	call   80107818 <p2v>
80108153:	8b 55 f0             	mov    -0x10(%ebp),%edx
80108156:	89 54 24 0c          	mov    %edx,0xc(%esp)
8010815a:	89 5c 24 08          	mov    %ebx,0x8(%esp)
8010815e:	89 44 24 04          	mov    %eax,0x4(%esp)
80108162:	8b 45 10             	mov    0x10(%ebp),%eax
80108165:	89 04 24             	mov    %eax,(%esp)
80108168:	e8 f1 9b ff ff       	call   80101d5e <readi>
8010816d:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80108170:	74 07                	je     80108179 <loaduvm+0xd1>
      return -1;
80108172:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80108177:	eb 18                	jmp    80108191 <loaduvm+0xe9>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
80108179:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108180:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108183:	3b 45 18             	cmp    0x18(%ebp),%eax
80108186:	0f 82 47 ff ff ff    	jb     801080d3 <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
8010818c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108191:	83 c4 24             	add    $0x24,%esp
80108194:	5b                   	pop    %ebx
80108195:	5d                   	pop    %ebp
80108196:	c3                   	ret    

80108197 <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80108197:	55                   	push   %ebp
80108198:	89 e5                	mov    %esp,%ebp
8010819a:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
8010819d:	8b 45 10             	mov    0x10(%ebp),%eax
801081a0:	85 c0                	test   %eax,%eax
801081a2:	79 0a                	jns    801081ae <allocuvm+0x17>
    return 0;
801081a4:	b8 00 00 00 00       	mov    $0x0,%eax
801081a9:	e9 c1 00 00 00       	jmp    8010826f <allocuvm+0xd8>
  if(newsz < oldsz)
801081ae:	8b 45 10             	mov    0x10(%ebp),%eax
801081b1:	3b 45 0c             	cmp    0xc(%ebp),%eax
801081b4:	73 08                	jae    801081be <allocuvm+0x27>
    return oldsz;
801081b6:	8b 45 0c             	mov    0xc(%ebp),%eax
801081b9:	e9 b1 00 00 00       	jmp    8010826f <allocuvm+0xd8>
  a = PGROUNDUP(oldsz);
801081be:	8b 45 0c             	mov    0xc(%ebp),%eax
801081c1:	05 ff 0f 00 00       	add    $0xfff,%eax
801081c6:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801081cb:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
801081ce:	e9 8d 00 00 00       	jmp    80108260 <allocuvm+0xc9>
    mem = kalloc();
801081d3:	e8 27 a9 ff ff       	call   80102aff <kalloc>
801081d8:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
801081db:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801081df:	75 2c                	jne    8010820d <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
801081e1:	c7 04 24 75 8c 10 80 	movl   $0x80108c75,(%esp)
801081e8:	e8 b4 81 ff ff       	call   801003a1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
801081ed:	8b 45 0c             	mov    0xc(%ebp),%eax
801081f0:	89 44 24 08          	mov    %eax,0x8(%esp)
801081f4:	8b 45 10             	mov    0x10(%ebp),%eax
801081f7:	89 44 24 04          	mov    %eax,0x4(%esp)
801081fb:	8b 45 08             	mov    0x8(%ebp),%eax
801081fe:	89 04 24             	mov    %eax,(%esp)
80108201:	e8 6b 00 00 00       	call   80108271 <deallocuvm>
      return 0;
80108206:	b8 00 00 00 00       	mov    $0x0,%eax
8010820b:	eb 62                	jmp    8010826f <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
8010820d:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108214:	00 
80108215:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010821c:	00 
8010821d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108220:	89 04 24             	mov    %eax,(%esp)
80108223:	e8 de ce ff ff       	call   80105106 <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
80108228:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010822b:	89 04 24             	mov    %eax,(%esp)
8010822e:	e8 d8 f5 ff ff       	call   8010780b <v2p>
80108233:	8b 55 f4             	mov    -0xc(%ebp),%edx
80108236:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010823d:	00 
8010823e:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108242:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108249:	00 
8010824a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010824e:	8b 45 08             	mov    0x8(%ebp),%eax
80108251:	89 04 24             	mov    %eax,(%esp)
80108254:	e8 d8 fa ff ff       	call   80107d31 <mappages>
  if(newsz >= KERNBASE)
    return 0;
  if(newsz < oldsz)
    return oldsz;
  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
80108259:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108260:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108263:	3b 45 10             	cmp    0x10(%ebp),%eax
80108266:	0f 82 67 ff ff ff    	jb     801081d3 <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
8010826c:	8b 45 10             	mov    0x10(%ebp),%eax
}
8010826f:	c9                   	leave  
80108270:	c3                   	ret    

80108271 <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80108271:	55                   	push   %ebp
80108272:	89 e5                	mov    %esp,%ebp
80108274:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
80108277:	8b 45 10             	mov    0x10(%ebp),%eax
8010827a:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010827d:	72 08                	jb     80108287 <deallocuvm+0x16>
    return oldsz;
8010827f:	8b 45 0c             	mov    0xc(%ebp),%eax
80108282:	e9 a4 00 00 00       	jmp    8010832b <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
80108287:	8b 45 10             	mov    0x10(%ebp),%eax
8010828a:	05 ff 0f 00 00       	add    $0xfff,%eax
8010828f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108294:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80108297:	e9 80 00 00 00       	jmp    8010831c <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
8010829c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010829f:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801082a6:	00 
801082a7:	89 44 24 04          	mov    %eax,0x4(%esp)
801082ab:	8b 45 08             	mov    0x8(%ebp),%eax
801082ae:	89 04 24             	mov    %eax,(%esp)
801082b1:	e8 e5 f9 ff ff       	call   80107c9b <walkpgdir>
801082b6:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
801082b9:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801082bd:	75 09                	jne    801082c8 <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
801082bf:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
801082c6:	eb 4d                	jmp    80108315 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
801082c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801082cb:	8b 00                	mov    (%eax),%eax
801082cd:	83 e0 01             	and    $0x1,%eax
801082d0:	84 c0                	test   %al,%al
801082d2:	74 41                	je     80108315 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
801082d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801082d7:	8b 00                	mov    (%eax),%eax
801082d9:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801082de:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
801082e1:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801082e5:	75 0c                	jne    801082f3 <deallocuvm+0x82>
        panic("kfree");
801082e7:	c7 04 24 8d 8c 10 80 	movl   $0x80108c8d,(%esp)
801082ee:	e8 4a 82 ff ff       	call   8010053d <panic>
      char *v = p2v(pa);
801082f3:	8b 45 ec             	mov    -0x14(%ebp),%eax
801082f6:	89 04 24             	mov    %eax,(%esp)
801082f9:	e8 1a f5 ff ff       	call   80107818 <p2v>
801082fe:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
80108301:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108304:	89 04 24             	mov    %eax,(%esp)
80108307:	e8 5a a7 ff ff       	call   80102a66 <kfree>
      *pte = 0;
8010830c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010830f:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80108315:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010831c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010831f:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108322:	0f 82 74 ff ff ff    	jb     8010829c <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
80108328:	8b 45 10             	mov    0x10(%ebp),%eax
}
8010832b:	c9                   	leave  
8010832c:	c3                   	ret    

8010832d <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
8010832d:	55                   	push   %ebp
8010832e:	89 e5                	mov    %esp,%ebp
80108330:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80108333:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80108337:	75 0c                	jne    80108345 <freevm+0x18>
    panic("freevm: no pgdir");
80108339:	c7 04 24 93 8c 10 80 	movl   $0x80108c93,(%esp)
80108340:	e8 f8 81 ff ff       	call   8010053d <panic>
  deallocuvm(pgdir, KERNBASE, 0);
80108345:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010834c:	00 
8010834d:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
80108354:	80 
80108355:	8b 45 08             	mov    0x8(%ebp),%eax
80108358:	89 04 24             	mov    %eax,(%esp)
8010835b:	e8 11 ff ff ff       	call   80108271 <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
80108360:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108367:	eb 3c                	jmp    801083a5 <freevm+0x78>
    if(pgdir[i] & PTE_P){
80108369:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010836c:	c1 e0 02             	shl    $0x2,%eax
8010836f:	03 45 08             	add    0x8(%ebp),%eax
80108372:	8b 00                	mov    (%eax),%eax
80108374:	83 e0 01             	and    $0x1,%eax
80108377:	84 c0                	test   %al,%al
80108379:	74 26                	je     801083a1 <freevm+0x74>
      char * v = p2v(PTE_ADDR(pgdir[i]));
8010837b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010837e:	c1 e0 02             	shl    $0x2,%eax
80108381:	03 45 08             	add    0x8(%ebp),%eax
80108384:	8b 00                	mov    (%eax),%eax
80108386:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010838b:	89 04 24             	mov    %eax,(%esp)
8010838e:	e8 85 f4 ff ff       	call   80107818 <p2v>
80108393:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
80108396:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108399:	89 04 24             	mov    %eax,(%esp)
8010839c:	e8 c5 a6 ff ff       	call   80102a66 <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
801083a1:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801083a5:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
801083ac:	76 bb                	jbe    80108369 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
801083ae:	8b 45 08             	mov    0x8(%ebp),%eax
801083b1:	89 04 24             	mov    %eax,(%esp)
801083b4:	e8 ad a6 ff ff       	call   80102a66 <kfree>
}
801083b9:	c9                   	leave  
801083ba:	c3                   	ret    

801083bb <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
801083bb:	55                   	push   %ebp
801083bc:	89 e5                	mov    %esp,%ebp
801083be:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801083c1:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801083c8:	00 
801083c9:	8b 45 0c             	mov    0xc(%ebp),%eax
801083cc:	89 44 24 04          	mov    %eax,0x4(%esp)
801083d0:	8b 45 08             	mov    0x8(%ebp),%eax
801083d3:	89 04 24             	mov    %eax,(%esp)
801083d6:	e8 c0 f8 ff ff       	call   80107c9b <walkpgdir>
801083db:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
801083de:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801083e2:	75 0c                	jne    801083f0 <clearpteu+0x35>
    panic("clearpteu");
801083e4:	c7 04 24 a4 8c 10 80 	movl   $0x80108ca4,(%esp)
801083eb:	e8 4d 81 ff ff       	call   8010053d <panic>
  *pte &= ~PTE_U;
801083f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083f3:	8b 00                	mov    (%eax),%eax
801083f5:	89 c2                	mov    %eax,%edx
801083f7:	83 e2 fb             	and    $0xfffffffb,%edx
801083fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083fd:	89 10                	mov    %edx,(%eax)
}
801083ff:	c9                   	leave  
80108400:	c3                   	ret    

80108401 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80108401:	55                   	push   %ebp
80108402:	89 e5                	mov    %esp,%ebp
80108404:	83 ec 48             	sub    $0x48,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
80108407:	e8 b9 f9 ff ff       	call   80107dc5 <setupkvm>
8010840c:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010840f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108413:	75 0a                	jne    8010841f <copyuvm+0x1e>
    return 0;
80108415:	b8 00 00 00 00       	mov    $0x0,%eax
8010841a:	e9 f1 00 00 00       	jmp    80108510 <copyuvm+0x10f>
  for(i = 0; i < sz; i += PGSIZE){
8010841f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108426:	e9 c0 00 00 00       	jmp    801084eb <copyuvm+0xea>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
8010842b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010842e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108435:	00 
80108436:	89 44 24 04          	mov    %eax,0x4(%esp)
8010843a:	8b 45 08             	mov    0x8(%ebp),%eax
8010843d:	89 04 24             	mov    %eax,(%esp)
80108440:	e8 56 f8 ff ff       	call   80107c9b <walkpgdir>
80108445:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108448:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010844c:	75 0c                	jne    8010845a <copyuvm+0x59>
      panic("copyuvm: pte should exist");
8010844e:	c7 04 24 ae 8c 10 80 	movl   $0x80108cae,(%esp)
80108455:	e8 e3 80 ff ff       	call   8010053d <panic>
    if(!(*pte & PTE_P))
8010845a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010845d:	8b 00                	mov    (%eax),%eax
8010845f:	83 e0 01             	and    $0x1,%eax
80108462:	85 c0                	test   %eax,%eax
80108464:	75 0c                	jne    80108472 <copyuvm+0x71>
      panic("copyuvm: page not present");
80108466:	c7 04 24 c8 8c 10 80 	movl   $0x80108cc8,(%esp)
8010846d:	e8 cb 80 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
80108472:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108475:	8b 00                	mov    (%eax),%eax
80108477:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010847c:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if((mem = kalloc()) == 0)
8010847f:	e8 7b a6 ff ff       	call   80102aff <kalloc>
80108484:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80108487:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010848b:	74 6f                	je     801084fc <copyuvm+0xfb>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
8010848d:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108490:	89 04 24             	mov    %eax,(%esp)
80108493:	e8 80 f3 ff ff       	call   80107818 <p2v>
80108498:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010849f:	00 
801084a0:	89 44 24 04          	mov    %eax,0x4(%esp)
801084a4:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801084a7:	89 04 24             	mov    %eax,(%esp)
801084aa:	e8 2a cd ff ff       	call   801051d9 <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
801084af:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801084b2:	89 04 24             	mov    %eax,(%esp)
801084b5:	e8 51 f3 ff ff       	call   8010780b <v2p>
801084ba:	8b 55 f4             	mov    -0xc(%ebp),%edx
801084bd:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
801084c4:	00 
801084c5:	89 44 24 0c          	mov    %eax,0xc(%esp)
801084c9:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801084d0:	00 
801084d1:	89 54 24 04          	mov    %edx,0x4(%esp)
801084d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801084d8:	89 04 24             	mov    %eax,(%esp)
801084db:	e8 51 f8 ff ff       	call   80107d31 <mappages>
801084e0:	85 c0                	test   %eax,%eax
801084e2:	78 1b                	js     801084ff <copyuvm+0xfe>
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
801084e4:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801084eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801084ee:	3b 45 0c             	cmp    0xc(%ebp),%eax
801084f1:	0f 82 34 ff ff ff    	jb     8010842b <copyuvm+0x2a>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
  }
  return d;
801084f7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801084fa:	eb 14                	jmp    80108510 <copyuvm+0x10f>
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
801084fc:	90                   	nop
801084fd:	eb 01                	jmp    80108500 <copyuvm+0xff>
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
801084ff:	90                   	nop
  }
  return d;

bad:
  freevm(d);
80108500:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108503:	89 04 24             	mov    %eax,(%esp)
80108506:	e8 22 fe ff ff       	call   8010832d <freevm>
  return 0;
8010850b:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108510:	c9                   	leave  
80108511:	c3                   	ret    

80108512 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80108512:	55                   	push   %ebp
80108513:	89 e5                	mov    %esp,%ebp
80108515:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80108518:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010851f:	00 
80108520:	8b 45 0c             	mov    0xc(%ebp),%eax
80108523:	89 44 24 04          	mov    %eax,0x4(%esp)
80108527:	8b 45 08             	mov    0x8(%ebp),%eax
8010852a:	89 04 24             	mov    %eax,(%esp)
8010852d:	e8 69 f7 ff ff       	call   80107c9b <walkpgdir>
80108532:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
80108535:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108538:	8b 00                	mov    (%eax),%eax
8010853a:	83 e0 01             	and    $0x1,%eax
8010853d:	85 c0                	test   %eax,%eax
8010853f:	75 07                	jne    80108548 <uva2ka+0x36>
    return 0;
80108541:	b8 00 00 00 00       	mov    $0x0,%eax
80108546:	eb 25                	jmp    8010856d <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
80108548:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010854b:	8b 00                	mov    (%eax),%eax
8010854d:	83 e0 04             	and    $0x4,%eax
80108550:	85 c0                	test   %eax,%eax
80108552:	75 07                	jne    8010855b <uva2ka+0x49>
    return 0;
80108554:	b8 00 00 00 00       	mov    $0x0,%eax
80108559:	eb 12                	jmp    8010856d <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
8010855b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010855e:	8b 00                	mov    (%eax),%eax
80108560:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108565:	89 04 24             	mov    %eax,(%esp)
80108568:	e8 ab f2 ff ff       	call   80107818 <p2v>
}
8010856d:	c9                   	leave  
8010856e:	c3                   	ret    

8010856f <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
8010856f:	55                   	push   %ebp
80108570:	89 e5                	mov    %esp,%ebp
80108572:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80108575:	8b 45 10             	mov    0x10(%ebp),%eax
80108578:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
8010857b:	e9 8b 00 00 00       	jmp    8010860b <copyout+0x9c>
    va0 = (uint)PGROUNDDOWN(va);
80108580:	8b 45 0c             	mov    0xc(%ebp),%eax
80108583:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108588:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
8010858b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010858e:	89 44 24 04          	mov    %eax,0x4(%esp)
80108592:	8b 45 08             	mov    0x8(%ebp),%eax
80108595:	89 04 24             	mov    %eax,(%esp)
80108598:	e8 75 ff ff ff       	call   80108512 <uva2ka>
8010859d:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
801085a0:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801085a4:	75 07                	jne    801085ad <copyout+0x3e>
      return -1;
801085a6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801085ab:	eb 6d                	jmp    8010861a <copyout+0xab>
    n = PGSIZE - (va - va0);
801085ad:	8b 45 0c             	mov    0xc(%ebp),%eax
801085b0:	8b 55 ec             	mov    -0x14(%ebp),%edx
801085b3:	89 d1                	mov    %edx,%ecx
801085b5:	29 c1                	sub    %eax,%ecx
801085b7:	89 c8                	mov    %ecx,%eax
801085b9:	05 00 10 00 00       	add    $0x1000,%eax
801085be:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
801085c1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085c4:	3b 45 14             	cmp    0x14(%ebp),%eax
801085c7:	76 06                	jbe    801085cf <copyout+0x60>
      n = len;
801085c9:	8b 45 14             	mov    0x14(%ebp),%eax
801085cc:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
801085cf:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085d2:	8b 55 0c             	mov    0xc(%ebp),%edx
801085d5:	89 d1                	mov    %edx,%ecx
801085d7:	29 c1                	sub    %eax,%ecx
801085d9:	89 c8                	mov    %ecx,%eax
801085db:	03 45 e8             	add    -0x18(%ebp),%eax
801085de:	8b 55 f0             	mov    -0x10(%ebp),%edx
801085e1:	89 54 24 08          	mov    %edx,0x8(%esp)
801085e5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801085e8:	89 54 24 04          	mov    %edx,0x4(%esp)
801085ec:	89 04 24             	mov    %eax,(%esp)
801085ef:	e8 e5 cb ff ff       	call   801051d9 <memmove>
    len -= n;
801085f4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085f7:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
801085fa:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085fd:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
80108600:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108603:	05 00 10 00 00       	add    $0x1000,%eax
80108608:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
8010860b:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
8010860f:	0f 85 6b ff ff ff    	jne    80108580 <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
80108615:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010861a:	c9                   	leave  
8010861b:	c3                   	ret    
