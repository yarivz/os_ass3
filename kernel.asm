
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 b0 10 00       	mov    $0x10b000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc e0 d6 10 80       	mov    $0x8010d6e0,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 03 3c 10 80       	mov    $0x80103c03,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 b4 96 10 	movl   $0x801096b4,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
80100049:	e8 08 5b 00 00       	call   80105b56 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 10 ec 10 80 04 	movl   $0x8010ec04,0x8010ec10
80100055:	ec 10 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 14 ec 10 80 04 	movl   $0x8010ec04,0x8010ec14
8010005f:	ec 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 14 d7 10 80 	movl   $0x8010d714,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 14 ec 10 80    	mov    0x8010ec14,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 04 ec 10 80 	movl   $0x8010ec04,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 14 ec 10 80       	mov    0x8010ec14,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 14 ec 10 80       	mov    %eax,0x8010ec14

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 04 ec 10 80 	cmpl   $0x8010ec04,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate fresh block.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
801000bd:	e8 b5 5a 00 00       	call   80105b77 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 14 ec 10 80       	mov    0x8010ec14,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	89 c2                	mov    %eax,%edx
801000f5:	83 ca 01             	or     $0x1,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
80100104:	e8 09 5b 00 00       	call   80105c12 <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 e0 d6 10 	movl   $0x8010d6e0,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 e1 55 00 00       	call   80105705 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 04 ec 10 80 	cmpl   $0x8010ec04,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 10 ec 10 80       	mov    0x8010ec10,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
8010017c:	e8 91 5a 00 00       	call   80105c12 <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 04 ec 10 80 	cmpl   $0x8010ec04,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 bb 96 10 80 	movl   $0x801096bb,(%esp)
8010019f:	e8 99 03 00 00       	call   8010053d <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 d7 25 00 00       	call   801027af <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 cc 96 10 80 	movl   $0x801096cc,(%esp)
801001f6:	e8 42 03 00 00       	call   8010053d <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	89 c2                	mov    %eax,%edx
80100202:	83 ca 04             	or     $0x4,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 9a 25 00 00       	call   801027af <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 d3 96 10 80 	movl   $0x801096d3,(%esp)
80100230:	e8 08 03 00 00       	call   8010053d <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
8010023c:	e8 36 59 00 00       	call   80105b77 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 14 ec 10 80    	mov    0x8010ec14,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 04 ec 10 80 	movl   $0x8010ec04,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 14 ec 10 80       	mov    0x8010ec14,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 14 ec 10 80       	mov    %eax,0x8010ec14

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	89 c2                	mov    %eax,%edx
8010028f:	83 e2 fe             	and    $0xfffffffe,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 d5 55 00 00       	call   80105877 <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 e0 d6 10 80 	movl   $0x8010d6e0,(%esp)
801002a9:	e8 64 59 00 00       	call   80105c12 <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	53                   	push   %ebx
801002b4:	83 ec 14             	sub    $0x14,%esp
801002b7:	8b 45 08             	mov    0x8(%ebp),%eax
801002ba:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002be:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
801002c2:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
801002c6:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
801002ca:	ec                   	in     (%dx),%al
801002cb:	89 c3                	mov    %eax,%ebx
801002cd:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
801002d0:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
801002d4:	83 c4 14             	add    $0x14,%esp
801002d7:	5b                   	pop    %ebx
801002d8:	5d                   	pop    %ebp
801002d9:	c3                   	ret    

801002da <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002da:	55                   	push   %ebp
801002db:	89 e5                	mov    %esp,%ebp
801002dd:	83 ec 08             	sub    $0x8,%esp
801002e0:	8b 55 08             	mov    0x8(%ebp),%edx
801002e3:	8b 45 0c             	mov    0xc(%ebp),%eax
801002e6:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002ea:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002ed:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002f1:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002f5:	ee                   	out    %al,(%dx)
}
801002f6:	c9                   	leave  
801002f7:	c3                   	ret    

801002f8 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002f8:	55                   	push   %ebp
801002f9:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002fb:	fa                   	cli    
}
801002fc:	5d                   	pop    %ebp
801002fd:	c3                   	ret    

801002fe <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002fe:	55                   	push   %ebp
801002ff:	89 e5                	mov    %esp,%ebp
80100301:	83 ec 48             	sub    $0x48,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
80100304:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100308:	74 19                	je     80100323 <printint+0x25>
8010030a:	8b 45 08             	mov    0x8(%ebp),%eax
8010030d:	c1 e8 1f             	shr    $0x1f,%eax
80100310:	89 45 10             	mov    %eax,0x10(%ebp)
80100313:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100317:	74 0a                	je     80100323 <printint+0x25>
    x = -xx;
80100319:	8b 45 08             	mov    0x8(%ebp),%eax
8010031c:	f7 d8                	neg    %eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100321:	eb 06                	jmp    80100329 <printint+0x2b>
  else
    x = xx;
80100323:	8b 45 08             	mov    0x8(%ebp),%eax
80100326:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100329:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100330:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80100333:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100336:	ba 00 00 00 00       	mov    $0x0,%edx
8010033b:	f7 f1                	div    %ecx
8010033d:	89 d0                	mov    %edx,%eax
8010033f:	0f b6 90 04 a0 10 80 	movzbl -0x7fef5ffc(%eax),%edx
80100346:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100349:	03 45 f4             	add    -0xc(%ebp),%eax
8010034c:	88 10                	mov    %dl,(%eax)
8010034e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  }while((x /= base) != 0);
80100352:	8b 55 0c             	mov    0xc(%ebp),%edx
80100355:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80100358:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010035b:	ba 00 00 00 00       	mov    $0x0,%edx
80100360:	f7 75 d4             	divl   -0x2c(%ebp)
80100363:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100366:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010036a:	75 c4                	jne    80100330 <printint+0x32>

  if(sign)
8010036c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100370:	74 23                	je     80100395 <printint+0x97>
    buf[i++] = '-';
80100372:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100375:	03 45 f4             	add    -0xc(%ebp),%eax
80100378:	c6 00 2d             	movb   $0x2d,(%eax)
8010037b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)

  while(--i >= 0)
8010037f:	eb 14                	jmp    80100395 <printint+0x97>
    consputc(buf[i]);
80100381:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100384:	03 45 f4             	add    -0xc(%ebp),%eax
80100387:	0f b6 00             	movzbl (%eax),%eax
8010038a:	0f be c0             	movsbl %al,%eax
8010038d:	89 04 24             	mov    %eax,(%esp)
80100390:	e8 bb 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
80100395:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100399:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010039d:	79 e2                	jns    80100381 <printint+0x83>
    consputc(buf[i]);
}
8010039f:	c9                   	leave  
801003a0:	c3                   	ret    

801003a1 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a1:	55                   	push   %ebp
801003a2:	89 e5                	mov    %esp,%ebp
801003a4:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a7:	a1 14 c6 10 80       	mov    0x8010c614,%eax
801003ac:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003af:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b3:	74 0c                	je     801003c1 <cprintf+0x20>
    acquire(&cons.lock);
801003b5:	c7 04 24 e0 c5 10 80 	movl   $0x8010c5e0,(%esp)
801003bc:	e8 b6 57 00 00       	call   80105b77 <acquire>

  if (fmt == 0)
801003c1:	8b 45 08             	mov    0x8(%ebp),%eax
801003c4:	85 c0                	test   %eax,%eax
801003c6:	75 0c                	jne    801003d4 <cprintf+0x33>
    panic("null fmt");
801003c8:	c7 04 24 da 96 10 80 	movl   $0x801096da,(%esp)
801003cf:	e8 69 01 00 00       	call   8010053d <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d4:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003da:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e1:	e9 20 01 00 00       	jmp    80100506 <cprintf+0x165>
    if(c != '%'){
801003e6:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003ea:	74 10                	je     801003fc <cprintf+0x5b>
      consputc(c);
801003ec:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ef:	89 04 24             	mov    %eax,(%esp)
801003f2:	e8 59 03 00 00       	call   80100750 <consputc>
      continue;
801003f7:	e9 06 01 00 00       	jmp    80100502 <cprintf+0x161>
    }
    c = fmt[++i] & 0xff;
801003fc:	8b 55 08             	mov    0x8(%ebp),%edx
801003ff:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100403:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100406:	01 d0                	add    %edx,%eax
80100408:	0f b6 00             	movzbl (%eax),%eax
8010040b:	0f be c0             	movsbl %al,%eax
8010040e:	25 ff 00 00 00       	and    $0xff,%eax
80100413:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100416:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010041a:	0f 84 08 01 00 00    	je     80100528 <cprintf+0x187>
      break;
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4d                	je     80100475 <cprintf+0xd4>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0x9f>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13b>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xae>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x149>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 53                	je     80100498 <cprintf+0xf7>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2b                	je     80100475 <cprintf+0xd4>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x149>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8b 00                	mov    (%eax),%eax
80100454:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
80100458:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010045f:	00 
80100460:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100467:	00 
80100468:	89 04 24             	mov    %eax,(%esp)
8010046b:	e8 8e fe ff ff       	call   801002fe <printint>
      break;
80100470:	e9 8d 00 00 00       	jmp    80100502 <cprintf+0x161>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100475:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100478:	8b 00                	mov    (%eax),%eax
8010047a:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
8010047e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100485:	00 
80100486:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
8010048d:	00 
8010048e:	89 04 24             	mov    %eax,(%esp)
80100491:	e8 68 fe ff ff       	call   801002fe <printint>
      break;
80100496:	eb 6a                	jmp    80100502 <cprintf+0x161>
    case 's':
      if((s = (char*)*argp++) == 0)
80100498:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049b:	8b 00                	mov    (%eax),%eax
8010049d:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004a0:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004a4:	0f 94 c0             	sete   %al
801004a7:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
801004ab:	84 c0                	test   %al,%al
801004ad:	74 20                	je     801004cf <cprintf+0x12e>
        s = "(null)";
801004af:	c7 45 ec e3 96 10 80 	movl   $0x801096e3,-0x14(%ebp)
      for(; *s; s++)
801004b6:	eb 17                	jmp    801004cf <cprintf+0x12e>
        consputc(*s);
801004b8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004bb:	0f b6 00             	movzbl (%eax),%eax
801004be:	0f be c0             	movsbl %al,%eax
801004c1:	89 04 24             	mov    %eax,(%esp)
801004c4:	e8 87 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004c9:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004cd:	eb 01                	jmp    801004d0 <cprintf+0x12f>
801004cf:	90                   	nop
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 de                	jne    801004b8 <cprintf+0x117>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x161>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x161>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 c0 fe ff ff    	jne    801003e6 <cprintf+0x45>
80100526:	eb 01                	jmp    80100529 <cprintf+0x188>
      consputc(c);
      continue;
    }
    c = fmt[++i] & 0xff;
    if(c == 0)
      break;
80100528:	90                   	nop
      consputc(c);
      break;
    }
  }

  if(locking)
80100529:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052d:	74 0c                	je     8010053b <cprintf+0x19a>
    release(&cons.lock);
8010052f:	c7 04 24 e0 c5 10 80 	movl   $0x8010c5e0,(%esp)
80100536:	e8 d7 56 00 00       	call   80105c12 <release>
}
8010053b:	c9                   	leave  
8010053c:	c3                   	ret    

8010053d <panic>:

void
panic(char *s)
{
8010053d:	55                   	push   %ebp
8010053e:	89 e5                	mov    %esp,%ebp
80100540:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100543:	e8 b0 fd ff ff       	call   801002f8 <cli>
  cons.locking = 0;
80100548:	c7 05 14 c6 10 80 00 	movl   $0x0,0x8010c614
8010054f:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
80100552:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100558:	0f b6 00             	movzbl (%eax),%eax
8010055b:	0f b6 c0             	movzbl %al,%eax
8010055e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100562:	c7 04 24 ea 96 10 80 	movl   $0x801096ea,(%esp)
80100569:	e8 33 fe ff ff       	call   801003a1 <cprintf>
  cprintf(s);
8010056e:	8b 45 08             	mov    0x8(%ebp),%eax
80100571:	89 04 24             	mov    %eax,(%esp)
80100574:	e8 28 fe ff ff       	call   801003a1 <cprintf>
  cprintf("\n");
80100579:	c7 04 24 f9 96 10 80 	movl   $0x801096f9,(%esp)
80100580:	e8 1c fe ff ff       	call   801003a1 <cprintf>
  getcallerpcs(&s, pcs);
80100585:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100588:	89 44 24 04          	mov    %eax,0x4(%esp)
8010058c:	8d 45 08             	lea    0x8(%ebp),%eax
8010058f:	89 04 24             	mov    %eax,(%esp)
80100592:	e8 ca 56 00 00       	call   80105c61 <getcallerpcs>
  for(i=0; i<10; i++)
80100597:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059e:	eb 1b                	jmp    801005bb <panic+0x7e>
    cprintf(" %p", pcs[i]);
801005a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a3:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a7:	89 44 24 04          	mov    %eax,0x4(%esp)
801005ab:	c7 04 24 fb 96 10 80 	movl   $0x801096fb,(%esp)
801005b2:	e8 ea fd ff ff       	call   801003a1 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b7:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005bb:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bf:	7e df                	jle    801005a0 <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005c1:	c7 05 c0 c5 10 80 01 	movl   $0x1,0x8010c5c0
801005c8:	00 00 00 
  for(;;)
    ;
801005cb:	eb fe                	jmp    801005cb <panic+0x8e>

801005cd <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005cd:	55                   	push   %ebp
801005ce:	89 e5                	mov    %esp,%ebp
801005d0:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d3:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005da:	00 
801005db:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005e2:	e8 f3 fc ff ff       	call   801002da <outb>
  pos = inb(CRTPORT+1) << 8;
801005e7:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005ee:	e8 bd fc ff ff       	call   801002b0 <inb>
801005f3:	0f b6 c0             	movzbl %al,%eax
801005f6:	c1 e0 08             	shl    $0x8,%eax
801005f9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005fc:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100603:	00 
80100604:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010060b:	e8 ca fc ff ff       	call   801002da <outb>
  pos |= inb(CRTPORT+1);
80100610:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100617:	e8 94 fc ff ff       	call   801002b0 <inb>
8010061c:	0f b6 c0             	movzbl %al,%eax
8010061f:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
80100622:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100626:	75 30                	jne    80100658 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100628:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010062b:	ba 67 66 66 66       	mov    $0x66666667,%edx
80100630:	89 c8                	mov    %ecx,%eax
80100632:	f7 ea                	imul   %edx
80100634:	c1 fa 05             	sar    $0x5,%edx
80100637:	89 c8                	mov    %ecx,%eax
80100639:	c1 f8 1f             	sar    $0x1f,%eax
8010063c:	29 c2                	sub    %eax,%edx
8010063e:	89 d0                	mov    %edx,%eax
80100640:	c1 e0 02             	shl    $0x2,%eax
80100643:	01 d0                	add    %edx,%eax
80100645:	c1 e0 04             	shl    $0x4,%eax
80100648:	89 ca                	mov    %ecx,%edx
8010064a:	29 c2                	sub    %eax,%edx
8010064c:	b8 50 00 00 00       	mov    $0x50,%eax
80100651:	29 d0                	sub    %edx,%eax
80100653:	01 45 f4             	add    %eax,-0xc(%ebp)
80100656:	eb 32                	jmp    8010068a <cgaputc+0xbd>
  else if(c == BACKSPACE){
80100658:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065f:	75 0c                	jne    8010066d <cgaputc+0xa0>
    if(pos > 0) --pos;
80100661:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100665:	7e 23                	jle    8010068a <cgaputc+0xbd>
80100667:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
8010066b:	eb 1d                	jmp    8010068a <cgaputc+0xbd>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066d:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100675:	01 d2                	add    %edx,%edx
80100677:	01 c2                	add    %eax,%edx
80100679:	8b 45 08             	mov    0x8(%ebp),%eax
8010067c:	66 25 ff 00          	and    $0xff,%ax
80100680:	80 cc 07             	or     $0x7,%ah
80100683:	66 89 02             	mov    %ax,(%edx)
80100686:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x119>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 a0 10 80       	mov    0x8010a000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 1a 58 00 00       	call   80105ed1 <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	01 c0                	add    %eax,%eax
801006c5:	8b 15 00 a0 10 80    	mov    0x8010a000,%edx
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 ca                	add    %ecx,%edx
801006d2:	89 44 24 08          	mov    %eax,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 14 24             	mov    %edx,(%esp)
801006e1:	e8 18 57 00 00       	call   80105dfe <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 e0 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 c7 fb ff ff       	call   801002da <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 b3 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 9d fb ff ff       	call   801002da <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 c0 c5 10 80       	mov    0x8010c5c0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 94 fb ff ff       	call   801002f8 <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 9e 75 00 00       	call   80107d19 <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 92 75 00 00       	call   80107d19 <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 86 75 00 00       	call   80107d19 <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 79 75 00 00       	call   80107d19 <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 22 fe ff ff       	call   801005cd <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
801007ba:	e8 b8 53 00 00       	call   80105b77 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 41 01 00 00       	jmp    80100905 <consoleintr+0x158>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 68                	je     8010083e <consoleintr+0x91>
801007d6:	e9 94 00 00 00       	jmp    8010086f <consoleintr+0xc2>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 59                	je     8010083e <consoleintr+0x91>
801007e5:	e9 85 00 00 00       	jmp    8010086f <consoleintr+0xc2>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 51 51 00 00       	call   80105940 <procdump>
      break;
801007ef:	e9 11 01 00 00       	jmp    80100905 <consoleintr+0x158>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 dc ee 10 80       	mov    %eax,0x8010eedc
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 dc ee 10 80    	mov    0x8010eedc,%edx
80100816:	a1 d8 ee 10 80       	mov    0x8010eed8,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	0f 84 db 00 00 00    	je     801008fe <consoleintr+0x151>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100823:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
80100828:	83 e8 01             	sub    $0x1,%eax
8010082b:	83 e0 7f             	and    $0x7f,%eax
8010082e:	0f b6 80 54 ee 10 80 	movzbl -0x7fef11ac(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100835:	3c 0a                	cmp    $0xa,%al
80100837:	75 bb                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100839:	e9 c0 00 00 00       	jmp    801008fe <consoleintr+0x151>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083e:	8b 15 dc ee 10 80    	mov    0x8010eedc,%edx
80100844:	a1 d8 ee 10 80       	mov    0x8010eed8,%eax
80100849:	39 c2                	cmp    %eax,%edx
8010084b:	0f 84 b0 00 00 00    	je     80100901 <consoleintr+0x154>
        input.e--;
80100851:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
80100856:	83 e8 01             	sub    $0x1,%eax
80100859:	a3 dc ee 10 80       	mov    %eax,0x8010eedc
        consputc(BACKSPACE);
8010085e:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100865:	e8 e6 fe ff ff       	call   80100750 <consputc>
      }
      break;
8010086a:	e9 92 00 00 00       	jmp    80100901 <consoleintr+0x154>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100873:	0f 84 8b 00 00 00    	je     80100904 <consoleintr+0x157>
80100879:	8b 15 dc ee 10 80    	mov    0x8010eedc,%edx
8010087f:	a1 d4 ee 10 80       	mov    0x8010eed4,%eax
80100884:	89 d1                	mov    %edx,%ecx
80100886:	29 c1                	sub    %eax,%ecx
80100888:	89 c8                	mov    %ecx,%eax
8010088a:	83 f8 7f             	cmp    $0x7f,%eax
8010088d:	77 75                	ja     80100904 <consoleintr+0x157>
        c = (c == '\r') ? '\n' : c;
8010088f:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
80100893:	74 05                	je     8010089a <consoleintr+0xed>
80100895:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100898:	eb 05                	jmp    8010089f <consoleintr+0xf2>
8010089a:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089f:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
801008a2:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
801008a7:	89 c1                	mov    %eax,%ecx
801008a9:	83 e1 7f             	and    $0x7f,%ecx
801008ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801008af:	88 91 54 ee 10 80    	mov    %dl,-0x7fef11ac(%ecx)
801008b5:	83 c0 01             	add    $0x1,%eax
801008b8:	a3 dc ee 10 80       	mov    %eax,0x8010eedc
        consputc(c);
801008bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008c0:	89 04 24             	mov    %eax,(%esp)
801008c3:	e8 88 fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c8:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008cc:	74 18                	je     801008e6 <consoleintr+0x139>
801008ce:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008d2:	74 12                	je     801008e6 <consoleintr+0x139>
801008d4:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
801008d9:	8b 15 d4 ee 10 80    	mov    0x8010eed4,%edx
801008df:	83 ea 80             	sub    $0xffffff80,%edx
801008e2:	39 d0                	cmp    %edx,%eax
801008e4:	75 1e                	jne    80100904 <consoleintr+0x157>
          input.w = input.e;
801008e6:	a1 dc ee 10 80       	mov    0x8010eedc,%eax
801008eb:	a3 d8 ee 10 80       	mov    %eax,0x8010eed8
          wakeup(&input.r);
801008f0:	c7 04 24 d4 ee 10 80 	movl   $0x8010eed4,(%esp)
801008f7:	e8 7b 4f 00 00       	call   80105877 <wakeup>
        }
      }
      break;
801008fc:	eb 06                	jmp    80100904 <consoleintr+0x157>
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
801008fe:	90                   	nop
801008ff:	eb 04                	jmp    80100905 <consoleintr+0x158>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100901:	90                   	nop
80100902:	eb 01                	jmp    80100905 <consoleintr+0x158>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
          input.w = input.e;
          wakeup(&input.r);
        }
      }
      break;
80100904:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
80100905:	8b 45 08             	mov    0x8(%ebp),%eax
80100908:	ff d0                	call   *%eax
8010090a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010090d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100911:	0f 89 ad fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
80100917:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
8010091e:	e8 ef 52 00 00       	call   80105c12 <release>
}
80100923:	c9                   	leave  
80100924:	c3                   	ret    

80100925 <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
80100925:	55                   	push   %ebp
80100926:	89 e5                	mov    %esp,%ebp
80100928:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
8010092b:	8b 45 08             	mov    0x8(%ebp),%eax
8010092e:	89 04 24             	mov    %eax,(%esp)
80100931:	e8 80 10 00 00       	call   801019b6 <iunlock>
  target = n;
80100936:	8b 45 10             	mov    0x10(%ebp),%eax
80100939:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
8010093c:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
80100943:	e8 2f 52 00 00       	call   80105b77 <acquire>
  while(n > 0){
80100948:	e9 a8 00 00 00       	jmp    801009f5 <consoleread+0xd0>
    while(input.r == input.w){
      if(proc->killed){
8010094d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100953:	8b 40 24             	mov    0x24(%eax),%eax
80100956:	85 c0                	test   %eax,%eax
80100958:	74 21                	je     8010097b <consoleread+0x56>
        release(&input.lock);
8010095a:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
80100961:	e8 ac 52 00 00       	call   80105c12 <release>
        ilock(ip);
80100966:	8b 45 08             	mov    0x8(%ebp),%eax
80100969:	89 04 24             	mov    %eax,(%esp)
8010096c:	e8 f7 0e 00 00       	call   80101868 <ilock>
        return -1;
80100971:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100976:	e9 a9 00 00 00       	jmp    80100a24 <consoleread+0xff>
      }
      sleep(&input.r, &input.lock);
8010097b:	c7 44 24 04 20 ee 10 	movl   $0x8010ee20,0x4(%esp)
80100982:	80 
80100983:	c7 04 24 d4 ee 10 80 	movl   $0x8010eed4,(%esp)
8010098a:	e8 76 4d 00 00       	call   80105705 <sleep>
8010098f:	eb 01                	jmp    80100992 <consoleread+0x6d>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100991:	90                   	nop
80100992:	8b 15 d4 ee 10 80    	mov    0x8010eed4,%edx
80100998:	a1 d8 ee 10 80       	mov    0x8010eed8,%eax
8010099d:	39 c2                	cmp    %eax,%edx
8010099f:	74 ac                	je     8010094d <consoleread+0x28>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
801009a1:	a1 d4 ee 10 80       	mov    0x8010eed4,%eax
801009a6:	89 c2                	mov    %eax,%edx
801009a8:	83 e2 7f             	and    $0x7f,%edx
801009ab:	0f b6 92 54 ee 10 80 	movzbl -0x7fef11ac(%edx),%edx
801009b2:	0f be d2             	movsbl %dl,%edx
801009b5:	89 55 f0             	mov    %edx,-0x10(%ebp)
801009b8:	83 c0 01             	add    $0x1,%eax
801009bb:	a3 d4 ee 10 80       	mov    %eax,0x8010eed4
    if(c == C('D')){  // EOF
801009c0:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009c4:	75 17                	jne    801009dd <consoleread+0xb8>
      if(n < target){
801009c6:	8b 45 10             	mov    0x10(%ebp),%eax
801009c9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009cc:	73 2f                	jae    801009fd <consoleread+0xd8>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009ce:	a1 d4 ee 10 80       	mov    0x8010eed4,%eax
801009d3:	83 e8 01             	sub    $0x1,%eax
801009d6:	a3 d4 ee 10 80       	mov    %eax,0x8010eed4
      }
      break;
801009db:	eb 20                	jmp    801009fd <consoleread+0xd8>
    }
    *dst++ = c;
801009dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801009e0:	89 c2                	mov    %eax,%edx
801009e2:	8b 45 0c             	mov    0xc(%ebp),%eax
801009e5:	88 10                	mov    %dl,(%eax)
801009e7:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
    --n;
801009eb:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009ef:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009f3:	74 0b                	je     80100a00 <consoleread+0xdb>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009f5:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f9:	7f 96                	jg     80100991 <consoleread+0x6c>
801009fb:	eb 04                	jmp    80100a01 <consoleread+0xdc>
      if(n < target){
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
      }
      break;
801009fd:	90                   	nop
801009fe:	eb 01                	jmp    80100a01 <consoleread+0xdc>
    }
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
80100a00:	90                   	nop
  }
  release(&input.lock);
80100a01:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
80100a08:	e8 05 52 00 00       	call   80105c12 <release>
  ilock(ip);
80100a0d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a10:	89 04 24             	mov    %eax,(%esp)
80100a13:	e8 50 0e 00 00       	call   80101868 <ilock>

  return target - n;
80100a18:	8b 45 10             	mov    0x10(%ebp),%eax
80100a1b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a1e:	89 d1                	mov    %edx,%ecx
80100a20:	29 c1                	sub    %eax,%ecx
80100a22:	89 c8                	mov    %ecx,%eax
}
80100a24:	c9                   	leave  
80100a25:	c3                   	ret    

80100a26 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a26:	55                   	push   %ebp
80100a27:	89 e5                	mov    %esp,%ebp
80100a29:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a2c:	8b 45 08             	mov    0x8(%ebp),%eax
80100a2f:	89 04 24             	mov    %eax,(%esp)
80100a32:	e8 7f 0f 00 00       	call   801019b6 <iunlock>
  acquire(&cons.lock);
80100a37:	c7 04 24 e0 c5 10 80 	movl   $0x8010c5e0,(%esp)
80100a3e:	e8 34 51 00 00       	call   80105b77 <acquire>
  for(i = 0; i < n; i++)
80100a43:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a4a:	eb 1d                	jmp    80100a69 <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a4f:	03 45 0c             	add    0xc(%ebp),%eax
80100a52:	0f b6 00             	movzbl (%eax),%eax
80100a55:	0f be c0             	movsbl %al,%eax
80100a58:	25 ff 00 00 00       	and    $0xff,%eax
80100a5d:	89 04 24             	mov    %eax,(%esp)
80100a60:	e8 eb fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a65:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a6c:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a6f:	7c db                	jl     80100a4c <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a71:	c7 04 24 e0 c5 10 80 	movl   $0x8010c5e0,(%esp)
80100a78:	e8 95 51 00 00       	call   80105c12 <release>
  ilock(ip);
80100a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a80:	89 04 24             	mov    %eax,(%esp)
80100a83:	e8 e0 0d 00 00       	call   80101868 <ilock>

  return n;
80100a88:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a8b:	c9                   	leave  
80100a8c:	c3                   	ret    

80100a8d <consoleinit>:

void
consoleinit(void)
{
80100a8d:	55                   	push   %ebp
80100a8e:	89 e5                	mov    %esp,%ebp
80100a90:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a93:	c7 44 24 04 ff 96 10 	movl   $0x801096ff,0x4(%esp)
80100a9a:	80 
80100a9b:	c7 04 24 e0 c5 10 80 	movl   $0x8010c5e0,(%esp)
80100aa2:	e8 af 50 00 00       	call   80105b56 <initlock>
  initlock(&input.lock, "input");
80100aa7:	c7 44 24 04 07 97 10 	movl   $0x80109707,0x4(%esp)
80100aae:	80 
80100aaf:	c7 04 24 20 ee 10 80 	movl   $0x8010ee20,(%esp)
80100ab6:	e8 9b 50 00 00       	call   80105b56 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100abb:	c7 05 8c f8 10 80 26 	movl   $0x80100a26,0x8010f88c
80100ac2:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ac5:	c7 05 88 f8 10 80 25 	movl   $0x80100925,0x8010f888
80100acc:	09 10 80 
  cons.locking = 1;
80100acf:	c7 05 14 c6 10 80 01 	movl   $0x1,0x8010c614
80100ad6:	00 00 00 

  picenable(IRQ_KBD);
80100ad9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae0:	e8 d8 37 00 00       	call   801042bd <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ae5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100aec:	00 
80100aed:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100af4:	e8 81 1e 00 00       	call   8010297a <ioapicenable>
}
80100af9:	c9                   	leave  
80100afa:	c3                   	ret    
	...

80100afc <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100afc:	55                   	push   %ebp
80100afd:	89 e5                	mov    %esp,%ebp
80100aff:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  if((ip = namei(path)) == 0)
80100b05:	8b 45 08             	mov    0x8(%ebp),%eax
80100b08:	89 04 24             	mov    %eax,(%esp)
80100b0b:	e8 fa 18 00 00       	call   8010240a <namei>
80100b10:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b13:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b17:	75 0a                	jne    80100b23 <exec+0x27>
    return -1;
80100b19:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1e:	e9 da 03 00 00       	jmp    80100efd <exec+0x401>
  ilock(ip);
80100b23:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b26:	89 04 24             	mov    %eax,(%esp)
80100b29:	e8 3a 0d 00 00       	call   80101868 <ilock>
  pgdir = 0;
80100b2e:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b35:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b3c:	00 
80100b3d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b44:	00 
80100b45:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b4b:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4f:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b52:	89 04 24             	mov    %eax,(%esp)
80100b55:	e8 04 12 00 00       	call   80101d5e <readi>
80100b5a:	83 f8 33             	cmp    $0x33,%eax
80100b5d:	0f 86 54 03 00 00    	jbe    80100eb7 <exec+0x3bb>
    goto bad;
  if(elf.magic != ELF_MAGIC)
80100b63:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b69:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6e:	0f 85 46 03 00 00    	jne    80100eba <exec+0x3be>
    goto bad;

  if((pgdir = setupkvm(kalloc)) == 0)
80100b74:	c7 04 24 10 2b 10 80 	movl   $0x80102b10,(%esp)
80100b7b:	e8 dd 82 00 00       	call   80108e5d <setupkvm>
80100b80:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b83:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b87:	0f 84 30 03 00 00    	je     80100ebd <exec+0x3c1>
    goto bad;

  // Load program into memory.
  sz = 0;
80100b8d:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b94:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b9b:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100ba1:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100ba4:	e9 c5 00 00 00       	jmp    80100c6e <exec+0x172>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba9:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100bac:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bb3:	00 
80100bb4:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb8:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bbe:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bc2:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bc5:	89 04 24             	mov    %eax,(%esp)
80100bc8:	e8 91 11 00 00       	call   80101d5e <readi>
80100bcd:	83 f8 20             	cmp    $0x20,%eax
80100bd0:	0f 85 ea 02 00 00    	jne    80100ec0 <exec+0x3c4>
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
80100bd6:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bdc:	83 f8 01             	cmp    $0x1,%eax
80100bdf:	75 7f                	jne    80100c60 <exec+0x164>
      continue;
    if(ph.memsz < ph.filesz)
80100be1:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be7:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bed:	39 c2                	cmp    %eax,%edx
80100bef:	0f 82 ce 02 00 00    	jb     80100ec3 <exec+0x3c7>
      goto bad;
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 16 86 00 00       	call   8010922f <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	0f 84 a0 02 00 00    	je     80100ec6 <exec+0x3ca>
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c26:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2c:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c32:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c38:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3c:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c40:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c43:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4b:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4e:	89 04 24             	mov    %eax,(%esp)
80100c51:	e8 ea 84 00 00       	call   80109140 <loaduvm>
80100c56:	85 c0                	test   %eax,%eax
80100c58:	0f 88 6b 02 00 00    	js     80100ec9 <exec+0x3cd>
80100c5e:	eb 01                	jmp    80100c61 <exec+0x165>
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
      continue;
80100c60:	90                   	nop
  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c61:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c65:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c68:	83 c0 20             	add    $0x20,%eax
80100c6b:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6e:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c75:	0f b7 c0             	movzwl %ax,%eax
80100c78:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7b:	0f 8f 28 ff ff ff    	jg     80100ba9 <exec+0xad>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c81:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c84:	89 04 24             	mov    %eax,(%esp)
80100c87:	e8 60 0e 00 00       	call   80101aec <iunlockput>
  ip = 0;
80100c8c:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c93:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c96:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca0:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ca6:	05 00 20 00 00       	add    $0x2000,%eax
80100cab:	89 44 24 08          	mov    %eax,0x8(%esp)
80100caf:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb2:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cb6:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cb9:	89 04 24             	mov    %eax,(%esp)
80100cbc:	e8 6e 85 00 00       	call   8010922f <allocuvm>
80100cc1:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc4:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100cc8:	0f 84 fe 01 00 00    	je     80100ecc <exec+0x3d0>
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cce:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd1:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cd6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cda:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cdd:	89 04 24             	mov    %eax,(%esp)
80100ce0:	e8 6e 87 00 00       	call   80109453 <clearpteu>
  sp = sz;
80100ce5:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ce8:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100ceb:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf2:	e9 81 00 00 00       	jmp    80100d78 <exec+0x27c>
    if(argc >= MAXARG)
80100cf7:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100cfb:	0f 87 ce 01 00 00    	ja     80100ecf <exec+0x3d3>
      goto bad;
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d01:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d04:	c1 e0 02             	shl    $0x2,%eax
80100d07:	03 45 0c             	add    0xc(%ebp),%eax
80100d0a:	8b 00                	mov    (%eax),%eax
80100d0c:	89 04 24             	mov    %eax,(%esp)
80100d0f:	e8 68 53 00 00       	call   8010607c <strlen>
80100d14:	f7 d0                	not    %eax
80100d16:	03 45 dc             	add    -0x24(%ebp),%eax
80100d19:	83 e0 fc             	and    $0xfffffffc,%eax
80100d1c:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d1f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d22:	c1 e0 02             	shl    $0x2,%eax
80100d25:	03 45 0c             	add    0xc(%ebp),%eax
80100d28:	8b 00                	mov    (%eax),%eax
80100d2a:	89 04 24             	mov    %eax,(%esp)
80100d2d:	e8 4a 53 00 00       	call   8010607c <strlen>
80100d32:	83 c0 01             	add    $0x1,%eax
80100d35:	89 c2                	mov    %eax,%edx
80100d37:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d3a:	c1 e0 02             	shl    $0x2,%eax
80100d3d:	03 45 0c             	add    0xc(%ebp),%eax
80100d40:	8b 00                	mov    (%eax),%eax
80100d42:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d46:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d4a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d51:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d54:	89 04 24             	mov    %eax,(%esp)
80100d57:	e8 ab 88 00 00       	call   80109607 <copyout>
80100d5c:	85 c0                	test   %eax,%eax
80100d5e:	0f 88 6e 01 00 00    	js     80100ed2 <exec+0x3d6>
      goto bad;
    ustack[3+argc] = sp;
80100d64:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d67:	8d 50 03             	lea    0x3(%eax),%edx
80100d6a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6d:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d74:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d78:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d7b:	c1 e0 02             	shl    $0x2,%eax
80100d7e:	03 45 0c             	add    0xc(%ebp),%eax
80100d81:	8b 00                	mov    (%eax),%eax
80100d83:	85 c0                	test   %eax,%eax
80100d85:	0f 85 6c ff ff ff    	jne    80100cf7 <exec+0x1fb>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100d8b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d8e:	83 c0 03             	add    $0x3,%eax
80100d91:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100d98:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100d9c:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100da3:	ff ff ff 
  ustack[1] = argc;
80100da6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100da9:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 01             	add    $0x1,%eax
80100db5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100dbc:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100dbf:	29 d0                	sub    %edx,%eax
80100dc1:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100dc7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dca:	83 c0 04             	add    $0x4,%eax
80100dcd:	c1 e0 02             	shl    $0x2,%eax
80100dd0:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 04             	add    $0x4,%eax
80100dd9:	c1 e0 02             	shl    $0x2,%eax
80100ddc:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100de0:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100de6:	89 44 24 08          	mov    %eax,0x8(%esp)
80100dea:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100ded:	89 44 24 04          	mov    %eax,0x4(%esp)
80100df1:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100df4:	89 04 24             	mov    %eax,(%esp)
80100df7:	e8 0b 88 00 00       	call   80109607 <copyout>
80100dfc:	85 c0                	test   %eax,%eax
80100dfe:	0f 88 d1 00 00 00    	js     80100ed5 <exec+0x3d9>
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e04:	8b 45 08             	mov    0x8(%ebp),%eax
80100e07:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e0d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e10:	eb 17                	jmp    80100e29 <exec+0x32d>
    if(*s == '/')
80100e12:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e15:	0f b6 00             	movzbl (%eax),%eax
80100e18:	3c 2f                	cmp    $0x2f,%al
80100e1a:	75 09                	jne    80100e25 <exec+0x329>
      last = s+1;
80100e1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e1f:	83 c0 01             	add    $0x1,%eax
80100e22:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e25:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e2c:	0f b6 00             	movzbl (%eax),%eax
80100e2f:	84 c0                	test   %al,%al
80100e31:	75 df                	jne    80100e12 <exec+0x316>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e33:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e39:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e3c:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e43:	00 
80100e44:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e4b:	89 14 24             	mov    %edx,(%esp)
80100e4e:	e8 db 51 00 00       	call   8010602e <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e53:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e59:	8b 40 04             	mov    0x4(%eax),%eax
80100e5c:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e5f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e65:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e68:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e6b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e71:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e74:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e76:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7c:	8b 40 18             	mov    0x18(%eax),%eax
80100e7f:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100e85:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100e88:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8e:	8b 40 18             	mov    0x18(%eax),%eax
80100e91:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100e94:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100e97:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e9d:	89 04 24             	mov    %eax,(%esp)
80100ea0:	e8 a9 80 00 00       	call   80108f4e <switchuvm>
  freevm(oldpgdir);
80100ea5:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ea8:	89 04 24             	mov    %eax,(%esp)
80100eab:	e8 15 85 00 00       	call   801093c5 <freevm>
  return 0;
80100eb0:	b8 00 00 00 00       	mov    $0x0,%eax
80100eb5:	eb 46                	jmp    80100efd <exec+0x401>
  ilock(ip);
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
    goto bad;
80100eb7:	90                   	nop
80100eb8:	eb 1c                	jmp    80100ed6 <exec+0x3da>
  if(elf.magic != ELF_MAGIC)
    goto bad;
80100eba:	90                   	nop
80100ebb:	eb 19                	jmp    80100ed6 <exec+0x3da>

  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;
80100ebd:	90                   	nop
80100ebe:	eb 16                	jmp    80100ed6 <exec+0x3da>

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
80100ec0:	90                   	nop
80100ec1:	eb 13                	jmp    80100ed6 <exec+0x3da>
    if(ph.type != ELF_PROG_LOAD)
      continue;
    if(ph.memsz < ph.filesz)
      goto bad;
80100ec3:	90                   	nop
80100ec4:	eb 10                	jmp    80100ed6 <exec+0x3da>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
80100ec6:	90                   	nop
80100ec7:	eb 0d                	jmp    80100ed6 <exec+0x3da>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
80100ec9:	90                   	nop
80100eca:	eb 0a                	jmp    80100ed6 <exec+0x3da>

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
    goto bad;
80100ecc:	90                   	nop
80100ecd:	eb 07                	jmp    80100ed6 <exec+0x3da>
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
    if(argc >= MAXARG)
      goto bad;
80100ecf:	90                   	nop
80100ed0:	eb 04                	jmp    80100ed6 <exec+0x3da>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
80100ed2:	90                   	nop
80100ed3:	eb 01                	jmp    80100ed6 <exec+0x3da>
  ustack[1] = argc;
  ustack[2] = sp - (argc+1)*4;  // argv pointer

  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;
80100ed5:	90                   	nop
  switchuvm(proc);
  freevm(oldpgdir);
  return 0;

 bad:
  if(pgdir)
80100ed6:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100eda:	74 0b                	je     80100ee7 <exec+0x3eb>
    freevm(pgdir);
80100edc:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100edf:	89 04 24             	mov    %eax,(%esp)
80100ee2:	e8 de 84 00 00       	call   801093c5 <freevm>
  if(ip)
80100ee7:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100eeb:	74 0b                	je     80100ef8 <exec+0x3fc>
    iunlockput(ip);
80100eed:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef0:	89 04 24             	mov    %eax,(%esp)
80100ef3:	e8 f4 0b 00 00       	call   80101aec <iunlockput>
  return -1;
80100ef8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100efd:	c9                   	leave  
80100efe:	c3                   	ret    
	...

80100f00 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f00:	55                   	push   %ebp
80100f01:	89 e5                	mov    %esp,%ebp
80100f03:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f06:	c7 44 24 04 0d 97 10 	movl   $0x8010970d,0x4(%esp)
80100f0d:	80 
80100f0e:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100f15:	e8 3c 4c 00 00       	call   80105b56 <initlock>
}
80100f1a:	c9                   	leave  
80100f1b:	c3                   	ret    

80100f1c <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f1c:	55                   	push   %ebp
80100f1d:	89 e5                	mov    %esp,%ebp
80100f1f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f22:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100f29:	e8 49 4c 00 00       	call   80105b77 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f2e:	c7 45 f4 14 ef 10 80 	movl   $0x8010ef14,-0xc(%ebp)
80100f35:	eb 29                	jmp    80100f60 <filealloc+0x44>
    if(f->ref == 0){
80100f37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f3a:	8b 40 04             	mov    0x4(%eax),%eax
80100f3d:	85 c0                	test   %eax,%eax
80100f3f:	75 1b                	jne    80100f5c <filealloc+0x40>
      f->ref = 1;
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f4b:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100f52:	e8 bb 4c 00 00       	call   80105c12 <release>
      return f;
80100f57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f5a:	eb 1e                	jmp    80100f7a <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f5c:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f60:	81 7d f4 74 f8 10 80 	cmpl   $0x8010f874,-0xc(%ebp)
80100f67:	72 ce                	jb     80100f37 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f69:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100f70:	e8 9d 4c 00 00       	call   80105c12 <release>
  return 0;
80100f75:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f7a:	c9                   	leave  
80100f7b:	c3                   	ret    

80100f7c <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f7c:	55                   	push   %ebp
80100f7d:	89 e5                	mov    %esp,%ebp
80100f7f:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f82:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100f89:	e8 e9 4b 00 00       	call   80105b77 <acquire>
  if(f->ref < 1)
80100f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80100f91:	8b 40 04             	mov    0x4(%eax),%eax
80100f94:	85 c0                	test   %eax,%eax
80100f96:	7f 0c                	jg     80100fa4 <filedup+0x28>
    panic("filedup");
80100f98:	c7 04 24 14 97 10 80 	movl   $0x80109714,(%esp)
80100f9f:	e8 99 f5 ff ff       	call   8010053d <panic>
  f->ref++;
80100fa4:	8b 45 08             	mov    0x8(%ebp),%eax
80100fa7:	8b 40 04             	mov    0x4(%eax),%eax
80100faa:	8d 50 01             	lea    0x1(%eax),%edx
80100fad:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb0:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fb3:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100fba:	e8 53 4c 00 00       	call   80105c12 <release>
  return f;
80100fbf:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fc2:	c9                   	leave  
80100fc3:	c3                   	ret    

80100fc4 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fc4:	55                   	push   %ebp
80100fc5:	89 e5                	mov    %esp,%ebp
80100fc7:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fca:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80100fd1:	e8 a1 4b 00 00       	call   80105b77 <acquire>
  if(f->ref < 1)
80100fd6:	8b 45 08             	mov    0x8(%ebp),%eax
80100fd9:	8b 40 04             	mov    0x4(%eax),%eax
80100fdc:	85 c0                	test   %eax,%eax
80100fde:	7f 0c                	jg     80100fec <fileclose+0x28>
    panic("fileclose");
80100fe0:	c7 04 24 1c 97 10 80 	movl   $0x8010971c,(%esp)
80100fe7:	e8 51 f5 ff ff       	call   8010053d <panic>
  if(--f->ref > 0){
80100fec:	8b 45 08             	mov    0x8(%ebp),%eax
80100fef:	8b 40 04             	mov    0x4(%eax),%eax
80100ff2:	8d 50 ff             	lea    -0x1(%eax),%edx
80100ff5:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff8:	89 50 04             	mov    %edx,0x4(%eax)
80100ffb:	8b 45 08             	mov    0x8(%ebp),%eax
80100ffe:	8b 40 04             	mov    0x4(%eax),%eax
80101001:	85 c0                	test   %eax,%eax
80101003:	7e 11                	jle    80101016 <fileclose+0x52>
    release(&ftable.lock);
80101005:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
8010100c:	e8 01 4c 00 00       	call   80105c12 <release>
    return;
80101011:	e9 82 00 00 00       	jmp    80101098 <fileclose+0xd4>
  }
  ff = *f;
80101016:	8b 45 08             	mov    0x8(%ebp),%eax
80101019:	8b 10                	mov    (%eax),%edx
8010101b:	89 55 e0             	mov    %edx,-0x20(%ebp)
8010101e:	8b 50 04             	mov    0x4(%eax),%edx
80101021:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80101024:	8b 50 08             	mov    0x8(%eax),%edx
80101027:	89 55 e8             	mov    %edx,-0x18(%ebp)
8010102a:	8b 50 0c             	mov    0xc(%eax),%edx
8010102d:	89 55 ec             	mov    %edx,-0x14(%ebp)
80101030:	8b 50 10             	mov    0x10(%eax),%edx
80101033:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101036:	8b 40 14             	mov    0x14(%eax),%eax
80101039:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
8010103c:	8b 45 08             	mov    0x8(%ebp),%eax
8010103f:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
8010104f:	c7 04 24 e0 ee 10 80 	movl   $0x8010eee0,(%esp)
80101056:	e8 b7 4b 00 00       	call   80105c12 <release>
  
  if(ff.type == FD_PIPE)
8010105b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010105e:	83 f8 01             	cmp    $0x1,%eax
80101061:	75 18                	jne    8010107b <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
80101063:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101067:	0f be d0             	movsbl %al,%edx
8010106a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010106d:	89 54 24 04          	mov    %edx,0x4(%esp)
80101071:	89 04 24             	mov    %eax,(%esp)
80101074:	e8 fe 34 00 00       	call   80104577 <pipeclose>
80101079:	eb 1d                	jmp    80101098 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
8010107b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010107e:	83 f8 02             	cmp    $0x2,%eax
80101081:	75 15                	jne    80101098 <fileclose+0xd4>
    begin_trans();
80101083:	e8 91 29 00 00       	call   80103a19 <begin_trans>
    iput(ff.ip);
80101088:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010108b:	89 04 24             	mov    %eax,(%esp)
8010108e:	e8 88 09 00 00       	call   80101a1b <iput>
    commit_trans();
80101093:	e8 ca 29 00 00       	call   80103a62 <commit_trans>
  }
}
80101098:	c9                   	leave  
80101099:	c3                   	ret    

8010109a <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
8010109a:	55                   	push   %ebp
8010109b:	89 e5                	mov    %esp,%ebp
8010109d:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010a0:	8b 45 08             	mov    0x8(%ebp),%eax
801010a3:	8b 00                	mov    (%eax),%eax
801010a5:	83 f8 02             	cmp    $0x2,%eax
801010a8:	75 38                	jne    801010e2 <filestat+0x48>
    ilock(f->ip);
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 40 10             	mov    0x10(%eax),%eax
801010b0:	89 04 24             	mov    %eax,(%esp)
801010b3:	e8 b0 07 00 00       	call   80101868 <ilock>
    stati(f->ip, st);
801010b8:	8b 45 08             	mov    0x8(%ebp),%eax
801010bb:	8b 40 10             	mov    0x10(%eax),%eax
801010be:	8b 55 0c             	mov    0xc(%ebp),%edx
801010c1:	89 54 24 04          	mov    %edx,0x4(%esp)
801010c5:	89 04 24             	mov    %eax,(%esp)
801010c8:	e8 4c 0c 00 00       	call   80101d19 <stati>
    iunlock(f->ip);
801010cd:	8b 45 08             	mov    0x8(%ebp),%eax
801010d0:	8b 40 10             	mov    0x10(%eax),%eax
801010d3:	89 04 24             	mov    %eax,(%esp)
801010d6:	e8 db 08 00 00       	call   801019b6 <iunlock>
    return 0;
801010db:	b8 00 00 00 00       	mov    $0x0,%eax
801010e0:	eb 05                	jmp    801010e7 <filestat+0x4d>
  }
  return -1;
801010e2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010e7:	c9                   	leave  
801010e8:	c3                   	ret    

801010e9 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010e9:	55                   	push   %ebp
801010ea:	89 e5                	mov    %esp,%ebp
801010ec:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010ef:	8b 45 08             	mov    0x8(%ebp),%eax
801010f2:	0f b6 40 08          	movzbl 0x8(%eax),%eax
801010f6:	84 c0                	test   %al,%al
801010f8:	75 0a                	jne    80101104 <fileread+0x1b>
    return -1;
801010fa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801010ff:	e9 9f 00 00 00       	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_PIPE)
80101104:	8b 45 08             	mov    0x8(%ebp),%eax
80101107:	8b 00                	mov    (%eax),%eax
80101109:	83 f8 01             	cmp    $0x1,%eax
8010110c:	75 1e                	jne    8010112c <fileread+0x43>
    return piperead(f->pipe, addr, n);
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 40 0c             	mov    0xc(%eax),%eax
80101114:	8b 55 10             	mov    0x10(%ebp),%edx
80101117:	89 54 24 08          	mov    %edx,0x8(%esp)
8010111b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010111e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101122:	89 04 24             	mov    %eax,(%esp)
80101125:	e8 cf 35 00 00       	call   801046f9 <piperead>
8010112a:	eb 77                	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_INODE){
8010112c:	8b 45 08             	mov    0x8(%ebp),%eax
8010112f:	8b 00                	mov    (%eax),%eax
80101131:	83 f8 02             	cmp    $0x2,%eax
80101134:	75 61                	jne    80101197 <fileread+0xae>
    ilock(f->ip);
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 40 10             	mov    0x10(%eax),%eax
8010113c:	89 04 24             	mov    %eax,(%esp)
8010113f:	e8 24 07 00 00       	call   80101868 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
80101144:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101147:	8b 45 08             	mov    0x8(%ebp),%eax
8010114a:	8b 50 14             	mov    0x14(%eax),%edx
8010114d:	8b 45 08             	mov    0x8(%ebp),%eax
80101150:	8b 40 10             	mov    0x10(%eax),%eax
80101153:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101157:	89 54 24 08          	mov    %edx,0x8(%esp)
8010115b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010115e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101162:	89 04 24             	mov    %eax,(%esp)
80101165:	e8 f4 0b 00 00       	call   80101d5e <readi>
8010116a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010116d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101171:	7e 11                	jle    80101184 <fileread+0x9b>
      f->off += r;
80101173:	8b 45 08             	mov    0x8(%ebp),%eax
80101176:	8b 50 14             	mov    0x14(%eax),%edx
80101179:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010117c:	01 c2                	add    %eax,%edx
8010117e:	8b 45 08             	mov    0x8(%ebp),%eax
80101181:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
80101184:	8b 45 08             	mov    0x8(%ebp),%eax
80101187:	8b 40 10             	mov    0x10(%eax),%eax
8010118a:	89 04 24             	mov    %eax,(%esp)
8010118d:	e8 24 08 00 00       	call   801019b6 <iunlock>
    return r;
80101192:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101195:	eb 0c                	jmp    801011a3 <fileread+0xba>
  }
  panic("fileread");
80101197:	c7 04 24 26 97 10 80 	movl   $0x80109726,(%esp)
8010119e:	e8 9a f3 ff ff       	call   8010053d <panic>
}
801011a3:	c9                   	leave  
801011a4:	c3                   	ret    

801011a5 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011a5:	55                   	push   %ebp
801011a6:	89 e5                	mov    %esp,%ebp
801011a8:	53                   	push   %ebx
801011a9:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011ac:	8b 45 08             	mov    0x8(%ebp),%eax
801011af:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011b3:	84 c0                	test   %al,%al
801011b5:	75 0a                	jne    801011c1 <filewrite+0x1c>
    return -1;
801011b7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011bc:	e9 23 01 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_PIPE)
801011c1:	8b 45 08             	mov    0x8(%ebp),%eax
801011c4:	8b 00                	mov    (%eax),%eax
801011c6:	83 f8 01             	cmp    $0x1,%eax
801011c9:	75 21                	jne    801011ec <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 40 0c             	mov    0xc(%eax),%eax
801011d1:	8b 55 10             	mov    0x10(%ebp),%edx
801011d4:	89 54 24 08          	mov    %edx,0x8(%esp)
801011d8:	8b 55 0c             	mov    0xc(%ebp),%edx
801011db:	89 54 24 04          	mov    %edx,0x4(%esp)
801011df:	89 04 24             	mov    %eax,(%esp)
801011e2:	e8 22 34 00 00       	call   80104609 <pipewrite>
801011e7:	e9 f8 00 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_INODE){
801011ec:	8b 45 08             	mov    0x8(%ebp),%eax
801011ef:	8b 00                	mov    (%eax),%eax
801011f1:	83 f8 02             	cmp    $0x2,%eax
801011f4:	0f 85 de 00 00 00    	jne    801012d8 <filewrite+0x133>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
801011fa:	c7 45 ec 00 06 00 00 	movl   $0x600,-0x14(%ebp)
    int i = 0;
80101201:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101208:	e9 a8 00 00 00       	jmp    801012b5 <filewrite+0x110>
      int n1 = n - i;
8010120d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101210:	8b 55 10             	mov    0x10(%ebp),%edx
80101213:	89 d1                	mov    %edx,%ecx
80101215:	29 c1                	sub    %eax,%ecx
80101217:	89 c8                	mov    %ecx,%eax
80101219:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
8010121c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010121f:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80101222:	7e 06                	jle    8010122a <filewrite+0x85>
        n1 = max;
80101224:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101227:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_trans();
8010122a:	e8 ea 27 00 00       	call   80103a19 <begin_trans>
      ilock(f->ip);
8010122f:	8b 45 08             	mov    0x8(%ebp),%eax
80101232:	8b 40 10             	mov    0x10(%eax),%eax
80101235:	89 04 24             	mov    %eax,(%esp)
80101238:	e8 2b 06 00 00       	call   80101868 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
8010123d:	8b 5d f0             	mov    -0x10(%ebp),%ebx
80101240:	8b 45 08             	mov    0x8(%ebp),%eax
80101243:	8b 48 14             	mov    0x14(%eax),%ecx
80101246:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101249:	89 c2                	mov    %eax,%edx
8010124b:	03 55 0c             	add    0xc(%ebp),%edx
8010124e:	8b 45 08             	mov    0x8(%ebp),%eax
80101251:	8b 40 10             	mov    0x10(%eax),%eax
80101254:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80101258:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010125c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101260:	89 04 24             	mov    %eax,(%esp)
80101263:	e8 61 0c 00 00       	call   80101ec9 <writei>
80101268:	89 45 e8             	mov    %eax,-0x18(%ebp)
8010126b:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010126f:	7e 11                	jle    80101282 <filewrite+0xdd>
        f->off += r;
80101271:	8b 45 08             	mov    0x8(%ebp),%eax
80101274:	8b 50 14             	mov    0x14(%eax),%edx
80101277:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010127a:	01 c2                	add    %eax,%edx
8010127c:	8b 45 08             	mov    0x8(%ebp),%eax
8010127f:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
80101282:	8b 45 08             	mov    0x8(%ebp),%eax
80101285:	8b 40 10             	mov    0x10(%eax),%eax
80101288:	89 04 24             	mov    %eax,(%esp)
8010128b:	e8 26 07 00 00       	call   801019b6 <iunlock>
      commit_trans();
80101290:	e8 cd 27 00 00       	call   80103a62 <commit_trans>

      if(r < 0)
80101295:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101299:	78 28                	js     801012c3 <filewrite+0x11e>
        break;
      if(r != n1)
8010129b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010129e:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012a1:	74 0c                	je     801012af <filewrite+0x10a>
        panic("short filewrite");
801012a3:	c7 04 24 2f 97 10 80 	movl   $0x8010972f,(%esp)
801012aa:	e8 8e f2 ff ff       	call   8010053d <panic>
      i += r;
801012af:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012b2:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012b8:	3b 45 10             	cmp    0x10(%ebp),%eax
801012bb:	0f 8c 4c ff ff ff    	jl     8010120d <filewrite+0x68>
801012c1:	eb 01                	jmp    801012c4 <filewrite+0x11f>
        f->off += r;
      iunlock(f->ip);
      commit_trans();

      if(r < 0)
        break;
801012c3:	90                   	nop
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c7:	3b 45 10             	cmp    0x10(%ebp),%eax
801012ca:	75 05                	jne    801012d1 <filewrite+0x12c>
801012cc:	8b 45 10             	mov    0x10(%ebp),%eax
801012cf:	eb 05                	jmp    801012d6 <filewrite+0x131>
801012d1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012d6:	eb 0c                	jmp    801012e4 <filewrite+0x13f>
  }
  panic("filewrite");
801012d8:	c7 04 24 3f 97 10 80 	movl   $0x8010973f,(%esp)
801012df:	e8 59 f2 ff ff       	call   8010053d <panic>
}
801012e4:	83 c4 24             	add    $0x24,%esp
801012e7:	5b                   	pop    %ebx
801012e8:	5d                   	pop    %ebp
801012e9:	c3                   	ret    
	...

801012ec <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012ec:	55                   	push   %ebp
801012ed:	89 e5                	mov    %esp,%ebp
801012ef:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f2:	8b 45 08             	mov    0x8(%ebp),%eax
801012f5:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801012fc:	00 
801012fd:	89 04 24             	mov    %eax,(%esp)
80101300:	e8 a1 ee ff ff       	call   801001a6 <bread>
80101305:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
80101308:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010130b:	83 c0 18             	add    $0x18,%eax
8010130e:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80101315:	00 
80101316:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010131d:	89 04 24             	mov    %eax,(%esp)
80101320:	e8 ac 4b 00 00       	call   80105ed1 <memmove>
  brelse(bp);
80101325:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101328:	89 04 24             	mov    %eax,(%esp)
8010132b:	e8 e7 ee ff ff       	call   80100217 <brelse>
}
80101330:	c9                   	leave  
80101331:	c3                   	ret    

80101332 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101332:	55                   	push   %ebp
80101333:	89 e5                	mov    %esp,%ebp
80101335:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
80101338:	8b 55 0c             	mov    0xc(%ebp),%edx
8010133b:	8b 45 08             	mov    0x8(%ebp),%eax
8010133e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101342:	89 04 24             	mov    %eax,(%esp)
80101345:	e8 5c ee ff ff       	call   801001a6 <bread>
8010134a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
8010134d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101350:	83 c0 18             	add    $0x18,%eax
80101353:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135a:	00 
8010135b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101362:	00 
80101363:	89 04 24             	mov    %eax,(%esp)
80101366:	e8 93 4a 00 00       	call   80105dfe <memset>
  log_write(bp);
8010136b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010136e:	89 04 24             	mov    %eax,(%esp)
80101371:	e8 44 27 00 00       	call   80103aba <log_write>
  brelse(bp);
80101376:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101379:	89 04 24             	mov    %eax,(%esp)
8010137c:	e8 96 ee ff ff       	call   80100217 <brelse>
}
80101381:	c9                   	leave  
80101382:	c3                   	ret    

80101383 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101383:	55                   	push   %ebp
80101384:	89 e5                	mov    %esp,%ebp
80101386:	53                   	push   %ebx
80101387:	83 ec 34             	sub    $0x34,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138a:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101391:	8b 45 08             	mov    0x8(%ebp),%eax
80101394:	8d 55 d8             	lea    -0x28(%ebp),%edx
80101397:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139b:	89 04 24             	mov    %eax,(%esp)
8010139e:	e8 49 ff ff ff       	call   801012ec <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013aa:	e9 11 01 00 00       	jmp    801014c0 <balloc+0x13d>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013af:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b2:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013b8:	85 c0                	test   %eax,%eax
801013ba:	0f 48 c2             	cmovs  %edx,%eax
801013bd:	c1 f8 0c             	sar    $0xc,%eax
801013c0:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c3:	c1 ea 03             	shr    $0x3,%edx
801013c6:	01 d0                	add    %edx,%eax
801013c8:	83 c0 03             	add    $0x3,%eax
801013cb:	89 44 24 04          	mov    %eax,0x4(%esp)
801013cf:	8b 45 08             	mov    0x8(%ebp),%eax
801013d2:	89 04 24             	mov    %eax,(%esp)
801013d5:	e8 cc ed ff ff       	call   801001a6 <bread>
801013da:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013dd:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e4:	e9 a7 00 00 00       	jmp    80101490 <balloc+0x10d>
      m = 1 << (bi % 8);
801013e9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013ec:	89 c2                	mov    %eax,%edx
801013ee:	c1 fa 1f             	sar    $0x1f,%edx
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 d3                	mov    %edx,%ebx
80101402:	89 c1                	mov    %eax,%ecx
80101404:	d3 e3                	shl    %cl,%ebx
80101406:	89 d8                	mov    %ebx,%eax
80101408:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
8010140b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140e:	8d 50 07             	lea    0x7(%eax),%edx
80101411:	85 c0                	test   %eax,%eax
80101413:	0f 48 c2             	cmovs  %edx,%eax
80101416:	c1 f8 03             	sar    $0x3,%eax
80101419:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141c:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101421:	0f b6 c0             	movzbl %al,%eax
80101424:	23 45 e8             	and    -0x18(%ebp),%eax
80101427:	85 c0                	test   %eax,%eax
80101429:	75 61                	jne    8010148c <balloc+0x109>
        bp->data[bi/8] |= m;  // Mark block in use.
8010142b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142e:	8d 50 07             	lea    0x7(%eax),%edx
80101431:	85 c0                	test   %eax,%eax
80101433:	0f 48 c2             	cmovs  %edx,%eax
80101436:	c1 f8 03             	sar    $0x3,%eax
80101439:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143c:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101441:	89 d1                	mov    %edx,%ecx
80101443:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101446:	09 ca                	or     %ecx,%edx
80101448:	89 d1                	mov    %edx,%ecx
8010144a:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144d:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
80101451:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101454:	89 04 24             	mov    %eax,(%esp)
80101457:	e8 5e 26 00 00       	call   80103aba <log_write>
        brelse(bp);
8010145c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145f:	89 04 24             	mov    %eax,(%esp)
80101462:	e8 b0 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101467:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010146a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146d:	01 c2                	add    %eax,%edx
8010146f:	8b 45 08             	mov    0x8(%ebp),%eax
80101472:	89 54 24 04          	mov    %edx,0x4(%esp)
80101476:	89 04 24             	mov    %eax,(%esp)
80101479:	e8 b4 fe ff ff       	call   80101332 <bzero>
        return b + bi;
8010147e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101481:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101484:	01 d0                	add    %edx,%eax
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
}
80101486:	83 c4 34             	add    $0x34,%esp
80101489:	5b                   	pop    %ebx
8010148a:	5d                   	pop    %ebp
8010148b:	c3                   	ret    

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010148c:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101490:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101497:	7f 15                	jg     801014ae <balloc+0x12b>
80101499:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010149c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010149f:	01 d0                	add    %edx,%eax
801014a1:	89 c2                	mov    %eax,%edx
801014a3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a6:	39 c2                	cmp    %eax,%edx
801014a8:	0f 82 3b ff ff ff    	jb     801013e9 <balloc+0x66>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014ae:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014b1:	89 04 24             	mov    %eax,(%esp)
801014b4:	e8 5e ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b9:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014c0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014c3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c6:	39 c2                	cmp    %eax,%edx
801014c8:	0f 82 e1 fe ff ff    	jb     801013af <balloc+0x2c>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014ce:	c7 04 24 49 97 10 80 	movl   $0x80109749,(%esp)
801014d5:	e8 63 f0 ff ff       	call   8010053d <panic>

801014da <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014da:	55                   	push   %ebp
801014db:	89 e5                	mov    %esp,%ebp
801014dd:	53                   	push   %ebx
801014de:	83 ec 34             	sub    $0x34,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014e1:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014e4:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e8:	8b 45 08             	mov    0x8(%ebp),%eax
801014eb:	89 04 24             	mov    %eax,(%esp)
801014ee:	e8 f9 fd ff ff       	call   801012ec <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014f3:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f6:	89 c2                	mov    %eax,%edx
801014f8:	c1 ea 0c             	shr    $0xc,%edx
801014fb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014fe:	c1 e8 03             	shr    $0x3,%eax
80101501:	01 d0                	add    %edx,%eax
80101503:	8d 50 03             	lea    0x3(%eax),%edx
80101506:	8b 45 08             	mov    0x8(%ebp),%eax
80101509:	89 54 24 04          	mov    %edx,0x4(%esp)
8010150d:	89 04 24             	mov    %eax,(%esp)
80101510:	e8 91 ec ff ff       	call   801001a6 <bread>
80101515:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101518:	8b 45 0c             	mov    0xc(%ebp),%eax
8010151b:	25 ff 0f 00 00       	and    $0xfff,%eax
80101520:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
80101523:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101526:	89 c2                	mov    %eax,%edx
80101528:	c1 fa 1f             	sar    $0x1f,%edx
8010152b:	c1 ea 1d             	shr    $0x1d,%edx
8010152e:	01 d0                	add    %edx,%eax
80101530:	83 e0 07             	and    $0x7,%eax
80101533:	29 d0                	sub    %edx,%eax
80101535:	ba 01 00 00 00       	mov    $0x1,%edx
8010153a:	89 d3                	mov    %edx,%ebx
8010153c:	89 c1                	mov    %eax,%ecx
8010153e:	d3 e3                	shl    %cl,%ebx
80101540:	89 d8                	mov    %ebx,%eax
80101542:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
80101545:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101548:	8d 50 07             	lea    0x7(%eax),%edx
8010154b:	85 c0                	test   %eax,%eax
8010154d:	0f 48 c2             	cmovs  %edx,%eax
80101550:	c1 f8 03             	sar    $0x3,%eax
80101553:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101556:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010155b:	0f b6 c0             	movzbl %al,%eax
8010155e:	23 45 ec             	and    -0x14(%ebp),%eax
80101561:	85 c0                	test   %eax,%eax
80101563:	75 0c                	jne    80101571 <bfree+0x97>
    panic("freeing free block");
80101565:	c7 04 24 5f 97 10 80 	movl   $0x8010975f,(%esp)
8010156c:	e8 cc ef ff ff       	call   8010053d <panic>
  bp->data[bi/8] &= ~m;
80101571:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101574:	8d 50 07             	lea    0x7(%eax),%edx
80101577:	85 c0                	test   %eax,%eax
80101579:	0f 48 c2             	cmovs  %edx,%eax
8010157c:	c1 f8 03             	sar    $0x3,%eax
8010157f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101582:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101587:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010158a:	f7 d1                	not    %ecx
8010158c:	21 ca                	and    %ecx,%edx
8010158e:	89 d1                	mov    %edx,%ecx
80101590:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101593:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 18 25 00 00       	call   80103aba <log_write>
  brelse(bp);
801015a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015a5:	89 04 24             	mov    %eax,(%esp)
801015a8:	e8 6a ec ff ff       	call   80100217 <brelse>
}
801015ad:	83 c4 34             	add    $0x34,%esp
801015b0:	5b                   	pop    %ebx
801015b1:	5d                   	pop    %ebp
801015b2:	c3                   	ret    

801015b3 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015b3:	55                   	push   %ebp
801015b4:	89 e5                	mov    %esp,%ebp
801015b6:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015b9:	c7 44 24 04 72 97 10 	movl   $0x80109772,0x4(%esp)
801015c0:	80 
801015c1:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
801015c8:	e8 89 45 00 00       	call   80105b56 <initlock>
}
801015cd:	c9                   	leave  
801015ce:	c3                   	ret    

801015cf <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015cf:	55                   	push   %ebp
801015d0:	89 e5                	mov    %esp,%ebp
801015d2:	83 ec 48             	sub    $0x48,%esp
801015d5:	8b 45 0c             	mov    0xc(%ebp),%eax
801015d8:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015dc:	8b 45 08             	mov    0x8(%ebp),%eax
801015df:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015e2:	89 54 24 04          	mov    %edx,0x4(%esp)
801015e6:	89 04 24             	mov    %eax,(%esp)
801015e9:	e8 fe fc ff ff       	call   801012ec <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015ee:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015f5:	e9 98 00 00 00       	jmp    80101692 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015fd:	c1 e8 03             	shr    $0x3,%eax
80101600:	83 c0 02             	add    $0x2,%eax
80101603:	89 44 24 04          	mov    %eax,0x4(%esp)
80101607:	8b 45 08             	mov    0x8(%ebp),%eax
8010160a:	89 04 24             	mov    %eax,(%esp)
8010160d:	e8 94 eb ff ff       	call   801001a6 <bread>
80101612:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101615:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101618:	8d 50 18             	lea    0x18(%eax),%edx
8010161b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010161e:	83 e0 07             	and    $0x7,%eax
80101621:	c1 e0 06             	shl    $0x6,%eax
80101624:	01 d0                	add    %edx,%eax
80101626:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
80101629:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010162c:	0f b7 00             	movzwl (%eax),%eax
8010162f:	66 85 c0             	test   %ax,%ax
80101632:	75 4f                	jne    80101683 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101634:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010163b:	00 
8010163c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101643:	00 
80101644:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101647:	89 04 24             	mov    %eax,(%esp)
8010164a:	e8 af 47 00 00       	call   80105dfe <memset>
      dip->type = type;
8010164f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101652:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101656:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
80101659:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010165c:	89 04 24             	mov    %eax,(%esp)
8010165f:	e8 56 24 00 00       	call   80103aba <log_write>
      brelse(bp);
80101664:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101667:	89 04 24             	mov    %eax,(%esp)
8010166a:	e8 a8 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
8010166f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101672:	89 44 24 04          	mov    %eax,0x4(%esp)
80101676:	8b 45 08             	mov    0x8(%ebp),%eax
80101679:	89 04 24             	mov    %eax,(%esp)
8010167c:	e8 e3 00 00 00       	call   80101764 <iget>
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
}
80101681:	c9                   	leave  
80101682:	c3                   	ret    
      dip->type = type;
      log_write(bp);   // mark it allocated on the disk
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
80101683:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101686:	89 04 24             	mov    %eax,(%esp)
80101689:	e8 89 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010168e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101692:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101695:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101698:	39 c2                	cmp    %eax,%edx
8010169a:	0f 82 5a ff ff ff    	jb     801015fa <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
801016a0:	c7 04 24 79 97 10 80 	movl   $0x80109779,(%esp)
801016a7:	e8 91 ee ff ff       	call   8010053d <panic>

801016ac <iupdate>:
}

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
801016ac:	55                   	push   %ebp
801016ad:	89 e5                	mov    %esp,%ebp
801016af:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016b2:	8b 45 08             	mov    0x8(%ebp),%eax
801016b5:	8b 40 04             	mov    0x4(%eax),%eax
801016b8:	c1 e8 03             	shr    $0x3,%eax
801016bb:	8d 50 02             	lea    0x2(%eax),%edx
801016be:	8b 45 08             	mov    0x8(%ebp),%eax
801016c1:	8b 00                	mov    (%eax),%eax
801016c3:	89 54 24 04          	mov    %edx,0x4(%esp)
801016c7:	89 04 24             	mov    %eax,(%esp)
801016ca:	e8 d7 ea ff ff       	call   801001a6 <bread>
801016cf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016d5:	8d 50 18             	lea    0x18(%eax),%edx
801016d8:	8b 45 08             	mov    0x8(%ebp),%eax
801016db:	8b 40 04             	mov    0x4(%eax),%eax
801016de:	83 e0 07             	and    $0x7,%eax
801016e1:	c1 e0 06             	shl    $0x6,%eax
801016e4:	01 d0                	add    %edx,%eax
801016e6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016f6:	8b 45 08             	mov    0x8(%ebp),%eax
801016f9:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101700:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
80101704:	8b 45 08             	mov    0x8(%ebp),%eax
80101707:	0f b7 50 14          	movzwl 0x14(%eax),%edx
8010170b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170e:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101712:	8b 45 08             	mov    0x8(%ebp),%eax
80101715:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101720:	8b 45 08             	mov    0x8(%ebp),%eax
80101723:	8b 50 18             	mov    0x18(%eax),%edx
80101726:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101729:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010172c:	8b 45 08             	mov    0x8(%ebp),%eax
8010172f:	8d 50 1c             	lea    0x1c(%eax),%edx
80101732:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101735:	83 c0 0c             	add    $0xc,%eax
80101738:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
8010173f:	00 
80101740:	89 54 24 04          	mov    %edx,0x4(%esp)
80101744:	89 04 24             	mov    %eax,(%esp)
80101747:	e8 85 47 00 00       	call   80105ed1 <memmove>
  log_write(bp);
8010174c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174f:	89 04 24             	mov    %eax,(%esp)
80101752:	e8 63 23 00 00       	call   80103aba <log_write>
  brelse(bp);
80101757:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010175a:	89 04 24             	mov    %eax,(%esp)
8010175d:	e8 b5 ea ff ff       	call   80100217 <brelse>
}
80101762:	c9                   	leave  
80101763:	c3                   	ret    

80101764 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101764:	55                   	push   %ebp
80101765:	89 e5                	mov    %esp,%ebp
80101767:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010176a:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101771:	e8 01 44 00 00       	call   80105b77 <acquire>

  // Is the inode already cached?
  empty = 0;
80101776:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010177d:	c7 45 f4 14 f9 10 80 	movl   $0x8010f914,-0xc(%ebp)
80101784:	eb 59                	jmp    801017df <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101786:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101789:	8b 40 08             	mov    0x8(%eax),%eax
8010178c:	85 c0                	test   %eax,%eax
8010178e:	7e 35                	jle    801017c5 <iget+0x61>
80101790:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101793:	8b 00                	mov    (%eax),%eax
80101795:	3b 45 08             	cmp    0x8(%ebp),%eax
80101798:	75 2b                	jne    801017c5 <iget+0x61>
8010179a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179d:	8b 40 04             	mov    0x4(%eax),%eax
801017a0:	3b 45 0c             	cmp    0xc(%ebp),%eax
801017a3:	75 20                	jne    801017c5 <iget+0x61>
      ip->ref++;
801017a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a8:	8b 40 08             	mov    0x8(%eax),%eax
801017ab:	8d 50 01             	lea    0x1(%eax),%edx
801017ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b1:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017b4:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
801017bb:	e8 52 44 00 00       	call   80105c12 <release>
      return ip;
801017c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c3:	eb 6f                	jmp    80101834 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017c5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017c9:	75 10                	jne    801017db <iget+0x77>
801017cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017ce:	8b 40 08             	mov    0x8(%eax),%eax
801017d1:	85 c0                	test   %eax,%eax
801017d3:	75 06                	jne    801017db <iget+0x77>
      empty = ip;
801017d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017d8:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017db:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017df:	81 7d f4 b4 08 11 80 	cmpl   $0x801108b4,-0xc(%ebp)
801017e6:	72 9e                	jb     80101786 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017e8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017ec:	75 0c                	jne    801017fa <iget+0x96>
    panic("iget: no inodes");
801017ee:	c7 04 24 8b 97 10 80 	movl   $0x8010978b,(%esp)
801017f5:	e8 43 ed ff ff       	call   8010053d <panic>

  ip = empty;
801017fa:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017fd:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
80101800:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101803:	8b 55 08             	mov    0x8(%ebp),%edx
80101806:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
80101808:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010180b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010180e:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101811:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101814:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010181b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010181e:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101825:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
8010182c:	e8 e1 43 00 00       	call   80105c12 <release>

  return ip;
80101831:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101834:	c9                   	leave  
80101835:	c3                   	ret    

80101836 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101836:	55                   	push   %ebp
80101837:	89 e5                	mov    %esp,%ebp
80101839:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010183c:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101843:	e8 2f 43 00 00       	call   80105b77 <acquire>
  ip->ref++;
80101848:	8b 45 08             	mov    0x8(%ebp),%eax
8010184b:	8b 40 08             	mov    0x8(%eax),%eax
8010184e:	8d 50 01             	lea    0x1(%eax),%edx
80101851:	8b 45 08             	mov    0x8(%ebp),%eax
80101854:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101857:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
8010185e:	e8 af 43 00 00       	call   80105c12 <release>
  return ip;
80101863:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101866:	c9                   	leave  
80101867:	c3                   	ret    

80101868 <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
80101868:	55                   	push   %ebp
80101869:	89 e5                	mov    %esp,%ebp
8010186b:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
8010186e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101872:	74 0a                	je     8010187e <ilock+0x16>
80101874:	8b 45 08             	mov    0x8(%ebp),%eax
80101877:	8b 40 08             	mov    0x8(%eax),%eax
8010187a:	85 c0                	test   %eax,%eax
8010187c:	7f 0c                	jg     8010188a <ilock+0x22>
    panic("ilock");
8010187e:	c7 04 24 9b 97 10 80 	movl   $0x8010979b,(%esp)
80101885:	e8 b3 ec ff ff       	call   8010053d <panic>

  acquire(&icache.lock);
8010188a:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101891:	e8 e1 42 00 00       	call   80105b77 <acquire>
  while(ip->flags & I_BUSY)
80101896:	eb 13                	jmp    801018ab <ilock+0x43>
    sleep(ip, &icache.lock);
80101898:	c7 44 24 04 e0 f8 10 	movl   $0x8010f8e0,0x4(%esp)
8010189f:	80 
801018a0:	8b 45 08             	mov    0x8(%ebp),%eax
801018a3:	89 04 24             	mov    %eax,(%esp)
801018a6:	e8 5a 3e 00 00       	call   80105705 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 e0 01             	and    $0x1,%eax
801018b4:	84 c0                	test   %al,%al
801018b6:	75 e0                	jne    80101898 <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018b8:	8b 45 08             	mov    0x8(%ebp),%eax
801018bb:	8b 40 0c             	mov    0xc(%eax),%eax
801018be:	89 c2                	mov    %eax,%edx
801018c0:	83 ca 01             	or     $0x1,%edx
801018c3:	8b 45 08             	mov    0x8(%ebp),%eax
801018c6:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018c9:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
801018d0:	e8 3d 43 00 00       	call   80105c12 <release>

  if(!(ip->flags & I_VALID)){
801018d5:	8b 45 08             	mov    0x8(%ebp),%eax
801018d8:	8b 40 0c             	mov    0xc(%eax),%eax
801018db:	83 e0 02             	and    $0x2,%eax
801018de:	85 c0                	test   %eax,%eax
801018e0:	0f 85 ce 00 00 00    	jne    801019b4 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018e6:	8b 45 08             	mov    0x8(%ebp),%eax
801018e9:	8b 40 04             	mov    0x4(%eax),%eax
801018ec:	c1 e8 03             	shr    $0x3,%eax
801018ef:	8d 50 02             	lea    0x2(%eax),%edx
801018f2:	8b 45 08             	mov    0x8(%ebp),%eax
801018f5:	8b 00                	mov    (%eax),%eax
801018f7:	89 54 24 04          	mov    %edx,0x4(%esp)
801018fb:	89 04 24             	mov    %eax,(%esp)
801018fe:	e8 a3 e8 ff ff       	call   801001a6 <bread>
80101903:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101906:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101909:	8d 50 18             	lea    0x18(%eax),%edx
8010190c:	8b 45 08             	mov    0x8(%ebp),%eax
8010190f:	8b 40 04             	mov    0x4(%eax),%eax
80101912:	83 e0 07             	and    $0x7,%eax
80101915:	c1 e0 06             	shl    $0x6,%eax
80101918:	01 d0                	add    %edx,%eax
8010191a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 10             	movzwl (%eax),%edx
80101923:	8b 45 08             	mov    0x8(%ebp),%eax
80101926:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010192a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192d:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101931:	8b 45 08             	mov    0x8(%ebp),%eax
80101934:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
80101938:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193b:	0f b7 50 04          	movzwl 0x4(%eax),%edx
8010193f:	8b 45 08             	mov    0x8(%ebp),%eax
80101942:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101946:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101949:	0f b7 50 06          	movzwl 0x6(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101954:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101957:	8b 50 08             	mov    0x8(%eax),%edx
8010195a:	8b 45 08             	mov    0x8(%ebp),%eax
8010195d:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101960:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101963:	8d 50 0c             	lea    0xc(%eax),%edx
80101966:	8b 45 08             	mov    0x8(%ebp),%eax
80101969:	83 c0 1c             	add    $0x1c,%eax
8010196c:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101973:	00 
80101974:	89 54 24 04          	mov    %edx,0x4(%esp)
80101978:	89 04 24             	mov    %eax,(%esp)
8010197b:	e8 51 45 00 00       	call   80105ed1 <memmove>
    brelse(bp);
80101980:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101983:	89 04 24             	mov    %eax,(%esp)
80101986:	e8 8c e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010198b:	8b 45 08             	mov    0x8(%ebp),%eax
8010198e:	8b 40 0c             	mov    0xc(%eax),%eax
80101991:	89 c2                	mov    %eax,%edx
80101993:	83 ca 02             	or     $0x2,%edx
80101996:	8b 45 08             	mov    0x8(%ebp),%eax
80101999:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010199c:	8b 45 08             	mov    0x8(%ebp),%eax
8010199f:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801019a3:	66 85 c0             	test   %ax,%ax
801019a6:	75 0c                	jne    801019b4 <ilock+0x14c>
      panic("ilock: no type");
801019a8:	c7 04 24 a1 97 10 80 	movl   $0x801097a1,(%esp)
801019af:	e8 89 eb ff ff       	call   8010053d <panic>
  }
}
801019b4:	c9                   	leave  
801019b5:	c3                   	ret    

801019b6 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019b6:	55                   	push   %ebp
801019b7:	89 e5                	mov    %esp,%ebp
801019b9:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019bc:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019c0:	74 17                	je     801019d9 <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 0c             	mov    0xc(%eax),%eax
801019c8:	83 e0 01             	and    $0x1,%eax
801019cb:	85 c0                	test   %eax,%eax
801019cd:	74 0a                	je     801019d9 <iunlock+0x23>
801019cf:	8b 45 08             	mov    0x8(%ebp),%eax
801019d2:	8b 40 08             	mov    0x8(%eax),%eax
801019d5:	85 c0                	test   %eax,%eax
801019d7:	7f 0c                	jg     801019e5 <iunlock+0x2f>
    panic("iunlock");
801019d9:	c7 04 24 b0 97 10 80 	movl   $0x801097b0,(%esp)
801019e0:	e8 58 eb ff ff       	call   8010053d <panic>
  acquire(&icache.lock);
801019e5:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
801019ec:	e8 86 41 00 00       	call   80105b77 <acquire>
  ip->flags &= ~I_BUSY;
801019f1:	8b 45 08             	mov    0x8(%ebp),%eax
801019f4:	8b 40 0c             	mov    0xc(%eax),%eax
801019f7:	89 c2                	mov    %eax,%edx
801019f9:	83 e2 fe             	and    $0xfffffffe,%edx
801019fc:	8b 45 08             	mov    0x8(%ebp),%eax
801019ff:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
80101a02:	8b 45 08             	mov    0x8(%ebp),%eax
80101a05:	89 04 24             	mov    %eax,(%esp)
80101a08:	e8 6a 3e 00 00       	call   80105877 <wakeup>
  release(&icache.lock);
80101a0d:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101a14:	e8 f9 41 00 00       	call   80105c12 <release>
}
80101a19:	c9                   	leave  
80101a1a:	c3                   	ret    

80101a1b <iput>:
// be recycled.
// If that was the last reference and the inode has no links
// to it, free the inode (and its content) on disk.
void
iput(struct inode *ip)
{
80101a1b:	55                   	push   %ebp
80101a1c:	89 e5                	mov    %esp,%ebp
80101a1e:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a21:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101a28:	e8 4a 41 00 00       	call   80105b77 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a30:	8b 40 08             	mov    0x8(%eax),%eax
80101a33:	83 f8 01             	cmp    $0x1,%eax
80101a36:	0f 85 93 00 00 00    	jne    80101acf <iput+0xb4>
80101a3c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a3f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a42:	83 e0 02             	and    $0x2,%eax
80101a45:	85 c0                	test   %eax,%eax
80101a47:	0f 84 82 00 00 00    	je     80101acf <iput+0xb4>
80101a4d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a50:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a54:	66 85 c0             	test   %ax,%ax
80101a57:	75 76                	jne    80101acf <iput+0xb4>
    // inode has no links: truncate and free inode.
    if(ip->flags & I_BUSY)
80101a59:	8b 45 08             	mov    0x8(%ebp),%eax
80101a5c:	8b 40 0c             	mov    0xc(%eax),%eax
80101a5f:	83 e0 01             	and    $0x1,%eax
80101a62:	84 c0                	test   %al,%al
80101a64:	74 0c                	je     80101a72 <iput+0x57>
      panic("iput busy");
80101a66:	c7 04 24 b8 97 10 80 	movl   $0x801097b8,(%esp)
80101a6d:	e8 cb ea ff ff       	call   8010053d <panic>
    ip->flags |= I_BUSY;
80101a72:	8b 45 08             	mov    0x8(%ebp),%eax
80101a75:	8b 40 0c             	mov    0xc(%eax),%eax
80101a78:	89 c2                	mov    %eax,%edx
80101a7a:	83 ca 01             	or     $0x1,%edx
80101a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a80:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a83:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101a8a:	e8 83 41 00 00       	call   80105c12 <release>
    itrunc(ip);
80101a8f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a92:	89 04 24             	mov    %eax,(%esp)
80101a95:	e8 72 01 00 00       	call   80101c0c <itrunc>
    ip->type = 0;
80101a9a:	8b 45 08             	mov    0x8(%ebp),%eax
80101a9d:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101aa3:	8b 45 08             	mov    0x8(%ebp),%eax
80101aa6:	89 04 24             	mov    %eax,(%esp)
80101aa9:	e8 fe fb ff ff       	call   801016ac <iupdate>
    acquire(&icache.lock);
80101aae:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101ab5:	e8 bd 40 00 00       	call   80105b77 <acquire>
    ip->flags = 0;
80101aba:	8b 45 08             	mov    0x8(%ebp),%eax
80101abd:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ac4:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac7:	89 04 24             	mov    %eax,(%esp)
80101aca:	e8 a8 3d 00 00       	call   80105877 <wakeup>
  }
  ip->ref--;
80101acf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ad2:	8b 40 08             	mov    0x8(%eax),%eax
80101ad5:	8d 50 ff             	lea    -0x1(%eax),%edx
80101ad8:	8b 45 08             	mov    0x8(%ebp),%eax
80101adb:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ade:	c7 04 24 e0 f8 10 80 	movl   $0x8010f8e0,(%esp)
80101ae5:	e8 28 41 00 00       	call   80105c12 <release>
}
80101aea:	c9                   	leave  
80101aeb:	c3                   	ret    

80101aec <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101aec:	55                   	push   %ebp
80101aed:	89 e5                	mov    %esp,%ebp
80101aef:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101af2:	8b 45 08             	mov    0x8(%ebp),%eax
80101af5:	89 04 24             	mov    %eax,(%esp)
80101af8:	e8 b9 fe ff ff       	call   801019b6 <iunlock>
  iput(ip);
80101afd:	8b 45 08             	mov    0x8(%ebp),%eax
80101b00:	89 04 24             	mov    %eax,(%esp)
80101b03:	e8 13 ff ff ff       	call   80101a1b <iput>
}
80101b08:	c9                   	leave  
80101b09:	c3                   	ret    

80101b0a <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101b0a:	55                   	push   %ebp
80101b0b:	89 e5                	mov    %esp,%ebp
80101b0d:	53                   	push   %ebx
80101b0e:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b11:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b15:	77 3e                	ja     80101b55 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b17:	8b 45 08             	mov    0x8(%ebp),%eax
80101b1a:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b1d:	83 c2 04             	add    $0x4,%edx
80101b20:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b24:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b27:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b2b:	75 20                	jne    80101b4d <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b30:	8b 00                	mov    (%eax),%eax
80101b32:	89 04 24             	mov    %eax,(%esp)
80101b35:	e8 49 f8 ff ff       	call   80101383 <balloc>
80101b3a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b40:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b43:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b46:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b49:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b50:	e9 b1 00 00 00       	jmp    80101c06 <bmap+0xfc>
  }
  bn -= NDIRECT;
80101b55:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b59:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b5d:	0f 87 97 00 00 00    	ja     80101bfa <bmap+0xf0>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b63:	8b 45 08             	mov    0x8(%ebp),%eax
80101b66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b69:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b6c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b70:	75 19                	jne    80101b8b <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b72:	8b 45 08             	mov    0x8(%ebp),%eax
80101b75:	8b 00                	mov    (%eax),%eax
80101b77:	89 04 24             	mov    %eax,(%esp)
80101b7a:	e8 04 f8 ff ff       	call   80101383 <balloc>
80101b7f:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b82:	8b 45 08             	mov    0x8(%ebp),%eax
80101b85:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b88:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101b8e:	8b 00                	mov    (%eax),%eax
80101b90:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b93:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b97:	89 04 24             	mov    %eax,(%esp)
80101b9a:	e8 07 e6 ff ff       	call   801001a6 <bread>
80101b9f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101ba2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ba5:	83 c0 18             	add    $0x18,%eax
80101ba8:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101bab:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bae:	c1 e0 02             	shl    $0x2,%eax
80101bb1:	03 45 ec             	add    -0x14(%ebp),%eax
80101bb4:	8b 00                	mov    (%eax),%eax
80101bb6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bbd:	75 2b                	jne    80101bea <bmap+0xe0>
      a[bn] = addr = balloc(ip->dev);
80101bbf:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bc2:	c1 e0 02             	shl    $0x2,%eax
80101bc5:	89 c3                	mov    %eax,%ebx
80101bc7:	03 5d ec             	add    -0x14(%ebp),%ebx
80101bca:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcd:	8b 00                	mov    (%eax),%eax
80101bcf:	89 04 24             	mov    %eax,(%esp)
80101bd2:	e8 ac f7 ff ff       	call   80101383 <balloc>
80101bd7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bda:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdd:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be2:	89 04 24             	mov    %eax,(%esp)
80101be5:	e8 d0 1e 00 00       	call   80103aba <log_write>
    }
    brelse(bp);
80101bea:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101bed:	89 04 24             	mov    %eax,(%esp)
80101bf0:	e8 22 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf8:	eb 0c                	jmp    80101c06 <bmap+0xfc>
  }

  panic("bmap: out of range");
80101bfa:	c7 04 24 c2 97 10 80 	movl   $0x801097c2,(%esp)
80101c01:	e8 37 e9 ff ff       	call   8010053d <panic>
}
80101c06:	83 c4 24             	add    $0x24,%esp
80101c09:	5b                   	pop    %ebx
80101c0a:	5d                   	pop    %ebp
80101c0b:	c3                   	ret    

80101c0c <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0c:	55                   	push   %ebp
80101c0d:	89 e5                	mov    %esp,%ebp
80101c0f:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c19:	eb 44                	jmp    80101c5f <itrunc+0x53>
    if(ip->addrs[i]){
80101c1b:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c21:	83 c2 04             	add    $0x4,%edx
80101c24:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c28:	85 c0                	test   %eax,%eax
80101c2a:	74 2f                	je     80101c5b <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2c:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c32:	83 c2 04             	add    $0x4,%edx
80101c35:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c39:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3c:	8b 00                	mov    (%eax),%eax
80101c3e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c42:	89 04 24             	mov    %eax,(%esp)
80101c45:	e8 90 f8 ff ff       	call   801014da <bfree>
      ip->addrs[i] = 0;
80101c4a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c50:	83 c2 04             	add    $0x4,%edx
80101c53:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c5a:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c5b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5f:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c63:	7e b6                	jle    80101c1b <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c65:	8b 45 08             	mov    0x8(%ebp),%eax
80101c68:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c6b:	85 c0                	test   %eax,%eax
80101c6d:	0f 84 8f 00 00 00    	je     80101d02 <itrunc+0xf6>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c73:	8b 45 08             	mov    0x8(%ebp),%eax
80101c76:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c79:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7c:	8b 00                	mov    (%eax),%eax
80101c7e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c82:	89 04 24             	mov    %eax,(%esp)
80101c85:	e8 1c e5 ff ff       	call   801001a6 <bread>
80101c8a:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c90:	83 c0 18             	add    $0x18,%eax
80101c93:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c96:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9d:	eb 2f                	jmp    80101cce <itrunc+0xc2>
      if(a[j])
80101c9f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca2:	c1 e0 02             	shl    $0x2,%eax
80101ca5:	03 45 e8             	add    -0x18(%ebp),%eax
80101ca8:	8b 00                	mov    (%eax),%eax
80101caa:	85 c0                	test   %eax,%eax
80101cac:	74 1c                	je     80101cca <itrunc+0xbe>
        bfree(ip->dev, a[j]);
80101cae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb1:	c1 e0 02             	shl    $0x2,%eax
80101cb4:	03 45 e8             	add    -0x18(%ebp),%eax
80101cb7:	8b 10                	mov    (%eax),%edx
80101cb9:	8b 45 08             	mov    0x8(%ebp),%eax
80101cbc:	8b 00                	mov    (%eax),%eax
80101cbe:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cc2:	89 04 24             	mov    %eax,(%esp)
80101cc5:	e8 10 f8 ff ff       	call   801014da <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cca:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cce:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cd1:	83 f8 7f             	cmp    $0x7f,%eax
80101cd4:	76 c9                	jbe    80101c9f <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101cd6:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101cd9:	89 04 24             	mov    %eax,(%esp)
80101cdc:	e8 36 e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ce1:	8b 45 08             	mov    0x8(%ebp),%eax
80101ce4:	8b 50 4c             	mov    0x4c(%eax),%edx
80101ce7:	8b 45 08             	mov    0x8(%ebp),%eax
80101cea:	8b 00                	mov    (%eax),%eax
80101cec:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cf0:	89 04 24             	mov    %eax,(%esp)
80101cf3:	e8 e2 f7 ff ff       	call   801014da <bfree>
    ip->addrs[NDIRECT] = 0;
80101cf8:	8b 45 08             	mov    0x8(%ebp),%eax
80101cfb:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	89 04 24             	mov    %eax,(%esp)
80101d12:	e8 95 f9 ff ff       	call   801016ac <iupdate>
}
80101d17:	c9                   	leave  
80101d18:	c3                   	ret    

80101d19 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d19:	55                   	push   %ebp
80101d1a:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d1c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d1f:	8b 00                	mov    (%eax),%eax
80101d21:	89 c2                	mov    %eax,%edx
80101d23:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d26:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d29:	8b 45 08             	mov    0x8(%ebp),%eax
80101d2c:	8b 50 04             	mov    0x4(%eax),%edx
80101d2f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d32:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d35:	8b 45 08             	mov    0x8(%ebp),%eax
80101d38:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d3c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3f:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d42:	8b 45 08             	mov    0x8(%ebp),%eax
80101d45:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d49:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d4c:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d50:	8b 45 08             	mov    0x8(%ebp),%eax
80101d53:	8b 50 18             	mov    0x18(%eax),%edx
80101d56:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d59:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d5c:	5d                   	pop    %ebp
80101d5d:	c3                   	ret    

80101d5e <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d5e:	55                   	push   %ebp
80101d5f:	89 e5                	mov    %esp,%ebp
80101d61:	53                   	push   %ebx
80101d62:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d65:	8b 45 08             	mov    0x8(%ebp),%eax
80101d68:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d6c:	66 83 f8 03          	cmp    $0x3,%ax
80101d70:	75 60                	jne    80101dd2 <readi+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d72:	8b 45 08             	mov    0x8(%ebp),%eax
80101d75:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d79:	66 85 c0             	test   %ax,%ax
80101d7c:	78 20                	js     80101d9e <readi+0x40>
80101d7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d81:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d85:	66 83 f8 09          	cmp    $0x9,%ax
80101d89:	7f 13                	jg     80101d9e <readi+0x40>
80101d8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d92:	98                   	cwtl   
80101d93:	8b 04 c5 80 f8 10 80 	mov    -0x7fef0780(,%eax,8),%eax
80101d9a:	85 c0                	test   %eax,%eax
80101d9c:	75 0a                	jne    80101da8 <readi+0x4a>
      return -1;
80101d9e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101da3:	e9 1b 01 00 00       	jmp    80101ec3 <readi+0x165>
    return devsw[ip->major].read(ip, dst, n);
80101da8:	8b 45 08             	mov    0x8(%ebp),%eax
80101dab:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101daf:	98                   	cwtl   
80101db0:	8b 14 c5 80 f8 10 80 	mov    -0x7fef0780(,%eax,8),%edx
80101db7:	8b 45 14             	mov    0x14(%ebp),%eax
80101dba:	89 44 24 08          	mov    %eax,0x8(%esp)
80101dbe:	8b 45 0c             	mov    0xc(%ebp),%eax
80101dc1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101dc5:	8b 45 08             	mov    0x8(%ebp),%eax
80101dc8:	89 04 24             	mov    %eax,(%esp)
80101dcb:	ff d2                	call   *%edx
80101dcd:	e9 f1 00 00 00       	jmp    80101ec3 <readi+0x165>
  }

  if(off > ip->size || off + n < off)
80101dd2:	8b 45 08             	mov    0x8(%ebp),%eax
80101dd5:	8b 40 18             	mov    0x18(%eax),%eax
80101dd8:	3b 45 10             	cmp    0x10(%ebp),%eax
80101ddb:	72 0d                	jb     80101dea <readi+0x8c>
80101ddd:	8b 45 14             	mov    0x14(%ebp),%eax
80101de0:	8b 55 10             	mov    0x10(%ebp),%edx
80101de3:	01 d0                	add    %edx,%eax
80101de5:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de8:	73 0a                	jae    80101df4 <readi+0x96>
    return -1;
80101dea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101def:	e9 cf 00 00 00       	jmp    80101ec3 <readi+0x165>
  if(off + n > ip->size)
80101df4:	8b 45 14             	mov    0x14(%ebp),%eax
80101df7:	8b 55 10             	mov    0x10(%ebp),%edx
80101dfa:	01 c2                	add    %eax,%edx
80101dfc:	8b 45 08             	mov    0x8(%ebp),%eax
80101dff:	8b 40 18             	mov    0x18(%eax),%eax
80101e02:	39 c2                	cmp    %eax,%edx
80101e04:	76 0c                	jbe    80101e12 <readi+0xb4>
    n = ip->size - off;
80101e06:	8b 45 08             	mov    0x8(%ebp),%eax
80101e09:	8b 40 18             	mov    0x18(%eax),%eax
80101e0c:	2b 45 10             	sub    0x10(%ebp),%eax
80101e0f:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e19:	e9 96 00 00 00       	jmp    80101eb4 <readi+0x156>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e1e:	8b 45 10             	mov    0x10(%ebp),%eax
80101e21:	c1 e8 09             	shr    $0x9,%eax
80101e24:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e28:	8b 45 08             	mov    0x8(%ebp),%eax
80101e2b:	89 04 24             	mov    %eax,(%esp)
80101e2e:	e8 d7 fc ff ff       	call   80101b0a <bmap>
80101e33:	8b 55 08             	mov    0x8(%ebp),%edx
80101e36:	8b 12                	mov    (%edx),%edx
80101e38:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e3c:	89 14 24             	mov    %edx,(%esp)
80101e3f:	e8 62 e3 ff ff       	call   801001a6 <bread>
80101e44:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e47:	8b 45 10             	mov    0x10(%ebp),%eax
80101e4a:	89 c2                	mov    %eax,%edx
80101e4c:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101e52:	b8 00 02 00 00       	mov    $0x200,%eax
80101e57:	89 c1                	mov    %eax,%ecx
80101e59:	29 d1                	sub    %edx,%ecx
80101e5b:	89 ca                	mov    %ecx,%edx
80101e5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e60:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e63:	89 cb                	mov    %ecx,%ebx
80101e65:	29 c3                	sub    %eax,%ebx
80101e67:	89 d8                	mov    %ebx,%eax
80101e69:	39 c2                	cmp    %eax,%edx
80101e6b:	0f 46 c2             	cmovbe %edx,%eax
80101e6e:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e71:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e74:	8d 50 18             	lea    0x18(%eax),%edx
80101e77:	8b 45 10             	mov    0x10(%ebp),%eax
80101e7a:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7f:	01 c2                	add    %eax,%edx
80101e81:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e84:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e88:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e8c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e8f:	89 04 24             	mov    %eax,(%esp)
80101e92:	e8 3a 40 00 00       	call   80105ed1 <memmove>
    brelse(bp);
80101e97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e9a:	89 04 24             	mov    %eax,(%esp)
80101e9d:	e8 75 e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ea5:	01 45 f4             	add    %eax,-0xc(%ebp)
80101ea8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eab:	01 45 10             	add    %eax,0x10(%ebp)
80101eae:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb1:	01 45 0c             	add    %eax,0xc(%ebp)
80101eb4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101eb7:	3b 45 14             	cmp    0x14(%ebp),%eax
80101eba:	0f 82 5e ff ff ff    	jb     80101e1e <readi+0xc0>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec0:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101ec3:	83 c4 24             	add    $0x24,%esp
80101ec6:	5b                   	pop    %ebx
80101ec7:	5d                   	pop    %ebp
80101ec8:	c3                   	ret    

80101ec9 <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ec9:	55                   	push   %ebp
80101eca:	89 e5                	mov    %esp,%ebp
80101ecc:	53                   	push   %ebx
80101ecd:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed0:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed3:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed7:	66 83 f8 03          	cmp    $0x3,%ax
80101edb:	75 60                	jne    80101f3d <writei+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edd:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee0:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee4:	66 85 c0             	test   %ax,%ax
80101ee7:	78 20                	js     80101f09 <writei+0x40>
80101ee9:	8b 45 08             	mov    0x8(%ebp),%eax
80101eec:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef0:	66 83 f8 09          	cmp    $0x9,%ax
80101ef4:	7f 13                	jg     80101f09 <writei+0x40>
80101ef6:	8b 45 08             	mov    0x8(%ebp),%eax
80101ef9:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101efd:	98                   	cwtl   
80101efe:	8b 04 c5 84 f8 10 80 	mov    -0x7fef077c(,%eax,8),%eax
80101f05:	85 c0                	test   %eax,%eax
80101f07:	75 0a                	jne    80101f13 <writei+0x4a>
      return -1;
80101f09:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f0e:	e9 46 01 00 00       	jmp    80102059 <writei+0x190>
    return devsw[ip->major].write(ip, src, n);
80101f13:	8b 45 08             	mov    0x8(%ebp),%eax
80101f16:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1a:	98                   	cwtl   
80101f1b:	8b 14 c5 84 f8 10 80 	mov    -0x7fef077c(,%eax,8),%edx
80101f22:	8b 45 14             	mov    0x14(%ebp),%eax
80101f25:	89 44 24 08          	mov    %eax,0x8(%esp)
80101f29:	8b 45 0c             	mov    0xc(%ebp),%eax
80101f2c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f30:	8b 45 08             	mov    0x8(%ebp),%eax
80101f33:	89 04 24             	mov    %eax,(%esp)
80101f36:	ff d2                	call   *%edx
80101f38:	e9 1c 01 00 00       	jmp    80102059 <writei+0x190>
  }

  if(off > ip->size || off + n < off)
80101f3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101f40:	8b 40 18             	mov    0x18(%eax),%eax
80101f43:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f46:	72 0d                	jb     80101f55 <writei+0x8c>
80101f48:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4b:	8b 55 10             	mov    0x10(%ebp),%edx
80101f4e:	01 d0                	add    %edx,%eax
80101f50:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f53:	73 0a                	jae    80101f5f <writei+0x96>
    return -1;
80101f55:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5a:	e9 fa 00 00 00       	jmp    80102059 <writei+0x190>
  if(off + n > MAXFILE*BSIZE)
80101f5f:	8b 45 14             	mov    0x14(%ebp),%eax
80101f62:	8b 55 10             	mov    0x10(%ebp),%edx
80101f65:	01 d0                	add    %edx,%eax
80101f67:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6c:	76 0a                	jbe    80101f78 <writei+0xaf>
    return -1;
80101f6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f73:	e9 e1 00 00 00       	jmp    80102059 <writei+0x190>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f78:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f7f:	e9 a1 00 00 00       	jmp    80102025 <writei+0x15c>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f84:	8b 45 10             	mov    0x10(%ebp),%eax
80101f87:	c1 e8 09             	shr    $0x9,%eax
80101f8a:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80101f91:	89 04 24             	mov    %eax,(%esp)
80101f94:	e8 71 fb ff ff       	call   80101b0a <bmap>
80101f99:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9c:	8b 12                	mov    (%edx),%edx
80101f9e:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa2:	89 14 24             	mov    %edx,(%esp)
80101fa5:	e8 fc e1 ff ff       	call   801001a6 <bread>
80101faa:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fad:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb0:	89 c2                	mov    %eax,%edx
80101fb2:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101fb8:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbd:	89 c1                	mov    %eax,%ecx
80101fbf:	29 d1                	sub    %edx,%ecx
80101fc1:	89 ca                	mov    %ecx,%edx
80101fc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc6:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc9:	89 cb                	mov    %ecx,%ebx
80101fcb:	29 c3                	sub    %eax,%ebx
80101fcd:	89 d8                	mov    %ebx,%eax
80101fcf:	39 c2                	cmp    %eax,%edx
80101fd1:	0f 46 c2             	cmovbe %edx,%eax
80101fd4:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fda:	8d 50 18             	lea    0x18(%eax),%edx
80101fdd:	8b 45 10             	mov    0x10(%ebp),%eax
80101fe0:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fe5:	01 c2                	add    %eax,%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 d4 3e 00 00       	call   80105ed1 <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 b2 1a 00 00       	call   80103aba <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 53 ff ff ff    	jb     80101f84 <writei+0xbb>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18d>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18d>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 56 f6 ff ff       	call   801016ac <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	83 c4 24             	add    $0x24,%esp
8010205c:	5b                   	pop    %ebx
8010205d:	5d                   	pop    %ebp
8010205e:	c3                   	ret    

8010205f <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205f:	55                   	push   %ebp
80102060:	89 e5                	mov    %esp,%ebp
80102062:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102065:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
8010206c:	00 
8010206d:	8b 45 0c             	mov    0xc(%ebp),%eax
80102070:	89 44 24 04          	mov    %eax,0x4(%esp)
80102074:	8b 45 08             	mov    0x8(%ebp),%eax
80102077:	89 04 24             	mov    %eax,(%esp)
8010207a:	e8 f6 3e 00 00       	call   80105f75 <strncmp>
}
8010207f:	c9                   	leave  
80102080:	c3                   	ret    

80102081 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80102081:	55                   	push   %ebp
80102082:	89 e5                	mov    %esp,%ebp
80102084:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102087:	8b 45 08             	mov    0x8(%ebp),%eax
8010208a:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208e:	66 83 f8 01          	cmp    $0x1,%ax
80102092:	74 0c                	je     801020a0 <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102094:	c7 04 24 d5 97 10 80 	movl   $0x801097d5,(%esp)
8010209b:	e8 9d e4 ff ff       	call   8010053d <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
801020a0:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a7:	e9 87 00 00 00       	jmp    80102133 <dirlookup+0xb2>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020ac:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020b3:	00 
801020b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b7:	89 44 24 08          	mov    %eax,0x8(%esp)
801020bb:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020be:	89 44 24 04          	mov    %eax,0x4(%esp)
801020c2:	8b 45 08             	mov    0x8(%ebp),%eax
801020c5:	89 04 24             	mov    %eax,(%esp)
801020c8:	e8 91 fc ff ff       	call   80101d5e <readi>
801020cd:	83 f8 10             	cmp    $0x10,%eax
801020d0:	74 0c                	je     801020de <dirlookup+0x5d>
      panic("dirlink read");
801020d2:	c7 04 24 e7 97 10 80 	movl   $0x801097e7,(%esp)
801020d9:	e8 5f e4 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801020de:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020e2:	66 85 c0             	test   %ax,%ax
801020e5:	74 47                	je     8010212e <dirlookup+0xad>
      continue;
    if(namecmp(name, de.name) == 0){
801020e7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ea:	83 c0 02             	add    $0x2,%eax
801020ed:	89 44 24 04          	mov    %eax,0x4(%esp)
801020f1:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f4:	89 04 24             	mov    %eax,(%esp)
801020f7:	e8 63 ff ff ff       	call   8010205f <namecmp>
801020fc:	85 c0                	test   %eax,%eax
801020fe:	75 2f                	jne    8010212f <dirlookup+0xae>
      // entry matches path element
      if(poff)
80102100:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102104:	74 08                	je     8010210e <dirlookup+0x8d>
        *poff = off;
80102106:	8b 45 10             	mov    0x10(%ebp),%eax
80102109:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210c:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210e:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102112:	0f b7 c0             	movzwl %ax,%eax
80102115:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102118:	8b 45 08             	mov    0x8(%ebp),%eax
8010211b:	8b 00                	mov    (%eax),%eax
8010211d:	8b 55 f0             	mov    -0x10(%ebp),%edx
80102120:	89 54 24 04          	mov    %edx,0x4(%esp)
80102124:	89 04 24             	mov    %eax,(%esp)
80102127:	e8 38 f6 ff ff       	call   80101764 <iget>
8010212c:	eb 19                	jmp    80102147 <dirlookup+0xc6>

  for(off = 0; off < dp->size; off += sizeof(de)){
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      continue;
8010212e:	90                   	nop
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212f:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102133:	8b 45 08             	mov    0x8(%ebp),%eax
80102136:	8b 40 18             	mov    0x18(%eax),%eax
80102139:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010213c:	0f 87 6a ff ff ff    	ja     801020ac <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
80102142:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102147:	c9                   	leave  
80102148:	c3                   	ret    

80102149 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102149:	55                   	push   %ebp
8010214a:	89 e5                	mov    %esp,%ebp
8010214c:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214f:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102156:	00 
80102157:	8b 45 0c             	mov    0xc(%ebp),%eax
8010215a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215e:	8b 45 08             	mov    0x8(%ebp),%eax
80102161:	89 04 24             	mov    %eax,(%esp)
80102164:	e8 18 ff ff ff       	call   80102081 <dirlookup>
80102169:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010216c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80102170:	74 15                	je     80102187 <dirlink+0x3e>
    iput(ip);
80102172:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102175:	89 04 24             	mov    %eax,(%esp)
80102178:	e8 9e f8 ff ff       	call   80101a1b <iput>
    return -1;
8010217d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102182:	e9 b8 00 00 00       	jmp    8010223f <dirlink+0xf6>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102187:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218e:	eb 44                	jmp    801021d4 <dirlink+0x8b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102193:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010219a:	00 
8010219b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219f:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021a2:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a6:	8b 45 08             	mov    0x8(%ebp),%eax
801021a9:	89 04 24             	mov    %eax,(%esp)
801021ac:	e8 ad fb ff ff       	call   80101d5e <readi>
801021b1:	83 f8 10             	cmp    $0x10,%eax
801021b4:	74 0c                	je     801021c2 <dirlink+0x79>
      panic("dirlink read");
801021b6:	c7 04 24 e7 97 10 80 	movl   $0x801097e7,(%esp)
801021bd:	e8 7b e3 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801021c2:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c6:	66 85 c0             	test   %ax,%ax
801021c9:	74 18                	je     801021e3 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021ce:	83 c0 10             	add    $0x10,%eax
801021d1:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d4:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d7:	8b 45 08             	mov    0x8(%ebp),%eax
801021da:	8b 40 18             	mov    0x18(%eax),%eax
801021dd:	39 c2                	cmp    %eax,%edx
801021df:	72 af                	jb     80102190 <dirlink+0x47>
801021e1:	eb 01                	jmp    801021e4 <dirlink+0x9b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      break;
801021e3:	90                   	nop
  }

  strncpy(de.name, name, DIRSIZ);
801021e4:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021eb:	00 
801021ec:	8b 45 0c             	mov    0xc(%ebp),%eax
801021ef:	89 44 24 04          	mov    %eax,0x4(%esp)
801021f3:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f6:	83 c0 02             	add    $0x2,%eax
801021f9:	89 04 24             	mov    %eax,(%esp)
801021fc:	e8 cc 3d 00 00       	call   80105fcd <strncpy>
  de.inum = inum;
80102201:	8b 45 10             	mov    0x10(%ebp),%eax
80102204:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102208:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010220b:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102212:	00 
80102213:	89 44 24 08          	mov    %eax,0x8(%esp)
80102217:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010221a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221e:	8b 45 08             	mov    0x8(%ebp),%eax
80102221:	89 04 24             	mov    %eax,(%esp)
80102224:	e8 a0 fc ff ff       	call   80101ec9 <writei>
80102229:	83 f8 10             	cmp    $0x10,%eax
8010222c:	74 0c                	je     8010223a <dirlink+0xf1>
    panic("dirlink");
8010222e:	c7 04 24 f4 97 10 80 	movl   $0x801097f4,(%esp)
80102235:	e8 03 e3 ff ff       	call   8010053d <panic>
  
  return 0;
8010223a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223f:	c9                   	leave  
80102240:	c3                   	ret    

80102241 <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
80102241:	55                   	push   %ebp
80102242:	89 e5                	mov    %esp,%ebp
80102244:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102247:	eb 04                	jmp    8010224d <skipelem+0xc>
    path++;
80102249:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
8010224d:	8b 45 08             	mov    0x8(%ebp),%eax
80102250:	0f b6 00             	movzbl (%eax),%eax
80102253:	3c 2f                	cmp    $0x2f,%al
80102255:	74 f2                	je     80102249 <skipelem+0x8>
    path++;
  if(*path == 0)
80102257:	8b 45 08             	mov    0x8(%ebp),%eax
8010225a:	0f b6 00             	movzbl (%eax),%eax
8010225d:	84 c0                	test   %al,%al
8010225f:	75 0a                	jne    8010226b <skipelem+0x2a>
    return 0;
80102261:	b8 00 00 00 00       	mov    $0x0,%eax
80102266:	e9 86 00 00 00       	jmp    801022f1 <skipelem+0xb0>
  s = path;
8010226b:	8b 45 08             	mov    0x8(%ebp),%eax
8010226e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
80102271:	eb 04                	jmp    80102277 <skipelem+0x36>
    path++;
80102273:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102277:	8b 45 08             	mov    0x8(%ebp),%eax
8010227a:	0f b6 00             	movzbl (%eax),%eax
8010227d:	3c 2f                	cmp    $0x2f,%al
8010227f:	74 0a                	je     8010228b <skipelem+0x4a>
80102281:	8b 45 08             	mov    0x8(%ebp),%eax
80102284:	0f b6 00             	movzbl (%eax),%eax
80102287:	84 c0                	test   %al,%al
80102289:	75 e8                	jne    80102273 <skipelem+0x32>
    path++;
  len = path - s;
8010228b:	8b 55 08             	mov    0x8(%ebp),%edx
8010228e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102291:	89 d1                	mov    %edx,%ecx
80102293:	29 c1                	sub    %eax,%ecx
80102295:	89 c8                	mov    %ecx,%eax
80102297:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
8010229a:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
8010229e:	7e 1c                	jle    801022bc <skipelem+0x7b>
    memmove(name, s, DIRSIZ);
801022a0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a7:	00 
801022a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022ab:	89 44 24 04          	mov    %eax,0x4(%esp)
801022af:	8b 45 0c             	mov    0xc(%ebp),%eax
801022b2:	89 04 24             	mov    %eax,(%esp)
801022b5:	e8 17 3c 00 00       	call   80105ed1 <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022ba:	eb 28                	jmp    801022e4 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022bc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022bf:	89 44 24 08          	mov    %eax,0x8(%esp)
801022c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c6:	89 44 24 04          	mov    %eax,0x4(%esp)
801022ca:	8b 45 0c             	mov    0xc(%ebp),%eax
801022cd:	89 04 24             	mov    %eax,(%esp)
801022d0:	e8 fc 3b 00 00       	call   80105ed1 <memmove>
    name[len] = 0;
801022d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022d8:	03 45 0c             	add    0xc(%ebp),%eax
801022db:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022de:	eb 04                	jmp    801022e4 <skipelem+0xa3>
    path++;
801022e0:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e4:	8b 45 08             	mov    0x8(%ebp),%eax
801022e7:	0f b6 00             	movzbl (%eax),%eax
801022ea:	3c 2f                	cmp    $0x2f,%al
801022ec:	74 f2                	je     801022e0 <skipelem+0x9f>
    path++;
  return path;
801022ee:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022f1:	c9                   	leave  
801022f2:	c3                   	ret    

801022f3 <namex>:
// Look up and return the inode for a path name.
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022f3:	55                   	push   %ebp
801022f4:	89 e5                	mov    %esp,%ebp
801022f6:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;
  if(*path == '/')
801022f9:	8b 45 08             	mov    0x8(%ebp),%eax
801022fc:	0f b6 00             	movzbl (%eax),%eax
801022ff:	3c 2f                	cmp    $0x2f,%al
80102301:	75 1c                	jne    8010231f <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
80102303:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010230a:	00 
8010230b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102312:	e8 4d f4 ff ff       	call   80101764 <iget>
80102317:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
8010231a:	e9 af 00 00 00       	jmp    801023ce <namex+0xdb>
{
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102325:	8b 40 68             	mov    0x68(%eax),%eax
80102328:	89 04 24             	mov    %eax,(%esp)
8010232b:	e8 06 f5 ff ff       	call   80101836 <idup>
80102330:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while((path = skipelem(path, name)) != 0){
80102333:	e9 96 00 00 00       	jmp    801023ce <namex+0xdb>
    ilock(ip);
80102338:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010233b:	89 04 24             	mov    %eax,(%esp)
8010233e:	e8 25 f5 ff ff       	call   80101868 <ilock>
    if(ip->type != T_DIR){
80102343:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102346:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010234a:	66 83 f8 01          	cmp    $0x1,%ax
8010234e:	74 15                	je     80102365 <namex+0x72>
      iunlockput(ip);
80102350:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102353:	89 04 24             	mov    %eax,(%esp)
80102356:	e8 91 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
8010235b:	b8 00 00 00 00       	mov    $0x0,%eax
80102360:	e9 a3 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102365:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102369:	74 1d                	je     80102388 <namex+0x95>
8010236b:	8b 45 08             	mov    0x8(%ebp),%eax
8010236e:	0f b6 00             	movzbl (%eax),%eax
80102371:	84 c0                	test   %al,%al
80102373:	75 13                	jne    80102388 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102375:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102378:	89 04 24             	mov    %eax,(%esp)
8010237b:	e8 36 f6 ff ff       	call   801019b6 <iunlock>
      return ip;
80102380:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102383:	e9 80 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102388:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238f:	00 
80102390:	8b 45 10             	mov    0x10(%ebp),%eax
80102393:	89 44 24 04          	mov    %eax,0x4(%esp)
80102397:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010239a:	89 04 24             	mov    %eax,(%esp)
8010239d:	e8 df fc ff ff       	call   80102081 <dirlookup>
801023a2:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a9:	75 12                	jne    801023bd <namex+0xca>
      iunlockput(ip);
801023ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023ae:	89 04 24             	mov    %eax,(%esp)
801023b1:	e8 36 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
801023b6:	b8 00 00 00 00       	mov    $0x0,%eax
801023bb:	eb 4b                	jmp    80102408 <namex+0x115>
    }
    iunlockput(ip);
801023bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023c0:	89 04 24             	mov    %eax,(%esp)
801023c3:	e8 24 f7 ff ff       	call   80101aec <iunlockput>
    ip = next;
801023c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023cb:	89 45 f4             	mov    %eax,-0xc(%ebp)
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
801023ce:	8b 45 10             	mov    0x10(%ebp),%eax
801023d1:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d5:	8b 45 08             	mov    0x8(%ebp),%eax
801023d8:	89 04 24             	mov    %eax,(%esp)
801023db:	e8 61 fe ff ff       	call   80102241 <skipelem>
801023e0:	89 45 08             	mov    %eax,0x8(%ebp)
801023e3:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e7:	0f 85 4b ff ff ff    	jne    80102338 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023ed:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023f1:	74 12                	je     80102405 <namex+0x112>
    iput(ip);
801023f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f6:	89 04 24             	mov    %eax,(%esp)
801023f9:	e8 1d f6 ff ff       	call   80101a1b <iput>
    return 0;
801023fe:	b8 00 00 00 00       	mov    $0x0,%eax
80102403:	eb 03                	jmp    80102408 <namex+0x115>
  }
  return ip;
80102405:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102408:	c9                   	leave  
80102409:	c3                   	ret    

8010240a <namei>:

struct inode*
namei(char *path)
{
8010240a:	55                   	push   %ebp
8010240b:	89 e5                	mov    %esp,%ebp
8010240d:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102410:	8d 45 ea             	lea    -0x16(%ebp),%eax
80102413:	89 44 24 08          	mov    %eax,0x8(%esp)
80102417:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241e:	00 
8010241f:	8b 45 08             	mov    0x8(%ebp),%eax
80102422:	89 04 24             	mov    %eax,(%esp)
80102425:	e8 c9 fe ff ff       	call   801022f3 <namex>
}
8010242a:	c9                   	leave  
8010242b:	c3                   	ret    

8010242c <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
8010242c:	55                   	push   %ebp
8010242d:	89 e5                	mov    %esp,%ebp
8010242f:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
80102432:	8b 45 0c             	mov    0xc(%ebp),%eax
80102435:	89 44 24 08          	mov    %eax,0x8(%esp)
80102439:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102440:	00 
80102441:	8b 45 08             	mov    0x8(%ebp),%eax
80102444:	89 04 24             	mov    %eax,(%esp)
80102447:	e8 a7 fe ff ff       	call   801022f3 <namex>
}
8010244c:	c9                   	leave  
8010244d:	c3                   	ret    
	...

80102450 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102450:	55                   	push   %ebp
80102451:	89 e5                	mov    %esp,%ebp
80102453:	53                   	push   %ebx
80102454:	83 ec 14             	sub    $0x14,%esp
80102457:	8b 45 08             	mov    0x8(%ebp),%eax
8010245a:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010245e:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102462:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102466:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
8010246a:	ec                   	in     (%dx),%al
8010246b:	89 c3                	mov    %eax,%ebx
8010246d:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102470:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102474:	83 c4 14             	add    $0x14,%esp
80102477:	5b                   	pop    %ebx
80102478:	5d                   	pop    %ebp
80102479:	c3                   	ret    

8010247a <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
8010247a:	55                   	push   %ebp
8010247b:	89 e5                	mov    %esp,%ebp
8010247d:	57                   	push   %edi
8010247e:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010247f:	8b 55 08             	mov    0x8(%ebp),%edx
80102482:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102485:	8b 45 10             	mov    0x10(%ebp),%eax
80102488:	89 cb                	mov    %ecx,%ebx
8010248a:	89 df                	mov    %ebx,%edi
8010248c:	89 c1                	mov    %eax,%ecx
8010248e:	fc                   	cld    
8010248f:	f3 6d                	rep insl (%dx),%es:(%edi)
80102491:	89 c8                	mov    %ecx,%eax
80102493:	89 fb                	mov    %edi,%ebx
80102495:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102498:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
8010249b:	5b                   	pop    %ebx
8010249c:	5f                   	pop    %edi
8010249d:	5d                   	pop    %ebp
8010249e:	c3                   	ret    

8010249f <outb>:

static inline void
outb(ushort port, uchar data)
{
8010249f:	55                   	push   %ebp
801024a0:	89 e5                	mov    %esp,%ebp
801024a2:	83 ec 08             	sub    $0x8,%esp
801024a5:	8b 55 08             	mov    0x8(%ebp),%edx
801024a8:	8b 45 0c             	mov    0xc(%ebp),%eax
801024ab:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801024af:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801024b2:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024b6:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024ba:	ee                   	out    %al,(%dx)
}
801024bb:	c9                   	leave  
801024bc:	c3                   	ret    

801024bd <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024bd:	55                   	push   %ebp
801024be:	89 e5                	mov    %esp,%ebp
801024c0:	56                   	push   %esi
801024c1:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024c2:	8b 55 08             	mov    0x8(%ebp),%edx
801024c5:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024c8:	8b 45 10             	mov    0x10(%ebp),%eax
801024cb:	89 cb                	mov    %ecx,%ebx
801024cd:	89 de                	mov    %ebx,%esi
801024cf:	89 c1                	mov    %eax,%ecx
801024d1:	fc                   	cld    
801024d2:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024d4:	89 c8                	mov    %ecx,%eax
801024d6:	89 f3                	mov    %esi,%ebx
801024d8:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024db:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024de:	5b                   	pop    %ebx
801024df:	5e                   	pop    %esi
801024e0:	5d                   	pop    %ebp
801024e1:	c3                   	ret    

801024e2 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
801024e2:	55                   	push   %ebp
801024e3:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
801024e5:	fb                   	sti    
}
801024e6:	5d                   	pop    %ebp
801024e7:	c3                   	ret    

801024e8 <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024e8:	55                   	push   %ebp
801024e9:	89 e5                	mov    %esp,%ebp
801024eb:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024ee:	90                   	nop
801024ef:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024f6:	e8 55 ff ff ff       	call   80102450 <inb>
801024fb:	0f b6 c0             	movzbl %al,%eax
801024fe:	89 45 fc             	mov    %eax,-0x4(%ebp)
80102501:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102504:	25 c0 00 00 00       	and    $0xc0,%eax
80102509:	83 f8 40             	cmp    $0x40,%eax
8010250c:	75 e1                	jne    801024ef <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
8010250e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102512:	74 11                	je     80102525 <idewait+0x3d>
80102514:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102517:	83 e0 21             	and    $0x21,%eax
8010251a:	85 c0                	test   %eax,%eax
8010251c:	74 07                	je     80102525 <idewait+0x3d>
    return -1;
8010251e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102523:	eb 05                	jmp    8010252a <idewait+0x42>
  return 0;
80102525:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010252a:	c9                   	leave  
8010252b:	c3                   	ret    

8010252c <ideinit>:

void
ideinit(void)
{
8010252c:	55                   	push   %ebp
8010252d:	89 e5                	mov    %esp,%ebp
8010252f:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102532:	c7 44 24 04 fc 97 10 	movl   $0x801097fc,0x4(%esp)
80102539:	80 
8010253a:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
80102541:	e8 10 36 00 00       	call   80105b56 <initlock>
  picenable(IRQ_IDE);
80102546:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010254d:	e8 6b 1d 00 00       	call   801042bd <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102552:	a1 20 77 12 80       	mov    0x80127720,%eax
80102557:	83 e8 01             	sub    $0x1,%eax
8010255a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010255e:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102565:	e8 10 04 00 00       	call   8010297a <ioapicenable>
  idewait(0);
8010256a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102571:	e8 72 ff ff ff       	call   801024e8 <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
80102576:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
8010257d:	00 
8010257e:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102585:	e8 15 ff ff ff       	call   8010249f <outb>
  for(i=0; i<1000; i++){
8010258a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102591:	eb 20                	jmp    801025b3 <ideinit+0x87>
    if(inb(0x1f7) != 0){
80102593:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
8010259a:	e8 b1 fe ff ff       	call   80102450 <inb>
8010259f:	84 c0                	test   %al,%al
801025a1:	74 0c                	je     801025af <ideinit+0x83>
      havedisk1 = 1;
801025a3:	c7 05 58 c6 10 80 01 	movl   $0x1,0x8010c658
801025aa:	00 00 00 
      break;
801025ad:	eb 0d                	jmp    801025bc <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
801025af:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801025b3:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025ba:	7e d7                	jle    80102593 <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025bc:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025c3:	00 
801025c4:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025cb:	e8 cf fe ff ff       	call   8010249f <outb>
}
801025d0:	c9                   	leave  
801025d1:	c3                   	ret    

801025d2 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025d2:	55                   	push   %ebp
801025d3:	89 e5                	mov    %esp,%ebp
801025d5:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025d8:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025dc:	75 0c                	jne    801025ea <idestart+0x18>
    panic("idestart");
801025de:	c7 04 24 00 98 10 80 	movl   $0x80109800,(%esp)
801025e5:	e8 53 df ff ff       	call   8010053d <panic>

  idewait(0);
801025ea:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025f1:	e8 f2 fe ff ff       	call   801024e8 <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025f6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025fd:	00 
801025fe:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
80102605:	e8 95 fe ff ff       	call   8010249f <outb>
  outb(0x1f2, 1);  // number of sectors
8010260a:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102611:	00 
80102612:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102619:	e8 81 fe ff ff       	call   8010249f <outb>
  outb(0x1f3, b->sector & 0xff);
8010261e:	8b 45 08             	mov    0x8(%ebp),%eax
80102621:	8b 40 08             	mov    0x8(%eax),%eax
80102624:	0f b6 c0             	movzbl %al,%eax
80102627:	89 44 24 04          	mov    %eax,0x4(%esp)
8010262b:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
80102632:	e8 68 fe ff ff       	call   8010249f <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
80102637:	8b 45 08             	mov    0x8(%ebp),%eax
8010263a:	8b 40 08             	mov    0x8(%eax),%eax
8010263d:	c1 e8 08             	shr    $0x8,%eax
80102640:	0f b6 c0             	movzbl %al,%eax
80102643:	89 44 24 04          	mov    %eax,0x4(%esp)
80102647:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
8010264e:	e8 4c fe ff ff       	call   8010249f <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
80102653:	8b 45 08             	mov    0x8(%ebp),%eax
80102656:	8b 40 08             	mov    0x8(%eax),%eax
80102659:	c1 e8 10             	shr    $0x10,%eax
8010265c:	0f b6 c0             	movzbl %al,%eax
8010265f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102663:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
8010266a:	e8 30 fe ff ff       	call   8010249f <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
8010266f:	8b 45 08             	mov    0x8(%ebp),%eax
80102672:	8b 40 04             	mov    0x4(%eax),%eax
80102675:	83 e0 01             	and    $0x1,%eax
80102678:	89 c2                	mov    %eax,%edx
8010267a:	c1 e2 04             	shl    $0x4,%edx
8010267d:	8b 45 08             	mov    0x8(%ebp),%eax
80102680:	8b 40 08             	mov    0x8(%eax),%eax
80102683:	c1 e8 18             	shr    $0x18,%eax
80102686:	83 e0 0f             	and    $0xf,%eax
80102689:	09 d0                	or     %edx,%eax
8010268b:	83 c8 e0             	or     $0xffffffe0,%eax
8010268e:	0f b6 c0             	movzbl %al,%eax
80102691:	89 44 24 04          	mov    %eax,0x4(%esp)
80102695:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010269c:	e8 fe fd ff ff       	call   8010249f <outb>
  if(b->flags & B_DIRTY){
801026a1:	8b 45 08             	mov    0x8(%ebp),%eax
801026a4:	8b 00                	mov    (%eax),%eax
801026a6:	83 e0 04             	and    $0x4,%eax
801026a9:	85 c0                	test   %eax,%eax
801026ab:	74 34                	je     801026e1 <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
801026ad:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
801026b4:	00 
801026b5:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026bc:	e8 de fd ff ff       	call   8010249f <outb>
    outsl(0x1f0, b->data, 512/4);
801026c1:	8b 45 08             	mov    0x8(%ebp),%eax
801026c4:	83 c0 18             	add    $0x18,%eax
801026c7:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026ce:	00 
801026cf:	89 44 24 04          	mov    %eax,0x4(%esp)
801026d3:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026da:	e8 de fd ff ff       	call   801024bd <outsl>
801026df:	eb 14                	jmp    801026f5 <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026e1:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026e8:	00 
801026e9:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026f0:	e8 aa fd ff ff       	call   8010249f <outb>
  }
}
801026f5:	c9                   	leave  
801026f6:	c3                   	ret    

801026f7 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026f7:	55                   	push   %ebp
801026f8:	89 e5                	mov    %esp,%ebp
801026fa:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026fd:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
80102704:	e8 6e 34 00 00       	call   80105b77 <acquire>
  if((b = idequeue) == 0){
80102709:	a1 54 c6 10 80       	mov    0x8010c654,%eax
8010270e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102711:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102715:	75 11                	jne    80102728 <ideintr+0x31>
    release(&idelock);
80102717:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
8010271e:	e8 ef 34 00 00       	call   80105c12 <release>
    // cprintf("spurious IDE interrupt\n");
    return;
80102723:	e9 85 00 00 00       	jmp    801027ad <ideintr+0xb6>
  }
  idequeue = b->qnext;
80102728:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010272b:	8b 40 14             	mov    0x14(%eax),%eax
8010272e:	a3 54 c6 10 80       	mov    %eax,0x8010c654

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
80102733:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102736:	8b 00                	mov    (%eax),%eax
80102738:	83 e0 04             	and    $0x4,%eax
8010273b:	85 c0                	test   %eax,%eax
8010273d:	75 2e                	jne    8010276d <ideintr+0x76>
8010273f:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102746:	e8 9d fd ff ff       	call   801024e8 <idewait>
8010274b:	85 c0                	test   %eax,%eax
8010274d:	78 1e                	js     8010276d <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
8010274f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102752:	83 c0 18             	add    $0x18,%eax
80102755:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
8010275c:	00 
8010275d:	89 44 24 04          	mov    %eax,0x4(%esp)
80102761:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
80102768:	e8 0d fd ff ff       	call   8010247a <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
8010276d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102770:	8b 00                	mov    (%eax),%eax
80102772:	89 c2                	mov    %eax,%edx
80102774:	83 ca 02             	or     $0x2,%edx
80102777:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010277a:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
8010277c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010277f:	8b 00                	mov    (%eax),%eax
80102781:	89 c2                	mov    %eax,%edx
80102783:	83 e2 fb             	and    $0xfffffffb,%edx
80102786:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102789:	89 10                	mov    %edx,(%eax)
  //wakeup(b);
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
8010278b:	a1 54 c6 10 80       	mov    0x8010c654,%eax
80102790:	85 c0                	test   %eax,%eax
80102792:	74 0d                	je     801027a1 <ideintr+0xaa>
    idestart(idequeue);
80102794:	a1 54 c6 10 80       	mov    0x8010c654,%eax
80102799:	89 04 24             	mov    %eax,(%esp)
8010279c:	e8 31 fe ff ff       	call   801025d2 <idestart>

  release(&idelock);
801027a1:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
801027a8:	e8 65 34 00 00       	call   80105c12 <release>
}
801027ad:	c9                   	leave  
801027ae:	c3                   	ret    

801027af <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027af:	55                   	push   %ebp
801027b0:	89 e5                	mov    %esp,%ebp
801027b2:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027b5:	8b 45 08             	mov    0x8(%ebp),%eax
801027b8:	8b 00                	mov    (%eax),%eax
801027ba:	83 e0 01             	and    $0x1,%eax
801027bd:	85 c0                	test   %eax,%eax
801027bf:	75 0c                	jne    801027cd <iderw+0x1e>
    panic("iderw: buf not busy");
801027c1:	c7 04 24 09 98 10 80 	movl   $0x80109809,(%esp)
801027c8:	e8 70 dd ff ff       	call   8010053d <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027cd:	8b 45 08             	mov    0x8(%ebp),%eax
801027d0:	8b 00                	mov    (%eax),%eax
801027d2:	83 e0 06             	and    $0x6,%eax
801027d5:	83 f8 02             	cmp    $0x2,%eax
801027d8:	75 0c                	jne    801027e6 <iderw+0x37>
    panic("iderw: nothing to do");
801027da:	c7 04 24 1d 98 10 80 	movl   $0x8010981d,(%esp)
801027e1:	e8 57 dd ff ff       	call   8010053d <panic>
  if(b->dev != 0 && !havedisk1)
801027e6:	8b 45 08             	mov    0x8(%ebp),%eax
801027e9:	8b 40 04             	mov    0x4(%eax),%eax
801027ec:	85 c0                	test   %eax,%eax
801027ee:	74 15                	je     80102805 <iderw+0x56>
801027f0:	a1 58 c6 10 80       	mov    0x8010c658,%eax
801027f5:	85 c0                	test   %eax,%eax
801027f7:	75 0c                	jne    80102805 <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027f9:	c7 04 24 32 98 10 80 	movl   $0x80109832,(%esp)
80102800:	e8 38 dd ff ff       	call   8010053d <panic>

  acquire(&idelock);  //DOC: acquire-lock
80102805:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
8010280c:	e8 66 33 00 00       	call   80105b77 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
80102811:	8b 45 08             	mov    0x8(%ebp),%eax
80102814:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC: insert-queue
8010281b:	c7 45 f4 54 c6 10 80 	movl   $0x8010c654,-0xc(%ebp)
80102822:	eb 0b                	jmp    8010282f <iderw+0x80>
80102824:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102827:	8b 00                	mov    (%eax),%eax
80102829:	83 c0 14             	add    $0x14,%eax
8010282c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010282f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102832:	8b 00                	mov    (%eax),%eax
80102834:	85 c0                	test   %eax,%eax
80102836:	75 ec                	jne    80102824 <iderw+0x75>
    ;
  *pp = b;
80102838:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010283b:	8b 55 08             	mov    0x8(%ebp),%edx
8010283e:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
80102840:	a1 54 c6 10 80       	mov    0x8010c654,%eax
80102845:	3b 45 08             	cmp    0x8(%ebp),%eax
80102848:	75 2c                	jne    80102876 <iderw+0xc7>
    idestart(b);
8010284a:	8b 45 08             	mov    0x8(%ebp),%eax
8010284d:	89 04 24             	mov    %eax,(%esp)
80102850:	e8 7d fd ff ff       	call   801025d2 <idestart>
  
  // Wait for request to finish.

    while((b->flags & (B_VALID|B_DIRTY)) != B_VALID) 
80102855:	eb 1f                	jmp    80102876 <iderw+0xc7>
    {
	release(&idelock);
80102857:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
8010285e:	e8 af 33 00 00       	call   80105c12 <release>
	sti();
80102863:	e8 7a fc ff ff       	call   801024e2 <sti>
	acquire(&idelock); 
80102868:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
8010286f:	e8 03 33 00 00       	call   80105b77 <acquire>
80102874:	eb 01                	jmp    80102877 <iderw+0xc8>
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.

    while((b->flags & (B_VALID|B_DIRTY)) != B_VALID) 
80102876:	90                   	nop
80102877:	8b 45 08             	mov    0x8(%ebp),%eax
8010287a:	8b 00                	mov    (%eax),%eax
8010287c:	83 e0 06             	and    $0x6,%eax
8010287f:	83 f8 02             	cmp    $0x2,%eax
80102882:	75 d3                	jne    80102857 <iderw+0xa8>
	release(&idelock);
	sti();
	acquire(&idelock); 
    }
    
    release(&idelock);
80102884:	c7 04 24 20 c6 10 80 	movl   $0x8010c620,(%esp)
8010288b:	e8 82 33 00 00       	call   80105c12 <release>
}
80102890:	c9                   	leave  
80102891:	c3                   	ret    
	...

80102894 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102894:	55                   	push   %ebp
80102895:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102897:	a1 b4 08 11 80       	mov    0x801108b4,%eax
8010289c:	8b 55 08             	mov    0x8(%ebp),%edx
8010289f:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
801028a1:	a1 b4 08 11 80       	mov    0x801108b4,%eax
801028a6:	8b 40 10             	mov    0x10(%eax),%eax
}
801028a9:	5d                   	pop    %ebp
801028aa:	c3                   	ret    

801028ab <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
801028ab:	55                   	push   %ebp
801028ac:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
801028ae:	a1 b4 08 11 80       	mov    0x801108b4,%eax
801028b3:	8b 55 08             	mov    0x8(%ebp),%edx
801028b6:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
801028b8:	a1 b4 08 11 80       	mov    0x801108b4,%eax
801028bd:	8b 55 0c             	mov    0xc(%ebp),%edx
801028c0:	89 50 10             	mov    %edx,0x10(%eax)
}
801028c3:	5d                   	pop    %ebp
801028c4:	c3                   	ret    

801028c5 <ioapicinit>:

void
ioapicinit(void)
{
801028c5:	55                   	push   %ebp
801028c6:	89 e5                	mov    %esp,%ebp
801028c8:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028cb:	a1 24 71 12 80       	mov    0x80127124,%eax
801028d0:	85 c0                	test   %eax,%eax
801028d2:	0f 84 9f 00 00 00    	je     80102977 <ioapicinit+0xb2>
    return;

  ioapic = (volatile struct ioapic*)IOAPIC;
801028d8:	c7 05 b4 08 11 80 00 	movl   $0xfec00000,0x801108b4
801028df:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028e2:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028e9:	e8 a6 ff ff ff       	call   80102894 <ioapicread>
801028ee:	c1 e8 10             	shr    $0x10,%eax
801028f1:	25 ff 00 00 00       	and    $0xff,%eax
801028f6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028f9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102900:	e8 8f ff ff ff       	call   80102894 <ioapicread>
80102905:	c1 e8 18             	shr    $0x18,%eax
80102908:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
8010290b:	0f b6 05 20 71 12 80 	movzbl 0x80127120,%eax
80102912:	0f b6 c0             	movzbl %al,%eax
80102915:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80102918:	74 0c                	je     80102926 <ioapicinit+0x61>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
8010291a:	c7 04 24 50 98 10 80 	movl   $0x80109850,(%esp)
80102921:	e8 7b da ff ff       	call   801003a1 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102926:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010292d:	eb 3e                	jmp    8010296d <ioapicinit+0xa8>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
8010292f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102932:	83 c0 20             	add    $0x20,%eax
80102935:	0d 00 00 01 00       	or     $0x10000,%eax
8010293a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010293d:	83 c2 08             	add    $0x8,%edx
80102940:	01 d2                	add    %edx,%edx
80102942:	89 44 24 04          	mov    %eax,0x4(%esp)
80102946:	89 14 24             	mov    %edx,(%esp)
80102949:	e8 5d ff ff ff       	call   801028ab <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
8010294e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102951:	83 c0 08             	add    $0x8,%eax
80102954:	01 c0                	add    %eax,%eax
80102956:	83 c0 01             	add    $0x1,%eax
80102959:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102960:	00 
80102961:	89 04 24             	mov    %eax,(%esp)
80102964:	e8 42 ff ff ff       	call   801028ab <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102969:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010296d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102970:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102973:	7e ba                	jle    8010292f <ioapicinit+0x6a>
80102975:	eb 01                	jmp    80102978 <ioapicinit+0xb3>
ioapicinit(void)
{
  int i, id, maxintr;

  if(!ismp)
    return;
80102977:	90                   	nop
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
80102978:	c9                   	leave  
80102979:	c3                   	ret    

8010297a <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010297a:	55                   	push   %ebp
8010297b:	89 e5                	mov    %esp,%ebp
8010297d:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102980:	a1 24 71 12 80       	mov    0x80127124,%eax
80102985:	85 c0                	test   %eax,%eax
80102987:	74 39                	je     801029c2 <ioapicenable+0x48>
    return;

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
80102989:	8b 45 08             	mov    0x8(%ebp),%eax
8010298c:	83 c0 20             	add    $0x20,%eax
8010298f:	8b 55 08             	mov    0x8(%ebp),%edx
80102992:	83 c2 08             	add    $0x8,%edx
80102995:	01 d2                	add    %edx,%edx
80102997:	89 44 24 04          	mov    %eax,0x4(%esp)
8010299b:	89 14 24             	mov    %edx,(%esp)
8010299e:	e8 08 ff ff ff       	call   801028ab <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
801029a3:	8b 45 0c             	mov    0xc(%ebp),%eax
801029a6:	c1 e0 18             	shl    $0x18,%eax
801029a9:	8b 55 08             	mov    0x8(%ebp),%edx
801029ac:	83 c2 08             	add    $0x8,%edx
801029af:	01 d2                	add    %edx,%edx
801029b1:	83 c2 01             	add    $0x1,%edx
801029b4:	89 44 24 04          	mov    %eax,0x4(%esp)
801029b8:	89 14 24             	mov    %edx,(%esp)
801029bb:	e8 eb fe ff ff       	call   801028ab <ioapicwrite>
801029c0:	eb 01                	jmp    801029c3 <ioapicenable+0x49>

void
ioapicenable(int irq, int cpunum)
{
  if(!ismp)
    return;
801029c2:	90                   	nop
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
}
801029c3:	c9                   	leave  
801029c4:	c3                   	ret    
801029c5:	00 00                	add    %al,(%eax)
	...

801029c8 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029c8:	55                   	push   %ebp
801029c9:	89 e5                	mov    %esp,%ebp
801029cb:	8b 45 08             	mov    0x8(%ebp),%eax
801029ce:	05 00 00 00 80       	add    $0x80000000,%eax
801029d3:	5d                   	pop    %ebp
801029d4:	c3                   	ret    

801029d5 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801029d5:	55                   	push   %ebp
801029d6:	89 e5                	mov    %esp,%ebp
801029d8:	8b 45 08             	mov    0x8(%ebp),%eax
801029db:	05 00 00 00 80       	add    $0x80000000,%eax
801029e0:	5d                   	pop    %ebp
801029e1:	c3                   	ret    

801029e2 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029e2:	55                   	push   %ebp
801029e3:	89 e5                	mov    %esp,%ebp
801029e5:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029e8:	c7 44 24 04 84 98 10 	movl   $0x80109884,0x4(%esp)
801029ef:	80 
801029f0:	c7 04 24 c0 08 11 80 	movl   $0x801108c0,(%esp)
801029f7:	e8 5a 31 00 00       	call   80105b56 <initlock>
  kmem.use_lock = 0;
801029fc:	c7 05 f4 08 11 80 00 	movl   $0x0,0x801108f4
80102a03:	00 00 00 
  freerange(vstart, vend);
80102a06:	8b 45 0c             	mov    0xc(%ebp),%eax
80102a09:	89 44 24 04          	mov    %eax,0x4(%esp)
80102a0d:	8b 45 08             	mov    0x8(%ebp),%eax
80102a10:	89 04 24             	mov    %eax,(%esp)
80102a13:	e8 26 00 00 00       	call   80102a3e <freerange>
}
80102a18:	c9                   	leave  
80102a19:	c3                   	ret    

80102a1a <kinit2>:

void
kinit2(void *vstart, void *vend)
{
80102a1a:	55                   	push   %ebp
80102a1b:	89 e5                	mov    %esp,%ebp
80102a1d:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
80102a20:	8b 45 0c             	mov    0xc(%ebp),%eax
80102a23:	89 44 24 04          	mov    %eax,0x4(%esp)
80102a27:	8b 45 08             	mov    0x8(%ebp),%eax
80102a2a:	89 04 24             	mov    %eax,(%esp)
80102a2d:	e8 0c 00 00 00       	call   80102a3e <freerange>
  kmem.use_lock = 1;
80102a32:	c7 05 f4 08 11 80 01 	movl   $0x1,0x801108f4
80102a39:	00 00 00 
}
80102a3c:	c9                   	leave  
80102a3d:	c3                   	ret    

80102a3e <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a3e:	55                   	push   %ebp
80102a3f:	89 e5                	mov    %esp,%ebp
80102a41:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a44:	8b 45 08             	mov    0x8(%ebp),%eax
80102a47:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a4c:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a51:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a54:	eb 12                	jmp    80102a68 <freerange+0x2a>
    kfree(p);
80102a56:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a59:	89 04 24             	mov    %eax,(%esp)
80102a5c:	e8 16 00 00 00       	call   80102a77 <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a61:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a68:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a6b:	05 00 10 00 00       	add    $0x1000,%eax
80102a70:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a73:	76 e1                	jbe    80102a56 <freerange+0x18>
    kfree(p);
}
80102a75:	c9                   	leave  
80102a76:	c3                   	ret    

80102a77 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a77:	55                   	push   %ebp
80102a78:	89 e5                	mov    %esp,%ebp
80102a7a:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80102a80:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a85:	85 c0                	test   %eax,%eax
80102a87:	75 1b                	jne    80102aa4 <kfree+0x2d>
80102a89:	81 7d 08 1c a4 12 80 	cmpl   $0x8012a41c,0x8(%ebp)
80102a90:	72 12                	jb     80102aa4 <kfree+0x2d>
80102a92:	8b 45 08             	mov    0x8(%ebp),%eax
80102a95:	89 04 24             	mov    %eax,(%esp)
80102a98:	e8 2b ff ff ff       	call   801029c8 <v2p>
80102a9d:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102aa2:	76 0c                	jbe    80102ab0 <kfree+0x39>
    panic("kfree");
80102aa4:	c7 04 24 89 98 10 80 	movl   $0x80109889,(%esp)
80102aab:	e8 8d da ff ff       	call   8010053d <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102ab0:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102ab7:	00 
80102ab8:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102abf:	00 
80102ac0:	8b 45 08             	mov    0x8(%ebp),%eax
80102ac3:	89 04 24             	mov    %eax,(%esp)
80102ac6:	e8 33 33 00 00       	call   80105dfe <memset>

  if(kmem.use_lock)
80102acb:	a1 f4 08 11 80       	mov    0x801108f4,%eax
80102ad0:	85 c0                	test   %eax,%eax
80102ad2:	74 0c                	je     80102ae0 <kfree+0x69>
    acquire(&kmem.lock);
80102ad4:	c7 04 24 c0 08 11 80 	movl   $0x801108c0,(%esp)
80102adb:	e8 97 30 00 00       	call   80105b77 <acquire>
  r = (struct run*)v;
80102ae0:	8b 45 08             	mov    0x8(%ebp),%eax
80102ae3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102ae6:	8b 15 f8 08 11 80    	mov    0x801108f8,%edx
80102aec:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102aef:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102af1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102af4:	a3 f8 08 11 80       	mov    %eax,0x801108f8
  if(kmem.use_lock)
80102af9:	a1 f4 08 11 80       	mov    0x801108f4,%eax
80102afe:	85 c0                	test   %eax,%eax
80102b00:	74 0c                	je     80102b0e <kfree+0x97>
    release(&kmem.lock);
80102b02:	c7 04 24 c0 08 11 80 	movl   $0x801108c0,(%esp)
80102b09:	e8 04 31 00 00       	call   80105c12 <release>
}
80102b0e:	c9                   	leave  
80102b0f:	c3                   	ret    

80102b10 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102b10:	55                   	push   %ebp
80102b11:	89 e5                	mov    %esp,%ebp
80102b13:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102b16:	a1 f4 08 11 80       	mov    0x801108f4,%eax
80102b1b:	85 c0                	test   %eax,%eax
80102b1d:	74 0c                	je     80102b2b <kalloc+0x1b>
    acquire(&kmem.lock);
80102b1f:	c7 04 24 c0 08 11 80 	movl   $0x801108c0,(%esp)
80102b26:	e8 4c 30 00 00       	call   80105b77 <acquire>
  r = kmem.freelist;
80102b2b:	a1 f8 08 11 80       	mov    0x801108f8,%eax
80102b30:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b33:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b37:	74 0a                	je     80102b43 <kalloc+0x33>
    kmem.freelist = r->next;
80102b39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b3c:	8b 00                	mov    (%eax),%eax
80102b3e:	a3 f8 08 11 80       	mov    %eax,0x801108f8
  if(kmem.use_lock)
80102b43:	a1 f4 08 11 80       	mov    0x801108f4,%eax
80102b48:	85 c0                	test   %eax,%eax
80102b4a:	74 0c                	je     80102b58 <kalloc+0x48>
    release(&kmem.lock);
80102b4c:	c7 04 24 c0 08 11 80 	movl   $0x801108c0,(%esp)
80102b53:	e8 ba 30 00 00       	call   80105c12 <release>
  return (char*)r;
80102b58:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b5b:	c9                   	leave  
80102b5c:	c3                   	ret    

80102b5d <shmget>:


int 
shmget(int key, uint size, int shmflg)
{
80102b5d:	55                   	push   %ebp
80102b5e:	89 e5                	mov    %esp,%ebp
80102b60:	83 ec 38             	sub    $0x38,%esp
  int numOfPages,i,j,ans;
  uint sz;
  if(key < 0 || key > 1023)
80102b63:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102b67:	78 09                	js     80102b72 <shmget+0x15>
80102b69:	81 7d 08 ff 03 00 00 	cmpl   $0x3ff,0x8(%ebp)
80102b70:	7e 16                	jle    80102b88 <shmget+0x2b>
  {
    cprintf("Illegal key exception, value must be between 0-1023\n");
80102b72:	c7 04 24 90 98 10 80 	movl   $0x80109890,(%esp)
80102b79:	e8 23 d8 ff ff       	call   801003a1 <cprintf>
    return -1;
80102b7e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b83:	e9 38 01 00 00       	jmp    80102cc0 <shmget+0x163>
  }
  
  switch(shmflg)
80102b88:	8b 45 10             	mov    0x10(%ebp),%eax
80102b8b:	83 f8 14             	cmp    $0x14,%eax
80102b8e:	74 0e                	je     80102b9e <shmget+0x41>
80102b90:	83 f8 15             	cmp    $0x15,%eax
80102b93:	0f 84 ef 00 00 00    	je     80102c88 <shmget+0x12b>
80102b99:	e9 1f 01 00 00       	jmp    80102cbd <shmget+0x160>
  {
    case CREAT:
      if(shm.refs[key][1][64] == 0)
80102b9e:	8b 45 08             	mov    0x8(%ebp),%eax
80102ba1:	c1 e0 03             	shl    $0x3,%eax
80102ba4:	89 c2                	mov    %eax,%edx
80102ba6:	c1 e2 06             	shl    $0x6,%edx
80102ba9:	01 d0                	add    %edx,%eax
80102bab:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102bb0:	8b 00                	mov    (%eax),%eax
80102bb2:	85 c0                	test   %eax,%eax
80102bb4:	0f 85 c5 00 00 00    	jne    80102c7f <shmget+0x122>
      {
	sz = PGROUNDUP(size);
80102bba:	8b 45 0c             	mov    0xc(%ebp),%eax
80102bbd:	05 ff 0f 00 00       	add    $0xfff,%eax
80102bc2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102bc7:	89 45 e8             	mov    %eax,-0x18(%ebp)
	numOfPages = sz/PGSIZE;
80102bca:	8b 45 e8             	mov    -0x18(%ebp),%eax
80102bcd:	c1 e8 0c             	shr    $0xc,%eax
80102bd0:	89 45 e4             	mov    %eax,-0x1c(%ebp)
	for(i=0;i<numOfPages;i++)
80102bd3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102bda:	eb 2d                	jmp    80102c09 <shmget+0xac>
	{
	  if((shm.seg[key][i] = kalloc()) == 0)
80102bdc:	e8 2f ff ff ff       	call   80102b10 <kalloc>
80102be1:	8b 55 08             	mov    0x8(%ebp),%edx
80102be4:	6b d2 64             	imul   $0x64,%edx,%edx
80102be7:	03 55 f4             	add    -0xc(%ebp),%edx
80102bea:	89 04 95 00 09 11 80 	mov    %eax,-0x7feef700(,%edx,4)
80102bf1:	8b 45 08             	mov    0x8(%ebp),%eax
80102bf4:	6b c0 64             	imul   $0x64,%eax,%eax
80102bf7:	03 45 f4             	add    -0xc(%ebp),%eax
80102bfa:	8b 04 85 00 09 11 80 	mov    -0x7feef700(,%eax,4),%eax
80102c01:	85 c0                	test   %eax,%eax
80102c03:	74 0e                	je     80102c13 <shmget+0xb6>
    case CREAT:
      if(shm.refs[key][1][64] == 0)
      {
	sz = PGROUNDUP(size);
	numOfPages = sz/PGSIZE;
	for(i=0;i<numOfPages;i++)
80102c05:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102c09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102c0c:	3b 45 e4             	cmp    -0x1c(%ebp),%eax
80102c0f:	7c cb                	jl     80102bdc <shmget+0x7f>
80102c11:	eb 01                	jmp    80102c14 <shmget+0xb7>
	{
	  if((shm.seg[key][i] = kalloc()) == 0)
	    break;
80102c13:	90                   	nop
	}
	if(i == numOfPages)
80102c14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102c17:	3b 45 e4             	cmp    -0x1c(%ebp),%eax
80102c1a:	75 2d                	jne    80102c49 <shmget+0xec>
	{
	  ans = (int)shm.seg[key][0];
80102c1c:	8b 45 08             	mov    0x8(%ebp),%eax
80102c1f:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
80102c25:	05 00 09 11 80       	add    $0x80110900,%eax
80102c2a:	8b 00                	mov    (%eax),%eax
80102c2c:	89 45 ec             	mov    %eax,-0x14(%ebp)
	  shm.refs[key][1][64] = numOfPages;
80102c2f:	8b 45 08             	mov    0x8(%ebp),%eax
80102c32:	c1 e0 03             	shl    $0x3,%eax
80102c35:	89 c2                	mov    %eax,%edx
80102c37:	c1 e2 06             	shl    $0x6,%edx
80102c3a:	01 d0                	add    %edx,%eax
80102c3c:	8d 90 44 a7 11 80    	lea    -0x7fee58bc(%eax),%edx
80102c42:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102c45:	89 02                	mov    %eax,(%edx)
      }
      else
      {
	ans = -1;
      }
      break;
80102c47:	eb 74                	jmp    80102cbd <shmget+0x160>
	  ans = (int)shm.seg[key][0];
	  shm.refs[key][1][64] = numOfPages;
	}
	else
	{
	  for(j=0;j<i;j++)
80102c49:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80102c50:	eb 1c                	jmp    80102c6e <shmget+0x111>
	    kfree(shm.seg[key][j]);
80102c52:	8b 45 08             	mov    0x8(%ebp),%eax
80102c55:	6b c0 64             	imul   $0x64,%eax,%eax
80102c58:	03 45 f0             	add    -0x10(%ebp),%eax
80102c5b:	8b 04 85 00 09 11 80 	mov    -0x7feef700(,%eax,4),%eax
80102c62:	89 04 24             	mov    %eax,(%esp)
80102c65:	e8 0d fe ff ff       	call   80102a77 <kfree>
	  ans = (int)shm.seg[key][0];
	  shm.refs[key][1][64] = numOfPages;
	}
	else
	{
	  for(j=0;j<i;j++)
80102c6a:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80102c6e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102c71:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102c74:	7c dc                	jl     80102c52 <shmget+0xf5>
	    kfree(shm.seg[key][j]);
	  ans = -1;
80102c76:	c7 45 ec ff ff ff ff 	movl   $0xffffffff,-0x14(%ebp)
      }
      else
      {
	ans = -1;
      }
      break;
80102c7d:	eb 3e                	jmp    80102cbd <shmget+0x160>
	  ans = -1;
	}
      }
      else
      {
	ans = -1;
80102c7f:	c7 45 ec ff ff ff ff 	movl   $0xffffffff,-0x14(%ebp)
      }
      break;
80102c86:	eb 35                	jmp    80102cbd <shmget+0x160>
    case GET:
      if(!shm.refs[key][1][64])
80102c88:	8b 45 08             	mov    0x8(%ebp),%eax
80102c8b:	c1 e0 03             	shl    $0x3,%eax
80102c8e:	89 c2                	mov    %eax,%edx
80102c90:	c1 e2 06             	shl    $0x6,%edx
80102c93:	01 d0                	add    %edx,%eax
80102c95:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102c9a:	8b 00                	mov    (%eax),%eax
80102c9c:	85 c0                	test   %eax,%eax
80102c9e:	75 09                	jne    80102ca9 <shmget+0x14c>
	ans = -1;
80102ca0:	c7 45 ec ff ff ff ff 	movl   $0xffffffff,-0x14(%ebp)
      else
	ans = (int)shm.seg[key][0];
      break;
80102ca7:	eb 13                	jmp    80102cbc <shmget+0x15f>
      break;
    case GET:
      if(!shm.refs[key][1][64])
	ans = -1;
      else
	ans = (int)shm.seg[key][0];
80102ca9:	8b 45 08             	mov    0x8(%ebp),%eax
80102cac:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
80102cb2:	05 00 09 11 80       	add    $0x80110900,%eax
80102cb7:	8b 00                	mov    (%eax),%eax
80102cb9:	89 45 ec             	mov    %eax,-0x14(%ebp)
      break;
80102cbc:	90                   	nop
  }
  return ans;
80102cbd:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80102cc0:	c9                   	leave  
80102cc1:	c3                   	ret    

80102cc2 <shmdel>:

int 
shmdel(int shmid)
{
80102cc2:	55                   	push   %ebp
80102cc3:	89 e5                	mov    %esp,%ebp
80102cc5:	83 ec 28             	sub    $0x28,%esp
  int key,ans = -1,numOfPages,i;
80102cc8:	c7 45 f0 ff ff ff ff 	movl   $0xffffffff,-0x10(%ebp)
  for(key = 0;key<numOfSegs;key++)
80102ccf:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102cd6:	e9 b0 00 00 00       	jmp    80102d8b <shmdel+0xc9>
  {
    if(shmid == (int)shm.seg[key][0])
80102cdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102cde:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
80102ce4:	05 00 09 11 80       	add    $0x80110900,%eax
80102ce9:	8b 00                	mov    (%eax),%eax
80102ceb:	3b 45 08             	cmp    0x8(%ebp),%eax
80102cee:	0f 85 93 00 00 00    	jne    80102d87 <shmdel+0xc5>
    {
      if(shm.refs[key][0][64]>0)
80102cf4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102cf7:	c1 e0 03             	shl    $0x3,%eax
80102cfa:	89 c2                	mov    %eax,%edx
80102cfc:	c1 e2 06             	shl    $0x6,%edx
80102cff:	01 d0                	add    %edx,%eax
80102d01:	05 40 a6 11 80       	add    $0x8011a640,%eax
80102d06:	8b 00                	mov    (%eax),%eax
80102d08:	85 c0                	test   %eax,%eax
80102d0a:	0f 8f 87 00 00 00    	jg     80102d97 <shmdel+0xd5>
      {
	break;
      }
      else
      {
	numOfPages=shm.refs[key][1][64];
80102d10:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102d13:	c1 e0 03             	shl    $0x3,%eax
80102d16:	89 c2                	mov    %eax,%edx
80102d18:	c1 e2 06             	shl    $0x6,%edx
80102d1b:	01 d0                	add    %edx,%eax
80102d1d:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102d22:	8b 00                	mov    (%eax),%eax
80102d24:	89 45 e8             	mov    %eax,-0x18(%ebp)
	for(i=0;i<numOfPages;i++)
80102d27:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80102d2e:	eb 47                	jmp    80102d77 <shmdel+0xb5>
	{
	    kfree(shm.seg[key][i]);
80102d30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102d33:	6b c0 64             	imul   $0x64,%eax,%eax
80102d36:	03 45 ec             	add    -0x14(%ebp),%eax
80102d39:	8b 04 85 00 09 11 80 	mov    -0x7feef700(,%eax,4),%eax
80102d40:	89 04 24             	mov    %eax,(%esp)
80102d43:	e8 2f fd ff ff       	call   80102a77 <kfree>
	    shm.refs[key][1][64]--;
80102d48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102d4b:	c1 e0 03             	shl    $0x3,%eax
80102d4e:	89 c2                	mov    %eax,%edx
80102d50:	c1 e2 06             	shl    $0x6,%edx
80102d53:	01 d0                	add    %edx,%eax
80102d55:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102d5a:	8b 00                	mov    (%eax),%eax
80102d5c:	8d 50 ff             	lea    -0x1(%eax),%edx
80102d5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102d62:	c1 e0 03             	shl    $0x3,%eax
80102d65:	89 c1                	mov    %eax,%ecx
80102d67:	c1 e1 06             	shl    $0x6,%ecx
80102d6a:	01 c8                	add    %ecx,%eax
80102d6c:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102d71:	89 10                	mov    %edx,(%eax)
	break;
      }
      else
      {
	numOfPages=shm.refs[key][1][64];
	for(i=0;i<numOfPages;i++)
80102d73:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80102d77:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102d7a:	3b 45 e8             	cmp    -0x18(%ebp),%eax
80102d7d:	7c b1                	jl     80102d30 <shmdel+0x6e>
	{
	    kfree(shm.seg[key][i]);
	    shm.refs[key][1][64]--;
	}
      }
      ans = numOfPages;
80102d7f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80102d82:	89 45 f0             	mov    %eax,-0x10(%ebp)
      break;
80102d85:	eb 11                	jmp    80102d98 <shmdel+0xd6>

int 
shmdel(int shmid)
{
  int key,ans = -1,numOfPages,i;
  for(key = 0;key<numOfSegs;key++)
80102d87:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102d8b:	83 7d f4 63          	cmpl   $0x63,-0xc(%ebp)
80102d8f:	0f 8e 46 ff ff ff    	jle    80102cdb <shmdel+0x19>
80102d95:	eb 01                	jmp    80102d98 <shmdel+0xd6>
  {
    if(shmid == (int)shm.seg[key][0])
    {
      if(shm.refs[key][0][64]>0)
      {
	break;
80102d97:	90                   	nop
      }
      ans = numOfPages;
      break;
    }
  }  
  return ans;
80102d98:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80102d9b:	c9                   	leave  
80102d9c:	c3                   	ret    

80102d9d <shmat>:

void *
shmat(int shmid, int shmflg)
{
80102d9d:	55                   	push   %ebp
80102d9e:	89 e5                	mov    %esp,%ebp
80102da0:	83 ec 48             	sub    $0x48,%esp
  int i,key,forFlag=0;
80102da3:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  void* ans = (void*)-1;
80102daa:	c7 45 e8 ff ff ff ff 	movl   $0xffffffff,-0x18(%ebp)
  char* mem;
  uint a;

  acquire(&shm.lock);
80102db1:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
80102db8:	e8 ba 2d 00 00       	call   80105b77 <acquire>
  for(key = 0;key<numOfSegs;key++)
80102dbd:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80102dc4:	e9 ce 01 00 00       	jmp    80102f97 <shmat+0x1fa>
  {
    if(shmid == (int)shm.seg[key][0])
80102dc9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102dcc:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
80102dd2:	05 00 09 11 80       	add    $0x80110900,%eax
80102dd7:	8b 00                	mov    (%eax),%eax
80102dd9:	3b 45 08             	cmp    0x8(%ebp),%eax
80102ddc:	0f 85 b1 01 00 00    	jne    80102f93 <shmat+0x1f6>
    {
      if(shm.refs[key][1][64]>0)
80102de2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102de5:	c1 e0 03             	shl    $0x3,%eax
80102de8:	89 c2                	mov    %eax,%edx
80102dea:	c1 e2 06             	shl    $0x6,%edx
80102ded:	01 d0                	add    %edx,%eax
80102def:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102df4:	8b 00                	mov    (%eax),%eax
80102df6:	85 c0                	test   %eax,%eax
80102df8:	0f 8e a5 01 00 00    	jle    80102fa3 <shmat+0x206>
      {
	a = PGROUNDUP(proc->sz);
80102dfe:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102e04:	8b 00                	mov    (%eax),%eax
80102e06:	05 ff 0f 00 00       	add    $0xfff,%eax
80102e0b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102e10:	89 45 e4             	mov    %eax,-0x1c(%ebp)
	ans = (void*)a;
80102e13:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102e16:	89 45 e8             	mov    %eax,-0x18(%ebp)
	if(a + PGSIZE >= KERNBASE)
80102e19:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102e1c:	05 00 10 00 00       	add    $0x1000,%eax
80102e21:	85 c0                	test   %eax,%eax
80102e23:	79 0c                	jns    80102e31 <shmat+0x94>
	{
	  ans = (void*)-1;
80102e25:	c7 45 e8 ff ff ff ff 	movl   $0xffffffff,-0x18(%ebp)
	  break;
80102e2c:	e9 73 01 00 00       	jmp    80102fa4 <shmat+0x207>
	}
	
	shm.refs[key][0][64]++;
80102e31:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102e34:	c1 e0 03             	shl    $0x3,%eax
80102e37:	89 c2                	mov    %eax,%edx
80102e39:	c1 e2 06             	shl    $0x6,%edx
80102e3c:	01 d0                	add    %edx,%eax
80102e3e:	05 40 a6 11 80       	add    $0x8011a640,%eax
80102e43:	8b 00                	mov    (%eax),%eax
80102e45:	8d 50 01             	lea    0x1(%eax),%edx
80102e48:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102e4b:	c1 e0 03             	shl    $0x3,%eax
80102e4e:	89 c1                	mov    %eax,%ecx
80102e50:	c1 e1 06             	shl    $0x6,%ecx
80102e53:	01 c8                	add    %ecx,%eax
80102e55:	05 40 a6 11 80       	add    $0x8011a640,%eax
80102e5a:	89 10                	mov    %edx,(%eax)
	shm.refs[key][0][proc->pid] = 1;
80102e5c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102e62:	8b 50 10             	mov    0x10(%eax),%edx
80102e65:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102e68:	01 c0                	add    %eax,%eax
80102e6a:	89 c1                	mov    %eax,%ecx
80102e6c:	c1 e1 06             	shl    $0x6,%ecx
80102e6f:	01 c8                	add    %ecx,%eax
80102e71:	01 d0                	add    %edx,%eax
80102e73:	05 10 27 00 00       	add    $0x2710,%eax
80102e78:	c7 04 85 00 09 11 80 	movl   $0x1,-0x7feef700(,%eax,4)
80102e7f:	01 00 00 00 
	proc->has_shm++;
80102e83:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102e89:	8b 90 8c 00 00 00    	mov    0x8c(%eax),%edx
80102e8f:	83 c2 01             	add    $0x1,%edx
80102e92:	89 90 8c 00 00 00    	mov    %edx,0x8c(%eax)
	
	for(i = 0;i < shm.refs[key][1][64] && a < KERNBASE;i++,a += PGSIZE)
80102e98:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102e9f:	e9 af 00 00 00       	jmp    80102f53 <shmat+0x1b6>
	{
	    forFlag = 1;
80102ea4:	c7 45 ec 01 00 00 00 	movl   $0x1,-0x14(%ebp)
	    mem = shm.seg[key][i];
80102eab:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102eae:	6b c0 64             	imul   $0x64,%eax,%eax
80102eb1:	03 45 f4             	add    -0xc(%ebp),%eax
80102eb4:	8b 04 85 00 09 11 80 	mov    -0x7feef700(,%eax,4),%eax
80102ebb:	89 45 e0             	mov    %eax,-0x20(%ebp)
	    switch(shmflg)
80102ebe:	8b 45 0c             	mov    0xc(%ebp),%eax
80102ec1:	83 f8 16             	cmp    $0x16,%eax
80102ec4:	74 07                	je     80102ecd <shmat+0x130>
80102ec6:	83 f8 17             	cmp    $0x17,%eax
80102ec9:	74 3c                	je     80102f07 <shmat+0x16a>
80102ecb:	eb 74                	jmp    80102f41 <shmat+0x1a4>
	    {
	      case SHM_RDONLY:
		mappages(proc->pgdir, (char*)a, PGSIZE, v2p(mem), PTE_U);
80102ecd:	8b 45 e0             	mov    -0x20(%ebp),%eax
80102ed0:	89 04 24             	mov    %eax,(%esp)
80102ed3:	e8 f0 fa ff ff       	call   801029c8 <v2p>
80102ed8:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80102edb:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80102ee2:	8b 52 04             	mov    0x4(%edx),%edx
80102ee5:	c7 44 24 10 04 00 00 	movl   $0x4,0x10(%esp)
80102eec:	00 
80102eed:	89 44 24 0c          	mov    %eax,0xc(%esp)
80102ef1:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102ef8:	00 
80102ef9:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80102efd:	89 14 24             	mov    %edx,(%esp)
80102f00:	e8 c4 5e 00 00       	call   80108dc9 <mappages>
		break;
80102f05:	eb 41                	jmp    80102f48 <shmat+0x1ab>
	      case SHM_RDWR:
		mappages(proc->pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
80102f07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80102f0a:	89 04 24             	mov    %eax,(%esp)
80102f0d:	e8 b6 fa ff ff       	call   801029c8 <v2p>
80102f12:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80102f15:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80102f1c:	8b 52 04             	mov    0x4(%edx),%edx
80102f1f:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80102f26:	00 
80102f27:	89 44 24 0c          	mov    %eax,0xc(%esp)
80102f2b:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102f32:	00 
80102f33:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80102f37:	89 14 24             	mov    %edx,(%esp)
80102f3a:	e8 8a 5e 00 00       	call   80108dc9 <mappages>
		break;
80102f3f:	eb 07                	jmp    80102f48 <shmat+0x1ab>
	      default:
		forFlag = 0;
80102f41:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
	
	shm.refs[key][0][64]++;
	shm.refs[key][0][proc->pid] = 1;
	proc->has_shm++;
	
	for(i = 0;i < shm.refs[key][1][64] && a < KERNBASE;i++,a += PGSIZE)
80102f48:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102f4c:	81 45 e4 00 10 00 00 	addl   $0x1000,-0x1c(%ebp)
80102f53:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102f56:	c1 e0 03             	shl    $0x3,%eax
80102f59:	89 c2                	mov    %eax,%edx
80102f5b:	c1 e2 06             	shl    $0x6,%edx
80102f5e:	01 d0                	add    %edx,%eax
80102f60:	05 44 a7 11 80       	add    $0x8011a744,%eax
80102f65:	8b 00                	mov    (%eax),%eax
80102f67:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102f6a:	7e 0b                	jle    80102f77 <shmat+0x1da>
80102f6c:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102f6f:	85 c0                	test   %eax,%eax
80102f71:	0f 89 2d ff ff ff    	jns    80102ea4 <shmat+0x107>
		break;
	      default:
		forFlag = 0;
	    } 
	}
	if(forFlag)
80102f77:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80102f7b:	74 0d                	je     80102f8a <shmat+0x1ed>
	  proc->sz = a;
80102f7d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102f83:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80102f86:	89 10                	mov    %edx,(%eax)
	else
	  ans = (void*)-1;
	break;
80102f88:	eb 1a                	jmp    80102fa4 <shmat+0x207>
	    } 
	}
	if(forFlag)
	  proc->sz = a;
	else
	  ans = (void*)-1;
80102f8a:	c7 45 e8 ff ff ff ff 	movl   $0xffffffff,-0x18(%ebp)
	break;
80102f91:	eb 11                	jmp    80102fa4 <shmat+0x207>
  void* ans = (void*)-1;
  char* mem;
  uint a;

  acquire(&shm.lock);
  for(key = 0;key<numOfSegs;key++)
80102f93:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80102f97:	83 7d f0 63          	cmpl   $0x63,-0x10(%ebp)
80102f9b:	0f 8e 28 fe ff ff    	jle    80102dc9 <shmat+0x2c>
80102fa1:	eb 01                	jmp    80102fa4 <shmat+0x207>
	else
	  ans = (void*)-1;
	break;
      }
      else
      	break;
80102fa3:	90                   	nop
    }
  }
  release(&shm.lock);
80102fa4:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
80102fab:	e8 62 2c 00 00       	call   80105c12 <release>
  return ans;
80102fb0:	8b 45 e8             	mov    -0x18(%ebp),%eax
}
80102fb3:	c9                   	leave  
80102fb4:	c3                   	ret    

80102fb5 <shmdt>:

int 
shmdt(const void *shmaddr)
{
80102fb5:	55                   	push   %ebp
80102fb6:	89 e5                	mov    %esp,%ebp
80102fb8:	83 ec 38             	sub    $0x38,%esp
  pte_t *pte;
  uint r, numOfPages;
  int key,found;
  pte = walkpgdir(proc->pgdir, (char*)shmaddr, 0);
80102fbb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102fc1:	8b 40 04             	mov    0x4(%eax),%eax
80102fc4:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102fcb:	00 
80102fcc:	8b 55 08             	mov    0x8(%ebp),%edx
80102fcf:	89 54 24 04          	mov    %edx,0x4(%esp)
80102fd3:	89 04 24             	mov    %eax,(%esp)
80102fd6:	e8 58 5d 00 00       	call   80108d33 <walkpgdir>
80102fdb:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  r = (int)p2v(PTE_ADDR(*pte)) ;
80102fde:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102fe1:	8b 00                	mov    (%eax),%eax
80102fe3:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102fe8:	89 04 24             	mov    %eax,(%esp)
80102feb:	e8 e5 f9 ff ff       	call   801029d5 <p2v>
80102ff0:	89 45 e0             	mov    %eax,-0x20(%ebp)
  acquire(&shm.lock);
80102ff3:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
80102ffa:	e8 78 2b 00 00       	call   80105b77 <acquire>
  for(found = 0,key = 0;key<numOfSegs;key++)
80102fff:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80103006:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
8010300d:	e9 04 01 00 00       	jmp    80103116 <shmdt+0x161>
  {    
    if((int)shm.seg[key][0] == r)
80103012:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103015:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
8010301b:	05 00 09 11 80       	add    $0x80110900,%eax
80103020:	8b 00                	mov    (%eax),%eax
80103022:	3b 45 e0             	cmp    -0x20(%ebp),%eax
80103025:	0f 85 e7 00 00 00    	jne    80103112 <shmdt+0x15d>
    {  
      if(shm.refs[key][1][64]>0)
8010302b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010302e:	c1 e0 03             	shl    $0x3,%eax
80103031:	89 c2                	mov    %eax,%edx
80103033:	c1 e2 06             	shl    $0x6,%edx
80103036:	01 d0                	add    %edx,%eax
80103038:	05 44 a7 11 80       	add    $0x8011a744,%eax
8010303d:	8b 00                	mov    (%eax),%eax
8010303f:	85 c0                	test   %eax,%eax
80103041:	0f 8e b5 00 00 00    	jle    801030fc <shmdt+0x147>
      { 
	if(shm.refs[key][0][64] <= 0)
80103047:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010304a:	c1 e0 03             	shl    $0x3,%eax
8010304d:	89 c2                	mov    %eax,%edx
8010304f:	c1 e2 06             	shl    $0x6,%edx
80103052:	01 d0                	add    %edx,%eax
80103054:	05 40 a6 11 80       	add    $0x8011a640,%eax
80103059:	8b 00                	mov    (%eax),%eax
8010305b:	85 c0                	test   %eax,%eax
8010305d:	7f 16                	jg     80103075 <shmdt+0xc0>
	{
	  cprintf("shmdt exception - trying to detach a segment with no references\n");
8010305f:	c7 04 24 c8 98 10 80 	movl   $0x801098c8,(%esp)
80103066:	e8 36 d3 ff ff       	call   801003a1 <cprintf>
	  return -1;
8010306b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103070:	e9 1f 01 00 00       	jmp    80103194 <shmdt+0x1df>
	}
	shm.refs[key][0][64]--;
80103075:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103078:	c1 e0 03             	shl    $0x3,%eax
8010307b:	89 c2                	mov    %eax,%edx
8010307d:	c1 e2 06             	shl    $0x6,%edx
80103080:	01 d0                	add    %edx,%eax
80103082:	05 40 a6 11 80       	add    $0x8011a640,%eax
80103087:	8b 00                	mov    (%eax),%eax
80103089:	8d 50 ff             	lea    -0x1(%eax),%edx
8010308c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010308f:	c1 e0 03             	shl    $0x3,%eax
80103092:	89 c1                	mov    %eax,%ecx
80103094:	c1 e1 06             	shl    $0x6,%ecx
80103097:	01 c8                	add    %ecx,%eax
80103099:	05 40 a6 11 80       	add    $0x8011a640,%eax
8010309e:	89 10                	mov    %edx,(%eax)
	shm.refs[key][0][proc->pid] = 0;
801030a0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801030a6:	8b 50 10             	mov    0x10(%eax),%edx
801030a9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030ac:	01 c0                	add    %eax,%eax
801030ae:	89 c1                	mov    %eax,%ecx
801030b0:	c1 e1 06             	shl    $0x6,%ecx
801030b3:	01 c8                	add    %ecx,%eax
801030b5:	01 d0                	add    %edx,%eax
801030b7:	05 10 27 00 00       	add    $0x2710,%eax
801030bc:	c7 04 85 00 09 11 80 	movl   $0x0,-0x7feef700(,%eax,4)
801030c3:	00 00 00 00 
	proc->has_shm--;
801030c7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801030cd:	8b 90 8c 00 00 00    	mov    0x8c(%eax),%edx
801030d3:	83 ea 01             	sub    $0x1,%edx
801030d6:	89 90 8c 00 00 00    	mov    %edx,0x8c(%eax)
	numOfPages = shm.refs[key][1][64];
801030dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030df:	c1 e0 03             	shl    $0x3,%eax
801030e2:	89 c2                	mov    %eax,%edx
801030e4:	c1 e2 06             	shl    $0x6,%edx
801030e7:	01 d0                	add    %edx,%eax
801030e9:	05 44 a7 11 80       	add    $0x8011a744,%eax
801030ee:	8b 00                	mov    (%eax),%eax
801030f0:	89 45 f4             	mov    %eax,-0xc(%ebp)
	found = 1;
801030f3:	c7 45 ec 01 00 00 00 	movl   $0x1,-0x14(%ebp)
	break;
801030fa:	eb 24                	jmp    80103120 <shmdt+0x16b>
      }
      else
      {
	cprintf("shmdt exception - trying to detach a segment with no pages\n");
801030fc:	c7 04 24 0c 99 10 80 	movl   $0x8010990c,(%esp)
80103103:	e8 99 d2 ff ff       	call   801003a1 <cprintf>
	return -1;
80103108:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010310d:	e9 82 00 00 00       	jmp    80103194 <shmdt+0x1df>
  uint r, numOfPages;
  int key,found;
  pte = walkpgdir(proc->pgdir, (char*)shmaddr, 0);
  r = (int)p2v(PTE_ADDR(*pte)) ;
  acquire(&shm.lock);
  for(found = 0,key = 0;key<numOfSegs;key++)
80103112:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80103116:	83 7d f0 63          	cmpl   $0x63,-0x10(%ebp)
8010311a:	0f 8e f2 fe ff ff    	jle    80103012 <shmdt+0x5d>
	cprintf("shmdt exception - trying to detach a segment with no pages\n");
	return -1;
      }
    }
  }
  release(&shm.lock);
80103120:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
80103127:	e8 e6 2a 00 00       	call   80105c12 <release>
  
  if(!found)
8010312c:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103130:	75 07                	jne    80103139 <shmdt+0x184>
    return -1;
80103132:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103137:	eb 5b                	jmp    80103194 <shmdt+0x1df>

  void *shmaddr2 = (void*)shmaddr;
80103139:	8b 45 08             	mov    0x8(%ebp),%eax
8010313c:	89 45 e8             	mov    %eax,-0x18(%ebp)

  for(; shmaddr2  < shmaddr + numOfPages*PGSIZE; shmaddr2 += PGSIZE)
8010313f:	eb 40                	jmp    80103181 <shmdt+0x1cc>
  {
    pte = walkpgdir(proc->pgdir, (char*)shmaddr2, 0);
80103141:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103147:	8b 40 04             	mov    0x4(%eax),%eax
8010314a:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80103151:	00 
80103152:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103155:	89 54 24 04          	mov    %edx,0x4(%esp)
80103159:	89 04 24             	mov    %eax,(%esp)
8010315c:	e8 d2 5b 00 00       	call   80108d33 <walkpgdir>
80103161:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(!pte)
80103164:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80103168:	75 07                	jne    80103171 <shmdt+0x1bc>
      shmaddr2 += (NPTENTRIES - 1) * PGSIZE;
8010316a:	81 45 e8 00 f0 3f 00 	addl   $0x3ff000,-0x18(%ebp)
    *pte = 0;
80103171:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103174:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  if(!found)
    return -1;

  void *shmaddr2 = (void*)shmaddr;

  for(; shmaddr2  < shmaddr + numOfPages*PGSIZE; shmaddr2 += PGSIZE)
8010317a:	81 45 e8 00 10 00 00 	addl   $0x1000,-0x18(%ebp)
80103181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103184:	c1 e0 0c             	shl    $0xc,%eax
80103187:	03 45 08             	add    0x8(%ebp),%eax
8010318a:	3b 45 e8             	cmp    -0x18(%ebp),%eax
8010318d:	77 b2                	ja     80103141 <shmdt+0x18c>
    if(!pte)
      shmaddr2 += (NPTENTRIES - 1) * PGSIZE;
    *pte = 0;
  }

  return 0;
8010318f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103194:	c9                   	leave  
80103195:	c3                   	ret    

80103196 <deallocshm>:

void 
deallocshm(int pid)
{
80103196:	55                   	push   %ebp
80103197:	89 e5                	mov    %esp,%ebp
80103199:	83 ec 38             	sub    $0x38,%esp
  uint a = 0;
8010319c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  int key, pa, numOfPages;
  pte_t *pte;
  
  acquire(&shm.lock);
801031a3:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
801031aa:	e8 c8 29 00 00       	call   80105b77 <acquire>
  for(key = 0;key<numOfSegs;key++)
801031af:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801031b6:	e9 74 01 00 00       	jmp    8010332f <deallocshm+0x199>
  {    
    if(shm.refs[key][0][proc->pid])
801031bb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801031c1:	8b 50 10             	mov    0x10(%eax),%edx
801031c4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031c7:	01 c0                	add    %eax,%eax
801031c9:	89 c1                	mov    %eax,%ecx
801031cb:	c1 e1 06             	shl    $0x6,%ecx
801031ce:	01 c8                	add    %ecx,%eax
801031d0:	01 d0                	add    %edx,%eax
801031d2:	05 10 27 00 00       	add    $0x2710,%eax
801031d7:	8b 04 85 00 09 11 80 	mov    -0x7feef700(,%eax,4),%eax
801031de:	85 c0                	test   %eax,%eax
801031e0:	0f 84 45 01 00 00    	je     8010332b <deallocshm+0x195>
    {
      for(; a  < proc->sz; a += PGSIZE)
801031e6:	e9 2c 01 00 00       	jmp    80103317 <deallocshm+0x181>
      {
	pte = walkpgdir(proc->pgdir, (char*)a, 0);
801031eb:	8b 55 f4             	mov    -0xc(%ebp),%edx
801031ee:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801031f4:	8b 40 04             	mov    0x4(%eax),%eax
801031f7:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801031fe:	00 
801031ff:	89 54 24 04          	mov    %edx,0x4(%esp)
80103203:	89 04 24             	mov    %eax,(%esp)
80103206:	e8 28 5b 00 00       	call   80108d33 <walkpgdir>
8010320b:	89 45 e8             	mov    %eax,-0x18(%ebp)
	if(!pte)
8010320e:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80103212:	75 0c                	jne    80103220 <deallocshm+0x8a>
	  a += (NPTENTRIES - 1) * PGSIZE;
80103214:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
8010321b:	e9 f0 00 00 00       	jmp    80103310 <deallocshm+0x17a>
	else if((*pte & PTE_P) != 0)
80103220:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103223:	8b 00                	mov    (%eax),%eax
80103225:	83 e0 01             	and    $0x1,%eax
80103228:	84 c0                	test   %al,%al
8010322a:	0f 84 e0 00 00 00    	je     80103310 <deallocshm+0x17a>
	{
	  pa = (int)p2v(PTE_ADDR(*pte));
80103230:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103233:	8b 00                	mov    (%eax),%eax
80103235:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010323a:	89 04 24             	mov    %eax,(%esp)
8010323d:	e8 93 f7 ff ff       	call   801029d5 <p2v>
80103242:	89 45 e4             	mov    %eax,-0x1c(%ebp)
	  if((int)shm.seg[key][0] == pa)
80103245:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103248:	69 c0 90 01 00 00    	imul   $0x190,%eax,%eax
8010324e:	05 00 09 11 80       	add    $0x80110900,%eax
80103253:	8b 00                	mov    (%eax),%eax
80103255:	3b 45 e4             	cmp    -0x1c(%ebp),%eax
80103258:	0f 85 b2 00 00 00    	jne    80103310 <deallocshm+0x17a>
	  {
	    void *b = (void*)a;
8010325e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103261:	89 45 ec             	mov    %eax,-0x14(%ebp)
	    numOfPages = shm.refs[key][1][64];
80103264:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103267:	c1 e0 03             	shl    $0x3,%eax
8010326a:	89 c2                	mov    %eax,%edx
8010326c:	c1 e2 06             	shl    $0x6,%edx
8010326f:	01 d0                	add    %edx,%eax
80103271:	05 44 a7 11 80       	add    $0x8011a744,%eax
80103276:	8b 00                	mov    (%eax),%eax
80103278:	89 45 e0             	mov    %eax,-0x20(%ebp)
	    for(; b  < (void*)a + numOfPages*PGSIZE; b += PGSIZE)
8010327b:	eb 40                	jmp    801032bd <deallocshm+0x127>
	    {
	      pte = walkpgdir(proc->pgdir, (char*)b, 0);
8010327d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103283:	8b 40 04             	mov    0x4(%eax),%eax
80103286:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010328d:	00 
8010328e:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103291:	89 54 24 04          	mov    %edx,0x4(%esp)
80103295:	89 04 24             	mov    %eax,(%esp)
80103298:	e8 96 5a 00 00       	call   80108d33 <walkpgdir>
8010329d:	89 45 e8             	mov    %eax,-0x18(%ebp)
	      if(!pte)
801032a0:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801032a4:	75 07                	jne    801032ad <deallocshm+0x117>
		b += (NPTENTRIES - 1) * PGSIZE;
801032a6:	81 45 ec 00 f0 3f 00 	addl   $0x3ff000,-0x14(%ebp)
	      *pte = 0;
801032ad:	8b 45 e8             	mov    -0x18(%ebp),%eax
801032b0:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
	  pa = (int)p2v(PTE_ADDR(*pte));
	  if((int)shm.seg[key][0] == pa)
	  {
	    void *b = (void*)a;
	    numOfPages = shm.refs[key][1][64];
	    for(; b  < (void*)a + numOfPages*PGSIZE; b += PGSIZE)
801032b6:	81 45 ec 00 10 00 00 	addl   $0x1000,-0x14(%ebp)
801032bd:	8b 45 e0             	mov    -0x20(%ebp),%eax
801032c0:	c1 e0 0c             	shl    $0xc,%eax
801032c3:	03 45 f4             	add    -0xc(%ebp),%eax
801032c6:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801032c9:	77 b2                	ja     8010327d <deallocshm+0xe7>
	      pte = walkpgdir(proc->pgdir, (char*)b, 0);
	      if(!pte)
		b += (NPTENTRIES - 1) * PGSIZE;
	      *pte = 0;
	    }
	    if(shm.refs[key][0][64]>0)
801032cb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801032ce:	c1 e0 03             	shl    $0x3,%eax
801032d1:	89 c2                	mov    %eax,%edx
801032d3:	c1 e2 06             	shl    $0x6,%edx
801032d6:	01 d0                	add    %edx,%eax
801032d8:	05 40 a6 11 80       	add    $0x8011a640,%eax
801032dd:	8b 00                	mov    (%eax),%eax
801032df:	85 c0                	test   %eax,%eax
801032e1:	7e 47                	jle    8010332a <deallocshm+0x194>
	      shm.refs[key][0][64]--;
801032e3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801032e6:	c1 e0 03             	shl    $0x3,%eax
801032e9:	89 c2                	mov    %eax,%edx
801032eb:	c1 e2 06             	shl    $0x6,%edx
801032ee:	01 d0                	add    %edx,%eax
801032f0:	05 40 a6 11 80       	add    $0x8011a640,%eax
801032f5:	8b 00                	mov    (%eax),%eax
801032f7:	8d 50 ff             	lea    -0x1(%eax),%edx
801032fa:	8b 45 f0             	mov    -0x10(%ebp),%eax
801032fd:	c1 e0 03             	shl    $0x3,%eax
80103300:	89 c1                	mov    %eax,%ecx
80103302:	c1 e1 06             	shl    $0x6,%ecx
80103305:	01 c8                	add    %ecx,%eax
80103307:	05 40 a6 11 80       	add    $0x8011a640,%eax
8010330c:	89 10                	mov    %edx,(%eax)
	    break;
8010330e:	eb 1a                	jmp    8010332a <deallocshm+0x194>
  acquire(&shm.lock);
  for(key = 0;key<numOfSegs;key++)
  {    
    if(shm.refs[key][0][proc->pid])
    {
      for(; a  < proc->sz; a += PGSIZE)
80103310:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80103317:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010331d:	8b 00                	mov    (%eax),%eax
8010331f:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103322:	0f 87 c3 fe ff ff    	ja     801031eb <deallocshm+0x55>
80103328:	eb 01                	jmp    8010332b <deallocshm+0x195>
		b += (NPTENTRIES - 1) * PGSIZE;
	      *pte = 0;
	    }
	    if(shm.refs[key][0][64]>0)
	      shm.refs[key][0][64]--;
	    break;
8010332a:	90                   	nop
  uint a = 0;
  int key, pa, numOfPages;
  pte_t *pte;
  
  acquire(&shm.lock);
  for(key = 0;key<numOfSegs;key++)
8010332b:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
8010332f:	83 7d f0 63          	cmpl   $0x63,-0x10(%ebp)
80103333:	0f 8e 82 fe ff ff    	jle    801031bb <deallocshm+0x25>
	  }
	}
      }
    }
  }
  release(&shm.lock);
80103339:	c7 04 24 60 70 12 80 	movl   $0x80127060,(%esp)
80103340:	e8 cd 28 00 00       	call   80105c12 <release>
}
80103345:	c9                   	leave  
80103346:	c3                   	ret    
	...

80103348 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80103348:	55                   	push   %ebp
80103349:	89 e5                	mov    %esp,%ebp
8010334b:	53                   	push   %ebx
8010334c:	83 ec 14             	sub    $0x14,%esp
8010334f:	8b 45 08             	mov    0x8(%ebp),%eax
80103352:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103356:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
8010335a:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
8010335e:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80103362:	ec                   	in     (%dx),%al
80103363:	89 c3                	mov    %eax,%ebx
80103365:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80103368:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
8010336c:	83 c4 14             	add    $0x14,%esp
8010336f:	5b                   	pop    %ebx
80103370:	5d                   	pop    %ebp
80103371:	c3                   	ret    

80103372 <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80103372:	55                   	push   %ebp
80103373:	89 e5                	mov    %esp,%ebp
80103375:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80103378:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
8010337f:	e8 c4 ff ff ff       	call   80103348 <inb>
80103384:	0f b6 c0             	movzbl %al,%eax
80103387:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
8010338a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010338d:	83 e0 01             	and    $0x1,%eax
80103390:	85 c0                	test   %eax,%eax
80103392:	75 0a                	jne    8010339e <kbdgetc+0x2c>
    return -1;
80103394:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103399:	e9 23 01 00 00       	jmp    801034c1 <kbdgetc+0x14f>
  data = inb(KBDATAP);
8010339e:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
801033a5:	e8 9e ff ff ff       	call   80103348 <inb>
801033aa:	0f b6 c0             	movzbl %al,%eax
801033ad:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
801033b0:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
801033b7:	75 17                	jne    801033d0 <kbdgetc+0x5e>
    shift |= E0ESC;
801033b9:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
801033be:	83 c8 40             	or     $0x40,%eax
801033c1:	a3 5c c6 10 80       	mov    %eax,0x8010c65c
    return 0;
801033c6:	b8 00 00 00 00       	mov    $0x0,%eax
801033cb:	e9 f1 00 00 00       	jmp    801034c1 <kbdgetc+0x14f>
  } else if(data & 0x80){
801033d0:	8b 45 fc             	mov    -0x4(%ebp),%eax
801033d3:	25 80 00 00 00       	and    $0x80,%eax
801033d8:	85 c0                	test   %eax,%eax
801033da:	74 45                	je     80103421 <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
801033dc:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
801033e1:	83 e0 40             	and    $0x40,%eax
801033e4:	85 c0                	test   %eax,%eax
801033e6:	75 08                	jne    801033f0 <kbdgetc+0x7e>
801033e8:	8b 45 fc             	mov    -0x4(%ebp),%eax
801033eb:	83 e0 7f             	and    $0x7f,%eax
801033ee:	eb 03                	jmp    801033f3 <kbdgetc+0x81>
801033f0:	8b 45 fc             	mov    -0x4(%ebp),%eax
801033f3:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
801033f6:	8b 45 fc             	mov    -0x4(%ebp),%eax
801033f9:	05 20 a0 10 80       	add    $0x8010a020,%eax
801033fe:	0f b6 00             	movzbl (%eax),%eax
80103401:	83 c8 40             	or     $0x40,%eax
80103404:	0f b6 c0             	movzbl %al,%eax
80103407:	f7 d0                	not    %eax
80103409:	89 c2                	mov    %eax,%edx
8010340b:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
80103410:	21 d0                	and    %edx,%eax
80103412:	a3 5c c6 10 80       	mov    %eax,0x8010c65c
    return 0;
80103417:	b8 00 00 00 00       	mov    $0x0,%eax
8010341c:	e9 a0 00 00 00       	jmp    801034c1 <kbdgetc+0x14f>
  } else if(shift & E0ESC){
80103421:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
80103426:	83 e0 40             	and    $0x40,%eax
80103429:	85 c0                	test   %eax,%eax
8010342b:	74 14                	je     80103441 <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
8010342d:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80103434:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
80103439:	83 e0 bf             	and    $0xffffffbf,%eax
8010343c:	a3 5c c6 10 80       	mov    %eax,0x8010c65c
  }

  shift |= shiftcode[data];
80103441:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103444:	05 20 a0 10 80       	add    $0x8010a020,%eax
80103449:	0f b6 00             	movzbl (%eax),%eax
8010344c:	0f b6 d0             	movzbl %al,%edx
8010344f:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
80103454:	09 d0                	or     %edx,%eax
80103456:	a3 5c c6 10 80       	mov    %eax,0x8010c65c
  shift ^= togglecode[data];
8010345b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010345e:	05 20 a1 10 80       	add    $0x8010a120,%eax
80103463:	0f b6 00             	movzbl (%eax),%eax
80103466:	0f b6 d0             	movzbl %al,%edx
80103469:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
8010346e:	31 d0                	xor    %edx,%eax
80103470:	a3 5c c6 10 80       	mov    %eax,0x8010c65c
  c = charcode[shift & (CTL | SHIFT)][data];
80103475:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
8010347a:	83 e0 03             	and    $0x3,%eax
8010347d:	8b 04 85 20 a5 10 80 	mov    -0x7fef5ae0(,%eax,4),%eax
80103484:	03 45 fc             	add    -0x4(%ebp),%eax
80103487:	0f b6 00             	movzbl (%eax),%eax
8010348a:	0f b6 c0             	movzbl %al,%eax
8010348d:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80103490:	a1 5c c6 10 80       	mov    0x8010c65c,%eax
80103495:	83 e0 08             	and    $0x8,%eax
80103498:	85 c0                	test   %eax,%eax
8010349a:	74 22                	je     801034be <kbdgetc+0x14c>
    if('a' <= c && c <= 'z')
8010349c:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
801034a0:	76 0c                	jbe    801034ae <kbdgetc+0x13c>
801034a2:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
801034a6:	77 06                	ja     801034ae <kbdgetc+0x13c>
      c += 'A' - 'a';
801034a8:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
801034ac:	eb 10                	jmp    801034be <kbdgetc+0x14c>
    else if('A' <= c && c <= 'Z')
801034ae:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
801034b2:	76 0a                	jbe    801034be <kbdgetc+0x14c>
801034b4:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
801034b8:	77 04                	ja     801034be <kbdgetc+0x14c>
      c += 'a' - 'A';
801034ba:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
801034be:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801034c1:	c9                   	leave  
801034c2:	c3                   	ret    

801034c3 <kbdintr>:

void
kbdintr(void)
{
801034c3:	55                   	push   %ebp
801034c4:	89 e5                	mov    %esp,%ebp
801034c6:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
801034c9:	c7 04 24 72 33 10 80 	movl   $0x80103372,(%esp)
801034d0:	e8 d8 d2 ff ff       	call   801007ad <consoleintr>
}
801034d5:	c9                   	leave  
801034d6:	c3                   	ret    
	...

801034d8 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801034d8:	55                   	push   %ebp
801034d9:	89 e5                	mov    %esp,%ebp
801034db:	83 ec 08             	sub    $0x8,%esp
801034de:	8b 55 08             	mov    0x8(%ebp),%edx
801034e1:	8b 45 0c             	mov    0xc(%ebp),%eax
801034e4:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801034e8:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801034eb:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801034ef:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801034f3:	ee                   	out    %al,(%dx)
}
801034f4:	c9                   	leave  
801034f5:	c3                   	ret    

801034f6 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
801034f6:	55                   	push   %ebp
801034f7:	89 e5                	mov    %esp,%ebp
801034f9:	53                   	push   %ebx
801034fa:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801034fd:	9c                   	pushf  
801034fe:	5b                   	pop    %ebx
801034ff:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80103502:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103505:	83 c4 10             	add    $0x10,%esp
80103508:	5b                   	pop    %ebx
80103509:	5d                   	pop    %ebp
8010350a:	c3                   	ret    

8010350b <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
8010350b:	55                   	push   %ebp
8010350c:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
8010350e:	a1 94 70 12 80       	mov    0x80127094,%eax
80103513:	8b 55 08             	mov    0x8(%ebp),%edx
80103516:	c1 e2 02             	shl    $0x2,%edx
80103519:	01 c2                	add    %eax,%edx
8010351b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010351e:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80103520:	a1 94 70 12 80       	mov    0x80127094,%eax
80103525:	83 c0 20             	add    $0x20,%eax
80103528:	8b 00                	mov    (%eax),%eax
}
8010352a:	5d                   	pop    %ebp
8010352b:	c3                   	ret    

8010352c <lapicinit>:
//PAGEBREAK!

void
lapicinit(int c)
{
8010352c:	55                   	push   %ebp
8010352d:	89 e5                	mov    %esp,%ebp
8010352f:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80103532:	a1 94 70 12 80       	mov    0x80127094,%eax
80103537:	85 c0                	test   %eax,%eax
80103539:	0f 84 47 01 00 00    	je     80103686 <lapicinit+0x15a>
    return;

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
8010353f:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80103546:	00 
80103547:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
8010354e:	e8 b8 ff ff ff       	call   8010350b <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80103553:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
8010355a:	00 
8010355b:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80103562:	e8 a4 ff ff ff       	call   8010350b <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80103567:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
8010356e:	00 
8010356f:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80103576:	e8 90 ff ff ff       	call   8010350b <lapicw>
  lapicw(TICR, 10000000); 
8010357b:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80103582:	00 
80103583:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
8010358a:	e8 7c ff ff ff       	call   8010350b <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
8010358f:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103596:	00 
80103597:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
8010359e:	e8 68 ff ff ff       	call   8010350b <lapicw>
  lapicw(LINT1, MASKED);
801035a3:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
801035aa:	00 
801035ab:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
801035b2:	e8 54 ff ff ff       	call   8010350b <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
801035b7:	a1 94 70 12 80       	mov    0x80127094,%eax
801035bc:	83 c0 30             	add    $0x30,%eax
801035bf:	8b 00                	mov    (%eax),%eax
801035c1:	c1 e8 10             	shr    $0x10,%eax
801035c4:	25 ff 00 00 00       	and    $0xff,%eax
801035c9:	83 f8 03             	cmp    $0x3,%eax
801035cc:	76 14                	jbe    801035e2 <lapicinit+0xb6>
    lapicw(PCINT, MASKED);
801035ce:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
801035d5:	00 
801035d6:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
801035dd:	e8 29 ff ff ff       	call   8010350b <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
801035e2:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
801035e9:	00 
801035ea:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
801035f1:	e8 15 ff ff ff       	call   8010350b <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
801035f6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801035fd:	00 
801035fe:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103605:	e8 01 ff ff ff       	call   8010350b <lapicw>
  lapicw(ESR, 0);
8010360a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80103611:	00 
80103612:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103619:	e8 ed fe ff ff       	call   8010350b <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
8010361e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80103625:	00 
80103626:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
8010362d:	e8 d9 fe ff ff       	call   8010350b <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80103632:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80103639:	00 
8010363a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80103641:	e8 c5 fe ff ff       	call   8010350b <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80103646:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
8010364d:	00 
8010364e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80103655:	e8 b1 fe ff ff       	call   8010350b <lapicw>
  while(lapic[ICRLO] & DELIVS)
8010365a:	90                   	nop
8010365b:	a1 94 70 12 80       	mov    0x80127094,%eax
80103660:	05 00 03 00 00       	add    $0x300,%eax
80103665:	8b 00                	mov    (%eax),%eax
80103667:	25 00 10 00 00       	and    $0x1000,%eax
8010366c:	85 c0                	test   %eax,%eax
8010366e:	75 eb                	jne    8010365b <lapicinit+0x12f>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80103670:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80103677:	00 
80103678:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
8010367f:	e8 87 fe ff ff       	call   8010350b <lapicw>
80103684:	eb 01                	jmp    80103687 <lapicinit+0x15b>

void
lapicinit(int c)
{
  if(!lapic) 
    return;
80103686:	90                   	nop
  while(lapic[ICRLO] & DELIVS)
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80103687:	c9                   	leave  
80103688:	c3                   	ret    

80103689 <cpunum>:

int
cpunum(void)
{
80103689:	55                   	push   %ebp
8010368a:	89 e5                	mov    %esp,%ebp
8010368c:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
8010368f:	e8 62 fe ff ff       	call   801034f6 <readeflags>
80103694:	25 00 02 00 00       	and    $0x200,%eax
80103699:	85 c0                	test   %eax,%eax
8010369b:	74 29                	je     801036c6 <cpunum+0x3d>
    static int n;
    if(n++ == 0)
8010369d:	a1 60 c6 10 80       	mov    0x8010c660,%eax
801036a2:	85 c0                	test   %eax,%eax
801036a4:	0f 94 c2             	sete   %dl
801036a7:	83 c0 01             	add    $0x1,%eax
801036aa:	a3 60 c6 10 80       	mov    %eax,0x8010c660
801036af:	84 d2                	test   %dl,%dl
801036b1:	74 13                	je     801036c6 <cpunum+0x3d>
      cprintf("cpu called from %x with interrupts enabled\n",
801036b3:	8b 45 04             	mov    0x4(%ebp),%eax
801036b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801036ba:	c7 04 24 48 99 10 80 	movl   $0x80109948,(%esp)
801036c1:	e8 db cc ff ff       	call   801003a1 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
801036c6:	a1 94 70 12 80       	mov    0x80127094,%eax
801036cb:	85 c0                	test   %eax,%eax
801036cd:	74 0f                	je     801036de <cpunum+0x55>
    return lapic[ID]>>24;
801036cf:	a1 94 70 12 80       	mov    0x80127094,%eax
801036d4:	83 c0 20             	add    $0x20,%eax
801036d7:	8b 00                	mov    (%eax),%eax
801036d9:	c1 e8 18             	shr    $0x18,%eax
801036dc:	eb 05                	jmp    801036e3 <cpunum+0x5a>
  return 0;
801036de:	b8 00 00 00 00       	mov    $0x0,%eax
}
801036e3:	c9                   	leave  
801036e4:	c3                   	ret    

801036e5 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
801036e5:	55                   	push   %ebp
801036e6:	89 e5                	mov    %esp,%ebp
801036e8:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
801036eb:	a1 94 70 12 80       	mov    0x80127094,%eax
801036f0:	85 c0                	test   %eax,%eax
801036f2:	74 14                	je     80103708 <lapiceoi+0x23>
    lapicw(EOI, 0);
801036f4:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801036fb:	00 
801036fc:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80103703:	e8 03 fe ff ff       	call   8010350b <lapicw>
}
80103708:	c9                   	leave  
80103709:	c3                   	ret    

8010370a <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
8010370a:	55                   	push   %ebp
8010370b:	89 e5                	mov    %esp,%ebp
}
8010370d:	5d                   	pop    %ebp
8010370e:	c3                   	ret    

8010370f <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
8010370f:	55                   	push   %ebp
80103710:	89 e5                	mov    %esp,%ebp
80103712:	83 ec 1c             	sub    $0x1c,%esp
80103715:	8b 45 08             	mov    0x8(%ebp),%eax
80103718:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
8010371b:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80103722:	00 
80103723:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
8010372a:	e8 a9 fd ff ff       	call   801034d8 <outb>
  outb(IO_RTC+1, 0x0A);
8010372f:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103736:	00 
80103737:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
8010373e:	e8 95 fd ff ff       	call   801034d8 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80103743:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
8010374a:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010374d:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80103752:	8b 45 f8             	mov    -0x8(%ebp),%eax
80103755:	8d 50 02             	lea    0x2(%eax),%edx
80103758:	8b 45 0c             	mov    0xc(%ebp),%eax
8010375b:	c1 e8 04             	shr    $0x4,%eax
8010375e:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80103761:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80103765:	c1 e0 18             	shl    $0x18,%eax
80103768:	89 44 24 04          	mov    %eax,0x4(%esp)
8010376c:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80103773:	e8 93 fd ff ff       	call   8010350b <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80103778:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
8010377f:	00 
80103780:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80103787:	e8 7f fd ff ff       	call   8010350b <lapicw>
  microdelay(200);
8010378c:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80103793:	e8 72 ff ff ff       	call   8010370a <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80103798:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
8010379f:	00 
801037a0:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
801037a7:	e8 5f fd ff ff       	call   8010350b <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
801037ac:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
801037b3:	e8 52 ff ff ff       	call   8010370a <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
801037b8:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801037bf:	eb 40                	jmp    80103801 <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
801037c1:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
801037c5:	c1 e0 18             	shl    $0x18,%eax
801037c8:	89 44 24 04          	mov    %eax,0x4(%esp)
801037cc:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
801037d3:	e8 33 fd ff ff       	call   8010350b <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
801037d8:	8b 45 0c             	mov    0xc(%ebp),%eax
801037db:	c1 e8 0c             	shr    $0xc,%eax
801037de:	80 cc 06             	or     $0x6,%ah
801037e1:	89 44 24 04          	mov    %eax,0x4(%esp)
801037e5:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
801037ec:	e8 1a fd ff ff       	call   8010350b <lapicw>
    microdelay(200);
801037f1:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
801037f8:	e8 0d ff ff ff       	call   8010370a <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
801037fd:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103801:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80103805:	7e ba                	jle    801037c1 <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80103807:	c9                   	leave  
80103808:	c3                   	ret    
80103809:	00 00                	add    %al,(%eax)
	...

8010380c <initlog>:

static void recover_from_log(void);

void
initlog(void)
{
8010380c:	55                   	push   %ebp
8010380d:	89 e5                	mov    %esp,%ebp
8010380f:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80103812:	c7 44 24 04 74 99 10 	movl   $0x80109974,0x4(%esp)
80103819:	80 
8010381a:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103821:	e8 30 23 00 00       	call   80105b56 <initlock>
  readsb(ROOTDEV, &sb);
80103826:	8d 45 e8             	lea    -0x18(%ebp),%eax
80103829:	89 44 24 04          	mov    %eax,0x4(%esp)
8010382d:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103834:	e8 b3 da ff ff       	call   801012ec <readsb>
  log.start = sb.size - sb.nlog;
80103839:	8b 55 e8             	mov    -0x18(%ebp),%edx
8010383c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010383f:	89 d1                	mov    %edx,%ecx
80103841:	29 c1                	sub    %eax,%ecx
80103843:	89 c8                	mov    %ecx,%eax
80103845:	a3 d4 70 12 80       	mov    %eax,0x801270d4
  log.size = sb.nlog;
8010384a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010384d:	a3 d8 70 12 80       	mov    %eax,0x801270d8
  log.dev = ROOTDEV;
80103852:	c7 05 e0 70 12 80 01 	movl   $0x1,0x801270e0
80103859:	00 00 00 
  recover_from_log();
8010385c:	e8 97 01 00 00       	call   801039f8 <recover_from_log>
}
80103861:	c9                   	leave  
80103862:	c3                   	ret    

80103863 <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
80103863:	55                   	push   %ebp
80103864:	89 e5                	mov    %esp,%ebp
80103866:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103869:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103870:	e9 89 00 00 00       	jmp    801038fe <install_trans+0x9b>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103875:	a1 d4 70 12 80       	mov    0x801270d4,%eax
8010387a:	03 45 f4             	add    -0xc(%ebp),%eax
8010387d:	83 c0 01             	add    $0x1,%eax
80103880:	89 c2                	mov    %eax,%edx
80103882:	a1 e0 70 12 80       	mov    0x801270e0,%eax
80103887:	89 54 24 04          	mov    %edx,0x4(%esp)
8010388b:	89 04 24             	mov    %eax,(%esp)
8010388e:	e8 13 c9 ff ff       	call   801001a6 <bread>
80103893:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
80103896:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103899:	83 c0 10             	add    $0x10,%eax
8010389c:	8b 04 85 a8 70 12 80 	mov    -0x7fed8f58(,%eax,4),%eax
801038a3:	89 c2                	mov    %eax,%edx
801038a5:	a1 e0 70 12 80       	mov    0x801270e0,%eax
801038aa:	89 54 24 04          	mov    %edx,0x4(%esp)
801038ae:	89 04 24             	mov    %eax,(%esp)
801038b1:	e8 f0 c8 ff ff       	call   801001a6 <bread>
801038b6:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801038b9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038bc:	8d 50 18             	lea    0x18(%eax),%edx
801038bf:	8b 45 ec             	mov    -0x14(%ebp),%eax
801038c2:	83 c0 18             	add    $0x18,%eax
801038c5:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801038cc:	00 
801038cd:	89 54 24 04          	mov    %edx,0x4(%esp)
801038d1:	89 04 24             	mov    %eax,(%esp)
801038d4:	e8 f8 25 00 00       	call   80105ed1 <memmove>
    bwrite(dbuf);  // write dst to disk
801038d9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801038dc:	89 04 24             	mov    %eax,(%esp)
801038df:	e8 f9 c8 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801038e4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038e7:	89 04 24             	mov    %eax,(%esp)
801038ea:	e8 28 c9 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801038ef:	8b 45 ec             	mov    -0x14(%ebp),%eax
801038f2:	89 04 24             	mov    %eax,(%esp)
801038f5:	e8 1d c9 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801038fa:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801038fe:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103903:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103906:	0f 8f 69 ff ff ff    	jg     80103875 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
8010390c:	c9                   	leave  
8010390d:	c3                   	ret    

8010390e <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
8010390e:	55                   	push   %ebp
8010390f:	89 e5                	mov    %esp,%ebp
80103911:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103914:	a1 d4 70 12 80       	mov    0x801270d4,%eax
80103919:	89 c2                	mov    %eax,%edx
8010391b:	a1 e0 70 12 80       	mov    0x801270e0,%eax
80103920:	89 54 24 04          	mov    %edx,0x4(%esp)
80103924:	89 04 24             	mov    %eax,(%esp)
80103927:	e8 7a c8 ff ff       	call   801001a6 <bread>
8010392c:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
8010392f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103932:	83 c0 18             	add    $0x18,%eax
80103935:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
80103938:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010393b:	8b 00                	mov    (%eax),%eax
8010393d:	a3 e4 70 12 80       	mov    %eax,0x801270e4
  for (i = 0; i < log.lh.n; i++) {
80103942:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103949:	eb 1b                	jmp    80103966 <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
8010394b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010394e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103951:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103955:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103958:	83 c2 10             	add    $0x10,%edx
8010395b:	89 04 95 a8 70 12 80 	mov    %eax,-0x7fed8f58(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
80103962:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103966:	a1 e4 70 12 80       	mov    0x801270e4,%eax
8010396b:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010396e:	7f db                	jg     8010394b <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
80103970:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103973:	89 04 24             	mov    %eax,(%esp)
80103976:	e8 9c c8 ff ff       	call   80100217 <brelse>
}
8010397b:	c9                   	leave  
8010397c:	c3                   	ret    

8010397d <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
8010397d:	55                   	push   %ebp
8010397e:	89 e5                	mov    %esp,%ebp
80103980:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103983:	a1 d4 70 12 80       	mov    0x801270d4,%eax
80103988:	89 c2                	mov    %eax,%edx
8010398a:	a1 e0 70 12 80       	mov    0x801270e0,%eax
8010398f:	89 54 24 04          	mov    %edx,0x4(%esp)
80103993:	89 04 24             	mov    %eax,(%esp)
80103996:	e8 0b c8 ff ff       	call   801001a6 <bread>
8010399b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
8010399e:	8b 45 f0             	mov    -0x10(%ebp),%eax
801039a1:	83 c0 18             	add    $0x18,%eax
801039a4:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
801039a7:	8b 15 e4 70 12 80    	mov    0x801270e4,%edx
801039ad:	8b 45 ec             	mov    -0x14(%ebp),%eax
801039b0:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801039b2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801039b9:	eb 1b                	jmp    801039d6 <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
801039bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039be:	83 c0 10             	add    $0x10,%eax
801039c1:	8b 0c 85 a8 70 12 80 	mov    -0x7fed8f58(,%eax,4),%ecx
801039c8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801039cb:	8b 55 f4             	mov    -0xc(%ebp),%edx
801039ce:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801039d2:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801039d6:	a1 e4 70 12 80       	mov    0x801270e4,%eax
801039db:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801039de:	7f db                	jg     801039bb <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801039e0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801039e3:	89 04 24             	mov    %eax,(%esp)
801039e6:	e8 f2 c7 ff ff       	call   801001dd <bwrite>
  brelse(buf);
801039eb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801039ee:	89 04 24             	mov    %eax,(%esp)
801039f1:	e8 21 c8 ff ff       	call   80100217 <brelse>
}
801039f6:	c9                   	leave  
801039f7:	c3                   	ret    

801039f8 <recover_from_log>:

static void
recover_from_log(void)
{
801039f8:	55                   	push   %ebp
801039f9:	89 e5                	mov    %esp,%ebp
801039fb:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801039fe:	e8 0b ff ff ff       	call   8010390e <read_head>
  install_trans(); // if committed, copy from log to disk
80103a03:	e8 5b fe ff ff       	call   80103863 <install_trans>
  log.lh.n = 0;
80103a08:	c7 05 e4 70 12 80 00 	movl   $0x0,0x801270e4
80103a0f:	00 00 00 
  write_head(); // clear the log
80103a12:	e8 66 ff ff ff       	call   8010397d <write_head>
}
80103a17:	c9                   	leave  
80103a18:	c3                   	ret    

80103a19 <begin_trans>:

void
begin_trans(void)
{
80103a19:	55                   	push   %ebp
80103a1a:	89 e5                	mov    %esp,%ebp
80103a1c:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
80103a1f:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103a26:	e8 4c 21 00 00       	call   80105b77 <acquire>
  while (log.busy) {
80103a2b:	eb 14                	jmp    80103a41 <begin_trans+0x28>
  sleep(&log, &log.lock);
80103a2d:	c7 44 24 04 a0 70 12 	movl   $0x801270a0,0x4(%esp)
80103a34:	80 
80103a35:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103a3c:	e8 c4 1c 00 00       	call   80105705 <sleep>

void
begin_trans(void)
{
  acquire(&log.lock);
  while (log.busy) {
80103a41:	a1 dc 70 12 80       	mov    0x801270dc,%eax
80103a46:	85 c0                	test   %eax,%eax
80103a48:	75 e3                	jne    80103a2d <begin_trans+0x14>
  sleep(&log, &log.lock);
  }
  log.busy = 1;
80103a4a:	c7 05 dc 70 12 80 01 	movl   $0x1,0x801270dc
80103a51:	00 00 00 
  release(&log.lock);
80103a54:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103a5b:	e8 b2 21 00 00       	call   80105c12 <release>
}
80103a60:	c9                   	leave  
80103a61:	c3                   	ret    

80103a62 <commit_trans>:

void
commit_trans(void)
{
80103a62:	55                   	push   %ebp
80103a63:	89 e5                	mov    %esp,%ebp
80103a65:	83 ec 18             	sub    $0x18,%esp
  if (log.lh.n > 0) {
80103a68:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103a6d:	85 c0                	test   %eax,%eax
80103a6f:	7e 19                	jle    80103a8a <commit_trans+0x28>
    write_head();    // Write header to disk -- the real commit
80103a71:	e8 07 ff ff ff       	call   8010397d <write_head>
    install_trans(); // Now install writes to home locations
80103a76:	e8 e8 fd ff ff       	call   80103863 <install_trans>
    log.lh.n = 0; 
80103a7b:	c7 05 e4 70 12 80 00 	movl   $0x0,0x801270e4
80103a82:	00 00 00 
    write_head();    // Erase the transaction from the log
80103a85:	e8 f3 fe ff ff       	call   8010397d <write_head>
  }
  
  acquire(&log.lock);
80103a8a:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103a91:	e8 e1 20 00 00       	call   80105b77 <acquire>
  log.busy = 0;
80103a96:	c7 05 dc 70 12 80 00 	movl   $0x0,0x801270dc
80103a9d:	00 00 00 
  wakeup(&log);
80103aa0:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103aa7:	e8 cb 1d 00 00       	call   80105877 <wakeup>
  release(&log.lock);
80103aac:	c7 04 24 a0 70 12 80 	movl   $0x801270a0,(%esp)
80103ab3:	e8 5a 21 00 00       	call   80105c12 <release>
}
80103ab8:	c9                   	leave  
80103ab9:	c3                   	ret    

80103aba <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
80103aba:	55                   	push   %ebp
80103abb:	89 e5                	mov    %esp,%ebp
80103abd:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103ac0:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103ac5:	83 f8 09             	cmp    $0x9,%eax
80103ac8:	7f 12                	jg     80103adc <log_write+0x22>
80103aca:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103acf:	8b 15 d8 70 12 80    	mov    0x801270d8,%edx
80103ad5:	83 ea 01             	sub    $0x1,%edx
80103ad8:	39 d0                	cmp    %edx,%eax
80103ada:	7c 0c                	jl     80103ae8 <log_write+0x2e>
    panic("too big a transaction");
80103adc:	c7 04 24 78 99 10 80 	movl   $0x80109978,(%esp)
80103ae3:	e8 55 ca ff ff       	call   8010053d <panic>
  if (!log.busy)
80103ae8:	a1 dc 70 12 80       	mov    0x801270dc,%eax
80103aed:	85 c0                	test   %eax,%eax
80103aef:	75 0c                	jne    80103afd <log_write+0x43>
    panic("write outside of trans");
80103af1:	c7 04 24 8e 99 10 80 	movl   $0x8010998e,(%esp)
80103af8:	e8 40 ca ff ff       	call   8010053d <panic>

  for (i = 0; i < log.lh.n; i++) {
80103afd:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103b04:	eb 1d                	jmp    80103b23 <log_write+0x69>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
80103b06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103b09:	83 c0 10             	add    $0x10,%eax
80103b0c:	8b 04 85 a8 70 12 80 	mov    -0x7fed8f58(,%eax,4),%eax
80103b13:	89 c2                	mov    %eax,%edx
80103b15:	8b 45 08             	mov    0x8(%ebp),%eax
80103b18:	8b 40 08             	mov    0x8(%eax),%eax
80103b1b:	39 c2                	cmp    %eax,%edx
80103b1d:	74 10                	je     80103b2f <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (!log.busy)
    panic("write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
80103b1f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103b23:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103b28:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103b2b:	7f d9                	jg     80103b06 <log_write+0x4c>
80103b2d:	eb 01                	jmp    80103b30 <log_write+0x76>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
      break;
80103b2f:	90                   	nop
  }
  log.lh.sector[i] = b->sector;
80103b30:	8b 45 08             	mov    0x8(%ebp),%eax
80103b33:	8b 40 08             	mov    0x8(%eax),%eax
80103b36:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103b39:	83 c2 10             	add    $0x10,%edx
80103b3c:	89 04 95 a8 70 12 80 	mov    %eax,-0x7fed8f58(,%edx,4)
  struct buf *lbuf = bread(b->dev, log.start+i+1);
80103b43:	a1 d4 70 12 80       	mov    0x801270d4,%eax
80103b48:	03 45 f4             	add    -0xc(%ebp),%eax
80103b4b:	83 c0 01             	add    $0x1,%eax
80103b4e:	89 c2                	mov    %eax,%edx
80103b50:	8b 45 08             	mov    0x8(%ebp),%eax
80103b53:	8b 40 04             	mov    0x4(%eax),%eax
80103b56:	89 54 24 04          	mov    %edx,0x4(%esp)
80103b5a:	89 04 24             	mov    %eax,(%esp)
80103b5d:	e8 44 c6 ff ff       	call   801001a6 <bread>
80103b62:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(lbuf->data, b->data, BSIZE);
80103b65:	8b 45 08             	mov    0x8(%ebp),%eax
80103b68:	8d 50 18             	lea    0x18(%eax),%edx
80103b6b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b6e:	83 c0 18             	add    $0x18,%eax
80103b71:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103b78:	00 
80103b79:	89 54 24 04          	mov    %edx,0x4(%esp)
80103b7d:	89 04 24             	mov    %eax,(%esp)
80103b80:	e8 4c 23 00 00       	call   80105ed1 <memmove>
  bwrite(lbuf);
80103b85:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b88:	89 04 24             	mov    %eax,(%esp)
80103b8b:	e8 4d c6 ff ff       	call   801001dd <bwrite>
  brelse(lbuf);
80103b90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b93:	89 04 24             	mov    %eax,(%esp)
80103b96:	e8 7c c6 ff ff       	call   80100217 <brelse>
  if (i == log.lh.n)
80103b9b:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103ba0:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103ba3:	75 0d                	jne    80103bb2 <log_write+0xf8>
    log.lh.n++;
80103ba5:	a1 e4 70 12 80       	mov    0x801270e4,%eax
80103baa:	83 c0 01             	add    $0x1,%eax
80103bad:	a3 e4 70 12 80       	mov    %eax,0x801270e4
  b->flags |= B_DIRTY; // XXX prevent eviction
80103bb2:	8b 45 08             	mov    0x8(%ebp),%eax
80103bb5:	8b 00                	mov    (%eax),%eax
80103bb7:	89 c2                	mov    %eax,%edx
80103bb9:	83 ca 04             	or     $0x4,%edx
80103bbc:	8b 45 08             	mov    0x8(%ebp),%eax
80103bbf:	89 10                	mov    %edx,(%eax)
}
80103bc1:	c9                   	leave  
80103bc2:	c3                   	ret    
	...

80103bc4 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
80103bc4:	55                   	push   %ebp
80103bc5:	89 e5                	mov    %esp,%ebp
80103bc7:	8b 45 08             	mov    0x8(%ebp),%eax
80103bca:	05 00 00 00 80       	add    $0x80000000,%eax
80103bcf:	5d                   	pop    %ebp
80103bd0:	c3                   	ret    

80103bd1 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80103bd1:	55                   	push   %ebp
80103bd2:	89 e5                	mov    %esp,%ebp
80103bd4:	8b 45 08             	mov    0x8(%ebp),%eax
80103bd7:	05 00 00 00 80       	add    $0x80000000,%eax
80103bdc:	5d                   	pop    %ebp
80103bdd:	c3                   	ret    

80103bde <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
80103bde:	55                   	push   %ebp
80103bdf:	89 e5                	mov    %esp,%ebp
80103be1:	53                   	push   %ebx
80103be2:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
80103be5:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80103be8:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
80103beb:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80103bee:	89 c3                	mov    %eax,%ebx
80103bf0:	89 d8                	mov    %ebx,%eax
80103bf2:	f0 87 02             	lock xchg %eax,(%edx)
80103bf5:	89 c3                	mov    %eax,%ebx
80103bf7:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80103bfa:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103bfd:	83 c4 10             	add    $0x10,%esp
80103c00:	5b                   	pop    %ebx
80103c01:	5d                   	pop    %ebp
80103c02:	c3                   	ret    

80103c03 <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
80103c03:	55                   	push   %ebp
80103c04:	89 e5                	mov    %esp,%ebp
80103c06:	83 e4 f0             	and    $0xfffffff0,%esp
80103c09:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103c0c:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
80103c13:	80 
80103c14:	c7 04 24 1c a4 12 80 	movl   $0x8012a41c,(%esp)
80103c1b:	e8 c2 ed ff ff       	call   801029e2 <kinit1>
  kvmalloc();      // kernel page table
80103c20:	e8 f5 52 00 00       	call   80108f1a <kvmalloc>
  mpinit();        // collect info about this machine
80103c25:	e8 63 04 00 00       	call   8010408d <mpinit>
  lapicinit(mpbcpu());
80103c2a:	e8 2e 02 00 00       	call   80103e5d <mpbcpu>
80103c2f:	89 04 24             	mov    %eax,(%esp)
80103c32:	e8 f5 f8 ff ff       	call   8010352c <lapicinit>
  seginit();       // set up segments
80103c37:	e8 81 4c 00 00       	call   801088bd <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103c3c:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103c42:	0f b6 00             	movzbl (%eax),%eax
80103c45:	0f b6 c0             	movzbl %al,%eax
80103c48:	89 44 24 04          	mov    %eax,0x4(%esp)
80103c4c:	c7 04 24 a5 99 10 80 	movl   $0x801099a5,(%esp)
80103c53:	e8 49 c7 ff ff       	call   801003a1 <cprintf>
  picinit();       // interrupt controller
80103c58:	e8 95 06 00 00       	call   801042f2 <picinit>
  ioapicinit();    // another interrupt controller
80103c5d:	e8 63 ec ff ff       	call   801028c5 <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
80103c62:	e8 26 ce ff ff       	call   80100a8d <consoleinit>
  uartinit();      // serial port
80103c67:	e8 9c 3f 00 00       	call   80107c08 <uartinit>
  pinit();         // process table
80103c6c:	e8 a3 0b 00 00       	call   80104814 <pinit>
  tvinit();        // trap vectors
80103c71:	e8 35 3b 00 00       	call   801077ab <tvinit>
  binit();         // buffer cache
80103c76:	e8 b9 c3 ff ff       	call   80100034 <binit>
  fileinit();      // file table
80103c7b:	e8 80 d2 ff ff       	call   80100f00 <fileinit>
  iinit();         // inode cache
80103c80:	e8 2e d9 ff ff       	call   801015b3 <iinit>
  ideinit();       // disk
80103c85:	e8 a2 e8 ff ff       	call   8010252c <ideinit>
  if(!ismp)
80103c8a:	a1 24 71 12 80       	mov    0x80127124,%eax
80103c8f:	85 c0                	test   %eax,%eax
80103c91:	75 05                	jne    80103c98 <main+0x95>
    timerinit();   // uniprocessor timer
80103c93:	e8 56 3a 00 00       	call   801076ee <timerinit>
  startothers();   // start other processors
80103c98:	e8 87 00 00 00       	call   80103d24 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103c9d:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
80103ca4:	8e 
80103ca5:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
80103cac:	e8 69 ed ff ff       	call   80102a1a <kinit2>
  userinit();      // first user process
80103cb1:	e8 72 12 00 00       	call   80104f28 <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
80103cb6:	e8 22 00 00 00       	call   80103cdd <mpmain>

80103cbb <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
80103cbb:	55                   	push   %ebp
80103cbc:	89 e5                	mov    %esp,%ebp
80103cbe:	83 ec 18             	sub    $0x18,%esp
  switchkvm(); 
80103cc1:	e8 6b 52 00 00       	call   80108f31 <switchkvm>
  seginit();
80103cc6:	e8 f2 4b 00 00       	call   801088bd <seginit>
  lapicinit(cpunum());
80103ccb:	e8 b9 f9 ff ff       	call   80103689 <cpunum>
80103cd0:	89 04 24             	mov    %eax,(%esp)
80103cd3:	e8 54 f8 ff ff       	call   8010352c <lapicinit>
  mpmain();
80103cd8:	e8 00 00 00 00       	call   80103cdd <mpmain>

80103cdd <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
80103cdd:	55                   	push   %ebp
80103cde:	89 e5                	mov    %esp,%ebp
80103ce0:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
80103ce3:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103ce9:	0f b6 00             	movzbl (%eax),%eax
80103cec:	0f b6 c0             	movzbl %al,%eax
80103cef:	89 44 24 04          	mov    %eax,0x4(%esp)
80103cf3:	c7 04 24 bc 99 10 80 	movl   $0x801099bc,(%esp)
80103cfa:	e8 a2 c6 ff ff       	call   801003a1 <cprintf>
  idtinit();       // load idt register
80103cff:	e8 1b 3c 00 00       	call   8010791f <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
80103d04:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103d0a:	05 a8 00 00 00       	add    $0xa8,%eax
80103d0f:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80103d16:	00 
80103d17:	89 04 24             	mov    %eax,(%esp)
80103d1a:	e8 bf fe ff ff       	call   80103bde <xchg>
  scheduler();     // start running processes
80103d1f:	e8 35 18 00 00       	call   80105559 <scheduler>

80103d24 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
80103d24:	55                   	push   %ebp
80103d25:	89 e5                	mov    %esp,%ebp
80103d27:	53                   	push   %ebx
80103d28:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
80103d2b:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
80103d32:	e8 9a fe ff ff       	call   80103bd1 <p2v>
80103d37:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
80103d3a:	b8 8a 00 00 00       	mov    $0x8a,%eax
80103d3f:	89 44 24 08          	mov    %eax,0x8(%esp)
80103d43:	c7 44 24 04 2c c5 10 	movl   $0x8010c52c,0x4(%esp)
80103d4a:	80 
80103d4b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103d4e:	89 04 24             	mov    %eax,(%esp)
80103d51:	e8 7b 21 00 00       	call   80105ed1 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103d56:	c7 45 f4 40 71 12 80 	movl   $0x80127140,-0xc(%ebp)
80103d5d:	e9 86 00 00 00       	jmp    80103de8 <startothers+0xc4>
    if(c == cpus+cpunum())  // We've started already.
80103d62:	e8 22 f9 ff ff       	call   80103689 <cpunum>
80103d67:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103d6d:	05 40 71 12 80       	add    $0x80127140,%eax
80103d72:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103d75:	74 69                	je     80103de0 <startothers+0xbc>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103d77:	e8 94 ed ff ff       	call   80102b10 <kalloc>
80103d7c:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
80103d7f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103d82:	83 e8 04             	sub    $0x4,%eax
80103d85:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103d88:	81 c2 00 10 00 00    	add    $0x1000,%edx
80103d8e:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
80103d90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103d93:	83 e8 08             	sub    $0x8,%eax
80103d96:	c7 00 bb 3c 10 80    	movl   $0x80103cbb,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
80103d9c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103d9f:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103da2:	c7 04 24 00 b0 10 80 	movl   $0x8010b000,(%esp)
80103da9:	e8 16 fe ff ff       	call   80103bc4 <v2p>
80103dae:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
80103db0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103db3:	89 04 24             	mov    %eax,(%esp)
80103db6:	e8 09 fe ff ff       	call   80103bc4 <v2p>
80103dbb:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103dbe:	0f b6 12             	movzbl (%edx),%edx
80103dc1:	0f b6 d2             	movzbl %dl,%edx
80103dc4:	89 44 24 04          	mov    %eax,0x4(%esp)
80103dc8:	89 14 24             	mov    %edx,(%esp)
80103dcb:	e8 3f f9 ff ff       	call   8010370f <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103dd0:	90                   	nop
80103dd1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103dd4:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
80103dda:	85 c0                	test   %eax,%eax
80103ddc:	74 f3                	je     80103dd1 <startothers+0xad>
80103dde:	eb 01                	jmp    80103de1 <startothers+0xbd>
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
    if(c == cpus+cpunum())  // We've started already.
      continue;
80103de0:	90                   	nop
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
80103de1:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
80103de8:	a1 20 77 12 80       	mov    0x80127720,%eax
80103ded:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103df3:	05 40 71 12 80       	add    $0x80127140,%eax
80103df8:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103dfb:	0f 87 61 ff ff ff    	ja     80103d62 <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
80103e01:	83 c4 24             	add    $0x24,%esp
80103e04:	5b                   	pop    %ebx
80103e05:	5d                   	pop    %ebp
80103e06:	c3                   	ret    
	...

80103e08 <p2v>:
80103e08:	55                   	push   %ebp
80103e09:	89 e5                	mov    %esp,%ebp
80103e0b:	8b 45 08             	mov    0x8(%ebp),%eax
80103e0e:	05 00 00 00 80       	add    $0x80000000,%eax
80103e13:	5d                   	pop    %ebp
80103e14:	c3                   	ret    

80103e15 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80103e15:	55                   	push   %ebp
80103e16:	89 e5                	mov    %esp,%ebp
80103e18:	53                   	push   %ebx
80103e19:	83 ec 14             	sub    $0x14,%esp
80103e1c:	8b 45 08             	mov    0x8(%ebp),%eax
80103e1f:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103e23:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80103e27:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80103e2b:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80103e2f:	ec                   	in     (%dx),%al
80103e30:	89 c3                	mov    %eax,%ebx
80103e32:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80103e35:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80103e39:	83 c4 14             	add    $0x14,%esp
80103e3c:	5b                   	pop    %ebx
80103e3d:	5d                   	pop    %ebp
80103e3e:	c3                   	ret    

80103e3f <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103e3f:	55                   	push   %ebp
80103e40:	89 e5                	mov    %esp,%ebp
80103e42:	83 ec 08             	sub    $0x8,%esp
80103e45:	8b 55 08             	mov    0x8(%ebp),%edx
80103e48:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e4b:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103e4f:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103e52:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103e56:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103e5a:	ee                   	out    %al,(%dx)
}
80103e5b:	c9                   	leave  
80103e5c:	c3                   	ret    

80103e5d <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
80103e5d:	55                   	push   %ebp
80103e5e:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
80103e60:	a1 64 c6 10 80       	mov    0x8010c664,%eax
80103e65:	89 c2                	mov    %eax,%edx
80103e67:	b8 40 71 12 80       	mov    $0x80127140,%eax
80103e6c:	89 d1                	mov    %edx,%ecx
80103e6e:	29 c1                	sub    %eax,%ecx
80103e70:	89 c8                	mov    %ecx,%eax
80103e72:	c1 f8 02             	sar    $0x2,%eax
80103e75:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
80103e7b:	5d                   	pop    %ebp
80103e7c:	c3                   	ret    

80103e7d <sum>:

static uchar
sum(uchar *addr, int len)
{
80103e7d:	55                   	push   %ebp
80103e7e:	89 e5                	mov    %esp,%ebp
80103e80:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
80103e83:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
80103e8a:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103e91:	eb 13                	jmp    80103ea6 <sum+0x29>
    sum += addr[i];
80103e93:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103e96:	03 45 08             	add    0x8(%ebp),%eax
80103e99:	0f b6 00             	movzbl (%eax),%eax
80103e9c:	0f b6 c0             	movzbl %al,%eax
80103e9f:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
80103ea2:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103ea6:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103ea9:	3b 45 0c             	cmp    0xc(%ebp),%eax
80103eac:	7c e5                	jl     80103e93 <sum+0x16>
    sum += addr[i];
  return sum;
80103eae:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103eb1:	c9                   	leave  
80103eb2:	c3                   	ret    

80103eb3 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103eb3:	55                   	push   %ebp
80103eb4:	89 e5                	mov    %esp,%ebp
80103eb6:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
80103eb9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ebc:	89 04 24             	mov    %eax,(%esp)
80103ebf:	e8 44 ff ff ff       	call   80103e08 <p2v>
80103ec4:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
80103ec7:	8b 45 0c             	mov    0xc(%ebp),%eax
80103eca:	03 45 f0             	add    -0x10(%ebp),%eax
80103ecd:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
80103ed0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103ed3:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103ed6:	eb 3f                	jmp    80103f17 <mpsearch1+0x64>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103ed8:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103edf:	00 
80103ee0:	c7 44 24 04 d0 99 10 	movl   $0x801099d0,0x4(%esp)
80103ee7:	80 
80103ee8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103eeb:	89 04 24             	mov    %eax,(%esp)
80103eee:	e8 82 1f 00 00       	call   80105e75 <memcmp>
80103ef3:	85 c0                	test   %eax,%eax
80103ef5:	75 1c                	jne    80103f13 <mpsearch1+0x60>
80103ef7:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
80103efe:	00 
80103eff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f02:	89 04 24             	mov    %eax,(%esp)
80103f05:	e8 73 ff ff ff       	call   80103e7d <sum>
80103f0a:	84 c0                	test   %al,%al
80103f0c:	75 05                	jne    80103f13 <mpsearch1+0x60>
      return (struct mp*)p;
80103f0e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f11:	eb 11                	jmp    80103f24 <mpsearch1+0x71>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
80103f13:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80103f17:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f1a:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103f1d:	72 b9                	jb     80103ed8 <mpsearch1+0x25>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
80103f1f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103f24:	c9                   	leave  
80103f25:	c3                   	ret    

80103f26 <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
80103f26:	55                   	push   %ebp
80103f27:	89 e5                	mov    %esp,%ebp
80103f29:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
80103f2c:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103f33:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f36:	83 c0 0f             	add    $0xf,%eax
80103f39:	0f b6 00             	movzbl (%eax),%eax
80103f3c:	0f b6 c0             	movzbl %al,%eax
80103f3f:	89 c2                	mov    %eax,%edx
80103f41:	c1 e2 08             	shl    $0x8,%edx
80103f44:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f47:	83 c0 0e             	add    $0xe,%eax
80103f4a:	0f b6 00             	movzbl (%eax),%eax
80103f4d:	0f b6 c0             	movzbl %al,%eax
80103f50:	09 d0                	or     %edx,%eax
80103f52:	c1 e0 04             	shl    $0x4,%eax
80103f55:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103f58:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103f5c:	74 21                	je     80103f7f <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
80103f5e:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103f65:	00 
80103f66:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103f69:	89 04 24             	mov    %eax,(%esp)
80103f6c:	e8 42 ff ff ff       	call   80103eb3 <mpsearch1>
80103f71:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103f74:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103f78:	74 50                	je     80103fca <mpsearch+0xa4>
      return mp;
80103f7a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103f7d:	eb 5f                	jmp    80103fde <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103f7f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f82:	83 c0 14             	add    $0x14,%eax
80103f85:	0f b6 00             	movzbl (%eax),%eax
80103f88:	0f b6 c0             	movzbl %al,%eax
80103f8b:	89 c2                	mov    %eax,%edx
80103f8d:	c1 e2 08             	shl    $0x8,%edx
80103f90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f93:	83 c0 13             	add    $0x13,%eax
80103f96:	0f b6 00             	movzbl (%eax),%eax
80103f99:	0f b6 c0             	movzbl %al,%eax
80103f9c:	09 d0                	or     %edx,%eax
80103f9e:	c1 e0 0a             	shl    $0xa,%eax
80103fa1:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103fa4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103fa7:	2d 00 04 00 00       	sub    $0x400,%eax
80103fac:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103fb3:	00 
80103fb4:	89 04 24             	mov    %eax,(%esp)
80103fb7:	e8 f7 fe ff ff       	call   80103eb3 <mpsearch1>
80103fbc:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103fbf:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103fc3:	74 05                	je     80103fca <mpsearch+0xa4>
      return mp;
80103fc5:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103fc8:	eb 14                	jmp    80103fde <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
80103fca:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103fd1:	00 
80103fd2:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
80103fd9:	e8 d5 fe ff ff       	call   80103eb3 <mpsearch1>
}
80103fde:	c9                   	leave  
80103fdf:	c3                   	ret    

80103fe0 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
80103fe0:	55                   	push   %ebp
80103fe1:	89 e5                	mov    %esp,%ebp
80103fe3:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103fe6:	e8 3b ff ff ff       	call   80103f26 <mpsearch>
80103feb:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103fee:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103ff2:	74 0a                	je     80103ffe <mpconfig+0x1e>
80103ff4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ff7:	8b 40 04             	mov    0x4(%eax),%eax
80103ffa:	85 c0                	test   %eax,%eax
80103ffc:	75 0a                	jne    80104008 <mpconfig+0x28>
    return 0;
80103ffe:	b8 00 00 00 00       	mov    $0x0,%eax
80104003:	e9 83 00 00 00       	jmp    8010408b <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
80104008:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010400b:	8b 40 04             	mov    0x4(%eax),%eax
8010400e:	89 04 24             	mov    %eax,(%esp)
80104011:	e8 f2 fd ff ff       	call   80103e08 <p2v>
80104016:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80104019:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80104020:	00 
80104021:	c7 44 24 04 d5 99 10 	movl   $0x801099d5,0x4(%esp)
80104028:	80 
80104029:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010402c:	89 04 24             	mov    %eax,(%esp)
8010402f:	e8 41 1e 00 00       	call   80105e75 <memcmp>
80104034:	85 c0                	test   %eax,%eax
80104036:	74 07                	je     8010403f <mpconfig+0x5f>
    return 0;
80104038:	b8 00 00 00 00       	mov    $0x0,%eax
8010403d:	eb 4c                	jmp    8010408b <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
8010403f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104042:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80104046:	3c 01                	cmp    $0x1,%al
80104048:	74 12                	je     8010405c <mpconfig+0x7c>
8010404a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010404d:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80104051:	3c 04                	cmp    $0x4,%al
80104053:	74 07                	je     8010405c <mpconfig+0x7c>
    return 0;
80104055:	b8 00 00 00 00       	mov    $0x0,%eax
8010405a:	eb 2f                	jmp    8010408b <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
8010405c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010405f:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80104063:	0f b7 c0             	movzwl %ax,%eax
80104066:	89 44 24 04          	mov    %eax,0x4(%esp)
8010406a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010406d:	89 04 24             	mov    %eax,(%esp)
80104070:	e8 08 fe ff ff       	call   80103e7d <sum>
80104075:	84 c0                	test   %al,%al
80104077:	74 07                	je     80104080 <mpconfig+0xa0>
    return 0;
80104079:	b8 00 00 00 00       	mov    $0x0,%eax
8010407e:	eb 0b                	jmp    8010408b <mpconfig+0xab>
  *pmp = mp;
80104080:	8b 45 08             	mov    0x8(%ebp),%eax
80104083:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104086:	89 10                	mov    %edx,(%eax)
  return conf;
80104088:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010408b:	c9                   	leave  
8010408c:	c3                   	ret    

8010408d <mpinit>:

void
mpinit(void)
{
8010408d:	55                   	push   %ebp
8010408e:	89 e5                	mov    %esp,%ebp
80104090:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80104093:	c7 05 64 c6 10 80 40 	movl   $0x80127140,0x8010c664
8010409a:	71 12 80 
  if((conf = mpconfig(&mp)) == 0)
8010409d:	8d 45 e0             	lea    -0x20(%ebp),%eax
801040a0:	89 04 24             	mov    %eax,(%esp)
801040a3:	e8 38 ff ff ff       	call   80103fe0 <mpconfig>
801040a8:	89 45 f0             	mov    %eax,-0x10(%ebp)
801040ab:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801040af:	0f 84 9c 01 00 00    	je     80104251 <mpinit+0x1c4>
    return;
  ismp = 1;
801040b5:	c7 05 24 71 12 80 01 	movl   $0x1,0x80127124
801040bc:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
801040bf:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040c2:	8b 40 24             	mov    0x24(%eax),%eax
801040c5:	a3 94 70 12 80       	mov    %eax,0x80127094
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801040ca:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040cd:	83 c0 2c             	add    $0x2c,%eax
801040d0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801040d3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040d6:	0f b7 40 04          	movzwl 0x4(%eax),%eax
801040da:	0f b7 c0             	movzwl %ax,%eax
801040dd:	03 45 f0             	add    -0x10(%ebp),%eax
801040e0:	89 45 ec             	mov    %eax,-0x14(%ebp)
801040e3:	e9 f4 00 00 00       	jmp    801041dc <mpinit+0x14f>
    switch(*p){
801040e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040eb:	0f b6 00             	movzbl (%eax),%eax
801040ee:	0f b6 c0             	movzbl %al,%eax
801040f1:	83 f8 04             	cmp    $0x4,%eax
801040f4:	0f 87 bf 00 00 00    	ja     801041b9 <mpinit+0x12c>
801040fa:	8b 04 85 18 9a 10 80 	mov    -0x7fef65e8(,%eax,4),%eax
80104101:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
80104103:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104106:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
80104109:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010410c:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80104110:	0f b6 d0             	movzbl %al,%edx
80104113:	a1 20 77 12 80       	mov    0x80127720,%eax
80104118:	39 c2                	cmp    %eax,%edx
8010411a:	74 2d                	je     80104149 <mpinit+0xbc>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
8010411c:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010411f:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80104123:	0f b6 d0             	movzbl %al,%edx
80104126:	a1 20 77 12 80       	mov    0x80127720,%eax
8010412b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010412f:	89 44 24 04          	mov    %eax,0x4(%esp)
80104133:	c7 04 24 da 99 10 80 	movl   $0x801099da,(%esp)
8010413a:	e8 62 c2 ff ff       	call   801003a1 <cprintf>
        ismp = 0;
8010413f:	c7 05 24 71 12 80 00 	movl   $0x0,0x80127124
80104146:	00 00 00 
      }
      if(proc->flags & MPBOOT)
80104149:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010414c:	0f b6 40 03          	movzbl 0x3(%eax),%eax
80104150:	0f b6 c0             	movzbl %al,%eax
80104153:	83 e0 02             	and    $0x2,%eax
80104156:	85 c0                	test   %eax,%eax
80104158:	74 15                	je     8010416f <mpinit+0xe2>
        bcpu = &cpus[ncpu];
8010415a:	a1 20 77 12 80       	mov    0x80127720,%eax
8010415f:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80104165:	05 40 71 12 80       	add    $0x80127140,%eax
8010416a:	a3 64 c6 10 80       	mov    %eax,0x8010c664
      cpus[ncpu].id = ncpu;
8010416f:	8b 15 20 77 12 80    	mov    0x80127720,%edx
80104175:	a1 20 77 12 80       	mov    0x80127720,%eax
8010417a:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80104180:	81 c2 40 71 12 80    	add    $0x80127140,%edx
80104186:	88 02                	mov    %al,(%edx)
      ncpu++;
80104188:	a1 20 77 12 80       	mov    0x80127720,%eax
8010418d:	83 c0 01             	add    $0x1,%eax
80104190:	a3 20 77 12 80       	mov    %eax,0x80127720
      p += sizeof(struct mpproc);
80104195:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80104199:	eb 41                	jmp    801041dc <mpinit+0x14f>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
8010419b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010419e:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
801041a1:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801041a4:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801041a8:	a2 20 71 12 80       	mov    %al,0x80127120
      p += sizeof(struct mpioapic);
801041ad:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801041b1:	eb 29                	jmp    801041dc <mpinit+0x14f>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
801041b3:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801041b7:	eb 23                	jmp    801041dc <mpinit+0x14f>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
801041b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041bc:	0f b6 00             	movzbl (%eax),%eax
801041bf:	0f b6 c0             	movzbl %al,%eax
801041c2:	89 44 24 04          	mov    %eax,0x4(%esp)
801041c6:	c7 04 24 f8 99 10 80 	movl   $0x801099f8,(%esp)
801041cd:	e8 cf c1 ff ff       	call   801003a1 <cprintf>
      ismp = 0;
801041d2:	c7 05 24 71 12 80 00 	movl   $0x0,0x80127124
801041d9:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801041dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041df:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801041e2:	0f 82 00 ff ff ff    	jb     801040e8 <mpinit+0x5b>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
801041e8:	a1 24 71 12 80       	mov    0x80127124,%eax
801041ed:	85 c0                	test   %eax,%eax
801041ef:	75 1d                	jne    8010420e <mpinit+0x181>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
801041f1:	c7 05 20 77 12 80 01 	movl   $0x1,0x80127720
801041f8:	00 00 00 
    lapic = 0;
801041fb:	c7 05 94 70 12 80 00 	movl   $0x0,0x80127094
80104202:	00 00 00 
    ioapicid = 0;
80104205:	c6 05 20 71 12 80 00 	movb   $0x0,0x80127120
    return;
8010420c:	eb 44                	jmp    80104252 <mpinit+0x1c5>
  }

  if(mp->imcrp){
8010420e:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104211:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
80104215:	84 c0                	test   %al,%al
80104217:	74 39                	je     80104252 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80104219:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80104220:	00 
80104221:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80104228:	e8 12 fc ff ff       	call   80103e3f <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
8010422d:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80104234:	e8 dc fb ff ff       	call   80103e15 <inb>
80104239:	83 c8 01             	or     $0x1,%eax
8010423c:	0f b6 c0             	movzbl %al,%eax
8010423f:	89 44 24 04          	mov    %eax,0x4(%esp)
80104243:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
8010424a:	e8 f0 fb ff ff       	call   80103e3f <outb>
8010424f:	eb 01                	jmp    80104252 <mpinit+0x1c5>
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
80104251:	90                   	nop
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
  }
}
80104252:	c9                   	leave  
80104253:	c3                   	ret    

80104254 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80104254:	55                   	push   %ebp
80104255:	89 e5                	mov    %esp,%ebp
80104257:	83 ec 08             	sub    $0x8,%esp
8010425a:	8b 55 08             	mov    0x8(%ebp),%edx
8010425d:	8b 45 0c             	mov    0xc(%ebp),%eax
80104260:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80104264:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80104267:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
8010426b:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
8010426f:	ee                   	out    %al,(%dx)
}
80104270:	c9                   	leave  
80104271:	c3                   	ret    

80104272 <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80104272:	55                   	push   %ebp
80104273:	89 e5                	mov    %esp,%ebp
80104275:	83 ec 0c             	sub    $0xc,%esp
80104278:	8b 45 08             	mov    0x8(%ebp),%eax
8010427b:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
8010427f:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80104283:	66 a3 00 c0 10 80    	mov    %ax,0x8010c000
  outb(IO_PIC1+1, mask);
80104289:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
8010428d:	0f b6 c0             	movzbl %al,%eax
80104290:	89 44 24 04          	mov    %eax,0x4(%esp)
80104294:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
8010429b:	e8 b4 ff ff ff       	call   80104254 <outb>
  outb(IO_PIC2+1, mask >> 8);
801042a0:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
801042a4:	66 c1 e8 08          	shr    $0x8,%ax
801042a8:	0f b6 c0             	movzbl %al,%eax
801042ab:	89 44 24 04          	mov    %eax,0x4(%esp)
801042af:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
801042b6:	e8 99 ff ff ff       	call   80104254 <outb>
}
801042bb:	c9                   	leave  
801042bc:	c3                   	ret    

801042bd <picenable>:

void
picenable(int irq)
{
801042bd:	55                   	push   %ebp
801042be:	89 e5                	mov    %esp,%ebp
801042c0:	53                   	push   %ebx
801042c1:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
801042c4:	8b 45 08             	mov    0x8(%ebp),%eax
801042c7:	ba 01 00 00 00       	mov    $0x1,%edx
801042cc:	89 d3                	mov    %edx,%ebx
801042ce:	89 c1                	mov    %eax,%ecx
801042d0:	d3 e3                	shl    %cl,%ebx
801042d2:	89 d8                	mov    %ebx,%eax
801042d4:	89 c2                	mov    %eax,%edx
801042d6:	f7 d2                	not    %edx
801042d8:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
801042df:	21 d0                	and    %edx,%eax
801042e1:	0f b7 c0             	movzwl %ax,%eax
801042e4:	89 04 24             	mov    %eax,(%esp)
801042e7:	e8 86 ff ff ff       	call   80104272 <picsetmask>
}
801042ec:	83 c4 04             	add    $0x4,%esp
801042ef:	5b                   	pop    %ebx
801042f0:	5d                   	pop    %ebp
801042f1:	c3                   	ret    

801042f2 <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
801042f2:	55                   	push   %ebp
801042f3:	89 e5                	mov    %esp,%ebp
801042f5:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
801042f8:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
801042ff:	00 
80104300:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80104307:	e8 48 ff ff ff       	call   80104254 <outb>
  outb(IO_PIC2+1, 0xFF);
8010430c:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80104313:	00 
80104314:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
8010431b:	e8 34 ff ff ff       	call   80104254 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80104320:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80104327:	00 
80104328:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
8010432f:	e8 20 ff ff ff       	call   80104254 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80104334:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
8010433b:	00 
8010433c:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80104343:	e8 0c ff ff ff       	call   80104254 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80104348:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
8010434f:	00 
80104350:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80104357:	e8 f8 fe ff ff       	call   80104254 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
8010435c:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80104363:	00 
80104364:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
8010436b:	e8 e4 fe ff ff       	call   80104254 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80104370:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80104377:	00 
80104378:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
8010437f:	e8 d0 fe ff ff       	call   80104254 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80104384:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
8010438b:	00 
8010438c:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80104393:	e8 bc fe ff ff       	call   80104254 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80104398:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
8010439f:	00 
801043a0:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
801043a7:	e8 a8 fe ff ff       	call   80104254 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
801043ac:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801043b3:	00 
801043b4:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
801043bb:	e8 94 fe ff ff       	call   80104254 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
801043c0:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
801043c7:	00 
801043c8:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
801043cf:	e8 80 fe ff ff       	call   80104254 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
801043d4:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
801043db:	00 
801043dc:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
801043e3:	e8 6c fe ff ff       	call   80104254 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
801043e8:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
801043ef:	00 
801043f0:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
801043f7:	e8 58 fe ff ff       	call   80104254 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
801043fc:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80104403:	00 
80104404:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
8010440b:	e8 44 fe ff ff       	call   80104254 <outb>

  if(irqmask != 0xFFFF)
80104410:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
80104417:	66 83 f8 ff          	cmp    $0xffff,%ax
8010441b:	74 12                	je     8010442f <picinit+0x13d>
    picsetmask(irqmask);
8010441d:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
80104424:	0f b7 c0             	movzwl %ax,%eax
80104427:	89 04 24             	mov    %eax,(%esp)
8010442a:	e8 43 fe ff ff       	call   80104272 <picsetmask>
}
8010442f:	c9                   	leave  
80104430:	c3                   	ret    
80104431:	00 00                	add    %al,(%eax)
	...

80104434 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80104434:	55                   	push   %ebp
80104435:	89 e5                	mov    %esp,%ebp
80104437:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
8010443a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80104441:	8b 45 0c             	mov    0xc(%ebp),%eax
80104444:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
8010444a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010444d:	8b 10                	mov    (%eax),%edx
8010444f:	8b 45 08             	mov    0x8(%ebp),%eax
80104452:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80104454:	e8 c3 ca ff ff       	call   80100f1c <filealloc>
80104459:	8b 55 08             	mov    0x8(%ebp),%edx
8010445c:	89 02                	mov    %eax,(%edx)
8010445e:	8b 45 08             	mov    0x8(%ebp),%eax
80104461:	8b 00                	mov    (%eax),%eax
80104463:	85 c0                	test   %eax,%eax
80104465:	0f 84 c8 00 00 00    	je     80104533 <pipealloc+0xff>
8010446b:	e8 ac ca ff ff       	call   80100f1c <filealloc>
80104470:	8b 55 0c             	mov    0xc(%ebp),%edx
80104473:	89 02                	mov    %eax,(%edx)
80104475:	8b 45 0c             	mov    0xc(%ebp),%eax
80104478:	8b 00                	mov    (%eax),%eax
8010447a:	85 c0                	test   %eax,%eax
8010447c:	0f 84 b1 00 00 00    	je     80104533 <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80104482:	e8 89 e6 ff ff       	call   80102b10 <kalloc>
80104487:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010448a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010448e:	0f 84 9e 00 00 00    	je     80104532 <pipealloc+0xfe>
    goto bad;
  p->readopen = 1;
80104494:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104497:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
8010449e:	00 00 00 
  p->writeopen = 1;
801044a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044a4:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
801044ab:	00 00 00 
  p->nwrite = 0;
801044ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044b1:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
801044b8:	00 00 00 
  p->nread = 0;
801044bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044be:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
801044c5:	00 00 00 
  initlock(&p->lock, "pipe");
801044c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044cb:	c7 44 24 04 2c 9a 10 	movl   $0x80109a2c,0x4(%esp)
801044d2:	80 
801044d3:	89 04 24             	mov    %eax,(%esp)
801044d6:	e8 7b 16 00 00       	call   80105b56 <initlock>
  (*f0)->type = FD_PIPE;
801044db:	8b 45 08             	mov    0x8(%ebp),%eax
801044de:	8b 00                	mov    (%eax),%eax
801044e0:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
801044e6:	8b 45 08             	mov    0x8(%ebp),%eax
801044e9:	8b 00                	mov    (%eax),%eax
801044eb:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
801044ef:	8b 45 08             	mov    0x8(%ebp),%eax
801044f2:	8b 00                	mov    (%eax),%eax
801044f4:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
801044f8:	8b 45 08             	mov    0x8(%ebp),%eax
801044fb:	8b 00                	mov    (%eax),%eax
801044fd:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104500:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80104503:	8b 45 0c             	mov    0xc(%ebp),%eax
80104506:	8b 00                	mov    (%eax),%eax
80104508:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
8010450e:	8b 45 0c             	mov    0xc(%ebp),%eax
80104511:	8b 00                	mov    (%eax),%eax
80104513:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80104517:	8b 45 0c             	mov    0xc(%ebp),%eax
8010451a:	8b 00                	mov    (%eax),%eax
8010451c:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80104520:	8b 45 0c             	mov    0xc(%ebp),%eax
80104523:	8b 00                	mov    (%eax),%eax
80104525:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104528:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
8010452b:	b8 00 00 00 00       	mov    $0x0,%eax
80104530:	eb 43                	jmp    80104575 <pipealloc+0x141>
  p = 0;
  *f0 = *f1 = 0;
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
    goto bad;
80104532:	90                   	nop
  (*f1)->pipe = p;
  return 0;

//PAGEBREAK: 20
 bad:
  if(p)
80104533:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104537:	74 0b                	je     80104544 <pipealloc+0x110>
    kfree((char*)p);
80104539:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010453c:	89 04 24             	mov    %eax,(%esp)
8010453f:	e8 33 e5 ff ff       	call   80102a77 <kfree>
  if(*f0)
80104544:	8b 45 08             	mov    0x8(%ebp),%eax
80104547:	8b 00                	mov    (%eax),%eax
80104549:	85 c0                	test   %eax,%eax
8010454b:	74 0d                	je     8010455a <pipealloc+0x126>
    fileclose(*f0);
8010454d:	8b 45 08             	mov    0x8(%ebp),%eax
80104550:	8b 00                	mov    (%eax),%eax
80104552:	89 04 24             	mov    %eax,(%esp)
80104555:	e8 6a ca ff ff       	call   80100fc4 <fileclose>
  if(*f1)
8010455a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010455d:	8b 00                	mov    (%eax),%eax
8010455f:	85 c0                	test   %eax,%eax
80104561:	74 0d                	je     80104570 <pipealloc+0x13c>
    fileclose(*f1);
80104563:	8b 45 0c             	mov    0xc(%ebp),%eax
80104566:	8b 00                	mov    (%eax),%eax
80104568:	89 04 24             	mov    %eax,(%esp)
8010456b:	e8 54 ca ff ff       	call   80100fc4 <fileclose>
  return -1;
80104570:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104575:	c9                   	leave  
80104576:	c3                   	ret    

80104577 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80104577:	55                   	push   %ebp
80104578:	89 e5                	mov    %esp,%ebp
8010457a:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
8010457d:	8b 45 08             	mov    0x8(%ebp),%eax
80104580:	89 04 24             	mov    %eax,(%esp)
80104583:	e8 ef 15 00 00       	call   80105b77 <acquire>
  if(writable){
80104588:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
8010458c:	74 1f                	je     801045ad <pipeclose+0x36>
    p->writeopen = 0;
8010458e:	8b 45 08             	mov    0x8(%ebp),%eax
80104591:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80104598:	00 00 00 
    wakeup(&p->nread);
8010459b:	8b 45 08             	mov    0x8(%ebp),%eax
8010459e:	05 34 02 00 00       	add    $0x234,%eax
801045a3:	89 04 24             	mov    %eax,(%esp)
801045a6:	e8 cc 12 00 00       	call   80105877 <wakeup>
801045ab:	eb 1d                	jmp    801045ca <pipeclose+0x53>
  } else {
    p->readopen = 0;
801045ad:	8b 45 08             	mov    0x8(%ebp),%eax
801045b0:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
801045b7:	00 00 00 
    wakeup(&p->nwrite);
801045ba:	8b 45 08             	mov    0x8(%ebp),%eax
801045bd:	05 38 02 00 00       	add    $0x238,%eax
801045c2:	89 04 24             	mov    %eax,(%esp)
801045c5:	e8 ad 12 00 00       	call   80105877 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
801045ca:	8b 45 08             	mov    0x8(%ebp),%eax
801045cd:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
801045d3:	85 c0                	test   %eax,%eax
801045d5:	75 25                	jne    801045fc <pipeclose+0x85>
801045d7:	8b 45 08             	mov    0x8(%ebp),%eax
801045da:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
801045e0:	85 c0                	test   %eax,%eax
801045e2:	75 18                	jne    801045fc <pipeclose+0x85>
    release(&p->lock);
801045e4:	8b 45 08             	mov    0x8(%ebp),%eax
801045e7:	89 04 24             	mov    %eax,(%esp)
801045ea:	e8 23 16 00 00       	call   80105c12 <release>
    kfree((char*)p);
801045ef:	8b 45 08             	mov    0x8(%ebp),%eax
801045f2:	89 04 24             	mov    %eax,(%esp)
801045f5:	e8 7d e4 ff ff       	call   80102a77 <kfree>
801045fa:	eb 0b                	jmp    80104607 <pipeclose+0x90>
  } else
    release(&p->lock);
801045fc:	8b 45 08             	mov    0x8(%ebp),%eax
801045ff:	89 04 24             	mov    %eax,(%esp)
80104602:	e8 0b 16 00 00       	call   80105c12 <release>
}
80104607:	c9                   	leave  
80104608:	c3                   	ret    

80104609 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80104609:	55                   	push   %ebp
8010460a:	89 e5                	mov    %esp,%ebp
8010460c:	53                   	push   %ebx
8010460d:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80104610:	8b 45 08             	mov    0x8(%ebp),%eax
80104613:	89 04 24             	mov    %eax,(%esp)
80104616:	e8 5c 15 00 00       	call   80105b77 <acquire>
  for(i = 0; i < n; i++){
8010461b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104622:	e9 a6 00 00 00       	jmp    801046cd <pipewrite+0xc4>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || proc->killed){
80104627:	8b 45 08             	mov    0x8(%ebp),%eax
8010462a:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80104630:	85 c0                	test   %eax,%eax
80104632:	74 0d                	je     80104641 <pipewrite+0x38>
80104634:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010463a:	8b 40 24             	mov    0x24(%eax),%eax
8010463d:	85 c0                	test   %eax,%eax
8010463f:	74 15                	je     80104656 <pipewrite+0x4d>
        release(&p->lock);
80104641:	8b 45 08             	mov    0x8(%ebp),%eax
80104644:	89 04 24             	mov    %eax,(%esp)
80104647:	e8 c6 15 00 00       	call   80105c12 <release>
        return -1;
8010464c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104651:	e9 9d 00 00 00       	jmp    801046f3 <pipewrite+0xea>
      }
      wakeup(&p->nread);
80104656:	8b 45 08             	mov    0x8(%ebp),%eax
80104659:	05 34 02 00 00       	add    $0x234,%eax
8010465e:	89 04 24             	mov    %eax,(%esp)
80104661:	e8 11 12 00 00       	call   80105877 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80104666:	8b 45 08             	mov    0x8(%ebp),%eax
80104669:	8b 55 08             	mov    0x8(%ebp),%edx
8010466c:	81 c2 38 02 00 00    	add    $0x238,%edx
80104672:	89 44 24 04          	mov    %eax,0x4(%esp)
80104676:	89 14 24             	mov    %edx,(%esp)
80104679:	e8 87 10 00 00       	call   80105705 <sleep>
8010467e:	eb 01                	jmp    80104681 <pipewrite+0x78>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80104680:	90                   	nop
80104681:	8b 45 08             	mov    0x8(%ebp),%eax
80104684:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
8010468a:	8b 45 08             	mov    0x8(%ebp),%eax
8010468d:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80104693:	05 00 02 00 00       	add    $0x200,%eax
80104698:	39 c2                	cmp    %eax,%edx
8010469a:	74 8b                	je     80104627 <pipewrite+0x1e>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
8010469c:	8b 45 08             	mov    0x8(%ebp),%eax
8010469f:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
801046a5:	89 c3                	mov    %eax,%ebx
801046a7:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
801046ad:	8b 55 f4             	mov    -0xc(%ebp),%edx
801046b0:	03 55 0c             	add    0xc(%ebp),%edx
801046b3:	0f b6 0a             	movzbl (%edx),%ecx
801046b6:	8b 55 08             	mov    0x8(%ebp),%edx
801046b9:	88 4c 1a 34          	mov    %cl,0x34(%edx,%ebx,1)
801046bd:	8d 50 01             	lea    0x1(%eax),%edx
801046c0:	8b 45 08             	mov    0x8(%ebp),%eax
801046c3:	89 90 38 02 00 00    	mov    %edx,0x238(%eax)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
801046c9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801046cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046d0:	3b 45 10             	cmp    0x10(%ebp),%eax
801046d3:	7c ab                	jl     80104680 <pipewrite+0x77>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
801046d5:	8b 45 08             	mov    0x8(%ebp),%eax
801046d8:	05 34 02 00 00       	add    $0x234,%eax
801046dd:	89 04 24             	mov    %eax,(%esp)
801046e0:	e8 92 11 00 00       	call   80105877 <wakeup>
  release(&p->lock);
801046e5:	8b 45 08             	mov    0x8(%ebp),%eax
801046e8:	89 04 24             	mov    %eax,(%esp)
801046eb:	e8 22 15 00 00       	call   80105c12 <release>
  return n;
801046f0:	8b 45 10             	mov    0x10(%ebp),%eax
}
801046f3:	83 c4 24             	add    $0x24,%esp
801046f6:	5b                   	pop    %ebx
801046f7:	5d                   	pop    %ebp
801046f8:	c3                   	ret    

801046f9 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
801046f9:	55                   	push   %ebp
801046fa:	89 e5                	mov    %esp,%ebp
801046fc:	53                   	push   %ebx
801046fd:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80104700:	8b 45 08             	mov    0x8(%ebp),%eax
80104703:	89 04 24             	mov    %eax,(%esp)
80104706:	e8 6c 14 00 00       	call   80105b77 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
8010470b:	eb 3a                	jmp    80104747 <piperead+0x4e>
    if(proc->killed){
8010470d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104713:	8b 40 24             	mov    0x24(%eax),%eax
80104716:	85 c0                	test   %eax,%eax
80104718:	74 15                	je     8010472f <piperead+0x36>
      release(&p->lock);
8010471a:	8b 45 08             	mov    0x8(%ebp),%eax
8010471d:	89 04 24             	mov    %eax,(%esp)
80104720:	e8 ed 14 00 00       	call   80105c12 <release>
      return -1;
80104725:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010472a:	e9 b6 00 00 00       	jmp    801047e5 <piperead+0xec>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
8010472f:	8b 45 08             	mov    0x8(%ebp),%eax
80104732:	8b 55 08             	mov    0x8(%ebp),%edx
80104735:	81 c2 34 02 00 00    	add    $0x234,%edx
8010473b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010473f:	89 14 24             	mov    %edx,(%esp)
80104742:	e8 be 0f 00 00       	call   80105705 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80104747:	8b 45 08             	mov    0x8(%ebp),%eax
8010474a:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80104750:	8b 45 08             	mov    0x8(%ebp),%eax
80104753:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104759:	39 c2                	cmp    %eax,%edx
8010475b:	75 0d                	jne    8010476a <piperead+0x71>
8010475d:	8b 45 08             	mov    0x8(%ebp),%eax
80104760:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80104766:	85 c0                	test   %eax,%eax
80104768:	75 a3                	jne    8010470d <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
8010476a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104771:	eb 49                	jmp    801047bc <piperead+0xc3>
    if(p->nread == p->nwrite)
80104773:	8b 45 08             	mov    0x8(%ebp),%eax
80104776:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
8010477c:	8b 45 08             	mov    0x8(%ebp),%eax
8010477f:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104785:	39 c2                	cmp    %eax,%edx
80104787:	74 3d                	je     801047c6 <piperead+0xcd>
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
80104789:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010478c:	89 c2                	mov    %eax,%edx
8010478e:	03 55 0c             	add    0xc(%ebp),%edx
80104791:	8b 45 08             	mov    0x8(%ebp),%eax
80104794:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
8010479a:	89 c3                	mov    %eax,%ebx
8010479c:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
801047a2:	8b 4d 08             	mov    0x8(%ebp),%ecx
801047a5:	0f b6 4c 19 34       	movzbl 0x34(%ecx,%ebx,1),%ecx
801047aa:	88 0a                	mov    %cl,(%edx)
801047ac:	8d 50 01             	lea    0x1(%eax),%edx
801047af:	8b 45 08             	mov    0x8(%ebp),%eax
801047b2:	89 90 34 02 00 00    	mov    %edx,0x234(%eax)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
801047b8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801047bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047bf:	3b 45 10             	cmp    0x10(%ebp),%eax
801047c2:	7c af                	jl     80104773 <piperead+0x7a>
801047c4:	eb 01                	jmp    801047c7 <piperead+0xce>
    if(p->nread == p->nwrite)
      break;
801047c6:	90                   	nop
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
801047c7:	8b 45 08             	mov    0x8(%ebp),%eax
801047ca:	05 38 02 00 00       	add    $0x238,%eax
801047cf:	89 04 24             	mov    %eax,(%esp)
801047d2:	e8 a0 10 00 00       	call   80105877 <wakeup>
  release(&p->lock);
801047d7:	8b 45 08             	mov    0x8(%ebp),%eax
801047da:	89 04 24             	mov    %eax,(%esp)
801047dd:	e8 30 14 00 00       	call   80105c12 <release>
  return i;
801047e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801047e5:	83 c4 24             	add    $0x24,%esp
801047e8:	5b                   	pop    %ebx
801047e9:	5d                   	pop    %ebp
801047ea:	c3                   	ret    
	...

801047ec <p2v>:
801047ec:	55                   	push   %ebp
801047ed:	89 e5                	mov    %esp,%ebp
801047ef:	8b 45 08             	mov    0x8(%ebp),%eax
801047f2:	05 00 00 00 80       	add    $0x80000000,%eax
801047f7:	5d                   	pop    %ebp
801047f8:	c3                   	ret    

801047f9 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
801047f9:	55                   	push   %ebp
801047fa:	89 e5                	mov    %esp,%ebp
801047fc:	53                   	push   %ebx
801047fd:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104800:	9c                   	pushf  
80104801:	5b                   	pop    %ebx
80104802:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80104805:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80104808:	83 c4 10             	add    $0x10,%esp
8010480b:	5b                   	pop    %ebx
8010480c:	5d                   	pop    %ebp
8010480d:	c3                   	ret    

8010480e <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
8010480e:	55                   	push   %ebp
8010480f:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104811:	fb                   	sti    
}
80104812:	5d                   	pop    %ebp
80104813:	c3                   	ret    

80104814 <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
80104814:	55                   	push   %ebp
80104815:	89 e5                	mov    %esp,%ebp
80104817:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
8010481a:	c7 44 24 04 31 9a 10 	movl   $0x80109a31,0x4(%esp)
80104821:	80 
80104822:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104829:	e8 28 13 00 00       	call   80105b56 <initlock>
  initlock(&swaplock, "swaplock");
8010482e:	c7 44 24 04 38 9a 10 	movl   $0x80109a38,0x4(%esp)
80104835:	80 
80104836:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
8010483d:	e8 14 13 00 00       	call   80105b56 <initlock>
}
80104842:	c9                   	leave  
80104843:	c3                   	ret    

80104844 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
80104844:	55                   	push   %ebp
80104845:	89 e5                	mov    %esp,%ebp
80104847:	83 ec 38             	sub    $0x38,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
8010484a:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104851:	e8 21 13 00 00       	call   80105b77 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104856:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
8010485d:	eb 11                	jmp    80104870 <allocproc+0x2c>
    if(p->state == UNUSED)
8010485f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104862:	8b 40 0c             	mov    0xc(%eax),%eax
80104865:	85 c0                	test   %eax,%eax
80104867:	74 26                	je     8010488f <allocproc+0x4b>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104869:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80104870:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
80104877:	72 e6                	jb     8010485f <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
80104879:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104880:	e8 8d 13 00 00       	call   80105c12 <release>
  return 0;
80104885:	b8 00 00 00 00       	mov    $0x0,%eax
8010488a:	e9 5a 01 00 00       	jmp    801049e9 <allocproc+0x1a5>
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == UNUSED)
      goto found;
8010488f:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80104890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104893:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
8010489a:	a1 04 c0 10 80       	mov    0x8010c004,%eax
8010489f:	8b 55 f4             	mov    -0xc(%ebp),%edx
801048a2:	89 42 10             	mov    %eax,0x10(%edx)
801048a5:	83 c0 01             	add    $0x1,%eax
801048a8:	a3 04 c0 10 80       	mov    %eax,0x8010c004
  release(&ptable.lock);
801048ad:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801048b4:	e8 59 13 00 00       	call   80105c12 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
801048b9:	e8 52 e2 ff ff       	call   80102b10 <kalloc>
801048be:	8b 55 f4             	mov    -0xc(%ebp),%edx
801048c1:	89 42 08             	mov    %eax,0x8(%edx)
801048c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048c7:	8b 40 08             	mov    0x8(%eax),%eax
801048ca:	85 c0                	test   %eax,%eax
801048cc:	75 14                	jne    801048e2 <allocproc+0x9e>
    p->state = UNUSED;
801048ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048d1:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
801048d8:	b8 00 00 00 00       	mov    $0x0,%eax
801048dd:	e9 07 01 00 00       	jmp    801049e9 <allocproc+0x1a5>
  }
  sp = p->kstack + KSTACKSIZE;
801048e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048e5:	8b 40 08             	mov    0x8(%eax),%eax
801048e8:	05 00 10 00 00       	add    $0x1000,%eax
801048ed:	89 45 f0             	mov    %eax,-0x10(%ebp)
  
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801048f0:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
801048f4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048f7:	8b 55 f0             	mov    -0x10(%ebp),%edx
801048fa:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
801048fd:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
80104901:	ba 60 77 10 80       	mov    $0x80107760,%edx
80104906:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104909:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
8010490b:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
8010490f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104912:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104915:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
80104918:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010491b:	8b 40 1c             	mov    0x1c(%eax),%eax
8010491e:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80104925:	00 
80104926:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010492d:	00 
8010492e:	89 04 24             	mov    %eax,(%esp)
80104931:	e8 c8 14 00 00       	call   80105dfe <memset>
  p->context->eip = (uint)forkret;
80104936:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104939:	8b 40 1c             	mov    0x1c(%eax),%eax
8010493c:	ba d9 56 10 80       	mov    $0x801056d9,%edx
80104941:	89 50 10             	mov    %edx,0x10(%eax)
  int i = 0;
80104944:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  char name[8];
  name[2] = '.'; name[3] = 's'; name[4] = 'w'; name[5] = 'a'; name[6] = 'p'; name[7] = 0;
8010494b:	c6 45 e6 2e          	movb   $0x2e,-0x1a(%ebp)
8010494f:	c6 45 e7 73          	movb   $0x73,-0x19(%ebp)
80104953:	c6 45 e8 77          	movb   $0x77,-0x18(%ebp)
80104957:	c6 45 e9 61          	movb   $0x61,-0x17(%ebp)
8010495b:	c6 45 ea 70          	movb   $0x70,-0x16(%ebp)
8010495f:	c6 45 eb 00          	movb   $0x0,-0x15(%ebp)
  name[1] = (char)(((int)'0')+p->pid % 10);
80104963:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104966:	8b 48 10             	mov    0x10(%eax),%ecx
80104969:	ba 67 66 66 66       	mov    $0x66666667,%edx
8010496e:	89 c8                	mov    %ecx,%eax
80104970:	f7 ea                	imul   %edx
80104972:	c1 fa 02             	sar    $0x2,%edx
80104975:	89 c8                	mov    %ecx,%eax
80104977:	c1 f8 1f             	sar    $0x1f,%eax
8010497a:	29 c2                	sub    %eax,%edx
8010497c:	89 d0                	mov    %edx,%eax
8010497e:	c1 e0 02             	shl    $0x2,%eax
80104981:	01 d0                	add    %edx,%eax
80104983:	01 c0                	add    %eax,%eax
80104985:	89 ca                	mov    %ecx,%edx
80104987:	29 c2                	sub    %eax,%edx
80104989:	89 d0                	mov    %edx,%eax
8010498b:	83 c0 30             	add    $0x30,%eax
8010498e:	88 45 e5             	mov    %al,-0x1b(%ebp)
  if((i=p->pid/10) == 0)
80104991:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104994:	8b 48 10             	mov    0x10(%eax),%ecx
80104997:	ba 67 66 66 66       	mov    $0x66666667,%edx
8010499c:	89 c8                	mov    %ecx,%eax
8010499e:	f7 ea                	imul   %edx
801049a0:	c1 fa 02             	sar    $0x2,%edx
801049a3:	89 c8                	mov    %ecx,%eax
801049a5:	c1 f8 1f             	sar    $0x1f,%eax
801049a8:	89 d1                	mov    %edx,%ecx
801049aa:	29 c1                	sub    %eax,%ecx
801049ac:	89 c8                	mov    %ecx,%eax
801049ae:	89 45 ec             	mov    %eax,-0x14(%ebp)
801049b1:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801049b5:	75 06                	jne    801049bd <allocproc+0x179>
    name[0] = '0';
801049b7:	c6 45 e4 30          	movb   $0x30,-0x1c(%ebp)
801049bb:	eb 09                	jmp    801049c6 <allocproc+0x182>
  else
    name[0] = (char)(((int)'0')+i);
801049bd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801049c0:	83 c0 30             	add    $0x30,%eax
801049c3:	88 45 e4             	mov    %al,-0x1c(%ebp)
  //release(&ptable.lock);
  safestrcpy(p->swapFileName, name, sizeof(name));
801049c6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049c9:	8d 90 80 00 00 00    	lea    0x80(%eax),%edx
801049cf:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801049d6:	00 
801049d7:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801049da:	89 44 24 04          	mov    %eax,0x4(%esp)
801049de:	89 14 24             	mov    %edx,(%esp)
801049e1:	e8 48 16 00 00       	call   8010602e <safestrcpy>
  return p;
801049e6:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801049e9:	c9                   	leave  
801049ea:	c3                   	ret    

801049eb <createInternalProcess>:


void createInternalProcess(const char *name, void (*entrypoint)())
{
801049eb:	55                   	push   %ebp
801049ec:	89 e5                	mov    %esp,%ebp
801049ee:	83 ec 28             	sub    $0x28,%esp
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
801049f1:	e8 4e fe ff ff       	call   80104844 <allocproc>
801049f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
801049f9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801049fd:	0f 84 f7 00 00 00    	je     80104afa <createInternalProcess+0x10f>
    return;

  // Copy process state from p.
  if((np->pgdir = setupkvm(kalloc)) == 0)
80104a03:	c7 04 24 10 2b 10 80 	movl   $0x80102b10,(%esp)
80104a0a:	e8 4e 44 00 00       	call   80108e5d <setupkvm>
80104a0f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104a12:	89 42 04             	mov    %eax,0x4(%edx)
80104a15:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a18:	8b 40 04             	mov    0x4(%eax),%eax
80104a1b:	85 c0                	test   %eax,%eax
80104a1d:	75 0c                	jne    80104a2b <createInternalProcess+0x40>
      panic("inswapper: out of memory?");
80104a1f:	c7 04 24 41 9a 10 80 	movl   $0x80109a41,(%esp)
80104a26:	e8 12 bb ff ff       	call   8010053d <panic>

  np->sz = PGSIZE;
80104a2b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a2e:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  np->parent = initproc;
80104a34:	8b 15 88 c6 10 80    	mov    0x8010c688,%edx
80104a3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a3d:	89 50 14             	mov    %edx,0x14(%eax)
  memset(np->tf, 0, sizeof(*np->tf));
80104a40:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a43:	8b 40 18             	mov    0x18(%eax),%eax
80104a46:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
80104a4d:	00 
80104a4e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104a55:	00 
80104a56:	89 04 24             	mov    %eax,(%esp)
80104a59:	e8 a0 13 00 00       	call   80105dfe <memset>
  np->tf->cs = (SEG_KCODE << 3)|0;
80104a5e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a61:	8b 40 18             	mov    0x18(%eax),%eax
80104a64:	66 c7 40 3c 08 00    	movw   $0x8,0x3c(%eax)
  np->tf->ds = (SEG_KDATA << 3)|0;
80104a6a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a6d:	8b 40 18             	mov    0x18(%eax),%eax
80104a70:	66 c7 40 2c 10 00    	movw   $0x10,0x2c(%eax)
  np->tf->es = np->tf->ds;
80104a76:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a79:	8b 40 18             	mov    0x18(%eax),%eax
80104a7c:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104a7f:	8b 52 18             	mov    0x18(%edx),%edx
80104a82:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104a86:	66 89 50 28          	mov    %dx,0x28(%eax)
  np->tf->ss = np->tf->ds;
80104a8a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a8d:	8b 40 18             	mov    0x18(%eax),%eax
80104a90:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104a93:	8b 52 18             	mov    0x18(%edx),%edx
80104a96:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104a9a:	66 89 50 48          	mov    %dx,0x48(%eax)
  np->tf->eflags = FL_IF;
80104a9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aa1:	8b 40 18             	mov    0x18(%eax),%eax
80104aa4:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  //np->tf->esp = (uint)entrypoint+PGSIZE;
  //np->tf->eip = (uint)entrypoint;
  np->context->eip = (uint)entrypoint;
80104aab:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aae:	8b 40 1c             	mov    0x1c(%eax),%eax
80104ab1:	8b 55 0c             	mov    0xc(%ebp),%edx
80104ab4:	89 50 10             	mov    %edx,0x10(%eax)

  inswapper = np;
80104ab7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aba:	a3 8c c6 10 80       	mov    %eax,0x8010c68c
  np->cwd = namei("/");
80104abf:	c7 04 24 5b 9a 10 80 	movl   $0x80109a5b,(%esp)
80104ac6:	e8 3f d9 ff ff       	call   8010240a <namei>
80104acb:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104ace:	89 42 68             	mov    %eax,0x68(%edx)
  safestrcpy(np->name, name, sizeof(name));
80104ad1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ad4:	8d 50 6c             	lea    0x6c(%eax),%edx
80104ad7:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80104ade:	00 
80104adf:	8b 45 08             	mov    0x8(%ebp),%eax
80104ae2:	89 44 24 04          	mov    %eax,0x4(%esp)
80104ae6:	89 14 24             	mov    %edx,(%esp)
80104ae9:	e8 40 15 00 00       	call   8010602e <safestrcpy>
  np->state = RUNNABLE;
80104aee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104af1:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
80104af8:	eb 01                	jmp    80104afb <createInternalProcess+0x110>
{
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
    return;
80104afa:	90                   	nop

  inswapper = np;
  np->cwd = namei("/");
  safestrcpy(np->name, name, sizeof(name));
  np->state = RUNNABLE;
}
80104afb:	c9                   	leave  
80104afc:	c3                   	ret    

80104afd <swapIn>:

void swapIn()
{
80104afd:	55                   	push   %ebp
80104afe:	89 e5                	mov    %esp,%ebp
80104b00:	83 ec 38             	sub    $0x38,%esp
  struct proc* t;
  for(;;)
  {
swapin:
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
80104b03:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
80104b0a:	e9 ff 01 00 00       	jmp    80104d0e <swapIn+0x211>
    {
      if(t->state != RUNNABLE_SUSPENDED)
80104b0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b12:	8b 40 0c             	mov    0xc(%eax),%eax
80104b15:	83 f8 07             	cmp    $0x7,%eax
80104b18:	0f 85 e8 01 00 00    	jne    80104d06 <swapIn+0x209>
	continue;
      
      //open file pid.swap
      if(holding(&ptable.lock))
80104b1e:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104b25:	e8 a4 11 00 00       	call   80105cce <holding>
80104b2a:	85 c0                	test   %eax,%eax
80104b2c:	74 0c                	je     80104b3a <swapIn+0x3d>
	release(&ptable.lock);
80104b2e:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104b35:	e8 d8 10 00 00       	call   80105c12 <release>
      if((t->swap = fileopen(t->swapFileName,O_RDONLY)) == 0)
80104b3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b3d:	83 e8 80             	sub    $0xffffff80,%eax
80104b40:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104b47:	00 
80104b48:	89 04 24             	mov    %eax,(%esp)
80104b4b:	e8 8f 21 00 00       	call   80106cdf <fileopen>
80104b50:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104b53:	89 42 7c             	mov    %eax,0x7c(%edx)
80104b56:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b59:	8b 40 7c             	mov    0x7c(%eax),%eax
80104b5c:	85 c0                	test   %eax,%eax
80104b5e:	75 1d                	jne    80104b7d <swapIn+0x80>
      {
	cprintf("fileopen failed\n");
80104b60:	c7 04 24 5d 9a 10 80 	movl   $0x80109a5d,(%esp)
80104b67:	e8 35 b8 ff ff       	call   801003a1 <cprintf>
	acquire(&ptable.lock);
80104b6c:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104b73:	e8 ff 0f 00 00       	call   80105b77 <acquire>
	break;
80104b78:	e9 9e 01 00 00       	jmp    80104d1b <swapIn+0x21e>
      }
      acquire(&ptable.lock);
80104b7d:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104b84:	e8 ee 0f 00 00       	call   80105b77 <acquire>
            
      // allocate virtual memory
      if((t->pgdir = setupkvm(kalloc)) == 0)
80104b89:	c7 04 24 10 2b 10 80 	movl   $0x80102b10,(%esp)
80104b90:	e8 c8 42 00 00       	call   80108e5d <setupkvm>
80104b95:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104b98:	89 42 04             	mov    %eax,0x4(%edx)
80104b9b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b9e:	8b 40 04             	mov    0x4(%eax),%eax
80104ba1:	85 c0                	test   %eax,%eax
80104ba3:	75 0c                	jne    80104bb1 <swapIn+0xb4>
	panic("inswapper: out of memory?");
80104ba5:	c7 04 24 41 9a 10 80 	movl   $0x80109a41,(%esp)
80104bac:	e8 8c b9 ff ff       	call   8010053d <panic>
      if(!allocuvm(t->pgdir, 0, t->sz))
80104bb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104bb4:	8b 10                	mov    (%eax),%edx
80104bb6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104bb9:	8b 40 04             	mov    0x4(%eax),%eax
80104bbc:	89 54 24 08          	mov    %edx,0x8(%esp)
80104bc0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104bc7:	00 
80104bc8:	89 04 24             	mov    %eax,(%esp)
80104bcb:	e8 5f 46 00 00       	call   8010922f <allocuvm>
80104bd0:	85 c0                	test   %eax,%eax
80104bd2:	75 11                	jne    80104be5 <swapIn+0xe8>
      {
	cprintf("allocuvm failed\n");
80104bd4:	c7 04 24 6e 9a 10 80 	movl   $0x80109a6e,(%esp)
80104bdb:	e8 c1 b7 ff ff       	call   801003a1 <cprintf>
	break;
80104be0:	e9 36 01 00 00       	jmp    80104d1b <swapIn+0x21e>
      }
      
      if(holding(&ptable.lock))
80104be5:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104bec:	e8 dd 10 00 00       	call   80105cce <holding>
80104bf1:	85 c0                	test   %eax,%eax
80104bf3:	74 0c                	je     80104c01 <swapIn+0x104>
	release(&ptable.lock);
80104bf5:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104bfc:	e8 11 10 00 00       	call   80105c12 <release>
      loaduvm(t->pgdir,0,t->swap->ip,0,t->sz);
80104c01:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c04:	8b 08                	mov    (%eax),%ecx
80104c06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c09:	8b 40 7c             	mov    0x7c(%eax),%eax
80104c0c:	8b 50 10             	mov    0x10(%eax),%edx
80104c0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c12:	8b 40 04             	mov    0x4(%eax),%eax
80104c15:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80104c19:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80104c20:	00 
80104c21:	89 54 24 08          	mov    %edx,0x8(%esp)
80104c25:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104c2c:	00 
80104c2d:	89 04 24             	mov    %eax,(%esp)
80104c30:	e8 0b 45 00 00       	call   80109140 <loaduvm>
      
      t->isSwapped = 0;
80104c35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c38:	c7 80 88 00 00 00 00 	movl   $0x0,0x88(%eax)
80104c3f:	00 00 00 
      int fd;
      for(fd = 0; fd < NOFILE; fd++)
80104c42:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104c49:	eb 60                	jmp    80104cab <swapIn+0x1ae>
      {
	//cprintf("fd = %d, t->ofile[fd] = %d, t->swap = %d\n",fd,proc->ofile[fd], t->swap);
	if(proc->ofile[fd] && proc->ofile[fd] == t->swap)
80104c4b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c51:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104c54:	83 c2 08             	add    $0x8,%edx
80104c57:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104c5b:	85 c0                	test   %eax,%eax
80104c5d:	74 48                	je     80104ca7 <swapIn+0x1aa>
80104c5f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c65:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104c68:	83 c2 08             	add    $0x8,%edx
80104c6b:	8b 54 90 08          	mov    0x8(%eax,%edx,4),%edx
80104c6f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c72:	8b 40 7c             	mov    0x7c(%eax),%eax
80104c75:	39 c2                	cmp    %eax,%edx
80104c77:	75 2e                	jne    80104ca7 <swapIn+0x1aa>
	{
	  //cprintf("fileclose swap in\n");
	  fileclose(proc->ofile[fd]);
80104c79:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c7f:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104c82:	83 c2 08             	add    $0x8,%edx
80104c85:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104c89:	89 04 24             	mov    %eax,(%esp)
80104c8c:	e8 33 c3 ff ff       	call   80100fc4 <fileclose>
	  proc->ofile[fd] = 0;
80104c91:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c97:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104c9a:	83 c2 08             	add    $0x8,%edx
80104c9d:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80104ca4:	00 
	  break;
80104ca5:	eb 0a                	jmp    80104cb1 <swapIn+0x1b4>
	release(&ptable.lock);
      loaduvm(t->pgdir,0,t->swap->ip,0,t->sz);
      
      t->isSwapped = 0;
      int fd;
      for(fd = 0; fd < NOFILE; fd++)
80104ca7:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104cab:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
80104caf:	7e 9a                	jle    80104c4b <swapIn+0x14e>
	  fileclose(proc->ofile[fd]);
	  proc->ofile[fd] = 0;
	  break;
	}
      }
      t->swap=0;
80104cb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cb4:	c7 40 7c 00 00 00 00 	movl   $0x0,0x7c(%eax)
      unlink(t->swapFileName);
80104cbb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cbe:	83 e8 80             	sub    $0xffffff80,%eax
80104cc1:	89 04 24             	mov    %eax,(%esp)
80104cc4:	e8 d1 1a 00 00       	call   8010679a <unlink>
      
      acquire(&ptable.lock);
80104cc9:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80104cd0:	e8 a2 0e 00 00       	call   80105b77 <acquire>
      t->state = RUNNABLE;
80104cd5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cd8:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      
      acquire(&swaplock);
80104cdf:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80104ce6:	e8 8c 0e 00 00       	call   80105b77 <acquire>
      swappedout--;
80104ceb:	a1 84 c6 10 80       	mov    0x8010c684,%eax
80104cf0:	83 e8 01             	sub    $0x1,%eax
80104cf3:	a3 84 c6 10 80       	mov    %eax,0x8010c684
      release(&swaplock);
80104cf8:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80104cff:	e8 0e 0f 00 00       	call   80105c12 <release>
80104d04:	eb 01                	jmp    80104d07 <swapIn+0x20a>
  {
swapin:
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
    {
      if(t->state != RUNNABLE_SUSPENDED)
	continue;
80104d06:	90                   	nop
{
  struct proc* t;
  for(;;)
  {
swapin:
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
80104d07:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80104d0e:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
80104d15:	0f 82 f4 fd ff ff    	jb     80104b0f <swapIn+0x12>
      acquire(&swaplock);
      swappedout--;
      release(&swaplock);
    }
   
    acquire(&swaplock);
80104d1b:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80104d22:	e8 50 0e 00 00       	call   80105b77 <acquire>
    if(swappedout > 0)
80104d27:	a1 84 c6 10 80       	mov    0x8010c684,%eax
80104d2c:	85 c0                	test   %eax,%eax
80104d2e:	7e 11                	jle    80104d41 <swapIn+0x244>
    {
      release(&swaplock);
80104d30:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80104d37:	e8 d6 0e 00 00       	call   80105c12 <release>
      goto swapin;
80104d3c:	e9 c2 fd ff ff       	jmp    80104b03 <swapIn+0x6>
    }
    else
      release(&swaplock);
80104d41:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80104d48:	e8 c5 0e 00 00       	call   80105c12 <release>

    proc->chan = inswapper;
80104d4d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d53:	8b 15 8c c6 10 80    	mov    0x8010c68c,%edx
80104d59:	89 50 20             	mov    %edx,0x20(%eax)
    proc->state = SLEEPING;
80104d5c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d62:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
     
     sched();
80104d69:	e8 87 08 00 00       	call   801055f5 <sched>
     proc->chan = 0;
80104d6e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d74:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)
  }
80104d7b:	e9 83 fd ff ff       	jmp    80104b03 <swapIn+0x6>

80104d80 <swapOut>:
}

void
swapOut()
{
80104d80:	55                   	push   %ebp
80104d81:	89 e5                	mov    %esp,%ebp
80104d83:	53                   	push   %ebx
80104d84:	83 ec 24             	sub    $0x24,%esp
    proc->swap = fileopen(proc->swapFileName,(O_CREATE | O_RDWR));
80104d87:	65 8b 1d 04 00 00 00 	mov    %gs:0x4,%ebx
80104d8e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d94:	83 e8 80             	sub    $0xffffff80,%eax
80104d97:	c7 44 24 04 02 02 00 	movl   $0x202,0x4(%esp)
80104d9e:	00 
80104d9f:	89 04 24             	mov    %eax,(%esp)
80104da2:	e8 38 1f 00 00       	call   80106cdf <fileopen>
80104da7:	89 43 7c             	mov    %eax,0x7c(%ebx)
    pte_t *pte;
    uint pa, j;
    for(j = 0; j < proc->sz; j += PGSIZE)
80104daa:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104db1:	e9 9a 00 00 00       	jmp    80104e50 <swapOut+0xd0>
    {
      if((pte = walkpgdir(proc->pgdir, (void *) j, 0)) == 0)
80104db6:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104db9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104dbf:	8b 40 04             	mov    0x4(%eax),%eax
80104dc2:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80104dc9:	00 
80104dca:	89 54 24 04          	mov    %edx,0x4(%esp)
80104dce:	89 04 24             	mov    %eax,(%esp)
80104dd1:	e8 5d 3f 00 00       	call   80108d33 <walkpgdir>
80104dd6:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104dd9:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80104ddd:	75 0c                	jne    80104deb <swapOut+0x6b>
	panic("walkpgdir: pte should exist");
80104ddf:	c7 04 24 7f 9a 10 80 	movl   $0x80109a7f,(%esp)
80104de6:	e8 52 b7 ff ff       	call   8010053d <panic>
      if(!(*pte & PTE_P))
80104deb:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104dee:	8b 00                	mov    (%eax),%eax
80104df0:	83 e0 01             	and    $0x1,%eax
80104df3:	85 c0                	test   %eax,%eax
80104df5:	75 0c                	jne    80104e03 <swapOut+0x83>
	panic("walkpgdir: page not present");
80104df7:	c7 04 24 9b 9a 10 80 	movl   $0x80109a9b,(%esp)
80104dfe:	e8 3a b7 ff ff       	call   8010053d <panic>
      pa = PTE_ADDR(*pte);
80104e03:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104e06:	8b 00                	mov    (%eax),%eax
80104e08:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80104e0d:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(filewrite(proc->swap, (char*)p2v(pa), PGSIZE) < 0)
80104e10:	8b 45 e8             	mov    -0x18(%ebp),%eax
80104e13:	89 04 24             	mov    %eax,(%esp)
80104e16:	e8 d1 f9 ff ff       	call   801047ec <p2v>
80104e1b:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104e22:	8b 52 7c             	mov    0x7c(%edx),%edx
80104e25:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80104e2c:	00 
80104e2d:	89 44 24 04          	mov    %eax,0x4(%esp)
80104e31:	89 14 24             	mov    %edx,(%esp)
80104e34:	e8 6c c3 ff ff       	call   801011a5 <filewrite>
80104e39:	85 c0                	test   %eax,%eax
80104e3b:	79 0c                	jns    80104e49 <swapOut+0xc9>
	panic("filewrite failed");
80104e3d:	c7 04 24 b7 9a 10 80 	movl   $0x80109ab7,(%esp)
80104e44:	e8 f4 b6 ff ff       	call   8010053d <panic>
swapOut()
{
    proc->swap = fileopen(proc->swapFileName,(O_CREATE | O_RDWR));
    pte_t *pte;
    uint pa, j;
    for(j = 0; j < proc->sz; j += PGSIZE)
80104e49:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80104e50:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e56:	8b 00                	mov    (%eax),%eax
80104e58:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80104e5b:	0f 87 55 ff ff ff    	ja     80104db6 <swapOut+0x36>
      if(filewrite(proc->swap, (char*)p2v(pa), PGSIZE) < 0)
	panic("filewrite failed");
    }

    int fd;
    for(fd = 0; fd < NOFILE; fd++)
80104e61:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104e68:	eb 63                	jmp    80104ecd <swapOut+0x14d>
    {
      if(proc->ofile[fd] && proc->ofile[fd] == proc->swap)
80104e6a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e70:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104e73:	83 c2 08             	add    $0x8,%edx
80104e76:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104e7a:	85 c0                	test   %eax,%eax
80104e7c:	74 4b                	je     80104ec9 <swapOut+0x149>
80104e7e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e84:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104e87:	83 c2 08             	add    $0x8,%edx
80104e8a:	8b 54 90 08          	mov    0x8(%eax,%edx,4),%edx
80104e8e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e94:	8b 40 7c             	mov    0x7c(%eax),%eax
80104e97:	39 c2                	cmp    %eax,%edx
80104e99:	75 2e                	jne    80104ec9 <swapOut+0x149>
      {
	fileclose(proc->ofile[fd]);
80104e9b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104ea1:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104ea4:	83 c2 08             	add    $0x8,%edx
80104ea7:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104eab:	89 04 24             	mov    %eax,(%esp)
80104eae:	e8 11 c1 ff ff       	call   80100fc4 <fileclose>
	proc->ofile[fd] = 0;
80104eb3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104eb9:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104ebc:	83 c2 08             	add    $0x8,%edx
80104ebf:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80104ec6:	00 
	break;
80104ec7:	eb 0a                	jmp    80104ed3 <swapOut+0x153>
      if(filewrite(proc->swap, (char*)p2v(pa), PGSIZE) < 0)
	panic("filewrite failed");
    }

    int fd;
    for(fd = 0; fd < NOFILE; fd++)
80104ec9:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104ecd:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
80104ed1:	7e 97                	jle    80104e6a <swapOut+0xea>
	fileclose(proc->ofile[fd]);
	proc->ofile[fd] = 0;
	break;
      }
    }
    proc->swap=0;
80104ed3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104ed9:	c7 40 7c 00 00 00 00 	movl   $0x0,0x7c(%eax)
    //freevm(proc->pgdir);
    deallocuvm(proc->pgdir,proc->sz,0);
80104ee0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104ee6:	8b 10                	mov    (%eax),%edx
80104ee8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104eee:	8b 40 04             	mov    0x4(%eax),%eax
80104ef1:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80104ef8:	00 
80104ef9:	89 54 24 04          	mov    %edx,0x4(%esp)
80104efd:	89 04 24             	mov    %eax,(%esp)
80104f00:	e8 04 44 00 00       	call   80109309 <deallocuvm>
    proc->state = SLEEPING_SUSPENDED;
80104f05:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104f0b:	c7 40 0c 06 00 00 00 	movl   $0x6,0xc(%eax)
    proc->isSwapped = 1;
80104f12:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104f18:	c7 80 88 00 00 00 01 	movl   $0x1,0x88(%eax)
80104f1f:	00 00 00 
}
80104f22:	83 c4 24             	add    $0x24,%esp
80104f25:	5b                   	pop    %ebx
80104f26:	5d                   	pop    %ebp
80104f27:	c3                   	ret    

80104f28 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
80104f28:	55                   	push   %ebp
80104f29:	89 e5                	mov    %esp,%ebp
80104f2b:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
80104f2e:	e8 11 f9 ff ff       	call   80104844 <allocproc>
80104f33:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
80104f36:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f39:	a3 88 c6 10 80       	mov    %eax,0x8010c688
  if((p->pgdir = setupkvm(kalloc)) == 0)
80104f3e:	c7 04 24 10 2b 10 80 	movl   $0x80102b10,(%esp)
80104f45:	e8 13 3f 00 00       	call   80108e5d <setupkvm>
80104f4a:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104f4d:	89 42 04             	mov    %eax,0x4(%edx)
80104f50:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f53:	8b 40 04             	mov    0x4(%eax),%eax
80104f56:	85 c0                	test   %eax,%eax
80104f58:	75 0c                	jne    80104f66 <userinit+0x3e>
    panic("userinit: out of memory?");
80104f5a:	c7 04 24 c8 9a 10 80 	movl   $0x80109ac8,(%esp)
80104f61:	e8 d7 b5 ff ff       	call   8010053d <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
80104f66:	ba 2c 00 00 00       	mov    $0x2c,%edx
80104f6b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f6e:	8b 40 04             	mov    0x4(%eax),%eax
80104f71:	89 54 24 08          	mov    %edx,0x8(%esp)
80104f75:	c7 44 24 04 00 c5 10 	movl   $0x8010c500,0x4(%esp)
80104f7c:	80 
80104f7d:	89 04 24             	mov    %eax,(%esp)
80104f80:	e8 30 41 00 00       	call   801090b5 <inituvm>
  p->sz = PGSIZE;
80104f85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f88:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
80104f8e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f91:	8b 40 18             	mov    0x18(%eax),%eax
80104f94:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
80104f9b:	00 
80104f9c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104fa3:	00 
80104fa4:	89 04 24             	mov    %eax,(%esp)
80104fa7:	e8 52 0e 00 00       	call   80105dfe <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104fac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104faf:	8b 40 18             	mov    0x18(%eax),%eax
80104fb2:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80104fb8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fbb:	8b 40 18             	mov    0x18(%eax),%eax
80104fbe:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
80104fc4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fc7:	8b 40 18             	mov    0x18(%eax),%eax
80104fca:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104fcd:	8b 52 18             	mov    0x18(%edx),%edx
80104fd0:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104fd4:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
80104fd8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fdb:	8b 40 18             	mov    0x18(%eax),%eax
80104fde:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104fe1:	8b 52 18             	mov    0x18(%edx),%edx
80104fe4:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104fe8:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
80104fec:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fef:	8b 40 18             	mov    0x18(%eax),%eax
80104ff2:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80104ff9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ffc:	8b 40 18             	mov    0x18(%eax),%eax
80104fff:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
80105006:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105009:	8b 40 18             	mov    0x18(%eax),%eax
8010500c:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
80105013:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105016:	83 c0 6c             	add    $0x6c,%eax
80105019:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105020:	00 
80105021:	c7 44 24 04 e1 9a 10 	movl   $0x80109ae1,0x4(%esp)
80105028:	80 
80105029:	89 04 24             	mov    %eax,(%esp)
8010502c:	e8 fd 0f 00 00       	call   8010602e <safestrcpy>
  p->cwd = namei("/");
80105031:	c7 04 24 5b 9a 10 80 	movl   $0x80109a5b,(%esp)
80105038:	e8 cd d3 ff ff       	call   8010240a <namei>
8010503d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105040:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
80105043:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105046:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)

  createInternalProcess("inswapper", swapIn);
8010504d:	c7 44 24 04 fd 4a 10 	movl   $0x80104afd,0x4(%esp)
80105054:	80 
80105055:	c7 04 24 ea 9a 10 80 	movl   $0x80109aea,(%esp)
8010505c:	e8 8a f9 ff ff       	call   801049eb <createInternalProcess>
}
80105061:	c9                   	leave  
80105062:	c3                   	ret    

80105063 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
80105063:	55                   	push   %ebp
80105064:	89 e5                	mov    %esp,%ebp
80105066:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
80105069:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010506f:	8b 00                	mov    (%eax),%eax
80105071:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
80105074:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80105078:	7e 34                	jle    801050ae <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
8010507a:	8b 45 08             	mov    0x8(%ebp),%eax
8010507d:	89 c2                	mov    %eax,%edx
8010507f:	03 55 f4             	add    -0xc(%ebp),%edx
80105082:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105088:	8b 40 04             	mov    0x4(%eax),%eax
8010508b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010508f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105092:	89 54 24 04          	mov    %edx,0x4(%esp)
80105096:	89 04 24             	mov    %eax,(%esp)
80105099:	e8 91 41 00 00       	call   8010922f <allocuvm>
8010509e:	89 45 f4             	mov    %eax,-0xc(%ebp)
801050a1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801050a5:	75 41                	jne    801050e8 <growproc+0x85>
      return -1;
801050a7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801050ac:	eb 58                	jmp    80105106 <growproc+0xa3>
  } else if(n < 0){
801050ae:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801050b2:	79 34                	jns    801050e8 <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
801050b4:	8b 45 08             	mov    0x8(%ebp),%eax
801050b7:	89 c2                	mov    %eax,%edx
801050b9:	03 55 f4             	add    -0xc(%ebp),%edx
801050bc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801050c2:	8b 40 04             	mov    0x4(%eax),%eax
801050c5:	89 54 24 08          	mov    %edx,0x8(%esp)
801050c9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801050cc:	89 54 24 04          	mov    %edx,0x4(%esp)
801050d0:	89 04 24             	mov    %eax,(%esp)
801050d3:	e8 31 42 00 00       	call   80109309 <deallocuvm>
801050d8:	89 45 f4             	mov    %eax,-0xc(%ebp)
801050db:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801050df:	75 07                	jne    801050e8 <growproc+0x85>
      return -1;
801050e1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801050e6:	eb 1e                	jmp    80105106 <growproc+0xa3>
  }
  proc->sz = sz;
801050e8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801050ee:	8b 55 f4             	mov    -0xc(%ebp),%edx
801050f1:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
801050f3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801050f9:	89 04 24             	mov    %eax,(%esp)
801050fc:	e8 4d 3e 00 00       	call   80108f4e <switchuvm>
  return 0;
80105101:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105106:	c9                   	leave  
80105107:	c3                   	ret    

80105108 <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
80105108:	55                   	push   %ebp
80105109:	89 e5                	mov    %esp,%ebp
8010510b:	57                   	push   %edi
8010510c:	56                   	push   %esi
8010510d:	53                   	push   %ebx
8010510e:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
80105111:	e8 2e f7 ff ff       	call   80104844 <allocproc>
80105116:	89 45 e0             	mov    %eax,-0x20(%ebp)
80105119:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
8010511d:	75 0a                	jne    80105129 <fork+0x21>
    return -1;
8010511f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105124:	e9 3a 01 00 00       	jmp    80105263 <fork+0x15b>
  
  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
80105129:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010512f:	8b 10                	mov    (%eax),%edx
80105131:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105137:	8b 40 04             	mov    0x4(%eax),%eax
8010513a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010513e:	89 04 24             	mov    %eax,(%esp)
80105141:	e8 53 43 00 00       	call   80109499 <copyuvm>
80105146:	8b 55 e0             	mov    -0x20(%ebp),%edx
80105149:	89 42 04             	mov    %eax,0x4(%edx)
8010514c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010514f:	8b 40 04             	mov    0x4(%eax),%eax
80105152:	85 c0                	test   %eax,%eax
80105154:	75 2c                	jne    80105182 <fork+0x7a>
    kfree(np->kstack);
80105156:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105159:	8b 40 08             	mov    0x8(%eax),%eax
8010515c:	89 04 24             	mov    %eax,(%esp)
8010515f:	e8 13 d9 ff ff       	call   80102a77 <kfree>
    np->kstack = 0;
80105164:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105167:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
8010516e:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105171:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
80105178:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010517d:	e9 e1 00 00 00       	jmp    80105263 <fork+0x15b>
  }
  np->sz = proc->sz;
80105182:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105188:	8b 10                	mov    (%eax),%edx
8010518a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010518d:	89 10                	mov    %edx,(%eax)
  np->parent = proc;
8010518f:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80105196:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105199:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *proc->tf;
8010519c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010519f:	8b 50 18             	mov    0x18(%eax),%edx
801051a2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801051a8:	8b 40 18             	mov    0x18(%eax),%eax
801051ab:	89 c3                	mov    %eax,%ebx
801051ad:	b8 13 00 00 00       	mov    $0x13,%eax
801051b2:	89 d7                	mov    %edx,%edi
801051b4:	89 de                	mov    %ebx,%esi
801051b6:	89 c1                	mov    %eax,%ecx
801051b8:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
801051ba:	8b 45 e0             	mov    -0x20(%ebp),%eax
801051bd:	8b 40 18             	mov    0x18(%eax),%eax
801051c0:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
801051c7:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801051ce:	eb 3d                	jmp    8010520d <fork+0x105>
    if(proc->ofile[i])
801051d0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801051d6:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801051d9:	83 c2 08             	add    $0x8,%edx
801051dc:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801051e0:	85 c0                	test   %eax,%eax
801051e2:	74 25                	je     80105209 <fork+0x101>
      np->ofile[i] = filedup(proc->ofile[i]);
801051e4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801051ea:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801051ed:	83 c2 08             	add    $0x8,%edx
801051f0:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801051f4:	89 04 24             	mov    %eax,(%esp)
801051f7:	e8 80 bd ff ff       	call   80100f7c <filedup>
801051fc:	8b 55 e0             	mov    -0x20(%ebp),%edx
801051ff:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80105202:	83 c1 08             	add    $0x8,%ecx
80105205:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *proc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
80105209:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
8010520d:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
80105211:	7e bd                	jle    801051d0 <fork+0xc8>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
80105213:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105219:	8b 40 68             	mov    0x68(%eax),%eax
8010521c:	89 04 24             	mov    %eax,(%esp)
8010521f:	e8 12 c6 ff ff       	call   80101836 <idup>
80105224:	8b 55 e0             	mov    -0x20(%ebp),%edx
80105227:	89 42 68             	mov    %eax,0x68(%edx)
 
  pid = np->pid;
8010522a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010522d:	8b 40 10             	mov    0x10(%eax),%eax
80105230:	89 45 dc             	mov    %eax,-0x24(%ebp)
  np->state = RUNNABLE;
80105233:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105236:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  safestrcpy(np->name, proc->name, sizeof(proc->name));
8010523d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105243:	8d 50 6c             	lea    0x6c(%eax),%edx
80105246:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105249:	83 c0 6c             	add    $0x6c,%eax
8010524c:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105253:	00 
80105254:	89 54 24 04          	mov    %edx,0x4(%esp)
80105258:	89 04 24             	mov    %eax,(%esp)
8010525b:	e8 ce 0d 00 00       	call   8010602e <safestrcpy>
  return pid;
80105260:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
80105263:	83 c4 2c             	add    $0x2c,%esp
80105266:	5b                   	pop    %ebx
80105267:	5e                   	pop    %esi
80105268:	5f                   	pop    %edi
80105269:	5d                   	pop    %ebp
8010526a:	c3                   	ret    

8010526b <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
8010526b:	55                   	push   %ebp
8010526c:	89 e5                	mov    %esp,%ebp
8010526e:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
80105271:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80105278:	a1 88 c6 10 80       	mov    0x8010c688,%eax
8010527d:	39 c2                	cmp    %eax,%edx
8010527f:	75 0c                	jne    8010528d <exit+0x22>
    panic("init exiting");
80105281:	c7 04 24 f4 9a 10 80 	movl   $0x80109af4,(%esp)
80105288:	e8 b0 b2 ff ff       	call   8010053d <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
8010528d:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80105294:	eb 44                	jmp    801052da <exit+0x6f>
    if(proc->ofile[fd]){
80105296:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010529c:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010529f:	83 c2 08             	add    $0x8,%edx
801052a2:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801052a6:	85 c0                	test   %eax,%eax
801052a8:	74 2c                	je     801052d6 <exit+0x6b>
      fileclose(proc->ofile[fd]);
801052aa:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052b0:	8b 55 f0             	mov    -0x10(%ebp),%edx
801052b3:	83 c2 08             	add    $0x8,%edx
801052b6:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801052ba:	89 04 24             	mov    %eax,(%esp)
801052bd:	e8 02 bd ff ff       	call   80100fc4 <fileclose>
      proc->ofile[fd] = 0;
801052c2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052c8:	8b 55 f0             	mov    -0x10(%ebp),%edx
801052cb:	83 c2 08             	add    $0x8,%edx
801052ce:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801052d5:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801052d6:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
801052da:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801052de:	7e b6                	jle    80105296 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  iput(proc->cwd);
801052e0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052e6:	8b 40 68             	mov    0x68(%eax),%eax
801052e9:	89 04 24             	mov    %eax,(%esp)
801052ec:	e8 2a c7 ff ff       	call   80101a1b <iput>
  proc->cwd = 0;
801052f1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052f7:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)
  
  if(proc->has_shm)
801052fe:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105304:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
8010530a:	85 c0                	test   %eax,%eax
8010530c:	74 11                	je     8010531f <exit+0xb4>
    deallocshm(proc->pid);		//deallocate any shared memory segments proc did not shmdt
8010530e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105314:	8b 40 10             	mov    0x10(%eax),%eax
80105317:	89 04 24             	mov    %eax,(%esp)
8010531a:	e8 77 de ff ff       	call   80103196 <deallocshm>
  
  acquire(&ptable.lock);
8010531f:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105326:	e8 4c 08 00 00       	call   80105b77 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
8010532b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105331:	8b 40 14             	mov    0x14(%eax),%eax
80105334:	89 04 24             	mov    %eax,(%esp)
80105337:	e8 98 04 00 00       	call   801057d4 <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010533c:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
80105343:	eb 3b                	jmp    80105380 <exit+0x115>
    if(p->parent == proc){
80105345:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105348:	8b 50 14             	mov    0x14(%eax),%edx
8010534b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105351:	39 c2                	cmp    %eax,%edx
80105353:	75 24                	jne    80105379 <exit+0x10e>
      p->parent = initproc;
80105355:	8b 15 88 c6 10 80    	mov    0x8010c688,%edx
8010535b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010535e:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
80105361:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105364:	8b 40 0c             	mov    0xc(%eax),%eax
80105367:	83 f8 05             	cmp    $0x5,%eax
8010536a:	75 0d                	jne    80105379 <exit+0x10e>
        wakeup1(initproc);
8010536c:	a1 88 c6 10 80       	mov    0x8010c688,%eax
80105371:	89 04 24             	mov    %eax,(%esp)
80105374:	e8 5b 04 00 00       	call   801057d4 <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105379:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80105380:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
80105387:	72 bc                	jb     80105345 <exit+0xda>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
80105389:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010538f:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
80105396:	e8 5a 02 00 00       	call   801055f5 <sched>
  panic("zombie exit");
8010539b:	c7 04 24 01 9b 10 80 	movl   $0x80109b01,(%esp)
801053a2:	e8 96 b1 ff ff       	call   8010053d <panic>

801053a7 <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
801053a7:	55                   	push   %ebp
801053a8:	89 e5                	mov    %esp,%ebp
801053aa:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
801053ad:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801053b4:	e8 be 07 00 00       	call   80105b77 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
801053b9:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801053c0:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
801053c7:	e9 9d 00 00 00       	jmp    80105469 <wait+0xc2>
      if(p->parent != proc)
801053cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053cf:	8b 50 14             	mov    0x14(%eax),%edx
801053d2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801053d8:	39 c2                	cmp    %eax,%edx
801053da:	0f 85 81 00 00 00    	jne    80105461 <wait+0xba>
        continue;
      havekids = 1;
801053e0:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
801053e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053ea:	8b 40 0c             	mov    0xc(%eax),%eax
801053ed:	83 f8 05             	cmp    $0x5,%eax
801053f0:	75 70                	jne    80105462 <wait+0xbb>
        // Found one.
        pid = p->pid;
801053f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053f5:	8b 40 10             	mov    0x10(%eax),%eax
801053f8:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
801053fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053fe:	8b 40 08             	mov    0x8(%eax),%eax
80105401:	89 04 24             	mov    %eax,(%esp)
80105404:	e8 6e d6 ff ff       	call   80102a77 <kfree>
        p->kstack = 0;
80105409:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010540c:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
80105413:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105416:	8b 40 04             	mov    0x4(%eax),%eax
80105419:	89 04 24             	mov    %eax,(%esp)
8010541c:	e8 a4 3f 00 00       	call   801093c5 <freevm>
        p->state = UNUSED;
80105421:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105424:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
8010542b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010542e:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
80105435:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105438:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
8010543f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105442:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
80105446:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105449:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
80105450:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105457:	e8 b6 07 00 00       	call   80105c12 <release>
        return pid;
8010545c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010545f:	eb 56                	jmp    801054b7 <wait+0x110>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc)
        continue;
80105461:	90                   	nop

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105462:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80105469:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
80105470:	0f 82 56 ff ff ff    	jb     801053cc <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
80105476:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010547a:	74 0d                	je     80105489 <wait+0xe2>
8010547c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105482:	8b 40 24             	mov    0x24(%eax),%eax
80105485:	85 c0                	test   %eax,%eax
80105487:	74 13                	je     8010549c <wait+0xf5>
      release(&ptable.lock);
80105489:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105490:	e8 7d 07 00 00       	call   80105c12 <release>
      return -1;
80105495:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010549a:	eb 1b                	jmp    801054b7 <wait+0x110>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
8010549c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054a2:	c7 44 24 04 40 77 12 	movl   $0x80127740,0x4(%esp)
801054a9:	80 
801054aa:	89 04 24             	mov    %eax,(%esp)
801054ad:	e8 53 02 00 00       	call   80105705 <sleep>
  }
801054b2:	e9 02 ff ff ff       	jmp    801053b9 <wait+0x12>
}
801054b7:	c9                   	leave  
801054b8:	c3                   	ret    

801054b9 <register_handler>:

void
register_handler(sighandler_t sighandler)
{
801054b9:	55                   	push   %ebp
801054ba:	89 e5                	mov    %esp,%ebp
801054bc:	83 ec 28             	sub    $0x28,%esp
  char* addr = uva2ka(proc->pgdir, (char*)proc->tf->esp);
801054bf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054c5:	8b 40 18             	mov    0x18(%eax),%eax
801054c8:	8b 40 44             	mov    0x44(%eax),%eax
801054cb:	89 c2                	mov    %eax,%edx
801054cd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054d3:	8b 40 04             	mov    0x4(%eax),%eax
801054d6:	89 54 24 04          	mov    %edx,0x4(%esp)
801054da:	89 04 24             	mov    %eax,(%esp)
801054dd:	e8 c8 40 00 00       	call   801095aa <uva2ka>
801054e2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if ((proc->tf->esp & 0xFFF) == 0)
801054e5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054eb:	8b 40 18             	mov    0x18(%eax),%eax
801054ee:	8b 40 44             	mov    0x44(%eax),%eax
801054f1:	25 ff 0f 00 00       	and    $0xfff,%eax
801054f6:	85 c0                	test   %eax,%eax
801054f8:	75 0c                	jne    80105506 <register_handler+0x4d>
    panic("esp_offset == 0");
801054fa:	c7 04 24 0d 9b 10 80 	movl   $0x80109b0d,(%esp)
80105501:	e8 37 b0 ff ff       	call   8010053d <panic>

    /* open a new frame */
  *(int*)(addr + ((proc->tf->esp - 4) & 0xFFF))
80105506:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010550c:	8b 40 18             	mov    0x18(%eax),%eax
8010550f:	8b 40 44             	mov    0x44(%eax),%eax
80105512:	83 e8 04             	sub    $0x4,%eax
80105515:	25 ff 0f 00 00       	and    $0xfff,%eax
8010551a:	03 45 f4             	add    -0xc(%ebp),%eax
          = proc->tf->eip;
8010551d:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80105524:	8b 52 18             	mov    0x18(%edx),%edx
80105527:	8b 52 38             	mov    0x38(%edx),%edx
8010552a:	89 10                	mov    %edx,(%eax)
  proc->tf->esp -= 4;
8010552c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105532:	8b 40 18             	mov    0x18(%eax),%eax
80105535:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
8010553c:	8b 52 18             	mov    0x18(%edx),%edx
8010553f:	8b 52 44             	mov    0x44(%edx),%edx
80105542:	83 ea 04             	sub    $0x4,%edx
80105545:	89 50 44             	mov    %edx,0x44(%eax)

    /* update eip */
  proc->tf->eip = (uint)sighandler;
80105548:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010554e:	8b 40 18             	mov    0x18(%eax),%eax
80105551:	8b 55 08             	mov    0x8(%ebp),%edx
80105554:	89 50 38             	mov    %edx,0x38(%eax)
}
80105557:	c9                   	leave  
80105558:	c3                   	ret    

80105559 <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
80105559:	55                   	push   %ebp
8010555a:	89 e5                	mov    %esp,%ebp
8010555c:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  
  for(;;){
    // Enable interrupts on this processor.
    sti();
8010555f:	e8 aa f2 ff ff       	call   8010480e <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
80105564:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
8010556b:	e8 07 06 00 00       	call   80105b77 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105570:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
80105577:	eb 62                	jmp    801055db <scheduler+0x82>
      if(p->state != RUNNABLE)
80105579:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010557c:	8b 40 0c             	mov    0xc(%eax),%eax
8010557f:	83 f8 03             	cmp    $0x3,%eax
80105582:	75 4f                	jne    801055d3 <scheduler+0x7a>
        continue;
    
      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
80105584:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105587:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
8010558d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105590:	89 04 24             	mov    %eax,(%esp)
80105593:	e8 b6 39 00 00       	call   80108f4e <switchuvm>
      p->state = RUNNING;
80105598:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010559b:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
801055a2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055a8:	8b 40 1c             	mov    0x1c(%eax),%eax
801055ab:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801055b2:	83 c2 04             	add    $0x4,%edx
801055b5:	89 44 24 04          	mov    %eax,0x4(%esp)
801055b9:	89 14 24             	mov    %edx,(%esp)
801055bc:	e8 e3 0a 00 00       	call   801060a4 <swtch>
      switchkvm();
801055c1:	e8 6b 39 00 00       	call   80108f31 <switchkvm>
                 
      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
801055c6:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
801055cd:	00 00 00 00 
801055d1:	eb 01                	jmp    801055d4 <scheduler+0x7b>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
801055d3:	90                   	nop
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801055d4:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
801055db:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
801055e2:	72 95                	jb     80105579 <scheduler+0x20>
                 
      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
801055e4:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801055eb:	e8 22 06 00 00       	call   80105c12 <release>

  }
801055f0:	e9 6a ff ff ff       	jmp    8010555f <scheduler+0x6>

801055f5 <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
801055f5:	55                   	push   %ebp
801055f6:	89 e5                	mov    %esp,%ebp
801055f8:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
801055fb:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105602:	e8 c7 06 00 00       	call   80105cce <holding>
80105607:	85 c0                	test   %eax,%eax
80105609:	75 0c                	jne    80105617 <sched+0x22>
    panic("sched ptable.lock");
8010560b:	c7 04 24 1d 9b 10 80 	movl   $0x80109b1d,(%esp)
80105612:	e8 26 af ff ff       	call   8010053d <panic>
  if(cpu->ncli != 1)
80105617:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010561d:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105623:	83 f8 01             	cmp    $0x1,%eax
80105626:	74 0c                	je     80105634 <sched+0x3f>
    panic("sched locks");
80105628:	c7 04 24 2f 9b 10 80 	movl   $0x80109b2f,(%esp)
8010562f:	e8 09 af ff ff       	call   8010053d <panic>
  if(proc->state == RUNNING)
80105634:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010563a:	8b 40 0c             	mov    0xc(%eax),%eax
8010563d:	83 f8 04             	cmp    $0x4,%eax
80105640:	75 0c                	jne    8010564e <sched+0x59>
    panic("sched running");
80105642:	c7 04 24 3b 9b 10 80 	movl   $0x80109b3b,(%esp)
80105649:	e8 ef ae ff ff       	call   8010053d <panic>
  if(readeflags()&FL_IF)
8010564e:	e8 a6 f1 ff ff       	call   801047f9 <readeflags>
80105653:	25 00 02 00 00       	and    $0x200,%eax
80105658:	85 c0                	test   %eax,%eax
8010565a:	74 0c                	je     80105668 <sched+0x73>
    panic("sched interruptible");
8010565c:	c7 04 24 49 9b 10 80 	movl   $0x80109b49,(%esp)
80105663:	e8 d5 ae ff ff       	call   8010053d <panic>
  intena = cpu->intena;
80105668:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010566e:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80105674:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80105677:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010567d:	8b 40 04             	mov    0x4(%eax),%eax
80105680:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80105687:	83 c2 1c             	add    $0x1c,%edx
8010568a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010568e:	89 14 24             	mov    %edx,(%esp)
80105691:	e8 0e 0a 00 00       	call   801060a4 <swtch>
  cpu->intena = intena;
80105696:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010569c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010569f:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
801056a5:	c9                   	leave  
801056a6:	c3                   	ret    

801056a7 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
801056a7:	55                   	push   %ebp
801056a8:	89 e5                	mov    %esp,%ebp
801056aa:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
801056ad:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801056b4:	e8 be 04 00 00       	call   80105b77 <acquire>
  proc->state = RUNNABLE;
801056b9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801056bf:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
801056c6:	e8 2a ff ff ff       	call   801055f5 <sched>
  release(&ptable.lock);
801056cb:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801056d2:	e8 3b 05 00 00       	call   80105c12 <release>
}
801056d7:	c9                   	leave  
801056d8:	c3                   	ret    

801056d9 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
801056d9:	55                   	push   %ebp
801056da:	89 e5                	mov    %esp,%ebp
801056dc:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
801056df:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801056e6:	e8 27 05 00 00       	call   80105c12 <release>

  if (first) {
801056eb:	a1 20 c0 10 80       	mov    0x8010c020,%eax
801056f0:	85 c0                	test   %eax,%eax
801056f2:	74 0f                	je     80105703 <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
801056f4:	c7 05 20 c0 10 80 00 	movl   $0x0,0x8010c020
801056fb:	00 00 00 
    initlog();
801056fe:	e8 09 e1 ff ff       	call   8010380c <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
80105703:	c9                   	leave  
80105704:	c3                   	ret    

80105705 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80105705:	55                   	push   %ebp
80105706:	89 e5                	mov    %esp,%ebp
80105708:	83 ec 18             	sub    $0x18,%esp
  if(proc == 0)
8010570b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105711:	85 c0                	test   %eax,%eax
80105713:	75 0c                	jne    80105721 <sleep+0x1c>
    panic("sleep");
80105715:	c7 04 24 5d 9b 10 80 	movl   $0x80109b5d,(%esp)
8010571c:	e8 1c ae ff ff       	call   8010053d <panic>

  if(lk == 0)
80105721:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80105725:	75 0c                	jne    80105733 <sleep+0x2e>
    panic("sleep without lk");
80105727:	c7 04 24 63 9b 10 80 	movl   $0x80109b63,(%esp)
8010572e:	e8 0a ae ff ff       	call   8010053d <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80105733:	81 7d 0c 40 77 12 80 	cmpl   $0x80127740,0xc(%ebp)
8010573a:	74 17                	je     80105753 <sleep+0x4e>
    acquire(&ptable.lock);  //DOC: sleeplock1
8010573c:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105743:	e8 2f 04 00 00       	call   80105b77 <acquire>
    release(lk);
80105748:	8b 45 0c             	mov    0xc(%ebp),%eax
8010574b:	89 04 24             	mov    %eax,(%esp)
8010574e:	e8 bf 04 00 00       	call   80105c12 <release>
  }

  // Go to sleep.
  proc->chan = chan;
80105753:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105759:	8b 55 08             	mov    0x8(%ebp),%edx
8010575c:	89 50 20             	mov    %edx,0x20(%eax)
  proc->state = SLEEPING;
8010575f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105765:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)

  // Swap out
  if(swapFlag)
8010576c:	a1 80 c6 10 80       	mov    0x8010c680,%eax
80105771:	85 c0                	test   %eax,%eax
80105773:	74 2b                	je     801057a0 <sleep+0x9b>
  {
    if(proc->pid > 3)
80105775:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010577b:	8b 40 10             	mov    0x10(%eax),%eax
8010577e:	83 f8 03             	cmp    $0x3,%eax
80105781:	7e 1d                	jle    801057a0 <sleep+0x9b>
    {
      release(&ptable.lock);
80105783:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
8010578a:	e8 83 04 00 00       	call   80105c12 <release>
      swapOut();
8010578f:	e8 ec f5 ff ff       	call   80104d80 <swapOut>
      acquire(&ptable.lock);
80105794:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
8010579b:	e8 d7 03 00 00       	call   80105b77 <acquire>
    }
  }
  
  sched();
801057a0:	e8 50 fe ff ff       	call   801055f5 <sched>
  
  // Tidy up.
  proc->chan = 0;
801057a5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801057ab:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
801057b2:	81 7d 0c 40 77 12 80 	cmpl   $0x80127740,0xc(%ebp)
801057b9:	74 17                	je     801057d2 <sleep+0xcd>
    release(&ptable.lock);
801057bb:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801057c2:	e8 4b 04 00 00       	call   80105c12 <release>
    acquire(lk);
801057c7:	8b 45 0c             	mov    0xc(%ebp),%eax
801057ca:	89 04 24             	mov    %eax,(%esp)
801057cd:	e8 a5 03 00 00       	call   80105b77 <acquire>
  }
}
801057d2:	c9                   	leave  
801057d3:	c3                   	ret    

801057d4 <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
801057d4:	55                   	push   %ebp
801057d5:	89 e5                	mov    %esp,%ebp
801057d7:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int found_suspended = 0;
801057da:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801057e1:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
801057e8:	eb 7e                	jmp    80105868 <wakeup1+0x94>
  {
    if(p->state == SLEEPING && p->chan == chan)
801057ea:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057ed:	8b 40 0c             	mov    0xc(%eax),%eax
801057f0:	83 f8 02             	cmp    $0x2,%eax
801057f3:	75 15                	jne    8010580a <wakeup1+0x36>
801057f5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057f8:	8b 40 20             	mov    0x20(%eax),%eax
801057fb:	3b 45 08             	cmp    0x8(%ebp),%eax
801057fe:	75 0a                	jne    8010580a <wakeup1+0x36>
      p->state = RUNNABLE;
80105800:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105803:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
    if(p->state == SLEEPING_SUSPENDED && p->chan == chan && !found_suspended)
8010580a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010580d:	8b 40 0c             	mov    0xc(%eax),%eax
80105810:	83 f8 06             	cmp    $0x6,%eax
80105813:	75 4c                	jne    80105861 <wakeup1+0x8d>
80105815:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105818:	8b 40 20             	mov    0x20(%eax),%eax
8010581b:	3b 45 08             	cmp    0x8(%ebp),%eax
8010581e:	75 41                	jne    80105861 <wakeup1+0x8d>
80105820:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105824:	75 3b                	jne    80105861 <wakeup1+0x8d>
    {
      acquire(&swaplock);
80105826:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
8010582d:	e8 45 03 00 00       	call   80105b77 <acquire>
      swappedout++;
80105832:	a1 84 c6 10 80       	mov    0x8010c684,%eax
80105837:	83 c0 01             	add    $0x1,%eax
8010583a:	a3 84 c6 10 80       	mov    %eax,0x8010c684
      p->state = RUNNABLE_SUSPENDED;
8010583f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105842:	c7 40 0c 07 00 00 00 	movl   $0x7,0xc(%eax)
      inswapper->state = RUNNABLE;
80105849:	a1 8c c6 10 80       	mov    0x8010c68c,%eax
8010584e:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&swaplock);
80105855:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
8010585c:	e8 b1 03 00 00       	call   80105c12 <release>
wakeup1(void *chan)
{
  struct proc *p;
  int found_suspended = 0;
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80105861:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80105868:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
8010586f:	0f 82 75 ff ff ff    	jb     801057ea <wakeup1+0x16>
      p->state = RUNNABLE_SUSPENDED;
      inswapper->state = RUNNABLE;
      release(&swaplock);
    }
  }
}
80105875:	c9                   	leave  
80105876:	c3                   	ret    

80105877 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80105877:	55                   	push   %ebp
80105878:	89 e5                	mov    %esp,%ebp
8010587a:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
8010587d:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105884:	e8 ee 02 00 00       	call   80105b77 <acquire>
  wakeup1(chan);
80105889:	8b 45 08             	mov    0x8(%ebp),%eax
8010588c:	89 04 24             	mov    %eax,(%esp)
8010588f:	e8 40 ff ff ff       	call   801057d4 <wakeup1>
  release(&ptable.lock);
80105894:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
8010589b:	e8 72 03 00 00       	call   80105c12 <release>
}
801058a0:	c9                   	leave  
801058a1:	c3                   	ret    

801058a2 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
801058a2:	55                   	push   %ebp
801058a3:	89 e5                	mov    %esp,%ebp
801058a5:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
801058a8:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
801058af:	e8 c3 02 00 00       	call   80105b77 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801058b4:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
801058bb:	eb 67                	jmp    80105924 <kill+0x82>
    if(p->pid == pid){
801058bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058c0:	8b 40 10             	mov    0x10(%eax),%eax
801058c3:	3b 45 08             	cmp    0x8(%ebp),%eax
801058c6:	75 55                	jne    8010591d <kill+0x7b>
      p->killed = 1;
801058c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058cb:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
801058d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058d5:	8b 40 0c             	mov    0xc(%eax),%eax
801058d8:	83 f8 02             	cmp    $0x2,%eax
801058db:	75 0c                	jne    801058e9 <kill+0x47>
        p->state = RUNNABLE;
801058dd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058e0:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
801058e7:	eb 21                	jmp    8010590a <kill+0x68>
      else if(p->state == SLEEPING_SUSPENDED)
801058e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058ec:	8b 40 0c             	mov    0xc(%eax),%eax
801058ef:	83 f8 06             	cmp    $0x6,%eax
801058f2:	75 16                	jne    8010590a <kill+0x68>
      {
        p->state = RUNNABLE_SUSPENDED;
801058f4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058f7:	c7 40 0c 07 00 00 00 	movl   $0x7,0xc(%eax)
	inswapper->state = RUNNABLE;
801058fe:	a1 8c c6 10 80       	mov    0x8010c68c,%eax
80105903:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      }
      release(&ptable.lock);
8010590a:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105911:	e8 fc 02 00 00       	call   80105c12 <release>
      return 0;
80105916:	b8 00 00 00 00       	mov    $0x0,%eax
8010591b:	eb 21                	jmp    8010593e <kill+0x9c>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010591d:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80105924:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
8010592b:	72 90                	jb     801058bd <kill+0x1b>
      }
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
8010592d:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105934:	e8 d9 02 00 00       	call   80105c12 <release>
  return -1;
80105939:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010593e:	c9                   	leave  
8010593f:	c3                   	ret    

80105940 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80105940:	55                   	push   %ebp
80105941:	89 e5                	mov    %esp,%ebp
80105943:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105946:	c7 45 f0 74 77 12 80 	movl   $0x80127774,-0x10(%ebp)
8010594d:	e9 db 00 00 00       	jmp    80105a2d <procdump+0xed>
    if(p->state == UNUSED)
80105952:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105955:	8b 40 0c             	mov    0xc(%eax),%eax
80105958:	85 c0                	test   %eax,%eax
8010595a:	0f 84 c5 00 00 00    	je     80105a25 <procdump+0xe5>
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80105960:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105963:	8b 40 0c             	mov    0xc(%eax),%eax
80105966:	83 f8 05             	cmp    $0x5,%eax
80105969:	77 23                	ja     8010598e <procdump+0x4e>
8010596b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010596e:	8b 40 0c             	mov    0xc(%eax),%eax
80105971:	8b 04 85 08 c0 10 80 	mov    -0x7fef3ff8(,%eax,4),%eax
80105978:	85 c0                	test   %eax,%eax
8010597a:	74 12                	je     8010598e <procdump+0x4e>
      state = states[p->state];
8010597c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010597f:	8b 40 0c             	mov    0xc(%eax),%eax
80105982:	8b 04 85 08 c0 10 80 	mov    -0x7fef3ff8(,%eax,4),%eax
80105989:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010598c:	eb 07                	jmp    80105995 <procdump+0x55>
    else
      state = "???";
8010598e:	c7 45 ec 74 9b 10 80 	movl   $0x80109b74,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80105995:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105998:	8d 50 6c             	lea    0x6c(%eax),%edx
8010599b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010599e:	8b 40 10             	mov    0x10(%eax),%eax
801059a1:	89 54 24 0c          	mov    %edx,0xc(%esp)
801059a5:	8b 55 ec             	mov    -0x14(%ebp),%edx
801059a8:	89 54 24 08          	mov    %edx,0x8(%esp)
801059ac:	89 44 24 04          	mov    %eax,0x4(%esp)
801059b0:	c7 04 24 78 9b 10 80 	movl   $0x80109b78,(%esp)
801059b7:	e8 e5 a9 ff ff       	call   801003a1 <cprintf>
    if(p->state == SLEEPING){
801059bc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059bf:	8b 40 0c             	mov    0xc(%eax),%eax
801059c2:	83 f8 02             	cmp    $0x2,%eax
801059c5:	75 50                	jne    80105a17 <procdump+0xd7>
      getcallerpcs((uint*)p->context->ebp+2, pc);
801059c7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059ca:	8b 40 1c             	mov    0x1c(%eax),%eax
801059cd:	8b 40 0c             	mov    0xc(%eax),%eax
801059d0:	83 c0 08             	add    $0x8,%eax
801059d3:	8d 55 c4             	lea    -0x3c(%ebp),%edx
801059d6:	89 54 24 04          	mov    %edx,0x4(%esp)
801059da:	89 04 24             	mov    %eax,(%esp)
801059dd:	e8 7f 02 00 00       	call   80105c61 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
801059e2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801059e9:	eb 1b                	jmp    80105a06 <procdump+0xc6>
        cprintf(" %p", pc[i]);
801059eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059ee:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
801059f2:	89 44 24 04          	mov    %eax,0x4(%esp)
801059f6:	c7 04 24 81 9b 10 80 	movl   $0x80109b81,(%esp)
801059fd:	e8 9f a9 ff ff       	call   801003a1 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80105a02:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80105a06:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80105a0a:	7f 0b                	jg     80105a17 <procdump+0xd7>
80105a0c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a0f:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80105a13:	85 c0                	test   %eax,%eax
80105a15:	75 d4                	jne    801059eb <procdump+0xab>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80105a17:	c7 04 24 85 9b 10 80 	movl   $0x80109b85,(%esp)
80105a1e:	e8 7e a9 ff ff       	call   801003a1 <cprintf>
80105a23:	eb 01                	jmp    80105a26 <procdump+0xe6>
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED)
      continue;
80105a25:	90                   	nop
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105a26:	81 45 f0 90 00 00 00 	addl   $0x90,-0x10(%ebp)
80105a2d:	81 7d f0 74 9b 12 80 	cmpl   $0x80129b74,-0x10(%ebp)
80105a34:	0f 82 18 ff ff ff    	jb     80105952 <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
80105a3a:	c9                   	leave  
80105a3b:	c3                   	ret    

80105a3c <getAllocatedPages>:

int getAllocatedPages(int pid) {
80105a3c:	55                   	push   %ebp
80105a3d:	89 e5                	mov    %esp,%ebp
80105a3f:	83 ec 38             	sub    $0x38,%esp
  struct proc* p;
  acquire(&ptable.lock);
80105a42:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105a49:	e8 29 01 00 00       	call   80105b77 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105a4e:	c7 45 f4 74 77 12 80 	movl   $0x80127774,-0xc(%ebp)
80105a55:	eb 12                	jmp    80105a69 <getAllocatedPages+0x2d>
    if(p->pid == pid){
80105a57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a5a:	8b 40 10             	mov    0x10(%eax),%eax
80105a5d:	3b 45 08             	cmp    0x8(%ebp),%eax
80105a60:	74 12                	je     80105a74 <getAllocatedPages+0x38>
}

int getAllocatedPages(int pid) {
  struct proc* p;
  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80105a62:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80105a69:	81 7d f4 74 9b 12 80 	cmpl   $0x80129b74,-0xc(%ebp)
80105a70:	72 e5                	jb     80105a57 <getAllocatedPages+0x1b>
80105a72:	eb 01                	jmp    80105a75 <getAllocatedPages+0x39>
    if(p->pid == pid){
     break;
80105a74:	90                   	nop
    }
  }
  release(&ptable.lock);
80105a75:	c7 04 24 40 77 12 80 	movl   $0x80127740,(%esp)
80105a7c:	e8 91 01 00 00       	call   80105c12 <release>
   int count= 0, j, k;
80105a81:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
   for (j=0; j<1024; j++) {
80105a88:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80105a8f:	eb 71                	jmp    80105b02 <getAllocatedPages+0xc6>
      if(p->pgdir){ 
80105a91:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a94:	8b 40 04             	mov    0x4(%eax),%eax
80105a97:	85 c0                	test   %eax,%eax
80105a99:	74 63                	je     80105afe <getAllocatedPages+0xc2>
	if (p->pgdir[j] & PTE_P) {
80105a9b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a9e:	8b 40 04             	mov    0x4(%eax),%eax
80105aa1:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105aa4:	c1 e2 02             	shl    $0x2,%edx
80105aa7:	01 d0                	add    %edx,%eax
80105aa9:	8b 00                	mov    (%eax),%eax
80105aab:	83 e0 01             	and    $0x1,%eax
80105aae:	84 c0                	test   %al,%al
80105ab0:	74 4c                	je     80105afe <getAllocatedPages+0xc2>
	  pte_t* pte= (pte_t*)p2v(PTE_ADDR(p->pgdir[j]));
80105ab2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ab5:	8b 40 04             	mov    0x4(%eax),%eax
80105ab8:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105abb:	c1 e2 02             	shl    $0x2,%edx
80105abe:	01 d0                	add    %edx,%eax
80105ac0:	8b 00                	mov    (%eax),%eax
80105ac2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80105ac7:	89 04 24             	mov    %eax,(%esp)
80105aca:	e8 1d ed ff ff       	call   801047ec <p2v>
80105acf:	89 45 e4             	mov    %eax,-0x1c(%ebp)
	  for (k=0; k<1024; k++) {
80105ad2:	c7 45 e8 00 00 00 00 	movl   $0x0,-0x18(%ebp)
80105ad9:	eb 1a                	jmp    80105af5 <getAllocatedPages+0xb9>
	      if ( pte[k] & PTE_U )
80105adb:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105ade:	c1 e0 02             	shl    $0x2,%eax
80105ae1:	03 45 e4             	add    -0x1c(%ebp),%eax
80105ae4:	8b 00                	mov    (%eax),%eax
80105ae6:	83 e0 04             	and    $0x4,%eax
80105ae9:	85 c0                	test   %eax,%eax
80105aeb:	74 04                	je     80105af1 <getAllocatedPages+0xb5>
		count++;
80105aed:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
   int count= 0, j, k;
   for (j=0; j<1024; j++) {
      if(p->pgdir){ 
	if (p->pgdir[j] & PTE_P) {
	  pte_t* pte= (pte_t*)p2v(PTE_ADDR(p->pgdir[j]));
	  for (k=0; k<1024; k++) {
80105af1:	83 45 e8 01          	addl   $0x1,-0x18(%ebp)
80105af5:	81 7d e8 ff 03 00 00 	cmpl   $0x3ff,-0x18(%ebp)
80105afc:	7e dd                	jle    80105adb <getAllocatedPages+0x9f>
     break;
    }
  }
  release(&ptable.lock);
   int count= 0, j, k;
   for (j=0; j<1024; j++) {
80105afe:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80105b02:	81 7d ec ff 03 00 00 	cmpl   $0x3ff,-0x14(%ebp)
80105b09:	7e 86                	jle    80105a91 <getAllocatedPages+0x55>
		count++;
	  }
	}
      }
   }
   return count;
80105b0b:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105b0e:	c9                   	leave  
80105b0f:	c3                   	ret    

80105b10 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80105b10:	55                   	push   %ebp
80105b11:	89 e5                	mov    %esp,%ebp
80105b13:	53                   	push   %ebx
80105b14:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80105b17:	9c                   	pushf  
80105b18:	5b                   	pop    %ebx
80105b19:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80105b1c:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80105b1f:	83 c4 10             	add    $0x10,%esp
80105b22:	5b                   	pop    %ebx
80105b23:	5d                   	pop    %ebp
80105b24:	c3                   	ret    

80105b25 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80105b25:	55                   	push   %ebp
80105b26:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80105b28:	fa                   	cli    
}
80105b29:	5d                   	pop    %ebp
80105b2a:	c3                   	ret    

80105b2b <sti>:

static inline void
sti(void)
{
80105b2b:	55                   	push   %ebp
80105b2c:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80105b2e:	fb                   	sti    
}
80105b2f:	5d                   	pop    %ebp
80105b30:	c3                   	ret    

80105b31 <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80105b31:	55                   	push   %ebp
80105b32:	89 e5                	mov    %esp,%ebp
80105b34:	53                   	push   %ebx
80105b35:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
80105b38:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80105b3b:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
80105b3e:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80105b41:	89 c3                	mov    %eax,%ebx
80105b43:	89 d8                	mov    %ebx,%eax
80105b45:	f0 87 02             	lock xchg %eax,(%edx)
80105b48:	89 c3                	mov    %eax,%ebx
80105b4a:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80105b4d:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80105b50:	83 c4 10             	add    $0x10,%esp
80105b53:	5b                   	pop    %ebx
80105b54:	5d                   	pop    %ebp
80105b55:	c3                   	ret    

80105b56 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80105b56:	55                   	push   %ebp
80105b57:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80105b59:	8b 45 08             	mov    0x8(%ebp),%eax
80105b5c:	8b 55 0c             	mov    0xc(%ebp),%edx
80105b5f:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80105b62:	8b 45 08             	mov    0x8(%ebp),%eax
80105b65:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80105b6b:	8b 45 08             	mov    0x8(%ebp),%eax
80105b6e:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80105b75:	5d                   	pop    %ebp
80105b76:	c3                   	ret    

80105b77 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80105b77:	55                   	push   %ebp
80105b78:	89 e5                	mov    %esp,%ebp
80105b7a:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80105b7d:	e8 76 01 00 00       	call   80105cf8 <pushcli>
  if(holding(lk))
80105b82:	8b 45 08             	mov    0x8(%ebp),%eax
80105b85:	89 04 24             	mov    %eax,(%esp)
80105b88:	e8 41 01 00 00       	call   80105cce <holding>
80105b8d:	85 c0                	test   %eax,%eax
80105b8f:	74 45                	je     80105bd6 <acquire+0x5f>
  {
    cprintf("lock = %s\n",lk->name);
80105b91:	8b 45 08             	mov    0x8(%ebp),%eax
80105b94:	8b 40 04             	mov    0x4(%eax),%eax
80105b97:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b9b:	c7 04 24 b1 9b 10 80 	movl   $0x80109bb1,(%esp)
80105ba2:	e8 fa a7 ff ff       	call   801003a1 <cprintf>
    if(proc)
80105ba7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105bad:	85 c0                	test   %eax,%eax
80105baf:	74 19                	je     80105bca <acquire+0x53>
      cprintf("pid = %d\n",proc->pid);
80105bb1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105bb7:	8b 40 10             	mov    0x10(%eax),%eax
80105bba:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bbe:	c7 04 24 bc 9b 10 80 	movl   $0x80109bbc,(%esp)
80105bc5:	e8 d7 a7 ff ff       	call   801003a1 <cprintf>
    panic("acquire");
80105bca:	c7 04 24 c6 9b 10 80 	movl   $0x80109bc6,(%esp)
80105bd1:	e8 67 a9 ff ff       	call   8010053d <panic>
  }

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80105bd6:	90                   	nop
80105bd7:	8b 45 08             	mov    0x8(%ebp),%eax
80105bda:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80105be1:	00 
80105be2:	89 04 24             	mov    %eax,(%esp)
80105be5:	e8 47 ff ff ff       	call   80105b31 <xchg>
80105bea:	85 c0                	test   %eax,%eax
80105bec:	75 e9                	jne    80105bd7 <acquire+0x60>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80105bee:	8b 45 08             	mov    0x8(%ebp),%eax
80105bf1:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80105bf8:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
80105bfb:	8b 45 08             	mov    0x8(%ebp),%eax
80105bfe:	83 c0 0c             	add    $0xc,%eax
80105c01:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c05:	8d 45 08             	lea    0x8(%ebp),%eax
80105c08:	89 04 24             	mov    %eax,(%esp)
80105c0b:	e8 51 00 00 00       	call   80105c61 <getcallerpcs>
}
80105c10:	c9                   	leave  
80105c11:	c3                   	ret    

80105c12 <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80105c12:	55                   	push   %ebp
80105c13:	89 e5                	mov    %esp,%ebp
80105c15:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80105c18:	8b 45 08             	mov    0x8(%ebp),%eax
80105c1b:	89 04 24             	mov    %eax,(%esp)
80105c1e:	e8 ab 00 00 00       	call   80105cce <holding>
80105c23:	85 c0                	test   %eax,%eax
80105c25:	75 0c                	jne    80105c33 <release+0x21>
    panic("release");
80105c27:	c7 04 24 ce 9b 10 80 	movl   $0x80109bce,(%esp)
80105c2e:	e8 0a a9 ff ff       	call   8010053d <panic>

  lk->pcs[0] = 0;
80105c33:	8b 45 08             	mov    0x8(%ebp),%eax
80105c36:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80105c3d:	8b 45 08             	mov    0x8(%ebp),%eax
80105c40:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
80105c47:	8b 45 08             	mov    0x8(%ebp),%eax
80105c4a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105c51:	00 
80105c52:	89 04 24             	mov    %eax,(%esp)
80105c55:	e8 d7 fe ff ff       	call   80105b31 <xchg>

  popcli();
80105c5a:	e8 e1 00 00 00       	call   80105d40 <popcli>
}
80105c5f:	c9                   	leave  
80105c60:	c3                   	ret    

80105c61 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80105c61:	55                   	push   %ebp
80105c62:	89 e5                	mov    %esp,%ebp
80105c64:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
80105c67:	8b 45 08             	mov    0x8(%ebp),%eax
80105c6a:	83 e8 08             	sub    $0x8,%eax
80105c6d:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80105c70:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80105c77:	eb 32                	jmp    80105cab <getcallerpcs+0x4a>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105c79:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80105c7d:	74 47                	je     80105cc6 <getcallerpcs+0x65>
80105c7f:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80105c86:	76 3e                	jbe    80105cc6 <getcallerpcs+0x65>
80105c88:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80105c8c:	74 38                	je     80105cc6 <getcallerpcs+0x65>
      break;
    pcs[i] = ebp[1];     // saved %eip
80105c8e:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105c91:	c1 e0 02             	shl    $0x2,%eax
80105c94:	03 45 0c             	add    0xc(%ebp),%eax
80105c97:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105c9a:	8b 52 04             	mov    0x4(%edx),%edx
80105c9d:	89 10                	mov    %edx,(%eax)
    ebp = (uint*)ebp[0]; // saved %ebp
80105c9f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105ca2:	8b 00                	mov    (%eax),%eax
80105ca4:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80105ca7:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105cab:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80105caf:	7e c8                	jle    80105c79 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80105cb1:	eb 13                	jmp    80105cc6 <getcallerpcs+0x65>
    pcs[i] = 0;
80105cb3:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105cb6:	c1 e0 02             	shl    $0x2,%eax
80105cb9:	03 45 0c             	add    0xc(%ebp),%eax
80105cbc:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80105cc2:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105cc6:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80105cca:	7e e7                	jle    80105cb3 <getcallerpcs+0x52>
    pcs[i] = 0;
}
80105ccc:	c9                   	leave  
80105ccd:	c3                   	ret    

80105cce <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80105cce:	55                   	push   %ebp
80105ccf:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
80105cd1:	8b 45 08             	mov    0x8(%ebp),%eax
80105cd4:	8b 00                	mov    (%eax),%eax
80105cd6:	85 c0                	test   %eax,%eax
80105cd8:	74 17                	je     80105cf1 <holding+0x23>
80105cda:	8b 45 08             	mov    0x8(%ebp),%eax
80105cdd:	8b 50 08             	mov    0x8(%eax),%edx
80105ce0:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105ce6:	39 c2                	cmp    %eax,%edx
80105ce8:	75 07                	jne    80105cf1 <holding+0x23>
80105cea:	b8 01 00 00 00       	mov    $0x1,%eax
80105cef:	eb 05                	jmp    80105cf6 <holding+0x28>
80105cf1:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105cf6:	5d                   	pop    %ebp
80105cf7:	c3                   	ret    

80105cf8 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80105cf8:	55                   	push   %ebp
80105cf9:	89 e5                	mov    %esp,%ebp
80105cfb:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
80105cfe:	e8 0d fe ff ff       	call   80105b10 <readeflags>
80105d03:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
80105d06:	e8 1a fe ff ff       	call   80105b25 <cli>
  if(cpu->ncli++ == 0)
80105d0b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105d11:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80105d17:	85 d2                	test   %edx,%edx
80105d19:	0f 94 c1             	sete   %cl
80105d1c:	83 c2 01             	add    $0x1,%edx
80105d1f:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105d25:	84 c9                	test   %cl,%cl
80105d27:	74 15                	je     80105d3e <pushcli+0x46>
    cpu->intena = eflags & FL_IF;
80105d29:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105d2f:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105d32:	81 e2 00 02 00 00    	and    $0x200,%edx
80105d38:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80105d3e:	c9                   	leave  
80105d3f:	c3                   	ret    

80105d40 <popcli>:

void
popcli(void)
{
80105d40:	55                   	push   %ebp
80105d41:	89 e5                	mov    %esp,%ebp
80105d43:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80105d46:	e8 c5 fd ff ff       	call   80105b10 <readeflags>
80105d4b:	25 00 02 00 00       	and    $0x200,%eax
80105d50:	85 c0                	test   %eax,%eax
80105d52:	74 0c                	je     80105d60 <popcli+0x20>
    panic("popcli - interruptible");
80105d54:	c7 04 24 d6 9b 10 80 	movl   $0x80109bd6,(%esp)
80105d5b:	e8 dd a7 ff ff       	call   8010053d <panic>
  if(--cpu->ncli < 0)
80105d60:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105d66:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80105d6c:	83 ea 01             	sub    $0x1,%edx
80105d6f:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105d75:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105d7b:	85 c0                	test   %eax,%eax
80105d7d:	79 0c                	jns    80105d8b <popcli+0x4b>
    panic("popcli");
80105d7f:	c7 04 24 ed 9b 10 80 	movl   $0x80109bed,(%esp)
80105d86:	e8 b2 a7 ff ff       	call   8010053d <panic>
  if(cpu->ncli == 0 && cpu->intena)
80105d8b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105d91:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105d97:	85 c0                	test   %eax,%eax
80105d99:	75 15                	jne    80105db0 <popcli+0x70>
80105d9b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105da1:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80105da7:	85 c0                	test   %eax,%eax
80105da9:	74 05                	je     80105db0 <popcli+0x70>
    sti();
80105dab:	e8 7b fd ff ff       	call   80105b2b <sti>
}
80105db0:	c9                   	leave  
80105db1:	c3                   	ret    
	...

80105db4 <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
80105db4:	55                   	push   %ebp
80105db5:	89 e5                	mov    %esp,%ebp
80105db7:	57                   	push   %edi
80105db8:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80105db9:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105dbc:	8b 55 10             	mov    0x10(%ebp),%edx
80105dbf:	8b 45 0c             	mov    0xc(%ebp),%eax
80105dc2:	89 cb                	mov    %ecx,%ebx
80105dc4:	89 df                	mov    %ebx,%edi
80105dc6:	89 d1                	mov    %edx,%ecx
80105dc8:	fc                   	cld    
80105dc9:	f3 aa                	rep stos %al,%es:(%edi)
80105dcb:	89 ca                	mov    %ecx,%edx
80105dcd:	89 fb                	mov    %edi,%ebx
80105dcf:	89 5d 08             	mov    %ebx,0x8(%ebp)
80105dd2:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80105dd5:	5b                   	pop    %ebx
80105dd6:	5f                   	pop    %edi
80105dd7:	5d                   	pop    %ebp
80105dd8:	c3                   	ret    

80105dd9 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
80105dd9:	55                   	push   %ebp
80105dda:	89 e5                	mov    %esp,%ebp
80105ddc:	57                   	push   %edi
80105ddd:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
80105dde:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105de1:	8b 55 10             	mov    0x10(%ebp),%edx
80105de4:	8b 45 0c             	mov    0xc(%ebp),%eax
80105de7:	89 cb                	mov    %ecx,%ebx
80105de9:	89 df                	mov    %ebx,%edi
80105deb:	89 d1                	mov    %edx,%ecx
80105ded:	fc                   	cld    
80105dee:	f3 ab                	rep stos %eax,%es:(%edi)
80105df0:	89 ca                	mov    %ecx,%edx
80105df2:	89 fb                	mov    %edi,%ebx
80105df4:	89 5d 08             	mov    %ebx,0x8(%ebp)
80105df7:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80105dfa:	5b                   	pop    %ebx
80105dfb:	5f                   	pop    %edi
80105dfc:	5d                   	pop    %ebp
80105dfd:	c3                   	ret    

80105dfe <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80105dfe:	55                   	push   %ebp
80105dff:	89 e5                	mov    %esp,%ebp
80105e01:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
80105e04:	8b 45 08             	mov    0x8(%ebp),%eax
80105e07:	83 e0 03             	and    $0x3,%eax
80105e0a:	85 c0                	test   %eax,%eax
80105e0c:	75 49                	jne    80105e57 <memset+0x59>
80105e0e:	8b 45 10             	mov    0x10(%ebp),%eax
80105e11:	83 e0 03             	and    $0x3,%eax
80105e14:	85 c0                	test   %eax,%eax
80105e16:	75 3f                	jne    80105e57 <memset+0x59>
    c &= 0xFF;
80105e18:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
80105e1f:	8b 45 10             	mov    0x10(%ebp),%eax
80105e22:	c1 e8 02             	shr    $0x2,%eax
80105e25:	89 c2                	mov    %eax,%edx
80105e27:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e2a:	89 c1                	mov    %eax,%ecx
80105e2c:	c1 e1 18             	shl    $0x18,%ecx
80105e2f:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e32:	c1 e0 10             	shl    $0x10,%eax
80105e35:	09 c1                	or     %eax,%ecx
80105e37:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e3a:	c1 e0 08             	shl    $0x8,%eax
80105e3d:	09 c8                	or     %ecx,%eax
80105e3f:	0b 45 0c             	or     0xc(%ebp),%eax
80105e42:	89 54 24 08          	mov    %edx,0x8(%esp)
80105e46:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e4a:	8b 45 08             	mov    0x8(%ebp),%eax
80105e4d:	89 04 24             	mov    %eax,(%esp)
80105e50:	e8 84 ff ff ff       	call   80105dd9 <stosl>
80105e55:	eb 19                	jmp    80105e70 <memset+0x72>
  } else
    stosb(dst, c, n);
80105e57:	8b 45 10             	mov    0x10(%ebp),%eax
80105e5a:	89 44 24 08          	mov    %eax,0x8(%esp)
80105e5e:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e61:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e65:	8b 45 08             	mov    0x8(%ebp),%eax
80105e68:	89 04 24             	mov    %eax,(%esp)
80105e6b:	e8 44 ff ff ff       	call   80105db4 <stosb>
  return dst;
80105e70:	8b 45 08             	mov    0x8(%ebp),%eax
}
80105e73:	c9                   	leave  
80105e74:	c3                   	ret    

80105e75 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
80105e75:	55                   	push   %ebp
80105e76:	89 e5                	mov    %esp,%ebp
80105e78:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80105e7b:	8b 45 08             	mov    0x8(%ebp),%eax
80105e7e:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
80105e81:	8b 45 0c             	mov    0xc(%ebp),%eax
80105e84:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
80105e87:	eb 32                	jmp    80105ebb <memcmp+0x46>
    if(*s1 != *s2)
80105e89:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105e8c:	0f b6 10             	movzbl (%eax),%edx
80105e8f:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105e92:	0f b6 00             	movzbl (%eax),%eax
80105e95:	38 c2                	cmp    %al,%dl
80105e97:	74 1a                	je     80105eb3 <memcmp+0x3e>
      return *s1 - *s2;
80105e99:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105e9c:	0f b6 00             	movzbl (%eax),%eax
80105e9f:	0f b6 d0             	movzbl %al,%edx
80105ea2:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105ea5:	0f b6 00             	movzbl (%eax),%eax
80105ea8:	0f b6 c0             	movzbl %al,%eax
80105eab:	89 d1                	mov    %edx,%ecx
80105ead:	29 c1                	sub    %eax,%ecx
80105eaf:	89 c8                	mov    %ecx,%eax
80105eb1:	eb 1c                	jmp    80105ecf <memcmp+0x5a>
    s1++, s2++;
80105eb3:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105eb7:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80105ebb:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105ebf:	0f 95 c0             	setne  %al
80105ec2:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105ec6:	84 c0                	test   %al,%al
80105ec8:	75 bf                	jne    80105e89 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
80105eca:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105ecf:	c9                   	leave  
80105ed0:	c3                   	ret    

80105ed1 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80105ed1:	55                   	push   %ebp
80105ed2:	89 e5                	mov    %esp,%ebp
80105ed4:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
80105ed7:	8b 45 0c             	mov    0xc(%ebp),%eax
80105eda:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
80105edd:	8b 45 08             	mov    0x8(%ebp),%eax
80105ee0:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
80105ee3:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105ee6:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105ee9:	73 54                	jae    80105f3f <memmove+0x6e>
80105eeb:	8b 45 10             	mov    0x10(%ebp),%eax
80105eee:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105ef1:	01 d0                	add    %edx,%eax
80105ef3:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105ef6:	76 47                	jbe    80105f3f <memmove+0x6e>
    s += n;
80105ef8:	8b 45 10             	mov    0x10(%ebp),%eax
80105efb:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
80105efe:	8b 45 10             	mov    0x10(%ebp),%eax
80105f01:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
80105f04:	eb 13                	jmp    80105f19 <memmove+0x48>
      *--d = *--s;
80105f06:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
80105f0a:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
80105f0e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105f11:	0f b6 10             	movzbl (%eax),%edx
80105f14:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105f17:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
80105f19:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105f1d:	0f 95 c0             	setne  %al
80105f20:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105f24:	84 c0                	test   %al,%al
80105f26:	75 de                	jne    80105f06 <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80105f28:	eb 25                	jmp    80105f4f <memmove+0x7e>
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
      *d++ = *s++;
80105f2a:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105f2d:	0f b6 10             	movzbl (%eax),%edx
80105f30:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105f33:	88 10                	mov    %dl,(%eax)
80105f35:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105f39:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105f3d:	eb 01                	jmp    80105f40 <memmove+0x6f>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
80105f3f:	90                   	nop
80105f40:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105f44:	0f 95 c0             	setne  %al
80105f47:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105f4b:	84 c0                	test   %al,%al
80105f4d:	75 db                	jne    80105f2a <memmove+0x59>
      *d++ = *s++;

  return dst;
80105f4f:	8b 45 08             	mov    0x8(%ebp),%eax
}
80105f52:	c9                   	leave  
80105f53:	c3                   	ret    

80105f54 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80105f54:	55                   	push   %ebp
80105f55:	89 e5                	mov    %esp,%ebp
80105f57:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
80105f5a:	8b 45 10             	mov    0x10(%ebp),%eax
80105f5d:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f61:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f64:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f68:	8b 45 08             	mov    0x8(%ebp),%eax
80105f6b:	89 04 24             	mov    %eax,(%esp)
80105f6e:	e8 5e ff ff ff       	call   80105ed1 <memmove>
}
80105f73:	c9                   	leave  
80105f74:	c3                   	ret    

80105f75 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
80105f75:	55                   	push   %ebp
80105f76:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80105f78:	eb 0c                	jmp    80105f86 <strncmp+0x11>
    n--, p++, q++;
80105f7a:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105f7e:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80105f82:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
80105f86:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105f8a:	74 1a                	je     80105fa6 <strncmp+0x31>
80105f8c:	8b 45 08             	mov    0x8(%ebp),%eax
80105f8f:	0f b6 00             	movzbl (%eax),%eax
80105f92:	84 c0                	test   %al,%al
80105f94:	74 10                	je     80105fa6 <strncmp+0x31>
80105f96:	8b 45 08             	mov    0x8(%ebp),%eax
80105f99:	0f b6 10             	movzbl (%eax),%edx
80105f9c:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f9f:	0f b6 00             	movzbl (%eax),%eax
80105fa2:	38 c2                	cmp    %al,%dl
80105fa4:	74 d4                	je     80105f7a <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
80105fa6:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105faa:	75 07                	jne    80105fb3 <strncmp+0x3e>
    return 0;
80105fac:	b8 00 00 00 00       	mov    $0x0,%eax
80105fb1:	eb 18                	jmp    80105fcb <strncmp+0x56>
  return (uchar)*p - (uchar)*q;
80105fb3:	8b 45 08             	mov    0x8(%ebp),%eax
80105fb6:	0f b6 00             	movzbl (%eax),%eax
80105fb9:	0f b6 d0             	movzbl %al,%edx
80105fbc:	8b 45 0c             	mov    0xc(%ebp),%eax
80105fbf:	0f b6 00             	movzbl (%eax),%eax
80105fc2:	0f b6 c0             	movzbl %al,%eax
80105fc5:	89 d1                	mov    %edx,%ecx
80105fc7:	29 c1                	sub    %eax,%ecx
80105fc9:	89 c8                	mov    %ecx,%eax
}
80105fcb:	5d                   	pop    %ebp
80105fcc:	c3                   	ret    

80105fcd <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80105fcd:	55                   	push   %ebp
80105fce:	89 e5                	mov    %esp,%ebp
80105fd0:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80105fd3:	8b 45 08             	mov    0x8(%ebp),%eax
80105fd6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
80105fd9:	90                   	nop
80105fda:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105fde:	0f 9f c0             	setg   %al
80105fe1:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105fe5:	84 c0                	test   %al,%al
80105fe7:	74 30                	je     80106019 <strncpy+0x4c>
80105fe9:	8b 45 0c             	mov    0xc(%ebp),%eax
80105fec:	0f b6 10             	movzbl (%eax),%edx
80105fef:	8b 45 08             	mov    0x8(%ebp),%eax
80105ff2:	88 10                	mov    %dl,(%eax)
80105ff4:	8b 45 08             	mov    0x8(%ebp),%eax
80105ff7:	0f b6 00             	movzbl (%eax),%eax
80105ffa:	84 c0                	test   %al,%al
80105ffc:	0f 95 c0             	setne  %al
80105fff:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80106003:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80106007:	84 c0                	test   %al,%al
80106009:	75 cf                	jne    80105fda <strncpy+0xd>
    ;
  while(n-- > 0)
8010600b:	eb 0c                	jmp    80106019 <strncpy+0x4c>
    *s++ = 0;
8010600d:	8b 45 08             	mov    0x8(%ebp),%eax
80106010:	c6 00 00             	movb   $0x0,(%eax)
80106013:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80106017:	eb 01                	jmp    8010601a <strncpy+0x4d>
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
80106019:	90                   	nop
8010601a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010601e:	0f 9f c0             	setg   %al
80106021:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80106025:	84 c0                	test   %al,%al
80106027:	75 e4                	jne    8010600d <strncpy+0x40>
    *s++ = 0;
  return os;
80106029:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010602c:	c9                   	leave  
8010602d:	c3                   	ret    

8010602e <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
8010602e:	55                   	push   %ebp
8010602f:	89 e5                	mov    %esp,%ebp
80106031:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80106034:	8b 45 08             	mov    0x8(%ebp),%eax
80106037:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
8010603a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010603e:	7f 05                	jg     80106045 <safestrcpy+0x17>
    return os;
80106040:	8b 45 fc             	mov    -0x4(%ebp),%eax
80106043:	eb 35                	jmp    8010607a <safestrcpy+0x4c>
  while(--n > 0 && (*s++ = *t++) != 0)
80106045:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80106049:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010604d:	7e 22                	jle    80106071 <safestrcpy+0x43>
8010604f:	8b 45 0c             	mov    0xc(%ebp),%eax
80106052:	0f b6 10             	movzbl (%eax),%edx
80106055:	8b 45 08             	mov    0x8(%ebp),%eax
80106058:	88 10                	mov    %dl,(%eax)
8010605a:	8b 45 08             	mov    0x8(%ebp),%eax
8010605d:	0f b6 00             	movzbl (%eax),%eax
80106060:	84 c0                	test   %al,%al
80106062:	0f 95 c0             	setne  %al
80106065:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80106069:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
8010606d:	84 c0                	test   %al,%al
8010606f:	75 d4                	jne    80106045 <safestrcpy+0x17>
    ;
  *s = 0;
80106071:	8b 45 08             	mov    0x8(%ebp),%eax
80106074:	c6 00 00             	movb   $0x0,(%eax)
  return os;
80106077:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010607a:	c9                   	leave  
8010607b:	c3                   	ret    

8010607c <strlen>:

int
strlen(const char *s)
{
8010607c:	55                   	push   %ebp
8010607d:	89 e5                	mov    %esp,%ebp
8010607f:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
80106082:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80106089:	eb 04                	jmp    8010608f <strlen+0x13>
8010608b:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010608f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80106092:	03 45 08             	add    0x8(%ebp),%eax
80106095:	0f b6 00             	movzbl (%eax),%eax
80106098:	84 c0                	test   %al,%al
8010609a:	75 ef                	jne    8010608b <strlen+0xf>
    ;
  return n;
8010609c:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010609f:	c9                   	leave  
801060a0:	c3                   	ret    
801060a1:	00 00                	add    %al,(%eax)
	...

801060a4 <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
801060a4:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
801060a8:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
801060ac:	55                   	push   %ebp
  pushl %ebx
801060ad:	53                   	push   %ebx
  pushl %esi
801060ae:	56                   	push   %esi
  pushl %edi
801060af:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
801060b0:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
801060b2:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
801060b4:	5f                   	pop    %edi
  popl %esi
801060b5:	5e                   	pop    %esi
  popl %ebx
801060b6:	5b                   	pop    %ebx
  popl %ebp
801060b7:	5d                   	pop    %ebp
  ret
801060b8:	c3                   	ret    
801060b9:	00 00                	add    %al,(%eax)
	...

801060bc <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from process p.
int
fetchint(struct proc *p, uint addr, int *ip)
{
801060bc:	55                   	push   %ebp
801060bd:	89 e5                	mov    %esp,%ebp
  if(addr >= p->sz || addr+4 > p->sz)
801060bf:	8b 45 08             	mov    0x8(%ebp),%eax
801060c2:	8b 00                	mov    (%eax),%eax
801060c4:	3b 45 0c             	cmp    0xc(%ebp),%eax
801060c7:	76 0f                	jbe    801060d8 <fetchint+0x1c>
801060c9:	8b 45 0c             	mov    0xc(%ebp),%eax
801060cc:	8d 50 04             	lea    0x4(%eax),%edx
801060cf:	8b 45 08             	mov    0x8(%ebp),%eax
801060d2:	8b 00                	mov    (%eax),%eax
801060d4:	39 c2                	cmp    %eax,%edx
801060d6:	76 07                	jbe    801060df <fetchint+0x23>
    return -1;
801060d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060dd:	eb 0f                	jmp    801060ee <fetchint+0x32>
  *ip = *(int*)(addr);
801060df:	8b 45 0c             	mov    0xc(%ebp),%eax
801060e2:	8b 10                	mov    (%eax),%edx
801060e4:	8b 45 10             	mov    0x10(%ebp),%eax
801060e7:	89 10                	mov    %edx,(%eax)
  return 0;
801060e9:	b8 00 00 00 00       	mov    $0x0,%eax
}
801060ee:	5d                   	pop    %ebp
801060ef:	c3                   	ret    

801060f0 <fetchstr>:
// Fetch the nul-terminated string at addr from process p.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(struct proc *p, uint addr, char **pp)
{
801060f0:	55                   	push   %ebp
801060f1:	89 e5                	mov    %esp,%ebp
801060f3:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= p->sz)
801060f6:	8b 45 08             	mov    0x8(%ebp),%eax
801060f9:	8b 00                	mov    (%eax),%eax
801060fb:	3b 45 0c             	cmp    0xc(%ebp),%eax
801060fe:	77 07                	ja     80106107 <fetchstr+0x17>
    return -1;
80106100:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106105:	eb 45                	jmp    8010614c <fetchstr+0x5c>
  *pp = (char*)addr;
80106107:	8b 55 0c             	mov    0xc(%ebp),%edx
8010610a:	8b 45 10             	mov    0x10(%ebp),%eax
8010610d:	89 10                	mov    %edx,(%eax)
  ep = (char*)p->sz;
8010610f:	8b 45 08             	mov    0x8(%ebp),%eax
80106112:	8b 00                	mov    (%eax),%eax
80106114:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
80106117:	8b 45 10             	mov    0x10(%ebp),%eax
8010611a:	8b 00                	mov    (%eax),%eax
8010611c:	89 45 fc             	mov    %eax,-0x4(%ebp)
8010611f:	eb 1e                	jmp    8010613f <fetchstr+0x4f>
    if(*s == 0)
80106121:	8b 45 fc             	mov    -0x4(%ebp),%eax
80106124:	0f b6 00             	movzbl (%eax),%eax
80106127:	84 c0                	test   %al,%al
80106129:	75 10                	jne    8010613b <fetchstr+0x4b>
      return s - *pp;
8010612b:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010612e:	8b 45 10             	mov    0x10(%ebp),%eax
80106131:	8b 00                	mov    (%eax),%eax
80106133:	89 d1                	mov    %edx,%ecx
80106135:	29 c1                	sub    %eax,%ecx
80106137:	89 c8                	mov    %ecx,%eax
80106139:	eb 11                	jmp    8010614c <fetchstr+0x5c>

  if(addr >= p->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)p->sz;
  for(s = *pp; s < ep; s++)
8010613b:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010613f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80106142:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80106145:	72 da                	jb     80106121 <fetchstr+0x31>
    if(*s == 0)
      return s - *pp;
  return -1;
80106147:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010614c:	c9                   	leave  
8010614d:	c3                   	ret    

8010614e <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
8010614e:	55                   	push   %ebp
8010614f:	89 e5                	mov    %esp,%ebp
80106151:	83 ec 0c             	sub    $0xc,%esp
  return fetchint(proc, proc->tf->esp + 4 + 4*n, ip);
80106154:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010615a:	8b 40 18             	mov    0x18(%eax),%eax
8010615d:	8b 50 44             	mov    0x44(%eax),%edx
80106160:	8b 45 08             	mov    0x8(%ebp),%eax
80106163:	c1 e0 02             	shl    $0x2,%eax
80106166:	01 d0                	add    %edx,%eax
80106168:	8d 48 04             	lea    0x4(%eax),%ecx
8010616b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106171:	8b 55 0c             	mov    0xc(%ebp),%edx
80106174:	89 54 24 08          	mov    %edx,0x8(%esp)
80106178:	89 4c 24 04          	mov    %ecx,0x4(%esp)
8010617c:	89 04 24             	mov    %eax,(%esp)
8010617f:	e8 38 ff ff ff       	call   801060bc <fetchint>
}
80106184:	c9                   	leave  
80106185:	c3                   	ret    

80106186 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
80106186:	55                   	push   %ebp
80106187:	89 e5                	mov    %esp,%ebp
80106189:	83 ec 18             	sub    $0x18,%esp
  int i;

  if(argint(n, &i) < 0)
8010618c:	8d 45 fc             	lea    -0x4(%ebp),%eax
8010618f:	89 44 24 04          	mov    %eax,0x4(%esp)
80106193:	8b 45 08             	mov    0x8(%ebp),%eax
80106196:	89 04 24             	mov    %eax,(%esp)
80106199:	e8 b0 ff ff ff       	call   8010614e <argint>
8010619e:	85 c0                	test   %eax,%eax
801061a0:	79 07                	jns    801061a9 <argptr+0x23>
    return -1;
801061a2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061a7:	eb 3d                	jmp    801061e6 <argptr+0x60>

  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
801061a9:	8b 45 fc             	mov    -0x4(%ebp),%eax
801061ac:	89 c2                	mov    %eax,%edx
801061ae:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801061b4:	8b 00                	mov    (%eax),%eax
801061b6:	39 c2                	cmp    %eax,%edx
801061b8:	73 16                	jae    801061d0 <argptr+0x4a>
801061ba:	8b 45 fc             	mov    -0x4(%ebp),%eax
801061bd:	89 c2                	mov    %eax,%edx
801061bf:	8b 45 10             	mov    0x10(%ebp),%eax
801061c2:	01 c2                	add    %eax,%edx
801061c4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801061ca:	8b 00                	mov    (%eax),%eax
801061cc:	39 c2                	cmp    %eax,%edx
801061ce:	76 07                	jbe    801061d7 <argptr+0x51>
    return -1;
801061d0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061d5:	eb 0f                	jmp    801061e6 <argptr+0x60>
  *pp = (char*)i;
801061d7:	8b 45 fc             	mov    -0x4(%ebp),%eax
801061da:	89 c2                	mov    %eax,%edx
801061dc:	8b 45 0c             	mov    0xc(%ebp),%eax
801061df:	89 10                	mov    %edx,(%eax)
  return 0;
801061e1:	b8 00 00 00 00       	mov    $0x0,%eax
}
801061e6:	c9                   	leave  
801061e7:	c3                   	ret    

801061e8 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
801061e8:	55                   	push   %ebp
801061e9:	89 e5                	mov    %esp,%ebp
801061eb:	83 ec 1c             	sub    $0x1c,%esp
  int addr;
  if(argint(n, &addr) < 0)
801061ee:	8d 45 fc             	lea    -0x4(%ebp),%eax
801061f1:	89 44 24 04          	mov    %eax,0x4(%esp)
801061f5:	8b 45 08             	mov    0x8(%ebp),%eax
801061f8:	89 04 24             	mov    %eax,(%esp)
801061fb:	e8 4e ff ff ff       	call   8010614e <argint>
80106200:	85 c0                	test   %eax,%eax
80106202:	79 07                	jns    8010620b <argstr+0x23>
    return -1;
80106204:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106209:	eb 1e                	jmp    80106229 <argstr+0x41>
  return fetchstr(proc, addr, pp);
8010620b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010620e:	89 c2                	mov    %eax,%edx
80106210:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106216:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80106219:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010621d:	89 54 24 04          	mov    %edx,0x4(%esp)
80106221:	89 04 24             	mov    %eax,(%esp)
80106224:	e8 c7 fe ff ff       	call   801060f0 <fetchstr>
}
80106229:	c9                   	leave  
8010622a:	c3                   	ret    

8010622b <syscall>:
[SYS_shmdt]	sys_shmdt,
};

void
syscall(void)
{
8010622b:	55                   	push   %ebp
8010622c:	89 e5                	mov    %esp,%ebp
8010622e:	53                   	push   %ebx
8010622f:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
80106232:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106238:	8b 40 18             	mov    0x18(%eax),%eax
8010623b:	8b 40 1c             	mov    0x1c(%eax),%eax
8010623e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num >= 0 && num < SYS_open && syscalls[num]) {
80106241:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106245:	78 2e                	js     80106275 <syscall+0x4a>
80106247:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
8010624b:	7f 28                	jg     80106275 <syscall+0x4a>
8010624d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106250:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
80106257:	85 c0                	test   %eax,%eax
80106259:	74 1a                	je     80106275 <syscall+0x4a>
    proc->tf->eax = syscalls[num]();
8010625b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106261:	8b 58 18             	mov    0x18(%eax),%ebx
80106264:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106267:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
8010626e:	ff d0                	call   *%eax
80106270:	89 43 1c             	mov    %eax,0x1c(%ebx)
80106273:	eb 73                	jmp    801062e8 <syscall+0xbd>
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
80106275:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
80106279:	7e 30                	jle    801062ab <syscall+0x80>
8010627b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010627e:	83 f8 1e             	cmp    $0x1e,%eax
80106281:	77 28                	ja     801062ab <syscall+0x80>
80106283:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106286:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
8010628d:	85 c0                	test   %eax,%eax
8010628f:	74 1a                	je     801062ab <syscall+0x80>
    proc->tf->eax = syscalls[num]();
80106291:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106297:	8b 58 18             	mov    0x18(%eax),%ebx
8010629a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010629d:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
801062a4:	ff d0                	call   *%eax
801062a6:	89 43 1c             	mov    %eax,0x1c(%ebx)
801062a9:	eb 3d                	jmp    801062e8 <syscall+0xbd>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
801062ab:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062b1:	8d 48 6c             	lea    0x6c(%eax),%ecx
801062b4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
  if(num >= 0 && num < SYS_open && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
801062ba:	8b 40 10             	mov    0x10(%eax),%eax
801062bd:	8b 55 f4             	mov    -0xc(%ebp),%edx
801062c0:	89 54 24 0c          	mov    %edx,0xc(%esp)
801062c4:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801062c8:	89 44 24 04          	mov    %eax,0x4(%esp)
801062cc:	c7 04 24 f4 9b 10 80 	movl   $0x80109bf4,(%esp)
801062d3:	e8 c9 a0 ff ff       	call   801003a1 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
801062d8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062de:	8b 40 18             	mov    0x18(%eax),%eax
801062e1:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
801062e8:	83 c4 24             	add    $0x24,%esp
801062eb:	5b                   	pop    %ebx
801062ec:	5d                   	pop    %ebp
801062ed:	c3                   	ret    
	...

801062f0 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
801062f0:	55                   	push   %ebp
801062f1:	89 e5                	mov    %esp,%ebp
801062f3:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
801062f6:	8d 45 f0             	lea    -0x10(%ebp),%eax
801062f9:	89 44 24 04          	mov    %eax,0x4(%esp)
801062fd:	8b 45 08             	mov    0x8(%ebp),%eax
80106300:	89 04 24             	mov    %eax,(%esp)
80106303:	e8 46 fe ff ff       	call   8010614e <argint>
80106308:	85 c0                	test   %eax,%eax
8010630a:	79 07                	jns    80106313 <argfd+0x23>
    return -1;
8010630c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106311:	eb 50                	jmp    80106363 <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
80106313:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106316:	85 c0                	test   %eax,%eax
80106318:	78 21                	js     8010633b <argfd+0x4b>
8010631a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010631d:	83 f8 0f             	cmp    $0xf,%eax
80106320:	7f 19                	jg     8010633b <argfd+0x4b>
80106322:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106328:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010632b:	83 c2 08             	add    $0x8,%edx
8010632e:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80106332:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106335:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106339:	75 07                	jne    80106342 <argfd+0x52>
    return -1;
8010633b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106340:	eb 21                	jmp    80106363 <argfd+0x73>
  if(pfd)
80106342:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80106346:	74 08                	je     80106350 <argfd+0x60>
    *pfd = fd;
80106348:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010634b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010634e:	89 10                	mov    %edx,(%eax)
  if(pf)
80106350:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80106354:	74 08                	je     8010635e <argfd+0x6e>
    *pf = f;
80106356:	8b 45 10             	mov    0x10(%ebp),%eax
80106359:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010635c:	89 10                	mov    %edx,(%eax)
  return 0;
8010635e:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106363:	c9                   	leave  
80106364:	c3                   	ret    

80106365 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
80106365:	55                   	push   %ebp
80106366:	89 e5                	mov    %esp,%ebp
80106368:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
8010636b:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80106372:	eb 30                	jmp    801063a4 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
80106374:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010637a:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010637d:	83 c2 08             	add    $0x8,%edx
80106380:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80106384:	85 c0                	test   %eax,%eax
80106386:	75 18                	jne    801063a0 <fdalloc+0x3b>
      proc->ofile[fd] = f;
80106388:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010638e:	8b 55 fc             	mov    -0x4(%ebp),%edx
80106391:	8d 4a 08             	lea    0x8(%edx),%ecx
80106394:	8b 55 08             	mov    0x8(%ebp),%edx
80106397:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
8010639b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010639e:	eb 0f                	jmp    801063af <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
801063a0:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801063a4:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
801063a8:	7e ca                	jle    80106374 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
801063aa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801063af:	c9                   	leave  
801063b0:	c3                   	ret    

801063b1 <sys_dup>:

int
sys_dup(void)
{
801063b1:	55                   	push   %ebp
801063b2:	89 e5                	mov    %esp,%ebp
801063b4:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
801063b7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801063ba:	89 44 24 08          	mov    %eax,0x8(%esp)
801063be:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801063c5:	00 
801063c6:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063cd:	e8 1e ff ff ff       	call   801062f0 <argfd>
801063d2:	85 c0                	test   %eax,%eax
801063d4:	79 07                	jns    801063dd <sys_dup+0x2c>
    return -1;
801063d6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063db:	eb 29                	jmp    80106406 <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
801063dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801063e0:	89 04 24             	mov    %eax,(%esp)
801063e3:	e8 7d ff ff ff       	call   80106365 <fdalloc>
801063e8:	89 45 f4             	mov    %eax,-0xc(%ebp)
801063eb:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801063ef:	79 07                	jns    801063f8 <sys_dup+0x47>
    return -1;
801063f1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063f6:	eb 0e                	jmp    80106406 <sys_dup+0x55>
  filedup(f);
801063f8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801063fb:	89 04 24             	mov    %eax,(%esp)
801063fe:	e8 79 ab ff ff       	call   80100f7c <filedup>
  return fd;
80106403:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106406:	c9                   	leave  
80106407:	c3                   	ret    

80106408 <sys_read>:

int
sys_read(void)
{
80106408:	55                   	push   %ebp
80106409:	89 e5                	mov    %esp,%ebp
8010640b:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010640e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106411:	89 44 24 08          	mov    %eax,0x8(%esp)
80106415:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010641c:	00 
8010641d:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106424:	e8 c7 fe ff ff       	call   801062f0 <argfd>
80106429:	85 c0                	test   %eax,%eax
8010642b:	78 35                	js     80106462 <sys_read+0x5a>
8010642d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106430:	89 44 24 04          	mov    %eax,0x4(%esp)
80106434:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010643b:	e8 0e fd ff ff       	call   8010614e <argint>
80106440:	85 c0                	test   %eax,%eax
80106442:	78 1e                	js     80106462 <sys_read+0x5a>
80106444:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106447:	89 44 24 08          	mov    %eax,0x8(%esp)
8010644b:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010644e:	89 44 24 04          	mov    %eax,0x4(%esp)
80106452:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106459:	e8 28 fd ff ff       	call   80106186 <argptr>
8010645e:	85 c0                	test   %eax,%eax
80106460:	79 07                	jns    80106469 <sys_read+0x61>
    return -1;
80106462:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106467:	eb 19                	jmp    80106482 <sys_read+0x7a>
  return fileread(f, p, n);
80106469:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010646c:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010646f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106472:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106476:	89 54 24 04          	mov    %edx,0x4(%esp)
8010647a:	89 04 24             	mov    %eax,(%esp)
8010647d:	e8 67 ac ff ff       	call   801010e9 <fileread>
}
80106482:	c9                   	leave  
80106483:	c3                   	ret    

80106484 <sys_write>:

int
sys_write(void)
{
80106484:	55                   	push   %ebp
80106485:	89 e5                	mov    %esp,%ebp
80106487:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010648a:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010648d:	89 44 24 08          	mov    %eax,0x8(%esp)
80106491:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106498:	00 
80106499:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801064a0:	e8 4b fe ff ff       	call   801062f0 <argfd>
801064a5:	85 c0                	test   %eax,%eax
801064a7:	78 35                	js     801064de <sys_write+0x5a>
801064a9:	8d 45 f0             	lea    -0x10(%ebp),%eax
801064ac:	89 44 24 04          	mov    %eax,0x4(%esp)
801064b0:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801064b7:	e8 92 fc ff ff       	call   8010614e <argint>
801064bc:	85 c0                	test   %eax,%eax
801064be:	78 1e                	js     801064de <sys_write+0x5a>
801064c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801064c3:	89 44 24 08          	mov    %eax,0x8(%esp)
801064c7:	8d 45 ec             	lea    -0x14(%ebp),%eax
801064ca:	89 44 24 04          	mov    %eax,0x4(%esp)
801064ce:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801064d5:	e8 ac fc ff ff       	call   80106186 <argptr>
801064da:	85 c0                	test   %eax,%eax
801064dc:	79 07                	jns    801064e5 <sys_write+0x61>
    return -1;
801064de:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064e3:	eb 19                	jmp    801064fe <sys_write+0x7a>
  return filewrite(f, p, n);
801064e5:	8b 4d f0             	mov    -0x10(%ebp),%ecx
801064e8:	8b 55 ec             	mov    -0x14(%ebp),%edx
801064eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064ee:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801064f2:	89 54 24 04          	mov    %edx,0x4(%esp)
801064f6:	89 04 24             	mov    %eax,(%esp)
801064f9:	e8 a7 ac ff ff       	call   801011a5 <filewrite>
}
801064fe:	c9                   	leave  
801064ff:	c3                   	ret    

80106500 <sys_close>:

int
sys_close(void)
{
80106500:	55                   	push   %ebp
80106501:	89 e5                	mov    %esp,%ebp
80106503:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
80106506:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106509:	89 44 24 08          	mov    %eax,0x8(%esp)
8010650d:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106510:	89 44 24 04          	mov    %eax,0x4(%esp)
80106514:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010651b:	e8 d0 fd ff ff       	call   801062f0 <argfd>
80106520:	85 c0                	test   %eax,%eax
80106522:	79 07                	jns    8010652b <sys_close+0x2b>
    return -1;
80106524:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106529:	eb 24                	jmp    8010654f <sys_close+0x4f>
  proc->ofile[fd] = 0;
8010652b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106531:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106534:	83 c2 08             	add    $0x8,%edx
80106537:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010653e:	00 
  fileclose(f);
8010653f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106542:	89 04 24             	mov    %eax,(%esp)
80106545:	e8 7a aa ff ff       	call   80100fc4 <fileclose>
  return 0;
8010654a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010654f:	c9                   	leave  
80106550:	c3                   	ret    

80106551 <sys_fstat>:

int
sys_fstat(void)
{
80106551:	55                   	push   %ebp
80106552:	89 e5                	mov    %esp,%ebp
80106554:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80106557:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010655a:	89 44 24 08          	mov    %eax,0x8(%esp)
8010655e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106565:	00 
80106566:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010656d:	e8 7e fd ff ff       	call   801062f0 <argfd>
80106572:	85 c0                	test   %eax,%eax
80106574:	78 1f                	js     80106595 <sys_fstat+0x44>
80106576:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
8010657d:	00 
8010657e:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106581:	89 44 24 04          	mov    %eax,0x4(%esp)
80106585:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010658c:	e8 f5 fb ff ff       	call   80106186 <argptr>
80106591:	85 c0                	test   %eax,%eax
80106593:	79 07                	jns    8010659c <sys_fstat+0x4b>
    return -1;
80106595:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010659a:	eb 12                	jmp    801065ae <sys_fstat+0x5d>
  return filestat(f, st);
8010659c:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010659f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801065a2:	89 54 24 04          	mov    %edx,0x4(%esp)
801065a6:	89 04 24             	mov    %eax,(%esp)
801065a9:	e8 ec aa ff ff       	call   8010109a <filestat>
}
801065ae:	c9                   	leave  
801065af:	c3                   	ret    

801065b0 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
801065b0:	55                   	push   %ebp
801065b1:	89 e5                	mov    %esp,%ebp
801065b3:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801065b6:	8d 45 d8             	lea    -0x28(%ebp),%eax
801065b9:	89 44 24 04          	mov    %eax,0x4(%esp)
801065bd:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801065c4:	e8 1f fc ff ff       	call   801061e8 <argstr>
801065c9:	85 c0                	test   %eax,%eax
801065cb:	78 17                	js     801065e4 <sys_link+0x34>
801065cd:	8d 45 dc             	lea    -0x24(%ebp),%eax
801065d0:	89 44 24 04          	mov    %eax,0x4(%esp)
801065d4:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801065db:	e8 08 fc ff ff       	call   801061e8 <argstr>
801065e0:	85 c0                	test   %eax,%eax
801065e2:	79 0a                	jns    801065ee <sys_link+0x3e>
    return -1;
801065e4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065e9:	e9 3c 01 00 00       	jmp    8010672a <sys_link+0x17a>
  if((ip = namei(old)) == 0)
801065ee:	8b 45 d8             	mov    -0x28(%ebp),%eax
801065f1:	89 04 24             	mov    %eax,(%esp)
801065f4:	e8 11 be ff ff       	call   8010240a <namei>
801065f9:	89 45 f4             	mov    %eax,-0xc(%ebp)
801065fc:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106600:	75 0a                	jne    8010660c <sys_link+0x5c>
    return -1;
80106602:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106607:	e9 1e 01 00 00       	jmp    8010672a <sys_link+0x17a>

  begin_trans();
8010660c:	e8 08 d4 ff ff       	call   80103a19 <begin_trans>

  ilock(ip);
80106611:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106614:	89 04 24             	mov    %eax,(%esp)
80106617:	e8 4c b2 ff ff       	call   80101868 <ilock>
  if(ip->type == T_DIR){
8010661c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010661f:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106623:	66 83 f8 01          	cmp    $0x1,%ax
80106627:	75 1a                	jne    80106643 <sys_link+0x93>
    iunlockput(ip);
80106629:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010662c:	89 04 24             	mov    %eax,(%esp)
8010662f:	e8 b8 b4 ff ff       	call   80101aec <iunlockput>
    commit_trans();
80106634:	e8 29 d4 ff ff       	call   80103a62 <commit_trans>
    return -1;
80106639:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010663e:	e9 e7 00 00 00       	jmp    8010672a <sys_link+0x17a>
  }

  ip->nlink++;
80106643:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106646:	0f b7 40 16          	movzwl 0x16(%eax),%eax
8010664a:	8d 50 01             	lea    0x1(%eax),%edx
8010664d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106650:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80106654:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106657:	89 04 24             	mov    %eax,(%esp)
8010665a:	e8 4d b0 ff ff       	call   801016ac <iupdate>
  iunlock(ip);
8010665f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106662:	89 04 24             	mov    %eax,(%esp)
80106665:	e8 4c b3 ff ff       	call   801019b6 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
8010666a:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010666d:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80106670:	89 54 24 04          	mov    %edx,0x4(%esp)
80106674:	89 04 24             	mov    %eax,(%esp)
80106677:	e8 b0 bd ff ff       	call   8010242c <nameiparent>
8010667c:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010667f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106683:	74 68                	je     801066ed <sys_link+0x13d>
    goto bad;
  ilock(dp);
80106685:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106688:	89 04 24             	mov    %eax,(%esp)
8010668b:	e8 d8 b1 ff ff       	call   80101868 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80106690:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106693:	8b 10                	mov    (%eax),%edx
80106695:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106698:	8b 00                	mov    (%eax),%eax
8010669a:	39 c2                	cmp    %eax,%edx
8010669c:	75 20                	jne    801066be <sys_link+0x10e>
8010669e:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066a1:	8b 40 04             	mov    0x4(%eax),%eax
801066a4:	89 44 24 08          	mov    %eax,0x8(%esp)
801066a8:	8d 45 e2             	lea    -0x1e(%ebp),%eax
801066ab:	89 44 24 04          	mov    %eax,0x4(%esp)
801066af:	8b 45 f0             	mov    -0x10(%ebp),%eax
801066b2:	89 04 24             	mov    %eax,(%esp)
801066b5:	e8 8f ba ff ff       	call   80102149 <dirlink>
801066ba:	85 c0                	test   %eax,%eax
801066bc:	79 0d                	jns    801066cb <sys_link+0x11b>
    iunlockput(dp);
801066be:	8b 45 f0             	mov    -0x10(%ebp),%eax
801066c1:	89 04 24             	mov    %eax,(%esp)
801066c4:	e8 23 b4 ff ff       	call   80101aec <iunlockput>
    goto bad;
801066c9:	eb 23                	jmp    801066ee <sys_link+0x13e>
  }
  iunlockput(dp);
801066cb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801066ce:	89 04 24             	mov    %eax,(%esp)
801066d1:	e8 16 b4 ff ff       	call   80101aec <iunlockput>
  iput(ip);
801066d6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066d9:	89 04 24             	mov    %eax,(%esp)
801066dc:	e8 3a b3 ff ff       	call   80101a1b <iput>

  commit_trans();
801066e1:	e8 7c d3 ff ff       	call   80103a62 <commit_trans>

  return 0;
801066e6:	b8 00 00 00 00       	mov    $0x0,%eax
801066eb:	eb 3d                	jmp    8010672a <sys_link+0x17a>
  ip->nlink++;
  iupdate(ip);
  iunlock(ip);

  if((dp = nameiparent(new, name)) == 0)
    goto bad;
801066ed:	90                   	nop
  commit_trans();

  return 0;

bad:
  ilock(ip);
801066ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066f1:	89 04 24             	mov    %eax,(%esp)
801066f4:	e8 6f b1 ff ff       	call   80101868 <ilock>
  ip->nlink--;
801066f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066fc:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106700:	8d 50 ff             	lea    -0x1(%eax),%edx
80106703:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106706:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
8010670a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010670d:	89 04 24             	mov    %eax,(%esp)
80106710:	e8 97 af ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80106715:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106718:	89 04 24             	mov    %eax,(%esp)
8010671b:	e8 cc b3 ff ff       	call   80101aec <iunlockput>
  commit_trans();
80106720:	e8 3d d3 ff ff       	call   80103a62 <commit_trans>
  return -1;
80106725:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010672a:	c9                   	leave  
8010672b:	c3                   	ret    

8010672c <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
8010672c:	55                   	push   %ebp
8010672d:	89 e5                	mov    %esp,%ebp
8010672f:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80106732:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80106739:	eb 4b                	jmp    80106786 <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010673b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010673e:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80106745:	00 
80106746:	89 44 24 08          	mov    %eax,0x8(%esp)
8010674a:	8d 45 e4             	lea    -0x1c(%ebp),%eax
8010674d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106751:	8b 45 08             	mov    0x8(%ebp),%eax
80106754:	89 04 24             	mov    %eax,(%esp)
80106757:	e8 02 b6 ff ff       	call   80101d5e <readi>
8010675c:	83 f8 10             	cmp    $0x10,%eax
8010675f:	74 0c                	je     8010676d <isdirempty+0x41>
      panic("isdirempty: readi");
80106761:	c7 04 24 10 9c 10 80 	movl   $0x80109c10,(%esp)
80106768:	e8 d0 9d ff ff       	call   8010053d <panic>
    if(de.inum != 0)
8010676d:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80106771:	66 85 c0             	test   %ax,%ax
80106774:	74 07                	je     8010677d <isdirempty+0x51>
      return 0;
80106776:	b8 00 00 00 00       	mov    $0x0,%eax
8010677b:	eb 1b                	jmp    80106798 <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
8010677d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106780:	83 c0 10             	add    $0x10,%eax
80106783:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106786:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106789:	8b 45 08             	mov    0x8(%ebp),%eax
8010678c:	8b 40 18             	mov    0x18(%eax),%eax
8010678f:	39 c2                	cmp    %eax,%edx
80106791:	72 a8                	jb     8010673b <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
80106793:	b8 01 00 00 00       	mov    $0x1,%eax
}
80106798:	c9                   	leave  
80106799:	c3                   	ret    

8010679a <unlink>:


int
unlink(char* path)
{
8010679a:	55                   	push   %ebp
8010679b:	89 e5                	mov    %esp,%ebp
8010679d:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ];
  uint off;

  if((dp = nameiparent(path, name)) == 0)
801067a0:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801067a3:	89 44 24 04          	mov    %eax,0x4(%esp)
801067a7:	8b 45 08             	mov    0x8(%ebp),%eax
801067aa:	89 04 24             	mov    %eax,(%esp)
801067ad:	e8 7a bc ff ff       	call   8010242c <nameiparent>
801067b2:	89 45 f4             	mov    %eax,-0xc(%ebp)
801067b5:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801067b9:	75 0a                	jne    801067c5 <unlink+0x2b>
    return -1;
801067bb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801067c0:	e9 85 01 00 00       	jmp    8010694a <unlink+0x1b0>

  begin_trans();
801067c5:	e8 4f d2 ff ff       	call   80103a19 <begin_trans>

  ilock(dp);
801067ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067cd:	89 04 24             	mov    %eax,(%esp)
801067d0:	e8 93 b0 ff ff       	call   80101868 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
801067d5:	c7 44 24 04 22 9c 10 	movl   $0x80109c22,0x4(%esp)
801067dc:	80 
801067dd:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801067e0:	89 04 24             	mov    %eax,(%esp)
801067e3:	e8 77 b8 ff ff       	call   8010205f <namecmp>
801067e8:	85 c0                	test   %eax,%eax
801067ea:	0f 84 45 01 00 00    	je     80106935 <unlink+0x19b>
801067f0:	c7 44 24 04 24 9c 10 	movl   $0x80109c24,0x4(%esp)
801067f7:	80 
801067f8:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801067fb:	89 04 24             	mov    %eax,(%esp)
801067fe:	e8 5c b8 ff ff       	call   8010205f <namecmp>
80106803:	85 c0                	test   %eax,%eax
80106805:	0f 84 2a 01 00 00    	je     80106935 <unlink+0x19b>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
8010680b:	8d 45 cc             	lea    -0x34(%ebp),%eax
8010680e:	89 44 24 08          	mov    %eax,0x8(%esp)
80106812:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80106815:	89 44 24 04          	mov    %eax,0x4(%esp)
80106819:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010681c:	89 04 24             	mov    %eax,(%esp)
8010681f:	e8 5d b8 ff ff       	call   80102081 <dirlookup>
80106824:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106827:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010682b:	0f 84 03 01 00 00    	je     80106934 <unlink+0x19a>
    goto bad;
  ilock(ip);
80106831:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106834:	89 04 24             	mov    %eax,(%esp)
80106837:	e8 2c b0 ff ff       	call   80101868 <ilock>

  if(ip->nlink < 1)
8010683c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010683f:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106843:	66 85 c0             	test   %ax,%ax
80106846:	7f 0c                	jg     80106854 <unlink+0xba>
    panic("unlink: nlink < 1");
80106848:	c7 04 24 27 9c 10 80 	movl   $0x80109c27,(%esp)
8010684f:	e8 e9 9c ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80106854:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106857:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010685b:	66 83 f8 01          	cmp    $0x1,%ax
8010685f:	75 1f                	jne    80106880 <unlink+0xe6>
80106861:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106864:	89 04 24             	mov    %eax,(%esp)
80106867:	e8 c0 fe ff ff       	call   8010672c <isdirempty>
8010686c:	85 c0                	test   %eax,%eax
8010686e:	75 10                	jne    80106880 <unlink+0xe6>
    iunlockput(ip);
80106870:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106873:	89 04 24             	mov    %eax,(%esp)
80106876:	e8 71 b2 ff ff       	call   80101aec <iunlockput>
    goto bad;
8010687b:	e9 b5 00 00 00       	jmp    80106935 <unlink+0x19b>
  }

  memset(&de, 0, sizeof(de));
80106880:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80106887:	00 
80106888:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010688f:	00 
80106890:	8d 45 e0             	lea    -0x20(%ebp),%eax
80106893:	89 04 24             	mov    %eax,(%esp)
80106896:	e8 63 f5 ff ff       	call   80105dfe <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010689b:	8b 45 cc             	mov    -0x34(%ebp),%eax
8010689e:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801068a5:	00 
801068a6:	89 44 24 08          	mov    %eax,0x8(%esp)
801068aa:	8d 45 e0             	lea    -0x20(%ebp),%eax
801068ad:	89 44 24 04          	mov    %eax,0x4(%esp)
801068b1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068b4:	89 04 24             	mov    %eax,(%esp)
801068b7:	e8 0d b6 ff ff       	call   80101ec9 <writei>
801068bc:	83 f8 10             	cmp    $0x10,%eax
801068bf:	74 0c                	je     801068cd <unlink+0x133>
    panic("unlink: writei");
801068c1:	c7 04 24 39 9c 10 80 	movl   $0x80109c39,(%esp)
801068c8:	e8 70 9c ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR){
801068cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801068d0:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801068d4:	66 83 f8 01          	cmp    $0x1,%ax
801068d8:	75 1c                	jne    801068f6 <unlink+0x15c>
    dp->nlink--;
801068da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068dd:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801068e1:	8d 50 ff             	lea    -0x1(%eax),%edx
801068e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068e7:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
801068eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068ee:	89 04 24             	mov    %eax,(%esp)
801068f1:	e8 b6 ad ff ff       	call   801016ac <iupdate>
  }
  iunlockput(dp);
801068f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068f9:	89 04 24             	mov    %eax,(%esp)
801068fc:	e8 eb b1 ff ff       	call   80101aec <iunlockput>

  ip->nlink--;
80106901:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106904:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106908:	8d 50 ff             	lea    -0x1(%eax),%edx
8010690b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010690e:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80106912:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106915:	89 04 24             	mov    %eax,(%esp)
80106918:	e8 8f ad ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
8010691d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106920:	89 04 24             	mov    %eax,(%esp)
80106923:	e8 c4 b1 ff ff       	call   80101aec <iunlockput>

  commit_trans();
80106928:	e8 35 d1 ff ff       	call   80103a62 <commit_trans>

  return 0;
8010692d:	b8 00 00 00 00       	mov    $0x0,%eax
80106932:	eb 16                	jmp    8010694a <unlink+0x1b0>
  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
    goto bad;
80106934:	90                   	nop
  commit_trans();

  return 0;

bad:
  iunlockput(dp);
80106935:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106938:	89 04 24             	mov    %eax,(%esp)
8010693b:	e8 ac b1 ff ff       	call   80101aec <iunlockput>
  commit_trans();
80106940:	e8 1d d1 ff ff       	call   80103a62 <commit_trans>
  return -1;
80106945:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010694a:	c9                   	leave  
8010694b:	c3                   	ret    

8010694c <sys_unlink>:


//PAGEBREAK!
int
sys_unlink(void)
{
8010694c:	55                   	push   %ebp
8010694d:	89 e5                	mov    %esp,%ebp
8010694f:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80106952:	8d 45 cc             	lea    -0x34(%ebp),%eax
80106955:	89 44 24 04          	mov    %eax,0x4(%esp)
80106959:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106960:	e8 83 f8 ff ff       	call   801061e8 <argstr>
80106965:	85 c0                	test   %eax,%eax
80106967:	79 0a                	jns    80106973 <sys_unlink+0x27>
    return -1;
80106969:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010696e:	e9 aa 01 00 00       	jmp    80106b1d <sys_unlink+0x1d1>
  if((dp = nameiparent(path, name)) == 0)
80106973:	8b 45 cc             	mov    -0x34(%ebp),%eax
80106976:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80106979:	89 54 24 04          	mov    %edx,0x4(%esp)
8010697d:	89 04 24             	mov    %eax,(%esp)
80106980:	e8 a7 ba ff ff       	call   8010242c <nameiparent>
80106985:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106988:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010698c:	75 0a                	jne    80106998 <sys_unlink+0x4c>
    return -1;
8010698e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106993:	e9 85 01 00 00       	jmp    80106b1d <sys_unlink+0x1d1>

  begin_trans();
80106998:	e8 7c d0 ff ff       	call   80103a19 <begin_trans>

  ilock(dp);
8010699d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801069a0:	89 04 24             	mov    %eax,(%esp)
801069a3:	e8 c0 ae ff ff       	call   80101868 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
801069a8:	c7 44 24 04 22 9c 10 	movl   $0x80109c22,0x4(%esp)
801069af:	80 
801069b0:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801069b3:	89 04 24             	mov    %eax,(%esp)
801069b6:	e8 a4 b6 ff ff       	call   8010205f <namecmp>
801069bb:	85 c0                	test   %eax,%eax
801069bd:	0f 84 45 01 00 00    	je     80106b08 <sys_unlink+0x1bc>
801069c3:	c7 44 24 04 24 9c 10 	movl   $0x80109c24,0x4(%esp)
801069ca:	80 
801069cb:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801069ce:	89 04 24             	mov    %eax,(%esp)
801069d1:	e8 89 b6 ff ff       	call   8010205f <namecmp>
801069d6:	85 c0                	test   %eax,%eax
801069d8:	0f 84 2a 01 00 00    	je     80106b08 <sys_unlink+0x1bc>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
801069de:	8d 45 c8             	lea    -0x38(%ebp),%eax
801069e1:	89 44 24 08          	mov    %eax,0x8(%esp)
801069e5:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801069e8:	89 44 24 04          	mov    %eax,0x4(%esp)
801069ec:	8b 45 f4             	mov    -0xc(%ebp),%eax
801069ef:	89 04 24             	mov    %eax,(%esp)
801069f2:	e8 8a b6 ff ff       	call   80102081 <dirlookup>
801069f7:	89 45 f0             	mov    %eax,-0x10(%ebp)
801069fa:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801069fe:	0f 84 03 01 00 00    	je     80106b07 <sys_unlink+0x1bb>
    goto bad;
  ilock(ip);
80106a04:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106a07:	89 04 24             	mov    %eax,(%esp)
80106a0a:	e8 59 ae ff ff       	call   80101868 <ilock>

  if(ip->nlink < 1)
80106a0f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106a12:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106a16:	66 85 c0             	test   %ax,%ax
80106a19:	7f 0c                	jg     80106a27 <sys_unlink+0xdb>
    panic("unlink: nlink < 1");
80106a1b:	c7 04 24 27 9c 10 80 	movl   $0x80109c27,(%esp)
80106a22:	e8 16 9b ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80106a27:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106a2a:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106a2e:	66 83 f8 01          	cmp    $0x1,%ax
80106a32:	75 1f                	jne    80106a53 <sys_unlink+0x107>
80106a34:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106a37:	89 04 24             	mov    %eax,(%esp)
80106a3a:	e8 ed fc ff ff       	call   8010672c <isdirempty>
80106a3f:	85 c0                	test   %eax,%eax
80106a41:	75 10                	jne    80106a53 <sys_unlink+0x107>
    iunlockput(ip);
80106a43:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106a46:	89 04 24             	mov    %eax,(%esp)
80106a49:	e8 9e b0 ff ff       	call   80101aec <iunlockput>
    goto bad;
80106a4e:	e9 b5 00 00 00       	jmp    80106b08 <sys_unlink+0x1bc>
  }

  memset(&de, 0, sizeof(de));
80106a53:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80106a5a:	00 
80106a5b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106a62:	00 
80106a63:	8d 45 e0             	lea    -0x20(%ebp),%eax
80106a66:	89 04 24             	mov    %eax,(%esp)
80106a69:	e8 90 f3 ff ff       	call   80105dfe <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80106a6e:	8b 45 c8             	mov    -0x38(%ebp),%eax
80106a71:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80106a78:	00 
80106a79:	89 44 24 08          	mov    %eax,0x8(%esp)
80106a7d:	8d 45 e0             	lea    -0x20(%ebp),%eax
80106a80:	89 44 24 04          	mov    %eax,0x4(%esp)
80106a84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a87:	89 04 24             	mov    %eax,(%esp)
80106a8a:	e8 3a b4 ff ff       	call   80101ec9 <writei>
80106a8f:	83 f8 10             	cmp    $0x10,%eax
80106a92:	74 0c                	je     80106aa0 <sys_unlink+0x154>
    panic("unlink: writei");
80106a94:	c7 04 24 39 9c 10 80 	movl   $0x80109c39,(%esp)
80106a9b:	e8 9d 9a ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR){
80106aa0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106aa3:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106aa7:	66 83 f8 01          	cmp    $0x1,%ax
80106aab:	75 1c                	jne    80106ac9 <sys_unlink+0x17d>
    dp->nlink--;
80106aad:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ab0:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106ab4:	8d 50 ff             	lea    -0x1(%eax),%edx
80106ab7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106aba:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80106abe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ac1:	89 04 24             	mov    %eax,(%esp)
80106ac4:	e8 e3 ab ff ff       	call   801016ac <iupdate>
  }
  iunlockput(dp);
80106ac9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106acc:	89 04 24             	mov    %eax,(%esp)
80106acf:	e8 18 b0 ff ff       	call   80101aec <iunlockput>

  ip->nlink--;
80106ad4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106ad7:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106adb:	8d 50 ff             	lea    -0x1(%eax),%edx
80106ade:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106ae1:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80106ae5:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106ae8:	89 04 24             	mov    %eax,(%esp)
80106aeb:	e8 bc ab ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80106af0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106af3:	89 04 24             	mov    %eax,(%esp)
80106af6:	e8 f1 af ff ff       	call   80101aec <iunlockput>

  commit_trans();
80106afb:	e8 62 cf ff ff       	call   80103a62 <commit_trans>

  return 0;
80106b00:	b8 00 00 00 00       	mov    $0x0,%eax
80106b05:	eb 16                	jmp    80106b1d <sys_unlink+0x1d1>
  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
    goto bad;
80106b07:	90                   	nop
  commit_trans();

  return 0;

bad:
  iunlockput(dp);
80106b08:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106b0b:	89 04 24             	mov    %eax,(%esp)
80106b0e:	e8 d9 af ff ff       	call   80101aec <iunlockput>
  commit_trans();
80106b13:	e8 4a cf ff ff       	call   80103a62 <commit_trans>
  return -1;
80106b18:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106b1d:	c9                   	leave  
80106b1e:	c3                   	ret    

80106b1f <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80106b1f:	55                   	push   %ebp
80106b20:	89 e5                	mov    %esp,%ebp
80106b22:	83 ec 48             	sub    $0x48,%esp
80106b25:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80106b28:	8b 55 10             	mov    0x10(%ebp),%edx
80106b2b:	8b 45 14             	mov    0x14(%ebp),%eax
80106b2e:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80106b32:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80106b36:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];
  if((dp = nameiparent(path, name)) == 0)
80106b3a:	8d 45 de             	lea    -0x22(%ebp),%eax
80106b3d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106b41:	8b 45 08             	mov    0x8(%ebp),%eax
80106b44:	89 04 24             	mov    %eax,(%esp)
80106b47:	e8 e0 b8 ff ff       	call   8010242c <nameiparent>
80106b4c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106b4f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106b53:	75 0a                	jne    80106b5f <create+0x40>
    return 0;
80106b55:	b8 00 00 00 00       	mov    $0x0,%eax
80106b5a:	e9 7e 01 00 00       	jmp    80106cdd <create+0x1be>
  ilock(dp);
80106b5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106b62:	89 04 24             	mov    %eax,(%esp)
80106b65:	e8 fe ac ff ff       	call   80101868 <ilock>
  if((ip = dirlookup(dp, name, &off)) != 0){
80106b6a:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106b6d:	89 44 24 08          	mov    %eax,0x8(%esp)
80106b71:	8d 45 de             	lea    -0x22(%ebp),%eax
80106b74:	89 44 24 04          	mov    %eax,0x4(%esp)
80106b78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106b7b:	89 04 24             	mov    %eax,(%esp)
80106b7e:	e8 fe b4 ff ff       	call   80102081 <dirlookup>
80106b83:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106b86:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106b8a:	74 47                	je     80106bd3 <create+0xb4>
    iunlockput(dp);
80106b8c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106b8f:	89 04 24             	mov    %eax,(%esp)
80106b92:	e8 55 af ff ff       	call   80101aec <iunlockput>
    ilock(ip);
80106b97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106b9a:	89 04 24             	mov    %eax,(%esp)
80106b9d:	e8 c6 ac ff ff       	call   80101868 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80106ba2:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80106ba7:	75 15                	jne    80106bbe <create+0x9f>
80106ba9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106bac:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106bb0:	66 83 f8 02          	cmp    $0x2,%ax
80106bb4:	75 08                	jne    80106bbe <create+0x9f>
      return ip;
80106bb6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106bb9:	e9 1f 01 00 00       	jmp    80106cdd <create+0x1be>
    iunlockput(ip);
80106bbe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106bc1:	89 04 24             	mov    %eax,(%esp)
80106bc4:	e8 23 af ff ff       	call   80101aec <iunlockput>
    return 0;
80106bc9:	b8 00 00 00 00       	mov    $0x0,%eax
80106bce:	e9 0a 01 00 00       	jmp    80106cdd <create+0x1be>
  }
  if((ip = ialloc(dp->dev, type)) == 0)
80106bd3:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80106bd7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106bda:	8b 00                	mov    (%eax),%eax
80106bdc:	89 54 24 04          	mov    %edx,0x4(%esp)
80106be0:	89 04 24             	mov    %eax,(%esp)
80106be3:	e8 e7 a9 ff ff       	call   801015cf <ialloc>
80106be8:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106beb:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106bef:	75 0c                	jne    80106bfd <create+0xde>
    panic("create: ialloc");
80106bf1:	c7 04 24 48 9c 10 80 	movl   $0x80109c48,(%esp)
80106bf8:	e8 40 99 ff ff       	call   8010053d <panic>
  ilock(ip);
80106bfd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c00:	89 04 24             	mov    %eax,(%esp)
80106c03:	e8 60 ac ff ff       	call   80101868 <ilock>
  ip->major = major;
80106c08:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c0b:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80106c0f:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80106c13:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c16:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80106c1a:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80106c1e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c21:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80106c27:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c2a:	89 04 24             	mov    %eax,(%esp)
80106c2d:	e8 7a aa ff ff       	call   801016ac <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
80106c32:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80106c37:	75 6a                	jne    80106ca3 <create+0x184>
    dp->nlink++;  // for ".."
80106c39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c3c:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80106c40:	8d 50 01             	lea    0x1(%eax),%edx
80106c43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c46:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80106c4a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c4d:	89 04 24             	mov    %eax,(%esp)
80106c50:	e8 57 aa ff ff       	call   801016ac <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80106c55:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c58:	8b 40 04             	mov    0x4(%eax),%eax
80106c5b:	89 44 24 08          	mov    %eax,0x8(%esp)
80106c5f:	c7 44 24 04 22 9c 10 	movl   $0x80109c22,0x4(%esp)
80106c66:	80 
80106c67:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c6a:	89 04 24             	mov    %eax,(%esp)
80106c6d:	e8 d7 b4 ff ff       	call   80102149 <dirlink>
80106c72:	85 c0                	test   %eax,%eax
80106c74:	78 21                	js     80106c97 <create+0x178>
80106c76:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106c79:	8b 40 04             	mov    0x4(%eax),%eax
80106c7c:	89 44 24 08          	mov    %eax,0x8(%esp)
80106c80:	c7 44 24 04 24 9c 10 	movl   $0x80109c24,0x4(%esp)
80106c87:	80 
80106c88:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106c8b:	89 04 24             	mov    %eax,(%esp)
80106c8e:	e8 b6 b4 ff ff       	call   80102149 <dirlink>
80106c93:	85 c0                	test   %eax,%eax
80106c95:	79 0c                	jns    80106ca3 <create+0x184>
      panic("create dots");
80106c97:	c7 04 24 57 9c 10 80 	movl   $0x80109c57,(%esp)
80106c9e:	e8 9a 98 ff ff       	call   8010053d <panic>
  }
  if(dirlink(dp, name, ip->inum) < 0)
80106ca3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106ca6:	8b 40 04             	mov    0x4(%eax),%eax
80106ca9:	89 44 24 08          	mov    %eax,0x8(%esp)
80106cad:	8d 45 de             	lea    -0x22(%ebp),%eax
80106cb0:	89 44 24 04          	mov    %eax,0x4(%esp)
80106cb4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106cb7:	89 04 24             	mov    %eax,(%esp)
80106cba:	e8 8a b4 ff ff       	call   80102149 <dirlink>
80106cbf:	85 c0                	test   %eax,%eax
80106cc1:	79 0c                	jns    80106ccf <create+0x1b0>
    panic("create: dirlink");
80106cc3:	c7 04 24 63 9c 10 80 	movl   $0x80109c63,(%esp)
80106cca:	e8 6e 98 ff ff       	call   8010053d <panic>
  iunlockput(dp);
80106ccf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106cd2:	89 04 24             	mov    %eax,(%esp)
80106cd5:	e8 12 ae ff ff       	call   80101aec <iunlockput>

  return ip;
80106cda:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80106cdd:	c9                   	leave  
80106cde:	c3                   	ret    

80106cdf <fileopen>:

struct file*
fileopen(char *path, int omode)
{
80106cdf:	55                   	push   %ebp
80106ce0:	89 e5                	mov    %esp,%ebp
80106ce2:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  struct inode *ip;

  if(omode & O_CREATE){
80106ce5:	8b 45 0c             	mov    0xc(%ebp),%eax
80106ce8:	25 00 02 00 00       	and    $0x200,%eax
80106ced:	85 c0                	test   %eax,%eax
80106cef:	74 40                	je     80106d31 <fileopen+0x52>
    begin_trans();
80106cf1:	e8 23 cd ff ff       	call   80103a19 <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80106cf6:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106cfd:	00 
80106cfe:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106d05:	00 
80106d06:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80106d0d:	00 
80106d0e:	8b 45 08             	mov    0x8(%ebp),%eax
80106d11:	89 04 24             	mov    %eax,(%esp)
80106d14:	e8 06 fe ff ff       	call   80106b1f <create>
80106d19:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80106d1c:	e8 41 cd ff ff       	call   80103a62 <commit_trans>
    if(ip == 0)
80106d21:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106d25:	75 5b                	jne    80106d82 <fileopen+0xa3>
      return 0;
80106d27:	b8 00 00 00 00       	mov    $0x0,%eax
80106d2c:	e9 f9 00 00 00       	jmp    80106e2a <fileopen+0x14b>
  } else {
    if((ip = namei(path)) == 0)
80106d31:	8b 45 08             	mov    0x8(%ebp),%eax
80106d34:	89 04 24             	mov    %eax,(%esp)
80106d37:	e8 ce b6 ff ff       	call   8010240a <namei>
80106d3c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106d3f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106d43:	75 0a                	jne    80106d4f <fileopen+0x70>
      return 0;
80106d45:	b8 00 00 00 00       	mov    $0x0,%eax
80106d4a:	e9 db 00 00 00       	jmp    80106e2a <fileopen+0x14b>
    ilock(ip);
80106d4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106d52:	89 04 24             	mov    %eax,(%esp)
80106d55:	e8 0e ab ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106d5a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106d5d:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106d61:	66 83 f8 01          	cmp    $0x1,%ax
80106d65:	75 1b                	jne    80106d82 <fileopen+0xa3>
80106d67:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80106d6b:	74 15                	je     80106d82 <fileopen+0xa3>
      iunlockput(ip);
80106d6d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106d70:	89 04 24             	mov    %eax,(%esp)
80106d73:	e8 74 ad ff ff       	call   80101aec <iunlockput>
      return 0;
80106d78:	b8 00 00 00 00       	mov    $0x0,%eax
80106d7d:	e9 a8 00 00 00       	jmp    80106e2a <fileopen+0x14b>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80106d82:	e8 95 a1 ff ff       	call   80100f1c <filealloc>
80106d87:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106d8a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106d8e:	74 14                	je     80106da4 <fileopen+0xc5>
80106d90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106d93:	89 04 24             	mov    %eax,(%esp)
80106d96:	e8 ca f5 ff ff       	call   80106365 <fdalloc>
80106d9b:	89 45 ec             	mov    %eax,-0x14(%ebp)
80106d9e:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80106da2:	79 23                	jns    80106dc7 <fileopen+0xe8>
    if(f)
80106da4:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106da8:	74 0b                	je     80106db5 <fileopen+0xd6>
      fileclose(f);
80106daa:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106dad:	89 04 24             	mov    %eax,(%esp)
80106db0:	e8 0f a2 ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
80106db5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106db8:	89 04 24             	mov    %eax,(%esp)
80106dbb:	e8 2c ad ff ff       	call   80101aec <iunlockput>
    return 0;
80106dc0:	b8 00 00 00 00       	mov    $0x0,%eax
80106dc5:	eb 63                	jmp    80106e2a <fileopen+0x14b>
  }
  iunlock(ip);
80106dc7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106dca:	89 04 24             	mov    %eax,(%esp)
80106dcd:	e8 e4 ab ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
80106dd2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106dd5:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80106ddb:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106dde:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106de1:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80106de4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106de7:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80106dee:	8b 45 0c             	mov    0xc(%ebp),%eax
80106df1:	83 e0 01             	and    $0x1,%eax
80106df4:	85 c0                	test   %eax,%eax
80106df6:	0f 94 c2             	sete   %dl
80106df9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106dfc:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106dff:	8b 45 0c             	mov    0xc(%ebp),%eax
80106e02:	83 e0 01             	and    $0x1,%eax
80106e05:	84 c0                	test   %al,%al
80106e07:	75 0a                	jne    80106e13 <fileopen+0x134>
80106e09:	8b 45 0c             	mov    0xc(%ebp),%eax
80106e0c:	83 e0 02             	and    $0x2,%eax
80106e0f:	85 c0                	test   %eax,%eax
80106e11:	74 07                	je     80106e1a <fileopen+0x13b>
80106e13:	b8 01 00 00 00       	mov    $0x1,%eax
80106e18:	eb 05                	jmp    80106e1f <fileopen+0x140>
80106e1a:	b8 00 00 00 00       	mov    $0x0,%eax
80106e1f:	89 c2                	mov    %eax,%edx
80106e21:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106e24:	88 50 09             	mov    %dl,0x9(%eax)
  return f;
80106e27:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80106e2a:	c9                   	leave  
80106e2b:	c3                   	ret    

80106e2c <sys_open>:

int
sys_open(void)
{
80106e2c:	55                   	push   %ebp
80106e2d:	89 e5                	mov    %esp,%ebp
80106e2f:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80106e32:	8d 45 e8             	lea    -0x18(%ebp),%eax
80106e35:	89 44 24 04          	mov    %eax,0x4(%esp)
80106e39:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106e40:	e8 a3 f3 ff ff       	call   801061e8 <argstr>
80106e45:	85 c0                	test   %eax,%eax
80106e47:	78 17                	js     80106e60 <sys_open+0x34>
80106e49:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106e4c:	89 44 24 04          	mov    %eax,0x4(%esp)
80106e50:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106e57:	e8 f2 f2 ff ff       	call   8010614e <argint>
80106e5c:	85 c0                	test   %eax,%eax
80106e5e:	79 0a                	jns    80106e6a <sys_open+0x3e>
    return -1;
80106e60:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106e65:	e9 46 01 00 00       	jmp    80106fb0 <sys_open+0x184>
  if(omode & O_CREATE){
80106e6a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106e6d:	25 00 02 00 00       	and    $0x200,%eax
80106e72:	85 c0                	test   %eax,%eax
80106e74:	74 40                	je     80106eb6 <sys_open+0x8a>
    begin_trans();
80106e76:	e8 9e cb ff ff       	call   80103a19 <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80106e7b:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106e7e:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106e85:	00 
80106e86:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106e8d:	00 
80106e8e:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80106e95:	00 
80106e96:	89 04 24             	mov    %eax,(%esp)
80106e99:	e8 81 fc ff ff       	call   80106b1f <create>
80106e9e:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80106ea1:	e8 bc cb ff ff       	call   80103a62 <commit_trans>
    if(ip == 0)
80106ea6:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106eaa:	75 5c                	jne    80106f08 <sys_open+0xdc>
      return -1;
80106eac:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106eb1:	e9 fa 00 00 00       	jmp    80106fb0 <sys_open+0x184>
  } else {
    if((ip = namei(path)) == 0)
80106eb6:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106eb9:	89 04 24             	mov    %eax,(%esp)
80106ebc:	e8 49 b5 ff ff       	call   8010240a <namei>
80106ec1:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106ec4:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106ec8:	75 0a                	jne    80106ed4 <sys_open+0xa8>
      return -1;
80106eca:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106ecf:	e9 dc 00 00 00       	jmp    80106fb0 <sys_open+0x184>
    ilock(ip);
80106ed4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ed7:	89 04 24             	mov    %eax,(%esp)
80106eda:	e8 89 a9 ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106edf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ee2:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106ee6:	66 83 f8 01          	cmp    $0x1,%ax
80106eea:	75 1c                	jne    80106f08 <sys_open+0xdc>
80106eec:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106eef:	85 c0                	test   %eax,%eax
80106ef1:	74 15                	je     80106f08 <sys_open+0xdc>
      iunlockput(ip);
80106ef3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ef6:	89 04 24             	mov    %eax,(%esp)
80106ef9:	e8 ee ab ff ff       	call   80101aec <iunlockput>
      return -1;
80106efe:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106f03:	e9 a8 00 00 00       	jmp    80106fb0 <sys_open+0x184>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80106f08:	e8 0f a0 ff ff       	call   80100f1c <filealloc>
80106f0d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106f10:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106f14:	74 14                	je     80106f2a <sys_open+0xfe>
80106f16:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f19:	89 04 24             	mov    %eax,(%esp)
80106f1c:	e8 44 f4 ff ff       	call   80106365 <fdalloc>
80106f21:	89 45 ec             	mov    %eax,-0x14(%ebp)
80106f24:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80106f28:	79 23                	jns    80106f4d <sys_open+0x121>
    if(f)
80106f2a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106f2e:	74 0b                	je     80106f3b <sys_open+0x10f>
      fileclose(f);
80106f30:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f33:	89 04 24             	mov    %eax,(%esp)
80106f36:	e8 89 a0 ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
80106f3b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f3e:	89 04 24             	mov    %eax,(%esp)
80106f41:	e8 a6 ab ff ff       	call   80101aec <iunlockput>
    return -1;
80106f46:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106f4b:	eb 63                	jmp    80106fb0 <sys_open+0x184>
  }
  iunlock(ip);
80106f4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f50:	89 04 24             	mov    %eax,(%esp)
80106f53:	e8 5e aa ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
80106f58:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f5b:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80106f61:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f64:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106f67:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80106f6a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f6d:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80106f74:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106f77:	83 e0 01             	and    $0x1,%eax
80106f7a:	85 c0                	test   %eax,%eax
80106f7c:	0f 94 c2             	sete   %dl
80106f7f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106f82:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106f85:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106f88:	83 e0 01             	and    $0x1,%eax
80106f8b:	84 c0                	test   %al,%al
80106f8d:	75 0a                	jne    80106f99 <sys_open+0x16d>
80106f8f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106f92:	83 e0 02             	and    $0x2,%eax
80106f95:	85 c0                	test   %eax,%eax
80106f97:	74 07                	je     80106fa0 <sys_open+0x174>
80106f99:	b8 01 00 00 00       	mov    $0x1,%eax
80106f9e:	eb 05                	jmp    80106fa5 <sys_open+0x179>
80106fa0:	b8 00 00 00 00       	mov    $0x0,%eax
80106fa5:	89 c2                	mov    %eax,%edx
80106fa7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106faa:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80106fad:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80106fb0:	c9                   	leave  
80106fb1:	c3                   	ret    

80106fb2 <sys_mkdir>:

int
sys_mkdir(void)
{
80106fb2:	55                   	push   %ebp
80106fb3:	89 e5                	mov    %esp,%ebp
80106fb5:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
80106fb8:	e8 5c ca ff ff       	call   80103a19 <begin_trans>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80106fbd:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106fc0:	89 44 24 04          	mov    %eax,0x4(%esp)
80106fc4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106fcb:	e8 18 f2 ff ff       	call   801061e8 <argstr>
80106fd0:	85 c0                	test   %eax,%eax
80106fd2:	78 2c                	js     80107000 <sys_mkdir+0x4e>
80106fd4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106fd7:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106fde:	00 
80106fdf:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106fe6:	00 
80106fe7:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106fee:	00 
80106fef:	89 04 24             	mov    %eax,(%esp)
80106ff2:	e8 28 fb ff ff       	call   80106b1f <create>
80106ff7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106ffa:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106ffe:	75 0c                	jne    8010700c <sys_mkdir+0x5a>
    commit_trans();
80107000:	e8 5d ca ff ff       	call   80103a62 <commit_trans>
    return -1;
80107005:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010700a:	eb 15                	jmp    80107021 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
8010700c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010700f:	89 04 24             	mov    %eax,(%esp)
80107012:	e8 d5 aa ff ff       	call   80101aec <iunlockput>
  commit_trans();
80107017:	e8 46 ca ff ff       	call   80103a62 <commit_trans>
  return 0;
8010701c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107021:	c9                   	leave  
80107022:	c3                   	ret    

80107023 <sys_mknod>:

int
sys_mknod(void)
{
80107023:	55                   	push   %ebp
80107024:	89 e5                	mov    %esp,%ebp
80107026:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
80107029:	e8 eb c9 ff ff       	call   80103a19 <begin_trans>
  if((len=argstr(0, &path)) < 0 ||
8010702e:	8d 45 ec             	lea    -0x14(%ebp),%eax
80107031:	89 44 24 04          	mov    %eax,0x4(%esp)
80107035:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010703c:	e8 a7 f1 ff ff       	call   801061e8 <argstr>
80107041:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107044:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107048:	78 5e                	js     801070a8 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
8010704a:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010704d:	89 44 24 04          	mov    %eax,0x4(%esp)
80107051:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80107058:	e8 f1 f0 ff ff       	call   8010614e <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
8010705d:	85 c0                	test   %eax,%eax
8010705f:	78 47                	js     801070a8 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80107061:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80107064:	89 44 24 04          	mov    %eax,0x4(%esp)
80107068:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010706f:	e8 da f0 ff ff       	call   8010614e <argint>
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
80107074:	85 c0                	test   %eax,%eax
80107076:	78 30                	js     801070a8 <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
80107078:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010707b:	0f bf c8             	movswl %ax,%ecx
8010707e:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107081:	0f bf d0             	movswl %ax,%edx
80107084:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80107087:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
8010708b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010708f:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80107096:	00 
80107097:	89 04 24             	mov    %eax,(%esp)
8010709a:	e8 80 fa ff ff       	call   80106b1f <create>
8010709f:	89 45 f0             	mov    %eax,-0x10(%ebp)
801070a2:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801070a6:	75 0c                	jne    801070b4 <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    commit_trans();
801070a8:	e8 b5 c9 ff ff       	call   80103a62 <commit_trans>
    return -1;
801070ad:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801070b2:	eb 15                	jmp    801070c9 <sys_mknod+0xa6>
  }
  iunlockput(ip);
801070b4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801070b7:	89 04 24             	mov    %eax,(%esp)
801070ba:	e8 2d aa ff ff       	call   80101aec <iunlockput>
  commit_trans();
801070bf:	e8 9e c9 ff ff       	call   80103a62 <commit_trans>
  return 0;
801070c4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801070c9:	c9                   	leave  
801070ca:	c3                   	ret    

801070cb <sys_chdir>:

int
sys_chdir(void)
{
801070cb:	55                   	push   %ebp
801070cc:	89 e5                	mov    %esp,%ebp
801070ce:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0)
801070d1:	8d 45 f0             	lea    -0x10(%ebp),%eax
801070d4:	89 44 24 04          	mov    %eax,0x4(%esp)
801070d8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801070df:	e8 04 f1 ff ff       	call   801061e8 <argstr>
801070e4:	85 c0                	test   %eax,%eax
801070e6:	78 14                	js     801070fc <sys_chdir+0x31>
801070e8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801070eb:	89 04 24             	mov    %eax,(%esp)
801070ee:	e8 17 b3 ff ff       	call   8010240a <namei>
801070f3:	89 45 f4             	mov    %eax,-0xc(%ebp)
801070f6:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801070fa:	75 07                	jne    80107103 <sys_chdir+0x38>
    return -1;
801070fc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107101:	eb 57                	jmp    8010715a <sys_chdir+0x8f>
  ilock(ip);
80107103:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107106:	89 04 24             	mov    %eax,(%esp)
80107109:	e8 5a a7 ff ff       	call   80101868 <ilock>
  if(ip->type != T_DIR){
8010710e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107111:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80107115:	66 83 f8 01          	cmp    $0x1,%ax
80107119:	74 12                	je     8010712d <sys_chdir+0x62>
    iunlockput(ip);
8010711b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010711e:	89 04 24             	mov    %eax,(%esp)
80107121:	e8 c6 a9 ff ff       	call   80101aec <iunlockput>
    return -1;
80107126:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010712b:	eb 2d                	jmp    8010715a <sys_chdir+0x8f>
  }
  iunlock(ip);
8010712d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107130:	89 04 24             	mov    %eax,(%esp)
80107133:	e8 7e a8 ff ff       	call   801019b6 <iunlock>
  iput(proc->cwd);
80107138:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010713e:	8b 40 68             	mov    0x68(%eax),%eax
80107141:	89 04 24             	mov    %eax,(%esp)
80107144:	e8 d2 a8 ff ff       	call   80101a1b <iput>
  proc->cwd = ip;
80107149:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010714f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80107152:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
80107155:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010715a:	c9                   	leave  
8010715b:	c3                   	ret    

8010715c <sys_exec>:

int
sys_exec(void)
{
8010715c:	55                   	push   %ebp
8010715d:	89 e5                	mov    %esp,%ebp
8010715f:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80107165:	8d 45 f0             	lea    -0x10(%ebp),%eax
80107168:	89 44 24 04          	mov    %eax,0x4(%esp)
8010716c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107173:	e8 70 f0 ff ff       	call   801061e8 <argstr>
80107178:	85 c0                	test   %eax,%eax
8010717a:	78 1a                	js     80107196 <sys_exec+0x3a>
8010717c:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
80107182:	89 44 24 04          	mov    %eax,0x4(%esp)
80107186:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010718d:	e8 bc ef ff ff       	call   8010614e <argint>
80107192:	85 c0                	test   %eax,%eax
80107194:	79 0a                	jns    801071a0 <sys_exec+0x44>
    return -1;
80107196:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010719b:	e9 e2 00 00 00       	jmp    80107282 <sys_exec+0x126>
  }
  memset(argv, 0, sizeof(argv));
801071a0:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801071a7:	00 
801071a8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801071af:	00 
801071b0:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801071b6:	89 04 24             	mov    %eax,(%esp)
801071b9:	e8 40 ec ff ff       	call   80105dfe <memset>
  for(i=0;; i++){
801071be:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
801071c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801071c8:	83 f8 1f             	cmp    $0x1f,%eax
801071cb:	76 0a                	jbe    801071d7 <sys_exec+0x7b>
      return -1;
801071cd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801071d2:	e9 ab 00 00 00       	jmp    80107282 <sys_exec+0x126>
    if(fetchint(proc, uargv+4*i, (int*)&uarg) < 0)
801071d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801071da:	c1 e0 02             	shl    $0x2,%eax
801071dd:	89 c2                	mov    %eax,%edx
801071df:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
801071e5:	8d 0c 02             	lea    (%edx,%eax,1),%ecx
801071e8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801071ee:	8d 95 68 ff ff ff    	lea    -0x98(%ebp),%edx
801071f4:	89 54 24 08          	mov    %edx,0x8(%esp)
801071f8:	89 4c 24 04          	mov    %ecx,0x4(%esp)
801071fc:	89 04 24             	mov    %eax,(%esp)
801071ff:	e8 b8 ee ff ff       	call   801060bc <fetchint>
80107204:	85 c0                	test   %eax,%eax
80107206:	79 07                	jns    8010720f <sys_exec+0xb3>
      return -1;
80107208:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010720d:	eb 73                	jmp    80107282 <sys_exec+0x126>
    if(uarg == 0){
8010720f:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
80107215:	85 c0                	test   %eax,%eax
80107217:	75 26                	jne    8010723f <sys_exec+0xe3>
      argv[i] = 0;
80107219:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010721c:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
80107223:	00 00 00 00 
      break;
80107227:	90                   	nop
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
80107228:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010722b:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
80107231:	89 54 24 04          	mov    %edx,0x4(%esp)
80107235:	89 04 24             	mov    %eax,(%esp)
80107238:	e8 bf 98 ff ff       	call   80100afc <exec>
8010723d:	eb 43                	jmp    80107282 <sys_exec+0x126>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
8010723f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107242:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107249:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
8010724f:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
80107252:	8b 95 68 ff ff ff    	mov    -0x98(%ebp),%edx
80107258:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010725e:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80107262:	89 54 24 04          	mov    %edx,0x4(%esp)
80107266:	89 04 24             	mov    %eax,(%esp)
80107269:	e8 82 ee ff ff       	call   801060f0 <fetchstr>
8010726e:	85 c0                	test   %eax,%eax
80107270:	79 07                	jns    80107279 <sys_exec+0x11d>
      return -1;
80107272:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107277:	eb 09                	jmp    80107282 <sys_exec+0x126>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
80107279:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
8010727d:	e9 43 ff ff ff       	jmp    801071c5 <sys_exec+0x69>
  return exec(path, argv);
}
80107282:	c9                   	leave  
80107283:	c3                   	ret    

80107284 <sys_pipe>:

int
sys_pipe(void)
{
80107284:	55                   	push   %ebp
80107285:	89 e5                	mov    %esp,%ebp
80107287:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
8010728a:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
80107291:	00 
80107292:	8d 45 ec             	lea    -0x14(%ebp),%eax
80107295:	89 44 24 04          	mov    %eax,0x4(%esp)
80107299:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801072a0:	e8 e1 ee ff ff       	call   80106186 <argptr>
801072a5:	85 c0                	test   %eax,%eax
801072a7:	79 0a                	jns    801072b3 <sys_pipe+0x2f>
    return -1;
801072a9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801072ae:	e9 9b 00 00 00       	jmp    8010734e <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
801072b3:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801072b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801072ba:	8d 45 e8             	lea    -0x18(%ebp),%eax
801072bd:	89 04 24             	mov    %eax,(%esp)
801072c0:	e8 6f d1 ff ff       	call   80104434 <pipealloc>
801072c5:	85 c0                	test   %eax,%eax
801072c7:	79 07                	jns    801072d0 <sys_pipe+0x4c>
    return -1;
801072c9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801072ce:	eb 7e                	jmp    8010734e <sys_pipe+0xca>
  fd0 = -1;
801072d0:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
801072d7:	8b 45 e8             	mov    -0x18(%ebp),%eax
801072da:	89 04 24             	mov    %eax,(%esp)
801072dd:	e8 83 f0 ff ff       	call   80106365 <fdalloc>
801072e2:	89 45 f4             	mov    %eax,-0xc(%ebp)
801072e5:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801072e9:	78 14                	js     801072ff <sys_pipe+0x7b>
801072eb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801072ee:	89 04 24             	mov    %eax,(%esp)
801072f1:	e8 6f f0 ff ff       	call   80106365 <fdalloc>
801072f6:	89 45 f0             	mov    %eax,-0x10(%ebp)
801072f9:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801072fd:	79 37                	jns    80107336 <sys_pipe+0xb2>
    if(fd0 >= 0)
801072ff:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107303:	78 14                	js     80107319 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
80107305:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010730b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010730e:	83 c2 08             	add    $0x8,%edx
80107311:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80107318:	00 
    fileclose(rf);
80107319:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010731c:	89 04 24             	mov    %eax,(%esp)
8010731f:	e8 a0 9c ff ff       	call   80100fc4 <fileclose>
    fileclose(wf);
80107324:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80107327:	89 04 24             	mov    %eax,(%esp)
8010732a:	e8 95 9c ff ff       	call   80100fc4 <fileclose>
    return -1;
8010732f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107334:	eb 18                	jmp    8010734e <sys_pipe+0xca>
  }
  fd[0] = fd0;
80107336:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107339:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010733c:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
8010733e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107341:	8d 50 04             	lea    0x4(%eax),%edx
80107344:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107347:	89 02                	mov    %eax,(%edx)
  return 0;
80107349:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010734e:	c9                   	leave  
8010734f:	c3                   	ret    

80107350 <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
80107350:	55                   	push   %ebp
80107351:	89 e5                	mov    %esp,%ebp
80107353:	83 ec 08             	sub    $0x8,%esp
  return fork();
80107356:	e8 ad dd ff ff       	call   80105108 <fork>
}
8010735b:	c9                   	leave  
8010735c:	c3                   	ret    

8010735d <sys_exit>:

int
sys_exit(void)
{
8010735d:	55                   	push   %ebp
8010735e:	89 e5                	mov    %esp,%ebp
80107360:	83 ec 08             	sub    $0x8,%esp
  exit();
80107363:	e8 03 df ff ff       	call   8010526b <exit>
  return 0;  // not reached
80107368:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010736d:	c9                   	leave  
8010736e:	c3                   	ret    

8010736f <sys_wait>:

int
sys_wait(void)
{
8010736f:	55                   	push   %ebp
80107370:	89 e5                	mov    %esp,%ebp
80107372:	83 ec 08             	sub    $0x8,%esp
  return wait();
80107375:	e8 2d e0 ff ff       	call   801053a7 <wait>
}
8010737a:	c9                   	leave  
8010737b:	c3                   	ret    

8010737c <sys_kill>:

int
sys_kill(void)
{
8010737c:	55                   	push   %ebp
8010737d:	89 e5                	mov    %esp,%ebp
8010737f:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
80107382:	8d 45 f4             	lea    -0xc(%ebp),%eax
80107385:	89 44 24 04          	mov    %eax,0x4(%esp)
80107389:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107390:	e8 b9 ed ff ff       	call   8010614e <argint>
80107395:	85 c0                	test   %eax,%eax
80107397:	79 07                	jns    801073a0 <sys_kill+0x24>
    return -1;
80107399:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010739e:	eb 0b                	jmp    801073ab <sys_kill+0x2f>
  return kill(pid);
801073a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073a3:	89 04 24             	mov    %eax,(%esp)
801073a6:	e8 f7 e4 ff ff       	call   801058a2 <kill>
}
801073ab:	c9                   	leave  
801073ac:	c3                   	ret    

801073ad <sys_getpid>:

int
sys_getpid(void)
{
801073ad:	55                   	push   %ebp
801073ae:	89 e5                	mov    %esp,%ebp
  return proc->pid;
801073b0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801073b6:	8b 40 10             	mov    0x10(%eax),%eax
}
801073b9:	5d                   	pop    %ebp
801073ba:	c3                   	ret    

801073bb <sys_sbrk>:

int
sys_sbrk(void)
{
801073bb:	55                   	push   %ebp
801073bc:	89 e5                	mov    %esp,%ebp
801073be:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
801073c1:	8d 45 f0             	lea    -0x10(%ebp),%eax
801073c4:	89 44 24 04          	mov    %eax,0x4(%esp)
801073c8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801073cf:	e8 7a ed ff ff       	call   8010614e <argint>
801073d4:	85 c0                	test   %eax,%eax
801073d6:	79 07                	jns    801073df <sys_sbrk+0x24>
    return -1;
801073d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801073dd:	eb 24                	jmp    80107403 <sys_sbrk+0x48>
  addr = proc->sz;
801073df:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801073e5:	8b 00                	mov    (%eax),%eax
801073e7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
801073ea:	8b 45 f0             	mov    -0x10(%ebp),%eax
801073ed:	89 04 24             	mov    %eax,(%esp)
801073f0:	e8 6e dc ff ff       	call   80105063 <growproc>
801073f5:	85 c0                	test   %eax,%eax
801073f7:	79 07                	jns    80107400 <sys_sbrk+0x45>
    return -1;
801073f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801073fe:	eb 03                	jmp    80107403 <sys_sbrk+0x48>
  return addr;
80107400:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80107403:	c9                   	leave  
80107404:	c3                   	ret    

80107405 <sys_sleep>:

int
sys_sleep(void)
{
80107405:	55                   	push   %ebp
80107406:	89 e5                	mov    %esp,%ebp
80107408:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
8010740b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010740e:	89 44 24 04          	mov    %eax,0x4(%esp)
80107412:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107419:	e8 30 ed ff ff       	call   8010614e <argint>
8010741e:	85 c0                	test   %eax,%eax
80107420:	79 07                	jns    80107429 <sys_sleep+0x24>
    return -1;
80107422:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107427:	eb 6c                	jmp    80107495 <sys_sleep+0x90>
  acquire(&tickslock);
80107429:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
80107430:	e8 42 e7 ff ff       	call   80105b77 <acquire>
  ticks0 = ticks;
80107435:	a1 c0 a3 12 80       	mov    0x8012a3c0,%eax
8010743a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
8010743d:	eb 34                	jmp    80107473 <sys_sleep+0x6e>
    if(proc->killed){
8010743f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107445:	8b 40 24             	mov    0x24(%eax),%eax
80107448:	85 c0                	test   %eax,%eax
8010744a:	74 13                	je     8010745f <sys_sleep+0x5a>
      release(&tickslock);
8010744c:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
80107453:	e8 ba e7 ff ff       	call   80105c12 <release>
      return -1;
80107458:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010745d:	eb 36                	jmp    80107495 <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
8010745f:	c7 44 24 04 80 9b 12 	movl   $0x80129b80,0x4(%esp)
80107466:	80 
80107467:	c7 04 24 c0 a3 12 80 	movl   $0x8012a3c0,(%esp)
8010746e:	e8 92 e2 ff ff       	call   80105705 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
80107473:	a1 c0 a3 12 80       	mov    0x8012a3c0,%eax
80107478:	89 c2                	mov    %eax,%edx
8010747a:	2b 55 f4             	sub    -0xc(%ebp),%edx
8010747d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107480:	39 c2                	cmp    %eax,%edx
80107482:	72 bb                	jb     8010743f <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
80107484:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
8010748b:	e8 82 e7 ff ff       	call   80105c12 <release>
  return 0;
80107490:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107495:	c9                   	leave  
80107496:	c3                   	ret    

80107497 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80107497:	55                   	push   %ebp
80107498:	89 e5                	mov    %esp,%ebp
8010749a:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
8010749d:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
801074a4:	e8 ce e6 ff ff       	call   80105b77 <acquire>
  xticks = ticks;
801074a9:	a1 c0 a3 12 80       	mov    0x8012a3c0,%eax
801074ae:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
801074b1:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
801074b8:	e8 55 e7 ff ff       	call   80105c12 <release>
  return xticks;
801074bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801074c0:	c9                   	leave  
801074c1:	c3                   	ret    

801074c2 <sys_enableSwapping>:

void
sys_enableSwapping(void)
{
801074c2:	55                   	push   %ebp
801074c3:	89 e5                	mov    %esp,%ebp
  swapFlag = 1;
801074c5:	c7 05 80 c6 10 80 01 	movl   $0x1,0x8010c680
801074cc:	00 00 00 
}
801074cf:	5d                   	pop    %ebp
801074d0:	c3                   	ret    

801074d1 <sys_disableSwapping>:

void
sys_disableSwapping(void)
{
801074d1:	55                   	push   %ebp
801074d2:	89 e5                	mov    %esp,%ebp
  swapFlag = 0;
801074d4:	c7 05 80 c6 10 80 00 	movl   $0x0,0x8010c680
801074db:	00 00 00 
}
801074de:	5d                   	pop    %ebp
801074df:	c3                   	ret    

801074e0 <sys_sleep2>:

int
sys_sleep2(void)
{
801074e0:	55                   	push   %ebp
801074e1:	89 e5                	mov    %esp,%ebp
801074e3:	83 ec 18             	sub    $0x18,%esp
  acquire(&tickslock);
801074e6:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
801074ed:	e8 85 e6 ff ff       	call   80105b77 <acquire>
  if(proc->killed){
801074f2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801074f8:	8b 40 24             	mov    0x24(%eax),%eax
801074fb:	85 c0                	test   %eax,%eax
801074fd:	74 13                	je     80107512 <sys_sleep2+0x32>
    release(&tickslock);
801074ff:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
80107506:	e8 07 e7 ff ff       	call   80105c12 <release>
    return -1;
8010750b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107510:	eb 25                	jmp    80107537 <sys_sleep2+0x57>
  }
  sleep(&swapFlag, &tickslock);
80107512:	c7 44 24 04 80 9b 12 	movl   $0x80129b80,0x4(%esp)
80107519:	80 
8010751a:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
80107521:	e8 df e1 ff ff       	call   80105705 <sleep>
  release(&tickslock);
80107526:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
8010752d:	e8 e0 e6 ff ff       	call   80105c12 <release>
  return 0;
80107532:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107537:	c9                   	leave  
80107538:	c3                   	ret    

80107539 <sys_wakeup2>:

int
sys_wakeup2(void)
{
80107539:	55                   	push   %ebp
8010753a:	89 e5                	mov    %esp,%ebp
8010753c:	83 ec 18             	sub    $0x18,%esp
  wakeup(&swapFlag);
8010753f:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
80107546:	e8 2c e3 ff ff       	call   80105877 <wakeup>
  return 0;
8010754b:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107550:	c9                   	leave  
80107551:	c3                   	ret    

80107552 <sys_getAllocatedPages>:

int
sys_getAllocatedPages(void)
{
80107552:	55                   	push   %ebp
80107553:	89 e5                	mov    %esp,%ebp
80107555:	83 ec 28             	sub    $0x28,%esp
  int pid;
  if(argint(0, &pid) < 0)
80107558:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010755b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010755f:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107566:	e8 e3 eb ff ff       	call   8010614e <argint>
8010756b:	85 c0                	test   %eax,%eax
8010756d:	79 07                	jns    80107576 <sys_getAllocatedPages+0x24>
    return -1;
8010756f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107574:	eb 0b                	jmp    80107581 <sys_getAllocatedPages+0x2f>
  return getAllocatedPages(pid);
80107576:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107579:	89 04 24             	mov    %eax,(%esp)
8010757c:	e8 bb e4 ff ff       	call   80105a3c <getAllocatedPages>
}
80107581:	c9                   	leave  
80107582:	c3                   	ret    

80107583 <sys_shmget>:

int 
sys_shmget(void)
{
80107583:	55                   	push   %ebp
80107584:	89 e5                	mov    %esp,%ebp
80107586:	83 ec 28             	sub    $0x28,%esp
  int key,size, shmflg;
  
  if(argint(0, &key) < 0)
80107589:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010758c:	89 44 24 04          	mov    %eax,0x4(%esp)
80107590:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107597:	e8 b2 eb ff ff       	call   8010614e <argint>
8010759c:	85 c0                	test   %eax,%eax
8010759e:	79 07                	jns    801075a7 <sys_shmget+0x24>
    return -1;
801075a0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801075a5:	eb 65                	jmp    8010760c <sys_shmget+0x89>
  
  if(argint(1, &size) < 0)
801075a7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801075aa:	89 44 24 04          	mov    %eax,0x4(%esp)
801075ae:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801075b5:	e8 94 eb ff ff       	call   8010614e <argint>
801075ba:	85 c0                	test   %eax,%eax
801075bc:	79 07                	jns    801075c5 <sys_shmget+0x42>
    return -1;
801075be:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801075c3:	eb 47                	jmp    8010760c <sys_shmget+0x89>
  if(size<0)
801075c5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801075c8:	85 c0                	test   %eax,%eax
801075ca:	79 07                	jns    801075d3 <sys_shmget+0x50>
    return -1;
801075cc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801075d1:	eb 39                	jmp    8010760c <sys_shmget+0x89>
  
  if(argint(2, &shmflg) < 0)
801075d3:	8d 45 ec             	lea    -0x14(%ebp),%eax
801075d6:	89 44 24 04          	mov    %eax,0x4(%esp)
801075da:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801075e1:	e8 68 eb ff ff       	call   8010614e <argint>
801075e6:	85 c0                	test   %eax,%eax
801075e8:	79 07                	jns    801075f1 <sys_shmget+0x6e>
    return -1;
801075ea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801075ef:	eb 1b                	jmp    8010760c <sys_shmget+0x89>
  
  return shmget(key, (uint)size,shmflg);
801075f1:	8b 4d ec             	mov    -0x14(%ebp),%ecx
801075f4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801075f7:	89 c2                	mov    %eax,%edx
801075f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075fc:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80107600:	89 54 24 04          	mov    %edx,0x4(%esp)
80107604:	89 04 24             	mov    %eax,(%esp)
80107607:	e8 51 b5 ff ff       	call   80102b5d <shmget>
}
8010760c:	c9                   	leave  
8010760d:	c3                   	ret    

8010760e <sys_shmdel>:

int 
sys_shmdel(void)
{
8010760e:	55                   	push   %ebp
8010760f:	89 e5                	mov    %esp,%ebp
80107611:	83 ec 28             	sub    $0x28,%esp
  int shmid;
  if(argint(0, &shmid) < 0)
80107614:	8d 45 f4             	lea    -0xc(%ebp),%eax
80107617:	89 44 24 04          	mov    %eax,0x4(%esp)
8010761b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107622:	e8 27 eb ff ff       	call   8010614e <argint>
80107627:	85 c0                	test   %eax,%eax
80107629:	79 07                	jns    80107632 <sys_shmdel+0x24>
    return -1;
8010762b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107630:	eb 0b                	jmp    8010763d <sys_shmdel+0x2f>
  
  return shmdel(shmid);
80107632:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107635:	89 04 24             	mov    %eax,(%esp)
80107638:	e8 85 b6 ff ff       	call   80102cc2 <shmdel>
}
8010763d:	c9                   	leave  
8010763e:	c3                   	ret    

8010763f <sys_shmat>:

void *
sys_shmat(void)
{
8010763f:	55                   	push   %ebp
80107640:	89 e5                	mov    %esp,%ebp
80107642:	83 ec 28             	sub    $0x28,%esp
  int shmid,shmflg;
  
  if(argint(0, &shmid) < 0)
80107645:	8d 45 f4             	lea    -0xc(%ebp),%eax
80107648:	89 44 24 04          	mov    %eax,0x4(%esp)
8010764c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107653:	e8 f6 ea ff ff       	call   8010614e <argint>
80107658:	85 c0                	test   %eax,%eax
8010765a:	79 07                	jns    80107663 <sys_shmat+0x24>
    return (void*)-1;
8010765c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107661:	eb 30                	jmp    80107693 <sys_shmat+0x54>
  
  if(argint(1, &shmflg) < 0)
80107663:	8d 45 f0             	lea    -0x10(%ebp),%eax
80107666:	89 44 24 04          	mov    %eax,0x4(%esp)
8010766a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80107671:	e8 d8 ea ff ff       	call   8010614e <argint>
80107676:	85 c0                	test   %eax,%eax
80107678:	79 07                	jns    80107681 <sys_shmat+0x42>
    return (void*)-1;
8010767a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010767f:	eb 12                	jmp    80107693 <sys_shmat+0x54>
  
  return shmat(shmid,shmflg);
80107681:	8b 55 f0             	mov    -0x10(%ebp),%edx
80107684:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107687:	89 54 24 04          	mov    %edx,0x4(%esp)
8010768b:	89 04 24             	mov    %eax,(%esp)
8010768e:	e8 0a b7 ff ff       	call   80102d9d <shmat>
}
80107693:	c9                   	leave  
80107694:	c3                   	ret    

80107695 <sys_shmdt>:

int 
sys_shmdt(void)
{
80107695:	55                   	push   %ebp
80107696:	89 e5                	mov    %esp,%ebp
80107698:	83 ec 28             	sub    $0x28,%esp
  void* shmaddr;
  if(argptr(0, (void*)&shmaddr,sizeof(void*)) < 0)
8010769b:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801076a2:	00 
801076a3:	8d 45 f4             	lea    -0xc(%ebp),%eax
801076a6:	89 44 24 04          	mov    %eax,0x4(%esp)
801076aa:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801076b1:	e8 d0 ea ff ff       	call   80106186 <argptr>
801076b6:	85 c0                	test   %eax,%eax
801076b8:	79 07                	jns    801076c1 <sys_shmdt+0x2c>
    return -1;
801076ba:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801076bf:	eb 0b                	jmp    801076cc <sys_shmdt+0x37>
  return shmdt(shmaddr);
801076c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076c4:	89 04 24             	mov    %eax,(%esp)
801076c7:	e8 e9 b8 ff ff       	call   80102fb5 <shmdt>
}
801076cc:	c9                   	leave  
801076cd:	c3                   	ret    
	...

801076d0 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801076d0:	55                   	push   %ebp
801076d1:	89 e5                	mov    %esp,%ebp
801076d3:	83 ec 08             	sub    $0x8,%esp
801076d6:	8b 55 08             	mov    0x8(%ebp),%edx
801076d9:	8b 45 0c             	mov    0xc(%ebp),%eax
801076dc:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801076e0:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801076e3:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801076e7:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801076eb:	ee                   	out    %al,(%dx)
}
801076ec:	c9                   	leave  
801076ed:	c3                   	ret    

801076ee <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
801076ee:	55                   	push   %ebp
801076ef:	89 e5                	mov    %esp,%ebp
801076f1:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
801076f4:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
801076fb:	00 
801076fc:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
80107703:	e8 c8 ff ff ff       	call   801076d0 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
80107708:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
8010770f:	00 
80107710:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
80107717:	e8 b4 ff ff ff       	call   801076d0 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
8010771c:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
80107723:	00 
80107724:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
8010772b:	e8 a0 ff ff ff       	call   801076d0 <outb>
  picenable(IRQ_TIMER);
80107730:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80107737:	e8 81 cb ff ff       	call   801042bd <picenable>
}
8010773c:	c9                   	leave  
8010773d:	c3                   	ret    
	...

80107740 <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
80107740:	1e                   	push   %ds
  pushl %es
80107741:	06                   	push   %es
  pushl %fs
80107742:	0f a0                	push   %fs
  pushl %gs
80107744:	0f a8                	push   %gs
  pushal
80107746:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
80107747:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
8010774b:	8e d8                	mov    %eax,%ds
  movw %ax, %es
8010774d:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
8010774f:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
80107753:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
80107755:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
80107757:	54                   	push   %esp
  call trap
80107758:	e8 de 01 00 00       	call   8010793b <trap>
  addl $4, %esp
8010775d:	83 c4 04             	add    $0x4,%esp

80107760 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80107760:	61                   	popa   
  popl %gs
80107761:	0f a9                	pop    %gs
  popl %fs
80107763:	0f a1                	pop    %fs
  popl %es
80107765:	07                   	pop    %es
  popl %ds
80107766:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
80107767:	83 c4 08             	add    $0x8,%esp
  iret
8010776a:	cf                   	iret   
	...

8010776c <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
8010776c:	55                   	push   %ebp
8010776d:	89 e5                	mov    %esp,%ebp
8010776f:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80107772:	8b 45 0c             	mov    0xc(%ebp),%eax
80107775:	83 e8 01             	sub    $0x1,%eax
80107778:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
8010777c:	8b 45 08             	mov    0x8(%ebp),%eax
8010777f:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80107783:	8b 45 08             	mov    0x8(%ebp),%eax
80107786:	c1 e8 10             	shr    $0x10,%eax
80107789:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
8010778d:	8d 45 fa             	lea    -0x6(%ebp),%eax
80107790:	0f 01 18             	lidtl  (%eax)
}
80107793:	c9                   	leave  
80107794:	c3                   	ret    

80107795 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
80107795:	55                   	push   %ebp
80107796:	89 e5                	mov    %esp,%ebp
80107798:	53                   	push   %ebx
80107799:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
8010779c:	0f 20 d3             	mov    %cr2,%ebx
8010779f:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return val;
801077a2:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801077a5:	83 c4 10             	add    $0x10,%esp
801077a8:	5b                   	pop    %ebx
801077a9:	5d                   	pop    %ebp
801077aa:	c3                   	ret    

801077ab <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
801077ab:	55                   	push   %ebp
801077ac:	89 e5                	mov    %esp,%ebp
801077ae:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
801077b1:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801077b8:	e9 c3 00 00 00       	jmp    80107880 <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
801077bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077c0:	8b 04 85 bc c0 10 80 	mov    -0x7fef3f44(,%eax,4),%eax
801077c7:	89 c2                	mov    %eax,%edx
801077c9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077cc:	66 89 14 c5 c0 9b 12 	mov    %dx,-0x7fed6440(,%eax,8)
801077d3:	80 
801077d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077d7:	66 c7 04 c5 c2 9b 12 	movw   $0x8,-0x7fed643e(,%eax,8)
801077de:	80 08 00 
801077e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077e4:	0f b6 14 c5 c4 9b 12 	movzbl -0x7fed643c(,%eax,8),%edx
801077eb:	80 
801077ec:	83 e2 e0             	and    $0xffffffe0,%edx
801077ef:	88 14 c5 c4 9b 12 80 	mov    %dl,-0x7fed643c(,%eax,8)
801077f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077f9:	0f b6 14 c5 c4 9b 12 	movzbl -0x7fed643c(,%eax,8),%edx
80107800:	80 
80107801:	83 e2 1f             	and    $0x1f,%edx
80107804:	88 14 c5 c4 9b 12 80 	mov    %dl,-0x7fed643c(,%eax,8)
8010780b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010780e:	0f b6 14 c5 c5 9b 12 	movzbl -0x7fed643b(,%eax,8),%edx
80107815:	80 
80107816:	83 e2 f0             	and    $0xfffffff0,%edx
80107819:	83 ca 0e             	or     $0xe,%edx
8010781c:	88 14 c5 c5 9b 12 80 	mov    %dl,-0x7fed643b(,%eax,8)
80107823:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107826:	0f b6 14 c5 c5 9b 12 	movzbl -0x7fed643b(,%eax,8),%edx
8010782d:	80 
8010782e:	83 e2 ef             	and    $0xffffffef,%edx
80107831:	88 14 c5 c5 9b 12 80 	mov    %dl,-0x7fed643b(,%eax,8)
80107838:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010783b:	0f b6 14 c5 c5 9b 12 	movzbl -0x7fed643b(,%eax,8),%edx
80107842:	80 
80107843:	83 e2 9f             	and    $0xffffff9f,%edx
80107846:	88 14 c5 c5 9b 12 80 	mov    %dl,-0x7fed643b(,%eax,8)
8010784d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107850:	0f b6 14 c5 c5 9b 12 	movzbl -0x7fed643b(,%eax,8),%edx
80107857:	80 
80107858:	83 ca 80             	or     $0xffffff80,%edx
8010785b:	88 14 c5 c5 9b 12 80 	mov    %dl,-0x7fed643b(,%eax,8)
80107862:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107865:	8b 04 85 bc c0 10 80 	mov    -0x7fef3f44(,%eax,4),%eax
8010786c:	c1 e8 10             	shr    $0x10,%eax
8010786f:	89 c2                	mov    %eax,%edx
80107871:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107874:	66 89 14 c5 c6 9b 12 	mov    %dx,-0x7fed643a(,%eax,8)
8010787b:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
8010787c:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80107880:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
80107887:	0f 8e 30 ff ff ff    	jle    801077bd <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
8010788d:	a1 bc c1 10 80       	mov    0x8010c1bc,%eax
80107892:	66 a3 c0 9d 12 80    	mov    %ax,0x80129dc0
80107898:	66 c7 05 c2 9d 12 80 	movw   $0x8,0x80129dc2
8010789f:	08 00 
801078a1:	0f b6 05 c4 9d 12 80 	movzbl 0x80129dc4,%eax
801078a8:	83 e0 e0             	and    $0xffffffe0,%eax
801078ab:	a2 c4 9d 12 80       	mov    %al,0x80129dc4
801078b0:	0f b6 05 c4 9d 12 80 	movzbl 0x80129dc4,%eax
801078b7:	83 e0 1f             	and    $0x1f,%eax
801078ba:	a2 c4 9d 12 80       	mov    %al,0x80129dc4
801078bf:	0f b6 05 c5 9d 12 80 	movzbl 0x80129dc5,%eax
801078c6:	83 c8 0f             	or     $0xf,%eax
801078c9:	a2 c5 9d 12 80       	mov    %al,0x80129dc5
801078ce:	0f b6 05 c5 9d 12 80 	movzbl 0x80129dc5,%eax
801078d5:	83 e0 ef             	and    $0xffffffef,%eax
801078d8:	a2 c5 9d 12 80       	mov    %al,0x80129dc5
801078dd:	0f b6 05 c5 9d 12 80 	movzbl 0x80129dc5,%eax
801078e4:	83 c8 60             	or     $0x60,%eax
801078e7:	a2 c5 9d 12 80       	mov    %al,0x80129dc5
801078ec:	0f b6 05 c5 9d 12 80 	movzbl 0x80129dc5,%eax
801078f3:	83 c8 80             	or     $0xffffff80,%eax
801078f6:	a2 c5 9d 12 80       	mov    %al,0x80129dc5
801078fb:	a1 bc c1 10 80       	mov    0x8010c1bc,%eax
80107900:	c1 e8 10             	shr    $0x10,%eax
80107903:	66 a3 c6 9d 12 80    	mov    %ax,0x80129dc6
  
  initlock(&tickslock, "time");
80107909:	c7 44 24 04 74 9c 10 	movl   $0x80109c74,0x4(%esp)
80107910:	80 
80107911:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
80107918:	e8 39 e2 ff ff       	call   80105b56 <initlock>
}
8010791d:	c9                   	leave  
8010791e:	c3                   	ret    

8010791f <idtinit>:

void
idtinit(void)
{
8010791f:	55                   	push   %ebp
80107920:	89 e5                	mov    %esp,%ebp
80107922:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
80107925:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
8010792c:	00 
8010792d:	c7 04 24 c0 9b 12 80 	movl   $0x80129bc0,(%esp)
80107934:	e8 33 fe ff ff       	call   8010776c <lidt>
}
80107939:	c9                   	leave  
8010793a:	c3                   	ret    

8010793b <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
8010793b:	55                   	push   %ebp
8010793c:	89 e5                	mov    %esp,%ebp
8010793e:	57                   	push   %edi
8010793f:	56                   	push   %esi
80107940:	53                   	push   %ebx
80107941:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
80107944:	8b 45 08             	mov    0x8(%ebp),%eax
80107947:	8b 40 30             	mov    0x30(%eax),%eax
8010794a:	83 f8 40             	cmp    $0x40,%eax
8010794d:	75 3e                	jne    8010798d <trap+0x52>
    if(proc->killed)
8010794f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107955:	8b 40 24             	mov    0x24(%eax),%eax
80107958:	85 c0                	test   %eax,%eax
8010795a:	74 05                	je     80107961 <trap+0x26>
      exit();
8010795c:	e8 0a d9 ff ff       	call   8010526b <exit>
    proc->tf = tf;
80107961:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107967:	8b 55 08             	mov    0x8(%ebp),%edx
8010796a:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
8010796d:	e8 b9 e8 ff ff       	call   8010622b <syscall>
    if(proc->killed)
80107972:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107978:	8b 40 24             	mov    0x24(%eax),%eax
8010797b:	85 c0                	test   %eax,%eax
8010797d:	0f 84 34 02 00 00    	je     80107bb7 <trap+0x27c>
      exit();
80107983:	e8 e3 d8 ff ff       	call   8010526b <exit>
    return;
80107988:	e9 2a 02 00 00       	jmp    80107bb7 <trap+0x27c>
  }

  switch(tf->trapno){
8010798d:	8b 45 08             	mov    0x8(%ebp),%eax
80107990:	8b 40 30             	mov    0x30(%eax),%eax
80107993:	83 e8 20             	sub    $0x20,%eax
80107996:	83 f8 1f             	cmp    $0x1f,%eax
80107999:	0f 87 bc 00 00 00    	ja     80107a5b <trap+0x120>
8010799f:	8b 04 85 1c 9d 10 80 	mov    -0x7fef62e4(,%eax,4),%eax
801079a6:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
801079a8:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801079ae:	0f b6 00             	movzbl (%eax),%eax
801079b1:	84 c0                	test   %al,%al
801079b3:	75 31                	jne    801079e6 <trap+0xab>
      acquire(&tickslock);
801079b5:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
801079bc:	e8 b6 e1 ff ff       	call   80105b77 <acquire>
      ticks++;
801079c1:	a1 c0 a3 12 80       	mov    0x8012a3c0,%eax
801079c6:	83 c0 01             	add    $0x1,%eax
801079c9:	a3 c0 a3 12 80       	mov    %eax,0x8012a3c0
      wakeup(&ticks);
801079ce:	c7 04 24 c0 a3 12 80 	movl   $0x8012a3c0,(%esp)
801079d5:	e8 9d de ff ff       	call   80105877 <wakeup>
      release(&tickslock);
801079da:	c7 04 24 80 9b 12 80 	movl   $0x80129b80,(%esp)
801079e1:	e8 2c e2 ff ff       	call   80105c12 <release>
    }
    lapiceoi();
801079e6:	e8 fa bc ff ff       	call   801036e5 <lapiceoi>
    break;
801079eb:	e9 41 01 00 00       	jmp    80107b31 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
801079f0:	e8 02 ad ff ff       	call   801026f7 <ideintr>
    lapiceoi();
801079f5:	e8 eb bc ff ff       	call   801036e5 <lapiceoi>
    break;
801079fa:	e9 32 01 00 00       	jmp    80107b31 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
801079ff:	e8 bf ba ff ff       	call   801034c3 <kbdintr>
    lapiceoi();
80107a04:	e8 dc bc ff ff       	call   801036e5 <lapiceoi>
    break;
80107a09:	e9 23 01 00 00       	jmp    80107b31 <trap+0x1f6>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80107a0e:	e8 a9 03 00 00       	call   80107dbc <uartintr>
    lapiceoi();
80107a13:	e8 cd bc ff ff       	call   801036e5 <lapiceoi>
    break;
80107a18:	e9 14 01 00 00       	jmp    80107b31 <trap+0x1f6>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
            cpu->id, tf->cs, tf->eip);
80107a1d:	8b 45 08             	mov    0x8(%ebp),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80107a20:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
80107a23:	8b 45 08             	mov    0x8(%ebp),%eax
80107a26:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80107a2a:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80107a2d:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107a33:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80107a36:	0f b6 c0             	movzbl %al,%eax
80107a39:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80107a3d:	89 54 24 08          	mov    %edx,0x8(%esp)
80107a41:	89 44 24 04          	mov    %eax,0x4(%esp)
80107a45:	c7 04 24 7c 9c 10 80 	movl   $0x80109c7c,(%esp)
80107a4c:	e8 50 89 ff ff       	call   801003a1 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
80107a51:	e8 8f bc ff ff       	call   801036e5 <lapiceoi>
    break;
80107a56:	e9 d6 00 00 00       	jmp    80107b31 <trap+0x1f6>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
80107a5b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107a61:	85 c0                	test   %eax,%eax
80107a63:	74 11                	je     80107a76 <trap+0x13b>
80107a65:	8b 45 08             	mov    0x8(%ebp),%eax
80107a68:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80107a6c:	0f b7 c0             	movzwl %ax,%eax
80107a6f:	83 e0 03             	and    $0x3,%eax
80107a72:	85 c0                	test   %eax,%eax
80107a74:	75 46                	jne    80107abc <trap+0x181>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80107a76:	e8 1a fd ff ff       	call   80107795 <rcr2>
              tf->trapno, cpu->id, tf->eip, rcr2());
80107a7b:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80107a7e:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
80107a81:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107a88:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80107a8b:	0f b6 ca             	movzbl %dl,%ecx
              tf->trapno, cpu->id, tf->eip, rcr2());
80107a8e:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80107a91:	8b 52 30             	mov    0x30(%edx),%edx
80107a94:	89 44 24 10          	mov    %eax,0x10(%esp)
80107a98:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80107a9c:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80107aa0:	89 54 24 04          	mov    %edx,0x4(%esp)
80107aa4:	c7 04 24 a0 9c 10 80 	movl   $0x80109ca0,(%esp)
80107aab:	e8 f1 88 ff ff       	call   801003a1 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80107ab0:	c7 04 24 d2 9c 10 80 	movl   $0x80109cd2,(%esp)
80107ab7:	e8 81 8a ff ff       	call   8010053d <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107abc:	e8 d4 fc ff ff       	call   80107795 <rcr2>
80107ac1:	89 c2                	mov    %eax,%edx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80107ac3:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107ac6:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80107ac9:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107acf:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107ad2:	0f b6 f0             	movzbl %al,%esi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80107ad5:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107ad8:	8b 58 34             	mov    0x34(%eax),%ebx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80107adb:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107ade:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80107ae1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107ae7:	83 c0 6c             	add    $0x6c,%eax
80107aea:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80107aed:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80107af3:	8b 40 10             	mov    0x10(%eax),%eax
80107af6:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80107afa:	89 7c 24 18          	mov    %edi,0x18(%esp)
80107afe:	89 74 24 14          	mov    %esi,0x14(%esp)
80107b02:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80107b06:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80107b0a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80107b0d:	89 54 24 08          	mov    %edx,0x8(%esp)
80107b11:	89 44 24 04          	mov    %eax,0x4(%esp)
80107b15:	c7 04 24 d8 9c 10 80 	movl   $0x80109cd8,(%esp)
80107b1c:	e8 80 88 ff ff       	call   801003a1 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80107b21:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b27:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80107b2e:	eb 01                	jmp    80107b31 <trap+0x1f6>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80107b30:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80107b31:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b37:	85 c0                	test   %eax,%eax
80107b39:	74 24                	je     80107b5f <trap+0x224>
80107b3b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b41:	8b 40 24             	mov    0x24(%eax),%eax
80107b44:	85 c0                	test   %eax,%eax
80107b46:	74 17                	je     80107b5f <trap+0x224>
80107b48:	8b 45 08             	mov    0x8(%ebp),%eax
80107b4b:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80107b4f:	0f b7 c0             	movzwl %ax,%eax
80107b52:	83 e0 03             	and    $0x3,%eax
80107b55:	83 f8 03             	cmp    $0x3,%eax
80107b58:	75 05                	jne    80107b5f <trap+0x224>
    exit();
80107b5a:	e8 0c d7 ff ff       	call   8010526b <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80107b5f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b65:	85 c0                	test   %eax,%eax
80107b67:	74 1e                	je     80107b87 <trap+0x24c>
80107b69:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b6f:	8b 40 0c             	mov    0xc(%eax),%eax
80107b72:	83 f8 04             	cmp    $0x4,%eax
80107b75:	75 10                	jne    80107b87 <trap+0x24c>
80107b77:	8b 45 08             	mov    0x8(%ebp),%eax
80107b7a:	8b 40 30             	mov    0x30(%eax),%eax
80107b7d:	83 f8 20             	cmp    $0x20,%eax
80107b80:	75 05                	jne    80107b87 <trap+0x24c>
    yield();
80107b82:	e8 20 db ff ff       	call   801056a7 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80107b87:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b8d:	85 c0                	test   %eax,%eax
80107b8f:	74 27                	je     80107bb8 <trap+0x27d>
80107b91:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80107b97:	8b 40 24             	mov    0x24(%eax),%eax
80107b9a:	85 c0                	test   %eax,%eax
80107b9c:	74 1a                	je     80107bb8 <trap+0x27d>
80107b9e:	8b 45 08             	mov    0x8(%ebp),%eax
80107ba1:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80107ba5:	0f b7 c0             	movzwl %ax,%eax
80107ba8:	83 e0 03             	and    $0x3,%eax
80107bab:	83 f8 03             	cmp    $0x3,%eax
80107bae:	75 08                	jne    80107bb8 <trap+0x27d>
    exit();
80107bb0:	e8 b6 d6 ff ff       	call   8010526b <exit>
80107bb5:	eb 01                	jmp    80107bb8 <trap+0x27d>
      exit();
    proc->tf = tf;
    syscall();
    if(proc->killed)
      exit();
    return;
80107bb7:	90                   	nop
    yield();

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
    exit();
}
80107bb8:	83 c4 3c             	add    $0x3c,%esp
80107bbb:	5b                   	pop    %ebx
80107bbc:	5e                   	pop    %esi
80107bbd:	5f                   	pop    %edi
80107bbe:	5d                   	pop    %ebp
80107bbf:	c3                   	ret    

80107bc0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80107bc0:	55                   	push   %ebp
80107bc1:	89 e5                	mov    %esp,%ebp
80107bc3:	53                   	push   %ebx
80107bc4:	83 ec 14             	sub    $0x14,%esp
80107bc7:	8b 45 08             	mov    0x8(%ebp),%eax
80107bca:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80107bce:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80107bd2:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80107bd6:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80107bda:	ec                   	in     (%dx),%al
80107bdb:	89 c3                	mov    %eax,%ebx
80107bdd:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80107be0:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80107be4:	83 c4 14             	add    $0x14,%esp
80107be7:	5b                   	pop    %ebx
80107be8:	5d                   	pop    %ebp
80107be9:	c3                   	ret    

80107bea <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80107bea:	55                   	push   %ebp
80107beb:	89 e5                	mov    %esp,%ebp
80107bed:	83 ec 08             	sub    $0x8,%esp
80107bf0:	8b 55 08             	mov    0x8(%ebp),%edx
80107bf3:	8b 45 0c             	mov    0xc(%ebp),%eax
80107bf6:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80107bfa:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80107bfd:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80107c01:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80107c05:	ee                   	out    %al,(%dx)
}
80107c06:	c9                   	leave  
80107c07:	c3                   	ret    

80107c08 <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80107c08:	55                   	push   %ebp
80107c09:	89 e5                	mov    %esp,%ebp
80107c0b:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80107c0e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107c15:	00 
80107c16:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80107c1d:	e8 c8 ff ff ff       	call   80107bea <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80107c22:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80107c29:	00 
80107c2a:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80107c31:	e8 b4 ff ff ff       	call   80107bea <outb>
  outb(COM1+0, 115200/9600);
80107c36:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80107c3d:	00 
80107c3e:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80107c45:	e8 a0 ff ff ff       	call   80107bea <outb>
  outb(COM1+1, 0);
80107c4a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107c51:	00 
80107c52:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80107c59:	e8 8c ff ff ff       	call   80107bea <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80107c5e:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80107c65:	00 
80107c66:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80107c6d:	e8 78 ff ff ff       	call   80107bea <outb>
  outb(COM1+4, 0);
80107c72:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107c79:	00 
80107c7a:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80107c81:	e8 64 ff ff ff       	call   80107bea <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80107c86:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80107c8d:	00 
80107c8e:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80107c95:	e8 50 ff ff ff       	call   80107bea <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80107c9a:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80107ca1:	e8 1a ff ff ff       	call   80107bc0 <inb>
80107ca6:	3c ff                	cmp    $0xff,%al
80107ca8:	74 6c                	je     80107d16 <uartinit+0x10e>
    return;
  uart = 1;
80107caa:	c7 05 d4 c6 10 80 01 	movl   $0x1,0x8010c6d4
80107cb1:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80107cb4:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80107cbb:	e8 00 ff ff ff       	call   80107bc0 <inb>
  inb(COM1+0);
80107cc0:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80107cc7:	e8 f4 fe ff ff       	call   80107bc0 <inb>
  picenable(IRQ_COM1);
80107ccc:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80107cd3:	e8 e5 c5 ff ff       	call   801042bd <picenable>
  ioapicenable(IRQ_COM1, 0);
80107cd8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107cdf:	00 
80107ce0:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80107ce7:	e8 8e ac ff ff       	call   8010297a <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80107cec:	c7 45 f4 9c 9d 10 80 	movl   $0x80109d9c,-0xc(%ebp)
80107cf3:	eb 15                	jmp    80107d0a <uartinit+0x102>
    uartputc(*p);
80107cf5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cf8:	0f b6 00             	movzbl (%eax),%eax
80107cfb:	0f be c0             	movsbl %al,%eax
80107cfe:	89 04 24             	mov    %eax,(%esp)
80107d01:	e8 13 00 00 00       	call   80107d19 <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80107d06:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80107d0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d0d:	0f b6 00             	movzbl (%eax),%eax
80107d10:	84 c0                	test   %al,%al
80107d12:	75 e1                	jne    80107cf5 <uartinit+0xed>
80107d14:	eb 01                	jmp    80107d17 <uartinit+0x10f>
  outb(COM1+4, 0);
  outb(COM1+1, 0x01);    // Enable receive interrupts.

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
    return;
80107d16:	90                   	nop
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
    uartputc(*p);
}
80107d17:	c9                   	leave  
80107d18:	c3                   	ret    

80107d19 <uartputc>:

void
uartputc(int c)
{
80107d19:	55                   	push   %ebp
80107d1a:	89 e5                	mov    %esp,%ebp
80107d1c:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80107d1f:	a1 d4 c6 10 80       	mov    0x8010c6d4,%eax
80107d24:	85 c0                	test   %eax,%eax
80107d26:	74 4d                	je     80107d75 <uartputc+0x5c>
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80107d28:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107d2f:	eb 10                	jmp    80107d41 <uartputc+0x28>
    microdelay(10);
80107d31:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80107d38:	e8 cd b9 ff ff       	call   8010370a <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80107d3d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80107d41:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80107d45:	7f 16                	jg     80107d5d <uartputc+0x44>
80107d47:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80107d4e:	e8 6d fe ff ff       	call   80107bc0 <inb>
80107d53:	0f b6 c0             	movzbl %al,%eax
80107d56:	83 e0 20             	and    $0x20,%eax
80107d59:	85 c0                	test   %eax,%eax
80107d5b:	74 d4                	je     80107d31 <uartputc+0x18>
    microdelay(10);
  outb(COM1+0, c);
80107d5d:	8b 45 08             	mov    0x8(%ebp),%eax
80107d60:	0f b6 c0             	movzbl %al,%eax
80107d63:	89 44 24 04          	mov    %eax,0x4(%esp)
80107d67:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80107d6e:	e8 77 fe ff ff       	call   80107bea <outb>
80107d73:	eb 01                	jmp    80107d76 <uartputc+0x5d>
uartputc(int c)
{
  int i;

  if(!uart)
    return;
80107d75:	90                   	nop
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
    microdelay(10);
  outb(COM1+0, c);
}
80107d76:	c9                   	leave  
80107d77:	c3                   	ret    

80107d78 <uartgetc>:

static int
uartgetc(void)
{
80107d78:	55                   	push   %ebp
80107d79:	89 e5                	mov    %esp,%ebp
80107d7b:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80107d7e:	a1 d4 c6 10 80       	mov    0x8010c6d4,%eax
80107d83:	85 c0                	test   %eax,%eax
80107d85:	75 07                	jne    80107d8e <uartgetc+0x16>
    return -1;
80107d87:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107d8c:	eb 2c                	jmp    80107dba <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80107d8e:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80107d95:	e8 26 fe ff ff       	call   80107bc0 <inb>
80107d9a:	0f b6 c0             	movzbl %al,%eax
80107d9d:	83 e0 01             	and    $0x1,%eax
80107da0:	85 c0                	test   %eax,%eax
80107da2:	75 07                	jne    80107dab <uartgetc+0x33>
    return -1;
80107da4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107da9:	eb 0f                	jmp    80107dba <uartgetc+0x42>
  return inb(COM1+0);
80107dab:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80107db2:	e8 09 fe ff ff       	call   80107bc0 <inb>
80107db7:	0f b6 c0             	movzbl %al,%eax
}
80107dba:	c9                   	leave  
80107dbb:	c3                   	ret    

80107dbc <uartintr>:

void
uartintr(void)
{
80107dbc:	55                   	push   %ebp
80107dbd:	89 e5                	mov    %esp,%ebp
80107dbf:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80107dc2:	c7 04 24 78 7d 10 80 	movl   $0x80107d78,(%esp)
80107dc9:	e8 df 89 ff ff       	call   801007ad <consoleintr>
}
80107dce:	c9                   	leave  
80107dcf:	c3                   	ret    

80107dd0 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80107dd0:	6a 00                	push   $0x0
  pushl $0
80107dd2:	6a 00                	push   $0x0
  jmp alltraps
80107dd4:	e9 67 f9 ff ff       	jmp    80107740 <alltraps>

80107dd9 <vector1>:
.globl vector1
vector1:
  pushl $0
80107dd9:	6a 00                	push   $0x0
  pushl $1
80107ddb:	6a 01                	push   $0x1
  jmp alltraps
80107ddd:	e9 5e f9 ff ff       	jmp    80107740 <alltraps>

80107de2 <vector2>:
.globl vector2
vector2:
  pushl $0
80107de2:	6a 00                	push   $0x0
  pushl $2
80107de4:	6a 02                	push   $0x2
  jmp alltraps
80107de6:	e9 55 f9 ff ff       	jmp    80107740 <alltraps>

80107deb <vector3>:
.globl vector3
vector3:
  pushl $0
80107deb:	6a 00                	push   $0x0
  pushl $3
80107ded:	6a 03                	push   $0x3
  jmp alltraps
80107def:	e9 4c f9 ff ff       	jmp    80107740 <alltraps>

80107df4 <vector4>:
.globl vector4
vector4:
  pushl $0
80107df4:	6a 00                	push   $0x0
  pushl $4
80107df6:	6a 04                	push   $0x4
  jmp alltraps
80107df8:	e9 43 f9 ff ff       	jmp    80107740 <alltraps>

80107dfd <vector5>:
.globl vector5
vector5:
  pushl $0
80107dfd:	6a 00                	push   $0x0
  pushl $5
80107dff:	6a 05                	push   $0x5
  jmp alltraps
80107e01:	e9 3a f9 ff ff       	jmp    80107740 <alltraps>

80107e06 <vector6>:
.globl vector6
vector6:
  pushl $0
80107e06:	6a 00                	push   $0x0
  pushl $6
80107e08:	6a 06                	push   $0x6
  jmp alltraps
80107e0a:	e9 31 f9 ff ff       	jmp    80107740 <alltraps>

80107e0f <vector7>:
.globl vector7
vector7:
  pushl $0
80107e0f:	6a 00                	push   $0x0
  pushl $7
80107e11:	6a 07                	push   $0x7
  jmp alltraps
80107e13:	e9 28 f9 ff ff       	jmp    80107740 <alltraps>

80107e18 <vector8>:
.globl vector8
vector8:
  pushl $8
80107e18:	6a 08                	push   $0x8
  jmp alltraps
80107e1a:	e9 21 f9 ff ff       	jmp    80107740 <alltraps>

80107e1f <vector9>:
.globl vector9
vector9:
  pushl $0
80107e1f:	6a 00                	push   $0x0
  pushl $9
80107e21:	6a 09                	push   $0x9
  jmp alltraps
80107e23:	e9 18 f9 ff ff       	jmp    80107740 <alltraps>

80107e28 <vector10>:
.globl vector10
vector10:
  pushl $10
80107e28:	6a 0a                	push   $0xa
  jmp alltraps
80107e2a:	e9 11 f9 ff ff       	jmp    80107740 <alltraps>

80107e2f <vector11>:
.globl vector11
vector11:
  pushl $11
80107e2f:	6a 0b                	push   $0xb
  jmp alltraps
80107e31:	e9 0a f9 ff ff       	jmp    80107740 <alltraps>

80107e36 <vector12>:
.globl vector12
vector12:
  pushl $12
80107e36:	6a 0c                	push   $0xc
  jmp alltraps
80107e38:	e9 03 f9 ff ff       	jmp    80107740 <alltraps>

80107e3d <vector13>:
.globl vector13
vector13:
  pushl $13
80107e3d:	6a 0d                	push   $0xd
  jmp alltraps
80107e3f:	e9 fc f8 ff ff       	jmp    80107740 <alltraps>

80107e44 <vector14>:
.globl vector14
vector14:
  pushl $14
80107e44:	6a 0e                	push   $0xe
  jmp alltraps
80107e46:	e9 f5 f8 ff ff       	jmp    80107740 <alltraps>

80107e4b <vector15>:
.globl vector15
vector15:
  pushl $0
80107e4b:	6a 00                	push   $0x0
  pushl $15
80107e4d:	6a 0f                	push   $0xf
  jmp alltraps
80107e4f:	e9 ec f8 ff ff       	jmp    80107740 <alltraps>

80107e54 <vector16>:
.globl vector16
vector16:
  pushl $0
80107e54:	6a 00                	push   $0x0
  pushl $16
80107e56:	6a 10                	push   $0x10
  jmp alltraps
80107e58:	e9 e3 f8 ff ff       	jmp    80107740 <alltraps>

80107e5d <vector17>:
.globl vector17
vector17:
  pushl $17
80107e5d:	6a 11                	push   $0x11
  jmp alltraps
80107e5f:	e9 dc f8 ff ff       	jmp    80107740 <alltraps>

80107e64 <vector18>:
.globl vector18
vector18:
  pushl $0
80107e64:	6a 00                	push   $0x0
  pushl $18
80107e66:	6a 12                	push   $0x12
  jmp alltraps
80107e68:	e9 d3 f8 ff ff       	jmp    80107740 <alltraps>

80107e6d <vector19>:
.globl vector19
vector19:
  pushl $0
80107e6d:	6a 00                	push   $0x0
  pushl $19
80107e6f:	6a 13                	push   $0x13
  jmp alltraps
80107e71:	e9 ca f8 ff ff       	jmp    80107740 <alltraps>

80107e76 <vector20>:
.globl vector20
vector20:
  pushl $0
80107e76:	6a 00                	push   $0x0
  pushl $20
80107e78:	6a 14                	push   $0x14
  jmp alltraps
80107e7a:	e9 c1 f8 ff ff       	jmp    80107740 <alltraps>

80107e7f <vector21>:
.globl vector21
vector21:
  pushl $0
80107e7f:	6a 00                	push   $0x0
  pushl $21
80107e81:	6a 15                	push   $0x15
  jmp alltraps
80107e83:	e9 b8 f8 ff ff       	jmp    80107740 <alltraps>

80107e88 <vector22>:
.globl vector22
vector22:
  pushl $0
80107e88:	6a 00                	push   $0x0
  pushl $22
80107e8a:	6a 16                	push   $0x16
  jmp alltraps
80107e8c:	e9 af f8 ff ff       	jmp    80107740 <alltraps>

80107e91 <vector23>:
.globl vector23
vector23:
  pushl $0
80107e91:	6a 00                	push   $0x0
  pushl $23
80107e93:	6a 17                	push   $0x17
  jmp alltraps
80107e95:	e9 a6 f8 ff ff       	jmp    80107740 <alltraps>

80107e9a <vector24>:
.globl vector24
vector24:
  pushl $0
80107e9a:	6a 00                	push   $0x0
  pushl $24
80107e9c:	6a 18                	push   $0x18
  jmp alltraps
80107e9e:	e9 9d f8 ff ff       	jmp    80107740 <alltraps>

80107ea3 <vector25>:
.globl vector25
vector25:
  pushl $0
80107ea3:	6a 00                	push   $0x0
  pushl $25
80107ea5:	6a 19                	push   $0x19
  jmp alltraps
80107ea7:	e9 94 f8 ff ff       	jmp    80107740 <alltraps>

80107eac <vector26>:
.globl vector26
vector26:
  pushl $0
80107eac:	6a 00                	push   $0x0
  pushl $26
80107eae:	6a 1a                	push   $0x1a
  jmp alltraps
80107eb0:	e9 8b f8 ff ff       	jmp    80107740 <alltraps>

80107eb5 <vector27>:
.globl vector27
vector27:
  pushl $0
80107eb5:	6a 00                	push   $0x0
  pushl $27
80107eb7:	6a 1b                	push   $0x1b
  jmp alltraps
80107eb9:	e9 82 f8 ff ff       	jmp    80107740 <alltraps>

80107ebe <vector28>:
.globl vector28
vector28:
  pushl $0
80107ebe:	6a 00                	push   $0x0
  pushl $28
80107ec0:	6a 1c                	push   $0x1c
  jmp alltraps
80107ec2:	e9 79 f8 ff ff       	jmp    80107740 <alltraps>

80107ec7 <vector29>:
.globl vector29
vector29:
  pushl $0
80107ec7:	6a 00                	push   $0x0
  pushl $29
80107ec9:	6a 1d                	push   $0x1d
  jmp alltraps
80107ecb:	e9 70 f8 ff ff       	jmp    80107740 <alltraps>

80107ed0 <vector30>:
.globl vector30
vector30:
  pushl $0
80107ed0:	6a 00                	push   $0x0
  pushl $30
80107ed2:	6a 1e                	push   $0x1e
  jmp alltraps
80107ed4:	e9 67 f8 ff ff       	jmp    80107740 <alltraps>

80107ed9 <vector31>:
.globl vector31
vector31:
  pushl $0
80107ed9:	6a 00                	push   $0x0
  pushl $31
80107edb:	6a 1f                	push   $0x1f
  jmp alltraps
80107edd:	e9 5e f8 ff ff       	jmp    80107740 <alltraps>

80107ee2 <vector32>:
.globl vector32
vector32:
  pushl $0
80107ee2:	6a 00                	push   $0x0
  pushl $32
80107ee4:	6a 20                	push   $0x20
  jmp alltraps
80107ee6:	e9 55 f8 ff ff       	jmp    80107740 <alltraps>

80107eeb <vector33>:
.globl vector33
vector33:
  pushl $0
80107eeb:	6a 00                	push   $0x0
  pushl $33
80107eed:	6a 21                	push   $0x21
  jmp alltraps
80107eef:	e9 4c f8 ff ff       	jmp    80107740 <alltraps>

80107ef4 <vector34>:
.globl vector34
vector34:
  pushl $0
80107ef4:	6a 00                	push   $0x0
  pushl $34
80107ef6:	6a 22                	push   $0x22
  jmp alltraps
80107ef8:	e9 43 f8 ff ff       	jmp    80107740 <alltraps>

80107efd <vector35>:
.globl vector35
vector35:
  pushl $0
80107efd:	6a 00                	push   $0x0
  pushl $35
80107eff:	6a 23                	push   $0x23
  jmp alltraps
80107f01:	e9 3a f8 ff ff       	jmp    80107740 <alltraps>

80107f06 <vector36>:
.globl vector36
vector36:
  pushl $0
80107f06:	6a 00                	push   $0x0
  pushl $36
80107f08:	6a 24                	push   $0x24
  jmp alltraps
80107f0a:	e9 31 f8 ff ff       	jmp    80107740 <alltraps>

80107f0f <vector37>:
.globl vector37
vector37:
  pushl $0
80107f0f:	6a 00                	push   $0x0
  pushl $37
80107f11:	6a 25                	push   $0x25
  jmp alltraps
80107f13:	e9 28 f8 ff ff       	jmp    80107740 <alltraps>

80107f18 <vector38>:
.globl vector38
vector38:
  pushl $0
80107f18:	6a 00                	push   $0x0
  pushl $38
80107f1a:	6a 26                	push   $0x26
  jmp alltraps
80107f1c:	e9 1f f8 ff ff       	jmp    80107740 <alltraps>

80107f21 <vector39>:
.globl vector39
vector39:
  pushl $0
80107f21:	6a 00                	push   $0x0
  pushl $39
80107f23:	6a 27                	push   $0x27
  jmp alltraps
80107f25:	e9 16 f8 ff ff       	jmp    80107740 <alltraps>

80107f2a <vector40>:
.globl vector40
vector40:
  pushl $0
80107f2a:	6a 00                	push   $0x0
  pushl $40
80107f2c:	6a 28                	push   $0x28
  jmp alltraps
80107f2e:	e9 0d f8 ff ff       	jmp    80107740 <alltraps>

80107f33 <vector41>:
.globl vector41
vector41:
  pushl $0
80107f33:	6a 00                	push   $0x0
  pushl $41
80107f35:	6a 29                	push   $0x29
  jmp alltraps
80107f37:	e9 04 f8 ff ff       	jmp    80107740 <alltraps>

80107f3c <vector42>:
.globl vector42
vector42:
  pushl $0
80107f3c:	6a 00                	push   $0x0
  pushl $42
80107f3e:	6a 2a                	push   $0x2a
  jmp alltraps
80107f40:	e9 fb f7 ff ff       	jmp    80107740 <alltraps>

80107f45 <vector43>:
.globl vector43
vector43:
  pushl $0
80107f45:	6a 00                	push   $0x0
  pushl $43
80107f47:	6a 2b                	push   $0x2b
  jmp alltraps
80107f49:	e9 f2 f7 ff ff       	jmp    80107740 <alltraps>

80107f4e <vector44>:
.globl vector44
vector44:
  pushl $0
80107f4e:	6a 00                	push   $0x0
  pushl $44
80107f50:	6a 2c                	push   $0x2c
  jmp alltraps
80107f52:	e9 e9 f7 ff ff       	jmp    80107740 <alltraps>

80107f57 <vector45>:
.globl vector45
vector45:
  pushl $0
80107f57:	6a 00                	push   $0x0
  pushl $45
80107f59:	6a 2d                	push   $0x2d
  jmp alltraps
80107f5b:	e9 e0 f7 ff ff       	jmp    80107740 <alltraps>

80107f60 <vector46>:
.globl vector46
vector46:
  pushl $0
80107f60:	6a 00                	push   $0x0
  pushl $46
80107f62:	6a 2e                	push   $0x2e
  jmp alltraps
80107f64:	e9 d7 f7 ff ff       	jmp    80107740 <alltraps>

80107f69 <vector47>:
.globl vector47
vector47:
  pushl $0
80107f69:	6a 00                	push   $0x0
  pushl $47
80107f6b:	6a 2f                	push   $0x2f
  jmp alltraps
80107f6d:	e9 ce f7 ff ff       	jmp    80107740 <alltraps>

80107f72 <vector48>:
.globl vector48
vector48:
  pushl $0
80107f72:	6a 00                	push   $0x0
  pushl $48
80107f74:	6a 30                	push   $0x30
  jmp alltraps
80107f76:	e9 c5 f7 ff ff       	jmp    80107740 <alltraps>

80107f7b <vector49>:
.globl vector49
vector49:
  pushl $0
80107f7b:	6a 00                	push   $0x0
  pushl $49
80107f7d:	6a 31                	push   $0x31
  jmp alltraps
80107f7f:	e9 bc f7 ff ff       	jmp    80107740 <alltraps>

80107f84 <vector50>:
.globl vector50
vector50:
  pushl $0
80107f84:	6a 00                	push   $0x0
  pushl $50
80107f86:	6a 32                	push   $0x32
  jmp alltraps
80107f88:	e9 b3 f7 ff ff       	jmp    80107740 <alltraps>

80107f8d <vector51>:
.globl vector51
vector51:
  pushl $0
80107f8d:	6a 00                	push   $0x0
  pushl $51
80107f8f:	6a 33                	push   $0x33
  jmp alltraps
80107f91:	e9 aa f7 ff ff       	jmp    80107740 <alltraps>

80107f96 <vector52>:
.globl vector52
vector52:
  pushl $0
80107f96:	6a 00                	push   $0x0
  pushl $52
80107f98:	6a 34                	push   $0x34
  jmp alltraps
80107f9a:	e9 a1 f7 ff ff       	jmp    80107740 <alltraps>

80107f9f <vector53>:
.globl vector53
vector53:
  pushl $0
80107f9f:	6a 00                	push   $0x0
  pushl $53
80107fa1:	6a 35                	push   $0x35
  jmp alltraps
80107fa3:	e9 98 f7 ff ff       	jmp    80107740 <alltraps>

80107fa8 <vector54>:
.globl vector54
vector54:
  pushl $0
80107fa8:	6a 00                	push   $0x0
  pushl $54
80107faa:	6a 36                	push   $0x36
  jmp alltraps
80107fac:	e9 8f f7 ff ff       	jmp    80107740 <alltraps>

80107fb1 <vector55>:
.globl vector55
vector55:
  pushl $0
80107fb1:	6a 00                	push   $0x0
  pushl $55
80107fb3:	6a 37                	push   $0x37
  jmp alltraps
80107fb5:	e9 86 f7 ff ff       	jmp    80107740 <alltraps>

80107fba <vector56>:
.globl vector56
vector56:
  pushl $0
80107fba:	6a 00                	push   $0x0
  pushl $56
80107fbc:	6a 38                	push   $0x38
  jmp alltraps
80107fbe:	e9 7d f7 ff ff       	jmp    80107740 <alltraps>

80107fc3 <vector57>:
.globl vector57
vector57:
  pushl $0
80107fc3:	6a 00                	push   $0x0
  pushl $57
80107fc5:	6a 39                	push   $0x39
  jmp alltraps
80107fc7:	e9 74 f7 ff ff       	jmp    80107740 <alltraps>

80107fcc <vector58>:
.globl vector58
vector58:
  pushl $0
80107fcc:	6a 00                	push   $0x0
  pushl $58
80107fce:	6a 3a                	push   $0x3a
  jmp alltraps
80107fd0:	e9 6b f7 ff ff       	jmp    80107740 <alltraps>

80107fd5 <vector59>:
.globl vector59
vector59:
  pushl $0
80107fd5:	6a 00                	push   $0x0
  pushl $59
80107fd7:	6a 3b                	push   $0x3b
  jmp alltraps
80107fd9:	e9 62 f7 ff ff       	jmp    80107740 <alltraps>

80107fde <vector60>:
.globl vector60
vector60:
  pushl $0
80107fde:	6a 00                	push   $0x0
  pushl $60
80107fe0:	6a 3c                	push   $0x3c
  jmp alltraps
80107fe2:	e9 59 f7 ff ff       	jmp    80107740 <alltraps>

80107fe7 <vector61>:
.globl vector61
vector61:
  pushl $0
80107fe7:	6a 00                	push   $0x0
  pushl $61
80107fe9:	6a 3d                	push   $0x3d
  jmp alltraps
80107feb:	e9 50 f7 ff ff       	jmp    80107740 <alltraps>

80107ff0 <vector62>:
.globl vector62
vector62:
  pushl $0
80107ff0:	6a 00                	push   $0x0
  pushl $62
80107ff2:	6a 3e                	push   $0x3e
  jmp alltraps
80107ff4:	e9 47 f7 ff ff       	jmp    80107740 <alltraps>

80107ff9 <vector63>:
.globl vector63
vector63:
  pushl $0
80107ff9:	6a 00                	push   $0x0
  pushl $63
80107ffb:	6a 3f                	push   $0x3f
  jmp alltraps
80107ffd:	e9 3e f7 ff ff       	jmp    80107740 <alltraps>

80108002 <vector64>:
.globl vector64
vector64:
  pushl $0
80108002:	6a 00                	push   $0x0
  pushl $64
80108004:	6a 40                	push   $0x40
  jmp alltraps
80108006:	e9 35 f7 ff ff       	jmp    80107740 <alltraps>

8010800b <vector65>:
.globl vector65
vector65:
  pushl $0
8010800b:	6a 00                	push   $0x0
  pushl $65
8010800d:	6a 41                	push   $0x41
  jmp alltraps
8010800f:	e9 2c f7 ff ff       	jmp    80107740 <alltraps>

80108014 <vector66>:
.globl vector66
vector66:
  pushl $0
80108014:	6a 00                	push   $0x0
  pushl $66
80108016:	6a 42                	push   $0x42
  jmp alltraps
80108018:	e9 23 f7 ff ff       	jmp    80107740 <alltraps>

8010801d <vector67>:
.globl vector67
vector67:
  pushl $0
8010801d:	6a 00                	push   $0x0
  pushl $67
8010801f:	6a 43                	push   $0x43
  jmp alltraps
80108021:	e9 1a f7 ff ff       	jmp    80107740 <alltraps>

80108026 <vector68>:
.globl vector68
vector68:
  pushl $0
80108026:	6a 00                	push   $0x0
  pushl $68
80108028:	6a 44                	push   $0x44
  jmp alltraps
8010802a:	e9 11 f7 ff ff       	jmp    80107740 <alltraps>

8010802f <vector69>:
.globl vector69
vector69:
  pushl $0
8010802f:	6a 00                	push   $0x0
  pushl $69
80108031:	6a 45                	push   $0x45
  jmp alltraps
80108033:	e9 08 f7 ff ff       	jmp    80107740 <alltraps>

80108038 <vector70>:
.globl vector70
vector70:
  pushl $0
80108038:	6a 00                	push   $0x0
  pushl $70
8010803a:	6a 46                	push   $0x46
  jmp alltraps
8010803c:	e9 ff f6 ff ff       	jmp    80107740 <alltraps>

80108041 <vector71>:
.globl vector71
vector71:
  pushl $0
80108041:	6a 00                	push   $0x0
  pushl $71
80108043:	6a 47                	push   $0x47
  jmp alltraps
80108045:	e9 f6 f6 ff ff       	jmp    80107740 <alltraps>

8010804a <vector72>:
.globl vector72
vector72:
  pushl $0
8010804a:	6a 00                	push   $0x0
  pushl $72
8010804c:	6a 48                	push   $0x48
  jmp alltraps
8010804e:	e9 ed f6 ff ff       	jmp    80107740 <alltraps>

80108053 <vector73>:
.globl vector73
vector73:
  pushl $0
80108053:	6a 00                	push   $0x0
  pushl $73
80108055:	6a 49                	push   $0x49
  jmp alltraps
80108057:	e9 e4 f6 ff ff       	jmp    80107740 <alltraps>

8010805c <vector74>:
.globl vector74
vector74:
  pushl $0
8010805c:	6a 00                	push   $0x0
  pushl $74
8010805e:	6a 4a                	push   $0x4a
  jmp alltraps
80108060:	e9 db f6 ff ff       	jmp    80107740 <alltraps>

80108065 <vector75>:
.globl vector75
vector75:
  pushl $0
80108065:	6a 00                	push   $0x0
  pushl $75
80108067:	6a 4b                	push   $0x4b
  jmp alltraps
80108069:	e9 d2 f6 ff ff       	jmp    80107740 <alltraps>

8010806e <vector76>:
.globl vector76
vector76:
  pushl $0
8010806e:	6a 00                	push   $0x0
  pushl $76
80108070:	6a 4c                	push   $0x4c
  jmp alltraps
80108072:	e9 c9 f6 ff ff       	jmp    80107740 <alltraps>

80108077 <vector77>:
.globl vector77
vector77:
  pushl $0
80108077:	6a 00                	push   $0x0
  pushl $77
80108079:	6a 4d                	push   $0x4d
  jmp alltraps
8010807b:	e9 c0 f6 ff ff       	jmp    80107740 <alltraps>

80108080 <vector78>:
.globl vector78
vector78:
  pushl $0
80108080:	6a 00                	push   $0x0
  pushl $78
80108082:	6a 4e                	push   $0x4e
  jmp alltraps
80108084:	e9 b7 f6 ff ff       	jmp    80107740 <alltraps>

80108089 <vector79>:
.globl vector79
vector79:
  pushl $0
80108089:	6a 00                	push   $0x0
  pushl $79
8010808b:	6a 4f                	push   $0x4f
  jmp alltraps
8010808d:	e9 ae f6 ff ff       	jmp    80107740 <alltraps>

80108092 <vector80>:
.globl vector80
vector80:
  pushl $0
80108092:	6a 00                	push   $0x0
  pushl $80
80108094:	6a 50                	push   $0x50
  jmp alltraps
80108096:	e9 a5 f6 ff ff       	jmp    80107740 <alltraps>

8010809b <vector81>:
.globl vector81
vector81:
  pushl $0
8010809b:	6a 00                	push   $0x0
  pushl $81
8010809d:	6a 51                	push   $0x51
  jmp alltraps
8010809f:	e9 9c f6 ff ff       	jmp    80107740 <alltraps>

801080a4 <vector82>:
.globl vector82
vector82:
  pushl $0
801080a4:	6a 00                	push   $0x0
  pushl $82
801080a6:	6a 52                	push   $0x52
  jmp alltraps
801080a8:	e9 93 f6 ff ff       	jmp    80107740 <alltraps>

801080ad <vector83>:
.globl vector83
vector83:
  pushl $0
801080ad:	6a 00                	push   $0x0
  pushl $83
801080af:	6a 53                	push   $0x53
  jmp alltraps
801080b1:	e9 8a f6 ff ff       	jmp    80107740 <alltraps>

801080b6 <vector84>:
.globl vector84
vector84:
  pushl $0
801080b6:	6a 00                	push   $0x0
  pushl $84
801080b8:	6a 54                	push   $0x54
  jmp alltraps
801080ba:	e9 81 f6 ff ff       	jmp    80107740 <alltraps>

801080bf <vector85>:
.globl vector85
vector85:
  pushl $0
801080bf:	6a 00                	push   $0x0
  pushl $85
801080c1:	6a 55                	push   $0x55
  jmp alltraps
801080c3:	e9 78 f6 ff ff       	jmp    80107740 <alltraps>

801080c8 <vector86>:
.globl vector86
vector86:
  pushl $0
801080c8:	6a 00                	push   $0x0
  pushl $86
801080ca:	6a 56                	push   $0x56
  jmp alltraps
801080cc:	e9 6f f6 ff ff       	jmp    80107740 <alltraps>

801080d1 <vector87>:
.globl vector87
vector87:
  pushl $0
801080d1:	6a 00                	push   $0x0
  pushl $87
801080d3:	6a 57                	push   $0x57
  jmp alltraps
801080d5:	e9 66 f6 ff ff       	jmp    80107740 <alltraps>

801080da <vector88>:
.globl vector88
vector88:
  pushl $0
801080da:	6a 00                	push   $0x0
  pushl $88
801080dc:	6a 58                	push   $0x58
  jmp alltraps
801080de:	e9 5d f6 ff ff       	jmp    80107740 <alltraps>

801080e3 <vector89>:
.globl vector89
vector89:
  pushl $0
801080e3:	6a 00                	push   $0x0
  pushl $89
801080e5:	6a 59                	push   $0x59
  jmp alltraps
801080e7:	e9 54 f6 ff ff       	jmp    80107740 <alltraps>

801080ec <vector90>:
.globl vector90
vector90:
  pushl $0
801080ec:	6a 00                	push   $0x0
  pushl $90
801080ee:	6a 5a                	push   $0x5a
  jmp alltraps
801080f0:	e9 4b f6 ff ff       	jmp    80107740 <alltraps>

801080f5 <vector91>:
.globl vector91
vector91:
  pushl $0
801080f5:	6a 00                	push   $0x0
  pushl $91
801080f7:	6a 5b                	push   $0x5b
  jmp alltraps
801080f9:	e9 42 f6 ff ff       	jmp    80107740 <alltraps>

801080fe <vector92>:
.globl vector92
vector92:
  pushl $0
801080fe:	6a 00                	push   $0x0
  pushl $92
80108100:	6a 5c                	push   $0x5c
  jmp alltraps
80108102:	e9 39 f6 ff ff       	jmp    80107740 <alltraps>

80108107 <vector93>:
.globl vector93
vector93:
  pushl $0
80108107:	6a 00                	push   $0x0
  pushl $93
80108109:	6a 5d                	push   $0x5d
  jmp alltraps
8010810b:	e9 30 f6 ff ff       	jmp    80107740 <alltraps>

80108110 <vector94>:
.globl vector94
vector94:
  pushl $0
80108110:	6a 00                	push   $0x0
  pushl $94
80108112:	6a 5e                	push   $0x5e
  jmp alltraps
80108114:	e9 27 f6 ff ff       	jmp    80107740 <alltraps>

80108119 <vector95>:
.globl vector95
vector95:
  pushl $0
80108119:	6a 00                	push   $0x0
  pushl $95
8010811b:	6a 5f                	push   $0x5f
  jmp alltraps
8010811d:	e9 1e f6 ff ff       	jmp    80107740 <alltraps>

80108122 <vector96>:
.globl vector96
vector96:
  pushl $0
80108122:	6a 00                	push   $0x0
  pushl $96
80108124:	6a 60                	push   $0x60
  jmp alltraps
80108126:	e9 15 f6 ff ff       	jmp    80107740 <alltraps>

8010812b <vector97>:
.globl vector97
vector97:
  pushl $0
8010812b:	6a 00                	push   $0x0
  pushl $97
8010812d:	6a 61                	push   $0x61
  jmp alltraps
8010812f:	e9 0c f6 ff ff       	jmp    80107740 <alltraps>

80108134 <vector98>:
.globl vector98
vector98:
  pushl $0
80108134:	6a 00                	push   $0x0
  pushl $98
80108136:	6a 62                	push   $0x62
  jmp alltraps
80108138:	e9 03 f6 ff ff       	jmp    80107740 <alltraps>

8010813d <vector99>:
.globl vector99
vector99:
  pushl $0
8010813d:	6a 00                	push   $0x0
  pushl $99
8010813f:	6a 63                	push   $0x63
  jmp alltraps
80108141:	e9 fa f5 ff ff       	jmp    80107740 <alltraps>

80108146 <vector100>:
.globl vector100
vector100:
  pushl $0
80108146:	6a 00                	push   $0x0
  pushl $100
80108148:	6a 64                	push   $0x64
  jmp alltraps
8010814a:	e9 f1 f5 ff ff       	jmp    80107740 <alltraps>

8010814f <vector101>:
.globl vector101
vector101:
  pushl $0
8010814f:	6a 00                	push   $0x0
  pushl $101
80108151:	6a 65                	push   $0x65
  jmp alltraps
80108153:	e9 e8 f5 ff ff       	jmp    80107740 <alltraps>

80108158 <vector102>:
.globl vector102
vector102:
  pushl $0
80108158:	6a 00                	push   $0x0
  pushl $102
8010815a:	6a 66                	push   $0x66
  jmp alltraps
8010815c:	e9 df f5 ff ff       	jmp    80107740 <alltraps>

80108161 <vector103>:
.globl vector103
vector103:
  pushl $0
80108161:	6a 00                	push   $0x0
  pushl $103
80108163:	6a 67                	push   $0x67
  jmp alltraps
80108165:	e9 d6 f5 ff ff       	jmp    80107740 <alltraps>

8010816a <vector104>:
.globl vector104
vector104:
  pushl $0
8010816a:	6a 00                	push   $0x0
  pushl $104
8010816c:	6a 68                	push   $0x68
  jmp alltraps
8010816e:	e9 cd f5 ff ff       	jmp    80107740 <alltraps>

80108173 <vector105>:
.globl vector105
vector105:
  pushl $0
80108173:	6a 00                	push   $0x0
  pushl $105
80108175:	6a 69                	push   $0x69
  jmp alltraps
80108177:	e9 c4 f5 ff ff       	jmp    80107740 <alltraps>

8010817c <vector106>:
.globl vector106
vector106:
  pushl $0
8010817c:	6a 00                	push   $0x0
  pushl $106
8010817e:	6a 6a                	push   $0x6a
  jmp alltraps
80108180:	e9 bb f5 ff ff       	jmp    80107740 <alltraps>

80108185 <vector107>:
.globl vector107
vector107:
  pushl $0
80108185:	6a 00                	push   $0x0
  pushl $107
80108187:	6a 6b                	push   $0x6b
  jmp alltraps
80108189:	e9 b2 f5 ff ff       	jmp    80107740 <alltraps>

8010818e <vector108>:
.globl vector108
vector108:
  pushl $0
8010818e:	6a 00                	push   $0x0
  pushl $108
80108190:	6a 6c                	push   $0x6c
  jmp alltraps
80108192:	e9 a9 f5 ff ff       	jmp    80107740 <alltraps>

80108197 <vector109>:
.globl vector109
vector109:
  pushl $0
80108197:	6a 00                	push   $0x0
  pushl $109
80108199:	6a 6d                	push   $0x6d
  jmp alltraps
8010819b:	e9 a0 f5 ff ff       	jmp    80107740 <alltraps>

801081a0 <vector110>:
.globl vector110
vector110:
  pushl $0
801081a0:	6a 00                	push   $0x0
  pushl $110
801081a2:	6a 6e                	push   $0x6e
  jmp alltraps
801081a4:	e9 97 f5 ff ff       	jmp    80107740 <alltraps>

801081a9 <vector111>:
.globl vector111
vector111:
  pushl $0
801081a9:	6a 00                	push   $0x0
  pushl $111
801081ab:	6a 6f                	push   $0x6f
  jmp alltraps
801081ad:	e9 8e f5 ff ff       	jmp    80107740 <alltraps>

801081b2 <vector112>:
.globl vector112
vector112:
  pushl $0
801081b2:	6a 00                	push   $0x0
  pushl $112
801081b4:	6a 70                	push   $0x70
  jmp alltraps
801081b6:	e9 85 f5 ff ff       	jmp    80107740 <alltraps>

801081bb <vector113>:
.globl vector113
vector113:
  pushl $0
801081bb:	6a 00                	push   $0x0
  pushl $113
801081bd:	6a 71                	push   $0x71
  jmp alltraps
801081bf:	e9 7c f5 ff ff       	jmp    80107740 <alltraps>

801081c4 <vector114>:
.globl vector114
vector114:
  pushl $0
801081c4:	6a 00                	push   $0x0
  pushl $114
801081c6:	6a 72                	push   $0x72
  jmp alltraps
801081c8:	e9 73 f5 ff ff       	jmp    80107740 <alltraps>

801081cd <vector115>:
.globl vector115
vector115:
  pushl $0
801081cd:	6a 00                	push   $0x0
  pushl $115
801081cf:	6a 73                	push   $0x73
  jmp alltraps
801081d1:	e9 6a f5 ff ff       	jmp    80107740 <alltraps>

801081d6 <vector116>:
.globl vector116
vector116:
  pushl $0
801081d6:	6a 00                	push   $0x0
  pushl $116
801081d8:	6a 74                	push   $0x74
  jmp alltraps
801081da:	e9 61 f5 ff ff       	jmp    80107740 <alltraps>

801081df <vector117>:
.globl vector117
vector117:
  pushl $0
801081df:	6a 00                	push   $0x0
  pushl $117
801081e1:	6a 75                	push   $0x75
  jmp alltraps
801081e3:	e9 58 f5 ff ff       	jmp    80107740 <alltraps>

801081e8 <vector118>:
.globl vector118
vector118:
  pushl $0
801081e8:	6a 00                	push   $0x0
  pushl $118
801081ea:	6a 76                	push   $0x76
  jmp alltraps
801081ec:	e9 4f f5 ff ff       	jmp    80107740 <alltraps>

801081f1 <vector119>:
.globl vector119
vector119:
  pushl $0
801081f1:	6a 00                	push   $0x0
  pushl $119
801081f3:	6a 77                	push   $0x77
  jmp alltraps
801081f5:	e9 46 f5 ff ff       	jmp    80107740 <alltraps>

801081fa <vector120>:
.globl vector120
vector120:
  pushl $0
801081fa:	6a 00                	push   $0x0
  pushl $120
801081fc:	6a 78                	push   $0x78
  jmp alltraps
801081fe:	e9 3d f5 ff ff       	jmp    80107740 <alltraps>

80108203 <vector121>:
.globl vector121
vector121:
  pushl $0
80108203:	6a 00                	push   $0x0
  pushl $121
80108205:	6a 79                	push   $0x79
  jmp alltraps
80108207:	e9 34 f5 ff ff       	jmp    80107740 <alltraps>

8010820c <vector122>:
.globl vector122
vector122:
  pushl $0
8010820c:	6a 00                	push   $0x0
  pushl $122
8010820e:	6a 7a                	push   $0x7a
  jmp alltraps
80108210:	e9 2b f5 ff ff       	jmp    80107740 <alltraps>

80108215 <vector123>:
.globl vector123
vector123:
  pushl $0
80108215:	6a 00                	push   $0x0
  pushl $123
80108217:	6a 7b                	push   $0x7b
  jmp alltraps
80108219:	e9 22 f5 ff ff       	jmp    80107740 <alltraps>

8010821e <vector124>:
.globl vector124
vector124:
  pushl $0
8010821e:	6a 00                	push   $0x0
  pushl $124
80108220:	6a 7c                	push   $0x7c
  jmp alltraps
80108222:	e9 19 f5 ff ff       	jmp    80107740 <alltraps>

80108227 <vector125>:
.globl vector125
vector125:
  pushl $0
80108227:	6a 00                	push   $0x0
  pushl $125
80108229:	6a 7d                	push   $0x7d
  jmp alltraps
8010822b:	e9 10 f5 ff ff       	jmp    80107740 <alltraps>

80108230 <vector126>:
.globl vector126
vector126:
  pushl $0
80108230:	6a 00                	push   $0x0
  pushl $126
80108232:	6a 7e                	push   $0x7e
  jmp alltraps
80108234:	e9 07 f5 ff ff       	jmp    80107740 <alltraps>

80108239 <vector127>:
.globl vector127
vector127:
  pushl $0
80108239:	6a 00                	push   $0x0
  pushl $127
8010823b:	6a 7f                	push   $0x7f
  jmp alltraps
8010823d:	e9 fe f4 ff ff       	jmp    80107740 <alltraps>

80108242 <vector128>:
.globl vector128
vector128:
  pushl $0
80108242:	6a 00                	push   $0x0
  pushl $128
80108244:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80108249:	e9 f2 f4 ff ff       	jmp    80107740 <alltraps>

8010824e <vector129>:
.globl vector129
vector129:
  pushl $0
8010824e:	6a 00                	push   $0x0
  pushl $129
80108250:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80108255:	e9 e6 f4 ff ff       	jmp    80107740 <alltraps>

8010825a <vector130>:
.globl vector130
vector130:
  pushl $0
8010825a:	6a 00                	push   $0x0
  pushl $130
8010825c:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80108261:	e9 da f4 ff ff       	jmp    80107740 <alltraps>

80108266 <vector131>:
.globl vector131
vector131:
  pushl $0
80108266:	6a 00                	push   $0x0
  pushl $131
80108268:	68 83 00 00 00       	push   $0x83
  jmp alltraps
8010826d:	e9 ce f4 ff ff       	jmp    80107740 <alltraps>

80108272 <vector132>:
.globl vector132
vector132:
  pushl $0
80108272:	6a 00                	push   $0x0
  pushl $132
80108274:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80108279:	e9 c2 f4 ff ff       	jmp    80107740 <alltraps>

8010827e <vector133>:
.globl vector133
vector133:
  pushl $0
8010827e:	6a 00                	push   $0x0
  pushl $133
80108280:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80108285:	e9 b6 f4 ff ff       	jmp    80107740 <alltraps>

8010828a <vector134>:
.globl vector134
vector134:
  pushl $0
8010828a:	6a 00                	push   $0x0
  pushl $134
8010828c:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80108291:	e9 aa f4 ff ff       	jmp    80107740 <alltraps>

80108296 <vector135>:
.globl vector135
vector135:
  pushl $0
80108296:	6a 00                	push   $0x0
  pushl $135
80108298:	68 87 00 00 00       	push   $0x87
  jmp alltraps
8010829d:	e9 9e f4 ff ff       	jmp    80107740 <alltraps>

801082a2 <vector136>:
.globl vector136
vector136:
  pushl $0
801082a2:	6a 00                	push   $0x0
  pushl $136
801082a4:	68 88 00 00 00       	push   $0x88
  jmp alltraps
801082a9:	e9 92 f4 ff ff       	jmp    80107740 <alltraps>

801082ae <vector137>:
.globl vector137
vector137:
  pushl $0
801082ae:	6a 00                	push   $0x0
  pushl $137
801082b0:	68 89 00 00 00       	push   $0x89
  jmp alltraps
801082b5:	e9 86 f4 ff ff       	jmp    80107740 <alltraps>

801082ba <vector138>:
.globl vector138
vector138:
  pushl $0
801082ba:	6a 00                	push   $0x0
  pushl $138
801082bc:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
801082c1:	e9 7a f4 ff ff       	jmp    80107740 <alltraps>

801082c6 <vector139>:
.globl vector139
vector139:
  pushl $0
801082c6:	6a 00                	push   $0x0
  pushl $139
801082c8:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
801082cd:	e9 6e f4 ff ff       	jmp    80107740 <alltraps>

801082d2 <vector140>:
.globl vector140
vector140:
  pushl $0
801082d2:	6a 00                	push   $0x0
  pushl $140
801082d4:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
801082d9:	e9 62 f4 ff ff       	jmp    80107740 <alltraps>

801082de <vector141>:
.globl vector141
vector141:
  pushl $0
801082de:	6a 00                	push   $0x0
  pushl $141
801082e0:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
801082e5:	e9 56 f4 ff ff       	jmp    80107740 <alltraps>

801082ea <vector142>:
.globl vector142
vector142:
  pushl $0
801082ea:	6a 00                	push   $0x0
  pushl $142
801082ec:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
801082f1:	e9 4a f4 ff ff       	jmp    80107740 <alltraps>

801082f6 <vector143>:
.globl vector143
vector143:
  pushl $0
801082f6:	6a 00                	push   $0x0
  pushl $143
801082f8:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
801082fd:	e9 3e f4 ff ff       	jmp    80107740 <alltraps>

80108302 <vector144>:
.globl vector144
vector144:
  pushl $0
80108302:	6a 00                	push   $0x0
  pushl $144
80108304:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80108309:	e9 32 f4 ff ff       	jmp    80107740 <alltraps>

8010830e <vector145>:
.globl vector145
vector145:
  pushl $0
8010830e:	6a 00                	push   $0x0
  pushl $145
80108310:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80108315:	e9 26 f4 ff ff       	jmp    80107740 <alltraps>

8010831a <vector146>:
.globl vector146
vector146:
  pushl $0
8010831a:	6a 00                	push   $0x0
  pushl $146
8010831c:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80108321:	e9 1a f4 ff ff       	jmp    80107740 <alltraps>

80108326 <vector147>:
.globl vector147
vector147:
  pushl $0
80108326:	6a 00                	push   $0x0
  pushl $147
80108328:	68 93 00 00 00       	push   $0x93
  jmp alltraps
8010832d:	e9 0e f4 ff ff       	jmp    80107740 <alltraps>

80108332 <vector148>:
.globl vector148
vector148:
  pushl $0
80108332:	6a 00                	push   $0x0
  pushl $148
80108334:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80108339:	e9 02 f4 ff ff       	jmp    80107740 <alltraps>

8010833e <vector149>:
.globl vector149
vector149:
  pushl $0
8010833e:	6a 00                	push   $0x0
  pushl $149
80108340:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80108345:	e9 f6 f3 ff ff       	jmp    80107740 <alltraps>

8010834a <vector150>:
.globl vector150
vector150:
  pushl $0
8010834a:	6a 00                	push   $0x0
  pushl $150
8010834c:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80108351:	e9 ea f3 ff ff       	jmp    80107740 <alltraps>

80108356 <vector151>:
.globl vector151
vector151:
  pushl $0
80108356:	6a 00                	push   $0x0
  pushl $151
80108358:	68 97 00 00 00       	push   $0x97
  jmp alltraps
8010835d:	e9 de f3 ff ff       	jmp    80107740 <alltraps>

80108362 <vector152>:
.globl vector152
vector152:
  pushl $0
80108362:	6a 00                	push   $0x0
  pushl $152
80108364:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80108369:	e9 d2 f3 ff ff       	jmp    80107740 <alltraps>

8010836e <vector153>:
.globl vector153
vector153:
  pushl $0
8010836e:	6a 00                	push   $0x0
  pushl $153
80108370:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80108375:	e9 c6 f3 ff ff       	jmp    80107740 <alltraps>

8010837a <vector154>:
.globl vector154
vector154:
  pushl $0
8010837a:	6a 00                	push   $0x0
  pushl $154
8010837c:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80108381:	e9 ba f3 ff ff       	jmp    80107740 <alltraps>

80108386 <vector155>:
.globl vector155
vector155:
  pushl $0
80108386:	6a 00                	push   $0x0
  pushl $155
80108388:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
8010838d:	e9 ae f3 ff ff       	jmp    80107740 <alltraps>

80108392 <vector156>:
.globl vector156
vector156:
  pushl $0
80108392:	6a 00                	push   $0x0
  pushl $156
80108394:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80108399:	e9 a2 f3 ff ff       	jmp    80107740 <alltraps>

8010839e <vector157>:
.globl vector157
vector157:
  pushl $0
8010839e:	6a 00                	push   $0x0
  pushl $157
801083a0:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
801083a5:	e9 96 f3 ff ff       	jmp    80107740 <alltraps>

801083aa <vector158>:
.globl vector158
vector158:
  pushl $0
801083aa:	6a 00                	push   $0x0
  pushl $158
801083ac:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
801083b1:	e9 8a f3 ff ff       	jmp    80107740 <alltraps>

801083b6 <vector159>:
.globl vector159
vector159:
  pushl $0
801083b6:	6a 00                	push   $0x0
  pushl $159
801083b8:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
801083bd:	e9 7e f3 ff ff       	jmp    80107740 <alltraps>

801083c2 <vector160>:
.globl vector160
vector160:
  pushl $0
801083c2:	6a 00                	push   $0x0
  pushl $160
801083c4:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
801083c9:	e9 72 f3 ff ff       	jmp    80107740 <alltraps>

801083ce <vector161>:
.globl vector161
vector161:
  pushl $0
801083ce:	6a 00                	push   $0x0
  pushl $161
801083d0:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
801083d5:	e9 66 f3 ff ff       	jmp    80107740 <alltraps>

801083da <vector162>:
.globl vector162
vector162:
  pushl $0
801083da:	6a 00                	push   $0x0
  pushl $162
801083dc:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
801083e1:	e9 5a f3 ff ff       	jmp    80107740 <alltraps>

801083e6 <vector163>:
.globl vector163
vector163:
  pushl $0
801083e6:	6a 00                	push   $0x0
  pushl $163
801083e8:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
801083ed:	e9 4e f3 ff ff       	jmp    80107740 <alltraps>

801083f2 <vector164>:
.globl vector164
vector164:
  pushl $0
801083f2:	6a 00                	push   $0x0
  pushl $164
801083f4:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
801083f9:	e9 42 f3 ff ff       	jmp    80107740 <alltraps>

801083fe <vector165>:
.globl vector165
vector165:
  pushl $0
801083fe:	6a 00                	push   $0x0
  pushl $165
80108400:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80108405:	e9 36 f3 ff ff       	jmp    80107740 <alltraps>

8010840a <vector166>:
.globl vector166
vector166:
  pushl $0
8010840a:	6a 00                	push   $0x0
  pushl $166
8010840c:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80108411:	e9 2a f3 ff ff       	jmp    80107740 <alltraps>

80108416 <vector167>:
.globl vector167
vector167:
  pushl $0
80108416:	6a 00                	push   $0x0
  pushl $167
80108418:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
8010841d:	e9 1e f3 ff ff       	jmp    80107740 <alltraps>

80108422 <vector168>:
.globl vector168
vector168:
  pushl $0
80108422:	6a 00                	push   $0x0
  pushl $168
80108424:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80108429:	e9 12 f3 ff ff       	jmp    80107740 <alltraps>

8010842e <vector169>:
.globl vector169
vector169:
  pushl $0
8010842e:	6a 00                	push   $0x0
  pushl $169
80108430:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80108435:	e9 06 f3 ff ff       	jmp    80107740 <alltraps>

8010843a <vector170>:
.globl vector170
vector170:
  pushl $0
8010843a:	6a 00                	push   $0x0
  pushl $170
8010843c:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80108441:	e9 fa f2 ff ff       	jmp    80107740 <alltraps>

80108446 <vector171>:
.globl vector171
vector171:
  pushl $0
80108446:	6a 00                	push   $0x0
  pushl $171
80108448:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
8010844d:	e9 ee f2 ff ff       	jmp    80107740 <alltraps>

80108452 <vector172>:
.globl vector172
vector172:
  pushl $0
80108452:	6a 00                	push   $0x0
  pushl $172
80108454:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
80108459:	e9 e2 f2 ff ff       	jmp    80107740 <alltraps>

8010845e <vector173>:
.globl vector173
vector173:
  pushl $0
8010845e:	6a 00                	push   $0x0
  pushl $173
80108460:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
80108465:	e9 d6 f2 ff ff       	jmp    80107740 <alltraps>

8010846a <vector174>:
.globl vector174
vector174:
  pushl $0
8010846a:	6a 00                	push   $0x0
  pushl $174
8010846c:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80108471:	e9 ca f2 ff ff       	jmp    80107740 <alltraps>

80108476 <vector175>:
.globl vector175
vector175:
  pushl $0
80108476:	6a 00                	push   $0x0
  pushl $175
80108478:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
8010847d:	e9 be f2 ff ff       	jmp    80107740 <alltraps>

80108482 <vector176>:
.globl vector176
vector176:
  pushl $0
80108482:	6a 00                	push   $0x0
  pushl $176
80108484:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
80108489:	e9 b2 f2 ff ff       	jmp    80107740 <alltraps>

8010848e <vector177>:
.globl vector177
vector177:
  pushl $0
8010848e:	6a 00                	push   $0x0
  pushl $177
80108490:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
80108495:	e9 a6 f2 ff ff       	jmp    80107740 <alltraps>

8010849a <vector178>:
.globl vector178
vector178:
  pushl $0
8010849a:	6a 00                	push   $0x0
  pushl $178
8010849c:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
801084a1:	e9 9a f2 ff ff       	jmp    80107740 <alltraps>

801084a6 <vector179>:
.globl vector179
vector179:
  pushl $0
801084a6:	6a 00                	push   $0x0
  pushl $179
801084a8:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
801084ad:	e9 8e f2 ff ff       	jmp    80107740 <alltraps>

801084b2 <vector180>:
.globl vector180
vector180:
  pushl $0
801084b2:	6a 00                	push   $0x0
  pushl $180
801084b4:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
801084b9:	e9 82 f2 ff ff       	jmp    80107740 <alltraps>

801084be <vector181>:
.globl vector181
vector181:
  pushl $0
801084be:	6a 00                	push   $0x0
  pushl $181
801084c0:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
801084c5:	e9 76 f2 ff ff       	jmp    80107740 <alltraps>

801084ca <vector182>:
.globl vector182
vector182:
  pushl $0
801084ca:	6a 00                	push   $0x0
  pushl $182
801084cc:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
801084d1:	e9 6a f2 ff ff       	jmp    80107740 <alltraps>

801084d6 <vector183>:
.globl vector183
vector183:
  pushl $0
801084d6:	6a 00                	push   $0x0
  pushl $183
801084d8:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
801084dd:	e9 5e f2 ff ff       	jmp    80107740 <alltraps>

801084e2 <vector184>:
.globl vector184
vector184:
  pushl $0
801084e2:	6a 00                	push   $0x0
  pushl $184
801084e4:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
801084e9:	e9 52 f2 ff ff       	jmp    80107740 <alltraps>

801084ee <vector185>:
.globl vector185
vector185:
  pushl $0
801084ee:	6a 00                	push   $0x0
  pushl $185
801084f0:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
801084f5:	e9 46 f2 ff ff       	jmp    80107740 <alltraps>

801084fa <vector186>:
.globl vector186
vector186:
  pushl $0
801084fa:	6a 00                	push   $0x0
  pushl $186
801084fc:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
80108501:	e9 3a f2 ff ff       	jmp    80107740 <alltraps>

80108506 <vector187>:
.globl vector187
vector187:
  pushl $0
80108506:	6a 00                	push   $0x0
  pushl $187
80108508:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
8010850d:	e9 2e f2 ff ff       	jmp    80107740 <alltraps>

80108512 <vector188>:
.globl vector188
vector188:
  pushl $0
80108512:	6a 00                	push   $0x0
  pushl $188
80108514:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80108519:	e9 22 f2 ff ff       	jmp    80107740 <alltraps>

8010851e <vector189>:
.globl vector189
vector189:
  pushl $0
8010851e:	6a 00                	push   $0x0
  pushl $189
80108520:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80108525:	e9 16 f2 ff ff       	jmp    80107740 <alltraps>

8010852a <vector190>:
.globl vector190
vector190:
  pushl $0
8010852a:	6a 00                	push   $0x0
  pushl $190
8010852c:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80108531:	e9 0a f2 ff ff       	jmp    80107740 <alltraps>

80108536 <vector191>:
.globl vector191
vector191:
  pushl $0
80108536:	6a 00                	push   $0x0
  pushl $191
80108538:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
8010853d:	e9 fe f1 ff ff       	jmp    80107740 <alltraps>

80108542 <vector192>:
.globl vector192
vector192:
  pushl $0
80108542:	6a 00                	push   $0x0
  pushl $192
80108544:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
80108549:	e9 f2 f1 ff ff       	jmp    80107740 <alltraps>

8010854e <vector193>:
.globl vector193
vector193:
  pushl $0
8010854e:	6a 00                	push   $0x0
  pushl $193
80108550:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
80108555:	e9 e6 f1 ff ff       	jmp    80107740 <alltraps>

8010855a <vector194>:
.globl vector194
vector194:
  pushl $0
8010855a:	6a 00                	push   $0x0
  pushl $194
8010855c:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80108561:	e9 da f1 ff ff       	jmp    80107740 <alltraps>

80108566 <vector195>:
.globl vector195
vector195:
  pushl $0
80108566:	6a 00                	push   $0x0
  pushl $195
80108568:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
8010856d:	e9 ce f1 ff ff       	jmp    80107740 <alltraps>

80108572 <vector196>:
.globl vector196
vector196:
  pushl $0
80108572:	6a 00                	push   $0x0
  pushl $196
80108574:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
80108579:	e9 c2 f1 ff ff       	jmp    80107740 <alltraps>

8010857e <vector197>:
.globl vector197
vector197:
  pushl $0
8010857e:	6a 00                	push   $0x0
  pushl $197
80108580:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
80108585:	e9 b6 f1 ff ff       	jmp    80107740 <alltraps>

8010858a <vector198>:
.globl vector198
vector198:
  pushl $0
8010858a:	6a 00                	push   $0x0
  pushl $198
8010858c:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80108591:	e9 aa f1 ff ff       	jmp    80107740 <alltraps>

80108596 <vector199>:
.globl vector199
vector199:
  pushl $0
80108596:	6a 00                	push   $0x0
  pushl $199
80108598:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
8010859d:	e9 9e f1 ff ff       	jmp    80107740 <alltraps>

801085a2 <vector200>:
.globl vector200
vector200:
  pushl $0
801085a2:	6a 00                	push   $0x0
  pushl $200
801085a4:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
801085a9:	e9 92 f1 ff ff       	jmp    80107740 <alltraps>

801085ae <vector201>:
.globl vector201
vector201:
  pushl $0
801085ae:	6a 00                	push   $0x0
  pushl $201
801085b0:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
801085b5:	e9 86 f1 ff ff       	jmp    80107740 <alltraps>

801085ba <vector202>:
.globl vector202
vector202:
  pushl $0
801085ba:	6a 00                	push   $0x0
  pushl $202
801085bc:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
801085c1:	e9 7a f1 ff ff       	jmp    80107740 <alltraps>

801085c6 <vector203>:
.globl vector203
vector203:
  pushl $0
801085c6:	6a 00                	push   $0x0
  pushl $203
801085c8:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
801085cd:	e9 6e f1 ff ff       	jmp    80107740 <alltraps>

801085d2 <vector204>:
.globl vector204
vector204:
  pushl $0
801085d2:	6a 00                	push   $0x0
  pushl $204
801085d4:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
801085d9:	e9 62 f1 ff ff       	jmp    80107740 <alltraps>

801085de <vector205>:
.globl vector205
vector205:
  pushl $0
801085de:	6a 00                	push   $0x0
  pushl $205
801085e0:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
801085e5:	e9 56 f1 ff ff       	jmp    80107740 <alltraps>

801085ea <vector206>:
.globl vector206
vector206:
  pushl $0
801085ea:	6a 00                	push   $0x0
  pushl $206
801085ec:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
801085f1:	e9 4a f1 ff ff       	jmp    80107740 <alltraps>

801085f6 <vector207>:
.globl vector207
vector207:
  pushl $0
801085f6:	6a 00                	push   $0x0
  pushl $207
801085f8:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
801085fd:	e9 3e f1 ff ff       	jmp    80107740 <alltraps>

80108602 <vector208>:
.globl vector208
vector208:
  pushl $0
80108602:	6a 00                	push   $0x0
  pushl $208
80108604:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80108609:	e9 32 f1 ff ff       	jmp    80107740 <alltraps>

8010860e <vector209>:
.globl vector209
vector209:
  pushl $0
8010860e:	6a 00                	push   $0x0
  pushl $209
80108610:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
80108615:	e9 26 f1 ff ff       	jmp    80107740 <alltraps>

8010861a <vector210>:
.globl vector210
vector210:
  pushl $0
8010861a:	6a 00                	push   $0x0
  pushl $210
8010861c:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
80108621:	e9 1a f1 ff ff       	jmp    80107740 <alltraps>

80108626 <vector211>:
.globl vector211
vector211:
  pushl $0
80108626:	6a 00                	push   $0x0
  pushl $211
80108628:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
8010862d:	e9 0e f1 ff ff       	jmp    80107740 <alltraps>

80108632 <vector212>:
.globl vector212
vector212:
  pushl $0
80108632:	6a 00                	push   $0x0
  pushl $212
80108634:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
80108639:	e9 02 f1 ff ff       	jmp    80107740 <alltraps>

8010863e <vector213>:
.globl vector213
vector213:
  pushl $0
8010863e:	6a 00                	push   $0x0
  pushl $213
80108640:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
80108645:	e9 f6 f0 ff ff       	jmp    80107740 <alltraps>

8010864a <vector214>:
.globl vector214
vector214:
  pushl $0
8010864a:	6a 00                	push   $0x0
  pushl $214
8010864c:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
80108651:	e9 ea f0 ff ff       	jmp    80107740 <alltraps>

80108656 <vector215>:
.globl vector215
vector215:
  pushl $0
80108656:	6a 00                	push   $0x0
  pushl $215
80108658:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
8010865d:	e9 de f0 ff ff       	jmp    80107740 <alltraps>

80108662 <vector216>:
.globl vector216
vector216:
  pushl $0
80108662:	6a 00                	push   $0x0
  pushl $216
80108664:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
80108669:	e9 d2 f0 ff ff       	jmp    80107740 <alltraps>

8010866e <vector217>:
.globl vector217
vector217:
  pushl $0
8010866e:	6a 00                	push   $0x0
  pushl $217
80108670:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
80108675:	e9 c6 f0 ff ff       	jmp    80107740 <alltraps>

8010867a <vector218>:
.globl vector218
vector218:
  pushl $0
8010867a:	6a 00                	push   $0x0
  pushl $218
8010867c:	68 da 00 00 00       	push   $0xda
  jmp alltraps
80108681:	e9 ba f0 ff ff       	jmp    80107740 <alltraps>

80108686 <vector219>:
.globl vector219
vector219:
  pushl $0
80108686:	6a 00                	push   $0x0
  pushl $219
80108688:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
8010868d:	e9 ae f0 ff ff       	jmp    80107740 <alltraps>

80108692 <vector220>:
.globl vector220
vector220:
  pushl $0
80108692:	6a 00                	push   $0x0
  pushl $220
80108694:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
80108699:	e9 a2 f0 ff ff       	jmp    80107740 <alltraps>

8010869e <vector221>:
.globl vector221
vector221:
  pushl $0
8010869e:	6a 00                	push   $0x0
  pushl $221
801086a0:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
801086a5:	e9 96 f0 ff ff       	jmp    80107740 <alltraps>

801086aa <vector222>:
.globl vector222
vector222:
  pushl $0
801086aa:	6a 00                	push   $0x0
  pushl $222
801086ac:	68 de 00 00 00       	push   $0xde
  jmp alltraps
801086b1:	e9 8a f0 ff ff       	jmp    80107740 <alltraps>

801086b6 <vector223>:
.globl vector223
vector223:
  pushl $0
801086b6:	6a 00                	push   $0x0
  pushl $223
801086b8:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
801086bd:	e9 7e f0 ff ff       	jmp    80107740 <alltraps>

801086c2 <vector224>:
.globl vector224
vector224:
  pushl $0
801086c2:	6a 00                	push   $0x0
  pushl $224
801086c4:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
801086c9:	e9 72 f0 ff ff       	jmp    80107740 <alltraps>

801086ce <vector225>:
.globl vector225
vector225:
  pushl $0
801086ce:	6a 00                	push   $0x0
  pushl $225
801086d0:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
801086d5:	e9 66 f0 ff ff       	jmp    80107740 <alltraps>

801086da <vector226>:
.globl vector226
vector226:
  pushl $0
801086da:	6a 00                	push   $0x0
  pushl $226
801086dc:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
801086e1:	e9 5a f0 ff ff       	jmp    80107740 <alltraps>

801086e6 <vector227>:
.globl vector227
vector227:
  pushl $0
801086e6:	6a 00                	push   $0x0
  pushl $227
801086e8:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
801086ed:	e9 4e f0 ff ff       	jmp    80107740 <alltraps>

801086f2 <vector228>:
.globl vector228
vector228:
  pushl $0
801086f2:	6a 00                	push   $0x0
  pushl $228
801086f4:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
801086f9:	e9 42 f0 ff ff       	jmp    80107740 <alltraps>

801086fe <vector229>:
.globl vector229
vector229:
  pushl $0
801086fe:	6a 00                	push   $0x0
  pushl $229
80108700:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
80108705:	e9 36 f0 ff ff       	jmp    80107740 <alltraps>

8010870a <vector230>:
.globl vector230
vector230:
  pushl $0
8010870a:	6a 00                	push   $0x0
  pushl $230
8010870c:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
80108711:	e9 2a f0 ff ff       	jmp    80107740 <alltraps>

80108716 <vector231>:
.globl vector231
vector231:
  pushl $0
80108716:	6a 00                	push   $0x0
  pushl $231
80108718:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
8010871d:	e9 1e f0 ff ff       	jmp    80107740 <alltraps>

80108722 <vector232>:
.globl vector232
vector232:
  pushl $0
80108722:	6a 00                	push   $0x0
  pushl $232
80108724:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
80108729:	e9 12 f0 ff ff       	jmp    80107740 <alltraps>

8010872e <vector233>:
.globl vector233
vector233:
  pushl $0
8010872e:	6a 00                	push   $0x0
  pushl $233
80108730:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
80108735:	e9 06 f0 ff ff       	jmp    80107740 <alltraps>

8010873a <vector234>:
.globl vector234
vector234:
  pushl $0
8010873a:	6a 00                	push   $0x0
  pushl $234
8010873c:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
80108741:	e9 fa ef ff ff       	jmp    80107740 <alltraps>

80108746 <vector235>:
.globl vector235
vector235:
  pushl $0
80108746:	6a 00                	push   $0x0
  pushl $235
80108748:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
8010874d:	e9 ee ef ff ff       	jmp    80107740 <alltraps>

80108752 <vector236>:
.globl vector236
vector236:
  pushl $0
80108752:	6a 00                	push   $0x0
  pushl $236
80108754:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
80108759:	e9 e2 ef ff ff       	jmp    80107740 <alltraps>

8010875e <vector237>:
.globl vector237
vector237:
  pushl $0
8010875e:	6a 00                	push   $0x0
  pushl $237
80108760:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
80108765:	e9 d6 ef ff ff       	jmp    80107740 <alltraps>

8010876a <vector238>:
.globl vector238
vector238:
  pushl $0
8010876a:	6a 00                	push   $0x0
  pushl $238
8010876c:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
80108771:	e9 ca ef ff ff       	jmp    80107740 <alltraps>

80108776 <vector239>:
.globl vector239
vector239:
  pushl $0
80108776:	6a 00                	push   $0x0
  pushl $239
80108778:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
8010877d:	e9 be ef ff ff       	jmp    80107740 <alltraps>

80108782 <vector240>:
.globl vector240
vector240:
  pushl $0
80108782:	6a 00                	push   $0x0
  pushl $240
80108784:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
80108789:	e9 b2 ef ff ff       	jmp    80107740 <alltraps>

8010878e <vector241>:
.globl vector241
vector241:
  pushl $0
8010878e:	6a 00                	push   $0x0
  pushl $241
80108790:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
80108795:	e9 a6 ef ff ff       	jmp    80107740 <alltraps>

8010879a <vector242>:
.globl vector242
vector242:
  pushl $0
8010879a:	6a 00                	push   $0x0
  pushl $242
8010879c:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
801087a1:	e9 9a ef ff ff       	jmp    80107740 <alltraps>

801087a6 <vector243>:
.globl vector243
vector243:
  pushl $0
801087a6:	6a 00                	push   $0x0
  pushl $243
801087a8:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
801087ad:	e9 8e ef ff ff       	jmp    80107740 <alltraps>

801087b2 <vector244>:
.globl vector244
vector244:
  pushl $0
801087b2:	6a 00                	push   $0x0
  pushl $244
801087b4:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
801087b9:	e9 82 ef ff ff       	jmp    80107740 <alltraps>

801087be <vector245>:
.globl vector245
vector245:
  pushl $0
801087be:	6a 00                	push   $0x0
  pushl $245
801087c0:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
801087c5:	e9 76 ef ff ff       	jmp    80107740 <alltraps>

801087ca <vector246>:
.globl vector246
vector246:
  pushl $0
801087ca:	6a 00                	push   $0x0
  pushl $246
801087cc:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
801087d1:	e9 6a ef ff ff       	jmp    80107740 <alltraps>

801087d6 <vector247>:
.globl vector247
vector247:
  pushl $0
801087d6:	6a 00                	push   $0x0
  pushl $247
801087d8:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
801087dd:	e9 5e ef ff ff       	jmp    80107740 <alltraps>

801087e2 <vector248>:
.globl vector248
vector248:
  pushl $0
801087e2:	6a 00                	push   $0x0
  pushl $248
801087e4:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
801087e9:	e9 52 ef ff ff       	jmp    80107740 <alltraps>

801087ee <vector249>:
.globl vector249
vector249:
  pushl $0
801087ee:	6a 00                	push   $0x0
  pushl $249
801087f0:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
801087f5:	e9 46 ef ff ff       	jmp    80107740 <alltraps>

801087fa <vector250>:
.globl vector250
vector250:
  pushl $0
801087fa:	6a 00                	push   $0x0
  pushl $250
801087fc:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80108801:	e9 3a ef ff ff       	jmp    80107740 <alltraps>

80108806 <vector251>:
.globl vector251
vector251:
  pushl $0
80108806:	6a 00                	push   $0x0
  pushl $251
80108808:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
8010880d:	e9 2e ef ff ff       	jmp    80107740 <alltraps>

80108812 <vector252>:
.globl vector252
vector252:
  pushl $0
80108812:	6a 00                	push   $0x0
  pushl $252
80108814:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80108819:	e9 22 ef ff ff       	jmp    80107740 <alltraps>

8010881e <vector253>:
.globl vector253
vector253:
  pushl $0
8010881e:	6a 00                	push   $0x0
  pushl $253
80108820:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
80108825:	e9 16 ef ff ff       	jmp    80107740 <alltraps>

8010882a <vector254>:
.globl vector254
vector254:
  pushl $0
8010882a:	6a 00                	push   $0x0
  pushl $254
8010882c:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80108831:	e9 0a ef ff ff       	jmp    80107740 <alltraps>

80108836 <vector255>:
.globl vector255
vector255:
  pushl $0
80108836:	6a 00                	push   $0x0
  pushl $255
80108838:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
8010883d:	e9 fe ee ff ff       	jmp    80107740 <alltraps>
	...

80108844 <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80108844:	55                   	push   %ebp
80108845:	89 e5                	mov    %esp,%ebp
80108847:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
8010884a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010884d:	83 e8 01             	sub    $0x1,%eax
80108850:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80108854:	8b 45 08             	mov    0x8(%ebp),%eax
80108857:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
8010885b:	8b 45 08             	mov    0x8(%ebp),%eax
8010885e:	c1 e8 10             	shr    $0x10,%eax
80108861:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
80108865:	8d 45 fa             	lea    -0x6(%ebp),%eax
80108868:	0f 01 10             	lgdtl  (%eax)
}
8010886b:	c9                   	leave  
8010886c:	c3                   	ret    

8010886d <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
8010886d:	55                   	push   %ebp
8010886e:	89 e5                	mov    %esp,%ebp
80108870:	83 ec 04             	sub    $0x4,%esp
80108873:	8b 45 08             	mov    0x8(%ebp),%eax
80108876:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
8010887a:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
8010887e:	0f 00 d8             	ltr    %ax
}
80108881:	c9                   	leave  
80108882:	c3                   	ret    

80108883 <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
80108883:	55                   	push   %ebp
80108884:	89 e5                	mov    %esp,%ebp
80108886:	83 ec 04             	sub    $0x4,%esp
80108889:	8b 45 08             	mov    0x8(%ebp),%eax
8010888c:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
80108890:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80108894:	8e e8                	mov    %eax,%gs
}
80108896:	c9                   	leave  
80108897:	c3                   	ret    

80108898 <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
80108898:	55                   	push   %ebp
80108899:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
8010889b:	8b 45 08             	mov    0x8(%ebp),%eax
8010889e:	0f 22 d8             	mov    %eax,%cr3
}
801088a1:	5d                   	pop    %ebp
801088a2:	c3                   	ret    

801088a3 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801088a3:	55                   	push   %ebp
801088a4:	89 e5                	mov    %esp,%ebp
801088a6:	8b 45 08             	mov    0x8(%ebp),%eax
801088a9:	05 00 00 00 80       	add    $0x80000000,%eax
801088ae:	5d                   	pop    %ebp
801088af:	c3                   	ret    

801088b0 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801088b0:	55                   	push   %ebp
801088b1:	89 e5                	mov    %esp,%ebp
801088b3:	8b 45 08             	mov    0x8(%ebp),%eax
801088b6:	05 00 00 00 80       	add    $0x80000000,%eax
801088bb:	5d                   	pop    %ebp
801088bc:	c3                   	ret    

801088bd <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
801088bd:	55                   	push   %ebp
801088be:	89 e5                	mov    %esp,%ebp
801088c0:	53                   	push   %ebx
801088c1:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
801088c4:	e8 c0 ad ff ff       	call   80103689 <cpunum>
801088c9:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801088cf:	05 40 71 12 80       	add    $0x80127140,%eax
801088d4:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
801088d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801088da:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
801088e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801088e3:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
801088e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801088ec:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
801088f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801088f3:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801088f7:	83 e2 f0             	and    $0xfffffff0,%edx
801088fa:	83 ca 0a             	or     $0xa,%edx
801088fd:	88 50 7d             	mov    %dl,0x7d(%eax)
80108900:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108903:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80108907:	83 ca 10             	or     $0x10,%edx
8010890a:	88 50 7d             	mov    %dl,0x7d(%eax)
8010890d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108910:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80108914:	83 e2 9f             	and    $0xffffff9f,%edx
80108917:	88 50 7d             	mov    %dl,0x7d(%eax)
8010891a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010891d:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80108921:	83 ca 80             	or     $0xffffff80,%edx
80108924:	88 50 7d             	mov    %dl,0x7d(%eax)
80108927:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010892a:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
8010892e:	83 ca 0f             	or     $0xf,%edx
80108931:	88 50 7e             	mov    %dl,0x7e(%eax)
80108934:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108937:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
8010893b:	83 e2 ef             	and    $0xffffffef,%edx
8010893e:	88 50 7e             	mov    %dl,0x7e(%eax)
80108941:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108944:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80108948:	83 e2 df             	and    $0xffffffdf,%edx
8010894b:	88 50 7e             	mov    %dl,0x7e(%eax)
8010894e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108951:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80108955:	83 ca 40             	or     $0x40,%edx
80108958:	88 50 7e             	mov    %dl,0x7e(%eax)
8010895b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010895e:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80108962:	83 ca 80             	or     $0xffffff80,%edx
80108965:	88 50 7e             	mov    %dl,0x7e(%eax)
80108968:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010896b:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
8010896f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108972:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
80108979:	ff ff 
8010897b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010897e:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
80108985:	00 00 
80108987:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010898a:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80108991:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108994:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010899b:	83 e2 f0             	and    $0xfffffff0,%edx
8010899e:	83 ca 02             	or     $0x2,%edx
801089a1:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801089a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801089aa:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
801089b1:	83 ca 10             	or     $0x10,%edx
801089b4:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801089ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801089bd:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
801089c4:	83 e2 9f             	and    $0xffffff9f,%edx
801089c7:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801089cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801089d0:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
801089d7:	83 ca 80             	or     $0xffffff80,%edx
801089da:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801089e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801089e3:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801089ea:	83 ca 0f             	or     $0xf,%edx
801089ed:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801089f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801089f6:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801089fd:	83 e2 ef             	and    $0xffffffef,%edx
80108a00:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80108a06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a09:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80108a10:	83 e2 df             	and    $0xffffffdf,%edx
80108a13:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80108a19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a1c:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80108a23:	83 ca 40             	or     $0x40,%edx
80108a26:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80108a2c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a2f:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80108a36:	83 ca 80             	or     $0xffffff80,%edx
80108a39:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80108a3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a42:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80108a49:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a4c:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
80108a53:	ff ff 
80108a55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a58:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80108a5f:	00 00 
80108a61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a64:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80108a6b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a6e:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80108a75:	83 e2 f0             	and    $0xfffffff0,%edx
80108a78:	83 ca 0a             	or     $0xa,%edx
80108a7b:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80108a81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a84:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80108a8b:	83 ca 10             	or     $0x10,%edx
80108a8e:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80108a94:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108a97:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80108a9e:	83 ca 60             	or     $0x60,%edx
80108aa1:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80108aa7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108aaa:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80108ab1:	83 ca 80             	or     $0xffffff80,%edx
80108ab4:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80108aba:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108abd:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80108ac4:	83 ca 0f             	or     $0xf,%edx
80108ac7:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80108acd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ad0:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80108ad7:	83 e2 ef             	and    $0xffffffef,%edx
80108ada:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80108ae0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ae3:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80108aea:	83 e2 df             	and    $0xffffffdf,%edx
80108aed:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80108af3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108af6:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80108afd:	83 ca 40             	or     $0x40,%edx
80108b00:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80108b06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b09:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80108b10:	83 ca 80             	or     $0xffffff80,%edx
80108b13:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80108b19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b1c:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80108b23:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b26:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80108b2d:	ff ff 
80108b2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b32:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80108b39:	00 00 
80108b3b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b3e:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80108b45:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b48:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80108b4f:	83 e2 f0             	and    $0xfffffff0,%edx
80108b52:	83 ca 02             	or     $0x2,%edx
80108b55:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80108b5b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b5e:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80108b65:	83 ca 10             	or     $0x10,%edx
80108b68:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80108b6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b71:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80108b78:	83 ca 60             	or     $0x60,%edx
80108b7b:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80108b81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b84:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80108b8b:	83 ca 80             	or     $0xffffff80,%edx
80108b8e:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80108b94:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108b97:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80108b9e:	83 ca 0f             	or     $0xf,%edx
80108ba1:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80108ba7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108baa:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80108bb1:	83 e2 ef             	and    $0xffffffef,%edx
80108bb4:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80108bba:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108bbd:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80108bc4:	83 e2 df             	and    $0xffffffdf,%edx
80108bc7:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80108bcd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108bd0:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80108bd7:	83 ca 40             	or     $0x40,%edx
80108bda:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80108be0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108be3:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80108bea:	83 ca 80             	or     $0xffffff80,%edx
80108bed:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80108bf3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108bf6:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80108bfd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c00:	05 b4 00 00 00       	add    $0xb4,%eax
80108c05:	89 c3                	mov    %eax,%ebx
80108c07:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c0a:	05 b4 00 00 00       	add    $0xb4,%eax
80108c0f:	c1 e8 10             	shr    $0x10,%eax
80108c12:	89 c1                	mov    %eax,%ecx
80108c14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c17:	05 b4 00 00 00       	add    $0xb4,%eax
80108c1c:	c1 e8 18             	shr    $0x18,%eax
80108c1f:	89 c2                	mov    %eax,%edx
80108c21:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c24:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80108c2b:	00 00 
80108c2d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c30:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80108c37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c3a:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80108c40:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c43:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80108c4a:	83 e1 f0             	and    $0xfffffff0,%ecx
80108c4d:	83 c9 02             	or     $0x2,%ecx
80108c50:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80108c56:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c59:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80108c60:	83 c9 10             	or     $0x10,%ecx
80108c63:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80108c69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c6c:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80108c73:	83 e1 9f             	and    $0xffffff9f,%ecx
80108c76:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80108c7c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c7f:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80108c86:	83 c9 80             	or     $0xffffff80,%ecx
80108c89:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80108c8f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108c92:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80108c99:	83 e1 f0             	and    $0xfffffff0,%ecx
80108c9c:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80108ca2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ca5:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80108cac:	83 e1 ef             	and    $0xffffffef,%ecx
80108caf:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80108cb5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108cb8:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80108cbf:	83 e1 df             	and    $0xffffffdf,%ecx
80108cc2:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80108cc8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ccb:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80108cd2:	83 c9 40             	or     $0x40,%ecx
80108cd5:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80108cdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108cde:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80108ce5:	83 c9 80             	or     $0xffffff80,%ecx
80108ce8:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80108cee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108cf1:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80108cf7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108cfa:	83 c0 70             	add    $0x70,%eax
80108cfd:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80108d04:	00 
80108d05:	89 04 24             	mov    %eax,(%esp)
80108d08:	e8 37 fb ff ff       	call   80108844 <lgdt>
  loadgs(SEG_KCPU << 3);
80108d0d:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80108d14:	e8 6a fb ff ff       	call   80108883 <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80108d19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108d1c:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80108d22:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80108d29:	00 00 00 00 
}
80108d2d:	83 c4 24             	add    $0x24,%esp
80108d30:	5b                   	pop    %ebx
80108d31:	5d                   	pop    %ebp
80108d32:	c3                   	ret    

80108d33 <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80108d33:	55                   	push   %ebp
80108d34:	89 e5                	mov    %esp,%ebp
80108d36:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80108d39:	8b 45 0c             	mov    0xc(%ebp),%eax
80108d3c:	c1 e8 16             	shr    $0x16,%eax
80108d3f:	c1 e0 02             	shl    $0x2,%eax
80108d42:	03 45 08             	add    0x8(%ebp),%eax
80108d45:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80108d48:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108d4b:	8b 00                	mov    (%eax),%eax
80108d4d:	83 e0 01             	and    $0x1,%eax
80108d50:	84 c0                	test   %al,%al
80108d52:	74 17                	je     80108d6b <walkpgdir+0x38>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80108d54:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108d57:	8b 00                	mov    (%eax),%eax
80108d59:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108d5e:	89 04 24             	mov    %eax,(%esp)
80108d61:	e8 4a fb ff ff       	call   801088b0 <p2v>
80108d66:	89 45 f4             	mov    %eax,-0xc(%ebp)
80108d69:	eb 4b                	jmp    80108db6 <walkpgdir+0x83>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80108d6b:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80108d6f:	74 0e                	je     80108d7f <walkpgdir+0x4c>
80108d71:	e8 9a 9d ff ff       	call   80102b10 <kalloc>
80108d76:	89 45 f4             	mov    %eax,-0xc(%ebp)
80108d79:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80108d7d:	75 07                	jne    80108d86 <walkpgdir+0x53>
      return 0;
80108d7f:	b8 00 00 00 00       	mov    $0x0,%eax
80108d84:	eb 41                	jmp    80108dc7 <walkpgdir+0x94>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80108d86:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108d8d:	00 
80108d8e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108d95:	00 
80108d96:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108d99:	89 04 24             	mov    %eax,(%esp)
80108d9c:	e8 5d d0 ff ff       	call   80105dfe <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80108da1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108da4:	89 04 24             	mov    %eax,(%esp)
80108da7:	e8 f7 fa ff ff       	call   801088a3 <v2p>
80108dac:	89 c2                	mov    %eax,%edx
80108dae:	83 ca 07             	or     $0x7,%edx
80108db1:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108db4:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80108db6:	8b 45 0c             	mov    0xc(%ebp),%eax
80108db9:	c1 e8 0c             	shr    $0xc,%eax
80108dbc:	25 ff 03 00 00       	and    $0x3ff,%eax
80108dc1:	c1 e0 02             	shl    $0x2,%eax
80108dc4:	03 45 f4             	add    -0xc(%ebp),%eax
}
80108dc7:	c9                   	leave  
80108dc8:	c3                   	ret    

80108dc9 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80108dc9:	55                   	push   %ebp
80108dca:	89 e5                	mov    %esp,%ebp
80108dcc:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80108dcf:	8b 45 0c             	mov    0xc(%ebp),%eax
80108dd2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108dd7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  //cprintf("mappages: a = %p\n",a);
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80108dda:	8b 45 0c             	mov    0xc(%ebp),%eax
80108ddd:	03 45 10             	add    0x10(%ebp),%eax
80108de0:	83 e8 01             	sub    $0x1,%eax
80108de3:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108de8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80108deb:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80108df2:	00 
80108df3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108df6:	89 44 24 04          	mov    %eax,0x4(%esp)
80108dfa:	8b 45 08             	mov    0x8(%ebp),%eax
80108dfd:	89 04 24             	mov    %eax,(%esp)
80108e00:	e8 2e ff ff ff       	call   80108d33 <walkpgdir>
80108e05:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108e08:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108e0c:	75 07                	jne    80108e15 <mappages+0x4c>
      return -1;
80108e0e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80108e13:	eb 46                	jmp    80108e5b <mappages+0x92>
    if(*pte & PTE_P)
80108e15:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108e18:	8b 00                	mov    (%eax),%eax
80108e1a:	83 e0 01             	and    $0x1,%eax
80108e1d:	84 c0                	test   %al,%al
80108e1f:	74 0c                	je     80108e2d <mappages+0x64>
      panic("remap");
80108e21:	c7 04 24 a4 9d 10 80 	movl   $0x80109da4,(%esp)
80108e28:	e8 10 77 ff ff       	call   8010053d <panic>
    *pte = pa | perm | PTE_P;
80108e2d:	8b 45 18             	mov    0x18(%ebp),%eax
80108e30:	0b 45 14             	or     0x14(%ebp),%eax
80108e33:	89 c2                	mov    %eax,%edx
80108e35:	83 ca 01             	or     $0x1,%edx
80108e38:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108e3b:	89 10                	mov    %edx,(%eax)
   //cprintf("mappages: pte = %p\n",pte);
    if(a == last)
80108e3d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108e40:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80108e43:	74 10                	je     80108e55 <mappages+0x8c>
      break;
    a += PGSIZE;
80108e45:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80108e4c:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
80108e53:	eb 96                	jmp    80108deb <mappages+0x22>
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
   //cprintf("mappages: pte = %p\n",pte);
    if(a == last)
      break;
80108e55:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80108e56:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108e5b:	c9                   	leave  
80108e5c:	c3                   	ret    

80108e5d <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm()
{
80108e5d:	55                   	push   %ebp
80108e5e:	89 e5                	mov    %esp,%ebp
80108e60:	53                   	push   %ebx
80108e61:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
80108e64:	e8 a7 9c ff ff       	call   80102b10 <kalloc>
80108e69:	89 45 f0             	mov    %eax,-0x10(%ebp)
80108e6c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108e70:	75 0a                	jne    80108e7c <setupkvm+0x1f>
    return 0;
80108e72:	b8 00 00 00 00       	mov    $0x0,%eax
80108e77:	e9 98 00 00 00       	jmp    80108f14 <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
80108e7c:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108e83:	00 
80108e84:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108e8b:	00 
80108e8c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108e8f:	89 04 24             	mov    %eax,(%esp)
80108e92:	e8 67 cf ff ff       	call   80105dfe <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
80108e97:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80108e9e:	e8 0d fa ff ff       	call   801088b0 <p2v>
80108ea3:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
80108ea8:	76 0c                	jbe    80108eb6 <setupkvm+0x59>
    panic("PHYSTOP too high");
80108eaa:	c7 04 24 aa 9d 10 80 	movl   $0x80109daa,(%esp)
80108eb1:	e8 87 76 ff ff       	call   8010053d <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80108eb6:	c7 45 f4 c0 c4 10 80 	movl   $0x8010c4c0,-0xc(%ebp)
80108ebd:	eb 49                	jmp    80108f08 <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
80108ebf:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80108ec2:	8b 48 0c             	mov    0xc(%eax),%ecx
                (uint)k->phys_start, k->perm) < 0)
80108ec5:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80108ec8:	8b 50 04             	mov    0x4(%eax),%edx
80108ecb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ece:	8b 58 08             	mov    0x8(%eax),%ebx
80108ed1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108ed4:	8b 40 04             	mov    0x4(%eax),%eax
80108ed7:	29 c3                	sub    %eax,%ebx
80108ed9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108edc:	8b 00                	mov    (%eax),%eax
80108ede:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80108ee2:	89 54 24 0c          	mov    %edx,0xc(%esp)
80108ee6:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80108eea:	89 44 24 04          	mov    %eax,0x4(%esp)
80108eee:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108ef1:	89 04 24             	mov    %eax,(%esp)
80108ef4:	e8 d0 fe ff ff       	call   80108dc9 <mappages>
80108ef9:	85 c0                	test   %eax,%eax
80108efb:	79 07                	jns    80108f04 <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80108efd:	b8 00 00 00 00       	mov    $0x0,%eax
80108f02:	eb 10                	jmp    80108f14 <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80108f04:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80108f08:	81 7d f4 00 c5 10 80 	cmpl   $0x8010c500,-0xc(%ebp)
80108f0f:	72 ae                	jb     80108ebf <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
80108f11:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80108f14:	83 c4 34             	add    $0x34,%esp
80108f17:	5b                   	pop    %ebx
80108f18:	5d                   	pop    %ebp
80108f19:	c3                   	ret    

80108f1a <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80108f1a:	55                   	push   %ebp
80108f1b:	89 e5                	mov    %esp,%ebp
80108f1d:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80108f20:	e8 38 ff ff ff       	call   80108e5d <setupkvm>
80108f25:	a3 18 a4 12 80       	mov    %eax,0x8012a418
  switchkvm();
80108f2a:	e8 02 00 00 00       	call   80108f31 <switchkvm>
}
80108f2f:	c9                   	leave  
80108f30:	c3                   	ret    

80108f31 <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
80108f31:	55                   	push   %ebp
80108f32:	89 e5                	mov    %esp,%ebp
80108f34:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
80108f37:	a1 18 a4 12 80       	mov    0x8012a418,%eax
80108f3c:	89 04 24             	mov    %eax,(%esp)
80108f3f:	e8 5f f9 ff ff       	call   801088a3 <v2p>
80108f44:	89 04 24             	mov    %eax,(%esp)
80108f47:	e8 4c f9 ff ff       	call   80108898 <lcr3>
}
80108f4c:	c9                   	leave  
80108f4d:	c3                   	ret    

80108f4e <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
80108f4e:	55                   	push   %ebp
80108f4f:	89 e5                	mov    %esp,%ebp
80108f51:	53                   	push   %ebx
80108f52:	83 ec 14             	sub    $0x14,%esp
  pushcli();
80108f55:	e8 9e cd ff ff       	call   80105cf8 <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
80108f5a:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108f60:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108f67:	83 c2 08             	add    $0x8,%edx
80108f6a:	89 d3                	mov    %edx,%ebx
80108f6c:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108f73:	83 c2 08             	add    $0x8,%edx
80108f76:	c1 ea 10             	shr    $0x10,%edx
80108f79:	89 d1                	mov    %edx,%ecx
80108f7b:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108f82:	83 c2 08             	add    $0x8,%edx
80108f85:	c1 ea 18             	shr    $0x18,%edx
80108f88:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
80108f8f:	67 00 
80108f91:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
80108f98:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
80108f9e:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108fa5:	83 e1 f0             	and    $0xfffffff0,%ecx
80108fa8:	83 c9 09             	or     $0x9,%ecx
80108fab:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80108fb1:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108fb8:	83 c9 10             	or     $0x10,%ecx
80108fbb:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80108fc1:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108fc8:	83 e1 9f             	and    $0xffffff9f,%ecx
80108fcb:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80108fd1:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108fd8:	83 c9 80             	or     $0xffffff80,%ecx
80108fdb:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80108fe1:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108fe8:	83 e1 f0             	and    $0xfffffff0,%ecx
80108feb:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80108ff1:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108ff8:	83 e1 ef             	and    $0xffffffef,%ecx
80108ffb:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80109001:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80109008:	83 e1 df             	and    $0xffffffdf,%ecx
8010900b:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80109011:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80109018:	83 c9 40             	or     $0x40,%ecx
8010901b:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80109021:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80109028:	83 e1 7f             	and    $0x7f,%ecx
8010902b:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80109031:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
80109037:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010903d:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
80109044:	83 e2 ef             	and    $0xffffffef,%edx
80109047:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
8010904d:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80109053:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
80109059:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010905f:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80109066:	8b 52 08             	mov    0x8(%edx),%edx
80109069:	81 c2 00 10 00 00    	add    $0x1000,%edx
8010906f:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
80109072:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
80109079:	e8 ef f7 ff ff       	call   8010886d <ltr>
  if(p->pgdir == 0)
8010907e:	8b 45 08             	mov    0x8(%ebp),%eax
80109081:	8b 40 04             	mov    0x4(%eax),%eax
80109084:	85 c0                	test   %eax,%eax
80109086:	75 0c                	jne    80109094 <switchuvm+0x146>
    panic("switchuvm: no pgdir");
80109088:	c7 04 24 bb 9d 10 80 	movl   $0x80109dbb,(%esp)
8010908f:	e8 a9 74 ff ff       	call   8010053d <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
80109094:	8b 45 08             	mov    0x8(%ebp),%eax
80109097:	8b 40 04             	mov    0x4(%eax),%eax
8010909a:	89 04 24             	mov    %eax,(%esp)
8010909d:	e8 01 f8 ff ff       	call   801088a3 <v2p>
801090a2:	89 04 24             	mov    %eax,(%esp)
801090a5:	e8 ee f7 ff ff       	call   80108898 <lcr3>
  popcli();
801090aa:	e8 91 cc ff ff       	call   80105d40 <popcli>
}
801090af:	83 c4 14             	add    $0x14,%esp
801090b2:	5b                   	pop    %ebx
801090b3:	5d                   	pop    %ebp
801090b4:	c3                   	ret    

801090b5 <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
801090b5:	55                   	push   %ebp
801090b6:	89 e5                	mov    %esp,%ebp
801090b8:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
801090bb:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
801090c2:	76 0c                	jbe    801090d0 <inituvm+0x1b>
    panic("inituvm: more than a page");
801090c4:	c7 04 24 cf 9d 10 80 	movl   $0x80109dcf,(%esp)
801090cb:	e8 6d 74 ff ff       	call   8010053d <panic>
  mem = kalloc();
801090d0:	e8 3b 9a ff ff       	call   80102b10 <kalloc>
801090d5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
801090d8:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801090df:	00 
801090e0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801090e7:	00 
801090e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801090eb:	89 04 24             	mov    %eax,(%esp)
801090ee:	e8 0b cd ff ff       	call   80105dfe <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
801090f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801090f6:	89 04 24             	mov    %eax,(%esp)
801090f9:	e8 a5 f7 ff ff       	call   801088a3 <v2p>
801090fe:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80109105:	00 
80109106:	89 44 24 0c          	mov    %eax,0xc(%esp)
8010910a:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80109111:	00 
80109112:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80109119:	00 
8010911a:	8b 45 08             	mov    0x8(%ebp),%eax
8010911d:	89 04 24             	mov    %eax,(%esp)
80109120:	e8 a4 fc ff ff       	call   80108dc9 <mappages>
  memmove(mem, init, sz);
80109125:	8b 45 10             	mov    0x10(%ebp),%eax
80109128:	89 44 24 08          	mov    %eax,0x8(%esp)
8010912c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010912f:	89 44 24 04          	mov    %eax,0x4(%esp)
80109133:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109136:	89 04 24             	mov    %eax,(%esp)
80109139:	e8 93 cd ff ff       	call   80105ed1 <memmove>
}
8010913e:	c9                   	leave  
8010913f:	c3                   	ret    

80109140 <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
80109140:	55                   	push   %ebp
80109141:	89 e5                	mov    %esp,%ebp
80109143:	53                   	push   %ebx
80109144:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;
  if((uint) addr % PGSIZE != 0)
80109147:	8b 45 0c             	mov    0xc(%ebp),%eax
8010914a:	25 ff 0f 00 00       	and    $0xfff,%eax
8010914f:	85 c0                	test   %eax,%eax
80109151:	74 0c                	je     8010915f <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
80109153:	c7 04 24 ec 9d 10 80 	movl   $0x80109dec,(%esp)
8010915a:	e8 de 73 ff ff       	call   8010053d <panic>
  for(i = 0; i < sz; i += PGSIZE){
8010915f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80109166:	e9 ad 00 00 00       	jmp    80109218 <loaduvm+0xd8>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
8010916b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010916e:	8b 55 0c             	mov    0xc(%ebp),%edx
80109171:	01 d0                	add    %edx,%eax
80109173:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010917a:	00 
8010917b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010917f:	8b 45 08             	mov    0x8(%ebp),%eax
80109182:	89 04 24             	mov    %eax,(%esp)
80109185:	e8 a9 fb ff ff       	call   80108d33 <walkpgdir>
8010918a:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010918d:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80109191:	75 0c                	jne    8010919f <loaduvm+0x5f>
      panic("loaduvm: address should exist");
80109193:	c7 04 24 0f 9e 10 80 	movl   $0x80109e0f,(%esp)
8010919a:	e8 9e 73 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
8010919f:	8b 45 ec             	mov    -0x14(%ebp),%eax
801091a2:	8b 00                	mov    (%eax),%eax
801091a4:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801091a9:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
801091ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801091af:	8b 55 18             	mov    0x18(%ebp),%edx
801091b2:	89 d1                	mov    %edx,%ecx
801091b4:	29 c1                	sub    %eax,%ecx
801091b6:	89 c8                	mov    %ecx,%eax
801091b8:	3d ff 0f 00 00       	cmp    $0xfff,%eax
801091bd:	77 11                	ja     801091d0 <loaduvm+0x90>
      n = sz - i;
801091bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801091c2:	8b 55 18             	mov    0x18(%ebp),%edx
801091c5:	89 d1                	mov    %edx,%ecx
801091c7:	29 c1                	sub    %eax,%ecx
801091c9:	89 c8                	mov    %ecx,%eax
801091cb:	89 45 f0             	mov    %eax,-0x10(%ebp)
801091ce:	eb 07                	jmp    801091d7 <loaduvm+0x97>
    else
      n = PGSIZE;
801091d0:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
801091d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801091da:	8b 55 14             	mov    0x14(%ebp),%edx
801091dd:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
801091e0:	8b 45 e8             	mov    -0x18(%ebp),%eax
801091e3:	89 04 24             	mov    %eax,(%esp)
801091e6:	e8 c5 f6 ff ff       	call   801088b0 <p2v>
801091eb:	8b 55 f0             	mov    -0x10(%ebp),%edx
801091ee:	89 54 24 0c          	mov    %edx,0xc(%esp)
801091f2:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801091f6:	89 44 24 04          	mov    %eax,0x4(%esp)
801091fa:	8b 45 10             	mov    0x10(%ebp),%eax
801091fd:	89 04 24             	mov    %eax,(%esp)
80109200:	e8 59 8b ff ff       	call   80101d5e <readi>
80109205:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80109208:	74 07                	je     80109211 <loaduvm+0xd1>
      return -1;
8010920a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010920f:	eb 18                	jmp    80109229 <loaduvm+0xe9>
{
  uint i, pa, n;
  pte_t *pte;
  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
80109211:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80109218:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010921b:	3b 45 18             	cmp    0x18(%ebp),%eax
8010921e:	0f 82 47 ff ff ff    	jb     8010916b <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
80109224:	b8 00 00 00 00       	mov    $0x0,%eax
}
80109229:	83 c4 24             	add    $0x24,%esp
8010922c:	5b                   	pop    %ebx
8010922d:	5d                   	pop    %ebp
8010922e:	c3                   	ret    

8010922f <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
8010922f:	55                   	push   %ebp
80109230:	89 e5                	mov    %esp,%ebp
80109232:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
80109235:	8b 45 10             	mov    0x10(%ebp),%eax
80109238:	85 c0                	test   %eax,%eax
8010923a:	79 0a                	jns    80109246 <allocuvm+0x17>
    return 0;
8010923c:	b8 00 00 00 00       	mov    $0x0,%eax
80109241:	e9 c1 00 00 00       	jmp    80109307 <allocuvm+0xd8>
  if(newsz < oldsz)
80109246:	8b 45 10             	mov    0x10(%ebp),%eax
80109249:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010924c:	73 08                	jae    80109256 <allocuvm+0x27>
    return oldsz;
8010924e:	8b 45 0c             	mov    0xc(%ebp),%eax
80109251:	e9 b1 00 00 00       	jmp    80109307 <allocuvm+0xd8>
  a = PGROUNDUP(oldsz);
80109256:	8b 45 0c             	mov    0xc(%ebp),%eax
80109259:	05 ff 0f 00 00       	add    $0xfff,%eax
8010925e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80109263:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
80109266:	e9 8d 00 00 00       	jmp    801092f8 <allocuvm+0xc9>
    mem = kalloc();
8010926b:	e8 a0 98 ff ff       	call   80102b10 <kalloc>
80109270:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
80109273:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80109277:	75 2c                	jne    801092a5 <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
80109279:	c7 04 24 2d 9e 10 80 	movl   $0x80109e2d,(%esp)
80109280:	e8 1c 71 ff ff       	call   801003a1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
80109285:	8b 45 0c             	mov    0xc(%ebp),%eax
80109288:	89 44 24 08          	mov    %eax,0x8(%esp)
8010928c:	8b 45 10             	mov    0x10(%ebp),%eax
8010928f:	89 44 24 04          	mov    %eax,0x4(%esp)
80109293:	8b 45 08             	mov    0x8(%ebp),%eax
80109296:	89 04 24             	mov    %eax,(%esp)
80109299:	e8 6b 00 00 00       	call   80109309 <deallocuvm>
      return 0;
8010929e:	b8 00 00 00 00       	mov    $0x0,%eax
801092a3:	eb 62                	jmp    80109307 <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
801092a5:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801092ac:	00 
801092ad:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801092b4:	00 
801092b5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801092b8:	89 04 24             	mov    %eax,(%esp)
801092bb:	e8 3e cb ff ff       	call   80105dfe <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
801092c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801092c3:	89 04 24             	mov    %eax,(%esp)
801092c6:	e8 d8 f5 ff ff       	call   801088a3 <v2p>
801092cb:	8b 55 f4             	mov    -0xc(%ebp),%edx
801092ce:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
801092d5:	00 
801092d6:	89 44 24 0c          	mov    %eax,0xc(%esp)
801092da:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801092e1:	00 
801092e2:	89 54 24 04          	mov    %edx,0x4(%esp)
801092e6:	8b 45 08             	mov    0x8(%ebp),%eax
801092e9:	89 04 24             	mov    %eax,(%esp)
801092ec:	e8 d8 fa ff ff       	call   80108dc9 <mappages>
  if(newsz >= KERNBASE)
    return 0;
  if(newsz < oldsz)
    return oldsz;
  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
801092f1:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801092f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801092fb:	3b 45 10             	cmp    0x10(%ebp),%eax
801092fe:	0f 82 67 ff ff ff    	jb     8010926b <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
80109304:	8b 45 10             	mov    0x10(%ebp),%eax
}
80109307:	c9                   	leave  
80109308:	c3                   	ret    

80109309 <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80109309:	55                   	push   %ebp
8010930a:	89 e5                	mov    %esp,%ebp
8010930c:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
8010930f:	8b 45 10             	mov    0x10(%ebp),%eax
80109312:	3b 45 0c             	cmp    0xc(%ebp),%eax
80109315:	72 08                	jb     8010931f <deallocuvm+0x16>
    return oldsz;
80109317:	8b 45 0c             	mov    0xc(%ebp),%eax
8010931a:	e9 a4 00 00 00       	jmp    801093c3 <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
8010931f:	8b 45 10             	mov    0x10(%ebp),%eax
80109322:	05 ff 0f 00 00       	add    $0xfff,%eax
80109327:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010932c:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
8010932f:	e9 80 00 00 00       	jmp    801093b4 <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
80109334:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109337:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010933e:	00 
8010933f:	89 44 24 04          	mov    %eax,0x4(%esp)
80109343:	8b 45 08             	mov    0x8(%ebp),%eax
80109346:	89 04 24             	mov    %eax,(%esp)
80109349:	e8 e5 f9 ff ff       	call   80108d33 <walkpgdir>
8010934e:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
80109351:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80109355:	75 09                	jne    80109360 <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
80109357:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
8010935e:	eb 4d                	jmp    801093ad <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
80109360:	8b 45 f0             	mov    -0x10(%ebp),%eax
80109363:	8b 00                	mov    (%eax),%eax
80109365:	83 e0 01             	and    $0x1,%eax
80109368:	84 c0                	test   %al,%al
8010936a:	74 41                	je     801093ad <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
8010936c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010936f:	8b 00                	mov    (%eax),%eax
80109371:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80109376:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
80109379:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010937d:	75 0c                	jne    8010938b <deallocuvm+0x82>
        panic("kfree");
8010937f:	c7 04 24 45 9e 10 80 	movl   $0x80109e45,(%esp)
80109386:	e8 b2 71 ff ff       	call   8010053d <panic>
      char *v = p2v(pa);
8010938b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010938e:	89 04 24             	mov    %eax,(%esp)
80109391:	e8 1a f5 ff ff       	call   801088b0 <p2v>
80109396:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
80109399:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010939c:	89 04 24             	mov    %eax,(%esp)
8010939f:	e8 d3 96 ff ff       	call   80102a77 <kfree>
      *pte = 0;
801093a4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801093a7:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
801093ad:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801093b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801093b7:	3b 45 0c             	cmp    0xc(%ebp),%eax
801093ba:	0f 82 74 ff ff ff    	jb     80109334 <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
801093c0:	8b 45 10             	mov    0x10(%ebp),%eax
}
801093c3:	c9                   	leave  
801093c4:	c3                   	ret    

801093c5 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
801093c5:	55                   	push   %ebp
801093c6:	89 e5                	mov    %esp,%ebp
801093c8:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
801093cb:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801093cf:	75 0c                	jne    801093dd <freevm+0x18>
    panic("freevm: no pgdir");
801093d1:	c7 04 24 4b 9e 10 80 	movl   $0x80109e4b,(%esp)
801093d8:	e8 60 71 ff ff       	call   8010053d <panic>
  deallocuvm(pgdir, KERNBASE, 0);
801093dd:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801093e4:	00 
801093e5:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
801093ec:	80 
801093ed:	8b 45 08             	mov    0x8(%ebp),%eax
801093f0:	89 04 24             	mov    %eax,(%esp)
801093f3:	e8 11 ff ff ff       	call   80109309 <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
801093f8:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801093ff:	eb 3c                	jmp    8010943d <freevm+0x78>
    if(pgdir[i] & PTE_P){
80109401:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109404:	c1 e0 02             	shl    $0x2,%eax
80109407:	03 45 08             	add    0x8(%ebp),%eax
8010940a:	8b 00                	mov    (%eax),%eax
8010940c:	83 e0 01             	and    $0x1,%eax
8010940f:	84 c0                	test   %al,%al
80109411:	74 26                	je     80109439 <freevm+0x74>
      char * v = p2v(PTE_ADDR(pgdir[i]));
80109413:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109416:	c1 e0 02             	shl    $0x2,%eax
80109419:	03 45 08             	add    0x8(%ebp),%eax
8010941c:	8b 00                	mov    (%eax),%eax
8010941e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80109423:	89 04 24             	mov    %eax,(%esp)
80109426:	e8 85 f4 ff ff       	call   801088b0 <p2v>
8010942b:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
8010942e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80109431:	89 04 24             	mov    %eax,(%esp)
80109434:	e8 3e 96 ff ff       	call   80102a77 <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80109439:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010943d:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
80109444:	76 bb                	jbe    80109401 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
80109446:	8b 45 08             	mov    0x8(%ebp),%eax
80109449:	89 04 24             	mov    %eax,(%esp)
8010944c:	e8 26 96 ff ff       	call   80102a77 <kfree>
}
80109451:	c9                   	leave  
80109452:	c3                   	ret    

80109453 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80109453:	55                   	push   %ebp
80109454:	89 e5                	mov    %esp,%ebp
80109456:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80109459:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80109460:	00 
80109461:	8b 45 0c             	mov    0xc(%ebp),%eax
80109464:	89 44 24 04          	mov    %eax,0x4(%esp)
80109468:	8b 45 08             	mov    0x8(%ebp),%eax
8010946b:	89 04 24             	mov    %eax,(%esp)
8010946e:	e8 c0 f8 ff ff       	call   80108d33 <walkpgdir>
80109473:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
80109476:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010947a:	75 0c                	jne    80109488 <clearpteu+0x35>
    panic("clearpteu");
8010947c:	c7 04 24 5c 9e 10 80 	movl   $0x80109e5c,(%esp)
80109483:	e8 b5 70 ff ff       	call   8010053d <panic>
  *pte &= ~PTE_U;
80109488:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010948b:	8b 00                	mov    (%eax),%eax
8010948d:	89 c2                	mov    %eax,%edx
8010948f:	83 e2 fb             	and    $0xfffffffb,%edx
80109492:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109495:	89 10                	mov    %edx,(%eax)
}
80109497:	c9                   	leave  
80109498:	c3                   	ret    

80109499 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80109499:	55                   	push   %ebp
8010949a:	89 e5                	mov    %esp,%ebp
8010949c:	83 ec 48             	sub    $0x48,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
8010949f:	e8 b9 f9 ff ff       	call   80108e5d <setupkvm>
801094a4:	89 45 f0             	mov    %eax,-0x10(%ebp)
801094a7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801094ab:	75 0a                	jne    801094b7 <copyuvm+0x1e>
    return 0;
801094ad:	b8 00 00 00 00       	mov    $0x0,%eax
801094b2:	e9 f1 00 00 00       	jmp    801095a8 <copyuvm+0x10f>
  for(i = 0; i < sz; i += PGSIZE){
801094b7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801094be:	e9 c0 00 00 00       	jmp    80109583 <copyuvm+0xea>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
801094c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801094c6:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801094cd:	00 
801094ce:	89 44 24 04          	mov    %eax,0x4(%esp)
801094d2:	8b 45 08             	mov    0x8(%ebp),%eax
801094d5:	89 04 24             	mov    %eax,(%esp)
801094d8:	e8 56 f8 ff ff       	call   80108d33 <walkpgdir>
801094dd:	89 45 ec             	mov    %eax,-0x14(%ebp)
801094e0:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801094e4:	75 0c                	jne    801094f2 <copyuvm+0x59>
      panic("copyuvm: pte should exist");
801094e6:	c7 04 24 66 9e 10 80 	movl   $0x80109e66,(%esp)
801094ed:	e8 4b 70 ff ff       	call   8010053d <panic>
    if(!(*pte & PTE_P))
801094f2:	8b 45 ec             	mov    -0x14(%ebp),%eax
801094f5:	8b 00                	mov    (%eax),%eax
801094f7:	83 e0 01             	and    $0x1,%eax
801094fa:	85 c0                	test   %eax,%eax
801094fc:	75 0c                	jne    8010950a <copyuvm+0x71>
      panic("copyuvm: page not present");
801094fe:	c7 04 24 80 9e 10 80 	movl   $0x80109e80,(%esp)
80109505:	e8 33 70 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
8010950a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010950d:	8b 00                	mov    (%eax),%eax
8010950f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80109514:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if((mem = kalloc()) == 0)
80109517:	e8 f4 95 ff ff       	call   80102b10 <kalloc>
8010951c:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010951f:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80109523:	74 6f                	je     80109594 <copyuvm+0xfb>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
80109525:	8b 45 e8             	mov    -0x18(%ebp),%eax
80109528:	89 04 24             	mov    %eax,(%esp)
8010952b:	e8 80 f3 ff ff       	call   801088b0 <p2v>
80109530:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80109537:	00 
80109538:	89 44 24 04          	mov    %eax,0x4(%esp)
8010953c:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010953f:	89 04 24             	mov    %eax,(%esp)
80109542:	e8 8a c9 ff ff       	call   80105ed1 <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
80109547:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010954a:	89 04 24             	mov    %eax,(%esp)
8010954d:	e8 51 f3 ff ff       	call   801088a3 <v2p>
80109552:	8b 55 f4             	mov    -0xc(%ebp),%edx
80109555:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010955c:	00 
8010955d:	89 44 24 0c          	mov    %eax,0xc(%esp)
80109561:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80109568:	00 
80109569:	89 54 24 04          	mov    %edx,0x4(%esp)
8010956d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80109570:	89 04 24             	mov    %eax,(%esp)
80109573:	e8 51 f8 ff ff       	call   80108dc9 <mappages>
80109578:	85 c0                	test   %eax,%eax
8010957a:	78 1b                	js     80109597 <copyuvm+0xfe>
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
8010957c:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80109583:	8b 45 f4             	mov    -0xc(%ebp),%eax
80109586:	3b 45 0c             	cmp    0xc(%ebp),%eax
80109589:	0f 82 34 ff ff ff    	jb     801094c3 <copyuvm+0x2a>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
  }
  return d;
8010958f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80109592:	eb 14                	jmp    801095a8 <copyuvm+0x10f>
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
80109594:	90                   	nop
80109595:	eb 01                	jmp    80109598 <copyuvm+0xff>
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
80109597:	90                   	nop
  }
  return d;

bad:
  freevm(d);
80109598:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010959b:	89 04 24             	mov    %eax,(%esp)
8010959e:	e8 22 fe ff ff       	call   801093c5 <freevm>
  return 0;
801095a3:	b8 00 00 00 00       	mov    $0x0,%eax
}
801095a8:	c9                   	leave  
801095a9:	c3                   	ret    

801095aa <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
801095aa:	55                   	push   %ebp
801095ab:	89 e5                	mov    %esp,%ebp
801095ad:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801095b0:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801095b7:	00 
801095b8:	8b 45 0c             	mov    0xc(%ebp),%eax
801095bb:	89 44 24 04          	mov    %eax,0x4(%esp)
801095bf:	8b 45 08             	mov    0x8(%ebp),%eax
801095c2:	89 04 24             	mov    %eax,(%esp)
801095c5:	e8 69 f7 ff ff       	call   80108d33 <walkpgdir>
801095ca:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
801095cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801095d0:	8b 00                	mov    (%eax),%eax
801095d2:	83 e0 01             	and    $0x1,%eax
801095d5:	85 c0                	test   %eax,%eax
801095d7:	75 07                	jne    801095e0 <uva2ka+0x36>
    return 0;
801095d9:	b8 00 00 00 00       	mov    $0x0,%eax
801095de:	eb 25                	jmp    80109605 <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
801095e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801095e3:	8b 00                	mov    (%eax),%eax
801095e5:	83 e0 04             	and    $0x4,%eax
801095e8:	85 c0                	test   %eax,%eax
801095ea:	75 07                	jne    801095f3 <uva2ka+0x49>
    return 0;
801095ec:	b8 00 00 00 00       	mov    $0x0,%eax
801095f1:	eb 12                	jmp    80109605 <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
801095f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801095f6:	8b 00                	mov    (%eax),%eax
801095f8:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801095fd:	89 04 24             	mov    %eax,(%esp)
80109600:	e8 ab f2 ff ff       	call   801088b0 <p2v>
}
80109605:	c9                   	leave  
80109606:	c3                   	ret    

80109607 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80109607:	55                   	push   %ebp
80109608:	89 e5                	mov    %esp,%ebp
8010960a:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
8010960d:	8b 45 10             	mov    0x10(%ebp),%eax
80109610:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
80109613:	e9 8b 00 00 00       	jmp    801096a3 <copyout+0x9c>
    va0 = (uint)PGROUNDDOWN(va);
80109618:	8b 45 0c             	mov    0xc(%ebp),%eax
8010961b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80109620:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
80109623:	8b 45 ec             	mov    -0x14(%ebp),%eax
80109626:	89 44 24 04          	mov    %eax,0x4(%esp)
8010962a:	8b 45 08             	mov    0x8(%ebp),%eax
8010962d:	89 04 24             	mov    %eax,(%esp)
80109630:	e8 75 ff ff ff       	call   801095aa <uva2ka>
80109635:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
80109638:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010963c:	75 07                	jne    80109645 <copyout+0x3e>
      return -1;
8010963e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80109643:	eb 6d                	jmp    801096b2 <copyout+0xab>
    n = PGSIZE - (va - va0);
80109645:	8b 45 0c             	mov    0xc(%ebp),%eax
80109648:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010964b:	89 d1                	mov    %edx,%ecx
8010964d:	29 c1                	sub    %eax,%ecx
8010964f:	89 c8                	mov    %ecx,%eax
80109651:	05 00 10 00 00       	add    $0x1000,%eax
80109656:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
80109659:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010965c:	3b 45 14             	cmp    0x14(%ebp),%eax
8010965f:	76 06                	jbe    80109667 <copyout+0x60>
      n = len;
80109661:	8b 45 14             	mov    0x14(%ebp),%eax
80109664:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
80109667:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010966a:	8b 55 0c             	mov    0xc(%ebp),%edx
8010966d:	89 d1                	mov    %edx,%ecx
8010966f:	29 c1                	sub    %eax,%ecx
80109671:	89 c8                	mov    %ecx,%eax
80109673:	03 45 e8             	add    -0x18(%ebp),%eax
80109676:	8b 55 f0             	mov    -0x10(%ebp),%edx
80109679:	89 54 24 08          	mov    %edx,0x8(%esp)
8010967d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80109680:	89 54 24 04          	mov    %edx,0x4(%esp)
80109684:	89 04 24             	mov    %eax,(%esp)
80109687:	e8 45 c8 ff ff       	call   80105ed1 <memmove>
    len -= n;
8010968c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010968f:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
80109692:	8b 45 f0             	mov    -0x10(%ebp),%eax
80109695:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
80109698:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010969b:	05 00 10 00 00       	add    $0x1000,%eax
801096a0:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
801096a3:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
801096a7:	0f 85 6b ff ff ff    	jne    80109618 <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
801096ad:	b8 00 00 00 00       	mov    $0x0,%eax
}
801096b2:	c9                   	leave  
801096b3:	c3                   	ret    
