
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 b0 10 00       	mov    $0x10b000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 60 d6 10 80       	mov    $0x8010d660,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 ff 33 10 80       	mov    $0x801033ff,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 f0 88 10 	movl   $0x801088f0,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
80100049:	e8 e4 50 00 00       	call   80105132 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 90 eb 10 80 84 	movl   $0x8010eb84,0x8010eb90
80100055:	eb 10 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 94 eb 10 80 84 	movl   $0x8010eb84,0x8010eb94
8010005f:	eb 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 94 d6 10 80 	movl   $0x8010d694,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 94 eb 10 80    	mov    0x8010eb94,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 84 eb 10 80 	movl   $0x8010eb84,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 94 eb 10 80       	mov    0x8010eb94,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 94 eb 10 80       	mov    %eax,0x8010eb94

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 84 eb 10 80 	cmpl   $0x8010eb84,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate fresh block.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
801000bd:	e8 91 50 00 00       	call   80105153 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 94 eb 10 80       	mov    0x8010eb94,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	89 c2                	mov    %eax,%edx
801000f5:	83 ca 01             	or     $0x1,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
80100104:	e8 e5 50 00 00       	call   801051ee <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 60 d6 10 	movl   $0x8010d660,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 c1 4c 00 00       	call   80104de5 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 84 eb 10 80 	cmpl   $0x8010eb84,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 90 eb 10 80       	mov    0x8010eb90,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
8010017c:	e8 6d 50 00 00       	call   801051ee <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 84 eb 10 80 	cmpl   $0x8010eb84,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 f7 88 10 80 	movl   $0x801088f7,(%esp)
8010019f:	e8 99 03 00 00       	call   8010053d <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 d1 25 00 00       	call   801027a9 <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 08 89 10 80 	movl   $0x80108908,(%esp)
801001f6:	e8 42 03 00 00       	call   8010053d <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	89 c2                	mov    %eax,%edx
80100202:	83 ca 04             	or     $0x4,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 94 25 00 00       	call   801027a9 <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 0f 89 10 80 	movl   $0x8010890f,(%esp)
80100230:	e8 08 03 00 00       	call   8010053d <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
8010023c:	e8 12 4f 00 00       	call   80105153 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 94 eb 10 80    	mov    0x8010eb94,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 84 eb 10 80 	movl   $0x8010eb84,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 94 eb 10 80       	mov    0x8010eb94,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 94 eb 10 80       	mov    %eax,0x8010eb94

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	89 c2                	mov    %eax,%edx
8010028f:	83 e2 fe             	and    $0xfffffffe,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 a6 4c 00 00       	call   80104f48 <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 60 d6 10 80 	movl   $0x8010d660,(%esp)
801002a9:	e8 40 4f 00 00       	call   801051ee <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	53                   	push   %ebx
801002b4:	83 ec 14             	sub    $0x14,%esp
801002b7:	8b 45 08             	mov    0x8(%ebp),%eax
801002ba:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002be:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
801002c2:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
801002c6:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
801002ca:	ec                   	in     (%dx),%al
801002cb:	89 c3                	mov    %eax,%ebx
801002cd:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
801002d0:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
801002d4:	83 c4 14             	add    $0x14,%esp
801002d7:	5b                   	pop    %ebx
801002d8:	5d                   	pop    %ebp
801002d9:	c3                   	ret    

801002da <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002da:	55                   	push   %ebp
801002db:	89 e5                	mov    %esp,%ebp
801002dd:	83 ec 08             	sub    $0x8,%esp
801002e0:	8b 55 08             	mov    0x8(%ebp),%edx
801002e3:	8b 45 0c             	mov    0xc(%ebp),%eax
801002e6:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002ea:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002ed:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002f1:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002f5:	ee                   	out    %al,(%dx)
}
801002f6:	c9                   	leave  
801002f7:	c3                   	ret    

801002f8 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002f8:	55                   	push   %ebp
801002f9:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002fb:	fa                   	cli    
}
801002fc:	5d                   	pop    %ebp
801002fd:	c3                   	ret    

801002fe <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002fe:	55                   	push   %ebp
801002ff:	89 e5                	mov    %esp,%ebp
80100301:	83 ec 48             	sub    $0x48,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
80100304:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100308:	74 19                	je     80100323 <printint+0x25>
8010030a:	8b 45 08             	mov    0x8(%ebp),%eax
8010030d:	c1 e8 1f             	shr    $0x1f,%eax
80100310:	89 45 10             	mov    %eax,0x10(%ebp)
80100313:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100317:	74 0a                	je     80100323 <printint+0x25>
    x = -xx;
80100319:	8b 45 08             	mov    0x8(%ebp),%eax
8010031c:	f7 d8                	neg    %eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100321:	eb 06                	jmp    80100329 <printint+0x2b>
  else
    x = xx;
80100323:	8b 45 08             	mov    0x8(%ebp),%eax
80100326:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100329:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100330:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80100333:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100336:	ba 00 00 00 00       	mov    $0x0,%edx
8010033b:	f7 f1                	div    %ecx
8010033d:	89 d0                	mov    %edx,%eax
8010033f:	0f b6 90 04 a0 10 80 	movzbl -0x7fef5ffc(%eax),%edx
80100346:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100349:	03 45 f4             	add    -0xc(%ebp),%eax
8010034c:	88 10                	mov    %dl,(%eax)
8010034e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  }while((x /= base) != 0);
80100352:	8b 55 0c             	mov    0xc(%ebp),%edx
80100355:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80100358:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010035b:	ba 00 00 00 00       	mov    $0x0,%edx
80100360:	f7 75 d4             	divl   -0x2c(%ebp)
80100363:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100366:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010036a:	75 c4                	jne    80100330 <printint+0x32>

  if(sign)
8010036c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100370:	74 23                	je     80100395 <printint+0x97>
    buf[i++] = '-';
80100372:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100375:	03 45 f4             	add    -0xc(%ebp),%eax
80100378:	c6 00 2d             	movb   $0x2d,(%eax)
8010037b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)

  while(--i >= 0)
8010037f:	eb 14                	jmp    80100395 <printint+0x97>
    consputc(buf[i]);
80100381:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100384:	03 45 f4             	add    -0xc(%ebp),%eax
80100387:	0f b6 00             	movzbl (%eax),%eax
8010038a:	0f be c0             	movsbl %al,%eax
8010038d:	89 04 24             	mov    %eax,(%esp)
80100390:	e8 bb 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
80100395:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100399:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010039d:	79 e2                	jns    80100381 <printint+0x83>
    consputc(buf[i]);
}
8010039f:	c9                   	leave  
801003a0:	c3                   	ret    

801003a1 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a1:	55                   	push   %ebp
801003a2:	89 e5                	mov    %esp,%ebp
801003a4:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a7:	a1 f4 c5 10 80       	mov    0x8010c5f4,%eax
801003ac:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003af:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b3:	74 0c                	je     801003c1 <cprintf+0x20>
    acquire(&cons.lock);
801003b5:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
801003bc:	e8 92 4d 00 00       	call   80105153 <acquire>

  if (fmt == 0)
801003c1:	8b 45 08             	mov    0x8(%ebp),%eax
801003c4:	85 c0                	test   %eax,%eax
801003c6:	75 0c                	jne    801003d4 <cprintf+0x33>
    panic("null fmt");
801003c8:	c7 04 24 16 89 10 80 	movl   $0x80108916,(%esp)
801003cf:	e8 69 01 00 00       	call   8010053d <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d4:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003da:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e1:	e9 20 01 00 00       	jmp    80100506 <cprintf+0x165>
    if(c != '%'){
801003e6:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003ea:	74 10                	je     801003fc <cprintf+0x5b>
      consputc(c);
801003ec:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ef:	89 04 24             	mov    %eax,(%esp)
801003f2:	e8 59 03 00 00       	call   80100750 <consputc>
      continue;
801003f7:	e9 06 01 00 00       	jmp    80100502 <cprintf+0x161>
    }
    c = fmt[++i] & 0xff;
801003fc:	8b 55 08             	mov    0x8(%ebp),%edx
801003ff:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100403:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100406:	01 d0                	add    %edx,%eax
80100408:	0f b6 00             	movzbl (%eax),%eax
8010040b:	0f be c0             	movsbl %al,%eax
8010040e:	25 ff 00 00 00       	and    $0xff,%eax
80100413:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100416:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010041a:	0f 84 08 01 00 00    	je     80100528 <cprintf+0x187>
      break;
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4d                	je     80100475 <cprintf+0xd4>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0x9f>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13b>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xae>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x149>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 53                	je     80100498 <cprintf+0xf7>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2b                	je     80100475 <cprintf+0xd4>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x149>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8b 00                	mov    (%eax),%eax
80100454:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
80100458:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010045f:	00 
80100460:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100467:	00 
80100468:	89 04 24             	mov    %eax,(%esp)
8010046b:	e8 8e fe ff ff       	call   801002fe <printint>
      break;
80100470:	e9 8d 00 00 00       	jmp    80100502 <cprintf+0x161>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100475:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100478:	8b 00                	mov    (%eax),%eax
8010047a:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
8010047e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100485:	00 
80100486:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
8010048d:	00 
8010048e:	89 04 24             	mov    %eax,(%esp)
80100491:	e8 68 fe ff ff       	call   801002fe <printint>
      break;
80100496:	eb 6a                	jmp    80100502 <cprintf+0x161>
    case 's':
      if((s = (char*)*argp++) == 0)
80100498:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049b:	8b 00                	mov    (%eax),%eax
8010049d:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004a0:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004a4:	0f 94 c0             	sete   %al
801004a7:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
801004ab:	84 c0                	test   %al,%al
801004ad:	74 20                	je     801004cf <cprintf+0x12e>
        s = "(null)";
801004af:	c7 45 ec 1f 89 10 80 	movl   $0x8010891f,-0x14(%ebp)
      for(; *s; s++)
801004b6:	eb 17                	jmp    801004cf <cprintf+0x12e>
        consputc(*s);
801004b8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004bb:	0f b6 00             	movzbl (%eax),%eax
801004be:	0f be c0             	movsbl %al,%eax
801004c1:	89 04 24             	mov    %eax,(%esp)
801004c4:	e8 87 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004c9:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004cd:	eb 01                	jmp    801004d0 <cprintf+0x12f>
801004cf:	90                   	nop
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 de                	jne    801004b8 <cprintf+0x117>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x161>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x161>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 c0 fe ff ff    	jne    801003e6 <cprintf+0x45>
80100526:	eb 01                	jmp    80100529 <cprintf+0x188>
      consputc(c);
      continue;
    }
    c = fmt[++i] & 0xff;
    if(c == 0)
      break;
80100528:	90                   	nop
      consputc(c);
      break;
    }
  }

  if(locking)
80100529:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052d:	74 0c                	je     8010053b <cprintf+0x19a>
    release(&cons.lock);
8010052f:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
80100536:	e8 b3 4c 00 00       	call   801051ee <release>
}
8010053b:	c9                   	leave  
8010053c:	c3                   	ret    

8010053d <panic>:

void
panic(char *s)
{
8010053d:	55                   	push   %ebp
8010053e:	89 e5                	mov    %esp,%ebp
80100540:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100543:	e8 b0 fd ff ff       	call   801002f8 <cli>
  cons.locking = 0;
80100548:	c7 05 f4 c5 10 80 00 	movl   $0x0,0x8010c5f4
8010054f:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
80100552:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100558:	0f b6 00             	movzbl (%eax),%eax
8010055b:	0f b6 c0             	movzbl %al,%eax
8010055e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100562:	c7 04 24 26 89 10 80 	movl   $0x80108926,(%esp)
80100569:	e8 33 fe ff ff       	call   801003a1 <cprintf>
  cprintf(s);
8010056e:	8b 45 08             	mov    0x8(%ebp),%eax
80100571:	89 04 24             	mov    %eax,(%esp)
80100574:	e8 28 fe ff ff       	call   801003a1 <cprintf>
  cprintf("\n");
80100579:	c7 04 24 35 89 10 80 	movl   $0x80108935,(%esp)
80100580:	e8 1c fe ff ff       	call   801003a1 <cprintf>
  getcallerpcs(&s, pcs);
80100585:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100588:	89 44 24 04          	mov    %eax,0x4(%esp)
8010058c:	8d 45 08             	lea    0x8(%ebp),%eax
8010058f:	89 04 24             	mov    %eax,(%esp)
80100592:	e8 a6 4c 00 00       	call   8010523d <getcallerpcs>
  for(i=0; i<10; i++)
80100597:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059e:	eb 1b                	jmp    801005bb <panic+0x7e>
    cprintf(" %p", pcs[i]);
801005a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a3:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a7:	89 44 24 04          	mov    %eax,0x4(%esp)
801005ab:	c7 04 24 37 89 10 80 	movl   $0x80108937,(%esp)
801005b2:	e8 ea fd ff ff       	call   801003a1 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b7:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005bb:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bf:	7e df                	jle    801005a0 <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005c1:	c7 05 a0 c5 10 80 01 	movl   $0x1,0x8010c5a0
801005c8:	00 00 00 
  for(;;)
    ;
801005cb:	eb fe                	jmp    801005cb <panic+0x8e>

801005cd <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005cd:	55                   	push   %ebp
801005ce:	89 e5                	mov    %esp,%ebp
801005d0:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d3:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005da:	00 
801005db:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005e2:	e8 f3 fc ff ff       	call   801002da <outb>
  pos = inb(CRTPORT+1) << 8;
801005e7:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005ee:	e8 bd fc ff ff       	call   801002b0 <inb>
801005f3:	0f b6 c0             	movzbl %al,%eax
801005f6:	c1 e0 08             	shl    $0x8,%eax
801005f9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005fc:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100603:	00 
80100604:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010060b:	e8 ca fc ff ff       	call   801002da <outb>
  pos |= inb(CRTPORT+1);
80100610:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100617:	e8 94 fc ff ff       	call   801002b0 <inb>
8010061c:	0f b6 c0             	movzbl %al,%eax
8010061f:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
80100622:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100626:	75 30                	jne    80100658 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100628:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010062b:	ba 67 66 66 66       	mov    $0x66666667,%edx
80100630:	89 c8                	mov    %ecx,%eax
80100632:	f7 ea                	imul   %edx
80100634:	c1 fa 05             	sar    $0x5,%edx
80100637:	89 c8                	mov    %ecx,%eax
80100639:	c1 f8 1f             	sar    $0x1f,%eax
8010063c:	29 c2                	sub    %eax,%edx
8010063e:	89 d0                	mov    %edx,%eax
80100640:	c1 e0 02             	shl    $0x2,%eax
80100643:	01 d0                	add    %edx,%eax
80100645:	c1 e0 04             	shl    $0x4,%eax
80100648:	89 ca                	mov    %ecx,%edx
8010064a:	29 c2                	sub    %eax,%edx
8010064c:	b8 50 00 00 00       	mov    $0x50,%eax
80100651:	29 d0                	sub    %edx,%eax
80100653:	01 45 f4             	add    %eax,-0xc(%ebp)
80100656:	eb 32                	jmp    8010068a <cgaputc+0xbd>
  else if(c == BACKSPACE){
80100658:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065f:	75 0c                	jne    8010066d <cgaputc+0xa0>
    if(pos > 0) --pos;
80100661:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100665:	7e 23                	jle    8010068a <cgaputc+0xbd>
80100667:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
8010066b:	eb 1d                	jmp    8010068a <cgaputc+0xbd>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066d:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100675:	01 d2                	add    %edx,%edx
80100677:	01 c2                	add    %eax,%edx
80100679:	8b 45 08             	mov    0x8(%ebp),%eax
8010067c:	66 25 ff 00          	and    $0xff,%ax
80100680:	80 cc 07             	or     $0x7,%ah
80100683:	66 89 02             	mov    %ax,(%edx)
80100686:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x119>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 a0 10 80       	mov    0x8010a000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 f6 4d 00 00       	call   801054ad <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	01 c0                	add    %eax,%eax
801006c5:	8b 15 00 a0 10 80    	mov    0x8010a000,%edx
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 ca                	add    %ecx,%edx
801006d2:	89 44 24 08          	mov    %eax,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 14 24             	mov    %edx,(%esp)
801006e1:	e8 f4 4c 00 00       	call   801053da <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 e0 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 c7 fb ff ff       	call   801002da <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 b3 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 9d fb ff ff       	call   801002da <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 a0 c5 10 80       	mov    0x8010c5a0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 94 fb ff ff       	call   801002f8 <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 da 67 00 00       	call   80106f55 <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 ce 67 00 00       	call   80106f55 <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 c2 67 00 00       	call   80106f55 <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 b5 67 00 00       	call   80106f55 <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 22 fe ff ff       	call   801005cd <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
801007ba:	e8 94 49 00 00       	call   80105153 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 41 01 00 00       	jmp    80100905 <consoleintr+0x158>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 68                	je     8010083e <consoleintr+0x91>
801007d6:	e9 94 00 00 00       	jmp    8010086f <consoleintr+0xc2>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 59                	je     8010083e <consoleintr+0x91>
801007e5:	e9 85 00 00 00       	jmp    8010086f <consoleintr+0xc2>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 ff 47 00 00       	call   80104fee <procdump>
      break;
801007ef:	e9 11 01 00 00       	jmp    80100905 <consoleintr+0x158>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 5c ee 10 80       	mov    %eax,0x8010ee5c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 5c ee 10 80    	mov    0x8010ee5c,%edx
80100816:	a1 58 ee 10 80       	mov    0x8010ee58,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	0f 84 db 00 00 00    	je     801008fe <consoleintr+0x151>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100823:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
80100828:	83 e8 01             	sub    $0x1,%eax
8010082b:	83 e0 7f             	and    $0x7f,%eax
8010082e:	0f b6 80 d4 ed 10 80 	movzbl -0x7fef122c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100835:	3c 0a                	cmp    $0xa,%al
80100837:	75 bb                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100839:	e9 c0 00 00 00       	jmp    801008fe <consoleintr+0x151>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083e:	8b 15 5c ee 10 80    	mov    0x8010ee5c,%edx
80100844:	a1 58 ee 10 80       	mov    0x8010ee58,%eax
80100849:	39 c2                	cmp    %eax,%edx
8010084b:	0f 84 b0 00 00 00    	je     80100901 <consoleintr+0x154>
        input.e--;
80100851:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
80100856:	83 e8 01             	sub    $0x1,%eax
80100859:	a3 5c ee 10 80       	mov    %eax,0x8010ee5c
        consputc(BACKSPACE);
8010085e:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100865:	e8 e6 fe ff ff       	call   80100750 <consputc>
      }
      break;
8010086a:	e9 92 00 00 00       	jmp    80100901 <consoleintr+0x154>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100873:	0f 84 8b 00 00 00    	je     80100904 <consoleintr+0x157>
80100879:	8b 15 5c ee 10 80    	mov    0x8010ee5c,%edx
8010087f:	a1 54 ee 10 80       	mov    0x8010ee54,%eax
80100884:	89 d1                	mov    %edx,%ecx
80100886:	29 c1                	sub    %eax,%ecx
80100888:	89 c8                	mov    %ecx,%eax
8010088a:	83 f8 7f             	cmp    $0x7f,%eax
8010088d:	77 75                	ja     80100904 <consoleintr+0x157>
        c = (c == '\r') ? '\n' : c;
8010088f:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
80100893:	74 05                	je     8010089a <consoleintr+0xed>
80100895:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100898:	eb 05                	jmp    8010089f <consoleintr+0xf2>
8010089a:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089f:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
801008a2:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
801008a7:	89 c1                	mov    %eax,%ecx
801008a9:	83 e1 7f             	and    $0x7f,%ecx
801008ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801008af:	88 91 d4 ed 10 80    	mov    %dl,-0x7fef122c(%ecx)
801008b5:	83 c0 01             	add    $0x1,%eax
801008b8:	a3 5c ee 10 80       	mov    %eax,0x8010ee5c
        consputc(c);
801008bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008c0:	89 04 24             	mov    %eax,(%esp)
801008c3:	e8 88 fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c8:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008cc:	74 18                	je     801008e6 <consoleintr+0x139>
801008ce:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008d2:	74 12                	je     801008e6 <consoleintr+0x139>
801008d4:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
801008d9:	8b 15 54 ee 10 80    	mov    0x8010ee54,%edx
801008df:	83 ea 80             	sub    $0xffffff80,%edx
801008e2:	39 d0                	cmp    %edx,%eax
801008e4:	75 1e                	jne    80100904 <consoleintr+0x157>
          input.w = input.e;
801008e6:	a1 5c ee 10 80       	mov    0x8010ee5c,%eax
801008eb:	a3 58 ee 10 80       	mov    %eax,0x8010ee58
          wakeup(&input.r);
801008f0:	c7 04 24 54 ee 10 80 	movl   $0x8010ee54,(%esp)
801008f7:	e8 4c 46 00 00       	call   80104f48 <wakeup>
        }
      }
      break;
801008fc:	eb 06                	jmp    80100904 <consoleintr+0x157>
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
801008fe:	90                   	nop
801008ff:	eb 04                	jmp    80100905 <consoleintr+0x158>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100901:	90                   	nop
80100902:	eb 01                	jmp    80100905 <consoleintr+0x158>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
          input.w = input.e;
          wakeup(&input.r);
        }
      }
      break;
80100904:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
80100905:	8b 45 08             	mov    0x8(%ebp),%eax
80100908:	ff d0                	call   *%eax
8010090a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010090d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100911:	0f 89 ad fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
80100917:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
8010091e:	e8 cb 48 00 00       	call   801051ee <release>
}
80100923:	c9                   	leave  
80100924:	c3                   	ret    

80100925 <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
80100925:	55                   	push   %ebp
80100926:	89 e5                	mov    %esp,%ebp
80100928:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
8010092b:	8b 45 08             	mov    0x8(%ebp),%eax
8010092e:	89 04 24             	mov    %eax,(%esp)
80100931:	e8 80 10 00 00       	call   801019b6 <iunlock>
  target = n;
80100936:	8b 45 10             	mov    0x10(%ebp),%eax
80100939:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
8010093c:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
80100943:	e8 0b 48 00 00       	call   80105153 <acquire>
  while(n > 0){
80100948:	e9 a8 00 00 00       	jmp    801009f5 <consoleread+0xd0>
    while(input.r == input.w){
      if(proc->killed){
8010094d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100953:	8b 40 24             	mov    0x24(%eax),%eax
80100956:	85 c0                	test   %eax,%eax
80100958:	74 21                	je     8010097b <consoleread+0x56>
        release(&input.lock);
8010095a:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
80100961:	e8 88 48 00 00       	call   801051ee <release>
        ilock(ip);
80100966:	8b 45 08             	mov    0x8(%ebp),%eax
80100969:	89 04 24             	mov    %eax,(%esp)
8010096c:	e8 f7 0e 00 00       	call   80101868 <ilock>
        return -1;
80100971:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100976:	e9 a9 00 00 00       	jmp    80100a24 <consoleread+0xff>
      }
      sleep(&input.r, &input.lock);
8010097b:	c7 44 24 04 a0 ed 10 	movl   $0x8010eda0,0x4(%esp)
80100982:	80 
80100983:	c7 04 24 54 ee 10 80 	movl   $0x8010ee54,(%esp)
8010098a:	e8 56 44 00 00       	call   80104de5 <sleep>
8010098f:	eb 01                	jmp    80100992 <consoleread+0x6d>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100991:	90                   	nop
80100992:	8b 15 54 ee 10 80    	mov    0x8010ee54,%edx
80100998:	a1 58 ee 10 80       	mov    0x8010ee58,%eax
8010099d:	39 c2                	cmp    %eax,%edx
8010099f:	74 ac                	je     8010094d <consoleread+0x28>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
801009a1:	a1 54 ee 10 80       	mov    0x8010ee54,%eax
801009a6:	89 c2                	mov    %eax,%edx
801009a8:	83 e2 7f             	and    $0x7f,%edx
801009ab:	0f b6 92 d4 ed 10 80 	movzbl -0x7fef122c(%edx),%edx
801009b2:	0f be d2             	movsbl %dl,%edx
801009b5:	89 55 f0             	mov    %edx,-0x10(%ebp)
801009b8:	83 c0 01             	add    $0x1,%eax
801009bb:	a3 54 ee 10 80       	mov    %eax,0x8010ee54
    if(c == C('D')){  // EOF
801009c0:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009c4:	75 17                	jne    801009dd <consoleread+0xb8>
      if(n < target){
801009c6:	8b 45 10             	mov    0x10(%ebp),%eax
801009c9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009cc:	73 2f                	jae    801009fd <consoleread+0xd8>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009ce:	a1 54 ee 10 80       	mov    0x8010ee54,%eax
801009d3:	83 e8 01             	sub    $0x1,%eax
801009d6:	a3 54 ee 10 80       	mov    %eax,0x8010ee54
      }
      break;
801009db:	eb 20                	jmp    801009fd <consoleread+0xd8>
    }
    *dst++ = c;
801009dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801009e0:	89 c2                	mov    %eax,%edx
801009e2:	8b 45 0c             	mov    0xc(%ebp),%eax
801009e5:	88 10                	mov    %dl,(%eax)
801009e7:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
    --n;
801009eb:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009ef:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009f3:	74 0b                	je     80100a00 <consoleread+0xdb>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009f5:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f9:	7f 96                	jg     80100991 <consoleread+0x6c>
801009fb:	eb 04                	jmp    80100a01 <consoleread+0xdc>
      if(n < target){
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
      }
      break;
801009fd:	90                   	nop
801009fe:	eb 01                	jmp    80100a01 <consoleread+0xdc>
    }
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
80100a00:	90                   	nop
  }
  release(&input.lock);
80100a01:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
80100a08:	e8 e1 47 00 00       	call   801051ee <release>
  ilock(ip);
80100a0d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a10:	89 04 24             	mov    %eax,(%esp)
80100a13:	e8 50 0e 00 00       	call   80101868 <ilock>

  return target - n;
80100a18:	8b 45 10             	mov    0x10(%ebp),%eax
80100a1b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a1e:	89 d1                	mov    %edx,%ecx
80100a20:	29 c1                	sub    %eax,%ecx
80100a22:	89 c8                	mov    %ecx,%eax
}
80100a24:	c9                   	leave  
80100a25:	c3                   	ret    

80100a26 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a26:	55                   	push   %ebp
80100a27:	89 e5                	mov    %esp,%ebp
80100a29:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a2c:	8b 45 08             	mov    0x8(%ebp),%eax
80100a2f:	89 04 24             	mov    %eax,(%esp)
80100a32:	e8 7f 0f 00 00       	call   801019b6 <iunlock>
  acquire(&cons.lock);
80100a37:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
80100a3e:	e8 10 47 00 00       	call   80105153 <acquire>
  for(i = 0; i < n; i++)
80100a43:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a4a:	eb 1d                	jmp    80100a69 <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a4f:	03 45 0c             	add    0xc(%ebp),%eax
80100a52:	0f b6 00             	movzbl (%eax),%eax
80100a55:	0f be c0             	movsbl %al,%eax
80100a58:	25 ff 00 00 00       	and    $0xff,%eax
80100a5d:	89 04 24             	mov    %eax,(%esp)
80100a60:	e8 eb fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a65:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a6c:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a6f:	7c db                	jl     80100a4c <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a71:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
80100a78:	e8 71 47 00 00       	call   801051ee <release>
  ilock(ip);
80100a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a80:	89 04 24             	mov    %eax,(%esp)
80100a83:	e8 e0 0d 00 00       	call   80101868 <ilock>

  return n;
80100a88:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a8b:	c9                   	leave  
80100a8c:	c3                   	ret    

80100a8d <consoleinit>:

void
consoleinit(void)
{
80100a8d:	55                   	push   %ebp
80100a8e:	89 e5                	mov    %esp,%ebp
80100a90:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a93:	c7 44 24 04 3b 89 10 	movl   $0x8010893b,0x4(%esp)
80100a9a:	80 
80100a9b:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
80100aa2:	e8 8b 46 00 00       	call   80105132 <initlock>
  initlock(&input.lock, "input");
80100aa7:	c7 44 24 04 43 89 10 	movl   $0x80108943,0x4(%esp)
80100aae:	80 
80100aaf:	c7 04 24 a0 ed 10 80 	movl   $0x8010eda0,(%esp)
80100ab6:	e8 77 46 00 00       	call   80105132 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100abb:	c7 05 0c f8 10 80 26 	movl   $0x80100a26,0x8010f80c
80100ac2:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ac5:	c7 05 08 f8 10 80 25 	movl   $0x80100925,0x8010f808
80100acc:	09 10 80 
  cons.locking = 1;
80100acf:	c7 05 f4 c5 10 80 01 	movl   $0x1,0x8010c5f4
80100ad6:	00 00 00 

  picenable(IRQ_KBD);
80100ad9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae0:	e8 d4 2f 00 00       	call   80103ab9 <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ae5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100aec:	00 
80100aed:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100af4:	e8 75 1e 00 00       	call   8010296e <ioapicenable>
}
80100af9:	c9                   	leave  
80100afa:	c3                   	ret    
	...

80100afc <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100afc:	55                   	push   %ebp
80100afd:	89 e5                	mov    %esp,%ebp
80100aff:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  if((ip = namei(path)) == 0)
80100b05:	8b 45 08             	mov    0x8(%ebp),%eax
80100b08:	89 04 24             	mov    %eax,(%esp)
80100b0b:	e8 fa 18 00 00       	call   8010240a <namei>
80100b10:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b13:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b17:	75 0a                	jne    80100b23 <exec+0x27>
    return -1;
80100b19:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1e:	e9 da 03 00 00       	jmp    80100efd <exec+0x401>
  ilock(ip);
80100b23:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b26:	89 04 24             	mov    %eax,(%esp)
80100b29:	e8 3a 0d 00 00       	call   80101868 <ilock>
  pgdir = 0;
80100b2e:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b35:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b3c:	00 
80100b3d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b44:	00 
80100b45:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b4b:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4f:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b52:	89 04 24             	mov    %eax,(%esp)
80100b55:	e8 04 12 00 00       	call   80101d5e <readi>
80100b5a:	83 f8 33             	cmp    $0x33,%eax
80100b5d:	0f 86 54 03 00 00    	jbe    80100eb7 <exec+0x3bb>
    goto bad;
  if(elf.magic != ELF_MAGIC)
80100b63:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b69:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6e:	0f 85 46 03 00 00    	jne    80100eba <exec+0x3be>
    goto bad;

  if((pgdir = setupkvm(kalloc)) == 0)
80100b74:	c7 04 24 f7 2a 10 80 	movl   $0x80102af7,(%esp)
80100b7b:	e8 19 75 00 00       	call   80108099 <setupkvm>
80100b80:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b83:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b87:	0f 84 30 03 00 00    	je     80100ebd <exec+0x3c1>
    goto bad;

  // Load program into memory.
  sz = 0;
80100b8d:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b94:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b9b:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100ba1:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100ba4:	e9 c5 00 00 00       	jmp    80100c6e <exec+0x172>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba9:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100bac:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bb3:	00 
80100bb4:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb8:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bbe:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bc2:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bc5:	89 04 24             	mov    %eax,(%esp)
80100bc8:	e8 91 11 00 00       	call   80101d5e <readi>
80100bcd:	83 f8 20             	cmp    $0x20,%eax
80100bd0:	0f 85 ea 02 00 00    	jne    80100ec0 <exec+0x3c4>
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
80100bd6:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bdc:	83 f8 01             	cmp    $0x1,%eax
80100bdf:	75 7f                	jne    80100c60 <exec+0x164>
      continue;
    if(ph.memsz < ph.filesz)
80100be1:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be7:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bed:	39 c2                	cmp    %eax,%edx
80100bef:	0f 82 ce 02 00 00    	jb     80100ec3 <exec+0x3c7>
      goto bad;
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 52 78 00 00       	call   8010846b <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	0f 84 a0 02 00 00    	je     80100ec6 <exec+0x3ca>
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c26:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2c:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c32:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c38:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3c:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c40:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c43:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4b:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4e:	89 04 24             	mov    %eax,(%esp)
80100c51:	e8 26 77 00 00       	call   8010837c <loaduvm>
80100c56:	85 c0                	test   %eax,%eax
80100c58:	0f 88 6b 02 00 00    	js     80100ec9 <exec+0x3cd>
80100c5e:	eb 01                	jmp    80100c61 <exec+0x165>
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
      continue;
80100c60:	90                   	nop
  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c61:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c65:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c68:	83 c0 20             	add    $0x20,%eax
80100c6b:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6e:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c75:	0f b7 c0             	movzwl %ax,%eax
80100c78:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7b:	0f 8f 28 ff ff ff    	jg     80100ba9 <exec+0xad>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c81:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c84:	89 04 24             	mov    %eax,(%esp)
80100c87:	e8 60 0e 00 00       	call   80101aec <iunlockput>
  ip = 0;
80100c8c:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c93:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c96:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca0:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ca6:	05 00 20 00 00       	add    $0x2000,%eax
80100cab:	89 44 24 08          	mov    %eax,0x8(%esp)
80100caf:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb2:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cb6:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cb9:	89 04 24             	mov    %eax,(%esp)
80100cbc:	e8 aa 77 00 00       	call   8010846b <allocuvm>
80100cc1:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc4:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100cc8:	0f 84 fe 01 00 00    	je     80100ecc <exec+0x3d0>
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cce:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd1:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cd6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cda:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cdd:	89 04 24             	mov    %eax,(%esp)
80100ce0:	e8 aa 79 00 00       	call   8010868f <clearpteu>
  sp = sz;
80100ce5:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ce8:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100ceb:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf2:	e9 81 00 00 00       	jmp    80100d78 <exec+0x27c>
    if(argc >= MAXARG)
80100cf7:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100cfb:	0f 87 ce 01 00 00    	ja     80100ecf <exec+0x3d3>
      goto bad;
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d01:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d04:	c1 e0 02             	shl    $0x2,%eax
80100d07:	03 45 0c             	add    0xc(%ebp),%eax
80100d0a:	8b 00                	mov    (%eax),%eax
80100d0c:	89 04 24             	mov    %eax,(%esp)
80100d0f:	e8 44 49 00 00       	call   80105658 <strlen>
80100d14:	f7 d0                	not    %eax
80100d16:	03 45 dc             	add    -0x24(%ebp),%eax
80100d19:	83 e0 fc             	and    $0xfffffffc,%eax
80100d1c:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d1f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d22:	c1 e0 02             	shl    $0x2,%eax
80100d25:	03 45 0c             	add    0xc(%ebp),%eax
80100d28:	8b 00                	mov    (%eax),%eax
80100d2a:	89 04 24             	mov    %eax,(%esp)
80100d2d:	e8 26 49 00 00       	call   80105658 <strlen>
80100d32:	83 c0 01             	add    $0x1,%eax
80100d35:	89 c2                	mov    %eax,%edx
80100d37:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d3a:	c1 e0 02             	shl    $0x2,%eax
80100d3d:	03 45 0c             	add    0xc(%ebp),%eax
80100d40:	8b 00                	mov    (%eax),%eax
80100d42:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d46:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d4a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d51:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d54:	89 04 24             	mov    %eax,(%esp)
80100d57:	e8 e7 7a 00 00       	call   80108843 <copyout>
80100d5c:	85 c0                	test   %eax,%eax
80100d5e:	0f 88 6e 01 00 00    	js     80100ed2 <exec+0x3d6>
      goto bad;
    ustack[3+argc] = sp;
80100d64:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d67:	8d 50 03             	lea    0x3(%eax),%edx
80100d6a:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6d:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d74:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d78:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d7b:	c1 e0 02             	shl    $0x2,%eax
80100d7e:	03 45 0c             	add    0xc(%ebp),%eax
80100d81:	8b 00                	mov    (%eax),%eax
80100d83:	85 c0                	test   %eax,%eax
80100d85:	0f 85 6c ff ff ff    	jne    80100cf7 <exec+0x1fb>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100d8b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d8e:	83 c0 03             	add    $0x3,%eax
80100d91:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100d98:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100d9c:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100da3:	ff ff ff 
  ustack[1] = argc;
80100da6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100da9:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 01             	add    $0x1,%eax
80100db5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100dbc:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100dbf:	29 d0                	sub    %edx,%eax
80100dc1:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100dc7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dca:	83 c0 04             	add    $0x4,%eax
80100dcd:	c1 e0 02             	shl    $0x2,%eax
80100dd0:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 04             	add    $0x4,%eax
80100dd9:	c1 e0 02             	shl    $0x2,%eax
80100ddc:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100de0:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100de6:	89 44 24 08          	mov    %eax,0x8(%esp)
80100dea:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100ded:	89 44 24 04          	mov    %eax,0x4(%esp)
80100df1:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100df4:	89 04 24             	mov    %eax,(%esp)
80100df7:	e8 47 7a 00 00       	call   80108843 <copyout>
80100dfc:	85 c0                	test   %eax,%eax
80100dfe:	0f 88 d1 00 00 00    	js     80100ed5 <exec+0x3d9>
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e04:	8b 45 08             	mov    0x8(%ebp),%eax
80100e07:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e0a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e0d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e10:	eb 17                	jmp    80100e29 <exec+0x32d>
    if(*s == '/')
80100e12:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e15:	0f b6 00             	movzbl (%eax),%eax
80100e18:	3c 2f                	cmp    $0x2f,%al
80100e1a:	75 09                	jne    80100e25 <exec+0x329>
      last = s+1;
80100e1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e1f:	83 c0 01             	add    $0x1,%eax
80100e22:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e25:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e2c:	0f b6 00             	movzbl (%eax),%eax
80100e2f:	84 c0                	test   %al,%al
80100e31:	75 df                	jne    80100e12 <exec+0x316>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e33:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e39:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e3c:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e43:	00 
80100e44:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e47:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e4b:	89 14 24             	mov    %edx,(%esp)
80100e4e:	e8 b7 47 00 00       	call   8010560a <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e53:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e59:	8b 40 04             	mov    0x4(%eax),%eax
80100e5c:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e5f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e65:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e68:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e6b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e71:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e74:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e76:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7c:	8b 40 18             	mov    0x18(%eax),%eax
80100e7f:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100e85:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100e88:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8e:	8b 40 18             	mov    0x18(%eax),%eax
80100e91:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100e94:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100e97:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e9d:	89 04 24             	mov    %eax,(%esp)
80100ea0:	e8 e5 72 00 00       	call   8010818a <switchuvm>
  freevm(oldpgdir);
80100ea5:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ea8:	89 04 24             	mov    %eax,(%esp)
80100eab:	e8 51 77 00 00       	call   80108601 <freevm>
  return 0;
80100eb0:	b8 00 00 00 00       	mov    $0x0,%eax
80100eb5:	eb 46                	jmp    80100efd <exec+0x401>
  ilock(ip);
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
    goto bad;
80100eb7:	90                   	nop
80100eb8:	eb 1c                	jmp    80100ed6 <exec+0x3da>
  if(elf.magic != ELF_MAGIC)
    goto bad;
80100eba:	90                   	nop
80100ebb:	eb 19                	jmp    80100ed6 <exec+0x3da>

  if((pgdir = setupkvm(kalloc)) == 0)
    goto bad;
80100ebd:	90                   	nop
80100ebe:	eb 16                	jmp    80100ed6 <exec+0x3da>

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
80100ec0:	90                   	nop
80100ec1:	eb 13                	jmp    80100ed6 <exec+0x3da>
    if(ph.type != ELF_PROG_LOAD)
      continue;
    if(ph.memsz < ph.filesz)
      goto bad;
80100ec3:	90                   	nop
80100ec4:	eb 10                	jmp    80100ed6 <exec+0x3da>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
80100ec6:	90                   	nop
80100ec7:	eb 0d                	jmp    80100ed6 <exec+0x3da>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
80100ec9:	90                   	nop
80100eca:	eb 0a                	jmp    80100ed6 <exec+0x3da>

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
    goto bad;
80100ecc:	90                   	nop
80100ecd:	eb 07                	jmp    80100ed6 <exec+0x3da>
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
    if(argc >= MAXARG)
      goto bad;
80100ecf:	90                   	nop
80100ed0:	eb 04                	jmp    80100ed6 <exec+0x3da>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
80100ed2:	90                   	nop
80100ed3:	eb 01                	jmp    80100ed6 <exec+0x3da>
  ustack[1] = argc;
  ustack[2] = sp - (argc+1)*4;  // argv pointer

  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;
80100ed5:	90                   	nop
  switchuvm(proc);
  freevm(oldpgdir);
  return 0;

 bad:
  if(pgdir)
80100ed6:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100eda:	74 0b                	je     80100ee7 <exec+0x3eb>
    freevm(pgdir);
80100edc:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100edf:	89 04 24             	mov    %eax,(%esp)
80100ee2:	e8 1a 77 00 00       	call   80108601 <freevm>
  if(ip)
80100ee7:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100eeb:	74 0b                	je     80100ef8 <exec+0x3fc>
    iunlockput(ip);
80100eed:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef0:	89 04 24             	mov    %eax,(%esp)
80100ef3:	e8 f4 0b 00 00       	call   80101aec <iunlockput>
  return -1;
80100ef8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100efd:	c9                   	leave  
80100efe:	c3                   	ret    
	...

80100f00 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f00:	55                   	push   %ebp
80100f01:	89 e5                	mov    %esp,%ebp
80100f03:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f06:	c7 44 24 04 49 89 10 	movl   $0x80108949,0x4(%esp)
80100f0d:	80 
80100f0e:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100f15:	e8 18 42 00 00       	call   80105132 <initlock>
}
80100f1a:	c9                   	leave  
80100f1b:	c3                   	ret    

80100f1c <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f1c:	55                   	push   %ebp
80100f1d:	89 e5                	mov    %esp,%ebp
80100f1f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f22:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100f29:	e8 25 42 00 00       	call   80105153 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f2e:	c7 45 f4 94 ee 10 80 	movl   $0x8010ee94,-0xc(%ebp)
80100f35:	eb 29                	jmp    80100f60 <filealloc+0x44>
    if(f->ref == 0){
80100f37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f3a:	8b 40 04             	mov    0x4(%eax),%eax
80100f3d:	85 c0                	test   %eax,%eax
80100f3f:	75 1b                	jne    80100f5c <filealloc+0x40>
      f->ref = 1;
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f4b:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100f52:	e8 97 42 00 00       	call   801051ee <release>
      return f;
80100f57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f5a:	eb 1e                	jmp    80100f7a <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f5c:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f60:	81 7d f4 f4 f7 10 80 	cmpl   $0x8010f7f4,-0xc(%ebp)
80100f67:	72 ce                	jb     80100f37 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f69:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100f70:	e8 79 42 00 00       	call   801051ee <release>
  return 0;
80100f75:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f7a:	c9                   	leave  
80100f7b:	c3                   	ret    

80100f7c <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f7c:	55                   	push   %ebp
80100f7d:	89 e5                	mov    %esp,%ebp
80100f7f:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f82:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100f89:	e8 c5 41 00 00       	call   80105153 <acquire>
  if(f->ref < 1)
80100f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80100f91:	8b 40 04             	mov    0x4(%eax),%eax
80100f94:	85 c0                	test   %eax,%eax
80100f96:	7f 0c                	jg     80100fa4 <filedup+0x28>
    panic("filedup");
80100f98:	c7 04 24 50 89 10 80 	movl   $0x80108950,(%esp)
80100f9f:	e8 99 f5 ff ff       	call   8010053d <panic>
  f->ref++;
80100fa4:	8b 45 08             	mov    0x8(%ebp),%eax
80100fa7:	8b 40 04             	mov    0x4(%eax),%eax
80100faa:	8d 50 01             	lea    0x1(%eax),%edx
80100fad:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb0:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fb3:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100fba:	e8 2f 42 00 00       	call   801051ee <release>
  return f;
80100fbf:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fc2:	c9                   	leave  
80100fc3:	c3                   	ret    

80100fc4 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fc4:	55                   	push   %ebp
80100fc5:	89 e5                	mov    %esp,%ebp
80100fc7:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fca:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80100fd1:	e8 7d 41 00 00       	call   80105153 <acquire>
  if(f->ref < 1)
80100fd6:	8b 45 08             	mov    0x8(%ebp),%eax
80100fd9:	8b 40 04             	mov    0x4(%eax),%eax
80100fdc:	85 c0                	test   %eax,%eax
80100fde:	7f 0c                	jg     80100fec <fileclose+0x28>
    panic("fileclose");
80100fe0:	c7 04 24 58 89 10 80 	movl   $0x80108958,(%esp)
80100fe7:	e8 51 f5 ff ff       	call   8010053d <panic>
  if(--f->ref > 0){
80100fec:	8b 45 08             	mov    0x8(%ebp),%eax
80100fef:	8b 40 04             	mov    0x4(%eax),%eax
80100ff2:	8d 50 ff             	lea    -0x1(%eax),%edx
80100ff5:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff8:	89 50 04             	mov    %edx,0x4(%eax)
80100ffb:	8b 45 08             	mov    0x8(%ebp),%eax
80100ffe:	8b 40 04             	mov    0x4(%eax),%eax
80101001:	85 c0                	test   %eax,%eax
80101003:	7e 11                	jle    80101016 <fileclose+0x52>
    release(&ftable.lock);
80101005:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
8010100c:	e8 dd 41 00 00       	call   801051ee <release>
    return;
80101011:	e9 82 00 00 00       	jmp    80101098 <fileclose+0xd4>
  }
  ff = *f;
80101016:	8b 45 08             	mov    0x8(%ebp),%eax
80101019:	8b 10                	mov    (%eax),%edx
8010101b:	89 55 e0             	mov    %edx,-0x20(%ebp)
8010101e:	8b 50 04             	mov    0x4(%eax),%edx
80101021:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80101024:	8b 50 08             	mov    0x8(%eax),%edx
80101027:	89 55 e8             	mov    %edx,-0x18(%ebp)
8010102a:	8b 50 0c             	mov    0xc(%eax),%edx
8010102d:	89 55 ec             	mov    %edx,-0x14(%ebp)
80101030:	8b 50 10             	mov    0x10(%eax),%edx
80101033:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101036:	8b 40 14             	mov    0x14(%eax),%eax
80101039:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
8010103c:	8b 45 08             	mov    0x8(%ebp),%eax
8010103f:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
8010104f:	c7 04 24 60 ee 10 80 	movl   $0x8010ee60,(%esp)
80101056:	e8 93 41 00 00       	call   801051ee <release>
  
  if(ff.type == FD_PIPE)
8010105b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010105e:	83 f8 01             	cmp    $0x1,%eax
80101061:	75 18                	jne    8010107b <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
80101063:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101067:	0f be d0             	movsbl %al,%edx
8010106a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010106d:	89 54 24 04          	mov    %edx,0x4(%esp)
80101071:	89 04 24             	mov    %eax,(%esp)
80101074:	e8 fa 2c 00 00       	call   80103d73 <pipeclose>
80101079:	eb 1d                	jmp    80101098 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
8010107b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010107e:	83 f8 02             	cmp    $0x2,%eax
80101081:	75 15                	jne    80101098 <fileclose+0xd4>
    begin_trans();
80101083:	e8 8d 21 00 00       	call   80103215 <begin_trans>
    iput(ff.ip);
80101088:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010108b:	89 04 24             	mov    %eax,(%esp)
8010108e:	e8 88 09 00 00       	call   80101a1b <iput>
    commit_trans();
80101093:	e8 c6 21 00 00       	call   8010325e <commit_trans>
  }
}
80101098:	c9                   	leave  
80101099:	c3                   	ret    

8010109a <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
8010109a:	55                   	push   %ebp
8010109b:	89 e5                	mov    %esp,%ebp
8010109d:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010a0:	8b 45 08             	mov    0x8(%ebp),%eax
801010a3:	8b 00                	mov    (%eax),%eax
801010a5:	83 f8 02             	cmp    $0x2,%eax
801010a8:	75 38                	jne    801010e2 <filestat+0x48>
    ilock(f->ip);
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 40 10             	mov    0x10(%eax),%eax
801010b0:	89 04 24             	mov    %eax,(%esp)
801010b3:	e8 b0 07 00 00       	call   80101868 <ilock>
    stati(f->ip, st);
801010b8:	8b 45 08             	mov    0x8(%ebp),%eax
801010bb:	8b 40 10             	mov    0x10(%eax),%eax
801010be:	8b 55 0c             	mov    0xc(%ebp),%edx
801010c1:	89 54 24 04          	mov    %edx,0x4(%esp)
801010c5:	89 04 24             	mov    %eax,(%esp)
801010c8:	e8 4c 0c 00 00       	call   80101d19 <stati>
    iunlock(f->ip);
801010cd:	8b 45 08             	mov    0x8(%ebp),%eax
801010d0:	8b 40 10             	mov    0x10(%eax),%eax
801010d3:	89 04 24             	mov    %eax,(%esp)
801010d6:	e8 db 08 00 00       	call   801019b6 <iunlock>
    return 0;
801010db:	b8 00 00 00 00       	mov    $0x0,%eax
801010e0:	eb 05                	jmp    801010e7 <filestat+0x4d>
  }
  return -1;
801010e2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010e7:	c9                   	leave  
801010e8:	c3                   	ret    

801010e9 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010e9:	55                   	push   %ebp
801010ea:	89 e5                	mov    %esp,%ebp
801010ec:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010ef:	8b 45 08             	mov    0x8(%ebp),%eax
801010f2:	0f b6 40 08          	movzbl 0x8(%eax),%eax
801010f6:	84 c0                	test   %al,%al
801010f8:	75 0a                	jne    80101104 <fileread+0x1b>
    return -1;
801010fa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801010ff:	e9 9f 00 00 00       	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_PIPE)
80101104:	8b 45 08             	mov    0x8(%ebp),%eax
80101107:	8b 00                	mov    (%eax),%eax
80101109:	83 f8 01             	cmp    $0x1,%eax
8010110c:	75 1e                	jne    8010112c <fileread+0x43>
    return piperead(f->pipe, addr, n);
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 40 0c             	mov    0xc(%eax),%eax
80101114:	8b 55 10             	mov    0x10(%ebp),%edx
80101117:	89 54 24 08          	mov    %edx,0x8(%esp)
8010111b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010111e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101122:	89 04 24             	mov    %eax,(%esp)
80101125:	e8 cb 2d 00 00       	call   80103ef5 <piperead>
8010112a:	eb 77                	jmp    801011a3 <fileread+0xba>
  if(f->type == FD_INODE){
8010112c:	8b 45 08             	mov    0x8(%ebp),%eax
8010112f:	8b 00                	mov    (%eax),%eax
80101131:	83 f8 02             	cmp    $0x2,%eax
80101134:	75 61                	jne    80101197 <fileread+0xae>
    ilock(f->ip);
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 40 10             	mov    0x10(%eax),%eax
8010113c:	89 04 24             	mov    %eax,(%esp)
8010113f:	e8 24 07 00 00       	call   80101868 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
80101144:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101147:	8b 45 08             	mov    0x8(%ebp),%eax
8010114a:	8b 50 14             	mov    0x14(%eax),%edx
8010114d:	8b 45 08             	mov    0x8(%ebp),%eax
80101150:	8b 40 10             	mov    0x10(%eax),%eax
80101153:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101157:	89 54 24 08          	mov    %edx,0x8(%esp)
8010115b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010115e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101162:	89 04 24             	mov    %eax,(%esp)
80101165:	e8 f4 0b 00 00       	call   80101d5e <readi>
8010116a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010116d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101171:	7e 11                	jle    80101184 <fileread+0x9b>
      f->off += r;
80101173:	8b 45 08             	mov    0x8(%ebp),%eax
80101176:	8b 50 14             	mov    0x14(%eax),%edx
80101179:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010117c:	01 c2                	add    %eax,%edx
8010117e:	8b 45 08             	mov    0x8(%ebp),%eax
80101181:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
80101184:	8b 45 08             	mov    0x8(%ebp),%eax
80101187:	8b 40 10             	mov    0x10(%eax),%eax
8010118a:	89 04 24             	mov    %eax,(%esp)
8010118d:	e8 24 08 00 00       	call   801019b6 <iunlock>
    return r;
80101192:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101195:	eb 0c                	jmp    801011a3 <fileread+0xba>
  }
  panic("fileread");
80101197:	c7 04 24 62 89 10 80 	movl   $0x80108962,(%esp)
8010119e:	e8 9a f3 ff ff       	call   8010053d <panic>
}
801011a3:	c9                   	leave  
801011a4:	c3                   	ret    

801011a5 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011a5:	55                   	push   %ebp
801011a6:	89 e5                	mov    %esp,%ebp
801011a8:	53                   	push   %ebx
801011a9:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011ac:	8b 45 08             	mov    0x8(%ebp),%eax
801011af:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011b3:	84 c0                	test   %al,%al
801011b5:	75 0a                	jne    801011c1 <filewrite+0x1c>
    return -1;
801011b7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011bc:	e9 23 01 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_PIPE)
801011c1:	8b 45 08             	mov    0x8(%ebp),%eax
801011c4:	8b 00                	mov    (%eax),%eax
801011c6:	83 f8 01             	cmp    $0x1,%eax
801011c9:	75 21                	jne    801011ec <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 40 0c             	mov    0xc(%eax),%eax
801011d1:	8b 55 10             	mov    0x10(%ebp),%edx
801011d4:	89 54 24 08          	mov    %edx,0x8(%esp)
801011d8:	8b 55 0c             	mov    0xc(%ebp),%edx
801011db:	89 54 24 04          	mov    %edx,0x4(%esp)
801011df:	89 04 24             	mov    %eax,(%esp)
801011e2:	e8 1e 2c 00 00       	call   80103e05 <pipewrite>
801011e7:	e9 f8 00 00 00       	jmp    801012e4 <filewrite+0x13f>
  if(f->type == FD_INODE){
801011ec:	8b 45 08             	mov    0x8(%ebp),%eax
801011ef:	8b 00                	mov    (%eax),%eax
801011f1:	83 f8 02             	cmp    $0x2,%eax
801011f4:	0f 85 de 00 00 00    	jne    801012d8 <filewrite+0x133>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
801011fa:	c7 45 ec 00 06 00 00 	movl   $0x600,-0x14(%ebp)
    int i = 0;
80101201:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101208:	e9 a8 00 00 00       	jmp    801012b5 <filewrite+0x110>
      int n1 = n - i;
8010120d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101210:	8b 55 10             	mov    0x10(%ebp),%edx
80101213:	89 d1                	mov    %edx,%ecx
80101215:	29 c1                	sub    %eax,%ecx
80101217:	89 c8                	mov    %ecx,%eax
80101219:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
8010121c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010121f:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80101222:	7e 06                	jle    8010122a <filewrite+0x85>
        n1 = max;
80101224:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101227:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_trans();
8010122a:	e8 e6 1f 00 00       	call   80103215 <begin_trans>
      ilock(f->ip);
8010122f:	8b 45 08             	mov    0x8(%ebp),%eax
80101232:	8b 40 10             	mov    0x10(%eax),%eax
80101235:	89 04 24             	mov    %eax,(%esp)
80101238:	e8 2b 06 00 00       	call   80101868 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
8010123d:	8b 5d f0             	mov    -0x10(%ebp),%ebx
80101240:	8b 45 08             	mov    0x8(%ebp),%eax
80101243:	8b 48 14             	mov    0x14(%eax),%ecx
80101246:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101249:	89 c2                	mov    %eax,%edx
8010124b:	03 55 0c             	add    0xc(%ebp),%edx
8010124e:	8b 45 08             	mov    0x8(%ebp),%eax
80101251:	8b 40 10             	mov    0x10(%eax),%eax
80101254:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80101258:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010125c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101260:	89 04 24             	mov    %eax,(%esp)
80101263:	e8 61 0c 00 00       	call   80101ec9 <writei>
80101268:	89 45 e8             	mov    %eax,-0x18(%ebp)
8010126b:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010126f:	7e 11                	jle    80101282 <filewrite+0xdd>
        f->off += r;
80101271:	8b 45 08             	mov    0x8(%ebp),%eax
80101274:	8b 50 14             	mov    0x14(%eax),%edx
80101277:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010127a:	01 c2                	add    %eax,%edx
8010127c:	8b 45 08             	mov    0x8(%ebp),%eax
8010127f:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
80101282:	8b 45 08             	mov    0x8(%ebp),%eax
80101285:	8b 40 10             	mov    0x10(%eax),%eax
80101288:	89 04 24             	mov    %eax,(%esp)
8010128b:	e8 26 07 00 00       	call   801019b6 <iunlock>
      commit_trans();
80101290:	e8 c9 1f 00 00       	call   8010325e <commit_trans>

      if(r < 0)
80101295:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101299:	78 28                	js     801012c3 <filewrite+0x11e>
        break;
      if(r != n1)
8010129b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010129e:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012a1:	74 0c                	je     801012af <filewrite+0x10a>
        panic("short filewrite");
801012a3:	c7 04 24 6b 89 10 80 	movl   $0x8010896b,(%esp)
801012aa:	e8 8e f2 ff ff       	call   8010053d <panic>
      i += r;
801012af:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012b2:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012b8:	3b 45 10             	cmp    0x10(%ebp),%eax
801012bb:	0f 8c 4c ff ff ff    	jl     8010120d <filewrite+0x68>
801012c1:	eb 01                	jmp    801012c4 <filewrite+0x11f>
        f->off += r;
      iunlock(f->ip);
      commit_trans();

      if(r < 0)
        break;
801012c3:	90                   	nop
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c7:	3b 45 10             	cmp    0x10(%ebp),%eax
801012ca:	75 05                	jne    801012d1 <filewrite+0x12c>
801012cc:	8b 45 10             	mov    0x10(%ebp),%eax
801012cf:	eb 05                	jmp    801012d6 <filewrite+0x131>
801012d1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012d6:	eb 0c                	jmp    801012e4 <filewrite+0x13f>
  }
  panic("filewrite");
801012d8:	c7 04 24 7b 89 10 80 	movl   $0x8010897b,(%esp)
801012df:	e8 59 f2 ff ff       	call   8010053d <panic>
}
801012e4:	83 c4 24             	add    $0x24,%esp
801012e7:	5b                   	pop    %ebx
801012e8:	5d                   	pop    %ebp
801012e9:	c3                   	ret    
	...

801012ec <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012ec:	55                   	push   %ebp
801012ed:	89 e5                	mov    %esp,%ebp
801012ef:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f2:	8b 45 08             	mov    0x8(%ebp),%eax
801012f5:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801012fc:	00 
801012fd:	89 04 24             	mov    %eax,(%esp)
80101300:	e8 a1 ee ff ff       	call   801001a6 <bread>
80101305:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
80101308:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010130b:	83 c0 18             	add    $0x18,%eax
8010130e:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80101315:	00 
80101316:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010131d:	89 04 24             	mov    %eax,(%esp)
80101320:	e8 88 41 00 00       	call   801054ad <memmove>
  brelse(bp);
80101325:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101328:	89 04 24             	mov    %eax,(%esp)
8010132b:	e8 e7 ee ff ff       	call   80100217 <brelse>
}
80101330:	c9                   	leave  
80101331:	c3                   	ret    

80101332 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101332:	55                   	push   %ebp
80101333:	89 e5                	mov    %esp,%ebp
80101335:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
80101338:	8b 55 0c             	mov    0xc(%ebp),%edx
8010133b:	8b 45 08             	mov    0x8(%ebp),%eax
8010133e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101342:	89 04 24             	mov    %eax,(%esp)
80101345:	e8 5c ee ff ff       	call   801001a6 <bread>
8010134a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
8010134d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101350:	83 c0 18             	add    $0x18,%eax
80101353:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135a:	00 
8010135b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101362:	00 
80101363:	89 04 24             	mov    %eax,(%esp)
80101366:	e8 6f 40 00 00       	call   801053da <memset>
  log_write(bp);
8010136b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010136e:	89 04 24             	mov    %eax,(%esp)
80101371:	e8 40 1f 00 00       	call   801032b6 <log_write>
  brelse(bp);
80101376:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101379:	89 04 24             	mov    %eax,(%esp)
8010137c:	e8 96 ee ff ff       	call   80100217 <brelse>
}
80101381:	c9                   	leave  
80101382:	c3                   	ret    

80101383 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101383:	55                   	push   %ebp
80101384:	89 e5                	mov    %esp,%ebp
80101386:	53                   	push   %ebx
80101387:	83 ec 34             	sub    $0x34,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138a:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101391:	8b 45 08             	mov    0x8(%ebp),%eax
80101394:	8d 55 d8             	lea    -0x28(%ebp),%edx
80101397:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139b:	89 04 24             	mov    %eax,(%esp)
8010139e:	e8 49 ff ff ff       	call   801012ec <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013aa:	e9 11 01 00 00       	jmp    801014c0 <balloc+0x13d>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013af:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b2:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013b8:	85 c0                	test   %eax,%eax
801013ba:	0f 48 c2             	cmovs  %edx,%eax
801013bd:	c1 f8 0c             	sar    $0xc,%eax
801013c0:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c3:	c1 ea 03             	shr    $0x3,%edx
801013c6:	01 d0                	add    %edx,%eax
801013c8:	83 c0 03             	add    $0x3,%eax
801013cb:	89 44 24 04          	mov    %eax,0x4(%esp)
801013cf:	8b 45 08             	mov    0x8(%ebp),%eax
801013d2:	89 04 24             	mov    %eax,(%esp)
801013d5:	e8 cc ed ff ff       	call   801001a6 <bread>
801013da:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013dd:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e4:	e9 a7 00 00 00       	jmp    80101490 <balloc+0x10d>
      m = 1 << (bi % 8);
801013e9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013ec:	89 c2                	mov    %eax,%edx
801013ee:	c1 fa 1f             	sar    $0x1f,%edx
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 d3                	mov    %edx,%ebx
80101402:	89 c1                	mov    %eax,%ecx
80101404:	d3 e3                	shl    %cl,%ebx
80101406:	89 d8                	mov    %ebx,%eax
80101408:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
8010140b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140e:	8d 50 07             	lea    0x7(%eax),%edx
80101411:	85 c0                	test   %eax,%eax
80101413:	0f 48 c2             	cmovs  %edx,%eax
80101416:	c1 f8 03             	sar    $0x3,%eax
80101419:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141c:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101421:	0f b6 c0             	movzbl %al,%eax
80101424:	23 45 e8             	and    -0x18(%ebp),%eax
80101427:	85 c0                	test   %eax,%eax
80101429:	75 61                	jne    8010148c <balloc+0x109>
        bp->data[bi/8] |= m;  // Mark block in use.
8010142b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142e:	8d 50 07             	lea    0x7(%eax),%edx
80101431:	85 c0                	test   %eax,%eax
80101433:	0f 48 c2             	cmovs  %edx,%eax
80101436:	c1 f8 03             	sar    $0x3,%eax
80101439:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143c:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101441:	89 d1                	mov    %edx,%ecx
80101443:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101446:	09 ca                	or     %ecx,%edx
80101448:	89 d1                	mov    %edx,%ecx
8010144a:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144d:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
80101451:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101454:	89 04 24             	mov    %eax,(%esp)
80101457:	e8 5a 1e 00 00       	call   801032b6 <log_write>
        brelse(bp);
8010145c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145f:	89 04 24             	mov    %eax,(%esp)
80101462:	e8 b0 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101467:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010146a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146d:	01 c2                	add    %eax,%edx
8010146f:	8b 45 08             	mov    0x8(%ebp),%eax
80101472:	89 54 24 04          	mov    %edx,0x4(%esp)
80101476:	89 04 24             	mov    %eax,(%esp)
80101479:	e8 b4 fe ff ff       	call   80101332 <bzero>
        return b + bi;
8010147e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101481:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101484:	01 d0                	add    %edx,%eax
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
}
80101486:	83 c4 34             	add    $0x34,%esp
80101489:	5b                   	pop    %ebx
8010148a:	5d                   	pop    %ebp
8010148b:	c3                   	ret    

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010148c:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101490:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101497:	7f 15                	jg     801014ae <balloc+0x12b>
80101499:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010149c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010149f:	01 d0                	add    %edx,%eax
801014a1:	89 c2                	mov    %eax,%edx
801014a3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a6:	39 c2                	cmp    %eax,%edx
801014a8:	0f 82 3b ff ff ff    	jb     801013e9 <balloc+0x66>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014ae:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014b1:	89 04 24             	mov    %eax,(%esp)
801014b4:	e8 5e ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b9:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014c0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014c3:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c6:	39 c2                	cmp    %eax,%edx
801014c8:	0f 82 e1 fe ff ff    	jb     801013af <balloc+0x2c>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014ce:	c7 04 24 85 89 10 80 	movl   $0x80108985,(%esp)
801014d5:	e8 63 f0 ff ff       	call   8010053d <panic>

801014da <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014da:	55                   	push   %ebp
801014db:	89 e5                	mov    %esp,%ebp
801014dd:	53                   	push   %ebx
801014de:	83 ec 34             	sub    $0x34,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014e1:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014e4:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e8:	8b 45 08             	mov    0x8(%ebp),%eax
801014eb:	89 04 24             	mov    %eax,(%esp)
801014ee:	e8 f9 fd ff ff       	call   801012ec <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014f3:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f6:	89 c2                	mov    %eax,%edx
801014f8:	c1 ea 0c             	shr    $0xc,%edx
801014fb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014fe:	c1 e8 03             	shr    $0x3,%eax
80101501:	01 d0                	add    %edx,%eax
80101503:	8d 50 03             	lea    0x3(%eax),%edx
80101506:	8b 45 08             	mov    0x8(%ebp),%eax
80101509:	89 54 24 04          	mov    %edx,0x4(%esp)
8010150d:	89 04 24             	mov    %eax,(%esp)
80101510:	e8 91 ec ff ff       	call   801001a6 <bread>
80101515:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101518:	8b 45 0c             	mov    0xc(%ebp),%eax
8010151b:	25 ff 0f 00 00       	and    $0xfff,%eax
80101520:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
80101523:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101526:	89 c2                	mov    %eax,%edx
80101528:	c1 fa 1f             	sar    $0x1f,%edx
8010152b:	c1 ea 1d             	shr    $0x1d,%edx
8010152e:	01 d0                	add    %edx,%eax
80101530:	83 e0 07             	and    $0x7,%eax
80101533:	29 d0                	sub    %edx,%eax
80101535:	ba 01 00 00 00       	mov    $0x1,%edx
8010153a:	89 d3                	mov    %edx,%ebx
8010153c:	89 c1                	mov    %eax,%ecx
8010153e:	d3 e3                	shl    %cl,%ebx
80101540:	89 d8                	mov    %ebx,%eax
80101542:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
80101545:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101548:	8d 50 07             	lea    0x7(%eax),%edx
8010154b:	85 c0                	test   %eax,%eax
8010154d:	0f 48 c2             	cmovs  %edx,%eax
80101550:	c1 f8 03             	sar    $0x3,%eax
80101553:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101556:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010155b:	0f b6 c0             	movzbl %al,%eax
8010155e:	23 45 ec             	and    -0x14(%ebp),%eax
80101561:	85 c0                	test   %eax,%eax
80101563:	75 0c                	jne    80101571 <bfree+0x97>
    panic("freeing free block");
80101565:	c7 04 24 9b 89 10 80 	movl   $0x8010899b,(%esp)
8010156c:	e8 cc ef ff ff       	call   8010053d <panic>
  bp->data[bi/8] &= ~m;
80101571:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101574:	8d 50 07             	lea    0x7(%eax),%edx
80101577:	85 c0                	test   %eax,%eax
80101579:	0f 48 c2             	cmovs  %edx,%eax
8010157c:	c1 f8 03             	sar    $0x3,%eax
8010157f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101582:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101587:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010158a:	f7 d1                	not    %ecx
8010158c:	21 ca                	and    %ecx,%edx
8010158e:	89 d1                	mov    %edx,%ecx
80101590:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101593:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 14 1d 00 00       	call   801032b6 <log_write>
  brelse(bp);
801015a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015a5:	89 04 24             	mov    %eax,(%esp)
801015a8:	e8 6a ec ff ff       	call   80100217 <brelse>
}
801015ad:	83 c4 34             	add    $0x34,%esp
801015b0:	5b                   	pop    %ebx
801015b1:	5d                   	pop    %ebp
801015b2:	c3                   	ret    

801015b3 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015b3:	55                   	push   %ebp
801015b4:	89 e5                	mov    %esp,%ebp
801015b6:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015b9:	c7 44 24 04 ae 89 10 	movl   $0x801089ae,0x4(%esp)
801015c0:	80 
801015c1:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
801015c8:	e8 65 3b 00 00       	call   80105132 <initlock>
}
801015cd:	c9                   	leave  
801015ce:	c3                   	ret    

801015cf <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015cf:	55                   	push   %ebp
801015d0:	89 e5                	mov    %esp,%ebp
801015d2:	83 ec 48             	sub    $0x48,%esp
801015d5:	8b 45 0c             	mov    0xc(%ebp),%eax
801015d8:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015dc:	8b 45 08             	mov    0x8(%ebp),%eax
801015df:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015e2:	89 54 24 04          	mov    %edx,0x4(%esp)
801015e6:	89 04 24             	mov    %eax,(%esp)
801015e9:	e8 fe fc ff ff       	call   801012ec <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015ee:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015f5:	e9 98 00 00 00       	jmp    80101692 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015fd:	c1 e8 03             	shr    $0x3,%eax
80101600:	83 c0 02             	add    $0x2,%eax
80101603:	89 44 24 04          	mov    %eax,0x4(%esp)
80101607:	8b 45 08             	mov    0x8(%ebp),%eax
8010160a:	89 04 24             	mov    %eax,(%esp)
8010160d:	e8 94 eb ff ff       	call   801001a6 <bread>
80101612:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101615:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101618:	8d 50 18             	lea    0x18(%eax),%edx
8010161b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010161e:	83 e0 07             	and    $0x7,%eax
80101621:	c1 e0 06             	shl    $0x6,%eax
80101624:	01 d0                	add    %edx,%eax
80101626:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
80101629:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010162c:	0f b7 00             	movzwl (%eax),%eax
8010162f:	66 85 c0             	test   %ax,%ax
80101632:	75 4f                	jne    80101683 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101634:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010163b:	00 
8010163c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101643:	00 
80101644:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101647:	89 04 24             	mov    %eax,(%esp)
8010164a:	e8 8b 3d 00 00       	call   801053da <memset>
      dip->type = type;
8010164f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101652:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101656:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
80101659:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010165c:	89 04 24             	mov    %eax,(%esp)
8010165f:	e8 52 1c 00 00       	call   801032b6 <log_write>
      brelse(bp);
80101664:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101667:	89 04 24             	mov    %eax,(%esp)
8010166a:	e8 a8 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
8010166f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101672:	89 44 24 04          	mov    %eax,0x4(%esp)
80101676:	8b 45 08             	mov    0x8(%ebp),%eax
80101679:	89 04 24             	mov    %eax,(%esp)
8010167c:	e8 e3 00 00 00       	call   80101764 <iget>
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
}
80101681:	c9                   	leave  
80101682:	c3                   	ret    
      dip->type = type;
      log_write(bp);   // mark it allocated on the disk
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
80101683:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101686:	89 04 24             	mov    %eax,(%esp)
80101689:	e8 89 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010168e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101692:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101695:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101698:	39 c2                	cmp    %eax,%edx
8010169a:	0f 82 5a ff ff ff    	jb     801015fa <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
801016a0:	c7 04 24 b5 89 10 80 	movl   $0x801089b5,(%esp)
801016a7:	e8 91 ee ff ff       	call   8010053d <panic>

801016ac <iupdate>:
}

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
801016ac:	55                   	push   %ebp
801016ad:	89 e5                	mov    %esp,%ebp
801016af:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016b2:	8b 45 08             	mov    0x8(%ebp),%eax
801016b5:	8b 40 04             	mov    0x4(%eax),%eax
801016b8:	c1 e8 03             	shr    $0x3,%eax
801016bb:	8d 50 02             	lea    0x2(%eax),%edx
801016be:	8b 45 08             	mov    0x8(%ebp),%eax
801016c1:	8b 00                	mov    (%eax),%eax
801016c3:	89 54 24 04          	mov    %edx,0x4(%esp)
801016c7:	89 04 24             	mov    %eax,(%esp)
801016ca:	e8 d7 ea ff ff       	call   801001a6 <bread>
801016cf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016d5:	8d 50 18             	lea    0x18(%eax),%edx
801016d8:	8b 45 08             	mov    0x8(%ebp),%eax
801016db:	8b 40 04             	mov    0x4(%eax),%eax
801016de:	83 e0 07             	and    $0x7,%eax
801016e1:	c1 e0 06             	shl    $0x6,%eax
801016e4:	01 d0                	add    %edx,%eax
801016e6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016f6:	8b 45 08             	mov    0x8(%ebp),%eax
801016f9:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101700:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
80101704:	8b 45 08             	mov    0x8(%ebp),%eax
80101707:	0f b7 50 14          	movzwl 0x14(%eax),%edx
8010170b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170e:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101712:	8b 45 08             	mov    0x8(%ebp),%eax
80101715:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101720:	8b 45 08             	mov    0x8(%ebp),%eax
80101723:	8b 50 18             	mov    0x18(%eax),%edx
80101726:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101729:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010172c:	8b 45 08             	mov    0x8(%ebp),%eax
8010172f:	8d 50 1c             	lea    0x1c(%eax),%edx
80101732:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101735:	83 c0 0c             	add    $0xc,%eax
80101738:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
8010173f:	00 
80101740:	89 54 24 04          	mov    %edx,0x4(%esp)
80101744:	89 04 24             	mov    %eax,(%esp)
80101747:	e8 61 3d 00 00       	call   801054ad <memmove>
  log_write(bp);
8010174c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174f:	89 04 24             	mov    %eax,(%esp)
80101752:	e8 5f 1b 00 00       	call   801032b6 <log_write>
  brelse(bp);
80101757:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010175a:	89 04 24             	mov    %eax,(%esp)
8010175d:	e8 b5 ea ff ff       	call   80100217 <brelse>
}
80101762:	c9                   	leave  
80101763:	c3                   	ret    

80101764 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101764:	55                   	push   %ebp
80101765:	89 e5                	mov    %esp,%ebp
80101767:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010176a:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101771:	e8 dd 39 00 00       	call   80105153 <acquire>

  // Is the inode already cached?
  empty = 0;
80101776:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010177d:	c7 45 f4 94 f8 10 80 	movl   $0x8010f894,-0xc(%ebp)
80101784:	eb 59                	jmp    801017df <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101786:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101789:	8b 40 08             	mov    0x8(%eax),%eax
8010178c:	85 c0                	test   %eax,%eax
8010178e:	7e 35                	jle    801017c5 <iget+0x61>
80101790:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101793:	8b 00                	mov    (%eax),%eax
80101795:	3b 45 08             	cmp    0x8(%ebp),%eax
80101798:	75 2b                	jne    801017c5 <iget+0x61>
8010179a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179d:	8b 40 04             	mov    0x4(%eax),%eax
801017a0:	3b 45 0c             	cmp    0xc(%ebp),%eax
801017a3:	75 20                	jne    801017c5 <iget+0x61>
      ip->ref++;
801017a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a8:	8b 40 08             	mov    0x8(%eax),%eax
801017ab:	8d 50 01             	lea    0x1(%eax),%edx
801017ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b1:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017b4:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
801017bb:	e8 2e 3a 00 00       	call   801051ee <release>
      return ip;
801017c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c3:	eb 6f                	jmp    80101834 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017c5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017c9:	75 10                	jne    801017db <iget+0x77>
801017cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017ce:	8b 40 08             	mov    0x8(%eax),%eax
801017d1:	85 c0                	test   %eax,%eax
801017d3:	75 06                	jne    801017db <iget+0x77>
      empty = ip;
801017d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017d8:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017db:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017df:	81 7d f4 34 08 11 80 	cmpl   $0x80110834,-0xc(%ebp)
801017e6:	72 9e                	jb     80101786 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017e8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017ec:	75 0c                	jne    801017fa <iget+0x96>
    panic("iget: no inodes");
801017ee:	c7 04 24 c7 89 10 80 	movl   $0x801089c7,(%esp)
801017f5:	e8 43 ed ff ff       	call   8010053d <panic>

  ip = empty;
801017fa:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017fd:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
80101800:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101803:	8b 55 08             	mov    0x8(%ebp),%edx
80101806:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
80101808:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010180b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010180e:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101811:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101814:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010181b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010181e:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101825:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
8010182c:	e8 bd 39 00 00       	call   801051ee <release>

  return ip;
80101831:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101834:	c9                   	leave  
80101835:	c3                   	ret    

80101836 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101836:	55                   	push   %ebp
80101837:	89 e5                	mov    %esp,%ebp
80101839:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010183c:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101843:	e8 0b 39 00 00       	call   80105153 <acquire>
  ip->ref++;
80101848:	8b 45 08             	mov    0x8(%ebp),%eax
8010184b:	8b 40 08             	mov    0x8(%eax),%eax
8010184e:	8d 50 01             	lea    0x1(%eax),%edx
80101851:	8b 45 08             	mov    0x8(%ebp),%eax
80101854:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101857:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
8010185e:	e8 8b 39 00 00       	call   801051ee <release>
  return ip;
80101863:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101866:	c9                   	leave  
80101867:	c3                   	ret    

80101868 <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
80101868:	55                   	push   %ebp
80101869:	89 e5                	mov    %esp,%ebp
8010186b:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
8010186e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101872:	74 0a                	je     8010187e <ilock+0x16>
80101874:	8b 45 08             	mov    0x8(%ebp),%eax
80101877:	8b 40 08             	mov    0x8(%eax),%eax
8010187a:	85 c0                	test   %eax,%eax
8010187c:	7f 0c                	jg     8010188a <ilock+0x22>
    panic("ilock");
8010187e:	c7 04 24 d7 89 10 80 	movl   $0x801089d7,(%esp)
80101885:	e8 b3 ec ff ff       	call   8010053d <panic>

  acquire(&icache.lock);
8010188a:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101891:	e8 bd 38 00 00       	call   80105153 <acquire>
  while(ip->flags & I_BUSY)
80101896:	eb 13                	jmp    801018ab <ilock+0x43>
    sleep(ip, &icache.lock);
80101898:	c7 44 24 04 60 f8 10 	movl   $0x8010f860,0x4(%esp)
8010189f:	80 
801018a0:	8b 45 08             	mov    0x8(%ebp),%eax
801018a3:	89 04 24             	mov    %eax,(%esp)
801018a6:	e8 3a 35 00 00       	call   80104de5 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 e0 01             	and    $0x1,%eax
801018b4:	84 c0                	test   %al,%al
801018b6:	75 e0                	jne    80101898 <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018b8:	8b 45 08             	mov    0x8(%ebp),%eax
801018bb:	8b 40 0c             	mov    0xc(%eax),%eax
801018be:	89 c2                	mov    %eax,%edx
801018c0:	83 ca 01             	or     $0x1,%edx
801018c3:	8b 45 08             	mov    0x8(%ebp),%eax
801018c6:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018c9:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
801018d0:	e8 19 39 00 00       	call   801051ee <release>

  if(!(ip->flags & I_VALID)){
801018d5:	8b 45 08             	mov    0x8(%ebp),%eax
801018d8:	8b 40 0c             	mov    0xc(%eax),%eax
801018db:	83 e0 02             	and    $0x2,%eax
801018de:	85 c0                	test   %eax,%eax
801018e0:	0f 85 ce 00 00 00    	jne    801019b4 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018e6:	8b 45 08             	mov    0x8(%ebp),%eax
801018e9:	8b 40 04             	mov    0x4(%eax),%eax
801018ec:	c1 e8 03             	shr    $0x3,%eax
801018ef:	8d 50 02             	lea    0x2(%eax),%edx
801018f2:	8b 45 08             	mov    0x8(%ebp),%eax
801018f5:	8b 00                	mov    (%eax),%eax
801018f7:	89 54 24 04          	mov    %edx,0x4(%esp)
801018fb:	89 04 24             	mov    %eax,(%esp)
801018fe:	e8 a3 e8 ff ff       	call   801001a6 <bread>
80101903:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101906:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101909:	8d 50 18             	lea    0x18(%eax),%edx
8010190c:	8b 45 08             	mov    0x8(%ebp),%eax
8010190f:	8b 40 04             	mov    0x4(%eax),%eax
80101912:	83 e0 07             	and    $0x7,%eax
80101915:	c1 e0 06             	shl    $0x6,%eax
80101918:	01 d0                	add    %edx,%eax
8010191a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 10             	movzwl (%eax),%edx
80101923:	8b 45 08             	mov    0x8(%ebp),%eax
80101926:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010192a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192d:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101931:	8b 45 08             	mov    0x8(%ebp),%eax
80101934:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
80101938:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193b:	0f b7 50 04          	movzwl 0x4(%eax),%edx
8010193f:	8b 45 08             	mov    0x8(%ebp),%eax
80101942:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101946:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101949:	0f b7 50 06          	movzwl 0x6(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101954:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101957:	8b 50 08             	mov    0x8(%eax),%edx
8010195a:	8b 45 08             	mov    0x8(%ebp),%eax
8010195d:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101960:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101963:	8d 50 0c             	lea    0xc(%eax),%edx
80101966:	8b 45 08             	mov    0x8(%ebp),%eax
80101969:	83 c0 1c             	add    $0x1c,%eax
8010196c:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101973:	00 
80101974:	89 54 24 04          	mov    %edx,0x4(%esp)
80101978:	89 04 24             	mov    %eax,(%esp)
8010197b:	e8 2d 3b 00 00       	call   801054ad <memmove>
    brelse(bp);
80101980:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101983:	89 04 24             	mov    %eax,(%esp)
80101986:	e8 8c e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010198b:	8b 45 08             	mov    0x8(%ebp),%eax
8010198e:	8b 40 0c             	mov    0xc(%eax),%eax
80101991:	89 c2                	mov    %eax,%edx
80101993:	83 ca 02             	or     $0x2,%edx
80101996:	8b 45 08             	mov    0x8(%ebp),%eax
80101999:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010199c:	8b 45 08             	mov    0x8(%ebp),%eax
8010199f:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801019a3:	66 85 c0             	test   %ax,%ax
801019a6:	75 0c                	jne    801019b4 <ilock+0x14c>
      panic("ilock: no type");
801019a8:	c7 04 24 dd 89 10 80 	movl   $0x801089dd,(%esp)
801019af:	e8 89 eb ff ff       	call   8010053d <panic>
  }
}
801019b4:	c9                   	leave  
801019b5:	c3                   	ret    

801019b6 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019b6:	55                   	push   %ebp
801019b7:	89 e5                	mov    %esp,%ebp
801019b9:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019bc:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019c0:	74 17                	je     801019d9 <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 0c             	mov    0xc(%eax),%eax
801019c8:	83 e0 01             	and    $0x1,%eax
801019cb:	85 c0                	test   %eax,%eax
801019cd:	74 0a                	je     801019d9 <iunlock+0x23>
801019cf:	8b 45 08             	mov    0x8(%ebp),%eax
801019d2:	8b 40 08             	mov    0x8(%eax),%eax
801019d5:	85 c0                	test   %eax,%eax
801019d7:	7f 0c                	jg     801019e5 <iunlock+0x2f>
    panic("iunlock");
801019d9:	c7 04 24 ec 89 10 80 	movl   $0x801089ec,(%esp)
801019e0:	e8 58 eb ff ff       	call   8010053d <panic>
  acquire(&icache.lock);
801019e5:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
801019ec:	e8 62 37 00 00       	call   80105153 <acquire>
  ip->flags &= ~I_BUSY;
801019f1:	8b 45 08             	mov    0x8(%ebp),%eax
801019f4:	8b 40 0c             	mov    0xc(%eax),%eax
801019f7:	89 c2                	mov    %eax,%edx
801019f9:	83 e2 fe             	and    $0xfffffffe,%edx
801019fc:	8b 45 08             	mov    0x8(%ebp),%eax
801019ff:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
80101a02:	8b 45 08             	mov    0x8(%ebp),%eax
80101a05:	89 04 24             	mov    %eax,(%esp)
80101a08:	e8 3b 35 00 00       	call   80104f48 <wakeup>
  release(&icache.lock);
80101a0d:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101a14:	e8 d5 37 00 00       	call   801051ee <release>
}
80101a19:	c9                   	leave  
80101a1a:	c3                   	ret    

80101a1b <iput>:
// be recycled.
// If that was the last reference and the inode has no links
// to it, free the inode (and its content) on disk.
void
iput(struct inode *ip)
{
80101a1b:	55                   	push   %ebp
80101a1c:	89 e5                	mov    %esp,%ebp
80101a1e:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a21:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101a28:	e8 26 37 00 00       	call   80105153 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a30:	8b 40 08             	mov    0x8(%eax),%eax
80101a33:	83 f8 01             	cmp    $0x1,%eax
80101a36:	0f 85 93 00 00 00    	jne    80101acf <iput+0xb4>
80101a3c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a3f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a42:	83 e0 02             	and    $0x2,%eax
80101a45:	85 c0                	test   %eax,%eax
80101a47:	0f 84 82 00 00 00    	je     80101acf <iput+0xb4>
80101a4d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a50:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a54:	66 85 c0             	test   %ax,%ax
80101a57:	75 76                	jne    80101acf <iput+0xb4>
    // inode has no links: truncate and free inode.
    if(ip->flags & I_BUSY)
80101a59:	8b 45 08             	mov    0x8(%ebp),%eax
80101a5c:	8b 40 0c             	mov    0xc(%eax),%eax
80101a5f:	83 e0 01             	and    $0x1,%eax
80101a62:	84 c0                	test   %al,%al
80101a64:	74 0c                	je     80101a72 <iput+0x57>
      panic("iput busy");
80101a66:	c7 04 24 f4 89 10 80 	movl   $0x801089f4,(%esp)
80101a6d:	e8 cb ea ff ff       	call   8010053d <panic>
    ip->flags |= I_BUSY;
80101a72:	8b 45 08             	mov    0x8(%ebp),%eax
80101a75:	8b 40 0c             	mov    0xc(%eax),%eax
80101a78:	89 c2                	mov    %eax,%edx
80101a7a:	83 ca 01             	or     $0x1,%edx
80101a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a80:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a83:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101a8a:	e8 5f 37 00 00       	call   801051ee <release>
    itrunc(ip);
80101a8f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a92:	89 04 24             	mov    %eax,(%esp)
80101a95:	e8 72 01 00 00       	call   80101c0c <itrunc>
    ip->type = 0;
80101a9a:	8b 45 08             	mov    0x8(%ebp),%eax
80101a9d:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101aa3:	8b 45 08             	mov    0x8(%ebp),%eax
80101aa6:	89 04 24             	mov    %eax,(%esp)
80101aa9:	e8 fe fb ff ff       	call   801016ac <iupdate>
    acquire(&icache.lock);
80101aae:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101ab5:	e8 99 36 00 00       	call   80105153 <acquire>
    ip->flags = 0;
80101aba:	8b 45 08             	mov    0x8(%ebp),%eax
80101abd:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ac4:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac7:	89 04 24             	mov    %eax,(%esp)
80101aca:	e8 79 34 00 00       	call   80104f48 <wakeup>
  }
  ip->ref--;
80101acf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ad2:	8b 40 08             	mov    0x8(%eax),%eax
80101ad5:	8d 50 ff             	lea    -0x1(%eax),%edx
80101ad8:	8b 45 08             	mov    0x8(%ebp),%eax
80101adb:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ade:	c7 04 24 60 f8 10 80 	movl   $0x8010f860,(%esp)
80101ae5:	e8 04 37 00 00       	call   801051ee <release>
}
80101aea:	c9                   	leave  
80101aeb:	c3                   	ret    

80101aec <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101aec:	55                   	push   %ebp
80101aed:	89 e5                	mov    %esp,%ebp
80101aef:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101af2:	8b 45 08             	mov    0x8(%ebp),%eax
80101af5:	89 04 24             	mov    %eax,(%esp)
80101af8:	e8 b9 fe ff ff       	call   801019b6 <iunlock>
  iput(ip);
80101afd:	8b 45 08             	mov    0x8(%ebp),%eax
80101b00:	89 04 24             	mov    %eax,(%esp)
80101b03:	e8 13 ff ff ff       	call   80101a1b <iput>
}
80101b08:	c9                   	leave  
80101b09:	c3                   	ret    

80101b0a <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101b0a:	55                   	push   %ebp
80101b0b:	89 e5                	mov    %esp,%ebp
80101b0d:	53                   	push   %ebx
80101b0e:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b11:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b15:	77 3e                	ja     80101b55 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b17:	8b 45 08             	mov    0x8(%ebp),%eax
80101b1a:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b1d:	83 c2 04             	add    $0x4,%edx
80101b20:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b24:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b27:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b2b:	75 20                	jne    80101b4d <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b30:	8b 00                	mov    (%eax),%eax
80101b32:	89 04 24             	mov    %eax,(%esp)
80101b35:	e8 49 f8 ff ff       	call   80101383 <balloc>
80101b3a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101b40:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b43:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b46:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b49:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b50:	e9 b1 00 00 00       	jmp    80101c06 <bmap+0xfc>
  }
  bn -= NDIRECT;
80101b55:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b59:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b5d:	0f 87 97 00 00 00    	ja     80101bfa <bmap+0xf0>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b63:	8b 45 08             	mov    0x8(%ebp),%eax
80101b66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b69:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b6c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b70:	75 19                	jne    80101b8b <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b72:	8b 45 08             	mov    0x8(%ebp),%eax
80101b75:	8b 00                	mov    (%eax),%eax
80101b77:	89 04 24             	mov    %eax,(%esp)
80101b7a:	e8 04 f8 ff ff       	call   80101383 <balloc>
80101b7f:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b82:	8b 45 08             	mov    0x8(%ebp),%eax
80101b85:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b88:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101b8e:	8b 00                	mov    (%eax),%eax
80101b90:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b93:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b97:	89 04 24             	mov    %eax,(%esp)
80101b9a:	e8 07 e6 ff ff       	call   801001a6 <bread>
80101b9f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101ba2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ba5:	83 c0 18             	add    $0x18,%eax
80101ba8:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101bab:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bae:	c1 e0 02             	shl    $0x2,%eax
80101bb1:	03 45 ec             	add    -0x14(%ebp),%eax
80101bb4:	8b 00                	mov    (%eax),%eax
80101bb6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bbd:	75 2b                	jne    80101bea <bmap+0xe0>
      a[bn] = addr = balloc(ip->dev);
80101bbf:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bc2:	c1 e0 02             	shl    $0x2,%eax
80101bc5:	89 c3                	mov    %eax,%ebx
80101bc7:	03 5d ec             	add    -0x14(%ebp),%ebx
80101bca:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcd:	8b 00                	mov    (%eax),%eax
80101bcf:	89 04 24             	mov    %eax,(%esp)
80101bd2:	e8 ac f7 ff ff       	call   80101383 <balloc>
80101bd7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bda:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdd:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be2:	89 04 24             	mov    %eax,(%esp)
80101be5:	e8 cc 16 00 00       	call   801032b6 <log_write>
    }
    brelse(bp);
80101bea:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101bed:	89 04 24             	mov    %eax,(%esp)
80101bf0:	e8 22 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf8:	eb 0c                	jmp    80101c06 <bmap+0xfc>
  }

  panic("bmap: out of range");
80101bfa:	c7 04 24 fe 89 10 80 	movl   $0x801089fe,(%esp)
80101c01:	e8 37 e9 ff ff       	call   8010053d <panic>
}
80101c06:	83 c4 24             	add    $0x24,%esp
80101c09:	5b                   	pop    %ebx
80101c0a:	5d                   	pop    %ebp
80101c0b:	c3                   	ret    

80101c0c <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0c:	55                   	push   %ebp
80101c0d:	89 e5                	mov    %esp,%ebp
80101c0f:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c19:	eb 44                	jmp    80101c5f <itrunc+0x53>
    if(ip->addrs[i]){
80101c1b:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c21:	83 c2 04             	add    $0x4,%edx
80101c24:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c28:	85 c0                	test   %eax,%eax
80101c2a:	74 2f                	je     80101c5b <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2c:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c32:	83 c2 04             	add    $0x4,%edx
80101c35:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c39:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3c:	8b 00                	mov    (%eax),%eax
80101c3e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c42:	89 04 24             	mov    %eax,(%esp)
80101c45:	e8 90 f8 ff ff       	call   801014da <bfree>
      ip->addrs[i] = 0;
80101c4a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c50:	83 c2 04             	add    $0x4,%edx
80101c53:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c5a:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c5b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5f:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c63:	7e b6                	jle    80101c1b <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c65:	8b 45 08             	mov    0x8(%ebp),%eax
80101c68:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c6b:	85 c0                	test   %eax,%eax
80101c6d:	0f 84 8f 00 00 00    	je     80101d02 <itrunc+0xf6>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c73:	8b 45 08             	mov    0x8(%ebp),%eax
80101c76:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c79:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7c:	8b 00                	mov    (%eax),%eax
80101c7e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c82:	89 04 24             	mov    %eax,(%esp)
80101c85:	e8 1c e5 ff ff       	call   801001a6 <bread>
80101c8a:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c90:	83 c0 18             	add    $0x18,%eax
80101c93:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c96:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9d:	eb 2f                	jmp    80101cce <itrunc+0xc2>
      if(a[j])
80101c9f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca2:	c1 e0 02             	shl    $0x2,%eax
80101ca5:	03 45 e8             	add    -0x18(%ebp),%eax
80101ca8:	8b 00                	mov    (%eax),%eax
80101caa:	85 c0                	test   %eax,%eax
80101cac:	74 1c                	je     80101cca <itrunc+0xbe>
        bfree(ip->dev, a[j]);
80101cae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb1:	c1 e0 02             	shl    $0x2,%eax
80101cb4:	03 45 e8             	add    -0x18(%ebp),%eax
80101cb7:	8b 10                	mov    (%eax),%edx
80101cb9:	8b 45 08             	mov    0x8(%ebp),%eax
80101cbc:	8b 00                	mov    (%eax),%eax
80101cbe:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cc2:	89 04 24             	mov    %eax,(%esp)
80101cc5:	e8 10 f8 ff ff       	call   801014da <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cca:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cce:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cd1:	83 f8 7f             	cmp    $0x7f,%eax
80101cd4:	76 c9                	jbe    80101c9f <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101cd6:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101cd9:	89 04 24             	mov    %eax,(%esp)
80101cdc:	e8 36 e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ce1:	8b 45 08             	mov    0x8(%ebp),%eax
80101ce4:	8b 50 4c             	mov    0x4c(%eax),%edx
80101ce7:	8b 45 08             	mov    0x8(%ebp),%eax
80101cea:	8b 00                	mov    (%eax),%eax
80101cec:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cf0:	89 04 24             	mov    %eax,(%esp)
80101cf3:	e8 e2 f7 ff ff       	call   801014da <bfree>
    ip->addrs[NDIRECT] = 0;
80101cf8:	8b 45 08             	mov    0x8(%ebp),%eax
80101cfb:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	89 04 24             	mov    %eax,(%esp)
80101d12:	e8 95 f9 ff ff       	call   801016ac <iupdate>
}
80101d17:	c9                   	leave  
80101d18:	c3                   	ret    

80101d19 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d19:	55                   	push   %ebp
80101d1a:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d1c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d1f:	8b 00                	mov    (%eax),%eax
80101d21:	89 c2                	mov    %eax,%edx
80101d23:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d26:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d29:	8b 45 08             	mov    0x8(%ebp),%eax
80101d2c:	8b 50 04             	mov    0x4(%eax),%edx
80101d2f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d32:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d35:	8b 45 08             	mov    0x8(%ebp),%eax
80101d38:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d3c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3f:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d42:	8b 45 08             	mov    0x8(%ebp),%eax
80101d45:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d49:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d4c:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d50:	8b 45 08             	mov    0x8(%ebp),%eax
80101d53:	8b 50 18             	mov    0x18(%eax),%edx
80101d56:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d59:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d5c:	5d                   	pop    %ebp
80101d5d:	c3                   	ret    

80101d5e <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d5e:	55                   	push   %ebp
80101d5f:	89 e5                	mov    %esp,%ebp
80101d61:	53                   	push   %ebx
80101d62:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d65:	8b 45 08             	mov    0x8(%ebp),%eax
80101d68:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d6c:	66 83 f8 03          	cmp    $0x3,%ax
80101d70:	75 60                	jne    80101dd2 <readi+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d72:	8b 45 08             	mov    0x8(%ebp),%eax
80101d75:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d79:	66 85 c0             	test   %ax,%ax
80101d7c:	78 20                	js     80101d9e <readi+0x40>
80101d7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d81:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d85:	66 83 f8 09          	cmp    $0x9,%ax
80101d89:	7f 13                	jg     80101d9e <readi+0x40>
80101d8b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d92:	98                   	cwtl   
80101d93:	8b 04 c5 00 f8 10 80 	mov    -0x7fef0800(,%eax,8),%eax
80101d9a:	85 c0                	test   %eax,%eax
80101d9c:	75 0a                	jne    80101da8 <readi+0x4a>
      return -1;
80101d9e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101da3:	e9 1b 01 00 00       	jmp    80101ec3 <readi+0x165>
    return devsw[ip->major].read(ip, dst, n);
80101da8:	8b 45 08             	mov    0x8(%ebp),%eax
80101dab:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101daf:	98                   	cwtl   
80101db0:	8b 14 c5 00 f8 10 80 	mov    -0x7fef0800(,%eax,8),%edx
80101db7:	8b 45 14             	mov    0x14(%ebp),%eax
80101dba:	89 44 24 08          	mov    %eax,0x8(%esp)
80101dbe:	8b 45 0c             	mov    0xc(%ebp),%eax
80101dc1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101dc5:	8b 45 08             	mov    0x8(%ebp),%eax
80101dc8:	89 04 24             	mov    %eax,(%esp)
80101dcb:	ff d2                	call   *%edx
80101dcd:	e9 f1 00 00 00       	jmp    80101ec3 <readi+0x165>
  }

  if(off > ip->size || off + n < off)
80101dd2:	8b 45 08             	mov    0x8(%ebp),%eax
80101dd5:	8b 40 18             	mov    0x18(%eax),%eax
80101dd8:	3b 45 10             	cmp    0x10(%ebp),%eax
80101ddb:	72 0d                	jb     80101dea <readi+0x8c>
80101ddd:	8b 45 14             	mov    0x14(%ebp),%eax
80101de0:	8b 55 10             	mov    0x10(%ebp),%edx
80101de3:	01 d0                	add    %edx,%eax
80101de5:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de8:	73 0a                	jae    80101df4 <readi+0x96>
    return -1;
80101dea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101def:	e9 cf 00 00 00       	jmp    80101ec3 <readi+0x165>
  if(off + n > ip->size)
80101df4:	8b 45 14             	mov    0x14(%ebp),%eax
80101df7:	8b 55 10             	mov    0x10(%ebp),%edx
80101dfa:	01 c2                	add    %eax,%edx
80101dfc:	8b 45 08             	mov    0x8(%ebp),%eax
80101dff:	8b 40 18             	mov    0x18(%eax),%eax
80101e02:	39 c2                	cmp    %eax,%edx
80101e04:	76 0c                	jbe    80101e12 <readi+0xb4>
    n = ip->size - off;
80101e06:	8b 45 08             	mov    0x8(%ebp),%eax
80101e09:	8b 40 18             	mov    0x18(%eax),%eax
80101e0c:	2b 45 10             	sub    0x10(%ebp),%eax
80101e0f:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e12:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e19:	e9 96 00 00 00       	jmp    80101eb4 <readi+0x156>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e1e:	8b 45 10             	mov    0x10(%ebp),%eax
80101e21:	c1 e8 09             	shr    $0x9,%eax
80101e24:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e28:	8b 45 08             	mov    0x8(%ebp),%eax
80101e2b:	89 04 24             	mov    %eax,(%esp)
80101e2e:	e8 d7 fc ff ff       	call   80101b0a <bmap>
80101e33:	8b 55 08             	mov    0x8(%ebp),%edx
80101e36:	8b 12                	mov    (%edx),%edx
80101e38:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e3c:	89 14 24             	mov    %edx,(%esp)
80101e3f:	e8 62 e3 ff ff       	call   801001a6 <bread>
80101e44:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e47:	8b 45 10             	mov    0x10(%ebp),%eax
80101e4a:	89 c2                	mov    %eax,%edx
80101e4c:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101e52:	b8 00 02 00 00       	mov    $0x200,%eax
80101e57:	89 c1                	mov    %eax,%ecx
80101e59:	29 d1                	sub    %edx,%ecx
80101e5b:	89 ca                	mov    %ecx,%edx
80101e5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e60:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e63:	89 cb                	mov    %ecx,%ebx
80101e65:	29 c3                	sub    %eax,%ebx
80101e67:	89 d8                	mov    %ebx,%eax
80101e69:	39 c2                	cmp    %eax,%edx
80101e6b:	0f 46 c2             	cmovbe %edx,%eax
80101e6e:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e71:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e74:	8d 50 18             	lea    0x18(%eax),%edx
80101e77:	8b 45 10             	mov    0x10(%ebp),%eax
80101e7a:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7f:	01 c2                	add    %eax,%edx
80101e81:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e84:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e88:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e8c:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e8f:	89 04 24             	mov    %eax,(%esp)
80101e92:	e8 16 36 00 00       	call   801054ad <memmove>
    brelse(bp);
80101e97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e9a:	89 04 24             	mov    %eax,(%esp)
80101e9d:	e8 75 e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ea5:	01 45 f4             	add    %eax,-0xc(%ebp)
80101ea8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eab:	01 45 10             	add    %eax,0x10(%ebp)
80101eae:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb1:	01 45 0c             	add    %eax,0xc(%ebp)
80101eb4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101eb7:	3b 45 14             	cmp    0x14(%ebp),%eax
80101eba:	0f 82 5e ff ff ff    	jb     80101e1e <readi+0xc0>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec0:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101ec3:	83 c4 24             	add    $0x24,%esp
80101ec6:	5b                   	pop    %ebx
80101ec7:	5d                   	pop    %ebp
80101ec8:	c3                   	ret    

80101ec9 <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ec9:	55                   	push   %ebp
80101eca:	89 e5                	mov    %esp,%ebp
80101ecc:	53                   	push   %ebx
80101ecd:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed0:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed3:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed7:	66 83 f8 03          	cmp    $0x3,%ax
80101edb:	75 60                	jne    80101f3d <writei+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edd:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee0:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee4:	66 85 c0             	test   %ax,%ax
80101ee7:	78 20                	js     80101f09 <writei+0x40>
80101ee9:	8b 45 08             	mov    0x8(%ebp),%eax
80101eec:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef0:	66 83 f8 09          	cmp    $0x9,%ax
80101ef4:	7f 13                	jg     80101f09 <writei+0x40>
80101ef6:	8b 45 08             	mov    0x8(%ebp),%eax
80101ef9:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101efd:	98                   	cwtl   
80101efe:	8b 04 c5 04 f8 10 80 	mov    -0x7fef07fc(,%eax,8),%eax
80101f05:	85 c0                	test   %eax,%eax
80101f07:	75 0a                	jne    80101f13 <writei+0x4a>
      return -1;
80101f09:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f0e:	e9 46 01 00 00       	jmp    80102059 <writei+0x190>
    return devsw[ip->major].write(ip, src, n);
80101f13:	8b 45 08             	mov    0x8(%ebp),%eax
80101f16:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1a:	98                   	cwtl   
80101f1b:	8b 14 c5 04 f8 10 80 	mov    -0x7fef07fc(,%eax,8),%edx
80101f22:	8b 45 14             	mov    0x14(%ebp),%eax
80101f25:	89 44 24 08          	mov    %eax,0x8(%esp)
80101f29:	8b 45 0c             	mov    0xc(%ebp),%eax
80101f2c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f30:	8b 45 08             	mov    0x8(%ebp),%eax
80101f33:	89 04 24             	mov    %eax,(%esp)
80101f36:	ff d2                	call   *%edx
80101f38:	e9 1c 01 00 00       	jmp    80102059 <writei+0x190>
  }

  if(off > ip->size || off + n < off)
80101f3d:	8b 45 08             	mov    0x8(%ebp),%eax
80101f40:	8b 40 18             	mov    0x18(%eax),%eax
80101f43:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f46:	72 0d                	jb     80101f55 <writei+0x8c>
80101f48:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4b:	8b 55 10             	mov    0x10(%ebp),%edx
80101f4e:	01 d0                	add    %edx,%eax
80101f50:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f53:	73 0a                	jae    80101f5f <writei+0x96>
    return -1;
80101f55:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5a:	e9 fa 00 00 00       	jmp    80102059 <writei+0x190>
  if(off + n > MAXFILE*BSIZE)
80101f5f:	8b 45 14             	mov    0x14(%ebp),%eax
80101f62:	8b 55 10             	mov    0x10(%ebp),%edx
80101f65:	01 d0                	add    %edx,%eax
80101f67:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6c:	76 0a                	jbe    80101f78 <writei+0xaf>
    return -1;
80101f6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f73:	e9 e1 00 00 00       	jmp    80102059 <writei+0x190>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f78:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f7f:	e9 a1 00 00 00       	jmp    80102025 <writei+0x15c>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f84:	8b 45 10             	mov    0x10(%ebp),%eax
80101f87:	c1 e8 09             	shr    $0x9,%eax
80101f8a:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f8e:	8b 45 08             	mov    0x8(%ebp),%eax
80101f91:	89 04 24             	mov    %eax,(%esp)
80101f94:	e8 71 fb ff ff       	call   80101b0a <bmap>
80101f99:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9c:	8b 12                	mov    (%edx),%edx
80101f9e:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa2:	89 14 24             	mov    %edx,(%esp)
80101fa5:	e8 fc e1 ff ff       	call   801001a6 <bread>
80101faa:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fad:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb0:	89 c2                	mov    %eax,%edx
80101fb2:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101fb8:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbd:	89 c1                	mov    %eax,%ecx
80101fbf:	29 d1                	sub    %edx,%ecx
80101fc1:	89 ca                	mov    %ecx,%edx
80101fc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc6:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc9:	89 cb                	mov    %ecx,%ebx
80101fcb:	29 c3                	sub    %eax,%ebx
80101fcd:	89 d8                	mov    %ebx,%eax
80101fcf:	39 c2                	cmp    %eax,%edx
80101fd1:	0f 46 c2             	cmovbe %edx,%eax
80101fd4:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fda:	8d 50 18             	lea    0x18(%eax),%edx
80101fdd:	8b 45 10             	mov    0x10(%ebp),%eax
80101fe0:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fe5:	01 c2                	add    %eax,%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 b0 34 00 00       	call   801054ad <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 ae 12 00 00       	call   801032b6 <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 53 ff ff ff    	jb     80101f84 <writei+0xbb>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18d>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18d>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 56 f6 ff ff       	call   801016ac <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	83 c4 24             	add    $0x24,%esp
8010205c:	5b                   	pop    %ebx
8010205d:	5d                   	pop    %ebp
8010205e:	c3                   	ret    

8010205f <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205f:	55                   	push   %ebp
80102060:	89 e5                	mov    %esp,%ebp
80102062:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102065:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
8010206c:	00 
8010206d:	8b 45 0c             	mov    0xc(%ebp),%eax
80102070:	89 44 24 04          	mov    %eax,0x4(%esp)
80102074:	8b 45 08             	mov    0x8(%ebp),%eax
80102077:	89 04 24             	mov    %eax,(%esp)
8010207a:	e8 d2 34 00 00       	call   80105551 <strncmp>
}
8010207f:	c9                   	leave  
80102080:	c3                   	ret    

80102081 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80102081:	55                   	push   %ebp
80102082:	89 e5                	mov    %esp,%ebp
80102084:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102087:	8b 45 08             	mov    0x8(%ebp),%eax
8010208a:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208e:	66 83 f8 01          	cmp    $0x1,%ax
80102092:	74 0c                	je     801020a0 <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102094:	c7 04 24 11 8a 10 80 	movl   $0x80108a11,(%esp)
8010209b:	e8 9d e4 ff ff       	call   8010053d <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
801020a0:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a7:	e9 87 00 00 00       	jmp    80102133 <dirlookup+0xb2>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020ac:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020b3:	00 
801020b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b7:	89 44 24 08          	mov    %eax,0x8(%esp)
801020bb:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020be:	89 44 24 04          	mov    %eax,0x4(%esp)
801020c2:	8b 45 08             	mov    0x8(%ebp),%eax
801020c5:	89 04 24             	mov    %eax,(%esp)
801020c8:	e8 91 fc ff ff       	call   80101d5e <readi>
801020cd:	83 f8 10             	cmp    $0x10,%eax
801020d0:	74 0c                	je     801020de <dirlookup+0x5d>
      panic("dirlink read");
801020d2:	c7 04 24 23 8a 10 80 	movl   $0x80108a23,(%esp)
801020d9:	e8 5f e4 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801020de:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020e2:	66 85 c0             	test   %ax,%ax
801020e5:	74 47                	je     8010212e <dirlookup+0xad>
      continue;
    if(namecmp(name, de.name) == 0){
801020e7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ea:	83 c0 02             	add    $0x2,%eax
801020ed:	89 44 24 04          	mov    %eax,0x4(%esp)
801020f1:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f4:	89 04 24             	mov    %eax,(%esp)
801020f7:	e8 63 ff ff ff       	call   8010205f <namecmp>
801020fc:	85 c0                	test   %eax,%eax
801020fe:	75 2f                	jne    8010212f <dirlookup+0xae>
      // entry matches path element
      if(poff)
80102100:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102104:	74 08                	je     8010210e <dirlookup+0x8d>
        *poff = off;
80102106:	8b 45 10             	mov    0x10(%ebp),%eax
80102109:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210c:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210e:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102112:	0f b7 c0             	movzwl %ax,%eax
80102115:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102118:	8b 45 08             	mov    0x8(%ebp),%eax
8010211b:	8b 00                	mov    (%eax),%eax
8010211d:	8b 55 f0             	mov    -0x10(%ebp),%edx
80102120:	89 54 24 04          	mov    %edx,0x4(%esp)
80102124:	89 04 24             	mov    %eax,(%esp)
80102127:	e8 38 f6 ff ff       	call   80101764 <iget>
8010212c:	eb 19                	jmp    80102147 <dirlookup+0xc6>

  for(off = 0; off < dp->size; off += sizeof(de)){
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      continue;
8010212e:	90                   	nop
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212f:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102133:	8b 45 08             	mov    0x8(%ebp),%eax
80102136:	8b 40 18             	mov    0x18(%eax),%eax
80102139:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010213c:	0f 87 6a ff ff ff    	ja     801020ac <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
80102142:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102147:	c9                   	leave  
80102148:	c3                   	ret    

80102149 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102149:	55                   	push   %ebp
8010214a:	89 e5                	mov    %esp,%ebp
8010214c:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214f:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102156:	00 
80102157:	8b 45 0c             	mov    0xc(%ebp),%eax
8010215a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215e:	8b 45 08             	mov    0x8(%ebp),%eax
80102161:	89 04 24             	mov    %eax,(%esp)
80102164:	e8 18 ff ff ff       	call   80102081 <dirlookup>
80102169:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010216c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80102170:	74 15                	je     80102187 <dirlink+0x3e>
    iput(ip);
80102172:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102175:	89 04 24             	mov    %eax,(%esp)
80102178:	e8 9e f8 ff ff       	call   80101a1b <iput>
    return -1;
8010217d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102182:	e9 b8 00 00 00       	jmp    8010223f <dirlink+0xf6>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102187:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218e:	eb 44                	jmp    801021d4 <dirlink+0x8b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102193:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010219a:	00 
8010219b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219f:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021a2:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a6:	8b 45 08             	mov    0x8(%ebp),%eax
801021a9:	89 04 24             	mov    %eax,(%esp)
801021ac:	e8 ad fb ff ff       	call   80101d5e <readi>
801021b1:	83 f8 10             	cmp    $0x10,%eax
801021b4:	74 0c                	je     801021c2 <dirlink+0x79>
      panic("dirlink read");
801021b6:	c7 04 24 23 8a 10 80 	movl   $0x80108a23,(%esp)
801021bd:	e8 7b e3 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801021c2:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c6:	66 85 c0             	test   %ax,%ax
801021c9:	74 18                	je     801021e3 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021ce:	83 c0 10             	add    $0x10,%eax
801021d1:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d4:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d7:	8b 45 08             	mov    0x8(%ebp),%eax
801021da:	8b 40 18             	mov    0x18(%eax),%eax
801021dd:	39 c2                	cmp    %eax,%edx
801021df:	72 af                	jb     80102190 <dirlink+0x47>
801021e1:	eb 01                	jmp    801021e4 <dirlink+0x9b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      break;
801021e3:	90                   	nop
  }

  strncpy(de.name, name, DIRSIZ);
801021e4:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021eb:	00 
801021ec:	8b 45 0c             	mov    0xc(%ebp),%eax
801021ef:	89 44 24 04          	mov    %eax,0x4(%esp)
801021f3:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f6:	83 c0 02             	add    $0x2,%eax
801021f9:	89 04 24             	mov    %eax,(%esp)
801021fc:	e8 a8 33 00 00       	call   801055a9 <strncpy>
  de.inum = inum;
80102201:	8b 45 10             	mov    0x10(%ebp),%eax
80102204:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102208:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010220b:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102212:	00 
80102213:	89 44 24 08          	mov    %eax,0x8(%esp)
80102217:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010221a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221e:	8b 45 08             	mov    0x8(%ebp),%eax
80102221:	89 04 24             	mov    %eax,(%esp)
80102224:	e8 a0 fc ff ff       	call   80101ec9 <writei>
80102229:	83 f8 10             	cmp    $0x10,%eax
8010222c:	74 0c                	je     8010223a <dirlink+0xf1>
    panic("dirlink");
8010222e:	c7 04 24 30 8a 10 80 	movl   $0x80108a30,(%esp)
80102235:	e8 03 e3 ff ff       	call   8010053d <panic>
  
  return 0;
8010223a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223f:	c9                   	leave  
80102240:	c3                   	ret    

80102241 <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
80102241:	55                   	push   %ebp
80102242:	89 e5                	mov    %esp,%ebp
80102244:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102247:	eb 04                	jmp    8010224d <skipelem+0xc>
    path++;
80102249:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
8010224d:	8b 45 08             	mov    0x8(%ebp),%eax
80102250:	0f b6 00             	movzbl (%eax),%eax
80102253:	3c 2f                	cmp    $0x2f,%al
80102255:	74 f2                	je     80102249 <skipelem+0x8>
    path++;
  if(*path == 0)
80102257:	8b 45 08             	mov    0x8(%ebp),%eax
8010225a:	0f b6 00             	movzbl (%eax),%eax
8010225d:	84 c0                	test   %al,%al
8010225f:	75 0a                	jne    8010226b <skipelem+0x2a>
    return 0;
80102261:	b8 00 00 00 00       	mov    $0x0,%eax
80102266:	e9 86 00 00 00       	jmp    801022f1 <skipelem+0xb0>
  s = path;
8010226b:	8b 45 08             	mov    0x8(%ebp),%eax
8010226e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
80102271:	eb 04                	jmp    80102277 <skipelem+0x36>
    path++;
80102273:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102277:	8b 45 08             	mov    0x8(%ebp),%eax
8010227a:	0f b6 00             	movzbl (%eax),%eax
8010227d:	3c 2f                	cmp    $0x2f,%al
8010227f:	74 0a                	je     8010228b <skipelem+0x4a>
80102281:	8b 45 08             	mov    0x8(%ebp),%eax
80102284:	0f b6 00             	movzbl (%eax),%eax
80102287:	84 c0                	test   %al,%al
80102289:	75 e8                	jne    80102273 <skipelem+0x32>
    path++;
  len = path - s;
8010228b:	8b 55 08             	mov    0x8(%ebp),%edx
8010228e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102291:	89 d1                	mov    %edx,%ecx
80102293:	29 c1                	sub    %eax,%ecx
80102295:	89 c8                	mov    %ecx,%eax
80102297:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
8010229a:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
8010229e:	7e 1c                	jle    801022bc <skipelem+0x7b>
    memmove(name, s, DIRSIZ);
801022a0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a7:	00 
801022a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022ab:	89 44 24 04          	mov    %eax,0x4(%esp)
801022af:	8b 45 0c             	mov    0xc(%ebp),%eax
801022b2:	89 04 24             	mov    %eax,(%esp)
801022b5:	e8 f3 31 00 00       	call   801054ad <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022ba:	eb 28                	jmp    801022e4 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022bc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022bf:	89 44 24 08          	mov    %eax,0x8(%esp)
801022c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c6:	89 44 24 04          	mov    %eax,0x4(%esp)
801022ca:	8b 45 0c             	mov    0xc(%ebp),%eax
801022cd:	89 04 24             	mov    %eax,(%esp)
801022d0:	e8 d8 31 00 00       	call   801054ad <memmove>
    name[len] = 0;
801022d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022d8:	03 45 0c             	add    0xc(%ebp),%eax
801022db:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022de:	eb 04                	jmp    801022e4 <skipelem+0xa3>
    path++;
801022e0:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e4:	8b 45 08             	mov    0x8(%ebp),%eax
801022e7:	0f b6 00             	movzbl (%eax),%eax
801022ea:	3c 2f                	cmp    $0x2f,%al
801022ec:	74 f2                	je     801022e0 <skipelem+0x9f>
    path++;
  return path;
801022ee:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022f1:	c9                   	leave  
801022f2:	c3                   	ret    

801022f3 <namex>:
// Look up and return the inode for a path name.
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022f3:	55                   	push   %ebp
801022f4:	89 e5                	mov    %esp,%ebp
801022f6:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;
  if(*path == '/')
801022f9:	8b 45 08             	mov    0x8(%ebp),%eax
801022fc:	0f b6 00             	movzbl (%eax),%eax
801022ff:	3c 2f                	cmp    $0x2f,%al
80102301:	75 1c                	jne    8010231f <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
80102303:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010230a:	00 
8010230b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102312:	e8 4d f4 ff ff       	call   80101764 <iget>
80102317:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
8010231a:	e9 af 00 00 00       	jmp    801023ce <namex+0xdb>
{
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102325:	8b 40 68             	mov    0x68(%eax),%eax
80102328:	89 04 24             	mov    %eax,(%esp)
8010232b:	e8 06 f5 ff ff       	call   80101836 <idup>
80102330:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while((path = skipelem(path, name)) != 0){
80102333:	e9 96 00 00 00       	jmp    801023ce <namex+0xdb>
    ilock(ip);
80102338:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010233b:	89 04 24             	mov    %eax,(%esp)
8010233e:	e8 25 f5 ff ff       	call   80101868 <ilock>
    if(ip->type != T_DIR){
80102343:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102346:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010234a:	66 83 f8 01          	cmp    $0x1,%ax
8010234e:	74 15                	je     80102365 <namex+0x72>
      iunlockput(ip);
80102350:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102353:	89 04 24             	mov    %eax,(%esp)
80102356:	e8 91 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
8010235b:	b8 00 00 00 00       	mov    $0x0,%eax
80102360:	e9 a3 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102365:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102369:	74 1d                	je     80102388 <namex+0x95>
8010236b:	8b 45 08             	mov    0x8(%ebp),%eax
8010236e:	0f b6 00             	movzbl (%eax),%eax
80102371:	84 c0                	test   %al,%al
80102373:	75 13                	jne    80102388 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102375:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102378:	89 04 24             	mov    %eax,(%esp)
8010237b:	e8 36 f6 ff ff       	call   801019b6 <iunlock>
      return ip;
80102380:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102383:	e9 80 00 00 00       	jmp    80102408 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102388:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238f:	00 
80102390:	8b 45 10             	mov    0x10(%ebp),%eax
80102393:	89 44 24 04          	mov    %eax,0x4(%esp)
80102397:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010239a:	89 04 24             	mov    %eax,(%esp)
8010239d:	e8 df fc ff ff       	call   80102081 <dirlookup>
801023a2:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a5:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a9:	75 12                	jne    801023bd <namex+0xca>
      iunlockput(ip);
801023ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023ae:	89 04 24             	mov    %eax,(%esp)
801023b1:	e8 36 f7 ff ff       	call   80101aec <iunlockput>
      return 0;
801023b6:	b8 00 00 00 00       	mov    $0x0,%eax
801023bb:	eb 4b                	jmp    80102408 <namex+0x115>
    }
    iunlockput(ip);
801023bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023c0:	89 04 24             	mov    %eax,(%esp)
801023c3:	e8 24 f7 ff ff       	call   80101aec <iunlockput>
    ip = next;
801023c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023cb:	89 45 f4             	mov    %eax,-0xc(%ebp)
  struct inode *ip, *next;
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
  while((path = skipelem(path, name)) != 0){
801023ce:	8b 45 10             	mov    0x10(%ebp),%eax
801023d1:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d5:	8b 45 08             	mov    0x8(%ebp),%eax
801023d8:	89 04 24             	mov    %eax,(%esp)
801023db:	e8 61 fe ff ff       	call   80102241 <skipelem>
801023e0:	89 45 08             	mov    %eax,0x8(%ebp)
801023e3:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e7:	0f 85 4b ff ff ff    	jne    80102338 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023ed:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023f1:	74 12                	je     80102405 <namex+0x112>
    iput(ip);
801023f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f6:	89 04 24             	mov    %eax,(%esp)
801023f9:	e8 1d f6 ff ff       	call   80101a1b <iput>
    return 0;
801023fe:	b8 00 00 00 00       	mov    $0x0,%eax
80102403:	eb 03                	jmp    80102408 <namex+0x115>
  }
  return ip;
80102405:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102408:	c9                   	leave  
80102409:	c3                   	ret    

8010240a <namei>:

struct inode*
namei(char *path)
{
8010240a:	55                   	push   %ebp
8010240b:	89 e5                	mov    %esp,%ebp
8010240d:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102410:	8d 45 ea             	lea    -0x16(%ebp),%eax
80102413:	89 44 24 08          	mov    %eax,0x8(%esp)
80102417:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241e:	00 
8010241f:	8b 45 08             	mov    0x8(%ebp),%eax
80102422:	89 04 24             	mov    %eax,(%esp)
80102425:	e8 c9 fe ff ff       	call   801022f3 <namex>
}
8010242a:	c9                   	leave  
8010242b:	c3                   	ret    

8010242c <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
8010242c:	55                   	push   %ebp
8010242d:	89 e5                	mov    %esp,%ebp
8010242f:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
80102432:	8b 45 0c             	mov    0xc(%ebp),%eax
80102435:	89 44 24 08          	mov    %eax,0x8(%esp)
80102439:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102440:	00 
80102441:	8b 45 08             	mov    0x8(%ebp),%eax
80102444:	89 04 24             	mov    %eax,(%esp)
80102447:	e8 a7 fe ff ff       	call   801022f3 <namex>
}
8010244c:	c9                   	leave  
8010244d:	c3                   	ret    
	...

80102450 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102450:	55                   	push   %ebp
80102451:	89 e5                	mov    %esp,%ebp
80102453:	53                   	push   %ebx
80102454:	83 ec 14             	sub    $0x14,%esp
80102457:	8b 45 08             	mov    0x8(%ebp),%eax
8010245a:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010245e:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102462:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102466:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
8010246a:	ec                   	in     (%dx),%al
8010246b:	89 c3                	mov    %eax,%ebx
8010246d:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102470:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102474:	83 c4 14             	add    $0x14,%esp
80102477:	5b                   	pop    %ebx
80102478:	5d                   	pop    %ebp
80102479:	c3                   	ret    

8010247a <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
8010247a:	55                   	push   %ebp
8010247b:	89 e5                	mov    %esp,%ebp
8010247d:	57                   	push   %edi
8010247e:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010247f:	8b 55 08             	mov    0x8(%ebp),%edx
80102482:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102485:	8b 45 10             	mov    0x10(%ebp),%eax
80102488:	89 cb                	mov    %ecx,%ebx
8010248a:	89 df                	mov    %ebx,%edi
8010248c:	89 c1                	mov    %eax,%ecx
8010248e:	fc                   	cld    
8010248f:	f3 6d                	rep insl (%dx),%es:(%edi)
80102491:	89 c8                	mov    %ecx,%eax
80102493:	89 fb                	mov    %edi,%ebx
80102495:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102498:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
8010249b:	5b                   	pop    %ebx
8010249c:	5f                   	pop    %edi
8010249d:	5d                   	pop    %ebp
8010249e:	c3                   	ret    

8010249f <outb>:

static inline void
outb(ushort port, uchar data)
{
8010249f:	55                   	push   %ebp
801024a0:	89 e5                	mov    %esp,%ebp
801024a2:	83 ec 08             	sub    $0x8,%esp
801024a5:	8b 55 08             	mov    0x8(%ebp),%edx
801024a8:	8b 45 0c             	mov    0xc(%ebp),%eax
801024ab:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801024af:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801024b2:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024b6:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024ba:	ee                   	out    %al,(%dx)
}
801024bb:	c9                   	leave  
801024bc:	c3                   	ret    

801024bd <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024bd:	55                   	push   %ebp
801024be:	89 e5                	mov    %esp,%ebp
801024c0:	56                   	push   %esi
801024c1:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024c2:	8b 55 08             	mov    0x8(%ebp),%edx
801024c5:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024c8:	8b 45 10             	mov    0x10(%ebp),%eax
801024cb:	89 cb                	mov    %ecx,%ebx
801024cd:	89 de                	mov    %ebx,%esi
801024cf:	89 c1                	mov    %eax,%ecx
801024d1:	fc                   	cld    
801024d2:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024d4:	89 c8                	mov    %ecx,%eax
801024d6:	89 f3                	mov    %esi,%ebx
801024d8:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024db:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024de:	5b                   	pop    %ebx
801024df:	5e                   	pop    %esi
801024e0:	5d                   	pop    %ebp
801024e1:	c3                   	ret    

801024e2 <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024e2:	55                   	push   %ebp
801024e3:	89 e5                	mov    %esp,%ebp
801024e5:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024e8:	90                   	nop
801024e9:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024f0:	e8 5b ff ff ff       	call   80102450 <inb>
801024f5:	0f b6 c0             	movzbl %al,%eax
801024f8:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024fb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024fe:	25 c0 00 00 00       	and    $0xc0,%eax
80102503:	83 f8 40             	cmp    $0x40,%eax
80102506:	75 e1                	jne    801024e9 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
80102508:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010250c:	74 11                	je     8010251f <idewait+0x3d>
8010250e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102511:	83 e0 21             	and    $0x21,%eax
80102514:	85 c0                	test   %eax,%eax
80102516:	74 07                	je     8010251f <idewait+0x3d>
    return -1;
80102518:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010251d:	eb 05                	jmp    80102524 <idewait+0x42>
  return 0;
8010251f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102524:	c9                   	leave  
80102525:	c3                   	ret    

80102526 <ideinit>:

void
ideinit(void)
{
80102526:	55                   	push   %ebp
80102527:	89 e5                	mov    %esp,%ebp
80102529:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
8010252c:	c7 44 24 04 38 8a 10 	movl   $0x80108a38,0x4(%esp)
80102533:	80 
80102534:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
8010253b:	e8 f2 2b 00 00       	call   80105132 <initlock>
  picenable(IRQ_IDE);
80102540:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102547:	e8 6d 15 00 00       	call   80103ab9 <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
8010254c:	a1 00 0f 11 80       	mov    0x80110f00,%eax
80102551:	83 e8 01             	sub    $0x1,%eax
80102554:	89 44 24 04          	mov    %eax,0x4(%esp)
80102558:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010255f:	e8 0a 04 00 00       	call   8010296e <ioapicenable>
  idewait(0);
80102564:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010256b:	e8 72 ff ff ff       	call   801024e2 <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
80102570:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
80102577:	00 
80102578:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010257f:	e8 1b ff ff ff       	call   8010249f <outb>
  for(i=0; i<1000; i++){
80102584:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010258b:	eb 20                	jmp    801025ad <ideinit+0x87>
    if(inb(0x1f7) != 0){
8010258d:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102594:	e8 b7 fe ff ff       	call   80102450 <inb>
80102599:	84 c0                	test   %al,%al
8010259b:	74 0c                	je     801025a9 <ideinit+0x83>
      havedisk1 = 1;
8010259d:	c7 05 38 c6 10 80 01 	movl   $0x1,0x8010c638
801025a4:	00 00 00 
      break;
801025a7:	eb 0d                	jmp    801025b6 <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
801025a9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801025ad:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025b4:	7e d7                	jle    8010258d <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025b6:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025bd:	00 
801025be:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025c5:	e8 d5 fe ff ff       	call   8010249f <outb>
}
801025ca:	c9                   	leave  
801025cb:	c3                   	ret    

801025cc <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025cc:	55                   	push   %ebp
801025cd:	89 e5                	mov    %esp,%ebp
801025cf:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025d2:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025d6:	75 0c                	jne    801025e4 <idestart+0x18>
    panic("idestart");
801025d8:	c7 04 24 3c 8a 10 80 	movl   $0x80108a3c,(%esp)
801025df:	e8 59 df ff ff       	call   8010053d <panic>

  idewait(0);
801025e4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025eb:	e8 f2 fe ff ff       	call   801024e2 <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025f0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025f7:	00 
801025f8:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025ff:	e8 9b fe ff ff       	call   8010249f <outb>
  outb(0x1f2, 1);  // number of sectors
80102604:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010260b:	00 
8010260c:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102613:	e8 87 fe ff ff       	call   8010249f <outb>
  outb(0x1f3, b->sector & 0xff);
80102618:	8b 45 08             	mov    0x8(%ebp),%eax
8010261b:	8b 40 08             	mov    0x8(%eax),%eax
8010261e:	0f b6 c0             	movzbl %al,%eax
80102621:	89 44 24 04          	mov    %eax,0x4(%esp)
80102625:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
8010262c:	e8 6e fe ff ff       	call   8010249f <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
80102631:	8b 45 08             	mov    0x8(%ebp),%eax
80102634:	8b 40 08             	mov    0x8(%eax),%eax
80102637:	c1 e8 08             	shr    $0x8,%eax
8010263a:	0f b6 c0             	movzbl %al,%eax
8010263d:	89 44 24 04          	mov    %eax,0x4(%esp)
80102641:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102648:	e8 52 fe ff ff       	call   8010249f <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
8010264d:	8b 45 08             	mov    0x8(%ebp),%eax
80102650:	8b 40 08             	mov    0x8(%eax),%eax
80102653:	c1 e8 10             	shr    $0x10,%eax
80102656:	0f b6 c0             	movzbl %al,%eax
80102659:	89 44 24 04          	mov    %eax,0x4(%esp)
8010265d:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
80102664:	e8 36 fe ff ff       	call   8010249f <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102669:	8b 45 08             	mov    0x8(%ebp),%eax
8010266c:	8b 40 04             	mov    0x4(%eax),%eax
8010266f:	83 e0 01             	and    $0x1,%eax
80102672:	89 c2                	mov    %eax,%edx
80102674:	c1 e2 04             	shl    $0x4,%edx
80102677:	8b 45 08             	mov    0x8(%ebp),%eax
8010267a:	8b 40 08             	mov    0x8(%eax),%eax
8010267d:	c1 e8 18             	shr    $0x18,%eax
80102680:	83 e0 0f             	and    $0xf,%eax
80102683:	09 d0                	or     %edx,%eax
80102685:	83 c8 e0             	or     $0xffffffe0,%eax
80102688:	0f b6 c0             	movzbl %al,%eax
8010268b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010268f:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102696:	e8 04 fe ff ff       	call   8010249f <outb>
  if(b->flags & B_DIRTY){
8010269b:	8b 45 08             	mov    0x8(%ebp),%eax
8010269e:	8b 00                	mov    (%eax),%eax
801026a0:	83 e0 04             	and    $0x4,%eax
801026a3:	85 c0                	test   %eax,%eax
801026a5:	74 34                	je     801026db <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
801026a7:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
801026ae:	00 
801026af:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026b6:	e8 e4 fd ff ff       	call   8010249f <outb>
    outsl(0x1f0, b->data, 512/4);
801026bb:	8b 45 08             	mov    0x8(%ebp),%eax
801026be:	83 c0 18             	add    $0x18,%eax
801026c1:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026c8:	00 
801026c9:	89 44 24 04          	mov    %eax,0x4(%esp)
801026cd:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026d4:	e8 e4 fd ff ff       	call   801024bd <outsl>
801026d9:	eb 14                	jmp    801026ef <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026db:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026e2:	00 
801026e3:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026ea:	e8 b0 fd ff ff       	call   8010249f <outb>
  }
}
801026ef:	c9                   	leave  
801026f0:	c3                   	ret    

801026f1 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026f1:	55                   	push   %ebp
801026f2:	89 e5                	mov    %esp,%ebp
801026f4:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026f7:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
801026fe:	e8 50 2a 00 00       	call   80105153 <acquire>
  if((b = idequeue) == 0){
80102703:	a1 34 c6 10 80       	mov    0x8010c634,%eax
80102708:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010270b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010270f:	75 11                	jne    80102722 <ideintr+0x31>
    release(&idelock);
80102711:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
80102718:	e8 d1 2a 00 00       	call   801051ee <release>
    // cprintf("spurious IDE interrupt\n");
    return;
8010271d:	e9 85 00 00 00       	jmp    801027a7 <ideintr+0xb6>
  }
  idequeue = b->qnext;
80102722:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102725:	8b 40 14             	mov    0x14(%eax),%eax
80102728:	a3 34 c6 10 80       	mov    %eax,0x8010c634

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
8010272d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102730:	8b 00                	mov    (%eax),%eax
80102732:	83 e0 04             	and    $0x4,%eax
80102735:	85 c0                	test   %eax,%eax
80102737:	75 2e                	jne    80102767 <ideintr+0x76>
80102739:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102740:	e8 9d fd ff ff       	call   801024e2 <idewait>
80102745:	85 c0                	test   %eax,%eax
80102747:	78 1e                	js     80102767 <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102749:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010274c:	83 c0 18             	add    $0x18,%eax
8010274f:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102756:	00 
80102757:	89 44 24 04          	mov    %eax,0x4(%esp)
8010275b:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
80102762:	e8 13 fd ff ff       	call   8010247a <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
80102767:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010276a:	8b 00                	mov    (%eax),%eax
8010276c:	89 c2                	mov    %eax,%edx
8010276e:	83 ca 02             	or     $0x2,%edx
80102771:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102774:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102776:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102779:	8b 00                	mov    (%eax),%eax
8010277b:	89 c2                	mov    %eax,%edx
8010277d:	83 e2 fb             	and    $0xfffffffb,%edx
80102780:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102783:	89 10                	mov    %edx,(%eax)
  //wakeup(b);
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
80102785:	a1 34 c6 10 80       	mov    0x8010c634,%eax
8010278a:	85 c0                	test   %eax,%eax
8010278c:	74 0d                	je     8010279b <ideintr+0xaa>
    idestart(idequeue);
8010278e:	a1 34 c6 10 80       	mov    0x8010c634,%eax
80102793:	89 04 24             	mov    %eax,(%esp)
80102796:	e8 31 fe ff ff       	call   801025cc <idestart>

  release(&idelock);
8010279b:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
801027a2:	e8 47 2a 00 00       	call   801051ee <release>
}
801027a7:	c9                   	leave  
801027a8:	c3                   	ret    

801027a9 <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027a9:	55                   	push   %ebp
801027aa:	89 e5                	mov    %esp,%ebp
801027ac:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027af:	8b 45 08             	mov    0x8(%ebp),%eax
801027b2:	8b 00                	mov    (%eax),%eax
801027b4:	83 e0 01             	and    $0x1,%eax
801027b7:	85 c0                	test   %eax,%eax
801027b9:	75 0c                	jne    801027c7 <iderw+0x1e>
    panic("iderw: buf not busy");
801027bb:	c7 04 24 45 8a 10 80 	movl   $0x80108a45,(%esp)
801027c2:	e8 76 dd ff ff       	call   8010053d <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027c7:	8b 45 08             	mov    0x8(%ebp),%eax
801027ca:	8b 00                	mov    (%eax),%eax
801027cc:	83 e0 06             	and    $0x6,%eax
801027cf:	83 f8 02             	cmp    $0x2,%eax
801027d2:	75 0c                	jne    801027e0 <iderw+0x37>
    panic("iderw: nothing to do");
801027d4:	c7 04 24 59 8a 10 80 	movl   $0x80108a59,(%esp)
801027db:	e8 5d dd ff ff       	call   8010053d <panic>
  if(b->dev != 0 && !havedisk1)
801027e0:	8b 45 08             	mov    0x8(%ebp),%eax
801027e3:	8b 40 04             	mov    0x4(%eax),%eax
801027e6:	85 c0                	test   %eax,%eax
801027e8:	74 15                	je     801027ff <iderw+0x56>
801027ea:	a1 38 c6 10 80       	mov    0x8010c638,%eax
801027ef:	85 c0                	test   %eax,%eax
801027f1:	75 0c                	jne    801027ff <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027f3:	c7 04 24 6e 8a 10 80 	movl   $0x80108a6e,(%esp)
801027fa:	e8 3e dd ff ff       	call   8010053d <panic>

  acquire(&idelock);  //DOC: acquire-lock
801027ff:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
80102806:	e8 48 29 00 00       	call   80105153 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
8010280b:	8b 45 08             	mov    0x8(%ebp),%eax
8010280e:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC: insert-queue
80102815:	c7 45 f4 34 c6 10 80 	movl   $0x8010c634,-0xc(%ebp)
8010281c:	eb 0b                	jmp    80102829 <iderw+0x80>
8010281e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102821:	8b 00                	mov    (%eax),%eax
80102823:	83 c0 14             	add    $0x14,%eax
80102826:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102829:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282c:	8b 00                	mov    (%eax),%eax
8010282e:	85 c0                	test   %eax,%eax
80102830:	75 ec                	jne    8010281e <iderw+0x75>
    ;
  *pp = b;
80102832:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102835:	8b 55 08             	mov    0x8(%ebp),%edx
80102838:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
8010283a:	a1 34 c6 10 80       	mov    0x8010c634,%eax
8010283f:	3b 45 08             	cmp    0x8(%ebp),%eax
80102842:	75 0b                	jne    8010284f <iderw+0xa6>
    idestart(b);
80102844:	8b 45 08             	mov    0x8(%ebp),%eax
80102847:	89 04 24             	mov    %eax,(%esp)
8010284a:	e8 7d fd ff ff       	call   801025cc <idestart>
  
  // Wait for request to finish.
    release(&idelock);
8010284f:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
80102856:	e8 93 29 00 00       	call   801051ee <release>
    while((b->flags & (B_VALID|B_DIRTY)) != B_VALID);
8010285b:	90                   	nop
8010285c:	8b 45 08             	mov    0x8(%ebp),%eax
8010285f:	8b 00                	mov    (%eax),%eax
80102861:	83 e0 06             	and    $0x6,%eax
80102864:	83 f8 02             	cmp    $0x2,%eax
80102867:	75 f3                	jne    8010285c <iderw+0xb3>
    
    
    //sleep(b, &idelock);

  if(holding(&idelock))
80102869:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
80102870:	e8 35 2a 00 00       	call   801052aa <holding>
80102875:	85 c0                	test   %eax,%eax
80102877:	74 0c                	je     80102885 <iderw+0xdc>
    release(&idelock);
80102879:	c7 04 24 00 c6 10 80 	movl   $0x8010c600,(%esp)
80102880:	e8 69 29 00 00       	call   801051ee <release>
}
80102885:	c9                   	leave  
80102886:	c3                   	ret    
	...

80102888 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102888:	55                   	push   %ebp
80102889:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
8010288b:	a1 34 08 11 80       	mov    0x80110834,%eax
80102890:	8b 55 08             	mov    0x8(%ebp),%edx
80102893:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102895:	a1 34 08 11 80       	mov    0x80110834,%eax
8010289a:	8b 40 10             	mov    0x10(%eax),%eax
}
8010289d:	5d                   	pop    %ebp
8010289e:	c3                   	ret    

8010289f <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
8010289f:	55                   	push   %ebp
801028a0:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
801028a2:	a1 34 08 11 80       	mov    0x80110834,%eax
801028a7:	8b 55 08             	mov    0x8(%ebp),%edx
801028aa:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
801028ac:	a1 34 08 11 80       	mov    0x80110834,%eax
801028b1:	8b 55 0c             	mov    0xc(%ebp),%edx
801028b4:	89 50 10             	mov    %edx,0x10(%eax)
}
801028b7:	5d                   	pop    %ebp
801028b8:	c3                   	ret    

801028b9 <ioapicinit>:

void
ioapicinit(void)
{
801028b9:	55                   	push   %ebp
801028ba:	89 e5                	mov    %esp,%ebp
801028bc:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028bf:	a1 04 09 11 80       	mov    0x80110904,%eax
801028c4:	85 c0                	test   %eax,%eax
801028c6:	0f 84 9f 00 00 00    	je     8010296b <ioapicinit+0xb2>
    return;

  ioapic = (volatile struct ioapic*)IOAPIC;
801028cc:	c7 05 34 08 11 80 00 	movl   $0xfec00000,0x80110834
801028d3:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028d6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028dd:	e8 a6 ff ff ff       	call   80102888 <ioapicread>
801028e2:	c1 e8 10             	shr    $0x10,%eax
801028e5:	25 ff 00 00 00       	and    $0xff,%eax
801028ea:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028ed:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028f4:	e8 8f ff ff ff       	call   80102888 <ioapicread>
801028f9:	c1 e8 18             	shr    $0x18,%eax
801028fc:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
801028ff:	0f b6 05 00 09 11 80 	movzbl 0x80110900,%eax
80102906:	0f b6 c0             	movzbl %al,%eax
80102909:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010290c:	74 0c                	je     8010291a <ioapicinit+0x61>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
8010290e:	c7 04 24 8c 8a 10 80 	movl   $0x80108a8c,(%esp)
80102915:	e8 87 da ff ff       	call   801003a1 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010291a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102921:	eb 3e                	jmp    80102961 <ioapicinit+0xa8>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102923:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102926:	83 c0 20             	add    $0x20,%eax
80102929:	0d 00 00 01 00       	or     $0x10000,%eax
8010292e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102931:	83 c2 08             	add    $0x8,%edx
80102934:	01 d2                	add    %edx,%edx
80102936:	89 44 24 04          	mov    %eax,0x4(%esp)
8010293a:	89 14 24             	mov    %edx,(%esp)
8010293d:	e8 5d ff ff ff       	call   8010289f <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102942:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102945:	83 c0 08             	add    $0x8,%eax
80102948:	01 c0                	add    %eax,%eax
8010294a:	83 c0 01             	add    $0x1,%eax
8010294d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102954:	00 
80102955:	89 04 24             	mov    %eax,(%esp)
80102958:	e8 42 ff ff ff       	call   8010289f <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010295d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102961:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102964:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102967:	7e ba                	jle    80102923 <ioapicinit+0x6a>
80102969:	eb 01                	jmp    8010296c <ioapicinit+0xb3>
ioapicinit(void)
{
  int i, id, maxintr;

  if(!ismp)
    return;
8010296b:	90                   	nop
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010296c:	c9                   	leave  
8010296d:	c3                   	ret    

8010296e <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010296e:	55                   	push   %ebp
8010296f:	89 e5                	mov    %esp,%ebp
80102971:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102974:	a1 04 09 11 80       	mov    0x80110904,%eax
80102979:	85 c0                	test   %eax,%eax
8010297b:	74 39                	je     801029b6 <ioapicenable+0x48>
    return;

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010297d:	8b 45 08             	mov    0x8(%ebp),%eax
80102980:	83 c0 20             	add    $0x20,%eax
80102983:	8b 55 08             	mov    0x8(%ebp),%edx
80102986:	83 c2 08             	add    $0x8,%edx
80102989:	01 d2                	add    %edx,%edx
8010298b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010298f:	89 14 24             	mov    %edx,(%esp)
80102992:	e8 08 ff ff ff       	call   8010289f <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102997:	8b 45 0c             	mov    0xc(%ebp),%eax
8010299a:	c1 e0 18             	shl    $0x18,%eax
8010299d:	8b 55 08             	mov    0x8(%ebp),%edx
801029a0:	83 c2 08             	add    $0x8,%edx
801029a3:	01 d2                	add    %edx,%edx
801029a5:	83 c2 01             	add    $0x1,%edx
801029a8:	89 44 24 04          	mov    %eax,0x4(%esp)
801029ac:	89 14 24             	mov    %edx,(%esp)
801029af:	e8 eb fe ff ff       	call   8010289f <ioapicwrite>
801029b4:	eb 01                	jmp    801029b7 <ioapicenable+0x49>

void
ioapicenable(int irq, int cpunum)
{
  if(!ismp)
    return;
801029b6:	90                   	nop
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
}
801029b7:	c9                   	leave  
801029b8:	c3                   	ret    
801029b9:	00 00                	add    %al,(%eax)
	...

801029bc <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029bc:	55                   	push   %ebp
801029bd:	89 e5                	mov    %esp,%ebp
801029bf:	8b 45 08             	mov    0x8(%ebp),%eax
801029c2:	05 00 00 00 80       	add    $0x80000000,%eax
801029c7:	5d                   	pop    %ebp
801029c8:	c3                   	ret    

801029c9 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029c9:	55                   	push   %ebp
801029ca:	89 e5                	mov    %esp,%ebp
801029cc:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029cf:	c7 44 24 04 be 8a 10 	movl   $0x80108abe,0x4(%esp)
801029d6:	80 
801029d7:	c7 04 24 40 08 11 80 	movl   $0x80110840,(%esp)
801029de:	e8 4f 27 00 00       	call   80105132 <initlock>
  kmem.use_lock = 0;
801029e3:	c7 05 74 08 11 80 00 	movl   $0x0,0x80110874
801029ea:	00 00 00 
  freerange(vstart, vend);
801029ed:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f0:	89 44 24 04          	mov    %eax,0x4(%esp)
801029f4:	8b 45 08             	mov    0x8(%ebp),%eax
801029f7:	89 04 24             	mov    %eax,(%esp)
801029fa:	e8 26 00 00 00       	call   80102a25 <freerange>
}
801029ff:	c9                   	leave  
80102a00:	c3                   	ret    

80102a01 <kinit2>:

void
kinit2(void *vstart, void *vend)
{
80102a01:	55                   	push   %ebp
80102a02:	89 e5                	mov    %esp,%ebp
80102a04:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
80102a07:	8b 45 0c             	mov    0xc(%ebp),%eax
80102a0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80102a0e:	8b 45 08             	mov    0x8(%ebp),%eax
80102a11:	89 04 24             	mov    %eax,(%esp)
80102a14:	e8 0c 00 00 00       	call   80102a25 <freerange>
  kmem.use_lock = 1;
80102a19:	c7 05 74 08 11 80 01 	movl   $0x1,0x80110874
80102a20:	00 00 00 
}
80102a23:	c9                   	leave  
80102a24:	c3                   	ret    

80102a25 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a25:	55                   	push   %ebp
80102a26:	89 e5                	mov    %esp,%ebp
80102a28:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a2b:	8b 45 08             	mov    0x8(%ebp),%eax
80102a2e:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a33:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a38:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a3b:	eb 12                	jmp    80102a4f <freerange+0x2a>
    kfree(p);
80102a3d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a40:	89 04 24             	mov    %eax,(%esp)
80102a43:	e8 16 00 00 00       	call   80102a5e <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a48:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a52:	05 00 10 00 00       	add    $0x1000,%eax
80102a57:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a5a:	76 e1                	jbe    80102a3d <freerange+0x18>
    kfree(p);
}
80102a5c:	c9                   	leave  
80102a5d:	c3                   	ret    

80102a5e <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a5e:	55                   	push   %ebp
80102a5f:	89 e5                	mov    %esp,%ebp
80102a61:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a64:	8b 45 08             	mov    0x8(%ebp),%eax
80102a67:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a6c:	85 c0                	test   %eax,%eax
80102a6e:	75 1b                	jne    80102a8b <kfree+0x2d>
80102a70:	81 7d 08 fc 3a 11 80 	cmpl   $0x80113afc,0x8(%ebp)
80102a77:	72 12                	jb     80102a8b <kfree+0x2d>
80102a79:	8b 45 08             	mov    0x8(%ebp),%eax
80102a7c:	89 04 24             	mov    %eax,(%esp)
80102a7f:	e8 38 ff ff ff       	call   801029bc <v2p>
80102a84:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a89:	76 0c                	jbe    80102a97 <kfree+0x39>
    panic("kfree");
80102a8b:	c7 04 24 c3 8a 10 80 	movl   $0x80108ac3,(%esp)
80102a92:	e8 a6 da ff ff       	call   8010053d <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a97:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102a9e:	00 
80102a9f:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102aa6:	00 
80102aa7:	8b 45 08             	mov    0x8(%ebp),%eax
80102aaa:	89 04 24             	mov    %eax,(%esp)
80102aad:	e8 28 29 00 00       	call   801053da <memset>

  if(kmem.use_lock)
80102ab2:	a1 74 08 11 80       	mov    0x80110874,%eax
80102ab7:	85 c0                	test   %eax,%eax
80102ab9:	74 0c                	je     80102ac7 <kfree+0x69>
    acquire(&kmem.lock);
80102abb:	c7 04 24 40 08 11 80 	movl   $0x80110840,(%esp)
80102ac2:	e8 8c 26 00 00       	call   80105153 <acquire>
  r = (struct run*)v;
80102ac7:	8b 45 08             	mov    0x8(%ebp),%eax
80102aca:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102acd:	8b 15 78 08 11 80    	mov    0x80110878,%edx
80102ad3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ad6:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ad8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102adb:	a3 78 08 11 80       	mov    %eax,0x80110878
  if(kmem.use_lock)
80102ae0:	a1 74 08 11 80       	mov    0x80110874,%eax
80102ae5:	85 c0                	test   %eax,%eax
80102ae7:	74 0c                	je     80102af5 <kfree+0x97>
    release(&kmem.lock);
80102ae9:	c7 04 24 40 08 11 80 	movl   $0x80110840,(%esp)
80102af0:	e8 f9 26 00 00       	call   801051ee <release>
}
80102af5:	c9                   	leave  
80102af6:	c3                   	ret    

80102af7 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102af7:	55                   	push   %ebp
80102af8:	89 e5                	mov    %esp,%ebp
80102afa:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102afd:	a1 74 08 11 80       	mov    0x80110874,%eax
80102b02:	85 c0                	test   %eax,%eax
80102b04:	74 0c                	je     80102b12 <kalloc+0x1b>
    acquire(&kmem.lock);
80102b06:	c7 04 24 40 08 11 80 	movl   $0x80110840,(%esp)
80102b0d:	e8 41 26 00 00       	call   80105153 <acquire>
  r = kmem.freelist;
80102b12:	a1 78 08 11 80       	mov    0x80110878,%eax
80102b17:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b1a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b1e:	74 0a                	je     80102b2a <kalloc+0x33>
    kmem.freelist = r->next;
80102b20:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b23:	8b 00                	mov    (%eax),%eax
80102b25:	a3 78 08 11 80       	mov    %eax,0x80110878
  if(kmem.use_lock)
80102b2a:	a1 74 08 11 80       	mov    0x80110874,%eax
80102b2f:	85 c0                	test   %eax,%eax
80102b31:	74 0c                	je     80102b3f <kalloc+0x48>
    release(&kmem.lock);
80102b33:	c7 04 24 40 08 11 80 	movl   $0x80110840,(%esp)
80102b3a:	e8 af 26 00 00       	call   801051ee <release>
  return (char*)r;
80102b3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b42:	c9                   	leave  
80102b43:	c3                   	ret    

80102b44 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b44:	55                   	push   %ebp
80102b45:	89 e5                	mov    %esp,%ebp
80102b47:	53                   	push   %ebx
80102b48:	83 ec 14             	sub    $0x14,%esp
80102b4b:	8b 45 08             	mov    0x8(%ebp),%eax
80102b4e:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b52:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102b56:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102b5a:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80102b5e:	ec                   	in     (%dx),%al
80102b5f:	89 c3                	mov    %eax,%ebx
80102b61:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102b64:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102b68:	83 c4 14             	add    $0x14,%esp
80102b6b:	5b                   	pop    %ebx
80102b6c:	5d                   	pop    %ebp
80102b6d:	c3                   	ret    

80102b6e <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b6e:	55                   	push   %ebp
80102b6f:	89 e5                	mov    %esp,%ebp
80102b71:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b74:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b7b:	e8 c4 ff ff ff       	call   80102b44 <inb>
80102b80:	0f b6 c0             	movzbl %al,%eax
80102b83:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b89:	83 e0 01             	and    $0x1,%eax
80102b8c:	85 c0                	test   %eax,%eax
80102b8e:	75 0a                	jne    80102b9a <kbdgetc+0x2c>
    return -1;
80102b90:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b95:	e9 23 01 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  data = inb(KBDATAP);
80102b9a:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102ba1:	e8 9e ff ff ff       	call   80102b44 <inb>
80102ba6:	0f b6 c0             	movzbl %al,%eax
80102ba9:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102bac:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102bb3:	75 17                	jne    80102bcc <kbdgetc+0x5e>
    shift |= E0ESC;
80102bb5:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102bba:	83 c8 40             	or     $0x40,%eax
80102bbd:	a3 3c c6 10 80       	mov    %eax,0x8010c63c
    return 0;
80102bc2:	b8 00 00 00 00       	mov    $0x0,%eax
80102bc7:	e9 f1 00 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  } else if(data & 0x80){
80102bcc:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bcf:	25 80 00 00 00       	and    $0x80,%eax
80102bd4:	85 c0                	test   %eax,%eax
80102bd6:	74 45                	je     80102c1d <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102bd8:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102bdd:	83 e0 40             	and    $0x40,%eax
80102be0:	85 c0                	test   %eax,%eax
80102be2:	75 08                	jne    80102bec <kbdgetc+0x7e>
80102be4:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102be7:	83 e0 7f             	and    $0x7f,%eax
80102bea:	eb 03                	jmp    80102bef <kbdgetc+0x81>
80102bec:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bef:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bf2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bf5:	05 20 a0 10 80       	add    $0x8010a020,%eax
80102bfa:	0f b6 00             	movzbl (%eax),%eax
80102bfd:	83 c8 40             	or     $0x40,%eax
80102c00:	0f b6 c0             	movzbl %al,%eax
80102c03:	f7 d0                	not    %eax
80102c05:	89 c2                	mov    %eax,%edx
80102c07:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c0c:	21 d0                	and    %edx,%eax
80102c0e:	a3 3c c6 10 80       	mov    %eax,0x8010c63c
    return 0;
80102c13:	b8 00 00 00 00       	mov    $0x0,%eax
80102c18:	e9 a0 00 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  } else if(shift & E0ESC){
80102c1d:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c22:	83 e0 40             	and    $0x40,%eax
80102c25:	85 c0                	test   %eax,%eax
80102c27:	74 14                	je     80102c3d <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c29:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c30:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c35:	83 e0 bf             	and    $0xffffffbf,%eax
80102c38:	a3 3c c6 10 80       	mov    %eax,0x8010c63c
  }

  shift |= shiftcode[data];
80102c3d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c40:	05 20 a0 10 80       	add    $0x8010a020,%eax
80102c45:	0f b6 00             	movzbl (%eax),%eax
80102c48:	0f b6 d0             	movzbl %al,%edx
80102c4b:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c50:	09 d0                	or     %edx,%eax
80102c52:	a3 3c c6 10 80       	mov    %eax,0x8010c63c
  shift ^= togglecode[data];
80102c57:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c5a:	05 20 a1 10 80       	add    $0x8010a120,%eax
80102c5f:	0f b6 00             	movzbl (%eax),%eax
80102c62:	0f b6 d0             	movzbl %al,%edx
80102c65:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c6a:	31 d0                	xor    %edx,%eax
80102c6c:	a3 3c c6 10 80       	mov    %eax,0x8010c63c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c71:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c76:	83 e0 03             	and    $0x3,%eax
80102c79:	8b 04 85 20 a5 10 80 	mov    -0x7fef5ae0(,%eax,4),%eax
80102c80:	03 45 fc             	add    -0x4(%ebp),%eax
80102c83:	0f b6 00             	movzbl (%eax),%eax
80102c86:	0f b6 c0             	movzbl %al,%eax
80102c89:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c8c:	a1 3c c6 10 80       	mov    0x8010c63c,%eax
80102c91:	83 e0 08             	and    $0x8,%eax
80102c94:	85 c0                	test   %eax,%eax
80102c96:	74 22                	je     80102cba <kbdgetc+0x14c>
    if('a' <= c && c <= 'z')
80102c98:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102c9c:	76 0c                	jbe    80102caa <kbdgetc+0x13c>
80102c9e:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102ca2:	77 06                	ja     80102caa <kbdgetc+0x13c>
      c += 'A' - 'a';
80102ca4:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102ca8:	eb 10                	jmp    80102cba <kbdgetc+0x14c>
    else if('A' <= c && c <= 'Z')
80102caa:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102cae:	76 0a                	jbe    80102cba <kbdgetc+0x14c>
80102cb0:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102cb4:	77 04                	ja     80102cba <kbdgetc+0x14c>
      c += 'a' - 'A';
80102cb6:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102cba:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102cbd:	c9                   	leave  
80102cbe:	c3                   	ret    

80102cbf <kbdintr>:

void
kbdintr(void)
{
80102cbf:	55                   	push   %ebp
80102cc0:	89 e5                	mov    %esp,%ebp
80102cc2:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102cc5:	c7 04 24 6e 2b 10 80 	movl   $0x80102b6e,(%esp)
80102ccc:	e8 dc da ff ff       	call   801007ad <consoleintr>
}
80102cd1:	c9                   	leave  
80102cd2:	c3                   	ret    
	...

80102cd4 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cd4:	55                   	push   %ebp
80102cd5:	89 e5                	mov    %esp,%ebp
80102cd7:	83 ec 08             	sub    $0x8,%esp
80102cda:	8b 55 08             	mov    0x8(%ebp),%edx
80102cdd:	8b 45 0c             	mov    0xc(%ebp),%eax
80102ce0:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102ce4:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ce7:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102ceb:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102cef:	ee                   	out    %al,(%dx)
}
80102cf0:	c9                   	leave  
80102cf1:	c3                   	ret    

80102cf2 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cf2:	55                   	push   %ebp
80102cf3:	89 e5                	mov    %esp,%ebp
80102cf5:	53                   	push   %ebx
80102cf6:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102cf9:	9c                   	pushf  
80102cfa:	5b                   	pop    %ebx
80102cfb:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80102cfe:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102d01:	83 c4 10             	add    $0x10,%esp
80102d04:	5b                   	pop    %ebx
80102d05:	5d                   	pop    %ebp
80102d06:	c3                   	ret    

80102d07 <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102d07:	55                   	push   %ebp
80102d08:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102d0a:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102d0f:	8b 55 08             	mov    0x8(%ebp),%edx
80102d12:	c1 e2 02             	shl    $0x2,%edx
80102d15:	01 c2                	add    %eax,%edx
80102d17:	8b 45 0c             	mov    0xc(%ebp),%eax
80102d1a:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102d1c:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102d21:	83 c0 20             	add    $0x20,%eax
80102d24:	8b 00                	mov    (%eax),%eax
}
80102d26:	5d                   	pop    %ebp
80102d27:	c3                   	ret    

80102d28 <lapicinit>:
//PAGEBREAK!

void
lapicinit(int c)
{
80102d28:	55                   	push   %ebp
80102d29:	89 e5                	mov    %esp,%ebp
80102d2b:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d2e:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102d33:	85 c0                	test   %eax,%eax
80102d35:	0f 84 47 01 00 00    	je     80102e82 <lapicinit+0x15a>
    return;

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d3b:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d42:	00 
80102d43:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d4a:	e8 b8 ff ff ff       	call   80102d07 <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d4f:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d56:	00 
80102d57:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d5e:	e8 a4 ff ff ff       	call   80102d07 <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d63:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d6a:	00 
80102d6b:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d72:	e8 90 ff ff ff       	call   80102d07 <lapicw>
  lapicw(TICR, 10000000); 
80102d77:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d7e:	00 
80102d7f:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d86:	e8 7c ff ff ff       	call   80102d07 <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d8b:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d92:	00 
80102d93:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102d9a:	e8 68 ff ff ff       	call   80102d07 <lapicw>
  lapicw(LINT1, MASKED);
80102d9f:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102da6:	00 
80102da7:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102dae:	e8 54 ff ff ff       	call   80102d07 <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102db3:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102db8:	83 c0 30             	add    $0x30,%eax
80102dbb:	8b 00                	mov    (%eax),%eax
80102dbd:	c1 e8 10             	shr    $0x10,%eax
80102dc0:	25 ff 00 00 00       	and    $0xff,%eax
80102dc5:	83 f8 03             	cmp    $0x3,%eax
80102dc8:	76 14                	jbe    80102dde <lapicinit+0xb6>
    lapicw(PCINT, MASKED);
80102dca:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102dd1:	00 
80102dd2:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102dd9:	e8 29 ff ff ff       	call   80102d07 <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102dde:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102de5:	00 
80102de6:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102ded:	e8 15 ff ff ff       	call   80102d07 <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102df2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102df9:	00 
80102dfa:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e01:	e8 01 ff ff ff       	call   80102d07 <lapicw>
  lapicw(ESR, 0);
80102e06:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e0d:	00 
80102e0e:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e15:	e8 ed fe ff ff       	call   80102d07 <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102e1a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e21:	00 
80102e22:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e29:	e8 d9 fe ff ff       	call   80102d07 <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e2e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e35:	00 
80102e36:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e3d:	e8 c5 fe ff ff       	call   80102d07 <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e42:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e49:	00 
80102e4a:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e51:	e8 b1 fe ff ff       	call   80102d07 <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e56:	90                   	nop
80102e57:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102e5c:	05 00 03 00 00       	add    $0x300,%eax
80102e61:	8b 00                	mov    (%eax),%eax
80102e63:	25 00 10 00 00       	and    $0x1000,%eax
80102e68:	85 c0                	test   %eax,%eax
80102e6a:	75 eb                	jne    80102e57 <lapicinit+0x12f>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e6c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e73:	00 
80102e74:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e7b:	e8 87 fe ff ff       	call   80102d07 <lapicw>
80102e80:	eb 01                	jmp    80102e83 <lapicinit+0x15b>

void
lapicinit(int c)
{
  if(!lapic) 
    return;
80102e82:	90                   	nop
  while(lapic[ICRLO] & DELIVS)
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102e83:	c9                   	leave  
80102e84:	c3                   	ret    

80102e85 <cpunum>:

int
cpunum(void)
{
80102e85:	55                   	push   %ebp
80102e86:	89 e5                	mov    %esp,%ebp
80102e88:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e8b:	e8 62 fe ff ff       	call   80102cf2 <readeflags>
80102e90:	25 00 02 00 00       	and    $0x200,%eax
80102e95:	85 c0                	test   %eax,%eax
80102e97:	74 29                	je     80102ec2 <cpunum+0x3d>
    static int n;
    if(n++ == 0)
80102e99:	a1 40 c6 10 80       	mov    0x8010c640,%eax
80102e9e:	85 c0                	test   %eax,%eax
80102ea0:	0f 94 c2             	sete   %dl
80102ea3:	83 c0 01             	add    $0x1,%eax
80102ea6:	a3 40 c6 10 80       	mov    %eax,0x8010c640
80102eab:	84 d2                	test   %dl,%dl
80102ead:	74 13                	je     80102ec2 <cpunum+0x3d>
      cprintf("cpu called from %x with interrupts enabled\n",
80102eaf:	8b 45 04             	mov    0x4(%ebp),%eax
80102eb2:	89 44 24 04          	mov    %eax,0x4(%esp)
80102eb6:	c7 04 24 cc 8a 10 80 	movl   $0x80108acc,(%esp)
80102ebd:	e8 df d4 ff ff       	call   801003a1 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102ec2:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102ec7:	85 c0                	test   %eax,%eax
80102ec9:	74 0f                	je     80102eda <cpunum+0x55>
    return lapic[ID]>>24;
80102ecb:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102ed0:	83 c0 20             	add    $0x20,%eax
80102ed3:	8b 00                	mov    (%eax),%eax
80102ed5:	c1 e8 18             	shr    $0x18,%eax
80102ed8:	eb 05                	jmp    80102edf <cpunum+0x5a>
  return 0;
80102eda:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102edf:	c9                   	leave  
80102ee0:	c3                   	ret    

80102ee1 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102ee1:	55                   	push   %ebp
80102ee2:	89 e5                	mov    %esp,%ebp
80102ee4:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102ee7:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80102eec:	85 c0                	test   %eax,%eax
80102eee:	74 14                	je     80102f04 <lapiceoi+0x23>
    lapicw(EOI, 0);
80102ef0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ef7:	00 
80102ef8:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102eff:	e8 03 fe ff ff       	call   80102d07 <lapicw>
}
80102f04:	c9                   	leave  
80102f05:	c3                   	ret    

80102f06 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102f06:	55                   	push   %ebp
80102f07:	89 e5                	mov    %esp,%ebp
}
80102f09:	5d                   	pop    %ebp
80102f0a:	c3                   	ret    

80102f0b <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102f0b:	55                   	push   %ebp
80102f0c:	89 e5                	mov    %esp,%ebp
80102f0e:	83 ec 1c             	sub    $0x1c,%esp
80102f11:	8b 45 08             	mov    0x8(%ebp),%eax
80102f14:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
80102f17:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102f1e:	00 
80102f1f:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102f26:	e8 a9 fd ff ff       	call   80102cd4 <outb>
  outb(IO_RTC+1, 0x0A);
80102f2b:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f32:	00 
80102f33:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f3a:	e8 95 fd ff ff       	call   80102cd4 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f3f:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f46:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f49:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f4e:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f51:	8d 50 02             	lea    0x2(%eax),%edx
80102f54:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f57:	c1 e8 04             	shr    $0x4,%eax
80102f5a:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f5d:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f61:	c1 e0 18             	shl    $0x18,%eax
80102f64:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f68:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f6f:	e8 93 fd ff ff       	call   80102d07 <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f74:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f7b:	00 
80102f7c:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f83:	e8 7f fd ff ff       	call   80102d07 <lapicw>
  microdelay(200);
80102f88:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f8f:	e8 72 ff ff ff       	call   80102f06 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f94:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102f9b:	00 
80102f9c:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fa3:	e8 5f fd ff ff       	call   80102d07 <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102fa8:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102faf:	e8 52 ff ff ff       	call   80102f06 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fb4:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102fbb:	eb 40                	jmp    80102ffd <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102fbd:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102fc1:	c1 e0 18             	shl    $0x18,%eax
80102fc4:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fc8:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fcf:	e8 33 fd ff ff       	call   80102d07 <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fd4:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fd7:	c1 e8 0c             	shr    $0xc,%eax
80102fda:	80 cc 06             	or     $0x6,%ah
80102fdd:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fe1:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fe8:	e8 1a fd ff ff       	call   80102d07 <lapicw>
    microdelay(200);
80102fed:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102ff4:	e8 0d ff ff ff       	call   80102f06 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102ff9:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80102ffd:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80103001:	7e ba                	jle    80102fbd <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80103003:	c9                   	leave  
80103004:	c3                   	ret    
80103005:	00 00                	add    %al,(%eax)
	...

80103008 <initlog>:

static void recover_from_log(void);

void
initlog(void)
{
80103008:	55                   	push   %ebp
80103009:	89 e5                	mov    %esp,%ebp
8010300b:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
8010300e:	c7 44 24 04 f8 8a 10 	movl   $0x80108af8,0x4(%esp)
80103015:	80 
80103016:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
8010301d:	e8 10 21 00 00       	call   80105132 <initlock>
  readsb(ROOTDEV, &sb);
80103022:	8d 45 e8             	lea    -0x18(%ebp),%eax
80103025:	89 44 24 04          	mov    %eax,0x4(%esp)
80103029:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103030:	e8 b7 e2 ff ff       	call   801012ec <readsb>
  log.start = sb.size - sb.nlog;
80103035:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103038:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010303b:	89 d1                	mov    %edx,%ecx
8010303d:	29 c1                	sub    %eax,%ecx
8010303f:	89 c8                	mov    %ecx,%eax
80103041:	a3 b4 08 11 80       	mov    %eax,0x801108b4
  log.size = sb.nlog;
80103046:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103049:	a3 b8 08 11 80       	mov    %eax,0x801108b8
  log.dev = ROOTDEV;
8010304e:	c7 05 c0 08 11 80 01 	movl   $0x1,0x801108c0
80103055:	00 00 00 
  recover_from_log();
80103058:	e8 97 01 00 00       	call   801031f4 <recover_from_log>
}
8010305d:	c9                   	leave  
8010305e:	c3                   	ret    

8010305f <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
8010305f:	55                   	push   %ebp
80103060:	89 e5                	mov    %esp,%ebp
80103062:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103065:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010306c:	e9 89 00 00 00       	jmp    801030fa <install_trans+0x9b>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103071:	a1 b4 08 11 80       	mov    0x801108b4,%eax
80103076:	03 45 f4             	add    -0xc(%ebp),%eax
80103079:	83 c0 01             	add    $0x1,%eax
8010307c:	89 c2                	mov    %eax,%edx
8010307e:	a1 c0 08 11 80       	mov    0x801108c0,%eax
80103083:	89 54 24 04          	mov    %edx,0x4(%esp)
80103087:	89 04 24             	mov    %eax,(%esp)
8010308a:	e8 17 d1 ff ff       	call   801001a6 <bread>
8010308f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
80103092:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103095:	83 c0 10             	add    $0x10,%eax
80103098:	8b 04 85 88 08 11 80 	mov    -0x7feef778(,%eax,4),%eax
8010309f:	89 c2                	mov    %eax,%edx
801030a1:	a1 c0 08 11 80       	mov    0x801108c0,%eax
801030a6:	89 54 24 04          	mov    %edx,0x4(%esp)
801030aa:	89 04 24             	mov    %eax,(%esp)
801030ad:	e8 f4 d0 ff ff       	call   801001a6 <bread>
801030b2:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801030b5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030b8:	8d 50 18             	lea    0x18(%eax),%edx
801030bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030be:	83 c0 18             	add    $0x18,%eax
801030c1:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801030c8:	00 
801030c9:	89 54 24 04          	mov    %edx,0x4(%esp)
801030cd:	89 04 24             	mov    %eax,(%esp)
801030d0:	e8 d8 23 00 00       	call   801054ad <memmove>
    bwrite(dbuf);  // write dst to disk
801030d5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030d8:	89 04 24             	mov    %eax,(%esp)
801030db:	e8 fd d0 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801030e0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030e3:	89 04 24             	mov    %eax,(%esp)
801030e6:	e8 2c d1 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801030eb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030ee:	89 04 24             	mov    %eax,(%esp)
801030f1:	e8 21 d1 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801030f6:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801030fa:	a1 c4 08 11 80       	mov    0x801108c4,%eax
801030ff:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103102:	0f 8f 69 ff ff ff    	jg     80103071 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
80103108:	c9                   	leave  
80103109:	c3                   	ret    

8010310a <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
8010310a:	55                   	push   %ebp
8010310b:	89 e5                	mov    %esp,%ebp
8010310d:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103110:	a1 b4 08 11 80       	mov    0x801108b4,%eax
80103115:	89 c2                	mov    %eax,%edx
80103117:	a1 c0 08 11 80       	mov    0x801108c0,%eax
8010311c:	89 54 24 04          	mov    %edx,0x4(%esp)
80103120:	89 04 24             	mov    %eax,(%esp)
80103123:	e8 7e d0 ff ff       	call   801001a6 <bread>
80103128:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
8010312b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010312e:	83 c0 18             	add    $0x18,%eax
80103131:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
80103134:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103137:	8b 00                	mov    (%eax),%eax
80103139:	a3 c4 08 11 80       	mov    %eax,0x801108c4
  for (i = 0; i < log.lh.n; i++) {
8010313e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103145:	eb 1b                	jmp    80103162 <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
80103147:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010314a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010314d:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103151:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103154:	83 c2 10             	add    $0x10,%edx
80103157:	89 04 95 88 08 11 80 	mov    %eax,-0x7feef778(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
8010315e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103162:	a1 c4 08 11 80       	mov    0x801108c4,%eax
80103167:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010316a:	7f db                	jg     80103147 <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
8010316c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010316f:	89 04 24             	mov    %eax,(%esp)
80103172:	e8 a0 d0 ff ff       	call   80100217 <brelse>
}
80103177:	c9                   	leave  
80103178:	c3                   	ret    

80103179 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103179:	55                   	push   %ebp
8010317a:	89 e5                	mov    %esp,%ebp
8010317c:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010317f:	a1 b4 08 11 80       	mov    0x801108b4,%eax
80103184:	89 c2                	mov    %eax,%edx
80103186:	a1 c0 08 11 80       	mov    0x801108c0,%eax
8010318b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010318f:	89 04 24             	mov    %eax,(%esp)
80103192:	e8 0f d0 ff ff       	call   801001a6 <bread>
80103197:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
8010319a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010319d:	83 c0 18             	add    $0x18,%eax
801031a0:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
801031a3:	8b 15 c4 08 11 80    	mov    0x801108c4,%edx
801031a9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031ac:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801031ae:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801031b5:	eb 1b                	jmp    801031d2 <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
801031b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801031ba:	83 c0 10             	add    $0x10,%eax
801031bd:	8b 0c 85 88 08 11 80 	mov    -0x7feef778(,%eax,4),%ecx
801031c4:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031c7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801031ca:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801031ce:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801031d2:	a1 c4 08 11 80       	mov    0x801108c4,%eax
801031d7:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801031da:	7f db                	jg     801031b7 <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801031dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031df:	89 04 24             	mov    %eax,(%esp)
801031e2:	e8 f6 cf ff ff       	call   801001dd <bwrite>
  brelse(buf);
801031e7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031ea:	89 04 24             	mov    %eax,(%esp)
801031ed:	e8 25 d0 ff ff       	call   80100217 <brelse>
}
801031f2:	c9                   	leave  
801031f3:	c3                   	ret    

801031f4 <recover_from_log>:

static void
recover_from_log(void)
{
801031f4:	55                   	push   %ebp
801031f5:	89 e5                	mov    %esp,%ebp
801031f7:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801031fa:	e8 0b ff ff ff       	call   8010310a <read_head>
  install_trans(); // if committed, copy from log to disk
801031ff:	e8 5b fe ff ff       	call   8010305f <install_trans>
  log.lh.n = 0;
80103204:	c7 05 c4 08 11 80 00 	movl   $0x0,0x801108c4
8010320b:	00 00 00 
  write_head(); // clear the log
8010320e:	e8 66 ff ff ff       	call   80103179 <write_head>
}
80103213:	c9                   	leave  
80103214:	c3                   	ret    

80103215 <begin_trans>:

void
begin_trans(void)
{
80103215:	55                   	push   %ebp
80103216:	89 e5                	mov    %esp,%ebp
80103218:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
8010321b:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80103222:	e8 2c 1f 00 00       	call   80105153 <acquire>
  while (log.busy) {
80103227:	eb 14                	jmp    8010323d <begin_trans+0x28>
  sleep(&log, &log.lock);
80103229:	c7 44 24 04 80 08 11 	movl   $0x80110880,0x4(%esp)
80103230:	80 
80103231:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80103238:	e8 a8 1b 00 00       	call   80104de5 <sleep>

void
begin_trans(void)
{
  acquire(&log.lock);
  while (log.busy) {
8010323d:	a1 bc 08 11 80       	mov    0x801108bc,%eax
80103242:	85 c0                	test   %eax,%eax
80103244:	75 e3                	jne    80103229 <begin_trans+0x14>
  sleep(&log, &log.lock);
  }
  log.busy = 1;
80103246:	c7 05 bc 08 11 80 01 	movl   $0x1,0x801108bc
8010324d:	00 00 00 
  release(&log.lock);
80103250:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80103257:	e8 92 1f 00 00       	call   801051ee <release>
}
8010325c:	c9                   	leave  
8010325d:	c3                   	ret    

8010325e <commit_trans>:

void
commit_trans(void)
{
8010325e:	55                   	push   %ebp
8010325f:	89 e5                	mov    %esp,%ebp
80103261:	83 ec 18             	sub    $0x18,%esp
  if (log.lh.n > 0) {
80103264:	a1 c4 08 11 80       	mov    0x801108c4,%eax
80103269:	85 c0                	test   %eax,%eax
8010326b:	7e 19                	jle    80103286 <commit_trans+0x28>
    write_head();    // Write header to disk -- the real commit
8010326d:	e8 07 ff ff ff       	call   80103179 <write_head>
    install_trans(); // Now install writes to home locations
80103272:	e8 e8 fd ff ff       	call   8010305f <install_trans>
    log.lh.n = 0; 
80103277:	c7 05 c4 08 11 80 00 	movl   $0x0,0x801108c4
8010327e:	00 00 00 
    write_head();    // Erase the transaction from the log
80103281:	e8 f3 fe ff ff       	call   80103179 <write_head>
  }
  
  acquire(&log.lock);
80103286:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
8010328d:	e8 c1 1e 00 00       	call   80105153 <acquire>
  log.busy = 0;
80103292:	c7 05 bc 08 11 80 00 	movl   $0x0,0x801108bc
80103299:	00 00 00 
  wakeup(&log);
8010329c:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
801032a3:	e8 a0 1c 00 00       	call   80104f48 <wakeup>
  release(&log.lock);
801032a8:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
801032af:	e8 3a 1f 00 00       	call   801051ee <release>
}
801032b4:	c9                   	leave  
801032b5:	c3                   	ret    

801032b6 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
801032b6:	55                   	push   %ebp
801032b7:	89 e5                	mov    %esp,%ebp
801032b9:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
801032bc:	a1 c4 08 11 80       	mov    0x801108c4,%eax
801032c1:	83 f8 09             	cmp    $0x9,%eax
801032c4:	7f 12                	jg     801032d8 <log_write+0x22>
801032c6:	a1 c4 08 11 80       	mov    0x801108c4,%eax
801032cb:	8b 15 b8 08 11 80    	mov    0x801108b8,%edx
801032d1:	83 ea 01             	sub    $0x1,%edx
801032d4:	39 d0                	cmp    %edx,%eax
801032d6:	7c 0c                	jl     801032e4 <log_write+0x2e>
    panic("too big a transaction");
801032d8:	c7 04 24 fc 8a 10 80 	movl   $0x80108afc,(%esp)
801032df:	e8 59 d2 ff ff       	call   8010053d <panic>
  if (!log.busy)
801032e4:	a1 bc 08 11 80       	mov    0x801108bc,%eax
801032e9:	85 c0                	test   %eax,%eax
801032eb:	75 0c                	jne    801032f9 <log_write+0x43>
    panic("write outside of trans");
801032ed:	c7 04 24 12 8b 10 80 	movl   $0x80108b12,(%esp)
801032f4:	e8 44 d2 ff ff       	call   8010053d <panic>

  for (i = 0; i < log.lh.n; i++) {
801032f9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103300:	eb 1d                	jmp    8010331f <log_write+0x69>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
80103302:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103305:	83 c0 10             	add    $0x10,%eax
80103308:	8b 04 85 88 08 11 80 	mov    -0x7feef778(,%eax,4),%eax
8010330f:	89 c2                	mov    %eax,%edx
80103311:	8b 45 08             	mov    0x8(%ebp),%eax
80103314:	8b 40 08             	mov    0x8(%eax),%eax
80103317:	39 c2                	cmp    %eax,%edx
80103319:	74 10                	je     8010332b <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (!log.busy)
    panic("write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
8010331b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010331f:	a1 c4 08 11 80       	mov    0x801108c4,%eax
80103324:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103327:	7f d9                	jg     80103302 <log_write+0x4c>
80103329:	eb 01                	jmp    8010332c <log_write+0x76>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
      break;
8010332b:	90                   	nop
  }
  log.lh.sector[i] = b->sector;
8010332c:	8b 45 08             	mov    0x8(%ebp),%eax
8010332f:	8b 40 08             	mov    0x8(%eax),%eax
80103332:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103335:	83 c2 10             	add    $0x10,%edx
80103338:	89 04 95 88 08 11 80 	mov    %eax,-0x7feef778(,%edx,4)
  struct buf *lbuf = bread(b->dev, log.start+i+1);
8010333f:	a1 b4 08 11 80       	mov    0x801108b4,%eax
80103344:	03 45 f4             	add    -0xc(%ebp),%eax
80103347:	83 c0 01             	add    $0x1,%eax
8010334a:	89 c2                	mov    %eax,%edx
8010334c:	8b 45 08             	mov    0x8(%ebp),%eax
8010334f:	8b 40 04             	mov    0x4(%eax),%eax
80103352:	89 54 24 04          	mov    %edx,0x4(%esp)
80103356:	89 04 24             	mov    %eax,(%esp)
80103359:	e8 48 ce ff ff       	call   801001a6 <bread>
8010335e:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(lbuf->data, b->data, BSIZE);
80103361:	8b 45 08             	mov    0x8(%ebp),%eax
80103364:	8d 50 18             	lea    0x18(%eax),%edx
80103367:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010336a:	83 c0 18             	add    $0x18,%eax
8010336d:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103374:	00 
80103375:	89 54 24 04          	mov    %edx,0x4(%esp)
80103379:	89 04 24             	mov    %eax,(%esp)
8010337c:	e8 2c 21 00 00       	call   801054ad <memmove>
  bwrite(lbuf);
80103381:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103384:	89 04 24             	mov    %eax,(%esp)
80103387:	e8 51 ce ff ff       	call   801001dd <bwrite>
  brelse(lbuf);
8010338c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010338f:	89 04 24             	mov    %eax,(%esp)
80103392:	e8 80 ce ff ff       	call   80100217 <brelse>
  if (i == log.lh.n)
80103397:	a1 c4 08 11 80       	mov    0x801108c4,%eax
8010339c:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010339f:	75 0d                	jne    801033ae <log_write+0xf8>
    log.lh.n++;
801033a1:	a1 c4 08 11 80       	mov    0x801108c4,%eax
801033a6:	83 c0 01             	add    $0x1,%eax
801033a9:	a3 c4 08 11 80       	mov    %eax,0x801108c4
  b->flags |= B_DIRTY; // XXX prevent eviction
801033ae:	8b 45 08             	mov    0x8(%ebp),%eax
801033b1:	8b 00                	mov    (%eax),%eax
801033b3:	89 c2                	mov    %eax,%edx
801033b5:	83 ca 04             	or     $0x4,%edx
801033b8:	8b 45 08             	mov    0x8(%ebp),%eax
801033bb:	89 10                	mov    %edx,(%eax)
}
801033bd:	c9                   	leave  
801033be:	c3                   	ret    
	...

801033c0 <v2p>:
801033c0:	55                   	push   %ebp
801033c1:	89 e5                	mov    %esp,%ebp
801033c3:	8b 45 08             	mov    0x8(%ebp),%eax
801033c6:	05 00 00 00 80       	add    $0x80000000,%eax
801033cb:	5d                   	pop    %ebp
801033cc:	c3                   	ret    

801033cd <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801033cd:	55                   	push   %ebp
801033ce:	89 e5                	mov    %esp,%ebp
801033d0:	8b 45 08             	mov    0x8(%ebp),%eax
801033d3:	05 00 00 00 80       	add    $0x80000000,%eax
801033d8:	5d                   	pop    %ebp
801033d9:	c3                   	ret    

801033da <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801033da:	55                   	push   %ebp
801033db:	89 e5                	mov    %esp,%ebp
801033dd:	53                   	push   %ebx
801033de:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
801033e1:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033e4:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
801033e7:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033ea:	89 c3                	mov    %eax,%ebx
801033ec:	89 d8                	mov    %ebx,%eax
801033ee:	f0 87 02             	lock xchg %eax,(%edx)
801033f1:	89 c3                	mov    %eax,%ebx
801033f3:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801033f6:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801033f9:	83 c4 10             	add    $0x10,%esp
801033fc:	5b                   	pop    %ebx
801033fd:	5d                   	pop    %ebp
801033fe:	c3                   	ret    

801033ff <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801033ff:	55                   	push   %ebp
80103400:	89 e5                	mov    %esp,%ebp
80103402:	83 e4 f0             	and    $0xfffffff0,%esp
80103405:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103408:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
8010340f:	80 
80103410:	c7 04 24 fc 3a 11 80 	movl   $0x80113afc,(%esp)
80103417:	e8 ad f5 ff ff       	call   801029c9 <kinit1>
  kvmalloc();      // kernel page table
8010341c:	e8 35 4d 00 00       	call   80108156 <kvmalloc>
  mpinit();        // collect info about this machine
80103421:	e8 63 04 00 00       	call   80103889 <mpinit>
  lapicinit(mpbcpu());
80103426:	e8 2e 02 00 00       	call   80103659 <mpbcpu>
8010342b:	89 04 24             	mov    %eax,(%esp)
8010342e:	e8 f5 f8 ff ff       	call   80102d28 <lapicinit>
  seginit();       // set up segments
80103433:	e8 c1 46 00 00       	call   80107af9 <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103438:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010343e:	0f b6 00             	movzbl (%eax),%eax
80103441:	0f b6 c0             	movzbl %al,%eax
80103444:	89 44 24 04          	mov    %eax,0x4(%esp)
80103448:	c7 04 24 29 8b 10 80 	movl   $0x80108b29,(%esp)
8010344f:	e8 4d cf ff ff       	call   801003a1 <cprintf>
  picinit();       // interrupt controller
80103454:	e8 95 06 00 00       	call   80103aee <picinit>
  ioapicinit();    // another interrupt controller
80103459:	e8 5b f4 ff ff       	call   801028b9 <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
8010345e:	e8 2a d6 ff ff       	call   80100a8d <consoleinit>
  uartinit();      // serial port
80103463:	e8 dc 39 00 00       	call   80106e44 <uartinit>
  pinit();         // process table
80103468:	e8 a3 0b 00 00       	call   80104010 <pinit>
  tvinit();        // trap vectors
8010346d:	e8 75 35 00 00       	call   801069e7 <tvinit>
  binit();         // buffer cache
80103472:	e8 bd cb ff ff       	call   80100034 <binit>
  fileinit();      // file table
80103477:	e8 84 da ff ff       	call   80100f00 <fileinit>
  iinit();         // inode cache
8010347c:	e8 32 e1 ff ff       	call   801015b3 <iinit>
  ideinit();       // disk
80103481:	e8 a0 f0 ff ff       	call   80102526 <ideinit>
  if(!ismp)
80103486:	a1 04 09 11 80       	mov    0x80110904,%eax
8010348b:	85 c0                	test   %eax,%eax
8010348d:	75 05                	jne    80103494 <main+0x95>
    timerinit();   // uniprocessor timer
8010348f:	e8 96 34 00 00       	call   8010692a <timerinit>
  startothers();   // start other processors
80103494:	e8 87 00 00 00       	call   80103520 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103499:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
801034a0:	8e 
801034a1:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
801034a8:	e8 54 f5 ff ff       	call   80102a01 <kinit2>
  userinit();      // first user process
801034ad:	e8 45 11 00 00       	call   801045f7 <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
801034b2:	e8 22 00 00 00       	call   801034d9 <mpmain>

801034b7 <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
801034b7:	55                   	push   %ebp
801034b8:	89 e5                	mov    %esp,%ebp
801034ba:	83 ec 18             	sub    $0x18,%esp
  switchkvm(); 
801034bd:	e8 ab 4c 00 00       	call   8010816d <switchkvm>
  seginit();
801034c2:	e8 32 46 00 00       	call   80107af9 <seginit>
  lapicinit(cpunum());
801034c7:	e8 b9 f9 ff ff       	call   80102e85 <cpunum>
801034cc:	89 04 24             	mov    %eax,(%esp)
801034cf:	e8 54 f8 ff ff       	call   80102d28 <lapicinit>
  mpmain();
801034d4:	e8 00 00 00 00       	call   801034d9 <mpmain>

801034d9 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801034d9:	55                   	push   %ebp
801034da:	89 e5                	mov    %esp,%ebp
801034dc:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
801034df:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034e5:	0f b6 00             	movzbl (%eax),%eax
801034e8:	0f b6 c0             	movzbl %al,%eax
801034eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801034ef:	c7 04 24 40 8b 10 80 	movl   $0x80108b40,(%esp)
801034f6:	e8 a6 ce ff ff       	call   801003a1 <cprintf>
  idtinit();       // load idt register
801034fb:	e8 5b 36 00 00       	call   80106b5b <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
80103500:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103506:	05 a8 00 00 00       	add    $0xa8,%eax
8010350b:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80103512:	00 
80103513:	89 04 24             	mov    %eax,(%esp)
80103516:	e8 bf fe ff ff       	call   801033da <xchg>
  scheduler();     // start running processes
8010351b:	e8 e7 16 00 00       	call   80104c07 <scheduler>

80103520 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
80103520:	55                   	push   %ebp
80103521:	89 e5                	mov    %esp,%ebp
80103523:	53                   	push   %ebx
80103524:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
80103527:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
8010352e:	e8 9a fe ff ff       	call   801033cd <p2v>
80103533:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
80103536:	b8 8a 00 00 00       	mov    $0x8a,%eax
8010353b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010353f:	c7 44 24 04 0c c5 10 	movl   $0x8010c50c,0x4(%esp)
80103546:	80 
80103547:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010354a:	89 04 24             	mov    %eax,(%esp)
8010354d:	e8 5b 1f 00 00       	call   801054ad <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103552:	c7 45 f4 20 09 11 80 	movl   $0x80110920,-0xc(%ebp)
80103559:	e9 86 00 00 00       	jmp    801035e4 <startothers+0xc4>
    if(c == cpus+cpunum())  // We've started already.
8010355e:	e8 22 f9 ff ff       	call   80102e85 <cpunum>
80103563:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103569:	05 20 09 11 80       	add    $0x80110920,%eax
8010356e:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103571:	74 69                	je     801035dc <startothers+0xbc>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103573:	e8 7f f5 ff ff       	call   80102af7 <kalloc>
80103578:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
8010357b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010357e:	83 e8 04             	sub    $0x4,%eax
80103581:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103584:	81 c2 00 10 00 00    	add    $0x1000,%edx
8010358a:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
8010358c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010358f:	83 e8 08             	sub    $0x8,%eax
80103592:	c7 00 b7 34 10 80    	movl   $0x801034b7,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
80103598:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010359b:	8d 58 f4             	lea    -0xc(%eax),%ebx
8010359e:	c7 04 24 00 b0 10 80 	movl   $0x8010b000,(%esp)
801035a5:	e8 16 fe ff ff       	call   801033c0 <v2p>
801035aa:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
801035ac:	8b 45 f0             	mov    -0x10(%ebp),%eax
801035af:	89 04 24             	mov    %eax,(%esp)
801035b2:	e8 09 fe ff ff       	call   801033c0 <v2p>
801035b7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801035ba:	0f b6 12             	movzbl (%edx),%edx
801035bd:	0f b6 d2             	movzbl %dl,%edx
801035c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801035c4:	89 14 24             	mov    %edx,(%esp)
801035c7:	e8 3f f9 ff ff       	call   80102f0b <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
801035cc:	90                   	nop
801035cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801035d0:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
801035d6:	85 c0                	test   %eax,%eax
801035d8:	74 f3                	je     801035cd <startothers+0xad>
801035da:	eb 01                	jmp    801035dd <startothers+0xbd>
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
    if(c == cpus+cpunum())  // We've started already.
      continue;
801035dc:	90                   	nop
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
801035dd:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
801035e4:	a1 00 0f 11 80       	mov    0x80110f00,%eax
801035e9:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801035ef:	05 20 09 11 80       	add    $0x80110920,%eax
801035f4:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801035f7:	0f 87 61 ff ff ff    	ja     8010355e <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801035fd:	83 c4 24             	add    $0x24,%esp
80103600:	5b                   	pop    %ebx
80103601:	5d                   	pop    %ebp
80103602:	c3                   	ret    
	...

80103604 <p2v>:
80103604:	55                   	push   %ebp
80103605:	89 e5                	mov    %esp,%ebp
80103607:	8b 45 08             	mov    0x8(%ebp),%eax
8010360a:	05 00 00 00 80       	add    $0x80000000,%eax
8010360f:	5d                   	pop    %ebp
80103610:	c3                   	ret    

80103611 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80103611:	55                   	push   %ebp
80103612:	89 e5                	mov    %esp,%ebp
80103614:	53                   	push   %ebx
80103615:	83 ec 14             	sub    $0x14,%esp
80103618:	8b 45 08             	mov    0x8(%ebp),%eax
8010361b:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010361f:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80103623:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80103627:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
8010362b:	ec                   	in     (%dx),%al
8010362c:	89 c3                	mov    %eax,%ebx
8010362e:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80103631:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80103635:	83 c4 14             	add    $0x14,%esp
80103638:	5b                   	pop    %ebx
80103639:	5d                   	pop    %ebp
8010363a:	c3                   	ret    

8010363b <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
8010363b:	55                   	push   %ebp
8010363c:	89 e5                	mov    %esp,%ebp
8010363e:	83 ec 08             	sub    $0x8,%esp
80103641:	8b 55 08             	mov    0x8(%ebp),%edx
80103644:	8b 45 0c             	mov    0xc(%ebp),%eax
80103647:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010364b:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010364e:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103652:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103656:	ee                   	out    %al,(%dx)
}
80103657:	c9                   	leave  
80103658:	c3                   	ret    

80103659 <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
80103659:	55                   	push   %ebp
8010365a:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
8010365c:	a1 44 c6 10 80       	mov    0x8010c644,%eax
80103661:	89 c2                	mov    %eax,%edx
80103663:	b8 20 09 11 80       	mov    $0x80110920,%eax
80103668:	89 d1                	mov    %edx,%ecx
8010366a:	29 c1                	sub    %eax,%ecx
8010366c:	89 c8                	mov    %ecx,%eax
8010366e:	c1 f8 02             	sar    $0x2,%eax
80103671:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
80103677:	5d                   	pop    %ebp
80103678:	c3                   	ret    

80103679 <sum>:

static uchar
sum(uchar *addr, int len)
{
80103679:	55                   	push   %ebp
8010367a:	89 e5                	mov    %esp,%ebp
8010367c:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
8010367f:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
80103686:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010368d:	eb 13                	jmp    801036a2 <sum+0x29>
    sum += addr[i];
8010368f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103692:	03 45 08             	add    0x8(%ebp),%eax
80103695:	0f b6 00             	movzbl (%eax),%eax
80103698:	0f b6 c0             	movzbl %al,%eax
8010369b:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
8010369e:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801036a2:	8b 45 fc             	mov    -0x4(%ebp),%eax
801036a5:	3b 45 0c             	cmp    0xc(%ebp),%eax
801036a8:	7c e5                	jl     8010368f <sum+0x16>
    sum += addr[i];
  return sum;
801036aa:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801036ad:	c9                   	leave  
801036ae:	c3                   	ret    

801036af <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
801036af:	55                   	push   %ebp
801036b0:	89 e5                	mov    %esp,%ebp
801036b2:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
801036b5:	8b 45 08             	mov    0x8(%ebp),%eax
801036b8:	89 04 24             	mov    %eax,(%esp)
801036bb:	e8 44 ff ff ff       	call   80103604 <p2v>
801036c0:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
801036c3:	8b 45 0c             	mov    0xc(%ebp),%eax
801036c6:	03 45 f0             	add    -0x10(%ebp),%eax
801036c9:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
801036cc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036cf:	89 45 f4             	mov    %eax,-0xc(%ebp)
801036d2:	eb 3f                	jmp    80103713 <mpsearch1+0x64>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801036d4:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801036db:	00 
801036dc:	c7 44 24 04 54 8b 10 	movl   $0x80108b54,0x4(%esp)
801036e3:	80 
801036e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036e7:	89 04 24             	mov    %eax,(%esp)
801036ea:	e8 62 1d 00 00       	call   80105451 <memcmp>
801036ef:	85 c0                	test   %eax,%eax
801036f1:	75 1c                	jne    8010370f <mpsearch1+0x60>
801036f3:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801036fa:	00 
801036fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036fe:	89 04 24             	mov    %eax,(%esp)
80103701:	e8 73 ff ff ff       	call   80103679 <sum>
80103706:	84 c0                	test   %al,%al
80103708:	75 05                	jne    8010370f <mpsearch1+0x60>
      return (struct mp*)p;
8010370a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010370d:	eb 11                	jmp    80103720 <mpsearch1+0x71>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
8010370f:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80103713:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103716:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103719:	72 b9                	jb     801036d4 <mpsearch1+0x25>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
8010371b:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103720:	c9                   	leave  
80103721:	c3                   	ret    

80103722 <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
80103722:	55                   	push   %ebp
80103723:	89 e5                	mov    %esp,%ebp
80103725:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
80103728:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
8010372f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103732:	83 c0 0f             	add    $0xf,%eax
80103735:	0f b6 00             	movzbl (%eax),%eax
80103738:	0f b6 c0             	movzbl %al,%eax
8010373b:	89 c2                	mov    %eax,%edx
8010373d:	c1 e2 08             	shl    $0x8,%edx
80103740:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103743:	83 c0 0e             	add    $0xe,%eax
80103746:	0f b6 00             	movzbl (%eax),%eax
80103749:	0f b6 c0             	movzbl %al,%eax
8010374c:	09 d0                	or     %edx,%eax
8010374e:	c1 e0 04             	shl    $0x4,%eax
80103751:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103754:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103758:	74 21                	je     8010377b <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
8010375a:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103761:	00 
80103762:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103765:	89 04 24             	mov    %eax,(%esp)
80103768:	e8 42 ff ff ff       	call   801036af <mpsearch1>
8010376d:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103770:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103774:	74 50                	je     801037c6 <mpsearch+0xa4>
      return mp;
80103776:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103779:	eb 5f                	jmp    801037da <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
8010377b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010377e:	83 c0 14             	add    $0x14,%eax
80103781:	0f b6 00             	movzbl (%eax),%eax
80103784:	0f b6 c0             	movzbl %al,%eax
80103787:	89 c2                	mov    %eax,%edx
80103789:	c1 e2 08             	shl    $0x8,%edx
8010378c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010378f:	83 c0 13             	add    $0x13,%eax
80103792:	0f b6 00             	movzbl (%eax),%eax
80103795:	0f b6 c0             	movzbl %al,%eax
80103798:	09 d0                	or     %edx,%eax
8010379a:	c1 e0 0a             	shl    $0xa,%eax
8010379d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
801037a0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801037a3:	2d 00 04 00 00       	sub    $0x400,%eax
801037a8:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
801037af:	00 
801037b0:	89 04 24             	mov    %eax,(%esp)
801037b3:	e8 f7 fe ff ff       	call   801036af <mpsearch1>
801037b8:	89 45 ec             	mov    %eax,-0x14(%ebp)
801037bb:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801037bf:	74 05                	je     801037c6 <mpsearch+0xa4>
      return mp;
801037c1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801037c4:	eb 14                	jmp    801037da <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
801037c6:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
801037cd:	00 
801037ce:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
801037d5:	e8 d5 fe ff ff       	call   801036af <mpsearch1>
}
801037da:	c9                   	leave  
801037db:	c3                   	ret    

801037dc <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
801037dc:	55                   	push   %ebp
801037dd:	89 e5                	mov    %esp,%ebp
801037df:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801037e2:	e8 3b ff ff ff       	call   80103722 <mpsearch>
801037e7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801037ea:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801037ee:	74 0a                	je     801037fa <mpconfig+0x1e>
801037f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037f3:	8b 40 04             	mov    0x4(%eax),%eax
801037f6:	85 c0                	test   %eax,%eax
801037f8:	75 0a                	jne    80103804 <mpconfig+0x28>
    return 0;
801037fa:	b8 00 00 00 00       	mov    $0x0,%eax
801037ff:	e9 83 00 00 00       	jmp    80103887 <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
80103804:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103807:	8b 40 04             	mov    0x4(%eax),%eax
8010380a:	89 04 24             	mov    %eax,(%esp)
8010380d:	e8 f2 fd ff ff       	call   80103604 <p2v>
80103812:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80103815:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
8010381c:	00 
8010381d:	c7 44 24 04 59 8b 10 	movl   $0x80108b59,0x4(%esp)
80103824:	80 
80103825:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103828:	89 04 24             	mov    %eax,(%esp)
8010382b:	e8 21 1c 00 00       	call   80105451 <memcmp>
80103830:	85 c0                	test   %eax,%eax
80103832:	74 07                	je     8010383b <mpconfig+0x5f>
    return 0;
80103834:	b8 00 00 00 00       	mov    $0x0,%eax
80103839:	eb 4c                	jmp    80103887 <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
8010383b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010383e:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103842:	3c 01                	cmp    $0x1,%al
80103844:	74 12                	je     80103858 <mpconfig+0x7c>
80103846:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103849:	0f b6 40 06          	movzbl 0x6(%eax),%eax
8010384d:	3c 04                	cmp    $0x4,%al
8010384f:	74 07                	je     80103858 <mpconfig+0x7c>
    return 0;
80103851:	b8 00 00 00 00       	mov    $0x0,%eax
80103856:	eb 2f                	jmp    80103887 <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
80103858:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010385b:	0f b7 40 04          	movzwl 0x4(%eax),%eax
8010385f:	0f b7 c0             	movzwl %ax,%eax
80103862:	89 44 24 04          	mov    %eax,0x4(%esp)
80103866:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103869:	89 04 24             	mov    %eax,(%esp)
8010386c:	e8 08 fe ff ff       	call   80103679 <sum>
80103871:	84 c0                	test   %al,%al
80103873:	74 07                	je     8010387c <mpconfig+0xa0>
    return 0;
80103875:	b8 00 00 00 00       	mov    $0x0,%eax
8010387a:	eb 0b                	jmp    80103887 <mpconfig+0xab>
  *pmp = mp;
8010387c:	8b 45 08             	mov    0x8(%ebp),%eax
8010387f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103882:	89 10                	mov    %edx,(%eax)
  return conf;
80103884:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80103887:	c9                   	leave  
80103888:	c3                   	ret    

80103889 <mpinit>:

void
mpinit(void)
{
80103889:	55                   	push   %ebp
8010388a:	89 e5                	mov    %esp,%ebp
8010388c:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
8010388f:	c7 05 44 c6 10 80 20 	movl   $0x80110920,0x8010c644
80103896:	09 11 80 
  if((conf = mpconfig(&mp)) == 0)
80103899:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010389c:	89 04 24             	mov    %eax,(%esp)
8010389f:	e8 38 ff ff ff       	call   801037dc <mpconfig>
801038a4:	89 45 f0             	mov    %eax,-0x10(%ebp)
801038a7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801038ab:	0f 84 9c 01 00 00    	je     80103a4d <mpinit+0x1c4>
    return;
  ismp = 1;
801038b1:	c7 05 04 09 11 80 01 	movl   $0x1,0x80110904
801038b8:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
801038bb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038be:	8b 40 24             	mov    0x24(%eax),%eax
801038c1:	a3 7c 08 11 80       	mov    %eax,0x8011087c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801038c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038c9:	83 c0 2c             	add    $0x2c,%eax
801038cc:	89 45 f4             	mov    %eax,-0xc(%ebp)
801038cf:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038d2:	0f b7 40 04          	movzwl 0x4(%eax),%eax
801038d6:	0f b7 c0             	movzwl %ax,%eax
801038d9:	03 45 f0             	add    -0x10(%ebp),%eax
801038dc:	89 45 ec             	mov    %eax,-0x14(%ebp)
801038df:	e9 f4 00 00 00       	jmp    801039d8 <mpinit+0x14f>
    switch(*p){
801038e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038e7:	0f b6 00             	movzbl (%eax),%eax
801038ea:	0f b6 c0             	movzbl %al,%eax
801038ed:	83 f8 04             	cmp    $0x4,%eax
801038f0:	0f 87 bf 00 00 00    	ja     801039b5 <mpinit+0x12c>
801038f6:	8b 04 85 9c 8b 10 80 	mov    -0x7fef7464(,%eax,4),%eax
801038fd:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
801038ff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103902:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
80103905:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103908:	0f b6 40 01          	movzbl 0x1(%eax),%eax
8010390c:	0f b6 d0             	movzbl %al,%edx
8010390f:	a1 00 0f 11 80       	mov    0x80110f00,%eax
80103914:	39 c2                	cmp    %eax,%edx
80103916:	74 2d                	je     80103945 <mpinit+0xbc>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
80103918:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010391b:	0f b6 40 01          	movzbl 0x1(%eax),%eax
8010391f:	0f b6 d0             	movzbl %al,%edx
80103922:	a1 00 0f 11 80       	mov    0x80110f00,%eax
80103927:	89 54 24 08          	mov    %edx,0x8(%esp)
8010392b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010392f:	c7 04 24 5e 8b 10 80 	movl   $0x80108b5e,(%esp)
80103936:	e8 66 ca ff ff       	call   801003a1 <cprintf>
        ismp = 0;
8010393b:	c7 05 04 09 11 80 00 	movl   $0x0,0x80110904
80103942:	00 00 00 
      }
      if(proc->flags & MPBOOT)
80103945:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103948:	0f b6 40 03          	movzbl 0x3(%eax),%eax
8010394c:	0f b6 c0             	movzbl %al,%eax
8010394f:	83 e0 02             	and    $0x2,%eax
80103952:	85 c0                	test   %eax,%eax
80103954:	74 15                	je     8010396b <mpinit+0xe2>
        bcpu = &cpus[ncpu];
80103956:	a1 00 0f 11 80       	mov    0x80110f00,%eax
8010395b:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103961:	05 20 09 11 80       	add    $0x80110920,%eax
80103966:	a3 44 c6 10 80       	mov    %eax,0x8010c644
      cpus[ncpu].id = ncpu;
8010396b:	8b 15 00 0f 11 80    	mov    0x80110f00,%edx
80103971:	a1 00 0f 11 80       	mov    0x80110f00,%eax
80103976:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
8010397c:	81 c2 20 09 11 80    	add    $0x80110920,%edx
80103982:	88 02                	mov    %al,(%edx)
      ncpu++;
80103984:	a1 00 0f 11 80       	mov    0x80110f00,%eax
80103989:	83 c0 01             	add    $0x1,%eax
8010398c:	a3 00 0f 11 80       	mov    %eax,0x80110f00
      p += sizeof(struct mpproc);
80103991:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103995:	eb 41                	jmp    801039d8 <mpinit+0x14f>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
80103997:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010399a:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
8010399d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801039a0:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801039a4:	a2 00 09 11 80       	mov    %al,0x80110900
      p += sizeof(struct mpioapic);
801039a9:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801039ad:	eb 29                	jmp    801039d8 <mpinit+0x14f>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
801039af:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801039b3:	eb 23                	jmp    801039d8 <mpinit+0x14f>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
801039b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039b8:	0f b6 00             	movzbl (%eax),%eax
801039bb:	0f b6 c0             	movzbl %al,%eax
801039be:	89 44 24 04          	mov    %eax,0x4(%esp)
801039c2:	c7 04 24 7c 8b 10 80 	movl   $0x80108b7c,(%esp)
801039c9:	e8 d3 c9 ff ff       	call   801003a1 <cprintf>
      ismp = 0;
801039ce:	c7 05 04 09 11 80 00 	movl   $0x0,0x80110904
801039d5:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801039d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039db:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801039de:	0f 82 00 ff ff ff    	jb     801038e4 <mpinit+0x5b>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
801039e4:	a1 04 09 11 80       	mov    0x80110904,%eax
801039e9:	85 c0                	test   %eax,%eax
801039eb:	75 1d                	jne    80103a0a <mpinit+0x181>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
801039ed:	c7 05 00 0f 11 80 01 	movl   $0x1,0x80110f00
801039f4:	00 00 00 
    lapic = 0;
801039f7:	c7 05 7c 08 11 80 00 	movl   $0x0,0x8011087c
801039fe:	00 00 00 
    ioapicid = 0;
80103a01:	c6 05 00 09 11 80 00 	movb   $0x0,0x80110900
    return;
80103a08:	eb 44                	jmp    80103a4e <mpinit+0x1c5>
  }

  if(mp->imcrp){
80103a0a:	8b 45 e0             	mov    -0x20(%ebp),%eax
80103a0d:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
80103a11:	84 c0                	test   %al,%al
80103a13:	74 39                	je     80103a4e <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80103a15:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103a1c:	00 
80103a1d:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103a24:	e8 12 fc ff ff       	call   8010363b <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103a29:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a30:	e8 dc fb ff ff       	call   80103611 <inb>
80103a35:	83 c8 01             	or     $0x1,%eax
80103a38:	0f b6 c0             	movzbl %al,%eax
80103a3b:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a3f:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a46:	e8 f0 fb ff ff       	call   8010363b <outb>
80103a4b:	eb 01                	jmp    80103a4e <mpinit+0x1c5>
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
80103a4d:	90                   	nop
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
  }
}
80103a4e:	c9                   	leave  
80103a4f:	c3                   	ret    

80103a50 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103a50:	55                   	push   %ebp
80103a51:	89 e5                	mov    %esp,%ebp
80103a53:	83 ec 08             	sub    $0x8,%esp
80103a56:	8b 55 08             	mov    0x8(%ebp),%edx
80103a59:	8b 45 0c             	mov    0xc(%ebp),%eax
80103a5c:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103a60:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103a63:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103a67:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103a6b:	ee                   	out    %al,(%dx)
}
80103a6c:	c9                   	leave  
80103a6d:	c3                   	ret    

80103a6e <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80103a6e:	55                   	push   %ebp
80103a6f:	89 e5                	mov    %esp,%ebp
80103a71:	83 ec 0c             	sub    $0xc,%esp
80103a74:	8b 45 08             	mov    0x8(%ebp),%eax
80103a77:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
80103a7b:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a7f:	66 a3 00 c0 10 80    	mov    %ax,0x8010c000
  outb(IO_PIC1+1, mask);
80103a85:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a89:	0f b6 c0             	movzbl %al,%eax
80103a8c:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a90:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103a97:	e8 b4 ff ff ff       	call   80103a50 <outb>
  outb(IO_PIC2+1, mask >> 8);
80103a9c:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103aa0:	66 c1 e8 08          	shr    $0x8,%ax
80103aa4:	0f b6 c0             	movzbl %al,%eax
80103aa7:	89 44 24 04          	mov    %eax,0x4(%esp)
80103aab:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103ab2:	e8 99 ff ff ff       	call   80103a50 <outb>
}
80103ab7:	c9                   	leave  
80103ab8:	c3                   	ret    

80103ab9 <picenable>:

void
picenable(int irq)
{
80103ab9:	55                   	push   %ebp
80103aba:	89 e5                	mov    %esp,%ebp
80103abc:	53                   	push   %ebx
80103abd:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
80103ac0:	8b 45 08             	mov    0x8(%ebp),%eax
80103ac3:	ba 01 00 00 00       	mov    $0x1,%edx
80103ac8:	89 d3                	mov    %edx,%ebx
80103aca:	89 c1                	mov    %eax,%ecx
80103acc:	d3 e3                	shl    %cl,%ebx
80103ace:	89 d8                	mov    %ebx,%eax
80103ad0:	89 c2                	mov    %eax,%edx
80103ad2:	f7 d2                	not    %edx
80103ad4:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
80103adb:	21 d0                	and    %edx,%eax
80103add:	0f b7 c0             	movzwl %ax,%eax
80103ae0:	89 04 24             	mov    %eax,(%esp)
80103ae3:	e8 86 ff ff ff       	call   80103a6e <picsetmask>
}
80103ae8:	83 c4 04             	add    $0x4,%esp
80103aeb:	5b                   	pop    %ebx
80103aec:	5d                   	pop    %ebp
80103aed:	c3                   	ret    

80103aee <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
80103aee:	55                   	push   %ebp
80103aef:	89 e5                	mov    %esp,%ebp
80103af1:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103af4:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103afb:	00 
80103afc:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b03:	e8 48 ff ff ff       	call   80103a50 <outb>
  outb(IO_PIC2+1, 0xFF);
80103b08:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103b0f:	00 
80103b10:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b17:	e8 34 ff ff ff       	call   80103a50 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80103b1c:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b23:	00 
80103b24:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103b2b:	e8 20 ff ff ff       	call   80103a50 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80103b30:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103b37:	00 
80103b38:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b3f:	e8 0c ff ff ff       	call   80103a50 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80103b44:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103b4b:	00 
80103b4c:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b53:	e8 f8 fe ff ff       	call   80103a50 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
80103b58:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b5f:	00 
80103b60:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b67:	e8 e4 fe ff ff       	call   80103a50 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80103b6c:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b73:	00 
80103b74:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103b7b:	e8 d0 fe ff ff       	call   80103a50 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80103b80:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103b87:	00 
80103b88:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b8f:	e8 bc fe ff ff       	call   80103a50 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80103b94:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103b9b:	00 
80103b9c:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103ba3:	e8 a8 fe ff ff       	call   80103a50 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
80103ba8:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103baf:	00 
80103bb0:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103bb7:	e8 94 fe ff ff       	call   80103a50 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
80103bbc:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103bc3:	00 
80103bc4:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bcb:	e8 80 fe ff ff       	call   80103a50 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
80103bd0:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103bd7:	00 
80103bd8:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bdf:	e8 6c fe ff ff       	call   80103a50 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
80103be4:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103beb:	00 
80103bec:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103bf3:	e8 58 fe ff ff       	call   80103a50 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
80103bf8:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103bff:	00 
80103c00:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103c07:	e8 44 fe ff ff       	call   80103a50 <outb>

  if(irqmask != 0xFFFF)
80103c0c:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
80103c13:	66 83 f8 ff          	cmp    $0xffff,%ax
80103c17:	74 12                	je     80103c2b <picinit+0x13d>
    picsetmask(irqmask);
80103c19:	0f b7 05 00 c0 10 80 	movzwl 0x8010c000,%eax
80103c20:	0f b7 c0             	movzwl %ax,%eax
80103c23:	89 04 24             	mov    %eax,(%esp)
80103c26:	e8 43 fe ff ff       	call   80103a6e <picsetmask>
}
80103c2b:	c9                   	leave  
80103c2c:	c3                   	ret    
80103c2d:	00 00                	add    %al,(%eax)
	...

80103c30 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103c30:	55                   	push   %ebp
80103c31:	89 e5                	mov    %esp,%ebp
80103c33:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103c36:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103c3d:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c40:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103c46:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c49:	8b 10                	mov    (%eax),%edx
80103c4b:	8b 45 08             	mov    0x8(%ebp),%eax
80103c4e:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103c50:	e8 c7 d2 ff ff       	call   80100f1c <filealloc>
80103c55:	8b 55 08             	mov    0x8(%ebp),%edx
80103c58:	89 02                	mov    %eax,(%edx)
80103c5a:	8b 45 08             	mov    0x8(%ebp),%eax
80103c5d:	8b 00                	mov    (%eax),%eax
80103c5f:	85 c0                	test   %eax,%eax
80103c61:	0f 84 c8 00 00 00    	je     80103d2f <pipealloc+0xff>
80103c67:	e8 b0 d2 ff ff       	call   80100f1c <filealloc>
80103c6c:	8b 55 0c             	mov    0xc(%ebp),%edx
80103c6f:	89 02                	mov    %eax,(%edx)
80103c71:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c74:	8b 00                	mov    (%eax),%eax
80103c76:	85 c0                	test   %eax,%eax
80103c78:	0f 84 b1 00 00 00    	je     80103d2f <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103c7e:	e8 74 ee ff ff       	call   80102af7 <kalloc>
80103c83:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103c86:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103c8a:	0f 84 9e 00 00 00    	je     80103d2e <pipealloc+0xfe>
    goto bad;
  p->readopen = 1;
80103c90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c93:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103c9a:	00 00 00 
  p->writeopen = 1;
80103c9d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ca0:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103ca7:	00 00 00 
  p->nwrite = 0;
80103caa:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cad:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103cb4:	00 00 00 
  p->nread = 0;
80103cb7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cba:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103cc1:	00 00 00 
  initlock(&p->lock, "pipe");
80103cc4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cc7:	c7 44 24 04 b0 8b 10 	movl   $0x80108bb0,0x4(%esp)
80103cce:	80 
80103ccf:	89 04 24             	mov    %eax,(%esp)
80103cd2:	e8 5b 14 00 00       	call   80105132 <initlock>
  (*f0)->type = FD_PIPE;
80103cd7:	8b 45 08             	mov    0x8(%ebp),%eax
80103cda:	8b 00                	mov    (%eax),%eax
80103cdc:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103ce2:	8b 45 08             	mov    0x8(%ebp),%eax
80103ce5:	8b 00                	mov    (%eax),%eax
80103ce7:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103ceb:	8b 45 08             	mov    0x8(%ebp),%eax
80103cee:	8b 00                	mov    (%eax),%eax
80103cf0:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103cf4:	8b 45 08             	mov    0x8(%ebp),%eax
80103cf7:	8b 00                	mov    (%eax),%eax
80103cf9:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103cfc:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103cff:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d02:	8b 00                	mov    (%eax),%eax
80103d04:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103d0a:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d0d:	8b 00                	mov    (%eax),%eax
80103d0f:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103d13:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d16:	8b 00                	mov    (%eax),%eax
80103d18:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103d1c:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d1f:	8b 00                	mov    (%eax),%eax
80103d21:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103d24:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103d27:	b8 00 00 00 00       	mov    $0x0,%eax
80103d2c:	eb 43                	jmp    80103d71 <pipealloc+0x141>
  p = 0;
  *f0 = *f1 = 0;
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
    goto bad;
80103d2e:	90                   	nop
  (*f1)->pipe = p;
  return 0;

//PAGEBREAK: 20
 bad:
  if(p)
80103d2f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103d33:	74 0b                	je     80103d40 <pipealloc+0x110>
    kfree((char*)p);
80103d35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d38:	89 04 24             	mov    %eax,(%esp)
80103d3b:	e8 1e ed ff ff       	call   80102a5e <kfree>
  if(*f0)
80103d40:	8b 45 08             	mov    0x8(%ebp),%eax
80103d43:	8b 00                	mov    (%eax),%eax
80103d45:	85 c0                	test   %eax,%eax
80103d47:	74 0d                	je     80103d56 <pipealloc+0x126>
    fileclose(*f0);
80103d49:	8b 45 08             	mov    0x8(%ebp),%eax
80103d4c:	8b 00                	mov    (%eax),%eax
80103d4e:	89 04 24             	mov    %eax,(%esp)
80103d51:	e8 6e d2 ff ff       	call   80100fc4 <fileclose>
  if(*f1)
80103d56:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d59:	8b 00                	mov    (%eax),%eax
80103d5b:	85 c0                	test   %eax,%eax
80103d5d:	74 0d                	je     80103d6c <pipealloc+0x13c>
    fileclose(*f1);
80103d5f:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d62:	8b 00                	mov    (%eax),%eax
80103d64:	89 04 24             	mov    %eax,(%esp)
80103d67:	e8 58 d2 ff ff       	call   80100fc4 <fileclose>
  return -1;
80103d6c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103d71:	c9                   	leave  
80103d72:	c3                   	ret    

80103d73 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103d73:	55                   	push   %ebp
80103d74:	89 e5                	mov    %esp,%ebp
80103d76:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103d79:	8b 45 08             	mov    0x8(%ebp),%eax
80103d7c:	89 04 24             	mov    %eax,(%esp)
80103d7f:	e8 cf 13 00 00       	call   80105153 <acquire>
  if(writable){
80103d84:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103d88:	74 1f                	je     80103da9 <pipeclose+0x36>
    p->writeopen = 0;
80103d8a:	8b 45 08             	mov    0x8(%ebp),%eax
80103d8d:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103d94:	00 00 00 
    wakeup(&p->nread);
80103d97:	8b 45 08             	mov    0x8(%ebp),%eax
80103d9a:	05 34 02 00 00       	add    $0x234,%eax
80103d9f:	89 04 24             	mov    %eax,(%esp)
80103da2:	e8 a1 11 00 00       	call   80104f48 <wakeup>
80103da7:	eb 1d                	jmp    80103dc6 <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103da9:	8b 45 08             	mov    0x8(%ebp),%eax
80103dac:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103db3:	00 00 00 
    wakeup(&p->nwrite);
80103db6:	8b 45 08             	mov    0x8(%ebp),%eax
80103db9:	05 38 02 00 00       	add    $0x238,%eax
80103dbe:	89 04 24             	mov    %eax,(%esp)
80103dc1:	e8 82 11 00 00       	call   80104f48 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103dc6:	8b 45 08             	mov    0x8(%ebp),%eax
80103dc9:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103dcf:	85 c0                	test   %eax,%eax
80103dd1:	75 25                	jne    80103df8 <pipeclose+0x85>
80103dd3:	8b 45 08             	mov    0x8(%ebp),%eax
80103dd6:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103ddc:	85 c0                	test   %eax,%eax
80103dde:	75 18                	jne    80103df8 <pipeclose+0x85>
    release(&p->lock);
80103de0:	8b 45 08             	mov    0x8(%ebp),%eax
80103de3:	89 04 24             	mov    %eax,(%esp)
80103de6:	e8 03 14 00 00       	call   801051ee <release>
    kfree((char*)p);
80103deb:	8b 45 08             	mov    0x8(%ebp),%eax
80103dee:	89 04 24             	mov    %eax,(%esp)
80103df1:	e8 68 ec ff ff       	call   80102a5e <kfree>
80103df6:	eb 0b                	jmp    80103e03 <pipeclose+0x90>
  } else
    release(&p->lock);
80103df8:	8b 45 08             	mov    0x8(%ebp),%eax
80103dfb:	89 04 24             	mov    %eax,(%esp)
80103dfe:	e8 eb 13 00 00       	call   801051ee <release>
}
80103e03:	c9                   	leave  
80103e04:	c3                   	ret    

80103e05 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103e05:	55                   	push   %ebp
80103e06:	89 e5                	mov    %esp,%ebp
80103e08:	53                   	push   %ebx
80103e09:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103e0c:	8b 45 08             	mov    0x8(%ebp),%eax
80103e0f:	89 04 24             	mov    %eax,(%esp)
80103e12:	e8 3c 13 00 00       	call   80105153 <acquire>
  for(i = 0; i < n; i++){
80103e17:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103e1e:	e9 a6 00 00 00       	jmp    80103ec9 <pipewrite+0xc4>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || proc->killed){
80103e23:	8b 45 08             	mov    0x8(%ebp),%eax
80103e26:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103e2c:	85 c0                	test   %eax,%eax
80103e2e:	74 0d                	je     80103e3d <pipewrite+0x38>
80103e30:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103e36:	8b 40 24             	mov    0x24(%eax),%eax
80103e39:	85 c0                	test   %eax,%eax
80103e3b:	74 15                	je     80103e52 <pipewrite+0x4d>
        release(&p->lock);
80103e3d:	8b 45 08             	mov    0x8(%ebp),%eax
80103e40:	89 04 24             	mov    %eax,(%esp)
80103e43:	e8 a6 13 00 00       	call   801051ee <release>
        return -1;
80103e48:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103e4d:	e9 9d 00 00 00       	jmp    80103eef <pipewrite+0xea>
      }
      wakeup(&p->nread);
80103e52:	8b 45 08             	mov    0x8(%ebp),%eax
80103e55:	05 34 02 00 00       	add    $0x234,%eax
80103e5a:	89 04 24             	mov    %eax,(%esp)
80103e5d:	e8 e6 10 00 00       	call   80104f48 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103e62:	8b 45 08             	mov    0x8(%ebp),%eax
80103e65:	8b 55 08             	mov    0x8(%ebp),%edx
80103e68:	81 c2 38 02 00 00    	add    $0x238,%edx
80103e6e:	89 44 24 04          	mov    %eax,0x4(%esp)
80103e72:	89 14 24             	mov    %edx,(%esp)
80103e75:	e8 6b 0f 00 00       	call   80104de5 <sleep>
80103e7a:	eb 01                	jmp    80103e7d <pipewrite+0x78>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103e7c:	90                   	nop
80103e7d:	8b 45 08             	mov    0x8(%ebp),%eax
80103e80:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103e86:	8b 45 08             	mov    0x8(%ebp),%eax
80103e89:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103e8f:	05 00 02 00 00       	add    $0x200,%eax
80103e94:	39 c2                	cmp    %eax,%edx
80103e96:	74 8b                	je     80103e23 <pipewrite+0x1e>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103e98:	8b 45 08             	mov    0x8(%ebp),%eax
80103e9b:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103ea1:	89 c3                	mov    %eax,%ebx
80103ea3:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103ea9:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103eac:	03 55 0c             	add    0xc(%ebp),%edx
80103eaf:	0f b6 0a             	movzbl (%edx),%ecx
80103eb2:	8b 55 08             	mov    0x8(%ebp),%edx
80103eb5:	88 4c 1a 34          	mov    %cl,0x34(%edx,%ebx,1)
80103eb9:	8d 50 01             	lea    0x1(%eax),%edx
80103ebc:	8b 45 08             	mov    0x8(%ebp),%eax
80103ebf:	89 90 38 02 00 00    	mov    %edx,0x238(%eax)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
80103ec5:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103ec9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ecc:	3b 45 10             	cmp    0x10(%ebp),%eax
80103ecf:	7c ab                	jl     80103e7c <pipewrite+0x77>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103ed1:	8b 45 08             	mov    0x8(%ebp),%eax
80103ed4:	05 34 02 00 00       	add    $0x234,%eax
80103ed9:	89 04 24             	mov    %eax,(%esp)
80103edc:	e8 67 10 00 00       	call   80104f48 <wakeup>
  release(&p->lock);
80103ee1:	8b 45 08             	mov    0x8(%ebp),%eax
80103ee4:	89 04 24             	mov    %eax,(%esp)
80103ee7:	e8 02 13 00 00       	call   801051ee <release>
  return n;
80103eec:	8b 45 10             	mov    0x10(%ebp),%eax
}
80103eef:	83 c4 24             	add    $0x24,%esp
80103ef2:	5b                   	pop    %ebx
80103ef3:	5d                   	pop    %ebp
80103ef4:	c3                   	ret    

80103ef5 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103ef5:	55                   	push   %ebp
80103ef6:	89 e5                	mov    %esp,%ebp
80103ef8:	53                   	push   %ebx
80103ef9:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103efc:	8b 45 08             	mov    0x8(%ebp),%eax
80103eff:	89 04 24             	mov    %eax,(%esp)
80103f02:	e8 4c 12 00 00       	call   80105153 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103f07:	eb 3a                	jmp    80103f43 <piperead+0x4e>
    if(proc->killed){
80103f09:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103f0f:	8b 40 24             	mov    0x24(%eax),%eax
80103f12:	85 c0                	test   %eax,%eax
80103f14:	74 15                	je     80103f2b <piperead+0x36>
      release(&p->lock);
80103f16:	8b 45 08             	mov    0x8(%ebp),%eax
80103f19:	89 04 24             	mov    %eax,(%esp)
80103f1c:	e8 cd 12 00 00       	call   801051ee <release>
      return -1;
80103f21:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103f26:	e9 b6 00 00 00       	jmp    80103fe1 <piperead+0xec>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103f2b:	8b 45 08             	mov    0x8(%ebp),%eax
80103f2e:	8b 55 08             	mov    0x8(%ebp),%edx
80103f31:	81 c2 34 02 00 00    	add    $0x234,%edx
80103f37:	89 44 24 04          	mov    %eax,0x4(%esp)
80103f3b:	89 14 24             	mov    %edx,(%esp)
80103f3e:	e8 a2 0e 00 00       	call   80104de5 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103f43:	8b 45 08             	mov    0x8(%ebp),%eax
80103f46:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f4c:	8b 45 08             	mov    0x8(%ebp),%eax
80103f4f:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f55:	39 c2                	cmp    %eax,%edx
80103f57:	75 0d                	jne    80103f66 <piperead+0x71>
80103f59:	8b 45 08             	mov    0x8(%ebp),%eax
80103f5c:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103f62:	85 c0                	test   %eax,%eax
80103f64:	75 a3                	jne    80103f09 <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f66:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103f6d:	eb 49                	jmp    80103fb8 <piperead+0xc3>
    if(p->nread == p->nwrite)
80103f6f:	8b 45 08             	mov    0x8(%ebp),%eax
80103f72:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f78:	8b 45 08             	mov    0x8(%ebp),%eax
80103f7b:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f81:	39 c2                	cmp    %eax,%edx
80103f83:	74 3d                	je     80103fc2 <piperead+0xcd>
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
80103f85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f88:	89 c2                	mov    %eax,%edx
80103f8a:	03 55 0c             	add    0xc(%ebp),%edx
80103f8d:	8b 45 08             	mov    0x8(%ebp),%eax
80103f90:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103f96:	89 c3                	mov    %eax,%ebx
80103f98:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103f9e:	8b 4d 08             	mov    0x8(%ebp),%ecx
80103fa1:	0f b6 4c 19 34       	movzbl 0x34(%ecx,%ebx,1),%ecx
80103fa6:	88 0a                	mov    %cl,(%edx)
80103fa8:	8d 50 01             	lea    0x1(%eax),%edx
80103fab:	8b 45 08             	mov    0x8(%ebp),%eax
80103fae:	89 90 34 02 00 00    	mov    %edx,0x234(%eax)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103fb4:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103fb8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103fbb:	3b 45 10             	cmp    0x10(%ebp),%eax
80103fbe:	7c af                	jl     80103f6f <piperead+0x7a>
80103fc0:	eb 01                	jmp    80103fc3 <piperead+0xce>
    if(p->nread == p->nwrite)
      break;
80103fc2:	90                   	nop
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80103fc3:	8b 45 08             	mov    0x8(%ebp),%eax
80103fc6:	05 38 02 00 00       	add    $0x238,%eax
80103fcb:	89 04 24             	mov    %eax,(%esp)
80103fce:	e8 75 0f 00 00       	call   80104f48 <wakeup>
  release(&p->lock);
80103fd3:	8b 45 08             	mov    0x8(%ebp),%eax
80103fd6:	89 04 24             	mov    %eax,(%esp)
80103fd9:	e8 10 12 00 00       	call   801051ee <release>
  return i;
80103fde:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80103fe1:	83 c4 24             	add    $0x24,%esp
80103fe4:	5b                   	pop    %ebx
80103fe5:	5d                   	pop    %ebp
80103fe6:	c3                   	ret    
	...

80103fe8 <p2v>:
80103fe8:	55                   	push   %ebp
80103fe9:	89 e5                	mov    %esp,%ebp
80103feb:	8b 45 08             	mov    0x8(%ebp),%eax
80103fee:	05 00 00 00 80       	add    $0x80000000,%eax
80103ff3:	5d                   	pop    %ebp
80103ff4:	c3                   	ret    

80103ff5 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80103ff5:	55                   	push   %ebp
80103ff6:	89 e5                	mov    %esp,%ebp
80103ff8:	53                   	push   %ebx
80103ff9:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103ffc:	9c                   	pushf  
80103ffd:	5b                   	pop    %ebx
80103ffe:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80104001:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80104004:	83 c4 10             	add    $0x10,%esp
80104007:	5b                   	pop    %ebx
80104008:	5d                   	pop    %ebp
80104009:	c3                   	ret    

8010400a <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
8010400a:	55                   	push   %ebp
8010400b:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
8010400d:	fb                   	sti    
}
8010400e:	5d                   	pop    %ebp
8010400f:	c3                   	ret    

80104010 <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
80104010:	55                   	push   %ebp
80104011:	89 e5                	mov    %esp,%ebp
80104013:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
80104016:	c7 44 24 04 b5 8b 10 	movl   $0x80108bb5,0x4(%esp)
8010401d:	80 
8010401e:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104025:	e8 08 11 00 00       	call   80105132 <initlock>
}
8010402a:	c9                   	leave  
8010402b:	c3                   	ret    

8010402c <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
8010402c:	55                   	push   %ebp
8010402d:	89 e5                	mov    %esp,%ebp
8010402f:	83 ec 38             	sub    $0x38,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
80104032:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104039:	e8 15 11 00 00       	call   80105153 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010403e:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
80104045:	eb 11                	jmp    80104058 <allocproc+0x2c>
    if(p->state == UNUSED)
80104047:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010404a:	8b 40 0c             	mov    0xc(%eax),%eax
8010404d:	85 c0                	test   %eax,%eax
8010404f:	74 26                	je     80104077 <allocproc+0x4b>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104051:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
80104058:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
8010405f:	72 e6                	jb     80104047 <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
80104061:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104068:	e8 81 11 00 00       	call   801051ee <release>
  return 0;
8010406d:	b8 00 00 00 00       	mov    $0x0,%eax
80104072:	e9 5a 01 00 00       	jmp    801041d1 <allocproc+0x1a5>
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == UNUSED)
      goto found;
80104077:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80104078:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010407b:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
80104082:	a1 04 c0 10 80       	mov    0x8010c004,%eax
80104087:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010408a:	89 42 10             	mov    %eax,0x10(%edx)
8010408d:	83 c0 01             	add    $0x1,%eax
80104090:	a3 04 c0 10 80       	mov    %eax,0x8010c004
  release(&ptable.lock);
80104095:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
8010409c:	e8 4d 11 00 00       	call   801051ee <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
801040a1:	e8 51 ea ff ff       	call   80102af7 <kalloc>
801040a6:	8b 55 f4             	mov    -0xc(%ebp),%edx
801040a9:	89 42 08             	mov    %eax,0x8(%edx)
801040ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040af:	8b 40 08             	mov    0x8(%eax),%eax
801040b2:	85 c0                	test   %eax,%eax
801040b4:	75 14                	jne    801040ca <allocproc+0x9e>
    p->state = UNUSED;
801040b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040b9:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
801040c0:	b8 00 00 00 00       	mov    $0x0,%eax
801040c5:	e9 07 01 00 00       	jmp    801041d1 <allocproc+0x1a5>
  }
  sp = p->kstack + KSTACKSIZE;
801040ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040cd:	8b 40 08             	mov    0x8(%eax),%eax
801040d0:	05 00 10 00 00       	add    $0x1000,%eax
801040d5:	89 45 f0             	mov    %eax,-0x10(%ebp)
  
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801040d8:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
801040dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040df:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040e2:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
801040e5:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
801040e9:	ba 9c 69 10 80       	mov    $0x8010699c,%edx
801040ee:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040f1:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
801040f3:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
801040f7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040fa:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040fd:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
80104100:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104103:	8b 40 1c             	mov    0x1c(%eax),%eax
80104106:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
8010410d:	00 
8010410e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104115:	00 
80104116:	89 04 24             	mov    %eax,(%esp)
80104119:	e8 bc 12 00 00       	call   801053da <memset>
  p->context->eip = (uint)forkret;
8010411e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104121:	8b 40 1c             	mov    0x1c(%eax),%eax
80104124:	ba b9 4d 10 80       	mov    $0x80104db9,%edx
80104129:	89 50 10             	mov    %edx,0x10(%eax)
  int i = 0;
8010412c:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  char name[8];
  name[2] = '.'; name[3] = 's'; name[4] = 'w'; name[5] = 'a'; name[6] = 'p'; name[7] = 0;
80104133:	c6 45 e6 2e          	movb   $0x2e,-0x1a(%ebp)
80104137:	c6 45 e7 73          	movb   $0x73,-0x19(%ebp)
8010413b:	c6 45 e8 77          	movb   $0x77,-0x18(%ebp)
8010413f:	c6 45 e9 61          	movb   $0x61,-0x17(%ebp)
80104143:	c6 45 ea 70          	movb   $0x70,-0x16(%ebp)
80104147:	c6 45 eb 00          	movb   $0x0,-0x15(%ebp)
  name[1] = (char)(((int)'0')+p->pid % 10);
8010414b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010414e:	8b 48 10             	mov    0x10(%eax),%ecx
80104151:	ba 67 66 66 66       	mov    $0x66666667,%edx
80104156:	89 c8                	mov    %ecx,%eax
80104158:	f7 ea                	imul   %edx
8010415a:	c1 fa 02             	sar    $0x2,%edx
8010415d:	89 c8                	mov    %ecx,%eax
8010415f:	c1 f8 1f             	sar    $0x1f,%eax
80104162:	29 c2                	sub    %eax,%edx
80104164:	89 d0                	mov    %edx,%eax
80104166:	c1 e0 02             	shl    $0x2,%eax
80104169:	01 d0                	add    %edx,%eax
8010416b:	01 c0                	add    %eax,%eax
8010416d:	89 ca                	mov    %ecx,%edx
8010416f:	29 c2                	sub    %eax,%edx
80104171:	89 d0                	mov    %edx,%eax
80104173:	83 c0 30             	add    $0x30,%eax
80104176:	88 45 e5             	mov    %al,-0x1b(%ebp)
  if((i=p->pid/10) == 0)
80104179:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010417c:	8b 48 10             	mov    0x10(%eax),%ecx
8010417f:	ba 67 66 66 66       	mov    $0x66666667,%edx
80104184:	89 c8                	mov    %ecx,%eax
80104186:	f7 ea                	imul   %edx
80104188:	c1 fa 02             	sar    $0x2,%edx
8010418b:	89 c8                	mov    %ecx,%eax
8010418d:	c1 f8 1f             	sar    $0x1f,%eax
80104190:	89 d1                	mov    %edx,%ecx
80104192:	29 c1                	sub    %eax,%ecx
80104194:	89 c8                	mov    %ecx,%eax
80104196:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104199:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010419d:	75 06                	jne    801041a5 <allocproc+0x179>
    name[0] = '0';
8010419f:	c6 45 e4 30          	movb   $0x30,-0x1c(%ebp)
801041a3:	eb 09                	jmp    801041ae <allocproc+0x182>
  else
    name[0] = (char)(((int)'0')+i);
801041a5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801041a8:	83 c0 30             	add    $0x30,%eax
801041ab:	88 45 e4             	mov    %al,-0x1c(%ebp)
  //release(&ptable.lock);
  safestrcpy(p->swapFileName, name, sizeof(name));
801041ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041b1:	8d 90 80 00 00 00    	lea    0x80(%eax),%edx
801041b7:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801041be:	00 
801041bf:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801041c2:	89 44 24 04          	mov    %eax,0x4(%esp)
801041c6:	89 14 24             	mov    %edx,(%esp)
801041c9:	e8 3c 14 00 00       	call   8010560a <safestrcpy>
  return p;
801041ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801041d1:	c9                   	leave  
801041d2:	c3                   	ret    

801041d3 <createInternalProcess>:


void createInternalProcess(const char *name, void (*entrypoint)())
{
801041d3:	55                   	push   %ebp
801041d4:	89 e5                	mov    %esp,%ebp
801041d6:	83 ec 28             	sub    $0x28,%esp
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
801041d9:	e8 4e fe ff ff       	call   8010402c <allocproc>
801041de:	89 45 f4             	mov    %eax,-0xc(%ebp)
801041e1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801041e5:	0f 84 f7 00 00 00    	je     801042e2 <createInternalProcess+0x10f>
    return;

  // Copy process state from p.
  if((np->pgdir = setupkvm(kalloc)) == 0)
801041eb:	c7 04 24 f7 2a 10 80 	movl   $0x80102af7,(%esp)
801041f2:	e8 a2 3e 00 00       	call   80108099 <setupkvm>
801041f7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041fa:	89 42 04             	mov    %eax,0x4(%edx)
801041fd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104200:	8b 40 04             	mov    0x4(%eax),%eax
80104203:	85 c0                	test   %eax,%eax
80104205:	75 0c                	jne    80104213 <createInternalProcess+0x40>
      panic("inswapper: out of memory?");
80104207:	c7 04 24 bc 8b 10 80 	movl   $0x80108bbc,(%esp)
8010420e:	e8 2a c3 ff ff       	call   8010053d <panic>

  np->sz = PGSIZE;
80104213:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104216:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  np->parent = initproc;
8010421c:	8b 15 48 c6 10 80    	mov    0x8010c648,%edx
80104222:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104225:	89 50 14             	mov    %edx,0x14(%eax)
  memset(np->tf, 0, sizeof(*np->tf));
80104228:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010422b:	8b 40 18             	mov    0x18(%eax),%eax
8010422e:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
80104235:	00 
80104236:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010423d:	00 
8010423e:	89 04 24             	mov    %eax,(%esp)
80104241:	e8 94 11 00 00       	call   801053da <memset>
  np->tf->cs = (SEG_KCODE << 3);
80104246:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104249:	8b 40 18             	mov    0x18(%eax),%eax
8010424c:	66 c7 40 3c 08 00    	movw   $0x8,0x3c(%eax)
  np->tf->ds = (SEG_KDATA << 3);
80104252:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104255:	8b 40 18             	mov    0x18(%eax),%eax
80104258:	66 c7 40 2c 10 00    	movw   $0x10,0x2c(%eax)
  np->tf->es = np->tf->ds;
8010425e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104261:	8b 40 18             	mov    0x18(%eax),%eax
80104264:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104267:	8b 52 18             	mov    0x18(%edx),%edx
8010426a:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
8010426e:	66 89 50 28          	mov    %dx,0x28(%eax)
  np->tf->ss = np->tf->ds;
80104272:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104275:	8b 40 18             	mov    0x18(%eax),%eax
80104278:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010427b:	8b 52 18             	mov    0x18(%edx),%edx
8010427e:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104282:	66 89 50 48          	mov    %dx,0x48(%eax)
  np->tf->eflags = FL_IF;
80104286:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104289:	8b 40 18             	mov    0x18(%eax),%eax
8010428c:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  //np->tf->esp = (uint)entrypoint+PGSIZE;
  //np->tf->eip = (uint)entrypoint;
  np->context->eip = (uint)entrypoint;
80104293:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104296:	8b 40 1c             	mov    0x1c(%eax),%eax
80104299:	8b 55 0c             	mov    0xc(%ebp),%edx
8010429c:	89 50 10             	mov    %edx,0x10(%eax)

  inswapper = np;
8010429f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042a2:	a3 4c c6 10 80       	mov    %eax,0x8010c64c
  np->cwd = namei("/");
801042a7:	c7 04 24 d6 8b 10 80 	movl   $0x80108bd6,(%esp)
801042ae:	e8 57 e1 ff ff       	call   8010240a <namei>
801042b3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801042b6:	89 42 68             	mov    %eax,0x68(%edx)
  safestrcpy(np->name, name, sizeof(name));
801042b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042bc:	8d 50 6c             	lea    0x6c(%eax),%edx
801042bf:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801042c6:	00 
801042c7:	8b 45 08             	mov    0x8(%ebp),%eax
801042ca:	89 44 24 04          	mov    %eax,0x4(%esp)
801042ce:	89 14 24             	mov    %edx,(%esp)
801042d1:	e8 34 13 00 00       	call   8010560a <safestrcpy>
  np->state = RUNNABLE;
801042d6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042d9:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
801042e0:	eb 01                	jmp    801042e3 <createInternalProcess+0x110>
{
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
    return;
801042e2:	90                   	nop

  inswapper = np;
  np->cwd = namei("/");
  safestrcpy(np->name, name, sizeof(name));
  np->state = RUNNABLE;
}
801042e3:	c9                   	leave  
801042e4:	c3                   	ret    

801042e5 <swapIn>:

void swapIn()
{
801042e5:	55                   	push   %ebp
801042e6:	89 e5                	mov    %esp,%ebp
801042e8:	81 ec 28 10 00 00    	sub    $0x1028,%esp
  struct proc* t;
  //int intena;
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
801042ee:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
801042f5:	e9 c6 01 00 00       	jmp    801044c0 <swapIn+0x1db>
    {
      if(t->state != RUNNABLE_SUSPENDED)
801042fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042fd:	8b 40 0c             	mov    0xc(%eax),%eax
80104300:	83 f8 07             	cmp    $0x7,%eax
80104303:	0f 85 af 01 00 00    	jne    801044b8 <swapIn+0x1d3>
	continue;
      cprintf("swapin %s\n",t->swapFileName);
80104309:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010430c:	83 e8 80             	sub    $0xffffff80,%eax
8010430f:	89 44 24 04          	mov    %eax,0x4(%esp)
80104313:	c7 04 24 d8 8b 10 80 	movl   $0x80108bd8,(%esp)
8010431a:	e8 82 c0 ff ff       	call   801003a1 <cprintf>
      cprintf("intena = %d\n",cpu->intena);
8010431f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104325:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
8010432b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010432f:	c7 04 24 e3 8b 10 80 	movl   $0x80108be3,(%esp)
80104336:	e8 66 c0 ff ff       	call   801003a1 <cprintf>
      pushcli();
8010433b:	e8 94 0f 00 00       	call   801052d4 <pushcli>
      //intena = cpu->intena;
      //cpu->intena = 1;
      //open file pid.swap
      if(holding(&ptable.lock))
80104340:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104347:	e8 5e 0f 00 00       	call   801052aa <holding>
8010434c:	85 c0                	test   %eax,%eax
8010434e:	74 0c                	je     8010435c <swapIn+0x77>
	release(&ptable.lock);
80104350:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104357:	e8 92 0e 00 00       	call   801051ee <release>
      if((t->swap = fileopen(t->swapFileName,O_RDONLY)) == 0)
8010435c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010435f:	83 e8 80             	sub    $0xffffff80,%eax
80104362:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104369:	00 
8010436a:	89 04 24             	mov    %eax,(%esp)
8010436d:	e8 97 1d 00 00       	call   80106109 <fileopen>
80104372:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104375:	89 42 7c             	mov    %eax,0x7c(%edx)
80104378:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010437b:	8b 40 7c             	mov    0x7c(%eax),%eax
8010437e:	85 c0                	test   %eax,%eax
80104380:	75 1d                	jne    8010439f <swapIn+0xba>
      {
	cprintf("fileopen failed\n");
80104382:	c7 04 24 f0 8b 10 80 	movl   $0x80108bf0,(%esp)
80104389:	e8 13 c0 ff ff       	call   801003a1 <cprintf>
	acquire(&ptable.lock);
8010438e:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104395:	e8 b9 0d 00 00       	call   80105153 <acquire>
	break;
8010439a:	e9 2e 01 00 00       	jmp    801044cd <swapIn+0x1e8>
      }
      acquire(&ptable.lock);
8010439f:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
801043a6:	e8 a8 0d 00 00       	call   80105153 <acquire>
      char buf[PGSIZE];
      int read=0;
801043ab:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
      
      // allocate virtual memory
      if(!allocuvm(t->pgdir, 0, t->sz))
801043b2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043b5:	8b 10                	mov    (%eax),%edx
801043b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043ba:	8b 40 04             	mov    0x4(%eax),%eax
801043bd:	89 54 24 08          	mov    %edx,0x8(%esp)
801043c1:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801043c8:	00 
801043c9:	89 04 24             	mov    %eax,(%esp)
801043cc:	e8 9a 40 00 00       	call   8010846b <allocuvm>
801043d1:	85 c0                	test   %eax,%eax
801043d3:	75 11                	jne    801043e6 <swapIn+0x101>
      {
	cprintf("allocuvm failed\n");
801043d5:	c7 04 24 01 8c 10 80 	movl   $0x80108c01,(%esp)
801043dc:	e8 c0 bf ff ff       	call   801003a1 <cprintf>
	break;
801043e1:	e9 e7 00 00 00       	jmp    801044cd <swapIn+0x1e8>
      }
      
      uint a = 0;
801043e6:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
      if(holding(&ptable.lock))
801043ed:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
801043f4:	e8 b1 0e 00 00       	call   801052aa <holding>
801043f9:	85 c0                	test   %eax,%eax
801043fb:	74 78                	je     80104475 <swapIn+0x190>
	release(&ptable.lock);
801043fd:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104404:	e8 e5 0d 00 00       	call   801051ee <release>
      for(; a < t->sz; a += PGSIZE)
80104409:	eb 6a                	jmp    80104475 <swapIn+0x190>
      {
	if((read = fileread(t->swap, buf, PGSIZE)) > 0)
8010440b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010440e:	8b 40 7c             	mov    0x7c(%eax),%eax
80104411:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80104418:	00 
80104419:	8d 95 ec ef ff ff    	lea    -0x1014(%ebp),%edx
8010441f:	89 54 24 04          	mov    %edx,0x4(%esp)
80104423:	89 04 24             	mov    %eax,(%esp)
80104426:	e8 be cc ff ff       	call   801010e9 <fileread>
8010442b:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010442e:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80104432:	7e 38                	jle    8010446c <swapIn+0x187>
	{
	  if(copyout(t->pgdir,a, buf, read) < 0)
80104434:	8b 55 ec             	mov    -0x14(%ebp),%edx
80104437:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010443a:	8b 40 04             	mov    0x4(%eax),%eax
8010443d:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104441:	8d 95 ec ef ff ff    	lea    -0x1014(%ebp),%edx
80104447:	89 54 24 08          	mov    %edx,0x8(%esp)
8010444b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010444e:	89 54 24 04          	mov    %edx,0x4(%esp)
80104452:	89 04 24             	mov    %eax,(%esp)
80104455:	e8 e9 43 00 00       	call   80108843 <copyout>
8010445a:	85 c0                	test   %eax,%eax
8010445c:	79 0e                	jns    8010446c <swapIn+0x187>
	  {
	    cprintf("copyout failed\n");
8010445e:	c7 04 24 12 8c 10 80 	movl   $0x80108c12,(%esp)
80104465:	e8 37 bf ff ff       	call   801003a1 <cprintf>
	    break;
8010446a:	eb 14                	jmp    80104480 <swapIn+0x19b>
      }
      
      uint a = 0;
      if(holding(&ptable.lock))
	release(&ptable.lock);
      for(; a < t->sz; a += PGSIZE)
8010446c:	81 45 f0 00 10 00 00 	addl   $0x1000,-0x10(%ebp)
80104473:	eb 01                	jmp    80104476 <swapIn+0x191>
80104475:	90                   	nop
80104476:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104479:	8b 00                	mov    (%eax),%eax
8010447b:	3b 45 f0             	cmp    -0x10(%ebp),%eax
8010447e:	77 8b                	ja     8010440b <swapIn+0x126>
	    cprintf("copyout failed\n");
	    break;
	  }
	}
      }
      t->isSwapped = 0;
80104480:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104483:	c7 80 88 00 00 00 00 	movl   $0x0,0x88(%eax)
8010448a:	00 00 00 
      fileclose(t->swap);
8010448d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104490:	8b 40 7c             	mov    0x7c(%eax),%eax
80104493:	89 04 24             	mov    %eax,(%esp)
80104496:	e8 29 cb ff ff       	call   80100fc4 <fileclose>
      t->state = RUNNABLE;
8010449b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010449e:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      acquire(&ptable.lock);
801044a5:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
801044ac:	e8 a2 0c 00 00       	call   80105153 <acquire>
      popcli();
801044b1:	e8 66 0e 00 00       	call   8010531c <popcli>
801044b6:	eb 01                	jmp    801044b9 <swapIn+0x1d4>
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
    {
      if(t->state != RUNNABLE_SUSPENDED)
	continue;
801044b8:	90                   	nop
{
  struct proc* t;
  //int intena;
  for(;;)
  {
    for(t = ptable.proc; t < &ptable.proc[NPROC]; t++)
801044b9:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
801044c0:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
801044c7:	0f 82 2d fe ff ff    	jb     801042fa <swapIn+0x15>
      popcli();
      //cpu->intena = intena;
      //release(&ptable.lock);
      // delete fild pid.swap
    }
    proc->chan = inswapper;
801044cd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044d3:	8b 15 4c c6 10 80    	mov    0x8010c64c,%edx
801044d9:	89 50 20             	mov    %edx,0x20(%eax)
    proc->state = SLEEPING;
801044dc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044e2:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
    sched();
801044e9:	e8 e7 07 00 00       	call   80104cd5 <sched>
  }
801044ee:	e9 fb fd ff ff       	jmp    801042ee <swapIn+0x9>

801044f3 <swapOut>:
}

void
swapOut()
{
801044f3:	55                   	push   %ebp
801044f4:	89 e5                	mov    %esp,%ebp
801044f6:	53                   	push   %ebx
801044f7:	83 ec 24             	sub    $0x24,%esp
    proc->swap = fileopen(proc->swapFileName,(O_CREATE | O_RDWR));
801044fa:	65 8b 1d 04 00 00 00 	mov    %gs:0x4,%ebx
80104501:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104507:	83 e8 80             	sub    $0xffffff80,%eax
8010450a:	c7 44 24 04 02 02 00 	movl   $0x202,0x4(%esp)
80104511:	00 
80104512:	89 04 24             	mov    %eax,(%esp)
80104515:	e8 ef 1b 00 00       	call   80106109 <fileopen>
8010451a:	89 43 7c             	mov    %eax,0x7c(%ebx)
    pte_t *pte;
    uint pa, j;
    for(j = 0; j < proc->sz; j += PGSIZE)
8010451d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104524:	e9 9a 00 00 00       	jmp    801045c3 <swapOut+0xd0>
    {
      if((pte = walkpgdir(proc->pgdir, (void *) j, 0)) == 0)
80104529:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010452c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104532:	8b 40 04             	mov    0x4(%eax),%eax
80104535:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010453c:	00 
8010453d:	89 54 24 04          	mov    %edx,0x4(%esp)
80104541:	89 04 24             	mov    %eax,(%esp)
80104544:	e8 26 3a 00 00       	call   80107f6f <walkpgdir>
80104549:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010454c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104550:	75 0c                	jne    8010455e <swapOut+0x6b>
	panic("walkpgdir: pte should exist");
80104552:	c7 04 24 22 8c 10 80 	movl   $0x80108c22,(%esp)
80104559:	e8 df bf ff ff       	call   8010053d <panic>
      if(!(*pte & PTE_P))
8010455e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104561:	8b 00                	mov    (%eax),%eax
80104563:	83 e0 01             	and    $0x1,%eax
80104566:	85 c0                	test   %eax,%eax
80104568:	75 0c                	jne    80104576 <swapOut+0x83>
	panic("walkpgdir: page not present");
8010456a:	c7 04 24 3e 8c 10 80 	movl   $0x80108c3e,(%esp)
80104571:	e8 c7 bf ff ff       	call   8010053d <panic>
      pa = PTE_ADDR(*pte);
80104576:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104579:	8b 00                	mov    (%eax),%eax
8010457b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80104580:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(filewrite(proc->swap, (char*)p2v(pa), PGSIZE) < 0)
80104583:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104586:	89 04 24             	mov    %eax,(%esp)
80104589:	e8 5a fa ff ff       	call   80103fe8 <p2v>
8010458e:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104595:	8b 52 7c             	mov    0x7c(%edx),%edx
80104598:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010459f:	00 
801045a0:	89 44 24 04          	mov    %eax,0x4(%esp)
801045a4:	89 14 24             	mov    %edx,(%esp)
801045a7:	e8 f9 cb ff ff       	call   801011a5 <filewrite>
801045ac:	85 c0                	test   %eax,%eax
801045ae:	79 0c                	jns    801045bc <swapOut+0xc9>
	panic("filewrite failed");
801045b0:	c7 04 24 5a 8c 10 80 	movl   $0x80108c5a,(%esp)
801045b7:	e8 81 bf ff ff       	call   8010053d <panic>
swapOut()
{
    proc->swap = fileopen(proc->swapFileName,(O_CREATE | O_RDWR));
    pte_t *pte;
    uint pa, j;
    for(j = 0; j < proc->sz; j += PGSIZE)
801045bc:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801045c3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045c9:	8b 00                	mov    (%eax),%eax
801045cb:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801045ce:	0f 87 55 ff ff ff    	ja     80104529 <swapOut+0x36>
	panic("walkpgdir: page not present");
      pa = PTE_ADDR(*pte);
      if(filewrite(proc->swap, (char*)p2v(pa), PGSIZE) < 0)
	panic("filewrite failed");
    }
    proc->state = SLEEPING_SUSPENDED;
801045d4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045da:	c7 40 0c 06 00 00 00 	movl   $0x6,0xc(%eax)
    proc->isSwapped = 1;
801045e1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045e7:	c7 80 88 00 00 00 01 	movl   $0x1,0x88(%eax)
801045ee:	00 00 00 
}
801045f1:	83 c4 24             	add    $0x24,%esp
801045f4:	5b                   	pop    %ebx
801045f5:	5d                   	pop    %ebp
801045f6:	c3                   	ret    

801045f7 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
801045f7:	55                   	push   %ebp
801045f8:	89 e5                	mov    %esp,%ebp
801045fa:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
801045fd:	e8 2a fa ff ff       	call   8010402c <allocproc>
80104602:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
80104605:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104608:	a3 48 c6 10 80       	mov    %eax,0x8010c648
  if((p->pgdir = setupkvm(kalloc)) == 0)
8010460d:	c7 04 24 f7 2a 10 80 	movl   $0x80102af7,(%esp)
80104614:	e8 80 3a 00 00       	call   80108099 <setupkvm>
80104619:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010461c:	89 42 04             	mov    %eax,0x4(%edx)
8010461f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104622:	8b 40 04             	mov    0x4(%eax),%eax
80104625:	85 c0                	test   %eax,%eax
80104627:	75 0c                	jne    80104635 <userinit+0x3e>
    panic("userinit: out of memory?");
80104629:	c7 04 24 6b 8c 10 80 	movl   $0x80108c6b,(%esp)
80104630:	e8 08 bf ff ff       	call   8010053d <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
80104635:	ba 2c 00 00 00       	mov    $0x2c,%edx
8010463a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010463d:	8b 40 04             	mov    0x4(%eax),%eax
80104640:	89 54 24 08          	mov    %edx,0x8(%esp)
80104644:	c7 44 24 04 e0 c4 10 	movl   $0x8010c4e0,0x4(%esp)
8010464b:	80 
8010464c:	89 04 24             	mov    %eax,(%esp)
8010464f:	e8 9d 3c 00 00       	call   801082f1 <inituvm>
  p->sz = PGSIZE;
80104654:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104657:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
8010465d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104660:	8b 40 18             	mov    0x18(%eax),%eax
80104663:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
8010466a:	00 
8010466b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104672:	00 
80104673:	89 04 24             	mov    %eax,(%esp)
80104676:	e8 5f 0d 00 00       	call   801053da <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
8010467b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010467e:	8b 40 18             	mov    0x18(%eax),%eax
80104681:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80104687:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010468a:	8b 40 18             	mov    0x18(%eax),%eax
8010468d:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
80104693:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104696:	8b 40 18             	mov    0x18(%eax),%eax
80104699:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010469c:	8b 52 18             	mov    0x18(%edx),%edx
8010469f:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801046a3:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
801046a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046aa:	8b 40 18             	mov    0x18(%eax),%eax
801046ad:	8b 55 f4             	mov    -0xc(%ebp),%edx
801046b0:	8b 52 18             	mov    0x18(%edx),%edx
801046b3:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801046b7:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
801046bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046be:	8b 40 18             	mov    0x18(%eax),%eax
801046c1:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
801046c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046cb:	8b 40 18             	mov    0x18(%eax),%eax
801046ce:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
801046d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046d8:	8b 40 18             	mov    0x18(%eax),%eax
801046db:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
801046e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046e5:	83 c0 6c             	add    $0x6c,%eax
801046e8:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
801046ef:	00 
801046f0:	c7 44 24 04 84 8c 10 	movl   $0x80108c84,0x4(%esp)
801046f7:	80 
801046f8:	89 04 24             	mov    %eax,(%esp)
801046fb:	e8 0a 0f 00 00       	call   8010560a <safestrcpy>
  p->cwd = namei("/");
80104700:	c7 04 24 d6 8b 10 80 	movl   $0x80108bd6,(%esp)
80104707:	e8 fe dc ff ff       	call   8010240a <namei>
8010470c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010470f:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
80104712:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104715:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)

  createInternalProcess("inswapper", swapIn);
8010471c:	c7 44 24 04 e5 42 10 	movl   $0x801042e5,0x4(%esp)
80104723:	80 
80104724:	c7 04 24 8d 8c 10 80 	movl   $0x80108c8d,(%esp)
8010472b:	e8 a3 fa ff ff       	call   801041d3 <createInternalProcess>
}
80104730:	c9                   	leave  
80104731:	c3                   	ret    

80104732 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
80104732:	55                   	push   %ebp
80104733:	89 e5                	mov    %esp,%ebp
80104735:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
80104738:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010473e:	8b 00                	mov    (%eax),%eax
80104740:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
80104743:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104747:	7e 34                	jle    8010477d <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
80104749:	8b 45 08             	mov    0x8(%ebp),%eax
8010474c:	89 c2                	mov    %eax,%edx
8010474e:	03 55 f4             	add    -0xc(%ebp),%edx
80104751:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104757:	8b 40 04             	mov    0x4(%eax),%eax
8010475a:	89 54 24 08          	mov    %edx,0x8(%esp)
8010475e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104761:	89 54 24 04          	mov    %edx,0x4(%esp)
80104765:	89 04 24             	mov    %eax,(%esp)
80104768:	e8 fe 3c 00 00       	call   8010846b <allocuvm>
8010476d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104770:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104774:	75 41                	jne    801047b7 <growproc+0x85>
      return -1;
80104776:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010477b:	eb 58                	jmp    801047d5 <growproc+0xa3>
  } else if(n < 0){
8010477d:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104781:	79 34                	jns    801047b7 <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
80104783:	8b 45 08             	mov    0x8(%ebp),%eax
80104786:	89 c2                	mov    %eax,%edx
80104788:	03 55 f4             	add    -0xc(%ebp),%edx
8010478b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104791:	8b 40 04             	mov    0x4(%eax),%eax
80104794:	89 54 24 08          	mov    %edx,0x8(%esp)
80104798:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010479b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010479f:	89 04 24             	mov    %eax,(%esp)
801047a2:	e8 9e 3d 00 00       	call   80108545 <deallocuvm>
801047a7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801047aa:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801047ae:	75 07                	jne    801047b7 <growproc+0x85>
      return -1;
801047b0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801047b5:	eb 1e                	jmp    801047d5 <growproc+0xa3>
  }
  proc->sz = sz;
801047b7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047bd:	8b 55 f4             	mov    -0xc(%ebp),%edx
801047c0:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
801047c2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047c8:	89 04 24             	mov    %eax,(%esp)
801047cb:	e8 ba 39 00 00       	call   8010818a <switchuvm>
  return 0;
801047d0:	b8 00 00 00 00       	mov    $0x0,%eax
}
801047d5:	c9                   	leave  
801047d6:	c3                   	ret    

801047d7 <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
801047d7:	55                   	push   %ebp
801047d8:	89 e5                	mov    %esp,%ebp
801047da:	57                   	push   %edi
801047db:	56                   	push   %esi
801047dc:	53                   	push   %ebx
801047dd:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
801047e0:	e8 47 f8 ff ff       	call   8010402c <allocproc>
801047e5:	89 45 e0             	mov    %eax,-0x20(%ebp)
801047e8:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
801047ec:	75 0a                	jne    801047f8 <fork+0x21>
    return -1;
801047ee:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801047f3:	e9 3a 01 00 00       	jmp    80104932 <fork+0x15b>
  
  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
801047f8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047fe:	8b 10                	mov    (%eax),%edx
80104800:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104806:	8b 40 04             	mov    0x4(%eax),%eax
80104809:	89 54 24 04          	mov    %edx,0x4(%esp)
8010480d:	89 04 24             	mov    %eax,(%esp)
80104810:	e8 c0 3e 00 00       	call   801086d5 <copyuvm>
80104815:	8b 55 e0             	mov    -0x20(%ebp),%edx
80104818:	89 42 04             	mov    %eax,0x4(%edx)
8010481b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010481e:	8b 40 04             	mov    0x4(%eax),%eax
80104821:	85 c0                	test   %eax,%eax
80104823:	75 2c                	jne    80104851 <fork+0x7a>
    kfree(np->kstack);
80104825:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104828:	8b 40 08             	mov    0x8(%eax),%eax
8010482b:	89 04 24             	mov    %eax,(%esp)
8010482e:	e8 2b e2 ff ff       	call   80102a5e <kfree>
    np->kstack = 0;
80104833:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104836:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
8010483d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104840:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
80104847:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010484c:	e9 e1 00 00 00       	jmp    80104932 <fork+0x15b>
  }
  np->sz = proc->sz;
80104851:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104857:	8b 10                	mov    (%eax),%edx
80104859:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010485c:	89 10                	mov    %edx,(%eax)
  np->parent = proc;
8010485e:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104865:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104868:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *proc->tf;
8010486b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010486e:	8b 50 18             	mov    0x18(%eax),%edx
80104871:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104877:	8b 40 18             	mov    0x18(%eax),%eax
8010487a:	89 c3                	mov    %eax,%ebx
8010487c:	b8 13 00 00 00       	mov    $0x13,%eax
80104881:	89 d7                	mov    %edx,%edi
80104883:	89 de                	mov    %ebx,%esi
80104885:	89 c1                	mov    %eax,%ecx
80104887:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
80104889:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010488c:	8b 40 18             	mov    0x18(%eax),%eax
8010488f:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
80104896:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
8010489d:	eb 3d                	jmp    801048dc <fork+0x105>
    if(proc->ofile[i])
8010489f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048a5:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801048a8:	83 c2 08             	add    $0x8,%edx
801048ab:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801048af:	85 c0                	test   %eax,%eax
801048b1:	74 25                	je     801048d8 <fork+0x101>
      np->ofile[i] = filedup(proc->ofile[i]);
801048b3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048b9:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801048bc:	83 c2 08             	add    $0x8,%edx
801048bf:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801048c3:	89 04 24             	mov    %eax,(%esp)
801048c6:	e8 b1 c6 ff ff       	call   80100f7c <filedup>
801048cb:	8b 55 e0             	mov    -0x20(%ebp),%edx
801048ce:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
801048d1:	83 c1 08             	add    $0x8,%ecx
801048d4:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *proc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
801048d8:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
801048dc:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
801048e0:	7e bd                	jle    8010489f <fork+0xc8>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
801048e2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048e8:	8b 40 68             	mov    0x68(%eax),%eax
801048eb:	89 04 24             	mov    %eax,(%esp)
801048ee:	e8 43 cf ff ff       	call   80101836 <idup>
801048f3:	8b 55 e0             	mov    -0x20(%ebp),%edx
801048f6:	89 42 68             	mov    %eax,0x68(%edx)
 
  pid = np->pid;
801048f9:	8b 45 e0             	mov    -0x20(%ebp),%eax
801048fc:	8b 40 10             	mov    0x10(%eax),%eax
801048ff:	89 45 dc             	mov    %eax,-0x24(%ebp)
  np->state = RUNNABLE;
80104902:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104905:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  safestrcpy(np->name, proc->name, sizeof(proc->name));
8010490c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104912:	8d 50 6c             	lea    0x6c(%eax),%edx
80104915:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104918:	83 c0 6c             	add    $0x6c,%eax
8010491b:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104922:	00 
80104923:	89 54 24 04          	mov    %edx,0x4(%esp)
80104927:	89 04 24             	mov    %eax,(%esp)
8010492a:	e8 db 0c 00 00       	call   8010560a <safestrcpy>
  return pid;
8010492f:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
80104932:	83 c4 2c             	add    $0x2c,%esp
80104935:	5b                   	pop    %ebx
80104936:	5e                   	pop    %esi
80104937:	5f                   	pop    %edi
80104938:	5d                   	pop    %ebp
80104939:	c3                   	ret    

8010493a <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
8010493a:	55                   	push   %ebp
8010493b:	89 e5                	mov    %esp,%ebp
8010493d:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
80104940:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104947:	a1 48 c6 10 80       	mov    0x8010c648,%eax
8010494c:	39 c2                	cmp    %eax,%edx
8010494e:	75 0c                	jne    8010495c <exit+0x22>
    panic("init exiting");
80104950:	c7 04 24 97 8c 10 80 	movl   $0x80108c97,(%esp)
80104957:	e8 e1 bb ff ff       	call   8010053d <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
8010495c:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104963:	eb 44                	jmp    801049a9 <exit+0x6f>
    if(proc->ofile[fd]){
80104965:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010496b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010496e:	83 c2 08             	add    $0x8,%edx
80104971:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104975:	85 c0                	test   %eax,%eax
80104977:	74 2c                	je     801049a5 <exit+0x6b>
      fileclose(proc->ofile[fd]);
80104979:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010497f:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104982:	83 c2 08             	add    $0x8,%edx
80104985:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104989:	89 04 24             	mov    %eax,(%esp)
8010498c:	e8 33 c6 ff ff       	call   80100fc4 <fileclose>
      proc->ofile[fd] = 0;
80104991:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104997:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010499a:	83 c2 08             	add    $0x8,%edx
8010499d:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801049a4:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801049a5:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
801049a9:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801049ad:	7e b6                	jle    80104965 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  iput(proc->cwd);
801049af:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049b5:	8b 40 68             	mov    0x68(%eax),%eax
801049b8:	89 04 24             	mov    %eax,(%esp)
801049bb:	e8 5b d0 ff ff       	call   80101a1b <iput>
  proc->cwd = 0;
801049c0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049c6:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
801049cd:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
801049d4:	e8 7a 07 00 00       	call   80105153 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
801049d9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049df:	8b 40 14             	mov    0x14(%eax),%eax
801049e2:	89 04 24             	mov    %eax,(%esp)
801049e5:	e8 f1 04 00 00       	call   80104edb <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801049ea:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
801049f1:	eb 3b                	jmp    80104a2e <exit+0xf4>
    if(p->parent == proc){
801049f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049f6:	8b 50 14             	mov    0x14(%eax),%edx
801049f9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049ff:	39 c2                	cmp    %eax,%edx
80104a01:	75 24                	jne    80104a27 <exit+0xed>
      p->parent = initproc;
80104a03:	8b 15 48 c6 10 80    	mov    0x8010c648,%edx
80104a09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a0c:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
80104a0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a12:	8b 40 0c             	mov    0xc(%eax),%eax
80104a15:	83 f8 05             	cmp    $0x5,%eax
80104a18:	75 0d                	jne    80104a27 <exit+0xed>
        wakeup1(initproc);
80104a1a:	a1 48 c6 10 80       	mov    0x8010c648,%eax
80104a1f:	89 04 24             	mov    %eax,(%esp)
80104a22:	e8 b4 04 00 00       	call   80104edb <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104a27:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
80104a2e:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
80104a35:	72 bc                	jb     801049f3 <exit+0xb9>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
80104a37:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104a3d:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
80104a44:	e8 8c 02 00 00       	call   80104cd5 <sched>
  panic("zombie exit");
80104a49:	c7 04 24 a4 8c 10 80 	movl   $0x80108ca4,(%esp)
80104a50:	e8 e8 ba ff ff       	call   8010053d <panic>

80104a55 <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
80104a55:	55                   	push   %ebp
80104a56:	89 e5                	mov    %esp,%ebp
80104a58:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
80104a5b:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104a62:	e8 ec 06 00 00       	call   80105153 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
80104a67:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104a6e:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
80104a75:	e9 9d 00 00 00       	jmp    80104b17 <wait+0xc2>
      if(p->parent != proc)
80104a7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a7d:	8b 50 14             	mov    0x14(%eax),%edx
80104a80:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104a86:	39 c2                	cmp    %eax,%edx
80104a88:	0f 85 81 00 00 00    	jne    80104b0f <wait+0xba>
        continue;
      havekids = 1;
80104a8e:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
80104a95:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a98:	8b 40 0c             	mov    0xc(%eax),%eax
80104a9b:	83 f8 05             	cmp    $0x5,%eax
80104a9e:	75 70                	jne    80104b10 <wait+0xbb>
        // Found one.
        pid = p->pid;
80104aa0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aa3:	8b 40 10             	mov    0x10(%eax),%eax
80104aa6:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
80104aa9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aac:	8b 40 08             	mov    0x8(%eax),%eax
80104aaf:	89 04 24             	mov    %eax,(%esp)
80104ab2:	e8 a7 df ff ff       	call   80102a5e <kfree>
        p->kstack = 0;
80104ab7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aba:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
80104ac1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ac4:	8b 40 04             	mov    0x4(%eax),%eax
80104ac7:	89 04 24             	mov    %eax,(%esp)
80104aca:	e8 32 3b 00 00       	call   80108601 <freevm>
        p->state = UNUSED;
80104acf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ad2:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
80104ad9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104adc:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
80104ae3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ae6:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
80104aed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104af0:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
80104af4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104af7:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
80104afe:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104b05:	e8 e4 06 00 00       	call   801051ee <release>
        return pid;
80104b0a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104b0d:	eb 56                	jmp    80104b65 <wait+0x110>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc)
        continue;
80104b0f:	90                   	nop

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104b10:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
80104b17:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
80104b1e:	0f 82 56 ff ff ff    	jb     80104a7a <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
80104b24:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104b28:	74 0d                	je     80104b37 <wait+0xe2>
80104b2a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b30:	8b 40 24             	mov    0x24(%eax),%eax
80104b33:	85 c0                	test   %eax,%eax
80104b35:	74 13                	je     80104b4a <wait+0xf5>
      release(&ptable.lock);
80104b37:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104b3e:	e8 ab 06 00 00       	call   801051ee <release>
      return -1;
80104b43:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104b48:	eb 1b                	jmp    80104b65 <wait+0x110>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
80104b4a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b50:	c7 44 24 04 20 0f 11 	movl   $0x80110f20,0x4(%esp)
80104b57:	80 
80104b58:	89 04 24             	mov    %eax,(%esp)
80104b5b:	e8 85 02 00 00       	call   80104de5 <sleep>
  }
80104b60:	e9 02 ff ff ff       	jmp    80104a67 <wait+0x12>
}
80104b65:	c9                   	leave  
80104b66:	c3                   	ret    

80104b67 <register_handler>:

void
register_handler(sighandler_t sighandler)
{
80104b67:	55                   	push   %ebp
80104b68:	89 e5                	mov    %esp,%ebp
80104b6a:	83 ec 28             	sub    $0x28,%esp
  char* addr = uva2ka(proc->pgdir, (char*)proc->tf->esp);
80104b6d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b73:	8b 40 18             	mov    0x18(%eax),%eax
80104b76:	8b 40 44             	mov    0x44(%eax),%eax
80104b79:	89 c2                	mov    %eax,%edx
80104b7b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b81:	8b 40 04             	mov    0x4(%eax),%eax
80104b84:	89 54 24 04          	mov    %edx,0x4(%esp)
80104b88:	89 04 24             	mov    %eax,(%esp)
80104b8b:	e8 56 3c 00 00       	call   801087e6 <uva2ka>
80104b90:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if ((proc->tf->esp & 0xFFF) == 0)
80104b93:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b99:	8b 40 18             	mov    0x18(%eax),%eax
80104b9c:	8b 40 44             	mov    0x44(%eax),%eax
80104b9f:	25 ff 0f 00 00       	and    $0xfff,%eax
80104ba4:	85 c0                	test   %eax,%eax
80104ba6:	75 0c                	jne    80104bb4 <register_handler+0x4d>
    panic("esp_offset == 0");
80104ba8:	c7 04 24 b0 8c 10 80 	movl   $0x80108cb0,(%esp)
80104baf:	e8 89 b9 ff ff       	call   8010053d <panic>

    /* open a new frame */
  *(int*)(addr + ((proc->tf->esp - 4) & 0xFFF))
80104bb4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bba:	8b 40 18             	mov    0x18(%eax),%eax
80104bbd:	8b 40 44             	mov    0x44(%eax),%eax
80104bc0:	83 e8 04             	sub    $0x4,%eax
80104bc3:	25 ff 0f 00 00       	and    $0xfff,%eax
80104bc8:	03 45 f4             	add    -0xc(%ebp),%eax
          = proc->tf->eip;
80104bcb:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104bd2:	8b 52 18             	mov    0x18(%edx),%edx
80104bd5:	8b 52 38             	mov    0x38(%edx),%edx
80104bd8:	89 10                	mov    %edx,(%eax)
  proc->tf->esp -= 4;
80104bda:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104be0:	8b 40 18             	mov    0x18(%eax),%eax
80104be3:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104bea:	8b 52 18             	mov    0x18(%edx),%edx
80104bed:	8b 52 44             	mov    0x44(%edx),%edx
80104bf0:	83 ea 04             	sub    $0x4,%edx
80104bf3:	89 50 44             	mov    %edx,0x44(%eax)

    /* update eip */
  proc->tf->eip = (uint)sighandler;
80104bf6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bfc:	8b 40 18             	mov    0x18(%eax),%eax
80104bff:	8b 55 08             	mov    0x8(%ebp),%edx
80104c02:	89 50 38             	mov    %edx,0x38(%eax)
}
80104c05:	c9                   	leave  
80104c06:	c3                   	ret    

80104c07 <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
80104c07:	55                   	push   %ebp
80104c08:	89 e5                	mov    %esp,%ebp
80104c0a:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  
  for(;;){
    // Enable interrupts on this processor.
    sti();
80104c0d:	e8 f8 f3 ff ff       	call   8010400a <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
80104c12:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104c19:	e8 35 05 00 00       	call   80105153 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104c1e:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
80104c25:	e9 8d 00 00 00       	jmp    80104cb7 <scheduler+0xb0>
      if(p->state != RUNNABLE)
80104c2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c2d:	8b 40 0c             	mov    0xc(%eax),%eax
80104c30:	83 f8 03             	cmp    $0x3,%eax
80104c33:	75 7a                	jne    80104caf <scheduler+0xa8>
        continue;
    
      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
80104c35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c38:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
80104c3e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c41:	89 04 24             	mov    %eax,(%esp)
80104c44:	e8 41 35 00 00       	call   8010818a <switchuvm>
      p->state = RUNNING;
80104c49:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c4c:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
80104c53:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c59:	8b 40 1c             	mov    0x1c(%eax),%eax
80104c5c:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104c63:	83 c2 04             	add    $0x4,%edx
80104c66:	89 44 24 04          	mov    %eax,0x4(%esp)
80104c6a:	89 14 24             	mov    %edx,(%esp)
80104c6d:	e8 0e 0a 00 00       	call   80105680 <swtch>
      switchkvm();
80104c72:	e8 f6 34 00 00       	call   8010816d <switchkvm>
      
      if(proc && proc->isSwapped)
80104c77:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c7d:	85 c0                	test   %eax,%eax
80104c7f:	74 21                	je     80104ca2 <scheduler+0x9b>
80104c81:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c87:	8b 80 88 00 00 00    	mov    0x88(%eax),%eax
80104c8d:	85 c0                	test   %eax,%eax
80104c8f:	74 11                	je     80104ca2 <scheduler+0x9b>
	freevm(proc->pgdir);
80104c91:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104c97:	8b 40 04             	mov    0x4(%eax),%eax
80104c9a:	89 04 24             	mov    %eax,(%esp)
80104c9d:	e8 5f 39 00 00       	call   80108601 <freevm>
      
      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
80104ca2:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80104ca9:	00 00 00 00 
80104cad:	eb 01                	jmp    80104cb0 <scheduler+0xa9>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
80104caf:	90                   	nop
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104cb0:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
80104cb7:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
80104cbe:	0f 82 66 ff ff ff    	jb     80104c2a <scheduler+0x23>
      
      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
80104cc4:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104ccb:	e8 1e 05 00 00       	call   801051ee <release>

  }
80104cd0:	e9 38 ff ff ff       	jmp    80104c0d <scheduler+0x6>

80104cd5 <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
80104cd5:	55                   	push   %ebp
80104cd6:	89 e5                	mov    %esp,%ebp
80104cd8:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
80104cdb:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104ce2:	e8 c3 05 00 00       	call   801052aa <holding>
80104ce7:	85 c0                	test   %eax,%eax
80104ce9:	75 0c                	jne    80104cf7 <sched+0x22>
    panic("sched ptable.lock");
80104ceb:	c7 04 24 c0 8c 10 80 	movl   $0x80108cc0,(%esp)
80104cf2:	e8 46 b8 ff ff       	call   8010053d <panic>
  if(cpu->ncli != 1)
80104cf7:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104cfd:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104d03:	83 f8 01             	cmp    $0x1,%eax
80104d06:	74 0c                	je     80104d14 <sched+0x3f>
    panic("sched locks");
80104d08:	c7 04 24 d2 8c 10 80 	movl   $0x80108cd2,(%esp)
80104d0f:	e8 29 b8 ff ff       	call   8010053d <panic>
  if(proc->state == RUNNING)
80104d14:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d1a:	8b 40 0c             	mov    0xc(%eax),%eax
80104d1d:	83 f8 04             	cmp    $0x4,%eax
80104d20:	75 0c                	jne    80104d2e <sched+0x59>
    panic("sched running");
80104d22:	c7 04 24 de 8c 10 80 	movl   $0x80108cde,(%esp)
80104d29:	e8 0f b8 ff ff       	call   8010053d <panic>
  if(readeflags()&FL_IF)
80104d2e:	e8 c2 f2 ff ff       	call   80103ff5 <readeflags>
80104d33:	25 00 02 00 00       	and    $0x200,%eax
80104d38:	85 c0                	test   %eax,%eax
80104d3a:	74 0c                	je     80104d48 <sched+0x73>
    panic("sched interruptible");
80104d3c:	c7 04 24 ec 8c 10 80 	movl   $0x80108cec,(%esp)
80104d43:	e8 f5 b7 ff ff       	call   8010053d <panic>
  intena = cpu->intena;
80104d48:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104d4e:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104d54:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80104d57:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104d5d:	8b 40 04             	mov    0x4(%eax),%eax
80104d60:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104d67:	83 c2 1c             	add    $0x1c,%edx
80104d6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d6e:	89 14 24             	mov    %edx,(%esp)
80104d71:	e8 0a 09 00 00       	call   80105680 <swtch>
  cpu->intena = intena;
80104d76:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104d7c:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104d7f:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104d85:	c9                   	leave  
80104d86:	c3                   	ret    

80104d87 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
80104d87:	55                   	push   %ebp
80104d88:	89 e5                	mov    %esp,%ebp
80104d8a:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104d8d:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104d94:	e8 ba 03 00 00       	call   80105153 <acquire>
  proc->state = RUNNABLE;
80104d99:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104d9f:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
80104da6:	e8 2a ff ff ff       	call   80104cd5 <sched>
  release(&ptable.lock);
80104dab:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104db2:	e8 37 04 00 00       	call   801051ee <release>
}
80104db7:	c9                   	leave  
80104db8:	c3                   	ret    

80104db9 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80104db9:	55                   	push   %ebp
80104dba:	89 e5                	mov    %esp,%ebp
80104dbc:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80104dbf:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104dc6:	e8 23 04 00 00       	call   801051ee <release>

  if (first) {
80104dcb:	a1 24 c0 10 80       	mov    0x8010c024,%eax
80104dd0:	85 c0                	test   %eax,%eax
80104dd2:	74 0f                	je     80104de3 <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
80104dd4:	c7 05 24 c0 10 80 00 	movl   $0x0,0x8010c024
80104ddb:	00 00 00 
    initlog();
80104dde:	e8 25 e2 ff ff       	call   80103008 <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
80104de3:	c9                   	leave  
80104de4:	c3                   	ret    

80104de5 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80104de5:	55                   	push   %ebp
80104de6:	89 e5                	mov    %esp,%ebp
80104de8:	83 ec 18             	sub    $0x18,%esp
  if(proc == 0)
80104deb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104df1:	85 c0                	test   %eax,%eax
80104df3:	75 0c                	jne    80104e01 <sleep+0x1c>
    panic("sleep");
80104df5:	c7 04 24 00 8d 10 80 	movl   $0x80108d00,(%esp)
80104dfc:	e8 3c b7 ff ff       	call   8010053d <panic>

  if(lk == 0)
80104e01:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104e05:	75 0c                	jne    80104e13 <sleep+0x2e>
    panic("sleep without lk");
80104e07:	c7 04 24 06 8d 10 80 	movl   $0x80108d06,(%esp)
80104e0e:	e8 2a b7 ff ff       	call   8010053d <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104e13:	81 7d 0c 20 0f 11 80 	cmpl   $0x80110f20,0xc(%ebp)
80104e1a:	74 17                	je     80104e33 <sleep+0x4e>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104e1c:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104e23:	e8 2b 03 00 00       	call   80105153 <acquire>
    release(lk);
80104e28:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e2b:	89 04 24             	mov    %eax,(%esp)
80104e2e:	e8 bb 03 00 00       	call   801051ee <release>
  }

  // Go to sleep.
  proc->chan = chan;
80104e33:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e39:	8b 55 08             	mov    0x8(%ebp),%edx
80104e3c:	89 50 20             	mov    %edx,0x20(%eax)
  proc->state = SLEEPING;
80104e3f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e45:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)

  // Swap out
  if(swapFlag)
80104e4c:	a1 08 c0 10 80       	mov    0x8010c008,%eax
80104e51:	85 c0                	test   %eax,%eax
80104e53:	74 2b                	je     80104e80 <sleep+0x9b>
  {
    if(proc->pid > 3)
80104e55:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e5b:	8b 40 10             	mov    0x10(%eax),%eax
80104e5e:	83 f8 03             	cmp    $0x3,%eax
80104e61:	7e 1d                	jle    80104e80 <sleep+0x9b>
    {
      release(&ptable.lock);
80104e63:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104e6a:	e8 7f 03 00 00       	call   801051ee <release>
      swapOut();
80104e6f:	e8 7f f6 ff ff       	call   801044f3 <swapOut>
      acquire(&ptable.lock);
80104e74:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104e7b:	e8 d3 02 00 00       	call   80105153 <acquire>
    }
  }
  
  sched();
80104e80:	e8 50 fe ff ff       	call   80104cd5 <sched>
  if(proc->pid>3)
80104e85:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e8b:	8b 40 10             	mov    0x10(%eax),%eax
80104e8e:	83 f8 03             	cmp    $0x3,%eax
80104e91:	7e 19                	jle    80104eac <sleep+0xc7>
    cprintf("pid = %d, after waking up\n",proc->pid);
80104e93:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104e99:	8b 40 10             	mov    0x10(%eax),%eax
80104e9c:	89 44 24 04          	mov    %eax,0x4(%esp)
80104ea0:	c7 04 24 17 8d 10 80 	movl   $0x80108d17,(%esp)
80104ea7:	e8 f5 b4 ff ff       	call   801003a1 <cprintf>
  // Tidy up.
  proc->chan = 0;
80104eac:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104eb2:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104eb9:	81 7d 0c 20 0f 11 80 	cmpl   $0x80110f20,0xc(%ebp)
80104ec0:	74 17                	je     80104ed9 <sleep+0xf4>
    release(&ptable.lock);
80104ec2:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104ec9:	e8 20 03 00 00       	call   801051ee <release>
    acquire(lk);
80104ece:	8b 45 0c             	mov    0xc(%ebp),%eax
80104ed1:	89 04 24             	mov    %eax,(%esp)
80104ed4:	e8 7a 02 00 00       	call   80105153 <acquire>
  }
}
80104ed9:	c9                   	leave  
80104eda:	c3                   	ret    

80104edb <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104edb:	55                   	push   %ebp
80104edc:	89 e5                	mov    %esp,%ebp
80104ede:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104ee1:	c7 45 fc 54 0f 11 80 	movl   $0x80110f54,-0x4(%ebp)
80104ee8:	eb 53                	jmp    80104f3d <wakeup1+0x62>
  {
    if(p->state == SLEEPING && p->chan == chan)
80104eea:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104eed:	8b 40 0c             	mov    0xc(%eax),%eax
80104ef0:	83 f8 02             	cmp    $0x2,%eax
80104ef3:	75 15                	jne    80104f0a <wakeup1+0x2f>
80104ef5:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104ef8:	8b 40 20             	mov    0x20(%eax),%eax
80104efb:	3b 45 08             	cmp    0x8(%ebp),%eax
80104efe:	75 0a                	jne    80104f0a <wakeup1+0x2f>
      p->state = RUNNABLE;
80104f00:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f03:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
    if(p->state == SLEEPING_SUSPENDED && p->chan == chan)
80104f0a:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f0d:	8b 40 0c             	mov    0xc(%eax),%eax
80104f10:	83 f8 06             	cmp    $0x6,%eax
80104f13:	75 21                	jne    80104f36 <wakeup1+0x5b>
80104f15:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f18:	8b 40 20             	mov    0x20(%eax),%eax
80104f1b:	3b 45 08             	cmp    0x8(%ebp),%eax
80104f1e:	75 16                	jne    80104f36 <wakeup1+0x5b>
    {
      p->state = RUNNABLE_SUSPENDED;
80104f20:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f23:	c7 40 0c 07 00 00 00 	movl   $0x7,0xc(%eax)
      inswapper->state = RUNNABLE;
80104f2a:	a1 4c c6 10 80       	mov    0x8010c64c,%eax
80104f2f:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104f36:	81 45 fc 8c 00 00 00 	addl   $0x8c,-0x4(%ebp)
80104f3d:	81 7d fc 54 32 11 80 	cmpl   $0x80113254,-0x4(%ebp)
80104f44:	72 a4                	jb     80104eea <wakeup1+0xf>
    {
      p->state = RUNNABLE_SUSPENDED;
      inswapper->state = RUNNABLE;
    }
  }
}
80104f46:	c9                   	leave  
80104f47:	c3                   	ret    

80104f48 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104f48:	55                   	push   %ebp
80104f49:	89 e5                	mov    %esp,%ebp
80104f4b:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
80104f4e:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104f55:	e8 f9 01 00 00       	call   80105153 <acquire>
  wakeup1(chan);
80104f5a:	8b 45 08             	mov    0x8(%ebp),%eax
80104f5d:	89 04 24             	mov    %eax,(%esp)
80104f60:	e8 76 ff ff ff       	call   80104edb <wakeup1>
  release(&ptable.lock);
80104f65:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104f6c:	e8 7d 02 00 00       	call   801051ee <release>
}
80104f71:	c9                   	leave  
80104f72:	c3                   	ret    

80104f73 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104f73:	55                   	push   %ebp
80104f74:	89 e5                	mov    %esp,%ebp
80104f76:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104f79:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104f80:	e8 ce 01 00 00       	call   80105153 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104f85:	c7 45 f4 54 0f 11 80 	movl   $0x80110f54,-0xc(%ebp)
80104f8c:	eb 44                	jmp    80104fd2 <kill+0x5f>
    if(p->pid == pid){
80104f8e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f91:	8b 40 10             	mov    0x10(%eax),%eax
80104f94:	3b 45 08             	cmp    0x8(%ebp),%eax
80104f97:	75 32                	jne    80104fcb <kill+0x58>
      p->killed = 1;
80104f99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f9c:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104fa3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fa6:	8b 40 0c             	mov    0xc(%eax),%eax
80104fa9:	83 f8 02             	cmp    $0x2,%eax
80104fac:	75 0a                	jne    80104fb8 <kill+0x45>
        p->state = RUNNABLE;
80104fae:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fb1:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104fb8:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104fbf:	e8 2a 02 00 00       	call   801051ee <release>
      return 0;
80104fc4:	b8 00 00 00 00       	mov    $0x0,%eax
80104fc9:	eb 21                	jmp    80104fec <kill+0x79>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104fcb:	81 45 f4 8c 00 00 00 	addl   $0x8c,-0xc(%ebp)
80104fd2:	81 7d f4 54 32 11 80 	cmpl   $0x80113254,-0xc(%ebp)
80104fd9:	72 b3                	jb     80104f8e <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104fdb:	c7 04 24 20 0f 11 80 	movl   $0x80110f20,(%esp)
80104fe2:	e8 07 02 00 00       	call   801051ee <release>
  return -1;
80104fe7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104fec:	c9                   	leave  
80104fed:	c3                   	ret    

80104fee <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104fee:	55                   	push   %ebp
80104fef:	89 e5                	mov    %esp,%ebp
80104ff1:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104ff4:	c7 45 f0 54 0f 11 80 	movl   $0x80110f54,-0x10(%ebp)
80104ffb:	e9 db 00 00 00       	jmp    801050db <procdump+0xed>
    if(p->state == UNUSED)
80105000:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105003:	8b 40 0c             	mov    0xc(%eax),%eax
80105006:	85 c0                	test   %eax,%eax
80105008:	0f 84 c5 00 00 00    	je     801050d3 <procdump+0xe5>
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
8010500e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105011:	8b 40 0c             	mov    0xc(%eax),%eax
80105014:	83 f8 05             	cmp    $0x5,%eax
80105017:	77 23                	ja     8010503c <procdump+0x4e>
80105019:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010501c:	8b 40 0c             	mov    0xc(%eax),%eax
8010501f:	8b 04 85 0c c0 10 80 	mov    -0x7fef3ff4(,%eax,4),%eax
80105026:	85 c0                	test   %eax,%eax
80105028:	74 12                	je     8010503c <procdump+0x4e>
      state = states[p->state];
8010502a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010502d:	8b 40 0c             	mov    0xc(%eax),%eax
80105030:	8b 04 85 0c c0 10 80 	mov    -0x7fef3ff4(,%eax,4),%eax
80105037:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010503a:	eb 07                	jmp    80105043 <procdump+0x55>
    else
      state = "???";
8010503c:	c7 45 ec 32 8d 10 80 	movl   $0x80108d32,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80105043:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105046:	8d 50 6c             	lea    0x6c(%eax),%edx
80105049:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010504c:	8b 40 10             	mov    0x10(%eax),%eax
8010504f:	89 54 24 0c          	mov    %edx,0xc(%esp)
80105053:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105056:	89 54 24 08          	mov    %edx,0x8(%esp)
8010505a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010505e:	c7 04 24 36 8d 10 80 	movl   $0x80108d36,(%esp)
80105065:	e8 37 b3 ff ff       	call   801003a1 <cprintf>
    if(p->state == SLEEPING){
8010506a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010506d:	8b 40 0c             	mov    0xc(%eax),%eax
80105070:	83 f8 02             	cmp    $0x2,%eax
80105073:	75 50                	jne    801050c5 <procdump+0xd7>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80105075:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105078:	8b 40 1c             	mov    0x1c(%eax),%eax
8010507b:	8b 40 0c             	mov    0xc(%eax),%eax
8010507e:	83 c0 08             	add    $0x8,%eax
80105081:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80105084:	89 54 24 04          	mov    %edx,0x4(%esp)
80105088:	89 04 24             	mov    %eax,(%esp)
8010508b:	e8 ad 01 00 00       	call   8010523d <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
80105090:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80105097:	eb 1b                	jmp    801050b4 <procdump+0xc6>
        cprintf(" %p", pc[i]);
80105099:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010509c:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
801050a0:	89 44 24 04          	mov    %eax,0x4(%esp)
801050a4:	c7 04 24 3f 8d 10 80 	movl   $0x80108d3f,(%esp)
801050ab:	e8 f1 b2 ff ff       	call   801003a1 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
801050b0:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801050b4:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801050b8:	7f 0b                	jg     801050c5 <procdump+0xd7>
801050ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801050bd:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
801050c1:	85 c0                	test   %eax,%eax
801050c3:	75 d4                	jne    80105099 <procdump+0xab>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
801050c5:	c7 04 24 43 8d 10 80 	movl   $0x80108d43,(%esp)
801050cc:	e8 d0 b2 ff ff       	call   801003a1 <cprintf>
801050d1:	eb 01                	jmp    801050d4 <procdump+0xe6>
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED)
      continue;
801050d3:	90                   	nop
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801050d4:	81 45 f0 8c 00 00 00 	addl   $0x8c,-0x10(%ebp)
801050db:	81 7d f0 54 32 11 80 	cmpl   $0x80113254,-0x10(%ebp)
801050e2:	0f 82 18 ff ff ff    	jb     80105000 <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
801050e8:	c9                   	leave  
801050e9:	c3                   	ret    
	...

801050ec <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
801050ec:	55                   	push   %ebp
801050ed:	89 e5                	mov    %esp,%ebp
801050ef:	53                   	push   %ebx
801050f0:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801050f3:	9c                   	pushf  
801050f4:	5b                   	pop    %ebx
801050f5:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
801050f8:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801050fb:	83 c4 10             	add    $0x10,%esp
801050fe:	5b                   	pop    %ebx
801050ff:	5d                   	pop    %ebp
80105100:	c3                   	ret    

80105101 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80105101:	55                   	push   %ebp
80105102:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80105104:	fa                   	cli    
}
80105105:	5d                   	pop    %ebp
80105106:	c3                   	ret    

80105107 <sti>:

static inline void
sti(void)
{
80105107:	55                   	push   %ebp
80105108:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
8010510a:	fb                   	sti    
}
8010510b:	5d                   	pop    %ebp
8010510c:	c3                   	ret    

8010510d <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
8010510d:	55                   	push   %ebp
8010510e:	89 e5                	mov    %esp,%ebp
80105110:	53                   	push   %ebx
80105111:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
80105114:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80105117:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
8010511a:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
8010511d:	89 c3                	mov    %eax,%ebx
8010511f:	89 d8                	mov    %ebx,%eax
80105121:	f0 87 02             	lock xchg %eax,(%edx)
80105124:	89 c3                	mov    %eax,%ebx
80105126:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80105129:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
8010512c:	83 c4 10             	add    $0x10,%esp
8010512f:	5b                   	pop    %ebx
80105130:	5d                   	pop    %ebp
80105131:	c3                   	ret    

80105132 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80105132:	55                   	push   %ebp
80105133:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80105135:	8b 45 08             	mov    0x8(%ebp),%eax
80105138:	8b 55 0c             	mov    0xc(%ebp),%edx
8010513b:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
8010513e:	8b 45 08             	mov    0x8(%ebp),%eax
80105141:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80105147:	8b 45 08             	mov    0x8(%ebp),%eax
8010514a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80105151:	5d                   	pop    %ebp
80105152:	c3                   	ret    

80105153 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80105153:	55                   	push   %ebp
80105154:	89 e5                	mov    %esp,%ebp
80105156:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80105159:	e8 76 01 00 00       	call   801052d4 <pushcli>
  if(holding(lk))
8010515e:	8b 45 08             	mov    0x8(%ebp),%eax
80105161:	89 04 24             	mov    %eax,(%esp)
80105164:	e8 41 01 00 00       	call   801052aa <holding>
80105169:	85 c0                	test   %eax,%eax
8010516b:	74 45                	je     801051b2 <acquire+0x5f>
  {
    cprintf("lock = %s\n",lk->name);
8010516d:	8b 45 08             	mov    0x8(%ebp),%eax
80105170:	8b 40 04             	mov    0x4(%eax),%eax
80105173:	89 44 24 04          	mov    %eax,0x4(%esp)
80105177:	c7 04 24 6f 8d 10 80 	movl   $0x80108d6f,(%esp)
8010517e:	e8 1e b2 ff ff       	call   801003a1 <cprintf>
    if(proc)
80105183:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105189:	85 c0                	test   %eax,%eax
8010518b:	74 19                	je     801051a6 <acquire+0x53>
      cprintf("pid = %d\n",proc->pid);
8010518d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105193:	8b 40 10             	mov    0x10(%eax),%eax
80105196:	89 44 24 04          	mov    %eax,0x4(%esp)
8010519a:	c7 04 24 7a 8d 10 80 	movl   $0x80108d7a,(%esp)
801051a1:	e8 fb b1 ff ff       	call   801003a1 <cprintf>
    panic("acquire");
801051a6:	c7 04 24 84 8d 10 80 	movl   $0x80108d84,(%esp)
801051ad:	e8 8b b3 ff ff       	call   8010053d <panic>
  }

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
801051b2:	90                   	nop
801051b3:	8b 45 08             	mov    0x8(%ebp),%eax
801051b6:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801051bd:	00 
801051be:	89 04 24             	mov    %eax,(%esp)
801051c1:	e8 47 ff ff ff       	call   8010510d <xchg>
801051c6:	85 c0                	test   %eax,%eax
801051c8:	75 e9                	jne    801051b3 <acquire+0x60>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
801051ca:	8b 45 08             	mov    0x8(%ebp),%eax
801051cd:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801051d4:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
801051d7:	8b 45 08             	mov    0x8(%ebp),%eax
801051da:	83 c0 0c             	add    $0xc,%eax
801051dd:	89 44 24 04          	mov    %eax,0x4(%esp)
801051e1:	8d 45 08             	lea    0x8(%ebp),%eax
801051e4:	89 04 24             	mov    %eax,(%esp)
801051e7:	e8 51 00 00 00       	call   8010523d <getcallerpcs>
}
801051ec:	c9                   	leave  
801051ed:	c3                   	ret    

801051ee <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
801051ee:	55                   	push   %ebp
801051ef:	89 e5                	mov    %esp,%ebp
801051f1:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
801051f4:	8b 45 08             	mov    0x8(%ebp),%eax
801051f7:	89 04 24             	mov    %eax,(%esp)
801051fa:	e8 ab 00 00 00       	call   801052aa <holding>
801051ff:	85 c0                	test   %eax,%eax
80105201:	75 0c                	jne    8010520f <release+0x21>
    panic("release");
80105203:	c7 04 24 8c 8d 10 80 	movl   $0x80108d8c,(%esp)
8010520a:	e8 2e b3 ff ff       	call   8010053d <panic>

  lk->pcs[0] = 0;
8010520f:	8b 45 08             	mov    0x8(%ebp),%eax
80105212:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80105219:	8b 45 08             	mov    0x8(%ebp),%eax
8010521c:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
80105223:	8b 45 08             	mov    0x8(%ebp),%eax
80105226:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010522d:	00 
8010522e:	89 04 24             	mov    %eax,(%esp)
80105231:	e8 d7 fe ff ff       	call   8010510d <xchg>

  popcli();
80105236:	e8 e1 00 00 00       	call   8010531c <popcli>
}
8010523b:	c9                   	leave  
8010523c:	c3                   	ret    

8010523d <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
8010523d:	55                   	push   %ebp
8010523e:	89 e5                	mov    %esp,%ebp
80105240:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
80105243:	8b 45 08             	mov    0x8(%ebp),%eax
80105246:	83 e8 08             	sub    $0x8,%eax
80105249:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
8010524c:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80105253:	eb 32                	jmp    80105287 <getcallerpcs+0x4a>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105255:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80105259:	74 47                	je     801052a2 <getcallerpcs+0x65>
8010525b:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80105262:	76 3e                	jbe    801052a2 <getcallerpcs+0x65>
80105264:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80105268:	74 38                	je     801052a2 <getcallerpcs+0x65>
      break;
    pcs[i] = ebp[1];     // saved %eip
8010526a:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010526d:	c1 e0 02             	shl    $0x2,%eax
80105270:	03 45 0c             	add    0xc(%ebp),%eax
80105273:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105276:	8b 52 04             	mov    0x4(%edx),%edx
80105279:	89 10                	mov    %edx,(%eax)
    ebp = (uint*)ebp[0]; // saved %ebp
8010527b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010527e:	8b 00                	mov    (%eax),%eax
80105280:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80105283:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105287:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
8010528b:	7e c8                	jle    80105255 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
8010528d:	eb 13                	jmp    801052a2 <getcallerpcs+0x65>
    pcs[i] = 0;
8010528f:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105292:	c1 e0 02             	shl    $0x2,%eax
80105295:	03 45 0c             	add    0xc(%ebp),%eax
80105298:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
8010529e:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
801052a2:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
801052a6:	7e e7                	jle    8010528f <getcallerpcs+0x52>
    pcs[i] = 0;
}
801052a8:	c9                   	leave  
801052a9:	c3                   	ret    

801052aa <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
801052aa:	55                   	push   %ebp
801052ab:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
801052ad:	8b 45 08             	mov    0x8(%ebp),%eax
801052b0:	8b 00                	mov    (%eax),%eax
801052b2:	85 c0                	test   %eax,%eax
801052b4:	74 17                	je     801052cd <holding+0x23>
801052b6:	8b 45 08             	mov    0x8(%ebp),%eax
801052b9:	8b 50 08             	mov    0x8(%eax),%edx
801052bc:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801052c2:	39 c2                	cmp    %eax,%edx
801052c4:	75 07                	jne    801052cd <holding+0x23>
801052c6:	b8 01 00 00 00       	mov    $0x1,%eax
801052cb:	eb 05                	jmp    801052d2 <holding+0x28>
801052cd:	b8 00 00 00 00       	mov    $0x0,%eax
}
801052d2:	5d                   	pop    %ebp
801052d3:	c3                   	ret    

801052d4 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
801052d4:	55                   	push   %ebp
801052d5:	89 e5                	mov    %esp,%ebp
801052d7:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
801052da:	e8 0d fe ff ff       	call   801050ec <readeflags>
801052df:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
801052e2:	e8 1a fe ff ff       	call   80105101 <cli>
  if(cpu->ncli++ == 0)
801052e7:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801052ed:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
801052f3:	85 d2                	test   %edx,%edx
801052f5:	0f 94 c1             	sete   %cl
801052f8:	83 c2 01             	add    $0x1,%edx
801052fb:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105301:	84 c9                	test   %cl,%cl
80105303:	74 15                	je     8010531a <pushcli+0x46>
    cpu->intena = eflags & FL_IF;
80105305:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010530b:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010530e:	81 e2 00 02 00 00    	and    $0x200,%edx
80105314:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
8010531a:	c9                   	leave  
8010531b:	c3                   	ret    

8010531c <popcli>:

void
popcli(void)
{
8010531c:	55                   	push   %ebp
8010531d:	89 e5                	mov    %esp,%ebp
8010531f:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80105322:	e8 c5 fd ff ff       	call   801050ec <readeflags>
80105327:	25 00 02 00 00       	and    $0x200,%eax
8010532c:	85 c0                	test   %eax,%eax
8010532e:	74 0c                	je     8010533c <popcli+0x20>
    panic("popcli - interruptible");
80105330:	c7 04 24 94 8d 10 80 	movl   $0x80108d94,(%esp)
80105337:	e8 01 b2 ff ff       	call   8010053d <panic>
  if(--cpu->ncli < 0)
8010533c:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105342:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80105348:	83 ea 01             	sub    $0x1,%edx
8010534b:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105351:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105357:	85 c0                	test   %eax,%eax
80105359:	79 0c                	jns    80105367 <popcli+0x4b>
    panic("popcli");
8010535b:	c7 04 24 ab 8d 10 80 	movl   $0x80108dab,(%esp)
80105362:	e8 d6 b1 ff ff       	call   8010053d <panic>
  if(cpu->ncli == 0 && cpu->intena)
80105367:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010536d:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105373:	85 c0                	test   %eax,%eax
80105375:	75 15                	jne    8010538c <popcli+0x70>
80105377:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010537d:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80105383:	85 c0                	test   %eax,%eax
80105385:	74 05                	je     8010538c <popcli+0x70>
    sti();
80105387:	e8 7b fd ff ff       	call   80105107 <sti>
}
8010538c:	c9                   	leave  
8010538d:	c3                   	ret    
	...

80105390 <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
80105390:	55                   	push   %ebp
80105391:	89 e5                	mov    %esp,%ebp
80105393:	57                   	push   %edi
80105394:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80105395:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105398:	8b 55 10             	mov    0x10(%ebp),%edx
8010539b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010539e:	89 cb                	mov    %ecx,%ebx
801053a0:	89 df                	mov    %ebx,%edi
801053a2:	89 d1                	mov    %edx,%ecx
801053a4:	fc                   	cld    
801053a5:	f3 aa                	rep stos %al,%es:(%edi)
801053a7:	89 ca                	mov    %ecx,%edx
801053a9:	89 fb                	mov    %edi,%ebx
801053ab:	89 5d 08             	mov    %ebx,0x8(%ebp)
801053ae:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
801053b1:	5b                   	pop    %ebx
801053b2:	5f                   	pop    %edi
801053b3:	5d                   	pop    %ebp
801053b4:	c3                   	ret    

801053b5 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
801053b5:	55                   	push   %ebp
801053b6:	89 e5                	mov    %esp,%ebp
801053b8:	57                   	push   %edi
801053b9:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
801053ba:	8b 4d 08             	mov    0x8(%ebp),%ecx
801053bd:	8b 55 10             	mov    0x10(%ebp),%edx
801053c0:	8b 45 0c             	mov    0xc(%ebp),%eax
801053c3:	89 cb                	mov    %ecx,%ebx
801053c5:	89 df                	mov    %ebx,%edi
801053c7:	89 d1                	mov    %edx,%ecx
801053c9:	fc                   	cld    
801053ca:	f3 ab                	rep stos %eax,%es:(%edi)
801053cc:	89 ca                	mov    %ecx,%edx
801053ce:	89 fb                	mov    %edi,%ebx
801053d0:	89 5d 08             	mov    %ebx,0x8(%ebp)
801053d3:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
801053d6:	5b                   	pop    %ebx
801053d7:	5f                   	pop    %edi
801053d8:	5d                   	pop    %ebp
801053d9:	c3                   	ret    

801053da <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
801053da:	55                   	push   %ebp
801053db:	89 e5                	mov    %esp,%ebp
801053dd:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
801053e0:	8b 45 08             	mov    0x8(%ebp),%eax
801053e3:	83 e0 03             	and    $0x3,%eax
801053e6:	85 c0                	test   %eax,%eax
801053e8:	75 49                	jne    80105433 <memset+0x59>
801053ea:	8b 45 10             	mov    0x10(%ebp),%eax
801053ed:	83 e0 03             	and    $0x3,%eax
801053f0:	85 c0                	test   %eax,%eax
801053f2:	75 3f                	jne    80105433 <memset+0x59>
    c &= 0xFF;
801053f4:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
801053fb:	8b 45 10             	mov    0x10(%ebp),%eax
801053fe:	c1 e8 02             	shr    $0x2,%eax
80105401:	89 c2                	mov    %eax,%edx
80105403:	8b 45 0c             	mov    0xc(%ebp),%eax
80105406:	89 c1                	mov    %eax,%ecx
80105408:	c1 e1 18             	shl    $0x18,%ecx
8010540b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010540e:	c1 e0 10             	shl    $0x10,%eax
80105411:	09 c1                	or     %eax,%ecx
80105413:	8b 45 0c             	mov    0xc(%ebp),%eax
80105416:	c1 e0 08             	shl    $0x8,%eax
80105419:	09 c8                	or     %ecx,%eax
8010541b:	0b 45 0c             	or     0xc(%ebp),%eax
8010541e:	89 54 24 08          	mov    %edx,0x8(%esp)
80105422:	89 44 24 04          	mov    %eax,0x4(%esp)
80105426:	8b 45 08             	mov    0x8(%ebp),%eax
80105429:	89 04 24             	mov    %eax,(%esp)
8010542c:	e8 84 ff ff ff       	call   801053b5 <stosl>
80105431:	eb 19                	jmp    8010544c <memset+0x72>
  } else
    stosb(dst, c, n);
80105433:	8b 45 10             	mov    0x10(%ebp),%eax
80105436:	89 44 24 08          	mov    %eax,0x8(%esp)
8010543a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010543d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105441:	8b 45 08             	mov    0x8(%ebp),%eax
80105444:	89 04 24             	mov    %eax,(%esp)
80105447:	e8 44 ff ff ff       	call   80105390 <stosb>
  return dst;
8010544c:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010544f:	c9                   	leave  
80105450:	c3                   	ret    

80105451 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
80105451:	55                   	push   %ebp
80105452:	89 e5                	mov    %esp,%ebp
80105454:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80105457:	8b 45 08             	mov    0x8(%ebp),%eax
8010545a:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
8010545d:	8b 45 0c             	mov    0xc(%ebp),%eax
80105460:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
80105463:	eb 32                	jmp    80105497 <memcmp+0x46>
    if(*s1 != *s2)
80105465:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105468:	0f b6 10             	movzbl (%eax),%edx
8010546b:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010546e:	0f b6 00             	movzbl (%eax),%eax
80105471:	38 c2                	cmp    %al,%dl
80105473:	74 1a                	je     8010548f <memcmp+0x3e>
      return *s1 - *s2;
80105475:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105478:	0f b6 00             	movzbl (%eax),%eax
8010547b:	0f b6 d0             	movzbl %al,%edx
8010547e:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105481:	0f b6 00             	movzbl (%eax),%eax
80105484:	0f b6 c0             	movzbl %al,%eax
80105487:	89 d1                	mov    %edx,%ecx
80105489:	29 c1                	sub    %eax,%ecx
8010548b:	89 c8                	mov    %ecx,%eax
8010548d:	eb 1c                	jmp    801054ab <memcmp+0x5a>
    s1++, s2++;
8010548f:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105493:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80105497:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010549b:	0f 95 c0             	setne  %al
8010549e:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801054a2:	84 c0                	test   %al,%al
801054a4:	75 bf                	jne    80105465 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
801054a6:	b8 00 00 00 00       	mov    $0x0,%eax
}
801054ab:	c9                   	leave  
801054ac:	c3                   	ret    

801054ad <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
801054ad:	55                   	push   %ebp
801054ae:	89 e5                	mov    %esp,%ebp
801054b0:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
801054b3:	8b 45 0c             	mov    0xc(%ebp),%eax
801054b6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
801054b9:	8b 45 08             	mov    0x8(%ebp),%eax
801054bc:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
801054bf:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054c2:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801054c5:	73 54                	jae    8010551b <memmove+0x6e>
801054c7:	8b 45 10             	mov    0x10(%ebp),%eax
801054ca:	8b 55 fc             	mov    -0x4(%ebp),%edx
801054cd:	01 d0                	add    %edx,%eax
801054cf:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801054d2:	76 47                	jbe    8010551b <memmove+0x6e>
    s += n;
801054d4:	8b 45 10             	mov    0x10(%ebp),%eax
801054d7:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
801054da:	8b 45 10             	mov    0x10(%ebp),%eax
801054dd:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
801054e0:	eb 13                	jmp    801054f5 <memmove+0x48>
      *--d = *--s;
801054e2:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
801054e6:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
801054ea:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054ed:	0f b6 10             	movzbl (%eax),%edx
801054f0:	8b 45 f8             	mov    -0x8(%ebp),%eax
801054f3:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
801054f5:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801054f9:	0f 95 c0             	setne  %al
801054fc:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105500:	84 c0                	test   %al,%al
80105502:	75 de                	jne    801054e2 <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80105504:	eb 25                	jmp    8010552b <memmove+0x7e>
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
      *d++ = *s++;
80105506:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105509:	0f b6 10             	movzbl (%eax),%edx
8010550c:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010550f:	88 10                	mov    %dl,(%eax)
80105511:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105515:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105519:	eb 01                	jmp    8010551c <memmove+0x6f>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
8010551b:	90                   	nop
8010551c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105520:	0f 95 c0             	setne  %al
80105523:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105527:	84 c0                	test   %al,%al
80105529:	75 db                	jne    80105506 <memmove+0x59>
      *d++ = *s++;

  return dst;
8010552b:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010552e:	c9                   	leave  
8010552f:	c3                   	ret    

80105530 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80105530:	55                   	push   %ebp
80105531:	89 e5                	mov    %esp,%ebp
80105533:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
80105536:	8b 45 10             	mov    0x10(%ebp),%eax
80105539:	89 44 24 08          	mov    %eax,0x8(%esp)
8010553d:	8b 45 0c             	mov    0xc(%ebp),%eax
80105540:	89 44 24 04          	mov    %eax,0x4(%esp)
80105544:	8b 45 08             	mov    0x8(%ebp),%eax
80105547:	89 04 24             	mov    %eax,(%esp)
8010554a:	e8 5e ff ff ff       	call   801054ad <memmove>
}
8010554f:	c9                   	leave  
80105550:	c3                   	ret    

80105551 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
80105551:	55                   	push   %ebp
80105552:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80105554:	eb 0c                	jmp    80105562 <strncmp+0x11>
    n--, p++, q++;
80105556:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
8010555a:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010555e:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
80105562:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105566:	74 1a                	je     80105582 <strncmp+0x31>
80105568:	8b 45 08             	mov    0x8(%ebp),%eax
8010556b:	0f b6 00             	movzbl (%eax),%eax
8010556e:	84 c0                	test   %al,%al
80105570:	74 10                	je     80105582 <strncmp+0x31>
80105572:	8b 45 08             	mov    0x8(%ebp),%eax
80105575:	0f b6 10             	movzbl (%eax),%edx
80105578:	8b 45 0c             	mov    0xc(%ebp),%eax
8010557b:	0f b6 00             	movzbl (%eax),%eax
8010557e:	38 c2                	cmp    %al,%dl
80105580:	74 d4                	je     80105556 <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
80105582:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105586:	75 07                	jne    8010558f <strncmp+0x3e>
    return 0;
80105588:	b8 00 00 00 00       	mov    $0x0,%eax
8010558d:	eb 18                	jmp    801055a7 <strncmp+0x56>
  return (uchar)*p - (uchar)*q;
8010558f:	8b 45 08             	mov    0x8(%ebp),%eax
80105592:	0f b6 00             	movzbl (%eax),%eax
80105595:	0f b6 d0             	movzbl %al,%edx
80105598:	8b 45 0c             	mov    0xc(%ebp),%eax
8010559b:	0f b6 00             	movzbl (%eax),%eax
8010559e:	0f b6 c0             	movzbl %al,%eax
801055a1:	89 d1                	mov    %edx,%ecx
801055a3:	29 c1                	sub    %eax,%ecx
801055a5:	89 c8                	mov    %ecx,%eax
}
801055a7:	5d                   	pop    %ebp
801055a8:	c3                   	ret    

801055a9 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
801055a9:	55                   	push   %ebp
801055aa:	89 e5                	mov    %esp,%ebp
801055ac:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
801055af:	8b 45 08             	mov    0x8(%ebp),%eax
801055b2:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
801055b5:	90                   	nop
801055b6:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801055ba:	0f 9f c0             	setg   %al
801055bd:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801055c1:	84 c0                	test   %al,%al
801055c3:	74 30                	je     801055f5 <strncpy+0x4c>
801055c5:	8b 45 0c             	mov    0xc(%ebp),%eax
801055c8:	0f b6 10             	movzbl (%eax),%edx
801055cb:	8b 45 08             	mov    0x8(%ebp),%eax
801055ce:	88 10                	mov    %dl,(%eax)
801055d0:	8b 45 08             	mov    0x8(%ebp),%eax
801055d3:	0f b6 00             	movzbl (%eax),%eax
801055d6:	84 c0                	test   %al,%al
801055d8:	0f 95 c0             	setne  %al
801055db:	83 45 08 01          	addl   $0x1,0x8(%ebp)
801055df:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
801055e3:	84 c0                	test   %al,%al
801055e5:	75 cf                	jne    801055b6 <strncpy+0xd>
    ;
  while(n-- > 0)
801055e7:	eb 0c                	jmp    801055f5 <strncpy+0x4c>
    *s++ = 0;
801055e9:	8b 45 08             	mov    0x8(%ebp),%eax
801055ec:	c6 00 00             	movb   $0x0,(%eax)
801055ef:	83 45 08 01          	addl   $0x1,0x8(%ebp)
801055f3:	eb 01                	jmp    801055f6 <strncpy+0x4d>
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
801055f5:	90                   	nop
801055f6:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801055fa:	0f 9f c0             	setg   %al
801055fd:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105601:	84 c0                	test   %al,%al
80105603:	75 e4                	jne    801055e9 <strncpy+0x40>
    *s++ = 0;
  return os;
80105605:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105608:	c9                   	leave  
80105609:	c3                   	ret    

8010560a <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
8010560a:	55                   	push   %ebp
8010560b:	89 e5                	mov    %esp,%ebp
8010560d:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80105610:	8b 45 08             	mov    0x8(%ebp),%eax
80105613:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
80105616:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010561a:	7f 05                	jg     80105621 <safestrcpy+0x17>
    return os;
8010561c:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010561f:	eb 35                	jmp    80105656 <safestrcpy+0x4c>
  while(--n > 0 && (*s++ = *t++) != 0)
80105621:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105625:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105629:	7e 22                	jle    8010564d <safestrcpy+0x43>
8010562b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010562e:	0f b6 10             	movzbl (%eax),%edx
80105631:	8b 45 08             	mov    0x8(%ebp),%eax
80105634:	88 10                	mov    %dl,(%eax)
80105636:	8b 45 08             	mov    0x8(%ebp),%eax
80105639:	0f b6 00             	movzbl (%eax),%eax
8010563c:	84 c0                	test   %al,%al
8010563e:	0f 95 c0             	setne  %al
80105641:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80105645:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80105649:	84 c0                	test   %al,%al
8010564b:	75 d4                	jne    80105621 <safestrcpy+0x17>
    ;
  *s = 0;
8010564d:	8b 45 08             	mov    0x8(%ebp),%eax
80105650:	c6 00 00             	movb   $0x0,(%eax)
  return os;
80105653:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105656:	c9                   	leave  
80105657:	c3                   	ret    

80105658 <strlen>:

int
strlen(const char *s)
{
80105658:	55                   	push   %ebp
80105659:	89 e5                	mov    %esp,%ebp
8010565b:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
8010565e:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80105665:	eb 04                	jmp    8010566b <strlen+0x13>
80105667:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010566b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010566e:	03 45 08             	add    0x8(%ebp),%eax
80105671:	0f b6 00             	movzbl (%eax),%eax
80105674:	84 c0                	test   %al,%al
80105676:	75 ef                	jne    80105667 <strlen+0xf>
    ;
  return n;
80105678:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010567b:	c9                   	leave  
8010567c:	c3                   	ret    
8010567d:	00 00                	add    %al,(%eax)
	...

80105680 <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
80105680:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80105684:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80105688:	55                   	push   %ebp
  pushl %ebx
80105689:	53                   	push   %ebx
  pushl %esi
8010568a:	56                   	push   %esi
  pushl %edi
8010568b:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
8010568c:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
8010568e:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
80105690:	5f                   	pop    %edi
  popl %esi
80105691:	5e                   	pop    %esi
  popl %ebx
80105692:	5b                   	pop    %ebx
  popl %ebp
80105693:	5d                   	pop    %ebp
  ret
80105694:	c3                   	ret    
80105695:	00 00                	add    %al,(%eax)
	...

80105698 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from process p.
int
fetchint(struct proc *p, uint addr, int *ip)
{
80105698:	55                   	push   %ebp
80105699:	89 e5                	mov    %esp,%ebp
  if(addr >= p->sz || addr+4 > p->sz)
8010569b:	8b 45 08             	mov    0x8(%ebp),%eax
8010569e:	8b 00                	mov    (%eax),%eax
801056a0:	3b 45 0c             	cmp    0xc(%ebp),%eax
801056a3:	76 0f                	jbe    801056b4 <fetchint+0x1c>
801056a5:	8b 45 0c             	mov    0xc(%ebp),%eax
801056a8:	8d 50 04             	lea    0x4(%eax),%edx
801056ab:	8b 45 08             	mov    0x8(%ebp),%eax
801056ae:	8b 00                	mov    (%eax),%eax
801056b0:	39 c2                	cmp    %eax,%edx
801056b2:	76 07                	jbe    801056bb <fetchint+0x23>
    return -1;
801056b4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056b9:	eb 0f                	jmp    801056ca <fetchint+0x32>
  *ip = *(int*)(addr);
801056bb:	8b 45 0c             	mov    0xc(%ebp),%eax
801056be:	8b 10                	mov    (%eax),%edx
801056c0:	8b 45 10             	mov    0x10(%ebp),%eax
801056c3:	89 10                	mov    %edx,(%eax)
  return 0;
801056c5:	b8 00 00 00 00       	mov    $0x0,%eax
}
801056ca:	5d                   	pop    %ebp
801056cb:	c3                   	ret    

801056cc <fetchstr>:
// Fetch the nul-terminated string at addr from process p.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(struct proc *p, uint addr, char **pp)
{
801056cc:	55                   	push   %ebp
801056cd:	89 e5                	mov    %esp,%ebp
801056cf:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= p->sz)
801056d2:	8b 45 08             	mov    0x8(%ebp),%eax
801056d5:	8b 00                	mov    (%eax),%eax
801056d7:	3b 45 0c             	cmp    0xc(%ebp),%eax
801056da:	77 07                	ja     801056e3 <fetchstr+0x17>
    return -1;
801056dc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056e1:	eb 45                	jmp    80105728 <fetchstr+0x5c>
  *pp = (char*)addr;
801056e3:	8b 55 0c             	mov    0xc(%ebp),%edx
801056e6:	8b 45 10             	mov    0x10(%ebp),%eax
801056e9:	89 10                	mov    %edx,(%eax)
  ep = (char*)p->sz;
801056eb:	8b 45 08             	mov    0x8(%ebp),%eax
801056ee:	8b 00                	mov    (%eax),%eax
801056f0:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
801056f3:	8b 45 10             	mov    0x10(%ebp),%eax
801056f6:	8b 00                	mov    (%eax),%eax
801056f8:	89 45 fc             	mov    %eax,-0x4(%ebp)
801056fb:	eb 1e                	jmp    8010571b <fetchstr+0x4f>
    if(*s == 0)
801056fd:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105700:	0f b6 00             	movzbl (%eax),%eax
80105703:	84 c0                	test   %al,%al
80105705:	75 10                	jne    80105717 <fetchstr+0x4b>
      return s - *pp;
80105707:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010570a:	8b 45 10             	mov    0x10(%ebp),%eax
8010570d:	8b 00                	mov    (%eax),%eax
8010570f:	89 d1                	mov    %edx,%ecx
80105711:	29 c1                	sub    %eax,%ecx
80105713:	89 c8                	mov    %ecx,%eax
80105715:	eb 11                	jmp    80105728 <fetchstr+0x5c>

  if(addr >= p->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)p->sz;
  for(s = *pp; s < ep; s++)
80105717:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010571b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010571e:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105721:	72 da                	jb     801056fd <fetchstr+0x31>
    if(*s == 0)
      return s - *pp;
  return -1;
80105723:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105728:	c9                   	leave  
80105729:	c3                   	ret    

8010572a <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
8010572a:	55                   	push   %ebp
8010572b:	89 e5                	mov    %esp,%ebp
8010572d:	83 ec 0c             	sub    $0xc,%esp
  return fetchint(proc, proc->tf->esp + 4 + 4*n, ip);
80105730:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105736:	8b 40 18             	mov    0x18(%eax),%eax
80105739:	8b 50 44             	mov    0x44(%eax),%edx
8010573c:	8b 45 08             	mov    0x8(%ebp),%eax
8010573f:	c1 e0 02             	shl    $0x2,%eax
80105742:	01 d0                	add    %edx,%eax
80105744:	8d 48 04             	lea    0x4(%eax),%ecx
80105747:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010574d:	8b 55 0c             	mov    0xc(%ebp),%edx
80105750:	89 54 24 08          	mov    %edx,0x8(%esp)
80105754:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80105758:	89 04 24             	mov    %eax,(%esp)
8010575b:	e8 38 ff ff ff       	call   80105698 <fetchint>
}
80105760:	c9                   	leave  
80105761:	c3                   	ret    

80105762 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
80105762:	55                   	push   %ebp
80105763:	89 e5                	mov    %esp,%ebp
80105765:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
80105768:	8d 45 fc             	lea    -0x4(%ebp),%eax
8010576b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010576f:	8b 45 08             	mov    0x8(%ebp),%eax
80105772:	89 04 24             	mov    %eax,(%esp)
80105775:	e8 b0 ff ff ff       	call   8010572a <argint>
8010577a:	85 c0                	test   %eax,%eax
8010577c:	79 07                	jns    80105785 <argptr+0x23>
    return -1;
8010577e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105783:	eb 3d                	jmp    801057c2 <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
80105785:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105788:	89 c2                	mov    %eax,%edx
8010578a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105790:	8b 00                	mov    (%eax),%eax
80105792:	39 c2                	cmp    %eax,%edx
80105794:	73 16                	jae    801057ac <argptr+0x4a>
80105796:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105799:	89 c2                	mov    %eax,%edx
8010579b:	8b 45 10             	mov    0x10(%ebp),%eax
8010579e:	01 c2                	add    %eax,%edx
801057a0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801057a6:	8b 00                	mov    (%eax),%eax
801057a8:	39 c2                	cmp    %eax,%edx
801057aa:	76 07                	jbe    801057b3 <argptr+0x51>
    return -1;
801057ac:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057b1:	eb 0f                	jmp    801057c2 <argptr+0x60>
  *pp = (char*)i;
801057b3:	8b 45 fc             	mov    -0x4(%ebp),%eax
801057b6:	89 c2                	mov    %eax,%edx
801057b8:	8b 45 0c             	mov    0xc(%ebp),%eax
801057bb:	89 10                	mov    %edx,(%eax)
  return 0;
801057bd:	b8 00 00 00 00       	mov    $0x0,%eax
}
801057c2:	c9                   	leave  
801057c3:	c3                   	ret    

801057c4 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
801057c4:	55                   	push   %ebp
801057c5:	89 e5                	mov    %esp,%ebp
801057c7:	83 ec 1c             	sub    $0x1c,%esp
  int addr;
  if(argint(n, &addr) < 0)
801057ca:	8d 45 fc             	lea    -0x4(%ebp),%eax
801057cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801057d1:	8b 45 08             	mov    0x8(%ebp),%eax
801057d4:	89 04 24             	mov    %eax,(%esp)
801057d7:	e8 4e ff ff ff       	call   8010572a <argint>
801057dc:	85 c0                	test   %eax,%eax
801057de:	79 07                	jns    801057e7 <argstr+0x23>
    return -1;
801057e0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057e5:	eb 1e                	jmp    80105805 <argstr+0x41>
  return fetchstr(proc, addr, pp);
801057e7:	8b 45 fc             	mov    -0x4(%ebp),%eax
801057ea:	89 c2                	mov    %eax,%edx
801057ec:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801057f2:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801057f5:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801057f9:	89 54 24 04          	mov    %edx,0x4(%esp)
801057fd:	89 04 24             	mov    %eax,(%esp)
80105800:	e8 c7 fe ff ff       	call   801056cc <fetchstr>
}
80105805:	c9                   	leave  
80105806:	c3                   	ret    

80105807 <syscall>:
[SYS_disableSwapping]	sys_disableSwapping,
};

void
syscall(void)
{
80105807:	55                   	push   %ebp
80105808:	89 e5                	mov    %esp,%ebp
8010580a:	53                   	push   %ebx
8010580b:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
8010580e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105814:	8b 40 18             	mov    0x18(%eax),%eax
80105817:	8b 40 1c             	mov    0x1c(%eax),%eax
8010581a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num >= 0 && num < SYS_open && syscalls[num]) {
8010581d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105821:	78 2e                	js     80105851 <syscall+0x4a>
80105823:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
80105827:	7f 28                	jg     80105851 <syscall+0x4a>
80105829:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010582c:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
80105833:	85 c0                	test   %eax,%eax
80105835:	74 1a                	je     80105851 <syscall+0x4a>
    proc->tf->eax = syscalls[num]();
80105837:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010583d:	8b 58 18             	mov    0x18(%eax),%ebx
80105840:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105843:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
8010584a:	ff d0                	call   *%eax
8010584c:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010584f:	eb 73                	jmp    801058c4 <syscall+0xbd>
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
80105851:	83 7d f4 0e          	cmpl   $0xe,-0xc(%ebp)
80105855:	7e 30                	jle    80105887 <syscall+0x80>
80105857:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010585a:	83 f8 17             	cmp    $0x17,%eax
8010585d:	77 28                	ja     80105887 <syscall+0x80>
8010585f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105862:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
80105869:	85 c0                	test   %eax,%eax
8010586b:	74 1a                	je     80105887 <syscall+0x80>
    proc->tf->eax = syscalls[num]();
8010586d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105873:	8b 58 18             	mov    0x18(%eax),%ebx
80105876:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105879:	8b 04 85 40 c0 10 80 	mov    -0x7fef3fc0(,%eax,4),%eax
80105880:	ff d0                	call   *%eax
80105882:	89 43 1c             	mov    %eax,0x1c(%ebx)
80105885:	eb 3d                	jmp    801058c4 <syscall+0xbd>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
80105887:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010588d:	8d 48 6c             	lea    0x6c(%eax),%ecx
80105890:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
  if(num >= 0 && num < SYS_open && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else if (num >= SYS_open && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
80105896:	8b 40 10             	mov    0x10(%eax),%eax
80105899:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010589c:	89 54 24 0c          	mov    %edx,0xc(%esp)
801058a0:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801058a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801058a8:	c7 04 24 b2 8d 10 80 	movl   $0x80108db2,(%esp)
801058af:	e8 ed aa ff ff       	call   801003a1 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
801058b4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801058ba:	8b 40 18             	mov    0x18(%eax),%eax
801058bd:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
801058c4:	83 c4 24             	add    $0x24,%esp
801058c7:	5b                   	pop    %ebx
801058c8:	5d                   	pop    %ebp
801058c9:	c3                   	ret    
	...

801058cc <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
801058cc:	55                   	push   %ebp
801058cd:	89 e5                	mov    %esp,%ebp
801058cf:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
801058d2:	8d 45 f0             	lea    -0x10(%ebp),%eax
801058d5:	89 44 24 04          	mov    %eax,0x4(%esp)
801058d9:	8b 45 08             	mov    0x8(%ebp),%eax
801058dc:	89 04 24             	mov    %eax,(%esp)
801058df:	e8 46 fe ff ff       	call   8010572a <argint>
801058e4:	85 c0                	test   %eax,%eax
801058e6:	79 07                	jns    801058ef <argfd+0x23>
    return -1;
801058e8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801058ed:	eb 50                	jmp    8010593f <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
801058ef:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058f2:	85 c0                	test   %eax,%eax
801058f4:	78 21                	js     80105917 <argfd+0x4b>
801058f6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058f9:	83 f8 0f             	cmp    $0xf,%eax
801058fc:	7f 19                	jg     80105917 <argfd+0x4b>
801058fe:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105904:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105907:	83 c2 08             	add    $0x8,%edx
8010590a:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
8010590e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105911:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105915:	75 07                	jne    8010591e <argfd+0x52>
    return -1;
80105917:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010591c:	eb 21                	jmp    8010593f <argfd+0x73>
  if(pfd)
8010591e:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80105922:	74 08                	je     8010592c <argfd+0x60>
    *pfd = fd;
80105924:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105927:	8b 45 0c             	mov    0xc(%ebp),%eax
8010592a:	89 10                	mov    %edx,(%eax)
  if(pf)
8010592c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105930:	74 08                	je     8010593a <argfd+0x6e>
    *pf = f;
80105932:	8b 45 10             	mov    0x10(%ebp),%eax
80105935:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105938:	89 10                	mov    %edx,(%eax)
  return 0;
8010593a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010593f:	c9                   	leave  
80105940:	c3                   	ret    

80105941 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
80105941:	55                   	push   %ebp
80105942:	89 e5                	mov    %esp,%ebp
80105944:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105947:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010594e:	eb 30                	jmp    80105980 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
80105950:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105956:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105959:	83 c2 08             	add    $0x8,%edx
8010595c:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105960:	85 c0                	test   %eax,%eax
80105962:	75 18                	jne    8010597c <fdalloc+0x3b>
      proc->ofile[fd] = f;
80105964:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010596a:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010596d:	8d 4a 08             	lea    0x8(%edx),%ecx
80105970:	8b 55 08             	mov    0x8(%ebp),%edx
80105973:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
80105977:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010597a:	eb 0f                	jmp    8010598b <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
8010597c:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105980:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
80105984:	7e ca                	jle    80105950 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
80105986:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010598b:	c9                   	leave  
8010598c:	c3                   	ret    

8010598d <sys_dup>:

int
sys_dup(void)
{
8010598d:	55                   	push   %ebp
8010598e:	89 e5                	mov    %esp,%ebp
80105990:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
80105993:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105996:	89 44 24 08          	mov    %eax,0x8(%esp)
8010599a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801059a1:	00 
801059a2:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801059a9:	e8 1e ff ff ff       	call   801058cc <argfd>
801059ae:	85 c0                	test   %eax,%eax
801059b0:	79 07                	jns    801059b9 <sys_dup+0x2c>
    return -1;
801059b2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059b7:	eb 29                	jmp    801059e2 <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
801059b9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059bc:	89 04 24             	mov    %eax,(%esp)
801059bf:	e8 7d ff ff ff       	call   80105941 <fdalloc>
801059c4:	89 45 f4             	mov    %eax,-0xc(%ebp)
801059c7:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801059cb:	79 07                	jns    801059d4 <sys_dup+0x47>
    return -1;
801059cd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059d2:	eb 0e                	jmp    801059e2 <sys_dup+0x55>
  filedup(f);
801059d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059d7:	89 04 24             	mov    %eax,(%esp)
801059da:	e8 9d b5 ff ff       	call   80100f7c <filedup>
  return fd;
801059df:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801059e2:	c9                   	leave  
801059e3:	c3                   	ret    

801059e4 <sys_read>:

int
sys_read(void)
{
801059e4:	55                   	push   %ebp
801059e5:	89 e5                	mov    %esp,%ebp
801059e7:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801059ea:	8d 45 f4             	lea    -0xc(%ebp),%eax
801059ed:	89 44 24 08          	mov    %eax,0x8(%esp)
801059f1:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801059f8:	00 
801059f9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105a00:	e8 c7 fe ff ff       	call   801058cc <argfd>
80105a05:	85 c0                	test   %eax,%eax
80105a07:	78 35                	js     80105a3e <sys_read+0x5a>
80105a09:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105a0c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a10:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105a17:	e8 0e fd ff ff       	call   8010572a <argint>
80105a1c:	85 c0                	test   %eax,%eax
80105a1e:	78 1e                	js     80105a3e <sys_read+0x5a>
80105a20:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a23:	89 44 24 08          	mov    %eax,0x8(%esp)
80105a27:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105a2a:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a2e:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105a35:	e8 28 fd ff ff       	call   80105762 <argptr>
80105a3a:	85 c0                	test   %eax,%eax
80105a3c:	79 07                	jns    80105a45 <sys_read+0x61>
    return -1;
80105a3e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a43:	eb 19                	jmp    80105a5e <sys_read+0x7a>
  return fileread(f, p, n);
80105a45:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80105a48:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105a4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a4e:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105a52:	89 54 24 04          	mov    %edx,0x4(%esp)
80105a56:	89 04 24             	mov    %eax,(%esp)
80105a59:	e8 8b b6 ff ff       	call   801010e9 <fileread>
}
80105a5e:	c9                   	leave  
80105a5f:	c3                   	ret    

80105a60 <sys_write>:

int
sys_write(void)
{
80105a60:	55                   	push   %ebp
80105a61:	89 e5                	mov    %esp,%ebp
80105a63:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105a66:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105a69:	89 44 24 08          	mov    %eax,0x8(%esp)
80105a6d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105a74:	00 
80105a75:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105a7c:	e8 4b fe ff ff       	call   801058cc <argfd>
80105a81:	85 c0                	test   %eax,%eax
80105a83:	78 35                	js     80105aba <sys_write+0x5a>
80105a85:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105a88:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a8c:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105a93:	e8 92 fc ff ff       	call   8010572a <argint>
80105a98:	85 c0                	test   %eax,%eax
80105a9a:	78 1e                	js     80105aba <sys_write+0x5a>
80105a9c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a9f:	89 44 24 08          	mov    %eax,0x8(%esp)
80105aa3:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105aa6:	89 44 24 04          	mov    %eax,0x4(%esp)
80105aaa:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105ab1:	e8 ac fc ff ff       	call   80105762 <argptr>
80105ab6:	85 c0                	test   %eax,%eax
80105ab8:	79 07                	jns    80105ac1 <sys_write+0x61>
    return -1;
80105aba:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105abf:	eb 19                	jmp    80105ada <sys_write+0x7a>
  return filewrite(f, p, n);
80105ac1:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80105ac4:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105ac7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105aca:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105ace:	89 54 24 04          	mov    %edx,0x4(%esp)
80105ad2:	89 04 24             	mov    %eax,(%esp)
80105ad5:	e8 cb b6 ff ff       	call   801011a5 <filewrite>
}
80105ada:	c9                   	leave  
80105adb:	c3                   	ret    

80105adc <sys_close>:

int
sys_close(void)
{
80105adc:	55                   	push   %ebp
80105add:	89 e5                	mov    %esp,%ebp
80105adf:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
80105ae2:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105ae5:	89 44 24 08          	mov    %eax,0x8(%esp)
80105ae9:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105aec:	89 44 24 04          	mov    %eax,0x4(%esp)
80105af0:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105af7:	e8 d0 fd ff ff       	call   801058cc <argfd>
80105afc:	85 c0                	test   %eax,%eax
80105afe:	79 07                	jns    80105b07 <sys_close+0x2b>
    return -1;
80105b00:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105b05:	eb 24                	jmp    80105b2b <sys_close+0x4f>
  proc->ofile[fd] = 0;
80105b07:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105b0d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105b10:	83 c2 08             	add    $0x8,%edx
80105b13:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80105b1a:	00 
  fileclose(f);
80105b1b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b1e:	89 04 24             	mov    %eax,(%esp)
80105b21:	e8 9e b4 ff ff       	call   80100fc4 <fileclose>
  return 0;
80105b26:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105b2b:	c9                   	leave  
80105b2c:	c3                   	ret    

80105b2d <sys_fstat>:

int
sys_fstat(void)
{
80105b2d:	55                   	push   %ebp
80105b2e:	89 e5                	mov    %esp,%ebp
80105b30:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80105b33:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105b36:	89 44 24 08          	mov    %eax,0x8(%esp)
80105b3a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105b41:	00 
80105b42:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105b49:	e8 7e fd ff ff       	call   801058cc <argfd>
80105b4e:	85 c0                	test   %eax,%eax
80105b50:	78 1f                	js     80105b71 <sys_fstat+0x44>
80105b52:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80105b59:	00 
80105b5a:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105b5d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b61:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105b68:	e8 f5 fb ff ff       	call   80105762 <argptr>
80105b6d:	85 c0                	test   %eax,%eax
80105b6f:	79 07                	jns    80105b78 <sys_fstat+0x4b>
    return -1;
80105b71:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105b76:	eb 12                	jmp    80105b8a <sys_fstat+0x5d>
  return filestat(f, st);
80105b78:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105b7b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b7e:	89 54 24 04          	mov    %edx,0x4(%esp)
80105b82:	89 04 24             	mov    %eax,(%esp)
80105b85:	e8 10 b5 ff ff       	call   8010109a <filestat>
}
80105b8a:	c9                   	leave  
80105b8b:	c3                   	ret    

80105b8c <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
80105b8c:	55                   	push   %ebp
80105b8d:	89 e5                	mov    %esp,%ebp
80105b8f:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105b92:	8d 45 d8             	lea    -0x28(%ebp),%eax
80105b95:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b99:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105ba0:	e8 1f fc ff ff       	call   801057c4 <argstr>
80105ba5:	85 c0                	test   %eax,%eax
80105ba7:	78 17                	js     80105bc0 <sys_link+0x34>
80105ba9:	8d 45 dc             	lea    -0x24(%ebp),%eax
80105bac:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bb0:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105bb7:	e8 08 fc ff ff       	call   801057c4 <argstr>
80105bbc:	85 c0                	test   %eax,%eax
80105bbe:	79 0a                	jns    80105bca <sys_link+0x3e>
    return -1;
80105bc0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105bc5:	e9 3c 01 00 00       	jmp    80105d06 <sys_link+0x17a>
  if((ip = namei(old)) == 0)
80105bca:	8b 45 d8             	mov    -0x28(%ebp),%eax
80105bcd:	89 04 24             	mov    %eax,(%esp)
80105bd0:	e8 35 c8 ff ff       	call   8010240a <namei>
80105bd5:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bd8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105bdc:	75 0a                	jne    80105be8 <sys_link+0x5c>
    return -1;
80105bde:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105be3:	e9 1e 01 00 00       	jmp    80105d06 <sys_link+0x17a>

  begin_trans();
80105be8:	e8 28 d6 ff ff       	call   80103215 <begin_trans>

  ilock(ip);
80105bed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bf0:	89 04 24             	mov    %eax,(%esp)
80105bf3:	e8 70 bc ff ff       	call   80101868 <ilock>
  if(ip->type == T_DIR){
80105bf8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bfb:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105bff:	66 83 f8 01          	cmp    $0x1,%ax
80105c03:	75 1a                	jne    80105c1f <sys_link+0x93>
    iunlockput(ip);
80105c05:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c08:	89 04 24             	mov    %eax,(%esp)
80105c0b:	e8 dc be ff ff       	call   80101aec <iunlockput>
    commit_trans();
80105c10:	e8 49 d6 ff ff       	call   8010325e <commit_trans>
    return -1;
80105c15:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c1a:	e9 e7 00 00 00       	jmp    80105d06 <sys_link+0x17a>
  }

  ip->nlink++;
80105c1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c22:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105c26:	8d 50 01             	lea    0x1(%eax),%edx
80105c29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c2c:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105c30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c33:	89 04 24             	mov    %eax,(%esp)
80105c36:	e8 71 ba ff ff       	call   801016ac <iupdate>
  iunlock(ip);
80105c3b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c3e:	89 04 24             	mov    %eax,(%esp)
80105c41:	e8 70 bd ff ff       	call   801019b6 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
80105c46:	8b 45 dc             	mov    -0x24(%ebp),%eax
80105c49:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80105c4c:	89 54 24 04          	mov    %edx,0x4(%esp)
80105c50:	89 04 24             	mov    %eax,(%esp)
80105c53:	e8 d4 c7 ff ff       	call   8010242c <nameiparent>
80105c58:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105c5b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105c5f:	74 68                	je     80105cc9 <sys_link+0x13d>
    goto bad;
  ilock(dp);
80105c61:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c64:	89 04 24             	mov    %eax,(%esp)
80105c67:	e8 fc bb ff ff       	call   80101868 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80105c6c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c6f:	8b 10                	mov    (%eax),%edx
80105c71:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c74:	8b 00                	mov    (%eax),%eax
80105c76:	39 c2                	cmp    %eax,%edx
80105c78:	75 20                	jne    80105c9a <sys_link+0x10e>
80105c7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c7d:	8b 40 04             	mov    0x4(%eax),%eax
80105c80:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c84:	8d 45 e2             	lea    -0x1e(%ebp),%eax
80105c87:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c8b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c8e:	89 04 24             	mov    %eax,(%esp)
80105c91:	e8 b3 c4 ff ff       	call   80102149 <dirlink>
80105c96:	85 c0                	test   %eax,%eax
80105c98:	79 0d                	jns    80105ca7 <sys_link+0x11b>
    iunlockput(dp);
80105c9a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c9d:	89 04 24             	mov    %eax,(%esp)
80105ca0:	e8 47 be ff ff       	call   80101aec <iunlockput>
    goto bad;
80105ca5:	eb 23                	jmp    80105cca <sys_link+0x13e>
  }
  iunlockput(dp);
80105ca7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105caa:	89 04 24             	mov    %eax,(%esp)
80105cad:	e8 3a be ff ff       	call   80101aec <iunlockput>
  iput(ip);
80105cb2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cb5:	89 04 24             	mov    %eax,(%esp)
80105cb8:	e8 5e bd ff ff       	call   80101a1b <iput>

  commit_trans();
80105cbd:	e8 9c d5 ff ff       	call   8010325e <commit_trans>

  return 0;
80105cc2:	b8 00 00 00 00       	mov    $0x0,%eax
80105cc7:	eb 3d                	jmp    80105d06 <sys_link+0x17a>
  ip->nlink++;
  iupdate(ip);
  iunlock(ip);

  if((dp = nameiparent(new, name)) == 0)
    goto bad;
80105cc9:	90                   	nop
  commit_trans();

  return 0;

bad:
  ilock(ip);
80105cca:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ccd:	89 04 24             	mov    %eax,(%esp)
80105cd0:	e8 93 bb ff ff       	call   80101868 <ilock>
  ip->nlink--;
80105cd5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cd8:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105cdc:	8d 50 ff             	lea    -0x1(%eax),%edx
80105cdf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ce2:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105ce6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ce9:	89 04 24             	mov    %eax,(%esp)
80105cec:	e8 bb b9 ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80105cf1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cf4:	89 04 24             	mov    %eax,(%esp)
80105cf7:	e8 f0 bd ff ff       	call   80101aec <iunlockput>
  commit_trans();
80105cfc:	e8 5d d5 ff ff       	call   8010325e <commit_trans>
  return -1;
80105d01:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105d06:	c9                   	leave  
80105d07:	c3                   	ret    

80105d08 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
80105d08:	55                   	push   %ebp
80105d09:	89 e5                	mov    %esp,%ebp
80105d0b:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105d0e:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80105d15:	eb 4b                	jmp    80105d62 <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105d17:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d1a:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105d21:	00 
80105d22:	89 44 24 08          	mov    %eax,0x8(%esp)
80105d26:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105d29:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d2d:	8b 45 08             	mov    0x8(%ebp),%eax
80105d30:	89 04 24             	mov    %eax,(%esp)
80105d33:	e8 26 c0 ff ff       	call   80101d5e <readi>
80105d38:	83 f8 10             	cmp    $0x10,%eax
80105d3b:	74 0c                	je     80105d49 <isdirempty+0x41>
      panic("isdirempty: readi");
80105d3d:	c7 04 24 ce 8d 10 80 	movl   $0x80108dce,(%esp)
80105d44:	e8 f4 a7 ff ff       	call   8010053d <panic>
    if(de.inum != 0)
80105d49:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80105d4d:	66 85 c0             	test   %ax,%ax
80105d50:	74 07                	je     80105d59 <isdirempty+0x51>
      return 0;
80105d52:	b8 00 00 00 00       	mov    $0x0,%eax
80105d57:	eb 1b                	jmp    80105d74 <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105d59:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d5c:	83 c0 10             	add    $0x10,%eax
80105d5f:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105d62:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105d65:	8b 45 08             	mov    0x8(%ebp),%eax
80105d68:	8b 40 18             	mov    0x18(%eax),%eax
80105d6b:	39 c2                	cmp    %eax,%edx
80105d6d:	72 a8                	jb     80105d17 <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
80105d6f:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105d74:	c9                   	leave  
80105d75:	c3                   	ret    

80105d76 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105d76:	55                   	push   %ebp
80105d77:	89 e5                	mov    %esp,%ebp
80105d79:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80105d7c:	8d 45 cc             	lea    -0x34(%ebp),%eax
80105d7f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d83:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105d8a:	e8 35 fa ff ff       	call   801057c4 <argstr>
80105d8f:	85 c0                	test   %eax,%eax
80105d91:	79 0a                	jns    80105d9d <sys_unlink+0x27>
    return -1;
80105d93:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d98:	e9 aa 01 00 00       	jmp    80105f47 <sys_unlink+0x1d1>
  if((dp = nameiparent(path, name)) == 0)
80105d9d:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105da0:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105da3:	89 54 24 04          	mov    %edx,0x4(%esp)
80105da7:	89 04 24             	mov    %eax,(%esp)
80105daa:	e8 7d c6 ff ff       	call   8010242c <nameiparent>
80105daf:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105db2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105db6:	75 0a                	jne    80105dc2 <sys_unlink+0x4c>
    return -1;
80105db8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dbd:	e9 85 01 00 00       	jmp    80105f47 <sys_unlink+0x1d1>

  begin_trans();
80105dc2:	e8 4e d4 ff ff       	call   80103215 <begin_trans>

  ilock(dp);
80105dc7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dca:	89 04 24             	mov    %eax,(%esp)
80105dcd:	e8 96 ba ff ff       	call   80101868 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105dd2:	c7 44 24 04 e0 8d 10 	movl   $0x80108de0,0x4(%esp)
80105dd9:	80 
80105dda:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105ddd:	89 04 24             	mov    %eax,(%esp)
80105de0:	e8 7a c2 ff ff       	call   8010205f <namecmp>
80105de5:	85 c0                	test   %eax,%eax
80105de7:	0f 84 45 01 00 00    	je     80105f32 <sys_unlink+0x1bc>
80105ded:	c7 44 24 04 e2 8d 10 	movl   $0x80108de2,0x4(%esp)
80105df4:	80 
80105df5:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105df8:	89 04 24             	mov    %eax,(%esp)
80105dfb:	e8 5f c2 ff ff       	call   8010205f <namecmp>
80105e00:	85 c0                	test   %eax,%eax
80105e02:	0f 84 2a 01 00 00    	je     80105f32 <sys_unlink+0x1bc>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
80105e08:	8d 45 c8             	lea    -0x38(%ebp),%eax
80105e0b:	89 44 24 08          	mov    %eax,0x8(%esp)
80105e0f:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105e12:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e16:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e19:	89 04 24             	mov    %eax,(%esp)
80105e1c:	e8 60 c2 ff ff       	call   80102081 <dirlookup>
80105e21:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105e24:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105e28:	0f 84 03 01 00 00    	je     80105f31 <sys_unlink+0x1bb>
    goto bad;
  ilock(ip);
80105e2e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e31:	89 04 24             	mov    %eax,(%esp)
80105e34:	e8 2f ba ff ff       	call   80101868 <ilock>

  if(ip->nlink < 1)
80105e39:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e3c:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105e40:	66 85 c0             	test   %ax,%ax
80105e43:	7f 0c                	jg     80105e51 <sys_unlink+0xdb>
    panic("unlink: nlink < 1");
80105e45:	c7 04 24 e5 8d 10 80 	movl   $0x80108de5,(%esp)
80105e4c:	e8 ec a6 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105e51:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e54:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105e58:	66 83 f8 01          	cmp    $0x1,%ax
80105e5c:	75 1f                	jne    80105e7d <sys_unlink+0x107>
80105e5e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e61:	89 04 24             	mov    %eax,(%esp)
80105e64:	e8 9f fe ff ff       	call   80105d08 <isdirempty>
80105e69:	85 c0                	test   %eax,%eax
80105e6b:	75 10                	jne    80105e7d <sys_unlink+0x107>
    iunlockput(ip);
80105e6d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e70:	89 04 24             	mov    %eax,(%esp)
80105e73:	e8 74 bc ff ff       	call   80101aec <iunlockput>
    goto bad;
80105e78:	e9 b5 00 00 00       	jmp    80105f32 <sys_unlink+0x1bc>
  }

  memset(&de, 0, sizeof(de));
80105e7d:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105e84:	00 
80105e85:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105e8c:	00 
80105e8d:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105e90:	89 04 24             	mov    %eax,(%esp)
80105e93:	e8 42 f5 ff ff       	call   801053da <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105e98:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105e9b:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105ea2:	00 
80105ea3:	89 44 24 08          	mov    %eax,0x8(%esp)
80105ea7:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105eaa:	89 44 24 04          	mov    %eax,0x4(%esp)
80105eae:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eb1:	89 04 24             	mov    %eax,(%esp)
80105eb4:	e8 10 c0 ff ff       	call   80101ec9 <writei>
80105eb9:	83 f8 10             	cmp    $0x10,%eax
80105ebc:	74 0c                	je     80105eca <sys_unlink+0x154>
    panic("unlink: writei");
80105ebe:	c7 04 24 f7 8d 10 80 	movl   $0x80108df7,(%esp)
80105ec5:	e8 73 a6 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR){
80105eca:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ecd:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105ed1:	66 83 f8 01          	cmp    $0x1,%ax
80105ed5:	75 1c                	jne    80105ef3 <sys_unlink+0x17d>
    dp->nlink--;
80105ed7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eda:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105ede:	8d 50 ff             	lea    -0x1(%eax),%edx
80105ee1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ee4:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105ee8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eeb:	89 04 24             	mov    %eax,(%esp)
80105eee:	e8 b9 b7 ff ff       	call   801016ac <iupdate>
  }
  iunlockput(dp);
80105ef3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ef6:	89 04 24             	mov    %eax,(%esp)
80105ef9:	e8 ee bb ff ff       	call   80101aec <iunlockput>

  ip->nlink--;
80105efe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f01:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105f05:	8d 50 ff             	lea    -0x1(%eax),%edx
80105f08:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f0b:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105f0f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f12:	89 04 24             	mov    %eax,(%esp)
80105f15:	e8 92 b7 ff ff       	call   801016ac <iupdate>
  iunlockput(ip);
80105f1a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f1d:	89 04 24             	mov    %eax,(%esp)
80105f20:	e8 c7 bb ff ff       	call   80101aec <iunlockput>

  commit_trans();
80105f25:	e8 34 d3 ff ff       	call   8010325e <commit_trans>

  return 0;
80105f2a:	b8 00 00 00 00       	mov    $0x0,%eax
80105f2f:	eb 16                	jmp    80105f47 <sys_unlink+0x1d1>
  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
    goto bad;
80105f31:	90                   	nop
  commit_trans();

  return 0;

bad:
  iunlockput(dp);
80105f32:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f35:	89 04 24             	mov    %eax,(%esp)
80105f38:	e8 af bb ff ff       	call   80101aec <iunlockput>
  commit_trans();
80105f3d:	e8 1c d3 ff ff       	call   8010325e <commit_trans>
  return -1;
80105f42:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105f47:	c9                   	leave  
80105f48:	c3                   	ret    

80105f49 <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105f49:	55                   	push   %ebp
80105f4a:	89 e5                	mov    %esp,%ebp
80105f4c:	83 ec 48             	sub    $0x48,%esp
80105f4f:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105f52:	8b 55 10             	mov    0x10(%ebp),%edx
80105f55:	8b 45 14             	mov    0x14(%ebp),%eax
80105f58:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105f5c:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105f60:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];
  if((dp = nameiparent(path, name)) == 0)
80105f64:	8d 45 de             	lea    -0x22(%ebp),%eax
80105f67:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f6b:	8b 45 08             	mov    0x8(%ebp),%eax
80105f6e:	89 04 24             	mov    %eax,(%esp)
80105f71:	e8 b6 c4 ff ff       	call   8010242c <nameiparent>
80105f76:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105f79:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105f7d:	75 0a                	jne    80105f89 <create+0x40>
    return 0;
80105f7f:	b8 00 00 00 00       	mov    $0x0,%eax
80105f84:	e9 7e 01 00 00       	jmp    80106107 <create+0x1be>
  ilock(dp);
80105f89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f8c:	89 04 24             	mov    %eax,(%esp)
80105f8f:	e8 d4 b8 ff ff       	call   80101868 <ilock>
  if((ip = dirlookup(dp, name, &off)) != 0){
80105f94:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105f97:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f9b:	8d 45 de             	lea    -0x22(%ebp),%eax
80105f9e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105fa2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fa5:	89 04 24             	mov    %eax,(%esp)
80105fa8:	e8 d4 c0 ff ff       	call   80102081 <dirlookup>
80105fad:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105fb0:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105fb4:	74 47                	je     80105ffd <create+0xb4>
    iunlockput(dp);
80105fb6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fb9:	89 04 24             	mov    %eax,(%esp)
80105fbc:	e8 2b bb ff ff       	call   80101aec <iunlockput>
    ilock(ip);
80105fc1:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105fc4:	89 04 24             	mov    %eax,(%esp)
80105fc7:	e8 9c b8 ff ff       	call   80101868 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105fcc:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105fd1:	75 15                	jne    80105fe8 <create+0x9f>
80105fd3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105fd6:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105fda:	66 83 f8 02          	cmp    $0x2,%ax
80105fde:	75 08                	jne    80105fe8 <create+0x9f>
      return ip;
80105fe0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105fe3:	e9 1f 01 00 00       	jmp    80106107 <create+0x1be>
    iunlockput(ip);
80105fe8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105feb:	89 04 24             	mov    %eax,(%esp)
80105fee:	e8 f9 ba ff ff       	call   80101aec <iunlockput>
    return 0;
80105ff3:	b8 00 00 00 00       	mov    $0x0,%eax
80105ff8:	e9 0a 01 00 00       	jmp    80106107 <create+0x1be>
  }
  if((ip = ialloc(dp->dev, type)) == 0)
80105ffd:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80106001:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106004:	8b 00                	mov    (%eax),%eax
80106006:	89 54 24 04          	mov    %edx,0x4(%esp)
8010600a:	89 04 24             	mov    %eax,(%esp)
8010600d:	e8 bd b5 ff ff       	call   801015cf <ialloc>
80106012:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106015:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106019:	75 0c                	jne    80106027 <create+0xde>
    panic("create: ialloc");
8010601b:	c7 04 24 06 8e 10 80 	movl   $0x80108e06,(%esp)
80106022:	e8 16 a5 ff ff       	call   8010053d <panic>
  ilock(ip);
80106027:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010602a:	89 04 24             	mov    %eax,(%esp)
8010602d:	e8 36 b8 ff ff       	call   80101868 <ilock>
  ip->major = major;
80106032:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106035:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80106039:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
8010603d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106040:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80106044:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80106048:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010604b:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80106051:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106054:	89 04 24             	mov    %eax,(%esp)
80106057:	e8 50 b6 ff ff       	call   801016ac <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
8010605c:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80106061:	75 6a                	jne    801060cd <create+0x184>
    dp->nlink++;  // for ".."
80106063:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106066:	0f b7 40 16          	movzwl 0x16(%eax),%eax
8010606a:	8d 50 01             	lea    0x1(%eax),%edx
8010606d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106070:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80106074:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106077:	89 04 24             	mov    %eax,(%esp)
8010607a:	e8 2d b6 ff ff       	call   801016ac <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
8010607f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106082:	8b 40 04             	mov    0x4(%eax),%eax
80106085:	89 44 24 08          	mov    %eax,0x8(%esp)
80106089:	c7 44 24 04 e0 8d 10 	movl   $0x80108de0,0x4(%esp)
80106090:	80 
80106091:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106094:	89 04 24             	mov    %eax,(%esp)
80106097:	e8 ad c0 ff ff       	call   80102149 <dirlink>
8010609c:	85 c0                	test   %eax,%eax
8010609e:	78 21                	js     801060c1 <create+0x178>
801060a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060a3:	8b 40 04             	mov    0x4(%eax),%eax
801060a6:	89 44 24 08          	mov    %eax,0x8(%esp)
801060aa:	c7 44 24 04 e2 8d 10 	movl   $0x80108de2,0x4(%esp)
801060b1:	80 
801060b2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060b5:	89 04 24             	mov    %eax,(%esp)
801060b8:	e8 8c c0 ff ff       	call   80102149 <dirlink>
801060bd:	85 c0                	test   %eax,%eax
801060bf:	79 0c                	jns    801060cd <create+0x184>
      panic("create dots");
801060c1:	c7 04 24 15 8e 10 80 	movl   $0x80108e15,(%esp)
801060c8:	e8 70 a4 ff ff       	call   8010053d <panic>
  }
  if(dirlink(dp, name, ip->inum) < 0)
801060cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060d0:	8b 40 04             	mov    0x4(%eax),%eax
801060d3:	89 44 24 08          	mov    %eax,0x8(%esp)
801060d7:	8d 45 de             	lea    -0x22(%ebp),%eax
801060da:	89 44 24 04          	mov    %eax,0x4(%esp)
801060de:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060e1:	89 04 24             	mov    %eax,(%esp)
801060e4:	e8 60 c0 ff ff       	call   80102149 <dirlink>
801060e9:	85 c0                	test   %eax,%eax
801060eb:	79 0c                	jns    801060f9 <create+0x1b0>
    panic("create: dirlink");
801060ed:	c7 04 24 21 8e 10 80 	movl   $0x80108e21,(%esp)
801060f4:	e8 44 a4 ff ff       	call   8010053d <panic>
  iunlockput(dp);
801060f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060fc:	89 04 24             	mov    %eax,(%esp)
801060ff:	e8 e8 b9 ff ff       	call   80101aec <iunlockput>

  return ip;
80106104:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80106107:	c9                   	leave  
80106108:	c3                   	ret    

80106109 <fileopen>:

struct file*
fileopen(char *path, int omode)
{
80106109:	55                   	push   %ebp
8010610a:	89 e5                	mov    %esp,%ebp
8010610c:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  struct inode *ip;

  if(omode & O_CREATE){
8010610f:	8b 45 0c             	mov    0xc(%ebp),%eax
80106112:	25 00 02 00 00       	and    $0x200,%eax
80106117:	85 c0                	test   %eax,%eax
80106119:	74 40                	je     8010615b <fileopen+0x52>
    begin_trans();
8010611b:	e8 f5 d0 ff ff       	call   80103215 <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80106120:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106127:	00 
80106128:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010612f:	00 
80106130:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80106137:	00 
80106138:	8b 45 08             	mov    0x8(%ebp),%eax
8010613b:	89 04 24             	mov    %eax,(%esp)
8010613e:	e8 06 fe ff ff       	call   80105f49 <create>
80106143:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80106146:	e8 13 d1 ff ff       	call   8010325e <commit_trans>
    if(ip == 0)
8010614b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010614f:	75 5b                	jne    801061ac <fileopen+0xa3>
      return 0;
80106151:	b8 00 00 00 00       	mov    $0x0,%eax
80106156:	e9 f9 00 00 00       	jmp    80106254 <fileopen+0x14b>
  } else {
    if((ip = namei(path)) == 0)
8010615b:	8b 45 08             	mov    0x8(%ebp),%eax
8010615e:	89 04 24             	mov    %eax,(%esp)
80106161:	e8 a4 c2 ff ff       	call   8010240a <namei>
80106166:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106169:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010616d:	75 0a                	jne    80106179 <fileopen+0x70>
      return 0;
8010616f:	b8 00 00 00 00       	mov    $0x0,%eax
80106174:	e9 db 00 00 00       	jmp    80106254 <fileopen+0x14b>
    ilock(ip);
80106179:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010617c:	89 04 24             	mov    %eax,(%esp)
8010617f:	e8 e4 b6 ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106184:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106187:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010618b:	66 83 f8 01          	cmp    $0x1,%ax
8010618f:	75 1b                	jne    801061ac <fileopen+0xa3>
80106191:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80106195:	74 15                	je     801061ac <fileopen+0xa3>
      iunlockput(ip);
80106197:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010619a:	89 04 24             	mov    %eax,(%esp)
8010619d:	e8 4a b9 ff ff       	call   80101aec <iunlockput>
      return 0;
801061a2:	b8 00 00 00 00       	mov    $0x0,%eax
801061a7:	e9 a8 00 00 00       	jmp    80106254 <fileopen+0x14b>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
801061ac:	e8 6b ad ff ff       	call   80100f1c <filealloc>
801061b1:	89 45 f0             	mov    %eax,-0x10(%ebp)
801061b4:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801061b8:	74 14                	je     801061ce <fileopen+0xc5>
801061ba:	8b 45 f0             	mov    -0x10(%ebp),%eax
801061bd:	89 04 24             	mov    %eax,(%esp)
801061c0:	e8 7c f7 ff ff       	call   80105941 <fdalloc>
801061c5:	89 45 ec             	mov    %eax,-0x14(%ebp)
801061c8:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801061cc:	79 23                	jns    801061f1 <fileopen+0xe8>
    if(f)
801061ce:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801061d2:	74 0b                	je     801061df <fileopen+0xd6>
      fileclose(f);
801061d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801061d7:	89 04 24             	mov    %eax,(%esp)
801061da:	e8 e5 ad ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
801061df:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061e2:	89 04 24             	mov    %eax,(%esp)
801061e5:	e8 02 b9 ff ff       	call   80101aec <iunlockput>
    return 0;
801061ea:	b8 00 00 00 00       	mov    $0x0,%eax
801061ef:	eb 63                	jmp    80106254 <fileopen+0x14b>
  }
  iunlock(ip);
801061f1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061f4:	89 04 24             	mov    %eax,(%esp)
801061f7:	e8 ba b7 ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
801061fc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801061ff:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80106205:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106208:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010620b:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
8010620e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106211:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80106218:	8b 45 0c             	mov    0xc(%ebp),%eax
8010621b:	83 e0 01             	and    $0x1,%eax
8010621e:	85 c0                	test   %eax,%eax
80106220:	0f 94 c2             	sete   %dl
80106223:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106226:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106229:	8b 45 0c             	mov    0xc(%ebp),%eax
8010622c:	83 e0 01             	and    $0x1,%eax
8010622f:	84 c0                	test   %al,%al
80106231:	75 0a                	jne    8010623d <fileopen+0x134>
80106233:	8b 45 0c             	mov    0xc(%ebp),%eax
80106236:	83 e0 02             	and    $0x2,%eax
80106239:	85 c0                	test   %eax,%eax
8010623b:	74 07                	je     80106244 <fileopen+0x13b>
8010623d:	b8 01 00 00 00       	mov    $0x1,%eax
80106242:	eb 05                	jmp    80106249 <fileopen+0x140>
80106244:	b8 00 00 00 00       	mov    $0x0,%eax
80106249:	89 c2                	mov    %eax,%edx
8010624b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010624e:	88 50 09             	mov    %dl,0x9(%eax)
  return f;
80106251:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80106254:	c9                   	leave  
80106255:	c3                   	ret    

80106256 <sys_open>:

int
sys_open(void)
{
80106256:	55                   	push   %ebp
80106257:	89 e5                	mov    %esp,%ebp
80106259:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
8010625c:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010625f:	89 44 24 04          	mov    %eax,0x4(%esp)
80106263:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010626a:	e8 55 f5 ff ff       	call   801057c4 <argstr>
8010626f:	85 c0                	test   %eax,%eax
80106271:	78 17                	js     8010628a <sys_open+0x34>
80106273:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106276:	89 44 24 04          	mov    %eax,0x4(%esp)
8010627a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106281:	e8 a4 f4 ff ff       	call   8010572a <argint>
80106286:	85 c0                	test   %eax,%eax
80106288:	79 0a                	jns    80106294 <sys_open+0x3e>
    return -1;
8010628a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010628f:	e9 46 01 00 00       	jmp    801063da <sys_open+0x184>
  if(omode & O_CREATE){
80106294:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106297:	25 00 02 00 00       	and    $0x200,%eax
8010629c:	85 c0                	test   %eax,%eax
8010629e:	74 40                	je     801062e0 <sys_open+0x8a>
    begin_trans();
801062a0:	e8 70 cf ff ff       	call   80103215 <begin_trans>
    ip = create(path, T_FILE, 0, 0);
801062a5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801062a8:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
801062af:	00 
801062b0:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801062b7:	00 
801062b8:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
801062bf:	00 
801062c0:	89 04 24             	mov    %eax,(%esp)
801062c3:	e8 81 fc ff ff       	call   80105f49 <create>
801062c8:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
801062cb:	e8 8e cf ff ff       	call   8010325e <commit_trans>
    if(ip == 0)
801062d0:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801062d4:	75 5c                	jne    80106332 <sys_open+0xdc>
      return -1;
801062d6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062db:	e9 fa 00 00 00       	jmp    801063da <sys_open+0x184>
  } else {
    if((ip = namei(path)) == 0)
801062e0:	8b 45 e8             	mov    -0x18(%ebp),%eax
801062e3:	89 04 24             	mov    %eax,(%esp)
801062e6:	e8 1f c1 ff ff       	call   8010240a <namei>
801062eb:	89 45 f4             	mov    %eax,-0xc(%ebp)
801062ee:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801062f2:	75 0a                	jne    801062fe <sys_open+0xa8>
      return -1;
801062f4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062f9:	e9 dc 00 00 00       	jmp    801063da <sys_open+0x184>
    ilock(ip);
801062fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106301:	89 04 24             	mov    %eax,(%esp)
80106304:	e8 5f b5 ff ff       	call   80101868 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106309:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010630c:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80106310:	66 83 f8 01          	cmp    $0x1,%ax
80106314:	75 1c                	jne    80106332 <sys_open+0xdc>
80106316:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106319:	85 c0                	test   %eax,%eax
8010631b:	74 15                	je     80106332 <sys_open+0xdc>
      iunlockput(ip);
8010631d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106320:	89 04 24             	mov    %eax,(%esp)
80106323:	e8 c4 b7 ff ff       	call   80101aec <iunlockput>
      return -1;
80106328:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010632d:	e9 a8 00 00 00       	jmp    801063da <sys_open+0x184>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80106332:	e8 e5 ab ff ff       	call   80100f1c <filealloc>
80106337:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010633a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010633e:	74 14                	je     80106354 <sys_open+0xfe>
80106340:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106343:	89 04 24             	mov    %eax,(%esp)
80106346:	e8 f6 f5 ff ff       	call   80105941 <fdalloc>
8010634b:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010634e:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80106352:	79 23                	jns    80106377 <sys_open+0x121>
    if(f)
80106354:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106358:	74 0b                	je     80106365 <sys_open+0x10f>
      fileclose(f);
8010635a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010635d:	89 04 24             	mov    %eax,(%esp)
80106360:	e8 5f ac ff ff       	call   80100fc4 <fileclose>
    iunlockput(ip);
80106365:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106368:	89 04 24             	mov    %eax,(%esp)
8010636b:	e8 7c b7 ff ff       	call   80101aec <iunlockput>
    return -1;
80106370:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106375:	eb 63                	jmp    801063da <sys_open+0x184>
  }
  iunlock(ip);
80106377:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010637a:	89 04 24             	mov    %eax,(%esp)
8010637d:	e8 34 b6 ff ff       	call   801019b6 <iunlock>

  f->type = FD_INODE;
80106382:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106385:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
8010638b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010638e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106391:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80106394:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106397:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
8010639e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801063a1:	83 e0 01             	and    $0x1,%eax
801063a4:	85 c0                	test   %eax,%eax
801063a6:	0f 94 c2             	sete   %dl
801063a9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801063ac:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
801063af:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801063b2:	83 e0 01             	and    $0x1,%eax
801063b5:	84 c0                	test   %al,%al
801063b7:	75 0a                	jne    801063c3 <sys_open+0x16d>
801063b9:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801063bc:	83 e0 02             	and    $0x2,%eax
801063bf:	85 c0                	test   %eax,%eax
801063c1:	74 07                	je     801063ca <sys_open+0x174>
801063c3:	b8 01 00 00 00       	mov    $0x1,%eax
801063c8:	eb 05                	jmp    801063cf <sys_open+0x179>
801063ca:	b8 00 00 00 00       	mov    $0x0,%eax
801063cf:	89 c2                	mov    %eax,%edx
801063d1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801063d4:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
801063d7:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
801063da:	c9                   	leave  
801063db:	c3                   	ret    

801063dc <sys_mkdir>:

int
sys_mkdir(void)
{
801063dc:	55                   	push   %ebp
801063dd:	89 e5                	mov    %esp,%ebp
801063df:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
801063e2:	e8 2e ce ff ff       	call   80103215 <begin_trans>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
801063e7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801063ea:	89 44 24 04          	mov    %eax,0x4(%esp)
801063ee:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063f5:	e8 ca f3 ff ff       	call   801057c4 <argstr>
801063fa:	85 c0                	test   %eax,%eax
801063fc:	78 2c                	js     8010642a <sys_mkdir+0x4e>
801063fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106401:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106408:	00 
80106409:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106410:	00 
80106411:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106418:	00 
80106419:	89 04 24             	mov    %eax,(%esp)
8010641c:	e8 28 fb ff ff       	call   80105f49 <create>
80106421:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106424:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106428:	75 0c                	jne    80106436 <sys_mkdir+0x5a>
    commit_trans();
8010642a:	e8 2f ce ff ff       	call   8010325e <commit_trans>
    return -1;
8010642f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106434:	eb 15                	jmp    8010644b <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80106436:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106439:	89 04 24             	mov    %eax,(%esp)
8010643c:	e8 ab b6 ff ff       	call   80101aec <iunlockput>
  commit_trans();
80106441:	e8 18 ce ff ff       	call   8010325e <commit_trans>
  return 0;
80106446:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010644b:	c9                   	leave  
8010644c:	c3                   	ret    

8010644d <sys_mknod>:

int
sys_mknod(void)
{
8010644d:	55                   	push   %ebp
8010644e:	89 e5                	mov    %esp,%ebp
80106450:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
80106453:	e8 bd cd ff ff       	call   80103215 <begin_trans>
  if((len=argstr(0, &path)) < 0 ||
80106458:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010645b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010645f:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106466:	e8 59 f3 ff ff       	call   801057c4 <argstr>
8010646b:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010646e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106472:	78 5e                	js     801064d2 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
80106474:	8d 45 e8             	lea    -0x18(%ebp),%eax
80106477:	89 44 24 04          	mov    %eax,0x4(%esp)
8010647b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106482:	e8 a3 f2 ff ff       	call   8010572a <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
80106487:	85 c0                	test   %eax,%eax
80106489:	78 47                	js     801064d2 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
8010648b:	8d 45 e4             	lea    -0x1c(%ebp),%eax
8010648e:	89 44 24 04          	mov    %eax,0x4(%esp)
80106492:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80106499:	e8 8c f2 ff ff       	call   8010572a <argint>
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
8010649e:	85 c0                	test   %eax,%eax
801064a0:	78 30                	js     801064d2 <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
801064a2:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801064a5:	0f bf c8             	movswl %ax,%ecx
801064a8:	8b 45 e8             	mov    -0x18(%ebp),%eax
801064ab:	0f bf d0             	movswl %ax,%edx
801064ae:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801064b1:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801064b5:	89 54 24 08          	mov    %edx,0x8(%esp)
801064b9:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801064c0:	00 
801064c1:	89 04 24             	mov    %eax,(%esp)
801064c4:	e8 80 fa ff ff       	call   80105f49 <create>
801064c9:	89 45 f0             	mov    %eax,-0x10(%ebp)
801064cc:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801064d0:	75 0c                	jne    801064de <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    commit_trans();
801064d2:	e8 87 cd ff ff       	call   8010325e <commit_trans>
    return -1;
801064d7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064dc:	eb 15                	jmp    801064f3 <sys_mknod+0xa6>
  }
  iunlockput(ip);
801064de:	8b 45 f0             	mov    -0x10(%ebp),%eax
801064e1:	89 04 24             	mov    %eax,(%esp)
801064e4:	e8 03 b6 ff ff       	call   80101aec <iunlockput>
  commit_trans();
801064e9:	e8 70 cd ff ff       	call   8010325e <commit_trans>
  return 0;
801064ee:	b8 00 00 00 00       	mov    $0x0,%eax
}
801064f3:	c9                   	leave  
801064f4:	c3                   	ret    

801064f5 <sys_chdir>:

int
sys_chdir(void)
{
801064f5:	55                   	push   %ebp
801064f6:	89 e5                	mov    %esp,%ebp
801064f8:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0)
801064fb:	8d 45 f0             	lea    -0x10(%ebp),%eax
801064fe:	89 44 24 04          	mov    %eax,0x4(%esp)
80106502:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106509:	e8 b6 f2 ff ff       	call   801057c4 <argstr>
8010650e:	85 c0                	test   %eax,%eax
80106510:	78 14                	js     80106526 <sys_chdir+0x31>
80106512:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106515:	89 04 24             	mov    %eax,(%esp)
80106518:	e8 ed be ff ff       	call   8010240a <namei>
8010651d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106520:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106524:	75 07                	jne    8010652d <sys_chdir+0x38>
    return -1;
80106526:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010652b:	eb 57                	jmp    80106584 <sys_chdir+0x8f>
  ilock(ip);
8010652d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106530:	89 04 24             	mov    %eax,(%esp)
80106533:	e8 30 b3 ff ff       	call   80101868 <ilock>
  if(ip->type != T_DIR){
80106538:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010653b:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010653f:	66 83 f8 01          	cmp    $0x1,%ax
80106543:	74 12                	je     80106557 <sys_chdir+0x62>
    iunlockput(ip);
80106545:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106548:	89 04 24             	mov    %eax,(%esp)
8010654b:	e8 9c b5 ff ff       	call   80101aec <iunlockput>
    return -1;
80106550:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106555:	eb 2d                	jmp    80106584 <sys_chdir+0x8f>
  }
  iunlock(ip);
80106557:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010655a:	89 04 24             	mov    %eax,(%esp)
8010655d:	e8 54 b4 ff ff       	call   801019b6 <iunlock>
  iput(proc->cwd);
80106562:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106568:	8b 40 68             	mov    0x68(%eax),%eax
8010656b:	89 04 24             	mov    %eax,(%esp)
8010656e:	e8 a8 b4 ff ff       	call   80101a1b <iput>
  proc->cwd = ip;
80106573:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106579:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010657c:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
8010657f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106584:	c9                   	leave  
80106585:	c3                   	ret    

80106586 <sys_exec>:

int
sys_exec(void)
{
80106586:	55                   	push   %ebp
80106587:	89 e5                	mov    %esp,%ebp
80106589:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
8010658f:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106592:	89 44 24 04          	mov    %eax,0x4(%esp)
80106596:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010659d:	e8 22 f2 ff ff       	call   801057c4 <argstr>
801065a2:	85 c0                	test   %eax,%eax
801065a4:	78 1a                	js     801065c0 <sys_exec+0x3a>
801065a6:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
801065ac:	89 44 24 04          	mov    %eax,0x4(%esp)
801065b0:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801065b7:	e8 6e f1 ff ff       	call   8010572a <argint>
801065bc:	85 c0                	test   %eax,%eax
801065be:	79 0a                	jns    801065ca <sys_exec+0x44>
    return -1;
801065c0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065c5:	e9 e2 00 00 00       	jmp    801066ac <sys_exec+0x126>
  }
  memset(argv, 0, sizeof(argv));
801065ca:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801065d1:	00 
801065d2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801065d9:	00 
801065da:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801065e0:	89 04 24             	mov    %eax,(%esp)
801065e3:	e8 f2 ed ff ff       	call   801053da <memset>
  for(i=0;; i++){
801065e8:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
801065ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801065f2:	83 f8 1f             	cmp    $0x1f,%eax
801065f5:	76 0a                	jbe    80106601 <sys_exec+0x7b>
      return -1;
801065f7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065fc:	e9 ab 00 00 00       	jmp    801066ac <sys_exec+0x126>
    if(fetchint(proc, uargv+4*i, (int*)&uarg) < 0)
80106601:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106604:	c1 e0 02             	shl    $0x2,%eax
80106607:	89 c2                	mov    %eax,%edx
80106609:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
8010660f:	8d 0c 02             	lea    (%edx,%eax,1),%ecx
80106612:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106618:	8d 95 68 ff ff ff    	lea    -0x98(%ebp),%edx
8010661e:	89 54 24 08          	mov    %edx,0x8(%esp)
80106622:	89 4c 24 04          	mov    %ecx,0x4(%esp)
80106626:	89 04 24             	mov    %eax,(%esp)
80106629:	e8 6a f0 ff ff       	call   80105698 <fetchint>
8010662e:	85 c0                	test   %eax,%eax
80106630:	79 07                	jns    80106639 <sys_exec+0xb3>
      return -1;
80106632:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106637:	eb 73                	jmp    801066ac <sys_exec+0x126>
    if(uarg == 0){
80106639:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
8010663f:	85 c0                	test   %eax,%eax
80106641:	75 26                	jne    80106669 <sys_exec+0xe3>
      argv[i] = 0;
80106643:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106646:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
8010664d:	00 00 00 00 
      break;
80106651:	90                   	nop
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
80106652:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106655:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
8010665b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010665f:	89 04 24             	mov    %eax,(%esp)
80106662:	e8 95 a4 ff ff       	call   80100afc <exec>
80106667:	eb 43                	jmp    801066ac <sys_exec+0x126>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
80106669:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010666c:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80106673:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
80106679:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
8010667c:	8b 95 68 ff ff ff    	mov    -0x98(%ebp),%edx
80106682:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106688:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010668c:	89 54 24 04          	mov    %edx,0x4(%esp)
80106690:	89 04 24             	mov    %eax,(%esp)
80106693:	e8 34 f0 ff ff       	call   801056cc <fetchstr>
80106698:	85 c0                	test   %eax,%eax
8010669a:	79 07                	jns    801066a3 <sys_exec+0x11d>
      return -1;
8010669c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801066a1:	eb 09                	jmp    801066ac <sys_exec+0x126>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
801066a3:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(proc, uarg, &argv[i]) < 0)
      return -1;
  }
801066a7:	e9 43 ff ff ff       	jmp    801065ef <sys_exec+0x69>
  return exec(path, argv);
}
801066ac:	c9                   	leave  
801066ad:	c3                   	ret    

801066ae <sys_pipe>:

int
sys_pipe(void)
{
801066ae:	55                   	push   %ebp
801066af:	89 e5                	mov    %esp,%ebp
801066b1:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
801066b4:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801066bb:	00 
801066bc:	8d 45 ec             	lea    -0x14(%ebp),%eax
801066bf:	89 44 24 04          	mov    %eax,0x4(%esp)
801066c3:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801066ca:	e8 93 f0 ff ff       	call   80105762 <argptr>
801066cf:	85 c0                	test   %eax,%eax
801066d1:	79 0a                	jns    801066dd <sys_pipe+0x2f>
    return -1;
801066d3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801066d8:	e9 9b 00 00 00       	jmp    80106778 <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
801066dd:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801066e0:	89 44 24 04          	mov    %eax,0x4(%esp)
801066e4:	8d 45 e8             	lea    -0x18(%ebp),%eax
801066e7:	89 04 24             	mov    %eax,(%esp)
801066ea:	e8 41 d5 ff ff       	call   80103c30 <pipealloc>
801066ef:	85 c0                	test   %eax,%eax
801066f1:	79 07                	jns    801066fa <sys_pipe+0x4c>
    return -1;
801066f3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801066f8:	eb 7e                	jmp    80106778 <sys_pipe+0xca>
  fd0 = -1;
801066fa:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106701:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106704:	89 04 24             	mov    %eax,(%esp)
80106707:	e8 35 f2 ff ff       	call   80105941 <fdalloc>
8010670c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010670f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106713:	78 14                	js     80106729 <sys_pipe+0x7b>
80106715:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106718:	89 04 24             	mov    %eax,(%esp)
8010671b:	e8 21 f2 ff ff       	call   80105941 <fdalloc>
80106720:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106723:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106727:	79 37                	jns    80106760 <sys_pipe+0xb2>
    if(fd0 >= 0)
80106729:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010672d:	78 14                	js     80106743 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
8010672f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106735:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106738:	83 c2 08             	add    $0x8,%edx
8010673b:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80106742:	00 
    fileclose(rf);
80106743:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106746:	89 04 24             	mov    %eax,(%esp)
80106749:	e8 76 a8 ff ff       	call   80100fc4 <fileclose>
    fileclose(wf);
8010674e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106751:	89 04 24             	mov    %eax,(%esp)
80106754:	e8 6b a8 ff ff       	call   80100fc4 <fileclose>
    return -1;
80106759:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010675e:	eb 18                	jmp    80106778 <sys_pipe+0xca>
  }
  fd[0] = fd0;
80106760:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106763:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106766:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
80106768:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010676b:	8d 50 04             	lea    0x4(%eax),%edx
8010676e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106771:	89 02                	mov    %eax,(%edx)
  return 0;
80106773:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106778:	c9                   	leave  
80106779:	c3                   	ret    
	...

8010677c <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
8010677c:	55                   	push   %ebp
8010677d:	89 e5                	mov    %esp,%ebp
8010677f:	83 ec 08             	sub    $0x8,%esp
  return fork();
80106782:	e8 50 e0 ff ff       	call   801047d7 <fork>
}
80106787:	c9                   	leave  
80106788:	c3                   	ret    

80106789 <sys_exit>:

int
sys_exit(void)
{
80106789:	55                   	push   %ebp
8010678a:	89 e5                	mov    %esp,%ebp
8010678c:	83 ec 08             	sub    $0x8,%esp
  exit();
8010678f:	e8 a6 e1 ff ff       	call   8010493a <exit>
  return 0;  // not reached
80106794:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106799:	c9                   	leave  
8010679a:	c3                   	ret    

8010679b <sys_wait>:

int
sys_wait(void)
{
8010679b:	55                   	push   %ebp
8010679c:	89 e5                	mov    %esp,%ebp
8010679e:	83 ec 08             	sub    $0x8,%esp
  return wait();
801067a1:	e8 af e2 ff ff       	call   80104a55 <wait>
}
801067a6:	c9                   	leave  
801067a7:	c3                   	ret    

801067a8 <sys_kill>:

int
sys_kill(void)
{
801067a8:	55                   	push   %ebp
801067a9:	89 e5                	mov    %esp,%ebp
801067ab:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
801067ae:	8d 45 f4             	lea    -0xc(%ebp),%eax
801067b1:	89 44 24 04          	mov    %eax,0x4(%esp)
801067b5:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801067bc:	e8 69 ef ff ff       	call   8010572a <argint>
801067c1:	85 c0                	test   %eax,%eax
801067c3:	79 07                	jns    801067cc <sys_kill+0x24>
    return -1;
801067c5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801067ca:	eb 0b                	jmp    801067d7 <sys_kill+0x2f>
  return kill(pid);
801067cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067cf:	89 04 24             	mov    %eax,(%esp)
801067d2:	e8 9c e7 ff ff       	call   80104f73 <kill>
}
801067d7:	c9                   	leave  
801067d8:	c3                   	ret    

801067d9 <sys_getpid>:

int
sys_getpid(void)
{
801067d9:	55                   	push   %ebp
801067da:	89 e5                	mov    %esp,%ebp
  return proc->pid;
801067dc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801067e2:	8b 40 10             	mov    0x10(%eax),%eax
}
801067e5:	5d                   	pop    %ebp
801067e6:	c3                   	ret    

801067e7 <sys_sbrk>:

int
sys_sbrk(void)
{
801067e7:	55                   	push   %ebp
801067e8:	89 e5                	mov    %esp,%ebp
801067ea:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
801067ed:	8d 45 f0             	lea    -0x10(%ebp),%eax
801067f0:	89 44 24 04          	mov    %eax,0x4(%esp)
801067f4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801067fb:	e8 2a ef ff ff       	call   8010572a <argint>
80106800:	85 c0                	test   %eax,%eax
80106802:	79 07                	jns    8010680b <sys_sbrk+0x24>
    return -1;
80106804:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106809:	eb 24                	jmp    8010682f <sys_sbrk+0x48>
  addr = proc->sz;
8010680b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106811:	8b 00                	mov    (%eax),%eax
80106813:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
80106816:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106819:	89 04 24             	mov    %eax,(%esp)
8010681c:	e8 11 df ff ff       	call   80104732 <growproc>
80106821:	85 c0                	test   %eax,%eax
80106823:	79 07                	jns    8010682c <sys_sbrk+0x45>
    return -1;
80106825:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010682a:	eb 03                	jmp    8010682f <sys_sbrk+0x48>
  return addr;
8010682c:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010682f:	c9                   	leave  
80106830:	c3                   	ret    

80106831 <sys_sleep>:

int
sys_sleep(void)
{
80106831:	55                   	push   %ebp
80106832:	89 e5                	mov    %esp,%ebp
80106834:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
80106837:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010683a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010683e:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106845:	e8 e0 ee ff ff       	call   8010572a <argint>
8010684a:	85 c0                	test   %eax,%eax
8010684c:	79 07                	jns    80106855 <sys_sleep+0x24>
    return -1;
8010684e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106853:	eb 6c                	jmp    801068c1 <sys_sleep+0x90>
  acquire(&tickslock);
80106855:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
8010685c:	e8 f2 e8 ff ff       	call   80105153 <acquire>
  ticks0 = ticks;
80106861:	a1 a0 3a 11 80       	mov    0x80113aa0,%eax
80106866:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
80106869:	eb 34                	jmp    8010689f <sys_sleep+0x6e>
    if(proc->killed){
8010686b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106871:	8b 40 24             	mov    0x24(%eax),%eax
80106874:	85 c0                	test   %eax,%eax
80106876:	74 13                	je     8010688b <sys_sleep+0x5a>
      release(&tickslock);
80106878:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
8010687f:	e8 6a e9 ff ff       	call   801051ee <release>
      return -1;
80106884:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106889:	eb 36                	jmp    801068c1 <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
8010688b:	c7 44 24 04 60 32 11 	movl   $0x80113260,0x4(%esp)
80106892:	80 
80106893:	c7 04 24 a0 3a 11 80 	movl   $0x80113aa0,(%esp)
8010689a:	e8 46 e5 ff ff       	call   80104de5 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
8010689f:	a1 a0 3a 11 80       	mov    0x80113aa0,%eax
801068a4:	89 c2                	mov    %eax,%edx
801068a6:	2b 55 f4             	sub    -0xc(%ebp),%edx
801068a9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801068ac:	39 c2                	cmp    %eax,%edx
801068ae:	72 bb                	jb     8010686b <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
801068b0:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
801068b7:	e8 32 e9 ff ff       	call   801051ee <release>
  return 0;
801068bc:	b8 00 00 00 00       	mov    $0x0,%eax
}
801068c1:	c9                   	leave  
801068c2:	c3                   	ret    

801068c3 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
801068c3:	55                   	push   %ebp
801068c4:	89 e5                	mov    %esp,%ebp
801068c6:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
801068c9:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
801068d0:	e8 7e e8 ff ff       	call   80105153 <acquire>
  xticks = ticks;
801068d5:	a1 a0 3a 11 80       	mov    0x80113aa0,%eax
801068da:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
801068dd:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
801068e4:	e8 05 e9 ff ff       	call   801051ee <release>
  return xticks;
801068e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801068ec:	c9                   	leave  
801068ed:	c3                   	ret    

801068ee <sys_enableSwapping>:

void
sys_enableSwapping(void)
{
801068ee:	55                   	push   %ebp
801068ef:	89 e5                	mov    %esp,%ebp
  swapFlag = 1;
801068f1:	c7 05 08 c0 10 80 01 	movl   $0x1,0x8010c008
801068f8:	00 00 00 
}
801068fb:	5d                   	pop    %ebp
801068fc:	c3                   	ret    

801068fd <sys_disableSwapping>:

void
sys_disableSwapping(void)
{
801068fd:	55                   	push   %ebp
801068fe:	89 e5                	mov    %esp,%ebp
  swapFlag = 0;
80106900:	c7 05 08 c0 10 80 00 	movl   $0x0,0x8010c008
80106907:	00 00 00 
}
8010690a:	5d                   	pop    %ebp
8010690b:	c3                   	ret    

8010690c <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
8010690c:	55                   	push   %ebp
8010690d:	89 e5                	mov    %esp,%ebp
8010690f:	83 ec 08             	sub    $0x8,%esp
80106912:	8b 55 08             	mov    0x8(%ebp),%edx
80106915:	8b 45 0c             	mov    0xc(%ebp),%eax
80106918:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010691c:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010691f:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106923:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106927:	ee                   	out    %al,(%dx)
}
80106928:	c9                   	leave  
80106929:	c3                   	ret    

8010692a <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
8010692a:	55                   	push   %ebp
8010692b:	89 e5                	mov    %esp,%ebp
8010692d:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
80106930:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
80106937:	00 
80106938:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
8010693f:	e8 c8 ff ff ff       	call   8010690c <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
80106944:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
8010694b:	00 
8010694c:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
80106953:	e8 b4 ff ff ff       	call   8010690c <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
80106958:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
8010695f:	00 
80106960:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
80106967:	e8 a0 ff ff ff       	call   8010690c <outb>
  picenable(IRQ_TIMER);
8010696c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106973:	e8 41 d1 ff ff       	call   80103ab9 <picenable>
}
80106978:	c9                   	leave  
80106979:	c3                   	ret    
	...

8010697c <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
8010697c:	1e                   	push   %ds
  pushl %es
8010697d:	06                   	push   %es
  pushl %fs
8010697e:	0f a0                	push   %fs
  pushl %gs
80106980:	0f a8                	push   %gs
  pushal
80106982:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
80106983:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
80106987:	8e d8                	mov    %eax,%ds
  movw %ax, %es
80106989:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
8010698b:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
8010698f:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
80106991:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
80106993:	54                   	push   %esp
  call trap
80106994:	e8 de 01 00 00       	call   80106b77 <trap>
  addl $4, %esp
80106999:	83 c4 04             	add    $0x4,%esp

8010699c <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
8010699c:	61                   	popa   
  popl %gs
8010699d:	0f a9                	pop    %gs
  popl %fs
8010699f:	0f a1                	pop    %fs
  popl %es
801069a1:	07                   	pop    %es
  popl %ds
801069a2:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
801069a3:	83 c4 08             	add    $0x8,%esp
  iret
801069a6:	cf                   	iret   
	...

801069a8 <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
801069a8:	55                   	push   %ebp
801069a9:	89 e5                	mov    %esp,%ebp
801069ab:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
801069ae:	8b 45 0c             	mov    0xc(%ebp),%eax
801069b1:	83 e8 01             	sub    $0x1,%eax
801069b4:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801069b8:	8b 45 08             	mov    0x8(%ebp),%eax
801069bb:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801069bf:	8b 45 08             	mov    0x8(%ebp),%eax
801069c2:	c1 e8 10             	shr    $0x10,%eax
801069c5:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
801069c9:	8d 45 fa             	lea    -0x6(%ebp),%eax
801069cc:	0f 01 18             	lidtl  (%eax)
}
801069cf:	c9                   	leave  
801069d0:	c3                   	ret    

801069d1 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
801069d1:	55                   	push   %ebp
801069d2:	89 e5                	mov    %esp,%ebp
801069d4:	53                   	push   %ebx
801069d5:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
801069d8:	0f 20 d3             	mov    %cr2,%ebx
801069db:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return val;
801069de:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801069e1:	83 c4 10             	add    $0x10,%esp
801069e4:	5b                   	pop    %ebx
801069e5:	5d                   	pop    %ebp
801069e6:	c3                   	ret    

801069e7 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
801069e7:	55                   	push   %ebp
801069e8:	89 e5                	mov    %esp,%ebp
801069ea:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
801069ed:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801069f4:	e9 c3 00 00 00       	jmp    80106abc <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
801069f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801069fc:	8b 04 85 a0 c0 10 80 	mov    -0x7fef3f60(,%eax,4),%eax
80106a03:	89 c2                	mov    %eax,%edx
80106a05:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a08:	66 89 14 c5 a0 32 11 	mov    %dx,-0x7feecd60(,%eax,8)
80106a0f:	80 
80106a10:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a13:	66 c7 04 c5 a2 32 11 	movw   $0x8,-0x7feecd5e(,%eax,8)
80106a1a:	80 08 00 
80106a1d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a20:	0f b6 14 c5 a4 32 11 	movzbl -0x7feecd5c(,%eax,8),%edx
80106a27:	80 
80106a28:	83 e2 e0             	and    $0xffffffe0,%edx
80106a2b:	88 14 c5 a4 32 11 80 	mov    %dl,-0x7feecd5c(,%eax,8)
80106a32:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a35:	0f b6 14 c5 a4 32 11 	movzbl -0x7feecd5c(,%eax,8),%edx
80106a3c:	80 
80106a3d:	83 e2 1f             	and    $0x1f,%edx
80106a40:	88 14 c5 a4 32 11 80 	mov    %dl,-0x7feecd5c(,%eax,8)
80106a47:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a4a:	0f b6 14 c5 a5 32 11 	movzbl -0x7feecd5b(,%eax,8),%edx
80106a51:	80 
80106a52:	83 e2 f0             	and    $0xfffffff0,%edx
80106a55:	83 ca 0e             	or     $0xe,%edx
80106a58:	88 14 c5 a5 32 11 80 	mov    %dl,-0x7feecd5b(,%eax,8)
80106a5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a62:	0f b6 14 c5 a5 32 11 	movzbl -0x7feecd5b(,%eax,8),%edx
80106a69:	80 
80106a6a:	83 e2 ef             	and    $0xffffffef,%edx
80106a6d:	88 14 c5 a5 32 11 80 	mov    %dl,-0x7feecd5b(,%eax,8)
80106a74:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a77:	0f b6 14 c5 a5 32 11 	movzbl -0x7feecd5b(,%eax,8),%edx
80106a7e:	80 
80106a7f:	83 e2 9f             	and    $0xffffff9f,%edx
80106a82:	88 14 c5 a5 32 11 80 	mov    %dl,-0x7feecd5b(,%eax,8)
80106a89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a8c:	0f b6 14 c5 a5 32 11 	movzbl -0x7feecd5b(,%eax,8),%edx
80106a93:	80 
80106a94:	83 ca 80             	or     $0xffffff80,%edx
80106a97:	88 14 c5 a5 32 11 80 	mov    %dl,-0x7feecd5b(,%eax,8)
80106a9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106aa1:	8b 04 85 a0 c0 10 80 	mov    -0x7fef3f60(,%eax,4),%eax
80106aa8:	c1 e8 10             	shr    $0x10,%eax
80106aab:	89 c2                	mov    %eax,%edx
80106aad:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106ab0:	66 89 14 c5 a6 32 11 	mov    %dx,-0x7feecd5a(,%eax,8)
80106ab7:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
80106ab8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106abc:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
80106ac3:	0f 8e 30 ff ff ff    	jle    801069f9 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80106ac9:	a1 a0 c1 10 80       	mov    0x8010c1a0,%eax
80106ace:	66 a3 a0 34 11 80    	mov    %ax,0x801134a0
80106ad4:	66 c7 05 a2 34 11 80 	movw   $0x8,0x801134a2
80106adb:	08 00 
80106add:	0f b6 05 a4 34 11 80 	movzbl 0x801134a4,%eax
80106ae4:	83 e0 e0             	and    $0xffffffe0,%eax
80106ae7:	a2 a4 34 11 80       	mov    %al,0x801134a4
80106aec:	0f b6 05 a4 34 11 80 	movzbl 0x801134a4,%eax
80106af3:	83 e0 1f             	and    $0x1f,%eax
80106af6:	a2 a4 34 11 80       	mov    %al,0x801134a4
80106afb:	0f b6 05 a5 34 11 80 	movzbl 0x801134a5,%eax
80106b02:	83 c8 0f             	or     $0xf,%eax
80106b05:	a2 a5 34 11 80       	mov    %al,0x801134a5
80106b0a:	0f b6 05 a5 34 11 80 	movzbl 0x801134a5,%eax
80106b11:	83 e0 ef             	and    $0xffffffef,%eax
80106b14:	a2 a5 34 11 80       	mov    %al,0x801134a5
80106b19:	0f b6 05 a5 34 11 80 	movzbl 0x801134a5,%eax
80106b20:	83 c8 60             	or     $0x60,%eax
80106b23:	a2 a5 34 11 80       	mov    %al,0x801134a5
80106b28:	0f b6 05 a5 34 11 80 	movzbl 0x801134a5,%eax
80106b2f:	83 c8 80             	or     $0xffffff80,%eax
80106b32:	a2 a5 34 11 80       	mov    %al,0x801134a5
80106b37:	a1 a0 c1 10 80       	mov    0x8010c1a0,%eax
80106b3c:	c1 e8 10             	shr    $0x10,%eax
80106b3f:	66 a3 a6 34 11 80    	mov    %ax,0x801134a6
  
  initlock(&tickslock, "time");
80106b45:	c7 44 24 04 34 8e 10 	movl   $0x80108e34,0x4(%esp)
80106b4c:	80 
80106b4d:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
80106b54:	e8 d9 e5 ff ff       	call   80105132 <initlock>
}
80106b59:	c9                   	leave  
80106b5a:	c3                   	ret    

80106b5b <idtinit>:

void
idtinit(void)
{
80106b5b:	55                   	push   %ebp
80106b5c:	89 e5                	mov    %esp,%ebp
80106b5e:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
80106b61:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
80106b68:	00 
80106b69:	c7 04 24 a0 32 11 80 	movl   $0x801132a0,(%esp)
80106b70:	e8 33 fe ff ff       	call   801069a8 <lidt>
}
80106b75:	c9                   	leave  
80106b76:	c3                   	ret    

80106b77 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
80106b77:	55                   	push   %ebp
80106b78:	89 e5                	mov    %esp,%ebp
80106b7a:	57                   	push   %edi
80106b7b:	56                   	push   %esi
80106b7c:	53                   	push   %ebx
80106b7d:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
80106b80:	8b 45 08             	mov    0x8(%ebp),%eax
80106b83:	8b 40 30             	mov    0x30(%eax),%eax
80106b86:	83 f8 40             	cmp    $0x40,%eax
80106b89:	75 3e                	jne    80106bc9 <trap+0x52>
    if(proc->killed)
80106b8b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b91:	8b 40 24             	mov    0x24(%eax),%eax
80106b94:	85 c0                	test   %eax,%eax
80106b96:	74 05                	je     80106b9d <trap+0x26>
      exit();
80106b98:	e8 9d dd ff ff       	call   8010493a <exit>
    proc->tf = tf;
80106b9d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106ba3:	8b 55 08             	mov    0x8(%ebp),%edx
80106ba6:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
80106ba9:	e8 59 ec ff ff       	call   80105807 <syscall>
    if(proc->killed)
80106bae:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106bb4:	8b 40 24             	mov    0x24(%eax),%eax
80106bb7:	85 c0                	test   %eax,%eax
80106bb9:	0f 84 34 02 00 00    	je     80106df3 <trap+0x27c>
      exit();
80106bbf:	e8 76 dd ff ff       	call   8010493a <exit>
    return;
80106bc4:	e9 2a 02 00 00       	jmp    80106df3 <trap+0x27c>
  }

  switch(tf->trapno){
80106bc9:	8b 45 08             	mov    0x8(%ebp),%eax
80106bcc:	8b 40 30             	mov    0x30(%eax),%eax
80106bcf:	83 e8 20             	sub    $0x20,%eax
80106bd2:	83 f8 1f             	cmp    $0x1f,%eax
80106bd5:	0f 87 bc 00 00 00    	ja     80106c97 <trap+0x120>
80106bdb:	8b 04 85 dc 8e 10 80 	mov    -0x7fef7124(,%eax,4),%eax
80106be2:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
80106be4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106bea:	0f b6 00             	movzbl (%eax),%eax
80106bed:	84 c0                	test   %al,%al
80106bef:	75 31                	jne    80106c22 <trap+0xab>
      acquire(&tickslock);
80106bf1:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
80106bf8:	e8 56 e5 ff ff       	call   80105153 <acquire>
      ticks++;
80106bfd:	a1 a0 3a 11 80       	mov    0x80113aa0,%eax
80106c02:	83 c0 01             	add    $0x1,%eax
80106c05:	a3 a0 3a 11 80       	mov    %eax,0x80113aa0
      wakeup(&ticks);
80106c0a:	c7 04 24 a0 3a 11 80 	movl   $0x80113aa0,(%esp)
80106c11:	e8 32 e3 ff ff       	call   80104f48 <wakeup>
      release(&tickslock);
80106c16:	c7 04 24 60 32 11 80 	movl   $0x80113260,(%esp)
80106c1d:	e8 cc e5 ff ff       	call   801051ee <release>
    }
    lapiceoi();
80106c22:	e8 ba c2 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106c27:	e9 41 01 00 00       	jmp    80106d6d <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
80106c2c:	e8 c0 ba ff ff       	call   801026f1 <ideintr>
    lapiceoi();
80106c31:	e8 ab c2 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106c36:	e9 32 01 00 00       	jmp    80106d6d <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
80106c3b:	e8 7f c0 ff ff       	call   80102cbf <kbdintr>
    lapiceoi();
80106c40:	e8 9c c2 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106c45:	e9 23 01 00 00       	jmp    80106d6d <trap+0x1f6>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80106c4a:	e8 a9 03 00 00       	call   80106ff8 <uartintr>
    lapiceoi();
80106c4f:	e8 8d c2 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106c54:	e9 14 01 00 00       	jmp    80106d6d <trap+0x1f6>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
            cpu->id, tf->cs, tf->eip);
80106c59:	8b 45 08             	mov    0x8(%ebp),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c5c:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
80106c5f:	8b 45 08             	mov    0x8(%ebp),%eax
80106c62:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c66:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80106c69:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106c6f:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c72:	0f b6 c0             	movzbl %al,%eax
80106c75:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106c79:	89 54 24 08          	mov    %edx,0x8(%esp)
80106c7d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106c81:	c7 04 24 3c 8e 10 80 	movl   $0x80108e3c,(%esp)
80106c88:	e8 14 97 ff ff       	call   801003a1 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
80106c8d:	e8 4f c2 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106c92:	e9 d6 00 00 00       	jmp    80106d6d <trap+0x1f6>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
80106c97:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c9d:	85 c0                	test   %eax,%eax
80106c9f:	74 11                	je     80106cb2 <trap+0x13b>
80106ca1:	8b 45 08             	mov    0x8(%ebp),%eax
80106ca4:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106ca8:	0f b7 c0             	movzwl %ax,%eax
80106cab:	83 e0 03             	and    $0x3,%eax
80106cae:	85 c0                	test   %eax,%eax
80106cb0:	75 46                	jne    80106cf8 <trap+0x181>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106cb2:	e8 1a fd ff ff       	call   801069d1 <rcr2>
              tf->trapno, cpu->id, tf->eip, rcr2());
80106cb7:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106cba:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106cbd:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80106cc4:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106cc7:	0f b6 ca             	movzbl %dl,%ecx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106cca:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106ccd:	8b 52 30             	mov    0x30(%edx),%edx
80106cd0:	89 44 24 10          	mov    %eax,0x10(%esp)
80106cd4:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106cd8:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106cdc:	89 54 24 04          	mov    %edx,0x4(%esp)
80106ce0:	c7 04 24 60 8e 10 80 	movl   $0x80108e60,(%esp)
80106ce7:	e8 b5 96 ff ff       	call   801003a1 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80106cec:	c7 04 24 92 8e 10 80 	movl   $0x80108e92,(%esp)
80106cf3:	e8 45 98 ff ff       	call   8010053d <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106cf8:	e8 d4 fc ff ff       	call   801069d1 <rcr2>
80106cfd:	89 c2                	mov    %eax,%edx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106cff:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d02:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d05:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106d0b:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d0e:	0f b6 f0             	movzbl %al,%esi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d11:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d14:	8b 58 34             	mov    0x34(%eax),%ebx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d17:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d1a:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d1d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d23:	83 c0 6c             	add    $0x6c,%eax
80106d26:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106d29:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d2f:	8b 40 10             	mov    0x10(%eax),%eax
80106d32:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106d36:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106d3a:	89 74 24 14          	mov    %esi,0x14(%esp)
80106d3e:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106d42:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106d46:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80106d49:	89 54 24 08          	mov    %edx,0x8(%esp)
80106d4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106d51:	c7 04 24 98 8e 10 80 	movl   $0x80108e98,(%esp)
80106d58:	e8 44 96 ff ff       	call   801003a1 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80106d5d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d63:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80106d6a:	eb 01                	jmp    80106d6d <trap+0x1f6>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80106d6c:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106d6d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d73:	85 c0                	test   %eax,%eax
80106d75:	74 24                	je     80106d9b <trap+0x224>
80106d77:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d7d:	8b 40 24             	mov    0x24(%eax),%eax
80106d80:	85 c0                	test   %eax,%eax
80106d82:	74 17                	je     80106d9b <trap+0x224>
80106d84:	8b 45 08             	mov    0x8(%ebp),%eax
80106d87:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106d8b:	0f b7 c0             	movzwl %ax,%eax
80106d8e:	83 e0 03             	and    $0x3,%eax
80106d91:	83 f8 03             	cmp    $0x3,%eax
80106d94:	75 05                	jne    80106d9b <trap+0x224>
    exit();
80106d96:	e8 9f db ff ff       	call   8010493a <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80106d9b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106da1:	85 c0                	test   %eax,%eax
80106da3:	74 1e                	je     80106dc3 <trap+0x24c>
80106da5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106dab:	8b 40 0c             	mov    0xc(%eax),%eax
80106dae:	83 f8 04             	cmp    $0x4,%eax
80106db1:	75 10                	jne    80106dc3 <trap+0x24c>
80106db3:	8b 45 08             	mov    0x8(%ebp),%eax
80106db6:	8b 40 30             	mov    0x30(%eax),%eax
80106db9:	83 f8 20             	cmp    $0x20,%eax
80106dbc:	75 05                	jne    80106dc3 <trap+0x24c>
    yield();
80106dbe:	e8 c4 df ff ff       	call   80104d87 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106dc3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106dc9:	85 c0                	test   %eax,%eax
80106dcb:	74 27                	je     80106df4 <trap+0x27d>
80106dcd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106dd3:	8b 40 24             	mov    0x24(%eax),%eax
80106dd6:	85 c0                	test   %eax,%eax
80106dd8:	74 1a                	je     80106df4 <trap+0x27d>
80106dda:	8b 45 08             	mov    0x8(%ebp),%eax
80106ddd:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106de1:	0f b7 c0             	movzwl %ax,%eax
80106de4:	83 e0 03             	and    $0x3,%eax
80106de7:	83 f8 03             	cmp    $0x3,%eax
80106dea:	75 08                	jne    80106df4 <trap+0x27d>
    exit();
80106dec:	e8 49 db ff ff       	call   8010493a <exit>
80106df1:	eb 01                	jmp    80106df4 <trap+0x27d>
      exit();
    proc->tf = tf;
    syscall();
    if(proc->killed)
      exit();
    return;
80106df3:	90                   	nop
    yield();

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
    exit();
}
80106df4:	83 c4 3c             	add    $0x3c,%esp
80106df7:	5b                   	pop    %ebx
80106df8:	5e                   	pop    %esi
80106df9:	5f                   	pop    %edi
80106dfa:	5d                   	pop    %ebp
80106dfb:	c3                   	ret    

80106dfc <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106dfc:	55                   	push   %ebp
80106dfd:	89 e5                	mov    %esp,%ebp
80106dff:	53                   	push   %ebx
80106e00:	83 ec 14             	sub    $0x14,%esp
80106e03:	8b 45 08             	mov    0x8(%ebp),%eax
80106e06:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106e0a:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80106e0e:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80106e12:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80106e16:	ec                   	in     (%dx),%al
80106e17:	89 c3                	mov    %eax,%ebx
80106e19:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80106e1c:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80106e20:	83 c4 14             	add    $0x14,%esp
80106e23:	5b                   	pop    %ebx
80106e24:	5d                   	pop    %ebp
80106e25:	c3                   	ret    

80106e26 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106e26:	55                   	push   %ebp
80106e27:	89 e5                	mov    %esp,%ebp
80106e29:	83 ec 08             	sub    $0x8,%esp
80106e2c:	8b 55 08             	mov    0x8(%ebp),%edx
80106e2f:	8b 45 0c             	mov    0xc(%ebp),%eax
80106e32:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106e36:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106e39:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106e3d:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106e41:	ee                   	out    %al,(%dx)
}
80106e42:	c9                   	leave  
80106e43:	c3                   	ret    

80106e44 <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106e44:	55                   	push   %ebp
80106e45:	89 e5                	mov    %esp,%ebp
80106e47:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106e4a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106e51:	00 
80106e52:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106e59:	e8 c8 ff ff ff       	call   80106e26 <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106e5e:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80106e65:	00 
80106e66:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106e6d:	e8 b4 ff ff ff       	call   80106e26 <outb>
  outb(COM1+0, 115200/9600);
80106e72:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80106e79:	00 
80106e7a:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106e81:	e8 a0 ff ff ff       	call   80106e26 <outb>
  outb(COM1+1, 0);
80106e86:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106e8d:	00 
80106e8e:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106e95:	e8 8c ff ff ff       	call   80106e26 <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80106e9a:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80106ea1:	00 
80106ea2:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106ea9:	e8 78 ff ff ff       	call   80106e26 <outb>
  outb(COM1+4, 0);
80106eae:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106eb5:	00 
80106eb6:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80106ebd:	e8 64 ff ff ff       	call   80106e26 <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106ec2:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106ec9:	00 
80106eca:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106ed1:	e8 50 ff ff ff       	call   80106e26 <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106ed6:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106edd:	e8 1a ff ff ff       	call   80106dfc <inb>
80106ee2:	3c ff                	cmp    $0xff,%al
80106ee4:	74 6c                	je     80106f52 <uartinit+0x10e>
    return;
  uart = 1;
80106ee6:	c7 05 50 c6 10 80 01 	movl   $0x1,0x8010c650
80106eed:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106ef0:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106ef7:	e8 00 ff ff ff       	call   80106dfc <inb>
  inb(COM1+0);
80106efc:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106f03:	e8 f4 fe ff ff       	call   80106dfc <inb>
  picenable(IRQ_COM1);
80106f08:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106f0f:	e8 a5 cb ff ff       	call   80103ab9 <picenable>
  ioapicenable(IRQ_COM1, 0);
80106f14:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106f1b:	00 
80106f1c:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106f23:	e8 46 ba ff ff       	call   8010296e <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106f28:	c7 45 f4 5c 8f 10 80 	movl   $0x80108f5c,-0xc(%ebp)
80106f2f:	eb 15                	jmp    80106f46 <uartinit+0x102>
    uartputc(*p);
80106f31:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f34:	0f b6 00             	movzbl (%eax),%eax
80106f37:	0f be c0             	movsbl %al,%eax
80106f3a:	89 04 24             	mov    %eax,(%esp)
80106f3d:	e8 13 00 00 00       	call   80106f55 <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106f42:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106f46:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f49:	0f b6 00             	movzbl (%eax),%eax
80106f4c:	84 c0                	test   %al,%al
80106f4e:	75 e1                	jne    80106f31 <uartinit+0xed>
80106f50:	eb 01                	jmp    80106f53 <uartinit+0x10f>
  outb(COM1+4, 0);
  outb(COM1+1, 0x01);    // Enable receive interrupts.

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
    return;
80106f52:	90                   	nop
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
    uartputc(*p);
}
80106f53:	c9                   	leave  
80106f54:	c3                   	ret    

80106f55 <uartputc>:

void
uartputc(int c)
{
80106f55:	55                   	push   %ebp
80106f56:	89 e5                	mov    %esp,%ebp
80106f58:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106f5b:	a1 50 c6 10 80       	mov    0x8010c650,%eax
80106f60:	85 c0                	test   %eax,%eax
80106f62:	74 4d                	je     80106fb1 <uartputc+0x5c>
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106f64:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106f6b:	eb 10                	jmp    80106f7d <uartputc+0x28>
    microdelay(10);
80106f6d:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80106f74:	e8 8d bf ff ff       	call   80102f06 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106f79:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106f7d:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106f81:	7f 16                	jg     80106f99 <uartputc+0x44>
80106f83:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106f8a:	e8 6d fe ff ff       	call   80106dfc <inb>
80106f8f:	0f b6 c0             	movzbl %al,%eax
80106f92:	83 e0 20             	and    $0x20,%eax
80106f95:	85 c0                	test   %eax,%eax
80106f97:	74 d4                	je     80106f6d <uartputc+0x18>
    microdelay(10);
  outb(COM1+0, c);
80106f99:	8b 45 08             	mov    0x8(%ebp),%eax
80106f9c:	0f b6 c0             	movzbl %al,%eax
80106f9f:	89 44 24 04          	mov    %eax,0x4(%esp)
80106fa3:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106faa:	e8 77 fe ff ff       	call   80106e26 <outb>
80106faf:	eb 01                	jmp    80106fb2 <uartputc+0x5d>
uartputc(int c)
{
  int i;

  if(!uart)
    return;
80106fb1:	90                   	nop
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
    microdelay(10);
  outb(COM1+0, c);
}
80106fb2:	c9                   	leave  
80106fb3:	c3                   	ret    

80106fb4 <uartgetc>:

static int
uartgetc(void)
{
80106fb4:	55                   	push   %ebp
80106fb5:	89 e5                	mov    %esp,%ebp
80106fb7:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106fba:	a1 50 c6 10 80       	mov    0x8010c650,%eax
80106fbf:	85 c0                	test   %eax,%eax
80106fc1:	75 07                	jne    80106fca <uartgetc+0x16>
    return -1;
80106fc3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106fc8:	eb 2c                	jmp    80106ff6 <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106fca:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106fd1:	e8 26 fe ff ff       	call   80106dfc <inb>
80106fd6:	0f b6 c0             	movzbl %al,%eax
80106fd9:	83 e0 01             	and    $0x1,%eax
80106fdc:	85 c0                	test   %eax,%eax
80106fde:	75 07                	jne    80106fe7 <uartgetc+0x33>
    return -1;
80106fe0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106fe5:	eb 0f                	jmp    80106ff6 <uartgetc+0x42>
  return inb(COM1+0);
80106fe7:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106fee:	e8 09 fe ff ff       	call   80106dfc <inb>
80106ff3:	0f b6 c0             	movzbl %al,%eax
}
80106ff6:	c9                   	leave  
80106ff7:	c3                   	ret    

80106ff8 <uartintr>:

void
uartintr(void)
{
80106ff8:	55                   	push   %ebp
80106ff9:	89 e5                	mov    %esp,%ebp
80106ffb:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80106ffe:	c7 04 24 b4 6f 10 80 	movl   $0x80106fb4,(%esp)
80107005:	e8 a3 97 ff ff       	call   801007ad <consoleintr>
}
8010700a:	c9                   	leave  
8010700b:	c3                   	ret    

8010700c <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
8010700c:	6a 00                	push   $0x0
  pushl $0
8010700e:	6a 00                	push   $0x0
  jmp alltraps
80107010:	e9 67 f9 ff ff       	jmp    8010697c <alltraps>

80107015 <vector1>:
.globl vector1
vector1:
  pushl $0
80107015:	6a 00                	push   $0x0
  pushl $1
80107017:	6a 01                	push   $0x1
  jmp alltraps
80107019:	e9 5e f9 ff ff       	jmp    8010697c <alltraps>

8010701e <vector2>:
.globl vector2
vector2:
  pushl $0
8010701e:	6a 00                	push   $0x0
  pushl $2
80107020:	6a 02                	push   $0x2
  jmp alltraps
80107022:	e9 55 f9 ff ff       	jmp    8010697c <alltraps>

80107027 <vector3>:
.globl vector3
vector3:
  pushl $0
80107027:	6a 00                	push   $0x0
  pushl $3
80107029:	6a 03                	push   $0x3
  jmp alltraps
8010702b:	e9 4c f9 ff ff       	jmp    8010697c <alltraps>

80107030 <vector4>:
.globl vector4
vector4:
  pushl $0
80107030:	6a 00                	push   $0x0
  pushl $4
80107032:	6a 04                	push   $0x4
  jmp alltraps
80107034:	e9 43 f9 ff ff       	jmp    8010697c <alltraps>

80107039 <vector5>:
.globl vector5
vector5:
  pushl $0
80107039:	6a 00                	push   $0x0
  pushl $5
8010703b:	6a 05                	push   $0x5
  jmp alltraps
8010703d:	e9 3a f9 ff ff       	jmp    8010697c <alltraps>

80107042 <vector6>:
.globl vector6
vector6:
  pushl $0
80107042:	6a 00                	push   $0x0
  pushl $6
80107044:	6a 06                	push   $0x6
  jmp alltraps
80107046:	e9 31 f9 ff ff       	jmp    8010697c <alltraps>

8010704b <vector7>:
.globl vector7
vector7:
  pushl $0
8010704b:	6a 00                	push   $0x0
  pushl $7
8010704d:	6a 07                	push   $0x7
  jmp alltraps
8010704f:	e9 28 f9 ff ff       	jmp    8010697c <alltraps>

80107054 <vector8>:
.globl vector8
vector8:
  pushl $8
80107054:	6a 08                	push   $0x8
  jmp alltraps
80107056:	e9 21 f9 ff ff       	jmp    8010697c <alltraps>

8010705b <vector9>:
.globl vector9
vector9:
  pushl $0
8010705b:	6a 00                	push   $0x0
  pushl $9
8010705d:	6a 09                	push   $0x9
  jmp alltraps
8010705f:	e9 18 f9 ff ff       	jmp    8010697c <alltraps>

80107064 <vector10>:
.globl vector10
vector10:
  pushl $10
80107064:	6a 0a                	push   $0xa
  jmp alltraps
80107066:	e9 11 f9 ff ff       	jmp    8010697c <alltraps>

8010706b <vector11>:
.globl vector11
vector11:
  pushl $11
8010706b:	6a 0b                	push   $0xb
  jmp alltraps
8010706d:	e9 0a f9 ff ff       	jmp    8010697c <alltraps>

80107072 <vector12>:
.globl vector12
vector12:
  pushl $12
80107072:	6a 0c                	push   $0xc
  jmp alltraps
80107074:	e9 03 f9 ff ff       	jmp    8010697c <alltraps>

80107079 <vector13>:
.globl vector13
vector13:
  pushl $13
80107079:	6a 0d                	push   $0xd
  jmp alltraps
8010707b:	e9 fc f8 ff ff       	jmp    8010697c <alltraps>

80107080 <vector14>:
.globl vector14
vector14:
  pushl $14
80107080:	6a 0e                	push   $0xe
  jmp alltraps
80107082:	e9 f5 f8 ff ff       	jmp    8010697c <alltraps>

80107087 <vector15>:
.globl vector15
vector15:
  pushl $0
80107087:	6a 00                	push   $0x0
  pushl $15
80107089:	6a 0f                	push   $0xf
  jmp alltraps
8010708b:	e9 ec f8 ff ff       	jmp    8010697c <alltraps>

80107090 <vector16>:
.globl vector16
vector16:
  pushl $0
80107090:	6a 00                	push   $0x0
  pushl $16
80107092:	6a 10                	push   $0x10
  jmp alltraps
80107094:	e9 e3 f8 ff ff       	jmp    8010697c <alltraps>

80107099 <vector17>:
.globl vector17
vector17:
  pushl $17
80107099:	6a 11                	push   $0x11
  jmp alltraps
8010709b:	e9 dc f8 ff ff       	jmp    8010697c <alltraps>

801070a0 <vector18>:
.globl vector18
vector18:
  pushl $0
801070a0:	6a 00                	push   $0x0
  pushl $18
801070a2:	6a 12                	push   $0x12
  jmp alltraps
801070a4:	e9 d3 f8 ff ff       	jmp    8010697c <alltraps>

801070a9 <vector19>:
.globl vector19
vector19:
  pushl $0
801070a9:	6a 00                	push   $0x0
  pushl $19
801070ab:	6a 13                	push   $0x13
  jmp alltraps
801070ad:	e9 ca f8 ff ff       	jmp    8010697c <alltraps>

801070b2 <vector20>:
.globl vector20
vector20:
  pushl $0
801070b2:	6a 00                	push   $0x0
  pushl $20
801070b4:	6a 14                	push   $0x14
  jmp alltraps
801070b6:	e9 c1 f8 ff ff       	jmp    8010697c <alltraps>

801070bb <vector21>:
.globl vector21
vector21:
  pushl $0
801070bb:	6a 00                	push   $0x0
  pushl $21
801070bd:	6a 15                	push   $0x15
  jmp alltraps
801070bf:	e9 b8 f8 ff ff       	jmp    8010697c <alltraps>

801070c4 <vector22>:
.globl vector22
vector22:
  pushl $0
801070c4:	6a 00                	push   $0x0
  pushl $22
801070c6:	6a 16                	push   $0x16
  jmp alltraps
801070c8:	e9 af f8 ff ff       	jmp    8010697c <alltraps>

801070cd <vector23>:
.globl vector23
vector23:
  pushl $0
801070cd:	6a 00                	push   $0x0
  pushl $23
801070cf:	6a 17                	push   $0x17
  jmp alltraps
801070d1:	e9 a6 f8 ff ff       	jmp    8010697c <alltraps>

801070d6 <vector24>:
.globl vector24
vector24:
  pushl $0
801070d6:	6a 00                	push   $0x0
  pushl $24
801070d8:	6a 18                	push   $0x18
  jmp alltraps
801070da:	e9 9d f8 ff ff       	jmp    8010697c <alltraps>

801070df <vector25>:
.globl vector25
vector25:
  pushl $0
801070df:	6a 00                	push   $0x0
  pushl $25
801070e1:	6a 19                	push   $0x19
  jmp alltraps
801070e3:	e9 94 f8 ff ff       	jmp    8010697c <alltraps>

801070e8 <vector26>:
.globl vector26
vector26:
  pushl $0
801070e8:	6a 00                	push   $0x0
  pushl $26
801070ea:	6a 1a                	push   $0x1a
  jmp alltraps
801070ec:	e9 8b f8 ff ff       	jmp    8010697c <alltraps>

801070f1 <vector27>:
.globl vector27
vector27:
  pushl $0
801070f1:	6a 00                	push   $0x0
  pushl $27
801070f3:	6a 1b                	push   $0x1b
  jmp alltraps
801070f5:	e9 82 f8 ff ff       	jmp    8010697c <alltraps>

801070fa <vector28>:
.globl vector28
vector28:
  pushl $0
801070fa:	6a 00                	push   $0x0
  pushl $28
801070fc:	6a 1c                	push   $0x1c
  jmp alltraps
801070fe:	e9 79 f8 ff ff       	jmp    8010697c <alltraps>

80107103 <vector29>:
.globl vector29
vector29:
  pushl $0
80107103:	6a 00                	push   $0x0
  pushl $29
80107105:	6a 1d                	push   $0x1d
  jmp alltraps
80107107:	e9 70 f8 ff ff       	jmp    8010697c <alltraps>

8010710c <vector30>:
.globl vector30
vector30:
  pushl $0
8010710c:	6a 00                	push   $0x0
  pushl $30
8010710e:	6a 1e                	push   $0x1e
  jmp alltraps
80107110:	e9 67 f8 ff ff       	jmp    8010697c <alltraps>

80107115 <vector31>:
.globl vector31
vector31:
  pushl $0
80107115:	6a 00                	push   $0x0
  pushl $31
80107117:	6a 1f                	push   $0x1f
  jmp alltraps
80107119:	e9 5e f8 ff ff       	jmp    8010697c <alltraps>

8010711e <vector32>:
.globl vector32
vector32:
  pushl $0
8010711e:	6a 00                	push   $0x0
  pushl $32
80107120:	6a 20                	push   $0x20
  jmp alltraps
80107122:	e9 55 f8 ff ff       	jmp    8010697c <alltraps>

80107127 <vector33>:
.globl vector33
vector33:
  pushl $0
80107127:	6a 00                	push   $0x0
  pushl $33
80107129:	6a 21                	push   $0x21
  jmp alltraps
8010712b:	e9 4c f8 ff ff       	jmp    8010697c <alltraps>

80107130 <vector34>:
.globl vector34
vector34:
  pushl $0
80107130:	6a 00                	push   $0x0
  pushl $34
80107132:	6a 22                	push   $0x22
  jmp alltraps
80107134:	e9 43 f8 ff ff       	jmp    8010697c <alltraps>

80107139 <vector35>:
.globl vector35
vector35:
  pushl $0
80107139:	6a 00                	push   $0x0
  pushl $35
8010713b:	6a 23                	push   $0x23
  jmp alltraps
8010713d:	e9 3a f8 ff ff       	jmp    8010697c <alltraps>

80107142 <vector36>:
.globl vector36
vector36:
  pushl $0
80107142:	6a 00                	push   $0x0
  pushl $36
80107144:	6a 24                	push   $0x24
  jmp alltraps
80107146:	e9 31 f8 ff ff       	jmp    8010697c <alltraps>

8010714b <vector37>:
.globl vector37
vector37:
  pushl $0
8010714b:	6a 00                	push   $0x0
  pushl $37
8010714d:	6a 25                	push   $0x25
  jmp alltraps
8010714f:	e9 28 f8 ff ff       	jmp    8010697c <alltraps>

80107154 <vector38>:
.globl vector38
vector38:
  pushl $0
80107154:	6a 00                	push   $0x0
  pushl $38
80107156:	6a 26                	push   $0x26
  jmp alltraps
80107158:	e9 1f f8 ff ff       	jmp    8010697c <alltraps>

8010715d <vector39>:
.globl vector39
vector39:
  pushl $0
8010715d:	6a 00                	push   $0x0
  pushl $39
8010715f:	6a 27                	push   $0x27
  jmp alltraps
80107161:	e9 16 f8 ff ff       	jmp    8010697c <alltraps>

80107166 <vector40>:
.globl vector40
vector40:
  pushl $0
80107166:	6a 00                	push   $0x0
  pushl $40
80107168:	6a 28                	push   $0x28
  jmp alltraps
8010716a:	e9 0d f8 ff ff       	jmp    8010697c <alltraps>

8010716f <vector41>:
.globl vector41
vector41:
  pushl $0
8010716f:	6a 00                	push   $0x0
  pushl $41
80107171:	6a 29                	push   $0x29
  jmp alltraps
80107173:	e9 04 f8 ff ff       	jmp    8010697c <alltraps>

80107178 <vector42>:
.globl vector42
vector42:
  pushl $0
80107178:	6a 00                	push   $0x0
  pushl $42
8010717a:	6a 2a                	push   $0x2a
  jmp alltraps
8010717c:	e9 fb f7 ff ff       	jmp    8010697c <alltraps>

80107181 <vector43>:
.globl vector43
vector43:
  pushl $0
80107181:	6a 00                	push   $0x0
  pushl $43
80107183:	6a 2b                	push   $0x2b
  jmp alltraps
80107185:	e9 f2 f7 ff ff       	jmp    8010697c <alltraps>

8010718a <vector44>:
.globl vector44
vector44:
  pushl $0
8010718a:	6a 00                	push   $0x0
  pushl $44
8010718c:	6a 2c                	push   $0x2c
  jmp alltraps
8010718e:	e9 e9 f7 ff ff       	jmp    8010697c <alltraps>

80107193 <vector45>:
.globl vector45
vector45:
  pushl $0
80107193:	6a 00                	push   $0x0
  pushl $45
80107195:	6a 2d                	push   $0x2d
  jmp alltraps
80107197:	e9 e0 f7 ff ff       	jmp    8010697c <alltraps>

8010719c <vector46>:
.globl vector46
vector46:
  pushl $0
8010719c:	6a 00                	push   $0x0
  pushl $46
8010719e:	6a 2e                	push   $0x2e
  jmp alltraps
801071a0:	e9 d7 f7 ff ff       	jmp    8010697c <alltraps>

801071a5 <vector47>:
.globl vector47
vector47:
  pushl $0
801071a5:	6a 00                	push   $0x0
  pushl $47
801071a7:	6a 2f                	push   $0x2f
  jmp alltraps
801071a9:	e9 ce f7 ff ff       	jmp    8010697c <alltraps>

801071ae <vector48>:
.globl vector48
vector48:
  pushl $0
801071ae:	6a 00                	push   $0x0
  pushl $48
801071b0:	6a 30                	push   $0x30
  jmp alltraps
801071b2:	e9 c5 f7 ff ff       	jmp    8010697c <alltraps>

801071b7 <vector49>:
.globl vector49
vector49:
  pushl $0
801071b7:	6a 00                	push   $0x0
  pushl $49
801071b9:	6a 31                	push   $0x31
  jmp alltraps
801071bb:	e9 bc f7 ff ff       	jmp    8010697c <alltraps>

801071c0 <vector50>:
.globl vector50
vector50:
  pushl $0
801071c0:	6a 00                	push   $0x0
  pushl $50
801071c2:	6a 32                	push   $0x32
  jmp alltraps
801071c4:	e9 b3 f7 ff ff       	jmp    8010697c <alltraps>

801071c9 <vector51>:
.globl vector51
vector51:
  pushl $0
801071c9:	6a 00                	push   $0x0
  pushl $51
801071cb:	6a 33                	push   $0x33
  jmp alltraps
801071cd:	e9 aa f7 ff ff       	jmp    8010697c <alltraps>

801071d2 <vector52>:
.globl vector52
vector52:
  pushl $0
801071d2:	6a 00                	push   $0x0
  pushl $52
801071d4:	6a 34                	push   $0x34
  jmp alltraps
801071d6:	e9 a1 f7 ff ff       	jmp    8010697c <alltraps>

801071db <vector53>:
.globl vector53
vector53:
  pushl $0
801071db:	6a 00                	push   $0x0
  pushl $53
801071dd:	6a 35                	push   $0x35
  jmp alltraps
801071df:	e9 98 f7 ff ff       	jmp    8010697c <alltraps>

801071e4 <vector54>:
.globl vector54
vector54:
  pushl $0
801071e4:	6a 00                	push   $0x0
  pushl $54
801071e6:	6a 36                	push   $0x36
  jmp alltraps
801071e8:	e9 8f f7 ff ff       	jmp    8010697c <alltraps>

801071ed <vector55>:
.globl vector55
vector55:
  pushl $0
801071ed:	6a 00                	push   $0x0
  pushl $55
801071ef:	6a 37                	push   $0x37
  jmp alltraps
801071f1:	e9 86 f7 ff ff       	jmp    8010697c <alltraps>

801071f6 <vector56>:
.globl vector56
vector56:
  pushl $0
801071f6:	6a 00                	push   $0x0
  pushl $56
801071f8:	6a 38                	push   $0x38
  jmp alltraps
801071fa:	e9 7d f7 ff ff       	jmp    8010697c <alltraps>

801071ff <vector57>:
.globl vector57
vector57:
  pushl $0
801071ff:	6a 00                	push   $0x0
  pushl $57
80107201:	6a 39                	push   $0x39
  jmp alltraps
80107203:	e9 74 f7 ff ff       	jmp    8010697c <alltraps>

80107208 <vector58>:
.globl vector58
vector58:
  pushl $0
80107208:	6a 00                	push   $0x0
  pushl $58
8010720a:	6a 3a                	push   $0x3a
  jmp alltraps
8010720c:	e9 6b f7 ff ff       	jmp    8010697c <alltraps>

80107211 <vector59>:
.globl vector59
vector59:
  pushl $0
80107211:	6a 00                	push   $0x0
  pushl $59
80107213:	6a 3b                	push   $0x3b
  jmp alltraps
80107215:	e9 62 f7 ff ff       	jmp    8010697c <alltraps>

8010721a <vector60>:
.globl vector60
vector60:
  pushl $0
8010721a:	6a 00                	push   $0x0
  pushl $60
8010721c:	6a 3c                	push   $0x3c
  jmp alltraps
8010721e:	e9 59 f7 ff ff       	jmp    8010697c <alltraps>

80107223 <vector61>:
.globl vector61
vector61:
  pushl $0
80107223:	6a 00                	push   $0x0
  pushl $61
80107225:	6a 3d                	push   $0x3d
  jmp alltraps
80107227:	e9 50 f7 ff ff       	jmp    8010697c <alltraps>

8010722c <vector62>:
.globl vector62
vector62:
  pushl $0
8010722c:	6a 00                	push   $0x0
  pushl $62
8010722e:	6a 3e                	push   $0x3e
  jmp alltraps
80107230:	e9 47 f7 ff ff       	jmp    8010697c <alltraps>

80107235 <vector63>:
.globl vector63
vector63:
  pushl $0
80107235:	6a 00                	push   $0x0
  pushl $63
80107237:	6a 3f                	push   $0x3f
  jmp alltraps
80107239:	e9 3e f7 ff ff       	jmp    8010697c <alltraps>

8010723e <vector64>:
.globl vector64
vector64:
  pushl $0
8010723e:	6a 00                	push   $0x0
  pushl $64
80107240:	6a 40                	push   $0x40
  jmp alltraps
80107242:	e9 35 f7 ff ff       	jmp    8010697c <alltraps>

80107247 <vector65>:
.globl vector65
vector65:
  pushl $0
80107247:	6a 00                	push   $0x0
  pushl $65
80107249:	6a 41                	push   $0x41
  jmp alltraps
8010724b:	e9 2c f7 ff ff       	jmp    8010697c <alltraps>

80107250 <vector66>:
.globl vector66
vector66:
  pushl $0
80107250:	6a 00                	push   $0x0
  pushl $66
80107252:	6a 42                	push   $0x42
  jmp alltraps
80107254:	e9 23 f7 ff ff       	jmp    8010697c <alltraps>

80107259 <vector67>:
.globl vector67
vector67:
  pushl $0
80107259:	6a 00                	push   $0x0
  pushl $67
8010725b:	6a 43                	push   $0x43
  jmp alltraps
8010725d:	e9 1a f7 ff ff       	jmp    8010697c <alltraps>

80107262 <vector68>:
.globl vector68
vector68:
  pushl $0
80107262:	6a 00                	push   $0x0
  pushl $68
80107264:	6a 44                	push   $0x44
  jmp alltraps
80107266:	e9 11 f7 ff ff       	jmp    8010697c <alltraps>

8010726b <vector69>:
.globl vector69
vector69:
  pushl $0
8010726b:	6a 00                	push   $0x0
  pushl $69
8010726d:	6a 45                	push   $0x45
  jmp alltraps
8010726f:	e9 08 f7 ff ff       	jmp    8010697c <alltraps>

80107274 <vector70>:
.globl vector70
vector70:
  pushl $0
80107274:	6a 00                	push   $0x0
  pushl $70
80107276:	6a 46                	push   $0x46
  jmp alltraps
80107278:	e9 ff f6 ff ff       	jmp    8010697c <alltraps>

8010727d <vector71>:
.globl vector71
vector71:
  pushl $0
8010727d:	6a 00                	push   $0x0
  pushl $71
8010727f:	6a 47                	push   $0x47
  jmp alltraps
80107281:	e9 f6 f6 ff ff       	jmp    8010697c <alltraps>

80107286 <vector72>:
.globl vector72
vector72:
  pushl $0
80107286:	6a 00                	push   $0x0
  pushl $72
80107288:	6a 48                	push   $0x48
  jmp alltraps
8010728a:	e9 ed f6 ff ff       	jmp    8010697c <alltraps>

8010728f <vector73>:
.globl vector73
vector73:
  pushl $0
8010728f:	6a 00                	push   $0x0
  pushl $73
80107291:	6a 49                	push   $0x49
  jmp alltraps
80107293:	e9 e4 f6 ff ff       	jmp    8010697c <alltraps>

80107298 <vector74>:
.globl vector74
vector74:
  pushl $0
80107298:	6a 00                	push   $0x0
  pushl $74
8010729a:	6a 4a                	push   $0x4a
  jmp alltraps
8010729c:	e9 db f6 ff ff       	jmp    8010697c <alltraps>

801072a1 <vector75>:
.globl vector75
vector75:
  pushl $0
801072a1:	6a 00                	push   $0x0
  pushl $75
801072a3:	6a 4b                	push   $0x4b
  jmp alltraps
801072a5:	e9 d2 f6 ff ff       	jmp    8010697c <alltraps>

801072aa <vector76>:
.globl vector76
vector76:
  pushl $0
801072aa:	6a 00                	push   $0x0
  pushl $76
801072ac:	6a 4c                	push   $0x4c
  jmp alltraps
801072ae:	e9 c9 f6 ff ff       	jmp    8010697c <alltraps>

801072b3 <vector77>:
.globl vector77
vector77:
  pushl $0
801072b3:	6a 00                	push   $0x0
  pushl $77
801072b5:	6a 4d                	push   $0x4d
  jmp alltraps
801072b7:	e9 c0 f6 ff ff       	jmp    8010697c <alltraps>

801072bc <vector78>:
.globl vector78
vector78:
  pushl $0
801072bc:	6a 00                	push   $0x0
  pushl $78
801072be:	6a 4e                	push   $0x4e
  jmp alltraps
801072c0:	e9 b7 f6 ff ff       	jmp    8010697c <alltraps>

801072c5 <vector79>:
.globl vector79
vector79:
  pushl $0
801072c5:	6a 00                	push   $0x0
  pushl $79
801072c7:	6a 4f                	push   $0x4f
  jmp alltraps
801072c9:	e9 ae f6 ff ff       	jmp    8010697c <alltraps>

801072ce <vector80>:
.globl vector80
vector80:
  pushl $0
801072ce:	6a 00                	push   $0x0
  pushl $80
801072d0:	6a 50                	push   $0x50
  jmp alltraps
801072d2:	e9 a5 f6 ff ff       	jmp    8010697c <alltraps>

801072d7 <vector81>:
.globl vector81
vector81:
  pushl $0
801072d7:	6a 00                	push   $0x0
  pushl $81
801072d9:	6a 51                	push   $0x51
  jmp alltraps
801072db:	e9 9c f6 ff ff       	jmp    8010697c <alltraps>

801072e0 <vector82>:
.globl vector82
vector82:
  pushl $0
801072e0:	6a 00                	push   $0x0
  pushl $82
801072e2:	6a 52                	push   $0x52
  jmp alltraps
801072e4:	e9 93 f6 ff ff       	jmp    8010697c <alltraps>

801072e9 <vector83>:
.globl vector83
vector83:
  pushl $0
801072e9:	6a 00                	push   $0x0
  pushl $83
801072eb:	6a 53                	push   $0x53
  jmp alltraps
801072ed:	e9 8a f6 ff ff       	jmp    8010697c <alltraps>

801072f2 <vector84>:
.globl vector84
vector84:
  pushl $0
801072f2:	6a 00                	push   $0x0
  pushl $84
801072f4:	6a 54                	push   $0x54
  jmp alltraps
801072f6:	e9 81 f6 ff ff       	jmp    8010697c <alltraps>

801072fb <vector85>:
.globl vector85
vector85:
  pushl $0
801072fb:	6a 00                	push   $0x0
  pushl $85
801072fd:	6a 55                	push   $0x55
  jmp alltraps
801072ff:	e9 78 f6 ff ff       	jmp    8010697c <alltraps>

80107304 <vector86>:
.globl vector86
vector86:
  pushl $0
80107304:	6a 00                	push   $0x0
  pushl $86
80107306:	6a 56                	push   $0x56
  jmp alltraps
80107308:	e9 6f f6 ff ff       	jmp    8010697c <alltraps>

8010730d <vector87>:
.globl vector87
vector87:
  pushl $0
8010730d:	6a 00                	push   $0x0
  pushl $87
8010730f:	6a 57                	push   $0x57
  jmp alltraps
80107311:	e9 66 f6 ff ff       	jmp    8010697c <alltraps>

80107316 <vector88>:
.globl vector88
vector88:
  pushl $0
80107316:	6a 00                	push   $0x0
  pushl $88
80107318:	6a 58                	push   $0x58
  jmp alltraps
8010731a:	e9 5d f6 ff ff       	jmp    8010697c <alltraps>

8010731f <vector89>:
.globl vector89
vector89:
  pushl $0
8010731f:	6a 00                	push   $0x0
  pushl $89
80107321:	6a 59                	push   $0x59
  jmp alltraps
80107323:	e9 54 f6 ff ff       	jmp    8010697c <alltraps>

80107328 <vector90>:
.globl vector90
vector90:
  pushl $0
80107328:	6a 00                	push   $0x0
  pushl $90
8010732a:	6a 5a                	push   $0x5a
  jmp alltraps
8010732c:	e9 4b f6 ff ff       	jmp    8010697c <alltraps>

80107331 <vector91>:
.globl vector91
vector91:
  pushl $0
80107331:	6a 00                	push   $0x0
  pushl $91
80107333:	6a 5b                	push   $0x5b
  jmp alltraps
80107335:	e9 42 f6 ff ff       	jmp    8010697c <alltraps>

8010733a <vector92>:
.globl vector92
vector92:
  pushl $0
8010733a:	6a 00                	push   $0x0
  pushl $92
8010733c:	6a 5c                	push   $0x5c
  jmp alltraps
8010733e:	e9 39 f6 ff ff       	jmp    8010697c <alltraps>

80107343 <vector93>:
.globl vector93
vector93:
  pushl $0
80107343:	6a 00                	push   $0x0
  pushl $93
80107345:	6a 5d                	push   $0x5d
  jmp alltraps
80107347:	e9 30 f6 ff ff       	jmp    8010697c <alltraps>

8010734c <vector94>:
.globl vector94
vector94:
  pushl $0
8010734c:	6a 00                	push   $0x0
  pushl $94
8010734e:	6a 5e                	push   $0x5e
  jmp alltraps
80107350:	e9 27 f6 ff ff       	jmp    8010697c <alltraps>

80107355 <vector95>:
.globl vector95
vector95:
  pushl $0
80107355:	6a 00                	push   $0x0
  pushl $95
80107357:	6a 5f                	push   $0x5f
  jmp alltraps
80107359:	e9 1e f6 ff ff       	jmp    8010697c <alltraps>

8010735e <vector96>:
.globl vector96
vector96:
  pushl $0
8010735e:	6a 00                	push   $0x0
  pushl $96
80107360:	6a 60                	push   $0x60
  jmp alltraps
80107362:	e9 15 f6 ff ff       	jmp    8010697c <alltraps>

80107367 <vector97>:
.globl vector97
vector97:
  pushl $0
80107367:	6a 00                	push   $0x0
  pushl $97
80107369:	6a 61                	push   $0x61
  jmp alltraps
8010736b:	e9 0c f6 ff ff       	jmp    8010697c <alltraps>

80107370 <vector98>:
.globl vector98
vector98:
  pushl $0
80107370:	6a 00                	push   $0x0
  pushl $98
80107372:	6a 62                	push   $0x62
  jmp alltraps
80107374:	e9 03 f6 ff ff       	jmp    8010697c <alltraps>

80107379 <vector99>:
.globl vector99
vector99:
  pushl $0
80107379:	6a 00                	push   $0x0
  pushl $99
8010737b:	6a 63                	push   $0x63
  jmp alltraps
8010737d:	e9 fa f5 ff ff       	jmp    8010697c <alltraps>

80107382 <vector100>:
.globl vector100
vector100:
  pushl $0
80107382:	6a 00                	push   $0x0
  pushl $100
80107384:	6a 64                	push   $0x64
  jmp alltraps
80107386:	e9 f1 f5 ff ff       	jmp    8010697c <alltraps>

8010738b <vector101>:
.globl vector101
vector101:
  pushl $0
8010738b:	6a 00                	push   $0x0
  pushl $101
8010738d:	6a 65                	push   $0x65
  jmp alltraps
8010738f:	e9 e8 f5 ff ff       	jmp    8010697c <alltraps>

80107394 <vector102>:
.globl vector102
vector102:
  pushl $0
80107394:	6a 00                	push   $0x0
  pushl $102
80107396:	6a 66                	push   $0x66
  jmp alltraps
80107398:	e9 df f5 ff ff       	jmp    8010697c <alltraps>

8010739d <vector103>:
.globl vector103
vector103:
  pushl $0
8010739d:	6a 00                	push   $0x0
  pushl $103
8010739f:	6a 67                	push   $0x67
  jmp alltraps
801073a1:	e9 d6 f5 ff ff       	jmp    8010697c <alltraps>

801073a6 <vector104>:
.globl vector104
vector104:
  pushl $0
801073a6:	6a 00                	push   $0x0
  pushl $104
801073a8:	6a 68                	push   $0x68
  jmp alltraps
801073aa:	e9 cd f5 ff ff       	jmp    8010697c <alltraps>

801073af <vector105>:
.globl vector105
vector105:
  pushl $0
801073af:	6a 00                	push   $0x0
  pushl $105
801073b1:	6a 69                	push   $0x69
  jmp alltraps
801073b3:	e9 c4 f5 ff ff       	jmp    8010697c <alltraps>

801073b8 <vector106>:
.globl vector106
vector106:
  pushl $0
801073b8:	6a 00                	push   $0x0
  pushl $106
801073ba:	6a 6a                	push   $0x6a
  jmp alltraps
801073bc:	e9 bb f5 ff ff       	jmp    8010697c <alltraps>

801073c1 <vector107>:
.globl vector107
vector107:
  pushl $0
801073c1:	6a 00                	push   $0x0
  pushl $107
801073c3:	6a 6b                	push   $0x6b
  jmp alltraps
801073c5:	e9 b2 f5 ff ff       	jmp    8010697c <alltraps>

801073ca <vector108>:
.globl vector108
vector108:
  pushl $0
801073ca:	6a 00                	push   $0x0
  pushl $108
801073cc:	6a 6c                	push   $0x6c
  jmp alltraps
801073ce:	e9 a9 f5 ff ff       	jmp    8010697c <alltraps>

801073d3 <vector109>:
.globl vector109
vector109:
  pushl $0
801073d3:	6a 00                	push   $0x0
  pushl $109
801073d5:	6a 6d                	push   $0x6d
  jmp alltraps
801073d7:	e9 a0 f5 ff ff       	jmp    8010697c <alltraps>

801073dc <vector110>:
.globl vector110
vector110:
  pushl $0
801073dc:	6a 00                	push   $0x0
  pushl $110
801073de:	6a 6e                	push   $0x6e
  jmp alltraps
801073e0:	e9 97 f5 ff ff       	jmp    8010697c <alltraps>

801073e5 <vector111>:
.globl vector111
vector111:
  pushl $0
801073e5:	6a 00                	push   $0x0
  pushl $111
801073e7:	6a 6f                	push   $0x6f
  jmp alltraps
801073e9:	e9 8e f5 ff ff       	jmp    8010697c <alltraps>

801073ee <vector112>:
.globl vector112
vector112:
  pushl $0
801073ee:	6a 00                	push   $0x0
  pushl $112
801073f0:	6a 70                	push   $0x70
  jmp alltraps
801073f2:	e9 85 f5 ff ff       	jmp    8010697c <alltraps>

801073f7 <vector113>:
.globl vector113
vector113:
  pushl $0
801073f7:	6a 00                	push   $0x0
  pushl $113
801073f9:	6a 71                	push   $0x71
  jmp alltraps
801073fb:	e9 7c f5 ff ff       	jmp    8010697c <alltraps>

80107400 <vector114>:
.globl vector114
vector114:
  pushl $0
80107400:	6a 00                	push   $0x0
  pushl $114
80107402:	6a 72                	push   $0x72
  jmp alltraps
80107404:	e9 73 f5 ff ff       	jmp    8010697c <alltraps>

80107409 <vector115>:
.globl vector115
vector115:
  pushl $0
80107409:	6a 00                	push   $0x0
  pushl $115
8010740b:	6a 73                	push   $0x73
  jmp alltraps
8010740d:	e9 6a f5 ff ff       	jmp    8010697c <alltraps>

80107412 <vector116>:
.globl vector116
vector116:
  pushl $0
80107412:	6a 00                	push   $0x0
  pushl $116
80107414:	6a 74                	push   $0x74
  jmp alltraps
80107416:	e9 61 f5 ff ff       	jmp    8010697c <alltraps>

8010741b <vector117>:
.globl vector117
vector117:
  pushl $0
8010741b:	6a 00                	push   $0x0
  pushl $117
8010741d:	6a 75                	push   $0x75
  jmp alltraps
8010741f:	e9 58 f5 ff ff       	jmp    8010697c <alltraps>

80107424 <vector118>:
.globl vector118
vector118:
  pushl $0
80107424:	6a 00                	push   $0x0
  pushl $118
80107426:	6a 76                	push   $0x76
  jmp alltraps
80107428:	e9 4f f5 ff ff       	jmp    8010697c <alltraps>

8010742d <vector119>:
.globl vector119
vector119:
  pushl $0
8010742d:	6a 00                	push   $0x0
  pushl $119
8010742f:	6a 77                	push   $0x77
  jmp alltraps
80107431:	e9 46 f5 ff ff       	jmp    8010697c <alltraps>

80107436 <vector120>:
.globl vector120
vector120:
  pushl $0
80107436:	6a 00                	push   $0x0
  pushl $120
80107438:	6a 78                	push   $0x78
  jmp alltraps
8010743a:	e9 3d f5 ff ff       	jmp    8010697c <alltraps>

8010743f <vector121>:
.globl vector121
vector121:
  pushl $0
8010743f:	6a 00                	push   $0x0
  pushl $121
80107441:	6a 79                	push   $0x79
  jmp alltraps
80107443:	e9 34 f5 ff ff       	jmp    8010697c <alltraps>

80107448 <vector122>:
.globl vector122
vector122:
  pushl $0
80107448:	6a 00                	push   $0x0
  pushl $122
8010744a:	6a 7a                	push   $0x7a
  jmp alltraps
8010744c:	e9 2b f5 ff ff       	jmp    8010697c <alltraps>

80107451 <vector123>:
.globl vector123
vector123:
  pushl $0
80107451:	6a 00                	push   $0x0
  pushl $123
80107453:	6a 7b                	push   $0x7b
  jmp alltraps
80107455:	e9 22 f5 ff ff       	jmp    8010697c <alltraps>

8010745a <vector124>:
.globl vector124
vector124:
  pushl $0
8010745a:	6a 00                	push   $0x0
  pushl $124
8010745c:	6a 7c                	push   $0x7c
  jmp alltraps
8010745e:	e9 19 f5 ff ff       	jmp    8010697c <alltraps>

80107463 <vector125>:
.globl vector125
vector125:
  pushl $0
80107463:	6a 00                	push   $0x0
  pushl $125
80107465:	6a 7d                	push   $0x7d
  jmp alltraps
80107467:	e9 10 f5 ff ff       	jmp    8010697c <alltraps>

8010746c <vector126>:
.globl vector126
vector126:
  pushl $0
8010746c:	6a 00                	push   $0x0
  pushl $126
8010746e:	6a 7e                	push   $0x7e
  jmp alltraps
80107470:	e9 07 f5 ff ff       	jmp    8010697c <alltraps>

80107475 <vector127>:
.globl vector127
vector127:
  pushl $0
80107475:	6a 00                	push   $0x0
  pushl $127
80107477:	6a 7f                	push   $0x7f
  jmp alltraps
80107479:	e9 fe f4 ff ff       	jmp    8010697c <alltraps>

8010747e <vector128>:
.globl vector128
vector128:
  pushl $0
8010747e:	6a 00                	push   $0x0
  pushl $128
80107480:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80107485:	e9 f2 f4 ff ff       	jmp    8010697c <alltraps>

8010748a <vector129>:
.globl vector129
vector129:
  pushl $0
8010748a:	6a 00                	push   $0x0
  pushl $129
8010748c:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80107491:	e9 e6 f4 ff ff       	jmp    8010697c <alltraps>

80107496 <vector130>:
.globl vector130
vector130:
  pushl $0
80107496:	6a 00                	push   $0x0
  pushl $130
80107498:	68 82 00 00 00       	push   $0x82
  jmp alltraps
8010749d:	e9 da f4 ff ff       	jmp    8010697c <alltraps>

801074a2 <vector131>:
.globl vector131
vector131:
  pushl $0
801074a2:	6a 00                	push   $0x0
  pushl $131
801074a4:	68 83 00 00 00       	push   $0x83
  jmp alltraps
801074a9:	e9 ce f4 ff ff       	jmp    8010697c <alltraps>

801074ae <vector132>:
.globl vector132
vector132:
  pushl $0
801074ae:	6a 00                	push   $0x0
  pushl $132
801074b0:	68 84 00 00 00       	push   $0x84
  jmp alltraps
801074b5:	e9 c2 f4 ff ff       	jmp    8010697c <alltraps>

801074ba <vector133>:
.globl vector133
vector133:
  pushl $0
801074ba:	6a 00                	push   $0x0
  pushl $133
801074bc:	68 85 00 00 00       	push   $0x85
  jmp alltraps
801074c1:	e9 b6 f4 ff ff       	jmp    8010697c <alltraps>

801074c6 <vector134>:
.globl vector134
vector134:
  pushl $0
801074c6:	6a 00                	push   $0x0
  pushl $134
801074c8:	68 86 00 00 00       	push   $0x86
  jmp alltraps
801074cd:	e9 aa f4 ff ff       	jmp    8010697c <alltraps>

801074d2 <vector135>:
.globl vector135
vector135:
  pushl $0
801074d2:	6a 00                	push   $0x0
  pushl $135
801074d4:	68 87 00 00 00       	push   $0x87
  jmp alltraps
801074d9:	e9 9e f4 ff ff       	jmp    8010697c <alltraps>

801074de <vector136>:
.globl vector136
vector136:
  pushl $0
801074de:	6a 00                	push   $0x0
  pushl $136
801074e0:	68 88 00 00 00       	push   $0x88
  jmp alltraps
801074e5:	e9 92 f4 ff ff       	jmp    8010697c <alltraps>

801074ea <vector137>:
.globl vector137
vector137:
  pushl $0
801074ea:	6a 00                	push   $0x0
  pushl $137
801074ec:	68 89 00 00 00       	push   $0x89
  jmp alltraps
801074f1:	e9 86 f4 ff ff       	jmp    8010697c <alltraps>

801074f6 <vector138>:
.globl vector138
vector138:
  pushl $0
801074f6:	6a 00                	push   $0x0
  pushl $138
801074f8:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
801074fd:	e9 7a f4 ff ff       	jmp    8010697c <alltraps>

80107502 <vector139>:
.globl vector139
vector139:
  pushl $0
80107502:	6a 00                	push   $0x0
  pushl $139
80107504:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80107509:	e9 6e f4 ff ff       	jmp    8010697c <alltraps>

8010750e <vector140>:
.globl vector140
vector140:
  pushl $0
8010750e:	6a 00                	push   $0x0
  pushl $140
80107510:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80107515:	e9 62 f4 ff ff       	jmp    8010697c <alltraps>

8010751a <vector141>:
.globl vector141
vector141:
  pushl $0
8010751a:	6a 00                	push   $0x0
  pushl $141
8010751c:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80107521:	e9 56 f4 ff ff       	jmp    8010697c <alltraps>

80107526 <vector142>:
.globl vector142
vector142:
  pushl $0
80107526:	6a 00                	push   $0x0
  pushl $142
80107528:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
8010752d:	e9 4a f4 ff ff       	jmp    8010697c <alltraps>

80107532 <vector143>:
.globl vector143
vector143:
  pushl $0
80107532:	6a 00                	push   $0x0
  pushl $143
80107534:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80107539:	e9 3e f4 ff ff       	jmp    8010697c <alltraps>

8010753e <vector144>:
.globl vector144
vector144:
  pushl $0
8010753e:	6a 00                	push   $0x0
  pushl $144
80107540:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80107545:	e9 32 f4 ff ff       	jmp    8010697c <alltraps>

8010754a <vector145>:
.globl vector145
vector145:
  pushl $0
8010754a:	6a 00                	push   $0x0
  pushl $145
8010754c:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80107551:	e9 26 f4 ff ff       	jmp    8010697c <alltraps>

80107556 <vector146>:
.globl vector146
vector146:
  pushl $0
80107556:	6a 00                	push   $0x0
  pushl $146
80107558:	68 92 00 00 00       	push   $0x92
  jmp alltraps
8010755d:	e9 1a f4 ff ff       	jmp    8010697c <alltraps>

80107562 <vector147>:
.globl vector147
vector147:
  pushl $0
80107562:	6a 00                	push   $0x0
  pushl $147
80107564:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80107569:	e9 0e f4 ff ff       	jmp    8010697c <alltraps>

8010756e <vector148>:
.globl vector148
vector148:
  pushl $0
8010756e:	6a 00                	push   $0x0
  pushl $148
80107570:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80107575:	e9 02 f4 ff ff       	jmp    8010697c <alltraps>

8010757a <vector149>:
.globl vector149
vector149:
  pushl $0
8010757a:	6a 00                	push   $0x0
  pushl $149
8010757c:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80107581:	e9 f6 f3 ff ff       	jmp    8010697c <alltraps>

80107586 <vector150>:
.globl vector150
vector150:
  pushl $0
80107586:	6a 00                	push   $0x0
  pushl $150
80107588:	68 96 00 00 00       	push   $0x96
  jmp alltraps
8010758d:	e9 ea f3 ff ff       	jmp    8010697c <alltraps>

80107592 <vector151>:
.globl vector151
vector151:
  pushl $0
80107592:	6a 00                	push   $0x0
  pushl $151
80107594:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80107599:	e9 de f3 ff ff       	jmp    8010697c <alltraps>

8010759e <vector152>:
.globl vector152
vector152:
  pushl $0
8010759e:	6a 00                	push   $0x0
  pushl $152
801075a0:	68 98 00 00 00       	push   $0x98
  jmp alltraps
801075a5:	e9 d2 f3 ff ff       	jmp    8010697c <alltraps>

801075aa <vector153>:
.globl vector153
vector153:
  pushl $0
801075aa:	6a 00                	push   $0x0
  pushl $153
801075ac:	68 99 00 00 00       	push   $0x99
  jmp alltraps
801075b1:	e9 c6 f3 ff ff       	jmp    8010697c <alltraps>

801075b6 <vector154>:
.globl vector154
vector154:
  pushl $0
801075b6:	6a 00                	push   $0x0
  pushl $154
801075b8:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
801075bd:	e9 ba f3 ff ff       	jmp    8010697c <alltraps>

801075c2 <vector155>:
.globl vector155
vector155:
  pushl $0
801075c2:	6a 00                	push   $0x0
  pushl $155
801075c4:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
801075c9:	e9 ae f3 ff ff       	jmp    8010697c <alltraps>

801075ce <vector156>:
.globl vector156
vector156:
  pushl $0
801075ce:	6a 00                	push   $0x0
  pushl $156
801075d0:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
801075d5:	e9 a2 f3 ff ff       	jmp    8010697c <alltraps>

801075da <vector157>:
.globl vector157
vector157:
  pushl $0
801075da:	6a 00                	push   $0x0
  pushl $157
801075dc:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
801075e1:	e9 96 f3 ff ff       	jmp    8010697c <alltraps>

801075e6 <vector158>:
.globl vector158
vector158:
  pushl $0
801075e6:	6a 00                	push   $0x0
  pushl $158
801075e8:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
801075ed:	e9 8a f3 ff ff       	jmp    8010697c <alltraps>

801075f2 <vector159>:
.globl vector159
vector159:
  pushl $0
801075f2:	6a 00                	push   $0x0
  pushl $159
801075f4:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
801075f9:	e9 7e f3 ff ff       	jmp    8010697c <alltraps>

801075fe <vector160>:
.globl vector160
vector160:
  pushl $0
801075fe:	6a 00                	push   $0x0
  pushl $160
80107600:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80107605:	e9 72 f3 ff ff       	jmp    8010697c <alltraps>

8010760a <vector161>:
.globl vector161
vector161:
  pushl $0
8010760a:	6a 00                	push   $0x0
  pushl $161
8010760c:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80107611:	e9 66 f3 ff ff       	jmp    8010697c <alltraps>

80107616 <vector162>:
.globl vector162
vector162:
  pushl $0
80107616:	6a 00                	push   $0x0
  pushl $162
80107618:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
8010761d:	e9 5a f3 ff ff       	jmp    8010697c <alltraps>

80107622 <vector163>:
.globl vector163
vector163:
  pushl $0
80107622:	6a 00                	push   $0x0
  pushl $163
80107624:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80107629:	e9 4e f3 ff ff       	jmp    8010697c <alltraps>

8010762e <vector164>:
.globl vector164
vector164:
  pushl $0
8010762e:	6a 00                	push   $0x0
  pushl $164
80107630:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80107635:	e9 42 f3 ff ff       	jmp    8010697c <alltraps>

8010763a <vector165>:
.globl vector165
vector165:
  pushl $0
8010763a:	6a 00                	push   $0x0
  pushl $165
8010763c:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80107641:	e9 36 f3 ff ff       	jmp    8010697c <alltraps>

80107646 <vector166>:
.globl vector166
vector166:
  pushl $0
80107646:	6a 00                	push   $0x0
  pushl $166
80107648:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
8010764d:	e9 2a f3 ff ff       	jmp    8010697c <alltraps>

80107652 <vector167>:
.globl vector167
vector167:
  pushl $0
80107652:	6a 00                	push   $0x0
  pushl $167
80107654:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80107659:	e9 1e f3 ff ff       	jmp    8010697c <alltraps>

8010765e <vector168>:
.globl vector168
vector168:
  pushl $0
8010765e:	6a 00                	push   $0x0
  pushl $168
80107660:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80107665:	e9 12 f3 ff ff       	jmp    8010697c <alltraps>

8010766a <vector169>:
.globl vector169
vector169:
  pushl $0
8010766a:	6a 00                	push   $0x0
  pushl $169
8010766c:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80107671:	e9 06 f3 ff ff       	jmp    8010697c <alltraps>

80107676 <vector170>:
.globl vector170
vector170:
  pushl $0
80107676:	6a 00                	push   $0x0
  pushl $170
80107678:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
8010767d:	e9 fa f2 ff ff       	jmp    8010697c <alltraps>

80107682 <vector171>:
.globl vector171
vector171:
  pushl $0
80107682:	6a 00                	push   $0x0
  pushl $171
80107684:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
80107689:	e9 ee f2 ff ff       	jmp    8010697c <alltraps>

8010768e <vector172>:
.globl vector172
vector172:
  pushl $0
8010768e:	6a 00                	push   $0x0
  pushl $172
80107690:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
80107695:	e9 e2 f2 ff ff       	jmp    8010697c <alltraps>

8010769a <vector173>:
.globl vector173
vector173:
  pushl $0
8010769a:	6a 00                	push   $0x0
  pushl $173
8010769c:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
801076a1:	e9 d6 f2 ff ff       	jmp    8010697c <alltraps>

801076a6 <vector174>:
.globl vector174
vector174:
  pushl $0
801076a6:	6a 00                	push   $0x0
  pushl $174
801076a8:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
801076ad:	e9 ca f2 ff ff       	jmp    8010697c <alltraps>

801076b2 <vector175>:
.globl vector175
vector175:
  pushl $0
801076b2:	6a 00                	push   $0x0
  pushl $175
801076b4:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
801076b9:	e9 be f2 ff ff       	jmp    8010697c <alltraps>

801076be <vector176>:
.globl vector176
vector176:
  pushl $0
801076be:	6a 00                	push   $0x0
  pushl $176
801076c0:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
801076c5:	e9 b2 f2 ff ff       	jmp    8010697c <alltraps>

801076ca <vector177>:
.globl vector177
vector177:
  pushl $0
801076ca:	6a 00                	push   $0x0
  pushl $177
801076cc:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
801076d1:	e9 a6 f2 ff ff       	jmp    8010697c <alltraps>

801076d6 <vector178>:
.globl vector178
vector178:
  pushl $0
801076d6:	6a 00                	push   $0x0
  pushl $178
801076d8:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
801076dd:	e9 9a f2 ff ff       	jmp    8010697c <alltraps>

801076e2 <vector179>:
.globl vector179
vector179:
  pushl $0
801076e2:	6a 00                	push   $0x0
  pushl $179
801076e4:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
801076e9:	e9 8e f2 ff ff       	jmp    8010697c <alltraps>

801076ee <vector180>:
.globl vector180
vector180:
  pushl $0
801076ee:	6a 00                	push   $0x0
  pushl $180
801076f0:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
801076f5:	e9 82 f2 ff ff       	jmp    8010697c <alltraps>

801076fa <vector181>:
.globl vector181
vector181:
  pushl $0
801076fa:	6a 00                	push   $0x0
  pushl $181
801076fc:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80107701:	e9 76 f2 ff ff       	jmp    8010697c <alltraps>

80107706 <vector182>:
.globl vector182
vector182:
  pushl $0
80107706:	6a 00                	push   $0x0
  pushl $182
80107708:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
8010770d:	e9 6a f2 ff ff       	jmp    8010697c <alltraps>

80107712 <vector183>:
.globl vector183
vector183:
  pushl $0
80107712:	6a 00                	push   $0x0
  pushl $183
80107714:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107719:	e9 5e f2 ff ff       	jmp    8010697c <alltraps>

8010771e <vector184>:
.globl vector184
vector184:
  pushl $0
8010771e:	6a 00                	push   $0x0
  pushl $184
80107720:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
80107725:	e9 52 f2 ff ff       	jmp    8010697c <alltraps>

8010772a <vector185>:
.globl vector185
vector185:
  pushl $0
8010772a:	6a 00                	push   $0x0
  pushl $185
8010772c:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
80107731:	e9 46 f2 ff ff       	jmp    8010697c <alltraps>

80107736 <vector186>:
.globl vector186
vector186:
  pushl $0
80107736:	6a 00                	push   $0x0
  pushl $186
80107738:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
8010773d:	e9 3a f2 ff ff       	jmp    8010697c <alltraps>

80107742 <vector187>:
.globl vector187
vector187:
  pushl $0
80107742:	6a 00                	push   $0x0
  pushl $187
80107744:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80107749:	e9 2e f2 ff ff       	jmp    8010697c <alltraps>

8010774e <vector188>:
.globl vector188
vector188:
  pushl $0
8010774e:	6a 00                	push   $0x0
  pushl $188
80107750:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80107755:	e9 22 f2 ff ff       	jmp    8010697c <alltraps>

8010775a <vector189>:
.globl vector189
vector189:
  pushl $0
8010775a:	6a 00                	push   $0x0
  pushl $189
8010775c:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80107761:	e9 16 f2 ff ff       	jmp    8010697c <alltraps>

80107766 <vector190>:
.globl vector190
vector190:
  pushl $0
80107766:	6a 00                	push   $0x0
  pushl $190
80107768:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
8010776d:	e9 0a f2 ff ff       	jmp    8010697c <alltraps>

80107772 <vector191>:
.globl vector191
vector191:
  pushl $0
80107772:	6a 00                	push   $0x0
  pushl $191
80107774:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80107779:	e9 fe f1 ff ff       	jmp    8010697c <alltraps>

8010777e <vector192>:
.globl vector192
vector192:
  pushl $0
8010777e:	6a 00                	push   $0x0
  pushl $192
80107780:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
80107785:	e9 f2 f1 ff ff       	jmp    8010697c <alltraps>

8010778a <vector193>:
.globl vector193
vector193:
  pushl $0
8010778a:	6a 00                	push   $0x0
  pushl $193
8010778c:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
80107791:	e9 e6 f1 ff ff       	jmp    8010697c <alltraps>

80107796 <vector194>:
.globl vector194
vector194:
  pushl $0
80107796:	6a 00                	push   $0x0
  pushl $194
80107798:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
8010779d:	e9 da f1 ff ff       	jmp    8010697c <alltraps>

801077a2 <vector195>:
.globl vector195
vector195:
  pushl $0
801077a2:	6a 00                	push   $0x0
  pushl $195
801077a4:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
801077a9:	e9 ce f1 ff ff       	jmp    8010697c <alltraps>

801077ae <vector196>:
.globl vector196
vector196:
  pushl $0
801077ae:	6a 00                	push   $0x0
  pushl $196
801077b0:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
801077b5:	e9 c2 f1 ff ff       	jmp    8010697c <alltraps>

801077ba <vector197>:
.globl vector197
vector197:
  pushl $0
801077ba:	6a 00                	push   $0x0
  pushl $197
801077bc:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
801077c1:	e9 b6 f1 ff ff       	jmp    8010697c <alltraps>

801077c6 <vector198>:
.globl vector198
vector198:
  pushl $0
801077c6:	6a 00                	push   $0x0
  pushl $198
801077c8:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
801077cd:	e9 aa f1 ff ff       	jmp    8010697c <alltraps>

801077d2 <vector199>:
.globl vector199
vector199:
  pushl $0
801077d2:	6a 00                	push   $0x0
  pushl $199
801077d4:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
801077d9:	e9 9e f1 ff ff       	jmp    8010697c <alltraps>

801077de <vector200>:
.globl vector200
vector200:
  pushl $0
801077de:	6a 00                	push   $0x0
  pushl $200
801077e0:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
801077e5:	e9 92 f1 ff ff       	jmp    8010697c <alltraps>

801077ea <vector201>:
.globl vector201
vector201:
  pushl $0
801077ea:	6a 00                	push   $0x0
  pushl $201
801077ec:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
801077f1:	e9 86 f1 ff ff       	jmp    8010697c <alltraps>

801077f6 <vector202>:
.globl vector202
vector202:
  pushl $0
801077f6:	6a 00                	push   $0x0
  pushl $202
801077f8:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
801077fd:	e9 7a f1 ff ff       	jmp    8010697c <alltraps>

80107802 <vector203>:
.globl vector203
vector203:
  pushl $0
80107802:	6a 00                	push   $0x0
  pushl $203
80107804:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107809:	e9 6e f1 ff ff       	jmp    8010697c <alltraps>

8010780e <vector204>:
.globl vector204
vector204:
  pushl $0
8010780e:	6a 00                	push   $0x0
  pushl $204
80107810:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80107815:	e9 62 f1 ff ff       	jmp    8010697c <alltraps>

8010781a <vector205>:
.globl vector205
vector205:
  pushl $0
8010781a:	6a 00                	push   $0x0
  pushl $205
8010781c:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
80107821:	e9 56 f1 ff ff       	jmp    8010697c <alltraps>

80107826 <vector206>:
.globl vector206
vector206:
  pushl $0
80107826:	6a 00                	push   $0x0
  pushl $206
80107828:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
8010782d:	e9 4a f1 ff ff       	jmp    8010697c <alltraps>

80107832 <vector207>:
.globl vector207
vector207:
  pushl $0
80107832:	6a 00                	push   $0x0
  pushl $207
80107834:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80107839:	e9 3e f1 ff ff       	jmp    8010697c <alltraps>

8010783e <vector208>:
.globl vector208
vector208:
  pushl $0
8010783e:	6a 00                	push   $0x0
  pushl $208
80107840:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80107845:	e9 32 f1 ff ff       	jmp    8010697c <alltraps>

8010784a <vector209>:
.globl vector209
vector209:
  pushl $0
8010784a:	6a 00                	push   $0x0
  pushl $209
8010784c:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
80107851:	e9 26 f1 ff ff       	jmp    8010697c <alltraps>

80107856 <vector210>:
.globl vector210
vector210:
  pushl $0
80107856:	6a 00                	push   $0x0
  pushl $210
80107858:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
8010785d:	e9 1a f1 ff ff       	jmp    8010697c <alltraps>

80107862 <vector211>:
.globl vector211
vector211:
  pushl $0
80107862:	6a 00                	push   $0x0
  pushl $211
80107864:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80107869:	e9 0e f1 ff ff       	jmp    8010697c <alltraps>

8010786e <vector212>:
.globl vector212
vector212:
  pushl $0
8010786e:	6a 00                	push   $0x0
  pushl $212
80107870:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
80107875:	e9 02 f1 ff ff       	jmp    8010697c <alltraps>

8010787a <vector213>:
.globl vector213
vector213:
  pushl $0
8010787a:	6a 00                	push   $0x0
  pushl $213
8010787c:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
80107881:	e9 f6 f0 ff ff       	jmp    8010697c <alltraps>

80107886 <vector214>:
.globl vector214
vector214:
  pushl $0
80107886:	6a 00                	push   $0x0
  pushl $214
80107888:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
8010788d:	e9 ea f0 ff ff       	jmp    8010697c <alltraps>

80107892 <vector215>:
.globl vector215
vector215:
  pushl $0
80107892:	6a 00                	push   $0x0
  pushl $215
80107894:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
80107899:	e9 de f0 ff ff       	jmp    8010697c <alltraps>

8010789e <vector216>:
.globl vector216
vector216:
  pushl $0
8010789e:	6a 00                	push   $0x0
  pushl $216
801078a0:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
801078a5:	e9 d2 f0 ff ff       	jmp    8010697c <alltraps>

801078aa <vector217>:
.globl vector217
vector217:
  pushl $0
801078aa:	6a 00                	push   $0x0
  pushl $217
801078ac:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
801078b1:	e9 c6 f0 ff ff       	jmp    8010697c <alltraps>

801078b6 <vector218>:
.globl vector218
vector218:
  pushl $0
801078b6:	6a 00                	push   $0x0
  pushl $218
801078b8:	68 da 00 00 00       	push   $0xda
  jmp alltraps
801078bd:	e9 ba f0 ff ff       	jmp    8010697c <alltraps>

801078c2 <vector219>:
.globl vector219
vector219:
  pushl $0
801078c2:	6a 00                	push   $0x0
  pushl $219
801078c4:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
801078c9:	e9 ae f0 ff ff       	jmp    8010697c <alltraps>

801078ce <vector220>:
.globl vector220
vector220:
  pushl $0
801078ce:	6a 00                	push   $0x0
  pushl $220
801078d0:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
801078d5:	e9 a2 f0 ff ff       	jmp    8010697c <alltraps>

801078da <vector221>:
.globl vector221
vector221:
  pushl $0
801078da:	6a 00                	push   $0x0
  pushl $221
801078dc:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
801078e1:	e9 96 f0 ff ff       	jmp    8010697c <alltraps>

801078e6 <vector222>:
.globl vector222
vector222:
  pushl $0
801078e6:	6a 00                	push   $0x0
  pushl $222
801078e8:	68 de 00 00 00       	push   $0xde
  jmp alltraps
801078ed:	e9 8a f0 ff ff       	jmp    8010697c <alltraps>

801078f2 <vector223>:
.globl vector223
vector223:
  pushl $0
801078f2:	6a 00                	push   $0x0
  pushl $223
801078f4:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
801078f9:	e9 7e f0 ff ff       	jmp    8010697c <alltraps>

801078fe <vector224>:
.globl vector224
vector224:
  pushl $0
801078fe:	6a 00                	push   $0x0
  pushl $224
80107900:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
80107905:	e9 72 f0 ff ff       	jmp    8010697c <alltraps>

8010790a <vector225>:
.globl vector225
vector225:
  pushl $0
8010790a:	6a 00                	push   $0x0
  pushl $225
8010790c:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
80107911:	e9 66 f0 ff ff       	jmp    8010697c <alltraps>

80107916 <vector226>:
.globl vector226
vector226:
  pushl $0
80107916:	6a 00                	push   $0x0
  pushl $226
80107918:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
8010791d:	e9 5a f0 ff ff       	jmp    8010697c <alltraps>

80107922 <vector227>:
.globl vector227
vector227:
  pushl $0
80107922:	6a 00                	push   $0x0
  pushl $227
80107924:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
80107929:	e9 4e f0 ff ff       	jmp    8010697c <alltraps>

8010792e <vector228>:
.globl vector228
vector228:
  pushl $0
8010792e:	6a 00                	push   $0x0
  pushl $228
80107930:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
80107935:	e9 42 f0 ff ff       	jmp    8010697c <alltraps>

8010793a <vector229>:
.globl vector229
vector229:
  pushl $0
8010793a:	6a 00                	push   $0x0
  pushl $229
8010793c:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
80107941:	e9 36 f0 ff ff       	jmp    8010697c <alltraps>

80107946 <vector230>:
.globl vector230
vector230:
  pushl $0
80107946:	6a 00                	push   $0x0
  pushl $230
80107948:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
8010794d:	e9 2a f0 ff ff       	jmp    8010697c <alltraps>

80107952 <vector231>:
.globl vector231
vector231:
  pushl $0
80107952:	6a 00                	push   $0x0
  pushl $231
80107954:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
80107959:	e9 1e f0 ff ff       	jmp    8010697c <alltraps>

8010795e <vector232>:
.globl vector232
vector232:
  pushl $0
8010795e:	6a 00                	push   $0x0
  pushl $232
80107960:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
80107965:	e9 12 f0 ff ff       	jmp    8010697c <alltraps>

8010796a <vector233>:
.globl vector233
vector233:
  pushl $0
8010796a:	6a 00                	push   $0x0
  pushl $233
8010796c:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
80107971:	e9 06 f0 ff ff       	jmp    8010697c <alltraps>

80107976 <vector234>:
.globl vector234
vector234:
  pushl $0
80107976:	6a 00                	push   $0x0
  pushl $234
80107978:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
8010797d:	e9 fa ef ff ff       	jmp    8010697c <alltraps>

80107982 <vector235>:
.globl vector235
vector235:
  pushl $0
80107982:	6a 00                	push   $0x0
  pushl $235
80107984:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
80107989:	e9 ee ef ff ff       	jmp    8010697c <alltraps>

8010798e <vector236>:
.globl vector236
vector236:
  pushl $0
8010798e:	6a 00                	push   $0x0
  pushl $236
80107990:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
80107995:	e9 e2 ef ff ff       	jmp    8010697c <alltraps>

8010799a <vector237>:
.globl vector237
vector237:
  pushl $0
8010799a:	6a 00                	push   $0x0
  pushl $237
8010799c:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
801079a1:	e9 d6 ef ff ff       	jmp    8010697c <alltraps>

801079a6 <vector238>:
.globl vector238
vector238:
  pushl $0
801079a6:	6a 00                	push   $0x0
  pushl $238
801079a8:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
801079ad:	e9 ca ef ff ff       	jmp    8010697c <alltraps>

801079b2 <vector239>:
.globl vector239
vector239:
  pushl $0
801079b2:	6a 00                	push   $0x0
  pushl $239
801079b4:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
801079b9:	e9 be ef ff ff       	jmp    8010697c <alltraps>

801079be <vector240>:
.globl vector240
vector240:
  pushl $0
801079be:	6a 00                	push   $0x0
  pushl $240
801079c0:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
801079c5:	e9 b2 ef ff ff       	jmp    8010697c <alltraps>

801079ca <vector241>:
.globl vector241
vector241:
  pushl $0
801079ca:	6a 00                	push   $0x0
  pushl $241
801079cc:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
801079d1:	e9 a6 ef ff ff       	jmp    8010697c <alltraps>

801079d6 <vector242>:
.globl vector242
vector242:
  pushl $0
801079d6:	6a 00                	push   $0x0
  pushl $242
801079d8:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
801079dd:	e9 9a ef ff ff       	jmp    8010697c <alltraps>

801079e2 <vector243>:
.globl vector243
vector243:
  pushl $0
801079e2:	6a 00                	push   $0x0
  pushl $243
801079e4:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
801079e9:	e9 8e ef ff ff       	jmp    8010697c <alltraps>

801079ee <vector244>:
.globl vector244
vector244:
  pushl $0
801079ee:	6a 00                	push   $0x0
  pushl $244
801079f0:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
801079f5:	e9 82 ef ff ff       	jmp    8010697c <alltraps>

801079fa <vector245>:
.globl vector245
vector245:
  pushl $0
801079fa:	6a 00                	push   $0x0
  pushl $245
801079fc:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
80107a01:	e9 76 ef ff ff       	jmp    8010697c <alltraps>

80107a06 <vector246>:
.globl vector246
vector246:
  pushl $0
80107a06:	6a 00                	push   $0x0
  pushl $246
80107a08:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107a0d:	e9 6a ef ff ff       	jmp    8010697c <alltraps>

80107a12 <vector247>:
.globl vector247
vector247:
  pushl $0
80107a12:	6a 00                	push   $0x0
  pushl $247
80107a14:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107a19:	e9 5e ef ff ff       	jmp    8010697c <alltraps>

80107a1e <vector248>:
.globl vector248
vector248:
  pushl $0
80107a1e:	6a 00                	push   $0x0
  pushl $248
80107a20:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
80107a25:	e9 52 ef ff ff       	jmp    8010697c <alltraps>

80107a2a <vector249>:
.globl vector249
vector249:
  pushl $0
80107a2a:	6a 00                	push   $0x0
  pushl $249
80107a2c:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
80107a31:	e9 46 ef ff ff       	jmp    8010697c <alltraps>

80107a36 <vector250>:
.globl vector250
vector250:
  pushl $0
80107a36:	6a 00                	push   $0x0
  pushl $250
80107a38:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80107a3d:	e9 3a ef ff ff       	jmp    8010697c <alltraps>

80107a42 <vector251>:
.globl vector251
vector251:
  pushl $0
80107a42:	6a 00                	push   $0x0
  pushl $251
80107a44:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80107a49:	e9 2e ef ff ff       	jmp    8010697c <alltraps>

80107a4e <vector252>:
.globl vector252
vector252:
  pushl $0
80107a4e:	6a 00                	push   $0x0
  pushl $252
80107a50:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80107a55:	e9 22 ef ff ff       	jmp    8010697c <alltraps>

80107a5a <vector253>:
.globl vector253
vector253:
  pushl $0
80107a5a:	6a 00                	push   $0x0
  pushl $253
80107a5c:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
80107a61:	e9 16 ef ff ff       	jmp    8010697c <alltraps>

80107a66 <vector254>:
.globl vector254
vector254:
  pushl $0
80107a66:	6a 00                	push   $0x0
  pushl $254
80107a68:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80107a6d:	e9 0a ef ff ff       	jmp    8010697c <alltraps>

80107a72 <vector255>:
.globl vector255
vector255:
  pushl $0
80107a72:	6a 00                	push   $0x0
  pushl $255
80107a74:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
80107a79:	e9 fe ee ff ff       	jmp    8010697c <alltraps>
	...

80107a80 <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80107a80:	55                   	push   %ebp
80107a81:	89 e5                	mov    %esp,%ebp
80107a83:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80107a86:	8b 45 0c             	mov    0xc(%ebp),%eax
80107a89:	83 e8 01             	sub    $0x1,%eax
80107a8c:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80107a90:	8b 45 08             	mov    0x8(%ebp),%eax
80107a93:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80107a97:	8b 45 08             	mov    0x8(%ebp),%eax
80107a9a:	c1 e8 10             	shr    $0x10,%eax
80107a9d:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
80107aa1:	8d 45 fa             	lea    -0x6(%ebp),%eax
80107aa4:	0f 01 10             	lgdtl  (%eax)
}
80107aa7:	c9                   	leave  
80107aa8:	c3                   	ret    

80107aa9 <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
80107aa9:	55                   	push   %ebp
80107aaa:	89 e5                	mov    %esp,%ebp
80107aac:	83 ec 04             	sub    $0x4,%esp
80107aaf:	8b 45 08             	mov    0x8(%ebp),%eax
80107ab2:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
80107ab6:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107aba:	0f 00 d8             	ltr    %ax
}
80107abd:	c9                   	leave  
80107abe:	c3                   	ret    

80107abf <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
80107abf:	55                   	push   %ebp
80107ac0:	89 e5                	mov    %esp,%ebp
80107ac2:	83 ec 04             	sub    $0x4,%esp
80107ac5:	8b 45 08             	mov    0x8(%ebp),%eax
80107ac8:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
80107acc:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107ad0:	8e e8                	mov    %eax,%gs
}
80107ad2:	c9                   	leave  
80107ad3:	c3                   	ret    

80107ad4 <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
80107ad4:	55                   	push   %ebp
80107ad5:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107ad7:	8b 45 08             	mov    0x8(%ebp),%eax
80107ada:	0f 22 d8             	mov    %eax,%cr3
}
80107add:	5d                   	pop    %ebp
80107ade:	c3                   	ret    

80107adf <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
80107adf:	55                   	push   %ebp
80107ae0:	89 e5                	mov    %esp,%ebp
80107ae2:	8b 45 08             	mov    0x8(%ebp),%eax
80107ae5:	05 00 00 00 80       	add    $0x80000000,%eax
80107aea:	5d                   	pop    %ebp
80107aeb:	c3                   	ret    

80107aec <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80107aec:	55                   	push   %ebp
80107aed:	89 e5                	mov    %esp,%ebp
80107aef:	8b 45 08             	mov    0x8(%ebp),%eax
80107af2:	05 00 00 00 80       	add    $0x80000000,%eax
80107af7:	5d                   	pop    %ebp
80107af8:	c3                   	ret    

80107af9 <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
80107af9:	55                   	push   %ebp
80107afa:	89 e5                	mov    %esp,%ebp
80107afc:	53                   	push   %ebx
80107afd:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
80107b00:	e8 80 b3 ff ff       	call   80102e85 <cpunum>
80107b05:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80107b0b:	05 20 09 11 80       	add    $0x80110920,%eax
80107b10:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
80107b13:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b16:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
80107b1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b1f:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
80107b25:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b28:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
80107b2c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b2f:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b33:	83 e2 f0             	and    $0xfffffff0,%edx
80107b36:	83 ca 0a             	or     $0xa,%edx
80107b39:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b3c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b3f:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b43:	83 ca 10             	or     $0x10,%edx
80107b46:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b49:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b4c:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b50:	83 e2 9f             	and    $0xffffff9f,%edx
80107b53:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b56:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b59:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b5d:	83 ca 80             	or     $0xffffff80,%edx
80107b60:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b66:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b6a:	83 ca 0f             	or     $0xf,%edx
80107b6d:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b70:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b73:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b77:	83 e2 ef             	and    $0xffffffef,%edx
80107b7a:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b7d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b80:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b84:	83 e2 df             	and    $0xffffffdf,%edx
80107b87:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b8a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b8d:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b91:	83 ca 40             	or     $0x40,%edx
80107b94:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b97:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b9a:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b9e:	83 ca 80             	or     $0xffffff80,%edx
80107ba1:	88 50 7e             	mov    %dl,0x7e(%eax)
80107ba4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ba7:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80107bab:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bae:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
80107bb5:	ff ff 
80107bb7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bba:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
80107bc1:	00 00 
80107bc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bc6:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80107bcd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bd0:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107bd7:	83 e2 f0             	and    $0xfffffff0,%edx
80107bda:	83 ca 02             	or     $0x2,%edx
80107bdd:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107be3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be6:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107bed:	83 ca 10             	or     $0x10,%edx
80107bf0:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107bf6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bf9:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107c00:	83 e2 9f             	and    $0xffffff9f,%edx
80107c03:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c0c:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107c13:	83 ca 80             	or     $0xffffff80,%edx
80107c16:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c1f:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c26:	83 ca 0f             	or     $0xf,%edx
80107c29:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c32:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c39:	83 e2 ef             	and    $0xffffffef,%edx
80107c3c:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c42:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c45:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c4c:	83 e2 df             	and    $0xffffffdf,%edx
80107c4f:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c58:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c5f:	83 ca 40             	or     $0x40,%edx
80107c62:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c68:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c6b:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c72:	83 ca 80             	or     $0xffffff80,%edx
80107c75:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c7b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c7e:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80107c85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c88:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
80107c8f:	ff ff 
80107c91:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c94:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80107c9b:	00 00 
80107c9d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ca0:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80107ca7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107caa:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107cb1:	83 e2 f0             	and    $0xfffffff0,%edx
80107cb4:	83 ca 0a             	or     $0xa,%edx
80107cb7:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107cbd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cc0:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107cc7:	83 ca 10             	or     $0x10,%edx
80107cca:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107cd0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cd3:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107cda:	83 ca 60             	or     $0x60,%edx
80107cdd:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107ce3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ce6:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107ced:	83 ca 80             	or     $0xffffff80,%edx
80107cf0:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107cf6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cf9:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d00:	83 ca 0f             	or     $0xf,%edx
80107d03:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d0c:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d13:	83 e2 ef             	and    $0xffffffef,%edx
80107d16:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d1f:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d26:	83 e2 df             	and    $0xffffffdf,%edx
80107d29:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d32:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d39:	83 ca 40             	or     $0x40,%edx
80107d3c:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d42:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d45:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d4c:	83 ca 80             	or     $0xffffff80,%edx
80107d4f:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d58:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107d5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d62:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80107d69:	ff ff 
80107d6b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d6e:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80107d75:	00 00 
80107d77:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d7a:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80107d81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d84:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107d8b:	83 e2 f0             	and    $0xfffffff0,%edx
80107d8e:	83 ca 02             	or     $0x2,%edx
80107d91:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107d97:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d9a:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107da1:	83 ca 10             	or     $0x10,%edx
80107da4:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107daa:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dad:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107db4:	83 ca 60             	or     $0x60,%edx
80107db7:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107dbd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dc0:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107dc7:	83 ca 80             	or     $0xffffff80,%edx
80107dca:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107dd0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dd3:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107dda:	83 ca 0f             	or     $0xf,%edx
80107ddd:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107de3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107de6:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107ded:	83 e2 ef             	and    $0xffffffef,%edx
80107df0:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107df6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107df9:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e00:	83 e2 df             	and    $0xffffffdf,%edx
80107e03:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e0c:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e13:	83 ca 40             	or     $0x40,%edx
80107e16:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e1c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e1f:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e26:	83 ca 80             	or     $0xffffff80,%edx
80107e29:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e32:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107e39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e3c:	05 b4 00 00 00       	add    $0xb4,%eax
80107e41:	89 c3                	mov    %eax,%ebx
80107e43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e46:	05 b4 00 00 00       	add    $0xb4,%eax
80107e4b:	c1 e8 10             	shr    $0x10,%eax
80107e4e:	89 c1                	mov    %eax,%ecx
80107e50:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e53:	05 b4 00 00 00       	add    $0xb4,%eax
80107e58:	c1 e8 18             	shr    $0x18,%eax
80107e5b:	89 c2                	mov    %eax,%edx
80107e5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e60:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80107e67:	00 00 
80107e69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e6c:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80107e73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e76:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80107e7c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e7f:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107e86:	83 e1 f0             	and    $0xfffffff0,%ecx
80107e89:	83 c9 02             	or     $0x2,%ecx
80107e8c:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107e92:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e95:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107e9c:	83 c9 10             	or     $0x10,%ecx
80107e9f:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107ea5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ea8:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107eaf:	83 e1 9f             	and    $0xffffff9f,%ecx
80107eb2:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107eb8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ebb:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107ec2:	83 c9 80             	or     $0xffffff80,%ecx
80107ec5:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107ecb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ece:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107ed5:	83 e1 f0             	and    $0xfffffff0,%ecx
80107ed8:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107ede:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ee1:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107ee8:	83 e1 ef             	and    $0xffffffef,%ecx
80107eeb:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107ef1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ef4:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107efb:	83 e1 df             	and    $0xffffffdf,%ecx
80107efe:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f04:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f07:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f0e:	83 c9 40             	or     $0x40,%ecx
80107f11:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f17:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f1a:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f21:	83 c9 80             	or     $0xffffff80,%ecx
80107f24:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f2d:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80107f33:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f36:	83 c0 70             	add    $0x70,%eax
80107f39:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80107f40:	00 
80107f41:	89 04 24             	mov    %eax,(%esp)
80107f44:	e8 37 fb ff ff       	call   80107a80 <lgdt>
  loadgs(SEG_KCPU << 3);
80107f49:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80107f50:	e8 6a fb ff ff       	call   80107abf <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80107f55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f58:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80107f5e:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80107f65:	00 00 00 00 
}
80107f69:	83 c4 24             	add    $0x24,%esp
80107f6c:	5b                   	pop    %ebx
80107f6d:	5d                   	pop    %ebp
80107f6e:	c3                   	ret    

80107f6f <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80107f6f:	55                   	push   %ebp
80107f70:	89 e5                	mov    %esp,%ebp
80107f72:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107f75:	8b 45 0c             	mov    0xc(%ebp),%eax
80107f78:	c1 e8 16             	shr    $0x16,%eax
80107f7b:	c1 e0 02             	shl    $0x2,%eax
80107f7e:	03 45 08             	add    0x8(%ebp),%eax
80107f81:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107f84:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f87:	8b 00                	mov    (%eax),%eax
80107f89:	83 e0 01             	and    $0x1,%eax
80107f8c:	84 c0                	test   %al,%al
80107f8e:	74 17                	je     80107fa7 <walkpgdir+0x38>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80107f90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f93:	8b 00                	mov    (%eax),%eax
80107f95:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107f9a:	89 04 24             	mov    %eax,(%esp)
80107f9d:	e8 4a fb ff ff       	call   80107aec <p2v>
80107fa2:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107fa5:	eb 4b                	jmp    80107ff2 <walkpgdir+0x83>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107fa7:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80107fab:	74 0e                	je     80107fbb <walkpgdir+0x4c>
80107fad:	e8 45 ab ff ff       	call   80102af7 <kalloc>
80107fb2:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107fb5:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107fb9:	75 07                	jne    80107fc2 <walkpgdir+0x53>
      return 0;
80107fbb:	b8 00 00 00 00       	mov    $0x0,%eax
80107fc0:	eb 41                	jmp    80108003 <walkpgdir+0x94>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107fc2:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107fc9:	00 
80107fca:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107fd1:	00 
80107fd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fd5:	89 04 24             	mov    %eax,(%esp)
80107fd8:	e8 fd d3 ff ff       	call   801053da <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80107fdd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fe0:	89 04 24             	mov    %eax,(%esp)
80107fe3:	e8 f7 fa ff ff       	call   80107adf <v2p>
80107fe8:	89 c2                	mov    %eax,%edx
80107fea:	83 ca 07             	or     $0x7,%edx
80107fed:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107ff0:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80107ff2:	8b 45 0c             	mov    0xc(%ebp),%eax
80107ff5:	c1 e8 0c             	shr    $0xc,%eax
80107ff8:	25 ff 03 00 00       	and    $0x3ff,%eax
80107ffd:	c1 e0 02             	shl    $0x2,%eax
80108000:	03 45 f4             	add    -0xc(%ebp),%eax
}
80108003:	c9                   	leave  
80108004:	c3                   	ret    

80108005 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80108005:	55                   	push   %ebp
80108006:	89 e5                	mov    %esp,%ebp
80108008:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
8010800b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010800e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108013:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80108016:	8b 45 0c             	mov    0xc(%ebp),%eax
80108019:	03 45 10             	add    0x10(%ebp),%eax
8010801c:	83 e8 01             	sub    $0x1,%eax
8010801f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108024:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80108027:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010802e:	00 
8010802f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108032:	89 44 24 04          	mov    %eax,0x4(%esp)
80108036:	8b 45 08             	mov    0x8(%ebp),%eax
80108039:	89 04 24             	mov    %eax,(%esp)
8010803c:	e8 2e ff ff ff       	call   80107f6f <walkpgdir>
80108041:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108044:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108048:	75 07                	jne    80108051 <mappages+0x4c>
      return -1;
8010804a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010804f:	eb 46                	jmp    80108097 <mappages+0x92>
    if(*pte & PTE_P)
80108051:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108054:	8b 00                	mov    (%eax),%eax
80108056:	83 e0 01             	and    $0x1,%eax
80108059:	84 c0                	test   %al,%al
8010805b:	74 0c                	je     80108069 <mappages+0x64>
      panic("remap");
8010805d:	c7 04 24 64 8f 10 80 	movl   $0x80108f64,(%esp)
80108064:	e8 d4 84 ff ff       	call   8010053d <panic>
    *pte = pa | perm | PTE_P;
80108069:	8b 45 18             	mov    0x18(%ebp),%eax
8010806c:	0b 45 14             	or     0x14(%ebp),%eax
8010806f:	89 c2                	mov    %eax,%edx
80108071:	83 ca 01             	or     $0x1,%edx
80108074:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108077:	89 10                	mov    %edx,(%eax)
    if(a == last)
80108079:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010807c:	3b 45 f0             	cmp    -0x10(%ebp),%eax
8010807f:	74 10                	je     80108091 <mappages+0x8c>
      break;
    a += PGSIZE;
80108081:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80108088:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
8010808f:	eb 96                	jmp    80108027 <mappages+0x22>
      return -1;
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
80108091:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80108092:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108097:	c9                   	leave  
80108098:	c3                   	ret    

80108099 <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm()
{
80108099:	55                   	push   %ebp
8010809a:	89 e5                	mov    %esp,%ebp
8010809c:	53                   	push   %ebx
8010809d:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
801080a0:	e8 52 aa ff ff       	call   80102af7 <kalloc>
801080a5:	89 45 f0             	mov    %eax,-0x10(%ebp)
801080a8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801080ac:	75 0a                	jne    801080b8 <setupkvm+0x1f>
    return 0;
801080ae:	b8 00 00 00 00       	mov    $0x0,%eax
801080b3:	e9 98 00 00 00       	jmp    80108150 <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
801080b8:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801080bf:	00 
801080c0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801080c7:	00 
801080c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801080cb:	89 04 24             	mov    %eax,(%esp)
801080ce:	e8 07 d3 ff ff       	call   801053da <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
801080d3:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
801080da:	e8 0d fa ff ff       	call   80107aec <p2v>
801080df:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
801080e4:	76 0c                	jbe    801080f2 <setupkvm+0x59>
    panic("PHYSTOP too high");
801080e6:	c7 04 24 6a 8f 10 80 	movl   $0x80108f6a,(%esp)
801080ed:	e8 4b 84 ff ff       	call   8010053d <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
801080f2:	c7 45 f4 a0 c4 10 80 	movl   $0x8010c4a0,-0xc(%ebp)
801080f9:	eb 49                	jmp    80108144 <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
801080fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
801080fe:	8b 48 0c             	mov    0xc(%eax),%ecx
                (uint)k->phys_start, k->perm) < 0)
80108101:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80108104:	8b 50 04             	mov    0x4(%eax),%edx
80108107:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010810a:	8b 58 08             	mov    0x8(%eax),%ebx
8010810d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108110:	8b 40 04             	mov    0x4(%eax),%eax
80108113:	29 c3                	sub    %eax,%ebx
80108115:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108118:	8b 00                	mov    (%eax),%eax
8010811a:	89 4c 24 10          	mov    %ecx,0x10(%esp)
8010811e:	89 54 24 0c          	mov    %edx,0xc(%esp)
80108122:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80108126:	89 44 24 04          	mov    %eax,0x4(%esp)
8010812a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010812d:	89 04 24             	mov    %eax,(%esp)
80108130:	e8 d0 fe ff ff       	call   80108005 <mappages>
80108135:	85 c0                	test   %eax,%eax
80108137:	79 07                	jns    80108140 <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80108139:	b8 00 00 00 00       	mov    $0x0,%eax
8010813e:	eb 10                	jmp    80108150 <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80108140:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80108144:	81 7d f4 e0 c4 10 80 	cmpl   $0x8010c4e0,-0xc(%ebp)
8010814b:	72 ae                	jb     801080fb <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
8010814d:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80108150:	83 c4 34             	add    $0x34,%esp
80108153:	5b                   	pop    %ebx
80108154:	5d                   	pop    %ebp
80108155:	c3                   	ret    

80108156 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80108156:	55                   	push   %ebp
80108157:	89 e5                	mov    %esp,%ebp
80108159:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
8010815c:	e8 38 ff ff ff       	call   80108099 <setupkvm>
80108161:	a3 f8 3a 11 80       	mov    %eax,0x80113af8
  switchkvm();
80108166:	e8 02 00 00 00       	call   8010816d <switchkvm>
}
8010816b:	c9                   	leave  
8010816c:	c3                   	ret    

8010816d <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
8010816d:	55                   	push   %ebp
8010816e:	89 e5                	mov    %esp,%ebp
80108170:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
80108173:	a1 f8 3a 11 80       	mov    0x80113af8,%eax
80108178:	89 04 24             	mov    %eax,(%esp)
8010817b:	e8 5f f9 ff ff       	call   80107adf <v2p>
80108180:	89 04 24             	mov    %eax,(%esp)
80108183:	e8 4c f9 ff ff       	call   80107ad4 <lcr3>
}
80108188:	c9                   	leave  
80108189:	c3                   	ret    

8010818a <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
8010818a:	55                   	push   %ebp
8010818b:	89 e5                	mov    %esp,%ebp
8010818d:	53                   	push   %ebx
8010818e:	83 ec 14             	sub    $0x14,%esp
  pushcli();
80108191:	e8 3e d1 ff ff       	call   801052d4 <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
80108196:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010819c:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081a3:	83 c2 08             	add    $0x8,%edx
801081a6:	89 d3                	mov    %edx,%ebx
801081a8:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081af:	83 c2 08             	add    $0x8,%edx
801081b2:	c1 ea 10             	shr    $0x10,%edx
801081b5:	89 d1                	mov    %edx,%ecx
801081b7:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081be:	83 c2 08             	add    $0x8,%edx
801081c1:	c1 ea 18             	shr    $0x18,%edx
801081c4:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
801081cb:	67 00 
801081cd:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
801081d4:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
801081da:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
801081e1:	83 e1 f0             	and    $0xfffffff0,%ecx
801081e4:	83 c9 09             	or     $0x9,%ecx
801081e7:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
801081ed:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
801081f4:	83 c9 10             	or     $0x10,%ecx
801081f7:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
801081fd:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108204:	83 e1 9f             	and    $0xffffff9f,%ecx
80108207:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010820d:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108214:	83 c9 80             	or     $0xffffff80,%ecx
80108217:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010821d:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108224:	83 e1 f0             	and    $0xfffffff0,%ecx
80108227:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010822d:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108234:	83 e1 ef             	and    $0xffffffef,%ecx
80108237:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010823d:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108244:	83 e1 df             	and    $0xffffffdf,%ecx
80108247:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010824d:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108254:	83 c9 40             	or     $0x40,%ecx
80108257:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010825d:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108264:	83 e1 7f             	and    $0x7f,%ecx
80108267:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010826d:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
80108273:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108279:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
80108280:	83 e2 ef             	and    $0xffffffef,%edx
80108283:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
80108289:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010828f:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
80108295:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010829b:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801082a2:	8b 52 08             	mov    0x8(%edx),%edx
801082a5:	81 c2 00 10 00 00    	add    $0x1000,%edx
801082ab:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
801082ae:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
801082b5:	e8 ef f7 ff ff       	call   80107aa9 <ltr>
  if(p->pgdir == 0)
801082ba:	8b 45 08             	mov    0x8(%ebp),%eax
801082bd:	8b 40 04             	mov    0x4(%eax),%eax
801082c0:	85 c0                	test   %eax,%eax
801082c2:	75 0c                	jne    801082d0 <switchuvm+0x146>
    panic("switchuvm: no pgdir");
801082c4:	c7 04 24 7b 8f 10 80 	movl   $0x80108f7b,(%esp)
801082cb:	e8 6d 82 ff ff       	call   8010053d <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
801082d0:	8b 45 08             	mov    0x8(%ebp),%eax
801082d3:	8b 40 04             	mov    0x4(%eax),%eax
801082d6:	89 04 24             	mov    %eax,(%esp)
801082d9:	e8 01 f8 ff ff       	call   80107adf <v2p>
801082de:	89 04 24             	mov    %eax,(%esp)
801082e1:	e8 ee f7 ff ff       	call   80107ad4 <lcr3>
  popcli();
801082e6:	e8 31 d0 ff ff       	call   8010531c <popcli>
}
801082eb:	83 c4 14             	add    $0x14,%esp
801082ee:	5b                   	pop    %ebx
801082ef:	5d                   	pop    %ebp
801082f0:	c3                   	ret    

801082f1 <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
801082f1:	55                   	push   %ebp
801082f2:	89 e5                	mov    %esp,%ebp
801082f4:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
801082f7:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
801082fe:	76 0c                	jbe    8010830c <inituvm+0x1b>
    panic("inituvm: more than a page");
80108300:	c7 04 24 8f 8f 10 80 	movl   $0x80108f8f,(%esp)
80108307:	e8 31 82 ff ff       	call   8010053d <panic>
  mem = kalloc();
8010830c:	e8 e6 a7 ff ff       	call   80102af7 <kalloc>
80108311:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
80108314:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010831b:	00 
8010831c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108323:	00 
80108324:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108327:	89 04 24             	mov    %eax,(%esp)
8010832a:	e8 ab d0 ff ff       	call   801053da <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
8010832f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108332:	89 04 24             	mov    %eax,(%esp)
80108335:	e8 a5 f7 ff ff       	call   80107adf <v2p>
8010833a:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80108341:	00 
80108342:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108346:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010834d:	00 
8010834e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108355:	00 
80108356:	8b 45 08             	mov    0x8(%ebp),%eax
80108359:	89 04 24             	mov    %eax,(%esp)
8010835c:	e8 a4 fc ff ff       	call   80108005 <mappages>
  memmove(mem, init, sz);
80108361:	8b 45 10             	mov    0x10(%ebp),%eax
80108364:	89 44 24 08          	mov    %eax,0x8(%esp)
80108368:	8b 45 0c             	mov    0xc(%ebp),%eax
8010836b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010836f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108372:	89 04 24             	mov    %eax,(%esp)
80108375:	e8 33 d1 ff ff       	call   801054ad <memmove>
}
8010837a:	c9                   	leave  
8010837b:	c3                   	ret    

8010837c <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
8010837c:	55                   	push   %ebp
8010837d:	89 e5                	mov    %esp,%ebp
8010837f:	53                   	push   %ebx
80108380:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
80108383:	8b 45 0c             	mov    0xc(%ebp),%eax
80108386:	25 ff 0f 00 00       	and    $0xfff,%eax
8010838b:	85 c0                	test   %eax,%eax
8010838d:	74 0c                	je     8010839b <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
8010838f:	c7 04 24 ac 8f 10 80 	movl   $0x80108fac,(%esp)
80108396:	e8 a2 81 ff ff       	call   8010053d <panic>
  for(i = 0; i < sz; i += PGSIZE){
8010839b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801083a2:	e9 ad 00 00 00       	jmp    80108454 <loaduvm+0xd8>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
801083a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083aa:	8b 55 0c             	mov    0xc(%ebp),%edx
801083ad:	01 d0                	add    %edx,%eax
801083af:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801083b6:	00 
801083b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801083bb:	8b 45 08             	mov    0x8(%ebp),%eax
801083be:	89 04 24             	mov    %eax,(%esp)
801083c1:	e8 a9 fb ff ff       	call   80107f6f <walkpgdir>
801083c6:	89 45 ec             	mov    %eax,-0x14(%ebp)
801083c9:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801083cd:	75 0c                	jne    801083db <loaduvm+0x5f>
      panic("loaduvm: address should exist");
801083cf:	c7 04 24 cf 8f 10 80 	movl   $0x80108fcf,(%esp)
801083d6:	e8 62 81 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
801083db:	8b 45 ec             	mov    -0x14(%ebp),%eax
801083de:	8b 00                	mov    (%eax),%eax
801083e0:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801083e5:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
801083e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083eb:	8b 55 18             	mov    0x18(%ebp),%edx
801083ee:	89 d1                	mov    %edx,%ecx
801083f0:	29 c1                	sub    %eax,%ecx
801083f2:	89 c8                	mov    %ecx,%eax
801083f4:	3d ff 0f 00 00       	cmp    $0xfff,%eax
801083f9:	77 11                	ja     8010840c <loaduvm+0x90>
      n = sz - i;
801083fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083fe:	8b 55 18             	mov    0x18(%ebp),%edx
80108401:	89 d1                	mov    %edx,%ecx
80108403:	29 c1                	sub    %eax,%ecx
80108405:	89 c8                	mov    %ecx,%eax
80108407:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010840a:	eb 07                	jmp    80108413 <loaduvm+0x97>
    else
      n = PGSIZE;
8010840c:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
80108413:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108416:	8b 55 14             	mov    0x14(%ebp),%edx
80108419:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
8010841c:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010841f:	89 04 24             	mov    %eax,(%esp)
80108422:	e8 c5 f6 ff ff       	call   80107aec <p2v>
80108427:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010842a:	89 54 24 0c          	mov    %edx,0xc(%esp)
8010842e:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80108432:	89 44 24 04          	mov    %eax,0x4(%esp)
80108436:	8b 45 10             	mov    0x10(%ebp),%eax
80108439:	89 04 24             	mov    %eax,(%esp)
8010843c:	e8 1d 99 ff ff       	call   80101d5e <readi>
80108441:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80108444:	74 07                	je     8010844d <loaduvm+0xd1>
      return -1;
80108446:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010844b:	eb 18                	jmp    80108465 <loaduvm+0xe9>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
8010844d:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108454:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108457:	3b 45 18             	cmp    0x18(%ebp),%eax
8010845a:	0f 82 47 ff ff ff    	jb     801083a7 <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
80108460:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108465:	83 c4 24             	add    $0x24,%esp
80108468:	5b                   	pop    %ebx
80108469:	5d                   	pop    %ebp
8010846a:	c3                   	ret    

8010846b <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
8010846b:	55                   	push   %ebp
8010846c:	89 e5                	mov    %esp,%ebp
8010846e:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
80108471:	8b 45 10             	mov    0x10(%ebp),%eax
80108474:	85 c0                	test   %eax,%eax
80108476:	79 0a                	jns    80108482 <allocuvm+0x17>
    return 0;
80108478:	b8 00 00 00 00       	mov    $0x0,%eax
8010847d:	e9 c1 00 00 00       	jmp    80108543 <allocuvm+0xd8>
  if(newsz < oldsz)
80108482:	8b 45 10             	mov    0x10(%ebp),%eax
80108485:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108488:	73 08                	jae    80108492 <allocuvm+0x27>
    return oldsz;
8010848a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010848d:	e9 b1 00 00 00       	jmp    80108543 <allocuvm+0xd8>
  a = PGROUNDUP(oldsz);
80108492:	8b 45 0c             	mov    0xc(%ebp),%eax
80108495:	05 ff 0f 00 00       	add    $0xfff,%eax
8010849a:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010849f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
801084a2:	e9 8d 00 00 00       	jmp    80108534 <allocuvm+0xc9>
    mem = kalloc();
801084a7:	e8 4b a6 ff ff       	call   80102af7 <kalloc>
801084ac:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
801084af:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801084b3:	75 2c                	jne    801084e1 <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
801084b5:	c7 04 24 ed 8f 10 80 	movl   $0x80108fed,(%esp)
801084bc:	e8 e0 7e ff ff       	call   801003a1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
801084c1:	8b 45 0c             	mov    0xc(%ebp),%eax
801084c4:	89 44 24 08          	mov    %eax,0x8(%esp)
801084c8:	8b 45 10             	mov    0x10(%ebp),%eax
801084cb:	89 44 24 04          	mov    %eax,0x4(%esp)
801084cf:	8b 45 08             	mov    0x8(%ebp),%eax
801084d2:	89 04 24             	mov    %eax,(%esp)
801084d5:	e8 6b 00 00 00       	call   80108545 <deallocuvm>
      return 0;
801084da:	b8 00 00 00 00       	mov    $0x0,%eax
801084df:	eb 62                	jmp    80108543 <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
801084e1:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801084e8:	00 
801084e9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801084f0:	00 
801084f1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801084f4:	89 04 24             	mov    %eax,(%esp)
801084f7:	e8 de ce ff ff       	call   801053da <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
801084fc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801084ff:	89 04 24             	mov    %eax,(%esp)
80108502:	e8 d8 f5 ff ff       	call   80107adf <v2p>
80108507:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010850a:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80108511:	00 
80108512:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108516:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010851d:	00 
8010851e:	89 54 24 04          	mov    %edx,0x4(%esp)
80108522:	8b 45 08             	mov    0x8(%ebp),%eax
80108525:	89 04 24             	mov    %eax,(%esp)
80108528:	e8 d8 fa ff ff       	call   80108005 <mappages>
  if(newsz >= KERNBASE)
    return 0;
  if(newsz < oldsz)
    return oldsz;
  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
8010852d:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108534:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108537:	3b 45 10             	cmp    0x10(%ebp),%eax
8010853a:	0f 82 67 ff ff ff    	jb     801084a7 <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
80108540:	8b 45 10             	mov    0x10(%ebp),%eax
}
80108543:	c9                   	leave  
80108544:	c3                   	ret    

80108545 <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80108545:	55                   	push   %ebp
80108546:	89 e5                	mov    %esp,%ebp
80108548:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
8010854b:	8b 45 10             	mov    0x10(%ebp),%eax
8010854e:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108551:	72 08                	jb     8010855b <deallocuvm+0x16>
    return oldsz;
80108553:	8b 45 0c             	mov    0xc(%ebp),%eax
80108556:	e9 a4 00 00 00       	jmp    801085ff <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
8010855b:	8b 45 10             	mov    0x10(%ebp),%eax
8010855e:	05 ff 0f 00 00       	add    $0xfff,%eax
80108563:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108568:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
8010856b:	e9 80 00 00 00       	jmp    801085f0 <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
80108570:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108573:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010857a:	00 
8010857b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010857f:	8b 45 08             	mov    0x8(%ebp),%eax
80108582:	89 04 24             	mov    %eax,(%esp)
80108585:	e8 e5 f9 ff ff       	call   80107f6f <walkpgdir>
8010858a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
8010858d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108591:	75 09                	jne    8010859c <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
80108593:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
8010859a:	eb 4d                	jmp    801085e9 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
8010859c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010859f:	8b 00                	mov    (%eax),%eax
801085a1:	83 e0 01             	and    $0x1,%eax
801085a4:	84 c0                	test   %al,%al
801085a6:	74 41                	je     801085e9 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
801085a8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085ab:	8b 00                	mov    (%eax),%eax
801085ad:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801085b2:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
801085b5:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801085b9:	75 0c                	jne    801085c7 <deallocuvm+0x82>
        panic("kfree");
801085bb:	c7 04 24 05 90 10 80 	movl   $0x80109005,(%esp)
801085c2:	e8 76 7f ff ff       	call   8010053d <panic>
      char *v = p2v(pa);
801085c7:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085ca:	89 04 24             	mov    %eax,(%esp)
801085cd:	e8 1a f5 ff ff       	call   80107aec <p2v>
801085d2:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
801085d5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801085d8:	89 04 24             	mov    %eax,(%esp)
801085db:	e8 7e a4 ff ff       	call   80102a5e <kfree>
      *pte = 0;
801085e0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085e3:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
801085e9:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801085f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801085f3:	3b 45 0c             	cmp    0xc(%ebp),%eax
801085f6:	0f 82 74 ff ff ff    	jb     80108570 <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
801085fc:	8b 45 10             	mov    0x10(%ebp),%eax
}
801085ff:	c9                   	leave  
80108600:	c3                   	ret    

80108601 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80108601:	55                   	push   %ebp
80108602:	89 e5                	mov    %esp,%ebp
80108604:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80108607:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010860b:	75 0c                	jne    80108619 <freevm+0x18>
    panic("freevm: no pgdir");
8010860d:	c7 04 24 0b 90 10 80 	movl   $0x8010900b,(%esp)
80108614:	e8 24 7f ff ff       	call   8010053d <panic>
  deallocuvm(pgdir, KERNBASE, 0);
80108619:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108620:	00 
80108621:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
80108628:	80 
80108629:	8b 45 08             	mov    0x8(%ebp),%eax
8010862c:	89 04 24             	mov    %eax,(%esp)
8010862f:	e8 11 ff ff ff       	call   80108545 <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
80108634:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010863b:	eb 3c                	jmp    80108679 <freevm+0x78>
    if(pgdir[i] & PTE_P){
8010863d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108640:	c1 e0 02             	shl    $0x2,%eax
80108643:	03 45 08             	add    0x8(%ebp),%eax
80108646:	8b 00                	mov    (%eax),%eax
80108648:	83 e0 01             	and    $0x1,%eax
8010864b:	84 c0                	test   %al,%al
8010864d:	74 26                	je     80108675 <freevm+0x74>
      char * v = p2v(PTE_ADDR(pgdir[i]));
8010864f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108652:	c1 e0 02             	shl    $0x2,%eax
80108655:	03 45 08             	add    0x8(%ebp),%eax
80108658:	8b 00                	mov    (%eax),%eax
8010865a:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010865f:	89 04 24             	mov    %eax,(%esp)
80108662:	e8 85 f4 ff ff       	call   80107aec <p2v>
80108667:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
8010866a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010866d:	89 04 24             	mov    %eax,(%esp)
80108670:	e8 e9 a3 ff ff       	call   80102a5e <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80108675:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80108679:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
80108680:	76 bb                	jbe    8010863d <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
80108682:	8b 45 08             	mov    0x8(%ebp),%eax
80108685:	89 04 24             	mov    %eax,(%esp)
80108688:	e8 d1 a3 ff ff       	call   80102a5e <kfree>
}
8010868d:	c9                   	leave  
8010868e:	c3                   	ret    

8010868f <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
8010868f:	55                   	push   %ebp
80108690:	89 e5                	mov    %esp,%ebp
80108692:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80108695:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010869c:	00 
8010869d:	8b 45 0c             	mov    0xc(%ebp),%eax
801086a0:	89 44 24 04          	mov    %eax,0x4(%esp)
801086a4:	8b 45 08             	mov    0x8(%ebp),%eax
801086a7:	89 04 24             	mov    %eax,(%esp)
801086aa:	e8 c0 f8 ff ff       	call   80107f6f <walkpgdir>
801086af:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
801086b2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801086b6:	75 0c                	jne    801086c4 <clearpteu+0x35>
    panic("clearpteu");
801086b8:	c7 04 24 1c 90 10 80 	movl   $0x8010901c,(%esp)
801086bf:	e8 79 7e ff ff       	call   8010053d <panic>
  *pte &= ~PTE_U;
801086c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086c7:	8b 00                	mov    (%eax),%eax
801086c9:	89 c2                	mov    %eax,%edx
801086cb:	83 e2 fb             	and    $0xfffffffb,%edx
801086ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086d1:	89 10                	mov    %edx,(%eax)
}
801086d3:	c9                   	leave  
801086d4:	c3                   	ret    

801086d5 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
801086d5:	55                   	push   %ebp
801086d6:	89 e5                	mov    %esp,%ebp
801086d8:	83 ec 48             	sub    $0x48,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
801086db:	e8 b9 f9 ff ff       	call   80108099 <setupkvm>
801086e0:	89 45 f0             	mov    %eax,-0x10(%ebp)
801086e3:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801086e7:	75 0a                	jne    801086f3 <copyuvm+0x1e>
    return 0;
801086e9:	b8 00 00 00 00       	mov    $0x0,%eax
801086ee:	e9 f1 00 00 00       	jmp    801087e4 <copyuvm+0x10f>
  for(i = 0; i < sz; i += PGSIZE){
801086f3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801086fa:	e9 c0 00 00 00       	jmp    801087bf <copyuvm+0xea>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
801086ff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108702:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108709:	00 
8010870a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010870e:	8b 45 08             	mov    0x8(%ebp),%eax
80108711:	89 04 24             	mov    %eax,(%esp)
80108714:	e8 56 f8 ff ff       	call   80107f6f <walkpgdir>
80108719:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010871c:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108720:	75 0c                	jne    8010872e <copyuvm+0x59>
      panic("copyuvm: pte should exist");
80108722:	c7 04 24 26 90 10 80 	movl   $0x80109026,(%esp)
80108729:	e8 0f 7e ff ff       	call   8010053d <panic>
    if(!(*pte & PTE_P))
8010872e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108731:	8b 00                	mov    (%eax),%eax
80108733:	83 e0 01             	and    $0x1,%eax
80108736:	85 c0                	test   %eax,%eax
80108738:	75 0c                	jne    80108746 <copyuvm+0x71>
      panic("copyuvm: page not present");
8010873a:	c7 04 24 40 90 10 80 	movl   $0x80109040,(%esp)
80108741:	e8 f7 7d ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
80108746:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108749:	8b 00                	mov    (%eax),%eax
8010874b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108750:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if((mem = kalloc()) == 0)
80108753:	e8 9f a3 ff ff       	call   80102af7 <kalloc>
80108758:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010875b:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010875f:	74 6f                	je     801087d0 <copyuvm+0xfb>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
80108761:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108764:	89 04 24             	mov    %eax,(%esp)
80108767:	e8 80 f3 ff ff       	call   80107aec <p2v>
8010876c:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108773:	00 
80108774:	89 44 24 04          	mov    %eax,0x4(%esp)
80108778:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010877b:	89 04 24             	mov    %eax,(%esp)
8010877e:	e8 2a cd ff ff       	call   801054ad <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
80108783:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80108786:	89 04 24             	mov    %eax,(%esp)
80108789:	e8 51 f3 ff ff       	call   80107adf <v2p>
8010878e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80108791:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80108798:	00 
80108799:	89 44 24 0c          	mov    %eax,0xc(%esp)
8010879d:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801087a4:	00 
801087a5:	89 54 24 04          	mov    %edx,0x4(%esp)
801087a9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801087ac:	89 04 24             	mov    %eax,(%esp)
801087af:	e8 51 f8 ff ff       	call   80108005 <mappages>
801087b4:	85 c0                	test   %eax,%eax
801087b6:	78 1b                	js     801087d3 <copyuvm+0xfe>
  uint pa, i;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
801087b8:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801087bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801087c2:	3b 45 0c             	cmp    0xc(%ebp),%eax
801087c5:	0f 82 34 ff ff ff    	jb     801086ff <copyuvm+0x2a>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
  }
  return d;
801087cb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801087ce:	eb 14                	jmp    801087e4 <copyuvm+0x10f>
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
801087d0:	90                   	nop
801087d1:	eb 01                	jmp    801087d4 <copyuvm+0xff>
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), PTE_W|PTE_U) < 0)
      goto bad;
801087d3:	90                   	nop
  }
  return d;

bad:
  freevm(d);
801087d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801087d7:	89 04 24             	mov    %eax,(%esp)
801087da:	e8 22 fe ff ff       	call   80108601 <freevm>
  return 0;
801087df:	b8 00 00 00 00       	mov    $0x0,%eax
}
801087e4:	c9                   	leave  
801087e5:	c3                   	ret    

801087e6 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
801087e6:	55                   	push   %ebp
801087e7:	89 e5                	mov    %esp,%ebp
801087e9:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801087ec:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801087f3:	00 
801087f4:	8b 45 0c             	mov    0xc(%ebp),%eax
801087f7:	89 44 24 04          	mov    %eax,0x4(%esp)
801087fb:	8b 45 08             	mov    0x8(%ebp),%eax
801087fe:	89 04 24             	mov    %eax,(%esp)
80108801:	e8 69 f7 ff ff       	call   80107f6f <walkpgdir>
80108806:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
80108809:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010880c:	8b 00                	mov    (%eax),%eax
8010880e:	83 e0 01             	and    $0x1,%eax
80108811:	85 c0                	test   %eax,%eax
80108813:	75 07                	jne    8010881c <uva2ka+0x36>
    return 0;
80108815:	b8 00 00 00 00       	mov    $0x0,%eax
8010881a:	eb 25                	jmp    80108841 <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
8010881c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010881f:	8b 00                	mov    (%eax),%eax
80108821:	83 e0 04             	and    $0x4,%eax
80108824:	85 c0                	test   %eax,%eax
80108826:	75 07                	jne    8010882f <uva2ka+0x49>
    return 0;
80108828:	b8 00 00 00 00       	mov    $0x0,%eax
8010882d:	eb 12                	jmp    80108841 <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
8010882f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108832:	8b 00                	mov    (%eax),%eax
80108834:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108839:	89 04 24             	mov    %eax,(%esp)
8010883c:	e8 ab f2 ff ff       	call   80107aec <p2v>
}
80108841:	c9                   	leave  
80108842:	c3                   	ret    

80108843 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80108843:	55                   	push   %ebp
80108844:	89 e5                	mov    %esp,%ebp
80108846:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80108849:	8b 45 10             	mov    0x10(%ebp),%eax
8010884c:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
8010884f:	e9 8b 00 00 00       	jmp    801088df <copyout+0x9c>
    va0 = (uint)PGROUNDDOWN(va);
80108854:	8b 45 0c             	mov    0xc(%ebp),%eax
80108857:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010885c:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
8010885f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108862:	89 44 24 04          	mov    %eax,0x4(%esp)
80108866:	8b 45 08             	mov    0x8(%ebp),%eax
80108869:	89 04 24             	mov    %eax,(%esp)
8010886c:	e8 75 ff ff ff       	call   801087e6 <uva2ka>
80108871:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
80108874:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80108878:	75 07                	jne    80108881 <copyout+0x3e>
      return -1;
8010887a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010887f:	eb 6d                	jmp    801088ee <copyout+0xab>
    n = PGSIZE - (va - va0);
80108881:	8b 45 0c             	mov    0xc(%ebp),%eax
80108884:	8b 55 ec             	mov    -0x14(%ebp),%edx
80108887:	89 d1                	mov    %edx,%ecx
80108889:	29 c1                	sub    %eax,%ecx
8010888b:	89 c8                	mov    %ecx,%eax
8010888d:	05 00 10 00 00       	add    $0x1000,%eax
80108892:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
80108895:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108898:	3b 45 14             	cmp    0x14(%ebp),%eax
8010889b:	76 06                	jbe    801088a3 <copyout+0x60>
      n = len;
8010889d:	8b 45 14             	mov    0x14(%ebp),%eax
801088a0:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
801088a3:	8b 45 ec             	mov    -0x14(%ebp),%eax
801088a6:	8b 55 0c             	mov    0xc(%ebp),%edx
801088a9:	89 d1                	mov    %edx,%ecx
801088ab:	29 c1                	sub    %eax,%ecx
801088ad:	89 c8                	mov    %ecx,%eax
801088af:	03 45 e8             	add    -0x18(%ebp),%eax
801088b2:	8b 55 f0             	mov    -0x10(%ebp),%edx
801088b5:	89 54 24 08          	mov    %edx,0x8(%esp)
801088b9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801088bc:	89 54 24 04          	mov    %edx,0x4(%esp)
801088c0:	89 04 24             	mov    %eax,(%esp)
801088c3:	e8 e5 cb ff ff       	call   801054ad <memmove>
    len -= n;
801088c8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801088cb:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
801088ce:	8b 45 f0             	mov    -0x10(%ebp),%eax
801088d1:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
801088d4:	8b 45 ec             	mov    -0x14(%ebp),%eax
801088d7:	05 00 10 00 00       	add    $0x1000,%eax
801088dc:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
801088df:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
801088e3:	0f 85 6b ff ff ff    	jne    80108854 <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
801088e9:	b8 00 00 00 00       	mov    $0x0,%eax
}
801088ee:	c9                   	leave  
801088ef:	c3                   	ret    
